...
impl InklingState {
    pub(crate) fn target(&self) -> &MlxHybridState {
        &self.target
    }
...
impl InklingBindings {
    ...
    pub(crate) fn selected_static_units(
                        ...
                        let local = binding
                            ...
                            .ok_or_else(|| {
                                Error::Parallel(format!(
                                    "Inkling static binding {:?} does not start with {prefix:?}",
                                    binding.name()
                                ))
                            })?
                            .to_string();
        ...
        push_leaf!(
            "embedding_norm",
            modules.embedding_norm,
            "model.embed_norm",
            false
        );
        push_leaf!("norm", modules.final_norm, "model.norm", false);
        push_leaf!(
            "output",
            modules.output,
            "lm_head",
...
fn forward_external_experts<P>(
    ...
    forward: &mut eredu_architectures::inkling::ForwardContext<Array>,
    stream: &Stream,
    provider: &mut P,
...
impl InklingModel {
    ...
    pub(crate) fn prompt_cache_layer_prefix_offsets(&self) -> Vec<i32> {
...
pub(crate) fn load_pipeline_config(model_dir: &Path) -> Result<ModelArgs, Error> {
