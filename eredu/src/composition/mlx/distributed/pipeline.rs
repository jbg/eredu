...
impl Gemma4PipelinePartition {
    ...
    fn begin_ingress(
        ...
        let mut begin_group = |group_index| {
            ...
            {
                return Ok::<Option<Array>, Error>(None);
            }
    ...
    fn finish_ingress(
        ...
        if let Some(inputs) = &per_layer_inputs {
            ...
            per_layer_inputs = Some(inputs.try_index_device((.., .., .., range), stream)?);
        }
        Ok(PipelinePayload {
            hidden,
...
impl eredu_nn::CompressedAttentionCache<Array> for PipelineHybridLayerState<'_> {
    ...
    fn append(
        &mut self,
        state: eredu_nn::CompressedAttentionState<Array>,
        stream: &Stream,
    ) -> Result<eredu_nn::CompressedAttentionView<Array>, eredu_nn::Error> {
        match self.0 {
    ...
    where
        F: FnMut(eredu_nn::CompressedAttentionBlock<Array>) -> Result<u64, eredu_nn::Error>,
    {
        match self.0 {
            PipelineLayerCache::CompressedLatent { cache, .. } => {
...
impl eredu_nn::PoolingAttentionCache<Array> for PipelineHybridLayerState<'_> {
    ...
    fn append_local(&mut self, keys: Array, context: &Stream) -> Result<Array, eredu_nn::Error> {
        match self.0 {
            PipelineLayerCache::PoolingAttention { cache, .. } => {
    ...
    fn accumulate_pooling_windows(
        ...
        absolute_offset: i32,
        context: &Stream,
    ) -> Result<eredu_nn::PoolingWindows<Array>, eredu_nn::Error> {
        match self.0 {
            PipelineLayerCache::PoolingAttention { cache, .. } => {
    ...
    fn replace_pooling_overlap(
        ...
        values: Array,
        gates: Array,
    ) -> Result<eredu_nn::PoolingOverlap<Array>, eredu_nn::Error> {
        match self.0 {
            PipelineLayerCache::PoolingAttention { cache, .. } => {
...
type QwenVlPipelinePartition = MediaPipelineRealization<
    ...
    QwenVlPipelineBindings,
    MlxModule<eredu_architectures::qwen::vl::Unit<MlxBackend>>,
    eredu_architectures::qwen::vl::PipelineVisionState<Array>,
>;
...
type QwenConditionalPipelinePartition = MediaPipelineRealization<
    ...
    QwenConditionalPipelineBindings,
    MlxModule<eredu_architectures::qwen::hybrid::ConditionalUnit<MlxBackend>>,
    eredu_architectures::qwen::hybrid::ConditionalPipelineVisionState<Array>,
>;
...
struct InklingIngressState {
    forward: eredu_runtime::LayeredForwardState<
        Array,
        eredu_architectures::inkling::ForwardContext<Array>,
    >,
    state: MlxHybridState,
}
...
impl InklingIngressState {
    ...
    fn replace_hidden(&mut self, hidden: Array) {
...
fn execute_neutral_partition_group<A, U, F>(
    ...
    forward: &mut eredu_runtime::LayeredForwardState<Array, F>,
        ...
        eredu_nn::Parameterized<Array>,
{
    ...
    let (partition_input, auxiliary) = match input {
        PipelineStageInput::Tokens(tokens) if owns_input => (
            eredu_architectures::decoder::PartitionInput::Tokens(tokens),
        ...
        PipelineStageInput::Hidden(payload) if !owns_input => (
            eredu_architectures::decoder::PartitionInput::Hidden(payload.hidden.clone()),
...
impl DeepSeekV3PipelinePartition {
    ...
    fn forward_stage(
        ...
        let target_input = match input {
            PipelineStageInput::Tokens(tokens) if owns_input => {
                eredu_architectures::deepseek::v3::TargetPartitionInput::Tokens(tokens)
            }
            PipelineStageInput::Hidden(payload) if !owns_input => {
                let boundary = eredu_architectures::deepseek::v3::TargetBoundary::decode(
                    payload.auxiliary.tensors().to_vec(),
                )
                .map_err(|error| Error::Parallel(error.to_string()))?;
        ...
        let auxiliary = PipelineAuxiliaryState::new(boundary.encode());
        ...
        if self.partition.ownership().owns_output() {
            ...
            let logits = match tensor_group {
                Some(group) => self
                    .architecture
                    .finish_partition_target_parallel(&hidden, group, stream)
                    .map_err(|error| Error::Parallel(error.to_string()))?,
                None => self
                    .architecture
                    .finish_partition_target(&hidden, stream)
                    .map_err(|error| Error::Parallel(error.to_string()))?,
            };
    ...
    fn forward_stage(
        ...
        let target_input = match input {
            PipelineStageInput::Tokens(tokens) => {
                eredu_architectures::deepseek::v4::TargetPartitionInput::Tokens(tokens)
            }
            PipelineStageInput::Hidden(payload) => {
                let boundary = boundary_schema
                    .decode(payload.auxiliary.tensors().to_vec())
                    .map_err(|error| Error::Parallel(error.to_string()))?;
        ...
        let mut forward = self
            .architecture
            .begin_routed_target_partition(target_input, mask, tensor_group, stream)
            .map_err(|error| Error::Parallel(error.to_string()))?;
        let state_layout = self
            .partition
            .state()
...
impl PipelineEmbeddedMtp for DeepSeekV4PipelinePartition {
    ...
    fn forward_embedded_mtp_draft(
        ...
        let hidden = self
            .architecture
            .begin_partition_prediction_hidden(hidden, stream)
            .map_err(|error| Error::Parallel(error.to_string()))?;
        let output = if let Some(expert_cache) = self.expert_storage.cache() {
                let assignment = self.expert_assignment.as_ref().ok_or_else(|| {
    ...
    fn prefill_embedded_mtp_cache(
        ...
        let Some((hidden, next)) = self
            .architecture
            .prepare_partition_prediction_replay(&output.hidden, tokens, stream)
            .map_err(|error| Error::Parallel(error.to_string()))?
        else {
            return Ok(true);
        };
...
impl PipelinePlacedIngress for QwenConditionalPipelinePartition {
    ...
    fn replace_placed_ingress_arrays(
        ...
        eredu_architectures::qwen::hybrid::ConditionalLayeredModel::<MlxBackend>::replace_pipeline_retained_values(state, arrays)
                .map_err(|error| Error::Parallel(error.to_string()))
    }
...
fn pipeline_static_role_binding_selection(
    authority: &[eredu_runtime::OwnedParameterGroupSpec],
    role: &str,
    bindings: &[WeightBinding],
) -> (BTreeSet<String>, Vec<WeightBinding>) {
    ...
    let selected = bindings
        .iter()
        .filter(|binding| parameter_name_in_targets(binding_target(binding), &targets))
        .cloned()
        .collect();
    (targets, selected)
}
...
fn decoder_partition_state_layout(
    ...
    let schedule = crate::LayerSchedule::new(policies.len(), policies)
        .map_err(|error| Error::Parallel(error.to_string()))?;
...
fn local_architecture_static_binding_targets<G, A>(
    partition: &eredu_runtime::ArchitecturePartition<G, A>,
) -> (BTreeSet<String>, BTreeSet<String>) {
    let mut targets = BTreeSet::new();
    let mut vocabulary_targets = BTreeSet::new();
    for group in partition.parameter_bindings() {
        for member in group.members() {
            targets.insert(member.target().to_owned());
            if group.role() == eredu_runtime::ParameterRole::Vocabulary {
                vocabulary_targets.insert(member.target().to_owned());
            }
        }
    }
    (targets, vocabulary_targets)
}
...
fn load_muse_glimmer_pipeline(
    ...
    if static_roles.contains(&"vision") {
...
fn load_neutral_qwen_vl_pipeline(
    ...
    if static_roles.contains(&"norm") {
        ...
        if static_roles.contains(&"output") {
            ...
            if let Some(head) = &mut modules.text.lm_head {
                loaded.load(
                    eredu_runtime::ParameterGroupOwner::static_role("output"),
                    head,
                    store.as_ref(),
...
impl MuseGlimmerPipelinePartition {
    ...
    fn forward_decoder(
        ...
        let (partition_input, auxiliary) = match input {
            PipelineStageInput::Tokens(tokens) => (
                eredu_architectures::muse_glimmer::TextPartitionInput::Tokens(tokens),
                PipelineAuxiliaryState::default(),
            ),
            PipelineStageInput::Hidden(payload) => (
        ...
        if self.partition.ownership().owns_output() {
            let logits =
                if let Some(execution) = execution.filter(|value| value.is_tensor_parallel()) {
                    ...
                    self.architecture
                        .finish_partition_text_parallel(&hidden, group, execution.stream())
                        .map_err(|error| Error::UnsupportedArchitecture(error.to_string()))?
                } else {
                    self.architecture
                        .finish_partition_text(&hidden, stream)
...
impl Gemma4PipelinePartition {
    ...
    fn forward_decoder(
        ...
        let decoder_group =
            architecture_decoder_group::<_, PipelineRangeState<'_>>(&self.architecture)?;
        forward.hidden = if let Some(expert_cache) = expert_cache {
            let assignment = assignment.as_ref().ok_or_else(|| {
...
impl InklingPipelinePartition {
    ...
    fn forward_decoder(
        ...
        let (partition_input, auxiliary) = match input {
            PipelineStageInput::Tokens(tokens) => (
                eredu_architectures::inkling::TextPartitionInput::Tokens(tokens),
                PipelineAuxiliaryState::default(),
            ),
            PipelineStageInput::Hidden(payload) => (
                eredu_architectures::inkling::TextPartitionInput::Hidden(payload.hidden.clone()),
                payload.auxiliary.clone(),
            ),
        };
        ...
        if self.range().end == self.args().text_config.num_hidden_layers as usize {
            Ok(PipelineStageOutput::EmbeddedMtpLogits {
                logits: match tensor_group {
                    Some(parallel) => self
                        .architecture
                        .project_target_logits_parallel(&hidden, parallel, stream),
                    None => self.architecture.project_target_logits(&hidden, stream),
                }
                .map_err(|error| Error::UnsupportedArchitecture(error.to_string()))?,
                hidden,
            })
...
fn load_lfm2_pipeline(
    ...
    if static_roles.contains(&"embedding") {
        ...
        loaded.load(
            eredu_runtime::ParameterGroupOwner::static_role("embedding"),
            &mut stage.static_modules_mut().embeddings,
            store.as_ref(),
            &bindings,
            quantize_on_load,
            weights_stream,
    ...
    if static_roles.contains(&"norm") {
        loaded.load(
            eredu_runtime::ParameterGroupOwner::static_role("norm"),
            &mut stage.static_modules_mut().norm,
            store.as_ref(),
            pipeline_static_bindings(&static_units, "norm")?,
            quantize_on_load,
            weights_stream,
    ...
    if static_roles.contains(&"output") {
        ...
        let output = stage
            ...
            .lm_head
            .as_mut()
            .expect("untied LFM2 output head");
        loaded.load(
            eredu_runtime::ParameterGroupOwner::static_role("output"),
            output,
            store.as_ref(),
...
fn load_nemotron_h_pipeline(
    ...
    if static_roles.contains(&"output") {
        ...
        let output = stage
            ...
            .lm_head
            .as_mut()
            .expect("untied Nemotron-H output head");
        loaded.load(
            eredu_runtime::ParameterGroupOwner::static_role("output"),
            output,
            store.as_ref(),
...
impl NemotronHPipelinePartition {
    ...
    fn forward_target(
        ...
        let target_input = match input {
            PipelineStageInput::Tokens(tokens) if owns_input => {
                eredu_architectures::nemotron_h::TargetPartitionInput::Tokens(tokens)
            }
            PipelineStageInput::Hidden(payload) if !owns_input => {
        ...
        if self.partition.ownership().owns_output() {
            ...
            let logits = match tensor_group {
                Some(group) => self
                    .architecture
                    .finish_partition_target_parallel(&hidden, group, stream),
                None => self.architecture.finish_partition_target(&hidden, stream),
            };
            let logits =
...
impl QwenHybridPipelinePartition {
    ...
    fn forward_target(
        ...
        let (partition_input, auxiliary) = match input {
            PipelineStageInput::Tokens(tokens) => (
                eredu_architectures::qwen::hybrid::TargetPartitionInput::Tokens(tokens),
                PipelineAuxiliaryState::default(),
            ),
            PipelineStageInput::Hidden(payload) => (
...
fn load_neutral_qwen_hybrid_pipeline(
    ...
    if static_roles.contains(&"embedding") {
        ...
        loaded.load(
            eredu_runtime::ParameterGroupOwner::static_role("embedding"),
            &mut stage.architecture.static_modules_mut().embeddings,
            store.as_ref(),
            &bindings,
            requested,
            weights_stream,
    ...
    if static_roles.contains(&"norm") {
        loaded.load(
            eredu_runtime::ParameterGroupOwner::static_role("norm"),
            &mut stage.architecture.static_modules_mut().norm,
            store.as_ref(),
            pipeline_static_bindings(&static_units, "norm")?,
            requested,
            weights_stream,
    ...
    if static_roles.contains(&"output") {
        ...
        if let Some(module) = &mut stage.architecture.static_modules_mut().lm_head {
            loaded.load(
                eredu_runtime::ParameterGroupOwner::static_role("output"),
                module,
                store.as_ref(),
...
fn load_kimi_linear_pipeline(
    ...
    if static_roles.contains(&"output") {
        ...
        let output = stage
            ...
            .lm_head
            .as_mut()
            .expect("untied Kimi Linear output head");
        loaded.load(
            eredu_runtime::ParameterGroupOwner::static_role("output"),
            output,
            store.as_ref(),
...
fn load_neutral_inkling_pipeline(
    ...
    if static_roles.contains(&"embedding_norm") {
        loaded.load(
            eredu_runtime::ParameterGroupOwner::static_role("embedding_norm"),
            &mut stage.static_modules_mut().embedding_norm,
            store.as_ref(),
    ...
    if static_roles.contains(&"audio") {
        let mut module = crate::backend::mlx::nn::shared::MlxModuleRef::new(
            stage
                .static_modules_mut()
                .audio
                .as_mut()
    ...
    if static_roles.contains(&"norm") {
        loaded.load(
            eredu_runtime::ParameterGroupOwner::static_role("norm"),
            &mut stage.static_modules_mut().final_norm,
            store.as_ref(),
            pipeline_static_bindings(&static_units, "norm")?,
            None,
            weights_stream,
    ...
    if static_roles.contains(&"output") {
        ...
        loaded.load(
            eredu_runtime::ParameterGroupOwner::static_role("output"),
            &mut stage.static_modules_mut().output,
            store.as_ref(),
            &bindings,
            quantize_on_load,
            weights_stream,
    ...
    if static_roles.contains(&"mtp") {
        let mut module = crate::backend::mlx::nn::shared::MlxModuleRef::new(
            stage
                .static_modules_mut()
                .mtp
                .as_mut()
                .expect("Inkling MTP static"),
        );
        loaded.load(
            eredu_runtime::ParameterGroupOwner::static_role("mtp"),
            &mut module,
            store.as_ref(),
...
fn load_neutral_deepseek_v3_pipeline(
    ...
    let (static_targets, vocabulary_targets) =
        local_architecture_static_binding_targets(&partition);
    let static_bindings = all_static_bindings
        .into_iter()
        .filter(|binding| parameter_name_in_targets(binding_target(binding), &static_targets))
        .collect::<Vec<_>>();
    let architecture_static_bindings = match parallel_layout.as_ref() {
        Some(layout) => {
            let (vocabulary, mut replicated): (Vec<_>, Vec<_>) =
                static_bindings.into_iter().partition(|binding| {
                    parameter_name_in_targets(binding_target(binding), &vocabulary_targets)
                });
            replicated.extend(shard_layer_bindings(
                vocabulary,
                "",
                store.as_ref(),
                layout,
    ...
    for role in ["embedding", "norm", "output"] {
        let (role_targets, bindings) = pipeline_static_role_binding_selection(
            partition.parameter_bindings(),
            role,
        ...
        if !bindings.is_empty() {
            loaded.load_excluding(
                ...
                weights_stream,
                stream,
                &|name| !role_targets.contains(name),
            )?;
        }
    }
...
fn load_neutral_deepseek_v4_pipeline(
    ...
    for role in ["embedding", "norm", "output", "mtp"] {
        let (role_targets, bindings) = pipeline_static_role_binding_selection(
            partition.parameter_bindings(),
            role,
            &architecture_static_bindings,
