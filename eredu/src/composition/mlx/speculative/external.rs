...
pub(crate) struct MuseTargetState {
    ...
    draft_context: Option<eredu_architectures::muse_glimmer::DFlashContext<Array>>,
    cache_len: i32,
}
...
pub(crate) struct MuseDraftState {
    ...
    draft_context: eredu_architectures::muse_glimmer::DFlashContext<Array>,
    cache_len: i32,
}
...
impl<'a> MuseGlimmerExternalExecutor<'a> {
    ...
    fn target_embeddings_on_draft(
        ...
        let hidden = self
            .target
            .embed_dflash_tokens(ids, streams.target())
            .map_err(|error| Exception::custom(error.to_string()))?;
        if !streams.is_split() {
            return Ok(hidden);
        }
    ...
    fn target_logits_on_draft(
        ...
        let logits = self
            .target
            .project_dflash_logits(&states, streams.target())
            .map_err(|error| Exception::custom(error.to_string()))?;
        if !streams.is_split() {
            return Ok(logits);
        }
...
impl SpeculativeExecutor for MuseGlimmerExternalExecutor<'_> {
    ...
    fn begin_proposal(
        ...
        let states = self
            ...
            .map_err(|error| Exception::custom(error.to_string()))?;
        let logits = self.target_logits_on_draft(&states, streams)?;
        Ok(MuseDraftState {
            logits,
            cursor: 0,
            proposal_capacity,
    ...
    fn submit_verification(
        ...
        let output = self
            .target
            .verify_dflash(&inputs, cache, &layers, streams.target())
            .map_err(|error| Exception::custom(error.to_string()))?;
        let completion = MlxSpeculativeCompletion::submit(
            std::iter::once(&output.logits).chain(output.target_states.iter()),
        )?;
        Ok(Submission {
...
impl<'a> Gemma4ExternalExecutor<'a> {
    ...
    fn state_at(
        ...
        for (policy, (keys, values)) in &output.shared_kv {
            let key_len = keys.dim(-2).min(cache_len);
            let value_len = values.dim(-2).min(cache_len);
            shared_kv.insert(
                *policy,
                (
...
impl SpeculativeExecutor for Gemma4ExternalExecutor<'_> {
    ...
    type DraftState = eredu_architectures::gemma4::AssistantState<Array>;
    ...
    fn begin_proposal(
        ...
        Ok(self
            .assistant
            .begin_round((*state.shared_kv).clone(), state.cache_len, state.hidden))
    }
    ...
    fn proposal_logits(
        ...
        self.assistant
            .draft_step(&embedding, state, streams.draft())
            .map_err(|error| Exception::custom(error.to_string()))
    }
    ...
    fn submit_verification(
        ...
        let output = self
            .target
            .verify_mtp(&inputs, cache, streams.target())
            .map_err(|error| Exception::custom(error.to_string()))?;
        let completion = MlxSpeculativeCompletion::submit([&output.logits])?;
        Ok(Submission {
    ...
    fn commit_verification(
        ...
        let replayed = self
            .target
            .verify_mtp(&retained, cache, streams.target())
            .map_err(|error| Exception::custom(error.to_string()))?;
        Ok(SpeculativeCommit {
            state: Self::state_at(
                &replayed,
