...
pub struct MlxRealtimeInput {
    ...
    pub(crate) forced_generated_audio_codebooks: Option<Vec<bool>>,
        ...
        transition
            ...
            .map(|target| match target.source() {
                ...
                RealtimeTargetSource::Forced => {
                    forced_token(target.slot(), input, stream).map(PredictionDirective::Force)
                }
...
mod tests {
    ...
    impl
        LayeredTraversalHook<
            ...
            eredu_architectures::moshi::ForwardContext<Array>,
            eredu_nn::Error,
        > for TeacherObservationCapture
    {
        fn before_unit(
            ...
            value: &Array,
            _forward: &mut eredu_architectures::moshi::ForwardContext<Array>,
            _context: &Stream,
        ...
        fn after_group(
            ...
            group: usize,
            _value: &Array,
            forward: &mut eredu_architectures::moshi::ForwardContext<Array>,
            _context: &Stream,
        ) -> Result<(), eredu_nn::Error> {
            if group == 0 {
                self.values.push((
    ...
    fn run_teacher_forced_fixture(
        ...
        for step in 0..text.dim(0) as usize {
            ...
            let mut directives = (0..depth_step.dim(1) as usize)
                .map(|prediction| {
                    token_column(&depth_step, prediction, stream).map(PredictionDirective::Force)
                })
                .collect::<Result<Vec<_>, _>>()
    ...
    fn run_personaplex_frame_fixture(
        ...
        let emitted_steps = Array::from_slice(
            &emitted_steps
                .into_iter()
