...
pub(crate) struct Gemma4Bindings {
    external_experts: bool,
}

impl Gemma4Bindings {
    pub(crate) const fn new(external_experts: bool) -> Self {
...
impl PreparedParts {
    ...
    pub(crate) fn decoder_parts(&self) -> Vec<DecoderInputPart<'_, Array>> {
        self.tokens
            .iter()
            .zip(&self.modalities)
            .zip(&self.projected)
    ...
    pub(crate) fn vision_input(&self) -> Option<VisionInput<'_, Array>> {
    ...
    pub(crate) fn audio_input(&self) -> Option<AudioInput<'_, Array>> {
