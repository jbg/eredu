...
type NeutralBlock = Unit<MlxBackend>;
...
impl eredu_runtime::ActivationObserver<Array, eredu_nn::Error> for NeutralNemotronHObserver<'_> {
...
pub(crate) fn load_model_args(model_dir: &Path) -> Result<ModelArgs, Error> {
    ...
    eredu_architectures::nemotron_h::model_args_from_config_reader(file)
        .map_err(|error| Error::UnsupportedArchitecture(error.to_string()))
}
...
impl NemotronHModel {
    ...
    pub fn forward(
        ...
        let input = eredu_architectures::nemotron_h::EmbeddedInput::target(tokens, None);
    ...
    fn forward_with_provider_context<P>(
        ...
        (
            Array,
            eredu_architectures::nemotron_h::ForwardContext<Array>,
        ),
        Error,
    >
    where
    ...
    fn forward_mtp_draft(
        ...
        let input = eredu_architectures::nemotron_h::EmbeddedInput::draft(tokens, hidden, depth);
        let expert_cache = self.expert_cache.take();
        let result = match expert_cache.as_ref() {
            Some(cache_store) => {
                let args = self.args.clone();
    ...
    for NemotronHTensorMtpTarget<'_>
{
    ...
    fn draft_logits(
        ...
        let output = self.model.forward_mtp_parallel(
            eredu_architectures::nemotron_h::EmbeddedInput::draft(&token, hidden, draft_index),
            cache,
            self.group,
            stream,
        )?;
...
pub(crate) fn prepare_gguf(
    ...
    if !matches!(architecture, "nemotron_h" | "nemotron_h_moe") {
        return Err(Error::UnsupportedArchitecture(format!(
            "Nemotron-H GGUF loader received architecture {architecture:?}"
        )));
    }
