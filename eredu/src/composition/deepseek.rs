...
impl eredu_runtime::ActivationObserver<Array, eredu_nn::Error> for NeutralDeepSeekObserver<'_> {
...
impl DeepSeekModel {
    ...
    pub(crate) fn new_state_with_options(
        &self,
    ...
    pub(crate) fn forward_embedded_with_expert_executor<'a, F>(
        &mut self,
        input: deepseek::mtp::EmbeddedInput<'a, Array>,
        state: &mut DeepSeekState,
        execute: &mut F,
    ...
    pub(crate) fn v3_args(&self) -> Option<&V3Args> {
        match &self.inner {
    ...
    pub(crate) fn v4_args(&self) -> Option<&V4Args> {
        match &self.inner {
    ...
    pub(crate) fn architecture_fingerprint(&self) -> String {
        match &self.inner {
    ...
    pub(crate) fn prompt_cache_identity(&self) -> Result<PromptCacheModelIdentity, Error> {
    ...
    pub(crate) fn prompt_cache_layer_prefix_offsets(&self) -> Result<Vec<i32>, Error> {
...
impl crate::composition::mlx::speculative::embedded::EmbeddedMtpTarget for DeepSeekModel {
    ...
    fn prefill_draft_cache(
        ...
        if matches!(&self.inner, DeepSeekModelInner::V4 { args, .. } if args.dspark.is_some()) {
            let _ = self.forward_embedded(
                deepseek::mtp::EmbeddedInput::dspark_context(&output.hidden),
                cache,
                stream,
            )?;
            return Ok(());
    ...
    fn draft_logits(
        ...
        self.forward_embedded(
            deepseek::mtp::EmbeddedInput::draft(&token, hidden, draft_index),
            cache,
            stream,
        )
    }
    ...
    fn advance_draft_cache(
        ...
        if matches!(&self.inner, DeepSeekModelInner::V4 { args, .. } if args.dspark.is_some()) {
            let _ = self.forward_embedded(
                deepseek::mtp::EmbeddedInput::dspark_context(hidden),
                cache,
                stream,
            )?;
        } else {
            for depth in 0..self.mtp_len() {
                let _ = self.forward_embedded(
                    deepseek::mtp::EmbeddedInput::draft(tokens, hidden, depth),
                    cache,
                    stream,
                )?;
            }
...
impl CausalModel<DeepSeekState> for DeepSeekModel {
    ...
    fn decode_logits(
        ...
        self.next_logits(input_tokens, state, stream)
            .map_err(|error| Exception::custom(error.to_string()))
    }
}

pub(crate) fn quantize_v3_store(
    store: Arc<dyn eredu_checkpoint::store::CheckpointSource>,
    source_args: &V3Args,
...
pub(crate) fn quantize_v4_store(
    store: Arc<dyn eredu_checkpoint::store::CheckpointSource>,
    source_args: &V4Args,
    quantization: WeightQuantization,
    stream: &Stream,
...
pub(crate) fn load_safetensors_external_experts(
    model_dir: &Path,
...
pub(crate) fn load_gguf_external_experts(
    checkpoint: &GgufCheckpoint,
