...
use crate::{
    ...
    composition::mlx_architectures::gemma4::model::{
...
struct Gemma4AssistantConfigSource {
    ...
    block_size: usize,
    text_config: crate::composition::mlx_architectures::gemma4::model::ModelArgsSource,
    #[serde(default)]
...
pub fn gemma4_assistant_config_from_value(value: &Value) -> Result<Gemma4AssistantConfig, Error> {
    ...
    let mut text_config =
        crate::composition::mlx_architectures::gemma4::model::model_args_from_source(
            source.text_config,
        )?;
    if !text_config
        .layer_schedule
    ...
    crate::composition::mlx_architectures::gemma4::model::validate_model_args(&text_config)?;
...
impl Gemma4AssistantDraftModel {
    ...
    pub fn new(mut config: Gemma4AssistantConfig, stream: &Stream) -> Result<Self, Exception> {
        ...
        crate::composition::mlx_architectures::gemma4::model::validate_model_args(
            &config.text_config,
        )
    ...
    fn forward(
        ...
        for layer in &mut self.model.layers {
            ...
            h = layer.forward(
                crate::composition::mlx_architectures::gemma4::model::AttentionInput {
                    x: &h,
                    mask: mask.as_ref(),
...
fn gemma4_assistant_config_from_gguf(
    ...
    let num_hidden_layers = gemma4::gguf_i32(metadata, &key("block_count"))?;
    ...
    let full_rope_theta =
        gemma4::gguf_optional_f32(metadata, &key("rope.freq_base"))?.unwrap_or(1_000_000.0);
    let sliding_rope_theta =
        gemma4::gguf_optional_f32(metadata, &key("rope.freq_base_swa"))?.unwrap_or(10_000.0);
    ...
    let num_kv_shared_layers =
        gemma4::gguf_optional_i64(metadata, &key("attention.shared_kv_layers"))?
            .map(i32::try_from)
            .transpose()
            .map_err(|_| {
                Error::UnsupportedArchitecture(
    ...
    let hidden_size_per_layer_input =
        gemma4::gguf_optional_i64(metadata, &key("embedding_length_per_layer_input"))?
            ...
            .unwrap_or(0);
    let draft_tokens = gemma4::gguf_optional_i64(metadata, &key("nextn_predict_layers"))?
        .unwrap_or(3)
        .checked_add(1)
    ...
    Ok(Gemma4AssistantConfig {
        ...
        backbone_hidden_size: gemma4::gguf_i32(metadata, &key("embedding_length_out"))?,
...
fn translate_gguf_weight_name(name: &str) -> String {
    ...
    let target =
        crate::composition::mlx_architectures::gemma4::model::translate_gguf_weight_name(name);
    target
