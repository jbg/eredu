...
use crate::{
    backend::mlx::architectures::qwen::dense::{gguf_i32_catalog, gguf_string},
    ...
    pub(crate) fn forward_tensor_parallel(
        ...
        let mlp = if let Some(dense) = &mut self.dense {
            let gate = crate::backend::mlx::nn::layers::silu(
                dense.gate_proj.forward(&normalized, stream)?,
                stream,
            )?;
            let up = dense.up_proj.forward(&normalized, stream)?;
            let partial = crate::backend::mlx::nn::parallel::forward_row_parallel(
                &mut dense.down_proj,
