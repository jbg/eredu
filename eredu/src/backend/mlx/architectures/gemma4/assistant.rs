...
use crate::{
    backend::mlx::architectures::gemma4::model::{
...
struct Gemma4AssistantConfigSource {
    ...
    block_size: usize,
    text_config: crate::backend::mlx::architectures::gemma4::model::ModelArgsSource,
    #[serde(default)]
...
pub fn gemma4_assistant_config_from_value(value: &Value) -> Result<Gemma4AssistantConfig, Error> {
    ...
    let mut text_config =
        crate::backend::mlx::architectures::gemma4::model::model_args_from_source(
            source.text_config,
        )?;
    if !text_config
        .layer_schedule
    ...
    crate::backend::mlx::architectures::gemma4::model::validate_model_args(&text_config)?;
...
impl Gemma4AssistantDraftModel {
    ...
    pub fn new(mut config: Gemma4AssistantConfig, stream: &Stream) -> Result<Self, Exception> {
        ...
        crate::backend::mlx::architectures::gemma4::model::validate_model_args(&config.text_config)
    ...
    fn forward(
        ...
        for layer in &mut self.model.layers {
            ...
            h = layer.forward(
                crate::backend::mlx::architectures::gemma4::model::AttentionInput {
                    x: &h,
                    mask: mask.as_ref(),
...
fn translate_gguf_weight_name(name: &str) -> String {
    ...
    let target =
        crate::backend::mlx::architectures::gemma4::model::translate_gguf_weight_name(name);
    target
