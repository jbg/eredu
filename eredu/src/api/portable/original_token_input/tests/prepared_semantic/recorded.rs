use super::*;
use crate::api::{
    ControlledGenerationEvent, ControlledGenerationRecord, ObservedGenerationEvent, TraceLimits,
};
...
fn recorded_session_preserves_ordinary_events_forcing_pause_timing_and_source_custody() {
    for mode in 0..4 {
        ...
        let ordinary = model
            .start_prepared_chat(make_request(), &cancel)
            .unwrap()
            .unwrap()
            .run(&cancel, &mut |event| ordinary_events.push(event))
            .unwrap();
        ...
        assert_eq!(
            session.run(collect(&mut records)).unwrap(),
        ...
        assert!(matches!(
            ...
            Some(ObservedGenerationEvent::Token { forced: true, .. })
        ));
        assert_eq!(first.timing, session.timing());
        assert_eq!(
            session.resume(collect(&mut records)).unwrap(),
        ...
        let semantic = records
            ...
            .filter_map(|record| match record.event.progress() {
                Some(ObservedGenerationEvent::Semantic { event, .. }) => Some(event),
                _ => None,
            })
            .collect::<Vec<_>>();
        ...
        assert!(
            ...
            "escaped records retain their actual producer"
        );
        drop(records);
        assert!(
            pool.live_charge_bytes().unwrap() > 0,
            "escaped output checkpoint retains its actual producer"
        );
        assert_eq!(checkpoint.next_prediction, expected_ids.len() as u64);
        drop(checkpoint);
...
fn recorded_failure_retains_original_prefix_and_cancellation_precedes_prediction() {
    for fail in [false, true] {
        ...
        if fail {
            ...
            assert_eq!(
                error.session_failure().unwrap().committed_token_ids(),
                Some(prefix.as_slice())
            );
            drop(session);
            drop((records, chat, source, model));
