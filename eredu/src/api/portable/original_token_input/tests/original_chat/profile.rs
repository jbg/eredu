    ...
    for (manual, enabled) in [(false, false), (true, true)] {
        ...
        let template = pool
            .compile_chat_template(
                ChatTemplatePlan::prepare_utf8(INKLING, "unrelated-profile-id").unwrap(),
            )
            .unwrap();
        ...
        let expected = ordinary
            ...
            .pop()
            .unwrap();
        let prepared = prepare_original_text_chat(
            &runtime,
            &template,
            &tokenizer,
            &request,
        ...
        let escaped = prepared.clone_render_for_test();
        ...
        let mut sink = |event: GenerationPlainTextEvent<'_>| {
        ...
        drop((request, ordinary, raw, json, template, tokenizer, runtime));
...
fn default_profile_without_controls_preserves_generation_policy_and_shared_cursor() {
    ...
    for manual in [false, true] {
        ...
        let mut raw = tokenizers::Tokenizer::from_bytes(&serde_json::to_vec(&source).unwrap()).unwrap();
        raw.add_special_tokens(crate::api::request::probes::muse::STRUCTURAL.iter()
            .map(|spelling|tokenizers::AddedToken::from(*spelling,true).normalized(false))
        ...
        let mut model = LoadedModel::from_runtime(
            runtime, ChatTokenizer::from_tokenizer(raw.clone()), LoadedTextModelConfig {
        ...
        // The ordinary facade, including its behavioral profile selector, is
        // the oracle. A raw false-toggle template render omits this suffix.
        let ordinary = model.prepare_chat(chat.clone()).unwrap();
        assert_eq!(ordinary.format_profile_identity.as_deref(),Some("muse-glimmer.atem.v1"));
        ...
        let source = model.compile_managed_chat_source(&tokenizer,file,&cancellation).unwrap().unwrap();
        let cold = pool.used_bytes().unwrap();
        let settings = PreparedChatGenerationSettings {
            overrides: GenerationConfigOverrides {max_new_tokens:Some(3),..Default::default()},
            inference: TextInferencePolicy {managed_memory_capacity_bytes:Some(u64::MAX),..Default::default()},
            ..Default::default()
        };
        facts.borrow_mut().short = true;
        let error = match model.start_managed_chat(&source,ManagedChatRequest::new(&chat,settings),&cancellation) {
            Err(error)=>error,Ok(_)=>panic!("short source must refuse"),
        };
        ...
        let mut emit=|event:GenerationPlainTextEvent<'_>|{
            if let GenerationPlainTextEvent::TextDelta(text)=event{visible.push_str(text);}
        };
        let output=if manual {
            let mut session=model.start_managed_chat(&source,ManagedChatRequest::new(&chat,settings),&cancellation).unwrap().unwrap();
            while session.finish_reason().is_none(){session=session.advance(&cancellation,&mut emit).unwrap();}
            session.into_output().unwrap_or_else(|_|panic!("terminal default-profile chat"))
        } else {
            model.generate_managed_chat(&source,ManagedChatRequest::new(&chat,settings),&cancellation,&mut emit).unwrap().unwrap()
        };
        ...
        drop((output,source,tokenizer,model,ordinary,raw,json,chat));
        assert!(pool.used_bytes().unwrap()>0);
