...
fn production_deepseek_templates_render_tools_history_and_exact_generation_prompts() {
    ...
    for (template, identity, preceding_tokens, golden_prompt_signature) in [
        (
            ...
            31,
            [
                0xf9, 0x55, 0x2c, 0xb9, 0x46, 0x0a, 0x24, 0xd9, 0x6f, 0xf0, 0x7c, 0xeb, 0x51, 0xa2,
                0xcd, 0xce, 0x0f, 0x72, 0x48, 0x9a, 0x52, 0x73, 0x3a, 0x0f, 0x48, 0x87, 0x60, 0xef,
                0x72, 0xd7, 0x3c, 0xf9,
            ],
        ),
        (
            ...
            41,
            [
                0xba, 0x1b, 0xd6, 0x99, 0x6a, 0x25, 0xc6, 0xe9, 0x12, 0x3c, 0x8f, 0xff, 0xc4, 0x68,
                0x61, 0xc4, 0xe6, 0x49, 0xdc, 0x65, 0xe2, 0x8b, 0xe4, 0x03, 0x69, 0x8f, 0x0c, 0xce,
                0x39, 0x6d, 0x70, 0x3b,
            ],
        ),
    ] {
    ...
    for (template, identity, expected_generation_prompt, golden_prompt_signature) in [
        (
            ...
            "",
            [
                0x02, 0xae, 0x17, 0x6a, 0x5a, 0x62, 0x18, 0x3d, 0x63, 0x3e, 0x65, 0xe1, 0x14, 0x24,
                0xb9, 0xc8, 0x87, 0x1b, 0xe9, 0x4c, 0x4e, 0x33, 0x79, 0xb6, 0xf7, 0x63, 0xaa, 0xb7,
                0x01, 0xd7, 0x14, 0x1e,
            ],
        ),
        (
            ...
            "  <｜Assistant｜>    </think>",
            [
                0x13, 0x39, 0xe8, 0xd1, 0xf9, 0xa3, 0xb8, 0x45, 0x80, 0x2d, 0xc4, 0xcf, 0x8c, 0x52,
                0x8f, 0x41, 0x37, 0x91, 0xe2, 0xc4, 0x78, 0x69, 0x14, 0xb0, 0x80, 0x25, 0xa6, 0xdf,
                0xf7, 0xca, 0x71, 0x0e,
            ],
        ),
    ] {
...
#[test]
fn missing_chat_template_has_no_family_fallback() {
    for model_type in ["llama", "gemma4"] {
        let dir = temp_model_dir(&json!({"model_type": model_type}).to_string());
        assert!(load_chat_template(&dir).unwrap().is_none());
        assert!(chat_template_kwargs(&dir).unwrap().is_empty());
        fs::remove_dir_all(dir).unwrap();
    }
}
