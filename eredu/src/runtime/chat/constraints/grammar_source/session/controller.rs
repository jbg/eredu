...
use eredu_core::{
    HostMetadataFunding, HostMetadataFundingError, HostPreparationAuthority, OriginalSourceWitness,
    PackedTokenFilter, PackedTokenFilterError, SharedTokenFilter,
    speculative::{
    ...
    fn copy_prepared_grammar(
        ...
        match result {
            ...
            Err(cause) => Err(OriginalPreparedGrammarControllerCopyError {
                ...
                history,
                validity: self.validity.clone(),
                source: Arc::clone(&original.parser().source),
                funding,
            }),
        }
    }
