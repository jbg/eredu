...
use super::{
    OriginalGrammarTokenParser, OriginalGrammarTokenParserCopyError,
    OriginalGrammarTokenParserError, OriginalGrammarVocabulary,
};
...
impl OriginalGrammarState {
    ...
    fn copy_controls() -> Option<usize> {
        let parts = [
            ...
            size_of::<HostMetadataFunding>(),
            size_of::<Arc<OriginalGrammarVocabulary>>(),
            size_of::<Storage>(),
            size_of::<Box<Storage>>(),
    ...
    pub(super) fn is_initial_controller(&self) -> bool {
        !self.terminal_eos_alias_committed && self.parser().parser().num_tokens() == 0
    }
    ...
    pub(super) fn try_activation(
        ...
        self.operation(|owner| {
            ...
            let headroom = vocabulary
                .declaration
                .template()
                .memory_policy()
                .estimate(bytes.len())
                .ok_or(Cause::Overflow)?;
            owner.funding.reserve_metadata(headroom)?;
            if vocabulary.trie_source().trie().decode(tokens.ids()) != bytes[..] {
