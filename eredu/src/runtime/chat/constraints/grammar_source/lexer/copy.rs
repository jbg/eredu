...
use llguidance::{
    api::ParserLimits,
    derivre::raw::{ParserAllocationFunding, ParserAllocationPreparationError},
    earley::{
...
enum Cause {
    ...
    #[error(transparent)]
    Backing(#[from] ParserAllocationPreparationError<HostMetadataFundingError>),
    #[error(transparent)]
    Lexer(#[from] PreparedLexerCopyFailure<HostMetadataFundingError>),
    #[error(transparent)]
    Parser(#[from] PreparedTokenParserError<HostMetadataFundingError>),
}
...
fn backing_callback(
    funding: HostMetadataFunding,
) -> impl Fn(usize) -> Result<(), HostMetadataFundingError> + Send + Sync + 'static {
...
impl OriginalGrammarTokenParser {
    pub(in crate::runtime::chat::constraints) fn copy_required_bytes(&self) -> Option<usize> {
        ...
        Self::copy_controls()?
            .checked_add(ParserAllocationFunding::preparation_bytes(&callback)?)?
            .checked_add(
                self.lexer
                    .lexer
                    .copy_required_bytes::<HostMetadataFundingError>()?,
            )?
            .checked_add(
                self.parser
                    .copy_required_bytes::<HostMetadataFundingError>()?,
        ...
        let parts = [
            ...
            size_of::<
                Result<
                    ...
                    ParserAllocationPreparationError<HostMetadataFundingError>,
                >,
            >(),
            size_of::<Result<PreparedLexer, PreparedLexerCopyFailure<HostMetadataFundingError>>>(),
            size_of::<
                Result<PreparedTokenParser, PreparedTokenParserError<HostMetadataFundingError>>,
            >(),
    ...
    pub(in crate::runtime::chat::constraints) fn try_copy(
        ...
        let result = (|| -> Result<PreparedTokenParser, Cause> {
            ...
            let backing = ParserAllocationFunding::prepare(backing_callback(funding.clone()))?;
            lexer = Some(
                self.lexer
                    .lexer
        ...
        match result {
            Ok(parser) => Ok(Self {
                ...
                lexer: OriginalGrammarLexer {
                    lexer: Box::new(lexer.expect("completed independent lexer")),
