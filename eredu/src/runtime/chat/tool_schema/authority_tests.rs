    ...
    let funding = crate::runtime::chat::preparation_memory::test_funding({
        ...
        move |bytes| {
            ...
            if calls.fetch_add(1, Ordering::SeqCst) == fail_at {
                Err(eredu_nn::workspace::HostMetadataFundingError::Capacity {
...
#[test]
fn funded_schema_source_shares_its_graph_and_retains_construction_accounts() {
    let schema = serde_json::json!({
        "$schema": "http://json-schema.org/draft-07/schema#",
        "type": "object",
        "properties": {"count": {"type": "integer", "minimum": 17}},
        "required": ["count"],
    ...
    #[derive(Debug)]
    struct Loan;
    impl eredu_nn::workspace::HostMetadataAccount for Loan {
        fn reserve_metadata(
            &self,
            _: usize,
    ...
    let loan = eredu_nn::workspace::HostMetadataFunding::new(Loan).unwrap();
    alias.validate(r#"{"count":41}"#, &loan).unwrap();
    let failure = alias.validate(r#"{"count":16}"#, &loan).unwrap_err();
    drop(alias);
    assert_eq!(retired.load(Ordering::SeqCst), 0);
    assert_eq!(authority_retired.load(Ordering::SeqCst), 0);
    drop(failure);
    assert_eq!(retired.load(Ordering::SeqCst), 1);
    assert_eq!(authority_retired.load(Ordering::SeqCst), 1);
}
...
fn schema_headroom_refusals_preserve_the_original_cause() {
    ...
    let (accepted, calls, _) = compilation_funding(usize::MAX);
    let source =
    ...
    for cut in 1..total {
        ...
        loop {
            if matches!(
                cause.downcast_ref::<eredu_nn::workspace::HostMetadataFundingError>(),
                Some(eredu_nn::workspace::HostMetadataFundingError::Capacity { available: 0, .. })
            ) {
                break;
            }
            cause = cause.source().expect("original typed funding refusal");
        }
...
fn original_borrowed_schema_input_matches_full_engine_and_retains_first_failure() {
    ...
    for raw in [r#"{"next":{"next":{}}}"#, r#"{"next":3}"#] {
        ...
        assert_eq!(
            actual.is_ok(),
            recursive_ordinary.is_valid(&value),
            "{raw}: {actual:?}"
        );
    }
    ...
    for raw in [
        r#"{"count":18446744073709551615,"name":"é🙂","items":[true,false]}"#,
    ...
    for extra in [
        ...
        for raw in [
            ...
            r#"{"p0":true,"mode":"absent"}"#,
            r#"{"p0":false,"extra":true}"#,
            r#"{"p18":7}"#,
        ] {
            let value: Value = serde_json::from_str(raw).unwrap();
    ...
    for raw in [
        ...
        r#"{"exact":18446744073709551614}"#,
        r#"{"exact":18446744073709551616.0}"#,
        r#"{"range":2.5}"#,
        r#"{"range":-0.0}"#,
        r#"{"upper":18446744073709551616.0}"#,
        r#"{"lower":-9223372036854775808}"#,
        r#"{"range":"not a number"}"#,
    ] {
        let value: Value = serde_json::from_str(raw).unwrap();
        let actual = source.validate(raw, &funding);
        assert_eq!(
    ...
    for raw in [
        ...
        r#"{"array":[[1,"é"],true,null]}"#,
        r#"{"array":[[1,"é🙂"],false,null]}"#,
        r#"{"mixed":17.0}"#,
        r#"{"mixed":18}"#,
        r#"{"single":[1,3]}"#,
    ] {
    ...
    impl HostMetadataAccount for Cut {
        fn reserve_metadata(&self, bytes: usize) -> Result<(), HostMetadataFundingError> {
    ...
    let funding = HostMetadataFunding::new(Cut {
        calls: calls.clone(),
        limit: limit.clone(),
    })
    .unwrap();
    ...
    assert_eq!(
        ...
        count - 1,
        "first late funding refusal prevents further spending"
    );
    let mut cause: &(dyn std::error::Error + 'static) = &failed;
    loop {
        if cause.downcast_ref::<HostMetadataFundingError>().is_some() {
            break;
        }
        cause = cause
            .source()
            .expect("reached recursive equality keeps its real funding cause");
    }
    ...
    for (raw, expected) in [
        ...
        (
            r#"{"object":{"a":[1,{"z":"é🙂"}],"b":true,"extra":0}}"#,
            false,
        ),
    ...
    match error.kind() {
        jsonschema::error::ValidationErrorKind::Constant { expected_value } => {
            assert_eq!(expected_value, &constant)
        }
    ...
    for raw in [
        ...
        r#"{"three":18446744073709551614}"#,
        r#"{"two":-0.0}"#,
        r#"{"two":1.5}"#,
        r#"{"wide":9007199254740994.0}"#,
        r#"{"two":"skip"}"#,
    ] {
        let value: Value = serde_json::from_str(raw).unwrap();
        let actual = source.validate(raw, &funding);
    ...
    for (pattern, texts) in [
        ...
        (
            r"^\S*$",
            vec!["", "é🙂", "a b", "a\u{a0}b", "a\u{feff}b", "a\u{85}b"],
        ),
    ] {
        ...
        let raw = r#"{"name":7}"#;
        assert_eq!(
            source.validate(raw, &funding).is_ok(),
            ordinary.is_valid(&serde_json::from_str(raw).unwrap())
        );
    }
    ...
    for count in [0, 1, 2, 3, 15, 16, 31] {
        for duplicate in [false, true] {
            let mut items: Vec<Value> = (0..count)
                .map(|i| serde_json::json!({"n":i,"nested":[true,null,"é🙂"]}))
                .collect();
            if duplicate && count > 1 {
                items[count - 1] = serde_json::json!({"n":0.0,"nested":[true,null,"é🙂"]});
            }
    ...
    impl HostMetadataAccount for UniqueCut {
        fn reserve_metadata(&self, bytes: usize) -> Result<(), HostMetadataFundingError> {
    ...
    let cut_funding = |fail_at| {
        let calls = Arc::new(AtomicUsize::new(0));
        let retired = Arc::new(AtomicBool::new(false));
        let funding = HostMetadataFunding::new(UniqueCut {
            calls: calls.clone(),
            fail_at,
            retired: retired.clone(),
        })
    ...
    for cut in 1..total {
        ...
        assert_eq!(
            calls.load(Ordering::SeqCst),
            cut + 1,
            "first failure must stop spending"
        );
        let mut cause: &(dyn std::error::Error + 'static) = &failure;
        let mut typed = false;
        loop {
    ...
    for raw in [
        ...
        r#"{"email":"a@example.invalid","blob":"e30="}"#,
        r#"{"email":7}"#,
        r#"{"blob":false}"#,
    ] {
        let value: Value = serde_json::from_str(raw).unwrap();
    ...
    for bounds in [
        serde_json::json!({}),
        serde_json::json!({"minContains":2}),
        serde_json::json!({"maxContains":2}),
        serde_json::json!({"minContains":1,"maxContains":2}),
    ...
    for pattern in ["^é", "^(?!bad)"] {
        let schema =
            serde_json::json!({"type":"object", "properties": {"name":{"pattern":pattern}}});
        let ordinary = compile(&schema).unwrap();
        let raw = r#"{"name":"é🙂"}"#;
        ...
        let total = calls.load(Ordering::SeqCst);
        assert!(total > 0);
        for raw in [r#"{"name":"é🙂"}"#, r#"{"name":"bad"}"#, r#"{"name":""}"#, r#"{"name":"abc"}"#] {
            assert_eq!(source.validate(raw, &funding).is_ok(), ordinary.is_valid(&serde_json::from_str(raw).unwrap()));
        }
        drop(funding);
        for cut in [setup, setup + (total - setup) / 2, total - 1] {
            let (funding, calls, retired) = cut_funding(cut);
            let failure = source.validate(raw, &funding).unwrap_err();
            assert_eq!(calls.load(Ordering::SeqCst), cut + 1);
            let mut cause: &(dyn std::error::Error + 'static) = &failure;
            loop {
                if matches!(cause.downcast_ref::<HostMetadataFundingError>(), Some(HostMetadataFundingError::Capacity { available: 0, .. })) { break; }
                cause = cause.source().expect("original typed allocation refusal");
            }
            drop(funding);
            assert!(!retired.load(Ordering::SeqCst));
            drop(failure);
