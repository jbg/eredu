...
use eredu_runtime::working_memory::{
    InferenceExecutionIdentity, MemoryLedger, WorkingMemoryError, WorkspaceCopyLimits,
};
...
impl Source {
    pub(super) fn validate(&self, runtime: &ModelRuntime<MockBackend>) -> Result<(), MockError> {
        ...
        if !self.execution.same_execution(&env.execution) || !self.pool.same_ledger(&env.pool) {
...
pub(super) fn preparation_bytes() -> Result<u64, WorkingMemoryError> {
    [
        size_of::<Source>(),
        size_of::<SavedComponents>(),
        size_of::<SavedSampling>(),
        size_of::<Sampling>(),
        size_of::<Native>(),
        size_of::<HostMetadataFunding>(),
        size_of::<WorkspaceCopyLimits>(),
        size_of::<Option<PendingTextInput<Prompt, MockToken>>>(),
...
pub(super) fn estimates(
    ...
    let native = Layout::new::<[usize; 2]>()
        .extend(Layout::new::<SourceData>())
        .ok()?
        .0
        .pad_to_align()
        .size()
    ...
    Some([native, sampling, pending].map(|n| SnapshotEstimate {
...
pub(super) fn capture(
    ...
    let shell = Layout::new::<[usize; 2]>()
        ...
        .pad_to_align()
        .size();
    let capture_plan = state
        .funded
        .as_ref()
        .map(|c| c.prepare_checkpoint())
        .transpose()
        .map_err(|e| {
            MockError::ProviderRetained(admitted_text::funded_error(
                e,
                state.original.as_ref().unwrap().funding(),
            ))
        })?;
    let capture_bytes = capture_plan
        .as_ref()
        .map(|p| p.required_bytes())
        .transpose()
        .map_err(|_| WorkingMemoryError::UnknownBound)?
        .unwrap_or(0);
    let copy_bytes = bytes
        .checked_add(shell)
        .and_then(|n| n.checked_add(usize::try_from(capture_bytes).ok()?))
        .ok_or(WorkingMemoryError::Overflow)?;
    let resolved = limits
        .memory_limits
        .resolve(env.pool.topology())
        .map_err(WorkingMemoryError::from)?;
    let mut additional = limits
        .additional_headroom
        .resolve(env.pool.topology())
        .map_err(WorkingMemoryError::from)?;
    if let Some(requirements) = &limits.additional_requirements {
        additional = additional
            .checked_add(requirements)
            .map_err(WorkingMemoryError::from)?;
    }
    let host_domain = env.pool.topology().host_domain();
    for (domain, charge) in additional.iter() {
        if domain != host_domain && charge.total().map_err(WorkingMemoryError::from)? != 0 {
            return Err(WorkingMemoryError::IdentityMismatch.into());
        }
    }
    let required = (copy_bytes as u64)
        .checked_add(limits.additional_host_metadata_bytes)
        .and_then(|n| n.checked_add(additional.get(host_domain).ok()?.total().ok()?))
        .ok_or(WorkingMemoryError::Overflow)?;
    let funding = env
        .pool
        .prepare_workspace_metadata(&env.execution, resolved)?;
    funding
        .reserve_metadata(usize::try_from(required).map_err(|_| WorkingMemoryError::Overflow)?)?;
    ...
    let capture = capture_plan
        .map(|p| p.construct(&capture_host))
        .transpose()
    ...
    let source = Source(Some(Rc::new(SourceData {
        ...
        frontier: state
            ...
            .as_ref()
            .unwrap()
            .completed_positions(state.sampling.prediction)?,
        capture,
        _host: host.clone(),
        _funding: funding,
    })));
