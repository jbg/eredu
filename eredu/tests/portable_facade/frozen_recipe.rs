...
fn source_preparation_refuses_before_prompt_sampling_or_submission() {
    ...
    let error = model
        .prepare_chat(&source, &request(), &crate::memory::limits(1), &cancel)
        .unwrap_err();
    assert!(
...
fn recognized_prepared_chat_clones_keep_exact_source_charge_after_model_drop() {
    ...
    let prepared = model
        ...
        .unwrap()
        .unwrap();
    let bytes = pool.live_charge_bytes().unwrap();
    assert!(bytes > 0);
...
fn cancelled_and_foreign_preparation_refuse_before_backend_work() {
    ...
    assert!(model
        ...
        .is_none());
    assert_eq!(pool.live_charge_bytes().unwrap(), before);
    let foreign = crate::memory::host_ledger(8 * 1024 * 1024 * 1024, 0).unwrap();
...
fn manual_and_uninterrupted_semantics_share_original_sources_and_output_custody() {
    ...
    for manual in [false, true] {
        ...
        assert!(
            pool.live_charge_bytes().unwrap() > baseline,
