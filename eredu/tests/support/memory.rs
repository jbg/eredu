...
pub trait LedgerFixture {
    fn live_charge_bytes(&self) -> Result<u64, WorkingMemoryError>;
...
pub fn finite_payload_capacity(limits: &MemoryLimitDeclarations) -> Option<u64> {
    match limits
        .resolve(topology_ref())
        .unwrap()
        .get(topology_ref().host_domain())
        .unwrap()
    {
        MemoryLimit::Finite(bytes) => Some(bytes.checked_sub(fixed_bytes()).unwrap()),
