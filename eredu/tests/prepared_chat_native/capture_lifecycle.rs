...
fn checkpoint<'a>(
    ...
    let invalid = session
        ...
        .err()
        .expect("invalid sampler change must refuse");
...
fn serial_branch<'a>(
    ...
    let mut serial = session
        .fork_snapshot(
            saved,
            eredu::api::PreparedChatResumeSettings::default(),
            native_limits(CAPACITY),
            cancellation,
        )
        .unwrap_or_else(fail)
    ...
    session = session
        .advance(cancellation, &mut |event| serial_events.push(event))
        .unwrap_or_else(fail);
...
fn terminal(
    ...
    let terminal_facts = terminal
        .resume_source_facts()
        .expect("actual saved native facts");
    ...
    assert_eq!(
        session
            .sampling_state()
            .unwrap_or_else(|error| panic!("restored terminal sampling failed: {error:?}")),
        saved_sampling
    );
    ...
    assert_eq!(
        restored_terminal.resume_source_facts(),
        Some(terminal_facts)
    );
    let mut terminal_branch = session
        .fork_snapshot(
            &restored_terminal,
            PreparedChatResumeSettings::default(),
            native_limits(CAPACITY),
        ...
        .unwrap_or_else(|error| panic!("terminal fork failed: {error:?}"))
        .unwrap();
    assert_eq!(terminal_branch.status(), terminal_status);
    assert_eq!(terminal_branch.next_prediction(), terminal_prediction);
    session
        .exchange(&mut terminal_branch)
        .unwrap_or_else(|error| panic!("terminal branch installation failed: {error:?}"));
    assert_eq!(session.status(), terminal_status);
    assert_eq!(
        session
            .sampling_state()
            .unwrap_or_else(|error| panic!("installed terminal sampling failed: {error:?}")),
        saved_sampling
    );
    let forked_terminal = session
        .snapshot(
            &terminal_budget,
            native_limits(CAPACITY),
            WorkspaceCopyLimits::new(native_limits(CAPACITY)),
    ...
    session = session
        .advance(cancellation, &mut |_| {
            panic!("terminal branch emitted another event")
        })
        .unwrap_or_else(|error| panic!("terminal advance failed: {error:?}"));
    assert_eq!(session.token_ids(), terminal_ids);
    assert_eq!(session.next_prediction(), terminal_prediction);
    assert_eq!(
        capture_deliveries.get(),
        delivered,
        "terminal restore/fork issued another capture"
    );
    session
        .exchange(&mut terminal_branch)
        .unwrap_or_else(|error| panic!("terminal branch return failed: {error:?}"));
    assert_eq!(session.token_ids(), terminal_ids);
    ...
    let original = session
        .run(cancellation, &mut |_| {
            panic!("terminal run emitted another event")
        })
