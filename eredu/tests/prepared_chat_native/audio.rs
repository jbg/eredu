...
fn authenticated_image_audio_tools_preserve_ordinary_manual_and_recorded_semantics() {
    for choice in [ToolChoice::Required, ToolChoice::Auto] {
        ...
        for (segment, at) in [(1, image), (3, audio)] {
            assert_eq!(
                binding.coordinates()[segment].rendered_range(),
                [at, at + 1]
            );
            assert_eq!(
                binding.coordinates()[segment].decoder_range(),
                Some([at as u64, at as u64 + 1])
            );
        }
        assert_eq!(
            binding.coordinates()[4].decoder_range().unwrap()[1],
            positions
        );
        let foreign = model
            .prepare_chat(&source, &policy, &native_limits(CAPACITY), &cancellation)
        ...
        for mode in 0..3 {
            ...
            let (output_ids, finish) = if mode == 2 {
                ...
                for (segment, at) in [(1, image), (3, audio)] {
                    assert_eq!(
                        attribution.segments[segment].plan.decoder_range,
                        [at as u64, at as u64 + 1]
                    );
                    assert!(matches!(
                        attribution.segments[segment].tokens,
                ...
                let text_ids = prompt
                    ...
                    .collect::<Vec<_>>();
                assert_eq!(
                    attribution.canonical_token_ids.as_slice(),
                    text_ids.as_slice()
                );
                let output = if mode == 0 {
                    ...
                    session
                        .into_output()
                        .unwrap_or_else(|_| panic!("terminal audio session"))
                };
                (output.token_ids.to_vec(), output.finish_reason)
            };
            assert!(
                output_ids.starts_with(&script[..5]),
                "{choice:?} mode {mode}: {output_ids:?}"
            );
            assert!(
                output_ids.len() >= 5,
                "prefill followed by multiple cached predictions"
            );
            ...
            let starts = rows
                .iter()
                .filter_map(|event| event.get("ToolCallStart"))
                .collect::<Vec<_>>();
            ...
            let fragments = rows
                .iter()
                .filter_map(|event| event.get("ToolArgumentsDelta"))
                .map(|delta| {
                    assert_eq!(delta["index"], 0);
                    delta["json_fragment"].as_str().unwrap()
                })
                .collect::<Vec<_>>();
            assert!(
                fragments.len() >= 2,
                "arguments must span committed token events"
            );
            assert_eq!(
            ...
            assert!(matches!(
                events.first(),
                Some(SemanticEvent::ToolCallStart { .. })
            ));
            assert!(matches!(
            ...
            assert!(events[1..events.len() - 2]
                .iter()
                .all(|event| matches!(event, SemanticEvent::ToolArgumentsDelta { .. })));
            assert!(matches!(
                events.last(),
            ...
            if let Some(expected) = &expected {
                assert_eq!(&result, expected, "{choice:?} mode {mode}");
            } else {
...
impl<'a> Prompt<'a> {
    fn new(text: [&'a [u32]; 3]) -> Self {
        Self {
            shapes: text.map(|part| [1, part.len()]),
            text,
    ...
    fn parts(&self) -> [HostInputPart<'_>; 5] {
        let text = |index: usize| HostInputPart {
            ...
            payload: HostTensorView {
                shape: self.shapes[index].as_slice(),
                values: HostTensorValues::U32(self.text[index]),
            },
            metadata: &[],
            extents: &[],
        };
        [
            ...
            HostInputPart {
                ...
                payload: HostTensorView {
                    shape: &[1, 4, 48],
                    values: HostTensorValues::F32(&self.patches),
                },
            ...
            HostInputPart {
                ...
                payload: HostTensorView {
                    shape: &[1, 4, 128],
                    values: HostTensorValues::F32(&self.audio),
                },
...
fn fixture() -> (tempfile::TempDir, Tokenizer, Vec<u32>) {
    ...
    tokenizer
        .add_special_tokens(
            [IMAGE, AUDIO].map(|value| AddedToken::from(value, true).normalized(false)),
        )
        .unwrap();
    tokenizer
    ...
    for tensor in checkpoint.common_tensors.iter().chain(
        ...
        for index in 0..tensor.shape.iter().product::<usize>() {
            let value: f32 =
                if tensor.key.ends_with("input_min") || tensor.key.ends_with("output_min") {
                    -4.0
                } else if tensor.key.ends_with("input_max") || tensor.key.ends_with("output_max") {
                    4.0
                } else if tensor.key.contains("norm") || tensor.key.ends_with(".layer_scalar") {
                    1.0
                } else if tensor.key.ends_with(".embed_tokens.weight") {
                    if previous.get(index % hidden).copied() == Some((index / hidden) as u32) {
                        1.0
                    } else {
                        0.0
                    }
                } else if tensor.key.ends_with("lm_head.weight") {
                    if script.get(index % hidden).copied() == Some((index / hidden) as u32) {
                        10.0
                    } else {
                        0.0
                    }
