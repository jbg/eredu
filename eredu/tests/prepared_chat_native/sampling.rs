    ...
    fn accept(&mut self, record: ControlledGenerationRecord) -> ControlFlow<()> {
        match &record.event {
            ControlledGenerationEvent::BranchStarted {
                lineage, inherited_token_ids, inherited_semantics, ..
            } => {
...
pub(super) fn check_recorded_child(
    ...
    let trace_limits = TraceLimits { per_record_bytes: 65536, total_bytes: 1 << 20 };
    let mut trace = Trace::default();
    let mut session = model.start_controlled_chat(
        PreparedChatRequest::new(chat, settings), trace_limits, Default::default(),
        |record| trace.accept(record),
    ).unwrap_or_else(fail).unwrap();
    for _ in 0..3 {
        session.step(|record| trace.accept(record)).unwrap_or_else(fail);
    }
    ...
    let parent_run = session.output_checkpoint().unwrap_or_else(fail).run_id.clone();
    session.force_next_token(script[3]).unwrap_or_else(fail);
    session.enable_snapshots(SnapshotLimits {
        max_snapshots: 1, max_branches: 1,
        retained_bytes: CAPACITY, cumulative_copy_bytes: CAPACITY * 8,
    }, CAPACITY, WorkspaceCopyLimits::new(CAPACITY)).unwrap_or_else(fail);
    let saved = session.snapshot(|record| trace.accept(record)).unwrap_or_else(fail);
    let before_fork = session.snapshot_usage().unwrap().cumulative_copy_bytes;
    let change = SamplingOverride { temperature: Some(0.5), reseed: Some(711) };
    let mut parent = session.fork(&saved, GenerationBranchOptions {
        trace_limits, capture_limits: None, sampling: Some(change), intervention: None,
    }, |record| trace.accept(record)).unwrap_or_else(fail);
    ...
    assert_eq!(session.sampling_state().unwrap_or_else(fail), parent_sampling);
    assert_eq!(session.token_ids(), &script[..3]);
    ...
    session.exchange(&mut parent, |record| trace.accept(record)).unwrap_or_else(fail);
    assert_eq!(session.output_checkpoint().unwrap_or_else(fail).run_id, child_run);
    assert_eq!(session.sampling_state().unwrap_or_else(fail), child_sampling);
    assert_eq!(session.token_ids(), &script[..3]);
    assert_eq!(session.pending_forced_token(), Some(script[3]));
    let changed = session.snapshot(|record| trace.accept(record)).unwrap_or_else(fail);
    for manual in [true, false] {
        if !manual {
            let spent = session.snapshot_usage().unwrap().cumulative_copy_bytes;
            session.restore(&changed, |record| trace.accept(record)).unwrap_or_else(fail);
        ...
        if manual {
            ...
            session.run(|record| trace.accept(record)).unwrap_or_else(fail);
        }
        ...
        let actual = prefix.iter().chain(&trace.semantics[start..]).cloned().collect::<Vec<_>>();
        assert_eq!(actual, expected_events, "changed child manual={manual}");
    }
    drop(changed);
    session.exchange(&mut parent, |record| trace.accept(record)).unwrap_or_else(fail);
    assert_eq!(session.output_checkpoint().unwrap_or_else(fail).run_id, parent_run);
    assert_eq!(session.sampling_state().unwrap_or_else(fail), parent_sampling);
    assert_eq!(session.token_ids(), &script[..3]);
    assert_eq!(session.pending_forced_token(), Some(script[3]));
    let start = trace.semantics.len();
    ...
    assert_eq!(session.snapshot_usage().unwrap().cumulative_copy_bytes, spent);
    drop(session);
