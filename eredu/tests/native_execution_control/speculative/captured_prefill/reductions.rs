//! Actual native reductions retain global attribution over the shared span driver.
use super::*;
        ...
        (CapturePayload::Summary(a), CapturePayload::Summary(b)) => {
            ...
            for (a, b) in [
                ...
                match (a, b) {
                    ...
                    (None, None) => (),
                    _ => panic!("finite statistic presence changed"),
                }
            }
        }
        (CapturePayload::Histogram(a), CapturePayload::Histogram(b)) => assert_eq!(a, b),
        _ => panic!("wrong logical payload"),
    }
}

#[test]
#[cfg_attr(feature = "metal", ignore = "run CPU-only native initialization")]
fn summary_histogram_prefill_matches_full_and_controlled_on_real_residencies() {
        ...
        for residency in super::super::super::v3_components::residencies() {
            for total in [5, 6] {
                ...
                for (lane, path) in [target_path, seed_path].into_iter().enumerate() {
                    for (kind, transform) in [
                        ...
                        CaptureTransform::Histogram {
                            edges: vec![-100., -1.234567, 0.246813, 1.987654, 100.],
                        },
                    ]
                    .into_iter()
                    .enumerate()
                ...
                for entry in &report.records {
                    ...
                    if let Some(CapturePayload::Summary(summary)) = &entry.record.payload {
                        assert!(summary.rms.unwrap() > 0.);
                    }
                }
                ...
                let controlled = model
                    .with_controlled_text_speculative(
                        ...
                        control,
                        |session| {
                            controlled_records.extend(session.step()?.unwrap().activations.iter().cloned());
