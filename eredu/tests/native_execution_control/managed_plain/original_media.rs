...
use eredu_backend_mlx::{native::MlxModelInput, backend::runtime::media::input};
...
pub(super) fn fixture() -> Fixture {
    ...
    std::fs::write(root.0.join("config.json"), serde_json::to_vec(&config).unwrap()).unwrap();
    ...
    write_tensor_plan_with_values(&root.0, resolved.architecture.checkpoint(), |key, _| {
        if key.ends_with("input_min") || key.ends_with("output_min") { Some(-4.0) }
        else if key.ends_with("input_max") || key.ends_with("output_max") { Some(4.0) }
        else { None }
    });
...
fn ordinary_prompt(parts: &[HostInputPart<'_>]) -> MlxModelInput {
    fn array(slot: HostTensorView<'_>) -> safemlx::Array {
        let shape = slot.shape.iter().map(|&v|i32::try_from(v).unwrap()).collect::<Vec<_>>();
        match slot.values {
            HostTensorValues::U32(v) => safemlx::Array::from_slice(v,&shape),
            HostTensorValues::I32(v) => safemlx::Array::from_slice(v,&shape),
            HostTensorValues::F32(v) => safemlx::Array::from_slice(v,&shape),
            HostTensorValues::Bool(v) => safemlx::Array::from_slice(v,&shape),
        }
    }
    let parts=parts.iter().map(|part| {
        let value=array(part.payload);
        let payload=match part.kind {
            ...
            _ => unreachable!("fixture source kind"),
        };
        input::input_part(part.modality,payload,
            part.metadata.iter().map(|(key,slot)|(*key,array(*slot))),part.extents.iter().cloned()).unwrap()
    }).collect::<Vec<_>>();
    MlxModelInput::from(input::ModelInput::new(&parts))
        .with_semantic_content_fingerprint("Gemma original public image/audio source").unwrap()
...
fn load_with_state(
    ...
    LoadedModel::load_execution_plan(
        &MlxBackendFactory::default().with_state_residency(state), &root.0, execution,
    ).unwrap_or_else(report_failure).into_parts().0
}

fn run(mode: &str) -> serde_json::Value {
    let root=fixture();
    let execution=ExecutionPlan::fully_resident(local_device_plan(LocalDevice::Accelerator(0)).unwrap())
        .with_required_session_capabilities(SessionCapabilities::new(true,true,true));
    let mut model=load_with_state(&root,&execution,eredu_runtime::CacheResidencyPolicy::Device);
    let settings=settings(0.0);
    if mode=="ordinary" {
        let chat=model.prepare_chat(ChatTemplateRequest {
            messages:vec![serde_json::json!({"role":"user","content":"hello"})],
            tools:vec![],tool_choice:ToolChoice::None,add_generation_prompt:true,..Default::default()
        }).unwrap_or_else(report_failure);
        let mut ordinary=settings;ordinary.inference=TextInferencePolicy {
            prefill_chunk_positions:NonZeroU64::new(2),..Default::default()
        };
        let prepared=with_parts(|parts|model.prepare_controlled_input(&chat,ordinary_prompt(parts),ordinary,
            PreparedInputInstrumentation::Unobserved,TraceLimits{per_record_bytes:1<<20,total_bytes:64<<20}))
            .unwrap_or_else(report_failure);
        assert_eq!(prepared.prompt_attribution().decoder_positions,5);
        let mut session=model.start_controlled_prepared_text(prepared,&[],Default::default(),|_|ControlFlow::Continue(()))
            .unwrap_or_else(report_failure);
        session.run(|_|ControlFlow::Continue(())).unwrap_or_else(report_failure);
        let ids=session.token_ids().to_vec();drop(session);
        assert_eq!(ids.len(),4);
    ...
    let source=model.compile_managed_plain_text_source(std::fs::File::open(root.0.join("tokenizer.json")).unwrap())
        .unwrap_or_else(report_failure);
    let input=with_parts(|parts|model.prepare_managed_model_input(parts,8<<30)).unwrap_or_else(report_failure);
    let request=ManagedPreparedInputRequest::from_original(input,settings);
    let cancel=GenerationCancellationToken::new();let mut visible=String::new();
    let mut emit=|event:GenerationPlainTextEvent<'_>|if let GenerationPlainTextEvent::TextDelta(text)=event {visible.push_str(text)};
    let output=if mode=="controlled" {
        let mut session=model.start_managed_prepared_input(&source,request,&cancel).unwrap_or_else(report_failure).unwrap();
        ...
        while session.finish_reason().is_none() {session=session.advance(&cancel,&mut emit).unwrap_or_else(report_failure);}
        session.into_output().unwrap_or_else(|_|panic!("terminal media request"))
    } else {
        model.generate_managed_prepared_input(&source,request,&cancel,&mut emit).unwrap_or_else(report_failure).unwrap()
    };
    assert_eq!(output.token_ids.as_ref().len(),4);assert_eq!(output.text.as_str(),visible);
    let address=output.text.as_str().as_ptr();drop(source);drop(model);
...
fn native_gemma_raw_original_media_matches_ordinary_managed_and_controlled() {
    verify_mode_results(
        ...
        "EREDU_PUBLIC_GEMMA_ORIGINAL_MEDIA_MODE",run);
}
