...
fn plan<const REDUCTION: u8, const UNIT: bool>() -> CapturePlan {
    ...
    .enumerate()
    {
        plan.selections.push(CaptureSelection {
            id: capture_ids::<UNIT>()[index].into(),
            path: capture_path::<UNIT>().into(),
            schedule: CaptureSchedule::default(),
            slices: if index == 1 {
                vec![CaptureSlice {
                    axis: if UNIT { "hidden" } else { "vocabulary" }.into(),
                    start: 1,
                    end: capture_width::<UNIT>() - 1,
                    stride: 3,
                }]
            } else {
                vec![]
    ...
    if REDUCTION != 0 {
        plan.selections.push(CaptureSelection {
            id: capture_ids::<UNIT>()[3].into(),
            path: capture_path::<UNIT>().into(),
            schedule: CaptureSchedule::default(),
            slices: vec![],
            transform: match REDUCTION {
                1 => CaptureTransform::Summary,
                2 => CaptureTransform::Histogram {
                    edges: HISTOGRAM_EDGES.to_vec(),
                },
...
fn run_topology<const REDUCTION: u8, const UNIT: bool>(
    mode: &str,
    ...
    let worker: fn(
        LoadedModel<eredu_backend_mlx::backend::MlxBackend<'_>>,
        Fixture,
...
fn ordinary<const REDUCTION: u8, const UNIT: bool>(
    model: LoadedModel<eredu_backend_mlx::backend::MlxBackend<'_>>,
    root: Fixture,
) -> serde_json::Value {
    capture_loaded::<REDUCTION, UNIT>("ordinary", model, root, true)
}
fn managed<const REDUCTION: u8, const UNIT: bool>(
    model: LoadedModel<eredu_backend_mlx::backend::MlxBackend<'_>>,
    root: Fixture,
) -> serde_json::Value {
    capture_loaded::<REDUCTION, UNIT>("managed", model, root, true)
}
fn controlled<const REDUCTION: u8, const UNIT: bool>(
    model: LoadedModel<eredu_backend_mlx::backend::MlxBackend<'_>>,
    root: Fixture,
) -> serde_json::Value {
    capture_loaded::<REDUCTION, UNIT>("controlled", model, root, true)
}

fn ordinary_four<const REDUCTION: u8, const UNIT: bool>(
    model: LoadedModel<eredu_backend_mlx::backend::MlxBackend<'_>>,
    root: Fixture,
) -> serde_json::Value {
    capture_loaded_world::<REDUCTION, UNIT, 4>("ordinary", model, root, true)
}
fn managed_four<const REDUCTION: u8, const UNIT: bool>(
    model: LoadedModel<eredu_backend_mlx::backend::MlxBackend<'_>>,
    root: Fixture,
) -> serde_json::Value {
    capture_loaded_world::<REDUCTION, UNIT, 4>("managed", model, root, true)
}
fn controlled_four<const REDUCTION: u8, const UNIT: bool>(
    model: LoadedModel<eredu_backend_mlx::backend::MlxBackend<'_>>,
    root: Fixture,
) -> serde_json::Value {
    capture_loaded_world::<REDUCTION, UNIT, 4>("controlled", model, root, true)
}
...
fn capture_loaded<const REDUCTION: u8, const UNIT: bool>(
    mode: &str,
    model: LoadedModel<eredu_backend_mlx::backend::MlxBackend<'_>>,
    root: Fixture,
    partitioned: bool,
...
fn capture_loaded_world<const REDUCTION: u8, const UNIT: bool, const WORLD: usize>(
    mode: &str,
    model: LoadedModel<eredu_backend_mlx::backend::MlxBackend<'_>>,
    root: Fixture,
    partitioned: bool,
...
fn capture_loaded_plan_with_capacity(
    ...
    let mut observer = |token: Option<u32>, frame: Option<SharedCapturedStep>, seconds: f64| {
        if token.is_none() {
            eprintln!(
                "PUBLIC_CAPTURE_NO_TOKEN_DELIVERY: outcome={:?} records={:?}",
    ...
    let output = if mode == "managed" {
        ...
        session
            .into_output()
            .unwrap_or_else(|_| panic!("terminal captured session"))
    };
    drop(observer);
    let mut frames = Vec::new();
    let mut ids = Vec::new();
    for (token, frame, seconds) in deliveries {
        ...
        let Some(frame) = frame else {
            panic!("paid shared receipt frame")
        };
...
fn evidence_and_values<const REDUCTION: u8, const UNIT: bool>(
    ids: &[u32],
    text: &str,
    frames: &[&CapturedStep],
    partitioned: bool,
    ...
    for (prediction, frame) in frames.iter().enumerate() {
        ...
        for (index, evidence) in frame.partitions.iter().enumerate() {
            ...
            if let Some(prior) = prior {
                ...
                assert_eq!(
                    evidence.context.execution_identity,
                    prior.execution_identity
                );
                assert_eq!(evidence.context.run_identity, prior.run_identity);
                assert_eq!(
                    evidence.context.capture_plan_identity,
                    prior.capture_plan_identity
                );
        ...
        for (index, record) in frame.records.iter().enumerate() {
            ...
            assert_eq!(
                ...
                Some([1, sequence, width].as_slice())
            );
            let columns = if index == 1 {
                (width - 2).div_ceil(3)
            } else {
                width
            };
            assert_eq!(
                ...
                Some([1, sequence, columns].as_slice())
            );
            if index == 3 && REDUCTION == 2 {
                ...
                for &value in values {
                    if !value.is_finite() {
                        non_finite += 1;
                    } else if value < HISTOGRAM_EDGES[0] {
                        below += 1;
                    } else if value > *HISTOGRAM_EDGES.last().unwrap() {
                        above += 1;
                    } else {
                        let bin = HISTOGRAM_EDGES
                ...
                assert_eq!(
                    counts.iter().sum::<u64>() + below + above + non_finite,
                    values.len() as u64
                );
                rows.push(
            ...
            if index == 3 {
                ...
                let expected = [
                    values.iter().copied().fold(f32::INFINITY, f32::min) as f64,
                ...
                for (actual, expected) in actual.into_iter().zip(expected) {
                    assert!(
                        ...
                        "summary {actual} versus raw oracle {expected}"
                    );
                }
                rows.push(
                    serde_json::json!({"id":record.selection_id,"shape":record.selected_shape,
            ...
            let TensorObservationData::F32(values) = tensor.data() else {
                panic!("floating captured activation")
            };
            assert_eq!(
                values.len(),
                if index == 2 {
            ...
            if index == 1 {
                let selected: Vec<_> = full
                    ...
                    .flat_map(|row| (1..width as usize - 1).step_by(3).map(|column| row[column]))
                    .collect();
                assert_eq!(values.as_slice(), selected.as_slice());
            }
            if index == 2 {
                assert_eq!(values.as_slice(), &full.unwrap()[..7]);
            }
            rows.push(
                serde_json::json!({"id":record.selection_id,"shape":record.selected_shape,
    ...
    for (actual, expected) in actual.iter().zip(expected) {
        ...
        for (actual, expected) in actual.iter().zip(expected) {
            ...
            for (index, (actual, expected)) in actual.iter().zip(expected).enumerate() {
                ...
                assert!(
                    (actual - expected).abs() <= 2e-5 + 1e-5 * expected.abs(),
...
fn saved_unit_capture(
    ...
    super::super::capture::check_saved_capture_loaded(
        model,
        root,
        true,
        capture_path::<true>(),
        capture_width::<true>() as usize,
        true,
    )
}
...
fn native_parallel_unit_capture_restore_and_fork_preserve_sources_and_spending() {
    compare_selected_modes(
        ...
        "EREDU_PUBLIC_MANAGED_TP_UNIT_SAVED_MODE",
        "PUBLIC_MANAGED_TP_UNIT_SAVED_RESULT:",
        "TP saved unit capture",
        2,
        &["lifecycle"],
        run_saved_unit,
    )
...
fn native_pipeline_unit_capture_receipts_match_ordinary_and_controlled() {
    compare_selected_modes_by(
        ...
        "EREDU_PUBLIC_MANAGED_PP_UNIT_CAPTURE_MODE",
        "PUBLIC_MANAGED_PP_UNIT_CAPTURE_RESULT:",
        "PP unit capture",
        2,
        &["ordinary", "managed", "controlled"],
        run_pipeline_unit,
...
fn native_combined_unit_capture_receipts_match_ordinary_and_controlled() {
    compare_selected_modes_by(
        ...
        "EREDU_PUBLIC_MANAGED_COMBINED_UNIT_CAPTURE_MODE",
        "PUBLIC_MANAGED_COMBINED_UNIT_CAPTURE_RESULT:",
        "combined unit capture",
        4,
        &["ordinary", "managed", "controlled"],
        run_combined_unit,
        compare,
...
fn native_parallel_histogram_receipts_match_ordinary_and_controlled() {
    compare_selected_modes_by(
        ...
        "EREDU_PUBLIC_MANAGED_TP_HISTOGRAM_MODE",
        "PUBLIC_MANAGED_TP_HISTOGRAM_RESULT:",
        "TP histogram capture",
        2,
        &["ordinary", "managed", "controlled"],
        run::<2, false>,
        compare,
...
fn native_pipeline_histogram_receipts_match_ordinary_and_controlled() {
    compare_selected_modes_by(
        ...
        "EREDU_PUBLIC_MANAGED_PP_HISTOGRAM_MODE",
        "PUBLIC_MANAGED_PP_HISTOGRAM_RESULT:",
        "PP histogram capture",
        2,
        &["ordinary", "managed", "controlled"],
        run_pipeline_histogram,
        compare,
    )
}
#[test]
...
fn native_combined_histogram_receipts_match_ordinary_and_controlled() {
    compare_selected_modes_by(
        ...
        "EREDU_PUBLIC_MANAGED_COMBINED_HISTOGRAM_MODE",
        "PUBLIC_MANAGED_COMBINED_HISTOGRAM_RESULT:",
        "combined histogram capture",
        4,
        &["ordinary", "managed", "controlled"],
        run_combined_histogram,
        compare,
    )
}
