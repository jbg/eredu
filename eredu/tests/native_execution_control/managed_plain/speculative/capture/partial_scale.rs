...
pub(super) fn compare(raw: &[f32], records: &[InterventionRecord], prediction: u64) {
    ...
    assert_eq!(records.len(), OPERATIONS.len());
    ...
    for (record, (id, factor, evidence)) in records.iter().zip(OPERATIONS) {
        ...
        for value in &mut effective[START..END] {
            *value *= factor;
        }
        ...
        for (actual, (side, position, expected)) in record.evidence.iter().zip([
            ...
            assert_eq!(
                ...
                Some([1, 1, (END - START) as u64].as_slice())
            );
            let payload = actual
                .payload
                .as_ref()
                .expect("ordered partial evidence was not skipped");
            match evidence {
                InterventionEvidence::Preview { max_elements } => {
                    ...
                    for (&a, &e) in values.iter().zip(&expected) {
                        close(f64::from(a), f64::from(e));
                    }
                }
                InterventionEvidence::Summary => {
                    let CapturePayload::Summary(summary) = payload else {
                        panic!("finite before/after Summary")
                    };
                    assert_eq!(summary.elements, (END - START) as u64);
                    assert_eq!(summary.finite, summary.elements);
                    assert_eq!(summary.non_finite, 0);
                    assert_eq!(summary.nan, 0);
                    assert_eq!(summary.positive_infinity, 0);
                    assert_eq!(summary.negative_infinity, 0);
                    ...
                    close(summary.mean.unwrap(), sum / n);
                    close(summary.rms.unwrap(), (square / n).sqrt());
                }
