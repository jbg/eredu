...
fn pressure(model:&LoadedModel<eredu_backend_mlx::backend::MlxBackend<'_>>) {
    let report=model.expert_cache_telemetry().unwrap().expect("selected independent bank");
...
fn native_original_independent_cache_speculation_matches_ordinary_and_controlled() {
    verify_mode_results(CASE,"EREDU_PUBLIC_SPECULATIVE_CACHE_MODE",|mode|{
        ...
        let (value,rounds)=run_artifacts_on_inspected(
            mode,make(),make(),eredu_core::DevicePlan::new("mlx","metal:0").unwrap(),
            DraftPlacementPlan::Target,settings(0.0),plan,Some(pressure),
        );
        ...
        assert!(rounds>=2,"expected multiple completed speculative rounds, got {rounds}");
        value
    });
}
