...
fn target_device()->eredu_core::DevicePlan {eredu_core::DevicePlan::new("mlx","cpu:0").unwrap()}
fn draft_placement()->DraftPlacementPlan {DraftPlacementPlan::Device {
...
fn native_original_cpu_target_gpu_assistant_matches_ordinary_and_controlled() {
    if let Ok(mode)=std::env::var(MODE) {
        ...
        let output=run_artifacts_on(&mode,target,draft,target_device(),draft_placement(),cpu_assistant::cpu_settings());
    ...
    for mode in ["ordinary","managed","controlled"] {
        ...
        let output:serde_json::Value=serde_json::from_str(stdout.lines()
            .find_map(|line|line.strip_prefix(RESULT)).expect("completed public CPU target marker")).unwrap();
        assert_eq!(output["ids"].as_array().unwrap().len(),4);
        if let Some(expected)=&expected {assert_eq!(&output,expected,"{mode}");}else{expected=Some(output);}
    }
}
#[test]
...
fn native_original_cpu_target_gpu_assistant_restores_forks_and_preserves_limits() {
    ...
    if let Ok(mode)=std::env::var(MODE) {
        let (target,draft)=super::super::super::speculative::artifacts();
        let output=run_continuation_artifacts_on(&mode,target,draft,target_device(),draft_placement(),cpu_assistant::cpu_settings());
        println!("\n{RESULT}{output}");return;
    }
...
fn native_original_cpu_target_selected_scores_capture_matches_ordinary_and_controlled(){
    capture::compare_cpu_target(capture::CaptureKind::Readouts,
...
fn native_original_cpu_target_partial_scale_capture_matches_ordinary_and_controlled(){
    capture::compare_cpu_target(capture::CaptureKind::PartialScale,
