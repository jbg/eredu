//! Full-source finite Summary matches ordinary logits through run and controlled advancement.
use super::*;
use eredu_core::capture::{CapturePayload, CaptureSummary};

const SUMMARY_CASE: &str = "managed_plain::capture::summary::native_summary_matches_full_prompt_and_decode_in_managed_and_controlled_sessions";
const SUMMARY_MODE: &str = "EREDU_ORIGINAL_SUMMARY_MODE";
const SUMMARY_RESULT: &str = "ORIGINAL_SUMMARY_RESULT:";
fn vocabulary() -> usize {
...
fn compare(values: &[f32], summary: &CaptureSummary) {
    ...
    for (actual, expected) in [
        ...
        assert!(
            (actual.unwrap() - expected).abs() < 3e-5,
...
fn native_summary_matches_full_prompt_and_decode_in_managed_and_controlled_sessions() {
    if let Ok(mode) = std::env::var(SUMMARY_MODE) {
        println!("\n{SUMMARY_RESULT}{}", run(&mode));
        return;
    }
    let mut expected = None;
    for mode in ["ordinary", "managed", "controlled"] {
        let result = std::process::Command::new(std::env::current_exe().unwrap())
            .args(["--exact", SUMMARY_CASE, "--ignored", "--nocapture"])
            .env(SUMMARY_MODE, mode)
            .output()
            .unwrap();
        ...
        let actual: serde_json::Value = serde_json::from_str(
            stdout
                .lines()
                .find_map(|line| line.strip_prefix(SUMMARY_RESULT))
