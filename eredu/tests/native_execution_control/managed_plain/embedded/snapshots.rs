...
use super::*;
use eredu_core::{execution_control::{ExecutionControlError,SnapshotLimits},speculative::SpeculativeControlError};
...
fn run_inner(kind: &str, capacity: u64, expect_refusal: bool) {
    let target=managed_fixture(super::super::super::speculative::captured_prefill::source(kind));
    ...
    let mut loaded=LoadedModel::load_execution_plan(&MlxBackendFactory::default(),&target.0,&execution).unwrap();
    let options=loaded.speculative_generation_options().unwrap().unwrap();
    let (model,drafting)=loaded.parts_mut();
    let source=model.compile_managed_plain_text_source(std::fs::File::open(target.0.join("tokenizer.json")).unwrap())
    ...
    let request=ManagedPlainTextSpeculativeRequest{
        ...
        drafting:drafting.as_speculative_draft().unwrap(),options,cancellation:Default::default(),
        on_event:move |event|if let SemanticEvent::TextDelta(text)=event{events.borrow_mut().push_str(text.as_str());},
    };
    let control=ControlledSpeculativeOptions{
        snapshots:Some(SnapshotLimits{max_snapshots:1,max_branches:1,retained_bytes:128<<20,cumulative_copy_bytes:1<<30}),
        ..Default::default()
    };
    macro_rules! at_stage {
        ($label:literal,$operation:expr)=>{{
            let result=$operation;
            if result.is_err(){eprintln!("CONTROL_STATE_STAGE={}",$label);}
            result?
        }};
    }
    ...
    let output=model.with_controlled_managed_plain_text_speculative(&source,request,control,|session|{
        let first=at_stage!("first commitment",session.step()).expect("first target commitment");
        ...
        assert!(matches!(session.snapshot(),Err(SpeculativeControlError::Control(ExecutionControlError::Limit("snapshot count")))));
        ...
        assert_eq!(after_fork.cumulative_copy_bytes,after_save.cumulative_copy_bytes,
            "inactive fork shares immutable saved state; activation pays the copy");
        assert!(matches!(session.fork(&saved),Err(SpeculativeControlError::Control(ExecutionControlError::Limit("branch count")))));
        ...
        assert!(session.snapshot_usage().cumulative_copy_bytes>after_fork.cumulative_copy_bytes);
        *visible.borrow_mut()=prefix_text.clone();
        ...
        assert_eq!(*visible.borrow(),expected_text,"decoder and RNG restore with the exact caches");
        ...
        *visible.borrow_mut()=prefix_text;
        ...
        assert_eq!(session.token_ids(),expected);
        assert_eq!(*visible.borrow(),expected_text,"fork isolates target and prediction mutation");
        let spent=session.snapshot_usage().cumulative_copy_bytes;
        ...
        assert_eq!(released.cumulative_copy_bytes,spent,"retirement cannot refund prior copies");
        Ok(())
    });
    if expect_refusal {
        ...
        assert!(reached_exchanged_continuation, "refusal must exercise the final exchanged continuation");
        assert!(budget_failure(&error), "the original neutral budget refusal must survive error custody: {error}");
        return;
    }
