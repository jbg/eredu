...
use eredu_core::{
    ...
    intervention::{InterventionEvidence, InterventionOutcome, InterventionPlan, INTERVENTION_SCHEMA_VERSION},
    speculative::{SpeculativeControlError, SpeculativeActivationDiscovery},
...
fn capture_plan(discovery: &SpeculativeActivationDiscovery) -> SpeculativeActivationPlan {
    let usage = CaptureUsage { captures: 512, retained_bytes: 32 << 20, host_bytes: 32 << 20, encoded_bytes: 32 << 20 };
    SpeculativeActivationPlan {
        schema_version: SPECULATIVE_ACTIVATION_SCHEMA_VERSION,
        bounds: CaptureInvocationBounds { batch: 1, max_sequence: 5, max_context: None, max_predictions: 4 },
        captures: CapturePlan { schema_version: CAPTURE_SCHEMA_VERSION, selections: super::selections(discovery),
            limits: CaptureLimits { per_step: usage, cumulative: CaptureUsage { captures: CAPTURES, ..usage },
                physical_native_bytes: None, on_limit: CaptureLimitPolicy::Skip } },
        interventions: InterventionPlan { schema_version: INTERVENTION_SCHEMA_VERSION, operations: vec![] },
    }
}

fn drain(session: &mut dyn ControlledSpeculativeSession) -> Result<Vec<SpeculativeActivationCapture>, SpeculativeControlError> {
    let mut committed = session.token_ids().to_vec();
    let mut records = Vec::new();
    while let Some(step) = session.step()? {
        super::append(step, &mut records, &mut committed);
        assert_eq!(committed, session.token_ids(), "tentative activations never enter committed output");
    }
    Ok(records)
}

fn trace(records: &[SpeculativeActivationCapture], shared: bool, edited: bool, prefix: bool) -> serde_json::Value {
    ...
    let rows: Vec<_> = records.iter().map(|envelope| {
        ...
        if shared {
            let value = envelope.captures.shared().expect("original frame retains paid shared custody");
    ...
    if prefix { assert_eq!(captured, CAPTURES); }
    else { assert_eq!(captured, 0); assert!(skipped > 0); }
    if edited { assert!(applied > 0); }
    rows.into()
}

fn run(mode: &str) -> serde_json::Value {
    assert!(matches!(mode, "ordinary" | "controlled"));
    let target = managed_fixture(super::super::super::super::speculative::captured_prefill::source("qwen"));
    let execution = ExecutionPlan::fully_resident(eredu_core::DevicePlan::new("mlx", "metal:0").unwrap())
        ...
        .with_drafting(DraftingPlan::Embedded { max_draft_tokens: 2, lookahead: false, adaptive_lookahead: false });
    let mut loaded = LoadedModel::load_execution_plan(&MlxBackendFactory::default(), &target.0, &execution).unwrap_or_else(report_failure);
    ...
    let initial = model.prepare_speculative_activations(raw.clone()).unwrap_or_else(report_failure);
    ...
    let mut operations = super::interventions::plan(&discovery, &raw.captures.selections).operations;
    let mut operation = operations.remove(1);
    operation.id = EDIT.into();
    operation.schedule = CaptureSchedule { prefill: false, ..Default::default() };
    operation.slices.clear();
    operation.evidence = InterventionEvidence::None;
    replacement.interventions.operations.push(operation);
    let edited = model.prepare_speculative_activations(replacement).unwrap_or_else(report_failure);
    let mut changed = raw;
    changed.captures.limits.cumulative.captures += 1;
    let foreign_capture = model.prepare_speculative_activations(changed).unwrap_or_else(report_failure);
    drop(discovery);
    let control = ControlledSpeculativeOptions { activations: Some(initial.clone()),
        snapshots: Some(SnapshotLimits { max_snapshots: 2, max_branches: 1, retained_bytes: 256 << 20, cumulative_copy_bytes: 2 << 30 }),
        ..Default::default() };
    let visible = Rc::new(RefCell::new(String::new()));
    let sink = visible.clone();
    let on_event = move |event| if let SemanticEvent::TextDelta(text) = event { sink.borrow_mut().push_str(text.as_str()); };
    let mut prefix = Vec::new();
    let (mut baseline, mut edited_tail, mut restored, mut forked) = (Vec::new(), Vec::new(), Vec::new(), Vec::new());
    ...
    let drive = |session: &mut dyn ControlledSpeculativeSession| {
        let first = session.step()?.expect("first commitment after complete prefill");
        ...
        assert_eq!(prefix.last().unwrap().captures.as_step().cumulative_usage.captures, CAPTURES);
        ...
        *visible.borrow_mut() = prefix_text.clone();
        ...
        assert!(session.readmit_activation_interventions(foreign_capture).is_err(), "C cannot be replaced or replenished");
        ...
        *visible.borrow_mut() = prefix_text.clone();
        ...
        assert_eq!(session.branch_info(&child)?.token_ids.as_ref(), baseline_ids);
        *visible.borrow_mut() = prefix_text;
        ...
        assert_eq!((session.snapshot_usage().snapshots, session.snapshot_usage().branches), (0, 0));
        Ok(())
    };
    let output = if mode == "ordinary" {
        let chat = model.prepare_chat(ChatTemplateRequest { messages: vec![serde_json::json!({"role":"user","content":PROMPT})],
            add_generation_prompt: false, tool_choice: ToolChoice::None, ..Default::default() }).unwrap();
        ...
        model.with_controlled_text_speculative(PreparedChatSpeculativeGenerationRequest {
            input: PreparedChatInput::token_ids(&chat, ids), drafting: drafting.as_speculative_draft().unwrap(),
            settings, options, caller_stop_sequences: &[], cancellation: Default::default(), on_event,
        }, control, drive).unwrap_or_else(report_failure)
    } else {
        let source = model.compile_managed_plain_text_source(std::fs::File::open(target.0.join("tokenizer.json")).unwrap()).unwrap_or_else(report_failure);
        model.with_controlled_managed_plain_text_speculative(&source, ManagedPlainTextSpeculativeRequest {
            text: ManagedPlainTextRequest::new(PROMPT, settings(0.0)), drafting: drafting.as_speculative_draft().unwrap(),
            options, cancellation: Default::default(), on_event,
        }, control, drive).unwrap_or_else(report_failure)
    };
    assert_eq!(output.token_ids(), edited_ids);
    let all: Vec<_> = prefix.iter().chain(&baseline).chain(&edited_tail).chain(&restored).chain(&forked).collect();
    assert!(all.windows(2).all(|pair| pair[0].invocation < pair[1].invocation), "snapshots never recycle physical capture IDs");
    let addresses: Vec<_> = all.iter().map(|record| record.captures.as_step().records.as_ptr()).collect();
    drop(loaded);
    for (record, address) in all.iter().zip(addresses) { assert_eq!(record.captures.as_step().records.as_ptr(), address); }
    ...
    super::compare(&trace(&restored, shared, false, false), &baseline, "restored original authority/state");
    super::compare(&trace(&forked, shared, true, false), &edited, "forked edited authority/state");
