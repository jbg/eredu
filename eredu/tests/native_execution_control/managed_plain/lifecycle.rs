...
fn chat_fixture(case: &str, mode_variable: &str, template: &str, rendered: &str) {
    ...
    if mode == "ordinary" {
        let prepared = model.prepare_chat(chat).unwrap();
        assert_eq!(prepared.rendered_prompt(), rendered);
        let prompt = model.encode(prepared.rendered_prompt(), false).unwrap();
    ...
    let error = match model.start_managed_chat(&source, short, &cancellation) {
        Err(error) => error,
        Ok(_) => panic!("one byte cannot admit rendering and generation"),
    };
    ...
    println!(
        "{MARKER}{}",
        serde_json::json!({
            "ids": output.token_ids.as_ref(), "text": output.text.as_str()
        })
    );
}
