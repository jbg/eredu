...
fn mul3(x: &[BigDigit], y: &[BigDigit]) -> BigUint {
    mul3_with_allocations(x, y, Allocator(&Unenforced)).expect("ordinary integer allocation")
}
fn mul3_with_allocations(
    x: &[BigDigit],
    y: &[BigDigit],
    ...
    if x.len().min(y.len()) > 32 {
        allocations.qualify("recursive integer multiplication")?;
    }
    let len = x
        .len()
        .checked_add(y.len())
    ...
    prod.data.normalize_with_allocations(allocations)?;
    Ok(prod)
}

fn scalar_mul(a: &mut BigUint, b: BigDigit) {
    scalar_mul_with_allocations(a, b, Allocator(&Unenforced)).expect("ordinary integer allocation")
}
fn scalar_mul_with_allocations(
    a: &mut BigUint,
    b: BigDigit,
    allocations: Allocator<'_>,
    ...
    match b {
        ...
        _ => {
            if b.is_power_of_two() {
                *a = super::shift::biguint_shl_with_allocations(
                    Cow::Owned(core::mem::replace(a, BigUint::ZERO)),
                    b.trailing_zeros(),
                    allocations,
                )?;
            } else {
                ...
                if carry != 0 {
                    a.data
                        .push_with_allocations(carry as BigDigit, allocations)?;
                }
            }
        }
    }
...
pub(super) fn multiply_with_allocations(
    left: Cow<'_, BigUint>,
    right: Cow<'_, BigUint>,
    allocations: Allocator<'_>,
) -> Result<BigUint, AllocationError> {
    ...
    match (&*left.data, &*right.data) {
        (&[], _) | (_, &[]) => Ok(BigUint::ZERO),
        (_, &[digit]) => {
            let mut value = super::allocated::into_owned(left, allocations)?;
            scalar_mul_with_allocations(&mut value, digit, allocations)?;
            Ok(value)
        }
        (&[digit], _) => {
            let mut value = super::allocated::into_owned(right, allocations)?;
            scalar_mul_with_allocations(&mut value, digit, allocations)?;
            Ok(value)
        }
        (x, y) => mul3_with_allocations(x, y, allocations),
    }
}
    ...
    ($(impl Mul<$Other:ty> for $Self:ty;)*) => {$(
        impl Mul<$Other> for $Self {
            ...
            fn mul(self, other: $Other) -> BigUint {
                multiply_with_allocations(self.into_cow(),other.into_cow(),Allocator(&Unenforced)).expect("ordinary integer allocation")
            }
        }
    )*}
}
