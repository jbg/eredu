...
fn from_radix_digits_be(v: &[u8], radix: u32) -> BigUint {
    from_radix_digits_be_with_allocations(v, radix, Allocator(&Unenforced))
        .expect("ordinary integer allocation")
}
fn from_radix_digits_be_with_allocations(
    v: &[u8],
    radix: u32,
    ...
    allocations.controls::<(
        ...
        usize,
        usize,
        crate::big_digit::DoubleBigDigit,
        Result<BigUint, AllocationError>,
    )>()?;
    ...
    let mut data = allocations.vector(big_digits.to_usize().unwrap_or(0))?;
    ...
    for chunk in tail.chunks(power) {
        if data.last() != Some(&0) {
            ...
            allocations.grow(&mut data, required)?;
            data.push(0);
        }
    ...
    let mut result = BigUint {
        data: BigDigits::from_vec(data),
    };
    result.data.normalize_with_allocations(allocations)?;
    Ok(result)
}
...
impl Num for BigUint {
    ...
    fn from_str_radix(s: &str, radix: u32) -> Result<BigUint, ParseBigIntError> {
        Self::from_str_radix_with_allocations(s, radix, &Unenforced)
            .expect("ordinary integer allocation")
    }
}
...
impl FromPrimitive for BigUint {
    ...
    fn from_f64(n: f64) -> Option<BigUint> {
        Self::from_f64_with_allocations(n, &Unenforced).expect("ordinary integer allocation")
    }
}
...
impl BigUint {
    pub fn from_str_radix_with_allocations(
        s: &str,
        radix: u32,
        source: &dyn crate::allocation::Allocation,
    ) -> Result<Result<BigUint, ParseBigIntError>, AllocationError> {
        ...
        if s.is_empty() {
            return Ok(Err(ParseBigIntError::empty()));
        }

        if s.starts_with('_') {
            // Must lead with a real digit!
            return Ok(Err(ParseBigIntError::invalid()));
        }

        // First normalize all characters to plain digit values
        let mut v = allocations.vector(s.len())?;
        for b in s.bytes() {
            let d = match b {
                b'0'..=b'9' => b - b'0',
                b'a'..=b'z' => b - b'a' + 10,
        ...
        let res = if radix.is_power_of_two() {
            // Powers of two can use bitwise masks and shifting instead of multiplication
            allocations.qualify("bitwise integer text source")?;
            let bits = ilog2(radix);
            v.reverse();
            if big_digit::BITS % bits == 0 {
                from_bitwise_digits_le(&v, bits)
            ...
            from_radix_digits_be_with_allocations(&v, radix, allocations)?
        };
        Ok(Ok(res))
    }
}
...
impl BigUint {
    pub fn from_f64_with_allocations(
        mut n: f64,
        source: &dyn crate::allocation::Allocation,
    ) -> Result<Option<BigUint>, AllocationError> {
        Allocator(source).controls::<(
            f64,
            u64,
            i16,
            i8,
        ...
        let mut ret = Self::from_u64_with_allocations(mantissa, source)?;
        match exponent.cmp(&0) {
            Greater => ret = ret.shl_with_allocations(exponent as usize, source)?,
            Equal => {}
            Less => ret = ret.shr_with_allocations((-exponent) as usize, source)?,
        }
        Ok(Some(ret))
    }
    /// Construct the same unsigned scalar representation with prospective storage.
    pub fn from_u64_with_allocations(
        ...
        cfg_digit_expr!(
            {
                if value > u64::from(u32::MAX) {
                    return Ok(Self {
                        data: BigDigits::from_slice_with_allocations(
        ...
        Ok(Self {
            data: BigDigits::from_digit(value as BigDigit),
        })
    }
}
