...
// The original portable carry worker is used on all targets.
#[inline]
...
impl SubAssign<&BigUint> for BigUint {
    fn sub_assign(&mut self, other: &BigUint) {
        self.sub_assign_with_allocations(other, &crate::allocation::Unenforced)
...
impl Sub<BigUint> for &BigUint {
    ...
    fn sub(self, mut other: BigUint) -> BigUint {
        ...
        if other_len < self.data.len() {
            let (lo, hi) = self.data.split_at(other_len);
            let lo_borrow = __sub2rev(lo, &mut other.data);
            other.data.extend_from_slice(hi);
            if lo_borrow != 0 {
