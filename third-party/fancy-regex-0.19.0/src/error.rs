...
#[non_exhaustive]
pub enum Error {
    /// The prospective storage policy or host allocator refused a destination.
...
pub enum CompileError {
    ...
    /// Immutable capture-free delegate construction failed.
    WorkspaceDfaBuildError(regex_automata::dfa::dense::BuildError),
...
pub enum RuntimeError {
    /// The selected delegated engine could not complete its search. Allocation
    /// refusal is reported separately through `Error::Allocation`.
    DelegateError(regex_automata::MatchError),
...
impl fmt::Display for CompileError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            CompileError::WorkspaceDfaBuildError(e) => write!(f, "DFA error: {}", e),
...
impl fmt::Display for RuntimeError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            RuntimeError::DelegateError(error) => write!(f, "Delegated regex engine failed: {}", error),
