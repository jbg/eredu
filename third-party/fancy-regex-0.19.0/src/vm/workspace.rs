...
use crate::compile;
use crate::{Error, Match, RegexBuilder, RegexOptions, RuntimeError};
...
pub enum PlanError {
    ...
    Instruction,
    /// This string workspace requires UTF-8 match boundaries.
    InputEncoding,
...
impl fmt::Display for PlanError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InputEncoding => f.write_str("string workspace requires Unicode input mode"),
        ...
        match self {
            Self::Compile(error) => {
                if let Error::CompileError(cause) = error {
                    match cause.as_ref() {
                        crate::CompileError::PikeVmBuildError(cause) => return Some(cause),
                        crate::CompileError::WorkspaceDfaBuildError(cause) => return Some(cause),
                        _ => {}
                    }
                }
                Some(error)
...
impl Source {
    ...
    pub(crate) fn new_options(pattern: String, options: RegexOptions) -> Result<Self, SourceError> {
        if options.bytes_mode != crate::BytesMode::Unicode {
            return Err(PlanError::InputEncoding.into());
        }
        let parsed = crate::source::ParsedSource::new(&pattern, &options)?;
        let info = parsed.analyze()?;
        let compile_options = parsed.compile_options(&options);
        let body = if !info.hard {
            let utf8 = matches!(compile_options.bytes_mode, crate::BytesMode::Unicode);
            let mut context = crate::to_hir::HirCtx::new(compile_options.unicode, utf8);
            let inner = match crate::to_hir::expr_to_hir(&parsed.tree.expr, &mut context) {
                Some(hir) => compile::compile_pikevm_from_hir(&hir, &compile_options, crate::allocation::Context::unenforced())?,
                None => {
                    let mut cooked = String::new();
                    parsed.tree.expr.to_str(&mut cooked, 0);
                    compile::compile_pikevm(&cooked, &compile_options, crate::allocation::Context::unenforced())?
                }
            };
            Body::Wrap {
                inner,
                explicit_group_zero: parsed.context.explicit_capture_group_0,
            }
        } else {
            Body::Fancy(compile::compile_workspace(&info, compile_options)?)
        };
        let source = Self {
            body,
            options,
...
impl Requirements {
    ...
    fn checked(
        ...
        let controls = [
            ...
            size_of::<crate::RegexInput<'static, str>>(),
            size_of::<Result<Option<regex_automata::HalfMatch>, regex_automata::MatchError>>(),
        ]
        .iter()
        .try_fold(0, |sum, &n| add(sum, n))?;
    ...
    for (pc, insn) in prog.body.iter().enumerate() {
        match insn {
            ...
            Insn::DfaDelegate(inner) => {
                ...
                if inner.start_kind() != StartKind::Anchored || inner.pattern_len() != 1 {
                    return Err(PlanError::Geometry);
                }
            }
            ...
            Insn::Split(x, y) | Insn::SplitUnanchored(x, y) => {
                target(*x)?;
                target(*y)?;
            }
            Insn::Jmp(n) => target(*n)?,
            ...
            Insn::SaveCaptureGroupStart(group) => {
                let start = group.checked_mul(2).ok_or(PlanError::CapacityOverflow)?;
                slot(start)?;
                slot(add(start, 1)?)?;
            }
    ...
    pub fn fail_reservation(mut self, failure: PrepareFailure) -> Result<Self, PlanError> {
        match failure {
            PrepareFailure::Outer(buffer) => {
                if self.requirements.capacity(buffer) == 0 {
                    return Err(PlanError::Geometry);
                }
...
impl<'s> Workspace<'s> {
    ...
    pub fn find_iter<'w, 't>(&'w mut self, text: &'t str) -> Matches<'w, 's, 't> {
        Matches {
            workspace: self,
            input: crate::RegexInput::new(text),
    ...
    fn search<'t>(
        ...
        match &self.source.body {
            ...
            Body::Fancy(prog) => {
                if super::run_inner(
                    prog,
                    &crate::RegexInput::new(text).from_pos(pos),
                    flags,
                    &self.source.options.hard_regex_runtime_options,
                    &mut self.storage.state,
                    &mut self.storage.slots,
                    &mut self.storage.delegates,
                    &mut super::Caches::Fixed,
                    crate::allocation::Context::fixed(),
                    |_| Ok(()),
                )?
                .is_some()
                {
...
impl<'t> Iterator for Matches<'_, '_, 't> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        crate::next_match(
            ...
            |input, flags| {
                workspace
                    .search(input.haystack(), input.effective_start(), flags)
                    .map(|found| found.map(|found| (found, (found.start(), found.end()))))
            },
        )
    }
}
