...
#[test]
fn checked_profile_rejects_instruction_families_available_to_general_search() {
    for pattern in [
        r"(a+)\1",
        r"(?>a+)b",
        r"a++b",
...
fn each_nested_real_partial_frontier_keeps_completed_siblings_and_outer_storage() {
    let source = Source::new(r"(?:a(b)|c(d))+(?!e)|(?:f(g)|h(i))+|a+").unwrap();
    let requirements = source.plan().unwrap().requirements();
...
fn five_released_pattern_programs_match_current_engine_ranges() {
    ...
    for pattern in RELEASED_PATTERNS {
        ...
        assert_eq!(source.plan().unwrap().requirements().delegates, 0);
        let ordinary = crate::Regex::new(pattern).unwrap();
        let mut workspace = source.plan().unwrap().prepare().unwrap();
        let identity = storage_identity(&workspace);
        for text in [
...
#[test]
fn string_workspace_does_not_adopt_a_byte_mode_program() {
    for mode in [crate::BytesMode::Ascii, crate::BytesMode::UnicodeBytes] {
        let mut builder = RegexBuilder::new(".");
        builder.bytes_mode(mode);
        assert!(builder.build().is_ok());
        assert!(matches!(
            builder.build_workspace_source(),
            Err(SourceError::Profile(PlanError::InputEncoding))
        ));
    }
}
