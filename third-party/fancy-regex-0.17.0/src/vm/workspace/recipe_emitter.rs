...
// Exact selected sources from the pinned ten-tokenizer inventory. These are
...
pub(crate) fn record(pattern: &str, options: &RegexOptions, inner: &PikeVM) {
    COMPILED.with(|slot| {
        let mut slot = slot.borrow_mut();
        if let Some(records) = slot.as_mut() {
            assert_defaults(options);
...
fn assert_defaults(options: &RegexOptions) {
    ...
    assert_eq!(options.backtrack_limit, defaults.backtrack_limit);
...
fn instruction(out: &mut String, insn: &Insn, records: &[Compiled], ordinal: &mut usize) {
    match insn {
        ...
        Insn::PikeDelegate(delegate) => {
            assert_eq!(delegate.ordinal, *ordinal);
            assert_eq!(
                records[*ordinal].states,
                delegate.inner.get_nfa().states().as_ptr()
            );
            write!(
                out,
                "InsnRecipe::Pike {{ pattern: {:?}, ordinal: {}, capture_groups: ",
                records[*ordinal].pattern, *ordinal
            )
            .unwrap();
            match delegate.capture_groups {
                Some(range) => write!(out, "Some(({}, {}))", range.start(), range.end()).unwrap(),
...
fn emit_fixed_source_recipes() {
    ...
    for (source_index, pattern) in PATTERNS.iter().enumerate() {
        ...
        let source = Source::new(pattern).expect("actual fixed source compilation");
        let records = recording.finish();
        assert_defaults(&source.options);
        assert_eq!(source.options.pattern, *pattern);
        assert_eq!(source.delegates().count(), records.len());
        if source_index != 0 {
            inventory.push_str(",\n");
        }
        write!(inventory, "{{\"source\":{},\"delegates\":[", source_index).unwrap();
        for (index, (inner, record)) in source.delegates().zip(records.iter()).enumerate() {
        ...
        writeln!(
            sources,
            "SourceRecipe {{ pattern: {:?}, syntax: {}, backtrack_limit: {}, body:",
            pattern,
            syntax(source.options.syntaxc),
            source.options.backtrack_limit
        )
        .unwrap();
        match &source.body {
            Body::Wrap {
                ...
                writeln!(
                    sources,
                    "BodyRecipe::Wrap {{ pattern: {:?}, explicit_group_zero: {} }} }},",
                    records[0].pattern, explicit_group_zero
                )
                .unwrap();
                inventory.push_str("],\"instructions\":0,\"saves\":0,\"literal_bytes\":0}");
            }
            Body::Fancy(prog) => {
                writeln!(
                    sources,
                    "BodyRecipe::Fancy {{ saves: {}, instructions: &[",
                    prog.n_saves
                )
                .unwrap();
                let mut ordinal = 0;
