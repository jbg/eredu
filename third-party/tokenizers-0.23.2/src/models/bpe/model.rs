...
impl BPE {
    ...
    pub(crate) fn configuration_comparison_control_bytes() -> Option<usize> {
        ...
        [
            size_of::<[&Self; 2]>(),
            size_of::<[&Storage; 2]>(),
            size_of::<super::storage::VocabIter<'_>>(),
            size_of::<super::storage::MergeIter<'_>>(),
            size_of::<(&str, &u32)>(),
            size_of::<(&Pair, &(u32, u32))>(),
            size_of::<[usize; 2]>(),
            size_of::<bool>(),
        ]
        .iter()
    ...
    /// Cache presence, identity and occupancy are execution details, not tokenizer policy.
...
impl BPE {
    /// Borrows scalar vocabulary lookups without exposing a mutable model,
    /// copying a vocabulary, or constructing a full tokenizer.
    pub fn vocabulary(&self) -> super::BpeVocabulary<'_> {
        super::BpeVocabulary::new(&self.storage)
    }
    ...
    fn merge_word(&self, w: &str) -> Result<Word> {
        ...
        self.fill_word(w, &mut word).map_err(|_| {
            Error::UnkTokenOutOfVocabulary(self.unk_token.as_ref().expect("missing unk").to_owned())
        })?;
    ...
    pub(super) fn fill_word(
        ...
        w: &str,
        word: &mut Word,
    ) -> std::result::Result<(), super::word::MissingUnknown> {
                        ...
                        (Some((unk_id, unk_len)), false) => {
                            ...
                            Some((
                                *self
                                    .storage
                                    .id(unk_token)
                                    .ok_or(super::word::MissingUnknown)?,
                                byte_len,
                            ))
                        }
                        _ => Some((
