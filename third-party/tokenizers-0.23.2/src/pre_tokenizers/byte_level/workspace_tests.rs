...
use crate::{
    ...
    EncodeIdsPlan, OffsetReferential, OffsetType, Tokenizer, TokenizerCompilePlan,
};
...
#[test]
fn complete_implicit_profile_matches_ordinary_ids_spellings_offsets_and_numeric_boundaries() {
    let value = source();
...
fn closed_byte_source_preserves_two_stage_serde_clone_and_ordinary_offset_worker() {
    ...
    // Ordinary byte-level parsing remains lazy until a normal encoding operation.
    let compiled = ByteLevel::new(false, true, true).with_compiled_regex(
        crate::utils::SysRegex::compile(
            fancy_regex::workspace::construction::Plan::new(DEFAULT_PATTERN).unwrap(),
        ).unwrap(),
    );
    let compiled = Sequence::new(vec![
        PreTokenizerWrapper::Digits(Digits::new(true)),
        PreTokenizerWrapper::ByteLevel(compiled),
    ]);
...
fn implicit_first_literal_and_each_real_workspace_frontier_keep_owned_causes() {
    use fancy_regex::workspace::{
        construction::ConstructionFailure as C,
    ...
    for target in targets {
        ...
        let crate::EncodeIdsError::RegexPreparation(retired) = error.cause() else {
...
fn accepted_source_never_initializes_ordinary_regex_in_a_fresh_process() {
    ...
    const TEST:&str="pre_tokenizers::byte_level::workspace_tests::accepted_source_never_initializes_ordinary_regex_in_a_fresh_process";
    ...
    use super::regex_initializations_for_test as count;
    assert_eq!(count(), 0);
    let value = source();
    let actual = packed(&value);
    for _ in 0..3 {
