...
/// Fixed development-only regex constructor and workspace failure selectors.
#[cfg(all(
    feature = "fancy-regex",
...
pub use fancy_regex::workspace::{
    ...
    PrepareFailure as TokenizerRegexWorkspaceFailure,
};
mod cache_policy;
mod encode_ids;
mod encoding;
pub use encode_ids::{
    ...
    EncodeIdsRequirements, NormalizationBuffer,
};
pub mod normalizer;
pub mod pattern;
...
pub use crate::utils::padding::{
    pad_encodings, PaddingDirection, PaddingParams, PaddingStrategy,
};
pub use crate::utils::truncation::{
    truncate_encodings, TruncationDirection, TruncationParams,
    TruncationStrategy,
};
pub use added_vocabulary::*;
pub use encoding::*;
pub use normalizer::{
    NormalizedString, OffsetReferential, SplitDelimiterBehavior,
};
...
pub trait Model {
    ...
    fn tokenize_in_pretokenized(
        ...
        match truncation {
            ...
            None => pretokenized
                .tokenize(|normalized| self.tokenize(normalized.get())),
        }
    }
}
...
pub trait PostProcessor {
    ...
    fn process(
        ...
        let encodings =
            self.process_encodings(encodings, add_special_tokens)?;
...
/// forward-vocabulary IDs followed by added reverse-map IDs, in unspecified order,
...
impl Tokenizer {
    ...
    pub fn from_pretrained<S: AsRef<str>>(
        identifier: S,
        params: Option<
            crate::utils::from_pretrained::FromPretrainedParameters,
        >,
    ) -> Result<Self> {
        let tokenizer_file = crate::utils::from_pretrained::from_pretrained(
            identifier, params,
        )?;
    ...
    D: Decoder,
{
    ...
    pub fn with_truncation(
        ...
        if let Some(trunc_params) = &trunc {
            ...
            let effective_max_length =
                trunc_params.max_length - n_added_tokens;
    ...
    fn encode_single_sequence(
        ...
        let encode =
            |is_pre_tokenized, subseq_idx, subseq| -> Result<Encoding> {
                let normalized = self
                    .added_vocabulary
        ...
        E: Into<EncodeInput<'s>>,
    {
        ...
        let pair_encoding = pair
            .map(|sequence| {
                self.encode_single_sequence(sequence, 1, OffsetType::None)
            })
            .transpose()?;
        ...
        E: Into<EncodeInput<'s>>,
    {
        ...
        let pair_encoding = pair
            .map(|sequence| {
                self.encode_single_sequence(sequence, 1, OffsetType::Byte)
            })
            .transpose()?;
        ...
        E: Into<EncodeInput<'s>>,
    {
        ...
        let pair_encoding = pair
            .map(|sequence| {
                self.encode_single_sequence(sequence, 1, OffsetType::Char)
            })
            .transpose()?;
    ...
    fn do_tokenize<P: Into<PreTokenizedString>>(
        ...
        let truncation = match self.truncation.as_ref() {
            Some(TruncationParams {
                ...
                strategy,
                ..
            }) if *strategy != TruncationStrategy::OnlySecond
                || type_id != 0 =>
            {
    ...
    N: Normalizer,
{
    ...
    fn do_normalize<V: Into<NormalizedString>>(
        &self,
        normalized: V,
    ...
    pub fn post_process(
        ...
        let (encoding, pair_encoding) = {
            if let Some(trunc) = &self.truncation {
                let n_added_tokens =
                    self.get_n_added_tokens(pair_encoding.is_some());
        ...
        let final_encoding = if let Some(processor) = &self.post_processor {
            ...
            let mut encodings = <dyn PostProcessor>::default_process(
                encodings,
                add_special_tokens,
            )?;
            if encodings.len() != 1 {
    ...
    D: Decoder + Send + Sync,
{
        ...
        ResultShunt::process(
            files.into_iter().flat_map(|filename| {
                match File::open(filename) {
                    ...
                    Err(e) => {
                        itertools::Either::Right(std::iter::once(Err(e)))
                    }
                }
            }),
            |sequences| -> Result<()> {
                ...
                trainer.feed(
                    ...
                    |seq| {
                        let normalized =
                            self.added_vocabulary.extract_and_normalize(
                                self.normalizer.as_ref(),
                                seq.as_ref(),
                            );
                        let pre_tokenized =
        ...
        S: AsRef<str> + Send,
    {
        ...
        trainer.feed(
            ...
            |seq| {
                let normalized = self.added_vocabulary.extract_and_normalize(
                    self.normalizer.as_ref(),
                    seq.as_ref(),
                );
                let pre_tokenized = self.do_pre_tokenize(normalized)?;
