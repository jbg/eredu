    ...
    pub(crate) fn visit_matches<E>(
            ...
            self.raw_matches(sentence, normalized)
        {
            ...
            if start_offset < start {
                visit(None, (start_offset, start))?;
            }
            visit(Some(id), (start, stop))?;
            start_offset = stop;
        }

        let total_byte_len = sentence.len();
        if start_offset != total_byte_len {
            visit(None, (start_offset, total_byte_len))?;
        }
    ...
    pub fn extract_and_normalize<N: Normalizer>(
        ...
        pretokenized
            .split(|_, mut sequence| {
                ...
                Ok(self.split_with_indices(sequence, true))
            })
            .expect("AddedVocabulary bad split");
...
impl std::fmt::Debug for LegacyAddedVocabulary {
...
impl LegacyAddedVocabulary {
    ...
    pub(crate) fn decode_token_ref(&self, id: u32) -> Option<&str> {
        self.added_tokens_map_r.get(&id).map(|token| {
...
impl Default for LegacyAddedVocabulary {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn can_add_special_tokens() {
        ...
        assert!(vocab.get_vocab().contains_key("test"));
        assert!(vocab.get_added_tokens_decoder().contains_key(&0));
    ...
    fn empty_matches() {
        ...
        let matches = vocab.find_matches("", false);
    ...
    fn content_preserved_with_normalizer() {
        ...
        let hello_id = vocab.get_vocab()["Hello"];
        let cls_id = vocab.get_vocab()["[CLS]"];
