...
impl EmailAddress {
    ...
    /// Parse with a prospective reservation before each reached storage producer.
    /// Ordinary parsing uses this same worker with an unenforced callback.
    pub fn parse_with_options_and_reservation<E>(
        address: &str,
        options: Options,
        reserve: impl FnMut(std::alloc::Layout) -> Result<(), E>,
    ) -> Result<Self, ParseStorageError<E>> {
        parse_address_with_reservation(address, options, reserve)
    }
...
/// Failure from the shared parser or its reached storage producer.
#[derive(Debug)]
pub enum ParseStorageError<E> {
    /// The ordinary parser rejected the address.
    Syntax(Error),
    /// The caller refused a reservation before its allocation.
    Funding(E),
    /// The host allocator refused the one requested allocation.
    Allocation(std::collections::TryReserveError),
    /// The reached layout cannot be represented.
    Overflow,
    /// The allocated destination did not match the requested capacity.
    Capacity,
}
impl<E: std::fmt::Display> std::fmt::Display for ParseStorageError<E> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Syntax(error) => std::fmt::Display::fmt(error, f),
            Self::Funding(error) => error.fmt(f),
            Self::Allocation(error) => std::fmt::Display::fmt(error, f),
            Self::Overflow => f.write_str("email source allocation extent overflow"),
            Self::Capacity => f.write_str("email source allocation destination changed"),
        }
    }
}
impl<E: std::error::Error + 'static> std::error::Error for ParseStorageError<E> {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            Self::Syntax(error) => Some(error),
            Self::Funding(error) => Some(error),
            Self::Allocation(error) => Some(error),
            Self::Overflow | Self::Capacity => None,
        }
    }
}
impl<E> From<Error> for ParseStorageError<E> {
...
fn parse_address(address: &str, options: Options) -> Result<EmailAddress, Error> {
    match parse_address_with_reservation(address, options, |_| {
        Ok::<(), std::convert::Infallible>(())
    }) {
        Ok(value) => Ok(value),
        Err(ParseStorageError::Syntax(error)) => Err(error),
        Err(ParseStorageError::Funding(error)) => match error {},
        Err(error) => panic!("{}", error),
    }
}

fn parse_address_with_reservation<E>(
    address: &str,
    options: Options,
    mut reserve: impl FnMut(std::alloc::Layout) -> Result<(), E>,
) -> Result<EmailAddress, ParseStorageError<E>> {
    reserve(std::alloc::Layout::new::<(
        &str,
        Options,
        (&str, &str, &str),
        Result<EmailAddress, ParseStorageError<E>>,
    ...
    .map_err(ParseStorageError::Funding)?;
    // Keep the original display/local/domain parser and its diagnostic order.
    ...
    match (
        ...
        (false, _, false) => Err(Error::UnsupportedDisplayName.into()),
        (true, true, true) => Err(Error::MissingDisplayName.into()),
        (true, true, false) => Err(Error::InvalidCharacter.into()),
        _ => {
            parse_local_part(local_part, options)?;
            parse_domain(domain, options)?;
            let layout = std::alloc::Layout::array::<u8>(address.len())
                .map_err(|_| ParseStorageError::Overflow)?;
            reserve(layout).map_err(ParseStorageError::Funding)?;
            let mut owned = String::new();
            owned
                .try_reserve_exact(address.len())
                .map_err(ParseStorageError::Allocation)?;
            if owned.capacity() != address.len() {
                return Err(ParseStorageError::Capacity);
            }
            owned.push_str(address);
            Ok(EmailAddress(owned))
        }
    }
}
