...
fn pairs(workspace: &mut Workspace, range: Range<usize>) {
    ...
    let mut prior = reference::recompose::new_canonical(prefix.chars().chain(input.chars()));
    loop {
        let a = actual.next();
        let b = prior.next();
        assert_eq!(a, b, "{input:?}");
...
#[test]
fn prefix_and_selected_input_share_canonical_composition_and_reserved_storage() {
    let input = "\u{301}|\u{1161}\u{11a8}|a\u{315}\u{300}";
    for prefix in ["e", "\u{1100}", "x\u{315}"] {
        let mut workspace = Plan::new(input, prefix).unwrap().prepare().unwrap();
        let pointers_before = pointers(&workspace);
        ...
        for part in input.split('|') {
            ...
            let expected: String = prefix
                ...
                .map(|x| x.0)
                .collect();
            assert_eq!(workspace.normalize(range).unwrap(), expected);
            start += part.len() + 1;
        }
        assert_eq!(workspace.normalize(0..0).unwrap(), "");
        assert_eq!(workspace.capacities(), capacities);
        assert_eq!(pointers(&workspace), pointers_before);
    }
    let mut empty = Plan::new("", "e").unwrap().prepare().unwrap();
