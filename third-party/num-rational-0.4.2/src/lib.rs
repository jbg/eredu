...
impl<T: Clone + Integer> Ratio<T> {
    ...
    pub fn new(numer: T, denom: T) -> Ratio<T> {
        Self::new_with_operations(numer, denom, &operations::Ordinary)
    ...
    /// Divide through the original reduction sequence with explicit producers.
    pub fn div_with_operations<O: operations::IntegerOperations<T>>(
        self,
        rhs: Self,
        operations: &O,
    ) -> Result<Self, O::Error> {
        operations.reserve_controls(core::mem::size_of::<(
            Self,
            Self,
            &O,
            T,
        ...
        let gcd_ac = operations.gcd(&self.numer, &rhs.numer)?;
        let gcd_bd = operations.gcd(&self.denom, &rhs.denom)?;
        let left_n = operations.div(self.numer, operations.clone(&gcd_ac)?)?;
        let right_d = operations.div(rhs.denom, operations.clone(&gcd_bd)?)?;
        let numer = operations.mul(left_n, right_d)?;
        let left_d = operations.div(self.denom, gcd_bd)?;
        let right_n = operations.div(rhs.numer, gcd_ac)?;
        let denom = operations.mul(left_d, right_n)?;
        Self::new_with_operations(numer, denom, operations)
    }
    ...
    /// **Panics if `denom` is zero.**
    fn reduce(&mut self) {
        self.reduce_with_operations(&operations::Ordinary)
            .unwrap_or_else(|error| match error {})
    }
    /// Reduce using the same arithmetic and explicit fallible integer producers.
    pub fn new_with_operations<O: operations::IntegerOperations<T>>(
        numer: T,
        denom: T,
        operations: &O,
    ) -> Result<Self, O::Error> {
        operations.reserve_controls(core::mem::size_of::<(Self, &O, Result<Self, O::Error>)>())?;
        let mut value = Self::new_raw(numer, denom);
        value.reduce_with_operations(operations)?;
        Ok(value)
    }
    fn reduce_with_operations<O: operations::IntegerOperations<T>>(
        &mut self,
        operations: &O,
    ) -> Result<(), O::Error> {
        operations.reserve_controls(core::mem::size_of::<(
        ...
        if self.numer.is_zero() {
            self.denom = operations.one()?;
            return Ok(());
        }
        if self.numer == self.denom {
            self.numer = operations.one()?;
            self.denom = operations.one()?;
            return Ok(());
        }
        let g = operations.gcd(&self.numer, &self.denom)?;

        let numerator = core::mem::replace(&mut self.numer, operations.zero()?);
        self.numer = operations.div(numerator, operations.clone(&g)?)?;
        let denominator = core::mem::replace(&mut self.denom, operations.zero()?);
        self.denom = operations.div(denominator, g)?;

        // keep denom positive!
        if self.denom < T::zero() {
            let numerator = core::mem::replace(&mut self.numer, operations.zero()?);
            self.numer = operations.sub(operations.zero()?, numerator)?;
            let denominator = core::mem::replace(&mut self.denom, operations.zero()?);
            self.denom = operations.sub(operations.zero()?, denominator)?;
        }
        Ok(())
    }
    ...
    T: Clone + Integer,
{
    ...
    fn div(self, rhs: Ratio<T>) -> Ratio<T> {
        self.div_with_operations(rhs, &operations::Ordinary)
            .unwrap_or_else(|error| match error {})
    }
}
