...
impl<T, A: Allocator> RawTable<T, A> {
    ...
    /// Fixed locals/transports of the actual read-only get/find/probe worker.
    /// Hashing and equality bodies remain separate caller obligations.
    pub(crate) fn lookup_control_bytes<E>(&self, equality: &E) -> Option<usize> {
        use core::mem::{size_of, size_of_val};
        let parts = [
            size_of::<(&Self, u64, E)>(), // get
            size_of::<(&Self, u64, E)>(), // find
            size_of_val(equality),
            size_of::<(&Self, &mut E)>(), // find_inner index closure
            size_of::<(&RawTableInner, u64, &mut dyn FnMut(usize) -> bool)>(),
            size_of::<Tag>(), size_of::<ProbeSeq>(), size_of::<Group>(),
            size_of::<crate::control::BitMask>(), size_of::<BitMaskIter>(),
            size_of::<(usize, usize)>(), size_of::<Option<usize>>(),
            size_of::<Bucket<T>>(), size_of::<Option<Bucket<T>>>(),
            size_of::<Option<&T>>(), size_of::<(&Self, usize)>(),
            size_of::<NonNull<T>>(), size_of::<(*const Tag, usize)>(),
            size_of::<(&T, bool)>(),
        ];
        parts.into_iter().try_fold(size_of_val(&parts), usize::checked_add)
    }

    /// Controls of insertion only when reserve has established a free slot.
    /// The query neither probes nor mutates; None prevents unpriced growth.
    pub(crate) fn reserved_insert_control_bytes<H>(&self, hasher: &H) -> Option<usize> {
        use core::mem::{size_of, size_of_val};
        if self.table.growth_left == 0 { return None; }
        let parts = [
            size_of::<(&mut Self, u64, T, H)>(), size_of_val(hasher),
            size_of::<(&RawTableInner, u64)>(), size_of::<ProbeSeq>(), size_of::<Group>(),
            size_of::<crate::control::BitMask>(), size_of::<BitMaskIter>(),
            size_of::<(usize, usize)>(), size_of::<Tag>(), size_of::<Bucket<T>>(),
            size_of::<(&mut Self, u64, usize, T)>(),
            size_of::<(&mut RawTableInner, usize, Tag, Tag)>(),
            size_of::<(NonNull<T>, *mut T, *mut Tag)>(), size_of::<(u64,bool)>(),
        ];
        parts.into_iter().try_fold(size_of_val(&parts), usize::checked_add)
    }
...
impl RawTableInner {
    ...
    {
        // Avoid `Option::ok_or_else` because it bloats LLVM IR.
        let Some(resize_capacity) = self.reserve_rehash_capacity(additional) else {
            return Err(fallibility.capacity_overflow());
        };
        if resize_capacity.is_none() {
            ...
            unsafe {
                self.resize_inner(
                    alloc,
                    resize_capacity.expect("resize branch"),
                    hasher,
                    fallibility,
                    layout,
                )
