    ...
    A: Allocator,
{
    ...
    /// Named controls for the same read-only map lookup used by contains/get.
    /// Does not hash, compare, probe or allocate. Hash/equivalence bodies and
    /// borrowed backing remain separate caller obligations.
    pub fn lookup_control_bytes<Q>(&self, value: &Q) -> Option<usize>
    where Q: Hash + Equivalent<T> + ?Sized,
    {
        use core::mem::{size_of, size_of_val};
        let parts = [self.map.lookup_control_bytes(value)?, size_of::<(&Self, &Q)>(),
            size_of::<Option<&T>>(), size_of::<bool>()];
