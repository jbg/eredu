    ...
    A: Allocator,
{
    ...
    /// Fixed controls for the actual borrowed find/probe worker. This does not
    /// invoke equality, hash, probe, allocate or change any table entry.
    pub fn lookup_control_bytes<E>(&self, equality: &E) -> Option<usize> {
        self.raw.lookup_control_bytes(equality)?
            .checked_add(core::mem::size_of::<(&Self, u64, E, Option<&T>)>())
    }

    /// Controls for the ordinary unique insertion when current spare capacity
    /// proves it cannot allocate or invoke the rehash callback. None requires
    /// reserve first. Hashing and callback storage remain caller obligations.
    pub fn reserved_insert_control_bytes<H>(&self, hasher: &H) -> Option<usize> {
        self.raw.reserved_insert_control_bytes(hasher)?
            .checked_add(core::mem::size_of::<(&mut Self, u64, T, H, OccupiedEntry<'_,T,A>)>())
    }
    ...
    /// Returns the exact allocation request made by [`Self::try_reserve`] for
    /// this table's current state and `additional` entries, without reserving,
    /// allocating, hashing, or changing the table.
    ///
    /// `None` means no allocator request, including in-place rehash after
    /// deletions. `Some(layout)` describes the complete new table allocation,
    /// while the old allocation is still live, not their size difference.
    /// This excludes allocator bookkeeping or excess usable capacity returned
    /// by a custom allocator. Any mutation invalidates this prospective result.
    /// An unrepresentable request returns [`TryReserveError::CapacityOverflow`].
    #[inline]
    pub fn try_reserve_layout(&self, additional: usize) -> Result<Option<core::alloc::Layout>, TryReserveError> {
        self.raw.try_reserve_layout(additional)
    }
}
...
#[cfg(test)]
#[path = "table/reserve_layout_tests.rs"]
mod reserve_layout_tests;
