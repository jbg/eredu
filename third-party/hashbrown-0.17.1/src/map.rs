    ...
    A: Allocator,
{
    ...
    /// Exact named control population of the pinned read-only lookup worker.
    /// This neither hashes the key, compares keys, probes the table nor allocates.
    /// The selected hasher/equivalence body and borrowed key backing must be
    /// independently qualified; no storage or execution authority is granted.
    pub fn lookup_control_bytes<Q>(&self, key: &Q) -> Option<usize>
    where
        Q: Hash + Equivalent<K> + ?Sized,
    {
        use core::mem::{size_of, size_of_val};
        let equality = equivalent_key::<Q, K, V>(key);
        let parts = [self.table.lookup_control_bytes(&equality)?,
            size_of::<(&Self, &Q)>(), size_of::<(&S, &Q)>(),
            size_of::<S::Hasher>(), size_of::<u64>(),
            size_of_val(&equality), size_of::<Option<&(K, V)>>(),
            size_of::<Option<&V>>(), size_of::<Option<(&K, &V)>>(),
            size_of::<(&Q, &(K, V))>(), size_of::<bool>(),
        ];
