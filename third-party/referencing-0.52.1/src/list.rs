...
impl<T> List<T> {
    ...
    pub fn push_front(&self, value: Arc<T>) -> Self {
        self.push_front_with_allocations(value, &crate::allocation::Unenforced)
            .unwrap()
    }
    /// Prepends one node after admitting its complete shared allocation.
    pub fn push_front_with_allocations(
        &self,
        value: Arc<T>,
        allocation: &dyn crate::allocation::Allocation,
    ) -> Result<Self, crate::allocation::AllocationError> {
        Ok(List {
            head: Some(crate::allocation::Allocator(allocation).arc(Node {
                value,
