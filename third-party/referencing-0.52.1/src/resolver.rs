...
#[derive(Debug)]
struct RegistryClosed;
impl fmt::Display for RegistryClosed {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("Retrieving external resources is not supported once the registry is populated")
    }
}
...
impl fmt::Debug for Resolver<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        struct Scopes<'a>(&'a List<Uri<String>>);
        impl fmt::Debug for Scopes<'_> {
            fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
                // URI syntax excludes literal quotes, backslashes and controls, so this is
                // the previous quoted diagnostic, streamed without allocating its String.
                f.write_str("\"[")?;
        ...
        f.debug_struct("Resolver")
            ...
            .field("scopes", &Scopes(&self.scopes))
            .finish()
    }
}
...
impl<'r> Resolver<'r> {
    ...
    pub fn lookup(&self, reference: &str) -> Result<Resolved<'r>, Error> {
        ...
        let Some(retrieved) = self.registry.resource_by_uri(&uri) else {
            return Err(Error::unretrievable(
                self.registry.allocation().copy_str(uri.as_str())?,
                Box::new(RegistryClosed),
            ));
        };
    ...
    #[inline]
    fn evolve(&self, base_uri: Arc<Uri<String>>) -> Result<Resolver<'r>, Error> {
            ...
            && (self.scopes.is_empty() || base_uri != self.base_uri)
        {
            Ok(Resolver {
                registry: self.registry,
                base_uri,
                scopes: self.scopes.push_front_with_allocations(
                    self.base_uri.clone(),
                    self.registry.allocation().0,
                )?,
            })
        } else {
            Ok(Resolver {
    ...
    /// Finds vocabulary identifiers with the registry's retained allocation policy.
    pub fn try_find_vocabularies(
        ...
        if let Some(resource) = self.registry.resource_by_uri(self.base_uri.as_ref()) {
            return self
                .registry
                .try_find_vocabularies(draft, resource.contents());
        }
        self.registry.try_find_vocabularies(draft, contents)
    }
}
