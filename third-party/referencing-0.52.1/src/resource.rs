        ...
        for segment in pointer.split('/') {
            if let Some(array) = contents.as_array() {
                let idx = match segment.parse::<usize>() {
                    Ok(index) => index,
                    Err(error) => {
                        return Err(Error::invalid_array_index(
                            allocation.copy_str(original_pointer)?,
                            allocation.copy_str(segment)?,
                            error,
                        ))
                    }
                };
                ...
                segments.try_push(idx, allocation)?;
            } else {
                let segment = unescape_segment_with_allocations(segment, allocation.0)?;
                ...
                segments.try_push(segment, allocation)?;
            }
            let last = &resolver;
...
pub fn unescape_segment(segment: &str) -> Cow<'_, str> {
    unescape_segment_with_allocations(segment, &crate::allocation::Unenforced).unwrap()
}

/// Unescape a segment while admitting owned storage before allocation.
pub fn unescape_segment_with_allocations<'a>(
    mut segment: &'a str,
    allocation: &dyn crate::allocation::Allocation,
) -> Result<Cow<'a, str>, crate::allocation::AllocationError> {
    ...
    crate::allocation::Allocator(allocation).grow_string(&mut buffer, segment.len())?;
    loop {
