...
impl StateID {
    ...
    /// Inserts the existing DEAD and MISSING payloads through a caller's
    /// ordinary or checked intern-table worker, preserving their exact IDs.
    pub fn seed_hash_cons<E>(mut insert: impl FnMut(&[u32]) -> Result<u32, E>) -> Result<(), E> {
        let id = insert(&[])?;
        assert!(id == Self::DEAD.as_u32());
        let id = insert(&[ExprRef::INVALID.as_u32()])?;
        assert!(id == Self::MISSING.as_u32());
        Ok(())
    }

    pub fn new_hash_cons() -> VecHashCons {
        let mut rx_sets = VecHashCons::new();
        Self::seed_hash_cons::<std::convert::Infallible>(|words| Ok(rx_sets.insert(words)))
            .expect("ordinary sentinel insertion");
        rx_sets
    }
}
...
impl AlphabetInfo {
    pub(crate) fn from_mapping(mapping: [u8; 256], size: usize) -> Self {
        Self { mapping, size }
    }
