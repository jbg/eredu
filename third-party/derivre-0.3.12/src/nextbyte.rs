...
impl NextByteCache {
    ...
    /// Runs the same allocation-free next-byte analysis used on an ordinary
    /// cache miss. The caller owns source validation and memo destinations.
    pub fn analyze(exprs: &ExprSet, r: ExprRef) -> NextByte {
        match exprs.get(r) {
            Expr::Or(_, args) => {
