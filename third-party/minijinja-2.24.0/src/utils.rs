...
/// Named owner of the ordinary limited-whitespace split closure.
pub(crate) struct SplitWhitespaceN<'a>{
    value:&'a str,maxsplits:usize,splits:usize,skip_ws:bool,
    split_start:Option<usize>,last_split_end:usize,chars:std::str::CharIndices<'a>,
}
impl<'a> SplitWhitespaceN<'a>{
    pub(crate) fn new(value:&'a str,maxsplits:usize)->Self {
        Self{value,maxsplits,splits:1,skip_ws:true,split_start:None,last_split_end:0,chars:value.char_indices()}
    }
}
impl<'a> Iterator for SplitWhitespaceN<'a>{
    ...
    fn next(&mut self)->Option<Self::Item>{
        for (idx, c) in self.chars.by_ref() {
            if self.splits >= self.maxsplits && !self.skip_ws {
                continue;
            } else if c.is_whitespace() {
                if let Some(old) = self.split_start {
                    let rv = &self.value[old..idx];
                    self.split_start = None;
                    self.last_split_end = idx;
                    self.splits += 1;
                    self.skip_ws = true;
                    return Some(rv);
                }
                ...
                self.skip_ws = false;
                if self.split_start.is_none() {
                    self.split_start = Some(idx);
                    self.last_split_end = idx;
                }
            }
        }

        let rest = &self.value[self.last_split_end..];
        if !rest.is_empty() {
            self.last_split_end = self.value.len();
            Some(rest)
        } else {
            None
        }
...
pub fn splitn_whitespace(s:&str,maxsplits:usize)->impl Iterator<Item=&str>+'_ {
    SplitWhitespaceN::new(s,maxsplits)
}
...
{
    safe_sort_with(|| seq.sort_by(f))
}
pub(crate) fn safe_sort_with(sort:impl FnOnce())->Result<(),Error> {
    #[cfg(feature = "internal_safe_search")]
    {
        if let Err(panic) = std::panic::catch_unwind(std::panic::AssertUnwindSafe(sort)) {
            let msg = panic
                .downcast_ref::<&str>()
                .copied()
