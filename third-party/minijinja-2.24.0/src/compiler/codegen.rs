...
use crate::compiler::instructions::{
    CompareOp, Instruction, Instructions, LOOP_FLAG_RECURSIVE, LOOP_FLAG_WITH_LOOP_VAR, LocalId,
    MAX_LOCALS,
};
...
impl<'source> CodeGenerator<'source> {
    ...
    pub fn start_if(&mut self) {
        let jump_instr = patch::begin(self, patch::Jump::IfFalse).unwrap();
        self.pending_block.push(PendingBlock::Branch { jump_instr });
    }

    /// Begins an else conditional
    pub fn start_else(&mut self) {
        let Some(PendingBlock::Branch { jump_instr }) = self.pending_block.pop() else {
            unreachable!()
        };
        let jump_instr = patch::otherwise(self, jump_instr).unwrap();
        self.pending_block.push(PendingBlock::Branch { jump_instr });
    }
    ...
    pub fn sc_bool(&mut self, and: bool) {
        if !matches!(self.pending_block.last(), Some(PendingBlock::ScBool { .. })) {
            unreachable!();
        }
        let kind = if and {
            patch::Jump::IfFalseOrPop
        } else {
            patch::Jump::IfTrueOrPop
        };
        let instruction = patch::begin(self, kind).unwrap();
        if let Some(PendingBlock::ScBool { jump_instrs }) = self.pending_block.last_mut() {
            jump_instrs.push(instruction);
        }
    }
    ...
    pub fn end_sc_bool(&mut self) {
        if let Some(PendingBlock::ScBool { jump_instrs }) = self.pending_block.pop() {
            for instruction in jump_instrs {
                patch::finish(self, instruction).unwrap();
            }
        }
    }
...
impl patch::Emitter for CodeGenerator<'_> {
    type Error = std::convert::Infallible;
    fn position(&self) -> u32 {
        self.next_instruction()
    }
    fn jump(&mut self, kind: patch::Jump) -> Result<u32, Self::Error> {
        // Ordinary short-circuit opcodes have always been added without a new
        // location. Keep their exact source metadata while sharing patching.
        Ok(match kind {
            patch::Jump::Always => self.add(Instruction::Jump(!0)),
            patch::Jump::IfFalse => self.add(Instruction::JumpIfFalse(!0)),
            patch::Jump::IfFalseOrPop => self.instructions.add(Instruction::JumpIfFalseOrPop(!0)),
            patch::Jump::IfTrueOrPop => self.instructions.add(Instruction::JumpIfTrueOrPop(!0)),
        })
    }
    fn patch(&mut self, instruction: u32, target: u32) -> Result<(), Self::Error> {
        match self.instructions.get_mut(instruction) {
            Some(
            ...
            _ => unreachable!("ordinary compiler jump patch"),
        }
        Ok(())
    }
}
