...
impl<'a> AssignmentTracker<'a> {
    fn is_assigned(&self, name: &str) -> bool {
        let found = self.assigned.iter().any(|x| x.contains(name));
        super::tests::record(super::tests::Event::Lookup(name.into(), found));
        found
    }

    fn assign(&mut self, name: &'a str) {
        super::tests::record(super::tests::Event::Assign(name.into()));
        self.assigned.last_mut().unwrap().insert(name);
    }

    fn assign_nested(&mut self, name: String) {
        super::tests::record(super::tests::Event::Nested(name.clone()));
    ...
    fn push(&mut self) {
        super::tests::record(super::tests::Event::Push);
        self.assigned.push(Default::default());
    }

    fn pop(&mut self) {
        super::tests::record(super::tests::Event::Pop);
        self.assigned.pop();
    }
}
    ...
    match expr {
        ast::Expr::Var(var) => {
            if !state.is_assigned(var.id) {
                super::tests::record(super::tests::Event::Capture(var.id.into()));
