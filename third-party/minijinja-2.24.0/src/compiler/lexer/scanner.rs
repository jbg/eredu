...
use super::literal::{LexError as Error, Lexeme as Token, NumberLiteral};
use crate::compiler::tokens::Span;
use crate::syntax::SyntaxConfig;
use crate::utils::{memchr, memstr};

use super::WhitespaceConfig;
...
#[cfg(feature = "unicode")]
pub(super) fn lex_identifier(s: &str) -> usize {
    s.chars()
        .enumerate()
        .map_while(|(idx, c)| {
            let cont = if c == '_' {
...
impl<'s> Scanner<'s> {
    ...
    pub fn next_token(&mut self) -> Result<Option<(Token<'s>, Span)>, Error> {
        loop {
            ...
            let outcome = match self.stack.last() {
                ...
                None => return Err(Error::empty_stack()),
            };
            match ok!(outcome) {
    ...
    #[inline]
    pub(crate) fn syntax_error(&self, msg: &'static str) -> Error {
        Error::syntax(msg, self.span(self.loc()))
    }
    ...
    fn eat_number(&mut self) -> Result<(Token<'s>, Span), Error> {
        ...
        if has_underscore && num.ends_with('_') {
    ...
    fn eat_string(&mut self, delim: u8) -> Result<(Token<'s>, Span), Error> {
        ...
        Ok(if has_escapes {
            (Token::Escaped(&s[1..s.len() - 1]), self.span(old_loc))
        } else {
    ...
    fn tokenize_root(&mut self) -> Result<ControlFlow<(Token<'s>, Span)>, Error> {
        ...
        let old_loc = self.loc();
        let (lead, span) = match self
            .syntax_config
            .find_start_marker(self.source, self.current_offset)
        {
...
// The root dispatch alone pushes one block/variable/line state. Expression
// scanners start with one variable state. Neither state recursively pushes.
struct StateStack {
    values: [Option<LexerState>; 2],
    len: usize,
}
impl StateStack {
    fn push(&mut self, state: LexerState) {
        self.values[self.len] = Some(state);
        self.len += 1;
    }
    fn pop(&mut self) -> Option<LexerState> {
        if self.len == 0 {
            return None;
        }
        self.len -= 1;
    ...
    fn last(&self) -> Option<&LexerState> {
        self.len
            .checked_sub(1)
            .and_then(|n| self.values[n].as_ref())
    }
}

// Closed default syntax consists entirely of static delimiters. Ordinary
// custom syntax retains its original owned config and matching implementation.
...
impl Syntax {
    fn find_start_marker(
        ...
        match self {
            Self::Default => find_start_marker_memchr(&source[offset..]),
            Self::Owned(config) => find_start_marker(source, offset, config),
        }
    }
    fn block_delimiters(&self) -> (&str, &str) {
        match self {
            Self::Default => ("{%", "%}"),
            Self::Owned(c) => c.block_delimiters(),
        }
    }
    fn variable_delimiters(&self) -> (&str, &str) {
        match self {
            Self::Default => ("{{", "}}"),
            Self::Owned(c) => c.variable_delimiters(),
        }
    }
    fn comment_delimiters(&self) -> (&str, &str) {
        match self {
            Self::Default => ("{#", "#}"),
            Self::Owned(c) => c.comment_delimiters(),
        }
    }
    #[cfg(feature = "custom_syntax")]
    fn line_statement_prefix(&self) -> Option<&str> {
        match self {
            Self::Default => None,
            Self::Owned(c) => c.line_statement_prefix(),
        }
    }
}
