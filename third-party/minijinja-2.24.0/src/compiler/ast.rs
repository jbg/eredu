...
impl Expr<'_> {
    ...
    pub fn as_const(&self) -> Option<Value> {
        super::fold::ordinary::expression(self)
    }
}

pub(super) fn eval_binop(op: BinOpKind, left: &Value, right: &Value) -> Option<Value> {
    match op {
        ...
        BinOpKind::ScAnd | BinOpKind::ScOr => {
            use crate::value::primitive::scalar::{self, Logical, Selection};
            ...
            Some(
                match scalar::logical(op, || left.is_true(), || right.is_true()) {
                    Selection::Left => left.clone(),
                    Selection::Right => right.clone(),
                    Selection::False => Value::from(false),
                },
            )
        }
    }
...
pub(super) fn eval_compare(op: CompareOpKind, left: &Value, right: &Value) -> Option<Value> {
    match op {
