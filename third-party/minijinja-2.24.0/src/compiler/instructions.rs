...
pub enum Instruction<'source> {
    ...
    /// Start collecting a filtered iterator without growing the operand stack.
    BeginCollection,
    /// Retain the accepted top operand in the current collection.
    AppendCollection,
    /// Return the completed collection as an ordinary sequence.
    EndCollection,
