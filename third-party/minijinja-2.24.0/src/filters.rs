...
/// Borrowed whitespace boundary shared by ordinary trim and closed text storage.
pub(crate) fn whitespace_trim_range(value: &str) -> std::ops::Range<usize> {
...
pub(crate) fn character_trim_range(value: &str, chars: &str) -> std::ops::Range<usize> {
    strip_range(value, Some(chars), true, true)
}
...
mod builtins {
    ...
    use crate::format_utils::{FormatConversion, FormatStyle, format_filter, format_printf_with};
    use crate::utils::safe_sort;
    use crate::value::merge_object::{MergeDict, MergeSeq};
    use crate::value::ops::{self, LenIterWrap, as_f64};
    use crate::value::{
    ...
    pub fn upper(value: StringInput<'_>) -> Value {
        let mut output = String::with_capacity(value.as_str().len());
        crate::value::primitive::case::upper(&mut output, value.as_str())
            .expect("writing uppercase into String cannot fail");
    ...
    pub fn replace(
        ...
        if safety_aware {
            let output = replace_plain(&value.format(state)?, from.as_str(), &to.format(state)?);
            Ok(Value::from_safe_string(output))
        } else {
            Ok(Value::from(replace_plain(value.as_str(), from.as_str(), to.as_str())))
        }
    }

    fn replace_plain(value: &str, from: &str, to: &str) -> String {
        let mut output = String::new();
        crate::value::primitive::replace::plain(&mut output, value, from, to)
    ...
    fn cmp_helper(a: &Value, b: &Value, case_sensitive: bool, reverse: bool) -> Ordering {
        let ordering = if !case_sensitive {
            if let (Some(a), Some(b)) = (a.as_str(), b.as_str()) {
                ...
                {
                    crate::value::primitive::case::ascii_fold_cmp(a,b)
                }
            } else {
                a.cmp(b)
            }
    ...
    pub fn dictsort(v: &Value, kwargs: Kwargs) -> Result<Value, Error> {
        ...
        let rv: Vec<_> = if let Some(iter) = v.as_object().and_then(|v| v.try_iter_pairs()) {
            iter.collect()
        } else {
            ok!(v.try_iter())
        ...
        let mut order: Vec<_> = (0..rv.len()).collect();
        crate::utils::safe_sort_with(|| {
            super::dictsort::ordinary(&mut order, |left,right| {
                let (a,b)=(&rv[left],&rv[right]);
                let (a, b) = if by_value { (&a.1, &b.1) } else { (&a.0, &b.0) };
                cmp_helper(a, b, case_sensitive, reverse)
            });
        })?;
        kwargs.assert_all_used()?;
        Ok(order.into_iter().map(|index| {
            let (key,value)=&rv[index];
            Value::from([key.clone(),value.clone()])
        }).collect())
    }
    ...
    pub fn join(
        ...
        fn join_plain(iter: impl Iterator<Item = Value>, joiner: &str) -> String {
            let mut output = String::new();
            crate::value::primitive::join::plain(&mut output, iter, joiner, |output, item| {
                if let Some(string) = item.as_str() {
                    output.push_str(string);
                } else {
                    // Preserve ordinary Display's ignored formatting failure.
                    write!(output, "{item}").ok();
                }
                Ok(())
            })
            .expect("writing join separators into String cannot fail");
            output
        }
    ...
    pub fn split(
        ...
        Ok(Value::from_iter(super::split_parts_limit(&string,split.as_deref(),maxsplits).map(wrap)))
    }
    ...
    pub fn default(state: &State, value: &Value, args: Rest<Value>) -> Result<Value, Error> {
        ...
        let selected =
            crate::value::primitive::default::use_fallback(value.is_undefined(), lax, || {
                Ok::<_, std::convert::Infallible>(value.is_true())
            })
    ...
    pub fn float(state: &State, value: &Value) -> Result<Value, Error> {
        match &value.0 {
            ...
            ValueRepr::Bool(x) => Ok(Value::from(crate::value::primitive::scalar::filter_float(crate::value::primitive::scalar::Scalar::Bool(*x)))),
            ValueRepr::String(..) | ValueRepr::SmallStr(_) =>
                crate::value::primitive::scalar::parse_float(value.as_str().unwrap())
                .map(Value::from)
        ...
        if let Some(attr) = ok!(kwargs.get::<Option<&str>>("attribute")) {
            let keys: Vec<_> = attr
                ...
                .filter_map(|key| {
                    ...
                    if !key.is_empty() { Some(trimmed) } else { None }
                })
                .collect();
    ...
    fn select_or_reject(
        ...
        super::selection::ordinary(
            ok!(state.undefined_behavior().try_iter(value)), invert,
            |value| {
                let test_value = if let Some(ref attr) = attr {
                    ok!(value.get_path(attr))
                } else { value.clone() };
                if let Some(ref test) = test {
                    let new_args = Some(test_value).into_iter()
                        .chain(args.0.iter().cloned()).collect::<Vec<_>>();
                    Ok(ok!(test.call(state, &new_args)).is_true())
                } else { Ok(test_value.is_true()) }
            },
        )
    }
    ...
    pub fn map(
        ...
        super::mapping::ordinary(
            ok!(state.undefined_behavior().try_iter(value)),
            &mut rv,
            |value| {
