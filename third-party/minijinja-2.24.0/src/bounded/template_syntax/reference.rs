// Independent unchanged pre-refactor expression/statement parser.
#![allow(dead_code)]
use std::borrow::Cow;
...
#[cfg(test)]
pub(super) fn depth_case(source: &str, initial: usize) -> (Result<ast::Expr<'_>, Error>, usize) {
    let mut parser = Parser::new(source, "<expression>", true, Default::default(), Default::default());
    parser.depth = initial;
    let value = parser.parse_expr();
    (value, parser.depth)
}

pub(super) fn repeated<'s>(
    source: &'s str,
) -> Vec<(
