//! Uppercase writes into the same admitted generated-text destination.
use super::{generated::Output, measured::Measured, *};
impl Engine<'_, '_, '_> {
    pub(super) fn apply_upper(&mut self, arguments: Option<u16>) -> Result<(), Error> {
        if arguments != Some(1) {
            return Err(Error::Geometry);
        }
        let value = self.pop()?;
        let value = self.string_value(value)?;
        let value = self.text_atom(value)?;
        ...
        primitive::case::upper(&mut output, value)
...
pub(super) fn control_bytes() -> Option<usize> {
    ...
    frames
        ...
        .checked_add(primitive::case::control_bytes::<Output<'_>>()?)
}
