...
use crate::utils::{AutoEscape, UndefinedBehavior, untrusted_size_hint};
use crate::value::{Kwargs, ObjectRepr, Value, ValueMap, ops, value_map_with_capacity};
    ...
    fn eval_impl(
        ...
        loop {
            ...
            if let Some(op) = shared::ordinary(instr) {
                let result = shared::step(
                    ...
                    &mut shared::ordinary::Ordinary {
                        ...
                        loaded_filters: &mut loaded_filters,
                        loaded_tests: &mut loaded_tests,
                        output: out,
            ...
            match instr {
                ...
                | Instruction::BuildList(Some(_))
                | Instruction::BeginCollection
                | Instruction::AppendCollection
                | Instruction::EndCollection
                ...
                | Instruction::JumpIfTrueOrPop(_) => unreachable!("shared dispatch"),
                Instruction::Swap => {
                    a = stack.pop();
                    b = stack.pop();
                    stack.push(a);
                ...
                Instruction::BuildList(None) => {
                    let count = stack.pop().try_into().unwrap();
                ...
                Instruction::CallFunction(name, arg_count) => {
                    ...
                    let rv = if *name == "super" {
                        ...
                        if let Some(loop_object) = func.downcast_object_ref::<Loop>() {
                            ...
                            // Reuse the resolved function and shared call storage.
                            // Custom contexts still observe exactly one lookup,
                            // and recursive Loop bindings used the branch above.
                            ctx_ok!(shared::step(
                                shared::Op::CallFunction(name, *arg_count, Some(func)),
                                pc,
                                &mut shared::ordinary::Ordinary {
                                    vm: self,
                                    state,
                ...
                Instruction::DupTop => unreachable!("shared duplicate"),
                Instruction::DiscardTop => unreachable!("shared discard"),
                ...
                | Instruction::Return
                | Instruction::Enclose(..)
                | Instruction::GetClosure => unreachable!("shared macro dispatcher"),
            }
            pc += 1;
        }
