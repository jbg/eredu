...
use crate::tests;
use crate::utils::{AutoEscape, write_escaped};
...
// One declaration drives both the ordinary globals and side-effect-free
// membership used by the closed text source. The query initializes no cache.
#[derive(Clone, Copy)]
enum BuiltinGlobal {
    Range,
    Dict,
...
#[cfg(feature = "builtins")]
const BUILTIN_GLOBALS: &[BuiltinGlobal] = &[
    BuiltinGlobal::Range,
    BuiltinGlobal::Dict,
    BuiltinGlobal::Debug,
...
impl BuiltinGlobal {
    ...
    fn value(self) -> Value {
        ...
        match self {
            Self::Range => BoxedFunction::new(functions::range).to_value(),
            Self::Dict => BoxedFunction::new(functions::dict).to_value(),
            Self::Debug => BoxedFunction::new(functions::debug).to_value(),
            Self::Namespace => BoxedFunction::new(functions::namespace).to_value(),
        }
    }
}
pub(crate) fn is_builtin_global(name: &str) -> bool {
    BUILTIN_GLOBALS.iter().any(|global| global.name() == name)
}
pub(crate) fn builtin_global_control_bytes() -> usize {
    std::mem::size_of::<(
        &str,
        std::slice::Iter<'static, BuiltinGlobal>,
        &BuiltinGlobal,
        BuiltinGlobal,
        bool,
    )>()
...
fn build_globals() -> BTreeMap<Cow<'static, str>, Value> {
    ...
    let mut rv = BTreeMap::new();
    #[cfg(feature = "builtins")]
    for global in BUILTIN_GLOBALS {
        rv.insert(global.name().into(), global.value());
    }
    rv
}
