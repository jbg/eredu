...
mod builtins {
    ...
    /// ```
    ///
    /// This function will refuse to create ranges over 100,000 items.
    #[cfg_attr(docsrs, doc(cfg(feature = "builtins")))]
    pub fn range(lower: isize, upper: Option<isize>, step: Option<isize>) -> Result<Value, Error> {
        use crate::value::primitive::range::Range;
        let plan = Range::prepare(lower,upper,step)
            .map_err(|cause|Error::new(ErrorKind::InvalidOperation,cause.message()))?;
        let rng=plan.start..plan.end;
        Ok(match plan.step {
            None => Value::make_iterable(move || rng.clone()),
            Some(step) if step>0 => {
                let iterator=rng.step_by(step as usize);
                Value::make_iterable(move || iterator.clone())
            }
            Some(_) => {
                let iterator=(0..plan.length).map(move |index|
                    plan.get(index).expect("validated integer range member"));
                Value::make_iterable(move || iterator.clone())
            }
        })
    }
