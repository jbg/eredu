...
pub use crate::value::argtypes::{
    ArgType, FunctionArgs, FunctionResult, Kwargs, Rest, StringInput, from_args,
};
...
impl PartialEq for Value {
    fn eq(&self, other: &Self) -> bool {
        if let (Some(a), Some(b)) = (
            primitive::scalar::project(self),
            primitive::scalar::project(other),
        ) {
            return primitive::scalar::equal(a, b, |side| {
                let value = match side {
                    primitive::scalar::Side::Left => self,
                    primitive::scalar::Side::Right => other,
                };
        ...
        match (&self.0, &other.0) {
            ...
            _ => match ops::coerce(self, other, false) {
                ...
                None => {
                    if self.as_object().is_some() && other.as_object().is_some() {
                        structural::objects_equal(self.clone(),other.clone())
                    } else { false }
                }
            },
        }
...
fn cmp_f64(left: f64, right: f64) -> Ordering {
    primitive::scalar::cmp_f64(left, right)
}
...
fn cmp_i128_u128(left: i128, right: u128) -> Ordering {
    primitive::scalar::cmp_i128_u128(left, right)
}

fn cmp_f64_i128(left: f64, right: i128) -> Ordering {
    primitive::scalar::cmp_f64_i128(left, right)
}

fn cmp_f64_u128(left: f64, right: u128) -> Ordering {
    primitive::scalar::cmp_f64_u128(left, right)
}
...
impl Ord for Value {
    fn cmp(&self, other: &Self) -> Ordering {
        if let (Some(a), Some(b)) = (
            ...
            return primitive::scalar::compare(a, b, |side| {
                let value = match side {
                    primitive::scalar::Side::Left => self,
                    primitive::scalar::Side::Right => other,
                };
...
impl fmt::Display for Value {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.0 {
            ...
            ValueRepr::F64(val) => {
                match primitive::text::float_display(val) {
                    primitive::text::FloatDisplay::Nan => f.write_str("NaN"),
                    primitive::text::FloatDisplay::Infinite(negative) => {
                        write!(f, "{}inf", if negative { "-" } else { "" })
                    }
                    primitive::text::FloatDisplay::Finite => {
                        // Preserve ordinary allocation and arbitrary Formatter write boundaries.
...
impl Value {
    ...
    pub fn as_usize(&self) -> Option<usize> {
        ...
        primitive::text::as_usize(primitive::scalar::project(self), || {
    ...
    pub fn len(&self) -> Option<usize> {
        ...
        match self.0 {
            ValueRepr::String(ref s, _) => Length::Text(s),
            ValueRepr::SmallStr(ref s) => Length::Text(s.as_str()),
            ValueRepr::Bytes(ref b) => Length::Known(Some(b.len())),
            ValueRepr::Object(ref dy) => Length::Known(dy.enumerator_len()),
            _ => Length::Known(None),
        }
        .get()
    }
    ...
    pub(crate) fn get_item_opt(&self, key: &Value) -> Option<Value> {
        fn index(value: &Value, len: impl Fn() -> Option<usize>) -> Option<usize> {
            primitive::sequence_index(value.as_i64(), len)
        }
