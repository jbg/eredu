...
use crate::allocation::{Allocation, Allocator, Unenforced};
...
impl Builder {
    ...
    {
        Self::new_with_allocations(patterns, &Unenforced)
            .expect("ordinary regex pattern allocation")
    }
    fn new_with_allocations<I, S>(patterns: I, policy: &dyn Allocation) -> Result<Builder, Error>
    where
        S: AsRef<str>,
        I: IntoIterator<Item = S>,
    {
        ...
        for pattern in patterns {
            let text = allocator.copy_str(pattern.as_ref())?;
            allocator.push(&mut builder.pats, text)?;
        }
        Ok(builder)
    }

    fn build_one_string(&self) -> Result<crate::Regex, Error> {
        self.build_one_string_with_allocations(&Unenforced)
    }

    fn build_one_string_with_allocations(
        &self,
        policy: &dyn Allocation,
    ) -> Result<crate::Regex, Error> {
        ...
        let pattern = Allocator::new(policy).arc_str(self.pats[0].as_str())?;
        meta::Builder::new()
            .configure(metac)
            .syntax(syntaxc)
            .build_with_allocations(&pattern, policy)
            .map(|meta| crate::Regex { meta, pattern })
            .map_err(|error| Error::from_meta_build_error_with_allocations(error, policy))
    }

    fn build_one_bytes(&self) -> Result<crate::bytes::Regex, Error> {
        self.build_one_bytes_with_allocations(&Unenforced)
    }

    fn build_one_bytes_with_allocations(
        &self,
        policy: &dyn Allocation,
    ) -> Result<crate::bytes::Regex, Error> {
        assert_eq!(1, self.pats.len());
        let metac = self
            .metac
            .clone()
    ...
    fn build_many_string(&self) -> Result<crate::RegexSet, Error> {
        self.build_many_string_with_allocations(&Unenforced)
    }

    fn build_many_string_with_allocations(
        &self,
        policy: &dyn Allocation,
    ) -> Result<crate::RegexSet, Error> {
        let metac = self
            .metac
            .clone()
        ...
        let syntaxc = self.syntaxc.clone().utf8(true);
        let patterns = crate::allocation::strings(&self.pats, policy)?;
        meta::Builder::new()
            .configure(metac)
            .syntax(syntaxc)
            .build_many_with_allocations(&patterns, policy)
            .map(|meta| crate::RegexSet { meta, patterns })
            .map_err(|error| Error::from_meta_build_error_with_allocations(error, policy))
    }

    fn build_many_bytes(&self) -> Result<crate::bytes::RegexSet, Error> {
        self.build_many_bytes_with_allocations(&Unenforced)
    }

    fn build_many_bytes_with_allocations(
        &self,
        policy: &dyn Allocation,
    ) -> Result<crate::bytes::RegexSet, Error> {
        let metac = self
            .metac
            .clone()
            .match_kind(MatchKind::All)
...
pub(crate) mod string {
    ...
    impl RegexBuilder {
        ...
        /// Copy the source pattern only after its actual destinations are admitted.
        pub fn new_with_allocations(
            pattern: &str,
            policy: &dyn Allocation,
        ) -> Result<RegexBuilder, Error> {
            Ok(RegexBuilder {
                builder: Builder::new_with_allocations([pattern], policy)?,
            })
        }
        ...
        /// Compile with prospective funding for the original source producers.
        pub fn build_with_allocations(&self, policy: &dyn Allocation) -> Result<Regex, Error> {
            self.builder.build_one_string_with_allocations(policy)
        }
    ...
    impl RegexSetBuilder {
        ...
        /// Copy source patterns through the same original prospective worker.
        pub fn new_with_allocations<I, S>(
            patterns: I,
            policy: &dyn Allocation,
        ) -> Result<RegexSetBuilder, Error>
        where
            S: AsRef<str>,
            I: IntoIterator<Item = S>,
        {
            Ok(RegexSetBuilder {
                builder: Builder::new_with_allocations(patterns, policy)?,
            })
        }
        ...
        /// Compile with prospective funding for the original source producers.
        pub fn build_with_allocations(&self, policy: &dyn Allocation) -> Result<RegexSet, Error> {
            self.builder.build_many_string_with_allocations(policy)
        }
...
pub(crate) mod bytes {
    ...
    impl RegexBuilder {
        ...
        /// Compile with prospective funding for the original source producers.
        pub fn build_with_allocations(&self, policy: &dyn Allocation) -> Result<Regex, Error> {
            self.builder.build_one_bytes_with_allocations(policy)
        }
    ...
    impl RegexSetBuilder {
        ...
        /// Compile with prospective funding for the original source producers.
        pub fn build_with_allocations(&self, policy: &dyn Allocation) -> Result<RegexSet, Error> {
            self.builder.build_many_bytes_with_allocations(policy)
        }
