...
impl From<crate::allocation::AllocationError> for Error {
    fn from(error: crate::allocation::AllocationError) -> Self {
        Self {
            kind: ErrorKind::Allocation(error),
            pattern: String::new(),
            span: Span::splat(crate::ast::Position::new(0, 1, 1)),
        }
    }
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum ErrorKind {
    /// Storage construction was refused before allocation.
...
impl core::fmt::Display for ErrorKind {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        ...
        let msg = match *self {
            Allocation(error) => return error.fmt(f),
...
impl Hir {
    ...
    fn into_parts(mut self) -> (HirKind, Properties) {
        (
            core::mem::replace(&mut self.kind, HirKind::Empty),
            core::mem::replace(&mut self.props, Properties(None)),
        )
    }
}
...
impl Hir {
    fn reserve_node(
        allocation: crate::allocation::Allocator<'_>,
    ) -> Result<(), crate::allocation::AllocationError> {
        // Properties are boxed once per newly produced node. The iterative
        // destructor uses capacities 1,2,4,... and needs <4 slots per node.
        allocation.reserve(
            core::mem::size_of::<PropertiesI>()
                + 4 * core::mem::size_of::<Hir>(),
        )
    }

    /// Construct this HIR through its original smart constructor after prospective storage admission.
    pub fn literal_with_allocations(
        bytes: Vec<u8>,
        allocation: crate::allocation::Allocator<'_>,
    ) -> Result<Hir, crate::allocation::AllocationError> {
        let bytes = allocation.boxed_slice(bytes)?;
        Self::literal_box_with_allocations(bytes, allocation)
    }
    ...
    #[inline]
    pub fn empty() -> Hir {
        Self::empty_with_allocations(crate::allocation::Allocator::unenforced()).expect("ordinary syntax allocation")
    }

    /// Construct this HIR through its original smart constructor after prospective storage admission.
    pub fn empty_with_allocations(
        allocation: crate::allocation::Allocator<'_>,
    ) -> Result<Hir, crate::allocation::AllocationError> {
        Self::reserve_node(allocation)?;
    ...
    #[inline]
    pub fn fail() -> Hir {
        Self::fail_with_allocations(crate::allocation::Allocator::unenforced())
            .expect("ordinary syntax allocation")
    }

    /// Construct this HIR through its original smart constructor after prospective storage admission.
    pub fn fail_with_allocations(
        allocation: crate::allocation::Allocator<'_>,
    ) -> Result<Hir, crate::allocation::AllocationError> {
    ...
    pub fn literal<B: Into<Box<[u8]>>>(lit: B) -> Hir {
        Self::literal_box_with_allocations(
            lit.into(),
            crate::allocation::Allocator::unenforced(),
        )
        .expect("ordinary syntax allocation")
    }

    fn literal_box_with_allocations(
        bytes: Box<[u8]>,
        allocation: crate::allocation::Allocator<'_>,
    ) -> Result<Hir, crate::allocation::AllocationError> {
        if bytes.is_empty() {
            return Hir::empty_with_allocations(allocation);
        }
    ...
    pub fn class_with_allocations(
        ...
        if class.is_empty() {
            return Hir::fail_with_allocations(allocation);
        } else if let Some(bytes) =
            class.literal_with_allocations(allocation)?
        {
            return Hir::literal_with_allocations(bytes, allocation);
        }
        Self::reserve_node(allocation)?;
        let props = Properties::class(&class);
        Ok(Hir {
    ...
    pub fn repetition(rep: Repetition) -> Hir {
        Self::repetition_with_allocations(
            rep,
            crate::allocation::Allocator::unenforced(),
        )
        .expect("ordinary syntax allocation")
    ...
    /// Construct this HIR through its original smart constructor after prospective storage admission.
    pub fn repetition_with_allocations(
        mut rep: Repetition,
        allocation: crate::allocation::Allocator<'_>,
    ) -> Result<Hir, crate::allocation::AllocationError> {
    ...
    /// Construct this HIR through its original smart constructor after prospective storage admission.
    pub fn capture_with_allocations(
        capture: Capture,
        allocation: crate::allocation::Allocator<'_>,
    ) -> Result<Hir, crate::allocation::AllocationError> {
        Self::reserve_node(allocation)?;
    ...
    /// Construct this HIR through its original smart constructor after prospective storage admission.
    pub fn concat_with_allocations(
        subs: Vec<Hir>,
        allocation: crate::allocation::Allocator<'_>,
    ) -> Result<Hir, crate::allocation::AllocationError> {
            ...
            match kind {
                HirKind::Literal(Literal(bytes)) => {
                    if let Some(ref mut prior_bytes) = prior_lit {
                        allocation.extend_copy(prior_bytes, &bytes)?;
                    } else {
                        prior_lit = Some(allocation.copy_slice(&bytes)?);
                    }
                }
                    ...
                    for sub2 in subs2 {
                        ...
                        match kind2 {
                            HirKind::Literal(Literal(bytes)) => {
                                if let Some(ref mut prior_bytes) = prior_lit {
                                    allocation
                                        .extend_copy(prior_bytes, &bytes)?;
                                } else {
                                    prior_lit =
                                        Some(allocation.copy_slice(&bytes)?);
                                }
                            }
                            kind2 => {
                ...
                kind => {
                    ...
                    allocation.push(&mut new, Hir { kind, props })?;
                }
            }
        }
        if let Some(prior_bytes) = prior_lit.take() {
            allocation.push(
                &mut new,
                Hir::literal_with_allocations(prior_bytes, allocation)?,
            )?;
        }
        if new.is_empty() {
            return Hir::empty_with_allocations(allocation);
        } else if new.len() == 1 {
            return Ok(new.pop().unwrap());
        }
        Self::reserve_node(allocation)?;
    ...
    pub fn alternation(subs: Vec<Hir>) -> Hir {
        Self::alternation_with_allocations(
            subs,
            crate::allocation::Allocator::unenforced(),
        )
        .expect("ordinary syntax allocation")
    ...
    /// Construct this HIR through its original smart constructor after prospective storage admission.
    pub fn alternation_with_allocations(
        subs: Vec<Hir>,
        allocation: crate::allocation::Allocator<'_>,
    ) -> Result<Hir, crate::allocation::AllocationError> {
        ...
        let mut new = Vec::new();
        allocation.grow(&mut new, subs.len())?;
        for sub in subs {
            let (kind, props) = sub.into_parts();
            match kind {
                HirKind::Alternation(subs2) => {
                    allocation.grow(&mut new, subs2.len())?;
                    new.extend(subs2);
                }
                kind => {
                    allocation.push(&mut new, Hir { kind, props })?;
        ...
        if let Some(singletons) = singleton_chars(&new, allocation)? {
            let it = singletons
                .into_iter()
                .map(|ch| ClassUnicodeRange { start: ch, end: ch });
            return Hir::class_with_allocations(
                Class::Unicode(ClassUnicode::new_with_allocations(
                    it, allocation,
                )?),
                allocation,
            );
        }
        if let Some(singletons) = singleton_bytes(&new, allocation)? {
            let it = singletons
                .into_iter()
                .map(|b| ClassBytesRange { start: b, end: b });
            return Hir::class_with_allocations(
                Class::Bytes(ClassBytes::new_with_allocations(
                    it, allocation,
                )?),
                allocation,
            );
        ...
        if let Some(cls) = class_chars(&new, allocation)? {
            return Hir::class_with_allocations(cls, allocation);
        }
        if let Some(cls) = class_bytes(&new, allocation)? {
            return Hir::class_with_allocations(cls, allocation);
        }
        ...
        new = match lift_common_prefix(new, allocation)? {
    ...
    /// Construct this HIR through its original smart constructor after prospective storage admission.
    pub fn dot_with_allocations(
        dot: Dot,
        allocation: crate::allocation::Allocator<'_>,
    ) -> Result<Hir, crate::allocation::AllocationError> {
        Ok(match dot {
            Dot::AnyChar => Hir::class_with_allocations(
                Class::Unicode(ClassUnicode::new_with_allocations(
            ...
            Dot::AnyByte => Hir::class_with_allocations(
                Class::Bytes(ClassBytes::new_with_allocations(
            ...
            Dot::AnyCharExcept(ch) => {
                let mut cls = ClassUnicode::new_with_allocations(
                    [ClassUnicodeRange::new(ch, ch)],
                    allocation,
                )?;
                cls.negate_with_allocations(allocation)?;
                Hir::class_with_allocations(Class::Unicode(cls), allocation)?
            }
            Dot::AnyCharExceptLF => Hir::class_with_allocations(
                Class::Unicode(ClassUnicode::new_with_allocations(
                    [
            ...
            Dot::AnyCharExceptCRLF => Hir::class_with_allocations(
                Class::Unicode(ClassUnicode::new_with_allocations(
                    [
                        ClassUnicodeRange::new('\0', '\x09'),
            ...
            Dot::AnyByteExcept(byte) => {
                let mut cls = ClassBytes::new_with_allocations(
                    [ClassBytesRange::new(byte, byte)],
                    allocation,
                )?;
                cls.negate_with_allocations(allocation)?;
                Hir::class_with_allocations(Class::Bytes(cls), allocation)?
            }
            Dot::AnyByteExceptLF => Hir::class_with_allocations(
                Class::Bytes(ClassBytes::new_with_allocations(
                    [
            ...
            Dot::AnyByteExceptCRLF => Hir::class_with_allocations(
                Class::Bytes(ClassBytes::new_with_allocations(
                    [
                        ClassBytesRange::new(b'\0', b'\x09'),
...
impl Class {
    ...
    /// Apply the operation using funded class storage.
    pub fn clone_with_allocations(
        &self,
        allocations: crate::allocation::Allocator<'_>,
    ) -> Result<Self, crate::allocation::AllocationError> {
        match self {
            Self::Unicode(x) => {
                x.clone_with_allocations(allocations).map(Self::Unicode)
            }
            Self::Bytes(x) => {
                x.clone_with_allocations(allocations).map(Self::Bytes)
            }
        }
    }
    ...
    pub fn literal_with_allocations(
        &self,
        allocations: crate::allocation::Allocator<'_>,
    ) -> Result<Option<Vec<u8>>, crate::allocation::AllocationError> {
        match self {
            Self::Unicode(x) => x.literal_with_allocations(allocations),
            Self::Bytes(x) => x.literal_with_allocations(allocations),
        }
    }
    ...
    pub fn negate_with_allocations(
        ...
        match self {
            Self::Unicode(x) => x.negate_with_allocations(allocations),
            Self::Bytes(x) => x.negate_with_allocations(allocations),
        }
    }

    /// Apply the operation using funded class storage.
    pub fn try_case_fold_simple_with_allocations(
        &mut self,
        allocations: crate::allocation::Allocator<'_>,
    ) -> Result<(), CaseFoldError> {
        match self {
            Self::Unicode(x) => {
                x.try_case_fold_simple_with_allocations(allocations)
            }
            Self::Bytes(x) => x
                .case_fold_simple_with_allocations(allocations)
                .map_err(CaseFoldError::from),
        }
    }
}
...
impl ClassUnicode {
        ...
        I: IntoIterator<Item = ClassUnicodeRange>,
    {
        ...
        .expect("ordinary class allocation")
    }

    /// Construct canonical ranges after funding every reached allocation.
    pub fn new_with_allocations<I: IntoIterator<Item = ClassUnicodeRange>>(
        ranges: I,
        allocations: crate::allocation::Allocator<'_>,
    ) -> Result<Self, crate::allocation::AllocationError> {
        Ok(Self {
            set: IntervalSet::new_with_allocations(ranges, allocations)?,
        })
    }

    /// Copy this class into newly funded storage.
    pub fn clone_with_allocations(
        &self,
        allocations: crate::allocation::Allocator<'_>,
    ) -> Result<Self, crate::allocation::AllocationError> {
        Ok(Self {
            set: self.set.clone_with_allocations(allocations)?,
        })
    }
    ...
    /// Add a range after funding any replacement storage.
    pub fn push_with_allocations(
        &mut self,
        range: ClassUnicodeRange,
        allocations: crate::allocation::Allocator<'_>,
    ) -> Result<(), crate::allocation::AllocationError> {
        self.set.push_with_allocations(range, allocations)
    }
    ...
    pub fn try_case_fold_simple(
        ...
        self.try_case_fold_simple_with_allocations(
            crate::allocation::Allocator::unenforced(),
        )
    }

    /// Expand simple case mappings using funded range storage.
    pub fn try_case_fold_simple_with_allocations(
        &mut self,
        allocations: crate::allocation::Allocator<'_>,
    ) -> Result<(), CaseFoldError> {
        self.set.case_fold_simple_with_allocations(allocations)
    }
    ...
    pub fn negate(&mut self) {
        self.negate_with_allocations(crate::allocation::Allocator::unenforced())
            .expect("ordinary class allocation")
    }

    /// Apply the set operation after funding each reached allocation.
    pub fn negate_with_allocations(
        &mut self,
        allocations: crate::allocation::Allocator<'_>,
    ) -> Result<(), crate::allocation::AllocationError> {
        self.set.negate_with_allocations(allocations)
    }

    /// Union this character class with the given character class, in place.
    pub fn union(&mut self, other: &ClassUnicode) {
        self.union_with_allocations(
            other,
            crate::allocation::Allocator::unenforced(),
        )
        .expect("ordinary class allocation")
    ...
    pub fn union_with_allocations(
        ...
        self.set.union_with_allocations(&other.set, allocations)
    }

    /// Intersect this character class with the given character class, in
    /// place.
    pub fn intersect(&mut self, other: &ClassUnicode) {
        self.intersect_with_allocations(
            other,
            crate::allocation::Allocator::unenforced(),
        )
        .expect("ordinary class allocation")
    ...
    pub fn intersect_with_allocations(
        ...
        self.set.intersect_with_allocations(&other.set, allocations)
    }

    /// Subtract the given character class from this character class, in place.
    pub fn difference(&mut self, other: &ClassUnicode) {
        self.difference_with_allocations(
            other,
            crate::allocation::Allocator::unenforced(),
        )
        .expect("ordinary class allocation")
    ...
    pub fn difference_with_allocations(
        ...
        self.set
            .difference_with_allocations(&other.set, allocations)
    }
    ...
    pub fn symmetric_difference(&mut self, other: &ClassUnicode) {
        self.symmetric_difference_with_allocations(
            other,
            crate::allocation::Allocator::unenforced(),
        )
        .expect("ordinary class allocation")
    ...
    pub fn symmetric_difference_with_allocations(
        ...
        self.set
            .symmetric_difference_with_allocations(&other.set, allocations)
    }
    ...
    /// is returned.
    pub fn literal(&self) -> Option<Vec<u8>> {
        self.literal_with_allocations(
            crate::allocation::Allocator::unenforced(),
        )
        .expect("ordinary class allocation")
    }

    /// Copy a singleton literal into newly funded storage.
    pub fn literal_with_allocations(
        &self,
        allocations: crate::allocation::Allocator<'_>,
    ) -> Result<Option<Vec<u8>>, crate::allocation::AllocationError> {
        ...
        Ok(Some(allocations.copy_slice(
            rs[0].start.encode_utf8(&mut [0; 4]).as_bytes(),
        )?))
    }
    ...
    pub fn to_byte_class(&self) -> Option<ClassBytes> {
        self.to_byte_class_with_allocations(
            crate::allocation::Allocator::unenforced(),
        )
        .expect("ordinary class allocation")
    }

    /// Convert ASCII ranges into newly funded equivalent storage.
    pub fn to_byte_class_with_allocations(
        &self,
        allocations: crate::allocation::Allocator<'_>,
    ) -> Result<Option<ClassBytes>, crate::allocation::AllocationError> {
        if !self.is_ascii() {
            return Ok(None);
        }
        Ok(Some(ClassBytes::new_with_allocations(
            self.ranges().iter().map(|r| ClassBytesRange {
    ...
    fn case_fold_simple(
        ...
        for cp in (start..=end).filter_map(char::from_u32) {
            for &cp_folded in folder.mapping(cp) {
                allocations.push(
                    ranges,
                    ClassUnicodeRange::new(cp_folded, cp_folded),
                )?;
            }
        }
        Ok(())
...
impl ClassBytes {
    ...
    /// Construct canonical ranges after funding every reached allocation.
    pub fn new_with_allocations<I: IntoIterator<Item = ClassBytesRange>>(
        ranges: I,
        allocations: crate::allocation::Allocator<'_>,
    ) -> Result<Self, crate::allocation::AllocationError> {
        Ok(Self {
    ...
    pub fn case_fold_simple(&mut self) {
        self.case_fold_simple_with_allocations(
            crate::allocation::Allocator::unenforced(),
        )
        .expect("ordinary class allocation")
    }

    /// Expand ASCII case mappings using funded range storage.
    pub fn case_fold_simple_with_allocations(
        &mut self,
        allocations: crate::allocation::Allocator<'_>,
    ) -> Result<(), crate::allocation::AllocationError> {
        match self.set.case_fold_simple_with_allocations(allocations) {
            Ok(()) => Ok(()),
            Err(CaseFoldError::Allocation(error)) => Err(error),
            Err(CaseFoldError::Unavailable) => {
                unreachable!("ASCII case folding is always available")
            }
        }
    }
    ...
    /// Convert ASCII ranges into newly funded equivalent storage.
    pub fn to_unicode_class_with_allocations(
        &self,
        allocations: crate::allocation::Allocator<'_>,
    ) -> Result<Option<ClassUnicode>, crate::allocation::AllocationError> {
        if !self.is_ascii() {
            return Ok(None);
        }
        Ok(Some(ClassUnicode::new_with_allocations(
            self.ranges().iter().map(|r| ClassUnicodeRange {
...
impl Interval for ClassBytesRange {
    ...
    fn case_fold_simple(
        ...
        if !ClassBytesRange::new(b'a', b'z').is_intersection_empty(self) {
            ...
            allocations
                .push(ranges, ClassBytesRange::new(lower - 32, upper - 32))?;
        }
        if !ClassBytesRange::new(b'A', b'Z').is_intersection_empty(self) {
            let lower = cmp::max(self.start, b'A');
            let upper = cmp::min(self.end, b'Z');
...
impl Drop for Hir {
    fn drop(&mut self) {
        ...
        crate::allocation::retirement_push(
            &mut stack,
            mem::replace(
                self,
                Hir {
        ...
        while let Some(mut expr) = stack.pop() {
            match expr.kind {
                ...
                HirKind::Concat(ref mut x) => {
                    for child in x.drain(..) {
                        crate::allocation::retirement_push(&mut stack, child);
                    }
                }
...
pub struct Properties(Option<Box<PropertiesI>>);
...
impl Properties {
    fn inner(&self) -> &PropertiesI {
        self.0
            .as_deref()
            .expect("retired HIR properties are never observed")
    }
    ...
    pub fn look_set_prefix_any(&self) -> LookSet {
        self.inner().look_set_prefix_any
    }
    ...
    pub fn look_set_suffix_any(&self) -> LookSet {
        self.inner().look_set_suffix_any
    }
    ...
    pub fn explicit_captures_len(&self) -> usize {
        self.inner().explicit_captures_len
    }
    ...
    pub fn static_explicit_captures_len(&self) -> Option<usize> {
        self.inner().static_explicit_captures_len
    }
    ...
    pub fn is_alternation_literal(&self) -> bool {
        self.inner().alternation_literal
    }
    ...
    pub fn memory_usage(&self) -> usize {
        self.0.as_ref().map_or(0, |_| core::mem::size_of::<PropertiesI>())
    }
    ...
    {
        Self::union_with_allocations(props, crate::allocation::Allocator::unenforced()).expect("ordinary properties allocation")
    }

    /// Compute the same property union with prospective ownership funding.
    pub fn union_with_allocations<I, P>(props: I, allocation: crate::allocation::Allocator<'_>) -> Result<Properties, crate::allocation::AllocationError>
    where I: IntoIterator<Item = P>, P: core::borrow::Borrow<Properties> {
        ...
        Ok(Properties(Some(allocation.boxed(props)?)))
    }
}
...
impl Properties {
    ...
    fn capture(capture: &Capture) -> Properties {
        ...
        Properties(Some(Box::new(PropertiesI {
...
fn class_chars(
    hirs: &[Hir],
    allocation: crate::allocation::Allocator<'_>,
) -> Result<Option<Class>, crate::allocation::AllocationError> {
    let mut cls = ClassUnicode::empty();
    for hir in hirs.iter() {
        match *hir.kind() {
            HirKind::Class(Class::Unicode(ref cls2)) => {
                cls.union_with_allocations(cls2, allocation)?;
            }
            HirKind::Class(Class::Bytes(ref cls2)) => {
                cls.union_with_allocations(
                    &match cls2
                        .to_unicode_class_with_allocations(allocation)?
                    {
                        Some(value) => value,
                        None => return Ok(None),
                    },
...
fn class_bytes(
    ...
    for hir in hirs.iter() {
        match *hir.kind() {
            HirKind::Class(Class::Unicode(ref cls2)) => {
                cls.union_with_allocations(
                    &match cls2.to_byte_class_with_allocations(allocation)? {
                        Some(value) => value,
                        None => return Ok(None),
                    },
                    allocation,
...
fn singleton_chars(
    hirs: &[Hir],
    allocation: crate::allocation::Allocator<'_>,
) -> Result<Option<Vec<char>>, crate::allocation::AllocationError> {
    ...
    for hir in hirs.iter() {
        ...
        allocation.push(&mut singletons, ch)?;
    }
    Ok(Some(singletons))
}
...
fn singleton_bytes(
    ...
    for hir in hirs.iter() {
        ...
        allocation.push(&mut singletons, literal[0])?;
    }
    Ok(Some(singletons))
}
...
fn lift_common_prefix(
    hirs: Vec<Hir>,
    allocation: crate::allocation::Allocator<'_>,
) -> Result<Result<Hir, Vec<Hir>>, crate::allocation::AllocationError> {
    if hirs.len() <= 1 {
        return Ok(Err(hirs));
    }
    let mut prefix = match hirs[0].kind() {
        HirKind::Concat(ref xs) => &**xs,
        _ => return Ok(Err(hirs)),
    };
    if prefix.is_empty() {
        return Ok(Err(hirs));
    }
    ...
    for h in hirs {
        ...
        allocation.push(
            &mut suffix_alts,
            Hir::concat_with_allocations(
                allocation.split_off(&mut concat, len)?,
                allocation,
            )?,
        )?;
    ...
    allocation.push(
        &mut concat,
        Hir::alternation_with_allocations(suffix_alts, allocation)?,
    )?;
    Ok(Ok(Hir::concat_with_allocations(concat, allocation)?))
}
