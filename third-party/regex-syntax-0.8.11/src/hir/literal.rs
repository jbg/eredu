...
impl Extractor {
    ...
    pub fn extract(&self, hir: &Hir) -> Seq {
        self.extract_with_allocations(hir, Allocator::unenforced())
            .expect("ordinary literal allocation")
    }

    /// Run the same literal worker with prospective allocation admission.
    pub fn extract_with_allocations(
        &self,
        hir: &Hir,
        allocation: Allocator<'_>,
    ) -> Result<Seq, AllocationError> {
        // The original post-order extraction uses admitted control rows instead
        // of growing the native stack with source nesting. Resume states retain
        // the same short-circuit and preference order as the recursive worker.
        enum Frame<'h> {
            Visit(&'h Hir),
            Concat {
                hirs: &'h [Hir],
        ...
        while let Some(frame) = stack.pop() {
            match frame {
                Frame::Visit(hir) => {
                    ...
                    match hir.kind() {
                        Empty | Look(_) => {
                            result = Some(Seq::singleton_with_allocations(
                                self::Literal::exact(Vec::new()),
                                allocation,
                            )?)
                        }
                        Literal(hir::Literal(bytes)) => {
                            let mut seq = Seq::singleton_with_allocations(
                                self::Literal::exact_with_allocations(bytes, allocation)?,
                                allocation,
                            )?;
                            self.enforce_literal_len(&mut seq);
                            result = Some(seq);
                        }
                        Class(hir::Class::Unicode(cls)) => {
                            result = Some(self.extract_class_unicode(cls, allocation)?)
                        }
                        Class(hir::Class::Bytes(cls)) => {
                            result = Some(self.extract_class_bytes(cls, allocation)?)
                        }
                        Capture(hir::Capture { sub, .. }) => {
                            allocation.push(&mut stack, Frame::Visit(sub))?
                        }
                        Repetition(rep) => {
                            allocation.push(&mut stack, Frame::Repetition(rep))?;
                            allocation.push(&mut stack, Frame::Visit(&rep.sub))?;
                        }
                        Concat(hirs) => {
                            let seq = Seq::singleton_with_allocations(
                                self::Literal::exact(Vec::new()),
                                allocation,
                            )?;
                            allocation.push(&mut stack, Frame::Concat { hirs, next: 0, seq })?;
                        }
                        Alternation(hirs) => allocation.push(
                            &mut stack,
                            Frame::Alternation {
                                hirs,
                                next: 0,
                ...
                Frame::Concat {
                    ...
                    if next != 0 {
                        seq = self.cross(seq, &mut result.take().unwrap(), allocation)?;
                    }
                    if seq.is_inexact() || next == hirs.len() {
                        result = Some(seq);
                        continue;
                    }
                    let index = if self.kind.is_suffix() {
                        hirs.len() - 1 - next
                    } else {
                        next
                    };
                    ...
                    allocation.push(&mut stack, Frame::Visit(&hirs[index]))?;
                }
                Frame::Alternation {
                    hirs,
                    next,
                    ...
                    if next != 0 {
                        seq = self.union(seq, &mut result.take().unwrap(), allocation)?;
                    }
                    if !seq.is_finite() || next == hirs.len() {
                        result = Some(seq);
                        continue;
                    }
                    allocation.push(
                    ...
                    allocation.push(&mut stack, Frame::Visit(&hirs[next]))?;
                }
                Frame::Repetition(rep) => {
                    result =
                        Some(self.extract_repetition(rep, result.take().unwrap(), allocation)?)
                }
            }
        }
        Ok(result.unwrap())
    ...
    fn extract_repetition(
        ...
        match *rep {
            hir::Repetition {
                ...
                let mut empty =
                    Seq::singleton_with_allocations(Literal::exact(vec![]), allocation)?;
                if !greedy {
                    mem::swap(&mut subseq, &mut empty);
                }
                self.union(subseq, &mut empty, allocation)
            }
            hir::Repetition {
                min,
                max: Some(max),
                ...
                let mut seq = Seq::singleton_with_allocations(Literal::exact(vec![]), allocation)?;
                for _ in 0..cmp::min(min, limit) {
                    ...
                    seq = self.cross(
                        seq,
                        &mut subseq.clone_with_allocations(allocation)?,
                        allocation,
                    )?;
                }
    ...
    /// sequence.
    fn extract_class_unicode(
        &self,
        cls: &hir::ClassUnicode,
        allocation: Allocator<'_>,
    ) -> Result<Seq, AllocationError> {
        if self.class_over_limit_unicode(cls) {
            return Ok(Seq::infinite());
        }
        let mut seq = Seq::empty();
        for r in cls.iter() {
            for ch in r.start()..=r.end() {
                seq.push_with_allocations(
                    Literal::exact_with_allocations(
    ...
    fn extract_class_bytes(
        ...
        for r in cls.iter() {
            for b in r.start()..=r.end() {
                seq.push_with_allocations(
                    Literal::exact_with_allocations(&[b], allocation)?,
                    allocation,
                )?;
            }
        }
    ...
    fn cross(
        ...
        if let ExtractKind::Suffix = self.kind {
            seq1.cross_reverse_with_allocations(seq2, allocation)?;
        } else {
            seq1.cross_forward_with_allocations(seq2, allocation)?;
        }
    ...
    fn union(
        ...
        seq1.union_with_allocations(seq2, allocation)?;
        assert!(seq1.len().map_or(true, |x| x <= self.limit_total));
        Ok(seq1)
    }
...
impl Seq {
    /// Clone the sequence and each byte destination through the same policy.
    pub fn clone_with_allocations(
        &self,
        allocation: Allocator<'_>,
    ) -> Result<Self, AllocationError> {
        let mut output = match self.literals {
            None => return Ok(Self::infinite()),
            Some(_) => Self::empty(),
        };
        let source = self.literals.as_ref().unwrap();
        let target = output.literals.as_mut().unwrap();
        allocation.grow(target, source.len())?;
        for literal in source {
            target.push(literal.clone_with_allocations(allocation)?);
        }
        Ok(output)
    }

    /// Construct one sequence after admitting its literal row.
    pub fn singleton_with_allocations(
        lit: Literal,
        allocation: Allocator<'_>,
    ) -> Result<Self, AllocationError> {
        let mut literals = Vec::new();
        allocation.push(&mut literals, lit)?;
        Ok(Self {
            literals: Some(literals),
        })
    }

    /// Copy a sequence of byte strings through the original insertion worker.
    pub fn new_with_allocations<I, B>(
        it: I,
        allocation: Allocator<'_>,
    ) -> Result<Self, AllocationError>
    where
        ...
        B: AsRef<[u8]>,
    {
        ...
        for bytes in it {
            output.push_with_allocations(
                Literal::exact_with_allocations(bytes.as_ref(), allocation)?,
                allocation,
            )?;
        }
        Ok(output)
    ...
    pub fn singleton(lit: Literal) -> Seq {
        Self::singleton_with_allocations(lit, Allocator::unenforced()).expect("ordinary singleton")
    }
    ...
    where
        ...
        B: AsRef<[u8]>,
    {
        Self::new_with_allocations(it, Allocator::unenforced()).expect("ordinary literal sequence")
    }
    ...
    pub fn push(&mut self, lit: Literal) {
        self.push_with_allocations(lit, Allocator::unenforced())
            .expect("ordinary literal allocation")
    }
    ...
    pub fn cross_forward(&mut self, other: &mut Seq) {
        self.cross_forward_with_allocations(other, Allocator::unenforced())
            .expect("ordinary literal allocation")
    }

    /// Run the same literal worker with prospective allocation admission.
    pub fn cross_forward_with_allocations(
        &mut self,
        other: &mut Seq,
        allocation: Allocator<'_>,
    ) -> Result<(), AllocationError> {
        ...
        let newcap = lits1
            .len()
            .checked_mul(lits2.len())
            .ok_or(AllocationError::SizeOverflow)?;
        let mut replacement = Vec::new();
        allocation.grow(&mut replacement, newcap)?;
        for selflit in mem::replace(lits1, replacement) {
            if !selflit.is_exact() {
                allocation.push(lits1, selflit)?;
                continue;
            }
            for otherlit in lits2.iter() {
                ...
                allocation.grow(
                    ...
                    selflit
                        .len()
                        .checked_add(otherlit.len())
                        .ok_or(AllocationError::SizeOverflow)?,
                )?;
                let mut newlit = Literal::exact(bytes);
                newlit.extend_with_allocations(&selflit, allocation)?;
                newlit.extend_with_allocations(&otherlit, allocation)?;
                if !otherlit.is_exact() {
                    newlit.make_inexact();
                }
                allocation.push(lits1, newlit)?;
            }
        }
        lits2.drain(..);
        self.dedup();
    ...
    pub fn cross_reverse(&mut self, other: &mut Seq) {
        self.cross_reverse_with_allocations(other, Allocator::unenforced())
            .expect("ordinary literal allocation")
    }

    /// Run the same literal worker with prospective allocation admission.
    pub fn cross_reverse_with_allocations(
        &mut self,
        other: &mut Seq,
        allocation: Allocator<'_>,
    ) -> Result<(), AllocationError> {
        ...
        let mut replacement = Vec::new();
        allocation.grow(&mut replacement, newcap)?;
        let selflits = mem::replace(lits1, replacement);
        for (i, otherlit) in lits2.drain(..).enumerate() {
            for selflit in selflits.iter() {
                if !selflit.is_exact() {
                    ...
                    // but wasteful.)
                    if i == 0 {
                        allocation.push(lits1, selflit.clone_with_allocations(allocation)?)?;
                    }
                    continue;
                }
                let mut bytes = Vec::new();
    ...
    pub fn union(&mut self, other: &mut Seq) {
        self.union_with_allocations(other, Allocator::unenforced())
            .expect("ordinary literal allocation")
    }
    ...
    pub fn union_with_allocations(
        ...
        allocation.grow(lits1, lits2.len())?;
        lits1.extend(lits2);
        self.dedup();
        Ok(())
    }
    ...
    pub fn union_into_empty(&mut self, other: &mut Seq) {
        self.union_into_empty_with_allocations(other, Allocator::unenforced())
            .expect("ordinary literal allocation")
    }

    /// Run the same literal worker with prospective allocation admission.
    pub fn union_into_empty_with_allocations(
        &mut self,
        other: &mut Seq,
        allocation: Allocator<'_>,
    ) -> Result<(), AllocationError> {
        ...
        let added = lits2.len();
        lits1.extend(lits2);
        lits1[first_empty..].rotate_right(added);
        self.dedup();
        Ok(())
    }
    ...
    pub fn sort(&mut self) {
        if let Some(ref mut lits) = self.literals {
            // Equal fully ordered literals are indistinguishable; no stable scratch is needed.
            lits.sort_unstable();
        }
    }
    ...
    pub fn minimize_by_preference(&mut self) {
        self.minimize_by_preference_with_allocations(Allocator::unenforced())
            .expect("ordinary literal allocation")
    }

    /// Run the same literal worker with prospective allocation admission.
    pub fn minimize_by_preference_with_allocations(
        &mut self,
        allocation: Allocator<'_>,
    ) -> Result<(), AllocationError> {
        if let Some(ref mut lits) = self.literals {
            PreferenceTrie::minimize(lits, false, allocation)?;
        }
        Ok(())
    }
    ...
    pub fn optimize_for_prefix_by_preference(&mut self) {
        self.optimize_for_prefix_by_preference_with_allocations(Allocator::unenforced())
            .expect("ordinary literal allocation")
    }

    /// Run the same literal worker with prospective allocation admission.
    pub fn optimize_for_prefix_by_preference_with_allocations(
        &mut self,
        allocation: Allocator<'_>,
    ) -> Result<(), AllocationError> {
        self.optimize_by_preference(true, allocation)
    }
    ...
    pub fn optimize_for_suffix_by_preference(&mut self) {
        self.optimize_for_suffix_by_preference_with_allocations(Allocator::unenforced())
            .expect("ordinary literal allocation")
    }

    /// Run the same literal worker with prospective allocation admission.
    pub fn optimize_for_suffix_by_preference_with_allocations(
        &mut self,
        allocation: Allocator<'_>,
    ) -> Result<(), AllocationError> {
        self.optimize_by_preference(false, allocation)
    }

    fn optimize_by_preference(
        &mut self,
        prefix: bool,
        allocation: Allocator<'_>,
    ) -> Result<(), AllocationError> {
        ...
        if prefix {
            if let Some(ref mut lits) = self.literals {
                PreferenceTrie::minimize(lits, true, allocation)?;
            }
        }
        ...
        let exact: Option<Seq> = if self.is_exact() {
            Some(self.clone_with_allocations(allocation)?)
        } else {
            None
        };
...
impl Clone for Seq {
    fn clone(&self) -> Self {
        self.clone_with_allocations(Allocator::unenforced())
            .expect("ordinary sequence clone")
    }
}
...
impl Literal {
    /// Clone byte storage after admitting its actual destination.
    pub fn clone_with_allocations(
        &self,
        allocation: Allocator<'_>,
    ) -> Result<Self, AllocationError> {
        Ok(Self {
            bytes: allocation.copy_slice(&self.bytes)?,
            exact: self.exact,
        })
    }

    /// Copy an exact literal after admitting its actual byte destination.
    pub fn exact_with_allocations(
        bytes: &[u8],
        allocation: Allocator<'_>,
    ) -> Result<Self, AllocationError> {
        Ok(Self {
            bytes: allocation.copy_slice(bytes)?,
            exact: true,
        })
    }
    ...
    pub fn extend(&mut self, lit: &Literal) {
        self.extend_with_allocations(lit, Allocator::unenforced())
            .expect("ordinary literal allocation")
    }

    /// Run the same literal worker with prospective allocation admission.
    pub fn extend_with_allocations(
        &mut self,
        lit: &Literal,
        allocation: Allocator<'_>,
    ) -> Result<(), AllocationError> {
        ...
        allocation.extend_copy(&mut self.bytes, &lit.bytes)?;
        Ok(())
    }
...
impl Clone for Literal {
    fn clone(&self) -> Self {
        self.clone_with_allocations(Allocator::unenforced())
            .expect("ordinary literal clone")
    }
}

impl From<u8> for Literal {
    fn from(byte: u8) -> Literal {
        Literal::exact_with_allocations(&[byte], Allocator::unenforced())
            .expect("ordinary byte literal")
    }
}
...
impl From<char> for Literal {
    fn from(ch: char) -> Literal {
        ...
        .expect("ordinary scalar literal")
    }
}
...
impl PreferenceTrie {
    ...
    fn minimize(
        ...
        let mut make_inexact = Vec::new();
        let mut failure = None;
        literals.retain_mut(|lit| {
            if failure.is_some() {
                return true;
            }
            match trie.insert(lit.as_bytes(), allocation) {
                Ok(Ok(_)) => true,
                Ok(Err(i)) => {
                    if !keep_exact {
                        if let Err(error) =
                            allocation.push(&mut make_inexact, i.checked_sub(1).unwrap())
                        {
                            failure = Some(error);
                            return true;
                        }
    ...
    fn insert(
        ...
        bytes: &[u8],
        allocation: Allocator<'_>,
    ) -> Result<Result<usize, usize>, AllocationError> {
        let mut prev = self.root(allocation)?;
        if let Some(idx) = self.matches[prev] {
            return Ok(Err(idx.get()));
        }
        for &b in bytes.iter() {
            match self.states[prev].trans.binary_search_by_key(&b, |t| t.0) {
                ...
                Err(i) => {
                    let next = self.create_state(allocation)?;
                    allocation.grow(&mut self.states[prev].trans, 1)?;
    ...
    fn root(&mut self, allocation: Allocator<'_>) -> Result<usize, AllocationError> {
        if !self.states.is_empty() {
            Ok(0)
        } else {
            self.create_state(allocation)
        }
    }

    /// Creates a new empty state and returns its ID.
    fn create_state(&mut self, allocation: Allocator<'_>) -> Result<usize, AllocationError> {
        let id = self.states.len();
        allocation.grow(&mut self.states, 1)?;
        allocation.grow(&mut self.matches, 1)?;
