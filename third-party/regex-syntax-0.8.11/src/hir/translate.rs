...
impl Translator {
    ...
    /// Translate with allocation admission. The funding owner must outlive
    /// translator storage, the returned expression and any error.
    pub fn translate_with_allocations(
        &mut self,
        pattern: &str,
        ast: &Ast,
        ...
        let mut translator = TranslatorI::new(self, pattern);
        translator.allocation = crate::allocation::Allocator::new(allocation);
        ast::visit(ast, translator)
    }
}
...
impl HirFrame {
    ...
    fn unwrap_expr(
        ...
        match self {
            HirFrame::Expr(expr) => Ok(expr),
            HirFrame::Literal(lit) => {
                Ok(Hir::literal_with_allocations(lit, allocation)?)
            }
...
impl<'t, 'p> Visitor for TranslatorI<'t, 'p> {
    ...
    fn reserve(&mut self, bytes: usize) -> Result<()> {
        Ok(self.allocation.reserve(bytes)?)
    }

    fn finish(self) -> Result<Hir> {
        // ... otherwise, we should have exactly one HIR on the stack.
        assert_eq!(self.trans().stack.borrow().len(), 1);
        Ok(self.pop().unwrap().unwrap_expr(self.allocation)?)
    }
    ...
    fn visit_post(&mut self, ast: &Ast) -> Result<()> {
        match *ast {
            Ast::Empty(_) => {
                self.push(HirFrame::Expr(Hir::empty_with_allocations(
                    self.allocation,
                )?))?;
            }
            Ast::Flags(ref x) => {
            ...
            Ast::ClassPerl(ref x) => {
                if self.flags().unicode() {
                    ...
                    self.push(HirFrame::Expr(Hir::class_with_allocations(
                        hcls,
                        self.allocation,
                    )?))?;
                } else {
            ...
            Ast::ClassBracketed(ref ast) => {
                if self.flags().unicode() {
                    ...
                    let expr = Hir::class_with_allocations(
            ...
            Ast::Repetition(ref x) => {
                let expr = self.pop().unwrap().unwrap_expr(self.allocation)?;
            ...
            Ast::Concat(_) => {
                ...
                while let Some(expr) = self.pop_concat_expr()? {
                    if !matches!(*expr.kind(), HirKind::Empty) {
                        self.allocation.push(&mut exprs, expr)?;
                    }
                }
                exprs.reverse();
                self.push(HirFrame::Expr(Hir::concat_with_allocations(
                    exprs,
                    self.allocation,
                )?))?;
            }
            Ast::Alternation(_) => {
                ...
                exprs.reverse();
                self.push(HirFrame::Expr(Hir::alternation_with_allocations(
                    exprs,
                    self.allocation,
                )?))?;
            }
    ...
    fn visit_class_set_item_post(
        ...
        match *ast {
            ...
            ast::ClassSetItem::Literal(ref x) => {
                if self.flags().unicode() {
                    let mut cls = self.pop().unwrap().unwrap_class_unicode();
                    cls.push_with_allocations(
                        hir::ClassUnicodeRange::new(x.c, x.c),
                        self.allocation,
                    )?;
                    self.push(HirFrame::ClassUnicode(cls))?;
                } else {
            ...
            ast::ClassSetItem::Range(ref x) => {
                if self.flags().unicode() {
                    let mut cls = self.pop().unwrap().unwrap_class_unicode();
                    cls.push_with_allocations(
                        hir::ClassUnicodeRange::new(x.start.c, x.end.c),
                        self.allocation,
                    )?;
                    self.push(HirFrame::ClassUnicode(cls))?;
                } else {
                    ...
                    cls.push_with_allocations(
                        hir::ClassBytesRange::new(start, end),
                        self.allocation,
                    )?;
                    self.push(HirFrame::ClassBytes(cls))?;
                }
            ...
            ast::ClassSetItem::Ascii(ref x) => {
                if self.flags().unicode() {
                    ...
                    let mut cls = self.pop().unwrap().unwrap_class_unicode();
                    cls.union_with_allocations(&xcls, self.allocation)?;
                    self.push(HirFrame::ClassUnicode(cls))?;
                } else {
            ...
            ast::ClassSetItem::Bracketed(ref ast) => {
                if self.flags().unicode() {
                    ...
                    cls2.union_with_allocations(&cls1, self.allocation)?;
    ...
    fn visit_class_set_binary_op_post(
        ...
        if self.flags().unicode() {
            ...
            if self.flags().case_insensitive() {
                rhs.try_case_fold_simple_with_allocations(self.allocation)
                    .map_err(|error| {
                        if let unicode::CaseFoldError::Allocation(error) =
                            error
                        {
                ...
                lhs.try_case_fold_simple_with_allocations(self.allocation)
                    .map_err(|error| {
                        if let unicode::CaseFoldError::Allocation(error) =
                            error
                        {
            ...
            match op.kind {
                Intersection => {
                    lhs.intersect_with_allocations(&rhs, self.allocation)?
                }
                Difference => {
                    lhs.difference_with_allocations(&rhs, self.allocation)?
                }
                SymmetricDifference => lhs
                    .symmetric_difference_with_allocations(
                        &rhs,
                        self.allocation,
                    )?,
            }
            cls.union_with_allocations(&lhs, self.allocation)?;
            self.push(HirFrame::ClassUnicode(cls))?;
        } else {
            ...
            let mut cls = self.pop().unwrap().unwrap_class_bytes();
            if self.flags().case_insensitive() {
                rhs.case_fold_simple_with_allocations(self.allocation)?;
                lhs.case_fold_simple_with_allocations(self.allocation)?;
            }
            match op.kind {
                Intersection => {
                    lhs.intersect_with_allocations(&rhs, self.allocation)?
...
struct TranslatorI<'t, 'p> {
    trans: &'t Translator,
    pattern: &'p str,
    allocation: crate::allocation::Allocator<'p>,
}
...
impl<'t, 'p> TranslatorI<'t, 'p> {
    ...
    fn new(trans: &'t Translator, pattern: &'p str) -> TranslatorI<'t, 'p> {
        TranslatorI {
            trans,
            pattern,
            allocation: crate::allocation::Allocator::unenforced(),
        }
    }
    ...
    fn push(&self, frame: HirFrame) -> Result<()> {
        Ok(self
            .allocation
            .push(&mut self.trans().stack.borrow_mut(), frame)?)
    }
    ...
    fn push_byte(&self, byte: u8) -> Result<()> {
        let mut stack = self.trans().stack.borrow_mut();
        if let Some(HirFrame::Literal(ref mut literal)) = stack.last_mut() {
            self.allocation.push(literal, byte)?;
        } else {
            self.allocation.push(
                &mut stack,
                HirFrame::Literal(self.allocation.copy_slice(&[byte])?),
            )?;
        }
        Ok(())
    }
    ...
    fn pop_concat_expr(&self) -> Result<Option<Hir>> {
        ...
        Ok(match frame {
            ...
            HirFrame::Literal(lit) => {
                Some(Hir::literal_with_allocations(lit, self.allocation)?)
            }
    ...
    fn pop_alt_expr(&self) -> Result<Option<Hir>> {
        let Some(frame) = self.pop() else {
            return Ok(None);
        };
        Ok(match frame {
    ...
    fn error(&self, span: Span, kind: ErrorKind) -> Error {
        match self.allocation.copy_str(self.pattern) {
            Ok(pattern) => Error {
                kind,
                pattern,
                span,
    ...
    fn case_fold_char(&self, span: Span, c: char) -> Result<Option<Hir>> {
        ...
        if self.flags().unicode() {
            ...
            let mut cls = hir::ClassUnicode::new_with_allocations(
                [hir::ClassUnicodeRange::new(c, c)],
                self.allocation,
            )?;
            cls.try_case_fold_simple_with_allocations(self.allocation)
                .map_err(|error| {
                    if let unicode::CaseFoldError::Allocation(error) = error {
                        return Error::from(error);
                    }
                    self.error(span, ErrorKind::UnicodeCaseUnavailable)
                })?;
            Ok(Some(Hir::class_with_allocations(
                hir::Class::Unicode(cls),
                self.allocation,
            )?))
        } else {
            ...
            let mut cls = hir::ClassBytes::new_with_allocations(
            ...
            cls.case_fold_simple_with_allocations(self.allocation)?;
            Ok(Some(Hir::class_with_allocations(
                hir::Class::Bytes(cls),
                self.allocation,
            )?))
    ...
    fn hir_dot(&self, span: Span) -> Result<Hir> {
        ...
        Ok(Hir::dot_with_allocations(dot, self.allocation)?)
    }
    ...
    fn hir_assertion(&self, asst: &ast::Assertion) -> Result<Hir> {
        ...
        Ok(match asst.kind {
            ast::AssertionKind::StartLine => Hir::look_with_allocations(
                if multi_line {
                    if crlf {
                        hir::Look::StartCRLF
                    } else {
            ...
            ast::AssertionKind::EndLine => Hir::look_with_allocations(
                if multi_line {
                    if crlf {
                        hir::Look::EndCRLF
                    } else {
            ...
            ast::AssertionKind::StartText => {
                Hir::look_with_allocations(hir::Look::Start, self.allocation)?
            }
            ast::AssertionKind::EndText => {
                Hir::look_with_allocations(hir::Look::End, self.allocation)?
            }
            ast::AssertionKind::WordBoundary => Hir::look_with_allocations(
                if unicode {
            ...
            ast::AssertionKind::NotWordBoundary => Hir::look_with_allocations(
                if unicode {
    ...
    fn hir_capture(&self, group: &ast::Group, expr: Hir) -> Result<Hir> {
        let (index, name) = match group.kind {
            ...
            ast::GroupKind::CaptureName { ref name, .. } => (
                ...
                Some(
                    self.allocation
                        .boxed_str(self.allocation.copy_str(&name.name)?)?,
                ),
            ),
            ...
            ast::GroupKind::NonCapturing(_) => return Ok(expr),
        };
        Ok(Hir::capture_with_allocations(
            hir::Capture {
                index,
                name,
                sub: self.allocation.boxed(expr)?,
            },
            self.allocation,
        )?)
    }

    fn hir_repetition(&self, rep: &ast::Repetition, expr: Hir) -> Result<Hir> {
        ...
        Ok(Hir::repetition_with_allocations(
            hir::Repetition {
                min,
                max,
                greedy,
    ...
    fn hir_unicode_class(
        ...
        let mut result = self.convert_unicode_class_error(
            &ast_class.span,
            unicode::class_with_allocations(query, self.allocation),
        );
    ...
    fn hir_perl_unicode_class(
        ...
        let result = match ast_class.kind {
            Digit => unicode::perl_digit_with_allocations(self.allocation),
            Space => unicode::perl_space_with_allocations(self.allocation),
            Word => unicode::perl_word_with_allocations(self.allocation),
        };
        let mut class =
        ...
        if ast_class.negated {
            class.negate_with_allocations(self.allocation)?;
        }
        Ok(class)
    }
    ...
    fn convert_unicode_class_error(
        ...
        result.map_err(|err| {
            let sp = span.clone();
            match err {
                unicode::Error::Allocation(error) => error.into(),
    ...
    fn unicode_fold_and_negate(
        ...
        if self.flags().case_insensitive() {
            class
                .try_case_fold_simple_with_allocations(self.allocation)
                .map_err(|error| {
                    if let unicode::CaseFoldError::Allocation(error) = error {
                        return Error::from(error);
                    }
    ...
    fn bytes_fold_and_negate(
        ...
        // Unicode scalar value.
        if self.flags().case_insensitive() {
            class.case_fold_simple_with_allocations(self.allocation)?;
        }
        if negated {
            class.negate_with_allocations(self.allocation)?;
        }
...
fn hir_ascii_class_bytes(
    kind: &ast::ClassAsciiKind,
    allocation: crate::allocation::Allocator<'_>,
) -> Result<hir::ClassBytes> {
    let ranges =
        ascii_class(kind).map(|(s, e)| hir::ClassBytesRange::new(s, e));
    Ok(hir::ClassBytes::new_with_allocations(ranges, allocation)?)
}
