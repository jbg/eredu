...
pub trait Visitor {
    ...
    /// Fund prospective visitor stack capacity before allocation.
    /// The default is the ordinary unenforced policy.
    fn reserve(&mut self, _bytes: usize) -> Result<(), Self::Err> {
        Ok(())
    }
...
impl<'a> HeapVisitor<'a> {
    fn grow<T, V: Visitor>(
        ...
        if values.len() == values.capacity() {
            ...
            let bytes =
                capacity.checked_mul(core::mem::size_of::<T>()).unwrap();
            visitor.reserve(bytes)?;
    ...
    fn visit<V: Visitor>(
        ...
        loop {
            ...
            if let Some(x) = self.induct(ast, &mut visitor)? {
                let child = x.child();
                Self::grow(&mut self.stack, &mut visitor)?;
    ...
    fn visit_class<V: Visitor>(
        ...
        loop {
            ...
            if let Some(x) = self.induct_class(&ast) {
                let child = x.child();
                Self::grow(&mut self.stack_class, visitor)?;
