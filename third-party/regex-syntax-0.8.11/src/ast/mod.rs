        ...
        while let Some(mut ast) = stack.pop() {
            match ast {
                ...
                Ast::Repetition(ref mut x) => {
                    crate::allocation::retirement_push(&mut stack, mem::replace(&mut x.ast, empty_ast()));
                }
                Ast::Group(ref mut x) => {
                    crate::allocation::retirement_push(&mut stack, mem::replace(&mut x.ast, empty_ast()));
                }
                Ast::Alternation(ref mut x) => {
                    for child in x.asts.drain(..) { crate::allocation::retirement_push(&mut stack, child); }
                }
            ...
            match set {
                ClassSet::Item(ref mut item) => match *item {
                    ...
                    ClassSetItem::Bracketed(ref mut x) => {
                        crate::allocation::retirement_push(
                            &mut stack,
                            mem::replace(&mut x.kind, empty_set()),
                        );
                    }
                    ClassSetItem::Union(ref mut x) => {
                        for child in x.items.drain(..) {
                            crate::allocation::retirement_push(
                                &mut stack,
                                ClassSet::Item(child),
                            );
                ...
                ClassSet::BinaryOp(ref mut op) => {
                    crate::allocation::retirement_push(
                        &mut stack,
                        mem::replace(&mut op.lhs, empty_set()),
                    );
                    crate::allocation::retirement_push(
                        &mut stack,
                        mem::replace(&mut op.rhs, empty_set()),
                    );
                }
            }
        }
