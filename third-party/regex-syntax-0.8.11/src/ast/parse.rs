...
struct ParserI<'s, P> {
    ...
    pattern: &'s str,
    allocation: crate::allocation::Allocator<'s>,
}
...
impl Parser {
    ...
    /// Parse using a borrowed allocation policy. The funding owner must
    /// outlive the resulting tree, parser storage and any returned error.
    pub fn parse_with_allocations(
        &mut self,
        pattern: &str,
        allocation: &dyn crate::allocation::Allocation,
    ) -> Result<Ast> {
        self.comments = RefCell::new(Vec::new());
        self.stack_group = RefCell::new(Vec::new());
        self.stack_class = RefCell::new(Vec::new());
        self.capture_names = RefCell::new(Vec::new());
        self.scratch = RefCell::new(String::new());
        let mut parser = ParserI::new(self, pattern);
        parser.allocation = crate::allocation::Allocator::new(allocation);
        parser.parse()
    }
...
impl<'s, P: Borrow<Parser>> ParserI<'s, P> {
    ...
    fn error(&self, span: Span, kind: ast::ErrorKind) -> ast::Error {
        match self.allocation.copy_str(self.pattern()) {
            Ok(pattern) => ast::Error {
                kind,
                pattern,
                span,
            },
            Err(error) => self.allocation_error(error),
        }
    }

    fn allocation_error(
        &self,
        error: crate::allocation::AllocationError,
    ) -> ast::Error {
        ast::Error {
            kind: ast::ErrorKind::Allocation(error),
            pattern: String::new(),
    ...
    fn reserve(&self, bytes: usize) -> Result<()> {
        self.allocation
            .reserve(bytes)
            .map_err(|e| self.allocation_error(e))
    }

    fn boxed<T>(&self, value: T) -> Result<Box<T>> {
        self.allocation
            .boxed(value)
            .map_err(|e| self.allocation_error(e))
    }

    fn push<T>(&self, values: &mut Vec<T>, value: T) -> Result<()> {
        self.allocation
            .push(values, value)
            .map_err(|e| self.allocation_error(e))
    }

    fn push_char(&self, value: &mut String, c: char) -> Result<()> {
        self.allocation
            .push_char(value, c)
            .map_err(|e| self.allocation_error(e))
    }

    fn copy_str(&self, value: &str) -> Result<String> {
        self.allocation
            .copy_str(value)
            .map_err(|e| self.allocation_error(e))
    }
    ...
    fn ast_node<T>(
        &self,
        value: T,
        make: impl FnOnce(T) -> Ast,
    ) -> Result<Ast> {
        // Every AST node funds its box plus one share of the iterative drop
        // stack (<4 slots per node) and a possible replacement empty span.
        self.reserve_ast::<T>()?;
        Ok(make(value))
    }

    fn reserve_ast<T>(&self) -> Result<()> {
        self.reserve(
            core::mem::size_of::<T>()
                + 4 * core::mem::size_of::<Ast>()
                + core::mem::size_of::<Span>(),
        )
    }

    fn concat_ast(&self, concat: ast::Concat) -> Result<Ast> {
        match concat.asts.len() {
            0 => self.reserve_ast::<Span>()?,
            1 => {}
            _ => self.reserve_ast::<ast::Concat>()?,
        }
        Ok(concat.into_ast())
    }

    fn alternation_ast(&self, alt: ast::Alternation) -> Result<Ast> {
        match alt.asts.len() {
            0 => self.reserve_ast::<Span>()?,
            1 => {}
            _ => self.reserve_ast::<ast::Alternation>()?,
        }
        Ok(alt.into_ast())
    }

    fn primitive_ast(&self, primitive: Primitive) -> Result<Ast> {
        match primitive {
            Primitive::Literal(x) => self.ast_node(x, Ast::literal),
            Primitive::Assertion(x) => self.ast_node(x, Ast::assertion),
            Primitive::Dot(x) => self.ast_node(x, Ast::dot),
            Primitive::Perl(x) => self.ast_node(x, Ast::class_perl),
            Primitive::Unicode(x) => self.ast_node(x, Ast::class_unicode),
        }
    }

    fn class_box<T>(&self, value: T) -> Result<Box<T>> {
        self.reserve(4 * core::mem::size_of::<ast::ClassSet>())?;
        self.boxed(value)
    }
    ...
    fn flags_add(
        ...
        if let Some(i) = flags.items.iter().position(|x| x.kind == item.kind) {
            return Ok(Some(i));
        }
        self.push(&mut flags.items, item)?;
        Ok(None)
    }

    fn union_push(
        &self,
        union: &mut ast::ClassSetUnion,
        item: ast::ClassSetItem,
    ) -> Result<()> {
        self.reserve(4 * core::mem::size_of::<ast::ClassSet>())?;
        self.allocation
            .grow(&mut union.items, 1)
            .map_err(|e| self.allocation_error(e))?;
        union.push(item);
        Ok(())
    }
            ...
            .binary_search_by_key(&cap.name.as_str(), |c| c.name.as_str())
        {
            Err(i) => {
                self.allocation
                    .grow(&mut names, 1)
                    .map_err(|e| self.allocation_error(e))?;
                let copy = ast::CaptureName {
                    name: self.copy_str(&cap.name)?,
                    span: cap.span,
                    index: cap.index,
                };
                names.insert(i, copy);
    ...
    /// EOF.
    fn bump_and_bump_space(&self) -> Result<bool> {
        if !self.bump() {
            return Ok(false);
        }
        self.bump_space()?;
        ...
        while !self.is_eof() {
            if self.char().is_whitespace() {
                ...
                while !self.is_eof() {
                    ...
                    self.push_char(&mut comment_text, c)?;
                }
                let comment = ast::Comment {
                    span: Span::new(start, self.pos()),
                    comment: comment_text,
                };
                self.push(&mut self.parser().comments.borrow_mut(), comment)?;
            } else {
                break;
            }
        }
    ...
    fn push_or_add_alternation(&self, concat: ast::Concat) -> Result<()> {
        use self::GroupState::*;

        let mut stack = self.parser().stack_group.borrow_mut();
        if let Some(&mut Alternation(ref mut alts)) = stack.last_mut() {
            self.push(&mut alts.asts, self.concat_ast(concat)?)?;
            return Ok(());
        }
        let span = Span::new(concat.span.start, self.pos());
        let mut asts = Vec::new();
        self.push(&mut asts, self.concat_ast(concat)?)?;
        self.push(&mut stack, Alternation(ast::Alternation { span, asts }))?;
        Ok(())
    }
    ...
    fn push_group(&self, mut concat: ast::Concat) -> Result<ast::Concat> {
        ...
        match self.parse_group()? {
            Either::Left(set) => {
                ...
                self.push(&mut concat.asts, self.ast_node(set, Ast::flags)?)?;
                Ok(concat)
            }
            Either::Right(group) => {
                ...
                self.push(
                    &mut self.parser().stack_group.borrow_mut(),
                    GroupState::Group {
                        concat,
                        group,
    ...
    fn pop_group(&self, mut group_concat: ast::Concat) -> Result<ast::Concat> {
        ...
        match alt {
            Some(mut alt) => {
                ...
                self.push(&mut alt.asts, self.concat_ast(group_concat)?)?;
                group.ast = self.boxed(self.alternation_ast(alt)?)?;
            }
            None => {
                group.ast = self.boxed(self.concat_ast(group_concat)?)?;
            }
        }
        self.push(&mut prior_concat.asts, self.ast_node(group, Ast::group)?)?;
        Ok(prior_concat)
    }
    ...
    fn pop_group_end(&self, mut concat: ast::Concat) -> Result<Ast> {
        ...
        let ast = match stack.pop() {
            None => self.concat_ast(concat),
            Some(GroupState::Alternation(mut alt)) => {
                alt.span.end = self.pos();
                self.push(&mut alt.asts, self.concat_ast(concat)?)?;
                self.ast_node(alt, Ast::alternation)
            }
    ...
    fn push_class_open(
        ...
        self.push(
            &mut self.parser().stack_class.borrow_mut(),
            ClassState::Open {
                union: parent_union,
                set: nested_set,
            },
    ...
    fn pop_class(
        ...
        let item = ast::ClassSet::Item(self.union_item(nested_union)?);
        ...
        match stack.pop() {
            ...
            Some(ClassState::Open { mut union, mut set }) => {
                ...
                if stack.is_empty() {
                    ...
                    self.union_push(
                        &mut union,
                        ast::ClassSetItem::Bracketed(self.class_box(set)?),
                    )?;
    ...
    fn push_class_op(
        ...
        let item = ast::ClassSet::Item(self.union_item(next_union)?);
    ...
    #[inline(never)]
    fn pop_class_op(&self, rhs: ast::ClassSet) -> Result<ast::ClassSet> {
        let mut stack = self.parser().stack_class.borrow_mut();
        ...
        self.reserve(4 * core::mem::size_of::<ast::ClassSet>())?;
        Ok(ast::ClassSet::BinaryOp(ast::ClassSetBinaryOp {
            span,
            kind,
            lhs: self.boxed(lhs)?,
            rhs: self.boxed(rhs)?,
...
impl<'s, P: Borrow<Parser>> ParserI<'s, P> {
        ...
        loop {
            ...
            match self.char() {
                ...
                '[' => {
                    ...
                    self.push(
                        &mut concat.asts,
                        self.ast_node(class, Ast::class_bracketed)?,
                    )?;
                }
                '?' => {
                ...
                _ => self.push(
                    &mut concat.asts,
                    self.primitive_ast(self.parse_primitive()?)?,
                )?,
            }
        }
    ...
    fn parse_group(&self) -> Result<Either<ast::SetFlags, ast::Group>> {
        ...
        if self.bump_if("?P<") || {
            ...
            Ok(Either::Right(ast::Group {
                ...
                ast: self.boxed(self.ast_node(self.span(), Ast::empty)?)?,
            }))
            ...
            if char_end == ')' {
                ...
                Ok(Either::Right(ast::Group {
                    ...
                    ast: self
                        .boxed(self.ast_node(self.span(), Ast::empty)?)?,
                }))
            }
        } else {
            let capture_index = self.next_capture_index(open_span)?;
    ...
    fn parse_flags(&self) -> Result<ast::Flags> {
        ...
        while self.char() != ':' && self.char() != ')' {
            if self.char() == '-' {
                ...
                if let Some(i) = self.flags_add(&mut flags, item)? {
                    return Err(self.error(
                        self.span_char(),
    ...
    fn maybe_parse_special_word_boundary(
        ...
        while !self.is_eof() && is_valid_char(self.char()) {
            self.push_char(&mut scratch, self.char())?;
            ...
            if let Some(i) = name.find("!=") {
                (
                    ...
                    ast::ClassUnicodeKind::NamedValue {
                        ...
                        name: self.copy_str(&name[..i])?,
                        value: self.copy_str(&name[i + 2..])?,
                    },
                )
                ...
                (start, ast::ClassUnicodeKind::Named(self.copy_str(&name)?))
            }
        } else {
            let start = self.pos();
            let c = self.char();
