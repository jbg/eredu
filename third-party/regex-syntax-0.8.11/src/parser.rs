...
impl Parser {
    ...
    /// Parse with allocation admission on the shared AST and HIR workers.
    /// The funding owner must outlive this parser, its returned HIR and errors.
    pub fn parse_with_allocations(
        &mut self,
        pattern: &str,
        allocation: &dyn crate::allocation::Allocation,
    ) -> Result<hir::Hir, Error> {
        let ast = self.ast.parse_with_allocations(pattern, allocation)?;
        let hir = self
            .hir
            .translate_with_allocations(pattern, &ast, allocation)?;
        Ok(hir)
    }
}
