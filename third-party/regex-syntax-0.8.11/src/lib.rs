...
pub fn escape_into(text: &str, buf: &mut String) {
    escape_into_with_allocations(text, buf, &allocation::Unenforced)
        .expect("regex literal allocation");
}

/// Appends the same escaped literal after funding its complete destination.
pub fn escape_into_with_allocations(
    text: &str,
    buf: &mut String,
    policy: &dyn allocation::Allocation,
) -> Result<(), allocation::AllocationError> {
    use allocation::{AllocationError, Allocator};
    let extra = text.chars().filter(|&c| is_meta_character(c)).count();
    let required = buf.len().checked_add(text.len())
        .and_then(|n| n.checked_add(extra)).ok_or(AllocationError::SizeOverflow)?;
    if required > buf.capacity() {
        Allocator::new(policy).reserve(required)?;
        buf.try_reserve_exact(required - buf.len()).map_err(|_| AllocationError::HostAllocation)?;
    }
    for c in text.chars() {
...
/// Return the other scalars in this character's Unicode simple case-fold class.
///
/// The result borrows the same static table used by HIR case folding and does
/// not allocate a temporary character class. The character itself is omitted.
/// An unavailable Unicode case table is reported through the existing fixed error.
pub fn simple_case_fold(character: char) -> Result<&'static [char], hir::CaseFoldError> {
    Ok(unicode::SimpleCaseFolder::new()?.mapping(character))
}
...
mod tests {
    ...
    #[test]
    fn escape_funds_complete_destination_before_mutating_existing_text() {
        use core::cell::Cell;
        use allocation::{Allocation, AllocationError};
        struct Funding { request: Cell<usize>, refuse: bool }
        impl Allocation for Funding {
            fn reserve(&self, bytes: usize) -> Result<(), AllocationError> {
                assert_eq!(self.request.replace(bytes), 0);
                if self.refuse { Err(AllocationError::Refused) } else { Ok(()) }
            }
        }
        let literal = "é[.*]Σ";
        let expected = "prefix:é\\[\\.\\*\\]Σ";
        for refuse in [false, true] {
            let funding = Funding { request: Cell::new(0), refuse };
            let mut output = String::from("prefix:");
            let result = escape_into_with_allocations(literal, &mut output, &funding);
            assert_eq!(funding.request.get(), expected.len());
            if refuse {
                assert_eq!(result, Err(AllocationError::Refused));
                assert_eq!(output, "prefix:");
            } else { result.unwrap(); assert_eq!(output, expected); }
        }
        let funding = Funding { request: Cell::new(0), refuse: true };
        let mut output = String::with_capacity(expected.len());
        output.push_str("prefix:");
        escape_into_with_allocations(literal, &mut output, &funding).unwrap();
        assert_eq!(output, expected);
        assert_eq!(funding.request.get(), 0);
    }
