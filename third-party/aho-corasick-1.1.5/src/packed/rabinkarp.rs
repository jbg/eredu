...
impl RabinKarp {
    pub(crate) fn visit_source_storage(
        ...
        if crate::source_storage::vector(&self.buckets, visitor) {
    ...
    pub(crate) fn new(
    ...
    ) -> Result<RabinKarp, AllocationError> {
        ...
        allocation.grow(&mut buckets, NUM_BUCKETS)?;
        ...
        for (id, pat) in patterns.iter() {
            ...
            allocation.push(&mut rk.buckets[bucket], (hash, id))?;
        }
        Ok(rk)
    }
