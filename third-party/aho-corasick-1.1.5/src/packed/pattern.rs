use crate::allocation::{sort_by, AllocationError, Allocator};
...
impl Patterns {
    pub(crate) fn visit_source_storage(
        ...
        if crate::source_storage::vector(&self.by_id, visitor) {
            for bytes in &self.by_id {
                crate::source_storage::vector(bytes, visitor);
            }
        }
        crate::source_storage::vector(&self.order, visitor);
    }
    ...
    pub(crate) fn add(
        ...
        let bytes = allocation.copy_slice(bytes)?;
        allocation.grow(&mut self.order, 1)?;
        allocation.grow(&mut self.by_id, 1)?;
        self.order.push(id);
        let len = bytes.len();
        self.by_id.push(bytes);
        self.minimum_len = cmp::min(self.minimum_len, len);
        self.total_pattern_bytes += len;
        Ok(())
    }
    ...
    pub(crate) fn set_match_kind(
        ...
        match self.kind {
            MatchKind::LeftmostFirst => {
                sort_by(&mut self.order, |a, b| a.cmp(b), allocation)?;
            }
            MatchKind::LeftmostLongest => {
                let (order, by_id) = (&mut self.order, &mut self.by_id);
                // Equal lengths retain insertion precedence, as in the original
                // stable sort. IDs are the original insertion order.
                sort_by(
                    order,
                    |&id1, &id2| {
                        by_id[id1]
    ...
    pub(crate) fn clone_with_allocations(
        &self,
        allocation: Allocator<'_>,
    ) -> Result<Self, AllocationError> {
        allocation.reserve(
            core::mem::size_of::<Self>()
                + core::mem::size_of::<Vec<Vec<u8>>>(),
        )?;
        let mut by_id = Vec::new();
        allocation.grow(&mut by_id, self.by_id.len())?;
        for bytes in &self.by_id {
            by_id.push(allocation.copy_slice(bytes)?);
        }
        Ok(Self {
            kind: self.kind,
            by_id,
            order: allocation.copy_slice(&self.order)?,
            minimum_len: self.minimum_len,
            total_pattern_bytes: self.total_pattern_bytes,
        })
    }
...
impl<'p> Pattern<'p> {
    ...
    /// Pack the same at-most-four low nybbles without allocating a key box.
    pub(crate) fn low_nybbles(&self, len: usize) -> u16 {
        assert!(len <= 4 && len <= self.len());
        self.bytes()
            .iter()
            .take(len)
            .fold(0, |key, &byte| (key << 4) | u16::from(byte & 0xF))
    }
