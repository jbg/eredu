...
impl<V: Vector, const BYTES: usize> Slim<V, BYTES> {
    pub(crate) fn visit_source_storage(
        ...
        self.teddy.visit_source_storage(visitor);
    }
    ...
    ) -> Result<Slim<V, BYTES>, AllocationError> {
        allocation.reserve(
            core::mem::size_of::<Self>()
                + core::mem::size_of::<[SlimMaskBuilder; BYTES]>()
                + core::mem::size_of::<[Mask<V>; BYTES]>(),
        )?;
        assert!(
            1 <= BYTES && BYTES <= 4,
            "only 1, 2, 3 or 4 bytes are supported"
        );
        let teddy = Teddy::new(patterns, allocation)?;
...
impl<V: FatVector, const BYTES: usize> Fat<V, BYTES> {
    ...
    pub(crate) unsafe fn new(
        patterns: Arc<Patterns>,
        allocation: Allocator<'_>,
    ) -> Result<Fat<V, BYTES>, AllocationError> {
        allocation.reserve(
            core::mem::size_of::<Self>()
                + core::mem::size_of::<[FatMaskBuilder; BYTES]>()
                + core::mem::size_of::<[Mask<V>; BYTES]>(),
        )?;
        assert!(
            1 <= BYTES && BYTES <= 4,
...
impl<const BUCKETS: usize> Teddy<BUCKETS> {
    fn visit_source_storage(
        ...
        for bucket in &self.buckets {
            crate::source_storage::vector(bucket, visitor);
        }
    }
    ...
    fn new(
        patterns: Arc<Patterns>,
        allocation: Allocator<'_>,
    ) -> Result<Teddy<BUCKETS>, AllocationError> {
        ...
        allocation.reserve(
            core::mem::size_of::<Self>()
                + core::mem::size_of::<Vec<(u16, usize)>>(),
        )?;
        let buckets = core::array::from_fn(|_| Vec::new());
        let mut t = Teddy { patterns, buckets };

        // Keys contain at most four nybbles; the packed source admits at most
        // 128 patterns. Keep this tiny membership table sorted, without boxed
        // keys or opaque tree nodes. Bucket assignment remains insertion based.
        let mut map: Vec<(u16, usize)> = Vec::new();
        for (id, pattern) in t.patterns.iter() {
            ...
            if let Ok(index) =
                map.binary_search_by_key(&lonybs, |&(key, _)| key)
            {
                let bucket = map[index].1;
                allocation.push(&mut t.buckets[bucket], id)?;
            } else {
                ...
                allocation.push(&mut t.buckets[bucket], id)?;
                let index = map
                    .binary_search_by_key(&lonybs, |&(key, _)| key)
                    .unwrap_err();
                allocation.grow(&mut map, 1)?;
                map.insert(index, (lonybs, bucket));
            }
        }
        Ok(t)
    }
...
impl SlimMaskBuilder {
    ...
    unsafe fn from_teddy<const BYTES: usize, V: Vector>(
        ...
        let mut mask_builders: [SlimMaskBuilder; BYTES] =
            core::array::from_fn(|_| SlimMaskBuilder::default());
        ...
        mask_builders.map(|builder| builder.build())
    }
}
...
impl FatMaskBuilder {
    ...
    unsafe fn from_teddy<const BYTES: usize, V: Vector>(
        ...
        // MSRV(1.63): Use core::array::from_fn to just build the array here
        // instead of creating a vector and turning it into an array.
        let mut mask_builders: [FatMaskBuilder; BYTES] =
            core::array::from_fn(|_| FatMaskBuilder::default());
        for (bucket_index, bucket) in teddy.buckets.iter().enumerate() {
            for pid in bucket.iter().copied() {
                let pat = teddy.patterns.get(pid);
                for (i, builder) in mask_builders.iter_mut().enumerate() {
