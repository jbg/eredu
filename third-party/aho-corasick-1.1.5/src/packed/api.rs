...
impl Builder {
    ...
    pub fn build(&self) -> Option<Searcher> {
        self.build_with_allocations(&Unenforced)
            .expect("ordinary packed construction allocation failed")
    }

    /// Build the same selected packed searcher with prospective admission.
    pub fn build_with_allocations(
        &self,
        funding: &dyn Allocation,
    ) -> Result<Option<Searcher>, AllocationError> {
        let allocation = Allocator::new(funding);
        allocation.reserve(
            core::mem::size_of::<Searcher>()
                + core::mem::size_of::<Patterns>(),
        )?;
        if self.inert || self.patterns.is_empty() {
            return Ok(None);
        }
        let mut patterns = self.patterns.clone_with_allocations(allocation)?;
        patterns.set_match_kind(self.config.kind, allocation)?;
        let patterns = allocation.arc(patterns)?;
        let rabinkarp = RabinKarp::new(&patterns, allocation)?;
        ...
        let (search_kind, minimum_len) = match self.config.force {
            None | Some(ForceAlgorithm::Teddy) => {
                ...
                let teddy = match self
                    .build_teddy(Arc::clone(&patterns), allocation)?
                {
                    None => return Ok(None),
                    Some(teddy) => teddy,
                };
        ...
        Ok(Some(Searcher { patterns, rabinkarp, search_kind, minimum_len }))
    }
    ...
    ) -> Result<Option<teddy::Searcher>, AllocationError> {
        teddy::Builder::new()
    ...
    pub fn add<P: AsRef<[u8]>>(&mut self, pattern: P) -> &mut Builder {
        self.add_with_allocations(pattern, &Unenforced)
            .expect("ordinary packed pattern allocation failed")
    }

    /// Add one pattern after admitting its owned spelling and index rows.
    pub fn add_with_allocations<P: AsRef<[u8]>>(
        &mut self,
        pattern: P,
        funding: &dyn Allocation,
    ) -> Result<&mut Builder, AllocationError> {
        if self.inert {
            return Ok(self);
        ...
        self.patterns.add(pattern, Allocator::new(funding))?;
        Ok(self)
    }
    ...
    where
        ...
        P: AsRef<[u8]>,
    {
        self.extend_with_allocations(patterns, &Unenforced)
            .expect("ordinary packed pattern allocation failed")
    }

    /// Add patterns on the original worker with prospective source funding.
    pub fn extend_with_allocations<I, P>(
        &mut self,
        patterns: I,
        funding: &dyn Allocation,
    ) -> Result<&mut Builder, AllocationError>
    ...
    {
        for p in patterns {
            self.add_with_allocations(p, funding)?;
        }
        Ok(self)
    }
...
impl Searcher {
    /// Visit actual packed source allocations, excluding this inline value.
    /// Shared patterns are visited separately from strategy and bucket backing.
    pub fn visit_source_storage(
        &self,
        visitor: &mut dyn FnMut(*const (), usize) -> bool,
    ) {
        if crate::source_storage::arc(&self.patterns, visitor) {
            self.patterns.visit_source_storage(visitor);
        }
        self.rabinkarp.visit_source_storage(visitor);
        if let SearchKind::Teddy(teddy) = &self.search_kind {
            teddy.visit_source_storage(visitor);
        }
    }
