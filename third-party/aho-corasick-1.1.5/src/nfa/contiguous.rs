...
unsafe impl Automaton for NFA {
    ...
    fn visit_source_storage(
        ...
        crate::source_storage::vector(&self.repr, visitor);
...
impl<'a> State<'a> {
    ...
    fn write(
        ...
        // This exact encoded row includes transitions, header and all matches.
        // Admit the whole write before emitting its first word.
        let trans_words = if kind == State::KIND_DENSE {
            2 + classes.alphabet_len()
        } else if kind == State::KIND_ONE {
            3
        } else {
            2 + u32_len(old_len) + old_len
        };
        let matches =
            if old.is_match() { nnfa.iter_matches(oldsid).count() } else { 0 };
        let match_words = if matches <= 1 {
            matches
        } else {
            matches.checked_add(1).ok_or(AllocationError::SizeOverflow)?
        };
        let words = trans_words
            .checked_add(match_words)
            .ok_or(AllocationError::SizeOverflow)?;
        allocation.grow(dst, words)?;
        if kind == State::KIND_DENSE {
            dst.push(kind);
...
impl Builder {
    ...
    pub fn build_from_noncontiguous_with_allocations(
        ...
        let mut index_to_state_id = Vec::new();
        allocation.resize_copy(
            &mut index_to_state_id,
            nnfa.states().len(),
            NFA::DEAD,
        )?;
        let mut nfa = NFA {
        ...
        allocation.shrink(&mut nfa.repr)?;
        allocation.shrink(&mut nfa.pattern_lens)?;
        Ok(nfa)
    }
