...
impl NFA {
    ...
    fn add_transition(
        ...
        if head == StateID::ZERO || byte < self.sparse[head].byte {
            let new_link = self.alloc_transition(allocation)?;
        ...
        if link_next == StateID::ZERO || byte < self.sparse[link_next].byte {
            let link = self.alloc_transition(allocation)?;
    ...
    fn add_match(
        ...
        let new_match_link = self.alloc_match(allocation)?;
    ...
    fn copy_matches(
        ...
        while link_src != StateID::ZERO {
            ...
            let matched =
                Match { pid: self.matches[link_src].pid, link: StateID::ZERO };
            allocation.push(&mut self.matches, matched)?;
    ...
    fn alloc_transition(
        ...
        allocation.push(&mut self.sparse, Transition::default())?;
        Ok(id)
    }
    ...
    fn alloc_match(
        ...
        allocation.push(&mut self.matches, Match::default())?;
        Ok(id)
    }
    ...
    fn alloc_dense_state(
        ...
        allocation.grow(&mut self.dense, self.byte_classes.alphabet_len())?;
        self.dense.extend(
...
unsafe impl Automaton for NFA {
    ...
    fn visit_source_storage(
        ...
        crate::source_storage::vector(&self.states, visitor);
        crate::source_storage::vector(&self.sparse, visitor);
        crate::source_storage::vector(&self.dense, visitor);
        crate::source_storage::vector(&self.matches, visitor);
        crate::source_storage::vector(&self.pattern_lens, visitor);
        if let Some(prefilter) = &self.prefilter {
            prefilter.visit_source_storage(visitor);
        }
    }
...
impl Builder {
    ...
    where
        ...
        P: AsRef<[u8]>,
    {
        self.build_with_allocations(patterns, &Unenforced)
    }

    /// Build on the original worker with prospective source funding.
    pub fn build_with_allocations<I, P>(
        &self,
        patterns: I,
        ...
        P: AsRef<[u8]>,
    {
        ...
        let nfa = Compiler::new(self, funding)?.compile(patterns)?;
        debug!(
...
impl<'a> Compiler<'a> {
    fn new(
        builder: &'a Builder,
        funding: &'a dyn Allocation,
    ) -> Result<Compiler<'a>, BuildError> {
        ...
        P: AsRef<[u8]>,
    {
        self.allocation.reserve(
            core::mem::size_of::<I::IntoIter>()
                + core::mem::size_of::<P>()
                + core::mem::size_of::<Result<NFA, BuildError>>(),
        )?;
        // Add dummy transition/match links, so that no valid link will point
        // to another link at index 0.
        self.allocation.push(&mut self.nfa.sparse, Transition::default())?;
        self.allocation.push(&mut self.nfa.matches, Match::default())?;
        ...
        self.allocation.push(&mut self.nfa.dense, NFA::DEAD)?;
        // the dead state, only used for leftmost and fixed to id==0
        self.nfa.alloc_state(0, self.allocation)?;
        ...
        self.nfa.special.start_unanchored_id =
            self.nfa.alloc_state(0, self.allocation)?;
        ...
        self.allocation.shrink(&mut self.nfa.states)?;
        ...
        self.shuffle()?;
        self.nfa.prefilter = self.prefilter.build(self.allocation)?;
        ...
        self.allocation.shrink(&mut self.nfa.sparse)?;
        self.allocation.shrink(&mut self.nfa.dense)?;
        self.allocation.shrink(&mut self.nfa.matches)?;
        self.allocation.shrink(&mut self.nfa.pattern_lens)?;
        Ok(self.nfa)
    }
        ...
        P: AsRef<[u8]>,
    {
        'PATTERNS: for (i, pat) in patterns.into_iter().enumerate() {
            ...
            self.allocation.push(&mut self.nfa.pattern_lens, patlen)?;
            ...
            if self.builder.prefilter {
                self.prefilter.add(pat, self.allocation)?;
            }
            ...
            for (depth, &b) in pat.iter().enumerate() {
                ...
                if next != NFA::FAIL {
                    ...
                    let next = self.nfa.alloc_state(depth, self.allocation)?;
                    self.nfa.add_transition(prev, b, next, self.allocation)?;
                    if self.builder.ascii_case_insensitive {
                        let b = opposite_ascii_case(b);
            ...
            self.nfa.add_match(prev, pid, self.allocation)?;
        }
        Ok(())
    }
    ...
    fn fill_failure_transitions(&mut self) -> Result<(), BuildError> {
        ...
        self.allocation.reserve(
            core::mem::size_of::<VecDeque<StateID>>()
                + core::mem::size_of::<QueuedSet>()
        ...
        while let Some(link) = self.nfa.next_link(start_uid, prev_link) {
            ...
            self.allocation.queue_push(&mut queue, t.next)?;
            seen.insert(t.next);
        ...
        while let Some(id) = queue.pop_front() {
            ...
            while let Some(link) = self.nfa.next_link(id, prev_link) {
                ...
                self.nfa.copy_matches(fail, t.next, self.allocation)?;
            }
    ...
    fn shuffle(&mut self) -> Result<(), BuildError> {
        ...
        // book-keeping for us.
        let mut remapper = Remapper::new(&self.nfa, 0, self.allocation)?;
        ...
        remapper.remap(&mut self.nfa, self.allocation)?;
        Ok(())
    }
    ...
    fn densify(&mut self) -> Result<(), BuildError> {
        for i in 0..self.nfa.states.len() {
            ...
            let dense = self.nfa.alloc_dense_state(self.allocation)?;
            let mut prev_link = None;
    ...
    fn queued_set(&self) -> Result<QueuedSet, AllocationError> {
        if self.builder.ascii_case_insensitive {
            QueuedSet::active(self.nfa.states.len(), self.allocation)
        } else {
            Ok(QueuedSet::inert())
        }
    }
    ...
    fn init_unanchored_start_state(&mut self) -> Result<(), BuildError> {
        ...
        self.nfa.init_full_state(start_uid, NFA::FAIL, self.allocation)?;
        self.nfa.init_full_state(start_aid, NFA::FAIL, self.allocation)?;
        Ok(())
    }
    ...
    fn set_anchored_start_state(&mut self) -> Result<(), BuildError> {
        ...
        self.nfa.copy_matches(start_uid, start_aid, self.allocation)?;
    ...
    fn add_dead_state_loop(&mut self) -> Result<(), BuildError> {
        self.nfa.init_full_state(NFA::DEAD, NFA::DEAD, self.allocation)?;
        Ok(())
    }
}
...
impl QueuedSet {
    ...
    fn active(
        states: usize,
        allocation: Allocator<'_>,
    ) -> Result<QueuedSet, AllocationError> {
        let mut set = Vec::new();
        allocation.resize_copy(&mut set, states, false)?;
        Ok(QueuedSet { set: Some(set) })
    }
    ...
    fn contains(&self, state_id: StateID) -> bool {
        match self.set {
            None => false,
            Some(ref set) => set[state_id.as_usize()],
        }
    }
}
