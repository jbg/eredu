...
impl AhoCorasickBuilder {
    ...
    /// Build the original selected automaton with prospective source admission.
    /// Allocation refusal never selects an alternative engine.
        ...
        P: AsRef<[u8]>,
    {
        ...
        allocation.reserve(core::mem::size_of::<AhoCorasick>())?;
        let nfa = self
            .nfa_noncontiguous
            .build_with_allocations(patterns, funding)?;
        let (aut, kind): (Arc<dyn AcAutomaton>, AhoCorasickKind) = match self
            .kind
        {
            None => {
                debug!(
                ...
                self.build_auto(nfa, allocation)?
            }
            Some(AhoCorasickKind::NoncontiguousNFA) => {
                debug!("forcefully chose noncontiguous NFA");
                (allocation.arc(nfa)?, AhoCorasickKind::NoncontiguousNFA)
            }
            Some(AhoCorasickKind::ContiguousNFA) => {
                debug!("forcefully chose contiguous NFA");
                let cnfa = self
                    .nfa_contiguous
                    .build_from_noncontiguous_with_allocations(
                        &nfa,
                        allocation.policy(),
                    )?;
                (allocation.arc(cnfa)?, AhoCorasickKind::ContiguousNFA)
            }
            Some(AhoCorasickKind::DFA) => {
                debug!("forcefully chose DFA");
                let dfa = self.dfa.build_from_noncontiguous_with_allocations(
                    &nfa,
                    allocation.policy(),
                )?;
                (allocation.arc(dfa)?, AhoCorasickKind::DFA)
            }
        };
    ...
    fn build_auto(
    ...
    ) -> Result<(Arc<dyn AcAutomaton>, AhoCorasickKind), BuildError> {
        ...
        if try_dfa {
            match self.dfa.build_from_noncontiguous_with_allocations(
                &nfa,
                allocation.policy(),
            ) {
                Ok(dfa) => {
                    debug!("chose a DFA");
                    return Ok((allocation.arc(dfa)?, AhoCorasickKind::DFA));
                }
                Err(_err) => {
                    if _err.allocation_error().is_some() {
                        return Err(_err);
                    }
                    debug!(
        ...
        match self.nfa_contiguous.build_from_noncontiguous_with_allocations(
            &nfa,
            allocation.policy(),
        ) {
            Ok(nfa) => {
        ...
        Ok((allocation.arc(nfa)?, AhoCorasickKind::NoncontiguousNFA))
    }
...
impl AhoCorasick {
    /// Visit the actual shared automaton shell and retained source allocations.
    /// The caller owns identity deduplication; false skips repeated owners.
    pub fn visit_source_storage(
        &self,
        visitor: &mut dyn FnMut(*const (), usize) -> bool,
    ) {
        if crate::source_storage::arc(&self.aut, visitor) {
            self.aut.as_ref().visit_source_storage(visitor);
        }
    }
}
