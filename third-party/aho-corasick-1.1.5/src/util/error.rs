...
impl From<AllocationError> for BuildError {
    fn from(error: AllocationError) -> Self {
        Self { kind: ErrorKind::Allocation(error) }
    }
}
impl BuildError {
    /// The fixed source allocation failure, if construction was refused.
    pub fn allocation_error(&self) -> Option<AllocationError> {
        match self.kind {
            ErrorKind::Allocation(error) => Some(error),
            _ => None,
        }
    }
...
impl core::fmt::Display for BuildError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self.kind {
            ErrorKind::Allocation(error) => error.fmt(f),
