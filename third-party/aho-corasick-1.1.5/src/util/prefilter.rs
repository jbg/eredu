...
impl Prefilter {
    /// Visit the shared filter shell and its actual owned source backing.
    pub fn visit_source_storage(
        &self,
        visitor: &mut dyn FnMut(*const (), usize) -> bool,
    ) {
        if crate::source_storage::arc(&self.finder, visitor) {
            self.finder.as_ref().visit_source_storage(visitor);
        }
    }
...
impl<P: PrefilterI + ?Sized> PrefilterI for Arc<P> {
    fn visit_source_storage(
        ...
        if crate::source_storage::arc(self, visitor) {
...
impl Builder {
    ...
    pub(crate) fn build(
        ...
        allocation.reserve(core::mem::size_of::<[Option<Prefilter>; 3]>())?;
        if !self.enabled {
        ...
        if !self.ascii_case_insensitive {
            if let Some(pre) = self.memmem.build(allocation)? {
        ...
        let (packed, patlen, minlen) = if self.ascii_case_insensitive {
            ...
            let packed = match self.packed.as_ref() {
                None => None,
                Some(builder) => match builder
                    .build_with_allocations(allocation.policy())?
                {
                    None => None,
                    Some(searcher) => {
                        let memory_usage = searcher.memory_usage();
                        Some(Prefilter {
                            finder: allocation.arc(Packed(searcher))?,
                            memory_usage,
                        })
                    }
                },
        ...
        Ok(
            match (
                self.start_bytes.build(allocation)?,
                self.rare_bytes.build(allocation)?,
            ) {
                ...
                (prestart @ Some(_), prerare @ Some(_)) => {
                    ...
                    let has_rarer_bytes = self.start_bytes.rank_sum
                        <= self.rare_bytes.rank_sum + 50;
                    if has_fewer_bytes {
                        debug!(
    ...
    pub(crate) fn add(
        ...
        self.memmem.add(bytes, allocation)?;
        if let Some(ref mut pbuilder) = self.packed {
            pbuilder.add_with_allocations(bytes, allocation.policy())?;
        }
        Ok(())
    }
}
...
impl PrefilterI for Packed {
    fn visit_source_storage(
        ...
        self.0.visit_source_storage(visitor);
    }
    fn find_in(&self, haystack: &[u8], span: Span) -> Candidate {
        self.0
...
impl MemmemBuilder {
    fn build(
        ...
        {
            let Some(pattern) = self.one.as_ref() else { return Ok(None) };
            assert_eq!(1, self.count);
            let finder = memchr::memmem::Finder::new(pattern)
                .into_owned_with_allocations(&allocation)
                .map_err(AllocationError::from)?;
            let finder = allocation.arc(Memmem(finder))?;
            Ok(Some(Prefilter { finder, memory_usage: pattern.len() }))
        }
    ...
    fn build(
        ...
        fn imp(
            ...
            let finder: Arc<dyn PrefilterI> = match len {
                0 => return Ok(None),
                1 => allocation.arc(RareBytesOne {
                    byte1: bytes[0],
                    offset: builder.byte_offsets.set[bytes[0] as usize],
                })?,
                2 => allocation.arc(RareBytesTwo {
                    offsets: builder.byte_offsets,
                    byte1: bytes[0],
                    byte2: bytes[1],
                })?,
                3 => allocation.arc(RareBytesThree {
                    offsets: builder.byte_offsets,
                    byte1: bytes[0],
                    byte2: bytes[1],
                    byte3: bytes[2],
            ...
            Ok(Some(Prefilter { finder, memory_usage: 0 }))
        }
...
impl StartBytesBuilder {
    ...
    fn build(
        ...
        fn imp(
            ...
            let finder: Arc<dyn PrefilterI> = match len {
                0 => return Ok(None),
                1 => allocation.arc(StartBytesOne { byte1: bytes[0] })?,
                2 => allocation
                    .arc(StartBytesTwo { byte1: bytes[0], byte2: bytes[1] })?,
                3 => allocation.arc(StartBytesThree {
                    byte1: bytes[0],
                    byte2: bytes[1],
                    byte3: bytes[2],
                })?,
