...
impl Remapper {
    ...
    pub(crate) fn new(
        ...
        allocation.grow(&mut map, r.state_len())?;
        map.extend((0..r.state_len()).map(|i| idx.to_state_id(i)));
        Ok(Remapper { map, idx })
    }
    ...
    pub(crate) fn remap(
        ...
        allocation.reserve(core::mem::size_of::<Vec<StateID>>())?;
        let oldmap = allocation.copy_slice(&self.map)?;
