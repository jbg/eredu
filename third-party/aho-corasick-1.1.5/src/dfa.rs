...
impl DFA {
    ...
    fn set_matches(
        ...
        for pid in pids {
            allocation.push(&mut self.matches[index], pid)?;
...
unsafe impl Automaton for DFA {
    ...
    fn visit_source_storage(
        ...
        crate::source_storage::vector(&self.trans, visitor);
        crate::source_storage::vector(&self.pattern_lens, visitor);
        if crate::source_storage::vector(&self.matches, visitor) {
            for row in &self.matches {
                crate::source_storage::vector(row, visitor);
            }
        }
...
impl Builder {
    ...
    /// Construct the same automaton with prospective source admission.
    ...
    {
        let nnfa =
            self.noncontiguous.build_with_allocations(patterns, funding)?;
        self.build_from_noncontiguous_with_allocations(&nnfa, funding)
    }
    ...
    pub fn build_from_noncontiguous(
        ...
        self.build_from_noncontiguous_with_allocations(nnfa, &Unenforced)
    }

    /// Convert the original NFA with prospective admission of each destination.
    pub fn build_from_noncontiguous_with_allocations(
        &self,
        nnfa: &noncontiguous::NFA,
        funding: &dyn Allocation,
    ) -> Result<DFA, BuildError> {
        ...
        allocation.reserve(
            core::mem::size_of::<Result<DFA, BuildError>>()
                + core::mem::size_of::<ByteClasses>()
                + core::mem::size_of::<Vec<StateID>>()
                + core::mem::size_of::<Vec<Vec<PatternID>>>(),
        )?;
        debug!("building DFA");
        ...
        allocation.resize_copy(&mut trans, trans_len, DFA::DEAD)?;
        let mut matches = Vec::new();
        allocation.grow(&mut matches, num_match_states)?;
        matches.resize_with(num_match_states, Vec::new);
        let mut dfa = DFA {
            trans,
            matches,
            matches_memory_usage: 0,
            pattern_lens: allocation.copy_slice(nnfa.pattern_lens_raw())?,
        ...
        match self.start_kind {
            StartKind::Both => {
                self.finish_build_both_starts(nnfa, &mut dfa, allocation)?;
            }
            StartKind::Unanchored => {
                self.finish_build_one_start(
                    Anchored::No,
                    nnfa,
                    &mut dfa,
                    allocation,
            ...
            StartKind::Anchored => self.finish_build_one_start(
                Anchored::Yes,
                nnfa,
                &mut dfa,
                allocation,
        ...
        allocation.shrink(&mut dfa.trans)?;
        allocation.shrink(&mut dfa.pattern_lens)?;
        allocation.shrink(&mut dfa.matches)?;
    ...
    fn finish_build_both_starts(
        ...
        let mut remap_unanchored = Vec::new();
        allocation.resize_copy(
            &mut remap_unanchored,
            nnfa.states().len(),
            DFA::DEAD,
        )?;
        let mut remap_anchored = Vec::new();
        allocation.resize_copy(
            &mut remap_anchored,
            nnfa.states().len(),
            DFA::DEAD,
        )?;
        let mut is_anchored = Vec::new();
        allocation.resize_copy(&mut is_anchored, dfa.state_len, false)?;
