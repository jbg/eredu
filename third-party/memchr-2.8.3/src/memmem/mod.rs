...
impl<'h, 'n> FindIter<'h, 'n> {
    ...
    pub fn into_owned(self) -> FindIter<'h, 'static> {
        self.into_owned_with_allocations(&crate::allocation::Unenforced)
            .expect("ordinary needle allocation")
    }

    /// Own the iterator's original finder through prospective needle admission.
    #[cfg(feature = "alloc")]
    pub fn into_owned_with_allocations(
        self,
        funding: &dyn crate::allocation::Allocation,
    ) -> Result<FindIter<'h, 'static>, crate::allocation::AllocationError>
    {
        Ok(FindIter {
            haystack: self.haystack,
            prestate: self.prestate,
            finder: self.finder.into_owned_with_allocations(funding)?,
            pos: self.pos,
        })
    }
}
...
impl<'h, 'n> FindRevIter<'h, 'n> {
    ...
    pub fn into_owned_with_allocations(
        self,
        funding: &dyn crate::allocation::Allocation,
    ) -> Result<FindRevIter<'h, 'static>, crate::allocation::AllocationError>
    {
        Ok(FindRevIter {
            haystack: self.haystack,
            finder: self.finder.into_owned_with_allocations(funding)?,
...
impl<'n> Finder<'n> {
    /// Visit the actual owned needle backing, excluding borrowed input and the
    /// inline search strategy. False stops descending into an existing owner.
    pub fn visit_source_storage(
        &self,
        visitor: &mut dyn FnMut(*const (), usize) -> bool,
    ) {
        self.needle.visit_source_storage(visitor);
    }
    ...
    /// Own the needle through the original worker after prospective admission.
    /// An already owned needle moves without another allocation or request.
    #[cfg(feature = "alloc")]
    pub fn into_owned_with_allocations(
        self,
        funding: &dyn crate::allocation::Allocation,
    ) -> Result<Finder<'static>, crate::allocation::AllocationError> {
        Ok(Finder {
            needle: self.needle.into_owned_with_allocations(funding)?,
...
impl<'n> FinderRev<'n> {
    ...
    pub fn into_owned_with_allocations(
        self,
        funding: &dyn crate::allocation::Allocation,
    ) -> Result<FinderRev<'static>, crate::allocation::AllocationError> {
        Ok(FinderRev {
            needle: self.needle.into_owned_with_allocations(funding)?,
            searcher: self.searcher,
        })
