...
impl<K, V> Core<K, V> {
    ...
    pub(crate) fn reserve(&mut self, additional: usize) {
        self.try_reserve(additional).expect("IndexMap reservation failed");
    }

    /// Reserve capacity without entry over-allocation.
    pub(crate) fn reserve_exact(&mut self, additional: usize) {
        self.try_reserve_exact(additional).expect("IndexMap exact reservation failed");
    }

    pub(crate) fn try_reserve(&mut self, additional: usize) -> Result<(), TryReserveError> {
        self.try_reserve_with(additional, false, |_| Ok::<_, core::convert::Infallible>(()))
            .map_err(crate::TryReserveWithError::into_allocation)
    }

    pub(crate) fn try_reserve_exact(&mut self, additional: usize) -> Result<(), TryReserveError> {
        self.try_reserve_with(additional, true, |_| Ok::<_, core::convert::Infallible>(()))
            .map_err(crate::TryReserveWithError::into_allocation)
    }

    pub(crate) fn try_reserve_with<E>(
        &mut self, additional: usize, exact: bool,
        mut reserve: impl FnMut(core::alloc::Layout) -> Result<(), E>,
    ) -> Result<(), crate::TryReserveWithError<E>> {
        use crate::TryReserveWithError::{Allocation, Funding};
        let layout = self.indices.try_reserve_layout(additional)
            .map_err(TryReserveError::from_hashbrown).map_err(Allocation)?;
        if let Some(layout) = layout { reserve(layout).map_err(Funding)?; }
        self.indices.try_reserve(additional, get_hash(&self.entries))
            .map_err(TryReserveError::from_hashbrown).map_err(Allocation)?;
        self.try_reserve_entries_with(additional, exact, reserve)
    }

    // The caller has already grown indices. This worker makes exactly one entry
    // allocation request. It never retries a smaller unreported destination.
    fn try_reserve_entries_with<E>(
        &mut self, additional: usize, exact: bool,
        mut reserve: impl FnMut(core::alloc::Layout) -> Result<(), E>,
    ) -> Result<(), crate::TryReserveWithError<E>> {
        use crate::TryReserveWithError::{Allocation, Funding};
        if additional <= self.entries.capacity() - self.entries.len() { return Ok(()); }
        let overflow = || Allocation(TryReserveError { kind: crate::TryReserveErrorKind::CapacityOverflow });
        let required = self.entries.len().checked_add(additional).ok_or_else(overflow)?;
        let capacity = if exact { required } else {
            required.max(self.indices.capacity().min(Self::MAX_ENTRIES_CAPACITY))
        };
        let layout = core::alloc::Layout::array::<Bucket<K,V>>(capacity).map_err(|_| overflow())?;
        reserve(layout).map_err(Funding)?;
        self.entries.try_reserve_exact(capacity - self.entries.len())
            .map_err(TryReserveError::from_alloc).map_err(Allocation)
    }

    pub(crate) fn allocation_size(&self) -> usize {
        self.indices.allocation_size() + self.entries.capacity() * core::mem::size_of::<Bucket<K,V>>()
    }
    ...
    fn reserve_entries(&mut self, additional: usize) {
        self.try_reserve_entries_with(additional, false, |_| Ok::<_, core::convert::Infallible>(()))
            .map_err(crate::TryReserveWithError::into_allocation)
            .expect("IndexMap entry reservation failed");
    }
