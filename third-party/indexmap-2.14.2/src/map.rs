...
impl<K, V, S> IndexMap<K, V, S> {
    ...
    /// Reserve through the ordinary worker after reporting each exact backing
    /// layout. Existing contents and accepted allocation prefixes remain in this
    /// collection on failure. Nested key/value payloads are separate producers.
    pub fn try_reserve_with<E>(
        &mut self, additional: usize,
        reserve: impl FnMut(core::alloc::Layout) -> Result<(), E>,
    ) -> Result<(), crate::TryReserveWithError<E>> {
        self.core.try_reserve_with(additional, false, reserve)
    }

    /// Actual retained index-table and entry-vector backing bytes.
    pub fn allocation_size(&self) -> usize {
        self.core.allocation_size()
    }
