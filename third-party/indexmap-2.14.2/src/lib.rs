...
/// A prospective reservation failed before the next allocation. A failure after
/// the index allocation preserves that reached backing in the original map.
#[derive(Debug)]
pub enum TryReserveWithError<E> {
    /// The caller refused the exact reached allocation layout.
    Funding(E),
    /// The collection or allocator could not provide the requested capacity.
    Allocation(TryReserveError),
}
impl<E: core::fmt::Display> core::fmt::Display for TryReserveWithError<E> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self { Self::Funding(error) => error.fmt(f), Self::Allocation(error) => error.fmt(f) }
    }
}
impl<E: core::error::Error + 'static> core::error::Error for TryReserveWithError<E> {
    fn source(&self) -> Option<&(dyn core::error::Error + 'static)> {
        match self { Self::Funding(error) => Some(error), Self::Allocation(error) => Some(error) }
    }
}
impl TryReserveWithError<core::convert::Infallible> {
    fn into_allocation(self) -> TryReserveError {
        match self { Self::Funding(never) => match never {}, Self::Allocation(error) => error }
    }
}
