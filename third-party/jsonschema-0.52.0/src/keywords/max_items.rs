...
impl MaxItemsValidator {
    ...
    pub(crate) fn compile<'a, F: Json>(
        ...
        Ok(ctx.funding().boxed(MaxItemsValidator { limit, location })?)
    }
}
...
impl<F: Json> Validate<F> for MaxItemsValidator {
    ...
    fn validate_body<'i>(
        ...
        if let Some(array) = instance.as_array() {
            if (array.len() as u64) > self.limit {
                return ctx.diagnostic::<F>(instance, location, tracker, &self.location, |_| Ok(crate::error::ValidationErrorKind::MaxItems { limit: self.limit }));
            }
        }
        Ok(())
    }
...
pub(crate) fn compile<'a, F: Json>(
    ...
    let location = crate::keywords::try_compile!(ctx.location().join_with_funding("maxItems", ctx.funding()));
