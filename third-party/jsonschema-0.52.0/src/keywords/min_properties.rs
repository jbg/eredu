...
impl MinPropertiesValidator {
    ...
    pub(crate) fn compile<'a, F: Json>(
        ...
        Ok(ctx.funding().boxed(MinPropertiesValidator { limit, location })?)
    }
}
...
impl<F: Json> Validate<F> for MinPropertiesValidator {
    ...
    fn validate_body<'i>(
        ...
        if let Some(object) = instance.as_object() {
            if (object.len() as u64) < self.limit {
                return ctx.diagnostic::<F>(instance, location, tracker, &self.location, |_| Ok(crate::error::ValidationErrorKind::MinProperties { limit: self.limit }));
            }
        }
        Ok(())
    }
...
pub(crate) fn compile<'a, F: Json>(
    ...
    let location = crate::keywords::try_compile!(ctx.location().join_with_funding("minProperties", ctx.funding()));
