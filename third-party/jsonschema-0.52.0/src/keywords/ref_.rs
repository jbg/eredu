...
impl<F: Json> Validate<F> for RefValidator<F> {
    fn original_source(&self, source: &mut crate::validator::source::Inspector<F>) -> Result<(), crate::validator::workspace::Error> {
        source.location(&self.ref_suffix)?; source.location(&self.ref_target_base)?; source.boxed(&self.inner)
    }
    ...
    fn original_controls(&self) -> Result<usize, crate::validator::workspace::Error> {
        crate::validator::workspace::body_controls::<F, Self>(&[
            ...
            std::mem::size_of::<&crate::keywords::BoxedValidator<F>>(),
        ])
    }
...
impl<F: Json> Validate<F> for DirectRefValidator<F> {
    fn original_source(&self, source: &mut crate::validator::source::Inspector<F>) -> Result<(), crate::validator::workspace::Error> {
        source.location(&self.ref_suffix)?; source.location(&self.ref_target_base)?; source.node(&self.inner)
    }
