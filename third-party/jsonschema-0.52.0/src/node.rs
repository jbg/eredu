...
impl<F: Json> Validate<F> for PendingSchemaNode<F> {
    fn original_source(&self, source: &mut crate::validator::source::Inspector<F>) -> Result<(), crate::validator::workspace::Error> {
        if !source.arc(&self.cell)? { return Ok(()); }
        let target = self.cell.get().ok_or(crate::validator::workspace::Error::Capacity)?;
        source.location(&target.location)?; source.uri(&target.absolute_path)?;
        source.node(&target.materialize())
    }
    ...
    fn original_controls(&self) -> Result<usize, crate::validator::workspace::Error> {
        crate::validator::workspace::body_controls::<F, Self>(&[
            ...
            std::mem::size_of::<Option<crate::node::SchemaNode<F>>>(),
            std::mem::size_of::<std::slice::Iter<'_, KeywordValidatorEntry<F>>>(),
            std::mem::size_of::<std::slice::Iter<'_, ArrayValidatorEntry<F>>>(),
...
impl<F: Json> Validate<F> for SchemaNode<F> {
    fn original_source(&self, source: &mut crate::validator::source::Inspector<F>) -> Result<(), crate::validator::workspace::Error> {
        source.location(&self.location)?; source.uri(&self.absolute_path)?;
        if !source.arc(&self.inner)? { return Ok(()); }
        source.formatted(&self.inner.formatted_schema_location)?;
        match &self.inner.validators {
            NodeValidators::Boolean { validator } => { if let Some(validator) = validator { source.boxed(validator)?; } },
            NodeValidators::Keyword(keywords) => {
                if let Some(value) = &keywords.unmatched_keywords { if source.arc(value)? { source.value(value)?; } }
                source.vector(&keywords.validators)?;
                for entry in &keywords.validators { source.location(&entry.location)?; source.uri(&entry.absolute_location)?; source.formatted(&entry.formatted_schema_location)?; source.boxed(&entry.validator)?; }
            },
            NodeValidators::Array { validators } => {
                source.vector(validators)?;
                for entry in validators { source.location(&entry.location)?; source.uri(&entry.absolute_location)?; source.formatted(&entry.formatted_schema_location)?; source.boxed(&entry.validator)?; }
            },
        }
        Ok(())
    }
