...
pub struct ParserLimits {
    ...
    /// Includes the DEAD and MISSING sentinel states; must be at least two.
    /// Enforced during construction, warming and later state growth. This is
    /// a state-count limit, not a byte bound for the complete parser.
...
/// A lexer cannot represent its two sentinel states within this configuration.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct InvalidLexerStateLimit {
    pub configured: usize,
    pub minimum: usize,
}

impl Display for InvalidLexerStateLimit {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            formatter,
            "lexer state limit {} is below the required minimum {}",
            self.configured, self.minimum
        )
    }
}

impl std::error::Error for InvalidLexerStateLimit {}

impl ParserLimits {
    /// Checks scalar configuration without allocating parser or lexer storage.
    pub fn validate_lexer_state_limit(&self) -> std::result::Result<(), InvalidLexerStateLimit> {
        const MINIMUM: usize = 2;
        if self.max_lexer_states < MINIMUM {
            Err(InvalidLexerStateLimit {
                configured: self.max_lexer_states,
                minimum: MINIMUM,
            })
        } else {
            Ok(())
