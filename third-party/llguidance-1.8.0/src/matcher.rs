...
use crate::{
    api::{InvalidLexerStateLimit, StopReason},
    earley::{ParserError, ParserStats},
...
// Preserve only these closed parser-owned causes. An arbitrary construction
// error can own unrelated resources; retaining that whole error would change
// the matcher's existing payload lifetime contract.
#[derive(Clone)]
enum MatcherFailureCause {
    InvalidLexerStateLimit(InvalidLexerStateLimit),
    Parser(ParserError),
}

impl MatcherFailureCause {
    fn from_error(error: &anyhow::Error) -> Option<Self> {
        error.chain().find_map(|cause| {
            cause
                .downcast_ref::<InvalidLexerStateLimit>()
                .copied()
                .map(Self::InvalidLexerStateLimit)
                .or_else(|| {
                    cause
...
struct MatcherFailure {
    message: String,
    cause: Option<MatcherFailureCause>,
}

impl MatcherFailure {
    fn from_error(error: anyhow::Error) -> Self {
        Self {
            cause: MatcherFailureCause::from_error(&error),
    ...
    fn error(&self) -> anyhow::Error {
        match &self.cause {
            Some(MatcherFailureCause::InvalidLexerStateLimit(cause)) => {
                anyhow::Error::new(*cause).context(self.message.clone())
            }
            Some(MatcherFailureCause::Parser(cause)) => {
                anyhow::Error::new(cause.clone()).context(self.message.clone())
            }
    ...
    fn stop_reason(&self) -> StopReason {
        match &self.cause {
            Some(MatcherFailureCause::Parser(cause)) => cause.stop_reason(),
...
impl Matcher {
    pub fn new(parser: Result<TokenParser>) -> Self {
        match parser {
            ...
            Err(e) => Matcher(MatcherState::Error(MatcherFailure::from_error(e))),
        }
    }
        ...
        match &mut self.0 {
            MatcherState::Normal(ref mut inner) => {
                ...
                match r {
                    Ok(r) => Ok(r),
                    Err(e) => {
                        let cause = MatcherFailureCause::from_error(&e).or_else(|| {
                            inner
                                .parser
                                .parser
                                .try_get_error()
                                .map(MatcherFailureCause::Parser)
                        });
                        let failure = MatcherFailure {
                            message: inner.parser.augment_err(e),
                            cause,
                        };
                        let error = failure.error();
                        self.0 = MatcherState::Error(failure);
                        Err(error)
                    }
                }
            }
            MatcherState::Error(e) => Err(e.error()),
        }
    }
    ...
    pub fn stop_reason(&self) -> StopReason {
        match &self.0 {
            ...
            MatcherState::Error(error) => error.stop_reason(),
        }
    }
    ...
    pub fn try_consume_tokens(&mut self, tokens: &[TokenId]) -> Result<usize> {
        self.with_inner(|inner| {
            crate::tokenparser::progress::try_consume(&mut inner.parser, tokens)
        })
    }
    ...
    pub fn get_error(&self) -> Option<String> {
        match &self.0 {
            ...
            MatcherState::Error(e) => Some(e.message.clone()),
        }
    }
