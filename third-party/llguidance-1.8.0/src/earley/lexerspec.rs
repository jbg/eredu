...
use super::{
    ...
    regexvec::{LexemeSet, MatchingLexemes, RegexVec},
};
...
mod input;
pub use input::{
    LexerInputFailure, LexerRootPlan, LexerRootRequirements, LexerRootSource, RegexVectorInput,
};
mod source_copy;
pub use source_copy::{
    LexemeCopyFailure, LexemeCopyPlan, LexerSourceCopyRequirements, LexerSpecCopyFailure,
    LexerSpecCopyPlan,
};
...
impl LexerSpec {
    ...
    pub fn token_range_lexemes(&self, possible: &LexemeSet) -> Vec<&LexemeSpec> {
        self.iter_token_range_lexemes(possible).collect()
    }
    pub(crate) fn iter_token_range_lexemes<'a, 'b>(
        &'a self,
        possible: &'b LexemeSet,
    ) -> impl Iterator<Item = &'a LexemeSpec> + 'b
    where
        'a: 'b,
    {
        possible
            .iter()
            .map(move |index| &self.lexemes[index.as_usize()])
            .filter(|spec| !spec.token_ranges.is_empty())
    }
    pub(crate) fn lexeme_ends_at_eos(&self, index: LexemeIdx) -> bool {
        self.lexemes[index.as_usize()].ends_at_eos
    }
    ...
    pub fn to_regex_vec(&self, limits: &mut ParserLimits) -> Result<RegexVec> {
        ...
        let roots = self.root_source_plan()?.compile()?;
        let input = roots.ordinary(self.regex_builder.exprset().clone())?;
        RegexVec::new_with_input(input, limits)
    }
