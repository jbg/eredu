...
fn compose(iter: Char16TrieIterator, starter: char, second: char) -> Option<char> {
    ...
    if v < HANGUL_V_COUNT {
        ...
        if l < HANGUL_L_COUNT {
            ...
            return Some(char::from_u32(HANGUL_S_BASE + lv).expect("validated Unicode input"));
        }
        return None;
    }
    if in_inclusive_range(second, '\u{11A8}', '\u{11C2}') {
        ...
        if lv < HANGUL_S_COUNT && lv % HANGUL_T_COUNT == 0 {
            ...
            // Safe, because the inputs are known to be in range.
            return Some(char::from_u32(HANGUL_S_BASE + lvt).expect("validated Unicode input"));
        }
    }
    None
}
...
impl CharacterAndClass {
    ...
    pub fn character(&self) -> char {
        ...
        // originally.
        char::from_u32(self.0 & 0xFFFFFF).expect("validated Unicode input")
    }
    /// This method must exist for Pernosco to apply its special rendering.
...
fn sort_slice_by_ccc(
    ...
    trie: &Trie,
    allocation: &allocation::Allocator<'_>,
) {
    if slice.len() < 2 || allocation.failure().is_some() {
        return;
    }
    slice
        .iter_mut()
        .for_each(|cc| cc.set_ccc_from_trie_if_not_already_set(trie));
    // Canonical classes are bytes. Stable insertion keeps short combining runs
    // inline; a stable counting pass gives long runs one explicit destination.
    // Equal classes retain source order, as the original stable sort requires.
    if slice.len() <= 32 {
        for i in 1..slice.len() {
            let mut j = i;
            while j > 0 && slice[j].ccc() < slice[j - 1].ccc() {
                slice.swap(j, j - 1);
                j -= 1;
            }
        }
    ...
    if allocation
        .reserve(core::mem::size_of::<(
            [usize; 256],
            usize,
            CharacterAndClass,
            alloc::vec::Vec<CharacterAndClass>,
        )>())
        .is_err()
    {
        return;
    ...
    for value in slice.iter() {
        offsets[value.ccc().0 as usize] += 1;
    }
    let mut next = 0;
    ...
    let Ok(mut output) = allocation.vector::<CharacterAndClass>(slice.len()) else {
        return;
    };
    output.resize(slice.len(), CharacterAndClass(0));
    for &value in slice.iter() {
        let index = &mut offsets[value.ccc().0 as usize];
        output[*index] = value;
        *index += 1;
    }
...
{
    delegate: I,
    allocations: allocation::Allocator<'data>,
    ...
    I: Iterator<Item = char>,
{
    ...
    fn new_with_supplements(
        ...
        Self::new_with_supplements_with_allocations(
            delegate,
            decompositions,
            tables,
            ...
            ignorable_behavior,
            &allocation::Unenforced,
        )
    }

    fn new_with_supplements_with_allocations(
        delegate: I,
        decompositions: &'data DecompositionData,
        tables: &'data DecompositionTables,
        supplementary_tables: Option<&'data DecompositionTables>,
        decomposition_passthrough_bound: u8,
        ignorable_behavior: IgnorableBehavior,
        allocation: &'data dyn allocation::Allocation,
    ) -> Self {
        let mut ret = Decomposition::<I> {
            delegate,
            allocations: allocation::Allocator::new(allocation),
        ...
        let _ = ret.allocations.reserve(core::mem::size_of::<(
            Self,
            Composition<'data, I>,
            AllocatedNormalization<'data, I>,
            Result<char, allocation::AllocationError>,
            Option<char>,
            CharacterAndClass,
            CharacterAndTrieValue,
        )>());
        ...
        if only_non_starters_in_trail {
            ...
            for u in tail.iter() {
                ...
                self.allocations.push(
                    &mut self.buffer,
                    CharacterAndClass::new_with_trie_value(CharacterAndTrieValue::new(
                        ch, trie_value,
                    )),
                );
                i += 1;
    ...
    fn push_decomposition32(
        ...
        if only_non_starters_in_trail {
            ...
            self.allocations.extend(
                &mut self.buffer,
                tail.iter().map(CharacterAndClass::new_with_placeholder),
            );
            (starter, 0)
        } else {
            let mut i = 0;
        ...
        let (starter, combining_start) = {
            ...
            if (decomposition & !(BACKWARD_COMBINING_MARKER | NON_ROUND_TRIP_MARKER)) == 0 {
                ...
                if !high_zeros && !low_zeros {
                    ...
                    self.allocations.push(
                        &mut self.buffer,
                        CharacterAndClass::new_with_placeholder(combining),
                    );
                    (starter, 0)
                } else if high_zeros {
                    ...
                    if hangul_offset < HANGUL_S_COUNT {
                        ...
                        // These checked conversions stay
                        // within the Hangul jamo block and the scalar
                        // value range by construction.
                        self.allocations.push(
                            &mut self.buffer,
                            CharacterAndClass::new_starter(
                                char::from_u32(HANGUL_V_BASE + v).expect("validated Unicode input"),
                            ),
                        );
                        let first =
                            char::from_u32(HANGUL_L_BASE + l).expect("validated Unicode input");
                        if t != 0 {
                            self.allocations.push(
                                &mut self.buffer,
                                CharacterAndClass::new_starter(
                                    char::from_u32(HANGUL_T_BASE + t)
    ...
    fn gather_and_sort_combining(&mut self, combining_start: usize) {
        if self.allocations.failure().is_some() {
            return;
        }
        ...
        while let Some(ch_and_trie_val) = self.delegate_next() {
            ...
            if !trie_value_has_ccc(ch_and_trie_val.trie_val) {
                ...
                self.allocations.push(
                    &mut self.buffer,
                    CharacterAndClass::new_with_trie_value(ch_and_trie_val),
                );
            } else {
                ...
                let mapped = match ch_and_trie_val.character {
                    ...
                    '\u{0344}' => {
                        ...
                        self.allocations.push(
                            &mut self.buffer,
                            CharacterAndClass::new('\u{0308}', CanonicalCombiningClass::Above),
                        );
                        CharacterAndClass::new('\u{0301}', CanonicalCombiningClass::Above)
                    }
                    '\u{0F73}' => {
                        ...
                        self.allocations.push(
                            &mut self.buffer,
                            CharacterAndClass::new('\u{0F71}', CanonicalCombiningClass(129)),
                        );
                ...
                self.allocations.push(&mut self.buffer, mapped);
            }
        }
        ...
        sort_slice_by_ccc(
            &mut self.buffer[combining_start..],
            self.trie,
            &self.allocations,
        );
    }
    ...
    I: Iterator<Item = char>,
{
    ...
    fn next(&mut self) -> Option<char> {
        ...
        let value = self.decomposing_next(c_and_trie_val);
        self.allocations.failure().is_none().then_some(value)
    }
}

/// Fallible view of the ordinary normalization iterator with an explicit loan.
pub struct AllocatedNormalization<'a, I: Iterator<Item = char>> {
    inner: Composition<'a, I>,
    finished: bool,
}
impl<I: Iterator<Item = char>> Iterator for AllocatedNormalization<'_, I> {
    type Item = Result<char, allocation::AllocationError>;
    fn next(&mut self) -> Option<Self::Item> {
        if self.finished {
            return None;
        }
        let value = self.inner.next();
        if let Some(error) = self.inner.decomposition.allocations.failure() {
            self.finished = true;
    ...
    I: Iterator<Item = char>,
{
    ...
    #[inline]
    fn next(&mut self) -> Option<char> {
        if self.decomposition.allocations.failure().is_some() {
            return None;
        }
...
macro_rules! normalizer_methods {
    () => {
        ...
        pub fn split_normalized_utf8<'a>(&self, text: &'a [u8]) -> (&'a str, &'a [u8]) {
            ...
            (str::from_utf8(head).expect("validated Unicode input"), tail)
        }
...
impl<'data> DecomposingNormalizerBorrowed<'data> {
        ...
        as_slice,
        {
            ...
            'fast: loop {
                ...
                'fastest: loop {
                    ...
                    // End of stream
                    sink.write_str(str::from_utf8(pending_slice).expect("validated Unicode input"))?;
                    return Ok(());
                }
                #[expect(clippy::indexing_slicing)]
                {decomposition.delegate = pending_slice[pending_slice.len() - code_unit_iter.as_slice().len() - 1..].chars();}
                ...
                if upcoming == char::REPLACEMENT_CHARACTER {
                    ...
                    sink.write_str(str::from_utf8(consumed_so_far_slice).expect("validated Unicode input") )?;
    ...
    decomposing_normalize_to!(
        ...
        as_slice,
        {
            ...
            #[expect(clippy::never_loop)]
            'fastwrap: loop {
                // Keep the UTF-16 fast-path cursor as a checked slice offset.
                let delegate_as_slice = decomposition.delegate.as_slice();
                let mut offset = 0usize;
                let end = delegate_as_slice.len();
                'fast: loop {
                    if offset != end {
                        let upcoming_code_unit = delegate_as_slice[offset];
                        offset += 1;
                        ...
                        'surrogateloop: loop {
                            ...
                            if likely(surrogate_base <= (0xDBFF - 0xD800)) {
                                if offset != end {
                                    let low = delegate_as_slice[offset];
                            ...
                            // unpaired surrogate
                            upcoming32 = 0xFFFD; // Replacement scalar, matching the later invalid-input check.
                            // trie_value already holds a decomposition to U+FFFD.
                            break 'surrogateloop;
                        }

                        let upcoming = char::from_u32(upcoming32).expect("validated Unicode input");
                        ...
                        if decomposition_starts_with_non_starter(
                            ...
                            decomposition.delegate = (&delegate_as_slice[offset..]).chars();
                            // Let this trie value to be reprocessed in case it is
                            // one of the rare decomposing ones.
                            decomposition.pending = Some(upcoming_with_trie_value);
                            decomposition.gather_and_sort_combining(0);
...
impl<'data> ComposingNormalizerBorrowed<'data> {
    ...
    fn normalize_iter_private<I: Iterator<Item = char>>(
        ...
        self.normalize_iter_private_with_allocations(
            iter,
            ignorable_behavior,
            &allocation::Unenforced,
        )
    ...
    fn normalize_iter_private_with_allocations<'scope, I: Iterator<Item = char>>(
        &self,
        iter: I,
        ignorable_behavior: IgnorableBehavior,
        allocation: &'scope dyn allocation::Allocation,
    ) -> Composition<'scope, I>
    where
        'data: 'scope,
    {
        Composition::new(
            Decomposition::new_with_supplements_with_allocations(
                iter,
    ...
    composing_normalize_to!(
        ...
        as_slice,
        {
            ...
            'fastwrap: loop {
                ...
                composition.decomposition.delegate = (&delegate_as_slice[offset..]).chars();
                break 'fastwrap;
            }
        },
        text,
