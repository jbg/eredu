...
impl Uts46MapperBorrowed<'_> {
    ...
    /// Run the same UTS 46 iterator with explicit prospective storage.
    pub fn map_normalize_with_allocations<'scope, I: Iterator<Item=char> + 'scope>(
        &'scope self, iter: I, allocation: &'scope dyn crate::allocation::Allocation,
    ) -> crate::AllocatedNormalization<'scope, I> {
        crate::AllocatedNormalization { inner: self.normalizer.normalize_iter_private_with_allocations(iter, crate::IgnorableBehavior::Ignored, allocation), finished: false }
    }
    ...
    /// Run the same UTS 46 iterator with explicit prospective storage.
    pub fn normalize_validate_with_allocations<'scope, I: Iterator<Item=char> + 'scope>(
        &'scope self, iter: I, allocation: &'scope dyn crate::allocation::Allocation,
    ) -> crate::AllocatedNormalization<'scope, I> {
        crate::AllocatedNormalization { inner: self.normalizer.normalize_iter_private_with_allocations(iter, crate::IgnorableBehavior::ReplacementCharacter, allocation), finished: false }
    }
