    ...
    pub fn decompose(&self, c: char) -> Decomposed {
        ...
        if t == 0 {
            ...
            return Decomposed::Expansion(
                ...
                char::from_u32(HANGUL_L_BASE + l).expect("validated Unicode scalar"),
                char::from_u32(HANGUL_V_BASE + v).expect("validated Unicode scalar"),
            );
        }
        let lv = lvt - t;
        // Invariant: lvt < 1172
        ...
        Decomposed::Expansion(
            ...
            // max will keep it in range, less than 0xD800
            char::from_u32(HANGUL_S_BASE + lv).expect("validated Unicode scalar"),
            char::from_u32(HANGUL_T_BASE + t).expect("validated Unicode scalar"),
        )
    }
