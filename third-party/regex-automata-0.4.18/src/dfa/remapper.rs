...
impl Remapper {
    ...
    ) -> Result<Remapper, AllocationError> {
        let idxmap = IndexMapper {
            stride2: r.stride2(),
        };
        let mut map = Vec::new();
        Allocator::new(funding).grow(&mut map, r.state_len())?;
        map.extend((0..r.state_len()).map(|i| idxmap.to_state_id(i)));
        Ok(Remapper { map, idxmap })
    }
    ...
    pub(super) fn remap_with_allocations(
        ...
        let oldmap = Allocator::new(funding).copy_slice(&self.map)?;
