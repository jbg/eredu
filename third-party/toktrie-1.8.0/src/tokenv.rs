...
pub trait TokenizerEnv: Send {
    ...
    fn tokenize_bytes_marker(&self, s: &[u8]) -> (Vec<TokenId>, usize) {
        ...
        trie.visit_marker_tokenization(s, |part| -> Result<(), std::convert::Infallible> {
            match part {
                crate::MarkerTokenizationPart::Bytes(bytes) => {
                    let new_tokens = self.tokenize_bytes(bytes);
                    for (idx, token) in new_tokens.iter().enumerate() {
                        if trie.is_special_token(*token) {
                            num_fixed_tokens = result.len() + idx + 1;
                        }
                    }
                    result.extend_from_slice(&new_tokens);
                }
                crate::MarkerTokenizationPart::Token(token) => {
                    result.push(token);
        ...
        .expect("infallible ordinary marker collector");
