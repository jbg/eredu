...
impl Clone for SimpleVob {
    fn clone(&self) -> Self {
        prepared::TokenMaskConstructionPlan::copy(self)
            .expect("valid token mask source")
            .compile()
            .expect("token mask copy construction")
    }
}
...
impl SimpleVob {
    ...
    pub fn alloc(size: usize) -> Self {
        prepared::TokenMaskConstructionPlan::zeroed(size)
            .expect("valid ordinary bit-vector shape")
            .compile()
            .expect("ordinary bit-vector allocation")
    }
    ...
    /// Compatibility alias using the same checked word access as `allow_token`.
    /// The vocabulary's extra sentinel bit is valid when its backing was reserved.
    #[inline(always)]
    pub fn allow_token_unchecked(&mut self, tok: TokenId) {
