...
impl Decoder {
    ...
    pub(crate) fn decode<'a, T: PunycodeCodeUnit + Copy, C: PunycodeCaller>(
        ...
        self.decode_with_allocations::<T, C>(
            input,
            &crate::allocation::Allocator::new(&crate::allocation::Unenforced),
        )
    }

    pub(crate) fn decode_with_allocations<'a, T: PunycodeCodeUnit + Copy, C: PunycodeCaller>(
        &'a mut self,
        input: &'a [T],
        ...
        loop {
            ...
            allocations.push(&mut self.insertions, (i as usize, c));
        ...
        // Every insertion moves prior indices at or beyond its own index;
        // final indices are unique, so an unstable sort preserves exact order.
        self.insertions.sort_unstable_by_key(|(i, _)| *i);
        Ok(Decode {
            base: base.iter(),
