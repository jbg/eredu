...
impl From<crate::punycode::PunycodeEncodeError> for ProcessingError {
    fn from(error: crate::punycode::PunycodeEncodeError) -> Self {
        match error {
            crate::punycode::PunycodeEncodeError::Sink => Self::SinkError,
            crate::punycode::PunycodeEncodeError::Overflow => unreachable!(
...
struct AllocatedString<'a, 'source> {
    value: &'a mut String,
    allocations: &'a Allocator<'source>,
}
impl Write for AllocatedString<'_, '_> {
    fn write_str(&mut self, text: &str) -> core::fmt::Result {
        self.allocations
            .append(self.value, text)
...
impl Uts46 {
    ...
    pub(crate) fn to_ascii_from_cow<'a>(
        ...
        self.to_ascii_from_cow_with_allocations(
            domain_name,
            ascii_deny_list,
            hyphens,
            dns_length,
        ...
        .expect("ordinary IDNA allocation")
    }

    /// The ordinary ToASCII operation with prospectively admitted storage.
    #[inline]
    pub fn to_ascii_with_allocations<'a>(
        &self,
        domain_name: &'a [u8],
        ascii_deny_list: AsciiDenyList,
        hyphens: Hyphens,
    ...
    ) -> Result<Result<Cow<'a, str>, crate::Errors>, AllocationError> {
        self.to_ascii_from_cow_with_allocations(
            Cow::Borrowed(domain_name),
            ascii_deny_list,
            hyphens,
    ...
    #[inline]
    fn to_ascii_from_cow_with_allocations<'a>(
        &self,
        domain_name: Cow<'a, [u8]>,
        ascii_deny_list: AsciiDenyList,
        hyphens: Hyphens,
        ...
        let allocations = Allocator::new(source);
        allocations.reserve(core::mem::size_of::<(
            Self,
            Cow<'a, [u8]>,
            String,
            AllocatedString<'_, '_>,
        ...
        let result = self.process_with_allocations(
            &domain_name,
            ascii_deny_list,
            hyphens,
        ...
        if let Some(error) = allocations.failure() {
            return Err(error);
        }
        Ok(match result {
            Ok(ProcessingSuccess::Passthrough) => {
                ...
                let cow = match domain_name {
                    Cow::Borrowed(v) => {
                        Cow::Borrowed(core::str::from_utf8(v).expect("validated Unicode input"))
                    }
                    Cow::Owned(v) => {
                        Cow::Owned(String::from_utf8(v).expect("validated Unicode input"))
                    }
                };
    ...
    pub fn to_user_interface<'a, OutputUnicode: FnMut(&[char], &[char], bool) -> bool>(
        ...
        self.to_user_interface_with_allocations(
            domain_name,
            ascii_deny_list,
            hyphens,
            output_as_unicode,
    ...
    /// The ordinary ToUnicode operation with prospectively admitted storage.
    #[inline]
    pub fn to_unicode_with_allocations<'a>(
        &self,
        domain_name: &'a [u8],
        ascii_deny_list: AsciiDenyList,
        hyphens: Hyphens,
        source: &dyn Allocation,
    ) -> Result<(Cow<'a, str>, Result<(), crate::Errors>), AllocationError> {
        self.to_user_interface_with_allocations(
            domain_name,
            ascii_deny_list,
            hyphens,
    ...
    /// The same display worker. The caller owns storage used by its callback.
    #[inline]
    pub fn to_user_interface_with_allocations<
        'a,
    ...
    >(
        ...
        Ok(match result {
            // SAFETY: `ProcessingSuccess::Passthrough` asserts that `domain_name` is ASCII.
            Ok(ProcessingSuccess::Passthrough) => (
                Cow::Borrowed(core::str::from_utf8(domain_name).expect("validated Unicode input")),
                Ok(()),
            ),
    ...
    ///   `core::str::from_utf8(domain_name).expect("validated Unicode input")` as the output. (This
    ...
    /// Run the same IDNA worker with prospective internal storage. The caller
    /// owns and qualifies the supplied output callback and sink independently.
    #[inline]
    pub fn process_with_allocations<
        W: Write + ?Sized,
        OutputUnicode: FnMut(&[char], &[char], bool) -> bool,
    >(
        &self,
        ...
        ascii_sink: Option<&mut W>,
        source: &dyn Allocation,
    ) -> Result<Result<ProcessingSuccess, ProcessingError>, AllocationError> {
        let allocations = Allocator::new(source);
        allocations.reserve(core::mem::size_of::<(
            Self,
            SmallVec<[char; 253]>,
            SmallVec<[AlreadyAsciiLabel<'_>; 8]>,
            Allocator<'_>,
            Result<ProcessingSuccess, ProcessingError>,
            Result<Result<ProcessingSuccess, ProcessingError>, AllocationError>,
        )>())?;
        let result = self.process_impl(
            domain_name,
            ascii_deny_list,
            hyphens,
            error_policy,
    ...
    fn process_impl<W: Write + ?Sized, OutputUnicode: FnMut(&[char], &[char], bool) -> bool>(
        &self,
        domain_name: &[u8],
        ascii_deny_list: AsciiDenyList,
        hyphens: Hyphens,
        ...
        if allocations.failure().is_some() {
            return Err(ProcessingError::SinkError);
        }
        ...
        for label in domain_buffer.split(|c| *c == '.') {
            ...
            if let AlreadyAsciiLabel::MixedCaseAscii(mixed_case) = input_punycode {
                    ...
                    mixed_case.iter().position(|c| c.is_ascii_uppercase())
                {
                    ...
                    sink.write_str(
                        core::str::from_utf8(slice_to_write).expect("validated Unicode input"),
                    )?;
                    for c in tail.iter() {
                    ...
                    sink.write_str(
                        core::str::from_utf8(mixed_case).expect("validated Unicode input"),
                    )?;
                } else {
            ...
            if passthrough {
                if !flushed_prefix {
                    ...
                    sink.write_str(
                        core::str::from_utf8(&domain_name[..passthrough_up_to_extended])
                ...
                for label in domain_buffer.split(|c| *c == '.') {
                    ...
                    if let AlreadyAsciiLabel::MixedCaseAscii(mixed_case) = input_punycode {
                            ...
                            mixed_case.iter().position(|c| c.is_ascii_uppercase())
                        {
                            ...
                            // SAFETY: `mixed_case` and `domain_name` up to `passthrough_up_to_extended` are known to be ASCII.
                            sink.write_str(
                                core::str::from_utf8(slice_to_write)
                                    .expect("validated Unicode input"),
                            )?;
                            for c in tail.iter() {
                                sink.write_char(char::from(c.to_ascii_lowercase()))?;
        ...
        for label in tail.split(|b| *b == b'.') {
            ...
            if seen_label {
                if in_prefix {
                    ...
                    allocations.push(domain_buffer, '.');
                    if allocations.failure().is_some() {
                        return (0, false, true);
                    }
                }
            ...
            if !label.is_empty() {
                    ...
                    if has_punycode_prefix(ascii) {
                            ...
                            && (ascii.len() - 4 <= PUNYCODE_DECODE_MAX_INPUT_LENGTH)
                        {
                            if let Ok(decode) = Decoder::default()
                                .decode_with_allocations::<u8, InternalCaller>(
                                    &ascii[4..],
                                    allocations,
                                )
                            {
                                ...
                                allocations.extend(&mut label_buffer, decode);
                                if allocations.failure().is_some() {
                                    return (0, false, true);
                                }
                                ...
                                had_errors = true;
                                allocations.push(domain_buffer, '\u{FFFD}');
                                if allocations.failure().is_some() {
                                    return (0, false, true);
                                }
                ...
                for c in ascii.iter().map(|c| {
                    ...
                    allocations.push(domain_buffer, c);
                    if allocations.failure().is_some() {
                        return (0, false, true);
                    }
                }
                ...
                allocations.push(already_punycode, AlreadyAsciiLabel::Other);
                if allocations.failure().is_some() {
                    return (0, false, true);
                }
                ...
                let mut mapping = self
                    .data
                    .map_normalize_with_allocations(non_ascii.chars(), allocations)
                    .map_while(|value| match value {
                        Ok(value) => Some(value),
                        Err(error) => {
                            allocations.fail(error);
                            None
                        }
                    })
                        ...
                        None | Some('.') => {
                                ...
                                .starts_with(&['x', 'n', '-', '-'])
                            {
                                ...
                                if !punycode_precondition_failed {
                                    if let Ok(decode) = Decoder::default()
                                        .decode_with_allocations::<char, InternalCaller>(
                ...
                // Empty label
                allocations.push(already_punycode, AlreadyAsciiLabel::MixedCaseAscii(label));
                if allocations.failure().is_some() {
                    return (0, false, true);
                }
            }
    ...
    fn after_punycode_decode(
        ...
        for c in self
            .data
            .normalize_validate_with_allocations(label_buffer.iter().copied(), allocations)
            .map_while(|value| match value {
                Ok(value) => Some(value),
                Err(error) => {
                    allocations.fail(error);
