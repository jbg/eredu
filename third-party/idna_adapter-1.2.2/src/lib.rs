...
pub use icu_normalizer::allocation;
...
impl Adapter {
    ...
    /// The same normalization iterator with prospective source storage.
    pub fn map_normalize_with_allocations<'scope, I: Iterator<Item = char> + 'scope>(
        &'scope self,
        iter: I,
        allocation: &'scope dyn allocation::Allocation,
    ) -> impl Iterator<Item = Result<char, allocation::AllocationError>> + 'scope {
        self.mapper.map_normalize_with_allocations(iter, allocation)
    }
    ...
    pub fn normalize_validate_with_allocations<'scope, I: Iterator<Item = char> + 'scope>(
        ...
        self.mapper
            .normalize_validate_with_allocations(iter, allocation)
    }
}
