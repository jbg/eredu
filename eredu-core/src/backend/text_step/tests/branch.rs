...
impl NativeTextStateBackend for Backend {
    ...
    fn validate_native_text_state(_: &ModelRuntime<Self>, _: &()) -> Result<(), io::Error> {
        Ok(())
    }
    fn exchange_native_text_state(_: &mut ModelRuntime<Self>, _: &mut ()) -> Result<(), io::Error> {
        Ok(())
    }
    fn exchange_text_branch(
        runtime: &mut ModelRuntime<Self>,
        installed: TextBranchSource<'_, Self>,
        incoming: TextBranchSource<'_, Self>,
        _: &mut (),
    ) -> Result<(), io::Error> {
        ...
        if facts.fail_branch_exchange {
            return Err(io::Error::other("injected branch placement refusal"));
        }
        facts.branch_exchanges.push((
            installed.context().run_identity().clone(),
            incoming.context().run_identity().clone(),
        ));
        Ok(())
    }
}
...
fn borrowed_branch_moves_actual_pending_input_and_keeps_each_attempt_sequence() {
    ...
    let (mut branch, marker) = parent
        .fork_completed(
            |runtime| Ok::<_, BranchError>(Some((start(runtime, &facts, 29), (), 73))),
            |error| error,
        )
        .unwrap()
        .unwrap();
    ...
    assert!(matches!(
        parent.inner.step,
        Some(PendingTextInput::Decode(_))
    ));
    parent.exchange_branch(&mut branch).unwrap();
    assert_ne!(parent.inner.step_context.run_identity(), &parent_run);
    assert!(
        matches!(parent.inner.step.as_ref(), Some(PendingTextInput::Prefill(p)) if p.ids == [29])
    );
    assert_eq!(parent.next().unwrap().unwrap().token_id, 7);
    parent.exchange_branch(&mut branch).unwrap();
    assert_eq!(parent.inner.step_context.run_identity(), &parent_run);
    ...
    assert!(matches!(
        replacement.exchange_branch(&mut branch),
...
fn failed_parent_placement_fences_prediction_and_copy_after_child_installation() {
    ...
    assert!(parent
        .fork_completed(
            |runtime| { Ok::<_, BranchError>(Some((start(runtime, &facts, 29), (), ()))) },
            |error| error
        )
        .is_err());
    facts.borrow_mut().fail_branch_exchange = false;
    assert!(matches!(
        parent.snapshot_source(),
