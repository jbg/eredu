...
impl<'a> CaptureTensorGeometry<'a> {
    /// Project the exact receipt interval through its retained spatial producer.
    /// Additive reductions preserve the original selection and export raw terms.
    pub fn prepare_receipt_partition(
        ...
        Self::prepare_receipt_kind(source, context, kind)?.apply_partition(
            projection,
            fragment,
            combination,
        )
    ...
    pub fn prepare_partition(
        ...
        Self::prepare_kind(source, selection, phase, prediction, invocation, kind)?.apply_partition(
            projection,
            fragment,
            combination,
        )
    ...
    pub fn prepare_partition_window(
        ...
        Self::prepare_partition_window_kind(
            source,
            selection,
            phase,
            prediction,
    ...
    pub(super) fn prepare_partition_window_kind(
        ...
        let selected =
            Self::prepare_kind(source, selection, phase, prediction, Some(logical), kind)?;
        let rows = CaptureWindowGeometry::from_resolved(
            &source.points()[selection],
            &source.plan().selections[selection],
            physical.batch,
            logical.sequence,
            selected.source_shape(),
        )?;
        ...
        let rows = rows
            .project_partition(projection, fragment)
    ...
    pub(super) fn apply_partition(
        ...
        if projection.global_shape().len() != rank
            || projection.local_shape().len() != rank
            || global.starts != self.starts()
            || global.ends != self.ends()
            || global.strides != self.strides()
            || global.shape.len() != rank
            || !projection
                .global_shape()
                .iter()
                .zip(self.source_shape())
                .all(|(a, b)| usize::try_from(*a).ok() == Some(*b))
        {
            return Err(E::Partition);
        }
        for axis in 0..rank {
            if global.shape[axis]
                != (self.ends[axis] - self.starts[axis]).div_ceil(self.strides[axis])
            {
                return Err(E::Partition);
            }
        }
        ...
        self.rank = rank;
        for axis in 0..rank {
            self.source_shape[axis] =
                usize::try_from(projection.local_shape()[axis]).map_err(|_| E::Overflow)?;
        ...
        if let CaptureTransform::Preview { max_elements } = self.native_transform() {
            self.elements = self
                .elements
                .min(usize::try_from(*max_elements).map_err(|_| E::Overflow)?);
            self.shape = [0; 32];
