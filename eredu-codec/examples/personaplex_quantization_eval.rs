...
fn main() -> EvalResult<()> {
    ...
    let pcm_array = Array::from_slice(&pcm, &[1, 1, pcm.len() as i32]);
    let input_tokens = mimi.encode(&pcm_array, stream)?;
    safemlx::transforms::eval([&input_tokens])?;
    ...
    let dense_output = generate_encoded_greedy(&mut dense, &input_tokens)?;
    ...
    let quantized_output = generate_encoded_greedy(&mut quantized, &input_tokens)?;
...
fn decode(mimi: &mut Mimi<Array>, tokens: &Array, stream: &Stream) -> EvalResult<Vec<f32>> {
    mimi.reset_decode_state();
    let decoded = mimi.decode(tokens, stream)?;
