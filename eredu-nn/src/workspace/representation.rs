...
impl WorkspaceRepresentation {
    ...
    pub fn with_dense_axis_order(mut self, axes: &[usize]) -> Option<Self> {
        ...
        let mut seen = 0u16;
        let mut encoded = (axes.len() as u32) << 24;
        for (position, &axis) in axes.iter().enumerate() {
            if axis >= axes.len() || seen & (1u16 << axis) != 0 {
                return None;
            }
            seen |= 1u16 << axis;
            encoded |= (axis as u32) << (position * 3);
        }
        if !self.row_contiguous {
            self.dense_axes = encoded;
        }
        Some(self)
    }
    ...
    pub fn dense_axis_at(self, rank: usize, position: usize) -> Option<usize> {
        ...
        if rank > 8 || (self.dense_axes >> 24) as usize != rank {
            return None;
        }
    ...
    pub fn with_element_strides(mut self, strides: &[u64]) -> Option<Self> {
        if strides.is_empty() || strides.len() > 4 {
            return None;
        }
        ...
        if !self.row_contiguous {
            self.element_strides = values;
            self.stride_rank = strides.len() as u8;
        }
        Some(self)
    }
    ...
    pub const fn element_stride_at(self, rank: usize, axis: usize) -> Option<u64> {
        if rank == 0 || rank > 4 || axis >= rank || self.stride_rank as usize != rank {
            return None;
        }
...
pub struct WorkspaceParameterRepresentation {
    ...
    backing: Option<WorkspaceExistingStorage>,
}
impl WorkspaceParameterRepresentation {
    ...
    /// Carries the actual retained backing supplied by the parameter producer.
    /// Parameter names select slots; only this original storage owner establishes sharing.
    pub fn with_backing(mut self, backing: WorkspaceExistingStorage) -> Self {
...
impl WorkspaceContext {
    /// Borrows the actual backing already installed for this parameter slot.
    /// A name selects the slot only; the returned root retains the producer's
    /// original identity, placement and controls. All layout evidence must match.
    pub fn retained_parameter_backing(
        &self,
        name: &str,
        ...
        self.charge_metadata(size_of::<(
            ...
            Option<WorkspaceExistingStorage>,
            Result<Option<WorkspaceExistingStorage>, Error>,
        )>())?;
        let rows = self.parameter_representations.borrow();
        let Some(rows) = rows.as_ref() else {
            return Ok(None);
        };
        let Ok(index) = rows.binary_search_by(|row| row.id.as_str().cmp(name)) else {
            return Ok(None);
        };
        let row = &rows[index];
        if row.layout.as_view() != layout || row.layout.representation() != layout.representation()
        {
            return Err(WorkspaceMetadataError::Unqualified.into());
        }
        Ok(row.backing.clone())
    }

    pub(super) fn parameter_backing(
        &self,
        ...
        let Ok(index) = rows.binary_search_by(|row| row.id.cmp(&parameter.id)) else {
            return Ok(None);
        };
        let row = &rows[index];
        Ok((row.layout == *layout)
            .then(|| row.backing.clone())
            .flatten())
    }
    ...
    pub fn install_parameter_representations(
        ...
        for row in &rows {
            if row
                .backing
                .as_ref()
                .is_some_and(|root| !Rc::ptr_eq(&root.context, &self.identity))
            {
                return Err(WorkspaceMetadataError::Unqualified.into());
            }
        }
...
fn normalize_parameter_representations(rows: &mut Vec<WorkspaceParameterRepresentation>) {
    ...
    rows.dedup_by(|next, previous| {
        ...
        if next.layout != previous.layout
            || next.layout.representation != previous.layout.representation
        {
            previous.layout.representation = None;
            previous.backing = None;
        } else if !matches!((&next.backing, &previous.backing),
...
mod dense_order_tests {
    ...
    fn element_stride_evidence_preserves_rank_and_does_not_infer_dense_storage() {
        ...
        assert!(!slice.row_contiguous());
        assert!(!slice.last_axis_contiguous());
        assert_eq!(
            (0..3)
                .map(|axis| slice.element_stride_at(3, axis))
                .collect::<Vec<_>>(),
            vec![Some(4), Some(1), Some(2)]
        );
        assert_eq!(slice.element_stride_at(2, 0), None);
        assert_eq!(slice.element_stride_at(3, 3), None);
    ...
    fn dense_axis_evidence_preserves_exact_rank_and_refuses_missing_permutations() {
        ...
        assert_eq!(
            (0..4)
                .map(|p| headed.dense_axis_at(4, p))
                .collect::<Vec<_>>(),
            vec![Some(0), Some(2), Some(1), Some(3)]
        );
        assert!(!headed.row_contiguous());
        assert!(headed.last_axis_contiguous());
        assert_eq!(headed.dense_axis_at(3, 0), None);
        assert_eq!(headed.dense_axis_at(4, 4), None);
        assert!(plain.with_dense_axis_order(&[0, 2, 2, 3]).is_none());
        assert!(plain.with_dense_axis_order(&[0, 1, 4, 3]).is_none());
        assert!(plain
            .with_dense_axis_order(&[0, 1, 2, 3, 4, 5, 6, 7, 8])
            .is_none());
        let row = WorkspaceRepresentation::new(WorkspaceFloatingType::Bfloat16, true);
        assert_eq!(row.dense_axis_at(4, 2), Some(2));
        assert_eq!(row.dtype(), WorkspaceFloatingType::Bfloat16);
    }
}
