...
runs `cargo package` for each crate. After each successful package, it adds that
archive to an ephemeral local registry. This lets the next crate resolve the
exact unpublished workspace version while retaining Cargo's normal package
...
- files omitted from the generated archive that cause package-only compilation
  to fail;
...
prerequisites. Packaging does not need an Apple or NVIDIA runner: target-native
Metal and CUDA coverage remains in the platform workflows. This keeps the
release gate routinely executable while still compiling each crate's default
packaged targets.
