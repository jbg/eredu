fn main() {
    let mut cases = 0;
    for pattern in [" ", "", "a", "ab", "é", "🙂"] { for content in ["▁", "", "xé"] { let unk=Some(0); let fallback=0; for scheme in ["always", "first", "never"] {
        ...
        let source = serde_json::json!({"version":"1.0","truncation":null,"padding":null,"added_tokens":[{"id":8,"content":"a ","single_word":false,"lstrip":false,"rstrip":false,"normalized":true,"special":false},{"id":9,"content":"<R>","single_word":false,"lstrip":false,"rstrip":false,"normalized":false,"special":true}],"normalizer":{"type":"Replace","pattern":{"String":pattern},"content":content},"post_processor":null,"decoder":null,
    ...
    println!("pristine literal normalization ordinary/source tokens, IDs, offsets and admitted IDs: {cases} cases");
}
