...
| Nemotron-H 4B | SafeMLX inference passed. Native Transformers loaded all 263 tensors, then its forward pass failed with `KeyError('mlp')`, so no cross-implementation comparison was produced. | Standard 128+128 run not reached. Short 11+16 correctness probe: 62.5 prompt tok/s; 6.8 decode tok/s. | 10.60 GiB on the short probe |
...
4. Investigate LFM2.5's small systematic numeric drift before changing the
   ...
   contract does not.
5. Pin or patch a working Nemotron-H Transformers reference path. Transformers
   5.14.1 recognizes the architecture and loads the checkpoint without remote
   code, but this pinned revision fails in native forward execution.
