...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pcm = MlxTensor::from_array(Array::zeros::<f32>(
        &[1, 1, frames * frame_samples],
        stream,
    )?);
    ...
    time("streaming_decode", audio_s, || {
        ...
        let decoded = MlxTensor::concatenate(&chunks, 2, stream)?;
        eval([decoded.as_array()])?;
        stream.synchronize()?;
        Ok(())
    })?;
