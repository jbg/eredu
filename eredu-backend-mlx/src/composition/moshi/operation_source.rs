...
use crate::backend::{
    ...
    runtime::execution::generic::{
        LayerwiseWorkspace, MlxLayerwisePolicy, MlxResidentPolicy, QualifiedRealtimeLayerwisePlan,
        QualifiedRealtimeNeuralPlan, RealtimeLayerwisePlan, RealtimeNeuralOwner,
        RealtimeNeuralPlan,
    },
...
impl<U: 'static> RealtimeOperationPolicy<U> for MlxResidentPolicy<U> {
    fn realtime_operation_plan(
        &self,
        stream: &Stream,
        _allocation: NativeAllocationFacts,
        _pool: &MemoryLedger,
        context: &WorkspaceContext,
    ) -> Result<RealtimeOperationPlan, Error> {
...
impl<U: 'static> RealtimeOperationPolicy<U> for MlxLayerwisePolicy<U, ()> {
    fn realtime_operation_plan(
        ...
        RealtimeOperationPlan::new_bounded(
            self.realtime_plan(source, stream, pool, context)?,
            context,
        )
    }
}
...
pub(crate) struct RealtimeOperationPlan {
    source: Box<dyn Source>,
    _funding: HostMetadataFunding,
}
pub(crate) struct RealtimeOperationRecipe {
    source: Box<dyn Qualified>,
    _funding: HostMetadataFunding,
}
trait Source {
    fn qualify(
...
impl RealtimeOperationPlan {
    pub(super) fn new<U: 'static>(
        plan: RealtimeNeuralPlan<U>,
        context: &WorkspaceContext,
    ) -> Result<Self, Error> {
        let funding = context
            .metadata_funding()
        ...
        reserve(
            &[
                ...
                size_of::<Self>(),
                size_of::<RealtimeNeuralPlan<U>>(),
                size_of::<Box<dyn Source>>(),
    ...
    fn new_bounded<U: 'static>(
        plan: RealtimeLayerwisePlan<U>,
        context: &WorkspaceContext,
    ) -> Result<Self, Error> {
        let funding = context
            .metadata_funding()
        ...
        reserve(
            &[
                size_of::<HostMetadataFunding>(),
                size_of::<Self>(),
                size_of::<RealtimeLayerwisePlan<U>>(),
                size_of::<Box<dyn Source>>(),
                size_of::<Result<Self, Error>>(),
            ],
            context,
    ...
    pub(crate) fn qualify(
        ...
        self.source.qualify(recipe, layerwise, context)
    }
}
impl<U: 'static> Source for RealtimeNeuralPlan<U> {
    fn qualify(
        ...
        if layerwise.is_some() {
            return Err(Error::Neural(WorkspaceMetadataError::Unqualified.into()));
        }
        let funding = context
            .metadata_funding()
            .ok_or_else(|| Error::Neural(WorkspaceMetadataError::Unqualified.into()))?;
        reserve(
            &[
                size_of::<HostMetadataFunding>(),
                size_of::<RealtimeOperationRecipe>(),
                size_of::<QualifiedRealtimeNeuralPlan<U>>(),
                size_of::<Box<dyn Qualified>>(),
                size_of::<Result<RealtimeOperationRecipe, Error>>(),
                size_of::<(
                    RealtimeNeuralPlan<U>,
        ...
        Ok(RealtimeOperationRecipe {
            source: Box::new((*self).qualify(recipe, context)?),
            _funding: funding,
        })
    }
}
...
impl<U: 'static> Source for RealtimeLayerwisePlan<U> {
    fn qualify(
        ...
        let source =
            layerwise.ok_or_else(|| Error::Neural(WorkspaceMetadataError::Unqualified.into()))?;
        let funding = context
            .metadata_funding()
            .ok_or_else(|| Error::Neural(WorkspaceMetadataError::Unqualified.into()))?;
        reserve(
            &[
                size_of::<HostMetadataFunding>(),
                size_of::<RealtimeOperationRecipe>(),
                size_of::<QualifiedRealtimeLayerwisePlan<U>>(),
                size_of::<Box<dyn Qualified>>(),
                size_of::<Result<RealtimeOperationRecipe, Error>>(),
                size_of::<(
                    RealtimeLayerwisePlan<U>,
                    SpeculativeNumericalRecipe,
                    &LayerwiseWorkspace,
                    &WorkspaceContext,
                )>(),
        ...
        Ok(RealtimeOperationRecipe {
            source: Box::new((*self).qualify(recipe, source, context)?),
            _funding: funding,
        })
    }
}
