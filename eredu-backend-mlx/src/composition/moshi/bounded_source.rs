...
use super::*;
use crate::backend::runtime::execution::generic::{prepare_layerwise_declarations,
...
pub(super) fn ordered_units(layout:&ExecutionUnitLayout,
    mut units:BTreeMap<ParameterGroupOwner,Vec<WeightBinding>>)->Result<Vec<Vec<WeightBinding>>,Error> {
    ...
    for ordinal in 0..layout.len() {
        ...
        let group=layout.group_id(address.group()).ok_or(Error::PrefillControl(
            eredu_runtime::working_memory::WorkingMemoryError::IdentityMismatch))?;
    ...
    if !units.is_empty(){return Err(Error::PrefillControl(
        eredu_runtime::working_memory::WorkingMemoryError::IdentityMismatch));}
    Ok(result)
}
...
pub(super) fn prepare(prepared:&moshi::PreparedMoshiRealtimeSource,
    backend:&crate::backend::MlxBackend<'static>)->Result<Option<PreparedLayerwiseManager>,Error> {
    let source=prepared.selected();
    let selected=source.selected();
    let residency=selected.residency();
    if !matches!(residency,LayerWeightResidency::LayerwiseHost(_)|LayerWeightResidency::DenseDiskStream(_))
        || prepared.lowering().transform().is_some() {
        return Ok(None);
    }
    if matches!(residency,LayerWeightResidency::DenseDiskStream(options)
        if options.samples_backend_memory()||options.samples_process_memory()) {return Ok(None);}
    match backend.original_copy_environment() {
        Ok(_)=>{},
        Err(crate::backend::OriginalCopyEnvironmentError::Memory(
            eredu_runtime::working_memory::WorkingMemoryError::UnknownBound))=>return Ok(None),
    ...
    let bindings=eredu_runtime::preflight_realtime_materialization_tasks::<MlxNeuralBackend>(
        tasks,prepared.source().as_ref()).map_err(|cause|Error::ArchitectureModel(cause.to_string()))?;
    ...
    let (static_bindings,unit_bindings)=selected_task_bindings(bindings,prepared.source().as_ref(),local_layout)?;
