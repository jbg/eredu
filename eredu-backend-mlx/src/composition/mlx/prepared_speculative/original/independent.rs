...
struct Lane<'lane, 'world, C: SpeculativeTokenFilterController> {
    ...
    partition: Option<crate::composition::mlx::session::OriginalModelPartitionPreparation>,
    ...
    V: SpeculativeGenerationVisitor,
{
    ...
    let controls = [
        ...
        size_of::<SpeculativeDraft<'_, MlxDrafter>>(),
        size_of::<V>(),
    ...
    let run = || {
        ...
        for lane in lanes {
            ...
            let controls = [
                ...
                size_of::<
                    Option<crate::composition::mlx::session::OriginalModelPartitionPreparation>,
                >(),
            ...
            let context_positions = u64::try_from(geometry.output_positions())
                .ok()
                .and_then(|output| input_positions.get().checked_add(output))
            ...
            let partition = session.prepare_original_model_partition_source(local)?;
            prepared
                .try_push(Lane {
                    lane,
                    input_positions,
        ...
        let with_target = |target: &mut crate::composition::mlx::model::Executable| {
            let with_draft =
                |draft: &mut crate::backend::MlxModel,
                 selected: &eredu_runtime::SelectedSpeculativeRealization| {
                    ...
                    let controls = [
                        ...
                        size_of::<
                            Result<AutoregressiveExecutor<'_, MlxAutoregressiveMechanisms>, Error>,
                        >(),
                    ...
                    for value in prepared {
                        ...
                        local
                            .reserve_metadata(schedule.control_bytes().ok_or(
                                Error::WorkspacePlanning(HostMetadataFundingError::Overflow),
                            )?)
                            .map_err(Error::WorkspacePlanning)?;
                        let pair = AutoregressiveSourcePair::prepare_funded(
                            target,
                            draft.executable(),
                            &schedule,
                        ...
                        .with_partition_source(eredu_runtime::speculative::autoregressive::AutoregressiveSource::Target, value.partition)?;
                        sources
                            .try_push(Bound {
                                sources: pair,
                        ...
                        plans.try_push(schedule).map_err(|cause| {
                            super::super::super::model::retain_planning_error(
                                cause,
                                funding.clone(),
                            )
                    ...
                    let invoke = || {
                        ...
                        let mut executor =
                            AutoregressiveExecutor::<MlxAutoregressiveMechanisms>::new(
                                target,
                                draft.executable_mut(),
                                capacity,
                            )
                            .with_schedules(plans, outer)?;
                        let mut runtime_lanes = executor.driver_buffer(count, outer)?;
                        let mut caches = executor.driver_buffer(count, outer)?;
                        for (index, (lane, source)) in
                            lanes.into_iter().zip(sources.iter()).enumerate()
                        {
                            let context = outer
                                .request_context(eredu_core::SpeculativeRequestId::new(index))?;
                    ...
                    funding
                        .reserve_metadata(size_of_val(&invoke))
                        .map_err(Error::WorkspacePlanning)?;
                    let result = invoke();
                    ...
                    for source in &sources {
                        if let Err(cause) = source.sources.request().close() {
                            if closed.is_ok() {
                                closed = Err(source.sources.retain_startup_error(cause));
                            }
                        }
                    }
                    match result {
