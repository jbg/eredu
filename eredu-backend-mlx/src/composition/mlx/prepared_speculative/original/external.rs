...
use crate::composition::mlx::{
    ...
    speculative::{
        external::{MlxExternalAssistantMechanisms, MlxExternalPredictionCache},
        ExternalInvocationSource, MlxExternalAssistant, OriginalExternalSources,
        OriginalSpeculativeNumericalPreparation,
    },
};
use eredu_architectures::{
    external_assistant::{ExternalSelectionSource, SelectedExternalAssistantVisitor},
    ExternalAssistantArchitecture, ExternalAssistantExecutorVisitor,
};
    ...
    V: SpeculativeGenerationVisitor,
{
    ...
    pay(
        ...
        [
            ...
            size_of::<
                eredu_core::SpeculativeBufferIntoIter<
    ...
    let run = || {
        ...
        pay(
            &funding,
            [
                size_of::<Option<PreparedOriginalCopyEnvironment>>(),
                size_of::<
                    Result<
                        Option<OriginalCopyEnvironment<'_>>,
                        crate::backend::OriginalCopyEnvironmentError,
                    >,
                >(),
                size_of::<
                    Result<PreparedOriginalCopyEnvironment, PreparedOriginalCopyEnvironmentError>,
                >(),
                ...
                size_of::<eredu_core::SpeculativeExecutionTopology>(),
        ...
        let draft_owner = match drafter.placement_environment().map_err(|cause| {
            ...
            Some(selected) => Some(
                PreparedOriginalCopyEnvironment::prepare(&selected, &host, &funding).map_err(
                    |cause| {
                        super::super::super::model::retain_planning_error(cause, funding.clone())
                    },
                )?,
        ...
        let draft_environment = match &draft_owner {
            Some(owner) => Some(owner.loan(backend.memory_ledger()).map_err(|cause| {
                super::super::super::model::retain_planning_error(cause, funding.clone())
            })?),
            None => None,
        };
        let draft = draft_environment.as_ref().unwrap_or(&environment);
        if drafter.stream() != draft.stream() || !environment.pool().same_ledger(draft.pool()) {
        ...
        for lane in lanes {
            ...
            let source = OriginalSpeculativeNumericalPreparation::prepare(
                model,
                backend.memory_ledger(),
                local.clone(),
            )?
            .with_parallel_source(session.original_workspace_parallel_source(local)?)?;
            ...
            let geometry = eredu_core::speculative::SpeculativeRequestGeometry::new(
                lane.config(),
                capacity.get(),
            );
        ...
        let mut control_sources = funding
            .metadata_vec(prepared.len())
            .map_err(Error::Neural)?;
        for lane in &prepared {
            if let Some(control) = lane.source.scheduling_control_projection()? {
                control_sources.push(control);
            }
        }
        let with_target = |target: &mut Executable| {
            target
                .erased()
                .install_speculative_parallel_controls(control_sources)?;

            let result = drafter.visit_selected_funded(
                funding.clone(),
                Select {
                    target,
                    lanes: prepared,
            ...
            let cleared = target
                .erased()
                .install_speculative_parallel_controls(Vec::new());
            match result {
                Err(cause) => Err(cause),
...
impl<C, V> SelectedExternalAssistantVisitor<MlxAssistantPreparationVisitor>
    for Select<'_, '_, '_, '_, C, V>
where
    C: SpeculativeTokenFilterController,
    V: SpeculativeGenerationVisitor,
{
    ...
    fn visit<A: ExternalAssistantArchitecture>(
        self,
        _assistant: &mut MlxExternalAssistant<A>,
        _selected: &eredu_runtime::SelectedSpeculativeRealization,
        _capture: &eredu_architectures::composite_execution::ExternalPredictionCaptureRequest,
    ) -> Self::Output {
    ...
    fn visit_source<A: ExternalAssistantArchitecture>(
        self,
        assistant: &mut MlxExternalAssistant<A>,
        selected: &ExternalSelectionSource,
    ) -> Self::Output {
        let count = self.lanes.len();
        pay(
            &self.funding,
            [
                size_of::<A::Executor<'_, MlxExternalAssistantMechanisms>>(),
                size_of::<Bound<'_>>(),
                size_of::<ExternalSchedulePlan<'_>>(),
                size_of::<PrefillControlPlan>(),
                size_of::<SpeculativeBuffer<Bound<'_>>>(),
                size_of::<SpeculativeBuffer<MlxExternalPredictionCache>>(),
        ...
        for value in self.lanes {
            ...
            let prefill = PrefillControlPlan::new(
                eredu_core::InferenceGeometry {
                    batch_size: 1,
                    cached_positions: value.frontier,
                    input_positions: value.input.get(),
                    max_output_tokens: value.output,
                    prefill_chunk_positions: value.chunk,
                    output: eredu_core::OutputDemand::LastPosition,
                },
                false,
            )
            .map_err(Error::PrefillControl)?;
            let schedule = ExternalSchedulePlan::new(
                selected.selected(),
                A::invocation_shape(),
                prefill,
                value.context,
            ...
            let source = OriginalExternalSources::prepare_from_source(
                value.source,
                schedule,
                value.preparation.limits().clone(),
                value.declaration,
            )?;
            ...
            let assistant_startup = numerical
                .request()
                .reserve_external_startup(OriginalExternalSpeculativeSource::Assistant, 0)
                .map_err(|cause| numerical.retain_startup_error(cause))?;

            let service = OriginalEmbeddedCachePreparation::new_external(
                assistant_startup,
                &value.preparation,
                numerical,
                self.environment,
            ...
            caches.try_push(cache).map_err(|cause| {
                super::super::super::model::retain_planning_error(cause, self.funding.clone())
            })?;
        ...
        let run = || {
            ...
            for source in &sources {
                let context = SpeculativeExecutionStreams::bind_original_external(
                    &source.sources,
                    self.environment,
                    self.draft,
                    self.topology,
                )?
                .with_original_cache_preparation(&source.cache)
                .map_err(Error::StorageSource)?;
                assignments.try_push(context).map_err(|cause| {
                    source
                        .sources
                        .numerical_sources()
                        .retain_startup_error(cause)
                })?;
            }
            ...
            for (index, (lane, source)) in lanes.into_iter().zip(sources.iter()).enumerate() {
                let context =
                    context.request_context(eredu_core::SpeculativeRequestId::new(index))?;
                pay(
                    source.preparation.metadata_funding(),
                    [
                ...
                runtime_lanes.try_push(prepared).map_err(|cause| {
                    source
                        .sources
                        .numerical_sources()
                        .retain_startup_error(cause)
            ...
            let target = self
                .target
                .erased_mut()
                .original_external_prediction_mut()
                .ok_or(Error::PrefillControl(WorkingMemoryError::UnknownBound))?;
            let runner = Runner {
                lanes: runtime_lanes,
                caches: &mut caches,
            ...
            A::visit_executor_borrowed::<MlxExternalAssistantMechanisms, _>(
                target,
                assistant,
                selected.capture(),
                runner,
        ...
        match result {
            Err(cause) => Err(retained(cause, &self.funding)),
...
fn prepare_cache(
    target: &Executable,
    selected: &ExternalSelectionSource,
    source: &OriginalExternalSources<'_>,
    preparation: &PreparedSemanticSource,
    ...
    pay(
        funding,
        [
            size_of::<MlxExternalPredictionCache>(),
            size_of::<Result<MlxExternalPredictionCache, Error>>(),
            size_of::<CacheHost>(),
            size_of::<HostPreparationAuthority>(),
            size_of::<ExternalSelectionSource>(),
            size_of::<(PreparedResidentDecoderCopy<'_>, ReplicatedTextControlOrigin)>(),
            size_of::<
                Result<
                    (PreparedResidentDecoderCopy<'_>, ReplicatedTextControlOrigin),
                    crate::composition::mlx::replicated_text::StartupCause,
                >,
            >(),
            size_of::<(MlxPredictionTargetState, u64)>(),
            size_of::<Result<(), String>>(),
            HostPreparationAuthority::retention_bytes::<CacheHost>()
                .ok_or(Error::PrefillControl(WorkingMemoryError::Overflow))?,
        ],
    )?;
    let startup = numerical
        .request()
        .reserve_external_startup(
            OriginalExternalSpeculativeSource::Target,
            u64::try_from(size_of::<MlxExternalPredictionCache>())
                .map_err(|_| Error::PrefillControl(WorkingMemoryError::Overflow))?,
        )
        .map_err(|cause| numerical.retain_startup_error(cause))?;
    let host = HostPreparationAuthority::retain(CacheHost {
        _startup: startup,
        _funding: funding.clone(),
    });

    let (state, origin) = target
        .erased()
        .prepare_original_external_target_source()
        .map_err(|cause| numerical.retain_startup_error(cause))?;
    ...
    let native = MlxPredictionTargetState::copy_original_source(
        state,
        environment,
        roots,
        mechanisms,
        funding,
        preparation.limits().clone(),
    )?;

    if native.generation_fixed() != Some(frontier) {
        return Err(numerical.retain_startup_error(WorkingMemoryError::IdentityMismatch));
    }
    let frontier = i32::try_from(frontier)
        .map_err(|_| numerical.retain_startup_error(WorkingMemoryError::Overflow))?;
    let mut cache =
        MlxExternalPredictionCache::from_selected_source(native, selected.clone(), host);
    cache
        .advance_frontier(frontier)
        .map_err(|cause| numerical.retain_error(Error::ArchitectureModel(cause)))?;
    Ok(cache)
}
...
impl<A, C, V> ExternalAssistantExecutorVisitor<A, MlxExternalAssistantMechanisms>
    for Runner<'_, '_, '_, C, V>
where
