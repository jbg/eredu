...
use crate::composition::mlx::{
    replicated_text::{
        OriginalEmbeddedCachePreparation, OriginalPredictionLane, OriginalPredictionTarget,
    },
...
use eredu_runtime::{
    prefill::PrefillControlPlan, speculative::embedded_occurrence::EmbeddedSchedulePlan,
    ...
    let result = (|| {
        ...
        session.with_model_operation_funded(funding.clone(), |target| {
            ...
            let result = target
                ...
                .unwrap_or(Err(Error::PrefillControl(WorkingMemoryError::UnknownBound)));
            let cleared = target
                .erased()
...
fn prepare<'lane, 'world, C: SpeculativeTokenFilterController>(
    backend: &MlxBackend<'_>,
    ...
    let result = (|| {
        ...
        .with_parallel_source(session.original_workspace_parallel_source(funding)?)?;
        let prediction = session
            .prepare_original_prediction_lane(&preparation, environment)?
            .ok_or(Error::PrefillControl(WorkingMemoryError::UnknownBound))?;
        let geometry = eredu_core::speculative::SpeculativeRequestGeometry::new(
            lane.config(),
            prediction.proposal_capacity(),
        );
        let output_positions = u64::try_from(geometry.output_positions())
            .map_err(|_| Error::PrefillControl(WorkingMemoryError::Overflow))?;
        let context = frontier
            .checked_add(input_positions.get())
            .and_then(|n| n.checked_add(output_positions))
            .ok_or(Error::PrefillControl(WorkingMemoryError::Overflow))?;
        let target = session.prepare_original_prediction_target(
            &prediction,
            &preparation,
            environment,
            frontier,
...
impl<C: SpeculativeTokenFilterController, V: SpeculativeGenerationVisitor>
    MlxEmbeddedExecutorContinuation for Continuation<'_, '_, '_, C, V>
{
    fn construction_controls(&self, bytes: Option<usize>) -> Result<(), Error> {
        self.funding
            .reserve_metadata(
                bytes.ok_or(Error::WorkspacePlanning(HostMetadataFundingError::Overflow))?,
            )
            .map_err(Error::WorkspacePlanning)
    }
    fn execute(
        ...
        let parts = [
            size_of::<Bound<'_>>(),
            size_of::<EmbeddedSchedulePlan<'_>>(),
        ...
        for value in prepared {
            if value.target.frontier() != value.cached_positions {
                return Err(Error::PrefillControl(WorkingMemoryError::IdentityMismatch));
            }
            let prefill = PrefillControlPlan::new(
                eredu_core::InferenceGeometry {
                    batch_size: 1,
                    cached_positions: value.cached_positions,
                    input_positions: value.input_positions.get(),
                    max_output_tokens: value.output_positions,
            ...
            let schedule = EmbeddedSchedulePlan::new(
                selected,
                value.prediction.occurrence_shape(),
                value.prediction.prefill_alignment(),
                value.prediction.initial_frontier(),
                prefill,
                value.context,
                value.lane.config(),
                self.options,
            ...
            .map_err(|cause| {
                super::super::super::model::retain_planning_error(
                    cause,
                    value.preparation.metadata_funding().clone(),
                )
            })?;
            let source = OriginalEmbeddedSources::prepare_from_source(
                value.source,
                schedule,
                value.preparation.limits().clone(),
            )?
            .with_intervention_declaration(value.declaration)?;
            let cache = OriginalEmbeddedCachePreparation::new(
                value.prediction,
                value.target,
                source.numerical_sources(),
            )
        ...
        let result = (|| {
            ...
            for (index, (lane, source)) in lanes.into_iter().zip(sources.iter()).enumerate() {
                let streams =
                    outer.request_context(eredu_core::SpeculativeRequestId::new(index))?;
                let (one_lane, one_cache) = prepare_lane_buffers(
                    executor,
                    lane,
                    &source.preparation,
                    streams,
                    |executor, streams| {
                        executor
                            .new_cache_with_context(streams)
                            .map_err(Error::StorageSource)
                    },
                )?;
                runtime_lanes.try_extend(one_lane).map_err(|cause| {
                    source
                        .sources
                        .numerical_sources()
                        .retain_startup_error(cause)
                })?;
                caches.try_extend(one_cache).map_err(|cause| {
                    source
                        .sources
                        .numerical_sources()
                        .retain_startup_error(cause)
