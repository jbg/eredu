...
use eredu_runtime::{speculative::{embedded_occurrence::EmbeddedInvocationWorkspace,
    external_occurrence::ExternalInvocation}, working_memory::{InferenceSpanWorkspacePlan,
    OriginalExternalSpeculativeRole, WorkingMemoryError}};
...
pub(crate) trait ModelEquation {
    ...
    fn record(&self) -> &ResidentSpanRecipe { &self.native_recipe().records()[0] }
    fn equation_domains(&self, preparation_graph_bytes: usize)
...
impl ModelEquation for EmbeddedEquationRecipe {
    fn plan(&self) -> &InferenceSpanWorkspacePlan { self.plan() }
    fn native_recipe(&self) -> &ResidentNativeRecipe { self.native_recipe() }
    fn identity(&self) -> EquationIdentity { EquationIdentity::Embedded(self.workspace()) }
    fn equation_domains(&self, preparation_graph_bytes: usize)
        -> Result<(ResidentCompletionRecipe, usize, usize, usize, u64), Error> {
...
impl ExternalEquationRecipe {
    pub(crate) fn new(invocation: ExternalInvocation, recipe: ResidentNativeRecipe)
        -> Result<Self, Error> {
    ...
    pub(crate) fn invocation(&self) -> ExternalInvocation { self.invocation }
    pub(crate) fn plan(&self) -> &InferenceSpanWorkspacePlan { self.recipe.plan() }
    pub(crate) fn native_recipe(&self) -> &ResidentNativeRecipe { &self.recipe }
    pub(crate) fn with_native_recipe<T>(&mut self, run: impl FnOnce(&mut ResidentNativeRecipe) -> T) -> T {
        run(&mut self.recipe)
    }
}
impl ModelEquation for ExternalEquationRecipe {
    fn plan(&self) -> &InferenceSpanWorkspacePlan { self.plan() }
    fn native_recipe(&self) -> &ResidentNativeRecipe { self.native_recipe() }
    fn identity(&self) -> EquationIdentity { EquationIdentity::External(self.invocation) }
    fn equation_domains(&self, preparation_graph_bytes: usize)
        -> Result<(ResidentCompletionRecipe, usize, usize, usize, u64), Error> {
        self.recipe.external_equation_domains(preparation_graph_bytes)
    }
    fn validation_control_bytes(&self) -> Result<u64, Error> {
...
pub(crate) trait ModelRole {
    ...
    fn prepare_validations(&self, recipe: &Self::Equation) -> Result<TokenValidationIngress, Error>;
}
impl ModelRole for OriginalEmbeddedSpeculativeRole {
    type Equation = EmbeddedEquationRecipe;
    fn validate_equation(&self, recipe: &Self::Equation) -> Result<(), Error> {
        self.validate_plan(recipe.plan()).and_then(|_| self.validate_invocation(recipe.workspace().invocation()))
            .map_err(Error::PrefillControl)
    }
    fn budget_custody(&self) -> OriginalSpeculativeBudgetCustody { self.budget_custody() }
    fn physical_bytes(&self) -> u64 { self.physical_bytes() }
    fn graph_bytes(&self) -> u64 { self.graph_bytes() }
    fn record_bytes(&self) -> u64 { self.record_bytes() }
    fn prepare_validations(&self, recipe: &Self::Equation) -> Result<TokenValidationIngress, Error> {
...
impl ModelRole for OriginalExternalSpeculativeRole {
    ...
    fn validate_equation(&self, recipe: &Self::Equation) -> Result<(), Error> {
        self.validate_plan(recipe.plan()).map_err(|cause| Error::PrefillControl(cause).at_speculative_stage("external equation plan"))?;
        if self.invocation() != recipe.invocation { return Err(Error::PrefillControl(WorkingMemoryError::IdentityMismatch).at_speculative_stage("external equation invocation")); }
        Ok(())
    }
    fn budget_custody(&self) -> OriginalSpeculativeBudgetCustody { self.budget_custody() }
    fn physical_bytes(&self) -> u64 { self.physical_bytes() }
