...
use crate::backend::{
    ...
    submission_recovery::addressable::{SpeculativeAddressableSources,SpeculativeAddressableSpan,AddressableExecutionRow},
};
...
fn total(parts:&[usize])->Option<usize>{parts.iter().copied().try_fold(size_of_val(parts),usize::checked_add)}
...
where F:FnOnce(&mut W,&Q,&ActiveEmbeddedNativeInvocation<'_,R>)->Result<T,Error>{
    fn invoke(&mut self)->Result<bool,Error>{
        let run=self.run.take().ok_or_else(identity)?;
        let result=run(self.work,self.payload,self.active);let success=result.is_ok();
        self.output=Some(result);Ok(success)
    }
}
impl AddressableModelSource {
    pub(crate) fn prepare(records:&[ResidentSpanRecipe],target:Option<HostSourceConstructionFacts>,
        ...
        let facts=facts.pop().flatten().ok_or_else(identity)?;
        let controls=source.controls().checked_add(u64::try_from(Self::binding_controls().ok_or_else(overflow)?)
            .map_err(|_|overflow())?).ok_or_else(overflow)?;
        Ok((Some(Self{pending:RefCell::new(Some(source.take(0)?.ok_or_else(identity)?)),
            access:RefCell::new(None),funding:funding.clone()}),Some(facts),controls))
    }
    fn binding_controls()->Option<usize>{
        total(&[size_of::<&Self>(),size_of::<Option<OriginalHostSourceBank>>(),
            ...
            size_of::<OriginalSelectedResidencyAccess>(),size_of::<&SpeculativeNeuralOwner>(),
            size_of::<Result<(),Error>>(),size_of::<&Self>(),
            size_of::<Option<crate::backend::runtime::execution::generic::SpeculativeSourcePartition<'_>>>()])
    }
    pub(crate) fn accept(&self,root:Option<OriginalHostSourceBank>)->Result<Option<OriginalHostSourceBank>,Error>{
        self.funding.reserve_metadata(Self::binding_controls().ok_or_else(overflow)?).map_err(Error::WorkspacePlanning)?;
        self.pending.try_borrow_mut().map_err(|_|identity())?.as_mut().ok_or_else(identity)?.accept(root)
    }
    pub(crate) fn bind(&self,bank:&SpeculativeNeuralOwner)->Result<(),Error>{
        ...
        if slot.is_some(){return Err(identity());}
        *slot=Some(bank.selected_residency_access()?);Ok(())
    }
    ...
    fn call_controls<W,Q,R:ModelRole,T,F>()->Option<u64>{
        u64::try_from(total(&[
            size_of::<Call<'_,'_,W,Q,R,T,F>>(),size_of::<&mut Call<'_,'_,W,Q,R,T,F>>(),
            size_of::<(&Self,&mut W,&Q,&ActiveEmbeddedNativeInvocation<'_,R>,F)>(),
            size_of::<SpeculativeAddressableSpan>(),size_of::<Option<SpeculativeAddressableSpan>>(),
            size_of::<OriginalSelectedResidencyAccess>(),size_of::<Option<OriginalSelectedResidencyAccess>>(),
            size_of::<std::cell::RefMut<'_,Option<SpeculativeAddressableSpan>>>(),
            size_of::<std::cell::RefMut<'_,Option<OriginalSelectedResidencyAccess>>>(),
            size_of::<OriginalBufferBudget>(),size_of::<AddressableExecutionRow>(),
            size_of::<Result<AddressableExecutionRow,Error>>(),size_of::<Result<(),Error>>(),
            size_of::<Result<bool,Error>>(),size_of::<Option<Result<T,Error>>>(),size_of::<Result<T,Error>>(),
            size_of::<bool>(),
        ])?).ok()
    }
    pub(crate) fn run<W,Q,R:ModelRole,T,F>(&self,work:&mut W,payload:&Q,
        active:&ActiveEmbeddedNativeInvocation<'_,R>,run:F)->Result<T,Error>
    where F:FnOnce(&mut W,&Q,&ActiveEmbeddedNativeInvocation<'_,R>)->Result<T,Error>{
        self.funding.reserve_metadata(usize::try_from(Self::call_controls::<W,Q,R,T,F>()
            .ok_or_else(overflow)?).map_err(|_|overflow())?).map_err(Error::WorkspacePlanning)?;
        let source=self.pending.try_borrow_mut().map_err(|_|identity())?.take().ok_or_else(identity)?;
        let access=self.access.try_borrow_mut().map_err(|_|identity())?.take().ok_or_else(identity)?;
        let row=source.activate(access,active.budget().clone(),active.observer(),None,&self.funding)?;
        let mut call=Call{run:Some(run),work,payload,active,output:None};
