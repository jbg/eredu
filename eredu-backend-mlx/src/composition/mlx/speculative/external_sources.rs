...
use crate::{backend::error::Error, composition::mlx::{model::retain_planning_error,
...
impl<'a> OriginalExternalSources<'a> {
    ...
    pub(crate) fn prepare_from_source(source: OriginalSpeculativeNumericalPreparation,
        schedule: ExternalSchedulePlan<'a>, capacity: u64,
        declaration: Option<OriginalInterventionDeclaration>) -> Result<Self, Error>
    {
        ...
        let numerical = source.bind(request)?.with_intervention_declaration(declaration)?;
        Ok(Self { cursor: RefCell::new(schedule.into_cursor()), scheduler_request: Cell::new(None), numerical })
    }
}
...
pub(crate) trait ExternalInvocationSource: std::fmt::Debug {
    ...
    fn claim(&self, kind: ExternalInvocationKind, geometry: InferenceGeometry,
...
impl ExternalInvocationSource for OriginalExternalSources<'_> {
    fn numerical_sources(&self) -> &OriginalSpeculativeNumericalSources { &self.numerical }
    fn claim(&self, kind: ExternalInvocationKind, geometry: InferenceGeometry,
        span: Option<SpeculativePrefillSpan>, origin: SpeculativeActivationOrigin)
        -> Result<ExternalOccurrenceClaim<'_>, Error>
    {
        let frames = [size_of::<ExternalOccurrenceClaim<'_>>(),
            size_of::<Result<ExternalOccurrenceClaim<'_>, Error>>(),
            size_of::<std::cell::RefMut<'_, ExternalOccurrenceCursor<'_>>>(),
            size_of::<(ExternalInvocationKind, InferenceGeometry, Option<SpeculativePrefillSpan>, SpeculativeActivationOrigin)>(),
        ...
        let invocation = cursor.plan().invocation(kind, geometry, span, origin)
            .map_err(|error| self.numerical.retain_startup_error(error))?;
        self.scheduler_request.set(Some(origin.request));
        cursor.claim(invocation).map_err(|error| self.numerical.retain_startup_error(error))
    }
    fn prepare_continuation(&self, committed: usize,
        status: eredu_core::generation::SpeculativeRequestStatus) -> Result<(), Error>
    {
        let frames = [size_of::<(&Self, usize, eredu_core::generation::SpeculativeRequestStatus)>(),
            size_of::<ExternalContinuation>(), size_of::<Result<ExternalContinuation, ExternalOccurrenceError>>(),
            size_of::<std::cell::RefMut<'_, ExternalOccurrenceCursor<'_>>>(),
            size_of::<Result<(), Error>>(), size_of::<Result<(), ExternalOccurrenceError>>(),
        ...
        let continuation = cursor.continuation(committed, status)
            .map_err(|error| self.numerical.retain_startup_error(error))?;
        self.numerical.request().prepare_external_continuation(&continuation, funding)
            .map_err(|error| self.numerical.retain_startup_error(error))?;
        cursor.install_continuation(continuation).map_err(|error| self.numerical.retain_startup_error(error))
    }
}
