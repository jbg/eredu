...
impl<'a> SpeculativeExecutionStreams<'a> {
    ...
    pub(crate) fn with_request_assignments(mut self, assignments: &'a [Self]) -> Result<Self, Error> {
        let (first, _) = self.original_numerical()
            .ok_or(Error::PrefillControl(WorkingMemoryError::IdentityMismatch))?;
        let controls = [std::mem::size_of::<Self>(), std::mem::size_of::<(&Self, &[Self])>(),
            std::mem::size_of::<Result<Self, Error>>(), std::mem::size_of::<std::slice::Iter<'_, Self>>(),
        ...
        for (index, value) in assignments.iter().enumerate() {
            let (source, environment) = value.original_numerical()
                .ok_or(Error::PrefillControl(WorkingMemoryError::IdentityMismatch))?;
            if value.batch_assignments.is_some() || value.batch_request.is_some()
                || value.embedded_invocation.is_some() || value.tensor_sources.is_some() || value.prefill_input.is_some()
                || value.target != self.target || value.draft != self.draft || value.topology != self.topology
                || value.memory_owner.is_some() || value.capture.is_some()
                || !source.target_origin().same_origin(first.target_origin())
                || !environment.pool().same_domain(first.pool())
                || (index == 0 && !std::ptr::eq(source, first)) {
                return Err(Error::PrefillControl(WorkingMemoryError::IdentityMismatch));
            }
            if assignments[..index].iter().any(|prior|
                std::ptr::eq(prior.original_numerical().expect("validated prefix").0.request(), source.request())) {
                return Err(Error::PrefillControl(WorkingMemoryError::IdentityMismatch));
            }
        }
    ...
    pub(crate) fn request_context(self, request: SpeculativeRequestId) -> Result<Self, Error> {
        match (self.batch_assignments, self.batch_request) {
            (Some(assignments), None) => {
                let mut selected = *assignments.get(request.index())
                    .ok_or(Error::PrefillControl(WorkingMemoryError::IdentityMismatch))?;
