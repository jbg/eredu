...
impl<'a> SpeculativeExecutionStreams<'a> {
    pub(crate) fn bind_original_external(
        ...
        let frames = [
            ...
            size_of::<(&dyn ExternalInvocationSource, &OriginalCopyEnvironment<'_>, &OriginalCopyEnvironment<'_>, SpeculativeExecutionTopology)>(),
            size_of::<[safemlx::StreamCopyPlan<()>; 2]>(),
            size_of::<Result<safemlx::StreamCopyPlan<()>, safemlx::StreamCopyCause>>(),
            size_of::<(bool, bool)>(),
            size_of::<(safemlx::DeviceType, safemlx::DeviceType, safemlx::CpuMatmulKernel, safemlx::CpuMatmulKernel)>(),
            size_of::<[usize; 2]>(),
        ];
        ...
        let valid = match topology {
            ...
            SpeculativeExecutionTopology::CrossDeviceSplit => !same_device &&
        ...
        if !valid || !target.pool().same_domain(draft.pool()) {
