...
fn ordinary_error(message: impl Into<String>) -> Error { Exception::custom(message).into() }
    ...
    A: ExternalAssistantArchitecture,
{
    ...
    ) -> Result<eredu_core::SpeculativeBuffer<eredu_core::SpeculativeScheduleState>, eredu_core::BackendFailure> { context.coordinate_speculative_step(local) }
    fn driver_identity(context:Self::Context<'_>)->Result<eredu_core::SpeculativeRequestIdentity,Self::Error>{super::autoregressive::host_containers::identity(context)}
    fn copy_sequence(source:eredu_core::SpeculativeSequenceRef<'_>,context:Self::Context<'_>)->Result<eredu_core::SpeculativeSequence,eredu_core::SpeculativeDriverError<Self::Error>>{super::autoregressive::sequence::copy(source,context)}
    fn sequence_copy_bytes(source:&eredu_core::SpeculativeSequence)->Option<u64>{super::autoregressive::sequence::copy_bytes(source)}
    fn take_retained_failure(error:Self::Error)->Result<eredu_core::BackendFailure,Self::Error>{error.take_retained_backend_failure()}
    ...
    fn prepare_control_continuation<'a>(committed: usize,
        ...
        match context.original_external() {
            Some(source) => source.prepare_continuation(committed, status).map_err(Error::from),
            None => Ok(()),
        }
    }
    ...
    fn source_context<'a, 'scope>(context: Self::Context<'a>,
        ...
        if context.original_external().is_some() {
            context.with_external_tensor_sources(sources).map_err(Error::from)
        } else { Ok(context) }
    }

    fn target_tokens_with_source<'a>(tokens: &[u32], context: Self::Context<'a>)
        -> Result<eredu_architectures::speculative_execution::EmbeddedPredictionTensor<MlxTensor>, Self::Error> {
        if let Some((sources, environment)) = context.original_numerical() {
            let preparation = context.original_cache_preparation().ok_or_else(||
                Error::from(sources.retain_startup_error(eredu_runtime::working_memory::WorkingMemoryError::IdentityMismatch)))?;
            super::prepared_prediction_token_ids(tokens, sources, environment, preparation).map_err(Error::from)
        } else {
    ...
    fn transfer_with_source<'a>(value: &eredu_architectures::speculative_execution::EmbeddedPredictionTensor<MlxTensor>,
        ...
        if let Some((sources,_)) = context.original_numerical() {
            ...
            let controls=[std::mem::size_of::<Self::Context<'_>>(),std::mem::size_of::<ExternalAssistantTransfer>(),
                std::mem::size_of::<Packet<MlxTensor>>(),std::mem::size_of::<Result<Packet<MlxTensor>,Self::Error>>(),
            ...
            match context.topology(){
                eredu_core::SpeculativeExecutionTopology::Single|eredu_core::SpeculativeExecutionTopology::SameDeviceSplit=>{
                    let placement=match direction {
                        ExternalAssistantTransfer::TargetToDraft=>eredu_core::SamplingPlacement::Draft,
                        ExternalAssistantTransfer::DraftToTarget=>eredu_core::SamplingPlacement::Target,
                    };
                    let evidence=value.evidence().ok_or_else(||<Self as ExternalAssistantExecutionMechanisms<A>>::state_refusal(context))?;
                    super::tensor_sources::input_array_environment(evidence,value.as_array(),context,placement,funding)?;
                ...
                eredu_core::SpeculativeExecutionTopology::CrossDeviceSplit=>{
                    ...
                    let evidence=transferred.evidence.ok_or_else(||<Self as ExternalAssistantExecutionMechanisms<A>>::state_refusal(context))?;
                    let host=crate::composition::mlx::replicated_text::cache_metadata(
                        Packet::<MlxTensor>::retained_control_bytes(),context).map_err(Error::StorageSource)?;
                    return Ok(Packet::from_prepared(transferred.output,evidence,host));
                }
                _=>return Err(<Self as ExternalAssistantExecutionMechanisms<A>>::state_refusal(context)),
            }
        }
    ...
    fn assistant_operation_with_evidence<I: eredu_architectures::external_assistant::invocation::ExternalAssistantOperation<A>>(
        ...
        <Self as ExternalAssistantExecutionMechanisms<A>>::assistant_operation::<I>(assistant, arguments, context)
    ...
    fn control_copy_tensor_with_source(value:&Self::Tensor,
        ...
        placement:ExternalAssistantTensorPlacement,context:Self::Context<'_>)
        ->Result<eredu_architectures::external_assistant::ExternalOperationResult<Self::Tensor>,Self::Error>{
        if context.original_external().is_some(){state::copy(value,evidence,placement,context)}
        else{<Self as ExternalAssistantExecutionMechanisms<A>>::control_copy_tensor(value,placement,context)
            .map(eredu_architectures::external_assistant::ExternalOperationResult::ordinary)}
    }
    ...
    fn token_prefix_with_source<'a>(value:&eredu_architectures::speculative_execution::EmbeddedPredictionTensor<Self::Tensor>,end:usize,context:Self::Context<'a>)
        ...
        if context.original_numerical().is_some(){
            let evidence=value.evidence().ok_or_else(||<Self as ExternalAssistantExecutionMechanisms<A>>::state_refusal(context))?;
            return super::sampling::numerical::tensor_axis_range_at(value,1,0,end,true,evidence,context,eredu_core::SamplingPlacement::Target);
        }
        <Self as ExternalAssistantExecutionMechanisms<A>>::token_prefix(value,end,context).map(eredu_architectures::speculative_execution::EmbeddedPredictionTensor::ordinary)
    }
    fn transfer_tensor_with_source<'a>(value:&Self::Tensor,
        evidence:Option<&eredu_architectures::speculative_execution::PreparedEmbeddedEvidence>,direction:ExternalAssistantTransfer,context:Self::Context<'a>)
        ->Result<eredu_architectures::external_assistant::ExternalOperationResult<Self::Tensor>,Self::Error>{
        if context.original_numerical().is_none(){return <Self as ExternalAssistantExecutionMechanisms<A>>::transfer(value,direction,context).map(eredu_architectures::external_assistant::ExternalOperationResult::ordinary);}
        state::transfer(value,evidence,direction,context)
    }
    fn join_tensor_sources<'a>(visit: impl FnMut(&mut dyn FnMut(&Self::Tensor)),
        evidence: &[&eredu_architectures::speculative_execution::PreparedEmbeddedEvidence], context: Self::Context<'a>,
    ) -> Result<Option<eredu_architectures::speculative_execution::PreparedEmbeddedEvidence>, Self::Error> {
    ...
    fn identity_text(arguments:std::fmt::Arguments<'_>,context:Self::Context<'_>)->Result<eredu_runtime::SpeculativeIdentity,Self::Error>{
        if let Some((sources,_))=context.original_numerical(){
            sources.metadata_funding().reserve_metadata(std::mem::size_of::<(std::fmt::Arguments<'_>,String,
                eredu_runtime::SpeculativeIdentity,Result<eredu_runtime::SpeculativeIdentity,Self::Error>)>()).map_err(Error::WorkspacePlanning)?;
            let value=sources.metadata_funding().metadata_string(arguments).map_err(Error::Neural)?;
            eredu_runtime::SpeculativeIdentity::new(value).map_err(|cause|sources.retain_startup_error(cause))
    ...
    fn capture_shape_matches(value:&Self::Tensor,entry:&eredu_runtime::SpeculativeCaptureEntry,
        context:Self::Context<'_>)->Result<bool,Self::Error>{
        if let Some((sources,_))=context.original_numerical(){
            sources.metadata_funding().reserve_metadata(std::mem::size_of::<(
                &Self::Tensor,&eredu_runtime::SpeculativeCaptureEntry,&[i32],bool,Result<bool,Self::Error>,
            )>()).map_err(Error::WorkspacePlanning)?;
        }
        let shape=value.as_array().shape();
        Ok(entry.matches_dimensions(shape.len(),|i|shape.get(i).and_then(|n|usize::try_from(*n).ok())))
    }
    fn capture_contract_error(error:eredu_runtime::SpeculativeCaptureError,
        context:Self::Context<'_>)->Self::Error{
    ...
    fn begin_prefill_chunk(
        ...
        assistant.observers.begin_prefill_chunk(chunk).map_err(Error::from)
    }
    fn begin_prefill_context(
        assistant: &mut Self::Assistant,
        frontier: u64,
    ) -> Result<(), Self::Error> {
        assistant.observers.begin_prefill_context(frontier).map_err(Error::from)
    }
    ...
    fn verify_target_with_evidence<'a>(
        target: &mut Self::Target, request: &ExternalPredictionCaptureRequest,
        tokens: &Self::Tensor, cache: &mut Self::NativeCache, context: Self::Context<'a>,
    ) -> Result<eredu_architectures::external_assistant::ExternalTargetResult<Self::Tensor>, Self::Error> {
    ...
    fn control_checkpoint<'a>(
        ...
        if context.original_external().is_some(){original_cache_copy(cache,context)}
        else{cache.control_copy(context.target(), &context.memory_pool()).map_err(Error::from)}
    }
    fn control_restore<'a>(
        ...
        let mut replacement=if context.original_external().is_some(){original_cache_copy(saved,context)?}
            else{saved.control_copy(context.target(), &context.memory_pool())?};
    ...
    fn control_copy_tensor<'a>(
        ...
        if let Some((sources,_))=context.original_numerical(){
            return Err(sources.retain_startup_error(eredu_runtime::working_memory::WorkingMemoryError::UnknownBound));
        }
        let owner =
            NativeMemoryOwner::acquire(&context.memory_pool()).map_err(Error::from)?;
    ...
    fn restore_checkpoint_native<'a>(
        ...
        if context.original_external().is_some(){
            ...
            replacement.inherit_retention(cache)?;*cache=replacement;Ok(())
        }else{cache.restore(checkpoint, context.target()).map_err(Error::from)}
    }
    ...
    fn observe_tensor(
        ...
        if !assistant.observers.has_tensor_observer() { return Ok(value); }
        assistant.observers.observe_tensor(path, &value).map_err(Error::from)
    }
    ...
    fn observe_logits(
        ...
        if !assistant.observers.has_logits_observer() { return Ok(value); }
        let value = value.ordinary().ok_or(Error::PrefillControl(
            eredu_runtime::working_memory::WorkingMemoryError::UnknownBound))?;
        assistant.observers.observe_logits(path, value).map(IndependentLogits::Ordinary).map_err(Error::from)
    }
    ...
    fn sequence_row<'a>(
        ...
        let row = i32::try_from(row)
            .map_err(|_| ordinary_error("external assistant row exceeds i32"))?;
        let stream = match placement {
            ExternalAssistantTensorPlacement::Target => context.target(),
            ExternalAssistantTensorPlacement::Draft => context.draft(),
        };
    ...
    fn into_logits_with_source<'a>(value: Self::Tensor,
        evidence: Option<&eredu_architectures::speculative_execution::PreparedEmbeddedEvidence>,
        context: Self::Context<'a>,
    ) -> Result<Self::Logits, Self::Error> { logits::selected(value, evidence, context) }

    fn into_logits_with_source_at<'a>(value:Self::Tensor,
        evidence:Option<&eredu_architectures::speculative_execution::PreparedEmbeddedEvidence>,
        placement:ExternalAssistantTensorPlacement,context:Self::Context<'a>,
    )->Result<Self::Logits,Self::Error>{logits::selected_at(value,evidence,placement,context)}

    fn logits_row_with_source<'a>(value: &Self::Tensor, row: usize,
        evidence: Option<&eredu_architectures::speculative_execution::PreparedEmbeddedEvidence>,
        placement: ExternalAssistantTensorPlacement, context: Self::Context<'a>,
    ) -> Result<Self::Logits, Self::Error> { logits::row(value, row, evidence, placement, context) }
    ...
    fn target_operation_with_source<'a>(target:&mut Self::Target,
        operation:ExternalPredictionTargetOperation<'_,Self::Tensor>,context:Self::Context<'a>,
    )->Result<eredu_architectures::speculative_execution::EmbeddedPredictionTensor<Self::Tensor>,Self::Error>{
    ...
    fn observe_borrowed_tensor<'a>(assistant:&mut Self::Assistant,path:&str,value:&Self::Tensor,
        evidence:Option<&eredu_architectures::speculative_execution::PreparedEmbeddedEvidence>,context:Self::Context<'a>)->Result<Self::Tensor,Self::Error>{
        if context.original_numerical().is_some(){
            if assistant.observers.has_tensor_observer(){return Err(<Self as ExternalAssistantExecutionMechanisms<A>>::state_refusal(context));}
            return state::alias(value,evidence,ExternalAssistantTensorPlacement::Target,context).map(|value|value.output);
        }
        <Self as ExternalAssistantExecutionMechanisms<A>>::observe_tensor(assistant,path,value.clone())
    }
    fn state_host_metadata<'a>(bytes:Option<usize>,context:Self::Context<'a>)->Result<eredu_core::HostPreparationAuthority,Self::Error>{
        crate::composition::mlx::replicated_text::cache_metadata(bytes,context).map_err(Error::StorageSource)
    }
    fn state_buffer<'a,T>(capacity:usize,context:Self::Context<'a>)->Result<eredu_core::SpeculativeBuffer<T>,Self::Error>{
    ...
    fn state_vector<T>(capacity:usize,context:Self::Context<'_>)->Result<Vec<T>,Self::Error>{
        if let Some((sources,_))=context.original_numerical(){
            let funding=sources.metadata_funding();
            (||{
                funding.reserve_metadata(std::mem::size_of::<(Vec<T>,Result<Vec<T>,Error>,usize)>()).map_err(Error::WorkspacePlanning)?;
                funding.metadata_vec(capacity).map_err(Error::Neural)
            })().map_err(|cause|sources.retain_error(cause))
        }else{Ok(Vec::with_capacity(capacity))}
    }
    ...
    fn submit_completion_with_sources<'a,'c,I>(values:I,
        evidence:&[&eredu_architectures::speculative_execution::PreparedEmbeddedEvidence],context:Self::Context<'c>)
        ...
        let Some((sources,environment))=context.original_numerical() else {
            return MlxSpeculativeCompletion::submit(values.into_iter().map(MlxTensor::as_array)).map(TypedSpeculativeCompletion::new).map_err(Error::from);
        };
    ...
    where
        Self::Tensor: 'a,
    {
        MlxSpeculativeCompletion::submit(values.into_iter().map(MlxTensor::as_array)).map(TypedSpeculativeCompletion::new).map_err(Error::from)
    }
}
...
fn original_cache_copy(cache:&MlxPredictionTargetState,context:SpeculativeExecutionStreams<'_>)->Result<MlxPredictionTargetState,Error>{
    let (sources,environment)=context.original_numerical().ok_or(Error::PrefillControl(eredu_runtime::working_memory::WorkingMemoryError::IdentityMismatch))?;
    sources.validate_environment(environment)?;
    let funding=sources.metadata_funding();
    funding.reserve_metadata(std::mem::size_of::<(&MlxPredictionTargetState,SpeculativeExecutionStreams<'_>,
        MlxPredictionTargetState,Result<MlxPredictionTargetState,Error>)>()).map_err(Error::WorkspacePlanning)?;
    let (roots,mechanisms)=sources.numerical_prerequisites();
    cache.copy_original(environment,roots,mechanisms,funding,sources.request().capacity_bytes())
        .map_err(|cause|sources.retain_error(cause))
}
