...
use crate::backend::{runtime::cache::state::CompletedResidentSource, OriginalCopyEnvironment};
...
pub(in crate::composition::mlx) fn completed_tensor_source(
    ...
    evidence
        ...
        .or_else(|| {
            evidence
                .get::<super::CompletedTensorSource>()
                .map(|source| &source.source)
        })
        .or_else(|| {
            evidence
                .get::<super::ExternalCompletedSource>()
                .map(|source| &source.source)
        })
}
...
pub(super) fn validate_external_tensor_sources(
    evidence: &[&PreparedEmbeddedEvidence],
    context: super::SpeculativeExecutionStreams<'_>,
) -> Result<(), Error> {
    ...
    let (sources, target) = context
        ...
        .ok_or_else(invalid)?;
    let (draft_sources, draft) = context
        .original_numerical_for(SamplingPlacement::Draft)
        .ok_or_else(invalid)?;
    if context.original_external().is_none()
        || !std::ptr::eq(sources, draft_sources)
        || !target.pool().same_ledger(draft.pool())
    {
        return Err(invalid());
    }
...
fn validate_tensor_inventory(
    ...
    sources: &OriginalSpeculativeNumericalSources,
    environment: &OriginalCopyEnvironment<'_>,
    draft: Option<&OriginalCopyEnvironment<'_>>,
) -> Result<(), Error> {
    let parts = [
        std::mem::size_of::<RegisteredTensorSources<'_>>(),
        std::mem::size_of::<std::slice::Iter<'_, &PreparedEmbeddedEvidence>>(),
        std::mem::size_of::<Result<(), Error>>(),
        std::mem::size_of::<bool>(),
        std::mem::size_of::<(
            &[&PreparedEmbeddedEvidence],
            super::SpeculativeExecutionStreams<'_>,
        )>(),
        std::mem::size_of::<(
            &[&PreparedEmbeddedEvidence],
    ...
    for value in evidence {
        ...
        if let Some(completed) = completed_tensor_source(value) {
            validate_inventory_source(
                InputSource::Completed(completed),
                sources,
                environment,
                draft,
            )?;
        ...
        for registered in registered_tensor_sources(value) {
            validate_inventory_source(
                InputSource::Registered(registered),
                sources,
                environment,
                draft,
            )?;
...
fn validate_inventory_source(
    source: InputSource<'_>,
    sources: &OriginalSpeculativeNumericalSources,
    target: &OriginalCopyEnvironment<'_>,
    draft: Option<&OriginalCopyEnvironment<'_>>,
) -> Result<(), Error> {
...
pub(in crate::composition::mlx) fn registered_tensor_source(
    evidence: &PreparedEmbeddedEvidence,
) -> Option<&RegisteredTensorSource> {
    evidence.get::<RegisteredTensorSource>()
}
...
impl<'a> Iterator for RegisteredTensorSources<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.direct.take().or_else(|| self.shared.next())
    }
}
pub(in crate::composition::mlx) fn registered_tensor_sources(
    evidence: &PreparedEmbeddedEvidence,
) -> RegisteredTensorSources<'_> {
    RegisteredTensorSources {
        direct: registered_tensor_source(evidence),
        shared: evidence
            .get::<super::ExternalCompletedSource>()
            .map_or(&[][..], |source| source.registered.as_slice())
            .iter(),
    }
}
pub(super) fn registered_source_for_array<'a>(
    evidence: &'a PreparedEmbeddedEvidence,
    array: &safemlx::Array,
    funding: &eredu_nn::workspace::HostMetadataFunding,
) -> Result<Option<&'a RegisteredTensorSource>, Error> {
    let parts = [
        std::mem::size_of::<RegisteredTensorSources<'_>>(),
    ...
    for source in registered_tensor_sources(evidence) {
        if source.matches_array(array, funding)? {
            found = Some(source);
        }
    }
    Ok(found)
...
fn validate_registered_inputs(
    ...
    let (sources, environment) = context
        .original_numerical_for(
            selected.unwrap_or(eredu_core::speculative::SamplingPlacement::Target),
        )
        .ok_or(Error::PrefillControl(WorkingMemoryError::IdentityMismatch))?;
    let parts = [
        std::mem::size_of::<RegisteredTensorSources<'_>>(),
        ...
        std::mem::size_of::<(
            super::SpeculativeExecutionStreams<'_>,
            &crate::backend::nn::workspace::ProjectedNativeStorage,
            Option<eredu_core::speculative::SamplingPlacement>,
            &eredu_nn::workspace::HostMetadataFunding,
        )>(),
        std::mem::size_of::<(
            super::SpeculativeExecutionStreams<'_>,
    ...
    fn validate(
        ...
        match self {
            Self::Completed(source) => {
                Ok(source.validate_request_source(sources.request(), stream, funding)?)
            }
            Self::Registered(source) => source.validate(sources.request(), stream, funding),
        }
    }
}
fn input_environment<'a>(
    source: InputSource<'_>,
    context: super::SpeculativeExecutionStreams<'a>,
    placement: eredu_core::speculative::SamplingPlacement,
    funding: &eredu_nn::workspace::HostMetadataFunding,
    ...
    let parts = [
        ...
        >(),
        std::mem::size_of::<SamplingPlacement>(),
        std::mem::size_of::<Result<bool, Error>>(),
        std::mem::size_of::<Result<(), Error>>(),
        std::mem::size_of::<Result<&OriginalCopyEnvironment<'_>, Error>>(),
    ...
    funding
        ...
        .map_err(Error::WorkspacePlanning)?;
    let invalid = || Error::PrefillControl(WorkingMemoryError::IdentityMismatch);
    let (sources, destination) = context
    ...
    let (other, origin) = context
        .original_numerical_for(opposite)
        .ok_or_else(invalid)?;
    if !std::ptr::eq(sources, other)
        || !destination.pool().same_ledger(origin.pool())
        || !source.matches(origin.stream(), funding)?
    {
        return Err(invalid());
    }
...
pub(super) fn completed_input_environment<'a>(
    source: &CompletedResidentSource,
    context: super::SpeculativeExecutionStreams<'a>,
    placement: eredu_core::speculative::SamplingPlacement,
    funding: &eredu_nn::workspace::HostMetadataFunding,
) -> Result<&'a OriginalCopyEnvironment<'a>, Error> {
...
pub(super) fn input_array_environment<'a>(
    evidence: &PreparedEmbeddedEvidence,
    array: &safemlx::Array,
    context: super::SpeculativeExecutionStreams<'a>,
    placement: eredu_core::speculative::SamplingPlacement,
    ...
    let source = if let Some(source) = registered_source_for_array(evidence, array, funding)? {
        ...
        InputSource::Completed(
            completed_tensor_source(evidence)
                .ok_or(Error::PrefillControl(WorkingMemoryError::IdentityMismatch))?,
        )
    };
    ...
    match source {
        ...
        InputSource::Completed(source) => {
            funding
                .reserve_metadata(CompletedResidentSource::array_source_control_bytes().ok_or(
                    Error::WorkspacePlanning(
                        eredu_nn::workspace::HostMetadataFundingError::Overflow,
                    ),
                )?)
                .map_err(Error::WorkspacePlanning)?;
...
pub(in crate::composition::mlx) fn validate_input_evidence(
    evidence: &PreparedEmbeddedEvidence,
    context: super::SpeculativeExecutionStreams<'_>,
    placement: eredu_core::speculative::SamplingPlacement,
    funding: &eredu_nn::workspace::HostMetadataFunding,
