...
use eredu_architectures::{
    external_assistant::ExternalOperationResult, speculative_execution::PreparedEmbeddedEvidence,
};
...
fn clone(value: &MlxTensor, funding: &HostMetadataFunding) -> Result<MlxTensor, Error> {
    ...
    let failure = |cause| {
        Error::StorageSource(eredu_core::BackendFailure::from_error(CloneFailure {
            cause,
            _funding: funding.clone(),
        }))
    };
    let mut slot = safemlx::PreparedArrayClone::try_prepare_for_inspection().map_err(&failure)?;
    slot.fill_for_inspection(value.as_array())
...
fn selected_numerical<'a>(
    context: SpeculativeExecutionStreams<'a>,
    placement: ExternalAssistantTensorPlacement,
) -> Option<(
    &'a super::super::OriginalSpeculativeNumericalSources,
    &'a crate::backend::OriginalCopyEnvironment<'a>,
)> {
...
pub(super) fn join(
    visit: impl FnMut(&mut dyn FnMut(&MlxTensor)),
...
pub(super) fn join_at(
    mut visit: impl FnMut(&mut dyn FnMut(&MlxTensor)),
    evidence: &[&PreparedEmbeddedEvidence],
    placement: ExternalAssistantTensorPlacement,
    context: SpeculativeExecutionStreams<'_>,
) -> Result<Option<PreparedEmbeddedEvidence>, Error> {
    ...
    let mut run = || {
        ...
        charge(
            ...
            &[
                ...
                size_of::<(
                    &[&PreparedEmbeddedEvidence],
                    ExternalAssistantTensorPlacement,
        ...
        charge(
            funding,
            &[
                size_of::<Option<&crate::backend::OriginalCopyEnvironment<'_>>>(),
        ...
        let mut completed = funding
            .metadata_vec(evidence.len())
            .map_err(Error::Neural)?;
        for proof in evidence {
            super::super::tensor_sources::validate_input_evidence(
                proof, context, selected, funding,
            )?;
            if let Some(source) = super::super::tensor_sources::completed_tensor_source(proof) {
                let origin = super::super::tensor_sources::completed_input_environment(
                    source, context, selected, funding,
                )?;
                if common_origin.is_some_and(|prior| prior.stream() != origin.stream()) {
                    return Err(invalid());
                }
        ...
        let origin_placement = if origin.stream() == context.target() {
            eredu_core::speculative::SamplingPlacement::Target
        } else if origin.stream() == context.draft() {
            eredu_core::speculative::SamplingPlacement::Draft
        } else {
            return Err(invalid());
        };
        ...
        let source = CompletedResidentSource::project_array_sources(
            |arrays| {
                visit(&mut |value| {
                    ...
                    let matched = (|| {
                        for proof in evidence {
                            if super::super::tensor_sources::registered_source_for_array(
                                proof,
                                value.as_array(),
                                funding,
                            )?
                    ...
                    match matched {
                        Ok(false) => arrays(value.as_array()),
                        Ok(true) => {}
        ...
        super::super::assistant::retain_external_evidence_for_placement(
            source,
            |arrays| visit(&mut |value| arrays(value.as_array())),
            evidence,
            context,
            origin_placement,
            funding,
...
pub(super) fn alias(
    ...
    let run = || {
        ...
        charge(
            funding,
            &[
                size_of::<ExternalOperationResult<MlxTensor>>(),
        ...
        super::super::tensor_sources::input_array_environment(
            proof,
            value.as_array(),
            context,
            match placement {
...
pub(super) fn transfer(
    ...
    match context.topology() {
        ...
        | eredu_core::SpeculativeExecutionTopology::SameDeviceSplit => {
            alias(value, evidence, destination, context)
        }
        eredu_core::SpeculativeExecutionTopology::CrossDeviceSplit => {
            copy_between(value, evidence, source, destination, context)
        }
        _ => Err(invalid()),
    }
}
pub(super) fn range(
    ...
    let Some((sources, environment)) = selected_numerical(context, placement) else {
        ...
        return super::super::sampling::numerical::native_axis_range(
            value.as_array(),
            axis,
            start,
            end,
        ...
        .map(MlxTensor::from_array)
        .map(ExternalOperationResult::ordinary);
    };
    let run = || {
        let funding = sources.metadata_funding();
        charge(
        ...
        let proof = evidence.ok_or_else(invalid)?;
        let leaf = match super::super::tensor_sources::registered_source_for_array(
            proof,
            value.as_array(),
            funding,
        )? {
            Some(source) => Some(retain_registered(source.clone(), funding)?),
            None => None,
        };
        ...
        let registered = super::super::tensor_sources::registered_source_for_array(
            current,
            packet.as_array(),
            funding,
        )?
        .is_some();
        let completed =
            super::super::tensor_sources::completed_tensor_source(current).map(|source| [source]);
        if !registered && completed.is_none() {
            return Err(invalid());
        }
        let source = CompletedResidentSource::project_array_sources(
            |visit| {
                if !registered {
                    visit(packet.as_array());
                }
            },
...
pub(super) fn buffer<T>(
    ...
    if context.original_numerical().is_none() {
        return Ok(eredu_core::SpeculativeBuffer::with_capacity(capacity));
    }
    let parts = [
    ...
    eredu_core::SpeculativeBuffer::try_new_retained(capacity, host).map_err(|cause| {
        Error::StorageSource(crate::composition::mlx::replicated_text::cache_error(
            cause, context,
        ))
    })
}
pub(super) fn dimension(
        ...
        if let Some((sources, _)) = context.original_numerical() {
            charge(
                ...
                &[
                    ...
                    size_of::<(&MlxTensor, usize, SpeculativeExecutionStreams<'_>)>(),
    ...
    run().map_err(|cause| match context.original_numerical() {
...
fn retain_registered(
    proof: super::super::RegisteredTensorSource,
    ...
    Ok(PreparedEmbeddedEvidence::from_prepared(
        proof,
        eredu_core::HostPreparationAuthority::retain(funding.clone()),
    ))
}
...
pub(super) fn copy_between(
    value: &MlxTensor,
    evidence: Option<&PreparedEmbeddedEvidence>,
    source_placement: ExternalAssistantTensorPlacement,
    placement: ExternalAssistantTensorPlacement,
    context: SpeculativeExecutionStreams<'_>,
) -> Result<ExternalOperationResult<MlxTensor>, Error> {
    let (sources, environment) = selected_numerical(context, placement).ok_or_else(invalid)?;
    let (source_request, source_environment) =
        selected_numerical(context, source_placement).ok_or_else(invalid)?;
    let run = || {
        let funding = sources.metadata_funding();
        charge(
            funding,
            &[
                ...
                size_of::<
                    Result<
                        crate::backend::array_copy::RegisteredArrayCopy,
        ...
        if !std::ptr::eq(environment, source_environment) {
            sources.validate_environment(source_environment)?;
        }
        if context.original_external().is_none()
            || !std::ptr::eq(sources, source_request)
            || selected_stream(context, placement) != environment.stream()
            || selected_stream(context, source_placement) != source_environment.stream()
            || !environment.pool().same_ledger(source_environment.pool())
        {
            return Err(invalid());
        }
        let evidence = evidence.ok_or_else(invalid)?;
        let completed = if let Some(source) =
            super::super::tensor_sources::registered_source_for_array(
                evidence,
                value.as_array(),
                funding,
            )? {
            source.validate(sources.request(), source_environment.stream(), funding)?;
            None
        } else {
            let source = super::super::tensor_sources::completed_tensor_source(evidence)
                .ok_or_else(invalid)?;
            source.validate_request_source(
                sources.request(),
                source_environment.stream(),
                funding,
            )?;
            charge(
                funding,
            ...
            source.array_source_account(value.as_array(), funding)?;
            Some(source)
        };
        let (roots, mechanisms) = sources.numerical_prerequisites();
        let copied = crate::backend::array_copy::IsolatedArrayCopy::new(value.as_array())
            .copy_completed(
                completed,
                environment,
                roots,
        ...
        let proof = super::super::RegisteredTensorSource::from_copy(
            &copied,
