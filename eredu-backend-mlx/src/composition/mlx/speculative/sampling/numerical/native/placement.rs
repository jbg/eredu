...
pub(super) fn matches_completed_input(
    ...
    let Some((context, placement)) = inputs else {
        return Ok(false);
    };
    ...
    let Some((other, origin)) = context.original_numerical_for(opposite) else {
        return Ok(false);
    };
    if !std::ptr::eq(sources, other) || !destination.pool().same_ledger(origin.pool()) {
        return Ok(false);
    }
...
pub(super) fn validate_completed_input(
    value: &OriginalNumericalValue,
