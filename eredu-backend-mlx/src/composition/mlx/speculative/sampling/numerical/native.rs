...
use eredu_runtime::generation::{
    LogitProgramError, SpeculativeCategoricalProgram, SpeculativeGreedyProgram,
    SpeculativeLogitProgram,
};
...
mod placement;
mod tensor_layout;
type InputPlacement<'a> = (
    crate::composition::mlx::speculative::SpeculativeExecutionStreams<'a>,
...
enum Cause {
    ...
    Capture(
        #[from]
        eredu_runtime::capture::FundedCaptureError<
...
pub(crate) enum NumericalOutput {
    ...
    KeyPair {
        current: OriginalNumericalKey,
        next: OriginalNumericalKey,
    },
    RandomToken {
        token: u32,
        advanced: OriginalNumericalKey,
    },
    RandomUnitInterval {
        value: f32,
        advanced: OriginalNumericalKey,
    ...
    CapturedLogits {
        ...
        capture: eredu_core::capture::SharedCapturedStep,
    },
...
impl NumericalProducer {
    ...
    pub(super) fn execute_inputs_at(
        context: crate::composition::mlx::speculative::SpeculativeExecutionStreams<'_>,
        placement: eredu_core::speculative::SamplingPlacement,
        kind: Kind,
        left: Option<&OriginalNumericalValue>,
    ...
    pub(super) fn execute_policy_at(
        context: crate::composition::mlx::speculative::SpeculativeExecutionStreams<'_>,
        placement: eredu_core::speculative::SamplingPlacement,
        policy: SpeculativeLogitProgram,
        left: &OriginalNumericalValue,
        ...
        capture: Option<capture::CaptureSource<'_>>,
    ...
    pub(super) fn execute_controlled_policy(
        ...
        left: &OriginalNumericalValue,
        history: &[u32],
        mask: eredu_runtime::generation::TokenMaskPlan<'_>,
    ) -> Result<NumericalOutput, Error> {
        Self::execute_inputs_with_mask(
            sources,
            environment,
    ...
    pub(super) fn execute_captured_policy(
        ...
        history: &[u32],
        mask: Option<eredu_runtime::generation::TokenMaskPlan<'_>>,
        capture: capture::CaptureSource<'_>,
        failed_capture: &mut Option<eredu_core::capture::SharedCapturedStep>,
    ) -> Result<NumericalOutput, Error> {
        Self::execute_inputs_with_mask(
            sources,
    ...
    pub(super) fn execute_token_ids(
        ...
        let length = u32::try_from(tokens.len()).map_err(|_| {
    ...
    pub(super) fn execute_token_concatenate(
        sources: &OriginalSpeculativeNumericalSources,
        environment: &OriginalCopyEnvironment<'_>,
        roots: &PrefillRootsRuntime,
        mechanisms: MlxMetalWorkspaceMechanisms,
        sequence: &super::token_projection::TokenSequence,
    ) -> Result<NumericalOutput, Error> {
        let parts = u32::try_from(sequence.values().len()).map_err(|_| model::overflow())?;
        let positions = sequence
            .values()
            .iter()
            .try_fold(0u32, |n, value| {
                let [1, width] = value.value().array.shape() else {
                    return None;
                };
        ...
        Self::execute_inputs_with_mask(
            ...
            roots,
            mechanisms,
            Kind::TokenConcatenate { parts, positions },
            None,
            None,
            None,
            None,
    ...
    fn execute_inputs_with_mask(
        ...
        tokens: Option<&[u32]>,
        inputs: Option<InputPlacement<'_>>,
        sequence: Option<&super::token_projection::TokenSequence>,
    ) -> Result<NumericalOutput, Error> {
        ...
        let result = (|| -> Result<NumericalOutput, Error> {
            sources.validate_environment(environment)?;
            let fixed = planning_control_bytes()
            ...
            if let Some((context, placement)) = inputs {
                let (selected, destination) = context
                    .original_numerical_for(placement)
            ...
            let token_shape =
                if let Kind::TokenIds { length } | Kind::RepeatedToken { length, .. } = kind {
                    if match kind {
                    ...
                    if tokens.is_some()
                        || sequence.is_none_or(|source| source.values().len() != parts as usize)
                    {
                        return Err(cold(Cause::Source));
                    }
                    Some([
                        1,
                        i32::try_from(positions).map_err(|_| cold(Cause::Source))?,
                    ])
                } else {
                    if tokens.is_some() || sequence.is_some() {
                        return Err(cold(Cause::Source));
                    }
                    None
                };
            ...
            if let Some(capture) = capture_source {
                capture.validate(sources)?;
            }
            let capture_host = capture_source
                .map(|capture| capture.host(program))
                .transpose()
                .map_err(|cause| sources.retain_startup_error(cause))?;

            if mask.is_some_and(|mask| {
                !matches!(kind, Kind::ProcessLogits(_)) || !mask.matches_shape(source_shape)
            }) {
                return Err(cold(Cause::Source));
            }

            if let Kind::ProcessLogits(policy) = kind {
                if history.map(<[u32]>::len) != Some(policy.history_len()) {
                    return Err(cold(Cause::Source));
                }
            }
            let expected = match kind {
                ...
                Kind::TensorRange { .. }
                | Kind::TensorAxisRange { .. }
                | Kind::TensorConcatenate { .. } => Meaning::Capture,
                Kind::NextKey | Kind::UniformUnitInterval | Kind::KeyAt { .. } => {
                    Meaning::RandomKey
                }
                _ => Meaning::Logits,
            };
            ...
            if (stream.device_type() != safemlx::DeviceType::Gpu
                && !(cpu
                    ...
                    && (capture_source.is_none() || matches!(kind, Kind::ProcessLogits(_)))))
                || usize::from(left.is_some())
                    + usize::from(right.is_some())
                    + sequence.map_or(0, |s| s.values().len())
                    != program.source_count()
            {
                return Err(cold(Cause::Source));
            }
            for (value, meaning) in left
                .map(|v| (v, expected))
                .into_iter()
                .chain(right.map(|v| {
                    (
                        v,
                        if matches!(kind, Kind::Categorical(_)) {
                            Meaning::RandomKey
                        } else if matches!(kind, Kind::TensorConcatenate { .. }) {
                            Meaning::Capture
                        } else {
                            Meaning::Logits
                        },
                ...
                .chain(
                    sequence
                        .into_iter()
                        .flat_map(|s| s.values().iter().map(|value| (value, Meaning::TokenIds))),
                )
            {
                if !value
                    .value()
                    ...
                    || (!stream.matches_source(&value.value().stream)
                        && !placement::matches_completed_input(
                            value,
                            inputs,
                            sources,
                            environment,
            ...
            if let Some(right) = right.filter(|_| !matches!(kind, Kind::Categorical(_))) {
                program
                    .validate_source(right.value().array.shape())
                    .map_err(|e| cold(Cause::Program(e)))?;
                if matches!(kind, Kind::TensorConcatenate { .. })
                    && left.is_none_or(|left| {
                        left.value().array.dtype() != right.value().array.dtype()
                    })
                {
                    return Err(cold(Cause::Source));
                }
            ...
            let metadata = |value: &OriginalNumericalValue| -> Result<WorkspaceTensor, Error> {
                if cpu_equations.is_some() {
                    return tensor_layout::completed(value, &context);
                }

                let dtype = match (value.value().meaning, value.value().array.dtype()) {
                    (Meaning::RandomKey | Meaning::TokenIds, safemlx::Dtype::Uint32) => {
                ...
                Ok(WorkspaceTensor::existing(
                    context
                        .layout(value.value().array.shape(), dtype)?
                        .with_representation(
                            if matches!(
                                kind,
                                Kind::TensorRange { .. }
                                ...
                                match value.value().array.dtype() {
                                    ...
                                    safemlx::Dtype::Float16 => {
                                        Some(eredu_nn::workspace::WorkspaceRepresentation::new(
                                            eredu_nn::workspace::WorkspaceFloatingType::Float16,
                                            false,
                                        ))
                                    }
                                    safemlx::Dtype::Bfloat16 => {
                                        Some(eredu_nn::workspace::WorkspaceRepresentation::new(
                                            eredu_nn::workspace::WorkspaceFloatingType::Bfloat16,
                                            false,
                                        ))
                                    }
                                    _ => None,
            ...
            let mut segment_metadata =
                context.metadata_vec(sequence.map_or(0, |s| s.values().len()))?;
            let mut segment_arrays =
                context.metadata_vec(sequence.map_or(0, |s| s.values().len()))?;
            let mut segment_sources =
                context.metadata_vec(sequence.map_or(0, |s| s.values().len()))?;
            if let Some(sequence) = sequence {
                let mut positions = 0u32;
                for value in sequence.values() {
                    program
                        .validate_source(value.value().array.shape())
                        .map_err(|cause| cold(Cause::Program(cause)))?;
                    positions = positions
                        .checked_add(value.value().array.shape()[1] as u32)
                ...
                if positions != program.shape()[1] as u32 {
                    return Err(cold(Cause::Source));
                }
            }
            ...
            context.begin_span();
            let capture_plan = capture_source
                .zip(capture_host.as_ref())
                .map(|(source, host)| capture::Plan::trace(source, host, left_meta()?, &context))
                .transpose()?;
            let (first_cut, first_report, second_report) = match kind {
                Kind::TokenIds { length } => {
                    ...
                    let input = WorkspaceTensor::existing(
                        context.layout(program.shape(), WorkspaceDtype::Uint32)?,
                        &context,
                    )?;
                    (
                        Cut::TokenIds(length),
                        context.finish_report(&[input])?,
                        None,
                    )
                }
                Kind::RepeatedToken { token, length } => {
                    let output = <WorkspaceTensor as eredu_nn::Tensor>::full_u32(
                        token,
                        program.shape(),
                        &context,
                    )?;
                ...
                Kind::TensorRange { start, end } | Kind::TokenRange { start, end } => {
                    ...
                    (
                        Cut::TensorRange {
                            ...
                            tokens: matches!(kind, Kind::TokenRange { .. }),
                        },
                        context.finish_report(&[output])?,
                        None,
                    )
                ...
                Kind::TensorAxisRange { axis, start, end } => {
                    let output = super::tensor::metadata_axis_range(
                        left_meta()?,
                        axis,
                        start,
                        end,
                ...
                Kind::TokenConcatenate { parts, .. } => {
                    let output = <WorkspaceTensor as eredu_nn::Tensor>::concatenate(
                        &segment_metadata,
                        1,
                        &context,
                    )?;
                ...
                Kind::TensorConcatenate { .. } => {
                    let output = super::tensor::metadata_concatenate(
                        left_meta()?,
                ...
                Kind::NextKey => {
                    ...
                    (
                        Cut::NextKey,
                        context.finish_report(&[random.into_key(), next])?,
                        None,
                    )
                }
                Kind::UniformUnitInterval => {
                    ...
                    (
                        ...
                        context.finish_report(&[draw, random.into_key()])?,
                        None,
                    )
                }
                Kind::KeyAt { position } => {
                    let key =
                        WorkspaceSamplingRandomState::key_at(left_meta()?, position, &context)?;
                ...
                Kind::Categorical(choice) => {
                    ...
                    let token = choice.construct::<WorkspaceSamplingBackend>(
                        &input,
                        &mut random,
                        &context,
                    )?;
                    (
                        ...
                        context.finish_report(&[token, random.into_key()])?,
                        None,
                    )
                }
                Kind::Greedy(choice) => {
                ...
                Kind::ProcessLogits(policy) => {
                    ...
                    let input = capture_plan
                        .as_ref()
                        .and_then(|plan| plan.effective.as_ref())
                        .unwrap_or(left_meta()?)
                        .clone();
                    let masked = mask
                        .map(|mask| {
                            WorkspaceSamplingBackend::apply_prepared_token_mask(
                                &input, mask, &context,
                            )
                        })
                        .transpose()?;
                    let output = policy
                        .process::<WorkspaceSamplingBackend>(
                            masked.as_ref().unwrap_or(&input),
                            history.ok_or_else(|| cold(Cause::Source))?,
                            &context,
                        )
                ...
                Kind::Normalize => {
                    ...
                    let output = program::normalize::<operators::Metadata>(left_meta()?, &context)?;
                ...
                Kind::Correction => {
                    if cpu
                        && (left_value()?.value().array.dtype() != safemlx::Dtype::Float32
                            || right
                                .ok_or_else(|| cold(Cause::Source))?
                                .value()
                                .array
            ...
            let source_values = [
                left.map(|v| v.value().provenance.source()),
                right.map(|v| v.value().provenance.source()),
            ];
            let one = source_values[0].map(|v| [v]);
            let two = source_values[0].zip(source_values[1]).map(|(a, b)| [a, b]);
            let admitted_sources = if sequence.is_some() {
                segment_sources.as_slice()
            } else {
                match program.source_count() {
                    0 => &[][..],
            ...
            let phase = sources
                .request()
                .reserve_numerical(requirements, admitted_sources)
                .map_err(|e| retain_planning_error(e, funding.clone()))?;
            let (custody, capture_invocation) = if let Some(host) = capture_host {
                let (custody, invocation) = phase
            ...
            let mut capture_invocation =
                capture::Invocation::new(capture_invocation, failed_capture);
            ...
            let output_stream = ValueStream::Numerical(
                stream_plan
            ...
            Ok(match kind {
                Kind::TokenIds { .. } | Kind::RepeatedToken { .. } => NumericalOutput::Tensor(
                    OriginalNumericalValue::completed(
                        output.value,
                        output_stream,
                        Meaning::TokenIds,
                        custody,
                ...
                | Kind::TokenRange { .. } => {
                    ...
                    Rc::get_mut(value.0.as_mut().expect("unpublished tensor view"))
                        .expect("unique completed tensor view")
                        ._readout_source = left.cloned().map(RetainedValues::One);
                ...
                Kind::TensorConcatenate { .. } => {
                    ...
                    Rc::get_mut(value.0.as_mut().expect("unpublished concatenated tensor"))
                        .expect("unique completed concatenation")
                        ._readout_source = Some(RetainedValues::Two([
                        left.expect("validated left source").clone(),
                        right.expect("validated right source").clone(),
                    ]));
                    NumericalOutput::Tensor(value)
                }
                Kind::CreateKey { .. } | Kind::KeyAt { .. } => {
                    NumericalOutput::Key(OriginalNumericalKey::completed(
                        output.value,
                        output_stream,
                        custody,
                        funding.clone(),
                ...
                Kind::NextKey => NumericalOutput::KeyPair {
                    current: OriginalNumericalKey::completed(
                        output.value,
                        output_stream.clone(),
                        custody.clone(),
                        funding.clone(),
                    ...
                    next: OriginalNumericalKey::completed(
                        output
                            .other
                ...
                Kind::Categorical(_) => NumericalOutput::RandomToken {
                    token: output
                        .token
                        .filter(|&token| {
                            token < *program.shape().last().expect("vocabulary") as u32
                        })
                        .ok_or_else(|| failure(Cause::Completion, &custody, funding))?,
                    advanced: OriginalNumericalKey::completed(
                        output
                ...
                Kind::Greedy(_) => NumericalOutput::Token(
                    output
                        .token
                        .filter(|&token| {
                            token < *program.shape().last().expect("source vocabulary") as u32
                        })
                        .ok_or_else(|| failure(Cause::Completion, &custody, funding))?,
                ),
                Kind::ProcessLogits(_) => {
                    let capture = capture_invocation
                        .as_mut()
                        .map(|invocation| {
                            invocation
                                .take_shared_step()
                                .map_err(|cause| failure(cause.into(), &custody, funding))?
                                .ok_or_else(|| failure(Cause::Completion, &custody, funding))
                        })
                        .transpose()?;
                    ...
                    let value = OriginalNumericalValue::completed(
                        output.value,
                        output_stream.clone(),
                        Meaning::Logits,
                        custody,
                    ...
                    .with_original_budget(output.budget);
                    match capture {
                        Some(capture) => NumericalOutput::CapturedLogits { value, capture },
                        None => NumericalOutput::Logits(value),
                    }
                }
                Kind::LogitsRow { .. } => {
                    let mut value = OriginalNumericalValue::completed(
                    ...
                    Rc::get_mut(value.0.as_mut().expect("unpublished readout"))
                        .expect("unique completed readout")
                        ._readout_source = left.cloned().map(RetainedValues::One);
                    NumericalOutput::Logits(value)
                }
                Kind::Normalize => NumericalOutput::Distribution(
                    OriginalNumericalValue::completed(
                        output.value,
                        output_stream.clone(),
                ...
                Kind::Correction => {
                    ...
                    if !program::correction_has_mass(mass) {
                        ...
                        let plan = second.as_ref().ok_or_else(|| {
                            failure(Cause::Unknown("correction second cut"), &custody, funding)
                        })?;
                        let output = run_cut(
                            plan,
                            environment,
                        ...
                        NumericalOutput::Correction(Some(
                            OriginalNumericalValue::completed(
                                output.value,
                                output_stream.clone(),
                                Meaning::Logits,
                            ...
                            .with_controller_choice(
                                left.and_then(|v| v.controller_choice()).cloned(),
                            ),
                        ))
                    }
                }
...
impl Plan {
    fn inspect(
        ...
        let ordinary_roots: usize = if matches!(
            cut,
            Cut::Difference | Cut::NextKey | Cut::UniformUnitInterval | Cut::Categorical(_)
        ) {
            2
        } else {
            1
        ...
        let roots = ordinary_roots
            .checked_add(capture.map_or(0, |capture| capture.roots))
        ...
        let eager = if let Cut::TokenIds(length) = cut {
            ...
            Some(safemlx::OriginalPromptInputFacts::inspect(
                &runtime,
                length as usize,
            )?)
        } else {
        ...
        let recipe = match (cpu, eager) {
            (true, Some(source)) => SpeculativeNumericalRecipe::inspect_cpu_token_ids(
                report, source, mechanism, context,
            ),
            (true, None)
                if matches!(
                    cut,
                    Cut::TensorConcatenate | Cut::TokenConcatenate(_) | Cut::RepeatedToken { .. }
                ) && capture.is_none() =>
            {
                SpeculativeNumericalRecipe::inspect_cpu_equations(
                    report,
                    mechanism,
                    cpu_equations.ok_or(Cause::Source)?,
                    context,
                )
            }
            ...
            (true, None)
                if matches!(cut, Cut::Probability(_) | Cut::LogitsRow(_)) && capture.is_none() =>
            {
            ...
            (true, None)
                if matches!(cut, Cut::Policy(_))
                    && capture.is_some_and(|capture| capture.cpu_readouts) =>
            {
                let capture = capture.ok_or(Cause::Source)?;
                SpeculativeNumericalRecipe::inspect_cpu_capture(
                    report,
                    capture.roots,
                    capture.completions,
                    mechanism,
            ...
            (false, Some(source)) => {
                SpeculativeNumericalRecipe::inspect_token_ids(report, source, mechanism, context)
            }
            (false, None) => SpeculativeNumericalRecipe::inspect_with_nested(
                report,
                roots,
                capture.map_or(0, |capture| capture.completions),
                mechanism,
                context,
            ),
        }
        ...
        let physical = population
            .capacity()
            .checked_add(eager.map_or(0, |source| source.mutable_bytes()))
            .ok_or(Cause::Overflow)?;
        ...
        let policy_controls = if matches!(cut, Cut::Policy(_)) {
            crate::backend::runtime::generation::processing_control_bytes()
                .ok_or(Cause::Unknown("logit policy controls"))?
        } else {
            0
        };
        ...
        let shared_stream = Layout::new::<[std::sync::atomic::AtomicUsize; 2]>()
            .extend(output_stream.shared_body_layout())
        ...
        let stream_parts = [
            output_stream
                .control_bytes()
                .ok_or(Cause::Unknown("output stream controls"))?,
            shared_stream,
            output_stream.owner_node_layout().size(),
            output_stream.native_wrapper_bytes(),
        ];
        let seed_stream_controls = stream_parts
            .into_iter()
            .try_fold(size_of_val(&stream_parts), usize::checked_add)
            .ok_or(Cause::Overflow)?;
        let key_at_controls = if matches!(cut, Cut::KeyAt(_)) {
            crate::backend::random::split_key_at_control_bytes()
                .ok_or(Cause::Unknown("key selection controls"))?
        } else {
            0
        };
        let uniform_controls = if matches!(cut, Cut::UniformUnitInterval) {
            crate::backend::random::uniform_unit_interval_control_bytes()
                .ok_or(Cause::Unknown("uniform controls"))?
        } else {
            0
        };
        ...
        let concatenate_controls =
            if matches!(cut, Cut::TensorConcatenate | Cut::TokenConcatenate(_)) {
                safemlx::ops::concatenate_axis_control_bytes()
                    .ok_or(Cause::Unknown("concatenation controls"))?
        ...
        let parts = [
            ...
            eager
                .map(|facts| {
                    facts
                        .control_bytes()
                        .ok_or(Cause::Unknown("eager input controls"))
                })
                .transpose()?
                .unwrap_or(0),
            ...
            size_of::<
                Result<safemlx::PreparedStreamCopy<Custody>, safemlx::StreamCopyError<Custody>>,
            >(),
            policy_controls,
            ...
            size_of::<
                Result<
                    ...
                    eredu_runtime::capture::FundedCaptureDrainError,
                >,
            >(),
            size_of::<MlxTensor>(),
            ...
            safemlx::original_scoped_evaluation_control_bytes()
                .ok_or(Cause::Unknown("scoped evaluation controls"))?,
            OriginalCopyEnvironment::control_bytes()
                .ok_or(Cause::Unknown("copy environment controls"))?,
            recipe
                .completion
                .graph
                .control_bytes()
...
fn planning_control_bytes() -> Option<usize> {
    let parts = [
        ...
        size_of::<Vec<&Array>>(),
        size_of::<Vec<SpeculativeNumericalSource<'_>>>(),
        size_of::<&[&Array]>(),
        ...
        size_of::<(
            ...
            &[u32],
            Option<eredu_runtime::generation::TokenMaskPlan<'_>>,
            Option<capture::CaptureSource<'_>>,
        ...
        size_of::<Option<eredu_runtime::generation::TokenMaskPlan<'_>>>(),
        size_of::<Planning>(),
        size_of::<capture::Plan>(),
        size_of::<Option<capture::Plan>>(),
        ...
        size_of::<
            std::iter::Chain<
                std::option::IntoIter<(&OriginalNumericalValue, Meaning)>,
                std::option::IntoIter<(&OriginalNumericalValue, Meaning)>,
            >,
        >(),
...
fn run_cut(
    ...
    let capture_roots = capture_plan
        .map(|capture| capture::Roots::prepare(capture.roots))
        .transpose()
    ...
    let result = (|| -> Result<CutOutput, Cause> {
        ...
        if plan.recipe.completion.nested_completions != 0 {
            let nested = plan
                ...
                .completion
                .nested_traversal()
                .ok_or(Cause::Unknown("nested traversal"))?;
            bank.as_mut()
                .ok_or(Cause::Source)?
        ...
        let effective = match (capture_plan, capture_invocation, capture_roots.as_ref()) {
            (Some(capture), Some(invocation), Some(roots)) => capture::observe(
                capture,
                &plan.recipe,
                invocation,
        ...
        let (value, mass) = match plan.cut {
            Cut::TokenIds(length) => {
                let tokens = tokens
                    .filter(|tokens| tokens.len() == length as usize)
                    .ok_or(Cause::Source)?;
                (Array::try_from_original_prompt_ids(tokens)?, None)
            }
            Cut::RepeatedToken { token, length } => (
                <MlxTensor as eredu_nn::Tensor>::full_u32(token, &[1, length as i32], stream)
                    .map_err(|cause| Cause::Backend(Error::Neural(cause)))?
                    .into_array(),
                None,
            ),
            Cut::TensorRange {
                ...
                super::tensor::native_axis_range(
                    &left_value()?.value().array,
                    axis,
                    start,
                    end,
            ...
            Cut::TokenConcatenate(parts) => {
                if segment_arrays.len() != parts as usize {
                    return Err(Cause::Source);
                }
                (
                    safemlx::ops::concatenate_axis(segment_arrays, 1, stream)?,
                    None,
                )
            }
            Cut::TensorConcatenate => (
            ...
            Cut::NextKey => {
                let mut random = crate::backend::random::RandomState::from_key(
                    left_value()?.value().array.clone(),
                );
            ...
            Cut::Categorical(choice) => {
                ...
                let mut random = crate::backend::random::RandomState::from_key(
                    right.ok_or(Cause::Source)?.value().array.clone(),
                );
            ...
            Cut::Greedy(choice) => {
                ...
                (
                    choice
                        .construct::<MlxSamplingBackend>(&input, stream)?
                        .into_array(),
                    None,
                )
            }
            Cut::Policy(policy) => {
                ...
                let masked = if let Some(mask) = mask {
                    if !mask.matches_shape(input.as_array().shape()) {
                        return Err(Cause::Source);
                    }
                    if mask.is_identity() {
                        Some(input.clone())
                        ...
                        invalid
                            .try_reserve_exact(mask.elements())
                            .map_err(Cause::MaskAllocation)?;
                        if invalid.capacity() != mask.elements() {
                            return Err(
                                eredu_runtime::generation::TokenMaskError::Destination.into()
                            );
                        }
                        mask.fill(&mut invalid)?;
                        Some(crate::backend::runtime::generation::apply_token_mask(
            ...
            Cut::Logarithm => (
                program::correction_logits::<operators::Native>(
                    &left_value()?.value().array,
                    stream,
                )?,
                None,
        ...
        drop(bank);
        {
            ...
            if let Some(capture) = &capture_roots {
                capture.append_to(&mut roots)?;
            }
        ...
        let scalar = if matches!(
            plan.cut,
            Cut::Probability(_) | Cut::Difference | Cut::UniformUnitInterval
        ) {
    ...
    let status = recovery
        .finish()
        .map_err(|cause| err(Cause::Backend(cause.into_error())))?;
