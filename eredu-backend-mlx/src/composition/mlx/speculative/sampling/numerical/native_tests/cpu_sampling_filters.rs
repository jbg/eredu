...
fn original_cpu_filters_preserve_shared_policy_rounding_and_output_custody(){
    ...
    let logits:Vec<f32>=(0..65).map(|i|((i*17)%41) as f32*0.125-2.5).collect();
    let (escaped,expected)={
        ...
        let parts=[HostInputPart{modality:eredu_core::InputModality::Text,
            ...
            payload:HostTensorView{shape:&[1,1,65],values:HostTensorValues::F32(&logits)},metadata:&[],extents:&[]}];
        ...
        for (top_k,top_p,min_p,temperature) in [(1,1.0,0.0,0.0),(7,1.0,0.0,0.0),
            (0,0.83,0.0,0.0),(0,1.0,0.15,0.0),(40,0.95,0.05,0.7)] {
            let sampler=GenerationSampler::new().top_k(top_k).top_p(top_p).min_p(min_p);
            ...
            let ordinary=policy.process::<MlxSamplingBackend>(&MlxTensor::from_array(source.value().array.clone()),
                &[],environment.stream()).unwrap();
            let expected=ordinary.as_array().evaluated().unwrap().try_to_vec::<f32>().unwrap();drop(ordinary);
            assert!(expected.iter().any(|x|x.is_finite()));assert!(expected.iter().any(|x|x.is_infinite()));
            if top_k>0&&top_p==1.0&&min_p==0.0 {
                let mut sorted=logits.clone();sorted.sort_by(|a,b|b.total_cmp(a));let threshold=sorted[top_k as usize-1];
                for (actual,original) in expected.iter().zip(&logits){assert_eq!(*actual,if *original<threshold{f32::NEG_INFINITY}else{*original});}
            }
            let mask=eredu_runtime::generation::TokenMaskPlan::new(&eredu_core::TokenFilter::All,
                source.value().array.shape(),None).unwrap();
            let output=NumericalProducer::execute_controlled_policy(sources,&environment,roots,mechanisms,
                policy,&source,&[],mask).unwrap();
            let NumericalOutput::Logits(output)=output else{panic!("filtered output lost logit meaning")};
        ...
        assert_eq!(pool.unquoted_owner_count().unwrap(),0);final_output.unwrap()
    };
