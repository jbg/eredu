...
fn exercise(policy_consumer:bool) {
    ...
    let logits=[-2.5f32,3.75,3.75,-0.125,1.5];
    {
        ...
        let ordinary=choice.construct::<MlxSamplingBackend>(&tensor,environment.stream()).unwrap();
        let expected=ordinary.as_array().evaluated().unwrap().as_slice::<u32>()[0];
        assert_eq!(expected,1);drop(ordinary);drop(tensor);
        if policy_consumer {
            let context=SpeculativeExecutionStreams::single(environment.stream())
                .with_original_numerical_sources(sources,&environment).unwrap();
            let processed=process_policy_at(&DefaultSampler,&source,0.0,&[],SamplingPlacement::Draft,context).unwrap();
            assert!(Rc::ptr_eq(processed.0.as_ref().unwrap(),source.0.as_ref().unwrap()),
                "the shared empty trace returns the same completed source owner");
            let actual=sample_greedy_at(&DefaultSampler,&processed,0.0,SamplingPlacement::Draft,context).unwrap();
            assert_eq!(actual,expected);
            commit_without_mutation(&DefaultSampler,&processed,actual,SamplingPlacement::Draft,context).unwrap();
            assert!(validate_commit_value(&processed,5,SamplingPlacement::Draft,context).is_err(),
            ...
            let ordinary=policy.process::<MlxSamplingBackend>(
                &MlxTensor::from_array(source.value().array.clone()),&[],environment.stream()).unwrap();
            let scaled_expected=ordinary.as_array().evaluated().unwrap().try_to_vec::<f32>().unwrap();
            drop(ordinary);
            for (&actual,&input) in scaled_expected.iter().zip(&logits) {
                assert_eq!(actual,input*(1.0/temperature));
            }
            let scaled=process_policy_at(&DefaultSampler,&source,temperature,&[],SamplingPlacement::Draft,context).unwrap();
            assert!(!Rc::ptr_eq(scaled.0.as_ref().unwrap(),source.0.as_ref().unwrap()));
            ...
            let actual=sample_greedy_at(&DefaultSampler,&scaled,0.0,SamplingPlacement::Draft,context).unwrap();
            assert_eq!(actual,expected);
            commit_without_mutation(&DefaultSampler,&scaled,actual,SamplingPlacement::Draft,context).unwrap();
        } else {
            let output=NumericalProducer::execute(sources,&environment,roots,mechanisms,
                program::SpeculativeNumericalKind::Greedy(choice),&source,None).unwrap();
            let NumericalOutput::Token(actual)=output else {panic!("greedy lost scalar token meaning")};
            assert_eq!(actual,expected);
        }
        assert!(source.value().provenance.source().belongs_to_request(pair.request()));
        assert_eq!(source.value().array.evaluated().unwrap().as_slice::<f32>(),logits);
        assert_eq!(pool.unquoted_owner_count().unwrap(),0);
    }
    drop((target,draft));drop(schedule);drop((target_config,draft_config,selected));settle(&pool,initial);
}
