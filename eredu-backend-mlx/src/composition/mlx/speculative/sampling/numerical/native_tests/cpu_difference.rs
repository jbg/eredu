...
fn original_cpu_correction_matches_mass_branches_and_retires_escaped_output() {
    ...
    let (escaped, expected) = {
        ...
        let left = source(&logits);
        let right = source(&other);
        let (ordinary, mass) = program::correction::<operators::Native>(
            &left.value().array,
            &right.value().array,
            environment.stream(),
        )
        .unwrap();
        let mass = mass.evaluated().unwrap().try_item::<f32>().unwrap();
        assert!(mass > f32::EPSILON);
        let ordinary =
            program::correction_logits::<operators::Native>(&ordinary, environment.stream())
                .unwrap();
        let expected = ordinary.evaluated().unwrap().try_to_vec::<f32>().unwrap();
        drop(ordinary);
        assert!(expected.iter().any(|v| v.is_finite()));
        assert!(expected.contains(&f32::NEG_INFINITY));
        let output = NumericalProducer::execute(
            sources,
            &environment,
            roots,
            mechanisms,
            program::SpeculativeNumericalKind::Correction,
            &left,
            Some(&right),
        )
        .unwrap();
        let NumericalOutput::Correction(Some(output)) = output else {
            panic!("positive mass lost corrected logits")
        };
        ...
        let zero = NumericalProducer::execute(
            sources,
            &environment,
            roots,
            mechanisms,
        ...
        assert!(NumericalProducer::execute(
            sources,
            &environment,
            roots,
            mechanisms,
