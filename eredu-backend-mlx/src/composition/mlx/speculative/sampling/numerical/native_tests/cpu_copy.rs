...
fn original_cpu_copy_preserves_prepared_source_and_escaped_custody() {
    ...
    let cpu_streams = PreparedExecutionStreams::for_cpu_factory(&pool)
        .unwrap()
        .unwrap();
    ...
    let escaped = {
        ...
        let copied = materializer.with_test_original_copy_source(&input, &pool, |original| {
            let source = original
                ...
                .unwrap()
                .identity();
            let copy = IsolatedArrayCopy::new(original.array())
                .copy_prepared(
                    &original,
                    &environment,
                    roots,
                ...
                .unwrap();
            copy.validate_completed_stream(environment.stream(), sources.metadata_funding())
                .unwrap();
            assert_ne!(
                copy.array()
                    .try_allocation_info()
    ...
    drop((target, draft));
    assert_eq!(
        escaped.value().array.evaluated().unwrap().as_slice::<i32>(),
        values
    );
