...
fn declaration_with_attempts(
    transform: Option<CaptureTransform>,
    vocabulary: usize,
    attempts: u64,
) -> AdmittedCapturePlan {
    assert!(attempts > 0);
    ...
    let point = ObservationPoint {
        path: path.into(),
        node_id: "actual.cpu.logits".into(),
        meaning: "raw model output".into(),
        ...
        axes: Some(
            [
                ...
                ("vocabulary", SymbolicDimension::Known(vocabulary)),
            ]
            .into_iter()
    ...
    let selections = if let Some(transform) = transform {
...
fn values(record: &CaptureRecord) -> &[f32] {
    let observation = match record.payload.as_ref().unwrap() {
        CapturePayload::Tensor(value) => value,
        CapturePayload::SharedTensor(value) => value.as_observation(),
        _ => panic!("raw capture changed payload"),
    };
    let TensorObservationData::F32(values) = observation.data() else {
        panic!("raw capture changed dtype")
    };
    values
}
#[test]
    ...
    let (escaped, evidence) = {
        ...
        let capture = pool
            .compile_capture_source(
                PreparedCapturePlanCopy::inspect(&declaration(None, 8)).unwrap(),
            )
            .unwrap();
        let policy =
        ...
        let mask = eredu_runtime::generation::TokenMaskPlan::new(
            &filter,
            input.value().array.shape(),
            None,
        )
        .unwrap();
        let mut expanded = Vec::with_capacity(mask.elements());
        mask.fill(&mut expanded).unwrap();
        let ordinary = crate::backend::runtime::generation::apply_token_mask(
            &MlxTensor::from_array(input.value().array.clone()),
            &expanded,
            environment.stream(),
        )
        .unwrap();
        ...
        drop(ordinary);
        let source =
            super::super::capture::CaptureSource::original(&capture, sources, &pool).unwrap();
        let mut failed = None;
        let result = NumericalProducer::execute_captured_policy(
            sources,
            &environment,
            roots,
            mechanisms,
        ...
        let NumericalOutput::CapturedLogits {
            value,
            capture: evidence,
        } = result
        else {
            panic!("capture lost logits")
        };
        assert_eq!(
            value.value().array.evaluated().unwrap().as_slice::<f32>(),
            expected
        );
        ...
        assert_eq!(
            evidence.records()[1].selected_shape.as_deref(),
            Some([1, 1, 4].as_slice())
        );
        ...
        let unsupported = pool
            .compile_capture_source(
                PreparedCapturePlanCopy::inspect(&declaration(
                    Some(CaptureTransform::TopCandidates { count: 2 }),
                    9,
                ))
                .unwrap(),
            )
            .unwrap();
        let unsupported =
            super::super::capture::CaptureSource::original(&unsupported, sources, &pool).unwrap();
        let rejected = NumericalProducer::execute_captured_policy(
            sources,
            &environment,
            roots,
            mechanisms,
        ...
        drop(failed);
        assert_eq!(
            input.value().array.evaluated().unwrap().as_slice::<f32>(),
            logits
        );
...
fn original_cpu_summary_uses_shared_chunk_program_and_retires_failed_and_escaped_sources() {
    ...
    let (escaped, evidence, expected) = {
        ...
        let prepared = pool
            ...
            .unwrap();
        let materializer = MlxPreparedInputMaterializer::prepare_admitted(&pool).unwrap();
        let copy = materializer.with_test_original_copy_source(&prepared, &pool, |original| {
            IsolatedArrayCopy::new(original.array())
                .copy_prepared(
                    &original,
                    &environment,
        ...
        let flat = input
            .value()
            .array
            .reshape(&[1025], environment.stream())
            .unwrap();
        let expected = SummaryProgram::new(1025)
            .unwrap()
            .execute(
                &flat,
                environment.stream(),
        ...
        let NumericalOutput::CapturedLogits {
            ...
            panic!("summary lost logits")
        };
        assert_eq!(evidence.records().len(), 1);
        let CapturePayload::Summary(actual) = evidence.records()[0].payload.as_ref().unwrap()
        else {
            panic!("summary changed payload")
        };
    ...
    assert!(pool.fixture_host_charge().unwrap() > loaded);
    drop((target, draft));
    let CapturePayload::Summary(actual) = evidence.records()[0].payload.as_ref().unwrap() else {
        panic!("escaped summary lost payload")
    };
    assert_eq!(actual, &expected);
...
fn original_cpu_histogram_preserves_fixed_edges_counts_and_escaped_sources() {
    ...
    let (escaped, evidence) = {
        ...
        let expected = HistogramProgram::new(1025, &edges)
            .unwrap()
            .execute(
                &flat,
                environment.stream(),
        ...
        let capture = pool
            .compile_capture_source(
                PreparedCapturePlanCopy::inspect(&declaration(
                    Some(CaptureTransform::Histogram {
                        edges: edges.to_vec(),
                    }),
                    1025,
                ))
        ...
        let NumericalOutput::CapturedLogits {
            ...
            panic!("histogram lost logits")
        };
        assert_eq!(evidence.records().len(), 1);
        let CapturePayload::Histogram(actual) = evidence.records()[0].payload.as_ref().unwrap()
        else {
            panic!("histogram changed payload")
        };
        assert_eq!(actual.edges, edges);
        assert_eq!(actual.counts, expected_counts);
        assert_eq!(
            (actual.below, actual.above, actual.non_finite),
            (expected.below, expected.above, expected.non_finite)
        );
        ...
        assert_eq!(
            actual.counts.iter().sum::<u64>() + actual.below + actual.above + actual.non_finite,
            1025
        );
        // Exact source geometry remains mandatory even with a complete worker.
        let wrong = pool
    ...
    assert!(pool.fixture_host_charge().unwrap() > loaded);
    drop((target, draft));
    let CapturePayload::Histogram(actual) = evidence.records()[0].payload.as_ref().unwrap() else {
        panic!("escaped histogram lost payload")
    };
    assert_eq!(actual.edges, edges);
    assert_eq!(actual.counts, [192, 256, 257]);
    assert_eq!(escaped.value().array.shape(), [1, 1025]);
...
fn original_cpu_token_scores_preserve_distribution_ties_domain_and_escaped_sources() {
    ...
    let (escaped, evidence, expected, partition) = {
        ...
        let input = registered::value_input(copy, Meaning::Logits, sources, &environment).unwrap();
        let mut allowed = vec![true; 1025];
        allowed[1] = false;
        let filter = eredu_core::TokenFilter::Allowed(allowed);
        let tokenizer = eredu_core::TokenFilter::Allowed(vec![true; 1025]);
        let domain = CaptureTokenDomain {
            filter: (&filter).into(),
            tokenizer_validity: &tokenizer,
        };
        let mut expected = Vec::new();
        let partition = TokenScoreProgram::new(1025, &ids)
            .unwrap()
            .execute(
                &input.value().array,
                environment.stream(),
        ...
        let capture = pool
            .compile_capture_source(
                PreparedCapturePlanCopy::inspect(&declaration(
                    Some(CaptureTransform::TokenScores {
            ...
            .unwrap();
        let source = super::super::capture::CaptureSource::original(&capture, sources, &pool)
            .unwrap()
            .with_domain(domain);
        let policy =
            <DefaultSampler as SpeculativeSampler<MlxSamplingBackend>>::prepared_logit_policy(
        ...
        let NumericalOutput::CapturedLogits {
            ...
            panic!("token scoring lost logits")
        };
        assert_eq!(evidence.records().len(), 1);
        let CapturePayload::TokenScores(actual) = evidence.records()[0].payload.as_ref().unwrap()
        else {
            panic!("token scoring changed payload")
        };
        assert_eq!(actual.scores, expected);
        assert_eq!(actual.log_partition, partition);
        assert_eq!(actual.vocabulary, 1025);
        assert_eq!(actual.domain, Some(domain.summary(1025)));
        assert_eq!(
            actual
                .scores
                .iter()
                .map(|s| s.target.token_id)
                .collect::<Vec<_>>(),
            ids
        );
        let independent = logits
            .iter()
            .map(|&x| (f64::from(x) - 7.0).exp())
            .sum::<f64>()
            .ln()
            + 7.0;
        ...
        for score in &actual.scores {
            ...
            assert_eq!(score.target.score, logits[id]);
            assert_eq!(score.target.allowed, id != 1);
            assert_eq!(
                score.rank,
                1 + logits.iter().filter(|&&x| x > logits[id]).count() as u64
            );
            ...
            assert_eq!(alternative.score, 7.0);
            assert_eq!(alternative.allowed, id == 1);
        }
        let wrong = pool
            .compile_capture_source(
                PreparedCapturePlanCopy::inspect(&declaration(
        ...
        let output = value
            ...
            .try_to_vec::<f32>()
            .unwrap();
        for (id, &score) in output.iter().enumerate() {
            assert_eq!(
                score,
                if id == 1 {
                    f32::NEG_INFINITY
    ...
    let CapturePayload::TokenScores(actual) = evidence.records()[0].payload.as_ref().unwrap()
        ...
        panic!("escaped scores lost payload")
    };
    assert_eq!(actual.scores, expected);
    assert_eq!(actual.log_partition, partition);
    assert_eq!(escaped.value().array.shape(), [1, 1025]);
...
fn original_cpu_candidates_preserve_full_sort_ties_domains_failure_and_escaped_sources() {
    ...
    use crate::backend::array_copy::{CandidateExtraction, CaptureCompletion};
    let artifact = tempfile::tempdir().unwrap();
    crate::tests::distributed_pipeline_ring::write_fixture(artifact.path());
    let pool = crate::tests::support::test_utils::initialize_original_sources();
    let backend = admitted_backend(&pool);
        ...
        for count in [3u64, 4097] {
            ...
            let make_input = |data: &[f32]| {
                ...
                let prepared = pool
                    ...
                    .unwrap();
                let copy =
                    materializer.with_test_original_copy_source(&prepared, &pool, |original| {
                        IsolatedArrayCopy::new(original.array())
                            .copy_prepared(
                                &original,
                                &environment,
            ...
            let row = input
                .value()
                .array
                .reshape(&[1, 1, 4097], environment.stream())
                .unwrap();
            ...
            program
                .read_with_completion(
                    ...
                    CaptureCompletion::Ordinary,
                    |id, score| {
                        expected.push(CaptureCandidate {
                            token_id: id,
                            score,
                            allowed: filter.allows(id),
                        });
                        Ok(())
                    },
                )
            ...
            let capture = pool
                .compile_capture_source(
                    PreparedCapturePlanCopy::inspect(&declaration_with_attempts(
                        Some(CaptureTransform::TopCandidates { count }),
                        4097,
                        2,
                    ))
                    .unwrap(),
            ...
            let NumericalOutput::CapturedLogits {
                ...
                panic!("candidate capture lost logits")
            };
            let CapturePayload::Candidates(actual) =
                evidence.records()[0].payload.as_ref().unwrap()
            else {
                panic!("candidates changed payload")
            };
            assert_eq!(actual.candidates, expected);
            assert_eq!(actual.domain, Some(domain.summary(4097)));
            ...
            independent
                .sort_by(|&a, &b| logits[b].partial_cmp(&logits[a]).unwrap().then(b.cmp(&a)));
            for (candidate, &id) in actual.candidates.iter().zip(&independent) {
                assert_eq!(candidate.token_id, id as u32);
                assert_eq!(candidate.score, logits[id]);
                assert_eq!(candidate.allowed, id != 2);
            }
            let output = value
                .value()
                .array
            ...
            let mut nonfinite = logits.clone();
            nonfinite[0] = f32::NAN;
            let bad = make_input(&nonfinite);
            ...
            while let Some(error) = cause {
                finite_refusal |=
                    error.to_string() == "candidate capture requires finite raw logits";
                cause = error.source();
            }
    ...
    for (value, evidence, expected) in &escaped {
        let CapturePayload::Candidates(actual) = evidence.records()[0].payload.as_ref().unwrap()
        else {
            panic!("escaped candidates lost payload")
        };
        assert_eq!(actual.candidates, *expected);
        assert_eq!(value.value().array.shape(), [1, 4097]);
    }
    drop(escaped);
    drop(schedule);
    drop((target_config, draft_config, selected));
...
fn original_cpu_scale_interventions_preserve_order_evidence_partial_regions_and_escaped_sources() {
    ...
    let intervention_plan = |partial: bool| {
        ...
        let declaration = InterventionDiscovery {
            ...
            points: vec![InterventionPoint {
                ...
                axes: [
                    ("batch", SymbolicDimension::Batch),
                    ("sequence", SymbolicDimension::Sequence),
                    ("vocabulary", SymbolicDimension::Known(19)),
                ]
                .into_iter()
                .map(|(name, dimension)| TensorAxis {
                    name: name.into(),
        ...
        InterventionPlan {
            schema_version: INTERVENTION_SCHEMA_VERSION,
            operations: [
                (0.5, InterventionEvidence::Preview { max_elements: 3 }),
                (-2.0, InterventionEvidence::Summary),
            ]
            .into_iter()
            .enumerate()
            .map(|(i, (factor, evidence))| InterventionOperation {
                id: i.to_string(),
                target: path.into(),
    ...
    let (escaped, evidence, partial_escaped, partial_evidence, expected, partial_expected) = {
        ...
        let original = input
            .value()
            .array
            .reshape(&[1, 1, 19], environment.stream())
            .unwrap();
        let mut ordinary = MlxTensor::from_array(original);
        let mut worker = NativeCapture {
        ...
        for operation in &plan.plan().operations {
            ordinary = eredu_runtime::intervention::apply_activation(
                &mut worker,
                &ordinary,
                &operation.action,
                &slice,
        ...
        let capture = pool
            .compile_capture_source(
                PreparedCapturePlanCopy::inspect(&declaration(
                    Some(CaptureTransform::FullTensor),
                    19,
                ))
                .unwrap(),
            )
        ...
        let NumericalOutput::CapturedLogits {
            ...
            panic!("intervention lost captured logits")
        };
        assert_eq!(
            value
                .value()
        ...
        assert_eq!(
            values(&first.evidence[1]),
            &logits[..3].iter().map(|v| v * 0.5).collect::<Vec<_>>()
        );
        let second = &evidence.interventions()[1];
        for (record, factor) in second.evidence.iter().zip([0.5, -1.0]) {
            let CapturePayload::Summary(summary) = record.payload.as_ref().unwrap() else {
                panic!("summary evidence changed kind")
            };
        ...
        let partial_expected = ordinary
            ...
            .unwrap();
        drop(ordinary);
        let mut coordinates = logits.clone();
        for value in &mut coordinates[1..3] {
            *value = -*value;
        }
        assert_eq!(partial_expected, coordinates);
        let partial = pool
            .compile_intervention_source(
                PreparedInterventionPlanCopy::inspect(&partial_plan).unwrap(),
            )
            .unwrap();
        let source = super::super::capture::CaptureSource::original(&capture, sources, &pool)
            .unwrap()
            .with_interventions(&partial);
        let result = NumericalProducer::execute_captured_policy(
            sources,
            &environment,
            roots,
        ...
        let NumericalOutput::CapturedLogits {
            value: partial_value,
            capture: partial_evidence,
        } = result
        else {
            panic!("partial intervention lost captured logits")
        };
        assert_eq!(
            partial_value
                .value()
        ...
        assert_eq!(
            values(&first.evidence[1]),
            &logits[1..3].iter().map(|v| v * 0.5).collect::<Vec<_>>()
        );
        for (record, factor) in partial_evidence.interventions()[1]
            .evidence
            .iter()
            .zip([0.5, -1.0])
        {
            let CapturePayload::Summary(summary) = record.payload.as_ref().unwrap() else {
                panic!("partial summary changed kind")
            };
            assert_eq!(summary.finite, 2);
            let mean = logits[1..3]
                .iter()
                .map(|v| f64::from(*v) * factor)
                .sum::<f64>()
                / 2.0;
        ...
        let wrong = pool
            ...
            .unwrap();
        let source = super::super::capture::CaptureSource::original(&wrong, sources, &pool)
            .unwrap()
            .with_interventions(&partial);
        let rejected = NumericalProducer::execute_captured_policy(
            sources,
    ...
    assert_eq!(
        values(&evidence.interventions()[0].evidence[0]),
        &logits[..3]
    );
    assert_eq!(
        partial_escaped
    ...
    assert_eq!(
        values(&partial_evidence.interventions()[0].evidence[0]),
        &logits[1..3]
    );
    drop((escaped, evidence, partial_escaped, partial_evidence));
    drop(schedule);
    drop((target_config, draft_config, selected));
    settle(&pool, initial);
}
