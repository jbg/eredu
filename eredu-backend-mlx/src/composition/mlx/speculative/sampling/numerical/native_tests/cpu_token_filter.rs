...
fn original_cpu_policy_composes_mask_temperature_and_escaped_custody(){
    ...
    let (escaped,scaled,scaled_expected)={
        ...
        let policy=<DefaultSampler as SpeculativeSampler<MlxSamplingBackend>>::prepared_logit_policy(&DefaultSampler)
            .unwrap().bind(0.0,history.len()).unwrap();
        let mut last=None;
        for forced in [None,Some(2)]{
            let mask=eredu_runtime::generation::TokenMaskPlan::new(&allowed,source.value().array.shape(),forced).unwrap();
            let mut invalid=Vec::with_capacity(mask.elements());mask.fill(&mut invalid).unwrap();
            let ordinary=crate::backend::runtime::generation::apply_token_mask(
                &MlxTensor::from_array(source.value().array.clone()),&invalid,environment.stream()).unwrap();
            let expected=ordinary.as_array().evaluated().unwrap().try_to_vec::<f32>().unwrap();drop(ordinary);
            for(i,&value)in expected.iter().enumerate(){
                let token=i%5;let kept=(token==0||token==2)&&forced.is_none_or(|forced|token==forced as usize);
                assert_eq!(value,if kept{logits[i]}else{f32::NEG_INFINITY});
            }
            let result=NumericalProducer::execute_controlled_policy(sources,&environment,roots,mechanisms,
                policy,&source,&history,mask).unwrap();
            let NumericalOutput::Logits(masked)=result else{panic!("mask lost logit meaning")};
            assert_eq!(masked.value().array.evaluated().unwrap().as_slice::<f32>(),expected);
            assert!(masked.value().provenance.source().belongs_to_request(pair.request()));
            last=Some(masked);
        }
        assert!(eredu_runtime::generation::TokenMaskPlan::new(&allowed,source.value().array.shape(),Some(4)).is_err());
        let scaled=<DefaultSampler as SpeculativeSampler<MlxSamplingBackend>>::prepared_logit_policy(&DefaultSampler)
            .unwrap().bind(0.7,history.len()).unwrap();
        let mask=eredu_runtime::generation::TokenMaskPlan::new(&allowed,source.value().array.shape(),None).unwrap();
        let mut invalid=Vec::with_capacity(mask.elements());mask.fill(&mut invalid).unwrap();
        let ordinary=crate::backend::runtime::generation::apply_token_mask(
            &MlxTensor::from_array(source.value().array.clone()),&invalid,environment.stream()).unwrap();
        let ordinary=scaled.process::<MlxSamplingBackend>(&ordinary,&history,environment.stream()).unwrap();
        let expected=ordinary.as_array().evaluated().unwrap().try_to_vec::<f32>().unwrap();drop(ordinary);
        for(i,&actual)in expected.iter().enumerate(){
            assert_eq!(actual,if i%5==0||i%5==2{logits[i]*(1.0/0.7f32)}else{f32::NEG_INFINITY});
        }
        let result=NumericalProducer::execute_controlled_policy(sources,&environment,roots,mechanisms,
            scaled,&source,&history,mask).unwrap();
        let NumericalOutput::Logits(scaled)=result else{panic!("composed policy lost logit meaning")};
        assert_eq!(scaled.value().array.evaluated().unwrap().try_to_vec::<f32>().unwrap(),expected);
        assert!(scaled.value().provenance.source().belongs_to_request(pair.request()));
        assert_eq!(source.value().array.evaluated().unwrap().as_slice::<f32>(),logits);
    ...
    assert_eq!(escaped.value().array.evaluated().unwrap().as_slice::<f32>(),
        &[f32::NEG_INFINITY,f32::NEG_INFINITY,3.25,f32::NEG_INFINITY,f32::NEG_INFINITY,
          f32::NEG_INFINITY,f32::NEG_INFINITY,-4.0,f32::NEG_INFINITY,f32::NEG_INFINITY]);
    assert_eq!(scaled.value().array.evaluated().unwrap().try_to_vec::<f32>().unwrap(),scaled_expected);
    drop((escaped,scaled));drop(schedule);drop((target_config,draft_config,selected));settle(&pool,initial);
}
