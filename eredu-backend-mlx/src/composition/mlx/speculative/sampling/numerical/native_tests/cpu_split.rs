...
fn original_cpu_split_and_position_match_ordinary_and_preserve_failed_state() {
    ...
    let expected=[ordinary_words(ordinary.as_array()),ordinary_words(&ordinary_next),
        ordinary_words(&split_key_at(&safemlx::random::key(SEED).unwrap(),3,cpu.stream()).unwrap())];
    ...
    let escaped={
        ...
        let root=create_key(SEED,context).unwrap();let mut state=root.clone();
        let next=next_key(&mut state,context).unwrap();
        let at=key_at(&root,SpeculativeDraftRandomPosition::new(3),context).unwrap();
        assert_eq!([key_words(&state),key_words(&next),key_words(&at)],expected);
        assert_ne!(expected[0],expected[1]);assert_eq!(key_words(&root),[(SEED>>32) as u32,SEED as u32]);
        let current=state.value().value().array.try_allocation_info().unwrap().unwrap().identity();
        assert_eq!(current,next.value().value().array.try_allocation_info().unwrap().unwrap().identity());
        assert_ne!(current,root.value().value().array.try_allocation_info().unwrap().unwrap().identity());
        pair.request().close().unwrap();let saved=key_words(&state);
        let error=next_key(&mut state,context).unwrap_err();assert_eq!(key_words(&state),saved);drop(error);
        assert_eq!(pool.unquoted_owner_count().unwrap(),0);[state,next,at]
    };
