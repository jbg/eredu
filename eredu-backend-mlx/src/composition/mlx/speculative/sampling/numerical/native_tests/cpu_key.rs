...
fn original_cpu_key_preserves_unsigned_seed_words_and_escaped_custody() {
    ...
    let escaped={
        ...
        let keys=seeds.map(|seed| {
            ...
            let expected=ordinary_words(&ordinary);drop(ordinary);
            let output=NumericalProducer::execute_inputs(sources,&environment,roots,mechanisms,
                program::SpeculativeNumericalKind::CreateKey {seed},None,None,None).unwrap();
            let NumericalOutput::Key(key)=output else {panic!("seed constructor lost key meaning")};
            assert_eq!(key_words(&key),expected);
            assert_eq!(expected,[(seed>>32) as u32,seed as u32]);
            assert!(key.value().value().provenance.source().belongs_to_request(pair.request()));
            assert!(key.value().value().array.try_allocation_info().unwrap().is_some());
            key
        });
        assert_ne!(keys[0].value().value().array.try_allocation_info().unwrap().unwrap().identity(),
            keys[1].value().value().array.try_allocation_info().unwrap().unwrap().identity());
        pair.request().close().unwrap();
        let refusal=NumericalProducer::execute_inputs(sources,&environment,roots,mechanisms,
            program::SpeculativeNumericalKind::CreateKey {seed:17},None,None,None).unwrap_err();
        assert!(std::error::Error::source(&refusal).is_some());drop(refusal);
        assert_eq!(pool.unquoted_owner_count().unwrap(),0);keys
    };
    assert!(pool.used_bytes().unwrap()>loaded);drop((target,draft));
    for (key,seed) in escaped.iter().zip(seeds) {assert_eq!(key_words(key),[(seed>>32) as u32,seed as u32]);}
