//! Actual CPU probability readout from the shared Normalize producer.
...
fn original_cpu_probability_selects_nonzero_coordinate_and_preserves_source_custody() {
    ...
    let (escaped,expected)={
        ...
        for token in [2u32,4] {
            let selected=operators::Native::select(&output.value().array,token,environment.stream()).unwrap();
            let ordinary=selected.evaluated().unwrap().as_slice::<f32>()[0];drop(selected);
            let result=NumericalProducer::execute(sources,&environment,roots,mechanisms,
                program::SpeculativeNumericalKind::ProbabilityAt {token},&output,None).unwrap();
            let NumericalOutput::Probability(actual)=result else {panic!("probability lost scalar meaning")};
            assert_eq!(actual,ordinary);assert_eq!(actual,expected[token as usize]);assert!(actual>0.0);
        }
        let refusal=NumericalProducer::execute(sources,&environment,roots,mechanisms,
            program::SpeculativeNumericalKind::ProbabilityAt {token:5},&output,None).unwrap_err();
