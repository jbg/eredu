...
fn original_cpu_probability_selects_nonzero_coordinate_and_preserves_source_custody() {
    ...
    let (escaped, expected) = {
        ...
        let source =
            ...
            .unwrap();
        let ordinary =
            program::normalize::<operators::Native>(&source.value().array, environment.stream())
                .unwrap();
        let expected = ordinary.evaluated().unwrap().as_slice::<f32>().to_vec();
        drop(ordinary);
        ...
        .unwrap();
        let NumericalOutput::Distribution(output) = output else {
            panic!("normalization lost probability meaning")
        };
        assert_eq!(output.value().meaning, Meaning::Probabilities);
        assert_eq!(output.value().array.shape(), &[1, 1, 5]);
        assert_eq!(
            output.value().array.evaluated().unwrap().as_slice::<f32>(),
            expected
        );
        assert!(expected.iter().all(|p| *p > 0.0));
        assert!((expected.iter().sum::<f32>() - 1.0).abs() < 1e-6);
        assert!(output
            .value()
            .provenance
            .source()
        ...
        for token in [2u32, 4] {
            let selected =
                operators::Native::select(&output.value().array, token, environment.stream())
                    .unwrap();
            let ordinary = selected.evaluated().unwrap().as_slice::<f32>()[0];
            drop(selected);
            ...
            .unwrap();
            let NumericalOutput::Probability(actual) = result else {
                panic!("probability lost scalar meaning")
            };
            assert_eq!(actual, ordinary);
            assert_eq!(actual, expected[token as usize]);
            assert!(actual > 0.0);
        }
        let refusal = NumericalProducer::execute(
            sources,
            &environment,
            roots,
            mechanisms,
            program::SpeculativeNumericalKind::ProbabilityAt { token: 5 },
            &output,
            None,
        )
        .unwrap_err();
