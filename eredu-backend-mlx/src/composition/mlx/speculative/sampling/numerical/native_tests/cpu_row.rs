...
fn original_cpu_row_readout_preserves_selected_values_and_escaped_parent() {
    ...
    let escaped = {
        ...
        let output = NumericalProducer::execute(
            ...
            roots,
            mechanisms,
            program::SpeculativeNumericalKind::LogitsRow { row: 1 },
            &source,
            None,
        )
        .unwrap();
        let NumericalOutput::Logits(row) = output else {
            panic!("row lost logit meaning")
        };
        assert_eq!(row.value().array.shape(), &[1, 5]);
        assert_eq!(
            row.value().array.evaluated().unwrap().as_slice::<f32>(),
            expected
        );
        assert_eq!(
            row.value()
        ...
        let choice =
            ...
            .greedy(0.0)
            .unwrap();
        let selected = NumericalProducer::execute(
            sources,
            &environment,
            roots,
            mechanisms,
        ...
        .unwrap();
        let NumericalOutput::Token(token) = selected else {
            panic!("row greedy lost token meaning")
        };
        assert_eq!(token, 3);
        assert!(row
            .value()
