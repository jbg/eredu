...
fn original_cpu_categorical_matches_ordinary_and_preserves_key_failure_and_custody() {
    ...
    let (escaped,expected)={
        ...
        let expected=std::array::from_fn::<_,3,_>(|_|{
            let token=MlxSamplingBackend::sample_processed(&tensor,1.0,Some(&mut ordinary),environment.stream()).unwrap();
            let token=token.as_array().evaluated().unwrap().as_slice::<u32>()[0];
            assert!(token<5);(token,ordinary_words(ordinary.as_array()))
        });
        ...
        for (token,key) in expected {
            assert_eq!(sample_stochastic(&DefaultSampler,&source,1.0,&mut state,context).unwrap(),token);
