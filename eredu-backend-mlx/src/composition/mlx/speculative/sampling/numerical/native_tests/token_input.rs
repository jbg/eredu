...
fn token_input_case(use_cpu: bool) {
    ...
    let escaped = {
        ...
        .unwrap();
        let environment = cpu.as_ref().unwrap_or(&backend).original_copy_environment().unwrap();
        assert_eq!(environment.stream().device_type().unwrap(),
            if use_cpu { safemlx::DeviceType::Cpu } else { safemlx::DeviceType::Gpu });
        ...
        let value = {
        if use_cpu {
            let backing = value.value().array.try_allocation_info().unwrap().unwrap().identity();
            let output = NumericalProducer::execute(sources, &environment, roots, mechanisms,
                program::SpeculativeNumericalKind::TokenRange { start: 1, end: 3 }, &value, None).unwrap();
            let NumericalOutput::Tensor(range) = output else { panic!("CPU range lost tensor meaning"); };
            assert_eq!(range.value().meaning, Meaning::TokenIds);
            assert_eq!(range.value().array.shape(), &[1,2]);
            assert_eq!(range.value().array.evaluated().unwrap().as_slice::<u32>(), &ids[1..3]);
            assert_eq!(range.value().array.try_allocation_info().unwrap().unwrap().identity(), backing);
            assert!(range.value().provenance.source().belongs_to_request(pair.request()));
        ...
        assert_eq!(value.value().array.shape(), if use_cpu { &[1,2][..] } else { &[1,4][..] });
