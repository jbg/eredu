...
fn prepared(array: &safemlx::Array, elements: usize) -> PreparedPendingTokenInput<'_> {
    if elements == 1 { PreparedPendingTokenInput::new_fixed(array) }
    else { PreparedPendingTokenInput::new_prefill_fixed(array, NonZeroU64::new(elements as u64).unwrap()) }
        .unwrap()
}
...
fn original_cpu_pending_input_preserves_signed_scalar_prefill_and_escaped_custody() {
    ...
    let schedule = AutoregressiveSchedulePlan::new(&selected, NonZeroUsize::new(1).unwrap(),
        NonZeroU64::new(2).unwrap(), NonZeroU64::new(32).unwrap(), &config,
    ...
    let escaped = {
        ...
        [
            (&[1usize][..], HostTensorValues::I32(&scalar), &scalar_expected[..]),
            (&[1usize, 1, 1][..], HostTensorValues::U32(&scalar_unsigned), &scalar_unsigned[..]),
            (&[1usize, 5][..], HostTensorValues::I32(&signed), &signed_expected[..]),
            (&[1usize, 5][..], HostTensorValues::U32(&unsigned), &unsigned[..]),
            ...
            let parts = [HostInputPart { modality: eredu_core::InputModality::Text,
                ...
                payload: HostTensorView { shape, values }, metadata: &[], extents: &[] }];
            let input = pool.compile_prepared_host_input(PreparedHostInputPlan::prepare(&parts).unwrap()).unwrap();
            let (copy, refused) = materializer.with_test_original_copy_source(&input, &pool, |original| {
                ...
                let ordinary = prepared(original.array(), expected.len()).copy_retained(environment.stream(), &ordinary_roots).unwrap();
                let ordinary_values = ordinary.array().evaluated().unwrap().try_to_vec::<u32>().unwrap();
                ...
                let refused = prepared(&alias, expected.len()).copy_registered_with_prepared(Some(&original),
                    &environment, roots, mechanisms, sources.metadata_funding(), REQUEST_CEILING).err().unwrap();
                drop(alias);
                let copy = prepared(original.array(), expected.len()).copy_registered_with_prepared(Some(&original),
                    &environment, roots, mechanisms, sources.metadata_funding(), REQUEST_CEILING).unwrap();
                ...
                assert_eq!(copy.array().evaluated().unwrap().as_slice::<u32>(), ordinary_values);
                (copy, refused)
            });
            drop(input);
    ...
    for (copy, _, expected) in &escaped {
        assert_eq!(copy.array().evaluated().unwrap().as_slice::<u32>(), *expected);
    }
    drop(escaped);
    drop(schedule);
