...
pub(crate) fn copy_value_to(
    ...
    let (sources, environment) = context
        .original_numerical_for(destination)
        .ok_or_else(invalid)?;
    ...
    if !std::ptr::eq(sources, prior) || !environment.pool().same_ledger(source_environment.pool()) {
        return Err(invalid());
    }
    NumericalProducer::validate_input_at(value, context, origin)?;
    sources.validate_environment(environment)?;
    let input = value.value();
    if !matches!(input.meaning, Meaning::RandomKey | Meaning::Logits) {
        return Err(invalid());
    }
