...
use eredu_runtime::replicated_session::{
    PrefillScoreLayout, PrefillSourceProgress, PrefillSpanOperation, ReplicatedTextSessionError,
};
use std::marker::PhantomData;

type Architecture<A> = PreparedCompositeArchitecture<A>;
type Session<A, D> = ReplicatedTextSession<
    Architecture<A>,
    MlxNeuralBackend,
    MlxReplicatedTextMechanisms<Architecture<A>, MlxHybridState>,
    D,
>;
type Receiver<'a> =
    dyn eredu_architectures::external_assistant::ExternalPrefillReceiver<MlxTensor, Error> + 'a;
...
) -> Result<PrefillSourceProgress<Option<MlxTensor>>, Error>
where
    ...
    >,
{
    let mut lowerer = MlxCompositePredictionInput::<A> {
        admission,
        processor,
    };
    prefill_input::with_source::<Architecture<A>, MlxHybridState, _, _>(
        &mut lowerer,
        input,
        context,
        |prepared, context| {
            let mut prepared = Some(prepared.and_then(|p| {
                let shape = <_ as PredictionPrefillPlan<
                    Architecture<A>,
                    MlxNeuralBackend,
                    MlxHybridState,
                >>::shape(&p)?;
                let chunk = <_ as PredictionPrefillPlan<
                    Architecture<A>,
                    MlxNeuralBackend,
                    MlxHybridState,
                >>::chunk_positions(&p);
                let whole = <_ as PredictionPrefillPlan<
                    Architecture<A>,
                    MlxNeuralBackend,
                    MlxHybridState,
                >>::requires_whole_input(&p);
                Ok((p, shape, chunk, whole))
            }));
            let facts = prepared
                .as_ref()
                .and_then(|p| p.as_ref().ok())
                .map(|(_, shape, chunk, whole)| (*shape, *chunk, *whole));
            let shape = facts.map(|f| f.0);
            let chunk = receiver.prefill_chunk_positions(
                facts.and_then(|f| f.1),
                shape.map(|s| s[1]),
                facts.is_some_and(|f| !f.2),
            );
            let progress = external_state::with_state::<A, D, _>(
                session,
                cache,
                context.target(),
                Some(context),
                |session, prior| {
                    charge(
                        ...
                        std::mem::size_of::<(
                            Span<'_, '_, '_, A, D>,
                            Result<Span<'_, '_, '_, A, D>, Error>,
                            PrefillSourceProgress<Option<MlxTensor>>,
                            Result<PrefillSourceProgress<Option<MlxTensor>>, SpanError>,
                        )>(),
                    )?;
                    ...
                    session
                        .try_prefill_speculative_source_with_operation(
                            ...
                            demand,
                            |geometry| {
                                let (source, _, _, _) = prepared
                                    .take()
                                    .expect("one source factory")
                                    .map_err(|e| neural(e, context))?;
                                <_ as PredictionPrefillPlan<
                                    Architecture<A>,
                                    MlxNeuralBackend,
                                    MlxHybridState,
                                >>::into_source(source, geometry)
                            },
                            cancellation,
                            context.target(),
            ...
            if let Some(Err(cause)) = prepared {
                return Err(cause);
            }
            Ok(progress)
        },
    ...
    O: eredu_runtime::ActivationObserver<MlxTensor, eredu_nn::Error> + ?Sized,
{
    fn has_speculative_span_authority(&self) -> bool {
        self.context.original_external().is_some()
    }
    fn score_layout(&self) -> PrefillScoreLayout {
        if self.context.original_external().is_some() {
    ...
    fn prepare(
        ...
        prefill_input::prepare_chunk::<Architecture<A>, MlxHybridState, P>(
            source,
            chunk,
            self.context,
        )
    ...
    fn execute<'s>(
        ...
        input: Result<
            <Architecture<A> as eredu_runtime::LayeredArchitecture<
                MlxNeuralBackend,
                MlxHybridState,
            >>::Input<'s>,
            SpanError,
    ...
    > {
        ...
        let execute =
            |session: &mut Session<A, D>,
             ...
             metadata: Option<&eredu_nn::workspace::WorkspaceContext>| {
                ...
                let (scores, captured, frontier, commit) = session
                    .prefill_capture_span(input, chunk.output, stream, &mut observer, |forward| {
                        let values = capture.values()?;
                        let output = match metadata {
                            Some(metadata) => A::external_prediction_capture_with_metadata(
                                request, forward, values, metadata,
                            ),
                            None => A::external_prediction_capture(request, forward, values),
                        }?;
                        output.ok_or_else(|| match metadata {
                            Some(metadata) => metadata.metadata_error(format_args!(
                                "target omitted selected external capture"
                            )),
                            None => {
                                eredu_nn::Error::backend("target omitted selected external capture")
                            }
                        })
                    })
                    .map_err(|e| external_state::failure(e, Some(context)))?;
        ...
        let consumed = self
            .receiver
            .consume_with_evidence(
                chunk,
                frontier,
                &mut scores,
                capture.ok_or_else(|| {
                    SpanError::Architecture(neural(Error::PrefillScopeUnavailable, context))
                })?,
                evidence,
            )
            .map_err(|e| neural(e, context));
