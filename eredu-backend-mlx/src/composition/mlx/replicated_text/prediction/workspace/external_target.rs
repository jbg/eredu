        ...
        >,
    {
        ...
        if !valid
            || capture.is_some()
                != matches!(
                    invocation.kind(),
                    Kind::TargetPrefill | Kind::TargetVerification
                )
        {
        ...
        let mut completion = CompletionTrace::new(recorder, &context)?;
        ...
        let recipe = match (local_recorder, parallel_recorder) {
            (Some(recorder), None) => {
                recorder.finish_external_operation(report.span_workspace_plan())
            }
            (None, Some(recorder)) => {
                recorder.finish_external_operation(report.span_workspace_plan())
            }
