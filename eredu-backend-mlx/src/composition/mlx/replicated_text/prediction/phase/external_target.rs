...
use crate::composition::mlx::speculative::{
    completed_tensor_source,
    embedded_native::{ActiveEmbeddedNativeInvocation, EmbeddedNativeLayout, ModelEquation},
    retain_external_evidence_for_placement, validate_input_evidence,
};
...
type ParallelArguments<'a, F> = (F, Option<&'a Capture>, &'a WorkspaceContext);
fn execute_parallel<A, D, F>(
    session: &mut Session<A, D>,
    (execute, capture, context): ParallelArguments<'_, F>,
) -> Result<Output, Error>
where
...
{
    execute(session, capture, Some(context))
}
...
impl Output {
    ...
    fn visit(&self, visit: &mut dyn FnMut(&Array)) {
        ...
        if let Some(capture) = &self.capture {
            capture.visit_values(&mut |v| visit(v.as_array()));
        }
    }
}
struct Payload {
...
fn prior_sources<'a>(
    ...
    let mut prior = funding.metadata_vec(
        inputs
            .len()
            .checked_add(usize::from(state.is_some()))
            .ok_or(Error::PrefillControl(WorkingMemoryError::Overflow))?,
    )?;
    ...
    ) -> Result<Output, Error>,
{
    let Some(selected) = context.original_external() else {
        ...
        let capture = request
            .map(|r| Capture::prepare::<A>(r, None))
            .transpose()?;
    ...
    let frames = [
        ...
        size_of::<(
            &mut Session<A, D>,
            Option<&ExternalPredictionCaptureRequest>,
            SpeculativeExecutionStreams<'_>,
    ...
    let positions = {
        ...
        match descriptor.shape() {
            [1, n, ..] if *n > 0 => {
                u64::try_from(*n).map_err(|e| sources.retain_startup_error(e))?
            }
            _ => return Err(sources.retain_startup_error(WorkingMemoryError::IdentityMismatch)),
        }
    };
    let frontier = if request.is_some() {
        session
    ...
    ) -> Result<Output, Error>,
{
    ...
    if request.is_some() {
        ...
        parts
            .recipe
            .with_native_recipe(|r| r.bind_external_target_completion(invocation, distribution))
            .map_err(|cause| sources.retain_error(cause))?;
    }
    let completion_stream =
        safemlx::StreamCopyPlan::<HostMetadataFunding>::capture(environment.stream())
            .map_err(|e| sources.retain_startup_error(e))?;
    let completion_controls = if request.is_some() {
        NestedCompletionOwner::control_bytes(completion_roots, validation_roots, &completion_stream)
            .and_then(|n| u64::try_from(n).ok())
            .ok_or_else(|| sources.retain_startup_error(WorkingMemoryError::Overflow))?
    } else {
        0
    };
    let (bank_controls, source_facts) = parts
        .recipe
        .with_native_recipe(|r| {
    ...
    let model_control = Mechanisms::<A>::prepare_session_embedded_model_control(
        session,
        parts.recipe.native_recipe(),
        sources.model_control_source(),
        funding,
    )?;
    let parallel_controls = Mechanisms::<A>::session_embedded_parallel_control_bytes::<
        D,
        ParallelArguments<'_, F>,
        Output,
    >(session)
    .ok_or_else(|| sources.retain_startup_error(WorkingMemoryError::Overflow))?;
    ...
    .map_err(|e| sources.retain_startup_error(e))?;
    let capture = request
        .map(|r| Capture::prepare::<A>(r, Some(&parts.context)))
        .transpose()
        .map_err(|cause| sources.retain_startup_error(cause))?;
    ...
    let mut payload = Payload {
        ...
        _prior: prior,
        _state_prior: state_prior
            .as_ref()
            .map(|source| source.try_clone_for_source(environment.stream(), funding))
            .transpose()
            .map_err(|cause| sources.retain_error(cause))?,
    };
    ...
    let execute_handler =
        |work: &mut Work<'_, A, D, F>,
         ...
         active: &ActiveEmbeddedNativeInvocation<'_, OriginalExternalSpeculativeRole>| {
            ...
            let output = Mechanisms::<A>::with_session_embedded_parallel(
                work.0,
                active.parallel_projection(),
                &q.parallel_funding,
                (execute, q.capture.as_ref(), &q.context),
                execute_parallel::<A, D, F>,
            );
            drop(activation);
            let output = output?;
            if let Some(completion) = q.completion.as_ref() {
    ...
    let addressable_callbacks =
        AddressableModelSource::callback_controls::<_, Payload, _, Output, _>(&execute_handler)
            .ok_or_else(|| sources.retain_startup_error(WorkingMemoryError::Overflow))?;
    let handlers = (
        |work: &mut Work<'_, A, D, F>,
         q: &Payload,
         ...
         materialized: OriginalMaterializedLoan<'_>| {
            ...
            let bank = Mechanisms::<A>::prepare_session_external_neural_bank(
                ...
                work.2
                    ...
                    .clone(),
                scope,
                q.addressable.is_some().then_some(&mut partition),
                Some(materialized),
            )?;
            if let Some(source) = q.addressable.as_ref() {
    ...
    if request.is_some() {
        payload.completion = Some(
            NestedCompletionOwner::prepare_external(
                completion_roots,
                validation_roots,
                &role,
                funding,
