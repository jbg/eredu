...
impl ScheduledCaptureBackend for Backend<'_> {
    ...
    fn validate_routed_invocation_source(&self, source: &RoutedUnitCaptureSource<'_, MlxTensor>,
        geometry: &CaptureRoutedUnitsGeometry<'_>) -> Result<TensorDtype, eredu_runtime::capture::FundedCaptureError<Self::Error>> {
        crate::backend::array_copy::CompletedRoutedCaptureSource::validate_borrowed(
            &routed_source(source), geometry.bank(), geometry.source_shape()[0] as u64)
            .map_err(eredu_runtime::capture::FundedCaptureError::Backend)?;
        floating_dtype(source.values.as_array()).map_err(eredu_runtime::capture::FundedCaptureError::Backend)
    }
    ...
    fn transform_routed_batch(&mut self, source: &RoutedUnitCaptureSource<'_, MlxTensor>,
        ...
        crate::backend::array_copy::execute_speculative_routed_capture(&routed_source(source), writer,
            self.stream, self.roots, self.custody, self.observer)
            .map_err(eredu_runtime::capture::FundedCaptureError::Backend)
    }
    fn routed_intervention_range(&self, source: &RoutedUnitCaptureSource<'_, MlxTensor>,
        ...
        self.edits.ok_or(FundedCaptureError::Backend(CaptureTensorNativeError::ClaimMismatch))?
            .routed_range(&routed_source(source), claim, self.custody).map_err(FundedCaptureError::Backend)
    }
    fn routed_intervention_usage(&self, source: &RoutedUnitCaptureSource<'_, MlxTensor>,
        claim: &eredu_runtime::working_memory::CaptureInterventionClaim<'_>)
        -> Result<CaptureUsage, eredu_runtime::capture::FundedCaptureError<Self::Error>> {
        use eredu_runtime::capture::FundedCaptureError;
        self.edits.ok_or(FundedCaptureError::Backend(CaptureTensorNativeError::ClaimMismatch))?
            .routed_usage(&routed_source(source), claim, self.custody).map_err(FundedCaptureError::Backend)
    }
    fn apply_routed_intervention(&mut self, source: &RoutedUnitCaptureSource<'_, MlxTensor>,
        ...
        use eredu_runtime::capture::FundedCaptureError;
        self.edits.ok_or(FundedCaptureError::Backend(CaptureTensorNativeError::ClaimMismatch))?
            .execute_routed(&routed_source(source), batch, self.stream, self.roots, self.custody, self.observer)
    ...
    fn intervention_projection_usage(
        &self, source:&MlxTensor, claim:&eredu_runtime::working_memory::CaptureInterventionClaim<'_>,
    ) -> Result<[CaptureUsage;2],eredu_runtime::capture::FundedCaptureError<Self::Error>> {
        self.edits.ok_or(eredu_runtime::capture::FundedCaptureError::Backend(CaptureTensorNativeError::ClaimMismatch))?
            .projection_usage(source.as_array(),claim,self.custody).map_err(eredu_runtime::capture::FundedCaptureError::Backend)
    }
    ...
    fn intervention_evidence_usage(
        &self,source:&MlxTensor,claim:&eredu_runtime::working_memory::CaptureInterventionEvidenceClaim<'_, '_>,
    )->Result<(TensorDtype,CaptureUsage),eredu_runtime::capture::FundedCaptureError<Self::Error>> {
        use eredu_runtime::{capture::FundedCaptureError,working_memory::CaptureInterventionEvidenceKind};
        self.edits.ok_or(FundedCaptureError::Backend(CaptureTensorNativeError::ClaimMismatch))?
            .check_evidence(source.as_array(),claim,self.custody,false).map_err(FundedCaptureError::Backend)?;
        Ok(match claim.kind() {
            CaptureInterventionEvidenceKind::Preview(claim)=>(
                self.validate_source(source,claim.geometry()).map_err(FundedCaptureError::Backend)?,
    ...
    fn capture_intervention_evidence<'a>(
        &mut self,source:&MlxTensor,claim:eredu_runtime::working_memory::CaptureInterventionEvidenceClaim<'a,'_>,
    )->Result<eredu_runtime::working_memory::ClaimedInterventionEvidence<'a>,eredu_runtime::capture::FundedCaptureError<Self::Error>> {
        use eredu_runtime::{capture::FundedCaptureError,working_memory::CaptureInterventionEvidenceKind};
        self.edits.ok_or(FundedCaptureError::Backend(CaptureTensorNativeError::ClaimMismatch))?
            .check_evidence(source.as_array(),&claim,self.custody,true).map_err(FundedCaptureError::Backend)?;
        ...
        Ok(match kind {
            CaptureInterventionEvidenceKind::Preview(claim)=>receipt.finish_preview(self.transform(source,claim).map_err(FundedCaptureError::Backend)?)?,
    ...
    fn preflight_generated(
        ...
        self.generated().prepare(prototype.as_array(), source, program, construct, claim)
            .map_err(eredu_runtime::capture::FundedCaptureError::Backend)
    }
    fn retain_generated_source(
        ...
        self.generated().source(prototype.as_array(), role, value.as_array(), claim)
            .map_err(eredu_runtime::capture::FundedCaptureError::Backend)
    }
    fn retain_generated_output(
        ...
        self.generated().retain(value.as_array(), claim)
            .map_err(eredu_runtime::capture::FundedCaptureError::Backend)
    }
...
fn routed_source<'a>(source: &RoutedUnitCaptureSource<'a, MlxTensor>) -> RoutedUnitCaptureSource<'a, Array> {
    RoutedUnitCaptureSource { values: source.values.as_array(), token_indices: source.token_indices.as_array(),
        selection_indices: source.selection_indices.as_array(), coefficients: source.coefficients.as_array(),
        source_groups: source.source_groups.as_array(), token_offset: source.token_offset,
