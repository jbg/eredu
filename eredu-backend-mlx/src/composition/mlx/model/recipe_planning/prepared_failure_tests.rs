...
use eredu_runtime::working_memory::{SpeculativeRequestError, WorkingMemoryError};
use std::sync::{
    Arc,
...
fn funding() -> (HostMetadataFunding, Arc<State>) {
    ...
    (
        HostMetadataFunding::new(Account(state.clone())).unwrap(),
        state,
    )
}
#[test]
fn prepaid_phase_failure_preserves_budget_source_without_a_second_debit() {
    ...
    assert!(planning_error_has_funding::<SpeculativeRequestError>(
        &error, &funding
    ));
    let mut source: Option<&(dyn StdError + 'static)> = Some(&error);
    let mut budget = false;
    while let Some(cause) = source {
        if let Some(WorkingMemoryError::Domain(eredu_core::MemoryDomainError::BudgetExceeded {
