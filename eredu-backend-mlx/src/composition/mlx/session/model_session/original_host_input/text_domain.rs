...
use eredu_runtime::{input::host::{HostInputPartView, HostTensorValues}, working_memory::OriginalTokenizer};
...
pub(in crate::composition::mlx::session::model_session) fn validate_text_domain(
    ...
    tokenizer.validate_pool(pool).map_err(|_| R::IdentityMismatch)?;
    ...
    completed.validate_request_source(pool, &prompt.parts, cache)
        .map_err(|_| R::IdentityMismatch)?;
    for part in completed.borrowed_semantics().source().parts() {
        if part.kind() != InputPayloadKind::TokenIds { continue; }
        let valid = match part.payload().values {
            HostTensorValues::U32(ids) => ids.iter().all(|&id| domain.allows(id)),
            HostTensorValues::I32(ids) => ids.iter().all(|&id|
                u32::try_from(id).is_ok_and(|id| domain.allows(id))),
            _ => return Err(R::InvalidToken),
        };
        if !valid { return Err(R::InvalidToken); }
    }
    Ok(())
}
