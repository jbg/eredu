...
use eredu_core::cache::{
    CachePolicyError, LayerCachePolicy, StateTensorDimension, StateTensorDtype, StateTensorPolicy,
    StateTensorPresence, StateTensorRole,
};
use eredu_core::{
    AttentionPolicy, CapabilityError, EstimationCompleteness, InputTokenCount, ObservationKind,
    RuntimeStateEstimate, StateMemoryAssumptions, StateMemoryLayout,
};
...
pub fn estimate_runtime_state(
    ...
    for (layer, policy) in layout.layer_layout().iter().enumerate() {
        let layer_positions = requested_positions.saturating_sub(u64::from(
            layout.layer_prefix_offsets()[layer].unsigned_abs(),
        ));
