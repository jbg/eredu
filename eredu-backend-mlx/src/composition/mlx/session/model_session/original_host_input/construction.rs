...
fn error_controls<E: std::error::Error + Send + Sync + 'static>() -> Option<usize> {
    size_of::<Failure<E>>().checked_add(BackendFailure::source_retention_peak_bytes::<Failure<E>>()?)
}
fn controls() -> Option<usize> {
    let parts = [
        error_controls::<InitializerError>()?, error_controls::<WorkingMemoryError>()?,
        error_controls::<OriginalPreparedHostInputError>()?, error_controls::<MediaSemanticError>()?,
        ...
        size_of::<PreparedHostInputPlan<'_>>(), size_of::<OriginalPreparedHostInput>(),
        size_of::<Result<OriginalPreparedHostInput, OriginalPreparedHostInputError>>(),
        size_of::<PreparedMediaSemanticCompile<'_, '_>>(), size_of::<Result<PreparedMediaSemanticCompile<'_, '_>, MediaSemanticError>>(),
        size_of::<OriginalPreparedMediaSemantics<'_>>(), size_of::<Result<OriginalPreparedMediaSemantics<'_>, SemanticError>>(),
        size_of::<MlxPreparedModelInputPlan<'_>>(), size_of::<Result<MlxPreparedModelInputPlan<'_>, WorkingMemoryError>>(),
        size_of::<MlxOriginalPreparedModelInput>(), size_of::<Result<MlxOriginalPreparedModelInput, MlxPreparedModelInputError>>(),
        size_of::<Result<MlxModelInput, MlxPreparedModelInputBindError>>(),
        size_of::<OriginalModelInput<MlxModelInput>>(),
        size_of::<Result<OriginalModelInput<MlxModelInput>, OriginalModelInputPublicationError<MlxModelInput>>>(),
...
pub(in crate::composition::mlx::session::model_session) fn prepare_model_input(
    ...
    let blueprint = model.inference_blueprint()
    ...
    let materializer = MlxPreparedInputMaterializer::prepare_admitted(pool)
        .map_err(|cause| failure(cause, &funding))?;
    let source = pool.compile_prepared_host_input(plan).map_err(|cause| failure(cause, &funding))?;
    let semantics = blueprint.plan_original_media_semantics(&source)
        .map_err(|cause| failure(cause, &funding))?.compile(pool)
        .map_err(|cause| failure(cause, &funding))?;
    let input = materializer.model_input_plan(&semantics)
        .map_err(|cause| failure(cause, &funding))?.materialize(pool)
        .map_err(|cause| failure(cause.retire_storage(), &funding))?;
    let input = input.bind(runtime, semantics)
        .map_err(|cause| failure(cause.retire_storage(), &funding))?;
