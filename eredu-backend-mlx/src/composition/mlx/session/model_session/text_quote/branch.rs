...
use eredu_runtime::replicated_session::{ControlBranchPlacement, ControlBranchSource};
...
pub(in crate::composition::mlx::session::model_session) fn exchange<'d>(
    runtime: &mut ModelRuntime<MlxBackend<'d>>,
    installed: TextBranchSource<'_, MlxBackend<'d>>,
    incoming: TextBranchSource<'_, MlxBackend<'d>>,
    slot: &mut super::super::MlxNativeTextState,
) -> Result<(), Error> {
    if installed.state().sampling.quote.is_none() && incoming.state().sampling.quote.is_none() {
        ...
        for source in [&installed, &incoming] {
            if source.state().sampling.sampler.is_funded()
                || source.state().funding.is_some()
    ...
    installed_quote
        .request
        .validate_branch_exchange(
            installed.context(),
            &incoming_quote.request,
            incoming.context(),
        )
        .map_err(memory)?;
    ...
    if slot.host_preparation.is_none() || installed_quote.same_owner(&incoming_quote) {
        return Err(mismatch());
    }
    let capacity = installed_quote
        .config
        .inference_policy()
        .memory_limits
        .resolve(runtime.backend().memory_ledger().topology())
        .map_err(|cause| memory(cause.into()))?;
    let funding = installed_quote
        .model_pool
...
fn validate_source(
    quote: &TextExecutionQuoteOwner,
    source: &ControlBranchSource,
    state: &generation::MlxTextGenerationState,
    context: &TextStepContext,
    pending: Option<PendingTextInput<&MlxModelInput, &MlxTextToken>>,
) -> Result<(), Error> {
    if quote
        .local_prediction_fixed(state.sampling.next_prediction)
        .map_err(memory)?
        != context.attempt()
        || state.sampling.temperature != quote.sampling_temperature()?
        || state.sampling.parameter_epoch != Some(quote.parameter_epoch)
        || !state.sampling.sampler.is_funded()
    {
        return Err(mismatch());
    }
    source
        .validate_frontier(quote.prediction_frontier(state.sampling.next_prediction)?)
        .map_err(memory)?;
    match pending {
        Some(PendingTextInput::Prefill(prompt)) => {
            ...
            quote
                ...
                .validate_source(
                    source.revision(),
                    source.admission(),
                    quote.request.geometry().cached_positions,
                )
                .map_err(memory)?;
            if let Some(input::OriginalMediaPacket::Original(packet)) =
                prompt.original_media.as_ref()
            {
                quote
                    .validate_completed_pending_input(prompt)
                    .map_err(memory)?;
                let semantics = prompt
                    .placement_semantics
                    .as_ref()
                    .or_else(|| quote.copied_media_semantics())
        ...
        Some(PendingTextInput::Decode(token)) => {
            if token.state_revision() != Some(source.revision()) {
                return Err(mismatch());
            }
            token
                .step_receipt()
                .ok_or_else(mismatch)?
                .validate_completed_source(&quote.request, context.attempt())
                .map_err(memory)?;
            source
                .admission()
                .ok_or_else(mismatch)?
...
fn place_pending(
    pending: Option<PendingTextInput<&mut MlxModelInput, &mut MlxTextToken>>,
    quote: &TextExecutionQuoteOwner,
    placement: &ControlBranchPlacement,
) {
    match pending {
        ...
        Some(PendingTextInput::Prefill(prompt)) => {
                ...
                prompt.original_media.as_ref()
            {
                ...
                let transition = placement
                    .media_transition(semantics.binding())
                    .expect("validated exact media placement");
                let semantics = semantics
                    .for_copied_state(&transition)
                    .expect("actual source-to-placement transition");
...
#[inline(never)]
fn exchange_funded<'d>(
    runtime: &mut ModelRuntime<MlxBackend<'d>>,
    installed: TextBranchSource<'_, MlxBackend<'d>>,
    incoming: TextBranchSource<'_, MlxBackend<'d>>,
    slot: &mut super::super::MlxNativeTextState,
    installed_quote: &TextExecutionQuoteOwner,
    incoming_quote: &TextExecutionQuoteOwner,
    funding: &HostMetadataFunding,
) -> Result<(), Error> {
    let parts = [
        size_of::<[ControlBranchSource; 2]>(),
        size_of::<[ControlBranchPlacement; 2]>(),
        ...
        size_of::<(
            TextBranchSource<'_, MlxBackend<'_>>,
            TextBranchSource<'_, MlxBackend<'_>>,
        )>(),
        size_of::<
            [Option<
    ...
    let sources = runtime
        ...
        .model
        .erased()
        .original_control_branch_sources(slot.state.as_ref())?;
    validate_source(
        installed_quote,
        &sources[0],
        installed.state(),
    ...
    if let Some(previous) = slot.displaced_placement.as_ref() {
        previous.validate_destination(&sources[1]).map_err(memory)?;
    }
    let incoming_source = slot
        .displaced_placement
        .as_ref()
    ...
    let pending = installed_quote
        .request
        .begin_branch_exchange(
            installed.context(),
            &incoming_quote.request,
            incoming.context(),
            funding,
    ...
    let mut installed_opening = (installed.context().attempt() == 0)
        .then(|| installed_quote.opening.placement_slot())
        .transpose()
        .map_err(memory)?;
    let mut incoming_opening = (incoming.context().attempt() == 0)
        .then(|| incoming_quote.opening.placement_slot())
        .transpose()
        .map_err(memory)?;
    ...
    let result = installed_quote.with_parallel_control(runtime, |runtime| {
        ...
        let [installed, mut incoming] = result
            .placements
            .expect("typed original branch exchange returns both placements");
        if let Some(previous) = slot.displaced_placement.take() {
            incoming = incoming
                .through(&previous)
                .expect("validated exact deferred placement");
        }
        std::mem::swap(&mut payload.state_memory, &mut slot.memory_retention);
        if let Some(opening) = installed_opening.as_mut() {
            **opening = Some(installed.revision().clone());
        }
        if let Some(opening) = incoming_opening.as_mut() {
            **opening = Some(incoming.revision().clone());
        }
