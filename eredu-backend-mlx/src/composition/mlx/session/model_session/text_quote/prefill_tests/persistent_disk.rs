...
fn persistent_cpu_disk_window_releases_later_request_accounts() {
    ...
    let inspection = eredu_architectures::configuration::inspect_artifact_with_prepared_gguf_headers(
        artifact.path(),
    ).unwrap();
    ...
    let source_baseline = pool.used_bytes().unwrap().checked_sub(retiring).unwrap();
    let mut runtime = target.into_runtime().unwrap();
    let stream = runtime.backend().stream().clone();
    assert!(runtime.backend().memory_pool().same_domain(&pool));
    {
        ...
        assert_eq!(workspace.destination_device_type(), safemlx::DeviceType::Cpu);
        assert!(workspace.disk_receipt().is_none(),
            "canonical loading retains the prepared foreground payload reader");
        assert_eq!(model.inference_blueprint().unwrap().selected().text_realization()
            .residency().device_depth(2), 2);
    }
    ...
    for request in 0..4 {
        ...
        let mut sequence = generation.take_prepared_sequence().unwrap().prepare_storage().unwrap();
        let mut tokens = Vec::new();
        for token in &mut generation {
            let token = token.unwrap_or_else(|error| panic!("request {request}: {error:?}"));
            sequence.commit(token.token_id(), eredu_core::TokenTerminalSignals::default()).unwrap();
        ...
        let state = runtime.session().payload.model.erased().resident_reset_source().unwrap();
        let numeric = state.state().retained_arrays().into_iter().map(|value| (
            value.shape().to_vec(), value.evaluated().unwrap().try_to_vec::<f32>().unwrap(),
        )).collect::<Vec<_>>();
        assert!(!numeric.is_empty());
        assert!(numeric.iter().flat_map(|(_, values)| values).all(|value| value.is_finite()));
        assert!(numeric.iter().flat_map(|(_, values)| values).any(|value| *value != 0.0));
        assert!(runtime.session().payload.model.erased().state_snapshot().iter()
        ...
        drop(state);
        runtime.prepare_reset_ordinary().unwrap()
            .reset_admitted(eredu_core::SessionResetLimits::new(CAPACITY)).unwrap();
        runtime.synchronize().unwrap();
        disk::reclaim();

        let report = runtime.session().residency_report().unwrap().unwrap();
        let disk_units = report.units().iter().filter(|unit| unit.planned_tier() == MemoryTier::Disk)
            .collect::<Vec<_>>();
        assert_eq!(disk_units.len(), 2);
        assert_eq!(disk_units.iter().filter(|unit| unit.device_resident()).count(), 1,
        ...
        runtime.session().payload.model.collect_retained_idle_storage(&mut nonstate, &mut decoder).unwrap();
        ...
        if let Some(original) = &retained_parameters {
            ...
            source_charge = Some(pool.used_bytes().unwrap());
