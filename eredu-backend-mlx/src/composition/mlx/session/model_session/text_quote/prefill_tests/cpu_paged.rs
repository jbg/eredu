    ...
    for original in [false, true] {
        ...
        let factory = crate::MlxBackendFactory::default().with_state_residency(
            eredu_runtime::CacheResidencyPolicy::Paged(
                // The fixture declares one KV head of width8, F32, two positions
                ...
                eredu_runtime::PagedCacheOptions::new(
                    2,
                    if spill {
                        2 * 2 * 1 * 2 * 8 * 4
                    } else {
        ...
        // Loading creates the actual transfer stream before state realization.
        // Only its independently identified process-registration accounts are
        // permanent; all model, manager, wrapper and request charges must retire.
        let transfer_births = runtime
            .session()
            .payload
            .model
            .erased()
            ...
            .unwrap()
            .state()
            .permanent_transfer_registry_bytes()
            .unwrap();
        assert!(transfer_births > 0);
        let baseline = baseline.checked_add(transfer_births).unwrap();
        let stream = runtime.backend().stream().clone();
        let input = [2, 5, 7, 3, 11, 13];
        let mut request = config(original);
        let mut policy = request.inference_policy().clone();
        policy.submission_tracking_capacity_bytes = None;
        policy.graph_metadata_capacity_bytes = None;
        request = request.with_inference_policy(policy.clone());
        let mut generation = if original {
            ...
            TextGeneration::new(&mut runtime, input.to_vec(), request)
        }
        .unwrap();
        ...
        for (step, output) in (&mut generation).enumerate() {
            let output = output.unwrap_or_else(|error| {
                panic!("paged CPU original={original}, spill={spill}, output={step}: {error:?}")
            });
            let token = output.token_id().unwrap();
            tokens.push(token);
        ...
        if spill {
            ...
            assert!(
                report.host_demotions > 0,
                "the actual store worker must run"
            );
            assert!(
                report.host_promotions > 0,
                "the actual reload worker must run"
            );
        }
