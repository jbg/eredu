...
use eredu_runtime::working_memory::{
    InferenceTextStep, OriginalNativeStorageMechanism, OriginalTextSamplingExtension,
    SamplingWorkspaceReport, TextHostControlFacts, WorkspaceSamplingRandomState,
...
impl SamplingRevisionOwner {
    fn new(revision: SamplingRevision) -> Result<Self, Error> {
        let bytes = [
            size_of::<SamplingRevision>(),
            size_of::<Self>(),
            size_of::<Result<Self, Error>>(),
            size_of::<Box<SamplingRevision>>(),
            size_of::<Option<Box<SamplingRevision>>>(),
            size_of::<SamplingRevision>(),
        ]
        .into_iter()
...
impl SamplingRevision {
    ...
    fn work(&self) -> Result<super::super::text_funding::FundedWorkOwner, Error> {
        super::super::text_funding::FundedWork::new_model_with_native(
            self.extension.funding().scope().map_err(memory)?,
            Some(self.extension.control_guard()),
            None,
            None,
            None,
            None,
...
impl TextExecutionQuote {
    pub(in crate::composition::mlx::session) fn sampling_temperature(&self) -> Result<f32, Error> {
        ...
        Ok(revision
            .as_ref()
            .map_or(self.config.sampling().temperature, |revision| {
                revision.temperature
            }))
    }
    pub(in crate::composition::mlx::session::model_session) fn sampling_work(
        &self,
        step: &InferenceTextStep,
    ...
    fn replace_sampling_at_boundary(
        ...
        let capacity = self
            ...
            .inference_policy()
            .memory_limits
            .resolve(self.model_pool.topology())
        ...
        let funding = self
            ...
            .map_err(Error::WorkspacePlanning)?;
        let result = self.prepare_sampling_revision(
            runtime, state, context, change, remaining, capacity, &funding,
        );
        match result {
            ...
            Err(cause) => Err(crate::composition::mlx::model::retain_planning_error(
                cause, funding,
            )),
        }
    }
    fn prepare_sampling_revision(
        ...
        let controls = [
            size_of::<SamplingRevision>(),
            size_of::<Option<SamplingRevisionOwner>>(),
            size_of::<Result<(SamplingRevisionOwner, Option<RandomState>), Error>>(),
            size_of::<SamplingWorkspaceReport>(),
        ...
        let physical = mechanism.sampling_program(
            quote.workspace(),
            &program,
            change.reseed().is_some(),
            funding,
        )?;
        ...
        let graph_capacity = graph_fit
            .select_for_policy(policy.graph_metadata_capacity_bytes)
            .map_err(memory)?;
        let graph_facts = graph::selected_facts(
            policy.graph_metadata_capacity_bytes,
            Some(graph_capacity),
            graph_fit.control_bytes().ok_or_else(unknown)?,
        )?
        .ok_or_else(unknown)?;
        let record_capacity = program
            .record_storage_requirement()?
            .select_for_policy(policy.submission_tracking_capacity_bytes)
            .map_err(memory)?;
        let tracking_facts = tracking::selected_facts(
            policy.submission_tracking_capacity_bytes,
            Some(record_capacity),
        )?
        .ok_or_else(unknown)?;
        let work_controls = super::super::text_funding::work_control_bytes()?
            .checked_mul(
                u64::try_from(physical.works())
                    .map_err(|_| memory(WorkingMemoryError::Overflow))?,
            )
            .ok_or_else(|| memory(WorkingMemoryError::Overflow))?;
        let pipeline_controls =
            pipeline_cache::control_bytes_for_attempts(program.kernel_attempts())?
                .ok_or_else(unknown)?;
        let admission = u64::try_from(controls)
            .ok()
            .and_then(|n| n.checked_add(pipeline_controls))
            .ok_or_else(|| memory(WorkingMemoryError::Overflow))?;
        let (sampling, event) = prediction::sampling_facts()?;
        let mut prepared = quote
            .prepare_controls(TextHostControlFacts::new(
                Some(admission),
                Some(0),
                Some(work_controls),
            ))
            .map_err(memory)?
            .with_sampling_prediction_scopes(sampling, event)
            .map_err(memory)?
            .with_submission_tracking(tracking_facts)
            .map_err(memory)?
            .with_graph_metadata(graph_facts)
            .map_err(memory)?
            .with_native_storage(native)
            .map_err(memory)?;
        if change.reseed().is_some() {
            prepared = prepared
                .with_sampling_reseed_scope(Some(preparation::reseed_control_bytes()?))
                .map_err(memory)?;
        }
        let mut extension = quote
            .with_controls(prepared)
            .map_err(memory)?
            .admit(capacity)
            .map_err(memory)?;
        let mut bank = extension
            .take_native_storage_bank::<MlxNativeStorage>(mechanism.selection())
            .map_err(memory)?
            .ok_or_else(unknown)?;
        bank.install(mechanism).map_err(|cause| {
            crate::backend::runtime::residency::storage::native_storage::retained_failure(
                cause,
                extension.control_guard(),
                true,
            )
        })?;
        let bank = BankOwner::new(bank);
        let graph = graph::allocate(
            extension
                .take_graph_metadata()
                .map_err(memory)?
                .ok_or_else(unknown)?,
        )?;
        let tracking = tracking::allocate(
            extension
                .take_submission_tracking()
                .map_err(memory)?
                .ok_or_else(unknown)?,
        )?;
        pipeline_cache::install_for_attempts(
            program.kernel_attempts().ok_or_else(unknown)?,
            &extension.control_guard(),
            &graph,
        )?;
        let roles = extension
            ...
            .map_err(memory)?
            .ok_or_else(unknown)?;
        let scopes = prediction::PredictionScopes::new(
            roles,
            Some(tracking.clone()),
            Some(graph.clone()),
            extension.control_guard(),
        ...
        let replacement = change
            ...
            .transpose()?;
        Ok((
            SamplingRevisionOwner::new(SamplingRevision {
