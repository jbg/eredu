...
const TRACKING: u64 = 1 << 20;
fn config(total: u64, tracking: u64) -> TextGenerationConfig {
    let config = fixture::config(4, total);
    let mut policy = config.inference_policy();
    policy.submission_tracking_capacity_bytes = NonZeroU64::new(tracking);
        ...
        for mode in 0..6 {
            ...
            for controlled in [false, true] {
                ...
                let retained = probe.as_ref().map(|probe| {
                    ...
                    let arena = quote.record_quota.as_ref().unwrap().clone();
                    assert!(arena.same_arena(quote.record_quota.as_ref().unwrap()));
                    assert!(arena.occupied_bytes() <= TRACKING as usize);
                ...
                if let Some((arena, held)) = retained {
                    ...
                    assert_eq!(
                        ...
                        0,
                        "actual Records have retired, arena remains"
                    );
                    drop(arena);
                }
...
#[test]
fn ordinary_tracking_capacity_is_in_the_first_quote_exact_and_minus_one_before_work() {
    let stream = fixture::stream();
    for route in 0..3 {
        let pool = WorkingMemoryPool::new(u64::MAX, 0).unwrap();
        ...
        let (preparation, quote) =
            super::super::admit(&runtime, &input, config(u64::MAX, TRACKING), &controller).unwrap();
        let required = preparation.request().memory_reservation().unwrap().bytes();
        assert!(required > TRACKING);
        let arena = quote.record_quota.as_ref().unwrap();
        assert_eq!(
            arena.occupied_bytes(),
            0,
            "cold arena construction has no Record"
        );
        ...
        let short = super::super::admit(
            &runtime,
            &input,
            config(baseline + required - 1, TRACKING),
            &controller,
        )
        .unwrap_err();
        assert!(matches!(
        ...
        let (preparation, quote) = super::super::admit(
            &runtime,
            &input,
            config(baseline + required, TRACKING),
            &controller,
        )
        .unwrap();
        assert_eq!(
...
fn ordinary_exhaustion_preserves_typed_source_and_original_arena_without_refill() {
    ...
    let capacity = (1..=TRACKING)
        .find(|n| {
            PreparedSubmissionRecordQuota::<OriginalSubmissionTracking>::layout(*n as usize).is_ok()
        })
        .unwrap();
    ...
    assert_eq!(
        *cause::<safemlx::error::SubmissionTrackingFailure>(&error),
        safemlx::error::SubmissionTrackingFailure::Exhausted
    );
    let preparation = probe.take();
    let quote = preparation.quote.as_ref().unwrap();
    let arena = quote.record_quota.as_ref().unwrap().clone();
...
#[test]
fn original_tracking_prompt_busy_preserves_the_same_input_work_node_and_arena() {
    ...
    for _ in 0..3 {
        ...
        assert_eq!(arena.occupied_bytes(), 0, "Busy has submitted no Record");
    }
