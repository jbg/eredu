...
use eredu_architectures::component_partition::{
    RoutedPartitionCaptureRank, RoutedPartitionCaptureSource,
};
use eredu_core::capture::PartitionCaptureContext;
use eredu_core::capture::{
    CaptureCoordinateProjectionPlan, CaptureRoutedUnitsGeometry, PartitionRoutedUnitCaptureLayout,
    PartitionRoutedUnitCaptureRequest, RoutedUnitGeometry,
};
use eredu_runtime::capture::partition::{
    PartitionCaptureRoutedFragmentGeometry, PartitionCaptureRoutedLocalSource,
    PartitionCaptureRoutedProducerSource, PreparedPartitionRoutedSource,
};
...
impl std::fmt::Debug for RowSource {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RoutedHostSource")
...
) -> Result<ResolvedCaptureSlice, Error> {
    Ok(ResolvedCaptureSlice {
        starts: copy(&axes[0], metadata)?,
        ends: copy(&axes[1], metadata)?,
        strides: copy(&axes[2], metadata)?,
        shape: copy(&axes[3], metadata)?,
    })
}
...
fn fragment_limit(
    selected: &RoutedPartitionCaptureSource<'_>,
    geometry: &CaptureRoutedUnitsGeometry<'_>,
    metadata: &HostMetadataFunding,
) -> Result<usize, Error> {
    let mut global = [0u64; 3];
    for (out, &n) in global.iter_mut().zip(geometry.source_shape()) {
        *out = u64::try_from(n).map_err(|_| overflow())?;
    }
    let mut shape = [0u64; 3];
    for (out, &n) in shape.iter_mut().zip(geometry.shape()) {
        *out = u64::try_from(n).map_err(|_| overflow())?;
    }
    let axes = [
        geometry.starts().try_into().map_err(|_| unknown())?,
        geometry.ends().try_into().map_err(|_| unknown())?,
        geometry.strides().try_into().map_err(|_| unknown())?,
        shape,
    ];
    let selected_slice = slice(axes, metadata)?;
    let mut total = 0usize;
    for rank in 0..selected.world_size() {
        let Some(local) = selected.rank(rank).filter(|local| local.produces) else {
            continue;
        };
...
pub(super) fn prepare(
    ...
    let selected = loaded
        ...
        .map_err(|cause| Error::Neural(metadata.metadata_source(cause)))?;
    let geometry = CaptureRoutedUnitsGeometry::prepare_receipt(source.admission(), context)
        .map_err(|cause| Error::Neural(metadata.metadata_source(cause)))?;
    ...
    if geometry.bank() != selected.geometry()
        || *declared != selected.geometry()
        || routing != selected.routing()
        || selected.effective()
            != (source.admission().points()[index].position
    ...
    let receipt = PartitionCaptureReceiptPlan::new_routed_shared_funded(
        source,
        context,
        &producers,
        selected.world_size(),
    ...
    let count = receipt
        .producers()
        .try_fold(0usize, |n, (_, p)| n.checked_add(p.fragments().len()))
        .ok_or_else(overflow)?;
    let bank = geometry.bank();
    let source_tokens = u64::try_from(geometry.source_shape()[0]).map_err(|_| overflow())?;
    ...
    if let Some(text) = inference.prefill(context.prediction) {
        eredu_core::capture::CaptureRoutedPrefillPlan::prepare(source.admission(), index, text)
            .map_err(|cause| Error::Neural(metadata.metadata_source(cause)))?;
    }
    Ok(Prototype {
        source: super::RowSource::Routed(RowSource {
            index,
            bank,
            source_tokens,
            native,
...
impl RowSource {
        ...
        <T::Completion as eredu_core::Completion>::Error: Send + Sync + 'static,
    {
        ...
        let selected = self
            ...
            .routed_capture_source(&selection.path)
            .map_err(|cause| Error::Neural(metadata.metadata_source(cause)))?;
        if selected.geometry() != self.bank {
            return Err(memory(WorkingMemoryError::IdentityMismatch));
        }
        PreparedPartitionRoutedSource::local_execution_metadata_bytes::<T>(
            self.native.len(),
    ...
    pub(super) fn execution_metadata_bytes(
        ...
        let selected = self
            ...
            .routed_capture_source(&selection.path)
            .map_err(|cause| Error::Neural(metadata.metadata_source(cause)))?;
        if selected.geometry() != self.bank || rank >= selected.world_size() {
            return Err(memory(WorkingMemoryError::IdentityMismatch));
        }
        let dtype = scalar.map(|dtype| match dtype {
        ...
        let local = match selected.rank(rank) {
            Some(local) => Some(PartitionCaptureRoutedLocalSource {
                producer: rank,
        ...
        let producers = (0..selected.world_size()).filter_map(|rank| {
            selected
                .rank(rank)
                .filter(|local| local.produces)
                .map(|local| PartitionCaptureRoutedProducerSource {
                    rank,
                    ownership: local.ownership,
                })
        });
        let native = self
            .native
            .iter()
            .map(|row| PartitionCaptureRoutedFragmentGeometry {
        ...
        W::metadata_vec_bytes::<PartitionCaptureRoutedProducerSource<'_>>(selected.producer_count())
            .and_then(|bytes| {
            ...
            .and_then(|bytes| {
                bytes.checked_add(PreparedPartitionRoutedSource::local_metadata_bytes(
                    producers, native, local,
                )?)
            })
            .ok_or_else(overflow)
    }
    pub(super) fn bind(
        ...
        let dtype = scalar.map(|value| match value {
            WorkspaceFloatingType::Float32 => TensorDtype::F32,
            WorkspaceFloatingType::Float16 => TensorDtype::F16,
            WorkspaceFloatingType::Bfloat16 => TensorDtype::Bf16,
        });
        ...
        for row in &self.native {
            let owner = selected
                ...
                .filter(|local| local.produces)
                .ok_or_else(unknown)?;
            native.push(PartitionCaptureRoutedFragmentGeometry {
                producer: row.producer,
                fragment: row.fragment,
                estimate: row.estimate,
                request: PartitionRoutedUnitCaptureRequest {
        ...
        let prepared = match inference {
            FrameGeometry::Text(text) if prediction == 0 => {
                PreparedPartitionRoutedSource::new_local_prefill(
                    source, self.index, &producers, &native, local, text, metadata,
                )
            }
            FrameGeometry::Text(_) => PreparedPartitionRoutedSource::new_local_decode(
                source, self.index, &producers, &native, local, prediction, metadata,
            ),
            FrameGeometry::Invocation(value) => PreparedPartitionRoutedSource::new_local_shaped(
                source,
                self.index,
                &producers,
                &native,
...
pub(super) fn controls() -> Option<usize> {
    let parts = [
        size_of::<RowSource>() * 2,
        size_of::<Native>() * 2,
        size_of::<LoadedPartitionCapture>() * 2,
        size_of::<Vec<Native>>(),
        size_of::<RoutedPartitionCaptureSource<'_>>() * 2,
        size_of::<Option<RoutedPartitionCaptureRank<'_>>>() * 2,
        size_of::<CaptureRoutedUnitsGeometry<'_>>() * 2,
        size_of::<CaptureCoordinateProjectionPlan<'_>>() * 2,
        size_of::<[[u64; 3]; 4]>() * 2,
        size_of::<[u64; 3]>() * 2,
        size_of::<ResolvedCaptureSlice>() * 2,
        size_of::<PartitionRoutedUnitCaptureRequest<'_>>() * 2,
        size_of::<PartitionRoutedUnitCaptureLayout<'_>>(),
        size_of::<PartitionCaptureRoutedLocalSource<'_>>(),
        size_of::<Vec<PartitionCaptureRoutedProducerSource<'_>>>() * 2,
        ...
        size_of::<Result<PreparedPartitionRoutedSource, Error>>(),
        ...
        size_of::<(
            &RoutedPartitionCaptureSource<'_>,
            &CaptureRoutedUnitsGeometry<'_>,
            &HostMetadataFunding,
        )>(),
        size_of::<([[u64; 3]; 4], &HostMetadataFunding)>(),
        size_of::<Option<TensorDtype>>(),
        size_of::<std::ops::Range<usize>>() * 3,
        ...
        size_of::<
            Result<
                eredu_core::capture::CaptureRoutedPrefillPlan<'_>,
                eredu_core::capture::CapturePrefillGeometryError,
            >,
        >(),
        size_of::<(
