...
pub(super) fn source<E: std::error::Error + Send + Sync + 'static>(
    ...
    metadata.source(RetainedFailure {
        cause,
        _funding: metadata.funding(),
    })
}
...
pub(super) fn validate_parts(
    ...
    let phase = |point: &ObservationPoint, phase| {
        support
            .points
            .iter()
            .find(|entry| entry.path == point.path)
            .is_some_and(|entry| {
                matches!(
                    match phase {
    ...
    metadata
        .charge(
            bytes
                .ok_or_else(|| source(metadata, eredu_core::HostMetadataFundingError::Overflow))?,
        )
        .map_err(|cause| metadata.error(cause))?;
    admission
        .revalidate_borrowed_declarations(catalog, support.schema_version, &support.capture, phase)
