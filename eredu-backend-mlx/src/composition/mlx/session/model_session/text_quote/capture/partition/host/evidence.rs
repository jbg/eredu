...
use eredu_runtime::{
    layered::PreparedCaptureSelection, working_memory::OriginalInterventionSource,
};
pub(in crate::composition::mlx) use model::{ModelEvidenceHostSource, ModelEvidenceHosts};
...
impl std::fmt::Debug for RowSource {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("EvidenceHostSource")
...
#[derive(Debug)]
struct Entry<T> {
    selection: Option<PreparedCaptureSelection>,
    hosts: T,
}
#[derive(Debug)]
...
pub(in crate::composition::mlx::session::model_session::text_quote::capture) struct Operation {
    pub(in crate::composition::mlx::session::model_session::text_quote::capture) source:
        SharedCapturePlan,
    pub(in crate::composition::mlx::session::model_session::text_quote::capture) selection:
        Option<PreparedCaptureSelection>,
    pub(in crate::composition::mlx::session::model_session::text_quote::capture) hosts:
        Vec<Option<HostRow>>,
}
fn controls() -> Option<usize> {
    let parts = [
        ...
        size_of::<Hosts>() * 2,
        size_of::<Operation>() * 2,
        size_of::<Entry<HostAdmission>>() * 2,
        size_of::<Entry<HostRows>>() * 2,
        size_of::<Vec<Option<Entry<HostAdmission>>>>(),
        size_of::<Vec<Option<Entry<HostRows>>>>(),
        size_of::<Vec<Option<Operation>>>(),
        size_of::<Option<PreparedCaptureSelection>>() * 2,
        size_of::<PartitionCaptureCoordinateProducer<'_>>() * 2,
        size_of::<Vec<PartitionCaptureCoordinateProducer<'_>>>(),
        size_of::<CaptureCoordinateProjectionPlan<'_>>() * 2,
        size_of::<CaptureTensorGeometry<'_>>() * 2,
        size_of::<CaptureSummaryGeometry<'_>>() * 2,
        size_of::<Result<ResolvedCaptureSlice, Error>>(),
        size_of::<Result<Option<Admission>, Error>>(),
        size_of::<Result<Hosts, Error>>(),
        size_of::<Result<Operation, Error>>(),
        size_of::<Result<Prototype, Error>>(),
        size_of::<Result<Option<HostAdmission>, Error>>(),
        size_of::<LoadedPartitionCapture>(),
        size_of::<(
            &MlxModelSession,
            &SharedCapturePlan,
        ...
        size_of::<std::ops::Range<u64>>() * 2,
        size_of::<(usize, u64, bool)>(),
...
impl Admission {
    pub(in crate::composition::mlx::session::model_session::text_quote::capture) fn prepare(
        ...
        original: Option<&OriginalInterventionSource>,
        ...
        if session.payload.distributed.is_none() {
            return Ok(None);
        };
        ...
        if plan
            ...
            .operations
            .iter()
            .all(|operation| operation.evidence == InterventionEvidence::None)
        {
            return Ok(None);
        };
        ...
        if plan.request() != parent.admission().request()
            ...
            || !selected.source().same_storage(parent)
        {
            return Err(memory(WorkingMemoryError::IdentityMismatch));
        }
        let mut entries = metadata.metadata_vec(plan.plan().operations.len())?;
        let mut bytes = 0u64;
        for (operation, entry) in plan.plan().operations.iter().enumerate() {
            ...
            let Some(hosts) = HostAdmission::prepare_evidence(
                session, parent, original, operation, companion, geometry, first, metadata,
            )?
            else {
                entries.push(None);
                continue;
            ...
            let prefill = first == 0
                && entry.schedule.includes(CapturePhase::Prefill, 0)
                && eredu_runtime::intervention::InterventionPrefillWindow::row_axis(
                    &plan.points()[operation],
                );
            let selection = if prefill {
                Some(
                    if selected.is_prepared_media() {
                        selected.paths().prepare_media_capture_selection(companion)
                    } else {
                        selected.paths().prepare_capture_selection(companion)
                    }
    ...
    pub(in crate::composition::mlx::session::model_session::text_quote::capture) const fn bytes(
        &self,
    ) -> u64 {
        self.bytes
    }
    pub(in crate::composition::mlx::session::model_session::text_quote::capture) fn protect(
        self,
        funding: &WorkingMemoryFundingRun,
        reservation: &WorkingMemoryReservation,
    ) -> Result<Hosts, Error> {
        self.metadata
            .reserve_metadata(controls().ok_or_else(overflow)?)?;
                ...
                entry
                    .map(|entry| {
                        Ok::<_, Error>(Entry {
                            ...
                            hosts: entry.hosts.protect(funding, reservation)?,
                        })
                    })
                    .transpose()?,
            );
...
impl Hosts {
    pub(in crate::composition::mlx::session::model_session::text_quote::capture) fn take(
        &mut self,
        original: &OriginalInterventionSource,
        geometry: InferenceGeometry,
        prediction: u64,
        metadata: &HostMetadataFunding,
    ) -> Result<Vec<Option<Operation>>, Error> {
        metadata.reserve_metadata(controls().ok_or_else(overflow)?)?;
        if !self.original.same_source(original) {
            return Err(memory(WorkingMemoryError::IdentityMismatch));
        }
        let plan = original.plan().admission();
        ...
        for (index, entry) in self.entries.iter_mut().enumerate() {
            if !plan.plan().operations[index]
                .schedule
                .includes(phase, prediction)
                || plan.plan().operations[index].evidence == InterventionEvidence::None
            {
                output.push(None);
                continue;
            }
            ...
            if hosts.len() != 2 || hosts.iter().any(Option::is_none) {
                return Err(memory(WorkingMemoryError::IdentityMismatch));
            }
            output.push(Some(Operation {
                source: source.clone(),
...
impl HostAdmission {
    fn prepare_evidence(
        ...
        metadata.reserve_metadata(controls().ok_or_else(overflow)?)?;
        let plan = original.plan().admission();
        let entry = plan.plan().operations.get(operation).ok_or_else(unknown)?;
        let (artifact, execution, setup) = loaded.source_labels();
        if setup != distributed.session_identity()
            || first >= plan.request().max_predictions
            || source.admission().plan().selections.len() != 2
        {
            return Err(memory(WorkingMemoryError::IdentityMismatch));
        }
        let count = (first..plan.request().max_predictions)
            .filter(|&prediction| {
        ...
        let mut frames = metadata.metadata_vec(count)?;
        let mut bytes = 0u64;
        for prediction in first..plan.request().max_predictions {
            ...
            let mut context = PreparedPartitionCaptureRunIdentity::prepare_host_context(
                source,
                artifact,
                execution,
                setup,
            ...
            for index in 0..2 {
                ...
                let super::RowSource::Evidence(ref owner) = row.source else {
                    unreachable!("evidence source")
                };
                let host = if owner.prefill {
                    PartitionFragmentHostPlan::prepare_prefill(&row.receipt, geometry)
                } else {
                    PartitionFragmentHostPlan::prepare(&row.receipt)
                }
                .map_err(|cause| Error::Neural(metadata.metadata_source(cause)))?;
                bytes = bytes
        ...
        Ok(Some(Self {
            frames,
            geometry: FrameGeometry::Text(geometry),
            bytes,
            source: source.clone(),
            metadata: metadata.clone(),
        }))
...
fn prepare(
    ...
    let selection = &admission.plan().selections[index];
    let selected = loaded
        .layouts()
        .component_capture_source(&selection.path)
        .map_err(|cause| Error::Neural(metadata.metadata_source(cause)))?;
    let point = &original.plan().admission().points()[operation];
    let prefill = geometry.prefill(context.prediction).is_some()
    ...
    for rank in 0..selected.world_size() {
        if let Some(local) = selected.rank(rank).filter(|local| local.produces) {
            maximum = maximum
                .checked_add(local.coordinates.local_count().max(1))
                .ok_or_else(overflow)?;
            producers.push(PartitionCaptureCoordinateProducer {
                rank,
    ...
    let limits = PartitionCaptureReceiptLimits {
        max_producers: selected.world_size(),
    ...
    let mut quotation = CaptureLedger::new(parent.admission());
    quotation.begin_step();
    let receipt = PartitionCaptureReceiptPlan::new_coordinates_shared_funded(
        source,
        context,
        axis,
        &producers,
    ...
    let member = loaded
        ...
        .ok_or_else(unknown)?
        .intervention_source(
            original.plan().admission(),
            operation,
            context.phase,
            context.prediction,
        )
        .map_err(|cause| Error::Neural(metadata.metadata_source(cause)))?;
    let local_shape = member
        .map(|member| {
            if member.axis() != axis
                || selected
                    .rank(rank)
                    .is_none_or(|row| row.coordinates != member.coordinates())
            {
                return Err(memory(WorkingMemoryError::IdentityMismatch));
            }
            let mut shape = copy(global, metadata)?;
            shape[axis] =
                u64::try_from(member.coordinates().local_count()).map_err(|_| overflow())?;
            Ok::<_, Error>(shape)
        })
        .transpose()?;
    let count = receipt
        .producers()
        .try_fold(0usize, |count, (_, p)| {
            count.checked_add(p.fragments().len())
        })
        .ok_or_else(overflow)?;
    ...
    for (producer, projection) in receipt.producers() {
        for fragment in 0..projection.fragments().len() {
            let estimate = if prefill {
                let plan = PartitionPrefillCaptureGeometry::prepare(
                    admission,
                    index,
                    projection,
                    fragment,
                    selected.combination(),
                    geometry.prefill(context.prediction).ok_or_else(unknown)?,
                )
                .map_err(|cause| Error::Neural(metadata.metadata_source(cause)))?;
                crate::composition::mlx::session::capture_workspace::estimate_partition_prefill(
                    &plan,
                )
            } else {
                let plan =
                    PartitionInvocationCaptureGeometry::from_receipt(&receipt, producer, fragment)
                        .map_err(|cause| Error::Neural(metadata.metadata_source(cause)))?;
                crate::composition::mlx::session::capture_workspace::estimate_partition_invocation(
                    &plan,
                )
            }
            .map_err(|cause| Error::Neural(metadata.metadata_source(cause)))?;
    ...
    Ok(Prototype {
        source: super::RowSource::Evidence(RowSource {
            index,
            axis,
            prefill,
...
impl RowSource {
    pub(super) fn prefill(&self) -> bool {
        self.prefill
    }
    pub(super) fn bind(
        self,
        ...
        prediction: u64,
        metadata: &HostMetadataFunding,
    ) -> Result<PreparedPartitionContiguousSource, Error> {
        metadata.reserve_metadata(controls().ok_or_else(overflow)?)?;
        let selected = self
            .loaded
            .layouts()
            .component_capture_source(&source.admission().plan().selections[self.index].path)
            .map_err(|cause| Error::Neural(metadata.metadata_source(cause)))?;
        if selected.combination() != self.combination {
            return Err(memory(WorkingMemoryError::IdentityMismatch));
        }
        let mut producers = metadata.metadata_vec(selected.producer_count())?;
        for rank in 0..selected.world_size() {
        ...
        for row in &self.native {
            native.push(PartitionCaptureFragmentGeometry {
        ...
        if !geometry.matches_prediction(prediction) {
            return Err(memory(WorkingMemoryError::IdentityMismatch));
        }
        match geometry {
            FrameGeometry::Invocation(invocation) => {
                PreparedPartitionContiguousSource::new_local_coordinates_shaped(
                    source,
                    self.index,
                    self.axis,
                    &producers,
            ...
            FrameGeometry::Text(inference) if self.prefill => {
                PreparedPartitionContiguousSource::new_local_coordinates(
                    source,
                    self.index,
                    self.axis,
                    &producers,
            ...
            FrameGeometry::Text(_) => {
                PreparedPartitionContiguousSource::new_local_coordinates_invocation(
                    source,
                    self.index,
                    self.axis,
                    &producers,
...
impl RowSource {
    pub(super) fn projected_execution_metadata_bytes<T: NativePartitionCaptureTransport>(
        &self,
        source: &SharedCapturePlan,
        metadata: &HostMetadataFunding,
    ) -> Result<usize, Error>
        ...
        <T::Completion as eredu_core::Completion>::Error: Send + Sync + 'static,
    {
        ...
        PreparedPartitionContiguousSource::local_execution_metadata_bytes::<T>(
            self.native.len(),
            selected.producer_count(),
        )
        .and_then(|bytes| {
            bytes.checked_add(W::metadata_vec_bytes::<
                PartitionCaptureCoordinateProducer<'_>,
            >(selected.producer_count())?)
        })
        .ok_or_else(overflow)
    }
    pub(super) fn execution_metadata_bytes(
        &self,
        source: &SharedCapturePlan,
        rank: usize,
        scalar: Option<WorkspaceFloatingType>,
        ...
        let producers = (0..selected.world_size()).filter_map(|rank| {
            selected
                .rank(rank)
                .filter(|local| local.produces)
                .map(|local| PartitionCaptureCoordinateProducer {
                    rank,
                    coordinates: local.coordinates,
                })
        });
        let selected_transform = &source.admission().plan().selections[self.index].transform;
        const RAW: CaptureTransform = CaptureTransform::Slice;
        let transform = if self.combination == PartitionCaptureCombination::SumF64ToF32
            && !matches!(selected_transform, CaptureTransform::Preview { .. })
        {
            &RAW
        } else {
            selected_transform
        ...
        controls()
            ...
            .and_then(|bytes| {
                bytes.checked_add(
                    W::metadata_vec_bytes::<PartitionCaptureFragmentGeometry<'_>>(
                        self.native.len(),
                    )?,
                )
            })
