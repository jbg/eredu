...
fn borrowed_revalidation_matches_existing_catalog_and_phase_decisions() {
    ...
    for mutation in 0..7 {
        ...
        match mutation {
            ...
            4 => {
                support.points[0].decode =
                    ObservationSupportStatus::Unsupported("unavailable".into())
            }
        ...
        let result = validate_parts(
            &admitted,
            &catalog,
            &support,
            WorkspaceReportMetadata::with_funding(&funding),
        );
...
fn cold_rejection_funds_every_reached_destination_and_retains_original_account() {
    ...
    assert!(
        requests >= 2,
        "comparison controls and typed error destination"
    );
    assert!(leaf::<CaptureRevalidationError>(&error));
    drop(funding);
    assert!(!state.retired.load(Ordering::SeqCst));
    ...
    for refuse in 0..requests {
        ...
        assert!(
            leaf::<HostMetadataFundingError>(&error),
            "refusal {refuse}: {error}"
        );
        assert!(
            !leaf::<CaptureRevalidationError>(&error),
            "unfunded rejection must not escape"
        );
        drop(error);
        drop(funding);
        assert!(state.retired.load(Ordering::SeqCst));
