...
pub(in crate::composition::mlx::session::model_session) struct PreparedResidencyFixture {
...
impl PreparedResidencyFixture {
    ...
    pub fn actual_disk_read_bytes(runtime: &Runtime) -> u64 {
        let workspace = runtime
            ...
            .unwrap()
            .unwrap();
        let mut total = workspace
            .detached_physical_read_bytes(0)
    ...
    #[track_caller]
    pub fn assert_disk_read_progress(
        &self,
        runtime: &Runtime,
        residency: usize,
        before: Option<u64>,
        ...
        match (before, self.disk_read_bytes(runtime, residency)) {
            (Some(before), Some(after)) => {
                ...
                assert!(
                    live > 0 && live <= depth,
                    "retained disk units exceed the selected device window"
                );
    ...
    pub fn load(
        &self,
        residency: usize,
        maximum_positions: Option<usize>,
    ) -> (Runtime, tempfile::TempDir, u64) {
        let before_target = self.pool.fixture_host_charge().unwrap();
        ...
        let target_baseline = self.pool.fixture_host_charge().unwrap();
        let source_birth = target_baseline.checked_sub(before_target).unwrap();
        assert!(
            source_birth > 0,
            "new prepared target must retain its actual process source accounts"
        );
        assert_eq!(
            self.pool.unquoted_owner_count().unwrap(),
            0,
        ...
        assert!(runtime.backend().memory_ledger().same_ledger(&self.pool));
        assert!(
            runtime.session().payload.model.has_published_idle_storage(),
        ...
        crate::backend::submission_recovery::wait_for_retirement(|| {
            ...
            if unquoted != 0 && !reported && started.elapsed().as_secs() >= 9 {
                eprintln!(
                    "prepared residency load at {caller}: residency={residency}, used={}, baseline={}, unquoted={unquoted}",
                    self.pool.fixture_host_charge().unwrap(),
                    self.baseline
                );
                reported = true;
            }
        ...
        if residency != 0 {
            ...
            assert!(
                workspace
                    .visit_native_copy_layouts(|rank, _dtype| {
                        assert!(rank > 0, "fixture weight reads have actual tensor geometry");
                        native_reads += 1;
                        Ok::<_, std::convert::Infallible>(())
                    })
                    .unwrap(),
                "selected source must expose its qualified native read/copy plans"
            );
            assert!(
                native_reads > 0,
                "selected source retains actual native parameter reads"
            );
            let report = runtime.session().residency_report().unwrap().unwrap();
            let tier = if residency == 2 {
                MemoryTier::Disk
            } else {
                MemoryTier::Host
            };
            ...
            assert!(
                pinned
                    .iter()
                    .all(|unit| unit.planned_tier() == MemoryTier::Device
            ...
            if residency == 2 {
                ...
                let read_bytes = Self::actual_disk_read_bytes(&runtime);
                // Each layer catalog borrows a slice of the same real file
                // source. Its diagnostics retain source-wide read counters.
                for layer in &layers {
                    let catalog = &report.unit_sources()[layer.id()];
                    assert_eq!(
                        catalog.backend,
                        eredu_checkpoint::store::WeightStoreBackend::Safetensors
                    );
                    assert_eq!(catalog.physical_read_bytes, read_bytes);
                    assert_eq!(catalog.currently_cached_shards, 0);
                    assert_eq!(catalog.touched_shard_paths, catalog.payload_shard_paths);
                }
                ...
                assert!(
                    layers
                        .iter()
                        .all(|unit| unit.policy() == ResidencyPolicy::Windowed)
                );
            }
        }
...
pub(super) fn target(
    ...
    eredu_core::ExecutionPlanTarget<MlxBackend<'static>>,
    tempfile::TempDir,
) {
    use eredu_core::ResidencyPlan;
    let artifact = if residency == 2 {
        disk::artifact()
    } else {
        host::artifact("llama")
    };
    ...
    let placement = match residency {
        ...
        1 => ResidencyPlan::LayerwiseHost {
            ...
            device_budget_bytes: Some(u64::MAX),
            host_budget_bytes: Some(u64::MAX),
        },
