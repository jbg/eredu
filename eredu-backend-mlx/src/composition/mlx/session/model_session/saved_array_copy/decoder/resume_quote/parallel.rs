...
use crate::backend::nn::workspace::{
    ...
    ResidentNativeRecipe, AddressableSources, MlxAddressableWorkspaceMechanisms,
};
use crate::composition::mlx::model::retain_planning_error;

pub(super) enum SavedWorkspace {
    Resident{context:WorkspaceContext,addressable:Option<AddressableSources>},
...
impl SavedWorkspace {
    pub(super) fn prepare(
        ...
        let addressable=funding.map(|funding|session.payload.model.prepare_addressable_workspace_sources(mechanisms,&session.payload.memory_pool,funding)).transpose()?.flatten();
        match (&session.payload.distributed, funding) {
            (Some(distributed), Some(funding)) => {
                ...
                let result = (|| {
                    ...
                    }.map(Self::Parallel).map_err(Error::Neural)
                })();
            ...
            (None, Some(funding)) => {
                let context=match &addressable {
                    Some(source)=>WorkspaceContext::new_with_metadata_funding(MlxAddressableWorkspaceMechanisms::new(mechanisms,source.clone()),funding.clone()),
                    None=>WorkspaceContext::new_with_metadata_funding(mechanisms,funding.clone()),
                }.map_err(|cause|retain_planning_error(cause,funding.clone()))?;
                Ok(Self::Resident{context,addressable})
            }
            (None, None) => Ok(Self::Resident{context:WorkspaceContext::new(mechanisms),addressable:None}),
        }
    }
    ...
    pub(super) fn addressable(&self)->Option<&AddressableSources>{match self{Self::Resident{addressable,..}=>addressable.as_ref(),Self::Parallel(value)=>value.addressable_sources()}}
...
type QuoteParts=(PreparedTextGenerationWorkspace,
    Option<crate::backend::runtime::execution::generic::LayerwiseWorkspace>,ResidentNativeRecipe);
...
pub(super) fn quote(
    ...
    parallel: &MlxParallelWorkspace,
    capture: Option<(&eredu_runtime::capture::FundedCaptureCheckpoint,
    ...
    let result = (|| {
        context
            .charge_metadata(std::mem::size_of::<(
                Result<QuoteParts, Error>, Option<TextInterventionQuote<'_>>,
        ...
        let layerwise=model.prepared_layerwise_workspace(context)?;
        let mut recorder = parallel.recorder(geometry).map_err(Error::Neural)?;
        if let Some(source) = &layerwise {
            recorder.bind_layerwise_constructor_source(source).map_err(Error::Neural)?;
        }
        ...
        let generation = if let Some((checkpoint, selection)) = capture {
            ...
            let manifest = blueprint.selected().communication_manifest().ok_or_else(unknown)?;
            if setup != distributed.session_identity() || execution != blueprint.execution_identity() {
                return Err(memory(WorkingMemoryError::IdentityMismatch));
            }
            let parameters = layerwise.as_ref().map(crate::composition::mlx::model::NativeLayerwiseParameters);
            model.quote_partitioned_saved_capture(blueprint, geometry, state, context, sampling,
                checkpoint, selection, interventions, &mut recorder, parallel.declaration_source(),
                (loaded.layouts(), manifest.rank()), parameters.as_ref().map(|source| source as
                    &dyn eredu_architectures::prepared_execution::WorkspaceLayerwiseParameters))
        } else { match layerwise.as_ref() {
            Some(source)=>blueprint.quote_partitioned_layerwise_text_with_existing_sampling_and_trace(
                geometry,state,context,sampling,&mut recorder,Some(parallel.declaration_source()),
        ...
        model.erased().bind_layerwise_neural_recipe(
            &runtime.session().payload.memory_pool,&mut recipe,Some(&funding),
        )?;
        if let Some(source)=&layerwise { source.bind_native_host_copies(&mut recipe)?; }
