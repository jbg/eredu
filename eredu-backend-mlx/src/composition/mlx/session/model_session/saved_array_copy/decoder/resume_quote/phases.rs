...
use crate::composition::mlx::session::intervention::{
    PreparedTextInterventions, TextInterventionQuote,
};
use eredu_runtime::working_memory::{RegisteredPreparedWorkspaceStorage, RegisteredWorkspaceCopy};
...
pub(super) struct Inputs {
    ...
    source_layout: eredu_runtime::SharedStateLayout,
    decoder_host_peak: u64,
    sampler_host_peak: u64,
    pending_host_bytes: Option<u64>,
    generic_publication_controls: u64,
    prepared_media: Option<media::PreparedSavedMedia>,
    ...
    source_native: ProjectedNativeStorage,
    ...
    copy_preparation: RegisteredWorkspaceCopy<StorageIdentity>,
    registered_source: WorkingMemoryStorage<StorageIdentity>,
    workspace: parallel::SavedWorkspace,
    prepared_parameters: Option<crate::composition::mlx::model::PreparedSavedParameterSource>,
    parameter_storage:
        Option<eredu_runtime::working_memory::RegisteredWorkspaceStorage<StorageIdentity>>,
}
pub(super) struct Traced {
    generation: PreparedTextGenerationWorkspace,
    layerwise: Option<LayerwiseWorkspace>,
    native_recipe: Option<ResidentNativeRecipe>,
    media_storage: Option<RegisteredPreparedWorkspaceStorage<StorageIdentity>>,
    inputs: Inputs,
...
pub(super) fn prepare(
    ...
    _controller: TextControllerWorkspace<'_>,
    ...
    let geometry = resume_geometry_impl(source, config.clone(), planning_metadata)?;
    ...
    let pending_plan = if geometry.max_output_tokens == 0 {
        None
    } else {
        source
            .sampling
    ...
    let generic_publication_controls = if original {
        0
    } else {
        let prepared = source
            .decoder
            ...
            .prepare_copy_fixed()
            .map_err(|cause| planned_error(cause, planning_metadata))?;
        super::super::resume_prompt::publication_control_bytes(
            &prepared,
            pending_plan.as_ref(),
            binding.key().is_some(),
        )
    ...
    let (copy_preparation, registered_source) = match host {
        ...
        None => {
            ...
            for array in binding
                .key()
                .into_iter()
                .chain(binding.pending().filter(|_| geometry.max_output_tokens > 0))
            {
            ...
            dense
                .visit_operands(&mut |_| input_count = input_count.and_then(|n| n.checked_add(1)))
                .map_err(
                    crate::backend::runtime::cache::state::SnapshotProjectionCause::into_error,
                )?;
            let input_count = input_count.ok_or_else(|| memory(WorkingMemoryError::Overflow))?;
            let mut native = None;
            let mut copy_projection = crate::backend::nn::workspace::OwnedArrayProjection::prepare(
                &mut native,
                &copy_context,
                input_count,
            )
    ...
    let prepared_parameters = if original {
    ...
    let mut parameter_storage = None;
    let prepared_media = if pending_plan.is_none() && geometry.max_output_tokens > 0 {
        Some(media::prepare(
            ...
            geometry,
            &context,
            &prepared_parameters.as_ref().ok_or_else(unknown)?.backings,
        )?)
    } else {
        if original {
            let roots = || {
                prepared_parameters
                    .as_ref()
                    .into_iter()
                    .flat_map(|source| source.backings.roots())
            };
            let count = prepared_parameters
                .as_ref()
                .map_or(0, |source| source.backings.len());
            let completed = prepared_parameters
                .as_ref()
                .map(|source| source.backings.completed_source(context))
                .transpose()
                .map_err(|cause| planned_error(cause, planning_metadata))?
                .flatten();
    ...
    let resume_copy = if original {
        ...
        dense
            .visit_snapshot_operands(&mut |source| {
                if let crate::backend::runtime::cache::state::SnapshotOperand::Array(array) = source
                {
                    if failure.is_none() {
        ...
        Some(
            match &pending_plan {
                Some(pending) => {
                    census.finish_resume_input_on(pending.numerical(), runtime.backend().stream())
                }
    ...
    let pending_host_bytes = pending_plan
        .as_ref()
        .map(|pending| pending.host_plan().initialization_peak_bytes());
    drop(projection);
    Ok(Inputs {
        original,
        geometry,
        ...
        sampler_host_peak,
        pending_host_bytes,
        generic_publication_controls,
        prepared_media,
        resume_copy,
        state: projected.state,
        source_native: projected.source_storage,
        decoder_copy: projected.copy,
        sampling_copy,
        copied_key,
        copied_input,
        random,
...
pub(super) fn trace(
    ...
    let context = inputs.workspace.context();
    context
        .charge_metadata(size_of::<(
            RefCell<Option<PreparedTextInterventions>>,
            Option<TextInterventionQuote<'_>>,
            Option<PreparedTextInterventions>,
        )>())
        .map_err(|cause| planned_error(cause, planning_metadata))?;
    let intervention_rows = RefCell::new(None);
    let interventions = source
        .capture_checkpoint()
        .and_then(|checkpoint| checkpoint.intervention_source())
        .map(|source| TextInterventionQuote {
            source,
            rows: &intervention_rows,
        });
    if !original && interventions.is_some() {
    ...
    let (generation, layerwise, native_recipe, media_storage) = if original {
        let (generation, layerwise, mut recipe, media_storage) = if geometry.max_output_tokens == 0
        {
            ...
            let equations = eredu_runtime::working_memory::quote_inference_workspace_with_context(
                geometry,
                context,
                |_| -> Result<WorkspaceTraceReport, Error> { Err(unknown()) },
            )
            .map_err(|cause| planned_error(cause, planning_metadata))?;
            let input = source
                .sampling
            ...
            let quote_sampling =
                |recorder: &mut dyn eredu_runtime::working_memory::SamplingWorkspaceObserver| {
            ...
            let (sampling, mut recipe) = if let Some(parallel) = inputs.workspace.parallel() {
            ...
            // A completed restore submits no equations, but its selected
            // layerwise source still authenticates the empty operation bank.
            model
                .erased()
                .bind_layerwise_neural_recipe(pool, &mut recipe, planning_metadata)?;
            let layerwise = inputs
                .prepared_parameters
                .take()
                .ok_or_else(unknown)?
                .layerwise;
            ...
            let media = media::quote(
                ...
                inputs.prepared_parameters.take().ok_or_else(unknown)?,
            )?;
            (
                media.generation,
                media.layerwise,
            ...
            let (generation, layerwise, recipe) = parallel::quote(
                runtime,
                geometry,
                &inputs.state,
                sampling,
...
pub(super) fn finish<'a>(
    runtime: &ModelRuntime<MlxBackend<'_>>,
    source: &child::Source<'a, '_>,
    config: TextGenerationConfig,
    controller: TextControllerWorkspace<'_>,
    traced: Traced,
    planning_metadata: Option<&HostMetadataFunding>,
) -> Result<PreparedSavedTextResumeQuote<'a>, Error> {
    let memory = |cause: WorkingMemoryError| planned_error(cause, planning_metadata);
    let unknown = || memory(WorkingMemoryError::UnknownBound);
    let session = runtime.session();
    let model = &session.payload.model;
    ...
    let report_error = |cause: eredu_runtime::working_memory::WorkspaceReportError| {
        if report_metadata.is_checked() {
            Error::Neural(report_metadata.error(cause))
        } else {
    ...
    let state_input = match source
        .sampling
        .pending_media()
        .filter(|_| geometry.max_output_tokens > 0)
    {
        Some(media) => media
            .packet()
            .borrowed_semantics()
    ...
    let state = if geometry.max_output_tokens == 0 {
        state
    } else {
        generation
            .equations
    ...
    let retained = decoder_host_peak
        ...
        .and_then(|n| n.checked_add(generic_publication_controls))
    ...
    let materialization = match layerwise.as_ref() {
        Some(workspace) => workspace.materialization_requirements(report_metadata)?,
        None => Some(zero()?),
    };
    if let (Some(materialization), Some(copy)) = (materialization, &sampling_copy.physical_domains)
    {
        let state_update = report_metadata
            .combine_domain_requirements(
                copy.retained_state.as_ref().ok_or_else(unknown)?,
                copy.state_transient.as_ref().ok_or_else(unknown)?,
                true,
            )
            .map_err(report_error)?;
        outside.physical_domains = Some(eredu_core::DomainExecutionWorkspaceEstimate {
            geometry,
            activations: report_metadata
                .placed_requirements(
                    pool.topology(),
                    pending_host_bytes.unwrap_or(0),
                    pool.host_placement(),
                )
                .map_err(report_error)?,
            attention: zero()?,
            ...
            state_update,
            materialization,
            retained: report_metadata
                .placed_requirements(pool.topology(), retained, pool.host_placement())
                .map_err(report_error)?,
        });
    }
    ...
    let continuation_metadata = original
        ...
        .flatten();
    let paged_source_metadata =
        (original && !source_native.paged_sources().is_empty()).then(|| context.clone());
    Ok(PreparedSavedTextResumeQuote {
        ...
        native_recipe,
        layerwise,
        _decoder_copy: decoder_copy,
        _sampling_copy: sampling_copy,
        _pending_host: pending_host,
        _copied_state: copied_state,
        source_native: Some(source_native),
        _copied_key: copied_key,
        _copied_input: copied_input,
        media_storage,
...
pub(super) fn control_bytes() -> Option<usize> {
    let parts = [
        size_of::<Inputs>() * 2,
        size_of::<Traced>() * 2,
        size_of::<Result<Inputs, Error>>(),
        size_of::<Result<Traced, Error>>(),
