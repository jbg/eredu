...
#[inline(never)]
pub(super) fn dispatch<'s>(
    source: &RoutedUnitCaptureSource<'s, Array>,
    completed: [safemlx::EvaluatedArray<'s>; 5],
    ...
    source_pin: WorkingMemoryStorage<StorageIdentity>,
) -> Result<(), Error> {
    match writer {
        RoutedWriter::Prefill(writer) => prefill(
            source,
            completed,
            bank,
            tokens,
            writer,
            scope,
            segment.ok_or_else(|| error(CaptureCarrierError::Identity))?,
            source_pin,
        ),
        RoutedWriter::Invocation(writer) => {
            invocation(source, completed, bank, tokens, writer, scope, source_pin)
        }
        RoutedWriter::Partition {
            ...
            units,
            ..
        } => partition(source, completed, writer, origins, units, scope, source_pin),
    }
}
#[inline(never)]
fn prefill<'s>(
    ...
    let prepared =
        CompletedRoutedCaptureSource::new(source, completed, bank, tokens).map_err(error)?;
    let transfer = RoutedCaptureTransfer::Prefill(
        writer
...
fn invocation<'s>(
    ...
    let prepared =
        CompletedRoutedCaptureSource::new(source, completed, bank, tokens).map_err(error)?;
    let transfer = RoutedCaptureTransfer::Invocation(
        writer
            .prepare_with_source(scope, source_pin)
            .map_err(error)?,
    );
    prepared.copy_routed(transfer).map_err(error)
}
...
fn partition<'s>(
    ...
    let prepared = CompletedPartitionRoutedCaptureSource::bind_layout(
        source,
        completed,
