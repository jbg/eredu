...
use super::*;
use crate::backend::array_copy::{CompletedRoutedCaptureSource, RoutedCaptureTransfer,
...
use eredu_runtime::working_memory::{CaptureRoutedPrefillWriter, CaptureRoutedBatchWriter};
...
impl RoutedWriter<'_, '_, '_, '_> {
    fn bank_tokens(&self) -> (eredu_core::capture::RoutedUnitGeometry, u64) {
        match self {
            Self::Prefill(w) => (w.fragment().plan().geometry().bank(), w.fragment().source_tokens()),
            Self::Invocation(w) => (w.geometry().bank(), w.geometry().source_shape()[0] as u64),
            Self::Partition { writer, .. } => (writer.source_layout().geometry, writer.invocation_source_tokens()),
        }
    }
    fn validate_native_scope(&self, scope: &WorkingMemoryFundingScope)
        -> Result<(), WorkingMemoryError> {
        match self { Self::Prefill(w) => w.validate_native_scope(scope), Self::Invocation(w) => w.validate_native_scope(scope),
...
impl FundedWork {
    ...
    pub(in crate::composition::mlx::session::model_session::text_funding) fn capture_routed_batch(
        &self, source: &RoutedUnitCaptureSource<'_, Array>,
        writer: CaptureRoutedBatchWriter<'_, '_>, stream: &Stream,
        completion: CaptureCompletion<'_>,
    ) -> Result<(), Error> {
    ...
    pub(in crate::composition::mlx::session::model_session::text_funding) fn capture_partition_routed(
        ...
        stream: &Stream, completion: CaptureCompletion<'_>, prefill: bool,
    ) -> Result<(), Error> {
        self.capture_routed(&source.source, RoutedWriter::Partition {
            writer, origins: source.origins, units: source.unit_coordinates, prefill,
        }, stream, completion)
    }
    fn capture_routed(
        &self, source: &RoutedUnitCaptureSource<'_, Array>,
        writer: RoutedWriter<'_, '_, '_, '_>, stream: &Stream,
        completion: CaptureCompletion<'_>,
    ) -> Result<(), Error> {
        let prefill = matches!(&writer, RoutedWriter::Prefill(_)
            | RoutedWriter::Partition { prefill: true, .. });
        if prefill {
            let mut slot = self.capture.try_borrow_mut().map_err(error)?;
            let capture: &mut CaptureCarrier = slot.as_mut()
            ...
            let segment = capture.segment.as_mut().ok_or_else(|| error(CaptureCarrierError::Identity))?;
            self.capture_routed_sources(source, writer, stream, completion,
                &capture.roots, &capture.publications, Some(segment))?;
            attempt.complete();
            Ok(())
        } else {
            self.capture_routed_sources(source, writer, stream, completion,
                &self.roots, &self.publications, None)
        }
    }
    ...
    fn capture_routed_sources(
        ...
        if let RoutedWriter::Partition { writer, origins, units, .. } = &writer {
            let layout = writer.source_layout();
            let shapes = [source.values.shape(), source.token_indices.shape(), source.selection_indices.shape(),
                source.coefficients.shape(), source.source_groups.shape()];
            let dtypes = [source.values.dtype(), source.token_indices.dtype(), source.selection_indices.dtype(),
                source.coefficients.dtype(), source.source_groups.dtype()];
            CompletedPartitionRoutedCaptureSource::validate_source_layouts(shapes, Some(dtypes), source.token_offset,
                PartitionRoutedCaptureLayout { geometry: layout.geometry, source_tokens: layout.source_tokens,
                    ownership: layout.ownership, origins: *origins, units }).map_err(error)?;
        ...
        let retain_publications = segment.is_some()
            || matches!(completion, CaptureCompletion::Ordinary);
        if matches!(completion, CaptureCompletion::Ordinary) && segment.is_none() {
            publications.try_borrow_mut().map_err(error)?
                .try_reserve_exact(5).map_err(error)?;
        }
        ...
        if let Some(segment) = segment.as_deref_mut() {
            ...
            if let RoutedWriter::Prefill(writer) = &writer {
                segment.validate_routed_prefill_fragment(writer.fragment()).map_err(error)?;
            }
        }
        ...
        let completed = completion.settle_retained(inputs, stream, roots).map_err(error)?;
        writer.validate_native_scope(scope).map_err(error)?;
        ...
        let source_pin = scope
            ...
            .map_err(error)?;
        copy::dispatch(source,completed,bank,tokens,writer,scope,segment,source_pin)?;
        Ok(())
    }
}
...
pub(super) fn control_bytes() -> Option<usize> {
    [
        ...
        size_of::<(&RefCell<Vec<Array>>, &RefCell<Vec<RetainedStoragePublication>>)>(),
        ...
        size_of::<[&[i32]; 5]>(), size_of::<[safemlx::Dtype; 5]>(),
    ]
    .into_iter()
    .try_fold(
