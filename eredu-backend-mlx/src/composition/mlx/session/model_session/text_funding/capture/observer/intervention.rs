...
use eredu_runtime::working_memory::{
    CaptureInterventionClaim, CaptureInterventionEvidenceClaim, CaptureInterventionEvidenceKind,
    ClaimedIntervention, ClaimedInterventionEvidence,
};
use std::mem::size_of;
mod partition;
mod prefill;
...
impl NativeScheduledCapture<'_> {
    fn scheduled_routing_row(
        ...
        match window {
            ...
            None => self.scheduled_intervention_row(phase, prediction),
        }
    }
    pub(super) fn scheduled_routing_usage(
        &self,
        rows: u64,
        claim: &CaptureInterventionClaim<'_>,
        window: Option<eredu_runtime::intervention::InterventionPrefillWindow>,
        ...
        let result = (|| {
            let row = self.scheduled_routing_row(claim, window)?;
            let scope = self
                .work
                .scope
                .try_borrow()
            ...
            row.routing_usage(rows, claim, scope, window)
                .map_err(|cause| Error::Other(Box::new(cause)))
        })();
        result.map_err(|cause| self.scheduled_failure(cause))
    }
    pub(super) fn scheduled_routing_control(
        &self,
        rows: u64,
        claim: &CaptureInterventionClaim<'_>,
        window: Option<eredu_runtime::intervention::InterventionPrefillWindow>,
    ...
    > {
        let result = (|| {
            ...
            let scope = self
                ...
                .map_err(|_| Error::PrefillScopeReentrant)?;
            let scope = scope.as_ref().ok_or(Error::PrefillScopeUnavailable)?;
            row.take_routing_control(rows, claim, scope, window)
                .map_err(|cause| Error::Other(Box::new(cause)))
        })();
        result.map_err(|cause| self.scheduled_failure(cause))
    }
    pub(super) fn scheduled_routing_result(
        &self,
        rows: u64,
        ...
        let result = (|| {
            ...
            let scope = self
                ...
                .map_err(|_| Error::PrefillScopeReentrant)?;
            let scope = scope.as_ref().ok_or(Error::PrefillScopeUnavailable)?;
            row.validate_routing_result(rows, original, effective, claim, scope, window)
                .map_err(|cause| Error::Other(Box::new(cause)))
        })();
        result.map_err(|cause| self.scheduled_failure(cause))
    }
    fn scheduled_intervention_row(
        &self,
        phase: CapturePhase,
        prediction: u64,
    ) -> Result<&PreparedModelInterventions, Error> {
    ...
    fn native_intervention_failure(
        &self,
        cause: FundedCaptureError<NativeFailure>,
    ) -> FundedCaptureError<Error> {
        match cause {
            FundedCaptureError::Backend(cause) => {
                self.scheduled_failure(Error::Other(Box::new(cause)))
            }
            ...
            FundedCaptureError::CandidateFinish(cause) => {
                FundedCaptureError::CandidateFinish(cause)
            }
            FundedCaptureError::TokenScoreFinish(cause) => {
                FundedCaptureError::TokenScoreFinish(cause)
            }
            FundedCaptureError::SummaryFinish(cause) => FundedCaptureError::SummaryFinish(cause),
            FundedCaptureError::HistogramFinish(cause) => {
                FundedCaptureError::HistogramFinish(cause)
            }
    ...
    pub(super) fn scheduled_intervention_usage(
        &self,
        source: &Array,
        claim: &CaptureInterventionClaim<'_>,
    ) -> Result<CaptureUsage, FundedCaptureError<Error>> {
    ...
    pub(super) fn scheduled_intervention_projection_usage(
        &self,
        source: &Array,
        claim: &CaptureInterventionClaim<'_>,
    ...
    pub(super) fn scheduled_intervention(
        &self,
        source: &Array,
        claim: CaptureInterventionClaim<'_>,
        charged: CaptureUsage,
        projection: [CaptureUsage; 2],
    ) -> Result<(Option<Array>, ClaimedIntervention), FundedCaptureError<Error>> {
        let (phase, prediction) = claim.coordinate();
        let row = self
            .scheduled_intervention_row(phase, prediction)
        ...
        claim
            .validate_native_custody(scope)
            .map_err(|cause| self.scheduled_failure(Error::Other(Box::new(cause))))?;
        row.execute_scheduled(
            source,
            claim,
            charged,
            projection,
    ...
    fn check_scheduled_evidence(
        ...
        let result = (|| {
            ...
            claim
                .validate_native_custody(scope)
                .map_err(|cause| Error::Other(Box::new(cause)))?;
            row.check_evidence_scheduled(source, claim, scope, execute)
                .map_err(|cause| Error::Other(Box::new(cause)))
        })();
        result.map_err(|cause| self.scheduled_failure(cause))
    }
    pub(super) fn scheduled_evidence_usage(
        &self,
        source: &Array,
        claim: &CaptureInterventionEvidenceClaim<'_, '_>,
    ...
    pub(super) fn scheduled_evidence<'a>(
        &mut self,
        source: &Array,
...
pub(super) fn control_bytes() -> Option<usize> {
    [
        ...
        size_of::<(
            &NativeScheduledCapture<'_>,
            u64,
            Option<eredu_runtime::RoutingDecision<'_, Array>>,
            eredu_runtime::RoutingDecision<'_, Array>,
            &CaptureInterventionClaim<'_>,
            Option<eredu_runtime::intervention::InterventionPrefillWindow>,
        )>(),
        size_of::<
        ...
        size_of::<CaptureInterventionEvidenceClaim<'_, '_>>(),
