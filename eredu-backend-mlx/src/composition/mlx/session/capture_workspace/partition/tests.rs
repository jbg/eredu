...
fn projected_prefill_quotes_actual_local_sources_and_preserves_raw_additive_reductions() {
    ...
    for transform in [CaptureTransform::Slice,CaptureTransform::Preview{max_elements:5},CaptureTransform::Summary,
        ...
        let slice=ResolvedCaptureSlice{starts:vec![0,1,1],ends:vec![2,3,7],strides:vec![1,1,2],shape:vec![2,2,3]};
        let combination=if sum{PartitionCaptureCombination::SumF64ToF32}else{PartitionCaptureCombination::Disjoint};
        let producer=|rank,range|PartitionCaptureProducer{rank,projection:CaptureContiguousProjectionPlan::prepare(&[2,3,7],&slice,2,range,1).unwrap().construct()};
        ...
        assert_eq!(native.local_shape,if sum{&[2,3,7][..]}else{&[2,3,4][..]});
        ...
        for k in 0..3 {
            context.begin_span();let mut roots=vec![];
            ...
            let (dtype,population)=equation.trace_fragment(k,&value,&context,&mut roots).unwrap();
            assert_eq!(dtype,WorkspaceFloatingType::Float32);
            let report=context.report(&roots).unwrap();assert!(report.unpriced_operations.is_empty());
...
#[cfg(all(target_vendor="apple",feature="metal",not(feature="cuda")))]
fn projected_source(transform:eredu_core::capture::CaptureTransform)->eredu_core::capture::SharedCapturePlan {
    use super::*;
    use eredu_core::*;
        let point=ObservationPoint{path:"block.output".into(),node_id:"block".into(),meaning:"projected activation".into(),
        ...
        let declaration=CapturePlan{schema_version:1,selections:vec![CaptureSelection{id:"projected".into(),path:"block.output".into(),
            schedule:CaptureSchedule{prefill:true,decode:false,..Default::default()},transform:transform.clone(),
            slices:vec![CaptureSlice{axis:"axis1".into(),start:1,end:3,stride:1},CaptureSlice{axis:"axis2".into(),start:1,end:7,stride:2}]}],
...
fn contiguous_cold_observer_preserves_source_scalar_and_local_completion_population(){
    ...
    let selected=MlxCpuMatmulMechanism::select(eredu_nn::CpuMatmulImplementation::Float32Tiles).unwrap();
    let cpu=MlxCpuWorkspaceMechanisms::new(native.allocation(),selected);
    let inference=InferenceGeometry{batch_size:1,cached_positions:2,input_positions:3,max_output_tokens:4,
        prefill_chunk_positions:1,output:OutputDemand::LastPosition};
    for transform in [CaptureTransform::Slice,CaptureTransform::Summary] {for produces in [false,true] {for empty in [false,true] {
        let source=projected_source(transform.clone());let context=WorkspaceContext::new(cpu);
        let range=if empty{0..1}else{3..7};
        let projection=observer::projection(source.admission(),0,2,range,&context).unwrap();
        assert_eq!(projection.fragments().is_empty(),empty);
        let transfers=Cell::new(CaptureNativePopulation::default());let scalars=[Cell::new(None)];let mut roots=vec![];
        let value=WorkspaceTensor::existing(context.layout(&[2,1,if empty{1}else{4}],WorkspaceDtype::Float32).unwrap()
            .with_representation(Some(WorkspaceRepresentation::new(WorkspaceFloatingType::Float32,true))),&context).unwrap();
        context.begin_state_span([&value]).unwrap();
        observer::trace(&source,0,&projection,PartitionCaptureCombination::Disjoint,0,produces,inference,1,1,
            &value,&context,&mut roots,Some(&transfers),Some(&scalars)).unwrap();
        assert_eq!(scalars[0].get(),Some(WorkspaceFloatingType::Float32));
        let population=transfers.get();
        if !produces||empty {
            assert_eq!((population.publications,population.completions,population.retained_roots),(0,1,1));
            assert_eq!(roots.len(),1);assert!(context.report(&roots).unwrap().operations.is_empty());
        }else{
            assert!(population.publications>0);assert!(population.completions>0);
            let report=context.report(&roots).unwrap();assert!(report.unpriced_operations.is_empty());
            assert!(!report.operations.is_empty());
        }
        // Geometry equality cannot substitute for the native scalar witness.
        let opaque=WorkspaceTensor::existing(context.layout(&[2, 1, if empty { 1 } else { 4 }],WorkspaceDtype::Float32).unwrap(),&context).unwrap();
        assert!(observer::trace(&source,0,&projection,PartitionCaptureCombination::Disjoint,0,produces,inference,1,1,
            &opaque,&context,&mut roots,Some(&transfers),Some(&scalars)).is_err());
        let wrong=WorkspaceTensor::existing(context.layout(&[2,2,if empty{1}else{4}],WorkspaceDtype::Float32).unwrap()
            .with_representation(Some(WorkspaceRepresentation::new(WorkspaceFloatingType::Float32,true))),&context).unwrap();
        assert!(observer::trace(&source,0,&projection,PartitionCaptureCombination::Disjoint,0,produces,inference,1,1,
            &wrong,&context,&mut roots,Some(&transfers),Some(&scalars)).is_err());
    }}}
}
