...
use super::*;
use eredu_runtime::capture::partition::{PartitionInvocationCaptureGeometry,PartitionInvocationCaptureKind};

pub(in crate::composition::mlx::session) fn estimate_geometry(source:&PartitionInvocationCaptureGeometry<'_>)
    ->std::result::Result<PartitionCaptureNativeEstimate,CaptureError> {
    let capture=match source.kind() {
        PartitionInvocationCaptureKind::Tensor(geometry)=>super::super::super::bounded_capture::estimate_tensor_geometry(geometry),
        PartitionInvocationCaptureKind::Summary(geometry)=>super::super::super::bounded_capture::estimate_summary(geometry),
        PartitionInvocationCaptureKind::Histogram(geometry)=>super::super::super::bounded_capture::estimate_histogram(geometry),
    }?;
...
impl<'a> PartitionInvocationEquation<'a> {
    pub(super) fn prepare(receipt:&'a PartitionCaptureReceiptPlan,producer:usize,fragment:usize,
        context:&WorkspaceContext)->Result<Self> {
        ...
        // This equation currently counts ordinary callback completions. Explicit
        // retained children require their own nested root/completion source.
        if receipt.context().invocation.is_some(){return Err(metadata.coordinate());}
    ...
    pub(super) fn from_geometry(source:PartitionInvocationCaptureGeometry<'a>,producer:usize,
        context:&WorkspaceContext)->Result<Self> {
        let metadata=Metadata::new(context)?;
        context.charge_metadata(Self::control_bytes().ok_or(eredu_nn::workspace::WorkspaceMetadataError::Overflow)?)?;
    ...
    pub(super) fn native_source(&self,dtype:TensorDtype)->PartitionCaptureFragmentSource<'_> {
        ...
        PartitionCaptureFragmentSource{producer:self.producer,fragment:self.source.fragment_index(),
            ...
            transform:self.source.native_transform(),dtype,estimate:self.estimate}
    }
    ...
    pub(super) fn trace(&self,value:&WorkspaceTensor,context:&WorkspaceContext,
        ...
        let population=match self.source.kind() {
            PartitionInvocationCaptureKind::Tensor(geometry)=>{
                let program=CaptureTensorSelection::from_geometry(geometry).map_err(|cause|metadata.error(cause))?;
            ...
            PartitionInvocationCaptureKind::Summary(geometry)=>{
                ...
                program.validate_workspace_source(value,context).map_err(|cause|metadata.error(cause))?;
                metadata.reserve(roots,1)?;roots.push(value.clone());
                program.trace(value,context,roots).map_err(|cause|metadata.error(cause))?;
                program.population()
            },
    ...
    fn control_bytes()->Option<usize> {
        let parts=[size_of::<Self>()*2,size_of::<Result<Self>>(),size_of::<PartitionInvocationCaptureGeometry<'_>>(),
            size_of::<PartitionCaptureNativeEstimate>()*2,size_of::<CaptureUsage>(),
            size_of::<(&PartitionCaptureReceiptPlan,usize,usize,&WorkspaceContext)>(),
            size_of::<(PartitionInvocationCaptureGeometry<'_>,usize,&WorkspaceContext)>(),
            ...
            size_of::<PreparedCaptureHistogram<'_>>(),PartitionInvocationCaptureGeometry::control_bytes()?];
...
pub(in crate::composition::mlx::session) fn trace_geometry(
    source:PartitionInvocationCaptureGeometry<'_>,producer:usize,
    value:&WorkspaceTensor,context:&WorkspaceContext,roots:&mut Vec<WorkspaceTensor>,
)->Result<CaptureNativePopulation> {
    let parts=[
        size_of::<(PartitionInvocationCaptureGeometry<'_>,usize,&WorkspaceTensor,&WorkspaceContext,&mut Vec<WorkspaceTensor>)>(),
