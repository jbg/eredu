        ...
        for sum in [false, true] {
            ...
            let value = WorkspaceTensor::existing(
                context
                    ...
                    .unwrap()
                    .with_representation(Some(scalar)),
                &context,
            )
            .unwrap();
            let mut roots = vec![];
            let (dtype, population) = equation.trace(&value, &context, &mut roots).unwrap();
            ...
            let value2 = WorkspaceTensor::existing(
                context2
                    .layout(&[2, 1, if sum { 7 } else { 4 }], WorkspaceDtype::Float32)
                    .unwrap()
                    .with_representation(Some(scalar)),
            ...
            let mut roots2 = vec![];
            super::super::observer::trace_invocation(
                &source,
                0,
                CapturePhase::Decode,
                2,
            ...
            let observed = transfers.get();
            assert_eq!(
                (
                    observed.publications,
                    observed.completions,
                    observed.controls,
                    observed.retained_roots
                ),
                (
            ...
            .unwrap();
            assert!(equation.trace(&opaque, &context, &mut roots).is_err());
            assert_eq!(roots.len(), before);
            let wrong = WorkspaceTensor::existing(
                context
                    .layout(&[2, 2, if sum { 7 } else { 4 }], WorkspaceDtype::Float32)
                    .unwrap()
                    .with_representation(Some(scalar)),
                &context,
            )
            .unwrap();
            assert!(equation.trace(&wrong, &context, &mut roots).is_err());
            assert_eq!(roots.len(), before);
            assert!(
...
fn source_axes(
    ...
    let point = ObservationPoint {
        ...
        node_id: "block".into(),
        meaning: "projected decode activation".into(),
...
#[cfg(all(target_vendor = "apple", feature = "metal", not(feature = "cuda")))]
#[test]
fn projected_explicit_invocations_preserve_two_three_one_physical_axes() {
    use super::*;
    use crate::backend::nn::workspace::{
        MlxCpuMatmulMechanism, MlxCpuWorkspaceMechanisms, MlxMetalWorkspaceMechanisms,
    };
    ...
    for transform in [
        ...
        let source = source_axes(transform, true);
        for (phase, rows) in [
            ...
            let context = WorkspaceContext::new(cpu);
            let axes = CaptureInvocationShape {
                batch: 1,
                sequence: rows,
                context: None,
            };
            let projection = super::super::observer::projection_at(
                source.admission(),
                0,
                phase,
                4,
            ...
            let make = |rows| {
                WorkspaceTensor::existing(
                    context
                        .layout(&[2, rows, 4], WorkspaceDtype::Float32)
                        .unwrap()
                        .with_representation(Some(scalar)),
                    &context,
                )
            ...
            .unwrap();
            assert_eq!(scalars[0].get(), Some(WorkspaceFloatingType::Float32));
            assert!(transfers.get().publications > 0);
            assert!(transfers.get().completions > 0);
            let before = roots.len();
            assert!(
                super::super::observer::trace_invocation(
                    &source,
