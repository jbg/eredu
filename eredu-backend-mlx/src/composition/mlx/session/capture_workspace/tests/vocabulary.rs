...
fn complete_vocabulary_cold_quotes_preserve_source_and_only_the_actual_reducer() {
    ...
    for scores in [false, true] {
        let point = ObservationPoint {
            ...
            node_id: "output".into(),
            meaning: "actual raw logits".into(),
        ...
        for local in [true, false] {
            ...
            let value = WorkspaceTensor::existing(
                context
                    .layout(&[1, 1, 4], WorkspaceDtype::Float32)
                    .unwrap()
            ...
            context.begin_span();
            observer
                .observe_value_with_origin(MODEL_LOGITS_OBSERVATION_PATH, &value, local)
                .unwrap();
            ...
            if local {
                ...
                local_usage = Some(observer.ledger.total());
            } else {
                assert_eq!(observer.ledger.total(), local_usage.unwrap());
                let native = transfers.get();
                assert_eq!(
                    (
                        native.publications,
                        native.completions,
                        native.controls,
                        native.retained_roots
                    ),
                    (0, 0, 0, 0)
                );
