...
fn routed_metadata(context: &WorkspaceContext) -> Result<Metadata> {
    ...
    context.charge_metadata(std::mem::size_of::<(
        ...
        (
            [&[i32]; 5],
            Option<[safemlx::Dtype; 5]>,
            u64,
            eredu_core::capture::RoutedUnitGeometry,
            u64,
        ),
...
struct RoutedSourceRepresentation {
    index: usize,
}

fn native_dtypes(
    values: [&WorkspaceTensor; 5],
    ...
    context.charge_metadata(std::mem::size_of::<(
        ...
        [safemlx::Dtype; 5],
        usize,
        RoutedSourceRepresentation,
        Option<safemlx::Dtype>,
    )>())?;
...
fn routed_dtypes(
    ...
    for (index, value) in values.iter().enumerate() {
        ...
        if !valid {
            return Err(context.metadata_source(RoutedLogicalDtype { index, dtype }));
        }
    }
    if native {
        native_dtypes(values, context).map(Some)
    } else {
        Ok(None)
    }
}
...
fn validate_routed_layouts(
    ...
    token_offset: u64,
    bank: eredu_core::capture::RoutedUnitGeometry,
    source_tokens: u64,
) -> std::result::Result<(usize, u64), crate::backend::array_copy::CaptureTensorNativeError> {
    match dtypes {
...
impl CaptureWorkspaceObserver<'_> {
    ...
    fn routed_batch(
        ...
        if !self.routed_active
            || self.routed_addressable_rows.is_some()
            || (self.placement.is_none()
                && (batch.origins.is_some() || batch.unit_coordinates.is_some()))
        {
        ...
        self.context
            .charge_metadata(std::mem::size_of::<eredu_runtime::prefill::PrefillChunk>())?;
        ...
        for index in 0..self.progress.len() {
            ...
            let (_, tokens) = if let Some(placement) = self.placement {
                partition::validate(
                    ...
                    self.routed_native_rows,
                    &self.context,
                )?
            } else {
                validate_routed_layouts(
                    shapes,
                    dtypes,
                    source.token_offset,
                    plan.geometry().bank(),
...
impl RoutedUnitObserver<WorkspaceTensor> for CaptureWorkspaceObserver<'_> {
    fn observe_addressable_source(
        &mut self,
        source: eredu_nn::workspace::WorkspaceAddressableObservationView<'_>,
    ) -> Result<eredu_nn::workspace::WorkspaceAddressableObservationSource> {
        self.routed_addressable_source(source)
    }

    fn observe_region_source(
        &mut self,
        source: eredu_nn::workspace::WorkspaceExpertObservationView<'_>,
    ...
    fn begin_invocation(
            ...
            if let Some(first) = self.routed_selection {
                for index in 0..self.statuses.len() {
                    if self.same_routed_selection(first, index)
                        && !matches!(
                            self.statuses[index],
                            CaptureRecordStatus::Missing | CaptureRecordStatus::Skipped
                        )
                    {
                        return Err(metadata.coordinate());
                    }
        ...
        self.routed_native_rows = if let Some(placement) = self.placement {
            ...
            let source_tokens = if let Some(chunk) = self.chunk.as_ref() {
                self.geometry
                    .batch_size
                    .checked_mul(chunk.input.end - chunk.input.start)
                    .ok_or(eredu_nn::workspace::WorkspaceMetadataError::Overflow)?
            } else {
                self.policy()?
                    .routed_geometry(first)
                    .map_err(|e| metadata.error(e))?
                    .source_shape()[0] as u64
            };
            Some(partition::begin(
                self.source,
                placement,
                first,
                input,
    ...
    fn finish_invocation(&mut self, success: bool) -> Result<()> {
        ...
        for index in 0..self.progress.len() {
                ...
                if decision == CapturePrefillHookDecision::First {
                    let usage =
                        super::super::bounded_capture::estimate_routed_geometry(plan.geometry())
                            .map_err(|e| metadata.error(e))?;
                    if row
                        .reserve_first(&mut self.progress[index], &mut self.ledger, usage)
                        .map_err(|e| metadata.error(e))?
                        .is_some()
                    {
                        continue;
            ...
            let effective = self.source.admission().points()[index].position
                == eredu_core::ObservationPosition::AfterIntervention;
            if self
                .routed_addressable_rows
                .unwrap_or(self.routed_tokens[usize::from(effective)])
    ...
    fn intervene(
        ...
        let (output, population) = if let Some(edits) = self.text_interventions {
            edits
                .try_borrow_mut()
                .map_err(|cause| metadata.error(cause))?
                .as_mut()
...
impl CaptureWorkspaceObserver<'_> {
    ...
    fn begin_partition_routed_input(&mut self, input: &WorkspaceTensor) -> Result<()> {
        ...
        for _ in 0..count {
            record_replica(self.transfers, &self.context, &mut self.roots, input)?;
        }
        Ok(())
    }
    ...
    fn prepare_routed_input(&mut self) -> Result<usize> {
        ...
        if let Some(chunk) = self.chunk.clone() {
            ...
            let policy = CaptureObservationStep::with_invocation(
                ...
                phase,
                prediction,
                self.invocation.map(|i| i.shape),
            )
            .and_then(|p| p.with_window(self.invocation.and_then(|i| i.window)))
            .map_err(|e| metadata.error(e))?;
            for index in 0..self.statuses.len() {
                if !self.same_routed_selection(first, index)
    ...
    fn retain_selected_routed_source(
        &mut self,
        index: usize,
        source: &RoutedUnitCaptureSource<'_, WorkspaceTensor>,
        geometry: &CaptureRoutedUnitsGeometry<'_>,
    ) -> Result<()> {
        let Some(placement) = self.placement else {
            return self.retain_routed_source(index, source);
        };
        ...
        if copies == 0 {
            ...
            for _ in 0..copies {
                ...
                record_population(
                    self.transfers,
                    &self.context,
                    CaptureNativePopulation::routed_partition()
                        .ok_or(eredu_nn::workspace::WorkspaceMetadataError::Overflow)?,
                )?;
            }
        }
    ...
    fn routed_invocation_batch(
        ...
        .and_then(|policy| {
            policy.with_window(self.invocation.and_then(|invocation| invocation.window))
        })
        .map_err(|cause| metadata.error(cause))?;
        let source = batch
            .capture_source()
    ...
    fn finish_routed_ordinary_invocation(&mut self, success: bool, first: usize) -> Result<()> {
        ...
        for index in 0..self.statuses.len() {
            ...
            let geometry = policy
                ...
                .map_err(|cause| metadata.error(cause))?;
            if self.routed_native_rows == Some(0)
                && self.statuses[index] == CaptureRecordStatus::Missing
            {
                self.statuses[index] = CaptureRecordStatus::Consumed;
                if let Some(usage) = policy
                    .window_metadata_usage(index)
            ...
            if self.statuses[index] != CaptureRecordStatus::Consumed
                || self
                    ...
                    != self
                        .routed_native_rows
                        .unwrap_or(geometry.source_shape()[0] as u64)
            {
                return Err(metadata.coordinate());
            }
        }
