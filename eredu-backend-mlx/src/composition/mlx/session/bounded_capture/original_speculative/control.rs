...
use eredu_core::{
    intervention::PreparedInterventionPlanCopy, speculative::AdmittedSpeculativeActivations,
    HostPreparationAuthority,
};
use eredu_runtime::{intervention::StaticInterventionPreflight, working_memory::MemoryLedger};
...
impl Source {
    pub(super) fn prepare(sources: &OriginalSpeculativeNumericalSources) -> Result<Self, Error> {
        ...
        Ok(Self {
            declaration: sources.activation_control_declaration()?,
            pool: sources.pool().clone(),
    ...
    > {
        let frames = [
            ...
            size_of::<
                Result<
                    StaticInterventionPreflight,
                    eredu_runtime::intervention::StaticInterventionScratchError,
                >,
            >(),
            ...
            PreparedInterventionPlanCopy::inspection_control_bytes()
                .ok_or(Error::WorkspacePlanning(HostMetadataFundingError::Overflow))?,
            OriginalInterventionDeclaration::activation_validation_control_bytes()
                .ok_or(Error::WorkspacePlanning(HostMetadataFundingError::Overflow))?,
            NativeInterventionEstimator::prepared_preflight_control_bytes()
                .ok_or(Error::WorkspacePlanning(HostMetadataFundingError::Overflow))?,
            StaticInterventionPreflight::required_bytes()
                .ok_or(Error::WorkspacePlanning(HostMetadataFundingError::Overflow))?,
        ...
        let source = if plan.interventions().is_empty() {
            None
        } else {
        ...
        let result = match &source {
            Some(source) => scratch.run_invocation_with_source(
                state.source().plan().admission(),
                source.plan(),
                &NativeInterventionEstimator,
                inherited,
            ),
            None => scratch.run_invocation_without_evidence(
                state.source().plan().admission(),
                plan.interventions(),
                &NativeInterventionEstimator,
                inherited,
