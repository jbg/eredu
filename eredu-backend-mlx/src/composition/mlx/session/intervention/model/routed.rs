...
use super::super::static_activation::{PreparedSparseActivation, SparseScalarEnvelope};
use super::*;
use crate::backend::array_copy::CompletedRoutedCaptureSource;
use eredu_nn::workspace::WorkspaceDtype;
use eredu_runtime::{
    intervention::{routed_intervention_full_component_count, PreparedRoutedInterventionRows},
    working_memory::RoutedInterventionBatch,
    RoutedUnitBatch, RoutedUnitInvocation,
};
...
pub(super) fn mechanism_facts() -> InterventionMechanismFacts<'static> {
    InterventionMechanismFacts {
        ...
        dtypes: &[InterventionDtype::Float32],
        score_stages: &[],
    }
}
pub(super) fn supports(action: &InterventionAction) -> bool {
    ...
    facts.operations.contains(&action.kind())
        && action
            .dtype()
            .is_some_and(|dtype| facts.dtypes.contains(&dtype))
}
struct Batch {
    range: [u64; 2],
    shape: [i32; 2],
...
impl Edit {
    pub(super) fn complete(&self) -> bool {
        self.finished && self.next == self.tokens
    }
}
impl PreparedModelInterventions {
            ...
            .position(|(index, point)| {
                !matches!(self.rows[index], Row::Inactive)
                    && point
                        .routed_units
                        .as_ref()
                        .is_some_and(|point| point.routing == path)
            })
    }
    fn same_routed(&self, first: usize, index: usize) -> bool {
        ...
        points
            .get(first)
            .and_then(|p| p.routed_units.as_ref())
            .zip(points.get(index).and_then(|p| p.routed_units.as_ref()))
    ...
    pub(crate) fn begin_routed(
        ...
        if !self.begun
            || self.sealed
            || input.origins.is_some()
            || input.unit_coordinates.is_some()
            || input.input.shape().split_last().and_then(|(_, axes)| {
                axes.iter()
                    .try_fold(1u64, |n, x| n.checked_mul(u64::try_from(*x).ok()?))
            }) != Some(tokens)
        {
        ...
        for index in 0..self.rows.len() {
            ...
            let components = bank
                .experts
                .checked_mul(bank.units_per_expert)
                .ok_or(WorkspaceMetadataError::Overflow)?;
            ...
            let logical_origin = window.map_or(0, |window| window.start);
            let logical_end = logical_origin
                .checked_add(tokens)
                .ok_or(WorkspaceMetadataError::Overflow)?;
            ...
            self.rows[index] = Row::Routed(Edit {
                bank,
                tokens,
                logical_origin,
                selected: slice,
    ...
    pub(crate) fn trace_routed(
        ...
        for index in 0..self.rows.len() {
            ...
            let (rows, chunk) = CompletedRoutedCaptureSource::validate_geometry(
                values.map(|v| v.shape()),
                source.token_offset,
                edit.bank,
                edit.tokens,
            )
            ...
            let shape = [
                i32::try_from(rows).map_err(|cause| context.metadata_source(cause))?,
                i32::try_from(edit.bank.units_per_expert)
                    .map_err(|cause| context.metadata_source(cause))?,
            ];
            let action = &self.source.plan().admission().plan().operations[index].action;
            let logical_range =
                [source.token_offset, end].map(|token| token.checked_add(edit.logical_origin));
            let logical_range = [
                logical_range[0].ok_or(WorkspaceMetadataError::Overflow)?,
                logical_range[1].ok_or(WorkspaceMetadataError::Overflow)?,
            ];
            let count =
                routed_intervention_full_component_count(edit.bank, &edit.selected, logical_range)
                    .map_err(|cause| context.metadata_source(cause))?
            ...
            let bytes = PreparedRoutedInterventionRows::required_bytes(&self.source, index, rows)
                .and_then(|n| {
                    n.checked_add(WorkspaceContext::metadata_vec_bytes::<i32>(
                        envelope.index_count(),
                    )?)
                })
            ...
            let output = envelope
                .trace_bound(input, context, retained)
                .map_err(|cause| context.metadata_source(cause))?;
            let controls = native
                .controls
                .checked_add(native.host_bytes)
                .and_then(|n| {
                    n.checked_add(CompletedRoutedCaptureSource::intervention_read_control_bytes()?)
                })
            ...
            let current = CaptureNativePopulation {
                publications: 0,
                completions: native
                    .completions
                    .checked_add(5)
                    .ok_or(WorkspaceMetadataError::Overflow)?,
                retained_roots: native
                    .retained_roots
                    .checked_add(5)
                    .ok_or(WorkspaceMetadataError::Overflow)?,
                controls,
            ...
            edit.batches.push(Batch {
                range: [source.token_offset, end],
                shape,
                envelope,
                funding,
            });
    ...
    fn checked_routed(
        ...
        scheduled: Option<eredu_runtime::intervention::InterventionPrefillWindow>,
    ) -> Result<(&Edit, &Batch), Failure> {
        ...
        if !self.sealed
            ...
            || self.scheduled_span != scheduled
        {
            return Err(Failure::ClaimMismatch);
        }
        let Some(Row::Routed(edit)) = self.rows.get(claim.index()) else {
            return Err(Failure::ClaimMismatch);
        };
        if !edit.complete() {
            return Err(Failure::ClaimMismatch);
        ...
        let (rows, chunk) =
            CompletedRoutedCaptureSource::validate_borrowed(source, edit.bank, edit.tokens)?;
        let end = source
            .token_offset
            .checked_add(chunk)
        ...
        let batch = edit
            .batches
            .iter()
            .find(|batch| batch.range == [source.token_offset, end])
            .ok_or(Failure::ClaimMismatch)?;
        if usize::try_from(batch.shape[0]).ok() != Some(rows) {
            return Err(Failure::ShapeMismatch);
        }
        Ok((edit, batch))
    }
    pub(crate) fn routed_range(
        &self,
        source: &RoutedUnitCaptureSource<'_, Array>,
        claim: &CaptureInterventionClaim<'_>,
        custody: &OriginalSpeculativeBudgetCustody,
    ) -> Result<[u64; 2], Failure> {
        self.checked_routed(source, claim, NativeCustody::Model(custody), None)
            .map(|(_, batch)| batch.range)
    }
    pub(crate) fn routed_usage(
        &self,
        source: &RoutedUnitCaptureSource<'_, Array>,
        claim: &CaptureInterventionClaim<'_>,
        custody: &OriginalSpeculativeBudgetCustody,
    ...
    pub(crate) fn routed_range_scheduled(
        &self,
        source: &RoutedUnitCaptureSource<'_, Array>,
        claim: &CaptureInterventionClaim<'_>,
        ...
        self.checked_routed(source, claim, NativeCustody::Scheduled(scope), span)
            .map(|(_, batch)| batch.range)
    }
    pub(crate) fn routed_usage_scheduled(
        &self,
        source: &RoutedUnitCaptureSource<'_, Array>,
        claim: &CaptureInterventionClaim<'_>,
        scope: &WorkingMemoryFundingScope,
    ...
    pub(crate) fn retained_routed_usage(&self, index: usize) -> Result<CaptureUsage, Failure> {
        match self.rows.get(index) {
            Some(Row::Routed(edit)) if self.sealed && edit.complete() => Ok(edit.usage),
            Some(Row::Inactive) => Ok(CaptureUsage::default()),
            _ => Err(Failure::ClaimMismatch),
        }
    }
    pub(crate) fn execute_routed_scheduled(
        &self,
        source: &RoutedUnitCaptureSource<'_, Array>,
        batch: RoutedInterventionBatch<'_, '_>,
        stream: &Stream,
    ...
    fn execute_routed_with(
        ...
        let span = batch.prefill_window();
        let (edit, selected) = self
            .checked_routed(source, batch.claim(), custody, span)
        ...
        let expected = match span {
            Some(span) => (
                ...
                [
                    span.range()[0]
                        .checked_add(selected.range[0])
                        .ok_or(FundedCaptureError::Backend(Failure::GeometryOverflow))?,
                    span.range()[0]
                        .checked_add(selected.range[1])
                        .ok_or(FundedCaptureError::Backend(Failure::GeometryOverflow))?,
                ],
            ),
            None => (edit.tokens, selected.range),
        };
        if batch.source_chunk() != expected {
            return Err(FundedCaptureError::Backend(Failure::ClaimMismatch));
        }
        let error = |cause| {
            FundedCaptureError::Backend(Failure::Workspace(selected.funding.metadata_source(cause)))
        };
        let claim = batch.claim();
        let mut rows = match span {
            Some(span) => PreparedRoutedInterventionRows::prepare_scheduled_prefill(
                &self.source,
                claim.index(),
                span,
                selected.range,
                selected.shape[0] as usize,
                selected.funding.clone(),
            ),
            None => PreparedRoutedInterventionRows::prepare(
                &self.source,
                claim.index(),
                self.phase,
                self.prediction,
        ...
        CompletedRoutedCaptureSource::settle_intervention_rows(
            source, &mut rows, stream, roots, observer,
        )
        .map_err(|cause| error(NativeFailure::Read(cause)))?;
        let lowered = rows
            .finish()
            .map_err(|cause| error(NativeFailure::Lowering(cause)))?;
        let dtype = self.source.plan().admission().plan().operations[claim.index()]
            .action
            .dtype()
            .ok_or_else(|| error(NativeFailure::Native(Failure::ClaimMismatch)))?;
        let prepared = PreparedSparseActivation::prepare(
            lowered,
            selected.shape,
            dtype,
            selected.funding.clone(),
        ...
        selected
            .envelope
            .validate_refinement(&prepared)
            .map_err(|cause| error(NativeFailure::Native(cause)))?;
        let (output, prepared) = prepared
            .execute(source.values, stream, observer, roots)
            .map_err(|cause| error(NativeFailure::Activation(cause)))?;
        batch
            .finish(prepared.lowered())
...
enum SourceFailure {
    #[error(
        "routed intervention requires a qualified source for its data-dependent selected index count"
    )]
    SelectedCount,
...
enum NativeFailure {
    #[error(transparent)]
    Lowering(#[from] eredu_runtime::intervention::PreparedRoutedInterventionError),
    #[error(transparent)]
    Read(#[from] crate::backend::array_copy::RoutedInterventionReadError),
    #[error(transparent)]
    Activation(#[from] super::super::SparseActivationFailure),
    #[error(transparent)]
    Native(#[from] Failure),
}
fn native_error_controls() -> Option<usize> {
    eredu_core::BackendFailure::source_retention_peak_bytes::<NativeFailure>()?
        .checked_mul(4)?
        .checked_add(size_of::<NativeFailure>())?
        .checked_add(size_of::<FundedCaptureError<Failure>>())
}
fn prepaid(
    bytes: usize,
    context: &WorkspaceContext,
) -> Result<HostMetadataFunding, eredu_nn::Error> {
    let parent = context
        .metadata_funding()
    ...
    eredu_core::HostMetadataFunding::from_prepaid(
        limit,
        eredu_core::HostPreparationAuthority::retain(parent),
    )
    .map(HostMetadataFunding::from)
...
pub(super) fn control_bytes() -> Option<usize> {
    let frames = [
        size_of::<Edit>(),
        size_of::<Batch>(),
        size_of::<Option<Batch>>(),
        size_of::<[RoutedUnitGeometry; 2]>(),
        size_of::<RoutedUnitBatch<'_, WorkspaceTensor>>(),
        size_of::<RoutedUnitInvocation<'_, WorkspaceTensor>>(),
        size_of::<RoutedInterventionBatch<'_, '_>>(),
        size_of::<PreparedRoutedInterventionRows>(),
        ...
        size_of::<[&WorkspaceTensor; 5]>(),
        size_of::<[u64; 2]>() * 3,
        size_of::<[Option<u64>; 2]>(),
        size_of::<Option<usize>>(),
        size_of::<
            Result<Option<usize>, eredu_runtime::intervention::RoutedInterventionLoweringError>,
        >(),
        ...
        SparseScalarEnvelope::control_bytes()?,
        native_error_controls()?,
