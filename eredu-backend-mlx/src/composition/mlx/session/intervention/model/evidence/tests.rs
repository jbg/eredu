...
use eredu_runtime::working_memory::{
    InferenceExecutionIdentity, MemoryLedger, OriginalInterventionSource,
};
fn sources(
    pool: &MemoryLedger,
    invocation: bool,
) -> (OriginalInterventionSource, AdmittedCapturePlan) {
...
fn scheduled_evidence_accounts_for_each_source_publication_and_completion() {
    ...
    for invocation in [true, false] {
        ...
        let mut model = if invocation {
            ...
            PreparedModelInterventions::prepare_scheduled_with_evidence(
                &source,
                &[true, true],
                CapturePhase::Prefill,
                0,
        ...
        let input = WorkspaceTensor::existing(
            context
                .layout(&[5, 2], eredu_nn::workspace::WorkspaceDtype::Float32)
                .unwrap(),
            &context,
        )
        .unwrap();
        ...
        drop((
            report, roots, output, input, model, context, source, capture,
        ));
        assert_eq!(pool.fixture_host_charge().unwrap(), 0);
    }
