...
mod remote;
#[derive(Clone, Copy)]
enum NativeCustody<'a> {
    Model(&'a OriginalSpeculativeBudgetCustody),
    Scheduled(&'a WorkingMemoryFundingScope),
}
impl NativeCustody<'_> {
    fn validate(self, claim: &CaptureInterventionClaim<'_>) -> Result<(), eredu_runtime::working_memory::WorkingMemoryError> {
        match self {
...
impl PreparedModelInterventions {
    ...
    pub(crate) fn prepare(
        ...
        Self::prepare_with_evidence(source,selected,phase,prediction,invocation,window,None,context)
    }
    pub(crate) fn prepare_with_evidence(
        source:&OriginalInterventionSource,selected:&[bool],phase:CapturePhase,prediction:u64,
        invocation:CaptureInvocationShape,window:Option<CaptureInvocationWindow>,
        evidence_skips:Option<&[[Option<CaptureSkipReason>;2]]>,context:&WorkspaceContext,
    )->Result<Self,eredu_nn::Error> {
        Self::prepare_selected(source, selected, phase, prediction, Some(invocation), window, evidence_skips, context)
    }
    ...
    pub(crate) fn prepare_scheduled(
        source: &OriginalInterventionSource, selected: &[bool], phase: CapturePhase,
        prediction: u64, context: &WorkspaceContext,
    ) -> Result<Self, eredu_nn::Error> {
    ...
    pub(crate) fn prepare_scheduled_with_evidence(
        source: &OriginalInterventionSource, selected: &[bool], phase: CapturePhase,
        prediction: u64, evidence_skips: Option<&[[Option<CaptureSkipReason>; 2]]>, context: &WorkspaceContext,
    ) -> Result<Self, eredu_nn::Error> {
        Self::prepare_selected(source, selected, phase, prediction, None, None, evidence_skips, context)
    }
    fn prepare_selected(
        source: &OriginalInterventionSource, selected: &[bool], phase: CapturePhase,
        prediction: u64, invocation: Option<CaptureInvocationShape>, window: Option<CaptureInvocationWindow>,
        evidence_skips: Option<&[[Option<CaptureSkipReason>; 2]]>, context: &WorkspaceContext,
    ) -> Result<Self, eredu_nn::Error> {
        ...
        if selected.len() != plan.points().len() || selected.len() != plan.plan().operations.len()
        ...
        match (plan.invocation_bounds(), invocation) {
            (Some(bounds), Some(invocation)) => bounds.validate_fixed(invocation, prediction)
                .map_err(|cause| context.metadata_source(cause))?,
            (None, None) if window.is_none() => (),
            _ => return Err(context.metadata_source(CaptureProtocolError::Invocation)),
        }
        if let Some(window) = window {
            window.validate_fixed(invocation.ok_or_else(|| context.metadata_source(CaptureProtocolError::Invocation))?)
                .map_err(|cause| context.metadata_source(cause))?;
        }
            ...
            .zip(selected).enumerate()
        {
            if !(point.stage == InterventionStage::Activation
                ...
                || (point.routed_units.is_some() && (invocation.is_none()
                    || operation.evidence != InterventionEvidence::None
                    || !routed::supports(&operation.action)))
                || operation.action.dtype().is_none()
                || !matches!((&operation.evidence, source.plan().evidence(index)),
                    (InterventionEvidence::None,None) | (InterventionEvidence::Preview{..}|InterventionEvidence::Summary,Some(_)))
                || source.plan().evidence(index).is_some_and(|companion|companion.operation()!=index || companion.geometry_source().plan().selections.len()!=2 || evidence_skips.is_none())
            {
        ...
        let evidence_skips=evidence_skips.map(|rows|->Result<_,eredu_nn::Error>{
            let mut copied=context.metadata_vec(rows.len())?;copied.extend_from_slice(rows);Ok(copied)
        }).transpose()?;
        Ok(Self {
            rows,
            evidence_skips,
    ...
    pub(crate) fn begin(
        ...
        self.source.reserve_capture_metadata(ledger).map_err(|cause|context.metadata_source(cause))?;
        Ok(())
    }
    ...
    pub(crate) fn trace(
            ...
            plan.plan().operations.iter().zip(plan.points()).enumerate()
        {
            ...
            if self.partition.as_ref().is_some_and(|binding| partition::remote(binding,index)) {continue;}
            ...
            if self.partition.is_some() {
                let (output, native)=partition::trace(self.partition.as_mut().expect("selected partition"),index,input,context,retained)?;
                self.rows[index]=Row::PartitionReady;
                population=population.checked_add(native).ok_or(WorkspaceMetadataError::Overflow)?;
            ...
            let metadata = if window.is_some() {
                let usage = eredu_runtime::intervention::intervention_window_metadata(operation, point)
                    .map_err(|cause| context.metadata_source(cause))?;
            ...
            let dtype = operation.action.dtype().ok_or_else(|| context.metadata_source(Failure::ShapeMismatch))?;
            PreparedStaticActivation::validate_workspace_source(input,dtype).map_err(|cause|context.metadata_source(cause))?;
            let geometry = window::prepare(plan, index, self.phase, self.prediction, physical,
                window, span, &shape, dtype, metadata, context, ledger)?;
            let action = geometry.payload.as_ref().and_then(|payload| payload.projected_action()).unwrap_or(&operation.action);
            let mut usage = CaptureUsage::default();
            let mut native = CaptureNativePopulation {
                controls: Self::execution_control_bytes().ok_or(WorkspaceMetadataError::Overflow)?,
                ..Default::default()
            };
            if span.is_some() {
                native.controls=native.controls.checked_add(prefill::execution_control_bytes().ok_or(WorkspaceMetadataError::Overflow)?)
                    .ok_or(WorkspaceMetadataError::Overflow)?;
            }
            let program = if geometry.overlap {
                let program = PreparedStaticActivation::new(action, &geometry.slice, &shape, dtype)
                    .map_err(|cause| context.metadata_source(cause))?;
                let actual = program.population().map_err(|cause| context.metadata_source(cause))?;
                usage = NativeInterventionEstimator.activation_usage(&shape, &geometry.slice, action)
                    .map_err(|cause| context.metadata_source(cause))?;
                ...
                native.controls = native.controls.checked_add(actual.controls)
                    .and_then(|n| n.checked_add(actual.host_bytes)).ok_or(WorkspaceMetadataError::Overflow)?;
                Some(program)
            } else { None };
            ...
            let (before,before_population)=evidence::trace(self,index,InterventionEvidenceSide::Before,input,context,ledger,retained)?;
            let output=program.map(|program|program.trace(input,context,retained).map_err(|cause|context.metadata_source(cause))).transpose()?;
            let (after,after_population)=evidence::trace(self,index,InterventionEvidenceSide::After,output.as_ref().unwrap_or(input),context,ledger,retained)?;
            let next=population.checked_add(native).and_then(|p|p.checked_add(before_population)).and_then(|p|p.checked_add(after_population)).ok_or(WorkspaceMetadataError::Overflow)?;
            ...
            population = next;
            if let Some(output) = output { effective = Some(output); }
        }
    ...
    pub(crate) fn finish(&mut self, context: &WorkspaceContext) -> Result<(), eredu_nn::Error> {
        if !self.begun || self.sealed || self.rows.iter().any(|row| matches!(row, Row::Pending)
    ...
    fn checked<'a>(
        ...
        Ok((
            edit,
            if edit.overlap {
                let action = edit.payload.as_ref().and_then(|payload| payload.projected_action()).unwrap_or(action);
                Some(PreparedStaticActivation::new(action, &edit.slice, &edit.shape, edit.dtype)?)
            } else { None },
        ))
    }
    pub(crate) fn usage(
    ...
    pub(crate) fn projection_usage(
        &self, value: &Array, claim: &CaptureInterventionClaim<'_>, custody: &OriginalSpeculativeBudgetCustody,
    ) -> Result<[CaptureUsage; 2], Failure> {
        self.checked(value, claim, NativeCustody::Model(custody)).map(|(edit, _)| edit.projection)
    }
    pub(crate) fn execute(
        ...
        self.execute_with(value, claim, charged, projection, stream, roots, NativeCustody::Model(custody), observer)
    }
    pub(crate) fn usage_scheduled(
        &self, value: &Array, claim: &CaptureInterventionClaim<'_>, scope: &WorkingMemoryFundingScope,
    ) -> Result<CaptureUsage, Failure> {
        self.checked(value, claim, NativeCustody::Scheduled(scope)).map(|(edit, _)| edit.usage)
    }
    pub(crate) fn projection_usage_scheduled(
        &self, value: &Array, claim: &CaptureInterventionClaim<'_>, scope: &WorkingMemoryFundingScope,
    ) -> Result<[CaptureUsage; 2], Failure> {
        self.checked(value, claim, NativeCustody::Scheduled(scope)).map(|(edit, _)| edit.projection)
    }
    pub(crate) fn execute_scheduled(
        &self, value: &Array, claim: CaptureInterventionClaim<'_>, charged: CaptureUsage,
        projection: [CaptureUsage; 2], stream: &Stream, roots: &RefCell<Vec<Array>>,
        scope: &WorkingMemoryFundingScope, observer: &OriginalScopeObserver,
    ) -> Result<(Option<Array>, ClaimedIntervention), FundedCaptureError<Failure>> {
        self.execute_with(value, claim, charged, projection, stream, roots, NativeCustody::Scheduled(scope), observer)
    }
    fn execute_with(
        &self, value: &Array, claim: CaptureInterventionClaim<'_>, charged: CaptureUsage,
        projection: [CaptureUsage; 2], stream: &Stream, roots: &RefCell<Vec<Array>>,
        custody: NativeCustody<'_>, observer: &OriginalScopeObserver,
    ) -> Result<(Option<Array>, ClaimedIntervention), FundedCaptureError<Failure>> {
        ...
        let charged = charged.checked_add(projection[0])?.checked_add(projection[1])?;
        Ok((output, claim.finish(charged)?))
    }
    fn execute_program(program:Option<PreparedStaticActivation<'_>>,value:&Array,stream:&Stream,
        ...
        program.map(|program|program.execute_array(value,stream,observer,roots))
            .transpose().map_err(FundedCaptureError::Backend)
    }
    pub(super) fn selected(&self,index:usize)->bool {
        self.rows.get(index).is_some_and(|row|!matches!(row,Row::Inactive))
    }
    ...
    fn execution_control_bytes() -> Option<usize> {
        let frames = [
            ...
            size_of::<(&Self, &Array, &CaptureInterventionClaim<'_>, &WorkingMemoryFundingScope)>(),
            size_of::<(&Self, &Array, CaptureInterventionClaim<'_>, CaptureUsage, [CaptureUsage; 2],
                &Stream, &RefCell<Vec<Array>>, &WorkingMemoryFundingScope, &OriginalScopeObserver)>(),
            size_of::<(&Self, &Array, CaptureInterventionClaim<'_>, CaptureUsage, [CaptureUsage; 2],
                &Stream, &RefCell<Vec<Array>>, NativeCustody<'_>, &OriginalScopeObserver)>(),
            ...
            size_of::<Result<(Option<crate::MlxTensor>, ClaimedIntervention), FundedCaptureError<Failure>>>(
            ),
