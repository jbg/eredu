...
use super::*;
use super::{
    original_branch::{OriginalRealtimeSessionState, RealtimeBranchSource},
    original_execution::{FrameNativeLayout, PreparedOriginalRealtimeFrame},
    retained_frame_source::{
        compile_frame_source, CompiledRealtimeFrameSource, RealtimeNativeFrameSource,
    },
};
use crate::backend::{
    nn::workspace::{MlxMetalWorkspaceMechanisms, ResidentExecutionMechanisms},
    OriginalCopyEnvironment,
};
use eredu_core::{
    scheduler::{SchedulerError, SchedulerProgress, WorkId},
    BackendFailure,
};
use eredu_nn::workspace::{HostMetadataFunding, WorkspaceContext};
use eredu_runtime::{
    working_memory::{RealtimeFrameRequirements, WorkingMemoryError},
    RealtimePayloadContract, RealtimePayloadState, RealtimeSessionScheduler,
};
use std::{
    cell::RefCell,
...
struct FundedCompletionFailure {
    #[source]
    cause: FrameCompletionFailure,
    _funding: HostMetadataFunding,
}
fn completion_failure_callback<'a>(
    first: Option<&'a FirstFrameFailure>,
    funding: Option<&'a HostMetadataFunding>,
) -> impl FnMut(WorkId, FrameCompletionFailure) + 'a + use<'a> {
    move |_, cause| {
        let first =
            first.expect("actual completion callback retains its prepaid first source slot");
        if first.borrow().is_none() {
            *first.borrow_mut() = Some(BackendFailure::from_error(FundedCompletionFailure {
                cause,
                _funding: funding
                    .expect("completion source paid before polling")
                    .clone(),
            }));
        }
    }
...
fn funded(cause: Error, funding: HostMetadataFunding, stage: &'static str) -> BackendFailure {
    match cause.take_retained_backend_failure() {
        Ok(cause) => cause,
        Err(cause) => BackendFailure::from_error(FundedFailure {
            stage,
            cause,
            _funding: funding,
        }),
...
fn u64_bytes(value: usize) -> Result<u64, Error> {
...
fn scheduler_callbacks<'a, 'model>(
    context: Option<&'a MlxRealtimeExecutionContext>,
    model: Option<&'a BorrowedModel<'model>>,
    first: Option<&'a FirstFrameFailure>,
    ...
    impl FnMut(
            WorkId,
            &RealtimeInputFrame,
            &mut MlxManagedFrameSessionBranch,
    ...
    (
        move |_, frame, state| {
            ...
            let model = model
                ...
                .try_borrow()
                .map_err(|_| identity().into_backend_failure())?;
            keep_first(
                context.prepare_realtime_frame(&model, frame, state, &limits),
                first,
            )
        },
        move |_, frame, branch| {
            ...
            let mut model = model
                ...
                .map_err(|_| identity().into_backend_failure())?;
            keep_first(
                context.submit_prepared_realtime_frame(&mut model, frame, branch),
                first,
            )
        },
    )
...
fn planning_control_bytes() -> Option<usize> {
    let callbacks = scheduler_callbacks(
        None,
        None,
        None,
    ...
    let parts = [
        size_of::<&'static str>(),
        size_of_val(&callbacks),
        size_of::<BorrowedModel<'_>>(),
        ...
        size_of::<RealtimeBranchSource<'_>>(),
        size_of::<CompiledRealtimeFrameSource>(),
        size_of::<FrameNativeLayout>(),
        size_of::<RealtimeFrameRequirements>(),
        size_of::<RealtimePayloadContract>(),
        size_of::<WorkspaceContext>(),
        size_of::<RealtimeNativeFrameSource<'_>>(),
        size_of::<ResidentExecutionMechanisms>(),
        size_of::<Result<MlxManagedFrameSessionBranch, BackendFailure>>(),
        size_of::<FundedFailure>(),
        size_of::<(
            &MlxRealtimeExecutionContext,
            &MoshiRealtimeExecution<MlxRealtimeExecution>,
            &RealtimeInputFrame,
            &OriginalRealtimeSessionState,
            u64,
            Option<u64>,
...
pub(super) fn submission_control_bytes() -> Option<usize> {
    let parts = [
        ...
        size_of::<(
            &MlxRealtimeExecutionContext,
            &mut MoshiRealtimeExecution<MlxRealtimeExecution>,
            &RealtimeInputFrame,
            &mut MlxManagedFrameSessionBranch,
        )>(),
        OriginalCopyEnvironment::control_bytes()?,
...
) -> Result<ResidentExecutionMechanisms, Error> {
    ResidentExecutionMechanisms::from_stream(
...
impl MlxRealtimeExecutionContext {
    ...
    /// physical-domain limits before cloning state.
    ...
    pub fn prepare_realtime_frame(
        &self,
        model: &MoshiRealtimeExecution<MlxRealtimeExecution>,
        frame: &RealtimeInputFrame,
        canonical: &OriginalRealtimeSessionState,
        limits: &eredu_core::MemoryLimitDeclarations,
    ) -> Result<MlxManagedFrameSessionBranch, BackendFailure> {
        ...
        let funding: HostMetadataFunding = self
            .backend()
            .memory_ledger()
            .prepare_workspace_metadata(
                resources.execution_identity(),
                limits
                    .resolve(self.backend().memory_ledger().topology())
                    .map_err(|cause| Error::PrefillControl(cause.into()).into_backend_failure())?,
            )
        ...
        let result = (|| {
            funding
                .reserve_metadata(planning_control_bytes().ok_or_else(overflow)?)
            ...
            let occurrence = canonical
                .frame_occurrence_for(frame.batch())
            ...
            let payload = canonical
                .payload_contract_for_frame(resources.ingress_contract(), frame.batch(), &funding)
                .map_err(|cause| Error::Neural(context.metadata_source(cause)))?;
            let generation = canonical.generation();
            let native = RealtimeNativeFrameSource {
                state: generation.model_state().model_state(),
                history: generation.model_state().payload_history(),
                schedule: generation.schedule_state(),
                sampling: generation.sampling(),
                samplers: generation.samplers(),
                random: generation.random_state(),
            };
            stage = "cold equation source";
            let source = compile_frame_source(
                model.executor(),
                native,
                ingress,
                &payload,
            ...
            let controls = layout
                .control_bytes()
                .checked_add(branch_source.host_bytes().ok_or_else(overflow)?)
                .ok_or_else(overflow)?;
            ...
            let mut requirements = RealtimeFrameRequirements::new(
                occurrence,
                Some(u64_bytes(layout.capacity.backing)?),
                Some(0),
                Some(u64_bytes(layout.capacity.graph)?),
                Some(u64_bytes(layout.capacity.records)?),
                Some(u64_bytes(controls)?),
                source.source_program.facts(),
                Arc::new(
                    crate::backend::managed_memory::cold_shared_allocator_placement()
                        .ok_or_else(identity)?
                        .clone(),
                ),
            )
            ...
            if let Some((copy, controls)) = branch_source.preparation()? {
                requirements = requirements
                    .with_preparation(copy, Some(u64_bytes(controls)?))
                    .map_err(Error::PrefillControl)?;
            }
            stage = "frame admission";
            let account = environment
                .pool()
                .reserve_realtime_frame(
                    resources.execution_identity(),
                    requirements,
                    limits
                        .resolve(environment.pool().topology())
                        .map_err(|cause| Error::PrefillControl(cause.into()))?,
                )
                .map_err(|cause| Error::Neural(context.metadata_source(cause)))?;
            stage = "accepted source installation";
            let mut preparation = PreparedOriginalRealtimeFrame::accept(
                account,
                source,
                layout,
                payload,
            ...
            let mut branch = branch_source
                .prepare(
                    &mut preparation,
                    Some(model.selected().completion().timeout()),
                )
                .map_err(Error::StorageSource)?;
            branch
                .install_preparation(preparation)
                .map_err(|_| identity())?;
            Ok(branch)
        })();
    ...
    pub fn submit_prepared_realtime_frame(
        &self,
        model: &mut MoshiRealtimeExecution<MlxRealtimeExecution>,
        frame: &RealtimeInputFrame,
        branch: &mut MlxManagedFrameSessionBranch,
    ) -> Result<MlxPrepublicationFrame, BackendFailure> {
        let preparation = branch
            .take_preparation()
            .ok_or_else(|| identity().into_backend_failure())?;
        let funding = preparation.funding().clone();
        let result = (|| {
            funding
                .reserve_metadata(submission_control_bytes().ok_or_else(overflow)?)
                .map_err(Error::WorkspacePlanning)?;
            let resources = model.executor().completion_resources();
    ...
    pub fn run_realtime_bounded(
        ...
        let (prepare, execute) =
            scheduler_callbacks(Some(self), Some(&model), None, limits.clone());
    ...
    pub fn run_realtime_distributed_bounded(
        ...
        let (session, _) = model
            ...
            .ok_or_else(|| identity().into_backend_failure())?;
        let transport = crate::backend::distributed::PreparedConsensusTransport::prepare(
            session,
            self.backend().memory_ledger(),
            resources.execution_identity(),
            limits
                .resolve(self.backend().memory_ledger().topology())
                .map_err(|cause| Error::PrefillControl(cause.into()).into_backend_failure())?,
        ...
        .map_err(Error::into_backend_failure)?;
        let callback_shape = scheduler_callbacks(
            None,
            None,
            None,
            eredu_core::MemoryLimitDeclarations::default(),
        ...
        let parts = [
            size_of_val(&callback_shape),
            size_of_val(&completion_shape),
            size_of::<FirstFrameFailure>(),
            size_of::<FrameCompletionFailure>(),
            size_of::<FundedCompletionFailure>(),
            ...
            BackendFailure::source_retention_peak_bytes::<FundedCompletionFailure>()
                .ok_or_else(|| overflow().into_backend_failure())?,
            size_of::<BorrowedModel<'_>>(),
            size_of::<FundedSchedulerFailure>(),
            size_of::<
                Result<
                    SchedulerProgress<RealtimeInputFrame, MlxPrepublicationFrame>,
                    BackendFailure,
                >,
            >(),
            size_of::<(&Self, u64, usize, u64, Option<u64>)>(),
            BackendFailure::source_retention_peak_bytes::<FundedSchedulerFailure>()
                .ok_or_else(|| overflow().into_backend_failure())?,
        ];
        let bytes = parts
            .into_iter()
        ...
        let model = RefCell::new(model);
        let (prepare, execute) =
            scheduler_callbacks(Some(self), Some(&model), Some(&first), limits.clone());
        let completion_failure =
            completion_failure_callback(Some(&first), Some(transport.funding()));
        let result = scheduler.run_distributed_bounded_with_preparation_and_errors(
            protocol,
            &transport,
            now,
            maximum_frames,
        ...
        if let Some(cause) = first.into_inner() {
            return Err(cause);
        }
        if let Some(cause) = transport.take_failure() {
            return Err(cause);
        }
        result.map_err(|cause| {
            BackendFailure::from_error(FundedSchedulerFailure {
                cause,
                _funding: transport.funding().clone(),
            })
        })
    }
}
