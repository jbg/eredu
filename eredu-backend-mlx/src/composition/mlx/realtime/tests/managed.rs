...
fn drive_managed_frame(model:&mut SelectedTestModel,scheduler:&mut MlxManagedRealtimeScheduler,
    request:RequestId,frame:RealtimeInputFrame)->RealtimeOutputFrame {
...
fn native_managed_realtime_frames_match_seeded_ordinary_and_refuse_before_branch() {
    ...
    for (index, frame) in inputs().into_iter().enumerate() {
        ...
        if index==1 {
            ...
            let branch=model.backend.prepare_realtime_frame(&model.model,&next,
                scheduler.request_state(request).unwrap(),CAPACITY,None).unwrap();
            <MlxManagedRealtimeSessionState as eredu_core::scheduler::SemanticStateTransaction>
                ::discard_branch(branch).unwrap();
            assert_eq!(scheduler.request_state(request).unwrap().generation()
                .schedule_state().frontier(),original_frontier+index+1);
...
fn verify_bounded(residency:WeightResidency,execution:ExecutionResidency) {
    ...
    let expected={
        let mut model=load(directory.path());
        let mut scheduler=selected_scheduler(&model,request,sampling);
        inputs().into_iter().map(|frame|tokens(&drive_selected_frame(
            &mut model,&mut scheduler,request,frame))).collect::<Vec<_>>()
    };
    crate::backend::ordinary_retirement::reclaim();
    let options=||MlxLoadRequest::from_normalized(
        eredu_runtime::NormalizedLoadRequest::default().with_weight_residency(residency.clone()));
    {
        let mut model=load_with_options(directory.path(),options());
        assert_eq!(model.model.executor().metadata().residency(),execution);
        let mut scheduler=selected_scheduler(&model,request,sampling);
        for (index,frame) in inputs().into_iter().enumerate() {
            assert_eq!(tokens(&drive_selected_frame(&mut model,&mut scheduler,request,frame)),
                expected[index],"ordinary bounded frame {index}");
        }
    }
    crate::backend::ordinary_retirement::reclaim();
    let mut model=load_with_options(directory.path(),options());
    ...
    let disk_before = if execution == ExecutionResidency::DenseDiskStream {
        let report = model.model.executor().dense_stream_report().unwrap().unwrap();
        let reads = report.residency().offload().transfer(TransferDirection::DiskToHost);
        Some((reads.count(), reads.bytes(), report.background().completed()))
    } else { None };
    for (index,frame) in inputs().into_iter().enumerate() {
        let actual=drive_managed_frame(&mut model,&mut scheduler,request,frame);
        assert_eq!(tokens(&actual),expected[index],"managed bounded frame {index}");
        assert_eq!(scheduler.request_state(request).unwrap().generation().schedule_state().frontier(),index+1);
    }
    ...
    if execution==ExecutionResidency::LayerwiseHost {
        ...
        let units=report.units().iter()
            .filter(|unit|unit.policy()==ResidencyPolicy::Windowed)
            .collect::<Vec<_>>();
        assert!(!units.is_empty(),"selected Host execution retains bounded units");
        assert!(units.iter().all(|unit|unit.planned_tier()==MemoryTier::Host
            && unit.host_resident() && unit.host_allocated_bytes()>0));
        let transfers=report.offload().transfer(TransferDirection::HostToDevice);
        assert!(transfers.count()>0 && transfers.bytes()>0,
    ...
    if execution==ExecutionResidency::DenseDiskStream {
        ...
        assert!(report.decode_forwards()>0,"selected disk worker executed cached frames");
...
fn native_managed_realtime_host_layerwise_matches_ordinary_across_forced_depth_tails() {
    verify_bounded(WeightResidency::layerwise_host(LayerwiseLoadOptions::default()),
...
fn native_managed_realtime_disk_prefetch_matches_ordinary_across_cached_frames() {
    verify_bounded(WeightResidency::dense_disk_stream(
        DenseDiskStreamLoadOptions::new(1<<20,1<<20,1,1).unwrap()),ExecutionResidency::DenseDiskStream);
}
