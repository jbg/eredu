...
use super::*;
use crate::tests::distributed_realtime_ring::{MODEL_WORKER_FIXTURE, MODEL_WORKER_RANK};
...
fn native_managed_realtime_tp2_matches_ordinary_across_forced_and_cached_frames() {
    ...
    let world = std::sync::Arc::new(crate::backend::runtime::distributed::Group::uncontracted(
        &native,
    ));
    let consensus =
        crate::backend::distributed::MlxRealtimeConsensusTransport::new(&native, backend.stream());
    let backend = backend.with_tensor_parallel_group(world);
    let model = backend.materialize_realtime_execution(source).unwrap();
    assert_eq!(
        model.selected().topology(),
        eredu_core::ParallelTopology::new(2, 1, 1, 1).unwrap()
    );
    ...
    for (index, frame) in inputs().into_iter().enumerate() {
        ...
        let actual = finish_distributed_frame(|| {
            ordinary.run_distributed_turn(
                PROTOCOL,
                &consensus,
                ...
                |_, frame, branch| {
                    parallel
                        .backend
                        .submit_realtime_frame(&mut parallel.model, frame, branch)
                },
            )
        });
        assert_eq!(
            tokens(&actual),
            expected[index],
            "ordinary TP rank {rank} frame {index}"
        );
    }
    let ordinary_model_collectives =
        crate::backend::runtime::distributed::contracted_collective_submissions();
    assert!(
        ordinary_model_collectives > 0,
        "ordinary TP must submit selected model collectives"
    );
    assert_eq!(
        crate::backend::runtime::distributed::original_model_collective_submissions(),
        0,
    ...
    for (index, frame) in inputs().into_iter().enumerate() {
        ...
        let actual = finish_distributed_frame(|| {
            parallel.backend.run_realtime_distributed_bounded(
                &mut parallel.model,
                &mut managed,
                PROTOCOL,
                std::time::Instant::now(),
        ...
        assert_eq!(
            tokens(&actual),
            expected[index],
            "managed TP rank {rank} frame {index}"
        );
        assert_eq!(
            managed
...
fn finish_distributed_frame<E: std::fmt::Debug>(
    mut turn: impl FnMut() -> Result<
        eredu_core::scheduler::SchedulerProgress<RealtimeInputFrame, MlxPrepublicationFrame>,
        E,
    >,
) -> RealtimeOutputFrame {
    ...
    loop {
        ...
        assert!(
            ...
            "distributed frame did not finish"
        );
