...
use super::{MlxTensor,MlxKeyValueState,RandomState,GenerationSampler,MlxRealtimeExecution,Error};
use crate::{composition::moshi::{RealtimeOperationPlan,RealtimeOperationRecipe,RealtimeWorkspaceVisitor},
    backend::{nn::workspace::{ExistingArrayProjection,ProjectedNativeStorage,
...
use std::{mem::{size_of,size_of_val},num::NonZeroU32};

#[derive(Debug,thiserror::Error)]
#[error("realtime retained frame source {stage}: {cause}")]
struct SourceFailure {stage:&'static str,#[source]cause:Error}
...
fn incomplete_witness(context:&WorkspaceContext,role:&'static str,ordinal:usize,array:&safemlx::Array)
    ->Option<Error> {
    let controls=size_of::<(&WorkspaceContext,&str,usize,&safemlx::Array)>()
        .checked_add(size_of::<Option<Error>>())?;
    if let Err(cause)=context.charge_metadata(controls) {return Some(Error::Neural(cause.into()));}
...
pub(super) struct FrameShape {
    batch:usize,input:usize,audio:Option<usize>,text:Option<usize>,
    mask:Option<Vec<bool>>,diagnostics:bool,
}
impl FrameShape {
    fn prepare(frame:&eredu_core::RealtimeInputFrame,context:&WorkspaceContext)->Result<Self,Error> {
        context.charge_metadata(size_of::<Self>()).map_err(|e|Error::Neural(e.into()))?;
        let mask=frame.forced_generated_audio_codebooks().map(|values| {
            let mut owned=context.metadata_vec(values.len())?;owned.extend_from_slice(values);Ok::<_,eredu_nn::Error>(owned)
        }).transpose().map_err(Error::Neural)?;
        Ok(Self{batch:frame.batch(),input:frame.input_audio_tokens().len(),
            audio:frame.forced_generated_audio_tokens().map(<[i32]>::len),
            text:frame.forced_text_tokens().map(<[i32]>::len),mask,diagnostics:frame.retains_diagnostics()})
    }
    pub(super) fn matches(&self,frame:&eredu_core::RealtimeInputFrame)->bool {
        self.batch==frame.batch()&&self.input==frame.input_audio_tokens().len()
...
impl RealtimeWorkspaceVisitor for Visitor<'_> {
    fn visit(&mut self,model:&mut eredu_architectures::moshi::PreparedMoshiWorkspaceFrame<'_>,
        parameters:&mut ExistingArrayProjection<'_>,layerwise:Option<&LayerwiseWorkspace>)
        ->Result<(),Error> {
        let context=self.context;
        let invalid=|reason:&'static str|Error::Neural(context.metadata_error(format_args!(
            "realtime retained source {reason}; frontier {:?}; history values {}",
            self.native.schedule.frontier(),self.native.history.len())));
        if self.result.is_some() {return Err(invalid("visitor repeated"));}
        if !parameters.is_complete() {return Err(invalid("parameter backing incomplete"));}
        let operations=self.operations.take().ok_or_else(||invalid("operation plan already consumed"))?;
        let batch=NonZeroU32::new(u32::try_from(self.ingress.frame().batch()).map_err(|_|invalid("batch extent overflow"))?)
            .ok_or_else(||invalid("zero batch"))?;
        let mut state=self.native.state.project_resident_workspace_with_storage(batch,context)
            .map_err(|cause|source_failure(context,"state projection",Error::Neural(cause)))?;
        if !state.storage.is_complete() {
            let mut cause=None;let mut ordinal=0usize;
            <MlxKeyValueState as eredu_runtime::RuntimeState<crate::backend::nn::shared::MlxNeuralBackend>>::visit_all_retained_values(
                self.native.state,&mut |value| {
                    if cause.is_none() {cause=incomplete_witness(context,"state",ordinal,value.as_array());}
        ...
        let count=self.native.history.len().checked_add(usize::from(self.native.random.is_some()))
        ...
        let funding=context.metadata_funding().ok_or_else(||invalid("metadata funding missing"))?;
        ...
        let random=self.native.random.map(|value|projection.project(value.as_array())
            .and_then(WorkspaceSamplingRandomState::from_key)).transpose()
            .map_err(|cause|source_failure(context,"random key projection",Error::Neural(cause)))?;
        if !projection.is_complete() {
            return Err(self.native.random.and_then(|value|incomplete_witness(context,"random key",0,value.as_array()))
                .unwrap_or_else(||invalid("history/random backing incomplete")));
        }
        let payloads=projection.try_into_storage()
            .map_err(|cause|source_failure(context,"history/random retained storage",Error::Neural(cause)))?;
        let source=super::frame_trace::RealtimeFrameTraceSource{
            ingress:self.ingress,payload:self.payload,state:&mut state.state,history:&history,
            schedule:self.native.schedule,sampling:self.native.sampling,samplers:self.native.samplers,
            random:random.as_ref(),
            native_alias_bytes:MlxTensor::host_clone_bytes().ok_or_else(||invalid("tensor alias control overflow"))?,
            random_copy_bytes:self.native.random.map(|_|RandomState::host_clone_bytes().ok_or_else(||invalid("random alias control overflow"))).transpose()?,
        };
        ...
        let parallel=self.parallel.map(|source|source.prepare_invocation(&traced.report)).transpose().map_err(Error::Neural)?;
        let capture=crate::backend::array_copy::CaptureNativePopulation::default();
        let numerical=match parallel.as_ref() {
            Some(parallel)=>SpeculativeNumericalRecipe::inspect_owned_parallel_child_with_sources(
                &traced.report,traced.completion_roots,self.mechanism,context,capture,layerwise,parallel),
            None=>SpeculativeNumericalRecipe::inspect_owned_child_with_sources(
                &traced.report,traced.completion_roots,self.mechanism,context,capture,layerwise),
        }.map_err(|cause|source_failure(context,"native numerical qualification",Error::Neural(cause)))?;
        let operations=operations.qualify(numerical,layerwise,context)
        ...
        let source_program=super::source_program::FrameSourceProgram::prepare(
            traced.initialized_inputs,operations.source_facts(),context)
        ...
        let schedule=self.ingress.contract().schedule().try_clone_with_host_source(&funding)
            .map_err(|cause|Error::Neural(context.metadata_source(cause)))?;
        let ingress=eredu_runtime::RealtimeIngressContract::new(schedule,self.ingress.contract().text_domain(),
            self.ingress.contract().audio_domain()).map_err(|cause|Error::Neural(context.metadata_source(cause)))?;
        self.result=Some(CompiledRealtimeFrameSource{operations,parallel,source_program,host:traced.host,initialized_inputs:traced.initialized_inputs,
            completion_roots:traced.completion_roots,coordinator:traced.coordinator,ingress,shape,_state:state.storage,_payloads:payloads,_funding:funding});
        Ok(())
    }
}
...
pub(crate) fn compile_frame_source(model:&MlxRealtimeExecution,native:RealtimeNativeFrameSource<'_>,
    ingress:RealtimeIngressSource<'_>,payload:&RealtimePayloadContract,runtime:&PreparedInputRuntime,
    stream:&Stream,pool:&eredu_runtime::working_memory::WorkingMemoryPool,mechanism:ResidentExecutionMechanisms,context:&WorkspaceContext)
    ->Result<CompiledRealtimeFrameSource,Error> {
    let frames=[size_of::<Visitor<'_>>(),size_of::<CompiledRealtimeFrameSource>(),
        size_of::<Result<CompiledRealtimeFrameSource,Error>>(),size_of::<RealtimeNativeFrameSource<'_>>(),
        size_of::<ProjectedNativeStorage>()*2,size_of::<RealtimePayloadHistory<WorkspaceTensor>>(),
        size_of::<Option<WorkspaceSamplingRandomState>>(),size_of::<RealtimeOperationPlan>(),
        size_of::<Option<MlxParallelWorkspace>>(),size_of::<Result<MlxParallelWorkspace,eredu_nn::Error>>(),
    ...
    let parallel=model.parallel_communication().map(|(communication,id)| {
        let source=communication.original_initialized_tensor_source(id,pool,&funding)?;
        MlxParallelWorkspaceMechanisms::new(mechanism,source).prepare_workspace().map_err(Error::Neural)
    }).transpose()?;
    let context=parallel.as_ref().map_or(context,MlxParallelWorkspace::context);
    let operations=model.realtime_operation_plan(stream,mechanism.allocation(),pool,context)
        .map_err(|cause|source_failure(context,"selected operation source",cause))?;
    let mut visitor=Visitor{native,ingress,payload,runtime,context,mechanism,parallel:parallel.as_ref(),operations:Some(operations),result:None};
    model.with_workspace_frame(mechanism.allocation(),context,&mut visitor)
