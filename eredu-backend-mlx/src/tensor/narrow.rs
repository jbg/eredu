...
pub(super) fn execute(
    ...
    if rank <= 4 {
        ...
        return input
            .as_array()
            .try_slice(&starts[..rank], &ends[..rank], &strides[..rank], stream)
    ...
    input
        .as_array()
        .try_slice(&starts, &ends, &strides, stream)
        .map(MlxTensor::from_array)
        .map_err(error)
}
...
pub(crate) fn control_bytes(rank: usize) -> Option<usize> {
    caller_control_bytes(rank, safemlx::OriginalScopeObserver::control_bytes()?)
}
/// The same ordinary coordinate/index caller. Its error path may inspect TLS,
/// but never constructs or retains an Original observer when none is installed.
pub(crate) fn ordinary_control_bytes(rank: usize) -> Option<usize> {
    caller_control_bytes(
        rank,
        size_of::<Result<Option<safemlx::OriginalScopeObserver>, safemlx::error::Exception>>(),
    )
}
fn caller_control_bytes(rank: usize, observer_controls: usize) -> Option<usize> {
    let frames = [
        range_index_control_bytes(rank)?,
        size_of::<TensorAxisRange<'_>>(),
        size_of::<Result<TensorAxisRange<'_>, TensorAxisRangeError>>(),
        size_of::<(&MlxTensor, usize, i32, i32, &Stream)>() * 2,
        size_of::<[i32; 4]>() * 3,
        size_of::<[Vec<i32>; 3]>(),
        size_of::<[&mut Vec<i32>; 3]>(),
        size_of::<std::array::IntoIter<&mut Vec<i32>, 3>>(),
        size_of::<usize>(),
    ...
    frames
        ...
        .checked_add(if rank > 4 {
            rank.checked_mul(3)?.checked_mul(size_of::<i32>())?
        } else {
            0
        })
}
...
fn range_index_control_bytes(rank: usize) -> Option<usize> {
    let frames = [
        ...
        size_of::<std::slice::Iter<'_, Index>>(),
        size_of::<&Index>(),
        size_of::<ArrayIndexOp<'_>>(),
    ...
    frames
        ...
        .checked_add(if rank > 5 {
            rank.checked_next_power_of_two()?
    ...
    fn native_axis_range_preserves_batched_values_empty_shapes_and_source_aliases() {
        ...
        for shape in [vec![2, 5, 3], vec![2, 1, 5, 1, 3]] {
            ...
            for (start, end) in [(3, 5), (4, 5), (0, 5), (5, 5)] {
                ...
                let ordinary = input
                    .index(&range.indexes().collect::<Vec<_>>(), &stream)
                    .unwrap();
        ...
        assert_eq!(
            control_bytes(5).unwrap() - control_bytes(4).unwrap(),
            2 * 3 * 5 * size_of::<i32>()
        );
    }
}
