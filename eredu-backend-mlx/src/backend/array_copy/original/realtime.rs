...
use eredu_runtime::working_memory::{
    OriginalRealtimeNative, RealtimeNativeRequirements, WorkingMemoryError,
};
...
impl OriginalCopyLayoutBuilder {
    ...
    pub(crate) fn finish_realtime<'a>(
        ...
        Ok(Some(RealtimeCopyPlan {
            ...
            source_clones: self.source_clones,
...
impl RealtimeCopyContext<'_> {
    pub(crate) fn retain_source(&self, source: &Array) -> Result<(), Error> {
        ...
        self.roots.borrow_mut().push(source.try_clone_handle()?);
        Ok(())
    }
    pub(crate) fn copy(&self, source: IsolatedArrayCopy<'_>) -> Result<Array, Error> {
        ...
        source
            .copy_retained(self.stream, self.roots)
            .map_err(Into::into)
    }
}
fn mismatch() -> Error {
...
fn callback<'a, I: 'static, T>(
    plan: Option<&'a RealtimeCopyPlan<'a>>,
    mut run: Option<&'a mut CopyCallback<'a, I, T>>,
) -> impl FnMut(
    &CopyWork<I>,
    &native_role::realtime::RealtimeRoleContext<'_>,
) -> Result<Result<T, Infallible>, Error>
       + 'a {
    move |work, role| {
        let plan = plan.expect("active retained copy plan");
        let mut graph =
            OperationEvent::prepare_resident_graph(plan.layout.graph, role.native().observer())?;
        graph.configure_nested_completions(
            &plan.layout.traversal,
            plan.layout.completion_attempts,
        )?;
        let context = RealtimeCopyContext {
            stream: plan.stream,
            roots: &work.roots,
            remaining: Cell::new(plan.operands),
            retained: Cell::new(plan.source_clones),
        };
        ...
        drop(graph);
        let output = result?;
        if context.remaining.get() != 0 || context.retained.get() != 0 {
            return Err(mismatch());
        }
        Ok(Ok(output))
    }
...
impl RealtimeCopyPlan<'_> {
    pub(crate) fn requirements(&self) -> Result<RealtimeNativeRequirements, WorkingMemoryError> {
        RealtimeNativeRequirements::new(
            u64::try_from(self.capacity.backing).ok(),
            u64::try_from(self.capacity.graph).ok(),
            u64::try_from(self.capacity.records).ok(),
            std::sync::Arc::new(
    ...
    fn root_count(&self) -> Option<usize> {
        self.operands
            .checked_mul(2)?
            .checked_add(self.source_clones)
    }
    fn worker_controls<I, T>(&self) -> Option<usize> {
        let parts = [
            ...
            size_of::<Self>(),
            size_of::<RealtimeCopyContext<'_>>(),
            size_of::<CopyWork<I>>(),
            size_of::<Result<T, Error>>(),
            size_of::<Result<Result<T, Infallible>, BackendFailure>>(),
            size_of::<&mut CopyCallback<'_, I, T>>(),
            size_of::<(usize, usize)>(),
            size_of::<Result<Array, Error>>(),
            size_of::<super::super::NativeCopy<'_>>(),
            size_of::<IsolatedArrayCopy<'_>>(),
            size_of::<HostMetadataFunding>(),
        ];
        sum(&parts)
    }
    pub(crate) fn control_bytes<I: 'static, T>(
        &self,
        runtime: &PreparedInputRuntime,
    ) -> Option<usize> {
        let callback = callback::<I, T>(None, None);
        self.worker_controls::<I, T>()?
            .checked_add(WorkspaceContext::metadata_vec_bytes::<Array>(
                self.root_count()?,
            )?)?
            .checked_add(
                native_role::realtime::control_bytes::<CopyWork<I>, T, Infallible>(
                    runtime,
                    self.capacity,
                    Some(PreparedPipelineCachePlan::new(self.layout.kernel_attempts)),
    ...
    pub(crate) fn run<I: 'static, T>(
        ...
        if claim.physical_bytes() != self.capacity.backing as u64
            || claim.graph_bytes() != self.capacity.graph as u64
            || claim.record_bytes() != self.capacity.records as u64
        {
            return Err(mismatch().into_backend_failure());
        }
        let roots = funding
            .metadata_vec(
                self.root_count()
            ...
            .map_err(|cause| Error::Neural(cause).into_backend_failure())?;
        let work = CopyWork {
            invocation,
        ...
        match native_role::realtime::run(
            work,
            claim,
            runtime,
            Some(PreparedPipelineCachePlan::new(self.layout.kernel_attempts)),
