...
use crate::backend::runtime::distributed::{
    completion::OriginalCommunicationU32Words, topology::OriginalCommunicationOwner,
};
use eredu_core::{
    consensus::{BoundedConsensusTransport, ConsensusTransport},
    run_preparation::{TextPreparationOutcome, TextPreparationStage, TextPreparationStatus},
    BackendFailure, TextStepContext,
};
use eredu_nn::workspace::{HostMetadataFunding, HostMetadataFundingError};
use eredu_runtime::{
    run_preparation::{
        TextPreparationAgreementError, TextPreparationCoordinator, TextPreparationTransport,
        TEXT_PREPARATION_WORDS,
    },
...
enum Origin {
    Text(TextStepContext),
    Reset(eredu_runtime::working_memory::SessionResetPreparationFunding),
}
struct Body {
...
impl std::fmt::Debug for MlxTextPreparationControl {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("MlxTextPreparationControl")
            .finish_non_exhaustive()
    }
}
#[derive(Debug, thiserror::Error)]
enum Cause {
    #[error("readiness source differs from the actual loaded session/request")]
    Identity,
    #[error(transparent)]
    Backend(Error),
    #[error(transparent)]
    Protocol(TextPreparationAgreementError),
    #[error(transparent)]
    Boundary(BackendFailure),
}
#[derive(Debug, thiserror::Error)]
#[error("{cause}")]
struct Failure {
...
impl MlxDistributedSession {
    ...
    pub(crate) fn prepare_original_readiness(
        &self,
        ...
        funding
            ...
            .map_err(Error::WorkspacePlanning)?;
        self.prepare_readiness_source(
            selected,
            world,
            pool,
            execution,
            capacity,
            Origin::Text(context.clone()),
            funding,
        )
    }
    pub(crate) fn prepare_reset_readiness(
        &self,
        selected: &eredu_runtime::CommunicationManifest,
        world: &NativeGroup,
        pool: &MemoryLedger,
        execution: &InferenceExecutionIdentity,
        capacity: eredu_core::MemoryLimits,
        preparation: eredu_runtime::working_memory::SessionResetPreparationFunding,
    ) -> Result<MlxTextPreparationControl, Error> {
        ...
        self.prepare_readiness_source(
            ...
            execution,
            capacity,
            Origin::Reset(preparation),
            funding,
        )
    }
    fn prepare_readiness_source(
        &self,
        selected: &eredu_runtime::CommunicationManifest,
        world: &NativeGroup,
        pool: &MemoryLedger,
        ...
        reserve(
            ...
            &[
                size_of::<Origin>(),
                size_of::<Body>(),
                size_of::<MlxTextPreparationControl>(),
                size_of::<Option<Rc<Body>>>(),
                size_of::<InferenceExecutionIdentity>(),
                size_of::<TextStepContext>(),
                size_of::<Result<MlxTextPreparationControl, Error>>(),
                size_of::<Option<Arc<TextPreparationCoordinator>>>(),
                Layout::new::<[usize; 2]>()
        ...
        let owner = self
            .original_communication_owner(selected, world, &funding)?
            .pin_registered_buffers(pool)?;
        let error = || {
            Error::with_original_control_source(
                failure(Cause::Identity, owner.source(), &funding),
                false,
            )
        };
...
impl MlxTextPreparationControl {
    fn body(&self) -> &Body {
        self.body.as_deref().expect("live preparation source")
    }
    ...
    pub(crate) fn retain_reset_rejection(&self, cause: BackendFailure) -> BackendFailure {
        ...
        if boundary {
            BackendFailure::new(
                cause.kind(),
                Failure {
                    cause: Cause::Boundary(cause),
                    source: self.body().owner.source().clone(),
                    funding: self.body().funding.clone(),
                },
            )
        } else {
            cause
        } // typed native/fixed refusal already has its own custody
    }
    pub(crate) fn agree(
        &self,
    ...
    ) -> Result<TextPreparationOutcome, BackendFailure> {
        let body = self.body();
        let funding = match &body.origin {
            Origin::Text(_) => pool
                .prepare_workspace_metadata(execution, body.capacity.clone())
        ...
        reserve(
            ...
            &[
                size_of::<Transport<'_>>(),
                size_of::<TextPreparationStage>(),
                size_of::<TextPreparationStatus>(),
                ...
                BackendFailure::source_retention_peak_bytes::<
                    eredu_core::run_preparation::TextPreparationCancelled,
                >()
                .ok_or_else(overflow)
                .map_err(Error::into_backend_failure)?,
                size_of::<TextPreparationOutcome>(),
                size_of::<Result<TextPreparationOutcome, TextPreparationAgreementError>>(),
                size_of::<Result<TextPreparationOutcome, BackendFailure>>(),
                size_of::<Failure>(),
                size_of::<Cause>(),
        ...
        .map_err(Error::into_backend_failure)?;
        let reset_stage = matches!(
            stage,
            TextPreparationStage::SessionReset | TextPreparationStage::SessionResetPublication
        );
        if reset_stage != matches!(body.origin, Origin::Reset(_)) {
            return Err(failure(Cause::Identity, body.owner.source(), &funding));
        }
        if !body.pool.same_ledger(pool)
            || !body.execution.same_execution(execution)
            || !session
                .preparation
                .as_ref()
                .is_some_and(|value| Arc::ptr_eq(value, &body.coordinator))
            || !session
                .communicators
                .control_world()
                .retained_source()
                .is_some_and(|actual| actual.same_source(body.owner.source()))
        {
            return Err(failure(Cause::Identity, body.owner.source(), &funding));
        }
...
impl Transport<'_> {
    fn error(&self, cause: Cause) -> Error {
        Error::with_original_control_source(
            failure(cause, self.source.body().owner.source(), self.funding),
            false,
        )
    }
}
impl ConsensusTransport for Transport<'_> {
    ...
    fn participant_count(&self) -> usize {
        self.session.manifest.world_size()
    }
    fn all_gather_words(&self, _: &[u32]) -> Result<Vec<u32>, Error> {
        Err(self.error(Cause::Identity))
    }
}
impl BoundedConsensusTransport for Transport<'_> {
    type Completion =
        crate::backend::runtime::distributed::topology::original_source::PreparationCompletion;
    ...
    fn submit_all_gather_words(
        ...
        reserve(
            ...
            &[
                ...
                size_of::<Submission<Self::GatherOutput, Self::Completion>>(),
                size_of::<&[u32; TEXT_PREPARATION_WORDS]>(),
        ...
        let words: &[u32; TEXT_PREPARATION_WORDS] =
            local.try_into().map_err(|_| self.error(Cause::Identity))?;
        let source = self.source.body().owner.borrow_funded(self.funding)?;
        let policy = match &self.source.body().origin {
            Origin::Reset(value) => Some(value),
            Origin::Text(_) => None,
        };
        ...
        let stream = source
            ...
            .ok_or_else(|| self.error(Cause::Identity))?;
