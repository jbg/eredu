...
#[derive(Debug,thiserror::Error)]
#[error("{cause}")]
struct Failure {#[source] cause:Cause,source:RetainedCommunicationSource,funding:WorkspaceMetadataFunding}
fn failure(cause:Cause,source:&RetainedCommunicationSource,funding:&WorkspaceMetadataFunding)->BackendFailure{
...
impl MlxDistributedSession {
    pub(crate) fn prepare_original_readiness(&self,selected:&eredu_runtime::CommunicationManifest,
        ...
        reserve(&funding,&[
            size_of::<Body>(),size_of::<MlxTextPreparationControl>(),size_of::<Option<Rc<Body>>>(),
        ...
        Ok(MlxTextPreparationControl{body:Some(Rc::new(Body{owner,coordinator:coordinator.clone(),pool:pool.clone(),
            execution:execution.clone(),context:context.clone(),capacity,funding:funding.clone()})),funding})
    }
}
impl MlxTextPreparationControl {
    ...
    pub(crate) fn agree(&self,session:&MlxDistributedSession,pool:&WorkingMemoryPool,execution:&InferenceExecutionIdentity,
        ...
        let body=self.body();
        let funding=pool.prepare_workspace_metadata(execution,body.capacity).map_err(|cause|Error::WorkspacePlanning(cause).into_backend_failure())?;
        reserve(&funding,&[
...
struct Transport<'a>{source:&'a MlxTextPreparationControl,session:&'a MlxDistributedSession,funding:&'a WorkspaceMetadataFunding}
impl Transport<'_>{
...
impl BoundedConsensusTransport for Transport<'_>{
    ...
    fn submit_all_gather_words(&self,local:&[u32])->Result<Submission<Self::GatherOutput,Self::Completion>,Error>{
        ...
        let frame=source.prepare_preparation_frame(words,&self.source.body().pool)?;
        let operation=frame.prepare(&source)?;
        let stream=source.world().retained_transport_stream().ok_or_else(||self.error(Cause::Identity))?;
        operation.start(&source,words,stream,&self.source.body().pool)
    }
