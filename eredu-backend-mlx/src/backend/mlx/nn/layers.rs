...
impl SwiGluMlp {
    ...
    pub fn forward_with_observer(
        ...
        stream: &Stream,
        prefix: &str,
        observer: &mut (impl RuntimeActivationObserver<MlxTensor, Exception> + ?Sized),
    ) -> Result<Array, Exception> {
        let gate = self.gate_proj.forward(input, stream)?;
        observer.observe(&format!("{prefix}.gate_proj"), MlxTensor::ref_cast(&gate))?;

        let up = self.up_proj.forward(input, stream)?;
        observer.observe(&format!("{prefix}.up_proj"), MlxTensor::ref_cast(&up))?;

        let activated_gate = silu(gate, stream)?;
        observer.observe(
            &format!("{prefix}.gate_activation"),
            MlxTensor::ref_cast(&activated_gate),
        )?;

        let down_proj_input = activated_gate.multiply(up, stream)?;
        observer.observe(
            &format!("{prefix}.down_proj_input"),
            MlxTensor::ref_cast(&down_proj_input),
        )?;

        let output = self.down_proj.forward(&down_proj_input, stream)?;
        observer.observe(&format!("{prefix}.down_proj"), MlxTensor::ref_cast(&output))?;
        Ok(output)
    }
}
