...
impl PreparedAddressableSource {
    pub(super) fn prepare(
        ...
        let specs = resolved.entries.iter().map(|entry|
            OffloadUnitSpec::new(entry.identity.unit_id(), entry.bytes,
                ResidencyPolicy::Cacheable, MemoryTier::Disk)).collect::<Result<Vec<_>,_>>()?;
        let definitions = resolved.entries.iter().map(|entry| entry.unit.clone()).collect::<Vec<_>>();
        let plan = OffloadPlan::new(options.storage, specs)?;
        let manager = ResidencyManager::prepare_original_addressable(
            resolved.store.clone(), &plan, &definitions, source_stream, device_stream, pool)
            .map_err(|cause| AddressableParameterBankError::Transformation {
                source: Box::new(Error::Other(Box::new(cause)))
            })?;
        if resolved.entries.is_empty() { return Ok(None); }
        ...
        Ok(Some(Self { origin, selected: declaration, options, resolved, manager }))
    }
...
impl MaterializedSelected {
    pub(super) fn into_bank(self, options: ParameterBankOptions, source_stream: Stream,
        device_stream: Stream, manager: Option<ResidencyManager>)
        ...
        let mut bank = AddressableParameterBank::new_shared_with_prepared_policy(
            self.store, self.entries, options, ResidencyPolicy::Cacheable, MemoryTier::Disk,
            source_stream, device_stream, self.formats, self.placements, self.report, manager,
        )?;
    ...
    source_stream: &Stream,
        ...
        if selected.transformations.is_empty() {
            let parameter_members = prepared_parameter_members(
                &selected.entries, &selected.parameter_targets, store.as_ref(),
            )?;
            return Ok(MaterializedSelected {
                store, entries: selected.entries, placements: selected.placements,
                parameter_members, formats: Vec::new(), report: None,
            });
        }
        ...
        Ok(MaterializedSelected {
            store: transformed.store, entries: transformed.entries,
            placements: selected.placements, parameter_members,
            formats: telemetry_formats, report: Some(transformed.report),
        })
}
