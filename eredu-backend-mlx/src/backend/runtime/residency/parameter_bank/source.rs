...
impl std::fmt::Debug for AddressableSourceFailure {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AddressableSourceFailure")
            .field("cause", &self.cause)
            .field("scope", &self.source.scope)
...
impl AddressableBankSourceLoan<'_> {
    ...
    pub(crate) fn members(&self) -> impl Iterator<Item = &PreparedBankParameterMember> {
        self.bank.parameter_members.iter().filter(|member| {
            self.owner
                .scope
                .is_none_or(|scope| scope == member.key.bank())
        })
    }
    ...
    pub(crate) fn unit_population(&self, bank: usize, unit: usize) -> Option<(usize, u64)> {
        if self.owner.scope.is_some_and(|scope| scope != bank) {
            return None;
        }
        self.bank
    ...
    pub(crate) fn parameter_revision(&self) -> u64 {
        self.bank.parameter_revision
    }
    /// Complete immutable catalog population used by coherent cache snapshots.
    pub(crate) fn resident_unit_population(&self) -> usize {
        self.bank.unit_banks.len()
    }
    ...
    pub(crate) fn member_bytes(&self, key: ParameterBankKey) -> Option<u64> {
        if self.owner.scope.is_some_and(|scope| scope != key.bank()) {
            return None;
        }
    ...
    pub(crate) fn replacement(
        &self,
        member: &PreparedBankParameterMember,
    ) -> Option<(&MlxTensor, usize)> {
        if !self.members().any(|actual| std::ptr::eq(actual, member)) {
            return None;
        }
        ...
        let row = self
            ...
            .iter()
            .filter(|candidate| {
                candidate.parameter == member.parameter && candidate.key < member.key
            })
            .count();
        Some((value, row))
    }
}
/// A descriptive source visitor, including its concrete callback storage.
pub(crate) trait AddressableSourceInspection {
    type Value;
    type Error;
    fn inspect(self, source: AddressableBankSourceLoan<'_>) -> Result<Self::Value, Self::Error>;
}
struct Callback<T, E, F>(F, std::marker::PhantomData<fn() -> Result<T, E>>);
impl<T, E, F> AddressableSourceInspection for Callback<T, E, F>
where
    F: for<'loan> FnOnce(AddressableBankSourceLoan<'loan>) -> Result<T, E>,
{
    type Value = T;
    type Error = E;
    fn inspect(self, source: AddressableBankSourceLoan<'_>) -> Result<T, E> {
        (self.0)(source)
    }
}

impl SharedAddressableParameterBank {
    pub(crate) fn workspace_source_control_bytes<V: AddressableSourceInspection>() -> Option<usize>
    {
        let frames = [
            size_of::<Self>(),
            size_of::<AddressableBankSourceLoan<'_>>(),
            size_of::<MutexGuard<'_, AddressableParameterBank>>(),
            size_of::<TryLockError<MutexGuard<'_, AddressableParameterBank>>>(),
            size_of::<V>(),
            size_of::<Result<V::Value, V::Error>>(),
            size_of::<Result<Result<V::Value, V::Error>, AddressableSourceFailure>>(),
            size_of::<AddressableSourceFailure>(),
            size_of::<AddressableSourceCause>(),
    ...
    pub(crate) fn with_workspace_source<T, E, F>(
        &self,
        funding: &HostMetadataFunding,
        inspect: F,
    ...
    where
        F: for<'loan> FnOnce(AddressableBankSourceLoan<'loan>) -> Result<T, E>,
    {
        self.with_workspace_inspection(funding, Callback(inspect, std::marker::PhantomData))
    }
    pub(crate) fn with_workspace_inspection<V: AddressableSourceInspection>(
        &self,
        funding: &HostMetadataFunding,
        inspect: V,
    ) -> Result<Result<V::Value, V::Error>, AddressableSourceFailure> {
        let failure = |cause| AddressableSourceFailure {
            cause,
            source: self.clone(),
            funding: funding.clone(),
        };
        let bytes = Self::workspace_source_control_bytes::<V>()
            .ok_or_else(|| failure(AddressableSourceCause::Overflow))?;
        funding
            .reserve_metadata(bytes)
            .map_err(|cause| failure(AddressableSourceCause::Funding(cause)))?;
        let bank = self.inner.try_lock().map_err(|cause| {
            failure(match cause {
                TryLockError::WouldBlock => AddressableSourceCause::Busy,
                TryLockError::Poisoned(_) => AddressableSourceCause::Poisoned,
            })
        })?;
        let source = AddressableBankSourceLoan {
            bank: &bank,
            owner: self,
        };
