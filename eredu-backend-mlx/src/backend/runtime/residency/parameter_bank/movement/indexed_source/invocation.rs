...
use crate::backend::runtime::{
    execution::generic::{OriginalSelectedResidencyAccess, OriginalSelectedResidencyAttempt},
    residency::manager::{
        ForegroundDiskSourceCapacity, ForegroundDiskSourceSeries, ForegroundDiskSubsetCeiling,
        ForegroundDiskWindowPlan, OriginalResidencySource, SupplementaryResidencySource,
        WindowPopulation,
    },
};
use crate::backend::submission_recovery::{
    Retention, Status,
    observed::{ObservedRecovery, PreparedObservedRecovery},
};
use eredu_runtime::{
    residency::ResidencyClosureSlot,
    working_memory::{HostSourceConstructionFacts, MemoryLedger, OriginalHostSourceBank},
};
...
pub(crate) struct IndexedConstructorPartitions {
    ...
    // The directory retires before the metadata child that allocated it.
    funding: HostMetadataFunding,
}
impl IndexedConstructorPartitions {
    ...
    pub(crate) fn select_count(
        ...
        funding
            .reserve_metadata(
                Self::select_count_control_bytes().ok_or(Error::PrefillControl(
                    eredu_runtime::working_memory::WorkingMemoryError::Overflow,
                ))?,
            )
            .map_err(|cause| {
                Error::Neural(eredu_nn::workspace::WorkspaceMetadataError::Funding(cause).into())
            })?;
...
impl IndexedResidencyPlan {
    fn row_frame_bytes() -> Option<usize> {
        Some(size_of::<(
            ...
            Option<AddressableChunkCensus>,
            AddressableChunkPlan,
    ...
    pub(crate) fn for_rows_control_bytes(&self) -> Option<usize> {
        Self::row_frame_bytes()?
            .checked_add(Self::clone_frame_bytes()?)?
            .checked_add(if self.read.is_some() {
                ForegroundDiskSubsetCeiling::clone_control_bytes()?
            } else {
                0
            })
    }
    ...
    pub(crate) fn for_rows(
        ...
        funding
            .reserve_metadata(Self::row_frame_bytes().ok_or_else(|| self.failure(Cause::Overflow))?)
        ...
        let first = AddressableChunkCensus::new(
            self.first.bank(),
            self.first.unit(),
            plan,
            0,
    ...
    pub(crate) fn materialized_population(
        &self,
    ) -> Result<
        crate::backend::runtime::residency::manager::ForegroundMaterializationPopulation,
        Error,
    > {
        self.read.as_ref().map_or_else(
            ...
            |read| {
                read.population()
                    .materialized
                    .checked_mul(self.first.plan().len())
                    .ok_or_else(|| self.failure(Cause::Overflow))
            },
        )
    }
    pub(crate) fn with_native_copy_source<T>(
        &self,
        visit: impl FnOnce(&SupplementaryResidencySource, WindowPopulation, usize) -> T,
    ) -> T {
    ...
    fn clone_frame_bytes() -> Option<usize> {
        let frames = [
            ...
            size_of::<IndexedBankSource>(),
            size_of::<SharedAddressableParameterBank>(),
            size_of::<SupplementaryResidencySource>(),
            size_of::<ResidencyManager>(),
            size_of::<Option<ForegroundDiskSubsetCeiling>>(),
            size_of::<Option<MemoryLedger>>(),
            size_of::<HostMetadataFunding>() * 2,
        ];
        frames
            .into_iter()
    ...
    pub(crate) fn invocation_control_bytes(&self) -> Result<usize, Error> {
        ...
        let parts=[Some(self.materialized_population()?.controls), Self::clone_frame_bytes(),
        ...
        parts
            .into_iter()
            .try_fold(size_of_val(&parts), |sum, n| sum.checked_add(n?))
            .ok_or_else(|| self.failure(Cause::Overflow))
    }
    ...
    pub(crate) fn clone_for_invocation(
        ...
        funding
            .reserve_metadata(Self::clone_frame_bytes().ok_or_else(|| fail(Cause::Overflow))?)
        ...
        Ok(Self {
            ...
            constructor_bytes: self.constructor_bytes,
            read: self
                .read
                .as_ref()
                .map(|read| read.clone_for_invocation(funding))
                .transpose()
                .map_err(|cause| fail(Cause::Funding(cause)))?,
            pool: self.pool.clone(),
            source_funding: self.source_funding.clone(),
    ...
    fn constructor_partition_frames() -> Option<usize> {
        let frames = [
            ...
            size_of::<Result<IndexedConstructorPartitions, Error>>(),
            size_of::<(
                &Self,
    ...
    pub(crate) fn constructor_partition_control_bytes(&self) -> Option<usize> {
        Self::constructor_partition_frames()?.checked_add(
            eredu_nn::workspace::WorkspaceContext::metadata_vec_bytes::<OriginalHostSourceBank>(
                self.first.plan().len(),
            )?,
        )
    }
    /// Direct root splitting. A region child is never treated as another root.
    pub(crate) fn partition_constructors(
        &self,
        access: &OriginalSelectedResidencyAccess,
        root: &mut OriginalHostSourceBank,
    ) -> Result<IndexedConstructorPartitions, Error> {
        self.partition_constructors_with_funding(access, root, &self.funding)
    }
    /// The accepted bank owns construction authority; the supplied metadata
    /// child pays only the directory and shared partitioning transports.
    pub(crate) fn partition_constructors_with_funding(
        &self,
        access: &OriginalSelectedResidencyAccess,
        root: &mut OriginalHostSourceBank,
        funding: &HostMetadataFunding,
        ...
        access
            .validate_source_bank(root)
            .map_err(|cause| self.failure(Cause::Consumer(cause)))?;
        let count = self.first.plan().len();
        let attempts = count
            .checked_mul(OriginalSelectedResidencyAttempt::construction_attempts())
            .ok_or_else(|| self.failure(Cause::Overflow))?;
        let bytes = self
            .constructor_bytes
            .checked_mul(u64::try_from(count).map_err(|_| self.failure(Cause::Overflow))?)
            .ok_or_else(|| self.failure(Cause::Overflow))?;
        if root.remaining_bytes() < bytes
            || root.remaining_attempts() < attempts
            || root.remaining_partitions() < count
        {
            return Err(self.failure(Cause::Identity));
        }
        funding
            .reserve_metadata(
                Self::constructor_partition_frames()
                    .ok_or_else(|| self.failure(Cause::Overflow))?,
            )
            .map_err(|cause| self.failure(Cause::Funding(cause)))?;
        let mut banks = funding
            .metadata_vec(count)
            .map_err(|cause| self.failure(Cause::Consumer(Error::Neural(cause))))?;
        for _ in 0..count {
            let bank = root
                .split(
                    self.constructor_bytes,
                    OriginalSelectedResidencyAttempt::construction_attempts(),
                )
                .map_err(|cause| self.failure(Cause::Budget(cause)))?;
            let facts = HostSourceConstructionFacts::new(
                self.constructor_bytes,
                OriginalSelectedResidencyAttempt::construction_attempts(),
                0,
            )
            .map_err(|_| self.failure(Cause::Geometry))?;
            access
                .validate_population(&bank, facts)
                .map_err(|cause| self.failure(Cause::Consumer(cause)))?;
            banks.push(bank);
        }
        Ok(IndexedConstructorPartitions {
            banks,
            funding: funding.clone(),
        })
    }
    fn validate_partitions(
        &self,
        access: &OriginalSelectedResidencyAccess,
        parts: &IndexedConstructorPartitions,
    ) -> Result<(), Error> {
        if parts.banks.len() != self.first.plan().len() {
            return Err(self.failure(Cause::Identity));
        }
        let facts = HostSourceConstructionFacts::new(
            self.constructor_bytes,
    ...
    pub(crate) fn constructor_facts(&self) -> Option<HostSourceConstructionFacts> {
        ...
        HostSourceConstructionFacts::new(
            ...
            calls.checked_mul(OriginalSelectedResidencyAttempt::construction_attempts())?,
            calls,
        )
        .ok()
    }
    ...
    pub(crate) fn read_source_matches(&self, series: &ForegroundDiskSourceSeries) -> bool {
        self.read
            .as_ref()
            .is_some_and(|source| series.matches_ceiling(source))
    }
    /// Accumulates only this actual source's finite sequential region facts.
    pub(crate) fn include_read_source(
        &self,
        series: &mut Option<ForegroundDiskSourceSeries>,
    ) -> Result<(), Error> {
        let Some(source) = &self.read else {
            return Ok(());
        };
        self.funding
            .reserve_metadata(
        ...
        match series {
            Some(series) => series
                .include(source, self.first.plan().len())
                .ok_or_else(|| self.failure(Cause::Identity))?,
            None => {
                *series = Some(
                    ForegroundDiskSourceSeries::new(source, self.first.plan().len())
                        .ok_or_else(|| self.failure(Cause::Overflow))?,
                )
            }
        }
    ...
    fn prepare_reads(
        &self,
        access: &OriginalSelectedResidencyAccess,
        reads: Option<OriginalHostSourceBank>,
    ) -> Result<Option<ForegroundDiskSourceCapacity>, Error> {
        match (&self.read, reads) {
            (Some(source), Some(bank)) => {
                if !source.matches_manager(&self.manager) {
                    return Err(self.failure(Cause::Identity));
                }
                access
                    .prepare_read_capacity(source, self.first.plan().len(), bank)
                    .map(Some)
                    .map_err(|cause| self.failure(Cause::Consumer(cause)))
            }
            (None, None) => Ok(None),
            _ => Err(self.failure(Cause::Identity)),
        }
    }
    fn validate_reads(
        &self,
        ...
        match (&self.read, reads) {
            (Some(source), Some(capacity)) if source.matches_manager(&self.manager) => access
                .validate_read_capacity(source, capacity)
                .map_err(|cause| self.failure(Cause::Consumer(cause))),
            (None, None) => Ok(()),
            _ => Err(self.failure(Cause::Identity)),
        }
    }
    pub(crate) fn requires_reads(&self) -> bool {
        self.read.is_some()
    }
    ...
    fn failure(&self, cause: Cause) -> Error {
        failed(cause, &self.bank, &self.funding, None)
    }
    /// Banks come from the accepted role; metadata funding creates no authority.
    pub(crate) fn prepare(
        self,
        access: OriginalSelectedResidencyAccess,
        constructors: OriginalHostSourceBank,
        reads: Option<OriginalHostSourceBank>,
    ...
    fn prepare_partitioned(
        ...
        self.manager
            ...
            .map_err(|e| self.failure(Cause::Source(e)))?;
        let selected = StreamCopyPlan::<CopyCustody>::capture(stream)
            .map_err(|_| self.failure(Cause::Identity))?;
        let bytes = OriginalIndexedResidencyInvocation::preparation_control_bytes(
            selected
                .control_bytes()
                .ok_or_else(|| self.failure(Cause::Overflow))?,
        )
        ...
        self.funding
            ...
            .map_err(|e| self.failure(Cause::Funding(e)))?;
        ...
        Ok(OriginalIndexedResidencyInvocation(Rc::new(Invocation {
            ...
            stream: selected,
            state: RefCell::new(InvocationState {
                constructors: constructors.banks.into_iter(),
                next: 0,
                failed: false,
                installed: false,
                closed: false,
                _constructor_funding: constructors.funding,
            }),
            capacity,
        })))
    }
...
impl IndexedBankSource {
    ...
    pub(crate) fn inspect_original_residency(
        &self,
        first: AddressableChunkCensus,
        pool: Option<&MemoryLedger>,
        funding: &HostMetadataFunding,
    ) -> Result<IndexedResidencyPlan, Error> {
        let binding = self;
        let fail = |cause| failed(cause, &binding.bank, funding, None);
        let controls = [
            size_of::<IndexedResidencyPlan>(),
            size_of::<Result<IndexedResidencyPlan, Error>>(),
            size_of::<(
                AddressableChunkCensus,
                &Self,
                Option<&MemoryLedger>,
            ...
            size_of::<[WindowPopulation; 2]>(),
            size_of::<Option<ForegroundDiskWindowPlan>>(),
            size_of::<Option<ForegroundDiskSubsetCeiling>>(),
            size_of::<[usize; 8]>(),
            size_of::<[u64; 3]>(),
            size_of::<[u8; ParameterBankKey::unit_id_buffer_bytes()]>(),
            AddressableChunkPlan::control_bytes(),
            OriginalIndexedResidencyInvocation::control_bytes()
        ...
        let (manager, source, eligible, revision) = binding
            .bank
            .with_workspace_source(funding, |bank| {
                ...
                let expected = AddressableChunkPlan::new(
                    ...
                    first.access(),
                    Some(maximum),
                    binding.options.prefill_compact_bank_target_bytes(),
                )
                .map_err(|_| fail(Cause::Geometry))?;
                if expected != first.plan() || !bank.same_source(&binding.bank) {
                    return Err(fail(Cause::Identity));
                }
        ...
        let source_bytes = OriginalResidencySource::constructor_storage_bytes(
            units,
            eligible
                .len()
                .checked_add(1)
        ...
        .and_then(|n| n.checked_add(source.projection_control_bytes()?))
        .ok_or_else(|| fail(Cause::Overflow))?;
        funding
            .reserve_metadata(source_bytes)
            .map_err(|e| fail(Cause::Funding(e)))?;
        let mut scratch = funding
            .metadata_vec(units)
            .map_err(|e| fail(Cause::Consumer(Error::Neural(e))))?;
        scratch.resize_with(units, ResidencyClosureSlot::default);
        let population = manager
            .selected_operation_ceiling(&source, &eligible, first.maximum_members(), &mut scratch)
            .map_err(|e| fail(Cause::Source(e)))?;
        let mut constructor_bytes =
            OriginalSelectedResidencyAttempt::construction_bytes(&source, population)
                .ok_or_else(|| fail(Cause::Overflow))?;
        let read = if source.foreground().is_some() {
            let pool = pool.ok_or_else(|| fail(Cause::Identity))?;
            let full = manager
                .selected_operation_population(&source, &eligible, &mut scratch)
                .map_err(|e| fail(Cause::Source(e)))?;
            let disk = ForegroundDiskWindowPlan::new_with_metadata(
                &manager,
                pool,
                full,
                &eligible,
            ...
            funding
                .reserve_metadata(
                    ForegroundDiskSubsetCeiling::control_bytes()
                        .ok_or_else(|| fail(Cause::Overflow))?,
                )
                .map_err(|e| fail(Cause::Funding(e)))?;
            let ceiling = disk
                .subset_ceiling(population.units)
                .ok_or_else(|| fail(Cause::Overflow))?;
            constructor_bytes = constructor_bytes
                .checked_add(ceiling.attempt_control_bytes())
                .ok_or_else(|| fail(Cause::Overflow))?;
            Some(ceiling)
        } else {
            manager
        ...
        Ok(IndexedResidencyPlan {
            binding: binding.clone(),
            bank: binding.bank.clone(),
            first,
            revision,
            native_window: population,
            source,
            manager,
            constructor_bytes,
            pool: read.as_ref().and(pool.cloned()),
            read,
            source_funding: funding.clone(),
            funding: funding.clone(),
        })
    }
}
struct InvocationState {
    ...
    installed: bool,
    closed: bool,
    _constructor_funding: HostMetadataFunding,
}
struct Invocation {
...
impl Retention for InvocationRetention {
...
impl OriginalIndexedResidencyInvocation {
    ...
    fn control_bytes() -> Option<usize> {
        let frames = [
            size_of::<Self>(),
            size_of::<Invocation>(),
            size_of::<InvocationState>(),
            size_of::<InvocationRetention>(),
            size_of::<Option<OriginalIndexedChunkSource>>(),
            size_of::<Option<Self>>(),
            size_of::<Result<Self, Error>>(),
            size_of::<std::cell::RefMut<'_, InvocationState>>(),
            size_of::<Result<(), Error>>(),
            size_of::<
                Result<OriginalHostSourceBank, eredu_runtime::working_memory::HostDestinationCause>,
            >(),
            size_of::<Result<OriginalIndexedChunkSource, Error>>(),
            OriginalSelectedResidencyAccess::control_bytes()?,
            Layout::new::<[usize; 2]>()
    ...
    fn fail(&self, cause: Cause) -> Error {
        self.0.plan.failure(cause)
    }
    fn validate(&self, movement: &MlxIndexedMovement, stream: &Stream) -> Result<(), Error> {
        self.0.access.validate_observer(&self.0.observer)?;
        if !self.0.stream.matches_source(stream) {
            return Err(self.fail(Cause::Identity));
        }
        let binding = movement
            .binding
            .as_ref()
            .ok_or_else(|| self.fail(Cause::Identity))?;
        if !binding.same_binding(&self.0.plan.binding) {
            return Err(self.fail(Cause::Identity));
        }
        binding
            .bank
            .with_workspace_source(&self.0.plan.funding, |source| {
                if !source.same_source(&self.0.plan.bank)
                    || source.parameter_revision() != self.0.plan.revision
                {
                    return Err(self.fail(Cause::Identity));
                }
                Ok(())
    ...
    pub(in crate::backend::runtime::residency::parameter_bank::movement) fn reserve_parent_adapter(
        ...
        let state = self
            ...
            .state
            .try_borrow()
            .map_err(|_| self.fail(Cause::Spent))?;
        if !state.installed || state.closed || state.failed {
            return Err(self.fail(Cause::Spent));
        }
        let frames = [
            size_of::<(&Self, &MlxIndexedMovement, &Stream, Option<usize>, usize)>(),
            size_of::<std::cell::Ref<'_, InvocationState>>(),
            size_of::<Result<(), Error>>(),
            size_of::<Result<MlxTensor, eredu_nn::Error>>(),
            size_of::<Result<MlxTensor, Error>>(),
            adapter.ok_or_else(|| self.fail(Cause::Overflow))?,
            arguments,
        ];
        let bytes = frames
            .into_iter()
        ...
        self.0
            ...
            .funding
            .reserve_metadata(bytes)
            .map_err(|cause| self.fail(Cause::Funding(cause)))
    }
    ...
    pub(in crate::backend::runtime::residency::parameter_bank::movement) fn begin_chunk(
        ...
        let mut state = self
            ...
            .try_borrow_mut()
            .map_err(|_| self.fail(Cause::Spent))?;
        if state.failed || state.closed || !state.installed {
            return Err(self.fail(Cause::Spent));
        }
        let expected = AddressableChunkCensus::new(
            p.first.bank(),
            p.first.unit(),
            p.first.plan(),
            state.next,
        ...
        if expected != Some(census) {
            state.failed = true;
            return Err(self.fail(Cause::Identity));
        }
        if let Some(previous) = movement.original.as_ref() {
            if !previous.body().completed.get() {
                state.failed = true;
                return Err(self.fail(Cause::Spent));
            }
        }
        if let Some(previous) = movement.original.take() {
            previous.body().closed.set(true);
            drop(previous);
        }
        state.next = state
            .next
        ...
        drop(state);
        let result = (|| {
            let source = movement.prepare_original_chunk(
                census,
                indices,
                &self.0.runtime,
                &self.0.observer,
            ...
            source.bind_selected_residency_source(
                self.0.access.clone(),
                child_bank,
                p.pool
                    .as_ref()
                    .zip(self.0.capacity.as_ref())
                    .map(|(pool, capacity)| (pool.clone(), capacity.clone())),
                stream,
            )?;
            Ok(source)
        })();
        match result {
            Ok(source) => {
                movement.original = Some(source);
                Ok(())
            }
            Err(cause) => {
                self.0.state.borrow_mut().failed = true;
                Err(cause)
            }
        }
    }
...
impl<P> Drop for InvocationGuard<'_, P> {
    fn drop(&mut self) {
        ...
        if let Some(child) = &child {
            child.body().closed.set(true);
        }
        if let Some(recovery) = self.recovery.as_mut() {
            recovery.retention_mut().child = child;
        }
        movement.invocation.take();
        if let Ok(mut state) = self.invocation.0.state.try_borrow_mut() {
            state.closed = true;
        }
...
impl OriginalIndexedResidencyInvocation {
    pub(in crate::backend::runtime::residency::parameter_bank::movement) fn matches_funding(
        ...
        self.0.plan.funding.same_account(funding)
    }
    ...
    pub(in crate::backend::runtime::residency::parameter_bank::movement) fn owner_control_bytes<
        P,
        R,
        E,
        F,
    >() -> Option<usize> {
        let frames = [
            ...
            size_of::<InvocationGuard<'_, P>>(),
            size_of::<(&mut P, fn(&mut P) -> &mut MlxIndexedMovement)>(),
            size_of::<Option<PendingInvocation>>(),
            size_of::<std::cell::RefMut<'_, Option<PendingInvocation>>>(),
            Self::control_bytes()?,
        ];
        frames
            .into_iter()
        ...
        F: FnOnce(&mut P) -> Result<R, E>,
    {
        ...
        invocation
            ...
            .reserve_metadata(
                Self::owner_control_bytes::<P, R, E, F>()
                    .ok_or_else(|| invocation.fail(Cause::Overflow))?,
            )
            .map_err(|e| invocation.fail(Cause::Funding(e)))?;
        if movement(owner).original.is_some() || movement(owner).invocation.is_some() {
            return Err(invocation.fail(Cause::Spent));
        }
        {
            let mut state = invocation
                ...
                .state
                .try_borrow_mut()
                .map_err(|_| invocation.fail(Cause::Spent))?;
            if state.installed || state.closed || state.failed {
                return Err(invocation.fail(Cause::Spent));
            }
        ...
        let pending = invocation
            ...
            .map_err(|_| invocation.fail(Cause::Spent))?
            .take()
            .ok_or_else(|| invocation.fail(Cause::Spent))?;
        let recovery = pending.activate(
            InvocationRetention {
                child: None,
                invocation: invocation.clone(),
            },
            invocation.0.observer.clone(),
        );
        ...
        let complete =
            invocation.0.state.try_borrow().is_ok_and(|state| {
                !state.failed && state.next == invocation.0.plan.first.plan().len()
            }) && (guard.movement)(guard.owner)
                .original
                .as_ref()
                .is_some_and(|source| source.body().completed.get());
        drop(guard);
        match result {
...
impl MlxIndexedMovement {
    ...
    pub(crate) fn with_original_residency_invocation<T, F>(
        &mut self,
        invocation: OriginalIndexedResidencyInvocation,
        stream: &Stream,
        run: F,
...
impl MlxIndexedMovement {
    ...
    pub(crate) fn inspect_original_residency(
        ...
        if self.original.is_some() || self.invocation.is_some() {
            return Err(failed(Cause::Spent, &binding.bank, funding, None));
        }
...
impl IndexedResidencyPlan {
    ...
    pub(crate) fn prepare_factory_partitioned(
        self,
        access: OriginalSelectedResidencyAccess,
        constructors: IndexedConstructorPartitions,
        reads: Option<OriginalHostSourceBank>,
    ...
    pub(crate) fn prepare_factory_shared(
        self,
        access: OriginalSelectedResidencyAccess,
        constructors: IndexedConstructorPartitions,
        reads: Option<ForegroundDiskSourceCapacity>,
        ...
        let source = StreamCopyPlan::<CopyCustody>::capture(stream)
            .map_err(|_| self.failure(Cause::Identity))?;
        let bytes = source
            .control_bytes()
            .and_then(Self::factory_control_bytes)
            .ok_or_else(|| self.failure(Cause::Overflow))?;
        self.funding
            .reserve_metadata(bytes)
            .map_err(|e| self.failure(Cause::Funding(e)))?;
        ...
        Ok(OriginalIndexedResidencyFactory {
            plan: self,
            access,
            constructors,
            reads,
...
impl OriginalIndexedResidencyFactory {
    ...
    fn request_control_bytes() -> usize {
        size_of::<(
            &Self,
            &eredu_runtime::expert::IndexedInvocationRequest<'_, MlxTensor>,
    ...
    pub(in crate::backend::runtime::residency::parameter_bank::movement) fn validate_request(
        ...
        self.plan
            .funding
            .reserve_metadata(Self::request_control_bytes())
    ...
    pub(in crate::backend::runtime::residency::parameter_bank::movement) fn matches_funding(
        ...
        self.plan.funding.same_account(funding)
    }
    pub(in crate::backend::runtime::residency::parameter_bank::movement) fn prepare(
        ...
        binding
            .bank
            .with_workspace_source(&self.plan.funding, |source| {
                let first = self.plan.first;
                ...
                .map_err(|_| self.plan.failure(Cause::Geometry))?;
                if !source.same_source(&self.plan.bank)
                    || source.parameter_revision() != self.plan.revision
                    || plan != first.plan()
                {
                    return Err(self.plan.failure(Cause::Identity));
                }
