...
use crate::backend::runtime::execution::generic::{
    OriginalOperationAccess, OriginalResidencyAttempt, OriginalSelectedResidencyAccess,
    OriginalSelectedResidencyAttempt,
};
use eredu_nn::Tensor;
...
pub(super) trait IndexedResidency {
    fn acquire(
        &mut self,
        source: &OriginalIndexedChunkSource,
...
impl<U: 'static> IndexedResidency for RegisteredResidency<U> {
    fn acquire(
        ...
        self.access
            .with_residency(&mut self.attempt, |slots, observer| {
                if !observer.same_scope(&source.body().observer) {
...
impl OriginalIndexedChunkSource {
    ...
    pub(crate) fn bind_residency<U: 'static>(
        &self,
        access: OriginalOperationAccess<U>,
        attempt: OriginalResidencyAttempt,
        stream: &Stream,
        ...
        let b = self.body();
        access
            .validate_observer(&b.observer)
            .map_err(|cause| self.failure(Cause::Consumer(cause)))?;
        let frames = [
            size_of::<RegisteredResidency<U>>(),
            size_of::<Box<RegisteredResidency<U>>>(),
            size_of::<Box<dyn IndexedResidency>>(),
        ...
        let mut slot = b
            .residency
            .try_borrow_mut()
            .map_err(|_| self.failure(Cause::Spent))?;
        if slot.is_some() || b.discover_attempted.get() {
    ...
    fn validate_demand(&self, demands: &IndexedDemandSource) -> Result<(), Error> {
        let b = self.body();
        let completed = demands
            .source()
            .and_then(|source| source.downcast_ref::<CompletedIds>())
        ...
        if !completed.source.same_owner(&b.identity)
            ...
            || !demands
                .funding()
                .is_some_and(|funding| funding.same_account(&b.funding))
        {
            return Err(self.failure(Cause::Identity));
        }
        Ok(())
    }
    pub(crate) fn with_demand_loan<R, E, F>(
        &self,
        demands: &IndexedDemandSource,
        run: F,
        ...
        F: FnOnce(Option<PreparedIndexedDemandLoan<'_>>) -> Result<R, E>,
    {
        ...
        let frames = [
            ...
            size_of::<PreparedIndexedDemandLoan<'_>>(),
    ...
    pub(crate) fn acquire_from_demand(
        &self,
        ...
        self.validate_demand(demands)?;
        let b = self.body();
        if request.access() != b.identity.census.access()
            || b.route_copies.get() != 2
            || !funding.same_account(&b.funding)
        {
            return Err(self.failure(Cause::Identity));
        }
        let mut source = b
    ...
    pub(crate) fn copy_route_value(
        ...
        let b = self.body();
        let frames = [
            size_of::<(&MlxTensor, &IndexedDemandSource, &Stream)>(),
        ...
        b.funding
            ...
            .map_err(|cause| self.failure(Cause::Funding(cause)))?;
        if b.route_copies.get() >= 2
            || b.acquire_attempted.get()
            || value.shape()
                != [
                    i32::try_from(b.identity.census.rows())
                        .map_err(|_| self.failure(Cause::Geometry))?,
                    i32::try_from(b.identity.census.routes())
                        .map_err(|_| self.failure(Cause::Geometry))?,
                ]
        {
...
impl IndexedResidency for SelectedResidency {
    fn acquire(
        ...
        if self.attempt.is_some() {
            return Err(source.failure(Cause::Spent));
        }
        let b = source.body();
        let frames = [
            size_of::<Vec<OffloadUnitId>>(),
            size_of::<OriginalSelectedResidencyAttempt>(),
            ...
            size_of::<(ParameterBankKey, u64)>(),
            size_of::<[u8; ParameterBankKey::unit_id_buffer_bytes()]>(),
            Layout::array::<OffloadUnitId>(entries.len())
                .map_err(|_| source.failure(Cause::Overflow))?
                .size(),
        ];
        let bytes = entries
            .iter()
            .try_fold(0usize, |bytes, (key, _)| {
                bytes.checked_add(key.unit_id_length())
            })
            .and_then(|bytes| {
                frames
                    .into_iter()
                    .try_fold(bytes.checked_add(size_of_val(&frames))?, usize::checked_add)
            })
            .ok_or_else(|| source.failure(Cause::Overflow))?;
        b.funding
            .reserve_metadata(bytes)
            .map_err(|cause| source.failure(Cause::Funding(cause)))?;
        let (manager, selected) = b
            .identity
            .bank
            .with_workspace_source(&b.funding, |actual| {
                if !actual.same_source(&b.identity.bank)
                    || actual.parameter_revision() != b.identity.parameter_revision
                {
                    return Err(source.failure(Cause::Identity));
                }
                ...
                let selected = manager
                    ...
                    .ok_or_else(|| source.failure(Cause::Identity))?;
        ...
        roots
            ...
            .map_err(|cause| source.failure(Cause::Allocation(cause)))?;
        for (key, _) in entries {
            roots.push(key.unit_id());
        }
        self.attempt = Some(
            self.access
                .prepare(
                    ...
                    self.disk
                        .as_ref()
                        .map(|(pool, capacity)| (pool, capacity, &b.funding)),
                )
                .map_err(|cause| source.failure(Cause::Consumer(cause)))?,
        );
        self.access.with_residency(
            self.attempt
                .as_mut()
                .ok_or_else(|| source.failure(Cause::Spent))?,
            |slots, observer| {
                if !observer.same_scope(&b.observer) {
                    return Err(source.failure(Cause::Identity));
                }
                source.acquire_from_slots(demands, entries, slots, stream)
            },
...
impl OriginalIndexedChunkSource {
    ...
    pub(crate) fn bind_selected_residency<U: 'static>(
        &self,
        access: OriginalOperationAccess<U>,
        bank: eredu_runtime::working_memory::OriginalHostSourceBank,
        stream: &Stream,
    ) -> Result<(), Error> {
        self.bind_selected_residency_source(
            access
    ...
    pub(crate) fn bind_selected_disk_residency<U: 'static>(
        &self,
        access: OriginalOperationAccess<U>,
        bank: eredu_runtime::working_memory::OriginalHostSourceBank,
    ...
    pub(crate) fn bind_selected_residency_source(
        &self,
        access: OriginalSelectedResidencyAccess,
        bank: eredu_runtime::working_memory::OriginalHostSourceBank,
        disk: Option<(
        ...
        access
            ...
            .map_err(|cause| self.failure(Cause::Consumer(cause)))?;
        let frames = [
            size_of::<SelectedResidency>(),
            Layout::new::<SelectedResidency>().size(),
            size_of::<Box<SelectedResidency>>(),
            size_of::<Box<dyn IndexedResidency>>(),
            size_of::<Option<Box<dyn IndexedResidency>>>(),
        ...
        *slot = Some(Box::new(SelectedResidency {
            attempt: None,
            disk,
            bank,
            access,
