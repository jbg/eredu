...
enum Cause {
    #[error("background forward order or completion boundary mismatch")]
    Order,
...
impl Drop for Step<'_> {
    fn drop(&mut self) {
        if self.source.is_some() { self.failed.set(true); }
    }
}
impl PreparedBackgroundForward {
    ...
    pub(crate) fn from_prepared(
        ...
        Self { reads: Some(reads), windows, startup }
    }
}
impl BackgroundHostCoordinator {
    ...
    pub(crate) fn control_bytes() -> Option<usize> {
        let fixed = [
            ...
            size_of::<Result<eredu_runtime::working_memory::HostThreadStartup, eredu_runtime::working_memory::WorkingMemoryError>>(),
    ...
    pub(crate) fn from_prepared(
        ...
        let bytes = Self::control_bytes().ok_or(Error::WorkspacePlanning(eredu_core::HostMetadataFundingError::Overflow))?;
        funding.reserve_metadata(bytes).map_err(Error::WorkspacePlanning)?;
        let value = Self { forwards, active: None, next_forward: 0, next_window: 0,
            failed: Cell::new(false), failure_issued: Cell::new(false), custody, funding };
        if value.forwards.iter().any(|row| row.as_ref().is_none_or(|forward|
            forward.reads.is_none() || forward.windows.iter().any(|(_, window)| window.is_none()))) {
            return Err(value.fail(Cause::Order));
        }
        Ok(value)
    }
    fn error(
        ...
        if issued.replace(true) { return Error::PrefillScopeUnavailable; }
        Error::Residency(ResidencyError::OriginalBackgroundCoordinator(Box::new(BackgroundCoordinatorFailure {
            cause, custody: custody.clone(), funding: funding.clone(),
        })))
    }
    ...
    pub(crate) fn close(&self) {
        ...
        if let Some((service, _)) = &self.active { service.close(); }
    }
    fn begin(&mut self, reservation: Option<&WorkingMemoryReservation>) -> Result<(), Error> {
        if self.failed.get() || self.active.is_some() { return Err(self.fail(Cause::Order)); }
        self.custody.validate_account(reservation).map_err(|cause| self.fail(cause.into()))?;
        let mut forward = match self.forwards.get_mut(self.next_forward).and_then(Option::take) {
            Some(value) => value,
            None => return Err(self.fail(Cause::Order)),
        };
        let startup = forward.startup.prepare_for_source(&self.custody, reservation)
            .map_err(|cause| self.fail(cause.into()))?;
        let reads = forward.reads.take().ok_or_else(|| self.fail(Cause::Order))?;
    ...
    where F: FnOnce(&BackgroundHostReadService, PreparedBackgroundHostWindow) -> Result<T, Error> {
        let controls = [size_of::<F>(), size_of::<T>(), size_of::<Result<T, Error>>(),
            size_of::<Option<usize>>(),
            size_of::<(&mut Self, usize, Option<&WorkingMemoryReservation>, F)>()];
        if self.failed.get() { return Err(Error::PrefillScopeUnavailable); }
        let bytes = match controls.into_iter().try_fold(size_of_val(&controls), usize::checked_add) {
            Some(value) => value,
            None => { self.close(); return Err(Error::WorkspacePlanning(eredu_core::HostMetadataFundingError::Overflow)); }
        };
        ...
        if self.active.is_none() {
            let first = self.forwards.get(self.next_forward).and_then(Option::as_ref)
                .and_then(|forward| forward.windows.first()).map(|(ordinal, _)| *ordinal);
            if first != Some(index) { return Err(self.fail(Cause::Order)); }
            self.begin(reservation)?;
        }
        let next = self.next_window.checked_add(1).ok_or_else(|| self.fail(Cause::Order))?;
        ...
        let window = match forward.windows.get_mut(self.next_window) {
            Some((ordinal, window)) if *ordinal == index => match window.take() {
                Some(value) => value,
                None => return Err(fail(Cause::Order)),
            },
            _ => return Err(fail(Cause::Order)),
        };
        let source = match service.attempt() {
            Ok(value) => value,
            Err(cause) => return Err(fail(cause.into())),
        };
        let step = Step { failed: &self.failed, source: Some(source) };
    ...
    pub(crate) fn finish_forward(&mut self) -> Result<BackgroundPrefetchReport, Error> {
        if self.failed.get() { return Err(self.fail(Cause::Order)); }
        ...
        if self.active.is_none() && self.forwards.get(self.next_forward)
            .and_then(Option::as_ref).is_some_and(|forward| forward.windows.is_empty()) {
            let forward = self.forwards[self.next_forward].take().expect("selected empty forward");
            self.next_forward = self.next_forward.checked_add(1).ok_or_else(|| self.fail(Cause::Order))?;
            drop(forward);
            return Ok(BackgroundPrefetchReport::default());
        }
        let complete = self.active.as_ref().is_some_and(|(_, forward)|
            self.next_window == forward.windows.len() && forward.windows.iter().all(|(_, window)| window.is_none()));
        if !complete { return Err(self.fail(Cause::Order)); }
