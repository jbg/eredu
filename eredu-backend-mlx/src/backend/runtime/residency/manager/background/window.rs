...
impl std::fmt::Debug for BackgroundHostWindowFailure {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BackgroundHostWindowFailure")
            ...
            .field("prefetch_units", &self.retained.prefetch.len())
            .finish()
    }
}
impl PreparedBackgroundHostWindow {
    pub(crate) fn control_bytes(
        ...
        let fixed = [
            ...
            Layout::array::<OffloadUnitId>(host.read_units().len())
                .ok()?
                .size(),
            ...
            size_of::<WindowCause>(),
            size_of::<BackgroundHostWindowFailure>(),
        ...
        for id in host.read_units() {
            bytes = bytes.checked_add(id.as_str().len())?;
        }
        bytes
            .checked_add(PreparedHostPublication::host_bytes(device)?)?
    ...
    pub(crate) fn prepare(
        ...
        if !host.matches_manager(manager)
            || !device.matches_manager(manager)
        ...
        funding
            ...
            .map_err(|cause| fail(cause.into()))?;
        let publication =
            PreparedHostPublication::prepare(manager, device, custody.clone(), funding.clone())?;
        let protection = PreparedHostProtection::prepare(
            manager,
            group,
            active,
            custody.clone(),
        ...
        prefetch
            .try_reserve_exact(host.read_units().len())
            .map_err(|cause| fail(cause.into()))?;
        for id in host.read_units() {
            prefetch.push(id.clone());
        }
        Ok(Self {
    ...
    fn fail(self, cause: WindowCause) -> ResidencyError {
        ResidencyError::OriginalHostWindow(Box::new(BackgroundHostWindowFailure {
            cause,
            retained: self,
        }))
    }
    fn submit(
        ...
        for id in &self.prefetch {
            let resident = {
                ...
                if !self.source.units().any(|unit| {
                    state
                        .control
                        .unit(id)
                        .is_some_and(|current| current == unit)
                }) {
                    return Err(ResidencyError::OriginalOperationDomain.into());
                }
                state
                    .control
                    .ledger()
                    .is_resident(id, MemoryTier::Host)
    ...
    pub(crate) fn promote(
        ...
        let attempt = match reads.attempt() {
            Ok(value) => value,
            Err(cause) => return Err(self.fail(cause.into())),
        };
        let outcome = (|| -> Result<ResidentTransfer, WindowCause> {
            ...
            let retained_host = {
                ...
                slots.background_host = Some(OriginalHostPublicationSlots {
                    publication: &mut self.publication,
                    reads,
                });
                manager.acquire_many_with_original_transfer(
                    requests
                        .get(..self.host_requested)
                        .ok_or(ResidencyError::OriginalOperationDomain)?,
                    MemoryTier::Host,
                    &mut slots,
                    observer,
