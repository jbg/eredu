...
impl SharedNativeInitializer for Constructor {
    ...
    fn required_storage_bytes(&self) -> Result<usize, WorkingMemoryError> {
        MlxParameterExclusions::construction_bytes(&self.names)?
            .checked_add(
                usize::try_from(ManagerCustody::storage_bytes()?)
    ...
    fn initialize(
        ...
        Ok(MlxParameterExclusions::construct(
            &self.names,
            ManagerCustody::new(custody),
        ))
    }
}
fn constructor(calls: &Arc<AtomicUsize>) -> Constructor {
    Constructor {
        names: vec!["a.independent.bank".into(), "z.shared.bank".into()],
        calls: calls.clone(),
    }
}
...
fn exact_exclusion_source_refuses_before_birth_and_keeps_escaped_account() {
    ...
    let bytes =
        MemoryLedger::shared_native_initialization_required_bytes(&constructor(&calls)).unwrap();
    let short = crate::memory_fixture::ledger(bytes - 1, 0).unwrap();
    let failure = short
        .initialize_shared_native(constructor(&calls))
        .unwrap_err();
    assert!(matches!(
    ...
    assert_eq!(
        native.unfunded_retained_bytes(),
        Some(0),
        "source backing is not charged again by each quote"
    );
    let foreign = ["a.independent.bank".to_owned(), "z.foreign.bank".to_owned()].into();
    assert!(matches!(
        native.for_selection(&foreign),
    ...
    assert_eq!(
        ...
        bytes,
        "cold alias outlives native owner without releasing its payer"
    );
...
fn unpriced_nonempty_exclusions_cannot_qualify_an_original_binding() {
    ...
    assert!(matches!(
        ordinary.for_selection(&["independent.bank".to_owned()].into()),
        Err(WorkingMemoryError::IdentityMismatch)
    ));
