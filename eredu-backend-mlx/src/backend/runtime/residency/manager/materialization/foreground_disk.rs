...
impl DiskInput {
    fn prepared(&self) -> &PreparedForegroundDiskRead {
        match self { Self::Deferred(value) => value, Self::Completed(value) => value.prepared() }
    }
    fn into_host(self) -> Result<ResidentHostOwner, ForegroundDiskReadError> {
        match self { Self::Deferred(value) => value.read()?.into_host(), Self::Completed(value) => value.into_host() }
    }
}
...
pub(in crate::backend::runtime::residency::manager) fn prepare_foreground_disk_into(
    ...
    prepare_disk_host_into(DiskInput::Deferred(batch), manager, reservation, observer, stream, arrays, retained)
}
...
pub(in crate::backend::runtime::residency::manager) fn prepare_background_disk_into(
    ...
    prepare_disk_host_into(DiskInput::Completed(batch), manager, reservation, observer, stream, arrays, retained)
}
fn prepare_disk_host_into(
    ...
    let result = (|| -> Result<(), Cause> {
        ...
        if !retained.metadata_custody().is_some_and(|actual| actual.same_account(&custody.metadata_custody())) {
