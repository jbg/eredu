...
use eredu_runtime::{
    ...
    working_memory::{
        OriginalHostSourceCustody, WorkingMemoryError, WorkingMemoryPool, WorkingMemoryReservation,
    },
};
use std::{
    alloc::Layout,
...
impl ForegroundDiskWindowPlan {
    pub(crate) fn new(
        manager: &ResidencyManager, pool: &WorkingMemoryPool, window: WindowPopulation,
        ids: &[OffloadUnitId], scratch: &mut [ResidencyClosureSlot],
    ) -> Result<Self, Error> {
    ...
    pub(crate) fn new_with_metadata(
        ...
        if let Some(funding) = funding {
            let bytes = [
                Layout::array::<UnitRead>(window.units).map_err(|_| overflow())?.size(),
                size_of::<Self>(), size_of::<Result<Self, Error>>(), size_of::<Vec<UnitRead>>(),
                size_of::<UnitRead>(), size_of::<ForegroundDiskReadLayout>(),
                size_of::<Result<ForegroundDiskReadLayout, WorkingMemoryError>>(),
                size_of::<std::collections::TryReserveError>(), size_of::<Box<std::collections::TryReserveError>>(),
                size_of::<WindowPopulation>(), size_of::<Result<(), ResidencyError>>(),
            ];
            funding.reserve_metadata(bytes.into_iter().try_fold(size_of_val(&bytes), usize::checked_add).ok_or_else(overflow)?)
    ...
    fn background_population(windows: &[Self], funding: &eredu_nn::workspace::HostMetadataFunding, repeated: bool) -> Result<Self, Error> {
        ...
        for (index, window) in windows.iter().enumerate() {
            if !source.same_source(&window.source) { return Err(identity()); }
            for row in &window.units {
                let previous = windows[..index].iter().flat_map(|part| &part.units)
                    .find(|old| old.ordinal == row.ordinal);
                if let Some(old) = previous {
                    if !same_layout(old.layout, row.layout) { return Err(identity()); }
                } else { count = count.checked_add(1).ok_or_else(overflow)?; }
            }
        }
        let controls = [
            Layout::array::<UnitRead>(count).map_err(|_| overflow())?.size(),
            size_of::<Self>(), size_of::<Result<Self, Error>>(),
            size_of::<Vec<UnitRead>>(), size_of::<UnitRead>(),
        ...
        funding.reserve_metadata(controls.into_iter()
            .try_fold(size_of_val(&controls), usize::checked_add).ok_or_else(overflow)?)
            .map_err(Error::WorkspacePlanning)?;
        let mut units = Vec::new();
        units.try_reserve_exact(count).map_err(|_| Error::PrefillControl(WorkingMemoryError::UnknownBound))?;
        for window in windows {
            for row in &window.units {
                if !units.iter().any(|old: &UnitRead| old.ordinal == row.ordinal) {
                    units.push(*row);
                }
            }
        }
        ...
        for row in &mut units {
            row.attempts = if repeated {
                windows.iter().try_fold(0usize, |count, window| {
                    count.checked_add(usize::from(window.units.iter().any(|candidate| candidate.ordinal == row.ordinal)))
                }).ok_or_else(overflow)?
            } else { 1 };
        }
        Ok(Self { units, source: source.clone(), background_repeated: repeated })
    }
    ...
    pub(crate) fn difference(&self, covered: &Self, funding: &eredu_nn::workspace::HostMetadataFunding) -> Result<Self, Error> {
        ...
        for row in self.units.iter().filter(selected) { units.push(*row); }
        Ok(Self { units, source: self.source.clone(), background_repeated: false })
    }
    pub(in crate::backend::runtime::residency::manager) fn permits_window_rearm(&self) -> bool { self.background_repeated }
    ...
    pub(crate) fn single_read_population(&self) -> Option<ForegroundDiskPopulation> {
        self.units.iter().try_fold(ForegroundDiskPopulation::default(), |sum, row| {
            sum.checked_add(ForegroundDiskPopulation { attempts: 1, outputs: row.layout.outputs,
                output_logical_bytes: row.layout.output_logical_bytes, source_backing_bytes: row.layout.source_backing_bytes,
    ...
    pub(in crate::backend::runtime::residency::manager) fn read_layout(
        &self, id: &OffloadUnitId,
    ) -> Option<ForegroundDiskReadLayout> {
        self.units.iter().find(|row| {
            self.source.units().nth(row.ordinal).is_some_and(|unit| unit.id() == id)
        }).map(|row| row.layout)
    }
    pub(in crate::backend::runtime::residency::manager) fn source(&self) -> &ForegroundDiskDescriptors {
        &self.source
    }
    pub(crate) fn read_units(&self) -> impl ExactSizeIterator<Item = &OffloadUnitId> {
        self.units.iter().map(|row| {
            self.source.units().nth(row.ordinal).expect("validated source ordinal").id()
        })
    }
    pub(crate) fn matches_manager(&self, manager: &ResidencyManager) -> bool {
        manager.original_foreground_disk_descriptors()
    ...
    pub(crate) fn source_control_bytes(&self) -> Option<u64> {
        self.units
            ...
            .try_fold(0u64, |sum, row| {
                sum.checked_add(u64::try_from(row.layout.source_control_bytes).ok()?.checked_mul(u64::try_from(row.attempts).ok()?)?)
            })
    }
    ...
    pub(crate) fn attempt_control_bytes(&self) -> Option<u64> {
        self.units.iter().try_fold(Self::attempt_fixed_control_bytes(self.units.len())?,
    ...
    pub(crate) fn prepare_source(
        ...
        if !custody
            .metadata_custody()
            .matches_domain(pool.shared_storage_domain())
        {
        ...
        if !custody.same_source(capacity.custody()) {
            return Err(ForegroundDiskSlotError { cause: Cause::Memory(WorkingMemoryError::IdentityMismatch), _custody: custody });
        }
        capacity
            .validate_account(reservation)
            .map_err(|cause| ForegroundDiskSlotError {
...
fn read_factory<'a>(
    ...
    pool: Option<&'a WorkingMemoryPool>,
    row: UnitRead,
...
impl ForegroundDiskWindowPlan {
    ...
    pub(crate) fn subset_ceiling(&self,maximum_units:usize)->Option<ForegroundDiskSubsetCeiling> {
        ...
        for row in &self.units {
            let one=ForegroundDiskPopulation{attempts:1,outputs:row.layout.outputs,
                output_logical_bytes:row.layout.output_logical_bytes,source_backing_bytes:row.layout.source_backing_bytes,
                maximum_rank:row.layout.maximum_rank}.checked_mul(row.attempts)?;
            ...
            maximum_source=maximum_source.max(u64::try_from(row.layout.source_control_bytes).ok()?
        ...
        let population=ForegroundDiskPopulation{
            attempts:full.attempts.min(upper.attempts),outputs:full.outputs.min(upper.outputs),
        ...
        Some(ForegroundDiskSubsetCeiling{source:self.source.clone(),population,
            source_controls:self.source_control_bytes()?.min(maximum_source.checked_mul(count)?),
...
impl ForegroundDiskSubsetCeiling {
    ...
    pub(crate) fn control_bytes()->Option<usize> {
        let frames=[size_of::<Self>(),size_of::<Option<Self>>(),
            size_of::<(&ForegroundDiskWindowPlan,usize)>(),
            size_of::<[ForegroundDiskPopulation;4]>(),size_of::<[u64;5]>(),
            size_of::<std::slice::Iter<'_,UnitRead>>(),size_of::<UnitRead>(),
            ...
            size_of::<OriginalHostSourceCustody>(),size_of::<Option<&WorkingMemoryReservation>>(),
            size_of::<Result<ForegroundDiskSourceCapacity,WorkingMemoryError>>(),
            size_of::<Result<(),WorkingMemoryError>>(),
            ForegroundDiskSourceCapacity::control_bytes()?];
        frames.into_iter().try_fold(size_of_val(&frames),usize::checked_add)
    }
    pub(crate) fn population(&self)->ForegroundDiskPopulation {self.population}
    pub(crate) fn attempt_control_bytes(&self)->u64 {self.attempt_controls}
    pub(crate) fn matches_manager(&self,manager:&ResidencyManager)->bool {
        manager.original_foreground_disk_descriptors().is_some_and(|source|self.source.same_source(source))
    }
    ...
    pub(crate) fn source_facts(&self,calls:usize)->Option<eredu_runtime::working_memory::HostSourceConstructionFacts> {
        let population=self.population.checked_mul(calls)?;
        let controls=self.source_controls.checked_mul(u64::try_from(calls).ok()?)?;
        let backing=if calls==0 {0}else{self.population.source_backing_bytes};
        eredu_runtime::working_memory::HostSourceConstructionFacts::new(controls,population.outputs,population.attempts)
            .ok()?.with_peak_backing(self.source.peak_selection(u64::try_from(backing).ok()?)).ok()
    }
    ...
    pub(crate) fn prepare_capacity(&self,calls:usize,bank:eredu_runtime::working_memory::OriginalHostSourceBank,
        custody:OriginalHostSourceCustody,reservation:Option<&WorkingMemoryReservation>,
    )->Result<ForegroundDiskSourceCapacity,WorkingMemoryError> {
        let facts=self.source_facts(calls).ok_or(WorkingMemoryError::Overflow)?;
        if !bank.belongs_to_source(&custody) || !bank.matches_facts(facts){return Err(WorkingMemoryError::IdentityMismatch);}
        let backing=if calls==0 {0}else{self.population.source_backing_bytes};
        ForegroundDiskSourceCapacity::with_source_account(self.source.peak_selection(
