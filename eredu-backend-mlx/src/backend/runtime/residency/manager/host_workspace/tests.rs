...
#[test]
fn failed_manager_and_unselected_device_do_not_receive_copy_facts() {
    ...
    manager
        .host_copy_workspace(&[id("owner")], facts())
        .unwrap();
    let (_dir, _store, cpu) = fixture(false, DeviceType::Cpu);
    drop(cpu.acquire(&id("owner"), MemoryTier::Host).unwrap());
    assert_unknown(cpu.host_copy_workspace(&[id("owner")], facts()));
}
...
fn retained_source_guard_keeps_exact_owners_and_defers_final_unpin() {
    ...
    let pins = snapshot.pin_sources_impl(&manager, true, None).unwrap();
    assert_eq!(
        host_pin_counts(&manager, &ids),
    ...
    assert!(matches!(
        snapshot.pin_sources_impl(&manager, true, None),
        Err(HostCopyWorkspaceError::Unproved {
            source: WorkingMemoryError::IdentityMismatch,
            ..
        })
    ...
    drop(replaced);
    assert_eq!(store.source_diagnostics().unwrap(), reads);
    drop(snapshot.pin_sources_impl(&manager, true, None).unwrap());
    crate::backend::ordinary_retirement::reclaim_all();
    assert_eq!(host_pin_counts(&manager, &ids), before);
}
