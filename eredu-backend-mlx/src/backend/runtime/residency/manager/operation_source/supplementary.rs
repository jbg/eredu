    ...
    pub(crate) fn selected_operation_population(
        ...
        if roots.is_empty()
            || roots
                .iter()
                .enumerate()
                .any(|(index, id)| !source.ids().contains(id) || roots[..index].contains(id))
        {
            return Err(OperationSourceFailure::Layout);
        }
...
impl ResidencyManager {
    ...
    pub(crate) fn selected_operation_ceiling(
        &self,
        source: &SupplementaryResidencySource,
        eligible: &[OffloadUnitId],
        maximum_requested: usize,
        scratch: &mut [ResidencyClosureSlot],
    ) -> Result<WindowPopulation, OperationSourceFailure> {
        self.validate_supplementary_source(source)?;
        if maximum_requested == 0
            || maximum_requested > eligible.len()
            || eligible
                .iter()
                .enumerate()
                .any(|(index, id)| !source.ids().contains(id) || eligible[..index].contains(id))
        {
            return Err(OperationSourceFailure::Layout);
        }
        ...
        maximum
            .subset_ceiling(full, maximum_requested)
...
impl WindowPopulation {
    fn component_maximum(&mut self, other: Self) {
        ...
        self.alias_owner_name_bytes = self
        ...
        self.recipe_materializations = self
            .recipe_materializations
            .max(other.recipe_materializations);
        self.recipe_depth = self.recipe_depth.max(other.recipe_depth);
        self.declaration_clone_bytes = self
            .declaration_clone_bytes
            .max(other.declaration_clone_bytes);
        self.declaration_clone_allocations = self
            .declaration_clone_allocations
            .max(other.declaration_clone_allocations);
        self.named.catalog_requested_bytes = self
            .named
            .catalog_requested_bytes
            .max(other.named.catalog_requested_bytes);
        self.named.catalog_allocations = self
            .named
            .catalog_allocations
            .max(other.named.catalog_allocations);
        self.named.destination_requested_bytes = self
            .named
            .destination_requested_bytes
            .max(other.named.destination_requested_bytes);
        self.named.destination_allocations = self
            .named
            .destination_allocations
            .max(other.named.destination_allocations);
    ...
    fn subset_ceiling(self, full: Self, count: usize) -> Option<Self> {
        ...
        out.requested = count;
        out.requested_id_bytes = full
            .requested_id_bytes
            .min(self.requested_id_bytes.checked_mul(count)?);
        out.units = full.units.min(self.units.checked_mul(count)?);
        out.unit_id_bytes = full
            .unit_id_bytes
            .min(self.unit_id_bytes.checked_mul(count)?);
        out.bindings = full.bindings.min(self.bindings.checked_mul(count)?);
        out.physical_bindings = full
            .physical_bindings
            .min(self.physical_bindings.checked_mul(count)?);
        out.aliases = full.aliases.min(self.aliases.checked_mul(count)?);
        out.local_aliases = full
            .local_aliases
            .min(self.local_aliases.checked_mul(count)?);
        out.binding_name_bytes = full
            .binding_name_bytes
            .min(self.binding_name_bytes.checked_mul(count)?);
        out.alias_owner_name_bytes = full
            .alias_owner_name_bytes
            .min(self.alias_owner_name_bytes.checked_mul(count)?);
        out.physical_bytes = full.physical_bytes.min(
            self.physical_bytes
                .checked_mul(u64::try_from(count).ok()?)?,
        );
        out.recipe_pending = full
            .recipe_pending
            .min(self.recipe_pending.checked_mul(count)?);
        out.recipe_materializations = full
            .recipe_materializations
            .min(self.recipe_materializations.checked_mul(count)?);
        out.recipe_depth = self.recipe_depth;
        out.declaration_clone_bytes = full
            .declaration_clone_bytes
            .min(self.declaration_clone_bytes.checked_mul(count)?);
        out.declaration_clone_allocations = full
            .declaration_clone_allocations
            .min(self.declaration_clone_allocations.checked_mul(count)?);
        out.named.catalog_requested_bytes = full
            .named
            .catalog_requested_bytes
            .min(self.named.catalog_requested_bytes.checked_mul(count)?);
        out.named.catalog_allocations = full
            .named
            .catalog_allocations
            .min(self.named.catalog_allocations.checked_mul(count)?);
        out.named.destination_requested_bytes = full
            .named
            .destination_requested_bytes
            .min(self.named.destination_requested_bytes.checked_mul(count)?);
        out.named.destination_allocations = full
            .named
            .destination_allocations
            .min(self.named.destination_allocations.checked_mul(count)?);
        ...
        out.named.physical_cells = full
            .named
            .physical_cells
            .min(self.named.physical_cells.checked_mul(count)?);
        Some(out)
    }
}
