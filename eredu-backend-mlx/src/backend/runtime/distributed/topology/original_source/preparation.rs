...
use crate::backend::runtime::distributed::completion::{
    prepared::ReadyCompletionResources, MlxNeuralCommunicationCompletion,
    OriginalCommunicationU32Words, PreparedCommunicationU32Words,
};
use safemlx::{
    distributed::GroupWorkerOperation, OriginalScopeObserver, PreparedInputPlan,
    PreparedInputRuntime,
};

// The final Arc allocation retires before its Q; every clone remains payload-free.
#[derive(Debug)]
pub(super) struct SharedPreparationCustody(
    Option<std::sync::Arc<eredu_runtime::working_memory::CommunicationPreparationCustody>>,
);
impl SharedPreparationCustody {
    pub(super) fn new(
        value: eredu_runtime::working_memory::CommunicationPreparationCustody,
    ) -> Self {
    ...
    pub(super) fn allocation_bytes() -> Option<usize> {
        std::alloc::Layout::new::<[usize; 2]>()
            .extend(std::alloc::Layout::new::<
                eredu_runtime::working_memory::CommunicationPreparationCustody,
            >())
            .ok()
...
impl Drop for SharedPreparationCustody {
    fn drop(&mut self) {
        if let Some(value) = self.0.take() {
            drop(std::sync::Arc::into_inner(value));
        }
    }
}
...
impl OriginalCommunicationSource<'_> {
    pub(crate) fn prepare_preparation_frame(
        &self,
        words: &[u32],
        pool: &eredu_runtime::working_memory::MemoryLedger,
        policy: Option<&eredu_runtime::working_memory::SessionResetPreparationFunding>,
    ) -> Result<OriginalPreparationFrame, Error> {
        reserve(
            self.funding(),
            &[
                size_of::<OriginalPreparationFrame>(),
                size_of::<Result<OriginalPreparationFrame, Error>>(),
                size_of::<(
                    &Self,
                    &[u32],
                ...
                size_of::<
                    Result<PreparedInputRuntime, eredu_runtime::working_memory::WorkingMemoryError>,
                >(),
                ...
                eredu_core::BackendFailure::source_retention_peak_bytes::<RetiredFailure>()
                    .ok_or_else(overflow)?,
                size_of::<FrameProducer<'_, '_>>(),
                size_of::<RetiredFailure>(),
                size_of::<
                    Result<
                        eredu_runtime::working_memory::PreparedCommunicationPreparation<Array>,
        ...
        let plan = runtime
            .u32(words, &shape)
            .map_err(|cause| failure(Cause::Input(cause), self.source(), self.funding()))?;
        let value = pool
            .prepare_communication(FrameProducer {
                source: self,
                plan,
                words,
                pool,
...
impl OriginalPreparationFrame {
    pub(crate) fn prepare<'a>(
        ...
        reserve(
            &self.funding,
            &[
                size_of::<PreparedOriginalPreparationGather<'a>>(),
                size_of::<Result<PreparedOriginalPreparationGather<'a>, Error>>(),
        ...
        let operation = source
            .world_cpu_operation_storage(self.value.output(), GroupWorkerOperation::Gather)?;
        let expected = self
            .word_count
        ...
        Ok(PreparedOriginalPreparationGather {
            operation,
            words,
            ready,
...
impl eredu_runtime::working_memory::CommunicationPreparationProducer for FrameProducer<'_, '_> {
    ...
    fn produce(
        ...
        let [value] = super::inputs::construct_admitted(self.source, [self.plan], custody)?;
        Ok(value)
    }
}
...
struct RetiredFailure {
    #[source]
    cause: eredu_runtime::working_memory::RetiredCommunicationPreparationError<Error>,
    source: RetainedCommunicationSource,
    funding: HostMetadataFunding,
}
fn retired_error(
