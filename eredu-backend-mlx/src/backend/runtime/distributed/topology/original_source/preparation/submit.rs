...
use eredu_core::{
    BoundedCompletion, BoundedCompletionOutcome, BoundedCompletionWait, Completion, Submission,
};
use eredu_runtime::working_memory::{
    CommunicationPreparationCustody, CommunicationPreparationProducer, MemoryLedger,
    PreparedCommunicationPreparation, WorkingMemoryError,
};
use safemlx::{
    OriginalBufferBudget, PreparedOriginalBufferBudget, PreparedPrefillFailure,
    PreparedSubmissionGraphQuota, PreparedSubmissionRecordQuota,
    RegisteredThreadRuntimeHousekeeping, RetainedPrefillFailure,
};
...
struct Custody {
    ...
    native: SharedPreparationCustody,
...
enum RoleCause {
    ...
    #[error("readiness scope observation unavailable: {0:?}")]
    Observation(safemlx::ScopedSubmissionProgress),
    #[error("readiness native completion is not terminal")]
    Incomplete,
    #[error(
        "readiness completion deadline cannot be represented or its cancellation is unavailable"
    )]
    Deadline,
}
#[derive(Debug, thiserror::Error)]
...
impl std::fmt::Debug for PreparationCompletion {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PreparationCompletion")
            .finish_non_exhaustive()
    }
}
struct Producer<'a, 'native> {
    operation: PreparedOriginalPreparationGather<'a>,
...
impl PreparedOriginalPreparationGather<'_> {
    pub(crate) fn start(
        ...
        reserve(
            &self.funding,
            &[
                size_of::<Producer<'_, '_>>(),
                size_of::<Started>(),
                size_of::<NativeCompletion>(),
                size_of::<PreparationCompletion>(),
                size_of::<Custody>(),
                size_of::<RoleCause>(),
                size_of::<RoleFailure>(),
                ...
                size_of::<
                    Result<
                        PreparedCommunicationPreparation<Started>,
                        eredu_runtime::working_memory::CommunicationPreparationError<
                            Producer<'_, '_>,
                        >,
                    >,
                >(),
                size_of::<(OriginalCommunicationU32Words, PreparationCompletion)>(),
                size_of::<[Option<usize>; 7]>(),
                size_of::<(
                    &OriginalCommunicationSource<'_>,
                    &[u32],
                    &Stream,
                    &MemoryLedger,
                    Option<&eredu_runtime::working_memory::SessionResetPreparationFunding>,
                )>(),
                eredu_core::BackendFailure::source_retention_peak_bytes::<RoleFailure>()
                    .ok_or_else(overflow)?,
                eredu_core::BackendFailure::source_retention_peak_bytes::<RetiredFailure>()
                    .ok_or_else(overflow)?,
                failure_control_bytes().ok_or_else(overflow)?,
                size_of::<std::cell::RefMut<'_, Option<Recovery<Retained>>>>(),
                size_of::<Option<&Recovery<Retained>>>(),
                size_of::<Status>(),
                ...
                size_of::<Option<Result<bool, Error>>>(),
                ...
                size_of::<BoundedCompletionOutcome>(),
        ...
        let failure_layout =
            PreparedPrefillFailure::<Custody>::layout().map_err(|_| error(Cause::Resource))?;
        let buffer = PreparedOriginalBufferBudget::<Custody>::layout(
            self.runtime(),
            self.backing_capacity(),
        )
        .map_err(|cause| error(Cause::Buffer(cause)))?;
        let native =
            safemlx::OriginalNativeControlLayout::inspect().map_err(|_| error(Cause::Resource))?;
        let shared = SharedPreparationCustody::allocation_bytes().ok_or_else(overflow)?;
        let sizes = [
            graph.total_bytes(),
            record.total_bytes(),
            failure_layout.total_bytes(),
            buffer.total_owner_bytes(),
            PreparedRecovery::<Retained, Custody>::control_bytes()
                .and_then(|n| usize::try_from(n).ok()),
            safemlx::PreparedThreadRuntimeHousekeeping::<Custody>::control_bytes(),
            OriginalScopeObserver::control_bytes(),
        ];
        let bytes = sizes
            .into_iter()
            .try_fold(size_of_val(&sizes), |n, value| n.checked_add(value?))
            .and_then(|n| n.checked_add(native.fixed_control_bytes))
            .and_then(|n| n.checked_add(shared))
            .and_then(|n| n.checked_add(self.backing_capacity()))
            .ok_or_else(overflow)?;
        let funding = self.funding.clone();
        let retained = self.source.clone();
        ...
        let mut prepared = pool
            .prepare_communication(producer)
            .map_err(|cause| retired_error(cause.retire(), &retained, &funding))?;
        let output =
            prepared.with_output(|value| value.words.take().expect("fresh readiness output"));
        Ok(Submission {
            output,
            completion: PreparationCompletion(prepared),
        })
    }
}
impl CommunicationPreparationProducer for Producer<'_, '_> {
    ...
    fn check_preparation_policy(
        &self,
        pool: &MemoryLedger,
        bytes: u64,
    ) -> Result<(), WorkingMemoryError> {
        self.policy
            .map_or(Ok(()), |policy| policy.validate_ledger(pool))
    }
    fn produce(self, raw: CommunicationPreparationCustody) -> Result<Started, Error> {
        let custody = Custody {
            source: self.operation.source.clone(),
            native: SharedPreparationCustody::new(raw),
            funding: self.operation.funding.clone(),
        };
        let error = |cause| fail(cause, &custody);
        let budget = PreparedOriginalBufferBudget::try_new(
            self.operation.runtime(),
            self.operation.backing_capacity(),
            custody.clone(),
        )
        .map_err(|error_| {
            let (cause, owner) = error_.into_parts();
            drop(owner);
            error(RoleCause::Buffer(cause))
        })?
        .try_allocate()
        .map_err(|error_| {
            let (cause, owner) = error_.into_parts();
        ...
        let graph =
            PreparedSubmissionGraphQuota::try_new(self.operation.graph_capacity(), custody.clone())
                .map_err(|error_| {
                    let (cause, owner) = error_.into_parts();
                    drop(owner);
                    error(RoleCause::Graph(cause))
                })?
                .try_allocate()
                .map_err(|error_| {
                    let (cause, owner) = error_.into_parts();
        ...
        .map_err(|error_| {
            let (cause, owner) = error_.into_parts();
            drop(owner);
            error(RoleCause::Housekeeping(cause))
        })?;
        ...
        let prepared = PreparedRecovery::new(retained, custody.clone())
            .map_err(|error_| {
                let cause = error_.cause;
                drop(error_.retention);
                drop(error_.custody);
                error(RoleCause::Scope(cause))
            })?
            .with_graph_quota(Some(graph))
            .with_record_quota(Some(records));
        let mut recovery = prepared.try_begin().map_err(|error_| {
            let cause = error_.cause;
            drop(error_.pending);
            error(RoleCause::Scope(cause))
        })?;
        let submitted = recovery.configure_scope_with_retention(|scope, retained| {
            scope
                ...
                .map_err(|cause| error(RoleCause::Observation(cause)))?;
            scope
                .require_original_native_controls()
                .map_err(|cause| error(RoleCause::Control(cause)))?;
            retained
                .failure
            ...
            scope
                ...
                .map_err(|cause| error(RoleCause::Buffer(cause)))?;
            let observer = OriginalScopeObserver::require_current()
                .map_err(|cause| error(RoleCause::Native(cause)))?;
...
impl NativeCompletion {
    ...
    fn try_finish(&self) -> Result<bool, Error> {
        safemlx::try_with_submission_retirement(|| {
            ...
            let Some(recovery) = slot.as_ref() else {
                return Ok(true);
            };
            let status = recovery.progress();
            if status.failed || status.blocked {
                return Err(fail(RoleCause::Incomplete, &self.custody));
            }
            if !status.settled {
                return Ok(false);
            }
            ...
            let recovery = slot.take().expect("observed readiness recovery");
            drop(slot);
    ...
    fn bounded(&mut self, wait: BoundedCompletionWait) -> Result<BoundedCompletionOutcome, Error> {
        ...
        let inner = self
            .inner
            .take()
            .ok_or_else(|| fail(RoleCause::Incomplete, &self.custody))?;
        let outcome = inner.wait_bounded(inner_wait)?;
        if !matches!(outcome, BoundedCompletionOutcome::Completed) {
            return Ok(outcome);
        }
        loop {
            if self.try_finish()? {
...
impl Completion for PreparationCompletion {
    ...
    fn resources_releasable(&self) -> bool {
        ...
        value
            .inner
            .as_ref()
            .is_none_or(Completion::resources_releasable)
            && value
                .recovery
                .borrow()
                .as_ref()
                .is_none_or(|recovery| recovery.progress().settled)
    }
    fn is_complete(&self) -> Result<bool, Error> {
        let value = &self.0.output().completion;
        let completed = match &value.inner {
            Some(inner) => inner.is_complete()?,
            None => value.recovery.borrow().is_none(),
        };
        if completed {
            value.try_finish()
        } else {
