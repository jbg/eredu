...
use eredu_runtime::{CommunicationCompletionPolicy,CommunicationGroupDescriptor,
    ...
    PartitionCommunicationAuthority,working_memory::InferenceExecutionIdentity};
use safemlx::{Device,DeviceType,OperationEvent,OperationEvalTraversalLimits,
    PreparedInputRuntime,PreparedSubmissionScopeOwner,SubmissionScope};
...
impl CommunicationPreparationProducer for FixtureProducer<'_,'_> {
    type Output=FixtureOutput;type Error=Error;
    fn source(&self)->&RetainedCommunicationSource{self.source.source()}
    fn frame(&self)->&[u32]{&[2,3,5]}
    ...
    fn required_storage_bytes(&self)->Result<usize,WorkingMemoryError>{
        ...
        let parts=[
            PreparedSubmissionGraphQuota::<Custody>::layout(GRAPH).unwrap().total_bytes().unwrap(),
            PreparedSubmissionRecordQuota::<Custody>::layout(RECORDS).unwrap().total_bytes().unwrap(),
            PreparedPrefillFailure::<Custody>::layout().unwrap().total_bytes().unwrap(),
            PreparedOriginalBufferBudget::<Custody>::layout(self.runtime,self.backing).unwrap().total_owner_bytes().unwrap(),
            self.backing,usize::try_from(PreparedRecovery::<Retained,Custody>::control_bytes().unwrap()).unwrap(),
            safemlx::PreparedThreadRuntimeHousekeeping::<Custody>::control_bytes().unwrap(),
            safemlx::OriginalNativeControlLayout::inspect().unwrap().fixed_control_bytes,
            OriginalScopeObserver::control_bytes().unwrap(),
            SharedPreparationCustody::allocation_bytes().unwrap(),
            child.allocation_bytes().unwrap(),child.native_retirement_control_bytes,
            child.prepared_bytes,child.preparation_control_bytes,child.begin_control_bytes,
            child.retirement_control_bytes,child.preparation_failure_bytes,child.begin_failure_bytes,
            size_of::<Self>(),size_of::<FixtureOutput>(),size_of::<NativeCompletion>(),
            size_of::<Custody>(),size_of::<Retained>(),size_of::<SubmissionScope>(),
        ];
        parts.into_iter().try_fold(size_of_val(&parts),usize::checked_add).ok_or(WorkingMemoryError::Overflow)
    }
    fn produce(self,raw:CommunicationPreparationCustody)->Result<FixtureOutput,Error>{
        let custody=Custody{source:self.source.source().clone(),native:SharedPreparationCustody::new(raw),funding:self.source.funding().clone()};
        let budget=PreparedOriginalBufferBudget::try_new(self.runtime,self.backing,custody.clone()).unwrap().try_allocate().unwrap();
        let graph=PreparedSubmissionGraphQuota::try_new(GRAPH,custody.clone()).unwrap().try_allocate().unwrap();
        let records=PreparedSubmissionRecordQuota::try_new(RECORDS,custody.clone()).unwrap().try_allocate().unwrap();
        let failure=PreparedPrefillFailure::try_new(custody.clone()).unwrap().try_allocate().unwrap();
        let child=PreparedSubmissionScopeOwner::try_new(custody.clone()).unwrap()
        ...
        let mut recovery=PreparedRecovery::new(retained,custody.clone()).unwrap_or_else(|error|panic!("fixture recovery preparation: {:?}",error.cause))
            .with_graph_quota(Some(graph)).with_record_quota(Some(records)).try_begin().unwrap_or_else(|error|panic!("fixture recovery begin: {:?}",error.cause));
        let (output,completion,observer,child)=recovery.configure_scope_with_retention(|scope,retained|{
            scope.enable_scoped_observation().unwrap();scope.require_original_native_controls().unwrap();
            retained.failure.bind_original_scope(scope).unwrap();scope.enable_original_native_controls().unwrap();
            ...
            let completion=self.ready.submit_original(self.source,&observer,self.stream,[&output]).unwrap();
        ...
        recovery.seal();
        Ok(FixtureOutput{output,child:Some(child),completion:Some(NativeCompletion{
            inner:Some(completion.into()),recovery:RefCell::new(Some(recovery)),observer,custody})})
    }
}
...
fn run_fixture(timeout:bool){
    ...
    let (runtime,_)=crate::backend::managed_memory::input_allocator::prepare_admitted(&pool).unwrap();
    ...
    let group=CommunicationGroupDescriptor::new(CollectiveGroupId::new(59),0,vec![0],Some(0),
    ...
    let manifest=CommunicationManifest::new(1,0,vec![group],vec![]).unwrap().with_completion_policy(
        CommunicationCompletionPolicy::new(Duration::from_secs(1),CompletionCancellationMode::QuarantineUntilComplete).unwrap());
    ...
    let input=Array::from_slice(&[2_f32,-3.,5.],&[3]);input.evaluated().unwrap();
    ...
    funding.reserve_metadata(OriginalBufferBudget::request_layout_control_bytes().unwrap()).unwrap();
    let backing=OriginalBufferBudget::request_layout(&runtime,input.nbytes()).unwrap().capacity();
    let traversal=OperationEvent::eval_traversal_layout(OperationEvalTraversalLimits{
        roots:1,arrays:4,tape_entries:3,input_edges:4,output_slots:3,streams:1,captures:8,
    }).unwrap();
    let mut ready=PreparedCompletionResources::prepare_original(&source,
        CompletionResourceLayout{arrays:1,counts:&[],groups:0,routes:0,streams:1},traversal).unwrap();
    ready.push_array(input.clone()).unwrap();ready.push_stream(stream.clone()).unwrap();
    let mut prepared=pool.prepare_communication(FixtureProducer{
        source:&source,pool:&pool,runtime:&runtime,stream:&stream,input:&input,backing,ready:ready.finish().unwrap(),
    }).unwrap();
    ...
    assert_eq!(prepared.output().output.completed_in_original_scope(&completion.observer)
        .unwrap().try_as_slice::<f32>().unwrap(),&[4.,9.,25.]);
    assert!(!completion.try_finish().unwrap(),"event readiness cannot retire its live descendant");
    assert!(completion.recovery.borrow().is_some(),"pending retains the same recovery node");
    ...
    if timeout {
        let wait=BoundedCompletionWait::new(Duration::from_millis(1),CompletionCancellationMode::QuarantineUntilComplete).unwrap();
        assert!(matches!(completion.bounded(wait).unwrap(),BoundedCompletionOutcome::DeadlineExceeded{
            cancellation:CompletionCancellationMode::QuarantineUntilComplete}));
        drop(completion);drop(prepared);
        crate::backend::submission_recovery::reap();
        safemlx::reclaim_allocation_owners();
        assert!(pool.used_bytes().unwrap()>baseline,"pending native child/recovery retain original source and producer account");
        child.seal();drop(child);
    }else{
        child.seal();drop(child);
        crate::backend::submission_recovery::wait_for_retirement(||{
            assert!(std::time::Instant::now()<retirement_deadline,"sealed child must allow terminal readiness");
    ...
    crate::backend::submission_recovery::wait_for_retirement(||{
        ...
        assert!(std::time::Instant::now()<retirement_deadline,"source/producer custody did not retire: {used} vs {baseline}");
        used==baseline
    });
...
fn readiness_event_does_not_retire_a_pending_enclosing_scope(){run_fixture(false);}
#[test]
#[ignore="requires fresh original allocator and native CPU source"]
fn readiness_deadline_keeps_pending_scope_custody_until_terminal(){run_fixture(true);}
