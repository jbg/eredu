...
use safemlx::{
    ...
    distributed::{GroupCpuOperationStorage, GroupWorkerOperation},
};
...
impl<'input> OriginalCommunicationOperation<'input> {
    pub(super) fn from_layout_binding(
        native: GroupCpuOperationStorage<'input>,
        persistent: OriginalCommunicatorPersistent<'input>,
        source: RetainedCommunicationSource,
        funding: HostMetadataFunding,
    ) -> Self {
        Self {
    ...
    pub(crate) fn native(&self) -> &GroupCpuOperationStorage<'_> {
        &self.native
    }
    /// Same-worker prospective metadata; this descriptor grants no execution authority.
    pub(crate) fn backing_control_bytes(native_bytes: usize) -> Option<usize> {
        let frames = [
    ...
    pub(crate) fn backing_storage<'source>(
        &'source self,
        runtime: &'source safemlx::PreparedInputRuntime,
    ) -> Result<OriginalCommunicationBacking<'source, 'input>, Error> {
        let controls = Self::backing_control_bytes(
            self.native
                .backing_storage_control_bytes()
    ...
    pub(crate) fn persistent(&self) -> &OriginalCommunicatorPersistent<'_> {
        &self.persistent
    }
    pub(crate) fn construct(
        ...
        let value = self
            .native
            .construct_original(observer, stream)
        ...
        Ok(OriginalCommunicationConstructed::from_constructor(
            value,
            self.source,
            self.funding,
        ))
...
impl<'a> OriginalCommunicationSource<'a> {
    /// Fixed lookup and failure transports of the actual operation selector.
    pub(crate) fn operation_lookup_control_bytes() -> Option<usize> {
        let frames = [
    ...
    fn operation_lookup_controls(&self) -> Result<(), Error> {
        self.funding
            .reserve_metadata(
                Self::operation_lookup_control_bytes()
                    .ok_or(Error::WorkspacePlanning(HostMetadataFundingError::Overflow))?,
            )
            .map_err(Error::WorkspacePlanning)
    }
    /// Same-worker prospective metadata; this descriptor grants no execution authority.
    pub(crate) fn operation_storage_control_bytes(native_bytes: usize) -> Option<usize> {
        let frames = [
            size_of::<OriginalCommunicationOperation<'_>>(),
            size_of::<OriginalCommunicatorPersistent<'_>>(),
            size_of::<Result<OriginalCommunicationOperation<'_>, Error>>(),
    ...
    fn operation_storage<'b>(
        &'b self,
        group: &'b Group,
        input: &'b Array,
        operation: GroupWorkerOperation,
        persistent: OriginalCommunicatorPersistent<'b>,
    ) -> Result<OriginalCommunicationOperation<'b>, Error> {
        let controls = Self::operation_storage_control_bytes(
            group
                .native_group()
        ...
        let native = group
            .native_group()
            .cpu_operation_storage(input, operation)
            .map_err(|_| failure(Cause::Resource, &self.source, &self.funding))?;
        Ok(OriginalCommunicationOperation {
            native,
            persistent,
            source: self.source.clone(),
    ...
    pub(crate) fn group_variable_cpu_operation_storage<'b>(
        &'b self,
        order: usize,
        input: &'b Array,
        matrix: &'b [usize],
        ...
        self.variable_operation_storage(
            group,
            input,
            matrix,
            transposed,
            self.group_persistent(order)?,
        )
    }
    ...
    pub(crate) fn world_variable_cpu_operation_storage<'b>(
        &'b self,
        order: usize,
        input: &'b Array,
        matrix: &'b [usize],
        ...
        self.operation_lookup_controls()?;
        let (group, descriptor, wave) = self
            .group(order)
            .ok_or_else(|| failure(Cause::Resource, &self.source, &self.funding))?;
        let plan = group
    ...
    fn variable_operation_storage<'b>(
        &'b self,
        group: &'b Group,
        input: &'b Array,
        matrix: &'b [usize],
        ...
        let frames = [
            size_of::<OriginalCommunicationOperation<'b>>(),
            ...
            size_of::<
                Option<crate::backend::runtime::distributed::group::LogicalVariableWorldPlan<'_>>,
            >(),
            ...
            group
                .native_group()
                .variable_cpu_operation_storage_control_bytes()
                .ok_or(Error::WorkspacePlanning(HostMetadataFundingError::Overflow))?,
        ];
        self.funding
            .reserve_metadata(
        ...
        let native = group
            .native_group()
            .variable_cpu_operation_storage(input, matrix, transposed)
            .map_err(|_| failure(Cause::Resource, &self.source, &self.funding))?;
        Ok(OriginalCommunicationOperation {
            native,
            persistent,
    ...
    pub(crate) fn status_operation_control_bytes() -> Option<usize> {
        let frames = [
            size_of::<crate::backend::runtime::distributed::group::StatusPlan<'_>>(),
            size_of::<(&Self, usize, &Array, GroupWorkerOperation)>(),
            size_of::<OriginalCommunicatorPersistent<'_>>(),
            size_of::<Result<OriginalCommunicationOperation<'_>, Error>>(),
    ...
    pub(crate) fn status_cpu_operation_storage<'b>(
        &'b self,
        order: usize,
        plan: crate::backend::runtime::distributed::group::StatusPlan<'_>,
        input: &'b Array,
        operation: GroupWorkerOperation,
    ) -> Result<OriginalCommunicationOperation<'b>, Error> {
        self.funding
            .reserve_metadata(
                Self::status_operation_control_bytes()
                    .ok_or(Error::WorkspacePlanning(HostMetadataFundingError::Overflow))?,
            )
            .map_err(Error::WorkspacePlanning)?;
        self.operation_lookup_controls()?;
        self.validate()?;
        let (group, descriptor, _) = self
            .group(order)
            .ok_or_else(|| failure(Cause::Resource, &self.source, &self.funding))?;
        let selected = self
            .source
        ...
        if selected.order() != order
            || !selected.requirement().exact_completion()
            || group
                .selected_status_plan()
                .map_err(|_| failure(Cause::LogicalWorldTransport, &self.source, &self.funding))?
                != Some(plan)
            || !plan.accepts(operation)
            || input.shape() != [1]
            || input.dtype() != safemlx::Dtype::Int32
            || persistent.native().has_unqualified_storage()
            || !persistent.native().is_for(group.native_group())
            || !persistent.source().same_source(&self.source)
        {
    ...
    pub(crate) fn world_cpu_operation_storage<'b>(
        &'b self,
        input: &'b Array,
        operation: GroupWorkerOperation,
    ) -> Result<OriginalCommunicationOperation<'b>, Error> {
    ...
    pub(crate) fn group_cpu_operation_storage<'b>(
        &'b self,
        order: usize,
        input: &'b Array,
        operation: GroupWorkerOperation,
...
impl<'a> OriginalCommunicationOperation<'a> {
    /// Same-worker prospective metadata; this descriptor grants no execution authority.
    pub(crate) fn completion_control_bytes(native_bytes: usize) -> Option<usize> {
        let parts = [
    ...
    pub(crate) fn with_completion(
        ...
        let controls = Self::completion_control_bytes(
            self.native
                .completion_storage_control_bytes()
                .ok_or(Error::WorkspacePlanning(HostMetadataFundingError::Overflow))?,
        )
        .ok_or(Error::WorkspacePlanning(HostMetadataFundingError::Overflow))?;
        self.funding
        ...
        let native = native
            .with_completion_storage()
            .map_err(|_| failure(Cause::Resource, &source, &funding))?;
        Ok(OriginalCommunicationCompletedOperation {
            native,
            persistent,
            source,
            funding,
...
impl<'a> OriginalCommunicationCompletedOperation<'a> {
    pub(super) fn from_retained_layout(
        native: safemlx::distributed::GroupCpuCompletionStorage<'a>,
        persistent: OriginalCommunicatorPersistent<'a>,
        source: RetainedCommunicationSource,
        funding: HostMetadataFunding,
    ...
    pub(crate) fn native(&self) -> &safemlx::distributed::GroupCpuCompletionStorage<'_> {
        &self.native
    }
    /// Same-worker prospective metadata; this descriptor grants no execution authority.
    pub(crate) fn prepare_resources_control_bytes() -> Option<usize> {
        use crate::backend::runtime::distributed::completion::prepared::ReadyCompletionResources;
        let parts = [
            size_of::<(&Self, &OriginalCommunicationSource<'_>, Option<usize>)>(),
            size_of::<Option<(&Group, &CommunicationGroupDescriptor, bool)>>(),
    ...
    > {
        ...
        let controls = Self::prepare_resources_control_bytes()
            .ok_or(Error::WorkspacePlanning(HostMetadataFundingError::Overflow))?;
        self.funding
            .reserve_metadata(controls)
            .map_err(Error::WorkspacePlanning)?;
        ...
        if !self.source.same_source(source.source())
            || !self.native.is_for_group(actual.native_group())
        {
            return Err(failure(Cause::Identity, &self.source, &self.funding));
        }
        ...
        let mut prepared = PreparedCompletionResources::prepare_original(
            ...
            self.native.traversal(),
        )?;
        match group {
            None => prepared.retain_control_world()?,
            Some(order) => prepared.retain_group(order)?,
        };
        prepared.finish()
    }
    /// Same-worker prospective metadata; this descriptor grants no execution authority.
    pub(crate) fn construction_control_bytes(native_bytes: usize) -> Option<usize> {
        let parts = [
            size_of::<Self>(),
            size_of::<OriginalCommunicationConstructed>(),
            size_of::<Result<OriginalCommunicationConstructed, Error>>(),
    ...
    pub(crate) fn construct(
        ...
        let controls = Self::construction_control_bytes(
            self.native
                .control_bytes()
                .ok_or(Error::WorkspacePlanning(HostMetadataFundingError::Overflow))?,
        )
