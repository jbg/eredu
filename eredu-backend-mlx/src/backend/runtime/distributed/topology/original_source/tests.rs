...
fn native_retained_communication_source_binds_exact_world_without_submission_and_retains_failures()
{
    ...
    drop(source);
    let failure = actual
        .bind_original_source(&manifest, &foreign, &authority, &funding)
        .err()
        .expect("equal native rank/size cannot replace the exact world");
    assert_eq!(
    ...
    assert!(
        ...
        "the original failure retains its account"
    );
    drop(failure);
...
fn paid_native_inventory_keeps_actual_source_and_h_without_submission_or_total_fit() {
    use safemlx::distributed::{GroupStorageDomain, GroupStorageKind};
    let stream = Stream::new_with_device(&safemlx::Device::new(safemlx::DeviceType::Cpu, 0));
    let world = NativeGroup::init(false, safemlx::distributed::Backend::Ring).unwrap();
    let id = CollectiveGroupId::new(7);
    let descriptor = CommunicationGroupDescriptor::new(
    ...
    assert!(inventory
        .native()
        .is_unqualified(GroupStorageDomain::SharedControl));
    let failure = source
        .group_inventory(1)
        .err()
        .expect("no fabricated source row");
    assert_eq!(
        crate::backend::runtime::distributed::group::native_collective_submissions(),
        0
    );
...
fn unqualified_actual_worker_source_refuses_without_submission_and_retains_failure_h() {
    use safemlx::{distributed::GroupWorkerOperation, Array};
    let stream = Stream::new_with_device(&safemlx::Device::new(safemlx::DeviceType::Cpu, 0));
    let world = NativeGroup::init(false, safemlx::distributed::Backend::Ring).unwrap();
    let id = CollectiveGroupId::new(7);
    let descriptor = CommunicationGroupDescriptor::new(
    ...
    let failure = source
        .world_worker_storage(&input, GroupWorkerOperation::Sum)
        .err()
        .expect("actual singleton fallback cannot supply Ring workers");
    assert!(source
        .group_worker_storage(1, &input, GroupWorkerOperation::Gather)
        .is_err());
    assert!(source
        .route_worker_storage(0, &input, GroupWorkerOperation::Send { peer: 0 })
        .is_err());
    assert_eq!(
        crate::backend::runtime::distributed::group::native_collective_submissions(),
        0
    ...
    assert!(
        pool.fixture_host_charge().unwrap() > 0,
        "refused worker query keeps exact source/H"
    );
    drop(failure);
    assert_eq!(pool.fixture_host_charge().unwrap(), 0);
}
...
fn repeated_source_query_errors_each_pay_retained_failure_and_keep_h() {
    ...
    let funding = HostMetadataFunding::new(Counted {
        inner: funding,
    ...
    let first = source
        .group_worker_storage(99, &input, GroupWorkerOperation::Gather)
        .err()
        .unwrap();
    ...
    assert!(
        after_second - after_first >= allowance,
        "escaped errors cannot reuse a source allowance"
    );
    ...
    assert!(
        pool.fixture_host_charge().unwrap() > 0,
        "each retained error owns its source/account"
    );
    drop(third);
    assert_eq!(pool.fixture_host_charge().unwrap(), 0);
}
...
fn retained_native_persistent_owners_are_paid_before_lending_and_keep_source_h() {
    ...
    assert!(
        reserved.load(Ordering::SeqCst) - before
            >= persistent.native().retained_owner_bytes().unwrap()
    );
    ...
    let failure = source
        .group_persistent(99)
        .err()
        .expect("exact declaration required");
    drop(group);
    drop(source);
    drop(funding);
    drop(failure);
    assert!(
        pool.fixture_host_charge().unwrap() > 0,
        "paid persistent native loan retains H"
    );
