...
impl OriginalParallelInvocation {
    ...
    where F:FnOnce(&mut Group,&WorkspaceMetadataFunding)->Result<T,E>,
    {
        ...
        if let Some(control)=control {control.validate_model_source(self.source())?;}
        self.bind(observer,compute)?;
        let mut context=self.context.try_copy_for_retention()
            .map_err(|_|failure(Cause::Resource,&state.source,&self.funding))?;
        if let Some(control)=control {context.bind_original_control_request(control.retained()?);}
        let output=run(&mut context,&self.funding);
        if output.is_ok(){self.finish_construction()?;}
...
impl OriginalParallelInvocation {
    ...
    pub(crate) fn has_neural_context(&self)->bool{self.state().quote_source.has_neural_context()}
}
