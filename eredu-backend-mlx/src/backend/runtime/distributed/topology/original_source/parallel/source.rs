...
struct Publication {
    group: Group,
    descriptor: eredu_runtime::PartitionOutputPublication,
    root: usize,
}
...
#[derive(Clone, Copy)]
enum SourceGroup {
    Neural(CollectiveGroupId),
    Publication(CollectiveGroupId),
}
impl SourceGroup {
    fn id(self) -> CollectiveGroupId {
        match self {
            Self::Neural(id) | Self::Publication(id) => id,
        }
    }
    fn operation(self) -> CommunicationOperation {
        match self {
...
impl OriginalCommunicationSource<'_> {
    ...
    pub(crate) fn prepare_model_parallel_source(
        &self,
        tensor: Option<CollectiveGroupId>,
        agreement: CollectiveGroupId,
        ...
        reserve(
            ...
            &[
                ...
                size_of::<Publication>(),
                size_of::<Option<Publication>>(),
                size_of::<eredu_runtime::PartitionOutputPublication>(),
                ...
                size_of::<(
                    &Self,
                    Option<CollectiveGroupId>,
                    CollectiveGroupId,
                    &eredu_runtime::working_memory::MemoryLedger,
                )>(),
                size_of::<SourceGroup>(),
                size_of::<Option<&mut Body>>(),
        ...
        let selected = tensor
            .map(SourceGroup::Neural)
            .unwrap_or(SourceGroup::Publication(publication.group));
        ...
        body.publication = Some(Publication {
            group,
    ...
    pub(crate) fn prepare_parallel_source(
        ...
        reserve(
            &self.funding,
            &[
                size_of::<(&Self, CollectiveGroupId, &PreparedInputRuntime, &Stream)>(),
                size_of::<Result<OriginalParallelSource, Error>>(),
                size_of::<SourceGroup>(),
            ],
        )?;
    ...
    fn prepare_source(
        ...
        let persistent = self.group_persistent(selected.order())?;
        if group.is_logical()
            && self.group_exchange(selected.order())?.is_none()
...
impl OriginalParallelSource {
    ...
    pub(super) fn has_neural_context(&self) -> bool {
        matches!(self.body().kind, SourceGroup::Neural(_))
    }
    pub(super) fn publication(
        &self,
    ) -> Option<(&Group, eredu_runtime::PartitionOutputPublication, usize)> {
        self.body()
            .publication
            .as_ref()
            .map(|value| (&value.group, value.descriptor, value.root))
    }
    pub(super) fn input_runtime(&self) -> &safemlx::PreparedInputRuntime {
        &self.body().runtime
    }
    pub(super) fn transport(&self) -> &Stream {
        self.body().transport.as_stream()
    }
    pub(crate) fn agreement_inputs(
        &self,
    ) -> Option<&super::super::agreement::OriginalAgreementInputs> {
        self.body().agreement.as_ref()
    }
    pub(in crate::backend::runtime::distributed::topology::original_source) fn take_control_owner(
        &self,
    ) -> Result<(), Error> {
        reserve(
            &self.funding,
        ...
        if self.body().control_taken.replace(true)
            || self.body().owner.is_none()
            || self.body().agreement.is_none()
        {
    ...
    pub(crate) fn communication_source(&self) -> Result<OriginalCommunicationSource<'_>, Error> {
        self.communication_source_funded(&self.funding)
    }
    fn communication_source_control_bytes() -> Option<usize> {
        let parts = [
    ...
    /// Complete source loan through the real retained owner and table binder.
    pub(crate) fn communication_source_funded_control_bytes() -> Option<usize> {
        Self::communication_source_control_bytes()?
            .checked_add(super::super::OriginalCommunicationOwner::borrow_funded_control_bytes()?)
    }
    /// Lend the same immutable source under an already admitted operation's
    /// metadata payer. Communicator identity and native authority are unchanged.
    pub(crate) fn communication_source_funded(
        &self,
        funding: &HostMetadataFunding,
    ) -> Result<OriginalCommunicationSource<'_>, Error> {
        funding
            .reserve_metadata(Self::communication_source_control_bytes().ok_or_else(overflow)?)
            .map_err(Error::WorkspacePlanning)?;
        self.body()
            .owner
            .as_ref()
            .ok_or_else(|| failure(Cause::Resource, &self.body().source, funding))?
            .borrow_funded(funding)
    }

    pub(in crate::backend::runtime::distributed::topology::original_source) fn retain_communication_owner(
        &mut self,
        owner: super::super::OriginalCommunicationOwner,
    ) -> Result<(), Error> {
        reserve(
            &self.funding,
            &[
        ...
        if body.owner.is_some()
            || !owner.source().same_source(&body.source)
    ...
    pub(crate) fn logical_collective_layout<'a>(
        &self,
        equation: WorkspaceOperationView<'a>,
    ) -> Result<
        Option<(
            usize,
            eredu_nn::workspace::WorkspaceLayoutView<'a>,
            safemlx::Dtype,
            GroupWorkerOperation,
        )>,
        Error,
    > {
        let Some(id) = self.logical_group_id() else {
            return Ok(None);
        };
        if !matches!(
            equation.kind,
            eredu_nn::workspace::WorkspaceOperationKindView::Collective(
                eredu_nn::workspace::WorkspaceCollectiveView::Sum { .. }
                    | eredu_nn::workspace::WorkspaceCollectiveView::GatherFirstAxis { .. }
            )
        ) {
            return Ok(None);
        }
        super::super::workspace_sum::parallel_layout(
            &self.body().source,
            &self.funding,
            id,
            equation,
    ...
    pub(crate) fn quote_collective<'a>(
        ...
        reserve(
            &self.funding,
            &[
                size_of::<(&Group, CollectiveGroupId)>(),
                size_of::<Option<&Publication>>(),
                failure_control_bytes().ok_or_else(overflow)?,
            ],
        )?;
        let (group, id) = match equation.kind {
            eredu_nn::workspace::WorkspaceOperationKindView::Collective(
                eredu_nn::workspace::WorkspaceCollectiveView::Broadcast { group, root, .. },
            ) => {
                let publication = body
                    .publication
                    .as_ref()
                    .ok_or_else(|| failure(Cause::Identity, &body.source, &self.funding))?;
                if group != publication.descriptor.group || root != publication.root {
                    return Err(failure(Cause::Identity, &body.source, &self.funding));
                }
        ...
        if body.authority.ensure_active().is_err()
            ...
            || !crate::backend::runtime::distributed::completion::group_source_available(group)
        {
    ...
    pub(crate) fn backing(
        ...
        let (output, scratch) = match births {
            ...
            _ => {
                return Err(failure(
                    Cause::BackingPopulation(births),
                    &body.source,
                    &self.funding,
                ))
            }
...
impl OriginalParallelSource {
    ...
    pub(crate) fn prepare_invocation_with_boundary(
        &self,
        operations: &[WorkspaceOperation],
        ...
        self.prepare_invocation_with_sources(operations, mechanism, addressable, None)
    }
    /// Cold communication facts retain the selected bank source mode. An
    /// ordinary program cannot be entered as an Original indexed child.
    pub(crate) fn prepare_invocation_with_sources(
        &self,
        operations: &[WorkspaceOperation],
        mechanism: Option<crate::backend::nn::workspace::ResidentExecutionMechanisms>,
        addressable: Option<&crate::backend::nn::workspace::AddressableSources>,
        ordinary: Option<&crate::backend::nn::workspace::OrdinaryAddressableSources>,
    ) -> Result<OriginalParallelInvocation, Error> {
        let body = self.body();
        reserve(
            &self.funding,
            ...
            .filter(|op| {
                matches!(
                    ...
                    WorkspaceOperationKind::Collective(
                        WorkspaceCollective::Sum { .. }
                            | WorkspaceCollective::GatherFirstAxis { .. }
                            | WorkspaceCollective::Broadcast { .. }
                    )
                )
            })
            .count();
        ...
        for (ordinal, operation) in operations.iter().enumerate() {
            match &operation.kind {
                WorkspaceOperationKind::Collective(
                    collective @ (WorkspaceCollective::Sum { .. }
                    | WorkspaceCollective::GatherFirstAxis { .. }
                    | WorkspaceCollective::Broadcast { .. }),
                ) => {
                    if publication_seen {
                    ...
                    let logical = if self.logical_group_id().is_some() && !publication_seen {
                        let mechanism = mechanism
                            .ok_or_else(|| failure(Cause::Resource, &body.source, &self.funding))?;
                    ...
                    let (native, backing) = if let Some(quote) = &logical {
                        ...
                        reserve(
                            &self.funding,
                            &[quoted.ownership_control_bytes().ok_or_else(overflow)?],
                        )?;
                        let native = quoted
                            .try_into_owned()
                            .map_err(|_| failure(Cause::Resource, &body.source, &self.funding))?;
                    ...
                    let operation = match collective {
                        ...
                        WorkspaceCollective::Broadcast { group, root, .. } => {
                            InvocationOperation::Broadcast {
                                group: *group,
                                root: *root,
                            }
                        }
                        WorkspaceCollective::GatherFirstAxis {
                            ...
                            reserve(
                                &self.funding,
                                &[
                                    Layout::array::<usize>(peer_widths.len())
                                        .map_err(|_| overflow())?
                                        .size(),
                                    size_of::<Vec<usize>>(),
                                    size_of::<Result<(), std::collections::TryReserveError>>(),
                            ...
                            widths.try_reserve_exact(peer_widths.len()).map_err(|_| {
                                failure(Cause::Resource, &body.source, &self.funding)
                            })?;
                            widths.extend_from_slice(peer_widths);
                            InvocationOperation::Gather {
                                axis: *axis,
                                widths,
                            }
                        }
        ...
        let expert_regions =
            self.prepare_expert_occurrences(operations, mechanism, addressable, ordinary)?;
        let actual = group
            .try_copy_for_retention()
            .map_err(|_| failure(Cause::Resource, &body.source, &self.funding))?;
        let context = group
        ...
        let state = Rc::new(State {
            ...
            expert_regions,
            next_expert: Cell::new(0),
            expert_active: Cell::new(None),
            expert_local_taken: Cell::new(false),
            expert_local_complete: Cell::new(false),
            expert_transport_next: Cell::new(0),
            expert_transport_pending: Cell::new(false),
            expert_count_taken: Cell::new(false),
            expert_count_complete: Cell::new(false),
            expert_vote_next: Cell::new(0),
            expert_vote_pending: Cell::new(false),
            bound: OnceCell::new(),
            next: Cell::new(0),
...
impl OriginalParallelSource {
    pub(crate) fn initialized_runtime(&self) -> &PreparedInputRuntime {
        &self.body().runtime
    }
}
