...
use eredu_runtime::working_memory::{ExistingStoragePinLayout, MemoryLedger, WorkingMemoryStorage};
    ...
    std::iter::once(&actual.control_world)
        .chain(
            actual
                .groups
                .values()
                .filter_map(|group| group.native.as_ref()),
        )
        .chain(
            actual
                .routes
                .values()
                .filter_map(|route| route.group.as_ref()),
        )
}

pub(super) fn pin(
    actual: &ParallelCommunicators,
    ...
    let population = 1usize
        .checked_add(actual.groups.len())
        .and_then(|n| n.checked_add(actual.routes.len()))
        .ok_or_else(overflow)?;
    let actual_groups = groups(actual);
    let controls = [
        size_of::<(
            &ParallelCommunicators,
        ...
        size_of::<
            Result<
                ExistingStoragePinLayout<StorageIdentity>,
    ...
    funding
        .reserve_metadata(layout.requested_bytes())
        .map_err(Error::WorkspacePlanning)?;
    ...
    let mut rows = funding
        .metadata_vec::<(StorageIdentity, u64)>(population)
    ...
    for group in actual_groups {
        ...
        if bytes != 0 {
            rows.push((StorageIdentity::GroupBuffer(buffer.identity()), bytes));
        }
    }
    ...
    let pins = layout
        .construct(pool, rows)
        .map_err(|cause| failure(Cause::Allocator(cause), source, funding))?;
    let custody = HostPreparationAuthority::retain(Custody {
        _pins: pins,
        _funding: funding.clone(),
    });
    let retained = source
        .with_host_preparation(&custody)
