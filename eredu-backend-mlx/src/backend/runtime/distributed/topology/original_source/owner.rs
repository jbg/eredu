...
impl std::fmt::Debug for OriginalCommunicationOwner {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("OriginalCommunicationOwner")
            .field("session", &self.source.session_identity())
...
impl Drop for OriginalCommunicationOwner {
    fn drop(&mut self) {
        ...
        if let Some(actual) = self.actual.take() {
            drop(Rc::into_inner(actual));
        }
    }
}
impl OriginalCommunicationOwner {
    pub(crate) fn bind(
        actual: &Rc<ParallelCommunicators>,
    ...
    pub(crate) fn borrow(&self) -> Result<OriginalCommunicationSource<'_>, Error> {
        self.borrow_funded(&self.funding)
    }
    /// Local transports of the actual funded source loan.
    fn borrow_control_bytes() -> Option<usize> {
        let controls = [
            size_of::<&Self>(),
            size_of::<OriginalCommunicationSource<'_>>(),
            size_of::<Result<OriginalCommunicationSource<'_>, Error>>(),
    ...
    /// Complete loan query, including the common retained-table binder.
    pub(crate) fn borrow_funded_control_bytes() -> Option<usize> {
        Self::borrow_control_bytes()?
            .checked_add(ParallelCommunicators::bind_original_source_control_bytes()?)
    }
    pub(crate) fn borrow_funded(
        &self,
        funding: &HostMetadataFunding,
        ...
        let mut loan = self.actual().bind_original_source_retaining(
            self.source.manifest(),
            self.actual().control_world.native_group(),
            &self.authority,
            funding,
            &self.source,
        )?;
        ...
        loan.preparation_funding = self.funding.clone();
        loan.registered_buffers = self.registered_buffers;
        Ok(loan)
    }
    pub(crate) fn pin_registered_buffers(
        mut self,
        ...
        let controls = [
            size_of::<Self>(),
            size_of::<Result<Self, Error>>(),
            size_of::<(&mut Self, &eredu_runtime::working_memory::MemoryLedger)>(),
            size_of::<(
                RetainedCommunicationSource,
                super::registered_buffers::RegisteredBuffers,
            )>(),
        ];
        self.funding
            .reserve_metadata(
            ...
            .map_err(Error::WorkspacePlanning)?;
        let (source, proof) =
            super::registered_buffers::pin(self.actual(), &self.source, &self.funding, pool)?;
        self.source = source;
    ...
    pub(crate) fn prepare_initialized_parallel_source(
        ...
        let controls = [
            size_of::<Self>(),
            size_of::<parallel::OriginalParallelSource>(),
    ...
    pub(crate) fn prepare_model_parallel_source(
        ...
        let mut source = {
            self_
                .borrow()?
                .prepare_model_parallel_source(tensor, agreement, publication, pool)?
        };
