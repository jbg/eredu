...
pub(crate) struct OriginalAgreementInputs {
    ...
    status_quotes: Vec<Option<chain::StatusQuote>>,
    group: CollectiveGroupId,
    source: RetainedCommunicationSource,
    origin_funding: HostMetadataFunding,
    funding: HostMetadataFunding,
}
impl OriginalCommunicationSource<'_> {
    pub(crate) fn prepare_agreement_inputs(
        &self,
        group: CollectiveGroupId,
        pool: &eredu_runtime::working_memory::MemoryLedger,
    ) -> Result<OriginalAgreementInputs, Error> {
        reserve(
            &self.funding,
            &[
                size_of::<OriginalAgreementInputs>(),
        ...
        let shape = [1_usize];
        let plans = [
            runtime.i32(&values[..1], &shape),
            runtime.i32(&values[1..], &shape),
        ];
        let [zero, one] = plans;
        let plans = [
            zero.map_err(|cause| failure(Cause::Input(cause), &self.source, &self.funding))?,
            one.map_err(|cause| failure(Cause::Input(cause), &self.source, &self.funding))?,
        ];
        let values = super::super::inputs::construct(self, plans)?;
        let mut inputs = OriginalAgreementInputs {
            values,
            runtime,
            status_quotes: Vec::new(),
            group,
            source: self.source.clone(),
            origin_funding: self.preparation_funding().clone(),
            funding: self.funding.clone(),
        };
        let count = self.source.manifest().groups().len();
        let mut quotes = self.funding.metadata_vec(count).map_err(Error::Neural)?;
        for order in 0..count {
            quotes.push(chain::StatusQuote::prepare(&inputs, self, order)?);
        }
        inputs.status_quotes = quotes;
        Ok(inputs)
    }
}
impl OriginalAgreementInputs {
    pub(super) fn status_quote(&self, order: usize) -> Option<&chain::StatusQuote> {
        self.status_quotes.get(order)?.as_ref()
    }
    ...
    pub(crate) fn group(&self) -> CollectiveGroupId {
        self.group
    }
    pub(super) fn value(&self, success: bool) -> &Array {
        &self.values[usize::from(success)]
    }
