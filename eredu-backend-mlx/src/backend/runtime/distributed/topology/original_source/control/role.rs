...
use super::*;
use crate::backend::{runtime::residency::storage::native_storage::BankOwner,
...
use safemlx::{OriginalBufferBudget,PreparedPrefillFailure,PreparedSubmissionGraphQuota,
    PreparedSubmissionRecordQuota,RetainedPrefillFailure,RegisteredThreadRuntimeHousekeeping};
...
impl std::fmt::Debug for OriginalParallelControlOwner {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("OriginalParallelControlOwner")
            .field("installed", &self.0.is_some()).finish_non_exhaustive()
    }
}
...
impl OriginalParallelControlOwner {
    fn owner(&self)->&Owner{self.0.as_deref().expect("live request control owner")}
    pub(crate) fn new(source:OriginalParallelSource,bank:BankOwner,execution:&InferenceRequest,
        controls:OriginalTextControlGuard)->Result<Self,Error>{
        Self::from_source(ControlSource::Model(source),bank,execution,controls)
    }
    pub(crate) fn new_control(source:OriginalParallelControlSource,bank:BankOwner,execution:&InferenceRequest,
        controls:OriginalTextControlGuard)->Result<Self,Error>{
        Self::from_source(ControlSource::Control(source),bank,execution,controls)
    }
    fn from_source(source:ControlSource,bank:BankOwner,execution:&InferenceRequest,
        controls:OriginalTextControlGuard)->Result<Self,Error>{
        reserve(source.funding(),&[size_of::<Self>(),size_of::<Owner>(),size_of::<ControlSource>(),size_of::<Result<Self,Error>>(),
            size_of::<OriginalParallelControlProjection>(),size_of::<Option<Rc<Owner>>>(),
            size_of::<Custody>(),size_of::<BankOwner>(),size_of::<InferenceRequest>(),
            Layout::new::<[usize;2]>().extend(Layout::new::<Owner>()).map_err(|_|overflow())?.0.pad_to_align().size(),
            failure_control_bytes().ok_or_else(overflow)?])?;
        let actual=source.communication_source()?;
        controls.validate_reservation(execution.memory_reservation().ok_or_else(||
            control_error(ControlCause::Identity,actual.source(),source.funding()))?)
            .map_err(|cause|control_error(ControlCause::WorkingMemory(cause),actual.source(),source.funding()))?;
        let custody=Custody{source:actual.source().clone(),raw:controls.metadata_custody(),funding:source.funding().clone()};
        {
            let bank=bank.try_borrow().map_err(|_|control_error(ControlCause::Identity,&custody.source,&custody.funding))?;
            bank.budget_for_controls(&controls).map_err(|cause|
                control_error(ControlCause::WorkingMemory(cause),&custody.source,&custody.funding))?;
        }
        drop(actual);
        let request=OriginalParallelControlRequest::from_source(source)?;
        Ok(Self(Some(Rc::new(Owner{request,bank,execution:execution.clone(),controls,token_sources:RefCell::new(None),
            running:Cell::new(false),failed:Cell::new(false),custody}))))
    }
    pub(crate) fn install(&self)->Result<(OriginalParallelControlInstallation,OriginalParallelControlProjection),Error>{
        ...
        let projection=OriginalParallelControlProjection{owner:Rc::downgrade(self.0.as_ref().expect("live owner")),
            active:Some(active.clone()),model_roots:None,fallback:self.owner().request.fallback.retained(),custody:self.owner().custody.clone()};
        Ok((OriginalParallelControlInstallation(Some(active)),projection))
    }

}
struct Activation {active:Cell<bool>,custody:Custody}
...
impl OriginalParallelControlInstallation {
    pub(crate) fn close(&self){if let Some(active)=&self.0{active.active.set(false);}}
}
impl Drop for OriginalParallelControlInstallation {
    fn drop(&mut self){self.close();if let Some(active)=self.0.take(){drop(Rc::into_inner(active));}}
}
...
impl Drop for OriginalParallelControlProjection {
    fn drop(&mut self){if let Some(active)=self.active.take(){drop(Rc::into_inner(active));}}
}
...
impl Clone for OriginalParallelControlProjection {
    fn clone(&self)->Self {
        Self{model_roots:self.model_roots.clone(),owner:self.owner.clone(),active:self.active.clone(),
            fallback:self.fallback.retained(),custody:self.custody.clone()}
    }
}
impl OriginalParallelControlProjection {
    ...
    pub(crate) fn retained(&self)->Result<Self,Error>{
        reserve(&self.custody.funding,&[Self::alias_control_bytes().ok_or_else(overflow)?,
            size_of::<Result<Self,Error>>()])?;
        Ok(self.clone())
    }
    pub(crate) fn validate_model_source(&self,source:&OriginalParallelSource)->Result<(),Error>{
        reserve(&self.custody.funding,&[size_of::<(&Self,&OriginalParallelSource)>(),
            size_of::<OriginalParallelControlOwner>(),size_of::<Result<(),Error>>(),
            failure_control_bytes().ok_or_else(overflow)?])?;
        let owner=self.upgrade().map_err(|cause|Error::with_original_control_source(cause,false))?;
        if !owner.owner().request.source.model().is_some_and(|actual|actual.same_source(source))
            || !self.custody.funding.same_account(source.funding()) {
            return Err(control_error(ControlCause::Identity,&self.custody.source,&self.custody.funding));
        }
        Ok(())
    }
    ...
    where F:FnOnce(Option<(&mut Option<Box<Group>>,&WorkspaceMetadataFunding)>)->Result<T,E>, {
        ...
        let inputs=owner.owner().request.source.agreement_inputs().ok_or_else(||
            control_error(ControlCause::Identity,&self.custody.source,&self.custody.funding))?;
        let selected=source.source().manifest().select_group_operation(inputs.group(),CommunicationOperation::FailureAgreement)
            .map_err(|cause|failure(Cause::Rank(cause),&self.custody.source,&self.custody.funding))?;
        let group=source.group(selected.order()).ok_or_else(||
            control_error(ControlCause::Identity,&self.custody.source,&self.custody.funding))?.0;
        if group.has_original_parallel() || group.has_original_control() || group.original_control_request().is_some() {
            return Err(control_error(ControlCause::Identity,&self.custody.source,&self.custody.funding));
        }
        reserve(&self.custody.funding,&[group.retention_copy_bytes().ok_or_else(overflow)?])?;
        let mut group=group.try_copy_for_retention().map_err(|_|failure(Cause::Resource,&self.custody.source,&self.custody.funding))?;
    ...
    where F:FnOnce(Option<(&Group,&WorkspaceMetadataFunding)>)->Result<T,E>, {
        ...
        if prepared.has_original_parallel() {
            let source=owner.owner().request.source.model().ok_or_else(||
                control_error(ControlCause::Identity,&self.custody.source,&self.custody.funding))?;
            prepared.validate_control_model_source(source)?;
        } else {
            let actual=owner.owner().request.source.communication_source()?;
            let inputs=owner.owner().request.source.agreement_inputs().ok_or_else(||
                control_error(ControlCause::Identity,&self.custody.source,&self.custody.funding))?;
            reserve(&self.custody.funding,&[CommunicationManifest::group_operation_control_bytes().ok_or_else(overflow)?])?;
            let selected=actual.source().manifest().select_group_operation(inputs.group(),CommunicationOperation::FailureAgreement)
                .map_err(|cause|failure(Cause::Rank(cause),&self.custody.source,&self.custody.funding))?;
            if prepared.has_original_control() || !actual.matches_group(selected.order(),prepared) {
                return Err(control_error(ControlCause::Identity,&self.custody.source,&self.custody.funding));
            }
        }
        Ok(run(Some((prepared,&self.custody.funding))))
    }
    ...
    pub(crate) fn prepare_boundary_source(&self,prepared:&Group,
        ...
        self.with_context(prepared,|bound|{
            ...
            reserve(&self.custody.funding,&[size_of::<eredu_runtime::PreparedBoundarySource>(),
                size_of::<Result<eredu_runtime::PreparedBoundarySource,Error>>(),
                size_of::<OriginalParallelControlOwner>(),size_of::<Option<usize>>(),
                failure_control_bytes().ok_or_else(overflow)?])?;
            let owner=self.upgrade().map_err(|cause|Error::with_original_control_source(cause,false))?;
            let actual=owner.owner().request.source.communication_source()?;
            let order=actual.source().manifest().routes().iter().position(|entry|entry.id()==route.descriptor().id())
                .ok_or_else(||control_error(ControlCause::Identity,&self.custody.source,&self.custody.funding))?;
            if !actual.matches_route(order,route){return Err(control_error(ControlCause::Identity,&self.custody.source,&self.custody.funding));}
            actual.source().prepare_boundary_source(route.descriptor().id(),&self.custody.funding)
                .map_err(|cause|failure(Cause::BoundaryFrame(cause),&self.custody.source,&self.custody.funding))
        })?
    }
    ...
    pub(crate) fn prepare_boundary_headers(&self,prepared:&Group,
        frames:eredu_runtime::PreparedBoundaryFrames<crate::MlxTensor>,
        route:&crate::backend::runtime::distributed::topology::CommunicationRouteRealization,
        observer:&OriginalScopeObserver)->Result<super::super::OriginalBoundaryHeaders,Error>{
        self.with_context(prepared,|bound|{
            if bound.is_none(){return Err(control_error(ControlCause::Identity,&self.custody.source,&self.custody.funding));}
    ...
    where F:FnOnce(Option<&Group>)->Result<T,E>, {
        ...
        self.run_group(event,Some(group),executor,|prepared|{
            let Some((prepared,actual_funding))=prepared else {
                return Err(Error::with_original_control_source(self.fallback.retained().into_failure(),false));
            };
            if !funding.same_account(actual_funding) {
                return Err(control_error(ControlCause::Identity,&self.custody.source,&self.custody.funding));
            }
            let binding=prepared.original_control().ok_or_else(||
                Error::with_original_control_source(self.fallback.retained().into_failure(),false))?;
    ...
    fn upgrade(&self)->Result<OriginalParallelControlOwner,eredu_core::BackendFailure>{
        if !self.active.as_ref().is_some_and(|active|active.active.get()) {
            return Err(self.fallback.retained().into_failure());
        }
        self.owner.upgrade().map(|owner|OriginalParallelControlOwner(Some(owner)))
            .ok_or_else(||self.fallback.retained().into_failure())
    }
    pub(crate) fn validate_execution(&self,execution:&InferenceExecutionIdentity)->Result<(),Error>{
        reserve(&self.custody.funding,&[size_of::<&Self>(),size_of::<OriginalParallelControlOwner>(),
            size_of::<Result<(),Error>>(),failure_control_bytes().ok_or_else(overflow)?])?;
        let owner=self.upgrade().map_err(|cause|Error::with_original_control_source(cause,false))?;
        owner.owner().execution.validate(execution,owner.owner().execution.geometry())
            .map_err(|cause|control_error(ControlCause::WorkingMemory(cause),&self.custody.source,&self.custody.funding))
    }
    ...
    where F:FnOnce(Option<(&Group,&WorkspaceMetadataFunding)>)->Result<T,E>, {
        ...
        let invocation=match owner.owner().request.prepare(event,target) {
            Ok(value)=>value,
            Err(cause)=>{owner.owner().failed.set(true);return Err(cause.into_backend_failure());}
        };
        if owner.owner().failed.get() || owner.owner().running.replace(true){
            owner.owner().failed.set(true);return Err(self.fallback.retained().into_failure());
        }
        let _running=Running{running:&owner.owner().running,failed:&owner.owner().failed};
        let result=run_role(invocation,&owner.owner().bank,&owner.owner().controls,&self.custody,stream,
            |group,funding|run(Some((group,funding))));
        if match &result{Err(_)=>true,Ok(value)=>value.is_err()}{owner.owner().failed.set(true);}
        result
    }
}
struct Running<'a>{running:&'a Cell<bool>,failed:&'a Cell<bool>}
impl Drop for Running<'_>{fn drop(&mut self){if std::thread::panicking(){self.failed.set(true);}self.running.set(false);}}
...
fn run_native_role<I,T,E,F>(invocation:I,capacity:AgreementCapacity,bank:&BankOwner,
    controls:&OriginalTextControlGuard,custody:&Custody,run:F)
    ->Result<Result<T,E>,eredu_core::BackendFailure>
where I:'static,F:FnOnce(&I,&OriginalScopeObserver)->Result<Result<T,E>,Error>, {
    reserve(&custody.funding,&[size_of::<I>(),size_of::<F>(),size_of::<AgreementCapacity>(),
        size_of::<(&BankOwner,&OriginalTextControlGuard,&Custody)>(),
        size_of::<Result<Result<T,E>,eredu_core::BackendFailure>>()]).map_err(Error::into_backend_failure)?;
    run_native_role_with_pipeline(invocation,capacity,None,bank,controls,custody,run)
}
fn run_native_role_with_pipeline<I,T,E,F>(invocation:I,capacity:AgreementCapacity,
    pipeline:Option<safemlx::PreparedPipelineCachePlan>,bank:&BankOwner,
    controls:&OriginalTextControlGuard,custody:&Custody,run:F)
    ->Result<Result<T,E>,eredu_core::BackendFailure>
where I:'static,F:FnOnce(&I,&OriginalScopeObserver)->Result<Result<T,E>,Error> {
    let timeout=custody.source.manifest().completion_policy()
        .ok_or_else(||overflow().into_backend_failure())?.timeout();
    crate::backend::submission_recovery::native_role::run(
        invocation,
        ...
        }, pipeline,bank,controls,custody,&custody.funding,timeout,run)
}

mod sampling;
pub(crate) use sampling::{OriginalSamplingSource, BoundSamplingSource};
...
mod capture;
pub(crate) use capture::OriginalCaptureTransport;
