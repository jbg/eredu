...
impl std::fmt::Debug for OriginalParallelControlSource {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("OriginalParallelControlSource")
            .field("agreement", &self.body().inputs.group())
            .finish_non_exhaustive()
    }
}
impl OriginalParallelControlSource {
    fn body(&self) -> &Body {
        self.body.as_deref().expect("live control source")
    }
...
impl OriginalCommunicationOwner {
    ...
    pub(crate) fn prepare_control_source(
        self,
        group: CollectiveGroupId,
        pool: &eredu_runtime::working_memory::MemoryLedger,
    ) -> Result<OriginalParallelControlSource, Error> {
        reserve(
            self.funding(),
            &[
                size_of::<Self>(),
                size_of::<Body>(),
                size_of::<OriginalParallelControlSource>(),
                ...
                size_of::<agreement::OriginalAgreementInputs>(),
                size_of::<AgreementCapacity>(),
                Layout::new::<[usize; 2]>()
        ...
        let inputs = {
            ...
            let native = actual
                ...
                .ok_or_else(|| failure(Cause::Resource, actual.source(), self_.funding()))?
                .0;
            if native.has_original_parallel()
                || native.has_original_control()
                || native.original_control_request().is_some()
                || native.retained_transport_stream().is_none()
            {
        ...
        let retained = self_.source().clone();
        let funding = self_.funding().clone();
        Ok(OriginalParallelControlSource {
...
impl ControlSource {
    pub(super) fn funding(&self) -> &HostMetadataFunding {
        match self {
            Self::Model(source) => source.funding(),
            Self::Control(source) => &source.funding,
        }
    }
    pub(super) fn communication_source(&self) -> Result<OriginalCommunicationSource<'_>, Error> {
        self.communication_source_funded(self.funding())
    }
    pub(super) fn communication_source_funded(
        &self,
        funding: &HostMetadataFunding,
    ) -> Result<OriginalCommunicationSource<'_>, Error> {
        match self {
            Self::Model(source) => source.communication_source_funded(funding),
            Self::Control(source) => source.body().owner.borrow_funded(funding),
        }
    }
    pub(super) fn agreement_inputs(&self) -> Option<&agreement::OriginalAgreementInputs> {
        match self {
            Self::Model(source) => source.agreement_inputs(),
            Self::Control(source) => Some(&source.body().inputs),
        }
    }
    pub(super) fn model(&self) -> Option<&OriginalParallelSource> {
        match self {
            Self::Model(source) => Some(source),
            Self::Control(_) => None,
        }
    }
    pub(super) fn take_control_owner(&self) -> Result<(), Error> {
        match self {
...
pub(super) fn agreement_capacity(
    ...
    reserve(
        ...
        &[
            size_of::<(
                ...
                &agreement::OriginalAgreementInputs,
            )>(),
    ...
    Ok(inputs
        ...
        .union(inputs.capacity(source, true)?))
}
...
pub(super) fn agreement_capacity_for_group(
    source: &OriginalCommunicationSource<'_>,
    inputs: &agreement::OriginalAgreementInputs,
    group: CollectiveGroupId,
) -> Result<AgreementCapacity, Error> {
    source
        .funding()
        .reserve_metadata(agreement_capacity_for_group_control_bytes().ok_or_else(overflow)?)
        .map_err(Error::WorkspacePlanning)?;
    Ok(inputs
        .capacity_for_group(source, group, false)?
        .union(inputs.capacity_for_group(source, group, true)?))
}
...
pub(super) fn model_agreement_capacity(
    source: &OriginalCommunicationSource<'_>,
    inputs: &agreement::OriginalAgreementInputs,
) -> Result<AgreementCapacity, Error> {
    reserve(
    ...
    for group in source.source().manifest().groups() {
        if group.id() == inputs.group()
            || group.local_index().is_none()
            || !group
                .requirements()
                .operations()
                .iter()
                .any(|operation| operation.operation() == CommunicationOperation::FailureAgreement)
        {
            continue;
        }
...
pub(super) fn agreement_capacity_for_group_control_bytes() -> Option<usize> {
    let parts = [
        size_of::<(
            &OriginalCommunicationSource<'_>,
            &agreement::OriginalAgreementInputs,
