...
impl OriginalParallelControlProjection {
    ...
    pub(crate) fn exercise_inactive_retirement(
        ...
        let descriptor = CommunicationGroupDescriptor::new(CollectiveGroupId::new(7), 0,
            vec![0], Some(0), eredu_runtime::CommunicationGroupRequirements::new([
        ...
        let manifest = CommunicationManifest::new(1, 0, vec![descriptor], vec![]).unwrap()
            .with_completion_policy(eredu_runtime::CommunicationCompletionPolicy::new(
        ...
        funding.reserve_metadata(Self::retirement_control_bytes().unwrap()).unwrap();
        let custody = Custody { source: source.source().clone(), raw, funding };
        let fallback = eredu_core::SharedBackendFailure::new(
            eredu_core::BackendFailureKind::InvalidSession, ControlCause::Closed);
        let activation = Rc::new(Activation { active: Cell::new(true), custody: custody.clone() });
        let installation = OriginalParallelControlInstallation(Some(activation.clone()));
        let projection = Self { model_roots: None, owner: Weak::new(), active: Some(activation),
            fallback, custody };
        assert!(projection.is_active());
        assert!(projection.upgrade().is_err(), "lexical custody supplies no request authority");
        let escaped = projection.clone();
        let slot = RefCell::new(Some(projection));
        assert!(Self::take_inactive(&mut slot.borrow_mut()).is_none());
        assert!(slot.borrow().is_some(), "active installation must remain installed");
        drop(installation); // the same close-on-drop path as actual submissions
        assert!(!escaped.is_active());
        assert!(escaped.upgrade().is_err(), "escaped closed view cannot submit");
        ...
        assert!(pool.used_bytes().unwrap() > 0, "escaped view retains its source payer");
        escaped
    }
}
