...
use crate::backend::runtime::distributed::completion::{
    MlxNeuralCommunicationCompletion, OriginalCommunicationBool,
};
use eredu_runtime::replicated_session::{
    ParallelControlClaim, ParallelControlCursor, ParallelControlEvent, ParallelControlIdentity,
};
...
fn reserve_bytes(funding: &HostMetadataFunding, bytes: Option<usize>) -> Result<(), Error> {
    funding
        .reserve_metadata(bytes.ok_or_else(overflow)?)
        .map_err(Error::WorkspacePlanning)
}
fn reserve(funding: &HostMetadataFunding, parts: &[usize]) -> Result<(), Error> {
    reserve_bytes(funding, metadata_bytes(parts))
}
fn control_error(
    cause: ControlCause,
...
impl OriginalParallelSource {
    ...
    pub(crate) fn control_capacity(&self) -> Result<AgreementCapacity, Error> {
        ...
        let inputs = self
            .agreement_inputs()
            .ok_or_else(|| failure(Cause::Resource, source.source(), self.funding()))?;
...
impl OriginalParallelControlRequest {
    /// Complete local preparation envelope. The execution worker debits these
    /// two parts at their original points, preserving failed-prefix spending.
    pub(crate) fn prepare_control_bytes() -> Option<usize> {
        Self::prepare_frame_control_bytes()?.checked_add(Self::prepare_selection_control_bytes()?)
    }

    /// Fixed local transports; nested source and native operations are separate.
    pub(crate) fn prepare_selection_control_bytes() -> Option<usize> {
        metadata_bytes(&[
            size_of::<Option<&Group>>(),
            size_of::<CollectiveGroupId>(),
    ...
    /// Fixed local transports; nested source and native operations are separate.
    pub(crate) fn prepare_frame_control_bytes() -> Option<usize> {
        metadata_bytes(&[
            size_of::<Self>(),
            size_of::<ParallelControlClaim>(),
            size_of::<ParallelControlIdentity>(),
            size_of::<ParallelControlEvent>(),
            size_of::<OriginalParallelControlInvocation>(),
            size_of::<State>(),
            size_of::<OriginalControlBinding>(),
            ...
            size_of::<
                Result<std::cell::RefMut<'_, ParallelControlCursor>, std::cell::BorrowMutError>,
            >(),
            size_of::<
    ...
    fn from_source(source: ControlSource) -> Result<Self, Error> {
        reserve(
            ...
            &[
                ...
                size_of::<RefCell<ParallelControlCursor>>(),
                ParallelControlCursor::control_bytes().ok_or_else(overflow)?,
                eredu_core::SharedBackendFailure::control_bytes::<Failure>()
                    .ok_or_else(overflow)?,
                size_of::<eredu_core::SharedBackendFailure>(),
                failure_control_bytes().ok_or_else(overflow)?,
            ],
        ...
        let cursor = ParallelControlCursor::new().map_err(|cause| {
            control_error(ControlCause::WorkingMemory(cause), &retained, &funding)
        })?;
        let fallback = eredu_core::SharedBackendFailure::new(
            eredu_core::BackendFailureKind::Other,
            Failure {
                cause: Cause::Control(ControlCause::Closed),
                _source: retained.clone(),
    ...
    pub(crate) fn prepare(
        ...
        self.prepare_with_funding(event, target, &self.funding)
    }
    /// The same exact cursor and native source, with metadata paid from an
    /// enclosing operation's consumed host partition. This issues no native grant.
    pub(crate) fn prepare_with_funding(
        &self,
        event: ParallelControlEvent,
        target: Option<&Group>,
        funding: &HostMetadataFunding,
        ...
        let claim = cursor.claim(event).map_err(|_| {
            Error::with_original_control_source(self.fallback.retained().into_failure(), false)
        })?;
        drop(cursor);
        reserve_bytes(funding, Self::prepare_frame_control_bytes())?;
        let source = self.source.communication_source_funded(funding)?;
        let inputs = self
            .source
            .agreement_inputs()
            .ok_or_else(|| control_error(ControlCause::Identity, &self.retained, funding))?;
        reserve_bytes(funding, Self::prepare_selection_control_bytes())?;
        let target_id = match target {
            Some(group) => source
                ...
                .iter()
                .enumerate()
                .find_map(|(order, descriptor)| {
                    source
                        .matches_group(order, group)
        ...
        let selected = source
            .source()
            .manifest()
            .select_group_operation(target_id, CommunicationOperation::FailureAgreement)
            .map_err(|cause| failure(Cause::Rank(cause), &self.retained, funding))?;
        let group = source
        ...
        let group = group
            ...
            .map_err(|_| failure(Cause::Resource, &self.retained, funding))?;
        let context = group
            .try_copy_for_retention()
            .map_err(|_| failure(Cause::Resource, &self.retained, funding))?;
        let state = Rc::new(State {
            group,
            group_order: selected.order(),
        ...
        let context = context.with_original_control(OriginalControlBinding {
            state: Rc::downgrade(&state),
            retained: self.retained.clone(),
            fallback: self.fallback.retained(),
            funding: funding.clone(),
        });
        Ok(OriginalParallelControlInvocation {
            context,
            state: Some(state),
            funding: funding.clone(),
        })
...
impl OriginalParallelControlInvocation {
    /// Fixed local transports; nested source and native operations are separate.
    pub(crate) fn context_control_bytes<T, E>(closure_bytes: usize) -> Option<usize> {
        metadata_bytes(&[
            closure_bytes,
            size_of::<T>(),
            size_of::<E>(),
    ...
    fn state(&self) -> &State {
        self.state.as_deref().expect("live control occurrence")
    }
    pub(crate) fn capacity(&self) -> AgreementCapacity {
        self.state().capacity
    }
    pub(crate) fn claim(&self) -> &ParallelControlClaim {
        &self.state().claim
    }
        ...
        F: FnOnce(&Group, &HostMetadataFunding) -> Result<T, E>,
    {
        ...
        reserve_bytes(
            &self.funding,
            Self::context_control_bytes::<T, E>(size_of::<F>()),
        )?;
        if state.closed.get()
            || state.bound.get().is_some()
            || state.claim.identity() != state.identity
        {
            return Err(control_error(
                ControlCause::Closed,
                &state.retained,
        ...
        let stream = StreamCopyPlan::capture(stream)
            .map_err(|_| control_error(ControlCause::Identity, &state.retained, &self.funding))?;
        state
            .bound
            .set(Bound {
        ...
        if result.is_ok() && !state.consumed.get() {
            return Err(control_error(
                ControlCause::Incomplete,
                &state.retained,
                &self.funding,
            ));
        }
...
impl Loan {
    fn state(&self) -> &State {
        self.0.as_deref().expect("live control loan")
    }
}
impl Drop for Loan {
    fn drop(&mut self) {
...
impl OriginalControlBinding {
    /// Fixed state validation; the actual retained stream comparison and
    /// communication-source loan are independently quoted by their producers.
    pub(crate) fn validation_control_bytes() -> Option<usize> {
        State::validation_control_bytes()
    }

    /// Fixed local transports; nested source and native operations are separate.
    pub(crate) fn agreement_control_bytes() -> Option<usize> {
        metadata_bytes(&[
            size_of::<(&Self, &Group, bool, &Stream)>(),
            size_of::<Result<(OriginalCommunicationBool, MlxNeuralCommunicationCompletion), Error>>(
            ),
            size_of::<agreement::PreparedOriginalAgreement<'_>>(),
            size_of::<AgreementCapacity>(),
            failure_control_bytes()?,
        ])
    }
    ...
    /// Fixed local transports; nested source and native operations are separate.
    pub(crate) fn loan_control_bytes() -> Option<usize> {
        metadata_bytes(&[
            size_of::<Loan>(),
            size_of::<Option<Rc<State>>>(),
            size_of::<Result<Loan, Error>>(),
            failure_control_bytes()?,
        ])
    }

    fn loan(&self) -> Result<Loan, Error> {
        reserve_bytes(&self.funding, Self::loan_control_bytes())?;
    ...
    /// Same fixed callback and source-loan frames paid by `with_group`.
    pub(crate) fn group_control_bytes<T, E>(closure_bytes: usize) -> Option<usize> {
        metadata_bytes(&[
            closure_bytes,
            size_of::<T>(),
            size_of::<E>(),
        ...
        F: FnOnce(Option<&Group>) -> Result<T, E>,
    {
        reserve_bytes(
            &self.funding,
            Self::group_control_bytes::<T, E>(size_of::<F>()),
        )?;
        let loan = self.loan()?;
        let state = loan.state();
        state.validate(prepared, executor)?;
        let source = state.source.communication_source_funded(&state.funding)?;
        if event != state.claim.event()
            || !funding.same_account(&state.funding)
    ...
    pub(crate) fn agree(
        ...
        reserve_bytes(&self.funding, Self::agreement_control_bytes())?;
        let loan = self.loan()?;
        let state = loan.state();
        state.validate(group, executor)?;
        if state.consumed.replace(true) {
            return Err(control_error(
                ControlCause::Exhausted,
                &state.retained,
                &self.funding,
            ));
        }
        ...
        let actual_group = source
            .group(state.group_order)
            .ok_or_else(|| control_error(ControlCause::Identity, &state.retained, &self.funding))?
            .1
            .id();
        ...
        let actual = AgreementCapacity {
            ...
            backing: operation.backing_capacity(),
        };
        if !state.capacity.covers(actual) {
            return Err(control_error(
                ControlCause::Identity,
                &state.retained,
                &self.funding,
...
impl State {
    ...
    fn validation_control_bytes() -> Option<usize> {
        metadata_bytes(&[
            ...
            size_of::<Option<&Bound>>(),
            failure_control_bytes()?,
        ])
    }

    fn validate(&self, group: &Group, stream: &Stream) -> Result<(), Error> {
        reserve_bytes(&self.funding, Self::validation_control_bytes())?;
        let bound = self
            .bound
            .get()
            .ok_or_else(|| control_error(ControlCause::Unbound, &self.retained, &self.funding))?;
        reserve(
            &self.funding,
            &[bound
                .stream
        ...
        let source = self.source.communication_source_funded(&self.funding)?;
        if self.claim.identity() != self.identity
...
mod role;
pub(crate) use role::SpeculativeModelRole;
pub(crate) use role::{
    CaptureSourceOwner, CaptureTransportBinding, OriginalCaptureTransport,
    OriginalParallelControlInstallation, OriginalParallelControlOwner,
    OriginalParallelControlProjection,
};

pub(crate) use role::{BoundSamplingSource, OriginalSamplingSource};
