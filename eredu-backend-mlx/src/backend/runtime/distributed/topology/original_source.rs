...
pub(in crate::backend::runtime::distributed) enum Cause {
    ...
    BoundaryHeader {
        cause: safemlx::OwnedHostCopyCause,
        #[source]
    ...
    Resource,
    #[error(
        "actual collective binding failed at occurrence {index:?}, equation {ordinal:?}: {cause}"
    )]
    NativeBinding {
        index: Option<usize>,
    ...
    #[error(
        "parallel workspace {operation:?} on {group:?}, local rank {rank}/{partitions}, lacks floating scalar evidence: input rank {input_rank}, width {input_width:?}, elements {input_elements:?}"
    )]
...
fn failure(
    ...
    let cause = match cause {
        Cause::Resource => Cause::ResourceAt(std::panic::Location::caller()),
        other => other,
    };
...
pub(crate) struct OriginalCommunicationSource<'a> {
    ...
    // Authentic source preparation custody remains distinct from the admitted
    // operation's metadata loan. Both retire after all retained source fields.
...
impl ParallelCommunicators {
    ...
    /// Fixed transports of the retained-table binder; this performs no binding.
    pub(crate) fn bind_original_source_control_bytes() -> Option<usize> {
        let controls = [
    ...
    fn bind_original_source_retaining<'a>(
        ...
        funding
            .reserve_metadata(
                Self::bind_original_source_control_bytes()
                    .ok_or(Error::WorkspacePlanning(HostMetadataFundingError::Overflow))?,
            )
            .map_err(Error::WorkspacePlanning)?;
        let fail = |cause| failure(cause, retained, funding);
        if !retained.same_source(&self.source)
            || self.source.manifest() != selected
            || self.session_identity != self.source.session_identity()
            || self.world_size != selected.world_size()
            || self.global_rank != selected.rank()
            || !self
                .control_world
                .native_group()
                .shares_native_handle(world)
            || !self
                .control_world
                .retained_source()
                .is_some_and(|source| source.same_source(&self.source))
            || self.groups.len() != selected.groups().len()
            || self.routes.len() != selected.routes().len()
        {
            return Err(fail(Cause::Identity));
        }
        if authority.ensure_active().is_err()
            || self.control_world.native_group().terminal_submission()
        ...
        for (order, descriptor) in selected.groups().iter().enumerate() {
            let (_, wave) = self
                .source
                .group(order)
                .ok_or_else(|| fail(Cause::Resource))?;
            let actual = self
                .groups
                .get(&descriptor.id())
                .ok_or_else(|| fail(Cause::Resource))?;
            ...
            if &actual.descriptor != descriptor
                || !group.shares_native_world(&self.control_world)
        ...
        for (order, descriptor) in selected.routes().iter().enumerate() {
            ...
            let route = self
                ...
                .get(&descriptor.id())
                .ok_or_else(|| fail(Cause::Resource))?;
            let endpoint = if descriptor.source() == selected.rank() {
                Some(CommunicationRouteEndpoint::Source)
            } else if descriptor.destination() == selected.rank() {
                ...
                || match (&route.group, endpoint) {
                    ...
                    (Some(group), Some(endpoint)) => {
                        !group.shares_native_world(&self.control_world)
                            ...
                            || route.peer_rank
                                != Some(if endpoint == CommunicationRouteEndpoint::Source {
                                    1
                                } else {
                                    0
                                })
        ...
        Ok(OriginalCommunicationSource {
            actual: self,
            authority,
            source: retained.clone(),
            registered_buffers: None,
            preparation_funding: funding.clone(),
            funding: funding.clone(),
        })
    }
}
impl<'a> OriginalCommunicationSource<'a> {
    /// The closed retained owner's genuine source construction account.
    /// This is an identity/custody witness, never an execution metadata grant.
    pub(crate) fn preparation_funding(&self) -> &HostMetadataFunding {
        &self.preparation_funding
    }

    /// Each revalidation can retain its own independently escaped failure.
    ...
    pub(crate) fn validate(&self) -> Result<(), Error> {
        ...
        self.funding
            ...
            .map_err(Error::WorkspacePlanning)?;
        if self.authority.ensure_active().is_err()
            || self
                .actual
                .control_world
                .native_group()
    ...
    pub(crate) fn matches_group(&self, order: usize, value: &Group) -> bool {
        self.group(order).is_some_and(|(actual, descriptor, wave)| {
            value
                .native_group()
                .shares_native_handle(actual.native_group())
                && value.matches_retained_group(&self.source, descriptor, wave)
        })
    }
    pub(crate) fn matches_route(
        &self,
        order: usize,
        ...
        self.route(order).is_some_and(|(actual, descriptor, wave)| {
            &value.descriptor == descriptor
                && value.endpoint == actual.endpoint
    ...
    pub(crate) fn world(&self) -> &'a Group {
        &self.actual.control_world
    }
    pub(crate) fn group(
        &self,
        order: usize,
    ) -> Option<(&'a Group, &CommunicationGroupDescriptor, bool)> {
        let (descriptor, wave) = self.source.group(order)?;
        Some((
            self.actual.groups.get(&descriptor.id())?.native.as_ref()?,
            descriptor,
            wave,
        ))
    }
    pub(crate) fn route(
        ...
        &'a CommunicationRouteRealization,
        &CommunicationRouteDescriptor,
        bool,
    )> {
...
pub(crate) use constructor_storage::{
    OriginalCommunicationConstructed, OriginalCommunicationConstructor,
    OriginalCommunicationEvaluation,
};
...
pub(crate) use operation_storage::{
    AcceptedCommunicationSource, OriginalCommunicationCompletedOperation,
    OriginalCommunicationOperation,
};

mod rank_collective;
pub(crate) use rank_collective::{
    AcceptedRankCollective, OriginalRankCollective, PreparedRankCollective,
};
...
pub(crate) use preparation::{
    OriginalPreparationFrame, PreparationCompletion, PreparedOriginalPreparationGather,
};

mod route_exchange;
pub(crate) use route_exchange::{
    AcceptedRouteExchange, AcceptedRouteSource, ConstructedRouteExchange, OriginalRouteExchange,
    OriginalRouteRound, OwnedOriginalRouteLayoutRound, PreparedRouteRound,
};
pub(crate) use route_exchange::{OriginalGroupExchange, OwnedOriginalExchangeLayoutRound};

mod boundary_headers;
pub(crate) use boundary_headers::{OriginalBoundaryHeader, OriginalBoundaryHeaders};
