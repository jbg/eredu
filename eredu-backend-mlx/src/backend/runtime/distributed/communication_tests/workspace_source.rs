...
use eredu_nn::workspace::{
    WorkspaceCollectiveView, WorkspaceDtype, WorkspaceFloatingType, WorkspaceLayout,
    WorkspaceLayoutList, WorkspaceOperationKindView, WorkspaceOperationView,
...
fn workspace_sum_source_binds_lazy_partials_and_retains_refused_sources() {
    super::collectives::run_subgroup_workers(
        2,
        "backend::runtime::distributed::communication_tests::workspace_source::worker",
        RANK,
    );
}
#[test]
fn worker() {
    ...
    let requirement = CommunicationOperationRequirement::tensors(
        ...
        [TensorDtype::F32],
        CommunicationTensorLimits::new(1, 2, 6, None).unwrap(),
        true,
    )
    .unwrap();
    ...
    let source = actual
        ...
        .unwrap();
    let layout = [WorkspaceLayout::new(&[2, 3], WorkspaceDtype::Float32)
        .unwrap()
        .with_representation(Some(WorkspaceRepresentation::new(
            WorkspaceFloatingType::Float32,
            false,
        )))];
    let equation = WorkspaceOperationView {
        kind: WorkspaceOperationKindView::Collective(WorkspaceCollectiveView::Sum {
            partitions: 2,
            rank,
        }),
        inputs: WorkspaceLayoutList::Owned(&layout),
        outputs: WorkspaceLayoutList::Owned(&layout),
    };
    let quote = source.sum_workspace_storage(id, equation).unwrap();
    assert_eq!(quote.native().graph_population(), (1, 1));
    assert_eq!(quote.native().output_geometry(), (2, 6));
    ...
    assert!(
        bound.with_completion().is_err(),
        "layout binding must not settle the lazy predecessor to qualify standalone completion"
    );
    let wrong = Array::from_slice(&[7_i32, -11, 23, 31, -5, 17], &[2, 3]);
    let mismatch = source
        .sum_workspace_storage(id, equation)
        .unwrap()
        .bind_actual(&source, &wrong)
        .err()
        .expect("same shape cannot substitute a different physical dtype");
    let logical = [WorkspaceLayout::new(&[2, 3], WorkspaceDtype::Float32).unwrap()];
    let ambiguous = WorkspaceOperationView {
        inputs: WorkspaceLayoutList::Owned(&logical),
        outputs: WorkspaceLayoutList::Owned(&logical),
        ..equation
    };
    let missing = source
        .sum_workspace_storage(id, ambiguous)
        .err()
        .expect("logical F32 cannot invent actual floating representation");
    ...
    let diagnostic = loop {
        ...
        if text.contains("lacks floating scalar evidence") {
            break text;
        }
    ...
    let changed = WorkspaceOperationView {
        kind: WorkspaceOperationKindView::Collective(WorkspaceCollectiveView::Sum {
            partitions: 2,
            rank: 1 - rank,
        }),
    ...
    let foreign = source
        .sum_workspace_storage(id, changed)
        .err()
        .expect("equal geometry must retain exact local rank");
    drop(source);
    drop((actual, authority, funding, world, lazy, base, wrong));
    assert!(
        pool.fixture_host_charge().unwrap() > 0,
        "escaped per-query failures retain their exact source/H"
    );
