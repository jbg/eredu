...
use safemlx::{
    OriginalScopeObserver, PreparedInputRuntime, PreparedOriginalBufferBudget,
...
fn accepted_ring_operation_completes_after_source_fence_and_retires_custody() {
    super::collectives::run_subgroup_workers(
        2,
        "backend::runtime::distributed::communication_tests::completed_operation::worker",
        RANK,
    );
}
...
#[test]
fn worker() {
    run_rank(OriginalRankCollective::Sum);
}
...
fn selected_rank_gather_keeps_axis_order_and_accepted_custody_after_fence() {
    super::collectives::run_subgroup_workers(
        2,
        "backend::runtime::distributed::communication_tests::completed_operation::gather_worker",
        RANK,
    );
}
#[test]
fn gather_worker() {
    run_rank(OriginalRankCollective::GatherAxisZero);
}
...
fn run_rank(collective: OriginalRankCollective) {
    ...
    let semantic = match collective {
        OriginalRankCollective::Sum => CommunicationOperation::AllReduceSum,
    ...
    let requirement = CommunicationOperationRequirement::tensors(
        semantic,
        [TensorDtype::F32],
        CommunicationTensorLimits::new(1, 2, 6, None)
            .unwrap()
            .with_output_tensor_elements(if collective == OriginalRankCollective::Sum {
                6
            } else {
                12
            })
    ...
    let actual =
        std::rc::Rc::new(ParallelCommunicators::from_manifest(&manifest, &world, &stream).unwrap());
    ...
    match collective {
        ...
        OriginalRankCollective::GatherAxisZero => assert_eq!(
            expected,
            [7., 31., -11., -5., 23., 17., 107., 131., 89., 95., 123., 117.]
        ),
    }
    drop(ordinary);

    let pool = crate::memory_fixture::ledger(256 << 20, 0).unwrap();
    ...
    assert_eq!(pool.fixture_host_charge().unwrap(), scratch.bytes() as u64);
    drop((scratch, other_scratch, other_world));
    let funding = pool
        .prepare_workspace_metadata(
            &InferenceExecutionIdentity::default(),
            crate::memory_fixture::resolved_limits(256 << 20),
        )
        .unwrap();
    let standalone = actual
    ...
    let standalone_cost = pool.fixture_host_charge().unwrap() - before;
    ...
    let empty_pool = crate::memory_fixture::ledger(256 << 20, 0).unwrap();
    let missing = crate::backend::runtime::distributed::topology::OriginalCommunicationOwner::bind(
        &actual, &manifest, &world, &authority, &funding,
    )
    .unwrap()
    .pin_registered_buffers(&empty_pool)
    .err()
    .expect("unregistered buffers cannot receive H credit");
    assert_eq!(empty_pool.fixture_host_charge().unwrap(), 0);
    ...
    let pinned_cost = pool.fixture_host_charge().unwrap() - before;
    assert_eq!(
        standalone_cost - pinned_cost,
        scratch_bytes,
    ...
    assert!(pool.fixture_host_charge().unwrap() >= scratch_bytes);
    ...
    let unknown = source
        .prepare_rank_collective(CollectiveGroupId::new(84), &input, collective, &allocator)
        .err()
        .expect("equal geometry does not invent a selected group");
    let other = if collective == OriginalRankCollective::Sum {
        OriginalRankCollective::GatherAxisZero
    } else {
        OriginalRankCollective::Sum
    };
    let unselected = source
        .prepare_rank_collective(CollectiveGroupId::new(83), &input, other, &allocator)
        .err()
        .expect("selected group must admit this exact semantic operation");
    let operation = source
        .prepare_rank_collective(CollectiveGroupId::new(83), &input, collective, &allocator)
        .unwrap();
    let backing = operation.backing_capacity();
    assert!(std::ptr::eq(operation.runtime(), &allocator));
    let graph = PreparedSubmissionGraphQuota::try_new(operation.graph_capacity(), ())
        .unwrap()
        .try_allocate()
        .unwrap();
    let records = PreparedSubmissionRecordQuota::try_new(operation.record_capacity(), ())
        .unwrap()
        .try_allocate()
        .unwrap();
    let budget = PreparedOriginalBufferBudget::try_new(&allocator, backing, ())
        .unwrap()
        .try_allocate()
        .unwrap();
    ...
    let refused = source
        .prepare_rank_collective(CollectiveGroupId::new(83), &input, collective, &allocator)
        .err()
        .expect("fenced source cannot construct a new operation");
    drop(source);
    drop((owner, actual, authority, funding, world));
    assert!(
        pool.fixture_host_charge().unwrap() > 0,
        "accepted value and destination retain their source/H"
    );
    ...
    drop((
        observer, scope, graph, records, budget, failure, stream, allocator,
    ));
    ...
    assert!(
        pool.fixture_host_charge().unwrap() > 0,
        "the independent fresh-source refusal retains its paid error"
    );
    ...
    assert_eq!(
        ...
        0,
        "accepted completion and refusal retire without source backedges"
    );
