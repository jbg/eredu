...
fn original_route_pair_keeps_both_roots_and_completes_after_source_fence() {
    super::collectives::run_subgroup_workers(
        2,
        "backend::runtime::distributed::communication_tests::original_route::worker",
        RANK,
    );
}
#[test]
fn worker() {
    ...
    let descriptor = CommunicationGroupDescriptor::new(
        ...
        vec![0, 1],
        Some(rank),
        CommunicationGroupRequirements::new([subgroup_requirement()]).unwrap(),
    )
    .unwrap();
    ...
    let manifest = CommunicationManifest::new(
        ...
        rank,
        vec![descriptor],
        vec![CommunicationRouteDescriptor::new(route_id, 0, 0, 1, route_requirement()).unwrap()],
    )
    .unwrap()
    ...
    input.evaluated().unwrap();
    eprintln!(
        "ORIGINAL_ROUTE_STAGE rank={rank} input-settled bytes={}",
        values.len() * 4
    );
    ...
    let bad = exchange
        .round_storage(&source, 1, &input)
        .err()
        .expect("no invented second round");
    let round = exchange.round_storage(&source, 0, &input).unwrap();
    assert_eq!(round.round(), 0);
    assert!(round.source().same_source(source.source()));
    let prepared = round.prepare(&source, &allocator).unwrap();
    eprintln!("ORIGINAL_ROUTE_STAGE rank={rank} pair-prepared");
    let graph = PreparedSubmissionGraphQuota::try_new(prepared.graph_capacity(), ())
        .unwrap()
        .try_allocate()
        .unwrap();
    let records = PreparedSubmissionRecordQuota::try_new(prepared.record_capacity(), ())
        .unwrap()
        .try_allocate()
        .unwrap();
    let budget = PreparedOriginalBufferBudget::try_new(&allocator, prepared.backing_capacity(), ())
        .unwrap()
        .try_allocate()
        .unwrap();
    ...
    drop(exchange);
    drop(source);
    drop((actual, authority, funding, world));
    ...
    for (which, array) in output.outputs().iter().enumerate() {
        ...
        for (index, actual) in actual.enumerate() {
            ...
            assert_eq!(
                actual,
                value(row * COLS + column, expected_rank),
                "output {which} logical index {index}"
            );
        }
    }
    ...
    assert!(
        ...
        "escaped round refusal retains its source and H"
    );
    drop(bad);
    crate::backend::submission_recovery::reap();
    safemlx::reclaim_allocation_owners();
    ...
    println!(
        "ORIGINAL_ROUTE_PAIR_PASS rank={rank} elements={} bytes={} strided=true",
        ROWS * COLS,
        ROWS * COLS * 4
    );
}
