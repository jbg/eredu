...
fn original_stream_consumers_spend_exact_slots_and_retain_resources_after_primary_drop() {
    ...
    let first = Stream::new_with_device(&device);
    let second = Stream::new_with_device(&device);
    let foreign = Stream::new_with_device(&device);
    ...
    let waits = safemlx::OperationEvent::wait_record_layout(2)
        .expect("current finite native WaitRecord producer");
    let mut prepared = PreparedCompletionResources::prepare_original_with_consumers(
        &source,
    ...
    prepared
        ...
        .unwrap();
    for stream in [&producer, &first, &second] {
    ...
    let completion = ready
        .submit_original(&source, &observer, &producer, std::slice::from_ref(&value))
        .unwrap();
    ...
    assert_eq!(
        consumers.slots.borrow().ready.remaining(),
        2,
        "foreign stream never consumes the caller's admitted wait slot"
    );
    completion.wait_on(&first, &observer).unwrap();
    completion.wait_on(&second, &observer).unwrap();
    assert!(
        completion.wait_on(&first, &observer).is_err(),
        "accepted waits cannot refill their finite bank"
    );
    assert_eq!(consumers.slots.borrow().ready.remaining(), 0);
    assert_eq!(consumers.slots.borrow().active.as_ptr(), buffer);
    let a = value.multiply(&left, &first).unwrap();
    let b = value.add(&right, &second).unwrap();
    let outputs = [a, b];
    let final_plan =
        safemlx::OperationEvent::eval_traversal_layout(safemlx::OperationEvalTraversalLimits {
            roots: 2,
        ...
        .unwrap();
    let final_event = safemlx::transforms::async_eval_with_original_prepared_traversal(
        &outputs,
        &observer,
        &producer,
        &final_plan,
    ...
    scope.seal();
    drop((final_event, outputs, value, left, right));
