...
impl Case {
    fn input(self) -> Array {
        match self {
            Self::Flag(value) => Array::from_slice(&[value], &[1]),
            Self::Agreement(value) => Array::from_slice(&[value], &[1]),
        }
    }
    fn zero(self) -> Array {
        match self {
            Self::Flag(_) => Array::from_slice(&[0_f32], &[1]),
            Self::Agreement(_) => Array::from_slice(&[0_i32], &[1]),
        }
    }
    fn prepare(self, source: &OriginalCommunicationSource<'_>) -> PreparedCommunicationScalar {
        match self {
            Self::Flag(_) => PreparedCommunicationScalar::prepare_flag(source),
            Self::Agreement(_) => PreparedCommunicationScalar::prepare_agreement(source, 0),
        }
        .unwrap()
    }
    fn ordinary(self, input: &Array) -> bool {
        ...
        match self {
            Self::Flag(_) => {
                let (result, completion) = completion.with_f32_flag(input.clone());
                completion.wait().unwrap();
                result.resolve().unwrap()
            }
            Self::Agreement(_) => {
                let (result, completion) = completion.with_failure_agreement(input.clone(), 1);
                completion.wait().unwrap();
                result.resolve().unwrap()
            }
        }
...
fn prepared_scalar_readouts_match_ordinary_and_keep_source_after_completion_and_refusal() {
    ...
    .enumerate()
    {
        ...
        let invalid = if index == 0 {
            let ready = PreparedCompletionResources::prepare_original(
                &source,
            ...
            .unwrap();
            Some((
                PreparedCommunicationScalar::prepare_flag(&source).unwrap(),
                ready,
                Array::from_slice(&[7_i32], &[1]),
            ))
        } else {
            None
        };
        ...
        if let Some((scalar, ready, output)) = invalid {
            refused = Some(
                scalar
                    .submit(ready, &source, &observer, &stream, output)
                    .unwrap_err(),
            );
        }
        ...
        let (result, completion) = scalar
            .submit(ready, &source, &observer, &stream, output)
            .unwrap();
        assert!(
            result.pending(),
            "submission does not prematurely publish the scalar result"
        );
        completion.wait().unwrap();
    ...
    for (result, expected) in escaped {
        assert_eq!(result.resolve().unwrap(), expected);
    }
    assert!(
        pool.fixture_host_charge().unwrap() > 0,
        "refused dtype retains its paid source error after scalar receipts retire"
    );
    drop(refused);
    crate::backend::submission_recovery::reap();
    safemlx::reclaim_allocation_owners();
