...
pub(super) enum Cause {
    #[error("distributed completion host destination does not match its prepared population or source")]
    Identity,
...
#[derive(Debug, thiserror::Error)]
#[error("{cause}")]
struct Failure { #[source] cause: Cause, custody: ResourceCustody }
pub(super) fn error(cause: Cause, custody: &ResourceCustody) -> Error {
    Error::with_original_control_source(eredu_core::BackendFailure::new(
        eredu_core::BackendFailureKind::Other, Failure { cause, custody: custody.clone() }), false)
}
pub(super) fn error_control_bytes()->Option<usize> {
    let controls=[size_of::<Failure>(),size_of::<Cause>(),size_of::<ResourceCustody>(),
        size_of::<Error>(),size_of::<(&ResourceCustody,Cause)>(),
...
#[derive(Clone, Copy, Debug)]
pub(crate) enum CompletionResourceKind { Communication, Distributed }
#[derive(Debug)]
...
impl OrphanDestination {
    fn bytes(kind: CompletionResourceKind) -> Option<usize> {
        match kind {
            CompletionResourceKind::Communication => Destination::<MlxCommunicationCompletion>::control_bytes(),
            CompletionResourceKind::Distributed => Destination::<generic::DistributedCompletionOrphan>::control_bytes(),
        }
    }
    fn new(kind: CompletionResourceKind, custody: &ResourceCustody) -> Self {
        match kind {
            CompletionResourceKind::Communication => Self::Communication(Destination::new(Some(custody.clone()))),
            CompletionResourceKind::Distributed => Self::Distributed(Destination::new(Some(custody.clone()))),
        }
    }
}
...
pub(super) enum PreparedCompletionRecovery {
    ...
    Original {
        ...
        pending:crate::backend::submission_recovery::observed::PreparedObservedRecovery<NativeOwner,ResourceCustody>,
...
impl PreparedCompletionRecovery {
    ...
    fn allocation_identity(&self)->usize {
        match self { Self::Owned(value)=>value.allocation_identity(),Self::Original { resources,.. }=>
            std::ptr::from_ref(resources) as usize }
    }
}
impl<'source, 'native> PreparedCompletionResources<'source, 'native> {
    pub(crate) fn control_bytes(layout: CompletionResourceLayout<'_>, kind: CompletionResourceKind) -> Option<usize> {
        Self::controls(layout,kind,None)
    }
    fn controls(layout: CompletionResourceLayout<'_>,kind:CompletionResourceKind,
        original:Option<&safemlx::OperationEvalTraversalLayout>)->Option<usize> {
        let fixed = [size_of::<Self>(), size_of::<ReadyCompletionResources>(),
            size_of::<NativeResources>(), // Rc payload
            2 * size_of::<usize>(), // Rc strong/weak header
            size_of::<NativeResources>(), // constructor transport
            size_of::<NativeOwner>(), size_of::<Option<Rc<NativeResources>>>(),
            size_of::<Result<(), Error>>(), size_of::<Option<Destination<std::rc::Weak<NativeResources>>>>(),
            vector_controls::<Array>()?, vector_controls::<Vec<usize>>()?, vector_controls::<bool>()?,
            vector_controls::<usize>()?, vector_controls::<Group>()?,
            vector_controls::<CommunicationRouteRealization>()?, vector_controls::<Stream>()?,
            size_of::<Result<Self, Error>>(), size_of::<Result<ReadyCompletionResources, Error>>(),
            size_of::<CompletionResourceLayout<'_>>(), size_of::<CompletionResourceKind>(),
            size_of::<ResourceCustody>(), size_of::<Failure>(), size_of::<Cause>(),
            size_of::<Result<(), TryReserveError>>(), size_of::<Option<usize>>(),
            ...
            size_of::<std::slice::Iter<'_, usize>>(), size_of::<[usize; 4]>(),
            size_of::<std::cell::RefMut<'_, destinations::Destinations<std::rc::Weak<NativeResources>>>>(),
            size_of::<destinations::Iter<'_, std::rc::Weak<NativeResources>>>(),
            Destination::<std::rc::Weak<NativeResources>>::control_bytes()?, OrphanDestination::bytes(kind)?,
            recovery_controls(original)?,
            eredu_core::BackendFailure::source_retention_peak_bytes::<Failure>()?,
            Layout::array::<Array>(layout.arrays).ok()?.size(),
            Layout::array::<Vec<usize>>(layout.counts.len()).ok()?.size(),
            Layout::array::<bool>(layout.counts.len()).ok()?.size(),
            Layout::array::<Group>(layout.groups).ok()?.size(),
            Layout::array::<CommunicationRouteRealization>(layout.routes).ok()?.size(),
            Layout::array::<Stream>(layout.streams).ok()?.size(),
        ];
        let bytes = fixed.into_iter().try_fold(size_of_val(&fixed), usize::checked_add)?;
        layout.counts.iter().try_fold(bytes, |sum,&length| sum.checked_add(Layout::array::<usize>(length).ok()?.size()))
    }
    pub(crate) fn prepare(source: &'source OriginalCommunicationSource<'native>, layout: CompletionResourceLayout<'_>,
        kind: CompletionResourceKind) -> Result<Self, Error> {
    ...
    pub(crate) fn prepare_original(source:&'source OriginalCommunicationSource<'native>,
        layout:CompletionResourceLayout<'_>,traversal:safemlx::OperationEvalTraversalLayout)->Result<Self,Error> {
        Self::prepare_with(source,layout,CompletionResourceKind::Communication,Some(traversal))
    }
    ...
    pub(crate) fn prepare_original_with_consumers(source:&'source OriginalCommunicationSource<'native>,
        layout:CompletionResourceLayout<'_>,traversal:safemlx::OperationEvalTraversalLayout,
        waits:safemlx::OperationWaitRecordLayout)->Result<Self,Error> {
    ...
    fn prepare_with(source:&'source OriginalCommunicationSource<'native>,layout:CompletionResourceLayout<'_>,
        kind:CompletionResourceKind,original:Option<safemlx::OperationEvalTraversalLayout>)->Result<Self,Error> {
        source.validate()?;
        source.funding().reserve_metadata(Self::controls(layout,kind,original.as_ref()).ok_or_else(overflow)?)
        ...
        for &length in layout.counts {
            ...
            values.resize(length,0); counts.push(values); count_filled.push(false);
        }
        ...
        Ok(Self { source, arrays, counts, count_filled, groups, routes, streams,
            limits:[layout.arrays,layout.groups,layout.routes,layout.streams], original, consumers:None, registry, orphan, custody })
    }
    pub(crate) fn push_array(&mut self, value:Array)->Result<(),Error> {
        if self.arrays.len()==self.limits[0] { return Err(error(Cause::Identity,&self.custody)); }
        self.arrays.push(value); Ok(())
    }
    pub(crate) fn set_counts(&mut self,index:usize,values:&[usize])->Result<(),Error> {
        let Some(target)=self.counts.get_mut(index) else { return Err(error(Cause::Identity,&self.custody)); };
        if self.count_filled[index] || target.len()!=values.len() { return Err(error(Cause::Identity,&self.custody)); }
        target.copy_from_slice(values); self.count_filled[index]=true; Ok(())
    }
    ...
    pub(crate) fn push_route(&mut self,value:CommunicationRouteRealization,order:usize)->Result<(),Error> {
        self.source.validate()?;
        if self.routes.len()==self.limits[2] || !self.source.matches_route(order,&value) {
            return Err(error(Cause::Identity,&self.custody)); }
        self.routes.push(value); Ok(())
    }
    ...
    pub(crate) fn push_stream(&mut self,value:Stream)->Result<(),Error> {
        if self.streams.len()==self.limits[3] { return Err(error(Cause::Identity,&self.custody)); }
        self.streams.push(value); Ok(())
    }
    pub(crate) fn finish(self)->Result<ReadyCompletionResources,Error> {
        self.source.validate()?;
        if [self.arrays.len(),self.groups.len(),self.routes.len(),self.streams.len()]!=self.limits
            || self.count_filled.iter().any(|filled| !filled) { return Err(error(Cause::Identity,&self.custody)); }
        let Self { source:_,arrays,counts,count_filled,groups,routes,streams,limits:_,original,consumers,registry,orphan,custody }=self;
        drop(count_filled);
        let resources=NativeResources::from_owned(arrays,counts,groups,routes,streams,Some(custody.clone()),Some(registry));
        let recovery=match original {
            Some(traversal)=>PreparedCompletionRecovery::Original {
                resources,
                pending:crate::backend::submission_recovery::observed::PreparedObservedRecovery::new(custody.clone()),
                housekeeping:safemlx::PreparedThreadRuntimeHousekeeping::new(original::reap_original_communication,custody.clone()),
                traversal,
            },
            None=>PreparedCompletionRecovery::Owned(PreparedRecovery::new(resources,custody.clone()).map_err(|failed| {
                let cause=error(Cause::Recovery(failed.cause),&custody);
                drop(failed.retention); drop(failed.custody); cause
            })?),
        };
        Ok(ReadyCompletionResources { recovery,orphan,consumers,custody })
    }
}
fn vector_controls<T>() -> Option<usize> {
    let parts=[size_of::<Vec<T>>(),size_of::<Result<Vec<T>,Error>>(),
        size_of::<Result<(),TryReserveError>>(),size_of::<(&ResourceCustody,usize)>(),
        size_of::<Layout>()];
    parts.into_iter().try_fold(size_of_val(&parts),usize::checked_add)
}
fn vector<T>(capacity:usize,custody:&ResourceCustody)->Result<Vec<T>,Error> {
    let mut values=Vec::new(); values.try_reserve_exact(capacity).map_err(|cause|error(Cause::Capacity(cause),custody))?; Ok(values)
}
...
impl ReadyCompletionResources {
    pub(crate) fn submit_original<'a,I>(self,source:&OriginalCommunicationSource<'_>,
        observer:&safemlx::OriginalScopeObserver,stream:&Stream,outputs:I)
        ->Result<OriginalCommunicationCompletion,Error>
    where I:IntoIterator<Item=&'a Array>, I::IntoIter:ExactSizeIterator {
        source.validate()?;
    ...
    fn submit_for_source<'a,I>(self,observer:&safemlx::OriginalScopeObserver,stream:&Stream,outputs:I)
        ->Result<OriginalCommunicationCompletion,Error>
    where I:IntoIterator<Item=&'a Array>, I::IntoIter:ExactSizeIterator {
        ...
        let parts=[size_of::<I>(),size_of::<I::IntoIter>(),
            size_of::<(&Self,&safemlx::OriginalScopeObserver,&Stream)>(),
            size_of::<usize>(),size_of::<Result<OriginalCommunicationCompletion,Error>>()];
        ...
        self.submit_event(observer,outputs.len(),||
            safemlx::transforms::async_eval_with_original_prepared_traversal(outputs,observer,stream,&traversal))
    }
    fn submit_event<F>(self,observer:&safemlx::OriginalScopeObserver,roots:usize,submit:F)
        ->Result<OriginalCommunicationCompletion,Error>
    where F:FnOnce()->Result<safemlx::OperationEvent,safemlx::error::Exception>{
        let parts=[size_of::<F>(),size_of::<(&Self,&safemlx::OriginalScopeObserver,usize)>(),
            size_of::<Result<safemlx::OperationEvent,safemlx::error::Exception>>(),
            size_of::<Result<OriginalCommunicationCompletion,Error>>(),error_control_bytes().ok_or_else(overflow)?];
        self.custody.funding.reserve_metadata(parts.into_iter().try_fold(size_of_val(&parts),usize::checked_add)
            .ok_or_else(overflow)?).map_err(Error::WorkspacePlanning)?;
        let Self { recovery,orphan,consumers,custody }=self;
        let PreparedCompletionRecovery::Original { resources,pending,housekeeping,traversal }=recovery else {
            return Err(error(Cause::Identity,&custody));
        };
        let OrphanDestination::Communication(destination)=orphan else {
            return Err(error(Cause::Identity,&custody));
        };
        if roots!=traversal.roots() { return Err(error(Cause::Identity,&custody)); }
        ...
        *recovery.retention().original_event.borrow_mut()=Some(event.clone());
        ...
        Ok(OriginalCommunicationCompletion(MlxCommunicationCompletion::from_original(
            NativeEvent::Original(event),recovery,destination,roots,consumers)))
    }
}
fn recovery_controls(traversal:Option<&safemlx::OperationEvalTraversalLayout>)->Option<usize> {
    let Some(traversal)=traversal else {
        return usize::try_from(PreparedRecovery::<NativeOwner,ResourceCustody>::control_bytes()?).ok();
    };
    use crate::backend::submission_recovery::observed::{PreparedObservedRecovery,operation::OperationRecovery};
    let event=Layout::new::<[usize;2]>().extend(Layout::new::<safemlx::OperationEvent>()).ok()?.0.pad_to_align().size();
    let parts=[size_of::<PreparedCompletionRecovery>(),size_of::<CompletionRecovery>(),
        size_of::<OriginalCommunicationCompletion>(),size_of::<MlxCommunicationCompletion>(),
        size_of::<NativeEvent>(),size_of::<Rc<safemlx::OperationEvent>>(),event,
        size_of::<Result<safemlx::OperationEvent,safemlx::error::Exception>>(),
        size_of::<Result<OriginalCommunicationCompletion,Error>>(),
        size_of::<(&OriginalCommunicationSource<'_>,&safemlx::OriginalScopeObserver,&Stream,&[Array])>(),
        ...
        size_of::<safemlx::OperationEvalTraversalLayout>(),traversal.query_control_bytes()?,
        safemlx::OperationEvent::control_bytes()?, original::completion_controls()?,
        safemlx::PreparedThreadRuntimeHousekeeping::<ResourceCustody>::control_bytes()?,
        usize::try_from(PreparedObservedRecovery::<NativeOwner,ResourceCustody>::control_bytes::<safemlx::error::Exception>()?).ok()?,
        usize::try_from(OperationRecovery::<NativeOwner,ResourceCustody>::control_bytes()?).ok()?,
