...
impl Scope {
    fn new()->Self {
        ...
        Self { scope,graph,records,failure }
    }
    fn finish(mut self,observer:safemlx::OriginalScopeObserver) {
        ...
        observer.retire_completed_records().unwrap();drop(observer);
...
fn prepared_event(source:&OriginalCommunicationSource<'_>,input:&Array,zero:&Array,stream:&Stream)->ReadyCompletionResources {
    ...
    let mut prepared=PreparedCompletionResources::prepare_original(source,
        CompletionResourceLayout { arrays:2,counts:&[],groups:1,routes:0,streams:1 },traversal).unwrap();
...
fn prepared_words_and_unicode_headers_share_resolver_and_keep_escaped_host_storage_paid() {
    ...
    let group=CommunicationGroupDescriptor::new(CollectiveGroupId::new(47),0,vec![0],Some(0),
    ...
    let input=Array::from_slice(&expected,&[4]);let zero=Array::from_slice(&[0_i32],&[1]);
    input.evaluated().unwrap();zero.evaluated().unwrap();
    let ordinary=MlxCommunicationCompletion::submit([&input],vec![input.clone()],vec![],vec![],vec![],vec![]).unwrap();
    let (ordinary_words,ordinary)=ordinary.with_i32_words(input.clone());ordinary.wait().unwrap();
    assert_eq!(ordinary_words.resolve().unwrap(),expected);drop(ordinary);
    let ready=prepared_event(&source,&input,&zero,&stream);let scope=Scope::new();
    ...
    assert!(words.result.value.borrow().ready.is_none());completion.wait().unwrap();
    assert!(completion.is_complete().unwrap());
    let escaped=words.resolve().unwrap();assert_eq!(escaped.as_slice(),expected);
    assert!(completion.is_complete().unwrap(),"polling consumed result does not allocate or republish its Vec");
    drop(completion);drop((input,zero));scope.finish(observer);
    ...
    let input=Array::from_slice(&unsigned,&[4]);input.evaluated().unwrap();input.evaluated().unwrap();
    let operation=source.world_cpu_operation_storage(&input,safemlx::distributed::GroupWorkerOperation::Gather)
        .unwrap().with_completion().unwrap();
    let prepared=PreparedCommunicationU32Words::prepare_operation(&source,&operation).unwrap();
    let ready=operation.prepare_resources(&source,None).unwrap();
    let scope=Scope::new();let observer=safemlx::OriginalScopeObserver::require_current().unwrap();
    let accepted=operation.construct_accepted(&source,&observer,&stream).unwrap();
    let (words,completion)=prepared.submit_accepted(accepted,ready).unwrap();
    completion.wait().unwrap();let unsigned_escaped=words.resolve().unwrap();
    assert_eq!(unsigned_escaped.as_slice(),unsigned);
    assert!(completion.is_complete().unwrap());
    drop((completion,input));scope.finish(observer);

    for (expectation,valid) in [("rôle→✓",true),("rôle→✗",false)] {
        let bytes="rôle→✓".as_bytes();
        let input=Array::from_slice(bytes,&[bytes.len() as i32]);let zero=Array::from_slice(&[0_u8],&[1]);
        input.evaluated().unwrap();zero.evaluated().unwrap();
        let ordinary=MlxCommunicationCompletion::submit([&input],vec![input.clone()],vec![],vec![],vec![],vec![]).unwrap()
            .with_boundary_headers([(input.clone(),expectation.as_bytes().to_vec())]);
        assert_eq!(ordinary.wait().is_ok(),valid);drop(ordinary);
        let ready=prepared_event(&source,&input,&zero,&stream);let scope=Scope::new();
        let observer=safemlx::OriginalScopeObserver::require_current().unwrap();
        safemlx::OperationEvent::validate_traversal_leaf(&input,&observer).unwrap();
        safemlx::OperationEvent::validate_traversal_leaf(&zero,&observer).unwrap();
        let output=input.add(&zero,&stream).unwrap();
        let header=PreparedCommunicationHeader::prepare(&source,output,expectation.as_bytes()).unwrap();
        let completion=header.submit(ready,&source,&observer,&stream).unwrap();
        let result=completion.wait();assert_eq!(result.is_ok(),valid);
        drop((result,completion,input,zero));scope.finish(observer);
    }
    drop(source);drop((actual,authority,funding,stream,world));
    crate::backend::submission_recovery::reap();safemlx::reclaim_allocation_owners();
    assert!(pool.used_bytes().unwrap()>0,"extracted word Vec retains its independent H");
    assert_eq!(escaped.as_slice(),expected);drop(escaped);
    assert_eq!(unsigned_escaped.as_slice(),unsigned);
    assert!(pool.used_bytes().unwrap()>0,"unsigned readout retains its own paid host destination");
