...
use super::prepared::{error, Cause, ReadyCompletionResources, ResourceCustody};
use super::*;
...
impl BoolResult {
    pub(super) fn ordinary() -> Self {
        Self::ordinary_owned(None)
    }
    pub(super) fn ordinary_owned(ordinary: Option<eredu_core::HostPreparationAuthority>) -> Self {
        Self {
    ...
    pub(super) fn ordinary_host(&self) -> Option<&eredu_core::HostPreparationAuthority> {
        self.ordinary.as_ref()
    }
    fn original(custody: ResourceCustody) -> Self {
        Self {
            value: Rc::new(Cell::new(None)),
    ...
    fn custody(&self) -> &ResourceCustody {
        self.custody.as_ref().expect("prepared original result")
    }
}
impl BoolResult {
    pub(super) fn original_custody(&self) -> Option<&ResourceCustody> {
        self.custody.as_ref()
    }
}
...
pub(crate) struct PreparedCommunicationScalar {
    kind: ScalarKind,
    result: BoolResult,
}
...
#[derive(Debug)]
pub(crate) struct OriginalCommunicationBool {
    result: BoolResult,
}
impl OriginalCommunicationBool {
    pub(crate) fn resolve(self) -> Result<bool, Error> {
        self.result
            .get()
            .ok_or_else(|| error(Cause::Identity, self.result.custody()))
    }
...
impl PreparedCommunicationScalar {
    ...
    fn prepare(
        ...
        let kind = match selector {
            ...
            Selector::Agreement(group) => ScalarKind::Agreement(
                source
                    .group(group)
        ...
        Ok(Self {
            kind,
            result: BoolResult::original(custody),
        })
    }
    ...
    pub(crate) fn submit(
        ...
        let expected = match self.kind {
            ScalarKind::Flag => safemlx::Dtype::Float32,
            ScalarKind::Agreement(_) => safemlx::Dtype::Int32,
        };
        if !self.result.custody().source.same_source(source.source())
            || output.dtype() != expected
            || output.size() != 1
        {
            return Err(error(Cause::Identity, self.result.custody()));
        }
        let mut completion =
            ready.submit_original(source, observer, stream, std::slice::from_ref(&output))?;
        completion
            .0
            .install_original_scalar(output, self.result.clone(), self.kind);
        Ok((
            OriginalCommunicationBool {
                result: self.result,
            },
            completion,
        ))
    ...
    pub(crate) fn submit_accepted(
        ...
        completion
            .0
            .install_original_scalar(value, self.result.clone(), self.kind);
        let result = OriginalCommunicationBool {
            result: self.result,
        };
        drop((source, funding));
    ...
    pub(crate) fn control_bytes() -> Option<usize> {
        let allocation = Layout::new::<[usize; 2]>()
            .extend(Layout::new::<Cell<Option<bool>>>())
            .ok()?
            .0
            .pad_to_align()
            .size();
