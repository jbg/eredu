...
impl<T> Default for Destinations<T> { fn default() -> Self { Self::new() } }
impl<T> Destinations<T> {
    pub(super) const fn new() -> Self { Self { head: None, len: 0 } }
    ...
    pub(super) fn push(&mut self, value: T) { self.push_prepared(value, Destination::new(None)); }
    ...
    pub(super) fn push_prepared(&mut self, value: T, mut destination: Destination<T>) {
        ...
        while let Some(existing) = link { link = &mut existing.next; }
        *link = Some(node);
        self.len = self.len.checked_add(1).expect("completion destination count overflow");
    }
    pub(super) fn iter(&self) -> Iter<'_, T> { Iter(self.head.as_deref()) }
    pub(super) fn iter_mut(&mut self) -> IterMut<'_, T> { IterMut(self.head.as_deref_mut()) }
    pub(super) fn retain(&mut self, mut retain: impl FnMut(&T) -> bool) {
        ...
        while let Some(node) = link.as_ref() {
            ...
            if retain(node.value.as_ref().expect("occupied completion destination")) {
                ...
                *link = node.next.take();
                ...
                let Node { value, next, custody } = *node;
                drop(value); drop(next); drop(custody);
            }
        }
    }
    pub(super) fn clear(&mut self) { self.retain(|_| false); }
}
impl<T> Drop for Destinations<T> { fn drop(&mut self) { self.clear(); } }
pub(super) struct Iter<'a, T>(Option<&'a Node<T>>);
impl<'a,T> Iterator for Iter<'a,T> {
    type Item=&'a T;
    fn next(&mut self)->Option<Self::Item> { let node=self.0.take()?; self.0=node.next.as_deref(); node.value.as_ref() }
}
pub(super) struct IterMut<'a, T>(Option<&'a mut Node<T>>);
impl<'a,T> Iterator for IterMut<'a,T> {
    type Item=&'a mut T;
    fn next(&mut self)->Option<Self::Item> { let node=self.0.take()?; self.0=node.next.as_deref_mut(); node.value.as_mut() }
}
impl<'a,T> IntoIterator for &'a mut Destinations<T> {
    type Item=&'a mut T; type IntoIter=IterMut<'a,T>;
    fn into_iter(self)->Self::IntoIter { self.iter_mut() }
}
#[cfg(test)]
impl<T> std::ops::Index<usize> for Destinations<T> {
    type Output=T;
    fn index(&self,index:usize)->&T { self.iter().nth(index).expect("completion index") }
}
#[cfg(test)]
impl<T> std::ops::IndexMut<usize> for Destinations<T> {
    fn index_mut(&mut self,index:usize)->&mut T { self.iter_mut().nth(index).expect("completion index") }
}
