...
fn prepared_completion_destinations_keep_actual_sources_refusals_and_quarantined_custody() {
    ...
    let pool = crate::memory_fixture::ledger(4 << 20, 0).unwrap();
    let funding = pool
        .prepare_workspace_metadata(
            &InferenceExecutionIdentity::default(),
            crate::memory_fixture::resolved_limits(4 << 20),
        )
        .unwrap();
    let source = actual
        .bind_original_source(&manifest, &world, &authority, &funding)
        .unwrap();
    let empty = CompletionResourceLayout {
        arrays: 0,
        counts: &[],
        groups: 0,
        routes: 0,
    ...
    let other =
        PreparedCompletionResources::prepare(&source, empty, CompletionResourceKind::Distributed)
            .unwrap()
            .finish()
            .unwrap();
    assert!(matches!(other.orphan, OrphanDestination::Distributed(_)));
    drop(other);
    let incomplete = PreparedCompletionResources::prepare(
        &source,
        CompletionResourceLayout { arrays: 1, ..empty },
        CompletionResourceKind::Distributed,
    )
    .unwrap();
    assert!(
        incomplete.finish().is_err(),
        "missing retained roots cannot publish a ready recovery"
    );
    ...
    let mut prepared = PreparedCompletionResources::prepare(
        &source,
        layout,
    ...
    assert_eq!(
        prepared.counts[0],
        [2, 3, 5],
        "duplicate writes cannot replace a prepared buffer"
    );
    assert!(
        prepared.push_group(Group::uncontracted(&world), 0).is_err(),
    ...
    assert!(matches!(
        completion.wait_bounded(policy.bounded_wait()).unwrap(),
    ...
    assert!(
        ...
        "the occupied prepared orphan node retains its own funding"
    );
    ...
    assert_eq!(
        ...
        0,
        "terminal retirement releases node before its funding"
    );
}
...
fn prepared_destination_panicking_predicate_keeps_remaining_owned_queue() {
    ...
    for _ in 0..3 {
        queue.push_prepared(Value(drops.clone()), Destination::new(None));
    }
...
fn original_communication_event_completes_inside_current_role_and_keeps_prepaid_quarantine() {
    ...
    use safemlx::{
        OperationEvalTraversalLimits, OperationEvent, OriginalScopeObserver,
    ...
    let actual = ParallelCommunicators::from_manifest(&manifest, &world, &stream).unwrap();
    let authority = PartitionCommunicationAuthority::from_manifest(&manifest).unwrap();
    let pool = crate::memory_fixture::ledger(8 << 20, 0).unwrap();
    let funding = pool
        .prepare_workspace_metadata(
            &InferenceExecutionIdentity::default(),
            crate::memory_fixture::resolved_limits(8 << 20),
        )
        .unwrap();
    let source = actual
        .bind_original_source(&manifest, &world, &authority, &funding)
    ...
    let mut prepared =
        PreparedCompletionResources::prepare_original(&source, layout, traversal).unwrap();
    prepared.push_array(left.clone()).unwrap();
    prepared.push_array(right.clone()).unwrap();
    prepared.retain_group(0).unwrap();
    prepared.push_stream(stream.clone()).unwrap();
    let ready = prepared.finish().unwrap();
    let bad = PreparedCompletionResources::prepare_original(
        &source,
    ...
    let graph = PreparedSubmissionGraphQuota::try_new(4 << 20, ())
        .unwrap()
        .try_allocate()
        .unwrap();
    ...
    let refused = bad
        .submit_original(&source, &observer, &stream, &[])
        .unwrap_err();
    ...
    let mut completion = ready
        .submit_original(&source, &observer, &stream, std::slice::from_ref(&value))
        .unwrap();
    ...
    assert!(
        pool.fixture_host_charge().unwrap() > 0,
        "prepared quarantine keeps its independent source/account"
    );
    super::super::communication::release_forced_pending_orphans();
...
fn selected_completion_copies_keep_source_order_and_failure_custody_without_submission() {
    ...
    let requirements = CommunicationGroupRequirements::new([
        ...
        CommunicationOperationRequirement::tensors(
            ...
            [TensorDtype::F32],
            CommunicationTensorLimits::new(1, 3, 4097, None).unwrap(),
            true,
        )
        .unwrap(),
    ])
    ...
    assert_eq!(
        prepared.groups.len(),
        1,
        "invalid source lookup leaves the paid prefix unchanged"
    );
    prepared.retain_group(0).unwrap();
    ...
    assert_eq!(
        prepared.groups.len(),
        2,
        "full destination cannot be replaced or overfilled"
    );
    ...
    let ready = prepared.finish().unwrap();
    drop(source);
    drop((actual, authority, funding, world, stream));
    assert!(pool.fixture_host_charge().unwrap() > 0);
    drop(ready);
    assert!(
        pool.fixture_host_charge().unwrap() > 0,
        "escaped invalid selections keep independently funded errors"
    );
    drop(first);
    assert!(pool.fixture_host_charge().unwrap() > 0);
    drop(second);
