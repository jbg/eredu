...
use eredu_runtime::working_memory::{OriginalRealtimeBudgetCustody,OriginalOperationMetadataCustody,
    OriginalHostSourceBank,HostSourceConstructionFacts,WorkingMemoryPool};
...
impl<U:'static> OriginalOperationPlan<'_,U> {
    ...
    pub(super) fn prepare_accepted_bank(self,observer:safemlx::OriginalScopeObserver,
        custody:OriginalOperationMetadataCustody,request:OperationRequest,
        controls:OperationControls,disk_source:Option<storage::ForegroundDiskSourceTicket>)
        ->Result<Box<dyn ErasedOwner>,Error> {
            ...
            drop(replaced);
        Ok(Box::new(OwnedBank{bank,slot:self.slot,registration:None}))
    }
}
...
pub(crate) struct RealtimeLayerwisePlan<U:'static> { plan:OriginalOperationPlan<'static,U> }
pub(crate) struct QualifiedRealtimeLayerwisePlan<U:'static> {
    plan:OriginalOperationPlan<'static,U>,recipe:SpeculativeNumericalRecipe,
}
impl<U:'static,P> MlxLayerwisePolicy<U,P> {
    pub(crate) fn realtime_plan(&self,source:super::super::host_workspace::LayerwiseWorkspace,
        stream:&Stream,pool:&WorkingMemoryPool,context:&WorkspaceContext)->Result<RealtimeLayerwisePlan<U>,Error> {
        let funding=context.metadata_funding().ok_or_else(unknown)?;
        context.charge_metadata(rc_layout::<super::super::host_workspace::LayerwiseWorkspace>()
            .and_then(|n|n.checked_add(size_of::<RealtimeLayerwisePlan<U>>()))
            .and_then(|n|n.checked_add(size_of::<Result<RealtimeLayerwisePlan<U>,Error>>()))
            .and_then(|n|n.checked_add(safemlx::StreamCopyPlan::<()>::capture_control_bytes().ok()?))
            .ok_or_else(overflow)?).map_err(|cause|Error::Neural(cause.into()))?;
        let windows=self.operation_source.as_ref().map_err(|_|unknown())?;
        source.validate_original_policy(&self.residency,&self.workspace_identity,self.unit_ids.as_slice())?;
        ...
        let residency=ResidencyPopulation::from_window_forwards(windows.controller_units,windows.windows(),1)?;
        let mut plan=self.operation_plan_from_population(None,
            OperationCounts{units:self.layout.len(),scopes:1},neural,Some(residency),None)?;
        plan.owned_sources=Some(Rc::new(source));
        plan=plan.with_preparation_funding(Some(&funding)).with_selected_stream(stream)?
            .with_foreground_disk_reads(pool)?;
        Ok(RealtimeLayerwisePlan{plan})
    }
}
impl<U:'static> RealtimeLayerwisePlan<U> {
    pub(crate) fn qualify(self,recipe:SpeculativeNumericalRecipe,
        actual:&super::super::host_workspace::LayerwiseWorkspace,context:&WorkspaceContext)
        ...
        if !actual.has_original_ready_host_source(&plan.manager,windows)
            && !(plan.foreground_disk_plan().is_some()&&actual.has_original_foreground_source(&plan.manager,windows)) {
            return Err(identity());
        }
        if let Some(disk) = plan.foreground_disk.as_mut().filter(|disk| disk.background().is_some()) {
            let select = |ordinals: Option<&[usize]>| disk.select_background_execution_ordinals(
                ordinals.ok_or_else(identity)?);
            context.charge_metadata(std::mem::size_of_val(&select))
                .map_err(|cause| Error::Neural(cause.into()))?;
            actual.with_execution_ordinals(select)?;
        }
        let recipe=recipe.with_realtime_boundaries(plan.neural.submissions,plan.neural.shape.consumers(),context)
...
impl<U:'static> QualifiedRealtimeLayerwisePlan<U> {
    ...
    pub(crate) fn source_facts(&self)->Option<HostSourceConstructionFacts> {self.plan.source_construction_facts()}
    pub(crate) fn control_bytes(&self)->Option<u64> {
        let frames=[size_of::<Self>(),size_of::<RealtimeNeuralOwner>(),
            size_of::<RealtimeOperationClaim<'_>>(),size_of::<OriginalRealtimeBudgetCustody>(),
            size_of::<OriginalOperationMetadataCustody>(),size_of::<Option<OriginalHostSourceBank>>(),
            size_of::<Option<storage::ForegroundDiskSourceTicket>>(),
            size_of::<Result<RealtimeNeuralOwner,Error>>(),
            size_of::<(safemlx::OriginalScopeObserver,OriginalRealtimeBudgetCustody)>(),
            size_of::<Result<(safemlx::OriginalScopeObserver,OriginalRealtimeBudgetCustody),Error>>()];
        let extra=frames.into_iter().try_fold(std::mem::size_of_val(&frames),usize::checked_add)?
        ...
        self.plan.control_bytes()?.checked_add(u64::try_from(extra).ok()?)
    }
    pub(crate) fn prepare(self,claim:RealtimeOperationClaim<'_>,source:Option<OriginalHostSourceBank>)
        ->Result<RealtimeNeuralOwner,Error> {
        let (observer,custody)=claim.prepare(usize::try_from(self.control_bytes().ok_or_else(unknown)?)
            .map_err(|_|overflow())?)?;
        let result=(|| {
            let previous=self.plan.slot.take();
            let busy=previous.as_ref().is_some_and(|view|view.value.strong_count()!=0);
            ...
            let source=match (self.plan.foreground_disk_plan(),source) {
                (Some(plan),Some(bank)) if bank.matches_facts(plan.source_facts().ok_or_else(unknown)?)=>
                    Some(storage::ForegroundDiskSourceTicket::Source{bank,custody:custody.clone().into()}),
                (None,None)=>None,
                _=>return Err(identity()),
            };
            let value=self.plan.prepare_accepted_bank(observer,custody.clone().into(),
                OperationRequest::Realtime(custody.clone()),OperationControls::Realtime(custody.clone()),source)?;
