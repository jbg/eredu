...
use crate::backend::runtime::residency::manager::{
    ForegroundDiskSourceCapacity, OriginalResidencySlots, SupplementaryResidencySource,
};
use eredu_nn::workspace::HostMetadataFunding;
use eredu_runtime::working_memory::{
    MemoryLedger, OriginalHostSourceBank, OriginalHostSourceCustody, WorkingMemoryReservation,
};
...
enum Source {
    ...
    Numerical {
        custody: eredu_runtime::working_memory::OriginalNumericalBudgetCustody,
        observer: safemlx::OriginalScopeObserver,
    },
}
/// Retains the exact installed role and any native budget loan supplied by its
/// owning producer. It contains no prepared storage or graph. A stale source
/// always refuses.
#[derive(Clone)]
pub(crate) struct OriginalSelectedResidencyAccess {
    source: Source,
    nested: Option<NestedScope>,
    materialized_recipe: Option<materialization::MaterializationOwner>,
}
#[derive(Clone)]
struct NestedScope {
    parent: safemlx::OriginalScopeObserver,
    child: safemlx::OriginalScopeObserver,
}
...
impl PreparedSelectedResidencyAccess {
    ...
    pub(crate) fn bind(
        &self,
        context: &crate::backend::submission_recovery::native_role::NativeRoleContext<'_>,
    ) -> Result<OriginalSelectedResidencyAccess, Error> {
        self.funding
            .reserve_metadata(Self::bind_control_bytes().ok_or_else(overflow)?)
            .map_err(Error::WorkspacePlanning)?;
        if !context.has_parent(&self.parent) || self.access.nested.is_some() {
            return Err(identity());
        }
        self.access.authenticate_observer(&self.parent)?;
        let access = OriginalSelectedResidencyAccess {
            source: self.access.source.clone(),
            nested: Some(NestedScope {
                parent: self.parent.clone(),
                child: context.observer().clone(),
            }),
            materialized_recipe: Some(materialization::MaterializationOwner::from_role(
                context,
                &self.funding,
            )),
        };
...
impl OriginalSelectedResidencyAccess {
    /// Binds the accepted standalone account to the scope created by its own
    /// numerical role. The constructor is unavailable without that role loan.
    ...
    pub(super) fn registered(
        ...
        Ok(Self {
            source: Source::Registered { registry, controls },
            nested: None,
    ...
    fn authenticate(&self) -> Result<safemlx::OriginalScopeObserver, Error> {
        ...
        if let Some(nested) = &self.nested {
            if !nested.child.same_scope(&observer) {
                return Err(identity());
            }
    ...
    fn authenticate_observer(
        &self,
        observer: &safemlx::OriginalScopeObserver,
    ) -> Result<(), Error> {
        match &self.source {
            Source::Registered { registry, controls } => {
                controls.validate(registry)?;
                registry.authenticate_observer(observer)
            }
            Source::Resident(source) => source.authenticate_observer(observer),
            Source::Numerical {
                observer: actual, ..
            } => {
                if actual.same_scope(observer) {
                    Ok(())
                } else {
                    Err(identity())
                }
    ...
    pub(crate) fn native_role_control_bytes() -> Option<usize> {
        let frames = [
            size_of::<PreparedSelectedResidencyAccess>(),
            size_of::<Result<PreparedSelectedResidencyAccess, Error>>(),
    ...
    pub(crate) fn prepare_native_role(
        &self,
        funding: &HostMetadataFunding,
    ) -> Result<PreparedSelectedResidencyAccess, Error> {
        funding
            .reserve_metadata(Self::native_role_control_bytes().ok_or_else(overflow)?)
            .map_err(Error::WorkspacePlanning)?;
        if self.nested.is_some() {
            return Err(identity());
        }
        Ok(PreparedSelectedResidencyAccess {
            access: self.clone(),
            parent: self.authenticate()?,
            funding: funding.clone(),
        })
    }
    fn same_source(&self, other: &Self) -> bool {
        if !match (&self.materialized_recipe, &other.materialized_recipe) {
            (None, None) => true,
        ...
        match (&self.source, &other.source) {
            (Source::Registered { registry: a, .. }, Source::Registered { registry: b, .. }) => {
                Rc::ptr_eq(a, b)
            }
            ...
            ) => a.same_account(b) && ao.same_scope(bo),
            _ => false,
        }
    }
    fn reservation(&self) -> Option<&WorkingMemoryReservation> {
        match &self.source {
            Source::Registered { registry, .. } => registry.request.memory_reservation(),
            Source::Resident(_) | Source::Numerical { .. } => None,
        }
    }
    fn custody(&self) -> OriginalHostSourceCustody {
        match &self.source {
            ...
            Source::Registered {
                controls: OperationControls::Speculative(role),
                ..
            } => role.budget_custody().into(),
            Source::Registered {
                controls: OperationControls::Realtime(custody),
                ..
            } => custody.clone().into(),
            Source::Resident(source) => source.custody(),
            Source::Numerical { custody, .. } => custody.clone().into(),
        }
    }
    ...
    pub(crate) fn metadata_custody(
        &self,
    ) -> Result<eredu_runtime::working_memory::OriginalOperationMetadataCustody, Error> {
        self.authenticate()?;
    ...
    pub(crate) fn validate_observer(
        ...
        if !self.authenticate()?.same_scope(observer) {
            return Err(identity());
        }
        Ok(())
    }
    ...
    pub(crate) fn validate_source_bank(&self, bank: &OriginalHostSourceBank) -> Result<(), Error> {
        self.authenticate()?;
        if !bank.belongs_to_source(&self.custody()) {
            return Err(identity());
        }
        Ok(())
    }
    ...
    pub(crate) fn validate_population(
        ...
        facts: eredu_runtime::working_memory::HostSourceConstructionFacts,
    ) -> Result<(), Error> {
        self.authenticate()?;
        if !bank.belongs_to_source(&self.custody()) || !bank.matches_facts(facts) {
            return Err(identity());
        }
        Ok(())
    }
    pub(crate) fn prepare_read_capacity(
        &self,
        ...
        self.authenticate()?;
        source
            .prepare_capacity(calls, bank, self.custody(), self.reservation())
            .map_err(memory)
    }
    /// Binds the one exact admitted reader source root before any region runs.
    pub(crate) fn prepare_read_series(
        &self,
        ...
        self.authenticate()?;
        source
            .prepare_capacity(bank, self.custody(), self.reservation())
            .map_err(memory)
    }
    ...
    pub(crate) fn validate_read_capacity(
        &self,
        source: &crate::backend::runtime::residency::manager::ForegroundDiskSubsetCeiling,
        ...
        self.authenticate()?;
        capacity
            .validate_account(self.reservation())
            .map_err(memory)?;
        if !capacity.custody().same_source(&self.custody()) || !source.matches_capacity(capacity) {
            return Err(identity());
        }
        Ok(())
    }
    ...
    pub(crate) fn prepare(
        ...
        let value = storage::PreparedSelectedResidency::prepare_with_disk(
            manager,
            source,
            roots,
            bank,
        ...
        Ok(OriginalSelectedResidencyAttempt {
            value,
            access: self.clone(),
        })
    }
    pub(crate) fn prepare_source(
        ...
        self.validate_bank(bank)?;
        let value = storage::PreparedSelectedResidency::prepare_source(
            manager,
            source,
            roots,
            bank,
    ...
    pub(crate) fn with_residency<T>(
        ...
        if !self.same_source(&attempt.access) {
            return Err(identity());
        }
        let observer = self.authenticate()?;
        let materialized = match &self.materialized_recipe {
            Some(owner) => Some(owner.clone()),
            None => match &self.source {
                Source::Registered { registry, .. } => registry.materialization_owner()?,
                _ => None,
            },
        };
        let mut slots = attempt.value.residency(self.reservation());
        slots.materialized_recipe = materialized
            .as_ref()
            .map(materialization::MaterializationOwner::loan);
        execute(&mut slots, &observer)
    }
    pub(crate) fn control_bytes() -> Option<usize> {
        let frames = [
            ...
            size_of::<(&Self, &safemlx::OriginalScopeObserver)>(),
