...
use crate::backend::runtime::residency::manager::{
    SelectedResidencySource, SupplementaryResidencySource,
};
use eredu_runtime::{
    residency::ResidencyClosureSlot,
    working_memory::{
        OriginalHostSourceBank, OriginalHostSourceCustody, OriginalHostSourceReceipt,
...
pub(in crate::backend::runtime::execution::generic::original_operations) struct PreparedSelectedResidency
{
    ...
    _source: SelectedResidencySource,
...
impl PreparedSelectedResidency {
    pub(in crate::backend::runtime::execution::generic::original_operations) fn scratch_bytes(
        source: &SupplementaryResidencySource,
    ) -> Option<u64> {
        Self::source_scratch_bytes(&SelectedResidencySource::Supplementary(source.clone()))
    }
    pub(in crate::backend::runtime::execution::generic::original_operations) fn source_scratch_bytes(
        source: &SelectedResidencySource,
    ) -> Option<u64> {
        let units = source.source().controller_units;
        let parts = [
            ResidencyClosureSlot::layout(units)?.size(),
            size_of::<Self>(),
            ...
            size_of::<
                Result<
                    ...
                    eredu_runtime::working_memory::OriginalHostSourceError,
                >,
            >(),
    ...
    pub(in crate::backend::runtime::execution::generic::original_operations) fn window_bytes(
        ...
        PreparedResidencyAttempt::control_bytes(source)?
            .checked_add(named_error_control_bytes()?)?
            .checked_add(u64::try_from(source.named.catalog_requested_bytes).ok()?)?
            .checked_add(
                u64::try_from(
                    size_of::<NameCatalogOwner>()
                        .checked_add(size_of::<
                            Result<
                                NameCatalogOwner,
                                crate::backend::runtime::residency::manager::NamePreparationError,
                            >,
                        >())?
                        .checked_add(size_of::<Result<PreparedResidencyAttempt, Error>>())?,
                )
                .ok()?,
            )
    }
    ...
    pub(in crate::backend::runtime::execution::generic::original_operations) fn prepare_source(
        manager: &ResidencyManager,
        source: &SelectedResidencySource,
        roots: &[OffloadUnitId],
        bank: &mut OriginalHostSourceBank,
        ...
        if let Some((pool, capacity, _)) = disk {
            if !custody.same_source(capacity.custody())
                ...
                || !source
                    .foreground()
                    .is_some_and(|source| capacity.matches_source(source.source()))
            {
                return Err(identity());
            }
        ...
        let units = source.source().controller_units;
        let scratch_receipt = bank
            .try_debit(Self::source_scratch_bytes(source).ok_or_else(overflow)?)
        ...
        if disk.is_none() {
            manager
                .validate_selected_host(source)
                .map_err(|_| identity())?;
        }
        let mut closure = Vec::new();
        closure
            .try_reserve_exact(units)
            .map_err(|cause| reserve_error(cause, &metadata))?;
        closure.resize(units, ResidencyClosureSlot::default());
        let population = manager
            .selected_source_population(source, roots, &mut closure)
            .map_err(|cause| failure(SelectedCause::Source(cause), &custody))?;
        ...
        let window_bytes = Self::window_bytes(population)
            .and_then(|bytes| {
                bytes.checked_add(
                    disk_plan
                        .as_ref()
            ...
            .ok_or_else(overflow)?;
        let window_receipt = bank
            .try_debit(window_bytes)
            .map_err(|cause| failure(SelectedCause::Debit(cause), &custody))?;
        let catalog = manager
            .prepare_name_catalog(roots, &mut closure, metadata.clone())
            .map_err(|cause| named_error(cause.into_source(), &metadata))?;
        let window = PreparedResidencyAttempt::new(
            population,
            MemoryTier::Device,
            None,
            &metadata,
            ...
            &[],
            None,
            disk.zip(disk_plan.as_ref())
                .map(|((pool, capacity, _), plan)| (plan, pool, capacity)),
            reservation,
        )?;
        Ok(Self {
            window,
            ...
            _scratch_receipt: scratch_receipt,
            _window_receipt: window_receipt,
            _custody: custody,
        })
    }
    pub(in crate::backend::runtime::execution::generic::original_operations) fn residency<'a>(
        ...
        self.window.residency(&mut self.closure, reservation)
    }
}
...
fn failure(cause: SelectedCause, custody: &OriginalHostSourceCustody) -> Error {
    Error::with_original_control_source(
        eredu_core::BackendFailure::from_error(SelectedPreparationFailure {
            cause,
