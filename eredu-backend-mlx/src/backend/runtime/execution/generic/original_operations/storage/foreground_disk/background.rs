...
impl BackgroundSelection {
    pub(in crate::backend::runtime::execution::generic::original_operations) fn new(source: OriginalResidencySource, options: DenseDiskStreamLoadOptions) -> Self {
        Self { source, options }
    }
}
...
impl BackgroundRequestPlan {
    pub(super) fn new(
        ...
        let controls = [
            ...
            size_of::<std::slice::Iter<'_, crate::backend::runtime::residency::manager::WindowPopulation>>(),
        ];
        funding.reserve_metadata(controls.into_iter().try_fold(size_of_val(&controls), usize::checked_add).ok_or_else(overflow)?)
            .map_err(Error::WorkspacePlanning)?;
        let declared = selection.source.foreground_identity().ok_or_else(identity)?;
        let exact = manager.background_operation_source(ids, declared.layout(), selection.options.host_lookahead()).map_err(|_| identity())?;
        if exact.foreground_identity() != Some(declared) || device.len() != exact.windows().len()
            || device.len() != device_sources.len() || device.is_empty() || selection.options.host_budget_bytes() == 0 {
            return Err(identity());
        }
        ...
        for window in exact.windows() {
            host.push(ForegroundDiskWindowPlan::new_with_metadata(manager, pool, *window, ids, &mut scratch, Some(funding))?);
        }
        ...
        let mut host_acquisitions = funding.metadata_vec::<crate::backend::runtime::residency::manager::WindowPopulation>(device.len())?;
        for (ordinal, device) in device.iter().enumerate() {
            ...
            if host_source.request_start != device_source.request_start { return Err(identity()); }
            ...
            let selected = if host_source.request_end < device_source.request_end { *host_source } else { *device_source };
            let promotion = ForegroundDiskWindowPlan::new_with_metadata(manager, pool, selected, ids, &mut scratch, Some(funding))?;
        ...
        Ok(Self { selection: selection.clone(), host, promotions, direct, host_acquisitions, reads, forwards, selected: None, funding: funding.clone() })
    }
    ...
    pub(super) fn select_execution_ordinals(&mut self, ordinals: &[usize]) -> Result<(), Error> {
        let controls = [size_of::<(&mut Self, &[usize])>(), size_of::<Option<Vec<usize>>>(),
            size_of::<std::slice::Iter<'_, usize>>(), size_of::<std::slice::Windows<'_, usize>>(),
            size_of::<Result<(), Error>>()];
        self.funding.reserve_metadata(controls.into_iter()
            .try_fold(size_of_val(&controls), usize::checked_add).ok_or_else(overflow)?)
            .map_err(Error::WorkspacePlanning)?;
        if self.selected.is_some() || ordinals.iter().any(|ordinal| *ordinal >= self.host.len())
    ...
    pub(super) fn population(&self) -> Option<ForegroundDiskPopulation> {
        self.direct.iter().try_fold(self.reads.population()?, |sum, row| sum.checked_add(row.population()?))
    }
    pub(super) fn source_control_bytes(&self) -> Option<u64> {
        self.direct.iter().try_fold(self.reads.source_control_bytes()?, |sum, row| sum.checked_add(row.source_control_bytes()?))
    }
    pub(crate) fn direct(&self, ordinal: usize) -> Option<&ForegroundDiskWindowPlan> { self.direct.get(ordinal) }
    pub(crate) fn host_acquisitions(&self) -> &[crate::backend::runtime::residency::manager::WindowPopulation] { &self.host_acquisitions }
    pub(super) fn backing_capacity(&self) -> Option<usize> {
        let total = self.population()?.checked_mul(self.forwards)?.source_backing_bytes;
        let ready = self.host.iter().try_fold(0usize, |peak, row| Some(peak.max(row.single_read_population()?.source_backing_bytes)))?;
        let direct = self.direct.iter().try_fold(0usize, |peak, row| Some(peak.max(row.population()?.source_backing_bytes)))?;
        let bounded = usize::try_from(self.selection.options.host_budget_bytes()).ok()?.checked_add(ready)?.checked_add(direct)?;
        ...
        Some(total.min(bounded))
    }
    pub(super) fn peak_selection(&self, bytes: u64) -> HostSourcePeakSelection { self.reads.peak_selection(bytes) }
    ...
    pub(crate) fn prepare(
        &self, manager: &ResidencyManager, pool: &WorkingMemoryPool,
        ids: &[OffloadUnitId], device: &[ForegroundDiskWindowPlan],
        ...
        let controls = [
            ...
            size_of::<Result<eredu_runtime::working_memory::HostThreadStartupPlan, WorkingMemoryError>>(),
            size_of::<(&Self, &ResidencyManager, &WorkingMemoryPool, &[OffloadUnitId], &[ForegroundDiskWindowPlan], OriginalHostSourceCustody, Option<&WorkingMemoryReservation>, &ForegroundDiskSourceCapacity)>(),
            size_of::<std::ops::Range<usize>>(), size_of::<(usize, &ForegroundDiskWindowPlan)>(),
            size_of::<std::iter::Enumerate<std::slice::Iter<'_, ForegroundDiskWindowPlan>>>(),
            size_of::<String>(), size_of::<std::fmt::Arguments<'_>>(),
            size_of::<OriginalHostSourceCustody>(),
        ];
        funding.reserve_metadata(controls.into_iter().try_fold(size_of_val(&controls), usize::checked_add).ok_or_else(overflow)?)
            .map_err(Error::WorkspacePlanning)?;
        if device.len() != self.host.len() || !capacity.custody().metadata_custody().same_account(&custody.metadata_custody()) {
            return Err(identity());
        }
        let declared = self.selection.source.foreground_identity().ok_or_else(identity)?;
        let exact = manager.background_operation_source(ids, declared.layout(), self.selection.options.host_lookahead()).map_err(|_| identity())?;
        if exact.foreground_identity() != Some(declared) { return Err(identity()); }
        let startup = BackgroundHostReadService::thread_plan().map_err(Error::PrefillControl)?;
        let mut forwards = funding.metadata_vec::<Option<PreparedBackgroundForward>>(self.forwards)?;
        for _ in 0..self.forwards {
            let reads = PreparedBackgroundHostReads::prepare(&self.reads, manager, pool, custody.clone(), reservation, capacity,
                funding.clone(), self.selection.options.background_queue_capacity())
                .map_err(|cause| Error::Neural(funding.metadata_source(cause)))?;
            let count = self.selected.as_ref().map_or(device.len(), Vec::len);
            let mut windows = funding.metadata_vec::<(usize, Option<PreparedBackgroundHostWindow>)>(count)?;
            for position in 0..count {
                let ordinal = self.selected.as_ref().map_or(position, |selected| selected[position]);
                let window = exact.windows().get(ordinal).ok_or_else(identity)?;
                let active = ids.get(window.request_start..window.request_end).ok_or_else(identity)?;
                let address = declared.layout().address(ordinal).ok_or_else(identity)?;
                let group = declared.layout().group_id(address.group()).ok_or_else(identity)?;
                ...
                let group = funding.metadata_string(format_args!("dense:{}:host", group.as_str()))?;
                windows.push((ordinal, Some(PreparedBackgroundHostWindow::prepare(manager, &self.host[ordinal], &self.promotions[ordinal],
                    &group, active, self.host_acquisitions[ordinal].requested, custody.clone(), funding.clone())
                    .map_err(|cause| Error::Neural(funding.metadata_source(cause)))?)));
            }
            forwards.push(Some(PreparedBackgroundForward::from_prepared(reads, windows, startup)));
        }
