...
use eredu_runtime::working_memory::{
    OriginalEmbeddedSpeculativeRole, OriginalSpeculativePrefillSpan, OriginalSpeculativeRole,
};
use eredu_runtime::{
...
impl<U: 'static> OriginalOperationPlan<'_, U> {
    fn with_prepared_speculative_source(mut self) -> Result<Self, Error> {
        if let Some(source) = self.retained_sources {
            if source.has_original_foreground_source(
                &self.manager,
                self.window_sources.as_ref().ok_or_else(unknown)?,
            ) {
                self.foreground_disk_loan =
                    Some(source.speculative_foreground().ok_or_else(unknown)?);
            }
        }
        Ok(self)
    }
    fn prepare_speculative_source(
        ...
        if source.has_original_foreground_source(&self.manager, windows) {
            ...
            self.foreground_disk_loan = Some(source.prepare_speculative_foreground(|| {
                storage::PreparedSpeculativeForegroundSource::new(
                    &self.manager,
                    pool,
                    windows.as_slice(),
    ...
    fn bind_role_recipe(&self, recipe: &mut ResidentNativeRecipe) -> Result<(), Error> {
        ...
        if !source.has_original_ready_host_source(&self.manager, windows)
            && !(self.foreground_disk_plan().is_some()
                && source.has_original_foreground_source(&self.manager, windows))
        {
            return Err(unknown());
        }
    ...
    fn for_speculative_span(mut self, recipe: &ResidentNativeRecipe) -> Result<Self, Error> {
    ...
    fn prepare_speculative_bank(
        ...
        let SpeculativeOperationRole::Autoregressive(role) = &tagged else {
            unreachable!()
        };
        let validation = (|| {
    ...
    fn prepare_embedded_bank(
        ...
        let SpeculativeOperationRole::Embedded(role) = &tagged else {
            unreachable!()
        };
        let validation = (|| {
            role.validate_plan(recipe.plan()).map_err(memory)?;
            role.validate_invocation(recipe.workspace().invocation())
                .map_err(memory)?;
            if role.geometry() != recipe.workspace().geometry() {
                return Err(identity());
            }
            Ok(())
        })();
    ...
    fn prepare_external_bank(
        ...
        validation.map_err(|cause| neural::boundary_error(cause, &tagged))?;
        self.prepare_role_bank(recipe, tagged, None, scope, partition, materialized)
    }
    fn prepare_role_bank(
        ...
        let result = (|| {
            ...
            .map_err(memory)?;
            let disk_source = match partition {
                Some(partition) => partition(disk_source)?,
                None => disk_source,
            };
            ...
            let value = value.prepare_accepted_bank(
                observer,
                custody,
                OperationRequest::Speculative(role.clone()),
                OperationControls::Speculative(role.clone()),
                disk_source,
                materialized.map(materialization::MaterializationOwner::from_loan),
            )?;
...
impl<U: 'static> SelectedOriginalOperationPlan<'_, U> {
            ...
            Self::Resident(plan) => {
                if recipe.invocation().execution_pass() == eredu_runtime::ExpertPass::Prefill {
                    plan.speculative_span_control_bytes(recipe)
                        .and_then(|bytes| {
                            bytes.checked_mul(u64::try_from(recipe.records().len()).ok()?)
                        })
                        .ok_or_else(|| {
                            unknown().at_speculative_stage("resident span control bound")
                        })
                } else {
                    plan.speculative_control_bytes(recipe).ok_or_else(|| {
                        unknown().at_speculative_stage("resident invocation control bound")
                    })
                }
            }
        }
    ...
    pub(crate) fn external_control_bytes(
        self,
        recipe: &ResidentNativeRecipe,
    ) -> Result<u64, Error> {
        if recipe.records().len() != 1 {
    ...
    pub(crate) fn prepare_external(
        ...
        match self {
            Self::Bounded(plan) => {
                plan.prepare_external_bank(recipe, invocation, role, scope, partition, materialized)
            }
            Self::Resident(plan) => {
                plan.prepare_external(recipe, invocation, role, scope, partition)
            }
        }
    }
    pub(crate) fn bind_embedded_recipe(
        ...
        match self {
            Self::Bounded(plan) => {
                recipe.with_native_recipe(|recipe| plan.bind_role_recipe(recipe))
            }
    ...
    pub(crate) fn prepare_embedded(
        ...
        match self {
            Self::Bounded(plan) => {
                plan.prepare_embedded_bank(recipe, role, scope, partition, materialized)
            }
    ...
    pub(crate) fn prepare_speculative(
        ...
        match self {
            Self::Bounded(plan) => {
                plan.prepare_speculative_bank(recipe, role, None, scope, partition, materialized)
            }
    ...
    pub(crate) fn prepare_speculative_span(
        ...
        match self {
            Self::Bounded(plan) => plan.prepare_speculative_bank(
                recipe,
                span.role().clone(),
                Some(span),
                scope,
