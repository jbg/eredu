...
pub(super) fn prepare(
    ...
    let projected = match project_partitioned_text_binding_destinations(sources, &context) {
        ...
        Err(PreparedExecutionError::UnavailablePrediction) => return Ok(None),
    ...
    let local = tasks
        ...
        .collect::<BTreeSet<_>>();
    let excluded = projected.excluded_parameters();
    let bind = |destinations,
        ...
        .ok_or_else(|| {
            Error::ArchitectureModel("invalid cold partition destination representation".into())
        })?;
        eredu_runtime::build_exact_replicated_text_bindings_for_targets(
            ...
            tasks,
            excluded,
            Some(projected.physical_layout()),
            |_task, recipe, source| {
    ...
    if projected.units().len() != unit_tasks.len() {
        return Err(Error::ArchitectureModel(
            "partition source destinations differ from local task rows".into(),
        ));
    }
    let unit_bindings = projected
        ...
        .iter()
        .zip(&unit_tasks)
        .map(|(destinations, tasks)| bind(destinations, tasks, excluded))
    ...
    for parameter in selected
        ...
        .iter()
        .filter(|parameter| {
            excluded.contains(parameter.name()) || !local.contains(parameter.name())
        })
    {
