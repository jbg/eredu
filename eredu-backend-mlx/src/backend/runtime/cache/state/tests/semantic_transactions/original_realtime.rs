...
fn append(state: &mut MlxKeyValueState, rows: usize, base: f32, stream: &Stream) {
    let keys = (0..rows * 8)
        .map(|index| base + index as f32 / 16.0)
        .collect::<Vec<_>>();
    let values = keys
        .iter()
        .map(|value| 1.0 - 3.0 * value)
        .collect::<Vec<_>>();
...
fn tail(state: &MlxKeyValueState) -> Vec<Vec<f32>> {
    let MlxKeyValueLayerState::Paged(cache) = &state.layers.slots()[0] else {
        panic!("actual pager")
    };
    ...
    cache.visit_realtime_tail_operands(&mut |array, copy| {
        assert!(
            !copy,
            "full-pager mutable tail branches retain immutable array aliases"
        );
...
fn paid_branch(
    ...
    let branch = plan
        .prepare(None, None, runtime, stream, &funding, None)
        .unwrap();
    assert!(pool.fixture_host_charge().unwrap() > initial);
    assert!(
        pool.fixture_host_charge().unwrap() <= initial + quoted,
        "source quote bounds actual branch controls"
    );
    drop(funding);
    assert!(
        pool.fixture_host_charge().unwrap() > 0,
        "branch owns its original Host source"
    );
...
fn original_full_paged_realtime_branch_preserves_pages_tail_and_host_custody() {
    ...
    assert!(
        pool.fixture_host_charge().unwrap() > 0,
        "canonical reporting retains the source after empty discard"
    );
    let first_source_pool = pool;
    let first_source_charge = first_source_pool.fixture_host_charge().unwrap();
    ...
    assert_eq!(branch.state.offset(), 9);
    assert!(ids(&pager).len() > original_ids.len());
    assert_eq!(canonical.offset(), 5);
    ...
    assert_eq!(
        pager.report().unwrap().mutable_tail_bytes,
        original_report.mutable_tail_bytes
    );
    assert!(
        pool.fixture_host_charge().unwrap() > 0,
        "rollback retains canonical reporting storage under its actual source"
    );
    assert!(
        pool.fixture_host_peak().unwrap() <= ceiling,
        "quoted controls include actual rollback work"
    );
    ...
    assert_eq!(
        discarded_reporting_pool.fixture_host_charge().unwrap(),
        0,
        "fresh reporting replaces and retires discarded-frame storage"
    );
    append(&mut resumed.state, 5, 31.0, &stream);
    canonical.commit_branch(resumed).unwrap();
    assert_eq!(canonical.offset(), 10);
    assert_eq!(canonical.offset(), ordinary.offset());
    assert_eq!(tail(&canonical), tail(&ordinary));
    assert_eq!(pager.report().unwrap().logical_cached_tokens, 10);
    assert!(
        original_ids.iter().all(|id| ids(&pager).contains(id)),
        "canonical sealed pages survive commit"
    );
    assert!(
        pool.fixture_host_charge().unwrap() > 0,
        "committed paid state retains its Host storage source"
    );
    let retained_revision =
        eredu_runtime::working_memory::InferenceStateRetention::inference_retention(&canonical)
            .revision()
            .clone();
    drop(canonical);
    assert_eq!(
    ...
    assert!(
        pool.fixture_host_charge().unwrap() > 0,
        "remaining manager alias still owns paid reporting storage"
    );
    drop(pager);
    assert_eq!(
        pool.fixture_host_charge().unwrap(),
        0,
        "last paid canonical and reporting owner retires the current account"
    );
    assert_eq!(
        first_source_pool.fixture_host_charge().unwrap(),
        first_source_charge,
    ...
    let error = RealtimeKvBranchPlan::inspect(&sliding)
        .err()
        .expect("sliding rollback remains unqualified");
    assert!(matches!(
        error,
