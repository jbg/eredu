//! Closed resident Pooling construction into the actual installable dense table.
...
use eredu_runtime::{
    ...
    working_memory::{
        FundedDenseHostSlots, InferencePromptCompletion, InitializedDenseDecoderSlots,
...
type DenseSlots<'a> =
    FundedDenseHostSlots<'a, MlxPoolingAttentionCache, MlxPoolingAttentionCache, StorageIdentity>;
...
impl<'a> PreparedResidentPoolingCopy<'a> {
    ...
    pub(crate) fn dense_initialization(
        ...
        DenseHostSlotInitialization<'a, MlxPoolingAttentionCache, MlxPoolingAttentionCache>,
    ...
    pub(crate) fn dense_initialization_peak_bytes_fixed(
        ...
        Ok(self
            ...
            .for_dense_destination::<MlxPoolingAttentionCache>()?
    ...
    pub(crate) fn copy_dense_with_preparation(
        ...
        let layers = super::super::super::resident_copy::host_copy::copy_slots(
            ...
            funding,
            |i, _| {
                self.copy_layer_retained(i, stream, roots)
                    .map_err(|cause| Error::Neural(funding.metadata_source(cause)))
            },
        )?;
        Ok(
            MlxPoolingAttentionState::from_prepared_layers(self.shared_layout().clone(), layers)
                .expect("same source layout and exact destination length"),
        )
    }
    ...
    pub(crate) fn copy_dense_retained(
        ...
        stream: &Stream,
        roots: &RefCell<Vec<Array>>,
    ) -> Result<PreparedDenseResidentPoolingState<'a>, ResidentPoolingCopyError> {
        ...
        let layers = destination
            .finish()
            .map_err(|_| WorkingMemoryError::IdentityMismatch)?;
        Ok(PreparedDenseResidentPoolingState {
            layers,
...
pub(crate) struct PreparedDenseResidentPoolingState<'a> {
    layers: DenseSlots<'a>,
    layout: SharedStateLayout,
}

impl PreparedDenseResidentPoolingState<'_> {
    ...
    /// Final destination array roots only, in layer order: local keys/values, then both ordered pooling streams.
    ...
    pub(crate) fn visit_operands<'s>(&'s self, visitor: &mut dyn FnMut(&'s Array)) {
        for index in 0..self.layers.len() {
            self.layers
                .get(index)
                .expect("completed dense layer count")
                .prepare_isolated_copy()
                .expect("validated resident layer")
...
#[derive(Debug)]
pub(crate) struct PublishedDenseResidentPoolingState {
    state: MlxPoolingAttentionState,
}

impl PublishedDenseResidentPoolingState {
    /// Moves the already published table without copying it. Extraction adds no
    /// completion or execution authority; only the typed Pooling adapter uses it.
    pub(crate) fn into_state(self) -> MlxPoolingAttentionState {
        self.state
    }
}

impl<'a> PreparedDenseResidentPoolingState<'a> {
    ...
    pub(crate) fn publish_for_control(
        ...
        DenseResidentPoolingPublishError<'a>,
    > {
        self.publish()
            .map(|(state, completion)| (PublishedDenseResidentPoolingState { state }, completion))
    }
    ...
    pub(crate) fn publish(
        ...
        DenseResidentPoolingPublishError<'a>,
    > {
        let Self { layers, layout } = self;
        let key =
            StorageIdentity::HostMetadata(layers.metadata().identity().registry_key().clone());
        match layers.publish(key) {
            Ok((layers, completion)) => Ok((
                MlxPoolingAttentionState::from_prepared_layers(layout, layers)
                    .expect("closed source and destination preserve exact layout length"),
                completion,
            )),
            Err(error) => {
                let (layers, error) = error.into_parts();
                Err(DenseResidentPoolingPublishError {
                    owner: Self { layers, layout },
                    error,
                })
            }
        }
...
impl fmt::Debug for PreparedDenseResidentPoolingState<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("PreparedDenseResidentPoolingState")
...
#[derive(Debug)]
pub(crate) struct DenseResidentPoolingPublishError<'a> {
    owner: PreparedDenseResidentPoolingState<'a>,
    error: HostSlotAttachmentError<WorkingMemoryError>,
}

impl<'a> DenseResidentPoolingPublishError<'a> {
    pub(crate) fn into_parts(
        self,
    ) -> (
        PreparedDenseResidentPoolingState<'a>,
...
impl fmt::Display for DenseResidentPoolingPublishError<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Display::fmt(&self.error, f)
    }
}

impl std::error::Error for DenseResidentPoolingPublishError<'_> {
