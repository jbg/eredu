//! Actual saved/live source projected through the closed Pooling dense-copy program.

use super::PreparedResidentPoolingCopy;
use crate::backend::{
...
pub(crate) struct ProjectedDenseResidentPoolingCopy {
...
impl<'a> PreparedResidentPoolingCopy<'a> {
    ...
    /// The selected local/pooling controls and geometry are validated by
    ...
    pub(crate) fn project_dense_workspace(
    ...
    ) -> Result<ProjectedDenseResidentPoolingCopy, Error> {
        let mut projection = ExistingArrayProjection::new(context);
        let mut retained =
            projection.project_roots(|visitor| self.visit_retained_arrays(visitor))?;
        ...
        Ok(ProjectedDenseResidentPoolingCopy {
            state,
            source_storage,
            copy,
        })
