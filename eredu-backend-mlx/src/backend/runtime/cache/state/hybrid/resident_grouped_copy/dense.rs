...
impl<'a> PreparedHybridGroupedCopy<'a> {
    ...
    pub(crate) fn copy_dense_with_preparation(
        ...
        if self.is_paged() {
            return Err(Error::Neural(
                funding.metadata_source(WorkingMemoryError::UnknownBound),
            ));
        }
    ...
    pub(crate) fn copy_dense_retained(
        self,
        slots: InitializedHybridDenseGroup<'a>,
        stream: &Stream,
        roots: &RefCell<Vec<Array>>,
    ...
    pub(crate) fn copy_dense_retained_with(
        ...
        let (layers, manager) = match (self.source, slots) {
            (Source::Live(s), InitializedHybridDenseGroup::Live(slots, paged)) => {
                let (layers, manager) = self.copy_dense_group(
                    slots,
                    s.layers
                        .prepare_copy_slots()
                        .map_err(other)?
        ...
        Ok(PreparedDenseHybridGroupedState {
            layers,
            layout: self.shared_layout().clone(),
            global_layer_start: self.global_layer_start(),
            manager,
    ...
    fn copy_dense_group<S>(
        ...
        (
            FundedDenseHostSlots<'a, S, MlxHybridLayerState, StorageIdentity>,
    ...
    > {
        ...
        let result = (|| {
            ...
            if let Some(work) = &mut paged {
                work.copy_pages(stream, roots, observe, &mut |_| Ok(()), true)?;
            }
            for i in 0..self.len() {
                ...
                ))) = &attention
                {
                    if let Some(manager) = &manager {
                        ...
                        manager = Some(cache.manager().clone());
                    }
                }
                ...
                let key = StorageIdentity::HostMetadata(
                    fixed.metadata().identity().registry_key().clone(),
                );
...
impl super::super::super::key_value::PagedSnapshotState for PreparedDenseHybridGroupedState<'_> {
    ...
    fn layer_count(&self) -> usize {
        match &self.layers {
            DenseStorage::Live(s) => s.len(),
            DenseStorage::Saved(s) => s.len(),
        }
    }
    ...
    fn layer(
        &self,
        index: usize,
    ) -> Option<super::super::super::key_value::PagedSnapshotLayer<'_>> {
        let layer = match &self.layers {
            DenseStorage::Live(s) => s.get(index),
            DenseStorage::Saved(s) => s.get(index),
        }?;
        Some(super::paged::attention(layer.attention.as_ref()))
    }
}
impl PreparedDenseHybridGroupedState<'_> {
    pub(crate) fn visit_complete_operands(
        &self,
        ...
        if let Some(manager) = &self.manager {
            ...
            if !manager.same_catalog(source.manager()) {
                return Err(SnapshotProjectionCause::Changed);
            }
            source.visit_arrays(&mut |array| {
                visitor(array);
                Ok(())
            })?;
        }
