...
use eredu_nn::workspace::{
    HostMetadataFunding, HostMetadataFundingError,
};
pub(super) fn control_bytes(bytes: usize) -> Option<usize> {
    let controls = [
        bytes,
        eredu_core::HostMetadataFunding::prepaid_control_bytes()?,
        WorkspaceContext::construction_bytes::<crate::backend::runtime::cache::state::paged_reset::HostMetadata>(
        )?,
        size_of::<HostFailure>(),
        ...
        eredu_core::BackendFailure::source_retention_peak_bytes::<HostMetadataFundingError>()?,
    ];
    controls
        .into_iter()
...
pub(super) fn prepare(
    ...
    WorkspaceContext::new_with_metadata_funding(
        crate::backend::runtime::cache::state::paged_reset::HostMetadata,
        funding,
    )
