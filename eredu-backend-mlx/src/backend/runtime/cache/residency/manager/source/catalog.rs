...
use eredu_runtime::cache::{
    CacheRecordTable, CacheTelemetryRows, PreparedCacheLifecycle, PreparedCacheTable,
    PreparedCacheTelemetry, RetiredCacheLifecycleStorage, RetiredCacheTelemetryStorage,
};
...
impl InstalledManagerCatalog {
    ...
    /// Borrows the installed source without constructing native work. The
    /// consumer owns its prepared destinations and pays its callback frames.
    pub(crate) fn with_source<R>(
        &self,
        selection: eredu_runtime::CacheBlockSelection,
        consume: impl for<'loan> FnOnce(CacheBlockSourceLoan<'loan>) -> Result<R, CacheSourceError>,
    ) -> Result<R, CacheSourceError> {
        self.manager.with_source_loan_inner(
            selection,
            Some(self.publication_controls),
            |cause| cause,
            |loan| {
                if loan.generation() != self.generation {
    ...
    pub(crate) fn source_control_bytes<R>(callback_bytes: usize) -> Option<usize> {
        let frames = [
            size_of::<&Self>(),
            size_of::<eredu_runtime::CacheBlockSelection>(),
            size_of::<Result<R, CacheSourceError>>(),
            CacheResidencyManager::source_loan_control_bytes::<R>(
                size_of::<&Self>().checked_add(callback_bytes)?,
            )?,
        ];
        frames
            .into_iter()
    ...
    /// Reuses the actual report worker after checking its installed destination.
    /// No catalog row is inserted by this operation.
    pub(crate) fn record_scan(
        ...
        state
            .telemetry
            .validate_prepared_layers(std::iter::once(layer))
            .map_err(CacheLifecycleError::from)?;
        reporting::record_scan(&mut state, layer, prefill, blocks, bytes, scratch);
        Ok(())
    }

    pub(crate) fn report_control_bytes() -> Option<usize> {
        let frames = [
            size_of::<(&Self, usize, bool, u64, u64, u64)>(),
...
impl PreparedManagerCatalog {
    ...
    pub(crate) fn control_bytes(
        ...
        frames.into_iter().try_fold(
            std::mem::size_of_val(&frames)
                ...
                .checked_add(
                    PreparedCacheTable::<usize, CacheLayerResidencyStats>::control_bytes(
                        current_layers,
                    )?,
                )?,
            usize::checked_add,
    ...
    pub(super) fn for_empty_manager(
        mut self,
        manager: CacheResidencyManager,
    ...
    fn install_inner(&mut self) -> Result<RetiredCatalog, CacheSourceError> {
        ...
        if state.generation != self.generation
            ...
            || !state
                .telemetry
                .can_install(self.telemetry.as_ref().expect("one telemetry installation"))
        {
            return Err(CacheSourceError::Identity);
        }
        ...
        let telemetry = state
            .telemetry
            .install_storage(self.telemetry.take().expect("one telemetry installation"))
...
impl CacheBlockSourceLoan<'_> {
        ...
        I: Iterator<Item = usize> + Clone,
    {
        let layers = self
            ...
            .chain(self.lifecycle.tail_layers())
            .chain(future_layers);
        let current = layers
            .clone()
            .enumerate()
            .filter(|(index, layer)| !layers.clone().take(*index).any(|earlier| earlier == *layer))
            .count();
    ...
    where
        I: Iterator<Item = usize> + Clone,
    {
        let (current_count, reported_count, traversal) = self
            .catalog_shape(future_layers)
            .ok_or_else(|| CacheSourceFailure::source(CacheSourceError::Overflow, context))?;
        context
            .charge_metadata(traversal)
        ...
        let telemetry = PreparedCacheTelemetry::control_bytes(reported_count)
            .ok_or_else(|| CacheSourceFailure::source(CacheSourceError::Overflow, context))?;
        let current =
            PreparedCacheTable::<usize, CacheLayerResidencyStats>::control_bytes(current_count)
                .ok_or_else(|| CacheSourceFailure::source(CacheSourceError::Overflow, context))?;
        let frames =
            PreparedManagerCatalog::control_bytes(blocks, tails, current_count, reported_count)
        ...
        Ok(PreparedManagerCatalog {
            ...
            telemetry: Some(
                PreparedCacheTelemetry::prepare(reported_count, context)
                    .map_err(|cause| CacheSourceFailure::metadata(cause, context))?,
            ),
            current_rows: Some({
                ...
                drop(
                    rows.install(prepared)
                        .expect("empty current-row destination"),
                );
                rows
            }),
