...
#[derive(Debug, thiserror::Error)]
#[error("ordinary cache transaction rollback: {cause}")]
struct OrdinaryFailure {
    #[source]
    cause: Cause,
    _host: eredu_core::HostPreparationAuthority,
}
fn source(cause: CacheSourceError) -> Cause {
    Cause::Source(cause)
}
impl CacheResidencyManager {
    ...
    pub(crate) fn rollback_realtime_checkpoint(
        &self,
        session: u64,
        layer: usize,
    ...
    /// Actual ordinary caller has already prepaid these fixed controls. The
    /// checkpoint owner supplies retained current/saved source identities.
    #[allow(clippy::too_many_arguments)]
    pub(crate) fn rollback_ordinary_checkpoint(
        &self,
        session: u64,
        layer: usize,
        expected_layer: usize,
        ...
        .map_err(|cause| {
            Exception::from_retained_source(OrdinaryFailure {
                cause,
                _host: host.clone(),
            })
        })
    ...
    fn rollback_checkpoint(
        ...
        current_offset: i64,
        original: &[CacheBlockId],
        retained_window: Option<(i64, i64)>,
    ) -> Result<(), Cause> {
        ...
        let selected = eredu_runtime::CacheBlockSelection::new(
            layer,
        ...
        for (id, record) in state.blocks.iter().filter(|(id, _)| selected.includes(id)) {
            ...
            if id.session_id != session
                || id.start < tail_start
                || id.end <= tail_start
                || id.end > current_offset
                || record.imported
                || state
                    .lifecycle
                    .is_leased(id)
                    .map_err(CacheResidencyError::from)?
                || lifecycle::retained_by_history(&state, id)
            {
                return Err(source(CacheSourceError::Identity));
            }
            removals = removals
        ...
        let generation = old_generation
            .checked_add(u64::try_from(removals).map_err(|_| source(CacheSourceError::Overflow))?)
            .ok_or_else(|| source(CacheSourceError::Overflow))?;
        ...
        for _ in 0..removals {
            ...
            let id = state
                ...
                .keys()
                .rev()
                .find(|id| selected.includes(id) && !original.contains(id))
                .cloned()
                .ok_or_else(|| source(CacheSourceError::Identity))?;
        ...
        if state.generation != old_generation
            || state
                .blocks
                .keys()
                .any(|id| selected.includes(id) && !original.contains(id))
        {
            return Err(source(CacheSourceError::Identity));
        }
    ...
    fn realtime_transaction_lock(&self) -> Result<MutexGuard<'_, CacheManagerState>, Cause> {
        ...
        if !self.borrowed_storage_complete(&state) {
            return Err(source(CacheSourceError::PendingStorage));
        }
        Ok(state)
    }
    pub(crate) fn ordinary_transaction_rollback_control_bytes() -> Option<usize> {
        Self::realtime_transaction_rollback_control_bytes()?
            .checked_add(size_of::<OrdinaryFailure>())?
            .checked_add(Exception::retained_source_control_bytes::<OrdinaryFailure>()?)?
            .checked_add(size_of::<(
