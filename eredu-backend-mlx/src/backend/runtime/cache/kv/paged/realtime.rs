...
impl Hasher for SourceHash {
    ...
    fn finish(&self) -> u64 {
        unreachable!("source hash is finalized as all 32 bytes")
    }
}
fn overflow() -> Error {
...
fn describe(source: &PagedKeyValueSource<'_>) -> Result<RealtimePagedSource, Error> {
    let manager = source.manager_source();
    let mut hash = SourceHash(Sha256::new());
    manager.session_id().hash(&mut hash);
    manager.generation().hash(&mut hash);
    let tail = manager.tail();
    tail.map(|t| (t.bytes, t.end)).hash(&mut hash);
    let mut blocks = 0usize;
    for block in manager.blocks() {
        block.id().hash(&mut hash);
        blocks = blocks.checked_add(1).ok_or_else(overflow)?;
    }
    ...
    let rollback_controls = CacheResidencyManager::realtime_transaction_rollback_control_bytes()
        .and_then(|n| n.checked_add(manager.publication_control_bytes()))
        .ok_or_else(overflow)?;
    Ok(RealtimePagedSource {
        identity: hash.0.finalize().into(),
        arrays,
        blocks,
        prepare_controls: prepare_controls().ok_or_else(overflow)?,
        rollback_controls,
    })
}
fn inspect(source: PagedKeyValueSource<'_>) -> Result<RealtimePagedSource, Error> {
    describe(&source)
}
fn checkpoint_callback<'a>(
    ids: Option<&'a mut Vec<CacheBlockId>>,
    expected: Option<RealtimePagedSource>,
...
fn prepare_controls() -> Option<usize> {
    ...
    let frames = [
        size_of::<RealtimePagedSource>(),
        size_of::<OriginalRollback>(),
        size_of::<SourceHash>(),
        size_of::<Sha256>(),
        size_of::<Result<RealtimePagedSource, Error>>(),
        size_of::<Result<Option<MutableCacheTail>, Error>>(),
        size_of::<[Option<Array>; 2]>(),
        size_of::<PagedKeyValueTransactionCheckpoint>(),
        size_of::<Result<(PagedKeyValueCache, PagedKeyValueTransactionCheckpoint), Error>>(),
        size_of::<(
            &PagedKeyValueCache,
            RealtimePagedSource,
            &HostMetadataFunding,
        ...
        PagedKeyValueCache::workspace_source_control_bytes::<Option<MutableCacheTail>, _>(
            &callback,
        )?,
    ];
    frames
...
impl RealtimePagedSource {
    pub(in crate::backend::runtime::cache) fn host_bytes(self) -> Option<usize> {
        self.prepare_controls
            .checked_add(self.rollback_controls)?
            .checked_add(WorkspaceContext::metadata_vec_bytes::<CacheBlockId>(
                self.blocks,
            )?)
    }
}
impl PagedKeyValueCache {
    ...
    pub(in crate::backend::runtime::cache) fn inspect_realtime_branch(
        &self,
    ) -> Result<RealtimePagedSource, Error> {
    ...
    pub(in crate::backend::runtime::cache) fn visit_realtime_tail_operands(
        &self,
        visit: &mut dyn FnMut(&Array, bool),
    ) {
        if let Some(array) = &self.tail_keys {
            visit(array, false);
        }
        if let Some(array) = &self.tail_values {
            visit(array, false);
        }
    }
    pub(in crate::backend::runtime::cache) fn prepare_realtime_branch(
        &self,
        expected: RealtimePagedSource,
        funding: &HostMetadataFunding,
        alias: &mut dyn FnMut(&Array) -> Result<Array, Error>,
    ) -> Result<(Self, PagedKeyValueTransactionCheckpoint), Error> {
        funding
            .reserve_metadata(expected.prepare_controls)
            .map_err(Error::WorkspacePlanning)?;
        let mut ids = funding
            .metadata_vec(expected.blocks)
            .map_err(Error::Neural)?;
        let tail = self.inspect_copy_source(
            source_error,
            checkpoint_callback(Some(&mut ids), Some(expected)),
        )?;
        ...
        let checkpoint = PagedKeyValueTransactionCheckpoint {
            ...
            offset: self.offset,
            tail_bytes: tail.map_or(0, |value| value.bytes),
            block_ids: ids,
            original: Some(OriginalRollback {
                tail,
                tail_start: self.tail_start,
                controls: expected.rollback_controls,
                funding: funding.clone(),
            }),
        };
        Ok((
            self.copied_with_tails(self.manager.clone(), keys, values, None),
            checkpoint,
        ))
    }
    pub(super) fn rollback_original_transaction(
        &self,
        checkpoint: &PagedKeyValueTransactionCheckpoint,
        original: &OriginalRollback,
    ) -> Result<(), Exception> {
        self.manager.rollback_realtime_checkpoint(
