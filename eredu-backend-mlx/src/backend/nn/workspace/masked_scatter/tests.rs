...
fn masked_scatter_prices_copies_offsets_and_full_source_cast() {
    ...
    assert_eq!(report.total_bytes, Some(2175 + 511 + 543 + 2175));
...
fn masked_scatter_rejects_bad_shapes_and_unrepresentable_metal_scan_domains() {
    ...
    assert!(
        emit(op.as_view(), allocation, &mut Emitter::count())
            .unwrap()
            .is_none()
    );
...
fn masked_scatter_scalar_follows_destination_cast_without_inventing_evidence() {
    ...
    for destination in [WorkspaceFloatingType::Float32, WorkspaceFloatingType::Float16, WorkspaceFloatingType::Bfloat16] {
        for source in [None, Some(WorkspaceFloatingType::Float32), Some(WorkspaceFloatingType::Float16), Some(WorkspaceFloatingType::Bfloat16)] {
            ...
            op.inputs[0] = op.inputs[0].clone().with_representation(Some(WorkspaceRepresentation::new(destination, false)));
            op.inputs[2] = op.inputs[2].clone().with_representation(source.map(|scalar| WorkspaceRepresentation::new(scalar, false)));
            assert_eq!(mechanism.output_representation(op.as_view(), 0),
    ...
    op.inputs[0] = op.inputs[0].clone().with_representation(Some(WorkspaceRepresentation::new(WorkspaceFloatingType::Float32, false)));
    assert!(mechanism.output_representation(op.as_view(), 0).is_none());
}
...
fn native_masked_scatter_source_cast_matches_all_reported_destination_types() {
    ...
    for (destination, dtype) in formats {
        let input = Array::from_slice(&[1_f32, 2., 3., 4., 5., 6.], &[1, 3, 2]).as_dtype(dtype, &stream).unwrap();
        let mask = Array::from_slice(&[false, true, true], &[1, 3]);
        for (source, source_dtype) in formats {
            let rows = Array::from_slice(&[-1_f32, -2., 7., 8.], &[2, 2]).as_dtype(source_dtype, &stream).unwrap();
            let output = safemlx::ops::indexing::masked_scatter(&input, &mask, &rows, &stream).unwrap();
            assert_eq!(output.dtype(), dtype);
            let op = operation(&[1, 3, 2], &[1, 3], &[2, 2]);
            let mut op = op;
            op.inputs[0] = op.inputs[0].clone().with_representation(Some(WorkspaceRepresentation::new(destination, false)));
            op.inputs[2] = op.inputs[2].clone().with_representation(Some(WorkspaceRepresentation::new(source, false)));
            assert_eq!(mechanism.output_representation(op.as_view(), 0).unwrap().dtype(), destination);
            let values = output.as_dtype(Dtype::Float32, &stream).unwrap();
            assert_eq!(values.evaluated().unwrap().as_slice::<f32>(), &[1., 2., -1., -2., 7., 8.]);
        }
    }
}
