...
pub(crate) enum LogicalCollectiveStages {
    Exchange {
        ...
        peer_scratch: Option<WorkspaceAllocationPopulation>,
        peer_capacity: BoundaryStageCapacity,
        result_recipe: SpeculativeNumericalRecipe,
        result_scratch: Option<WorkspaceAllocationPopulation>,
    ...
    RoutedPeer {
        value: usize,
        step: usize,
        pairs: [OwnedOriginalExchangeLayoutRound; 1],
        peer_recipe: SpeculativeNumericalRecipe,
        peer_scratch: Option<WorkspaceAllocationPopulation>,
        peer_capacity: BoundaryStageCapacity,
    },
...
impl LogicalCollectiveQuote {
    ...
    pub(crate) fn prepare(
        ...
        let kind = match native {
            ...
            _ => {
                return Err(
                    source.neural_error(crate::backend::error::Error::PrefillScopeUnavailable)
                );
            }
        };
    ...
    pub(crate) fn prepare_group(
        ...
        input: eredu_nn::workspace::WorkspaceLayoutView<'_>,
        dtype: safemlx::Dtype,
        mechanism: ResidentExecutionMechanisms,
    ) -> Result<Self, Error> {
        let protocol = match kind {
            LogicalCollectiveKind::Sum => eredu_runtime::CommunicationOperation::AllReduceSum,
    ...
    pub(crate) fn prepare_agreement(
        ...
        let invalid = || {
            context.metadata_error(format_args!(
                "provider agreement differs from its selected control declaration"
            ))
        };
        let actual = source
    ...
    fn prepare_group_protocol(
        ...
        if selected
            ...
            .is_some()
        {
            let value = routed::RoutedCollectiveQuote::prepare(
                source, &actual, order, kind, &layout, dtype, mechanism, protocol, &context,
            )?;
            let output = u64::try_from(value.result_capacity.backing).map_err(|_| invalid())?;
            let scratch =
                u64::try_from(value.scratch().ok_or_else(invalid)?).map_err(|_| invalid())?;
            return Ok(Self {
                group,
                order,
                rank: selected.rank(),
                ...
                input: layout,
                native_dtype: dtype,
                stages: LogicalCollectiveStages::Routed(value),
                output,
                scratch,
                source: source.clone(),
            });
        }
        let Some(exchange) = actual
            .group_exchange(order)
            .map_err(|cause| source.neural_error(cause))?
        else {
            let value = packed::PackedWorldQuote::prepare(
                source, &actual, order, kind, &layout, dtype, mechanism, &context,
            )?;
            ...
            let retained = match kind {
                LogicalCollectiveKind::Sum => value
                    .world
                    .backing_capacity()
                    .max(value.result_capacity.backing),
            ...
            return Ok(Self {
                ...
                input: layout,
                native_dtype: dtype,
                stages: LogicalCollectiveStages::Packed(value),
                output,
                scratch,
                source: source.clone(),
            });
        ...
        for round in 0..exchange.rounds() {
            pairs.push(
                exchange
                    .round_layout_storage(&actual, round, input.shape(), dtype)
                    .map_err(|cause| source.neural_error(cause))?
        ...
        let peer_scratch = context.new_allocation_scratch()?;
        let peer_report = context.finish_report(&[peer.clone()])?;
        let peer_recipe = super::numerical(&peer_report, 1, mechanism, &context)?;
        ...
        let result_recipe = super::numerical(&result_report, 1, mechanism, &context)?;
        ...
        let scratch = pairs
            ...
            .try_fold(0usize, |total, pair| {
                total
                    .checked_add(pair.backing_capacity())?
                    .checked_add(peer_capacity.backing)
            })
            .and_then(|n| u64::try_from(n).ok())
            .ok_or_else(invalid)?;
        Ok(Self {
            ...
            input: layout,
            native_dtype: dtype,
            stages: LogicalCollectiveStages::Exchange {
                pairs,
                peer_recipe,
                peer_scratch,
                peer_capacity,
    ...
    pub(crate) fn prepare_routed_peer(
        source: &OriginalParallelSource,
        order: usize,
        value: usize,
        step: usize,
        ...
        Self::prepare_routed_peer_protocol(
            source,
            order,
            value,
            step,
    ...
    fn prepare_routed_peer_protocol(
        source: &OriginalParallelSource,
        order: usize,
        value: usize,
        step: usize,
        ...
        let invalid = || {
            context.metadata_error(format_args!(
                "routed count peer differs from its retained itinerary"
            ))
        };
        let actual = source
            .communication_source()
        ...
        let pair = actual
            .group_routed_exchange_layout(order, value, step, input.shape(), dtype)
            .map_err(|cause| source.neural_error(cause))?
            .ok_or_else(invalid)?
            .try_into_owned(source)
            .map_err(|cause| source.neural_error(cause))?;
        ...
        Ok(Self {
            ...
            order,
            rank: group.rank(),
            kind: LogicalCollectiveKind::Gather,
            protocol,
            input: layout,
            native_dtype: dtype,
            stages: LogicalCollectiveStages::RoutedPeer {
                value,
                step,
                pairs: [pair],
                peer_recipe,
    ...
    pub(crate) fn routed(&self) -> Option<&routed::RoutedCollectiveQuote> {
        match &self.stages {
            LogicalCollectiveStages::Routed(value) => Some(value),
            _ => None,
        }
    }
    pub(crate) fn routed_step(&self) -> Option<(usize, usize)> {
        match &self.stages {
            LogicalCollectiveStages::RoutedPeer { value, step, .. } => Some((*value, *step)),
            _ => None,
        }
    }
    ...
    pub(crate) fn packed(&self) -> Option<&packed::PackedWorldQuote> {
        match &self.stages {
            LogicalCollectiveStages::Packed(value) => Some(value),
            _ => None,
        }
    }
    pub(crate) fn exchange_pairs(&self) -> Option<&[OwnedOriginalExchangeLayoutRound]> {
        match &self.stages {
            LogicalCollectiveStages::Exchange { pairs, .. } => Some(pairs),
            LogicalCollectiveStages::RoutedPeer { pairs, .. } => Some(pairs.as_slice()),
            _ => None,
        }
    }
    pub(crate) fn arithmetic_capacity(&self, peer: bool) -> Option<BoundaryStageCapacity> {
        match &self.stages {
            LogicalCollectiveStages::Exchange {
                peer_capacity,
                result_capacity,
                ..
            } => Some(if peer {
            ...
            LogicalCollectiveStages::RoutedPeer { peer_capacity, .. } if peer => {
                Some(*peer_capacity)
            }
            _ => None,
        }
    }
    /// Complete child population, including each independently executed pair
    /// and its numerical continuation. Existing inputs retain their own owners.
    pub(crate) fn allocation_scratch(
        &self,
        context: &WorkspaceContext,
        mechanism: ResidentExecutionMechanisms,
    ) -> Result<Option<WorkspaceAllocationPopulation>, Error> {
        self.allocation_population(context, mechanism, true)
    }
    pub(crate) fn operation_scratch(
        ...
        self.allocation_population(context, mechanism, false)
    }
    pub(crate) fn output_allocations(&self) -> Option<&WorkspaceAllocationPopulation> {
        match &self.stages {
            LogicalCollectiveStages::Exchange { result_scratch, .. } => result_scratch.as_ref(),
            LogicalCollectiveStages::Packed(value) => value.output_source.as_ref(),
            LogicalCollectiveStages::Routed(value) => value.result_scratch.as_ref(),
            LogicalCollectiveStages::RoutedPeer { peer_scratch, .. } => peer_scratch.as_ref(),
        }
    }
    fn allocation_population(
        &self,
        context: &WorkspaceContext,
        mechanism: ResidentExecutionMechanisms,
        include_output: bool,
        ...
        let count = match &self.stages {
            LogicalCollectiveStages::Exchange { pairs, .. } => pairs
                .len()
                .checked_mul(2)
                .and_then(|n| n.checked_add(1))
                .ok_or_else(overflow)?,
            LogicalCollectiveStages::Packed(_) => 3,
            LogicalCollectiveStages::Routed(value) => value
                .values
                .iter()
                .try_fold(1usize, |n, row| n.checked_add(row.steps.len()))
                .ok_or_else(overflow)?,
            LogicalCollectiveStages::RoutedPeer { pairs, .. } => {
                pairs.len().checked_add(1).ok_or_else(overflow)?
            }
        };
        ...
        match &self.stages {
            LogicalCollectiveStages::Exchange {
                ...
                let (Some(peer), Some(result)) = (peer_scratch, result_scratch) else {
                    return Ok(None);
                };
                for pair in pairs {
                    populations.push(super::scratch::native_cpu(
                        context,
                        mechanism,
                        pair.backing_capacity(),
                        pair.maximum_backing_births().ok_or_else(overflow)?,
                    )?);
                    populations.push(peer.clone());
                }
                if include_output {
                    populations.push(result.clone());
                }
            }
            LogicalCollectiveStages::Packed(value) => {
                let (Some(scratch), Some(output)) = (&value.operation_source, &value.output_source)
                else {
                    return Ok(None);
                };
                populations.push(scratch.clone());
                if include_output {
                    populations.push(output.clone());
                }
            }
            LogicalCollectiveStages::Routed(value) => {
                let Some(result) = &value.result_scratch else {
                    return Ok(None);
                };
                for row in &value.values {
                    for step in &row.steps {
                        let Some(population) =
                            step.source.value().allocation_scratch(context, mechanism)?
                        else {
                            return Ok(None);
                        };
            ...
            LogicalCollectiveStages::RoutedPeer {
                pairs,
                peer_scratch,
                ..
            } => {
                let Some(peer) = peer_scratch else {
                    return Ok(None);
                };
                for pair in pairs {
                    populations.push(super::scratch::native_cpu(
    ...
    pub(crate) fn maximum_backing_births(&self) -> Option<usize> {
        match &self.stages {
            LogicalCollectiveStages::Exchange {
                ...
                .iter()
                .try_fold(result_recipe.storage.maximum_births(), |total, pair| {
                    total
                        .checked_add(pair.maximum_backing_births()?)?
            ...
            LogicalCollectiveStages::RoutedPeer {
                ...
                .iter()
                .try_fold(peer_recipe.storage.maximum_births(), |total, pair| {
                    total.checked_add(pair.maximum_backing_births()?)
                }),
        }
    }
}
