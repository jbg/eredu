...
use crate::backend::runtime::distributed::topology::original_source::{
    OriginalCommunicationSource, parallel::RetainedLogicalCollective,
};
...
impl RoutedCollectiveQuote {
    pub(super) fn prepare(source: &OriginalParallelSource, actual: &OriginalCommunicationSource<'_>,
        order: usize, kind: LogicalCollectiveKind, layout: &WorkspaceLayout,
        dtype: safemlx::Dtype, mechanism: ResidentExecutionMechanisms,
        protocol:eredu_runtime::CommunicationOperation, context: &WorkspaceContext)
        -> Result<Self, Error> {
        context.charge_metadata(size_of::<(Self, Result<Self, Error>, Vec<RoutedValueQuote>,
            Vec<RoutedStepQuote>, Vec<(usize, WorkspaceTensor)>, LogicalCollectiveQuote,
            WorkspaceTraceReport, SpeculativeNumericalRecipe, BoundaryStageCapacity, Vec<bool>,
            std::ops::Range<usize>, Option<usize>, usize, bool)>())?;
        let invalid = || context.metadata_error(format_args!("routed model source differs from its retained plan"));
        let group = actual.group(order).ok_or_else(invalid)?.0;
        let plan = group.logical_routed_plan().map_err(|_| invalid())?.ok_or_else(invalid)?;
        if plan.len() != group.size() || plan.len() == 0 { return Err(invalid()); }
        ...
        for (value, route) in plan.values().enumerate() {
            ...
            if *occupied { return Err(invalid()); }
            *occupied = true;
            let count = (0..route.steps()).filter(|step| route.exchange(*step).is_some()).count();
            let mut steps = context.metadata_vec(count)?;
            for step in 0..route.steps() {
                if route.exchange(step).is_none() { continue; }
                let mut quote = LogicalCollectiveQuote::prepare_routed_peer_protocol(source, order, value, step,
                    layout.as_view(), dtype, mechanism, protocol)?;
                quote.kind = kind;
            ...
            values.push(RoutedValueQuote { source_rank: route.source_rank(), steps });
            // Each routed result is a completed, separately retained descriptor.
            results.push((route.source_rank(), WorkspaceTensor::existing(layout.clone(), context)?));
        }
        context.begin_span();
        let ops = logical_collective::Workspace(context);
        let result = match kind {
            LogicalCollectiveKind::Sum => logical_collective::routed::sum(&ops, results)?,
            LogicalCollectiveKind::Gather => logical_collective::routed::gather(&ops, results, layout.shape())?,
        };
        ...
        Ok(Self { values, result_recipe, result_capacity })
    }
    pub(super) fn scratch(&self) -> Option<usize> {
        self.values.iter().try_fold(0usize, |total, value| value.steps.iter().try_fold(total, |total, step| {
            total.checked_add(usize::try_from(step.source.value().output).ok()?)?
                .checked_add(usize::try_from(step.source.value().scratch).ok()?)
        }))
    }
    pub(super) fn maximum_births(&self) -> Option<usize> {
        self.values.iter().try_fold(self.result_recipe.storage.maximum_births(), |total, value|
