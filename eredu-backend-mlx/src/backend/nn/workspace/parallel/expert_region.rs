...
mod movement;
pub(crate) use movement::{ExpertMovementQuote, ExpertMovementKind};
#[path = "expert_region/transport.rs"]
mod transport;
pub(crate) use transport::{ExpertTransportQuote,ExpertTransferProfile};
...
mod local_transport;
pub(crate) use local_transport::{ExpertLocalTransport,ExpertLocalStage,ExpertLocalStageBound};

#[path="expert_region/observation.rs"]
mod observation;
pub(crate) use observation::{ExpertLocalObservationSource,SourceObserver as GroupedSourceObserver};
...
impl ExpertLocalQuote {
    ...
    pub(crate) fn prepare_with_observation(source:&OriginalParallelSource,operation:WorkspaceOperationView<'_>,
        ...
        let invalid = || context.metadata_error(format_args!("expert child differs from its retained region source"));
        let WorkspaceOperationKindView::ExpertRegion(declaration) = operation.kind else { return Err(invalid()); };
        let observation_descriptor=declaration.observation();
        let view = declaration.as_view();
        view.validate()?;
        if operation.inputs.len() < 4 || operation.outputs.len() != 1 + usize::from(view.kernel.separate_bias(view.tensor_partitions)) {
            return Err(invalid());
        }
        let actual = source.communication_source().map_err(|cause| source.neural_error(cause))?;
        let selected = actual.source().manifest().select_group_operation(view.group,
        ...
        if descriptor.members().len() != view.peers || descriptor.local_index() != Some(view.rank) { return Err(invalid()); }
        let first = operation.inputs.get(0).ok_or_else(invalid)?;
        if first.shape().last().copied() != Some(view.kernel.dimensions().0)
            || first.representation().is_none() { return Err(invalid()); }
        let mut inputs = context.metadata_vec(operation.inputs.len())?;
        for value in operation.inputs.iter() {
            inputs.push(context.layout(value.shape(), value.dtype())?.with_representation(value.representation()));
        }
        ...
        let empty = local_trace_with_outputs(&declaration, &inputs, 0, mechanism, source.funding(), observation, |values| {
        ...
        for rows in candidates {
            ...
            let recipe = local_trace_with_outputs(&declaration, &inputs, rows, mechanism, source.funding(), observation, |values| {
                if rows == maximum_rows { outputs.clear(); for value in values { outputs.push(value.layout().clone()); } }
                Ok(())
            })?;
            if rows == maximum_rows { selected = Some(recipe); }
    ...
    pub(crate) fn parent_capture_population(&self)->crate::backend::array_copy::CaptureNativePopulation {
        self.observation.map(|source|source.parent_population(self.capture_publications)).unwrap_or_default()
    }
    ...
    pub(crate) fn matches_source(&self, source: &OriginalParallelSource) -> bool { self.source.same_source(source) }
    pub(crate) fn source(&self)->&OriginalParallelSource{&self.source}
    pub(crate) fn matches(&self, view: WorkspaceExpertRegionView<'_>) -> bool { self.declaration.as_view() == view }
    ...
    pub(crate) fn actual(&self, rows: usize, native: &[safemlx::Array]) -> Result<SpeculativeNumericalRecipe, Error> {
        ...
        for (index, array) in native.iter().enumerate() {
            if index == 1 { actual.push(context.layout(&[i32::try_from(rows).map_err(|_| invalid())?, 1], WorkspaceDtype::Int32)?); }
            ...
            if (index >= 3 && array.shape() != selected.shape())
                || crate::backend::nn::workspace::byte_view::Dtype::from_layout(selected.as_view())
                    .is_none_or(|dtype| dtype.native() != array.dtype()) { return Err(invalid()); }
            let value = projection.project(array)?;
            actual.push(value.layout().clone());
        }
        if !projection.is_complete() { return Err(invalid()); }
        let recipe = local_trace_with_outputs(&self.declaration, &actual, rows, self.mechanism, self.source.funding(), self.observation, |_|Ok(()))?;
        let capacity = boundary::capacity(recipe, self.source.agreement_inputs().ok_or_else(invalid)?.runtime(), &context)?;
        if capacity.graph > self.capacity.graph || capacity.records > self.capacity.records
            || capacity.backing > self.capacity.backing || recipe.storage.maximum_births() > self.maximum_births
            || recipe.controls > self.controls || recipe.kernels > self.kernels { return Err(invalid()); }
        Ok(recipe)
    }
}
...
where F: FnOnce(&[WorkspaceTensor])->Result<(),Error> {
    ...
    context.charge_metadata(size_of::<(WorkspaceContext, WorkspaceTraceReport, Vec<WorkspaceTensor>,
        Vec<&WorkspaceTensor>, GroupSelection<WorkspaceTensor>, Result<SpeculativeNumericalRecipe, Error>)>())?;
    let invalid = || context.metadata_error(format_args!("expert local equation differs from its retained constructor"));
    let rows = i32::try_from(rows).map_err(|_| invalid())?;
    let first = source.first().ok_or_else(invalid)?;
    let input = WorkspaceTensor::existing(context.layout(&[rows, declaration.as_view().kernel.dimensions().0], first.dtype())?
        .with_representation(first.representation()), &context)?;
    let mut observed=observation.map(|source|observation::SourceObserver::new(source,&input,&context)).transpose()?;
    if rows == 0 {
        context.begin_span();
        let mut output = input.zeros_like(&context)?;
        if matches!(declaration.as_view().kernel, eredu_nn::workspace::WorkspaceExpertKernel::Linear(_)) {
            output = output.reshape(&[0, declaration.as_view().kernel.dimensions().1], &context)?;
        }
        let bias = declaration.as_view().kernel.separate_bias(declaration.as_view().tensor_partitions);
        let mut outputs = context.metadata_vec(1 + usize::from(bias))?;
        if bias { outputs.push(output.clone()); }
        outputs.push(output);
        observe_outputs(&outputs)?;
        return observation::finish(&context,&outputs,observed,mechanism);
    }
    let mut parameters = context.metadata_vec(source.len().checked_sub(4).ok_or_else(invalid)?)?;
    for value in &source[4..] { parameters.push(WorkspaceTensor::existing(context.layout(value.shape(), value.dtype())?.with_representation(value.representation()), &context)?); }
    let mut borrowed = context.metadata_vec(parameters.len())?;
    borrowed.extend(parameters.iter());
    let ids = WorkspaceTensor::existing(context.layout(&[rows, 1], WorkspaceDtype::Int32)?, &context)?;
    let scalar = |index: usize| -> Result<WorkspaceTensor, Error> {
        let layout = source.get(index).ok_or_else(invalid)?;
        WorkspaceTensor::existing(context.layout(&[rows, 1], layout.dtype())?.with_representation(layout.representation()), &context)
    };
    let routes = GroupSelection::new(ids, scalar(2)?, scalar(3)?);
    context.begin_span();
    if let Some(observer)=&mut observed { observer.bind_groups(&routes)?; }
    let output = declaration.kernel().trace_with_parameters(&borrowed, &input, &routes,
        declaration.as_view().tensor_partitions, &context,
        observed.as_mut().map(|observer|observer as &mut dyn eredu_nn::GroupedUnitObserver<WorkspaceTensor>))?;
    let (partial, bias) = output.into_parts();
    let mut outputs = context.metadata_vec(1 + usize::from(bias.is_some()))?;
    outputs.push(partial); outputs.extend(bias);
    observe_outputs(&outputs)?;
    observation::finish(&context,&outputs,observed,mechanism)
}
