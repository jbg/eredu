...
use eredu_nn::workspace::{WorkspaceRepresentation, WorkspaceAllocationPopulation};
...
impl ExpertReorderEnvelope {
    ...
    pub(super) fn prepare_with_scratch(
        source: &OriginalParallelSource,
        mechanism: ResidentExecutionMechanisms,
        maximum_rows: usize,
        width: i32,
        dtype: safemlx::Dtype,
        members: usize,
    ) -> Result<(Self, Vec<Option<WorkspaceAllocationPopulation>>), Error> {
        Self::prepare_kind(
            source,
            mechanism,
            maximum_rows,
    ...
    pub(super) fn prepare_join_with_scratch(
        source: &OriginalParallelSource,
        mechanism: ResidentExecutionMechanisms,
        maximum_rows: usize,
        width: i32,
    ...
    pub(super) fn prepare_empty_with_scratch(
        source: &OriginalParallelSource,
        mechanism: ResidentExecutionMechanisms,
        maximum_rows: usize,
        width: i32,
    ...
    fn prepare_kind(
        ...
        let invalid = || {
            context.metadata_error(format_args!(
                "variable block worker has no complete finite source"
            ))
        };
        let scalar = Dtype::from_native(dtype).ok_or_else(invalid)?;
        if members == 0 || width <= 0 {
            return Err(invalid());
        }
        let mut scratch = context.metadata_vec(if retain_scratch { 6 } else { 0 })?;
        let mut result = Self {
            maximum_rows,
            width,
            dtype,
        ...
        for (index, &rows) in candidates.iter().enumerate() {
            if index > 0 && candidates[..index].contains(&rows) {
                continue;
            }
            let arities = if empty || rows == 0 {
                [0, 0]
            } else {
                [1, members.min(rows)]
            };
            for (at, &arity) in arities.iter().enumerate() {
                if at > 0 && arities[..at].contains(&arity) {
                    continue;
                }
                let mut counts = context.metadata_vec(members)?;
                counts.resize(members, 0usize);
                if arity > 0 {
                    for count in &mut counts[..arity] {
                        *count = 1;
                    }
                    counts[arity - 1] = counts[arity - 1]
                        .checked_add(rows - arity)
                        .ok_or_else(invalid)?;
                }
                ...
                let existing = |rows: usize| -> Result<WorkspaceTensor, Error> {
                    ...
                    let layout = context
                        .layout(&shape, scalar.logical())?
                        .with_representation(
                            scalar
                                .floating()
                                .map(|dtype| WorkspaceRepresentation::new(dtype, true)),
                        );
                ...
                let output = if join {
                    ...
                    for &count in &counts {
                        inputs.push(existing(count)?);
                    }
                    context.begin_span();
                    blocks::join(&logical_collective::Workspace(&context), &inputs)?
                } else {
                ...
                if retain_scratch {
                    scratch.push(context.new_allocation_scratch()?);
                }
                ...
                let ordinary = OrdinaryParallelControls::numerical(recipe).and_then(|value| {
                    value.metadata(blocks::controls::<
                        logical_collective::Native<'_>,
                        std::ops::Range<usize>,
                    >(2, members)?)
                });
                result.ordinary = match (result.ordinary, ordinary) {
    ...
    fn resources(
        ...
        graph <= self.capacity.graph
            ...
            && recipe.storage.maximum_births() <= self.births
    ...
    pub(crate) fn covers_join(
        self,
        inputs: &[safemlx::Array],
        graph: usize,
        records: usize,
        backing: usize,
        recipe: &SpeculativeNumericalRecipe,
    ) -> bool {
        if !self.join || inputs.len() != self.members {
            return false;
        }
        ...
        rows.is_some_and(|rows| rows <= self.maximum_rows)
            && self.resources(graph, records, backing, recipe)
    }
}
