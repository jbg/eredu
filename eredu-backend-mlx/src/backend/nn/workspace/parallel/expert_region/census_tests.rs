...
use eredu_nn::workspace::{
    HostMetadataAccount, HostMetadataFundingError, WorkspaceExpertKernel, WorkspaceFloatingType,
    WorkspaceRepresentation,
};
use eredu_nn::{
    GatedProductGroupLayout, GatedProductPolicy, GroupedGatedProductSpec, GroupedLinearActivation,
    GroupedLinearSpec, GroupedProjectionSpec, GroupedRelu2Spec, LinearFormatSpec, ParameterSpec,
};
...
fn components(recipe: SpeculativeNumericalRecipe) -> [u64; 6] {
    [
        recipe.graph_capacity as u64,
        recipe.record_capacity as u64,
        recipe.storage.mutable_bytes(),
        recipe.storage.maximum_births() as u64,
        recipe.kernels as u64,
        recipe.controls,
    ]
}
#[test]
fn expert_branch_census_bounds_actual_recipes_across_chunk_and_singleton_transitions() {
    ...
    let mechanism =
        ResidentExecutionMechanisms::Metal(MlxMetalWorkspaceMechanisms::current_host().unwrap());
    for kernel in [
        ...
        let view = WorkspaceExpertRegionView {
            addressable: None,
            bank: 0,
            unit: 0,
            prefill: true,
        ...
        let mut inputs = vec![
            floating(&[65, 32]),
            context.layout(&[65, 1], WorkspaceDtype::Int32).unwrap(),
            floating(&[65, 1]),
            floating(&[65, 1]),
        ];
        match kernel {
            WorkspaceExpertKernel::Gated(_) => {
                inputs.push(floating(&[2, 64, 32]));
                inputs.push(floating(&[2, 32, 32]));
            }
        ...
        let actual = (0..=130)
            .map(|rows| {
                components(
                    local_trace(&declaration, &inputs, rows, mechanism, &funding)
                        .unwrap_or_else(|cause| panic!("{kernel:?}, actual rows={rows}: {cause}")),
                )
            })
            .collect::<Vec<_>>();
        for maximum in [
            0, 1, 2, 31, 32, 33, 63, 64, 65, 66, 95, 96, 97, 127, 128, 129, 130,
        ] {
            let mut ceiling = actual[0];
            for rows in super::super::super::grouped::expert_row_candidates(kernel, maximum) {
                for (bound, value) in ceiling.iter_mut().zip(actual[rows]) {
...
fn expert_observed_child_keeps_sparse_sources_inside_its_native_frontier() {
    ...
    for rows in [0usize, 1, 65] {
        let plain = local_trace_with_outputs(
            &declaration,
            &inputs,
            rows,
            mechanism,
        ...
        .unwrap();
        let observed = local_trace_with_outputs(
            &declaration,
            &inputs,
            rows,
            mechanism,
        ...
        let schedule = mechanism
            .grouped_observation_schedule(declaration.kernel(), rows as u32)
            .unwrap()
            .unwrap();
        let envelope = eredu_nn::workspace::WorkspaceGroupedObservationEnvelope::new(
            rows, 1, 32, schedule, false,
        )
        .unwrap();
        let added = 10 * envelope.maximum_callbacks();
        assert_eq!(
            observed.completion.nested_completions,
            plain.completion.nested_completions + added
        );
        assert_eq!(
            observed
                .completion
        ...
        if rows > 0 {
            assert!(observed.controls > plain.controls);
        }
    }
}
