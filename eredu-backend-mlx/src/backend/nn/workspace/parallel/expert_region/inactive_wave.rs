...
use eredu_nn::workspace::{
    WorkspaceExpertInactiveWave, WorkspaceExpertTransfer, WorkspaceFloatingType,
};
...
impl ExpertInactiveWaveQuote {
    pub(crate) fn prepare(
        source: &OriginalParallelSource,
        declaration: WorkspaceExpertInactiveWave,
        ...
        let invalid = || {
            context.metadata_error(format_args!(
                "inactive expert wave has an incomplete retained source"
            ))
        };
        ...
        let counts = ExpertCountQuote::prepare_for(
            source,
            mechanism,
            declaration.provider.expert_group,
            declaration.rank,
            declaration.peers,
        )?;
        let provider =
            ExpertProviderQuote::prepare_for(source, mechanism, declaration.provider.groups())?;
        let geometry = ExpertTransferGeometry {
            group: declaration.provider.expert_group,
            rank: declaration.rank,
            peers: declaration.peers,
            selected_rows: declaration.selected_rows,
            input_width: declaration.input_width,
            output_width: declaration.output_width,
            input_dtype: dtype,
            scores_dtype: dtype,
            coefficients_dtype: dtype,
            output_dtype: dtype,
            bias_dtype: declaration
                .transfers
                .iter()
                .any(|v| v == WorkspaceExpertTransfer::ReverseBias)
                .then_some(dtype),
            transfers: declaration.transfers,
        };
        let transport = ExpertTransportQuote::prepare_for(source, mechanism, geometry)?;
        let mut bytes = counts
            .backing
            .checked_add(provider.backing)
            .ok_or_else(invalid)?;
        let mut births = counts
            .births
            .checked_add(provider.births)
            .ok_or_else(invalid)?;
        ...
        let mut extra_parents = context.metadata_vec(
            declaration
                .transfers
                .len()
        ...
        for payload in declaration.transfers.iter() {
            let width = match payload {
                ...
                WorkspaceExpertTransfer::ForwardScores
                | WorkspaceExpertTransfer::ForwardCoefficients => 1,
                WorkspaceExpertTransfer::ReverseOutput | WorkspaceExpertTransfer::ReverseBias => {
                    declaration.output_width
                }
                _ => continue,
            };
            let parent =
            ...
            parent.begin_span();
            let prototype =
                eredu_nn::workspace::WorkspaceLayoutView::new(&[], WorkspaceDtype::Float32)?
        ...
        let parent_completions = declaration
            .transfers
            .len()
            .checked_add(metadata)
            .ok_or_else(invalid)?;
        let child_scratch =
            super::super::scratch::itinerary(&context, mechanism, &counts, &transport, &provider)?;
        if let Some(source) = &child_scratch {
            bytes = source.backing_bytes().ok_or_else(invalid)?;
        }
        let aggregate = ExpertRegionAggregate {
            child_bytes: bytes,
            child_scratch,
            child_births: births,
            host_bytes: 0,
            parent_completions,
            indexed_parent: false,
            empty_slices: metadata,
            empty_slice,
            extra_parents,
