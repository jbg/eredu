...
struct CountStep {
    value: usize,
    step: usize,
    source: RetainedLogicalCollective,
}
enum CountItinerary {
    Physical,
...
pub(crate) struct ExpertCountQuote {
    ...
    pub(crate) native_scratch: Option<eredu_nn::workspace::WorkspaceAllocationPopulation>,
...
impl ExpertCountQuote {
    ...
    pub(super) fn prepare_for(
        ...
        let invalid = || {
            context.metadata_error(format_args!(
                "expert count itinerary differs from its retained group"
            ))
        };
        let actual = source
        ...
        if descriptor.members().len() != peers
            || descriptor.local_index() != Some(rank)
            || group.rank() != rank
        {
            return Err(invalid());
        }
        ...
        let mut add_native =
            |native: &safemlx::distributed::Group, words: usize| -> Result<(), Error> {
                ...
                let layout = native
                    .cpu_layout_storage(
                        &shape,
                        safemlx::Dtype::Int32,
                        safemlx::distributed::GroupWorkerOperation::Gather,
                    )
                    .map_err(|cause| context.metadata_source(cause))?;
                ...
                native_scratch = Some(super::super::scratch::native_cpu(
                    ...
                    mechanism,
                    bytes,
                    layout.evaluation().logical_backing_population().0,
                )?);
                backing = backing
                    .checked_add(u64::try_from(bytes).map_err(|_| invalid())?)
                    .ok_or_else(invalid)?;
                births = births
                    .checked_add(layout.evaluation().logical_backing_population().0)
                    .ok_or_else(invalid)?;
                context.charge_metadata(
                    layout
        ...
        let (itinerary, has_world) = if !group.is_logical() {
            ...
            let layout = context.layout(
                &[i32::try_from(words).map_err(|_| invalid())?],
            ...
            let (itinerary, packed) = if let Some(plan) = plan {
                ...
                for (value, route) in plan.values().enumerate() {
                    for step in 0..route.steps() {
                        ...
                        let quote = LogicalCollectiveQuote::prepare_routed_peer(
                            source,
                            order,
                            value,
                            step,
                        ...
                        steps.push(CountStep {
                            value,
                            step,
                            source: RetainedLogicalCollective::retain_prepared(source, quote)
                                .map_err(|cause| source.neural_error(cause))?,
                        });
                    }
                }
                ...
                let quote = LogicalCollectiveQuote::prepare_group(
                    source,
                    order,
                ...
                (
                    CountItinerary::Logical(
                        RetainedLogicalCollective::retain_prepared(source, quote)
                            .map_err(|cause| source.neural_error(cause))?,
                    ),
                    packed,
                )
            ...
            if extra_world {
                ...
                let persistent = actual
                    ...
                    .map_err(|cause| source.neural_error(cause))?;
                if persistent.native().has_unqualified_storage()
                    || !persistent.native().is_for(actual.world().native_group())
                    || !persistent.source().same_source(actual.source())
                {
                    return Err(invalid());
                }
        ...
        let mut add = |source: &RetainedLogicalCollective| -> Result<(), Error> {
            ...
            backing = backing
                .checked_add(quote.output)
                .and_then(|n| n.checked_add(quote.scratch))
                .ok_or_else(invalid)?;
            births = births
                .checked_add(quote.maximum_backing_births().ok_or_else(invalid)?)
                .ok_or_else(invalid)?;
    ...
    pub(crate) fn allocation_scratch(
        ...
        mechanism: ResidentExecutionMechanisms,
    ) -> Result<Option<eredu_nn::workspace::WorkspaceAllocationPopulation>, Error> {
        let count = self
            .logical_sources()
            .count()
            .checked_add(usize::from(self.native_scratch.is_some()))
            .ok_or(WorkspaceMetadataError::Overflow)?;
        let mut populations = context.metadata_vec(count)?;
        if let Some(native) = &self.native_scratch {
            populations.push(native.clone());
        }
        for child in self.logical_sources() {
            let Some(child) = child.allocation_scratch(context, mechanism)? else {
                return Ok(None);
            };
            populations.push(child);
        }
    ...
    pub(crate) fn logical_sources(&self) -> impl Iterator<Item = &LogicalCollectiveQuote> {
        let single = match &self.itinerary {
            CountItinerary::Logical(source) => Some(source.value()),
            _ => None,
        };
        let steps = match &self.itinerary {
            CountItinerary::Routed(steps) => steps.as_slice(),
            _ => &[],
        };
        single
            .into_iter()
            .chain(steps.iter().map(|step| step.source.value()))
    }
    ...
    ) -> Option<RetainedLogicalCollective> {
        match (&self.itinerary, selected) {
            (CountItinerary::Logical(source), None) => Some(source.clone()),
            (CountItinerary::Routed(steps), Some((value, step))) => steps
                .iter()
                .find(|row| row.value == value && row.step == step)
