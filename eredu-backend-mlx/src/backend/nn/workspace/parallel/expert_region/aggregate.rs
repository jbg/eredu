...
impl ExpertRegionAggregate {
    pub(super) fn prepare(local:&ExpertLocalQuote)->Result<Self,Error> {
        ...
        context.charge_metadata(size_of::<(Self,Result<Self,Error>,WorkspaceContext,[i32;1],usize,u64)>())?;
        let invalid=||context.metadata_error(format_args!("expert region requires its complete count/parent/child source"));
        ...
        for (kind,count) in view.movement.counts().into_iter().enumerate() {
            ...
            bytes=bytes.checked_add(u64::try_from(backing.checked_mul(count).ok_or_else(invalid)?).map_err(|_|invalid())?).ok_or_else(invalid)?;
            births=births.checked_add(created.checked_mul(count).ok_or_else(invalid)?).ok_or_else(invalid)?;
        }
        ...
        let metadata=view.transfers.iter().filter(|payload|matches!(payload,
        ...
        let empty_slices=view.movement.row_gathers.checked_add(view.movement.scalar_gathers)
            .and_then(|n|n.checked_add(view.movement.indexed_adds)).and_then(|n|n.checked_add(metadata))
            .and_then(|n|n.checked_add(1)).ok_or_else(invalid)?;
        let parent=WorkspaceContext::new_with_metadata_funding(local.mechanism,source.funding().clone())?;
        ...
        for layout in &local.outputs {
            let dtype=crate::backend::nn::workspace::byte_view::Dtype::from_layout(layout.as_view()).ok_or_else(invalid)?;
            let shape=[view.source_rows.max(1),usize::try_from(view.kernel.dimensions().1).map_err(|_|invalid())?];
            ...
            let plan=runtime.zeros(dtype.native(),&shape).map_err(|cause|context.metadata_source(cause))?;
        ...
        let host_bytes=if view.kernel.reduction()==eredu_nn::GroupReduction::SequentialGroupOrder {
