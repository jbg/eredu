...
enum Projection {
    Tied(PhysicalEmbedding),
    Untied(PhysicalLinear),
}
impl Projection {
    fn new(
        tied: bool,
        ...
        if tied {
            let mut module =
                unloaded_embedding(VOCAB, WIDTH, format.weight_quantization(), stream).unwrap();
            match &mut module {
                ...
                PhysicalEmbedding::Quantized(module) => {
                    ...
                    module.scales.value = scales.cloned();
                    module.biases.value = biases.cloned();
                }
            }
            Self::Tied(module)
        } else {
            let mut module = PhysicalLinear::unloaded(WIDTH, VOCAB, false, format, stream).unwrap();
            module.weight.value = weight.clone();
            module.scales.value = scales.cloned();
            module.biases.value = biases.cloned();
            Self::Untied(module)
        }
...
fn native_selected_readout_preserves_half_and_affine_tied_untied_projection() {
    ...
    let hidden_layout =
        WorkspaceLayout::new(&[2, POSITIONS, WIDTH], WorkspaceDtype::Float32).unwrap();
    ...
    for dtype in [Dtype::Float32, Dtype::Float16, Dtype::Bfloat16] {
        ...
        for affine in [false, true] {
            ...
            let (weight, scales, biases) = if affine {
                let encoded = safemlx::ops::quantize_with_mode(
                    &dense,
                    GROUP,
                    BITS,
                ...
                let biases = encoded
                    .biases
                    .expect("actual affine bias companion")
                    .as_dtype(dtype, &stream)
                    .unwrap();
                assert_eq!(encoded.weight.dtype(), Dtype::Uint32);
                assert_eq!(encoded.weight.shape(), [VOCAB, WIDTH * BITS / 32]);
                assert_eq!(scales.shape(), [VOCAB, WIDTH / GROUP]);
                assert_eq!(biases.shape(), scales.shape());
                assert_eq!(scales.dtype(), dtype);
                assert_eq!(biases.dtype(), dtype);
                (encoded.weight, Some(scales), Some(biases))
            } else {
                (dense.clone(), None, None)
            };
            for tied in [false, true] {
                ...
                for strided in [false, true] {
                    let hidden =
                        MlxTensor::from_array(floating(&hidden_layout, 0, dtype, strided, &stream));
                    ...
                    let reference_last = reference
                        .narrow_axis(1, POSITIONS - 1, POSITIONS, &stream)
                        .unwrap();
                    ...
                    .is_none());
                    let sequence =
                        execute_readout(&hidden, OutputDemand::Sequence, 1, &stream, &mut project)
                            .unwrap()
                            .unwrap();
                    ...
                    assert_eq!(sequence.as_array().dtype(), dtype);
                    assert_eq!(last.as_array().dtype(), dtype);
                    compare(&values(&sequence, &stream), &reference_values, dtype);
                    compare(
                        &values(&last, &stream),
                        &values(&reference_last, &stream),
                        dtype,
                    );
...
fn assert_cold_projection_scalar(
    ...
    let mut projection = ExistingArrayProjection::with_source_count(
        &context,
        3 + usize::from(scales.is_some()) + usize::from(biases.is_some()),
    )
    .unwrap();
    let input = projection.project(input.as_array()).unwrap();
    let weight_spec = ParameterSpec::trainable("projection.weight").unwrap();
    let mut parameters = vec![WorkspaceParameterRepresentation::new(
        weight_spec.id.clone(),
    ...
    let format = if let (Some(scales), Some(biases)) = (scales, biases) {
        ...
        parameters.push(WorkspaceParameterRepresentation::new(
            scale.id.clone(),
        ...
        LinearFormatSpec::unscaled(encoding).unwrap()
    };
