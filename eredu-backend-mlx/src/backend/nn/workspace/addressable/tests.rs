...
use crate::backend::runtime::residency::parameter_bank::{
    AddressableParameterBank, IndexedBankSource, ParameterBankEntry, ParameterBankKey,
    SelectedAddressableEntries, SharedAddressableParameterBank,
};
use eredu_checkpoint::store::{SafetensorsWeightStore, TensorSelection};
use eredu_core::residency::OffloadConfig;
use eredu_nn::workspace::{
    HostMetadataAccount, HostMetadataFundingError, WorkspaceExpertRegionView,
};
use eredu_nn::{
...
use eredu_runtime::{
    AddressableBankDistribution, AddressableBankMemberPlacement, ExecutionGroupId, OffloadUnit,
    ParameterBankLoadOptions, WeightBinding,
};
...
fn check_local_quote(gated: bool, maximum: usize, chunk_rows: usize, device_type: DeviceType) {
    ...
    let fields = if gated {
        vec![
            ("gate_up_proj", vec![1, 8, 4]),
            ("down_proj", vec![1, 4, 4]),
        ]
    } else {
        vec![("weight", vec![1, 4, 4])]
    };
    let values = fields
        .iter()
        .map(|(_, shape)| {
            (0..shape.iter().product::<usize>())
                .flat_map(|n| ((n as f32 + 1.0) / 17.0).to_le_bytes())
                .collect::<Vec<_>>()
        })
        .collect::<Vec<_>>();
    serialize_to_file(
    ...
    let mut selected = SelectedAddressableEntries {
    ...
    for (ordinal, member) in [0, 2, 5].into_iter().enumerate() {
        ...
        let bindings = fields
            ...
            .map(|((name, _), bytes)| {
                selected
                    .parameter_targets
                    .insert((key, (*name).into()), (*name).into());
                WeightBinding::new(
                    *name,
                    format!("member.{ordinal}.{name}"),
        ...
        selected.entries.push(
            ParameterBankEntry::new(
                key,
                OffloadUnit::new(key.unit_id(), bindings).unwrap(),
                member_bytes,
            )
            .unwrap(),
        );
        selected.expected_bytes.insert(key, member_bytes);
        selected.placements.insert(
            key,
            AddressableBankMemberPlacement::new(
                ExecutionGroupId::new("layers").unwrap(),
                0,
                "layers.0",
                AddressableBankDistribution::ExpertParallel,
            )
            .unwrap(),
        );
    }
    ...
    let options = ParameterBankLoadOptions::new(
        OffloadConfig::new(Some(scratch), Some(scratch), 1).unwrap(),
        scratch,
        bulk_target,
    )
    .unwrap();
    ...
    let prepared = AddressableParameterBank::prepare_selected_manager(
        source.clone(),
        selected.clone(),
        options,
        &host,
    ...
    .unwrap()
    .expect("actual prepared indexed manager");
    let bank = AddressableParameterBank::new_selected_shared_with_manager(
        source,
        selected,
        options,
        host,
    ...
    let sources = AddressableSources::new(
        [(0, &source)].into_iter(),
        mechanism,
        &runtime,
        Some(&pool),
    ...
    let kernel = if gated {
        WorkspaceGroupedBank::GatedProduct(
    ...
    let kernel_view = match &kernel {
        WorkspaceGroupedBank::GatedProduct(value) => {
            eredu_nn::workspace::WorkspaceExpertKernel::Gated(value)
        }
        WorkspaceGroupedBank::Linear(value) => {
            eredu_nn::workspace::WorkspaceExpertKernel::Linear(value)
        }
        _ => unreachable!(),
    };
    ...
    .unwrap()
    .workspace_source();
    let local = WorkspaceAddressableRegionView {
        owner_group: "layers",
        bank: 0,
        unit: 0,
        prefill: true,
        chunks,
        local_members: Some(&[0, 2, 5]),
        kernel: kernel_view,
        tensor_partitions: None,
        compact_scratch_bytes: scratch,
        bulk_target_bytes: bulk_target,
        callback_control_bytes: 0,
    };
    let region = WorkspaceExpertRegionView {
        bank: 0,
        unit: 0,
        prefill: true,
        ...
        rank: 0,
        peers: 2,
        source_rows: maximum / 2,
        routes_per_row: 1,
        owners: &[0, 1, 0, 1, 1, 0],
        owner_local: &[0, 0, 1, 1, 2, 2],
        kernel: local.kernel,
        tensor_partitions: None,
    ...
    let operation = WorkspaceOperation {
        kind: WorkspaceOperationKind::ExpertRegion(Box::new(region)),
        inputs: vec![
            floating(&[(maximum / 2) as i32, 4]),
            context
                .layout(&[(maximum / 2) as i32, 1], WorkspaceDtype::Int32)
                .unwrap(),
            floating(&[(maximum / 2) as i32, 1]),
            floating(&[(maximum / 2) as i32, 1]),
        ],
        outputs: vec![floating(&[(maximum / 2) as i32, 4])],
    };
    ...
    let declaration = quote.declaration.as_view().retain(&context).unwrap();
    let direct = WorkspaceOperationView {
        kind: WorkspaceOperationKindView::AddressableRegion(&declaration),
        inputs: WorkspaceLayoutList::Owned(&quote.inputs),
        outputs: WorkspaceLayoutList::Owned(&quote.outputs),
    };
    assert!(safemlx::OriginalScopeObserver::try_current()
        .unwrap()
        .is_none());
    let ordinary = OrdinaryAddressableSources::new(sources.clone()).unwrap();
    let program = ordinary.program(std::slice::from_ref(&operation)).unwrap();
    use crate::backend::runtime::residency::parameter_bank::OrdinaryIndexedRequestProgram;
    assert_eq!(program.len(), 1);
    let occurrence = program.occurrence(0).unwrap();
    assert!(occurrence.local);
    assert!(occurrence.identity.source().same_binding(&source));
    assert_eq!(occurrence.declaration.chunks.rows, maximum);
    assert_eq!(occurrence.declaration.local_members, Some(&[0, 2, 5][..]));
    assert!(program.occurrence(1).is_none());
    assert!(safemlx::OriginalScopeObserver::try_current()
        .unwrap()
        .is_none());
    let baseline =
        AddressableEquationSource::prepare(&source, direct, mechanism, &funding, |_, _| Ok(0))
            .unwrap();
    let mut compact_visits = 0usize;
    let with_compact =
        AddressableEquationSource::prepare(&source, direct, mechanism, &funding, |identity, _| {
            assert!(identity.source().same_binding(&source));
            assert_eq!(identity.first().unwrap().total_rows(), maximum);
            compact_visits += 1;
            Ok(7)
        })
        .unwrap();
    assert!(compact_visits > 0);
    assert_eq!(with_compact.outputs, baseline.outputs);
    assert_eq!(
        with_compact.host_bytes - baseline.host_bytes,
        7 * u64::try_from(maximum.div_ceil(chunk_rows)).unwrap(),
        "compact alternatives contribute once per actual full or tail invocation",
    );
    assert!(safemlx::OriginalScopeObserver::try_current()
        .unwrap()
        .is_none());
    assert_eq!(
        quote.constructor_facts.maximum_partitions(),
        maximum.div_ceil(chunk_rows)
    );
    ...
    for rows in 1..=maximum {
        ...
        assert_eq!(
            cold_account.0.load(Ordering::SeqCst),
            cold_before,
            "runtime specialization must not debit the retained cold source"
        );
        let execution_debit = execution_account.0.load(Ordering::SeqCst) - execution_before;
        assert!(
            execution_debit > 0 && execution_debit <= quote.specialization_control_bytes().unwrap(),
            "the accepted caller pays every specialization owner within its quoted census"
        );
        ...
        let actual = sources
            .quote(WorkspaceOperationView {
                kind: WorkspaceOperationKindView::AddressableRegion(&declaration),
                inputs: WorkspaceLayoutList::Owned(&specialized.inputs),
                outputs: WorkspaceLayoutList::Owned(&specialized.outputs),
            })
            .unwrap();
        let equation = actual.equation;
        let completed = actual.numerical;
        assert!(
            completed.completion.graph.primitives() > equation.completion.graph.primitives(),
            "rows={rows}: actual copies add constructors to the shared graph"
        );
        assert!(
            completed.graph_capacity > equation.graph_capacity,
            "rows={rows}: copy/aggregate graph producers survive"
        );
        assert!(
            completed.record_capacity > equation.record_capacity,
            "rows={rows}: copy/aggregate records survive"
        );
        assert!(
            completed.completion.nested_completions > equation.completion.nested_completions,
            "rows={rows}: independent source completion frontiers survive"
        );
        assert!(
            completed.storage.maximum_births() > equation.storage.maximum_births(),
            "rows={rows}: copy destinations stay charged"
        );
        assert!(
            completed.storage.mutable_bytes() > equation.storage.mutable_bytes(),
            "rows={rows}: copied physical storage stays charged"
        );
        if device_type == DeviceType::Cpu {
            let tape = completed.completion.traversal.limits().tape_entries;
            assert_eq!(
                tape,
                equation.completion.traversal.limits().tape_entries,
                "rows={rows}: separate copy tasks never become equation tape entries"
            );
        } else {
            assert!(
                completed.kernels > equation.kernels,
                "rows={rows}: source copy and aggregate kernels survive"
            );
        }
        assert!(
            actual.capacity.graph <= quote.native_capacity().graph,
            "rows={rows}"
        );
        assert!(
            actual.capacity.records <= quote.native_capacity().records,
            "rows={rows}"
        );
        assert!(
            actual.capacity.backing <= quote.native_capacity().backing,
            "rows={rows}"
        );
        assert!(
            actual.numerical.controls <= envelope.controls,
            "rows={rows}"
        );
        assert!(actual.numerical.kernels <= envelope.kernels, "rows={rows}");
        assert!(
            actual.numerical.completion.nested_completions
                <= envelope.completion.nested_completions,
            "rows={rows}"
        );
        assert!(
            actual.numerical.completion.traversal.limits().captures
                <= envelope.completion.traversal.limits().captures,
            "rows={rows}"
        );
        assert!(
            actual.numerical.storage.maximum_births() <= envelope.storage.maximum_births(),
            "rows={rows}"
        );
        assert!(actual.host_bytes <= quote.host_capacity(), "rows={rows}");
    }
    assert!(sources
        .local_quote(operation.as_view())
        .unwrap()
        .same_quote(&quote));
    let mut wrong = local;
    wrong.bulk_target_bytes += 1;
    assert!(
        source.first_census(wrong, &funding).is_err(),
        "selected options stay authenticated"
    );
}
