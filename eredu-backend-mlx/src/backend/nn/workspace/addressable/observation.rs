...
pub(super) fn source_groups(
    ...
    let value = value.reshape(
        &[
            i32::try_from(source.chunks.rows).map_err(|_| WorkspaceMetadataError::Overflow)?,
            i32::try_from(source.chunks.routes).map_err(|_| WorkspaceMetadataError::Overflow)?,
        ],
        &context,
    )?;
    Ok(owner
...
pub(super) fn inspect(
    ...
    context.charge_metadata(size_of::<(
        ...
        WorkspaceAddressableObservationLayout,
        Vec<WorkspaceLayout>,
        WorkspaceLayout,
        Result<WorkspaceAddressableObservationLayout, Error>,
    )>())?;
    ...
    for member in 1..source.chunks.members {
        for field in 0..parameters.fields {
            let a = &parameters.rows[member * parameters.fields + field].layout;
            let b = &parameters.rows[field].layout;
            if a != b || a.representation() != b.representation() {
    ...
    let local =
        super::quote::normalized_inputs(source, inputs, rows, mechanism, funding, &context)?;
    let groups = source_groups(source, inputs, mechanism, funding, &context)?;
    let mut fields = context.metadata_vec(
        members
            .checked_mul(parameters.fields)
            .ok_or(WorkspaceMetadataError::Overflow)?,
    )?;
    for row in &parameters.rows[..members * parameters.fields] {
        fields.push(
            context
    ...
    let child = AddressableChildSource::prepare_with_observation(
        ...
        mechanism,
        funding,
        Some(WorkspaceAddressableObservationSource::default()),
        Some(&groups),
    )?;
    let representation = child
        .unit_representation
        .ok_or(WorkspaceMetadataError::Unqualified)?;
    let compact = source.kernel.compact(
        i32::try_from(members).map_err(|_| WorkspaceMetadataError::Overflow)?,
        &context,
    )?;
    let schedule = mechanism
        .grouped_observation_schedule(
            &compact,
            u32::try_from(rows).map_err(|_| WorkspaceMetadataError::Overflow)?,
        )?
        .ok_or(WorkspaceMetadataError::Unqualified)?;
    let width = match source.kernel {
    ...
    let envelope = WorkspaceGroupedObservationEnvelope::new(
        rows,
        source.chunks.routes,
        usize::try_from(width).map_err(|_| WorkspaceMetadataError::Unqualified)?,
        schedule,
        false,
    )?;
    let selected = rows
        .checked_mul(source.chunks.routes)
        .ok_or(WorkspaceMetadataError::Overflow)?;
    Ok(WorkspaceAddressableObservationLayout {
        envelope,
        units: context
            .layout(
                &[
                    i32::try_from(selected).map_err(|_| WorkspaceMetadataError::Overflow)?,
                    width,
                ],
...
mod tests {
    ...
    fn projection(name: &str) -> eredu_nn::GroupedProjectionSpec {
        eredu_nn::GroupedProjectionSpec::new(
            eredu_nn::ParameterSpec::trainable(name).unwrap(),
            None,
    ...
    fn addressable_child_keeps_full_route_source_across_compact_cardinalities() {
        ...
        let mechanism = ResidentExecutionMechanisms::Metal(
        ...
        let spec = eredu_nn::GroupedGatedProductSpec::new(
            2,
            32,
            32,
            32,
        ...
        let source = WorkspaceAddressableRegionView {
            ...
            local_members: None,
            kernel: WorkspaceExpertKernel::Gated(&spec),
            tensor_partitions: None,
            compact_scratch_bytes: 0,
            bulk_target_bytes: 0,
        ...
        let per_callback =
            crate::backend::array_copy::CaptureNativePopulation::routed_partition().unwrap();
        let retention = WorkspaceGroupedSourceRetention {
            partition_copies: 1,
            replica_settlements: 0,
            callback_control_bytes: per_callback.controls,
        };
        for rows in [1, 65] {
            for members in [1, 2] {
                let inputs = [
                    floating(&[rows, 32]),
                    context.layout(&[rows, 2], WorkspaceDtype::Int32).unwrap(),
                    floating(&[rows, 2]),
                    floating(&[rows, 2]),
                ];
                let mut fields = Vec::new();
                for _ in 0..members {
                    fields.extend([floating(&[1, 64, 32]), floating(&[1, 32, 32])]);
                }
                let baseline = AddressableChildSource::prepare_with_observation(
                    source,
                    &inputs,
                    members,
                    &fields,
                ...
                .unwrap();
                let observed = AddressableChildSource::prepare_with_observation(
                    source,
                    &inputs,
                    members,
                    &fields,
                    mechanism,
                    &funding,
                    Some(WorkspaceAddressableObservationSource {
                        before: retention,
                        after: retention,
                        unit_dtype: baseline.unit_representation.map(|v| v.dtype()),
                    }),
                    Some(&full),
                )
                .unwrap();
                ...
                let schedule = mechanism
                    .grouped_observation_schedule(&bank, rows as u32)
                    .unwrap()
                    .unwrap();
                let envelope =
                    WorkspaceGroupedObservationEnvelope::new(rows as usize, 2, 32, schedule, false)
                        .unwrap();
                assert_eq!(
                    observed.capture.retained_roots,
                    10 * envelope.maximum_callbacks()
                );
                assert_eq!(
                    observed.capture.publications,
                    observed.capture.retained_roots
                );
                ...
                .unwrap();
                let base = SpeculativeNumericalRecipe::inspect_owned_child_with_capture(
                    &baseline.report,
                    1,
                    mechanism,
                    &context,
                ...
                assert_eq!(
                    ...
                    base.completion.nested_completions + observed.capture.completions
                );
                let truncated = context.layout(&[rows, 2], WorkspaceDtype::Int32).unwrap();
                assert!(AddressableChildSource::prepare_with_observation(
                    source,
                    &inputs,
                    members,
                    &fields,
    ...
    fn addressable_sequential_child_qualifies_the_actual_compact_reduction() {
        ...
        .unwrap()
        .with_reduction(eredu_nn::GroupReduction::SequentialGroupOrder);
        ...
        for (rows, routes, members) in [(1, 1, 1usize), (2, 2, 1), (65, 2, 2)] {
            let source = WorkspaceAddressableRegionView {
                ...
                chunks: WorkspaceAddressableChunkPlan {
                    rows: rows as usize,
                    chunk_rows: rows as usize,
                    routes: routes as usize,
                    members: 2,
                },
                local_members: None,
                kernel: WorkspaceExpertKernel::Gated(&spec),
            ...
            let groups = context
                .layout(&[rows, routes], WorkspaceDtype::Int32)
                .unwrap();
            let inputs = [
                floating(&[rows, 16]),
                groups.clone(),
                floating(&[rows, routes]),
                floating(&[rows, routes]),
            ];
            let mut parameters = Vec::new();
            for _ in 0..members {
                parameters.extend([floating(&[1, 12, 16]), floating(&[1, 16, 6])]);
            }
            for observation in [None, Some(WorkspaceAddressableObservationSource::default())] {
                ...
                .unwrap();
                assert!(
                    recipe.graph_capacity > 0 && recipe.record_capacity > 0 && recipe.kernels > 0
                );
