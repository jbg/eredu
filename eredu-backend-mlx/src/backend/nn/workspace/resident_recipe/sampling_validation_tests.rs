...
fn terminal_resume_recipe_has_known_empty_equations_and_no_prediction_completion() {
    ...
    use eredu_runtime::working_memory::{
        quote_inference_workspace_with_context, quote_sampling_workspace_with_observer,
    };
    ...
    let equations = quote_inference_workspace_with_context(
        ...
        |_| -> Result<WorkspaceTraceReport, Error> {
            panic!("terminal restore invoked model equations")
        },
    )
    .unwrap();
    ...
    let sampling = quote_sampling_workspace_with_observer(
        &sampler,
        0.0,
        None,
        &layout,
    ...
    let copy = crate::backend::array_copy::OriginalCopyLayoutBuilder::new()
        .finish_resume_completed_input(0)
        .unwrap();
    assert_eq!(
        (copy.births(), copy.roots(), copy.logical_bytes()),
        (0, 0, 0)
    );
    recipe.bind_resume_copy(copy).unwrap();
    assert!(recipe
        .resume_graph_storage_requirement()
        .unwrap()
        .full_capacity
        .is_some());
    assert!(recipe
