...
use eredu_nn::{GatedProductGroupLayout,GatedProductPolicy,GroupSelection,
    ...
    GroupedProjectionSpec,GroupedUnitBatch,GroupedUnitObserver,LinearFormatSpec,
    ParameterSpec,Tensor,TensorParallelGroupedGatedProductOperator};
struct Observe;
impl GroupedUnitObserver<WorkspaceTensor> for Observe {
    fn observe(&mut self,_:&GroupedUnitBatch<'_,WorkspaceTensor>)->Result<(),Error>{Ok(())}
}
fn specification(group_size:i32)->GroupedGatedProductSpec {
    let projection=|name:&str|GroupedProjectionSpec::new(
        ...
        LinearFormatSpec::affine(eredu_checkpoint::LinearFormat::Affine(
            ...
            ParameterSpec::trainable(format!("{name}.biases")).unwrap()).unwrap()).unwrap();
    GroupedGatedProductSpec::new(2,64,64,64,GatedProductPolicy::ordinary_silu(),
...
fn affine_grouped_sources_keep_companions_across_selected_gather_and_observed_chunks() {
    ...
    for group_size in [16,64] {
        for (positions,observed) in [(2,false),(65,false),(2,true),(65,true)] {
            ...
            let mut module=WorkspaceBackend::grouped_gated_product(specification(group_size),&context).unwrap();
            ...
            let outputs=if observed {
                vec![module.forward_grouped_with_unit_observer(&input,&selection,&context,Some(&mut Observe)).unwrap()]
            } else {
                let (value,bias)=module.forward_grouped_tensor_parallel(&input,&selection,2,&context).unwrap().into_parts();
                vec![value,bias.expect("actual separate TP output bias")]
            };
            let report=context.report(&outputs).unwrap();
            let operations=report.operations.iter().filter(|operation|matches!(operation.kind,WorkspaceOperationKind::Grouped{..}));
            ...
            for operation in operations {
                ...
                for changed in [WorkspaceLayout::new(&[2,128,1],WorkspaceDtype::Uint32).unwrap(),
                    WorkspaceLayout::new(&[2,128,7],WorkspaceDtype::Float32).unwrap()] {
                    let mut inputs=operation.inputs.clone();inputs[6]=changed;
                    assert!(super::super::sources::counts(WorkspaceOperationView{
                        inputs:WorkspaceLayoutList::Owned(&inputs),..view}).is_none());
                }
            }
            ...
            let reduced=recorder.reduce_trace(&report,None,0,outputs.len()).unwrap();
