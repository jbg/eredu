...
impl ResidentRecipeRecorder {
    ...
    pub(super) fn reduce_cpu_trace(
        ...
        self.reduce_cpu_trace_with_parallel(
            report,
            input_operation,
            retained_roots,
            output_roots,
    ...
    pub(super) fn reduce_cpu_trace_with_parallel(
            ...
            .filter(|op| {
                matches!(
                    ...
                    WorkspaceOperationKind::ValueCompletion
                        ...
                        | WorkspaceOperationKind::AddressableRegion(_)
                )
            })
        ...
        for (index, operation) in report.operations.iter().enumerate() {
            if matches!(
                ...
                // The actual control owner quotes its native child separately.
            ...
            if matches!(operation.kind, WorkspaceOperationKind::AddressableRegion(_)) {
                if let Some(source) = self.ordinary_addressable_sources.as_ref() {
                    let quote = source.equation(operation.as_view())?;
                    let storage = quote.storage().ok_or_else(|| {
                        self.metadata_error(
                            quote
                                .missing_source()
                                .unwrap_or("ordinary addressable payload source is incomplete"),
                        )
                    })?;
                    let (child_validations, producers) =
                        quote.validation_population().ok_or_else(|| {
                            self.metadata_error("ordinary indexed validation source is incomplete")
                        })?;
                    validations = validations
                        .checked_add(child_validations)
                        .ok_or_else(overflow)?;
                    validation_bytes = validation_bytes
                        .checked_add(producers.mutable_bytes())
                        .ok_or_else(overflow)?;
                    validation_births = validation_births
                        .checked_add(producers.maximum_births())
                        .ok_or_else(overflow)?;
                    bytes = bytes
                        .checked_add(storage.mutable_bytes())
                        .ok_or_else(overflow)?;
                    child_births = child_births
                        .checked_add(storage.maximum_births())
                        .ok_or_else(overflow)?;
                    // Child construction and discovery share this lazy DAG.
                    // Their retained raw population joins the enclosing ordinary
                    // control query; no independent completed arena is implied.
                    arrays = arrays
                        .checked_add(operation.inputs.len())
                        .and_then(|n| n.checked_add(operation.outputs.len()))
                        .ok_or_else(overflow)?;
                    continue;
                }
                let Some(source) = self.addressable_sources.as_ref() else {
                    self.missing_cpu_source(
                        index,
                        operation,
                        "addressable source",
                        &mut missing,
                        &mut missing_operation_detail,
                    )?;
                    continue;
                };
                let quote = source.quote(operation.as_view())?;
                bytes = bytes
                    .checked_add(u64::try_from(quote.capacity.backing).map_err(|_| overflow())?)
                    .ok_or_else(overflow)?;
                child_births = child_births
                    .checked_add(quote.numerical.storage.maximum_births())
                    .ok_or_else(overflow)?;
                arrays = arrays
                    .checked_add(operation.inputs.len())
                    .and_then(|n| n.checked_add(operation.outputs.len()))
            ...
            if matches!(
                ...
                if let Some((controls, nested)) =
                    super::super::cpu::value_frontier(operation.as_view())
                {
                    population.controls = population
                        .controls
            ...
            if matches!(
                ...
                bytes = bytes
                    ...
                    .ok_or_else(overflow)?;
                child_births = child_births
                    .checked_add(wave.provider.births)
                    .ok_or_else(overflow)?;
                continue;
            }
            if matches!(
                ...
                if !self.mechanism.allocation().original_storage {
                    if let Some(local) = parallel.and_then(|source| source.expert_occurrence(index))
                    {
                        let (count, producers) = local.ordinary_validations;
                        validations = validations.checked_add(count).ok_or_else(overflow)?;
                        validation_bytes = validation_bytes
                            .checked_add(producers.mutable_bytes())
                            .ok_or_else(overflow)?;
                        validation_births = validation_births
                ...
                child_births = child_births
                    .checked_add(region.child_births)
                    .ok_or_else(overflow)?;
                nested_completions = nested_completions
                    .checked_add(region.parent_completions)
                    .ok_or_else(overflow)?;
                ...
                arrays = arrays
                    .checked_add(operation.outputs.len())
                    .ok_or_else(overflow)?;
                    ...
                    .chain(region.extra_parents.iter().map(|operation| (operation, 1)))
                {
                    ...
                    population
                        .add(CpuPopulation {
                            construction_entries: repeated(source.construction_entries)?,
                            primitives: repeated(source.primitives)?,
                            input_edges: repeated(source.input_edges)?,
                            hidden_leaves: repeated(source.hidden_leaves)?,
                            maximum_operands: source.maximum_operands,
                            maximum_captures: source.maximum_captures,
                            births: repeated(source.births)?,
                            extents: repeated(source.extents)?,
                            controls: repeated(source.controls)?,
                        })
                        .ok_or_else(overflow)?;
                    seeds = seeds
                        .checked_add(repeated(plan.seeds)?)
                        .ok_or_else(overflow)?;
                    arrays = arrays
                        .checked_add(repeated(
                            source
                                .primitives
                                .checked_add(slice.inputs.len())
                                .and_then(|v| v.checked_add(plan.seeds))
                                .and_then(|v| v.checked_add(source.hidden_leaves))
                                .ok_or_else(overflow)?,
                        )?)
                        .ok_or_else(overflow)?;
                    shells =
                        shells.and_then(|v| v.checked_add(plan.parameter_shells.checked_mul(n)?));
                    bytes = bytes
                        .checked_add(
                            plan.output_bytes
                    ...
                    if plan.validations != 0 {
                        self.missing_cpu_source(
                            index,
                            slice,
                            "expert parent validation source",
                            &mut missing,
                            &mut missing_operation_detail,
                        )?;
                    }
            ...
            if matches!(operation.kind, WorkspaceOperationKind::Collective(_)) {
                ...
                parallel_entries = parallel_entries
                    .checked_add(profile.primitives)
                    .ok_or_else(overflow)?;
                parallel_edges = parallel_edges
                    .checked_add(profile.edges)
                    .ok_or_else(overflow)?;
                parallel_births = parallel_births
                    .checked_add(profile.births)
                    .ok_or_else(overflow)?;
                child_births = child_births
                    .checked_add(profile.child_births)
                    .ok_or_else(overflow)?;
                parallel_graph_extents = parallel_graph_extents
                    .checked_add(profile.graph_extents)
                    .ok_or_else(overflow)?;
                parallel_controls = parallel_controls
                    .checked_add(profile.controls)
                    .ok_or_else(overflow)?;
                nested_completions = nested_completions
                    .checked_add(profile.nested)
                    .ok_or_else(overflow)?;
            ...
            if super::super::parameter_decode::gguf::inspect(operation.as_view())
                .map_err(|cause| self.metadata_source(cause))?
                .is_some()
            {
                nested_completions = nested_completions.checked_add(2).ok_or_else(overflow)?;
            }
            if matches!(operation.kind, WorkspaceOperationKind::Grouped { .. }) {
                grouped_outputs = grouped_outputs
                    .merge(
                        super::super::cpu::grouped::output_storage(operation.as_view())
                            .map_err(|cause| self.metadata_source(cause))?,
                    )
                    .ok_or_else(overflow)?;
            }
            if matches!(operation.kind, WorkspaceOperationKind::Attention { .. }) {
                grouped_outputs = grouped_outputs
                    .merge(super::super::cpu::attention::output_storage(
                        operation.as_view(),
                    ))
                    .ok_or_else(overflow)?;
            }
            if matches!(
                ...
                retained_source_seeds =
                    retained_source_seeds.checked_add(1).ok_or_else(overflow)?;
                nested_completions = nested_completions.checked_add(1).ok_or_else(overflow)?;
            }
            nested_completions = nested_completions
                .checked_add(super::super::cpu::blockwise::nested_completions(
                    operation.as_view(),
                ))
                .ok_or_else(overflow)?;
            nested_completions = nested_completions
                .checked_add(
                    super::super::cpu::router::nested_completions(operation.as_view(), cpu)
                        .map_err(|cause| self.metadata_source(cause))?,
                )
                .ok_or_else(overflow)?;
            ...
            if plan.validations != 0 {
                validation_bytes = validation_bytes
                    ...
                    .ok_or_else(overflow)?;
                validation_births = validation_births
                    .checked_add(plan.population.births)
        ...
        let roots = retained_roots
            ...
            .ok_or_else(overflow)?;
        let construction_entries = population
            .construction_entries
            .checked_add(parallel_entries)
            .ok_or_else(overflow)?;
        let primitives = population
            .primitives
            .checked_add(parallel_entries)
            .ok_or_else(overflow)?;
        ...
        Ok(ReducedTrace {
            ...
            mutable_storage: missing.is_none().then_some(CertifiedSpanStorage {
                mutable_bytes: bytes,
                maximum_births: population
                    .births
                    .checked_add(seeds)
                    .and_then(|n| n.checked_add(parallel_births))
                    .and_then(|n| n.checked_add(child_births))
                    .ok_or_else(overflow)?,
            }),
...
mod tests {
    ...
    fn cpu_prepared_token_equation_prices_input_and_host_without_admitting_generic_initialization()
    {
        ...
        for dtype in [WorkspaceDtype::Int32, WorkspaceDtype::Uint32] {
            for count in [1, 2, 5] {
                ...
                let input =
                    WorkspaceTensor::prepared_token_input(&[2, count], dtype, &context).unwrap();
                ...
                let geometry = InferenceGeometry {
                    batch_size: 2,
                    cached_positions: 0,
                    input_positions: count as u64,
                    max_output_tokens: 1,
                    prefill_chunk_positions: count as u64,
                ...
                let recorder =
                    ResidentRecipeRecorder::with_cpu_context(geometry, ordinary, cpu, &context)
                        .unwrap();
                assert!(recorder
                    .reduce_cpu_trace(&report, Some(0), 0, 1, cpu)
                    .unwrap()
                    .first_missing_operation
                    .is_none());
                assert_eq!(
                    recorder
                        .reduce_cpu_trace(&report, None, 0, 1, cpu)
                        .unwrap()
                        .first_missing_operation,
                    Some(0)
                );
                let generic = WorkspaceContext::new(cpu);
                generic.begin_state_span([]).unwrap();
                ...
                metal.begin_state_span([]).unwrap();
                let input =
                    WorkspaceTensor::prepared_token_input(&[2, count], dtype, &metal).unwrap();
                let report = metal.report(&[input]).unwrap();
                assert!(report.inference_transient_bytes().is_some());
            }
        }
    }
    ...
    fn cpu_original_prompt_quote_keeps_full_backing_and_authenticated_input_source() {
        ...
        let cpu = MlxCpuWorkspaceMechanisms::new(ordinary.allocation(), matmul);
        let ids = [7; 11];
        let input = eredu_runtime::working_memory::OriginalTokenInputLayout::prepare(
        ...
        for chunk in [1, 4, 11] {
            ...
            let context = WorkspaceContext::new(cpu);
            let quoted = eredu_runtime::working_memory::quote_original_token_prompt_workspace(
                geometry, &input, &context,
            )
            .unwrap();
            assert!(quoted.peak().bytes().is_some());
            assert_eq!(
                quoted.tensor_peak_bytes(),
                Some(ordinary.allocation().fixed_buffer_capacity(44).unwrap())
            );
            assert!(
                quoted.host_peak_bytes().unwrap() > 0,
                "closed text identity remains priced"
            );
            if let Some(prior) = peak {
                assert_eq!(quoted.peak().bytes(), Some(prior));
            }
            peak = quoted.peak().bytes();
            let report = context.report(&[]).unwrap();
            assert_eq!(report.operations.len(), 1);
            assert!(super::super::super::basic::is_prepared_token_input(
                report.operations[0].as_view()
            ));
            assert!(
                report.unpriced_operations.is_empty() && report.unpriced_host_operations.is_empty()
            );
            let ordinary_context = WorkspaceContext::new(cpu.ordinary_storage());
            let ordinary_prompt = eredu_runtime::working_memory::quote_text_prompt_workspace(
                geometry,
                Some(44),
                &ordinary_context,
            )
            .unwrap();
            assert!(ordinary_prompt.peak().bytes().is_some());
            assert!(ordinary_prompt.physical_domains().is_some());
            assert_eq!(
                ordinary_prompt.tensor_peak_bytes(),
                Some(ordinary.allocation().fixed_buffer_capacity(44).unwrap())
            );
            let ordinary_report = ordinary_context.report(&[]).unwrap();
            assert!(matches!(
                ordinary_report.operations[0].kind,
                WorkspaceOperationKind::Elementwise("text_prompt_u32")
            ));
            let plan = cpu
                .plan(ordinary_report.operations[0].as_view())
                .unwrap()
                .unwrap();
            assert_eq!(
                (plan.seeds, plan.population.primitives, plan.scratch_bytes),
                (1, 0, 0)
            );
            let generic = WorkspaceContext::new(cpu);
            let value =
                WorkspaceTensor::initialized(&[1, 11], WorkspaceDtype::Uint32, &generic).unwrap();
            assert!(
                generic.report(&[value]).unwrap().total_bytes.is_none(),
                "generic initialization still requires its own actual producer"
            );
        }
    }
    struct Bind<'a> {
            ...
            for (shape, biased) in [
                ...
                assert_eq!(
                    reduced
                        .graph
                        .as_ref()
                        .expect("qualified shared constructor")
                        .maximum_operands(),
                    4
                );
                assert!(
                    reduced.traversal.is_some(),
                    "qualified actual CPU traversal"
                );
    ...
    fn cpu_mixed_dense_quotes_each_conversion_at_its_own_extent() {
        ...
        use WorkspaceFloatingType::{Bfloat16 as B, Float16 as H, Float32 as F};
        ...
        for (left_dtype, weight_dtype, bias_dtype, casts) in
            [(F, B, H, 2usize), (H, F, B, 2), (B, H, F, 2), (B, B, F, 1)]
        {
            let context = WorkspaceContext::new(cpu);
            ...
            let input = represented(&[1, 2, 257], left_dtype);
            let weight = represented(&[19, 257], weight_dtype);
            ...
            let mut projection = WorkspaceBackend::linear(
                LinearSpec {
                    ...
                    output: 19,
                    weight: ParameterSpec::trainable("cpu.weight").unwrap(),
                    bias: Some(ParameterSpec::trainable("cpu.bias").unwrap()),
                    format: LinearFormatSpec::unscaled(LinearFormat::Dense).unwrap(),
                },
                &context,
            )
            ...
            assert_eq!(
                ...
                Some(WorkspaceRepresentation::new(F, true))
            );
            let report = context.report(&[output]).unwrap();
            let plan = cpu.plan(report.operations[0].as_view()).unwrap().unwrap();
            assert_eq!(plan.population.primitives, 6 + casts);
            assert_eq!(plan.population.births, 2 + casts);
            let matrix_dtype = if left_dtype == weight_dtype {
                left_dtype
            } else {
                F
            };
            let capacity = |elements: u64| {
                ordinary
                    .allocation()
                    .fixed_buffer_capacity(elements * 4)
                    .unwrap()
            };
            let expected = capacity(38)
                + if left_dtype != matrix_dtype {
                    capacity(514)
                } else {
                    0
                }
                + if weight_dtype != matrix_dtype {
                    capacity(4883)
                } else {
                    0
                }
            ...
            invalid.operations[0].inputs[1] = invalid.operations[0].inputs[1]
                .clone()
            ...
            for input in &mut invalid.operations[0].inputs[..2] {
                *input = input
                    .clone()
                    .with_representation(Some(WorkspaceRepresentation::new(H, true)));
            }
            let supported = selected
                .selected()
                .float16_geometry(2, 2, 19, 257, 1)
                .is_ok();
    ...
    fn cpu_selected_f16_dense_joins_matrix_and_post_bias_conversion_sources() {
        let ordinary = MlxMetalWorkspaceMechanisms::current_host().unwrap();
        let selected =
            MlxCpuMatmulMechanism::select(CpuMatmulImplementation::Float32AndFloat16Tiles).unwrap();
        let cpu = MlxCpuWorkspaceMechanisms::new(ordinary.allocation(), selected);
        use WorkspaceFloatingType::{Float16 as H, Float32 as F};
        for bias_dtype in [H, F] {
            ...
            let input = represented(&[2, 3, 23], H);
            let weight = represented(&[17, 23], H);
            ...
            assert_eq!(
                output.layout().representation(),
                Some(WorkspaceRepresentation::new(bias_dtype, true))
            );
            let report = context.report(&[output]).unwrap();
            let plan = cpu.plan(report.operations[0].as_view()).unwrap().unwrap();
            let cast = usize::from(bias_dtype == F);
            assert_eq!(plan.population.primitives, 6 + cast);
            assert_eq!(plan.population.births, 2 + cast);
            let recorder = ResidentRecipeRecorder::with_cpu_context(
                InferenceGeometry {
                    batch_size: 2,
                    cached_positions: 0,
            ...
            missing.operations[0].inputs[1] = missing.operations[0].inputs[1]
                .clone()
                .with_representation(None);
    ...
    fn cpu_mixed_rms_preserves_module_promotion_and_tensor_cast_back() {
        use eredu_nn::{NormalizationConstructionSpec, NormalizationOperator, NormalizationScale};
        use WorkspaceFloatingType::{Bfloat16 as B, Float16 as H, Float32 as F};
        let ordinary = MlxMetalWorkspaceMechanisms::current_host().unwrap();
        let Some(selected) = MlxCpuMatmulMechanism::select(CpuMatmulImplementation::Float32Tiles)
        else {
        ...
        for (input_dtype, gain_dtype) in [(F, B), (F, H), (B, F), (H, F), (B, H)] {
            ...
            let input = represented(&[2, 3, 17], input_dtype);
            let gain = represented(&[17], gain_dtype);
            let mut module = WorkspaceBackend::normalization(
                NormalizationConstructionSpec {
                    groups: None,
                    dimensions: 17,
                    epsilon: 1e-6,
                    scale: NormalizationScale::Learned(
                        ParameterSpec::trainable("cpu.weight").unwrap(),
                    ),
                },
                &context,
            )
            ...
            context.begin_span();
            let module_output = module.forward(&input, &context).unwrap();
            let tensor_output =
                WorkspaceBackend::rms_norm_with_weight(&input, &gain, 1e-6, &context).unwrap();
            assert_eq!(
                module_output.layout().representation(),
                Some(WorkspaceRepresentation::new(F, true))
            );
            assert_eq!(
                tensor_output.layout().representation(),
                Some(WorkspaceRepresentation::new(input_dtype, true))
            );
            ...
            assert_eq!(
                module_plan.population.primitives, 26,
                "mixed input/gain does not create the equal-half GPU probe cast"
            );
            assert_eq!(
                tensor_plan.population.primitives,
                module_plan.population.primitives + 1
            );
            assert_eq!(
                tensor_plan.population.births,
                module_plan.population.births + 1
            );
    ...
    #[test]
    fn cpu_dense_source_qualifies_selected_addmm_and_rejects_unspecified_layout_and_default_blas() {
        let ordinary = MlxMetalWorkspaceMechanisms::current_host().unwrap();
        let Some(selected) = MlxCpuMatmulMechanism::select(CpuMatmulImplementation::Float32Tiles)
        else {
            assert!(std::env::var_os("EREDU_REQUIRE_QUALIFIED_RECORD_LAYOUT").is_none());
        ...
        let report = context.report(&[output]).unwrap();
        let biased = cpu.plan(report.operations[0].as_view()).unwrap().unwrap();
        assert_eq!(biased.population.births, 1);
        assert_eq!(biased.scratch_bytes, 0);
        assert_eq!(biased.population.primitives, 3); // transpose, bias broadcast, AddMM
    ...
    fn cpu_pointwise_residual_trace_composes_exact_sources_and_stored_half_scalar() {
        ...
        for dtype in [
            ...
            context.begin_span();
            let scaled = input.multiply(&half_scalar, &context).unwrap();
            assert_eq!(
                scaled.layout().representation(),
            ...
            assert_eq!(plan.population.primitives, 2 + casts);
            assert_eq!(plan.population.births, 1 + casts);
            let capacity = |elements: u64| {
                ordinary
                    .allocation()
                    .fixed_buffer_capacity(elements * 4)
            ...
            assert_eq!(
                plan.scratch_bytes,
                capacity(1) + if casts == 2 { capacity(6) } else { 0 }
            );
            assert_eq!(
                recorder
                    .reduce_trace(&mixed, None, 0, 1)
                    .unwrap()
                    .first_missing_operation,
                None
            );
            mixed.operations[0].inputs[1] = mixed.operations[0].inputs[1]
                .clone()
                .with_representation(None);
    ...
    fn cpu_sentinel_lookup_keeps_domain_completion_and_physical_output() {
        for sentinel in [-1, -7] {
            lookup_sources(
                ...
                eredu_nn::EmbeddingLookupPolicy::ZeroSentinel(sentinel),
            );
        }
    }
    fn lookup_sources(dtypes: &[WorkspaceFloatingType], policy: eredu_nn::EmbeddingLookupPolicy) {
        use eredu_nn::{EmbeddingLookupPolicy, EmbeddingOperator, EmbeddingSpec};
        let ordinary = MlxMetalWorkspaceMechanisms::current_host().unwrap();
        let Some(selected) = MlxCpuMatmulMechanism::select(CpuMatmulImplementation::Float32Tiles)
        else {
            assert!(std::env::var_os("EREDU_REQUIRE_QUALIFIED_RECORD_LAYOUT").is_none());
        ...
        for &dtype in dtypes {
            for ids_shape in [&[][..], &[2, 2][..]] {
                ...
                let weight = WorkspaceTensor::existing(
                    context
                        .layout(&[37, 13], WorkspaceDtype::Float32)
                        .unwrap()
                ...
                let ids = WorkspaceTensor::existing(
                    context.layout(ids_shape, WorkspaceDtype::Uint32).unwrap(),
                    &context,
                )
                .unwrap();
                let mut lookup = WorkspaceBackend::embedding(
                    EmbeddingSpec {
                        vocabulary: 37,
                        dimensions: 13,
                        weight: ParameterSpec::trainable("cpu.weight").unwrap(),
                ...
                .unwrap();
                lookup.visit_parameters_mut(&mut Bind {
                    weight: &weight,
                    bias: &weight,
                });
                context.begin_span();
                let output = lookup.lookup(&ids, policy, &context).unwrap();
                let report = context.report(&[output]).unwrap();
                let plan = cpu.plan(report.operations[0].as_view()).unwrap().unwrap();
                assert_eq!(plan.dtype, dtype);
                ...
                assert_eq!(
                    plan.population.primitives,
                    (if ids_shape.is_empty() { 48 } else { 50 }) + if sentinel { 27 } else { 0 }
                );
                let mut recorder = ResidentRecipeRecorder::new(
                    InferenceGeometry {
                        batch_size: 1,
                ...
                let reduced = recorder.reduce_trace(&report, None, 0, 1).unwrap();
                assert_eq!(reduced.first_missing_operation, None);
                assert_eq!(reduced.validation_roots, 1);
                assert_eq!(reduced.traversal.unwrap().roots(), 2);
                assert_eq!(reduced.graph.unwrap().seeds(), seeds);
                assert_eq!(
                    reduced.mutable_storage.unwrap().maximum_births(),
                    plan.population.births + seeds
                );
                assert_eq!(
                    reduced.validation_producers.unwrap().mutable_bytes(),
                    plan.output_bytes + plan.scratch_bytes
                );
                assert_eq!(
                    reduced.validation_producers.unwrap().maximum_births(),
                    plan.population.births + seeds
                );
                let dispatch = reduced.dispatch.unwrap();
                assert_eq!(dispatch.gpu_entries, 0);
                ...
                let WorkspaceOperationKind::Embedding(_, policy) = &mut wrong.operations[0].kind
                else {
                    panic!("lookup lost identity")
                };
                *policy = EmbeddingLookupPolicy::ZeroSentinel(0);
                assert!(
                    recorder.reduce_trace(&wrong, None, 0, 1).is_err(),
    ...
    fn rms_quote_sources(dtypes: &[WorkspaceFloatingType]) {
        ...
        for &dtype in dtypes {
            for shape in [&[17][..], &[2, 3, 17][..]] {
                ...
                let input = represented(shape);
                let weight = represented(&[17]);
                let mut normalization = WorkspaceBackend::normalization(
                    NormalizationConstructionSpec {
                        groups: None,
                        dimensions: 17,
                        epsilon: 1e-6,
                ...
                .unwrap();
                normalization.visit_parameters_mut(&mut Bind {
                    weight: &weight,
                    bias: &weight,
                });
                context.begin_span();
                let output = normalization.forward(&input, &context).unwrap();
                let report = context.report(&[output]).unwrap();
                let plan = cpu.plan(report.operations[0].as_view()).unwrap().unwrap();
                assert_eq!(plan.dtype, dtype);
                ...
                let reduced = recorder.reduce_trace(&report, None, 0, 1).unwrap();
                assert_eq!(reduced.first_missing_operation, None);
                assert_eq!(reduced.graph.unwrap().seeds(), 2);
                assert!(reduced.dispatch.unwrap().cpu_model.is_some());
                context.begin_span();
                let output =
                    WorkspaceBackend::rms_norm_without_weight(&input, 1e-6, &context).unwrap();
    ...
    fn cpu_half_unit_rms_quotes_half_mean_then_actual_f32_epsilon_promotion() {
        ...
        let ordinary = MlxMetalWorkspaceMechanisms::current_host().unwrap();
        let selected =
            MlxCpuMatmulMechanism::select(CpuMatmulImplementation::Float32Tiles).unwrap();
        let cpu = MlxCpuWorkspaceMechanisms::new(ordinary.allocation(), selected);
        for dtype in [
            WorkspaceFloatingType::Float16,
            WorkspaceFloatingType::Bfloat16,
            ...
            for width in [1, 17, 1031] {
                ...
                let input = WorkspaceTensor::existing(
                    context
                        .layout(&[2, 3, width], WorkspaceDtype::Float32)
                        .unwrap()
                        .with_representation(Some(WorkspaceRepresentation::new(dtype, true))),
                    &context,
                )
    ...
    fn cpu_activations_preserve_wide_intermediates_and_compose_with_residuals() {
        ...
        for dtype in [
            ...
            for silu in [true, false] {
                ...
                let input = WorkspaceTensor::existing(
                    context
                        .layout(&[2, 17], WorkspaceDtype::Float32)
                        .unwrap()
                        .with_representation(Some(WorkspaceRepresentation::new(dtype, true))),
                    &context,
                )
                ...
                let activation = if silu {
                    WorkspaceBackend::silu(input.clone(), &context).unwrap()
                } else {
                    WorkspaceBackend::sigmoid(input.clone(), &context).unwrap()
                };
                let output = activation.multiply(&input, &context).unwrap();
                let report = context.report(&[output]).unwrap();
                let plan = cpu.plan(report.operations[0].as_view()).unwrap().unwrap();
                assert_eq!(plan.dtype, dtype);
                assert_eq!(plan.seeds, usize::from(silu));
                assert_eq!(
                    plan.population.primitives,
                    (if silu { 13 } else { 2 })
                        + usize::from(dtype != WorkspaceFloatingType::Float32)
                );
                let mut recorder = ResidentRecipeRecorder::new(
                    InferenceGeometry {
                        batch_size: 1,
                ...
                let reduced = recorder.reduce_trace(&report, None, 0, 1).unwrap();
                assert_eq!(reduced.first_missing_operation, None);
                assert_eq!(reduced.graph.unwrap().seeds(), usize::from(silu));
                assert!(reduced.dispatch.unwrap().cpu_model.is_some());
                let mut unknown = report;
                unknown.operations[0].inputs[0] = unknown.operations[0].inputs[0]
                    .clone()
                    .with_representation(None);
        ...
        let context = WorkspaceContext::new(cpu);
        let input = value(&[2, 3], &context, true);
        context.begin_span();
        let exponential = WorkspaceBackend::exp(input, &context).unwrap();
        let report = context.report(&[exponential]).unwrap();
        assert!(report
            .operations
            .iter()
            .all(|operation| cpu.plan(operation.as_view()).unwrap().is_some()));
    }
    ...
    fn cpu_target_embedding_scale_keeps_exact_f32_output_and_seed_custody() {
        ...
        let cpu = MlxCpuWorkspaceMechanisms::new(ordinary.allocation(), selected);
        let context = WorkspaceContext::new(cpu);
        let input = value(&[1, 3, 32], &context, true);
        context.begin_span();
        ...
        let output = scaled.add(&input, &context).unwrap();
        let report = context.report(&[output]).unwrap();
        let plan = cpu.plan(report.operations[0].as_view()).unwrap().unwrap();
        assert_eq!(plan.seeds, 1);
        assert_eq!(plan.population.primitives, 5);
        ...
        assert_eq!(reduced.first_missing_operation, None);
        assert!(reduced.dispatch.unwrap().cpu_model.is_some());
        assert_eq!(reduced.graph.unwrap().seeds(), 1);
        let mut unknown = report;
        unknown.operations[0].inputs[0] = unknown.operations[0].inputs[0]
            .clone()
    ...
    fn cpu_half_scalar_and_gelu_sources_preserve_actual_f32_promotion() {
        ...
        for dtype in [
            ...
            let input = WorkspaceTensor::existing(
                context
                    .layout(&[2, 3, 17], WorkspaceDtype::Float32)
                    .unwrap()
                    .with_representation(Some(WorkspaceRepresentation::new(dtype, true))),
                &context,
            )
            ...
            assert_eq!(
                (
                    scale.population.primitives,
                    scale.population.births,
                    scale.seeds
                ),
                (5, 3, 1)
            );
            ...
            assert!(scale.scratch_bytes > scale.output_bytes);
            assert!(gelu.scratch_bytes > gelu.output_bytes);
            let recorder = ResidentRecipeRecorder::with_cpu_context(
                InferenceGeometry {
                    batch_size: 2,
                    cached_positions: 0,
    ...
    fn cpu_singleton_concatenate_preserves_integer_and_strided_source_facts() {
        ...
        let cpu = MlxCpuWorkspaceMechanisms::new(ordinary.allocation(), selected);
        for (dtype, representation) in [
            (WorkspaceDtype::Int32, None),
            (
            ...
            .unwrap();
            context.begin_span();
            let output = WorkspaceTensor::concatenate(&[input], 1, &context).unwrap();
            assert_eq!(output.layout().representation(), representation);
            let report = context.report(&[output]).unwrap();
            let plan = cpu.plan(report.operations[0].as_view()).unwrap().unwrap();
            assert_eq!(plan.alias_input, Some(0));
            assert_eq!(plan.population.primitives, 0);
    ...
    fn native_cpu_singleton_concatenate_keeps_backing_and_ordinary_axis_semantics() {
        ...
        let strided = source.transpose_axes(&[1, 0], &stream).unwrap();
        strided.evaluated().unwrap();
        ...
        assert_eq!(alias.shape(), &[3, 2]);
        assert_eq!(alias.dtype(), safemlx::Dtype::Int32);
        assert_eq!(
            alias.try_allocation_info().unwrap().unwrap().identity(),
            before
        );
        ...
        assert_eq!(
            read.evaluated().unwrap().as_slice::<i32>(),
            &[1, 4, 2, 5, 3, 6]
        );
    }
    ...
    fn cpu_attention_head_transpose_keeps_source_and_marks_strided_output() {
        ...
        let cpu = MlxCpuWorkspaceMechanisms::new(ordinary.allocation(), selected);
        let context = WorkspaceContext::new(cpu);
        let input = value(&[1, 3, 4, 8], &context, true);
        context.begin_span();
        let output = input.transpose_axes(&[0, 2, 1, 3], &context).unwrap();
        ...
        malformed.operations[0].outputs[0] = context
            .layout(&[1, 2, 6, 8], WorkspaceDtype::Float32)
            .unwrap();
        assert!(cpu.plan(malformed.operations[0].as_view()).is_err());
        let repeated = value(&[2, 2, 8], &context, true);
        context.begin_span();
    ...
    fn cpu_gated_product_reuses_activation_source_and_retains_its_intermediate() {
        ...
        for dtype in [
            ...
            let up = value();
            context.begin_span();
            let output = WorkspaceBackend::gated_product(
                gate.clone(),
                up.clone(),
            ...
            .unwrap();
            let fused = context.report(&[output]).unwrap();
            let plan = cpu.plan(fused.operations[0].as_view()).unwrap().unwrap();
            context.begin_span();
            let activated = WorkspaceBackend::silu(gate.clone(), &context).unwrap();
            let output = activated.multiply(&up, &context).unwrap();
            ...
            assert_eq!(
                plan.population.primitives,
                activation.population.primitives + product.population.primitives
            );
            assert_eq!(
                plan.population.births,
                activation.population.births + product.population.births
            );
            assert_eq!(
                plan.output_bytes + plan.scratch_bytes,
                activation.output_bytes
                    + activation.scratch_bytes
                    + product.output_bytes
            ...
            recorder.cpu = Some(cpu);
            let reduced = recorder.reduce_trace(&fused, None, 0, 1).unwrap();
            assert_eq!(reduced.first_missing_operation, None);
            assert!(reduced.dispatch.unwrap().cpu_model.is_some());
            let mut bounded = fused;
            bounded.operations[0].kind = WorkspaceOperationKind::GatedProduct(
                GatedProductPolicy::bounded_silu(2.0).unwrap(),
            );
    ...
    fn cpu_row_views_preserve_source_alias_and_allocate_only_the_real_result() {
        ...
        for dtype in [
            ...
            for operation in &report.operations[..3] {
                ...
                assert!(matches!(
                    bound.outputs.as_slice(),
                    [eredu_nn::workspace::WorkspaceOutputStorage::AliasInput(0)]
                ));
            }
            let mut recorder = ResidentRecipeRecorder::new(
                InferenceGeometry {
            ...
            let reduced = recorder.reduce_trace(&report, None, 0, 1).unwrap();
            assert_eq!(reduced.first_missing_operation, None);
            assert_eq!(reduced.mutable_storage.unwrap().maximum_births(), 1);
    ...
    fn cpu_gemma_step_concatenation_covers_both_sources_and_copy_jobs() {
        ...
        for dtype in [
            ...
            let embedding = source(&[1, 1, 32]);
            let previous_hidden = source(&[1, 1, 32]);
            context.begin_span();
            let output =
                WorkspaceTensor::concatenate(&[embedding, previous_hidden], -1, &context).unwrap();
    ...
    fn cpu_gemma_unit_axis_views_and_exact_gelu_preserve_source_sequence() {
        ...
        context.begin_span();
        let headed = input.transpose_axes(&[0, 2, 1, 3], &context).unwrap();
        ...
        assert_eq!(gelu.seeds, 3);
        assert_eq!(gelu.population.primitives, 22);
        assert!(gelu.scratch_bytes > gelu.output_bytes);
        assert_eq!(gelu.dtype, WorkspaceFloatingType::Float32);
        let mut recorder = ResidentRecipeRecorder::new(
            InferenceGeometry {
                batch_size: 1,
                cached_positions: 3,
        ...
        let square = source(&[2, 2]);
        context.begin_span();
        let transposed = square.transpose_axes(&[1, 0], &context).unwrap();
        assert!(!transposed
            .layout()
            .representation()
            .unwrap()
        ...
        assert_eq!(
            cpu.plan(report.operations[0].as_view())
                .unwrap()
                .unwrap()
                .alias_input,
            Some(0)
    ...
    fn cpu_gemma_rotary_quote_retains_frequency_ranges_and_half_products() {
        ...
        let input = source(&[1, 4, 1, 8]);
        let mut rope = eredu_nn::workspace::WorkspaceBackend::rotary(spec, &context).unwrap();
        context.begin_span();
        let output = rope
            .forward(&input, RotaryPosition::Offset(2), &context)
            .unwrap();
        let report = context.report(&[output]).unwrap();
        assert_eq!(report.operations.len(), 1);
        let plan = cpu.plan(report.operations[0].as_view()).unwrap().unwrap();
        assert_eq!(plan.seeds, 3);
        assert!(plan.population.primitives > 50);
        assert!(plan.scratch_bytes > plan.output_bytes);
        let mut recorder = ResidentRecipeRecorder::new(
            InferenceGeometry {
                batch_size: 1,
    ...
    fn cpu_causal_mask_quote_keeps_integer_coordinates_and_boolean_window_results() {
        ...
        for (q, offset, window) in [(3, 0, None), (2, 3, None), (3, 5, Some(0)), (3, 5, Some(2))] {
            ...
            context.begin_span();
            let output =
                eredu_nn::workspace::WorkspaceBackend::causal_mask(q, offset, window, &context)
                    .unwrap();
            let report = context.report(&[output]).unwrap();
            assert_eq!(report.operations.len(), 1);
            ...
            assert_eq!(
                operation.outputs.get(0).unwrap().dtype(),
                WorkspaceDtype::Bool
            );
            ...
            assert_eq!(
                plan.population.primitives,
                if window.is_some() { 24 } else { 9 }
            );
            assert!(plan.scratch_bytes > 0);
            assert!(plan.population.births >= 3);
            let mut recorder = ResidentRecipeRecorder::new(
                InferenceGeometry {
                    batch_size: 1,
                    cached_positions: offset as u64,
                    input_positions: q as u64,
                    max_output_tokens: 0,
                    prefill_chunk_positions: q as u64,
                    output: eredu_core::OutputDemand::Sequence,
                },
                ordinary,
            );
    ...
    fn cpu_explicit_boolean_attention_quote_counts_mask_frontend_and_gqa_select() {
        ...
        for heads in [2, 4] {
            ...
            let q = source(&[1, heads, 3, 8]);
            let k = source(&[1, 2, 5, 8]);
            let v = source(&[1, 2, 5, 8]);
            let mask = WorkspaceTensor::existing(
                context.layout(&[3, 5], WorkspaceDtype::Bool).unwrap(),
                &context,
            )
            .unwrap();
            context.begin_span();
            let output = eredu_nn::workspace::WorkspaceBackend::attention(
                q,
                k,
                v,
                8f32.sqrt().recip(),
    ...
    fn cpu_half_attention_quote_joins_precise_dtype_mask_and_strided_value_sources() {
        ...
        for (q_type, k_type, v_type, expected) in [(H, H, H, H), (B, B, B, B), (H, B, H, F)] {
            for masked in [false, true] {
                ...
                let q = source(&[1, 4, 3, 8], q_type);
                let k = source(&[1, 2, 5, 8], k_type);
                let value = source(&[1, 5, 2, 8], v_type)
                ...
                assert_eq!(
                    output.layout().representation(),
                    Some(WorkspaceRepresentation::new(expected, true))
                );
                let report = context.report(&[output]).unwrap();
                let plan = cpu.plan(report.operations[0].as_view()).unwrap().unwrap();
                assert_eq!(plan.dtype, expected);
                assert_eq!(plan.rank, 5);
                assert_eq!(plan.seeds, 1 + usize::from(masked));
                let recorder = ResidentRecipeRecorder::with_cpu_context(
                    InferenceGeometry {
                        batch_size: 1,
                        cached_positions: 2,
    ...
    fn cpu_gemma_grouped_attention_quote_retains_rank_five_score_and_value_paths() {
        ...
        let q = source(&[1, 4, 1, 8]);
        let k = source(&[1, 2, 3, 8]);
        let v = source(&[1, 2, 3, 8]);
        context.begin_span();
        let output = WorkspaceTensor::scaled_dot_product_attention(
            &q,
            &k,
            &v,
            8f32.sqrt().recip(),
            eredu_nn::AttentionMask::None,
            &context,
        )
        .unwrap();
        let report = context.report(&[output]).unwrap();
        ...
        let platform = MlxCpuWorkspaceMechanisms::new(
            ordinary.allocation(),
            MlxCpuMatmulMechanism::select(CpuMatmulImplementation::PlatformDefault).unwrap(),
        );
        assert!(platform
    ...
    fn cpu_exact_head_permutation_proves_final_axis_for_repeated_prefill_geometry() {
        ...
        for shape in [[1, 3, 3, 8], [1, 8, 3, 8], [1, 1, 3, 8]] {
            ...
            missing.kind = WorkspaceOperationKind::View("transpose");
            assert!(cpu.plan(missing.as_view()).unwrap().is_none());
            assert!(cpu.output_representation(missing.as_view(), 0).is_none());
            let mut invalid = report.operations[0].clone();
            invalid.kind = WorkspaceOperationKind::Transpose(vec![0, 2, 1, 2]);
    ...
    fn cpu_strided_head_rms_and_cached_concatenation_share_exact_row_sources() {
        ...
        for positions in [1, 3, 8] {
            ...
            let input = source(&[1, positions, 4, 8]);
            let gain = source(&[8]);
            context.begin_span();
            let headed = input.transpose_axes(&[0, 2, 1, 3], &context).unwrap();
            let normalized = eredu_nn::workspace::WorkspaceBackend::rms_norm_with_weight(
                &headed, &gain, 1e-6, &context,
            )
            .unwrap();
            assert!(normalized
                .layout()
                ...
                .row_contiguous());
            let joined =
                WorkspaceTensor::concatenate(&[headed.clone(), normalized], 2, &context).unwrap();
    ...
    fn cpu_masked_attention_prices_the_actual_strided_value_copy() {
        ...
        let q = source(&[1, 4, 3, 8]);
        let k = source(&[1, 2, 5, 8]);
        let raw_v = source(&[1, 5, 2, 8]);
        ...
        let mut compact = report.operations[0].clone();
        compact.inputs[2] =
            compact.inputs[2]
                .clone()
                .with_representation(Some(WorkspaceRepresentation::new(
    ...
    fn cpu_head_join_prices_copy_before_row_normalization_and_keeps_alias_refusal() {
        ...
        for positions in [1, 3, 4] {
            ...
            let input = source(&[1, 4, positions, 8]);
            let gain = source(&[32]);
            context.begin_span();
            ...
            let output = eredu_nn::workspace::WorkspaceBackend::rms_norm_with_weight(
                &joined, &gain, 1e-6, &context,
            )
            .unwrap();
            let report = context.report(&[output]).unwrap();
        ...
        let input = WorkspaceTensor::existing(
            context
                .layout(&[4, 3, 8], WorkspaceDtype::Float32)
                .unwrap()
                .with_representation(Some(WorkspaceRepresentation::new(
                    WorkspaceFloatingType::Float32,
                    true,
        ...
        .unwrap();
        context.begin_span();
        let headed = input.transpose_axes(&[1, 0, 2], &context).unwrap();
    ...
    fn cpu_ordered_readout_composes_integer_sources_and_real_selected_replicas() {
        ...
        for index_dtype in [WorkspaceDtype::Int32, WorkspaceDtype::Uint32] {
            ...
            let hidden = source(&[2, 3, 32]);
            let weight = source(&[32, 32]);
            let centroids = source(&[2, 3, 4]);
            let ordering =
                WorkspaceTensor::existing(context.layout(&[32], index_dtype).unwrap(), &context)
                    .unwrap();
            context.begin_span();
            ...
            let replicas = ordinary
                .allocation()
                .fixed_buffer_capacity(2 * 3 * 16 * 32 * 4)
                .unwrap();
...
fn cpu_value_frontiers_quote_exact_borrowed_roots_without_tensor_births() {
    let ordinary = MlxMetalWorkspaceMechanisms::current_host().unwrap();
    let choice = super::super::MlxCpuMatmulMechanism::select(
    ...
    for count in [1, 5] {
        ...
        context.retain_values(&refs).unwrap();
        context.complete_values(&refs).unwrap();
        ...
        let reduced = recorder
            .reduce_cpu_trace(&report, None, count, count, cpu)
            .unwrap();
        assert_eq!(reduced.first_missing_operation, None);
        assert_eq!(reduced.nested_completions, 1);
        assert_eq!(reduced.nested_root_capacity, count);
        assert_eq!(reduced.mutable_storage.unwrap().maximum_births(), 0);
        assert!(
            reduced.graph.is_some()
                && reduced.dispatch.is_some()
                && reduced.query_controls.is_some()
        );
    }
}
