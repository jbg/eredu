...
use super::*;
use crate::{MlxTensor, backend::{MlxBackend, MlxDeviceIdentity,
    managed_memory::gpu_stream::PreparedExecutionStreams,
    nn::shared::MlxNeuralBackend, runtime::cache::kv::ConcatKeyValueCache}};
use eredu_architectures::{gemma4::{Assistant, AssistantConfig, AssistantState,
    assistant::invocation::{DraftStep, DraftStepArguments}},
    external_assistant::invocation::ExternalAssistantOperation};
use eredu_nn::{CpuMatmulImplementation, ParameterMetadata, ParameterVisitorMut, Parameterized, Tensor};
use safemlx::{Array, Device, DeviceType, OperationEvent, OriginalBufferBudget, OriginalScopeObserver,
...
fn tensor(shape: &[i32], phase: usize) -> MlxTensor {
    ...
    let data=(0..count).map(|i|(((i*13+phase*7)%31) as f32-15.0)*0.025).collect::<Vec<_>>();
...
fn project(value:&Array, context:&WorkspaceContext)->WorkspaceTensor {
    ...
    let (dtype,representation)=match descriptor.facts().dtype() {
        safemlx::Dtype::Float32=>(WorkspaceDtype::Float32,
    ...
    WorkspaceTensor::existing(context.layout(descriptor.shape(),dtype).unwrap()
...
fn state(hidden:&MlxTensor,keys:&MlxTensor,values:&MlxTensor)->AssistantState<MlxTensor> {
    AssistantState {evidence:None,hidden:hidden.clone(),kv_offset:3,
        shared_kv:std::collections::HashMap::from([(eredu_core::AttentionPolicy::Full,
            (keys.clone(),values.clone()))]).into()}
}
fn compare(actual:&Array, expected:&[f32]) {
    let read=actual.evaluated().unwrap();let actual=read.try_iter::<f32>().unwrap();
    assert_eq!(actual.len(),expected.len());
    for (a,b) in actual.zip(expected) {
        assert!(a.is_finite() && (a-b).abs()<=2e-5+2e-5*b.abs(),"actual {a}, ordinary {b}");
    }
}
#[derive(Debug)]
...
fn complete_draft_step(ordered:bool) {
    ...
    if ordered {config.use_ordered_embeddings=true;config.num_centroids=4;config.centroid_intermediate_top_k=2;}
    let mut module=Assistant::<MlxNeuralBackend>::new(config.clone(),stream).unwrap();
    let mut sources=BindNative {source:Vec::new(),ordered};module.visit_parameters_mut(&mut sources);
    let embedding=tensor(&[1,1,32],101);let hidden=tensor(&[1,1,32],102);
    let keys=tensor(&[1,2,3,8],103);let values=tensor(&[1,2,3,8],104);
    for value in [&embedding,&hidden,&keys,&values] {value.as_array().evaluated().unwrap();}
    let mut actual_state=state(&hidden,&keys,&values);
    let mut reference_state=state(&hidden,&keys,&values);
    let reference=DraftStep::execute::<MlxNeuralBackend,ConcatKeyValueCache>(&mut module,
        DraftStepArguments {embedding:&embedding,state:&mut reference_state},stream).unwrap();
    let expected=reference.as_array().evaluated().unwrap().try_to_vec::<f32>().unwrap();
    let expected_hidden=reference_state.hidden.as_array().evaluated().unwrap().try_to_vec::<f32>().unwrap();
    ...
    let mut binding=BindQuote {sources:&sources.source,index:0,context:&context};
    quoted.visit_parameters_mut(&mut binding);assert_eq!(binding.index,sources.source.len());
    let arguments=DraftStepArguments {embedding:&embedding,state:&mut actual_state};
    let mut projected=DraftStep::project(&arguments,|v|Ok(project(v.as_array(),&context)),&context).unwrap();
    ...
    let mut closing=Vec::new();DraftStep::visit_projected(&projected,&mut |v|closing.push(v.clone()));
    assert_eq!(opening.len(),4);assert_eq!(closing.len(),4);closing.push(out);
    let report=context.report(&closing).unwrap();
    assert_eq!(report.operations.iter().any(|op|matches!(op.kind,
        WorkspaceOperationKind::MaskedOutputProjection{..})),ordered);
    ...
    assert_eq!(reduced.validation_roots,0);assert_eq!(reduced.nested_completions,0);
    ...
    let graph_bytes=usize::try_from(graph_capacity::ResidentGraphStorage::for_completion(completion)
    ...
    let graph=PreparedSubmissionGraphQuota::try_new(graph_bytes,owner.clone()).unwrap().try_allocate().unwrap();
    let records=PreparedSubmissionRecordQuota::try_new(record_bytes,owner.clone()).unwrap().try_allocate().unwrap();
    ...
    roots.bind_scope(&scope).unwrap();scope.enable_original_native_controls().unwrap();
    scope.bind_original_buffer_budget(&budget).unwrap();
    let observer=OriginalScopeObserver::require_current().unwrap();
    for (_,leaf) in &sources.source {OperationEvent::validate_traversal_leaf(leaf,&observer).unwrap();}
    for value in [&embedding,&hidden,&keys,&values] {OperationEvent::validate_traversal_leaf(value.as_array(),&observer).unwrap();}
    let bank=OperationEvent::prepare_resident_graph(completion.graph,&observer).unwrap();
    let output=DraftStep::execute_with_metadata::<MlxNeuralBackend,ConcatKeyValueCache>(&mut module,
        DraftStepArguments {embedding:&embedding,state:&mut actual_state},stream,&context).unwrap();
    drop(bank);
    for value in [&embedding,&actual_state.hidden,&keys,&values,&output] {roots.append(value.as_array()).unwrap();}
    ...
    assert!(!observer.status().failed());assert_eq!(actual_state.kv_offset,4);
    ...
    assert!(!released.load(Ordering::SeqCst),"escaped logits/state retain original native custody");
    compare(output.as_array(),&expected);compare(actual_state.hidden.as_array(),&expected_hidden);
    drop(output);safemlx::reclaim_allocation_owners();
    assert!(!released.load(Ordering::SeqCst),"the actual updated hidden state owns its backing independently");
    drop(actual_state);
