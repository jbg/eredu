...
impl CpuCaptureLoan<'_> {
    pub(crate) fn validate(&self) -> Result<(), CaptureTensorNativeError> {
        ...
        if self.stream.device_type()? != safemlx::DeviceType::Cpu
            ...
            || self.recipe.kernels != 0
            || completion.dispatch.is_none_or(|dispatch| {
                dispatch.cpu_model.is_none()
        ...
        if !self.observer.same_scope(&current) {
            return Err(CaptureTensorNativeError::ClaimMismatch);
        }
        Ok(())
    }
...
impl SpeculativeNumericalRecipe {
    pub(crate) fn cpu_capture_loan<'a>(
        &'a self,
        stream: &'a Stream,
        observer: &'a OriginalScopeObserver,
    ...
    pub(crate) fn inspect_cpu_equations(
    ...
    pub(crate) fn inspect_cpu_capture(
        report: &WorkspaceTraceReport,
        roots: usize,
        nested: usize,
        ordinary: MlxMetalWorkspaceMechanisms,
        ...
        if roots == 0 {
            return Err(
                context.metadata_error(format_args!("CPU numerical capture source is incomplete"))
            );
        }
    ...
    pub(crate) fn inspect_cpu_outputs(
        report: &WorkspaceTraceReport,
        outputs: usize,
        ordinary: MlxMetalWorkspaceMechanisms,
        cpu: MlxCpuWorkspaceMechanisms,
        ...
        if outputs == 0 {
            return Err(
                context.metadata_error(format_args!("CPU numerical stage has no output roots"))
            );
        }
        Self::inspect_cpu_report(report, outputs, 0, 0, ordinary, cpu, context)
    }

    fn inspect_cpu_report(
        report: &WorkspaceTraceReport,
        outputs: usize,
        roots: usize,
        nested: usize,
        ...
        let invalid =
            || context.metadata_error(format_args!("CPU numerical equation source is incomplete"));
        ...
        let (traversal, dispatch, frontier_controls) = super::super::capture::extend_frontier(
            reduced.traversal.ok_or_else(invalid)?,
            graph_source,
            dispatch,
            roots,
        )
        ...
        let frames = [
            ...
            size_of::<ResidentDispatchPopulation>() * 2,
            ...
            size_of::<usize>(),
            CpuCaptureLoan::control_bytes().ok_or_else(invalid)?,
        ...
        let controls = frames
            ...
            .and_then(|n| n.checked_add(frontier_controls))
            .ok_or_else(invalid)?;
...
mod tests {
    ...
    fn cpu_raw_capture_joins_mask_sources_and_each_actual_completion_frontier() {
        ...
        let input = WorkspaceTensor::existing_with_storage(
            context
                .layout(&[1, 8], WorkspaceDtype::Float32)
                .unwrap()
        ...
        let full = context
            .execute(
                ...
                &[&row],
                vec![row.layout().clone()],
            )
            .unwrap()
            .remove(0);
        let slice = row
            .static_slice(&[0, 0, 2], &[1, 1, 6], &[1, 1, 1], &context)
            .unwrap();
        let slice = context
            .execute(
                WorkspaceOperationKind::Elementwise("capture_cast_f32"),
                &[&slice],
                vec![slice.layout().clone()],
            )
            .unwrap()
            .remove(0);
        let output = context
            .execute(
                WorkspaceOperationKind::Sampling(WorkspaceSamplingOperation::TokenFilter),
                &[&input],
        ...
        let recipe = SpeculativeNumericalRecipe::inspect_cpu_capture(
            &report, 11, 2, ordinary, cpu, &context,
        )
        .unwrap();
        ...
        assert_eq!(state.retained_bytes, report.retained_bytes);
        ...
        context.begin_span();
        let stepped = row
            .static_slice(&[0, 0, 0], &[1, 1, 8], &[1, 1, 2], &context)
            .unwrap();
        let report = context.finish_report(&[stepped]).unwrap();
        assert!(SpeculativeNumericalRecipe::inspect_cpu_capture(
            &report, 6, 1, ordinary, cpu, &context
        )
        .is_err());
    }
    #[test]
    fn cpu_preview_retains_flatten_and_prefix_sources() {
        ...
        let input = WorkspaceTensor::existing_with_storage(
            context
                .layout(&[1, 1, 8], WorkspaceDtype::Float32)
                .unwrap()
                .with_representation(Some(WorkspaceRepresentation::new(
                    WorkspaceFloatingType::Float32,
                    true,
        ...
        context.begin_state_span([&input]).unwrap();
        let rectangle = input
            .static_slice(&[0, 0, 1], &[1, 1, 7], &[1, 1, 1], &context)
            .unwrap();
        ...
        let output = context
            .execute(
                WorkspaceOperationKind::Elementwise("capture_cast_f32"),
                &[&prefix],
                vec![prefix.layout().clone()],
            )
            .unwrap()
            .remove(0);
        let report = context.finish_report(&[output]).unwrap();
        let recipe =
            SpeculativeNumericalRecipe::inspect_cpu_capture(&report, 6, 1, ordinary, cpu, &context)
                .unwrap();
        assert_eq!(recipe.kernels, 0);
        assert_eq!(recipe.completion.nested_completions, 1);
        assert_eq!(
            recipe
                .completion
                .dispatch
        ...
        assert_eq!(prefix.shape(), [3]);
        assert!(prefix.layout().representation().unwrap().row_contiguous());
        let WorkspaceOperationKindView::StaticSlice {
            starts,
            ends,
            strides,
        } = report.operations[2].as_view().kind
        else {
            panic!("preview lost exact prefix coordinates")
        };
        assert_eq!(
            (starts, ends, strides),
            ([0].as_slice(), [3].as_slice(), [1].as_slice())
        );
    }
}
