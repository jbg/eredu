...
pub(super) fn extend(
    ...
    let primitives = 18usize.checked_add(top_k.checked_mul(8)?)?.checked_mul(calls)?;
    let edges = 21usize.checked_add(top_k.checked_mul(9)?)?.checked_mul(calls)?;
    ...
    value.maximum_births = value.maximum_births.checked_add(primitives)?
        .checked_add(seeds)?.checked_add(calls.checked_mul(3)?)?;
    ...
    value.additional_sort_kernels = Some(value.additional_sort_kernels?
