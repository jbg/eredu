...
impl ResidentNativeRecipe {
    pub(crate) fn bind_external_target_completion(
        &mut self,
        invocation: eredu_runtime::speculative::external_occurrence::ExternalInvocation,
        roots: Vec<usize>,
        ...
        ) || self.plan().geometry() != invocation.geometry()
            || self.records.len() != 1
            || roots.len() != 1
            || roots[0] == 0
            || self.records[0].prediction_roots.is_some()
        {
    ...
    pub(crate) fn external_equation_completion(
        &self,
    ...
    pub(crate) fn external_equation_domains(
        &self,
