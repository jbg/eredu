...
use crate::{
    backend::{
        ...
        runtime::{
            cache::kv::ConcatKeyValueCache,
            checkpoint::binding::populate_module_from_arrays_excluding,
        },
...
use eredu_architectures::{
    ...
    muse_glimmer::{
        assistant::{
            invocation::{
                FusedProposal, FusedProposalArguments, RawContext, RawContextArguments,
                UpdateContext, UpdateContextArguments,
            },
            DFlash, DFlashConfig,
        },
        DecoderConfig,
    },
    ExternalAssistantArchitecture, ExternalAssistantTargetProfile,
    MaterializedExternalAssistantVisitor,
};
use eredu_nn::{ParameterMetadata, ParameterVisitorMut, Parameterized, Tensor};
use safemlx::{
    Array, DeviceType, Dtype, OperationEvent, OriginalBufferBudget, OriginalScopeObserver,
    PrefillRoots, PrefillRootsRuntime, PreparedOriginalBufferBudget, PreparedPipelineCachePlan,
...
fn tensor(shape: &[i32], phase: usize) -> MlxTensor {
    ...
    let data = (0..count)
        .map(|i| half::bf16::from_f32((((i * 13 + phase * 7) % 31) as f32 - 15.0) * 0.025))
        .collect::<Vec<_>>();
...
fn compare(actual: &Array, expected: &[f32]) {
    ...
    for (a, b) in actual.into_iter().zip(expected) {
        assert!(
            a.is_finite() && (a - b).abs() <= 0.02 + 0.02 * b.abs(),
            "actual {a}, ordinary {b}"
        );
    }
}
fn project(value: &Array, context: &WorkspaceContext) -> WorkspaceTensor {
    ...
    let floating = match descriptor.facts().dtype() {
        ...
        other => panic!("unexpected released source dtype {other:?}"),
    };
    ...
    WorkspaceTensor::existing(
        context
            ...
            .with_representation(Some(WorkspaceRepresentation::new(
                floating,
                descriptor.row_contiguous().unwrap(),
            ))),
        context,
    )
    .unwrap()
...
struct BindQuote<'a> {
    sources: &'a BTreeMap<String, Array>,
    count: usize,
...
impl<'a> ParameterVisitorMut<'a, WorkspaceTensor> for BindQuote<'_> {
    fn visit_mut(
        ...
        let source = self
            ...
            .expect("actual retained parameter binding");
...
struct Invoke<'a> {
    ...
    stream: &'a Stream,
    allocator: &'a safemlx::PreparedInputRuntime,
}
impl MaterializedExternalAssistantVisitor<MlxAssistantPreparationVisitor> for Invoke<'_> {
    ...
    fn visit<A: ExternalAssistantArchitecture>(self, assistant: &mut MlxExternalAssistant<A>) {
        assert_eq!(
            A::configuration_model_type(&assistant.config),
        ...
        populate_module_from_arrays_excluding(&mut module, assistant.source.values(), |_| false)
            .unwrap();
        let sources = assistant.source.values();
        assert!(!sources.is_empty());
        let taps = (0..self.config.target_layer_ids.len())
            .map(|i| tensor(&[1, 2, self.config.hidden_size], 101 + i))
            .collect::<Vec<_>>();
        let embeddings = tensor(
            &[1, self.config.block_size as i32, self.config.hidden_size],
            211,
        );
        let raw = RawContext::execute::<MlxNeuralBackend, ConcatKeyValueCache>(
            &mut module,
            RawContextArguments {
                previous: None,
                states: &taps,
        ...
        .unwrap();
        let committed = UpdateContext::execute::<MlxNeuralBackend, ConcatKeyValueCache>(
            &mut module,
            UpdateContextArguments {
                previous: None,
                pending: &raw,
                absolute_end: 2,
            },
        ...
        let arguments = FusedProposalArguments {
            embeddings: &embeddings,
            committed: &committed,
            absolute_end: 2,
        };
        FusedProposal::visit_arguments(&arguments, &mut |v| {
            v.as_array().evaluated().unwrap();
        });
        let reference = FusedProposal::execute::<MlxNeuralBackend, ConcatKeyValueCache>(
            &mut module,
            FusedProposalArguments {
                embeddings: &embeddings,
                committed: &committed,
                absolute_end: 2,
            },
        ...
        let mut projected = FusedProposal::project(
            &arguments,
        ...
        FusedProposal::visit_projected(&projected, &mut |v| opening.push(v.clone()));
        context.begin_state_span(&opening).unwrap();
        let traced = FusedProposal::trace(&mut quoted, &mut projected, &context).unwrap();
        let retained = opening.len();
        let mut closing = Vec::new();
        FusedProposal::visit_projected(&projected, &mut |v| closing.push(v.clone()));
        closing.push(traced);
        ...
        assert_eq!(
            reduced.first_missing_operation, None,
            "released fused source must be complete"
        );
        assert_eq!(reduced.validation_roots, 0);
        assert_eq!(
            reduced.nested_completions, 0,
            "short fused block has no explicit tiled completion"
        );
        ...
        let released: [Arc<AtomicBool>; 6] =
            std::array::from_fn(|_| Arc::new(AtomicBool::new(false)));
        let owners = released
            .each_ref()
            .map(|flag| Arc::new(Lifetime(flag.clone())));
        let retired = || released.iter().all(|flag| flag.load(Ordering::SeqCst));
        let graph = PreparedSubmissionGraphQuota::try_new(graph_bytes, owners[0].clone())
            .unwrap()
            .try_allocate()
            .unwrap();
        ...
        let pipeline_plan = PreparedPipelineCachePlan::new(
            completion
                .dispatch
                .expect("Metal dispatch source")
                .kernel_attempts,
        );
        context
            .charge_metadata(
            ...
            .unwrap();
        let pipeline = pipeline_plan.realize(owners[5].clone()).unwrap();
        pipeline.install(&graph).unwrap();
        let records = PreparedSubmissionRecordQuota::try_new(record_bytes, owners[1].clone())
            .unwrap()
            .try_allocate()
            .unwrap();
        let budget =
            PreparedOriginalBufferBudget::try_new(self.allocator, physical, owners[2].clone())
                .unwrap()
                .try_allocate()
                .unwrap();
        let failure = PreparedPrefillFailure::try_new(owners[3].clone())
            .unwrap()
            .try_allocate()
            .unwrap();
        let runtime = PrefillRootsRuntime::prepare_for_stream(self.stream, self.stream).unwrap();
        let mut roots =
            PrefillRoots::new_retained(&runtime, closing.len(), &graph, &failure).unwrap();
        let mut scope = SubmissionScope::try_begin_retaining(
            PreparedSubmissionScopeOwner::try_new(owners[4].clone())
                .unwrap()
        ...
        FusedProposal::visit_arguments(&arguments, &mut |v| {
            OperationEvent::validate_traversal_leaf(v.as_array(), &observer).unwrap()
        });
        let bank = OperationEvent::prepare_resident_graph(completion.graph, &observer).unwrap();
        let output = FusedProposal::execute_with_metadata::<MlxNeuralBackend, ConcatKeyValueCache>(
            &mut module,
            FusedProposalArguments {
                embeddings: &embeddings,
                committed: &committed,
        ...
        .unwrap();
        drop(bank);
        FusedProposal::visit_arguments(&arguments, &mut |v| roots.append(v.as_array()).unwrap());
        roots.append(output.as_array()).unwrap();
        roots
            .complete_current_scope_on_stream_prepared(self.stream, &completion.traversal)
        ...
        loop {
            ...
            assert!(
                ...
                "released Muse terminal observation: {status:?}"
            );
            if progress == safemlx::ScopedSubmissionProgress::Observed && status.is_settled() {
                break;
            }
            assert!(
                std::time::Instant::now() < deadline,
                "released Muse terminal observation: {progress:?}, {status:?}"
            );
        ...
        .unwrap();
        drop((owners, module, committed, raw, taps, embeddings));
        safemlx::reclaim_allocation_owners();
        assert!(
            !retired(),
            "escaped fused output retains actual native custody"
        );
        compare(output.as_array(), &expected);
        drop(output);
        loop {
...
fn released_muse_fused_source_matches_ordinary_and_retains_escaped_output() {
    let path = std::path::PathBuf::from(
        std::env::var_os("EREDU_RELEASED_MUSE_ASSISTANT").expect("released assistant path"),
    );
    let target_path =
        std::env::var_os("EREDU_RELEASED_MUSE_TARGET_CONFIG").expect("released target config path");
    let config =
        DFlashConfig::from_hf_json(&std::fs::read(path.join("config.json")).unwrap()).unwrap();
    config.validate_released().unwrap();
    let target = ExternalAssistantTargetProfile::MuseGlimmer(
        DecoderConfig::from_hf_json(&std::fs::read(target_path).unwrap()).unwrap(),
    );
    let selected = eredu_architectures::prepare_external_assistant(&path)
        .unwrap()
        .select_materialization(
            None,
            eredu_checkpoint::store::DEFAULT_MAX_CACHED_SHARDS,
            MlxAssistantPreparationVisitor::lowering_for_native_source_test,
        )
        .unwrap()
    ...
    let identity =
        MlxDeviceIdentity::from_realized_device(&device, Some(MlxAcceleratorFamily::Metal))
            .unwrap();
    ...
    let mut materialized = prepared
        .visit(MlxAssistantPreparationVisitor::for_native_source_test(
            environment.stream(),
            &pool,
        ))
        .unwrap();
    materialized.visit(Invoke {
        config: &config,
