...
fn source(group_size: i32, supplied: bool) -> WorkspaceOperation {
    ...
    let spec = TopKGroupSelectorSpec::new(
        ...
        .unwrap(),
        TopKGroupSelectionSpec::new(4, 1, GroupScoring::Sigmoid, true)
            .unwrap()
            .with_weight_policy(1e-6, 1.0)
            .unwrap(),
    )
    .unwrap()
    ...
    let input = WorkspaceTensor::existing(
        WorkspaceLayout::new(&[1, 2, 64], WorkspaceDtype::Float32).unwrap(),
        &context,
    )
    .unwrap();
...
fn affine_selector_reuses_projection_source_and_preserves_replacement_branch() {
    ...
    for group_size in [16, 64] {
        for supplied in [false, true] {
            ...
            assert_eq!(
                packed.helper_controls, 0,
                "packed QMM does not probe BF16 matmul"
            );
            ...
            missing.inputs[weight + 1] =
                missing.inputs[weight + 1].clone().with_representation(None);
            assert!(
                lowering(missing.as_view()).is_none(),
                "packed scales need actual precision"
            );
            let mut malformed = operation.clone();
            malformed.inputs[weight + 2] = WorkspaceLayout::new(&[4, 3], WorkspaceDtype::Float32)
                .unwrap()
                .with_representation(Some(WorkspaceRepresentation::new(
                    WorkspaceFloatingType::Float32,
                    true,
            ...
            assert!(
                lowering(malformed.as_view()).is_none(),
                "affine bias is a physical companion"
            );
            let mut replacement = missing;
            replacement.inputs[weight] = WorkspaceLayout::new(&[4, 64], WorkspaceDtype::Float32)
                .unwrap()
                .with_representation(Some(WorkspaceRepresentation::new(
                    WorkspaceFloatingType::Float32,
                    false,
                )));
            replacement.inputs[weight + 2] = replacement.inputs[weight + 2]
                .clone()
                .with_representation(None);
            let dense = lowering(replacement.as_view()).unwrap();
            assert_eq!(
                dense.helper_controls,
                crate::backend::nn::matrix::row_projection_probe_control_bytes().unwrap()
            );
            assert!(
                mechanism.operation_bound(&replacement).unwrap().is_some(),
                "floating replacement must use matrix storage, ignoring unused packed companions"
            );
            super::super::super::routing::with_selector_projection(
                replacement.as_view(),
                |projection| {
                    assert_eq!(
                        projection.inputs.len(),
                        3,
                        "input, actual weight and ordinary bias"
                    );
                    ...
                    assert!(
                        matches!(projection.kind, WorkspaceOperationKindView::Projection(format)
