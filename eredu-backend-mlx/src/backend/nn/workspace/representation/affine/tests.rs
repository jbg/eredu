...
fn affine_projection_requires_actual_companion_sources_and_keeps_replacement_branch() {
    for (input, scale, bias, expected) in [
        ...
        for ordinal in [0, 2, 3] {
            ...
            assert_eq!(
                scalar(&unknown),
                None,
                "missing actual floating operand {ordinal}"
            );
        }
    }
    ...
    assert_eq!(
        scalar(&op),
        Some(F::Float16),
        "unused packed companions cannot widen replacement matmul"
    );
    op.inputs.push(floating_layout(&[32], Some(F::Float32)));
    assert_eq!(
        scalar(&op),
        Some(F::Float32),
        "ordinary output bias participates after projection"
    );
}
...
fn affine_projection_refuses_wrong_roles_and_preserves_empty_leading_dimensions() {
    ...
    for (ordinal, replacement) in [
        (
            1,
            WorkspaceLayout::new(&[32, 9], WorkspaceDtype::Uint32).unwrap(),
        ),
        (2, floating_layout(&[32, 2], Some(F::Float32))),
        (
            3,
            WorkspaceLayout::new(&[32, 1], WorkspaceDtype::Uint32).unwrap(),
        ),
    ] {
        let mut changed = original.clone();
        changed.inputs[ordinal] = replacement;
