...
fn native_concatenate_preserves_alias_layout_and_materializes_canonical_rows() {
        ...
        for empty in [false, true] {
            for transposed in [false, true] {
                ...
                let mut input = Array::from_slice(data, &[if empty { 0 } else { 2 }, 3])
                    ...
                    .unwrap();
                if transposed {
                    input = input.transpose_axes(&[1, 0], &stream).unwrap();
                }
                ...
                for count in [1, 2] {
                    ...
                    let mut projection =
                        ExistingArrayProjection::with_source_count(&context, 2).unwrap();
                    let source = projection.project(&input).unwrap();
                    let expected = projection.project(&actual).unwrap();
                    let symbolic =
                        WorkspaceTensor::concatenate(&vec![source.clone(); count], 0, &context)
                            .unwrap();
                    ...
                    let original = crate::MlxTensor::from_array(input.clone())
                        .to_f32_vec(&stream)
                        .unwrap();
    ...
    let unknown = WorkspaceTensor::existing(
        context.layout(&[1, 3], WorkspaceDtype::Float32).unwrap(),
        &context,
    )
    .unwrap();
    for count in [1, 2] {
        let output =
            WorkspaceTensor::concatenate(&vec![unknown.clone(); count], 0, &context).unwrap();
