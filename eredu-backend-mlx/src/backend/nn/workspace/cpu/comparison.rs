//! F32 and integer-scalar comparisons with the existing typed Bool workers.
use super::*;
...
pub(super) fn inspect(
    ...
    if matches!(
        ...
        return scalar::inspect(operation, mechanism);
    }
    let (binary, logical) = match operation.kind {
        WorkspaceOperationKindView::Elementwise("less") => (Some(CpuBinaryOperation::Less), false),
        WorkspaceOperationKindView::Elementwise("less_equal") => {
            (Some(CpuBinaryOperation::LessEqual), false)
        }
        WorkspaceOperationKindView::Elementwise("greater") => {
            (Some(CpuBinaryOperation::Greater), false)
        }
        WorkspaceOperationKindView::Elementwise("greater_equal") => {
            (Some(CpuBinaryOperation::GreaterEqual), false)
        }
        WorkspaceOperationKindView::Elementwise("logical_and") => {
            (Some(CpuBinaryOperation::LogicalAnd), true)
        }
        WorkspaceOperationKindView::Elementwise("logical_or") => {
            (Some(CpuBinaryOperation::LogicalOr), true)
        }
    ...
    if operation.inputs.len() != count || operation.outputs.len() != 1 {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU comparison operand population differs",
        ));
    }
    ...
    if output.dtype() != WorkspaceDtype::Bool || output.shape() != left.shape() {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU comparison changes output geometry or Bool dtype",
        ));
    }
    ...
    for value in operation.inputs.iter() {
        ...
        {
            return Err(MlxWorkspaceFactError::descriptor(
                "CPU comparison changes its source dtype or geometry",
            ));
        }
        if !logical && !strided_capture::f32_source(value) {
            return Ok(None);
        }
    }
    ...
    for value in operation.inputs.iter() {
        if rank != 0 && value.shape().is_empty() {
            let Some(source) = OperationEvent::cpu_broadcast_alias_layout(0, rank, false) else {
                return Ok(None);
            };
    ...
    match binary {
        Some(kind) => {
            let Some(source) =
                OperationEvent::cpu_binary_layout(kind, native, rank, elements, false)
            else {
                return Ok(None);
            };
            if source.backing_births() != 1 || population.binary(source).is_none() {
                return Ok(None);
            }
        }
        None => {
            let Some(source) = OperationEvent::cpu_unary_layout(
            ...
            if source.backing_births() != 1 || population.unary(source).is_none() {
                return Ok(None);
            }
        }
    }
    let frames = [
        strided_capture::control_bytes().ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?,
...
mod tests {
    ...
    fn cpu_histogram_trace_preserves_edges_and_each_scalar_frontier() {
        ...
        for elements in [1, 19, 1025] {
            ...
            let flat = WorkspaceTensor::existing_with_storage(
                context
                    .layout(&[elements], WorkspaceDtype::Float32)
                    .unwrap()
