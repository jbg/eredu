...
pub(super) fn inspect(operation:WorkspaceOperationView<'_>)
    ...
    if !super::super::basic::is_static_slice(operation) {
        return Err(MlxWorkspaceFactError::descriptor("CPU static slice coordinates differ"));
    }
    ...
    let frames=[if representation.is_some() && !empty {
        ...
        size_of::<[i32;4]>(),size_of::<Option<i32>>(),size_of::<&[i32]>()*3,
        size_of::<[i64;4]>(),size_of::<Option<[i64;4]>>(),size_of::<[u64;4]>(),
...
pub(super) fn representation(operation:WorkspaceOperationView<'_>,input:WorkspaceRepresentation)
    ...
    for axis in (0..source.shape().len()).rev() {
        if output.shape()[axis]>1 {
            ...
            if selected[axis]!=dense_stride {rows=false;}
        }
    ...
    WorkspaceRepresentation::new(input.dtype(),rows)
        ...
        .with_element_strides(&selected[..source.shape().len()]).expect("qualified bounded slice strides")
}
...
mod tests {
    ...
    fn capture_slice_preserves_exact_coordinates_source_alias_and_row_gaps() {
        ...
        for dtype in [WorkspaceFloatingType::Float32,WorkspaceFloatingType::Float16,WorkspaceFloatingType::Bfloat16] {
            ...
            let storage=eredu_nn::workspace::WorkspaceExistingStorage::try_new(Some(4096),&context).unwrap();
            ...
            let selected=source.static_slice(&[0,1,2],&[2,3,6],&[1,1,1],&context).unwrap();
            assert_eq!(selected.shape(),[2,2,4]);
            let rep=selected.layout().representation().unwrap();
            assert_eq!(rep.dtype(),dtype);assert!(!rep.row_contiguous());assert!(rep.last_axis_contiguous());
            ...
            let op=report.operations[0].as_view();let plan=cpu.plan(op).unwrap().unwrap();
            assert_eq!(plan.alias_input,Some(0));assert_eq!(plan.population.births,0);
            assert_eq!(plan.population.primitives,1);assert_eq!(plan.seeds,0);
            assert_eq!(plan.validations,0);assert_eq!(plan.output_bytes,0);assert_eq!(plan.scratch_bytes,0);
            assert!(plan.population.extents>0 && plan.population.controls>0);
            let WorkspaceOperationKindView::StaticSlice {starts,ends,strides}=op.kind else {panic!("slice coordinates lost");};
            assert_eq!(starts,[0,1,2]);assert_eq!(ends,[2,3,6]);assert_eq!(strides,[1,1,1]);
            // A single row's contiguous interval remains row-major despite its
            // offset within the unchanged larger backing owner.
            let row=source.static_slice(&[1,2,2],&[2,3,6],&[1,1,1],&context).unwrap();
            assert!(row.layout().representation().unwrap().row_contiguous());
        }
        let source=WorkspaceLayoutView::new(&[2,3,7],WorkspaceDtype::Float32).unwrap()
        ...
        let outputs=[output];let inputs=[source];
        let base=WorkspaceOperationView {kind:WorkspaceOperationKindView::StaticSlice {
            starts:&[0,1,2],ends:&[2,3,6],strides:&[1,1,1]},
            inputs:WorkspaceLayoutList::Views(&inputs),outputs:WorkspaceLayoutList::Views(&outputs)};
        let malformed=WorkspaceOperationView {kind:WorkspaceOperationKindView::StaticSlice {
            starts:&[0,1],ends:&[2,3,6],strides:&[1,1,1]},..base};
        assert!(cpu.plan(malformed).is_err());
        let erased=[source.with_representation(None)];
        assert!(cpu.plan(WorkspaceOperationView {inputs:WorkspaceLayoutList::Views(&erased),..base}).unwrap().is_none());
        let strided=[source.with_representation(Some(WorkspaceRepresentation::new(WorkspaceFloatingType::Float32,false)))];
        assert!(cpu.plan(WorkspaceOperationView {inputs:WorkspaceLayoutList::Views(&strided),..base}).unwrap().is_none());
        let step_output=[WorkspaceLayoutView::new(&[2,2,2],WorkspaceDtype::Float32).unwrap()];
        let stepped=WorkspaceOperationView {kind:WorkspaceOperationKindView::StaticSlice {
            starts:&[0,1,2],ends:&[2,3,6],strides:&[1,1,2]},outputs:WorkspaceLayoutList::Views(&step_output),..base};
        let selected=cpu.plan(stepped).unwrap().unwrap();assert_eq!(selected.alias_input,Some(0));
        let facts=cpu.output_representation(stepped,0).unwrap();
        assert!(!facts.row_contiguous());assert!(!facts.last_axis_contiguous());
        let generic=WorkspaceOperationView {kind:WorkspaceOperationKindView::Index {selected_axes:0},..base};
    ...
    fn positive_capture_slice_quotes_exact_steps_and_preserves_cast_alias() {
        ...
        for (ends,steps,shape,row,last) in [
            ...
            let context=WorkspaceContext::new(cpu);
            let source=WorkspaceTensor::existing(context.layout(&[2,1,4],WorkspaceDtype::Float32).unwrap()
                .with_representation(Some(WorkspaceRepresentation::new(WorkspaceFloatingType::Float32,true))),&context).unwrap();
            context.begin_state_span([&source]).unwrap();
            let selected=source.static_slice(&[0,0,0],&ends,&steps,&context).unwrap();assert_eq!(selected.shape(),shape);
            let representation=selected.layout().representation().unwrap();
            assert_eq!((representation.row_contiguous(),representation.last_axis_contiguous()),(row,last));
            let mut outputs=context.metadata_vec(1).unwrap();
            outputs.push(context.layout(&shape,WorkspaceDtype::Float32).unwrap());
            let output=context.execute(WorkspaceOperationKind::Elementwise("capture_cast_f32"),
                &[&selected],outputs).unwrap().remove(0);
            assert_eq!(output.layout().representation(),Some(representation));
            let report=context.report(&[output]).unwrap();assert!(report.unpriced_operations.is_empty());
            assert!(report.unpriced_host_operations.is_empty());assert_eq!(report.tensor_buffers.total_bytes,Some(0));
            let plan=cpu.plan(report.operations[0].as_view()).unwrap().unwrap();
            assert_eq!((plan.population.primitives,plan.population.births,plan.alias_input),(1,0,Some(0)));
        }
    }
    #[test]
    fn stepped_preview_flatten_uses_actual_native_alias_or_copy_and_retains_strides() {
        ...
        for (width,copied) in [(4,false),(5,true)] {
            ...
            let source=WorkspaceTensor::existing_with_storage(context.layout(&[2,1,width],WorkspaceDtype::Float32).unwrap()
                .with_representation(Some(WorkspaceRepresentation::new(WorkspaceFloatingType::Float32,true))),&storage,&context).unwrap();
            context.begin_state_span([&source]).unwrap();
            let selected=source.static_slice(&[0,0,0],&[2,1,3],&[1,1,2],&context).unwrap();
            ...
            assert_eq!(flat_rep.element_stride_at(1,0),if copied{None}else{Some(2)});
            let preview=flat.static_slice(&[0],&[3],&[1],&context).unwrap();
            let mut outputs=context.metadata_vec(1).unwrap();outputs.push(context.layout(&[3],WorkspaceDtype::Float32).unwrap());
            let output=context.execute(WorkspaceOperationKind::Elementwise("capture_cast_f32"),&[&preview],outputs).unwrap().remove(0);
            assert_eq!(output.layout().representation(),preview.layout().representation());
            ...
            let erase=[report.operations[1].as_view().inputs.get(0).unwrap().with_representation(Some(
                WorkspaceRepresentation::new(WorkspaceFloatingType::Float32,false)))];
            let operation=WorkspaceOperationView{inputs:WorkspaceLayoutList::Views(&erase),..report.operations[1].as_view()};
            assert!(cpu.plan(operation).unwrap().is_none(),"shape alone cannot recover selected strides");
        }
    }
    #[test]
    fn narrow_axis_quotes_exact_multi_position_batched_slice_and_preserves_custody() {
        ...
        for (start,end) in [(3,5),(4,5),(0,5)] {
            let context=WorkspaceContext::new(cpu);
            let storage=WorkspaceExistingStorage::try_new(Some(4096),&context).unwrap();
            let source=WorkspaceTensor::existing_with_storage(context.layout(&[2,5,3],WorkspaceDtype::Float32).unwrap()
                .with_representation(Some(WorkspaceRepresentation::new(WorkspaceFloatingType::Float32,true))),&storage,&context).unwrap();
            context.begin_state_span([&source]).unwrap();
            ...
            let WorkspaceOperationKindView::StaticSlice {starts,ends,strides}=report.operations[0].as_view().kind else {
                panic!("narrow axis lost exact coordinates");
            };
            assert_eq!(starts,[0,start,0]); assert_eq!(ends,[2,end,3]); assert_eq!(strides,[1,1,1]);
            assert_eq!(report.state.as_ref().unwrap().retained_bytes,Some(4096));
            assert_eq!(report.state.as_ref().unwrap().displaced_bytes,Some(0));
            assert_eq!(report.tensor_buffers.total_bytes,Some(0));
            assert!(report.unpriced_operations.is_empty()); assert!(report.unpriced_host_operations.is_empty());
            let plan=cpu.plan(report.operations[0].as_view()).unwrap().unwrap();
            assert_eq!(plan.population.births,0); assert_eq!(plan.alias_input,Some(0));
            assert_eq!(plan.population.primitives,usize::from(start!=0||end!=5));
            let recipe=SpeculativeNumericalRecipe::inspect_cpu_capture(&report,2,0,ordinary,cpu,&context).unwrap();
