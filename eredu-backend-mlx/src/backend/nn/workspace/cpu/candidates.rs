...
impl Program {
    ...
    fn copy(&mut self, source: CpuCopyEvalLayout, inputs: usize, bytes: u64) -> Option<()> {
        self.bytes = self
            .bytes
            .checked_add(bytes.checked_mul(u64::try_from(source.backing_births()).ok()?)?)?;
...
pub(super) fn inspect(
    ...
    let Some(geometry) = super::super::candidates::geometry(operation)? else {
        return Ok(None);
    };
    ...
    if input.representation() != expected || input.elements()? > i32::MAX as u64 {
        return Ok(None);
    }
    let width = geometry.vocabulary as usize;
    let count = geometry.count as usize;
    let shape = [geometry.vocabulary as i32];
    let row =
        WorkspaceLayoutView::new(&shape, WorkspaceDtype::Float32)?.with_representation(expected);
    ...
    let finite = child(
        WorkspaceOperationKindView::Elementwise("is_finite"),
        row,
        mask,
    )?;
    let cast = child(
        WorkspaceOperationKindView::Elementwise("bool_to_u32"),
        mask,
        unsigned,
    )?;
    let sum = child(
        WorkspaceOperationKindView::Reduction("sum", 0, false),
        unsigned,
        scalar,
    )?;
    let (Some(finite), Some(cast), Some(sum)) = (finite, cast, sum) else {
        return Ok(None);
    };
    let sorted_bytes = mechanism
        .allocation
        .fixed_buffer_capacity(u64::from(geometry.vocabulary) * 4)?;
    let selected_bytes = mechanism
        .allocation
        .fixed_buffer_capacity(u64::from(geometry.count) * 4)?;
    ...
    let source = (|| {
        ...
        p.child(sum)?;
        p.copy(
            OperationEvent::cpu_argsort_layout(Dtype::Float32, 1, width, 1, false)?,
            1,
            sorted_bytes,
        )?;
        if count < width {
            p.copy(OperationEvent::cpu_slice_layout(1, false, false)?, 1, 0)?;
        }
        ...
        p.copy(
            OperationEvent::cpu_contiguous_layout(1, false)?,
            1,
            selected_bytes,
        )?;
    ...
    let output_bytes = selected_bytes
        .checked_mul(2)
    ...
    let frames = [
        ...
        size_of::<super::super::candidates::Geometry>(),
        size_of::<Option<super::super::candidates::Geometry>>(),
        ...
        size_of::<OperationPlan>() * 4,
        size_of::<Option<OperationPlan>>() * 3,
        size_of::<(
        ...
        size_of::<u64>() * 4,
        size_of_val(&child),
        size_of::<(&mut Program, OperationPlan)>(),
        size_of::<(&mut Program, CpuCopyEvalLayout, usize, u64)>(),
        size_of::<(
            WorkspaceOperationKindView<'_>,
            WorkspaceLayoutView<'_>,
            WorkspaceLayoutView<'_>,
        )>(),
        size_of::<&mut Program>(),
        size_of::<WorkspaceLayoutList<'_>>(),
        size_of_val(&operation.outputs.iter()),
    ];
    p.population.controls = frames.into_iter().try_fold(
        p.population
            .controls
    ...
    Ok(Some(OperationPlan {
        ...
        parameter_shells: p
            .shells
            .checked_add(crate::backend::array_copy::CandidateExtraction::ROOTS)
            .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?,
        seeds: p.seeds,
        validations: 0,
    }))
    ...
    fn cpu_candidate_composite_preserves_two_outputs_full_sort_and_three_reads() {
        ...
        for (width, count, rows) in [(1, 1, 1), (19, 5, 2), (4097, 3, 1), (4097, 4097, 1)] {
            ...
            let input = WorkspaceTensor::existing_with_storage(
                context
                    .layout(&[1, rows, width], WorkspaceDtype::Float32)
                    .unwrap()
            ...
            let report = context
                .finish_report(&[input.clone(), output[0].clone(), output[1].clone()])
                .unwrap();
            assert!(
            ...
            assert_eq!(
                ...
                2 * ordinary
                    .allocation()
                    .fixed_buffer_capacity(count as u64 * 4)
                    .unwrap()
            );
            assert!(
                plan.scratch_bytes
            ...
            assert_eq!(
                ...
                report.retained_bytes.map(|bytes| bytes + 65536)
            );
        ...
        let input = WorkspaceTensor::existing(
            context
                .layout(&[1, 1, 19], WorkspaceDtype::Float32)
                .unwrap(),
            &context,
        )
        .unwrap();
        let output = CandidateExtraction::borrowed(&[1, 1, 19], 5)
            .unwrap()
            .trace(&input, &context)
            .unwrap();
