...
fn existing(shape: &[i32], dtype: WorkspaceDtype, context: &WorkspaceContext) -> WorkspaceTensor {
    WorkspaceTensor::existing(
        context.layout(shape, dtype).unwrap().with_representation(
            (dtype == WorkspaceDtype::Float32).then_some(WorkspaceRepresentation::new(
...
fn cpu_paged_stages_preserve_exact_rows_outputs_and_nested_completions() {
    ...
    for arithmetic in [AttentionArithmetic::Fused, AttentionArithmetic::InputScores] {
        for mask_type in [
            None,
            Some(WorkspaceDtype::Bool),
            Some(WorkspaceDtype::Float32),
        ] {
            for heads in [2, 4] {
                for queries in [1, 3] {
                    let end = queries + 4;
                    ...
                    for strided in [false, true] {
                        ...
                        let q = if strided {
                            existing(&[1, queries, heads, 4], WorkspaceDtype::Float32, &context)
                                .transpose_axes(&[0, 2, 1, 3], &context)
                                .unwrap()
                        } else {
                            existing(&[1, heads, queries, 4], WorkspaceDtype::Float32, &context)
                        };
                        let mask =
                            mask_type.map(|dtype| existing(&[queries, end], dtype, &context));
                        ...
                        context
                            ...
                            .unwrap();
                        let mut accumulator =
                            WorkspaceBackend::begin_blockwise_attention_with_options(
                                spec, options, &context,
                            )
                            .unwrap();
                        ...
                        assert_eq!(recipe.completion.nested_traversal().unwrap().roots(), 3);
                        assert!(recipe.completion.ordinary_cpu_controls().is_some());
                        ...
                        for operation in &report.operations {
                            ...
                            let calls = cpu
                                ...
                                .unwrap()
                                .expect(
                                "the selected page stage retains its actual ordinary safe callers",
                            );
                            ...
                            match stage {
                                ...
                                Stage::Accumulate { value_pass, .. } => {
                                    ...
                                    if strided {
                                        ...
                                        dense_births.push(plan.population.births);
                                    }
                                    accumulation += 1;
                                    if arithmetic == AttentionArithmetic::InputScores {
                                        let mut views =
                                            operation.as_view().inputs.iter().collect::<Vec<_>>();
                                        views[0] = views[0].with_representation(Some(
                                        ...
                                        assert!(
                                            cpu.plan(WorkspaceOperationView {
                                                inputs: WorkspaceLayoutList::Views(&views),
                                                ..operation.as_view()
                                            })
                                            .unwrap()
                                            .is_none()
                                ...
                                Stage::Finish { input_scores, .. } => {
                                    assert_eq!(plan.alias_input.is_some(), input_scores)
                                }
                            }
                        }
                    }
