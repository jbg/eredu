...
#[derive(Clone, Copy)]
enum Buffer { Index, Mask, Scalar, Value }
impl Population {
    fn births(&mut self, buffer: Option<Buffer>, count: usize) -> Option<()> {
        ...
        *slot=slot.checked_add(count)?;Some(())
    }
    fn copy(&mut self, source: CpuCopyEvalLayout, inputs: usize, buffer: Option<Buffer>) -> Option<()> {
        self.births(buffer,source.backing_births())?;
        self.native.copy(source,inputs)
    }
    fn cast(&mut self, source:Dtype,destination:Dtype,rank:usize,count:usize,buffer:Buffer)->Option<()> {
        self.copy(OperationEvent::cpu_cast_layout(source,destination,rank,count,false)?,1,Some(buffer))
    }
    fn alias(&mut self,rank:usize,output_rank:usize)->Option<()> {
        self.copy(OperationEvent::cpu_broadcast_alias_layout(rank,output_rank,false)?,1,None)
    }
    fn binary(&mut self,kind:CpuBinaryOperation,dtype:Dtype,rank:usize,count:usize,scalar:bool)->Option<()> {
        self.binary_output(kind, dtype, rank, count, scalar, Buffer::Mask)
    }
    fn binary_output(&mut self,kind:CpuBinaryOperation,dtype:Dtype,rank:usize,count:usize,
        scalar:bool, output:Buffer)->Option<()> {
        let input=if dtype==Dtype::Bool {Buffer::Mask} else {Buffer::Index};
        ...
        self.cast(dtype,dtype,if scalar {0}else{rank},if scalar {1}else{count},
            if scalar {Buffer::Scalar}else{input})?;
        self.alias(rank,rank)?;self.alias(if scalar {0}else{rank},rank)?;
...
fn token_validation_population(index_dtype:Dtype,rank:usize,count:usize,sentinel:bool)
    ->Option<Population> {
    let mut source=Population::default();
    source.cast(index_dtype,Dtype::Int32,rank,count,Buffer::Index)?;
    if count!=0 {
        source.binary(CpuBinaryOperation::GreaterEqual,Dtype::Int32,rank,count,true)?;
        source.binary(CpuBinaryOperation::Less,Dtype::Int32,rank,count,true)?;
        source.binary(CpuBinaryOperation::LogicalAnd,Dtype::Bool,rank,count,false)?;
        if sentinel {
            source.binary(CpuBinaryOperation::Equal,Dtype::Int32,rank,count,true)?;
            source.binary(CpuBinaryOperation::LogicalOr,Dtype::Bool,rank,count,false)?;
        }
        source.cast(Dtype::Bool,Dtype::Bool,rank,count,Buffer::Mask)?;
        let not=OperationEvent::cpu_unary_layout(CpuUnaryOperation::LogicalNot,Dtype::Bool,rank,false)?;
        source.births(Some(Buffer::Mask),not.backing_births())?;source.native.unary(not)?;
        if rank!=0 {
    ...
    let frames=[size_of::<(Dtype,usize,usize,bool)>(),size_of::<Population>(),size_of::<Option<Population>>()];
    source.native.controls=frames.into_iter().try_fold(source.native.controls.checked_add(size_of_val(&frames))?,usize::checked_add)?;
    Some(source)
}
...
pub(super) fn inspect_token_validation(operation:WorkspaceOperationView<'_>,mechanism:MlxCpuWorkspaceMechanisms)
    ...
    let [input,output]=super::super::sampling::token_validation_layouts(operation)
        .ok_or_else(||MlxWorkspaceFactError::descriptor("CPU token validation domain or geometry differs"))?;
    let rank=input.shape().len();let count=usize::try_from(input.elements()?)?;
    if rank>4 || count>i32::MAX as usize {return Ok(None);}
    let dtype=if input.dtype()==WorkspaceDtype::Int32 {Dtype::Int32}else{Dtype::Uint32};
    let Some(mut source)=token_validation_population(dtype,rank,count,false) else{return Ok(None)};
    let output_birth=usize::from(dtype!=Dtype::Int32);
    let index_births=source.index_births.checked_sub(output_birth)
        .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?;
    let index_capacity=mechanism.allocation.fixed_buffer_capacity(output.bytes()?)?;
    let mask_capacity=mechanism.allocation.fixed_buffer_capacity(input.elements()?)?;
    ...
    let scratch_bytes=facts::add(facts::add(
        ...
        facts::mul(scalar_capacity,u64::try_from(source.scalar_births.checked_add(seeds)
            .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?)?)?)?;
    let frames=[size_of::<WorkspaceOperationView<'_>>(),size_of::<MlxCpuWorkspaceMechanisms>(),
        size_of::<[WorkspaceLayoutView<'_>;2]>(),size_of::<Option<[WorkspaceLayoutView<'_>;2]>>(),
        size_of::<Population>(),size_of::<Option<Population>>(),size_of::<OperationPlan>(),
        size_of::<facts::FactResult<Option<OperationPlan>>>(),size_of::<(Dtype,usize,usize,bool)>(),
        size_of::<usize>()*5,size_of::<u64>()*4];
...
pub(super) fn inspect(operation:WorkspaceOperationView<'_>,mechanism:MlxCpuWorkspaceMechanisms)
    ...
    let (operation, parallel_policy) = match operation.kind {
        WorkspaceOperationKindView::VocabularyParallelLookup { policy, .. } => {
            let Some(child) = super::super::parallel_lookup::embedding(operation)? else { return Ok(None); };
            (child, Some(policy))
        }
        _ => (operation, None),
    };
    ...
    if format.encoding()!=LinearFormat::Dense {return Ok(None);}
    format.validate_fixed()?;
    if operation.inputs.len()!=2||operation.outputs.len()!=1 {
        return Err(MlxWorkspaceFactError::descriptor("CPU strict lookup population differs"));
    }
    ...
    if rank>2||ids.shape().iter().any(|&n|n<=0)||weight.shape().len()!=2||
        weight.dtype()!=WorkspaceDtype::Float32||weight.shape().iter().any(|&n|n<=0)||
        output.dtype()!=WorkspaceDtype::Float32 {return Ok(None);}
    let Some(representation)=weight.representation() else {return Ok(None)};
    ...
    if !output.shape().iter().copied().eq(ids.shape().iter().copied().chain(std::iter::once(weight.shape()[1]))) {
        return Err(MlxWorkspaceFactError::descriptor("CPU strict lookup output differs"));
    }
    ...
    if count>i32::MAX as usize||source_elements>i32::MAX as usize||output_elements>i32::MAX as usize {return Ok(None);}
    let source=(|| {
        ...
        if parallel {
            ...
            source.binary_output(CpuBinaryOperation::Subtract,Dtype::Int32,rank,count,true,Buffer::Index)?;
            source.cast(Dtype::Bool,Dtype::Bool,rank,count,Buffer::Mask)?;
            source.cast(Dtype::Int32,Dtype::Int32,rank,count,Buffer::Index)?;
            source.cast(Dtype::Int32,Dtype::Int32,0,1,Buffer::Scalar)?;
            source.alias(rank,rank)?;source.alias(rank,rank)?;source.alias(0,rank)?;
            source.copy(OperationEvent::cpu_typed_select_broadcast_layout(Dtype::Int32,rank,count,false)?,
                3,Some(Buffer::Index))?;
        } else {
        ...
        source.cast(Dtype::Int32,Dtype::Int32,rank,count,Buffer::Index)?;
        source.copy(OperationEvent::cpu_gather_layout(native_dtype,Dtype::Int32,2,rank,
            source_elements,count,columns,false)?,2,None)?;
        source.copy(OperationEvent::cpu_squeeze_layout(rank+2,false)?,1,None)?;
        ...
        if parallel || sentinel {
            ...
            source.alias(rank+1,rank+1)?;source.alias(0,rank+1)?;
            // zeros_like's eager scalar is I32, broadcast before Full's
            // actual cast to the gathered table dtype (including F16/BF16).
            source.cast(Dtype::Int32,native_dtype,rank+1,output_elements,Buffer::Value)?;
            source.copy(OperationEvent::cpu_scalar_full_layout(native_dtype,rank+1,output_elements,false)?,
                1,Some(Buffer::Value))?;
            source.cast(Dtype::Bool,Dtype::Bool,rank+1,count,Buffer::Mask)?;
            source.cast(native_dtype,native_dtype,rank+1,output_elements,Buffer::Value)?;
            source.cast(native_dtype,native_dtype,rank+1,output_elements,Buffer::Value)?;
            for _ in 0..3 {source.alias(rank+1,rank+1)?;}
            source.copy(OperationEvent::cpu_typed_select_broadcast_layout(native_dtype,rank+1,output_elements,false)?,
                3,Some(Buffer::Value))?;
        }
        Some(source)
    })();
    let Some(mut source)=source else {return Ok(None)};
    let classified=source.index_births.checked_add(source.mask_births).and_then(|n|n.checked_add(source.scalar_births))
        .and_then(|n|n.checked_add(source.value_births))
        .and_then(|n|n.checked_add(1)).ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?;
    if classified!=source.native.births {return Err(MlxWorkspaceFactError::descriptor("CPU lookup physical source count differs"));}
    ...
    let seeds=5usize + usize::from(parallel)
        + usize::from(sentinel) * if parallel {1} else {3};
    let index_capacity=mechanism.allocation.fixed_buffer_capacity(facts::mul(ids.elements()?,4)?)?;
    let mask_capacity=mechanism.allocation.fixed_buffer_capacity(ids.elements()?)?;
    let scalar_capacity=mechanism.allocation.fixed_buffer_capacity(4)?;
    let scratch_bytes=facts::add(facts::add(
    ...
    let scratch_bytes=facts::add(scratch_bytes,
    ...
    let frames=[size_of::<Population>()*3,size_of::<Option<Population>>(),size_of::<OperationPlan>(),
        size_of::<Option<OperationPlan>>(),size_of::<WorkspaceOperationView<'_>>(),
        size_of::<WorkspaceLayoutView<'_>>()*3,size_of::<MlxCpuWorkspaceMechanisms>(),
        size_of::<CpuPopulation>()*2,size_of::<Option<CpuCopyEvalLayout>>(),size_of::<CpuCopyEvalLayout>(),
        ...
        size_of::<(Dtype,Dtype,usize,usize,Buffer)>(),size_of::<(CpuBinaryOperation,Dtype,usize,usize,bool)>(),
        ...
        size_of::<Option<Buffer>>(),size_of::<Option<()>>(),size_of::<WorkspaceFloatingType>(),
        size_of::<usize>()*12,size_of::<u64>()*5,size_of::<std::ops::Range<usize>>(),
        size_of::<Option<EmbeddingLookupPolicy>>(),size_of::<EmbeddingLookupPolicy>(),size_of::<bool>()*2,
        size_of::<(CpuBinaryOperation,Dtype,usize,usize,bool,Buffer)>(),
        if parallel { crate::backend::nn::shared::parallel_vocabulary_lookup_control_bytes()
            .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)? } else { 0 }];
    source.native.controls=frames.into_iter().try_fold(source.native.controls.checked_add(size_of_val(&frames))
        .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?,|n,b|n.checked_add(b).ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW))?;
    Ok(Some(OperationPlan { alias_input: None,dtype,population:source.native,
        output_bytes,
        scratch_bytes,rank:rank+2,parameter_shells:if parallel {6}else{1},seeds,validations:1}))
}
