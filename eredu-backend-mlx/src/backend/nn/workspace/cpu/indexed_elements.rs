...
pub(super) fn inspect(
    ...
    if !super::super::basic::indexed_elements(operation) {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU sparse element source geometry differs",
        ));
    }
    ...
    if selected == 0 || elements > i32::MAX as usize || selected > i32::MAX as usize {
        return Ok(None);
    }
    let plan = (|| -> Option<CpuPopulation> {
        ...
        if update {
            ...
            p.copy(
                OperationEvent::cpu_cast_layout(Dtype::Int32, Dtype::Int32, 1, selected, false)?,
                1,
            )?;
            p.copy(
            ...
            p.controls = p
                .controls
                .checked_add(safemlx::Array::flat_index_update_control_bytes()?)?;
        } else {
            p.copy(
                OperationEvent::cpu_cast_layout(Dtype::Int32, Dtype::Int32, 1, selected, false)?,
                1,
    ...
    let Some(mut population) = plan else {
        return Ok(None);
    };
    let frames = [
    ...
    Ok(Some(OperationPlan {
        ...
        population,
        output_bytes: mechanism.allocation.fixed_buffer_capacity(
            operation
                .outputs
                .get(0)
