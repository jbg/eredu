...
pub(super) fn inspect(
    ...
    if linear && !constructed && operation.inputs.len() == 3 {
        return Ok(None);
    }
    ...
    let Some(left_dtype) = left.representation().map(|r| r.dtype()) else {return Ok(None)};
    let Some(weight_dtype) = weight.representation().map(|r| r.dtype()) else {return Ok(None)};
    ...
    let bias_dtype=operation.inputs.get(2).and_then(|bias|bias.representation().map(|r|r.dtype()));
    let dtype=bias_dtype.map_or(matrix_dtype,|bias|super::super::representation::promote(matrix_dtype,bias));
    ...
    let Ok(geometry) = geometry else { return Ok(None); };
    ...
    let native = match matrix_dtype {
        WorkspaceFloatingType::Float32 => mechanism.matmul.eval_layout(geometry, false),
        WorkspaceFloatingType::Bfloat16 => {
            OperationEvent::cpu_bf16_matmul_layout(2, m as usize, n as usize, k as usize, 1, false)
        }
        WorkspaceFloatingType::Float16 => mechanism.matmul.float16_eval_layout(geometry, 0, false),
    };
    ...
    let mut cast = |population:&mut CpuPopulation,from,to,rank,elements:usize|->Option<()> {
        if from==to {return Some(());}
        let source=OperationEvent::cpu_cast_layout(native_dtype(from),native_dtype(to),rank,elements,false)?;
        ...
        if to!=WorkspaceFloatingType::Float32||source.backing_births()!=1{return None;}
        let bytes=mechanism.allocation.fixed_buffer_capacity(u64::try_from(elements).ok()?.checked_mul(4)?).ok()?;
    ...
    let source = (|| {
        ...
        cast(&mut population,left_dtype,matrix_dtype,rank,usize::try_from(left.elements().ok()?).ok()?)?;
        cast(&mut population,weight_dtype,matrix_dtype,2,usize::try_from(weight.elements().ok()?).ok()?)?;
        if rank > 2 {
            population.copy(OperationEvent::cpu_reshape_alias_layout(rank, 2, false)?, 1)?;
        }
        population.copy(native, 2)?;
        if rank > 2 {
            population.copy(OperationEvent::cpu_reshape_alias_layout(2, rank, false)?, 1)?;
        }
        if operation.inputs.len() == 3 {
            cast(&mut population,matrix_dtype,dtype,rank,usize::try_from(output.elements().ok()?).ok()?)?;
            cast(&mut population,bias_dtype?,dtype,1,usize::try_from(n).ok()?)?;
            population.copy(
    ...
    let scratch_bytes = facts::add(converted_bytes,if operation.inputs.len() == 3 {
        output_bytes
    } else {
        0
    })?;
    ...
    let frames = [
        ...
        size_of::<(&mut CpuPopulation,WorkspaceFloatingType,WorkspaceFloatingType,usize,usize)>(),
        size_of::<u64>()*2,size_of::<Option<u64>>(),size_of::<Option<()>>(),
        size_of::<Result<u64,MlxWorkspaceFactError>>(),
        // Actual closure capture, not an assumed callable layout.
        cast_controls,size_of::<usize>(),
        size_of::<[u32; 5]>(),
        size_of::<(i32, i32)>(),
