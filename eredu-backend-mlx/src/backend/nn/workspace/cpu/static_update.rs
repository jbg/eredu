...
pub(super) fn inspect(
    ...
    let WorkspaceOperationKindView::StaticSliceUpdate {
        starts,
        ends,
        strides,
    } = operation.kind
    ...
    let Some([source, update]) = operation.inputs.array() else {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU static update operand population differs",
        ));
    };
    let Some([output]) = operation.outputs.array() else {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU static update output population differs",
        ));
    };
    ...
    {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU static update coordinates or representation differ",
        ));
    }
    if !(1..=4).contains(&rank)
        || source.dtype() != WorkspaceDtype::Float32
        || source.elements()? > i32::MAX as u64
    {
        return Ok(None);
    }
    let Some(precision) = source.representation() else {
        return Ok(None);
    };
    let dtype = precision.dtype();
    if update.representation().is_none_or(|r| r.dtype() != dtype) {
        return Ok(None);
    }
    let whole = starts.iter().all(|&n| n == 0)
        && ends == source.shape()
        && update.shape() == source.shape();
    let mut population = CpuPopulation::default();
    if !whole {
        if [source, update].iter().any(|value| {
            !value
                .representation()
        ...
        let Some(native) = OperationEvent::cpu_static_update_layout(
            ...
            usize::try_from(update.elements()?)?,
            false,
        ) else {
            return Ok(None);
        };
    ...
    Ok(Some(OperationPlan {
        dtype,
        population,
        alias_input: whole.then_some(1),
        output_bytes: if whole {
            0
        } else {
            mechanism.allocation.fixed_buffer_capacity(facts::mul(
                output.elements()?,
...
mod tests {
    ...
    fn cpu_static_scale_preserves_full_alias_and_prices_partial_replacement() {
        ...
        for width in [1, 19, 1025] {
            ...
            let source = WorkspaceTensor::existing_with_storage(
                context
                    .layout(&[1, 1, width], WorkspaceDtype::Float32)
                    .unwrap()
            ...
            let program =
                PreparedStaticActivation::new(&action, &slice, &shape, InterventionDtype::Float32)
                    .unwrap();
            let population = program.population().unwrap();
            // Select, scalar, cast, multiply and update each settle their retained prefix.
            assert_eq!(population.retained_roots, 5);
            assert_eq!(population.completions, 5);
            ...
            assert_eq!(first.alias_input, Some(0));
            assert_eq!(first.population.primitives, 0);
            assert_eq!(first.parameter_shells, 1);
            let last = cpu
                .plan(report.operations.last().unwrap().as_view())
                .unwrap()
                .unwrap();
            assert_eq!(last.alias_input, Some(1));
            assert_eq!(last.population.primitives, 0);
            assert_eq!(last.population.births, 0);
            assert_eq!(last.output_bytes, 0);
            assert_eq!(last.scratch_bytes, 0);
            ...
            assert_eq!(
                ...
                report.retained_bytes.map(|bytes| bytes + 16384)
            );
            assert_eq!(report.state.as_ref().unwrap().displaced_bytes, Some(0));
        }
        let base = WorkspaceLayoutView::new(&[1, 1, 19], WorkspaceDtype::Float32)
            .unwrap()
            .with_representation(Some(WorkspaceRepresentation::new(
                WorkspaceFloatingType::Float32,
                true,
        ...
        let op = WorkspaceOperationView {
            kind: WorkspaceOperationKindView::StaticSliceUpdate {
                starts: &[0, 0, 0],
                ends: &[1, 1, 19],
                strides: &[1, 1, 1],
            },
        ...
        for bad in [
            ...
            for index in 0..2 {
                ...
                changed[index] = base.with_representation(bad);
                assert!(cpu
                    .plan(WorkspaceOperationView {
                        inputs: WorkspaceLayoutList::Views(&changed),
                        ..op
                    })
                    .unwrap()
                    .is_none());
        ...
        let partial = [
            base,
            WorkspaceLayoutView::new(&[1, 1, 2], WorkspaceDtype::Float32)
                .unwrap()
        ...
        let partial = cpu
            .plan(WorkspaceOperationView {
                ...
                inputs: WorkspaceLayoutList::Views(&partial),
                ..op
            })
            .unwrap()
            .unwrap();
        assert_eq!(partial.alias_input, None);
        assert_eq!(partial.population.primitives, 1);
        assert_eq!(partial.population.births, 1);
        assert_eq!(partial.population.input_edges, 2);
        assert_eq!(partial.parameter_shells, 0);
        assert!(partial.output_bytes >= 19 * 4);
        assert_eq!(partial.scratch_bytes, 0);
        assert!(cpu
            .plan(WorkspaceOperationView {
                kind: WorkspaceOperationKindView::StaticSliceUpdate {
                    starts: &[0, 0],
        ...
        let strided = [
            base,
            base.with_representation(Some(WorkspaceRepresentation::new(
                WorkspaceFloatingType::Float32,
                false,
            ))),
        ];
        assert_eq!(
            cpu.output_representation(
    ...
    fn cpu_partial_static_update_quotes_batched_rectangles_and_complete_scale_trace() {
        ...
        .unwrap();
        let update = WorkspaceTensor::existing_with_storage(
            context
                .layout(&[2, 2, 3], WorkspaceDtype::Float32)
                .unwrap()
                .with_representation(known),
            &replacement,
            &context,
        ...
        let output = source
            .update_static_slice(&update, &[0, 1, 2], &[2, 3, 5], &[1; 3], &context)
            .unwrap();
        assert_eq!(output.shape(), [2, 3, 7]);
        assert_eq!(output.layout().representation(), known);
        let report = context
            .finish_report(&[source.clone(), update.clone(), output])
            .unwrap();
        assert!(
        ...
        assert_eq!(
            plan.output_bytes,
            ordinary.allocation().fixed_buffer_capacity(168).unwrap()
        );
        assert_eq!(
            report.state.as_ref().unwrap().retained_bytes,
            Some(216 + plan.output_bytes + ordinary.allocation().host_control_bytes().unwrap())
        );
        assert_eq!(report.state.as_ref().unwrap().displaced_bytes, Some(0));
        let recipe =
            SpeculativeNumericalRecipe::inspect_cpu_capture(&report, 3, 0, ordinary, cpu, &context)
                .unwrap();
        assert_eq!(recipe.kernels, 0);
        ...
        for bad in [
            ...
            let values = [
                op.inputs.get(0).unwrap(),
                op.inputs.get(1).unwrap().with_representation(bad),
            ];
            assert!(cpu
                .plan(WorkspaceOperationView {
                    inputs: WorkspaceLayoutList::Views(&values),
                    ..op
                })
                .unwrap()
                .is_none());
        ...
        for (dims, starts, ends, selected_shape) in [
            ...
            let source = WorkspaceTensor::existing_with_storage(
                context
                    .layout(&dims, WorkspaceDtype::Float32)
                    .unwrap()
                    .with_representation(known),
                &storage,
                &context,
            ...
            let update = cpu
                ...
                .unwrap()
                .unwrap();
            assert_eq!(update.population.births, 1);
            assert_eq!(update.alias_input, None);
            assert_eq!(
                report.state.as_ref().unwrap().retained_bytes,
                report.retained_bytes.map(|bytes| bytes + input_bytes)
            );
            let recipe = SpeculativeNumericalRecipe::inspect_cpu_capture(
                &report,
                roots.len(),
