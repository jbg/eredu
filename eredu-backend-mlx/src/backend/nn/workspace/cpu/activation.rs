...
pub(super) fn inspect(
    ...
    if operation.inputs.len() != 1 || operation.outputs.len() != 1 {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU activation population differs",
        ));
    }
    ...
    let source = (|| {
        ...
        if matches!(native, Dtype::Bfloat16 | Dtype::Float16) {
            population.copy(
                OperationEvent::cpu_cast_layout(native, Dtype::Float32, rank, count, false)?,
                1,
            )?;
        }
        if silu {
            ...
            for (kind, right_scalar) in [
                (CpuBinaryOperation::Add, true),
                (CpuBinaryOperation::Divide, false),
            ] {
                population.copy(
                ...
                if right_scalar {
                    scalars = scalars.checked_add(cast.backing_births())?;
                }
                ...
                population.copy(
                    OperationEvent::cpu_broadcast_alias_layout(right_rank, rank, false)?,
                    1,
                )?;
        ...
        population.controls = population
            .controls
            .checked_add(crate::backend::nn::arithmetic::cpu_pointwise_probe_control_bytes()?)?;
    ...
    let scratch_full = population
        .births
        .checked_sub(scalar_births)
        .and_then(|n| n.checked_sub(1))
        .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?;
    let scratch_bytes = facts::add(
        facts::mul(full, u64::try_from(scratch_full)?)?,
        facts::mul(
            scalar,
            u64::try_from(
                scalar_births
    ...
    let frames = [
        ...
        size_of::<[(CpuBinaryOperation, bool); 2]>(),
        size_of::<std::array::IntoIter<(CpuBinaryOperation, bool), 2>>(),
        size_of::<(CpuBinaryOperation, bool)>(),
        size_of::<(usize, usize)>(),
        size_of::<usize>() * 7,
        size_of::<u64>() * 3,
