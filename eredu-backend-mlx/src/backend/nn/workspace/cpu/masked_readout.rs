...
impl Program {
    ...
    fn copy(&mut self, source: CpuCopyEvalLayout, inputs: usize, elements: usize) -> Option<()> {
        self.physical(elements, source.backing_births())?;
    ...
    fn cast(&mut self, dtype: Dtype, rank: usize, elements: usize) -> Option<()> {
        self.copy(
            OperationEvent::cpu_cast_layout(dtype, dtype, rank, elements, false)?,
            1,
            elements,
        )
    }
    fn gather(
        ...
        self.alias(index_rank, index_rank)?;
        self.cast(index, index_rank, indices)?;
        self.copy(
            OperationEvent::cpu_gather_layout(
                dtype, index, 2, index_rank, table, indices, width, false,
            )?,
            2,
            indices.checked_mul(width)?,
        )?;
        self.copy(
            OperationEvent::cpu_squeeze_layout(index_rank + 2, false)?,
            1,
            0,
        )
    }
...
pub(super) fn inspect(
    ...
    let Some(geometry) = super::super::readout::geometry(operation)? else {
        return Ok(None);
    };
    if geometry.batch <= 0 || geometry.sequence <= 0 {
        return Ok(None);
    }
    ...
    let (b, s, h, v, c, k) = (
        usize::try_from(geometry.batch)?,
        usize::try_from(geometry.sequence)?,
        usize::try_from(geometry.hidden)?,
        usize::try_from(geometry.vocabulary)?,
        usize::try_from(geometry.centroids)?,
        usize::try_from(geometry.selected)?,
    );
    let checked = || -> Option<[usize; 7]> {
        let rows = b.checked_mul(s)?;
        let hidden = rows.checked_mul(h)?;
        let centroids = rows.checked_mul(c)?;
        let picks = rows.checked_mul(k / (v / c))?;
        let selected = rows.checked_mul(k)?;
        let weights = selected.checked_mul(h)?;
    ...
    let Some([rows, _hidden, centroids, picks, selected, _weights, output]) = checked() else {
        return Ok(None);
    };
    let source = (|| {
        let mut p = Program {
        ...
        p.native.add(CpuPopulation {
            ...
            maximum_captures: 0,
            births: 1,
            extents: partition.allocation_extents(),
            controls: partition.control_bytes()?,
        })?;
        p.physical(centroids, 1)?;
        // Static top-centroid slice and unchanged-shape reshape candidate.
        p.copy(OperationEvent::cpu_slice_layout(3, false, false)?, 1, 0)?;
        p.reshape(3, 3)?;
        p.reshape(1, 2)?;
        ...
        if k > 1 {
            p.copy(
                OperationEvent::cpu_row_min_layout(3, k, rows, false)?,
                1,
                rows,
            )?;
        }
        ...
        p.cast(Dtype::Float32, 3, rows)?;
        p.cast(Dtype::Float32, 0, 1)?;
        p.alias(3, 3)?;
        p.alias(0, 3)?;
        ...
        p.physical(rows, subtract.backing_births())?;
        p.native.binary(subtract)?;
        p.alias(3, 3)?;
        p.cast(Dtype::Float32, 3, output)?;
        p.copy(
            OperationEvent::cpu_row_full_layout(3, v, rows, false)?,
            1,
            output,
        )?;
        p.reshape(4, 3)?;
        ...
        p.copy(
            OperationEvent::cpu_scatter_axis_layout(index, 3, output, selected, false)?,
            3,
            output,
        )?;
        ...
        p.native.controls = p
            .native
            .controls
            .checked_add(crate::tensor::masked_readout_control_bytes(false)?)?;
        Some(p)
    })();
    ...
    let output_bytes = mechanism
        .allocation
        .fixed_buffer_capacity(facts::mul(u64::try_from(output)?, 4)?)?;
    ...
    let frames = [
        ...
        size_of::<super::super::readout::ReadoutGeometry>() * 2,
        ...
        size_of::<safemlx::CpuArgPartitionLayout>(),
        size_of::<Option<safemlx::CpuArgPartitionLayout>>(),
        size_of::<MlxCpuWorkspaceMechanisms>(),
        size_of::<Dtype>() * 2,
        size_of::<usize>() * 17,
        size_of::<[usize; 7]>() * 2,
        size_of::<Option<[usize; 7]>>(),
        size_of::<std::array::IntoIter<usize, 7>>(),
        size_of::<(&mut Program, usize, usize)>(),
        size_of::<(&mut Program, CpuCopyEvalLayout, usize, usize)>(),
        size_of::<(&mut Program, Dtype, usize, usize)>(),
        size_of::<(&mut Program, Dtype, Dtype, usize, usize, usize, usize)>(),
