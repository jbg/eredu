...
pub(super) fn inspect(
    ...
    if operation.inputs.len() != if update { 2 } else { 1 } || operation.outputs.len() != 1 {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU selected-token operand population differs",
        ));
    }
    ...
    let output = operation
        .outputs
        .get(0)
        .expect("checked token-score output");
    if input.shape().len() != 1 || input.shape()[0] < 2 {
        return Ok(None);
    }
    ...
    if input.dtype() != WorkspaceDtype::Float32 || input.representation() != expected {
        return Ok(None);
    }
    let elements = usize::try_from(input.shape()[0])?;
    let mut population = CpuPopulation::default();
    if update {
        let WorkspaceOperationKindView::SliceUpdate { starts } = operation.kind else {
            unreachable!()
        };
        ...
        if value.shape() != [1]
            ...
            || starts.len() != 1
            || starts[0] < 0
            || starts[0] >= input.shape()[0]
            || output.shape() != input.shape()
            || output.dtype() != input.dtype()
        {
            return Err(MlxWorkspaceFactError::descriptor(
                "CPU scalar overwrite changes coordinates or dtype",
            ));
        }
        ...
        let Some(source) = OperationEvent::cpu_scalar_update_layout(elements, false) else {
            return Ok(None);
        };
        if source.backing_births() != 1 || population.copy(source, 2).is_none() {
            return Ok(None);
        }
    } else {
        if !output.shape().is_empty() || output.dtype() != WorkspaceDtype::Uint32 {
            return Err(MlxWorkspaceFactError::descriptor(
                "CPU selected-token argmax changes scalar output",
            ));
        }
        ...
        let Some(reduce) = OperationEvent::cpu_arg_reduce_layout(1, elements, 1, false) else {
            return Ok(None);
        };
        let Some(squeeze) = OperationEvent::cpu_squeeze_layout(1, false) else {
            return Ok(None);
        };
        if reduce.backing_births() != 1
            || squeeze.backing_births() != 0
            || population.copy(reduce, 1).is_none()
            || population.copy(squeeze, 1).is_none()
        {
            return Ok(None);
        }
    }
    ...
    fn cpu_token_score_trace_joins_full_distribution_and_each_scalar_frontier() {
        ...
        for width in [1, 19, 1025] {
            ...
            let input = WorkspaceTensor::existing_with_storage(
                context
                    .layout(&[1, 2, width], WorkspaceDtype::Float32)
                    .unwrap()
            ...
            assert_eq!(
                ...
                2 + chunks + ids.len() * if width == 1 { 2 } else { 4 }
            );
        ...
        let output = context
            .execute(
                ...
                &[&input],
                vec![context.layout(&[], WorkspaceDtype::Uint32).unwrap()],
            )
            .unwrap()
            .remove(0);
