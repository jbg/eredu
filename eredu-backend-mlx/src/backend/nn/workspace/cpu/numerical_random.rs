...
pub(in crate::backend::nn::workspace) fn categorical(
    rank: usize,
    columns: usize,
...
pub(super) fn categorical_draw(rank: usize, columns: usize) -> Option<CpuPopulation> {
    if !(2..=3).contains(&rank) || columns <= 1 {
        return None;
    }
    ...
    if population.primitives != 18 || population.births != 13 {
        return None;
    }
    let parts = [
        ...
        size_of::<(usize, usize)>(),
        size_of::<[CpuUnaryOperation; 4]>(),
        size_of::<std::array::IntoIter<CpuUnaryOperation, 4>>(),
...
pub(super) fn uniform_draw(rank: usize, elements: usize) -> Option<CpuPopulation> {
    if !(1..=3).contains(&rank) || elements == 0 || elements > i32::MAX as usize {
        return None;
    }
    let mut population = CpuPopulation::default();
    ...
    population.copy(
        OperationEvent::cpu_random_bits_layout(rank, elements, false)?,
        1,
    )?;
    population.copy(
    ...
    if population.births != 7 || population.primitives != 11 {
        return None;
    }
    let parts = [
        size_of::<CpuPopulation>(),
        ...
        size_of::<safemlx::CpuBinaryEvalLayout>(),
        size_of::<Option<safemlx::CpuBinaryEvalLayout>>(),
        size_of::<[CpuBinaryOperation; 4]>(),
        size_of::<std::array::IntoIter<CpuBinaryOperation, 4>>(),
        size_of::<std::ops::Range<usize>>(),
