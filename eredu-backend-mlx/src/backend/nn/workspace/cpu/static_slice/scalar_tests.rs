    ...
    for dtype in [
        ...
        for (rows, start, end, step) in [
            ...
            let input = WorkspaceTensor::existing_with_storage(
                context.layout(&[rows, 1], dtype).unwrap(),
                &storage,
                &context,
            )
            .unwrap();
            ...
            let output = input
                .static_slice(&[start, 0], &[end, 1], &[step, 1], &context)
                .unwrap();
        ...
        let context = WorkspaceContext::new(cpu);
        let input =
            WorkspaceTensor::existing(context.layout(&[2, 7, 5, 3], dtype).unwrap(), &context)
                .unwrap();
        context.begin_span();
        let output = input
            .static_slice(&[0, 1, 0, 1], &[2, 7, 5, 3], &[1, 2, 2, 1], &context)
            .unwrap();
...
fn cpu_scalar_static_slice_checks_coordinates_and_normalized_precision() {
    ...
    let floating = Some(WorkspaceRepresentation::new(
    ...
    for normalized in [
        WorkspaceOperationView {
            inputs: WorkspaceLayoutList::Views(&normalized_source),
            ..operation
        },
        WorkspaceOperationView {
            outputs: WorkspaceLayoutList::Views(&normalized_output),
            ..operation
        },
    ] {
    ...
    assert!(cpu
        .plan(WorkspaceOperationView {
            inputs: WorkspaceLayoutList::Views(&float_source),
            outputs: WorkspaceLayoutList::Views(&float_output),
            ..operation
        })
        .unwrap()
        .is_none());
