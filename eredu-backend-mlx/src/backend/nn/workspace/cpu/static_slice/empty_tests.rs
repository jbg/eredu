...
fn cpu_empty_static_slice_prices_allocator_backing_without_retaining_source() {
    ...
    let cases = [
        ...
        (WorkspaceDtype::Uint32, None),
        (WorkspaceDtype::Uint8, None),
        (WorkspaceDtype::Bool, None),
        (
        ...
        for shape in [&[3][..], &[1, 1][..], &[2, 3, 4][..], &[2, 3, 4, 5][..]] {
            ...
            let input = WorkspaceTensor::existing_with_storage(
                context.layout(shape, dtype).unwrap().with_representation(
                    precision.map(|dtype| WorkspaceRepresentation::new(dtype, true)),
                ),
                &storage,
                &context,
            )
            ...
            assert_eq!(
                ...
                Some(capacity + ordinary.allocation().host_control_bytes().unwrap())
            );
            ...
            assert_eq!(
                (plan.population.primitives, plan.population.maximum_captures),
                (1, 1)
            );
            assert!(plan.population.extents > 0 && plan.population.controls > 0);
            assert_eq!(
                cpu.output_representation(operation, 0).map(|r| r.dtype()),
                precision
            );
...
fn cpu_empty_static_slice_keeps_identity_alias_and_exact_coordinates_distinct() {
    ...
    let storage = WorkspaceExistingStorage::try_new(Some(8192), &context).unwrap();
    let input = WorkspaceTensor::existing_with_storage(
        context.layout(&[0, 8], WorkspaceDtype::Int32).unwrap(),
        &storage,
        &context,
    )
    .unwrap();
    ...
    assert!(cpu
        .plan(WorkspaceOperationView {
            inputs: WorkspaceLayoutList::Views(&float_inputs),
            outputs: WorkspaceLayoutList::Views(&float_outputs),
            ..operation
        })
        .unwrap()
        .is_none());
