...
pub(super) fn inspect(
    ...
    let Some([input]) = operation.inputs.array() else {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU classification input population differs",
        ));
    };
    let Some([output]) = operation.outputs.array() else {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU classification output population differs",
        ));
    };
    let cast = matches!(kind, Kind::MaskU32);
    let (from, to) = if cast {
        (WorkspaceDtype::Bool, WorkspaceDtype::Uint32)
    } else {
        (WorkspaceDtype::Float32, WorkspaceDtype::Bool)
    };
    if input.dtype() != from || output.dtype() != to || input.shape() != output.shape() {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU classification changes declared source geometry",
        ));
    }
    ...
    let source = (|| {
        if cast {
            let source =
                OperationEvent::cpu_cast_layout(Dtype::Bool, Dtype::Uint32, rank, elements, false)?;
        ...
        for (kind, dtype, count) in [
            (CpuBinaryOperation::Equal, Dtype::Float32, equal),
    ...
    if source.is_none() || population.births == 0 {
        return Ok(None);
    }
...
mod tests {
    ...
    fn cpu_capture_classification_keeps_actual_mask_and_conversion_sources() {
            ...
            for name in [
                ...
                let mask = context
                    .execute(
                        ...
                        &[&input],
                        vec![context.layout(shape, WorkspaceDtype::Bool).unwrap()],
                    )
                    .unwrap()
                    .remove(0);
                let count = context
                    .execute(
                        WorkspaceOperationKind::Elementwise("bool_to_u32"),
                        &[&mask],
                        vec![context.layout(shape, WorkspaceDtype::Uint32).unwrap()],
                    )
                    .unwrap()
                    .remove(0);
                ...
                assert!(report
                    .tensor_buffers
                    .retained_bytes
                    .is_some_and(|n| n >= count.layout().bytes().unwrap()));
        ...
        assert!(context
            .execute(
                WorkspaceOperationKind::Elementwise("is_nan"),
                &[&source],
                vec![wrong]
            )
            .is_err());
