...
pub(super) fn inspect(
    ...
    let [output] = operation
        .outputs
        .array()
        .ok_or_else(|| MlxWorkspaceFactError::descriptor("CPU sampler output differs"))?;
    let source = (|| -> Option<OperationPlan> {
        let mut population = CpuPopulation::default();
        let mut alias_input = None;
        let mut output_bytes = mechanism
            .allocation
            .fixed_buffer_capacity(output.bytes().ok()?)
            .ok()?;
        ...
        match kind {
            ...
            S::SelectRandomKey { index } => {
                ...
                if table.dtype() != WorkspaceDtype::Uint32
                    || table.shape().len() != 2
                    || table.shape()[0] <= 0
                    || table.shape()[1] != 2
            ...
            S::ReadToken => {
                ...
                if token.dtype() != WorkspaceDtype::Uint32
                    || token.elements().ok()? != 1
                    || token != output
                {
                    return None;
            ...
            S::Greedy | S::Categorical => {
                ...
                if !(2..=3).contains(&rank)
                    ...
                    || !scores.representation().is_some_and(|r| {
                        r.dtype() == WorkspaceFloatingType::Float32 && r.row_contiguous()
                    })
                    || output.dtype() != WorkspaceDtype::Uint32
                ...
                if columns <= 1 || columns > i32::MAX as usize {
                    return None;
                }
                if matches!(kind, S::Greedy) {
                    ...
                    let row = mechanism
                        .allocation
                        .fixed_buffer_capacity((columns as u64).checked_mul(4)?)
                        .ok()?;
