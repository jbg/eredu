//! Actual CPU normalization fallbacks: shared reductions and affine workers.
use super::*;
...
pub(super) fn inspect(
    ...
    if let WorkspaceOperationKindView::LayerNorm { weight, bias } = operation.kind {
        return layer::inspect(operation, mechanism, weight, bias);
    }
    if let WorkspaceOperationKindView::ConstructedNormalization(spec) = operation.kind {
        if let Some(groups) = spec.groups {
            return grouped::inspect(operation, mechanism, spec, groups);
        }
    }
    let (learned, constructed, offset) = match operation.kind {
        WorkspaceOperationKindView::Normalization("rms", None) => {
            (operation.inputs.len() == 2, false, false)
        }
    ...
    if operation.outputs.len() != 1 || operation.inputs.len() != 1 + usize::from(learned) {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU RMS operand population differs",
        ));
    }
    ...
    if input.shape() != output.shape() {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU RMS output shape differs",
        ));
    }
    let Some(input_dtype) = input.representation().map(|r| r.dtype()) else {
        return Ok(None);
    };
    let gain_dtype = if learned {
        let Some(gain) = operation
            .inputs
            .get(1)
            .and_then(|gain| gain.representation())
        else {
            return Ok(None);
        };
        gain.dtype()
    ...
    let input_native = native_dtype(input_dtype);
    let gain_native = native_dtype(gain_dtype);
    ...
    let dtype = if learned && !constructed {
        input_dtype
    } else {
        promoted
    };
    ...
    if operation
        .inputs
        .get(1)
        .is_some_and(|w| w.shape() != [input.shape()[rank - 1]])
    {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU RMS gain shape differs",
        ));
    }
    if let WorkspaceOperationKindView::ConstructedNormalization(spec) = operation.kind {
        if spec.dimensions != input.shape()[rank - 1] {
            return Err(MlxWorkspaceFactError::descriptor(
                "CPU RMS construction width differs",
            ));
        }
    }
    ...
    let source = (|| {
        ...
        if offset {
            ...
            p.birth(Buffer::Vector, add.backing_births())?;
            p.native.binary(add)?;
        }
        if learned {
        ...
        if learned {
            ...
            if !constructed {
                p.cast(native, input_native, rank, count, Buffer::Full)?;
            }
        }
        p.native.controls =
            p.native
    ...
    {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU RMS physical source population differs",
        ));
    }
