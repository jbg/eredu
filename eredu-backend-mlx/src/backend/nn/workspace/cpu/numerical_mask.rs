...
pub(in crate::backend::nn::workspace) fn token_filter(
    rank: usize,
    elements: usize,
) -> Option<CpuPopulation> {
    if !(2..=3).contains(&rank) || elements == 0 || elements > i32::MAX as usize {
        return None;
    }
    ...
    population.copy(
        OperationEvent::cpu_cast_layout(Dtype::Bool, Dtype::Bool, rank, elements, false)?,
        1,
    )?;
    population.copy(
        OperationEvent::cpu_cast_layout(Dtype::Float32, Dtype::Float32, 0, 1, false)?,
        1,
    )?;
    population.copy(
        OperationEvent::cpu_cast_layout(Dtype::Float32, Dtype::Float32, rank, elements, false)?,
        1,
    )?;
    population.copy(
    ...
    if population.primitives != 7 || population.input_edges != 9 {
        return None;
    }
    let parts = [
...
pub(super) fn inspect(
    ...
    let Some([input]) = operation.inputs.array() else {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU token mask input population differs",
        ));
    };
    let Some([output]) = operation.outputs.array() else {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU token mask output population differs",
        ));
    };
    ...
    {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU token mask changes floating geometry",
        ));
    }
    ...
    let Some(mut population) = token_filter(rank, elements) else {
        return Ok(None);
    };
    let parts = [
    ...
    let output_bytes = mechanism
        .allocation
        .fixed_buffer_capacity(facts::mul(elements as u64, 4)?)?;
    ...
    let bool_bytes = mechanism
        .allocation
        .fixed_buffer_capacity(elements as u64)?;
    let scalar_bytes = mechanism.allocation.fixed_buffer_capacity(4)?;
    let scratch_bytes = facts::add(
        output_bytes,
        facts::add(facts::mul(bool_bytes, 2)?, facts::mul(scalar_bytes, 2)?)?,
    )?;
    ...
    fn optional_cpu_mask_retains_full_input_alias_and_prices_masked_branch() {
        ...
        for shape in [&[1, 17][..], &[1, 1, 17][..]] {
            for optional in [false, true] {
                ...
                let input =
                    WorkspaceTensor::existing_with_storage(layout, &storage, &context).unwrap();
                ...
                assert_eq!(
                    report.closing_storage.bytes,
                    Some(
                        output_capacity
                            + ordinary.allocation().host_control_bytes().unwrap()
                            + if optional { 4096 } else { 0 }
                    )
                );
                let calls = cpu
                    ...
                    .unwrap();
                assert!(
                    calls.metadata_bytes
                        > safemlx::Array::ordinary_clone_control_bytes().unwrap() as u64
                );
                assert!(
                    ordinary
                        .ordinary_call_controls(report.operations[0].as_view())
