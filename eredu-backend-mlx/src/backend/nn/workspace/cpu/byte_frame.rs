...
pub(super) fn inspect(
    ...
    if output.dtype() != WorkspaceDtype::Uint8 {
        return Ok(None);
    }
    let rank = output.shape().len();
    let count = output.elements()?;
    if rank > 5 || count == 0 || count > i32::MAX as u64 || output.shape().iter().any(|&n| n <= 0) {
        return Ok(None);
    }
    ...
    match kind {
        0 => {
            if !basic::is_scalar_u8(operation) {
                return Err(MlxWorkspaceFactError::descriptor(
                    "CPU byte seed is not one eager U8 scalar",
                ));
            }
            population.controls = basic::scalar_u8_control_bytes()
        ...
        1 => {
            ...
            if input.elements()? != count {
                return Err(MlxWorkspaceFactError::descriptor(
                    "CPU byte reshape changes element count",
                ));
            }
            if input.shape() == output.shape() {
                ...
                let Some(source) =
                    OperationEvent::cpu_reshape_copy_layout(input.shape().len(), rank, false)
                else {
                    return Ok(None);
                };
        ...
        2 => {
            if operation.inputs.len() != 2 || rank != 1 {
                return Ok(None);
            }
            ...
            if left.shape().len() != 1
                || right.shape().len() != 1
                || left.shape()[0].checked_add(right.shape()[0]) != Some(output.shape()[0])
            {
                return Err(MlxWorkspaceFactError::descriptor(
                    "CPU byte concatenation dimensions differ",
                ));
            }
            let Some(source) = OperationEvent::cpu_concatenate_layout(
                Dtype::Uint8,
                rank,
                usize::try_from(left.elements()?)?,
                usize::try_from(right.elements()?)?,
                false,
            ) else {
                return Ok(None);
            };
            ...
            if source.backing_births() != 1 || population.concatenate(source, 2).is_none() {
                return Ok(None);
            }
        }
        3 => {
            if !basic::is_static_slice(operation) {
                return Err(MlxWorkspaceFactError::descriptor(
                    "CPU byte slice coordinates differ",
                ));
            }
            ...
            if input.shape() == output.shape() {
                ...
                let Some(source) = OperationEvent::cpu_slice_layout(rank, false, false) else {
                    return Ok(None);
                };
        ...
        4 => {
            ...
            if input.shape() != output.shape() || !scalar.shape().is_empty() {
                return Ok(None);
            }
            let Some(broadcast) = OperationEvent::cpu_broadcast_alias_layout(0, rank, false) else {
                return Ok(None);
            };
            let Some(add) = OperationEvent::cpu_binary_layout(
            ...
            if broadcast.backing_births() != 0
                || add.backing_births() != 1
                || population.copy(broadcast, 1).is_none()
                || population.binary(add).is_none()
            {
                return Ok(None);
            }
        }
    ...
    Ok(Some(OperationPlan {
        ...
        population,
        alias_input: alias,
        output_bytes: if alias.is_some() {
            0
        } else {
            mechanism.allocation.fixed_buffer_capacity(count)?
        },
        scratch_bytes: 0,
        rank,
        ...
        for dtype in [
            ...
            for header_len in [3, 4] {
                ...
                let source = WorkspaceTensor::existing_with_storage(
                    context
                        .layout(&[1, 2, 3], WorkspaceDtype::Float32)
                        .unwrap()
                ...
                let header = WorkspaceTensor::existing(
                    context
                        .layout(&[header_len], WorkspaceDtype::Uint8)
                        .unwrap(),
                    &context,
                )
                .unwrap();
                ...
                let frame =
                    boundary_frame::encode(&boundary_frame::Workspace(&context), &source, &header)
                        .unwrap();
                let (copied_header, decoded) = boundary_frame::split(
                ...
                .unwrap();
                assert_eq!(copied_header.shape(), [header_len]);
                assert_eq!(decoded.shape(), source.shape());
                assert_eq!(
                    decoded.layout().representation(),
                ...
                assert_eq!(
                    report
                        ...
                        .filter(|op| matches!(
                            op.kind,
                            WorkspaceOperationKind::Elementwise("scalar_u8")
                        ))
                        .count(),
                    usize::from(header_len == 3)
                );
                assert!(report
                    .operations
                    .iter()
                    .any(|op| matches!(op.kind, WorkspaceOperationKind::StaticSlice { .. })));
                ...
                let generic = context
                    .report(&[
                        WorkspaceTensor::initialized(&[], WorkspaceDtype::Uint8, &context).unwrap(),
                    ])
                    .unwrap();
                let initialize = generic
                    .operations
                    .iter()
                    .find(|op| matches!(op.kind, WorkspaceOperationKind::Initialize))
                    .unwrap();
