...
pub(super) fn inspect(operation:WorkspaceOperationView<'_>,mechanism:MlxCpuWorkspaceMechanisms)
    ...
    if operation.inputs.len()!=1||operation.outputs.len()!=1 {
        return Err(MlxWorkspaceFactError::descriptor("CPU storage copy requires one source and destination"));
    }
    ...
    if input.dtype()!=output.dtype()||input.shape()!=output.shape(){
        return Err(MlxWorkspaceFactError::descriptor("CPU storage copy changes logical geometry"));
    }
    let rank=input.shape().len();
    if rank>5||input.shape().iter().any(|&n|n<0)||input.elements()?>i32::MAX as u64{return Ok(None);}
    let representation=input.representation();
    let dtype=match input.dtype() {
        WorkspaceDtype::Float32=>match representation {Some(value)=>value.dtype(),None=>return Ok(None)},
        WorkspaceDtype::Int32|WorkspaceDtype::Uint32|WorkspaceDtype::Uint8|WorkspaceDtype::Bool=>WorkspaceFloatingType::Float32,
    };
    if output.representation().is_some_and(|value|input.dtype()!=WorkspaceDtype::Float32||value.dtype()!=dtype){return Ok(None);}
    let bytes=mechanism.allocation.fixed_buffer_capacity(output.bytes()?)?;
    ...
    if compaction {
        let Some(source)=OperationEvent::cpu_contiguous_layout(rank,false) else{return Ok(None)};
        if source.backing_births()!=1||population.copy(source,1).is_none(){return Ok(None);}
    }
    let eager=if deep {
        let Some(controls)=safemlx::original_scoped_deep_copy_control_bytes(rank) else{return Ok(None)};
        controls
    }else{0};
    let frames=[eager,size_of::<WorkspaceOperationView<'_>>(),size_of::<WorkspaceLayoutView<'_>>()*2,
        ...
        size_of::<CpuCopyEvalLayout>(),size_of::<Option<CpuCopyEvalLayout>>(),size_of::<WorkspaceFloatingType>(),
        size_of::<bool>()*2,size_of::<u64>(),size_of::<usize>()*2,size_of::<Option<usize>>(),
        size_of::<std::slice::Iter<'_,i32>>(),size_of::<(&safemlx::Array,bool,&safemlx::Stream)>(),
    ...
    Ok(Some(OperationPlan {dtype,population,alias_input:None,output_bytes:bytes,
        scratch_bytes:if deep&&compaction {bytes}else{0},rank,parameter_shells:0,
        ...
        seeds:usize::from(deep),validations:0}))
}
