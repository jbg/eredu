...
pub(super) fn inspect(
    ...
    if !same {
        let Some(native) = OperationEvent::cpu_byte_view_layout(
            source.native(),
            target.native(),
            rank,
        ...
        if native.backing_births() != usize::from(copy) || population.copy(native, 1).is_none() {
            return Ok(None);
        };
    }
    let frames = [
        ...
        size_of::<geometry::Dtype>() * 2,
        size_of::<Option<geometry::Dtype>>(),
        size_of::<Option<(u64, usize)>>(),
    ...
    Ok(Some(OperationPlan {
        dtype: target.floating().unwrap_or(WorkspaceFloatingType::Float32),
        population,
        alias_input: alias.then_some(0),
        output_bytes: if copy {
            mechanism
                .allocation
                .fixed_buffer_capacity(bytes.max(output.bytes()?))?
        } else {
            0
        },
        scratch_bytes: 0,
...
pub(super) fn representation(
    ...
    let target = geometry::selected(name)?;
    let dtype = target.floating()?;
    let input = operation.inputs.get(0)?;
    let source = geometry::Dtype::from_layout(input)?;
    ...
    let rows = target.bytes() > source.bytes()
...
mod tests {
    ...
    fn cpu_byte_view_keeps_exact_scalar_ratio_alias_and_possible_copy_source() {
        ...
        for dtype in [
            ...
            assert_eq!(bytes.layout().dtype(), WorkspaceDtype::Uint8);
            assert_eq!(bytes.layout().representation(), None);
            ...
            assert_eq!(encoded.tensor_buffers.total_bytes, Some(0));
            assert_eq!(encoded.state.as_ref().unwrap().retained_bytes, Some(4096));
            let target = match dtype {
                WorkspaceFloatingType::Float32 => geometry::Dtype::F32,
                WorkspaceFloatingType::Float16 => geometry::Dtype::F16,
                WorkspaceFloatingType::Bfloat16 => geometry::Dtype::Bf16,
            };
            context.begin_span();
            let decoded = geometry::trace(&bytes, target, &context).unwrap();
            assert_eq!(decoded.shape(), source.shape());
            assert_eq!(
                decoded.layout().representation(),
                Some(WorkspaceRepresentation::new(dtype, true))
            );
            let report = context.report(&[decoded]).unwrap();
            ...
            assert!(plan.output_bytes >= report.operations[0].outputs[0].bytes().unwrap());
            ...
            let raw = WorkspaceTensor::existing(
                metal.layout(bytes.shape(), WorkspaceDtype::Uint8).unwrap(),
                &metal,
            )
            .unwrap();
