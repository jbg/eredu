...
impl Program {
    fn storage(&mut self, elements: usize, itemsize: u64, births: usize) -> Option<()> {
        ...
        self.bytes = self
            .bytes
            .checked_add(capacity.checked_mul(u64::try_from(births).ok()?)?)?;
        Some(())
    }
    fn copy(&mut self, source: CpuCopyEvalLayout, inputs: usize, elements: usize) -> Option<()> {
        self.storage(elements, 4, source.backing_births())?;
    ...
    fn binary(
        ...
        let source =
            OperationEvent::cpu_binary_layout(kind, Dtype::Float32, rank, elements, false)?;
        self.storage(elements, if mask { 1 } else { 4 }, source.backing_births())?;
        self.population.binary(source)
    }
    fn seed(&mut self) -> Option<()> {
        self.seeds = self.seeds.checked_add(1)?;
        self.storage(1, 4, 1)?;
        self.population.controls = self
            .population
            .controls
            .checked_add(basic::scalar_f32_control_bytes()?)?;
        Some(())
    }
    fn mask(&mut self, rank: usize, width: usize) -> Option<()> {
        ...
        let source = if width == 1 {
            OperationEvent::cpu_select_layout(Dtype::Float32, rank, width, false)?
        } else {
            OperationEvent::cpu_select_broadcast_layout(rank, width, false)?
        };
    ...
    fn cpu_sampler_filters_join_only_their_actual_one_row_sources() {
        ...
        for rank in [2, 3] {
            for width in [1, 19, 65, 4097] {
                ...
                for kind in [
                    WorkspaceSamplingOperation::TopK { keep: 1 },
                    WorkspaceSamplingOperation::TopK { keep: 7 },
                    WorkspaceSamplingOperation::TopP,
                    WorkspaceSamplingOperation::MinP,
                ] {
                    if matches!(kind,WorkspaceSamplingOperation::TopK{keep} if keep as i32>=width) {
                        continue;
                    }
                    ...
                    let output = context
                        .execute(
                            WorkspaceOperationKind::Sampling(kind.clone()),
                            &[&input],
                    ...
                    assert_eq!(
                        plan.seeds,
                        match kind {
                            WorkspaceSamplingOperation::TopK { .. } => 1,
                            WorkspaceSamplingOperation::TopP => 3,
                            _ => 2,
                        }
                    );
                    assert_eq!(
                        ...
                        ordinary
                            .allocation()
                            .fixed_buffer_capacity(width as u64 * 4)
                            .unwrap()
                    );
        ...
        let input = WorkspaceTensor::existing(
            context
                .layout(&[2, 1, 65], WorkspaceDtype::Float32)
                .unwrap()
                .with_representation(Some(WorkspaceRepresentation::new(
                    WorkspaceFloatingType::Float32,
                    true,
        ...
        let output = context
            .execute(
                WorkspaceOperationKind::Sampling(WorkspaceSamplingOperation::TopP),
                &[&input],
                vec![input.layout().clone()],
            )
            .unwrap()
...
pub(super) fn inspect(
    ...
    {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU filter changes its score geometry",
        ));
    }
    ...
    if input.elements()? != width as u64 || width > i32::MAX as usize {
        return Ok(None);
    }
    let mut p = Program {
        population: CpuPopulation::default(),
        bytes: 0,
        seeds: 0,
    ...
    let source = (|| {
        match kind {
            S::TopK { keep } if *keep != 0 && (*keep as usize) < width => {
                ...
                p.copy(
                    OperationEvent::cpu_partition_row_layout(rank, width, false)?,
                    1,
                    width,
                )?;
                ...
                if keep > 1 {
                    p.copy(
                        OperationEvent::cpu_row_min_layout(rank, keep, 1, false)?,
                        1,
                        1,
                    )?;
                }
            ...
            S::TopP => {
                let negative = OperationEvent::cpu_unary_layout(
                    CpuUnaryOperation::Negative,
                    Dtype::Float32,
                    rank,
                    false,
                )?;
                p.storage(width, 4, negative.backing_births())?;
                p.population.unary(negative)?;
                p.copy(
                    OperationEvent::cpu_argsort_layout(Dtype::Float32, rank, width, 1, false)?,
                    1,
                    width,
                )?;
                ...
                p.copy(
                    OperationEvent::cpu_gather_axis_row_layout(rank, width, false)?,
                    2,
                    width,
                )?;
                p.softmax(rank, width)?;
                p.copy(
                    OperationEvent::cpu_scan_sum_row_layout(rank, width, false)?,
                    1,
                    width,
                )?;
                ...
                p.alias(0, rank)?;
                p.copy(
                    OperationEvent::cpu_scalar_full_layout(Dtype::Float32, rank, width, false)?,
                    1,
                    width,
                )?;
                ...
                p.copy(
                    OperationEvent::cpu_scatter_axis_layout(
                        Dtype::Uint32,
                        rank,
                        width,
                        width,
    ...
    let frames = [
        ...
        size_of::<(&mut Program, usize, u64, usize)>(),
        size_of::<(&mut Program, CpuCopyEvalLayout, usize, usize)>(),
        size_of::<(&mut Program, usize, usize)>(),
        size_of::<(&mut Program, CpuBinaryOperation, usize, usize, bool)>(),
        size_of::<&mut Program>(),
