...
) -> crate::backend::nn::tensor::GroupedOutputStorage {
...
pub(super) fn matmul(
    ...
    let (rank, m, n, k, batches) = (
        u32::try_from(rank).ok()?,
        u32::try_from(m).ok()?,
        u32::try_from(n).ok()?,
        u32::try_from(k).ok()?,
        u32::try_from(batches).ok()?,
    );
    let selected = mechanism.matmul.selected();
    let geometry = if dtype == Dtype::Float16 {
        selected.float16_geometry(rank, m, n, k, batches)
    } else {
        selected.geometry(rank, m, n, k, batches)
    }
    .ok()?;
    let native = match dtype {
        Dtype::Float32 => {
            if copies == 0 {
                mechanism.matmul.eval_layout(geometry, false)?
            } else {
                mechanism
                    .matmul
                    .eval_layout_with_copies(geometry, copies, false)?
            }
        }
        Dtype::Float16 => mechanism
            .matmul
            .float16_eval_layout(geometry, copies, false)?,
        Dtype::Bfloat16 => OperationEvent::cpu_bf16_matmul_copy_layout(
            rank as usize,
            m as usize,
            n as usize,
            k as usize,
...
pub(super) fn inspect(
    ...
    if let Some(plan) = sliding::inspect(operation, mechanism)? {
        return Ok(Some(plan));
    }
    if let Some(plan) = explicit::inspect(operation, mechanism)? {
        return Ok(Some(plan));
    }
    if !matches!(
        operation.kind,
    ...
    if !(3..=4).contains(&operation.inputs.len()) || operation.outputs.len() != 1 {
        return Ok(None);
    }
    let q = operation.inputs.get(0).expect("query source");
    let k = operation.inputs.get(1).expect("key source");
    let v = operation.inputs.get(2).expect("value source");
    let output = operation.outputs.get(0).expect("attention output");
    for input in operation
        .inputs
        .slice(0..3)
        .expect("three attention operands")
        .iter()
    {
        if input.shape().len() != 4
            ...
            || !input
                .representation()
                .is_some_and(|r| r.last_axis_contiguous())
        {
            return Ok(None);
        }
    }
    let query_dtype = q
        .representation()
        .expect("checked query representation")
        .dtype();
    let key_dtype = k
        .representation()
        .expect("checked key representation")
        .dtype();
    let value_dtype = v
        .representation()
        .expect("checked value representation")
        .dtype();
    let dtype = super::super::representation::promote(
        super::super::representation::promote(query_dtype, key_dtype),
        value_dtype,
    );
    ...
    if k.shape()[0] != b
        ...
        || k.shape()[3] != d
        || h % kv != 0
        || output.shape() != [b, h, q_len, dv]
        || output.dtype() != WorkspaceDtype::Float32
    {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU attention query/key/value geometry differs",
        ));
    }
    // Q scaling produces a dense result. K keeps its source layout until the
    // first matmul, whose existing copy branch compacts noncanonical rows.
    let key_copies =
        usize::from(key_dtype == dtype && !k.representation().is_some_and(|r| r.row_contiguous()));
    let value_copies = usize::from(
        value_dtype == dtype && !v.representation().is_some_and(|r| r.row_contiguous()),
    );
    ...
    if let Some(mask) = mask {
        ...
        if mask.shape().len() > 4
            ...
            || mask
                ...
                .iter()
                .rev()
                .zip([b, h, q_len, k_len].iter().rev())
                .any(|(a, b)| *a != 1 && a != b)
        {
            return Err(MlxWorkspaceFactError::descriptor(
                "CPU attention mask does not broadcast to scores",
            ));
        }
    }
    let (b, h, q_len, d, kv, k_len, dv) = (
        usize::try_from(b)?,
        usize::try_from(h)?,
        usize::try_from(q_len)?,
        usize::try_from(d)?,
        usize::try_from(kv)?,
        usize::try_from(k_len)?,
        usize::try_from(dv)?,
    );
    let checked = || -> Option<(usize, usize, usize, usize, usize, usize)> {
        ...
        if [queries, keys, values, scores, output]
            .into_iter()
    ...
    let Some((batches, queries, keys, values, scores, produced)) = checked() else {
        return Ok(None);
    };
    let repeats = h / kv;
    let rank = if repeats > 1 { 5 } else { 4 };
    let source = (|| {
        ...
        cast(
            &mut p,
            native_dtype(value_dtype),
            native,
            4,
            values / repeats,
        )?;
        cast(&mut p, native, native, 0, 1)?;
        cast(&mut p, native, native, 4, queries)?;
        ...
        if repeats > 1 {
            ...
            p.copy(
                OperationEvent::cpu_expand_dims_alias_layout(4, 5, false)?,
                1,
            )?;
            p.copy(
                OperationEvent::cpu_expand_dims_alias_layout(4, 5, false)?,
        ...
        if let Some(mask) = mask {
            p.copy(
                OperationEvent::cpu_broadcast_alias_layout(mask.shape().len(), 4, false)?,
                1,
            )?;
        }
        p.copy(OperationEvent::cpu_transpose_alias_layout(rank, false)?, 1)?;
        matmul(
            &mut p, mechanism, native, rank, q_len, k_len, d, batches, key_copies,
        )?;
        if mask.is_some() {
            ...
            p.copy(
                OperationEvent::cpu_cast_layout(Dtype::Bool, Dtype::Bool, rank, scores, false)?,
                1,
            )?;
            ...
            p.copy(
                OperationEvent::cpu_typed_select_broadcast_layout(native, rank, scores, false)?,
                3,
            )?;
        }
        cast(&mut p, native, native, rank, scores)?;
        ...
        if repeats > 1 {
            p.copy(OperationEvent::cpu_reshape_alias_layout(5, 4, false)?, 1)?;
        }
        let controls = if mask.is_some() {
            OperationEvent::cpu_sdpa_array_mask_control_bytes(rank, queries, keys, values, scores)?
        } else {
            OperationEvent::cpu_sdpa_fallback_control_bytes(rank, queries, keys, values, scores)?
        };
    ...
    let maximum = [queries, keys, values, scores, produced]
        .into_iter()
        .max()
        .expect("fixed nonempty extents");
    let capacity = mechanism
        .allocation
        .fixed_buffer_capacity(facts::mul(u64::try_from(maximum)?, 4)?)?;
    let output_bytes = mechanism
        .allocation
        .fixed_buffer_capacity(facts::mul(u64::try_from(produced)?, 4)?)?;
    ...
    let frames = [
        ...
        size_of::<usize>() * 20,
        size_of::<[usize; 5]>() * 2,
        size_of::<std::array::IntoIter<usize, 5>>(),
        size_of::<(usize, usize, usize, usize, usize, usize)>(),
        size_of::<Option<(usize, usize, usize, usize, usize, usize)>>(),
        size_of::<[i32; 4]>(),
        size_of::<(
            &mut CpuPopulation,
        ...
        size_of::<(&mut CpuPopulation, Dtype, Dtype, usize, usize)>(),
