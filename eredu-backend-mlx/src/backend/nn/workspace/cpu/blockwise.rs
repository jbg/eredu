...
pub(super) fn inspect(
    ...
    if policy.output_type != Some(WorkspaceFloatingType::Float32) {
        return Ok(None);
    }
    ...
    if let Stage::Accumulate { .. } = stage {
        ...
        if [1, 2].into_iter().any(|i| {
            !operation
                .inputs
                .get(i)
                .is_some_and(|v| v.representation().is_some_and(|r| r.row_contiguous()))
        }) {
            return Ok(None);
        }
    }
        ...
        && policy.options.arithmetic == eredu_nn::AttentionArithmetic::InputScores
    {
        ...
        let Some(strides) = super::views::physical_strides(query, representation) else {
            return Ok(None);
        };
    ...
    let source = match stage {
        ...
        Stage::Finish {
            shape,
            input_scores,
        } => source::finish(shape, input_scores),
    };
    ...
    let maximum = mechanism
        .allocation
        .fixed_buffer_capacity(facts::mul(u64::try_from(source.maximum)?, 4)?)?;
    let total = facts::add(
        facts::mul(maximum, u64::try_from(source.native.births)?)?,
        facts::mul(
            mechanism.allocation.fixed_buffer_capacity(4)?,
            u64::try_from(source.seeds)?,
        )?,
    )?;
    let scratch_bytes = total
        .checked_sub(output_bytes)
        .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?;
    let shells = match stage {
        Stage::Begin { sink, .. } => usize::from(sink.is_some()),
        Stage::Accumulate { q, k, .. } => 2 * usize::from(q[1] == k[1]),
        _ => 0,
    };
    ...
    let frames = [
        shared,
        super::views::physical_stride_control_bytes()
            .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?,
        size_of::<source::Source>() * 3,
        size_of::<Option<source::Source>>(),
        ...
        size_of::<(
            ...
            eredu_nn::workspace::WorkspaceBlockwisePolicy,
            Stage<'_>,
            usize,
        )>(),
        size_of::<(
        ...
        size_of::<std::array::IntoIter<(usize, usize), 3>>(),
        size_of::<usize>() * 24,
        size_of::<u64>() * 4,
        size_of::<bool>() * 4,
        size_of::<Option<()>>(),
    ...
    let rank = match stage {
        Stage::Accumulate { q, k, .. } if q[1] != k[1] => 5,
        _ => 4,
    };
...
pub(super) fn emit_outputs(
    ...
    let Descriptor { stage, .. } =
        descriptor::decode(operation)?.expect("selected blockwise operation");
    match stage {
        Stage::Begin { mask, .. } => {
            sink.output(facts::Output::AliasInput(0))?;
            if mask.is_some() {
                sink.output(facts::Output::AliasInput(1))?;
            }
        }
        Stage::Finish {
            input_scores: true, ..
