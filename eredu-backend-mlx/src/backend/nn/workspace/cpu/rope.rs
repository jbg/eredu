...
pub(super) fn inspect(operation:WorkspaceOperationView<'_>,mechanism:MlxCpuWorkspaceMechanisms)
    ...
    if spec.arithmetic!=RotaryArithmetic::Native||!matches!(spec.algorithm,RotaryAlgorithm::Default|RotaryAlgorithm::Yarn{..})||spec.traditional {
        return Ok(None);
    }
    ...
    let batches=usize::try_from(input.shape()[0])?.checked_mul(if yarn {usize::try_from(input.shape()[1])?}else{1})
        .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?;
    let batch_elements=count/batches;
    let seeds=batches.checked_mul(3).and_then(|n|n.checked_add(usize::from(yarn)))
        .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?;
    let source=(|| {
        ...
        if yarn {
            ...
            binary(&mut p,CpuBinaryOperation::Multiply,4,count,(4,count,Dtype::Float32),(0,1,Dtype::Float32))?;
            p.copy(OperationEvent::cpu_reshape_alias_layout(4,3,false)?,1)?;
            p.hidden_leaves=1;
        }
        let mut batch=CpuPopulation::default();
        if batches!=1 {batch.copy(OperationEvent::cpu_slice_layout(rank, false, false)?,1)?;}
        fallback(&mut batch,rank,dimensions,batch_elements,positions,yarn)?;
        p.add(CpuPopulation {
            construction_entries:batch.construction_entries.checked_mul(batches)?,
            primitives:batch.primitives.checked_mul(batches)?,input_edges:batch.input_edges.checked_mul(batches)?,
            hidden_leaves:0,maximum_operands:batch.maximum_operands,maximum_captures:batch.maximum_captures,
            births:batch.births.checked_mul(batches)?,extents:batch.extents.checked_mul(batches)?,
        ...
        if batches!=1 {
            ...
            for _ in 0..batches {cast(&mut p,Dtype::Float32,rank,batch_elements)?;}
            p.concatenate(OperationEvent::cpu_concatenate_many_layout(Dtype::Float32,rank,batches,count,false)?,batches)?;
        }
        if yarn {p.copy(OperationEvent::cpu_reshape_alias_layout(3,4,false)?,1)?;}
        p.controls=p.controls.checked_add(crate::backend::nn::rope::rotary_control_bytes(false,batches)?)?;
        Some(p)
    })();
    ...
    let scratch=facts::mul(full,u64::try_from(population.births.checked_sub(1)
        .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?)?)?;
    let scratch_bytes=facts::add(scratch,facts::mul(mechanism.allocation.fixed_buffer_capacity(4)?,u64::try_from(seeds)?)?)?;
    let frames=[size_of::<CpuPopulation>()*3,size_of::<Option<CpuPopulation>>(),size_of::<OperationPlan>(),
        ...
        size_of::<MlxCpuWorkspaceMechanisms>(),size_of::<eredu_nn::RotarySpec>(),size_of::<i32>(),size_of::<usize>()*12,size_of::<bool>(),
        size_of::<(&mut CpuPopulation,usize,usize,usize,usize,bool)>(),size_of::<std::ops::Range<usize>>(),
    ...
    Ok(Some(OperationPlan{dtype:WorkspaceFloatingType::Float32,population,output_bytes:full,scratch_bytes,
        rank:4,parameter_shells:0,alias_input:None,seeds,validations:0}))
}
...
fn fallback(p:&mut CpuPopulation,rank:usize,dimensions:usize,count:usize,positions:usize,yarn:bool)->Option<()> {
    let half=dimensions/2;let theta=positions.checked_mul(half)?;
    if rank==3 {p.copy(OperationEvent::cpu_expand_dims_alias_layout(3,4,false)?,1)?;}
    p.copy(OperationEvent::cpu_arange_float_layout(positions,false)?,0)?;
    binary(p,CpuBinaryOperation::Add,1,positions,(1,positions,Dtype::Float32),(0,1,Dtype::Int32))?;
    binary(p,CpuBinaryOperation::Multiply,1,positions,(1,positions,Dtype::Float32),(0,1,Dtype::Float32))?;
    if yarn {
        binary(p,CpuBinaryOperation::Divide,1,half,(0,1,Dtype::Float32),(1,half,Dtype::Float32))?;
    }else{
        p.copy(OperationEvent::cpu_arange_float_layout(half,false)?,0)?;
        binary(p,CpuBinaryOperation::Multiply,1,half,(1,half,Dtype::Float32),(0,1,Dtype::Float32))?;
        unary(p,CpuUnaryOperation::Exponential,1,half)?;
    }
    p.copy(OperationEvent::cpu_expand_dims_alias_layout(1,2,false)?,1)?;
    binary(p,CpuBinaryOperation::Multiply,2,theta,(2,positions,Dtype::Float32),(1,half,Dtype::Float32))?;
    for kind in [CpuUnaryOperation::Cosine,CpuUnaryOperation::Sine] {
        unary(p,kind,2,theta)?;cast(p,Dtype::Float32,2,theta)?;
    }
    p.copy(OperationEvent::cpu_slice_layout(4, false, false)?,1)?;
    p.copy(OperationEvent::cpu_slice_layout(4, false, false)?,1)?;
    for combine in [CpuBinaryOperation::Subtract,CpuBinaryOperation::Add] {
        for _ in 0..2 {
            binary(p,CpuBinaryOperation::Multiply,4,count/2,(4,count/2,Dtype::Float32),(2,theta,Dtype::Float32))?;
        }
        binary(p,combine,4,count/2,(4,count/2,Dtype::Float32),(4,count/2,Dtype::Float32))?;
    }
    cast(p,Dtype::Float32,4,count/2)?;cast(p,Dtype::Float32,4,count/2)?;
    p.concatenate(OperationEvent::cpu_concatenate_layout(Dtype::Float32,4,count/2,count/2,false)?,2)?;
    p.copy(OperationEvent::cpu_reshape_alias_layout(4,rank,false)?,1)?;
    p.controls=p.controls.checked_add(OperationEvent::cpu_rope_fallback_control_bytes(rank,dimensions,count)?)?;
    Some(())
}
...
mod tests {
    ...
    fn cpu_transposed_head_rotary_preserves_exact_sources_and_dense_output() {
        ...
        for (batch,positions,heads,width) in [(1,2,2,4),(2,3,2,8)] {
            ...
            assert_eq!(plan.seeds,3*batch as usize);assert_eq!(plan.alias_input,None);
