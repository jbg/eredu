...
pub(super) fn inspect(
    ...
    if !matches!(operation.kind, WorkspaceOperationKindView::Concatenate) {
        return Ok(None);
    }
    if operation.inputs.len() == 1 && operation.outputs.len() == 1 {
        ...
        if input.shape() != output.shape() || input.dtype() != output.dtype() {
            return Err(MlxWorkspaceFactError::descriptor(
                "CPU singleton concatenate changes its source",
            ));
        }
        ...
        let controls = controls
            .into_iter()
            .try_fold(size_of_val(&controls), |n, b| {
                n.checked_add(b)
                    .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)
            })?;
        return Ok(Some(OperationPlan {
            dtype: input
                .representation()
                .map_or(WorkspaceFloatingType::Float32, |r| r.dtype()),
            population: CpuPopulation {
                controls,
                ..CpuPopulation::default()
            },
            alias_input: Some(0),
            output_bytes: 0,
            scratch_bytes: 0,
    ...
    if inputs < 2 || operation.outputs.len() != 1 {
        return Ok(None);
    }
    ...
    if !(1..=4).contains(&rank) || output.shape().len() != rank {
        return Ok(None);
    }
    ...
    if output.dtype() != first.dtype() || output.shape().iter().any(|&n| n < 0) {
        return Ok(None);
    }
    ...
    for input in operation.inputs.iter() {
        if input.shape().len() != rank
            ...
            || match floating {
                Some(dtype) => !input
                    .representation()
                    .is_some_and(|r| r.dtype() == dtype && r.last_axis_contiguous()),
    ...
    for candidate in 0..rank {
        ...
        for input in operation.inputs.iter() {
            for index in 0..rank {
                if index != candidate && input.shape()[index] != output.shape()[index] {
                    valid = false;
                }
            }
            extent = extent
                .checked_add(input.shape()[candidate])
                .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?;
        }
        if valid && extent == output.shape()[candidate] {
            axis = Some(candidate);
            break;
        }
    }
    if axis.is_none() || elements != output.elements()? {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU concatenate output extent differs",
        ));
    }
    ...
    let Some(source) = OperationEvent::cpu_concatenate_many_layout(
        native,
        rank,
        inputs,
    ...
    if source.backing_births() != usize::from(elements != 0)
        || population.concatenate(source, inputs).is_none()
    {
        return Ok(None);
    }
    ...
    fn cpu_ordered_logical_members_share_one_exact_concat_and_completion() {
        ...
        for count in [2, 8] {
            for dtype in [
                ...
                let world = WorkspaceTensor::existing(
                    context
                        .layout(&[count, 1, 2, 19], WorkspaceDtype::Float32)
                        .unwrap()
                ...
                let stacked =
                    logical_collective::packed::gather_stacked(&ops, &world, &members).unwrap();
                let output = logical_collective::packed::flatten(
                    &ops,
                    &stacked,
                    &[1, 2, 19],
                    count as usize,
...
mod empty_integer_tests {
    ...
    fn value(context: &WorkspaceContext, rows: i32, dtype: WorkspaceDtype) -> WorkspaceTensor {
        let representation = (dtype == WorkspaceDtype::Float32).then_some(
        ...
        for dtype in [WorkspaceDtype::Int32, WorkspaceDtype::Float32] {
            for counts in [[0usize, 0, 0], [0, 3, 0], [2, 0, 3]] {
                for join in [false, true] {
                    ...
                    let ops = logical_collective::Workspace(&context);
                    let output = if join {
                        blocks::join(&ops, &inputs).unwrap()
                    } else {
                        blocks::concatenate(&ops, &source, &counts, (0..3).rev()).unwrap()
                    };
                    assert_eq!(output.shape(), source.shape());
                    assert_eq!(output.layout().dtype(), dtype);
                    if dtype == WorkspaceDtype::Int32 {
                    ...
                    for operation in &report.operations {
                        ...
                        } else if super::super::super::zero_fill::dtype(operation.as_view())
                            .is_some()
                        {
    ...
    fn cpu_integer_join_rejects_mixed_dtype_and_false_join_geometry() {
        ...
        for (right, output) in [(b, bad), (mixed, b)] {
            ...
            let operation = WorkspaceOperationView {
                kind: WorkspaceOperationKindView::Concatenate,
