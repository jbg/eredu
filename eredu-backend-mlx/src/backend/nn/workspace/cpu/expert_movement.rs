...
fn gather(
    ...
    if output.dtype() != input.dtype()
        || !output
            .shape()
            .iter()
            .eq(indices.shape().iter().chain(&input.shape()[1..]))
    {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU take-axis output geometry differs",
        ));
    }
    let Some((dtype, native)) = floating(input) else {
        return Ok(None);
    };
    ...
    if [count, input_elements, output_elements]
        .into_iter()
        .any(|n| n > i32::MAX as usize)
        || (count != 0 && input.shape()[0] == 0)
    {
        return Ok(None);
    }
    // One index selects the complete trailing slice. The same native Gather
    // authenticates source ranks through four and the joined rank through five.
    let width = input.shape()[1..].iter().try_fold(1usize, |n, &extent| {
        n.checked_mul(extent as usize)
            .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)
    })?;
    let mut population = CpuPopulation::default();
    let mut scratch_bytes = 0;
    let (seeds, validations) = if count == 0 {
        (0, 0)
    } else {
        let Some(prefix) = embedding::checked_take_indices(1, count, mechanism)? else {
            return Ok(None);
        };
        population
            .add(prefix.population)
            .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?;
    ...
    let Some(source) = OperationEvent::cpu_gather_layout(
        native,
        Dtype::Int32,
        rank,
        1,
    ...
    if source.backing_births() != usize::from(output_elements != 0)
        || population.copy(source, 2).is_none()
    {
        return Ok(None);
    }
    let Some(squeeze) = OperationEvent::cpu_squeeze_layout(rank + 1, false) else {
        return Ok(None);
    };
    ...
    let frames = [
        ...
        size_of::<(WorkspaceFloatingType, Dtype)>(),
        size_of::<Option<(WorkspaceFloatingType, Dtype)>>(),
        size_of::<[usize; 5]>(),
        size_of::<u64>(),
        size_of::<(usize, usize)>(),
        ...
        size_of::<(&safemlx::Array, &safemlx::Array, i32, &safemlx::Stream)>(),
        size_of::<(&safemlx::Array, i32, &safemlx::Stream)>(),
        size_of::<bool>(),
        crate::backend::nn::expert_movement::control_bytes::<
            crate::backend::nn::expert_movement::Native<'_>,
        >()
        .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?,
    ];
    ...
    Ok(Some(OperationPlan {
        ...
        scratch_bytes,
        rank: rank + 1,
        parameter_shells: usize::from(count == 0),
        seeds,
        validations,
    }))
}
...
fn add(
    ...
    if !super::super::basic::indexed_row_add(operation) {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU additive rows geometry differs",
        ));
    }
    ...
    if base.shape().iter().chain(updates.shape()).any(|&n| n < 0)
        || base.shape()[1] <= 0
        || floating(base) != Some((WorkspaceFloatingType::Float32, Dtype::Float32))
        || floating(updates) != Some((WorkspaceFloatingType::Float32, Dtype::Float32))
    {
        return Ok(None);
    }
    ...
    if [output_elements, update_elements]
        .into_iter()
        .any(|n| n > i32::MAX as usize)
    {
        return Ok(None);
    ...
    if output_elements != 0 {
        ...
        if base.shape()[1] != 1 {
            let broadcast = if update_elements == 0 {
                OperationEvent::cpu_empty_broadcast_layout(2, 2, false)
            } else {
                OperationEvent::cpu_broadcast_alias_layout(2, 2, false)
            };
        ...
        let Some(scatter) = OperationEvent::cpu_scatter_add_rows_layout(
            index,
            output_elements,
            update_elements,
            false,
        ...
        if scatter.backing_births() != 1 || population.copy(scatter, 3).is_none() {
            return Ok(None);
        }
    ...
    Ok(Some(OperationPlan {
        ...
        output_bytes: if output_elements == 0 {
            0
        } else {
            mechanism
                .allocation
        ...
        scratch_bytes: 0,
        rank: 2,
        parameter_shells: usize::from(output_elements == 0),
        seeds: 0,
        validations: 0,
    }))
}
