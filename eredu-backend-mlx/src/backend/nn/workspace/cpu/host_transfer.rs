...
pub(in crate::backend::nn::workspace) fn is_load(kind: WorkspaceOperationKindView<'_>) -> bool {
    matches!(
        kind,
        WorkspaceOperationKindView::HostTransferFloating(_)
            | WorkspaceOperationKindView::HostLoadStoredFloating(..)
    )
}
pub(super) fn inspect(
    ...
    if !store && !is_load(operation.kind) {
        return Ok(None);
    }
    let Some(dtype) = basic::host_transfer_dtype(operation) else {
        return Ok(None);
    };
    let value = if store {
        operation.inputs.get(0)
    } else {
        operation.outputs.get(0)
    }
    ...
    let Some(source) = OperationEvent::cpu_host_transfer_layout(dtype, rank, store, false) else {
        return Ok(None);
    };
    let mut population = CpuPopulation::default();
    if population.copy(source, 1).is_none() {
        return Ok(None);
    }
    let frontend = if store {
        ...
        let Some(compact) = OperationEvent::cpu_contiguous_layout(rank, false) else {
            return Ok(None);
        };
        if population.copy(compact, 1).is_none() {
            return Ok(None);
        }
    ...
    let Some((controls, extent)) = frontend else {
        return Ok(None);
    };
    let frames = [
        controls,
    ...
    Ok(Some(OperationPlan {
        ...
        population,
        alias_input: None,
        output_bytes: if store { 0 } else { bytes },
        scratch_bytes: if store { bytes } else { 0 },
        rank,
        parameter_shells: 0,
        seeds: 0,
        validations: 0,
