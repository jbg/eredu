...
pub(super) fn inspect(operation:WorkspaceOperationView<'_>,mechanism:MlxCpuWorkspaceMechanisms)
    ...
    let activation=WorkspaceOperationView {kind:WorkspaceOperationKindView::Elementwise(name),
        inputs:operation.inputs.slice(0..1).expect("checked source slice"),outputs:operation.outputs};
    let activation=if name=="silu" {super::activation::inspect(activation,mechanism)?}
        else {super::gelu::inspect(activation,mechanism)?};
    let Some(activation)=activation else{return Ok(None)};
    let activated=gate.with_representation(Some(WorkspaceRepresentation::new(activation.dtype,true)));
    let inputs=[activated,up];
    let product=WorkspaceOperationView {kind:WorkspaceOperationKindView::Elementwise("multiply"),
        inputs:WorkspaceLayoutList::Views(&inputs),outputs:operation.outputs};
    ...
    let frames=[size_of::<WorkspaceOperationView<'_>>()*3,size_of::<WorkspaceLayoutView<'_>>()*5,
