...
pub(super) fn inspect(
    ...
    let WorkspaceOperationKindView::GatedProduct(policy) = operation.kind else {
        return Ok(None);
    };
    policy.validate_fixed()?;
    if policy.sigmoid_multiplier() != 1.0
        || policy.gate_upper_bound().is_some()
        || policy.up_absolute_bound().is_some()
    ...
    if operation.inputs.len() != 2 || operation.outputs.len() != 1 {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU gated-product population differs",
        ));
    }
    ...
    if gate.shape() != up.shape() {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU gated-product shape differs",
        ));
    }
    ...
    let name = match policy.activation() {
        GatedProductActivation::Silu => "silu",
        GatedProductActivation::GeluApproximate => "gelu_approximate",
        _ => return Ok(None),
    };
    let activation = WorkspaceOperationView {
        kind: WorkspaceOperationKindView::Elementwise(name),
        inputs: operation.inputs.slice(0..1).expect("checked source slice"),
        outputs: operation.outputs,
    };
    let activation = if name == "silu" {
        super::activation::inspect(activation, mechanism)?
    } else {
        super::gelu::inspect(activation, mechanism)?
    };
    let Some(activation) = activation else {
        return Ok(None);
    };
    let activated =
        gate.with_representation(Some(WorkspaceRepresentation::new(activation.dtype, true)));
    let inputs = [activated, up];
    let product = WorkspaceOperationView {
        kind: WorkspaceOperationKindView::Elementwise("multiply"),
        inputs: WorkspaceLayoutList::Views(&inputs),
        outputs: operation.outputs,
    };
    let Some(mut product) = super::pointwise::inspect(product, mechanism)? else {
        return Ok(None);
    };
    product
        .population
        .add(activation.population)
        .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?;
    product.scratch_bytes = facts::add(
        product.scratch_bytes,
        facts::add(activation.output_bytes, activation.scratch_bytes)?,
    )?;
    ...
    let frames = [
        ...
        size_of::<OperationPlan>() * 2,
        size_of::<Option<OperationPlan>>() * 2,
        size_of::<MlxCpuWorkspaceMechanisms>(),
        size_of::<eredu_nn::GatedProductPolicy>(),
        ...
        size_of::<(
            crate::MlxTensor,
            crate::MlxTensor,
            eredu_nn::GatedProductPolicy,
            &safemlx::Stream,
        )>(),
        size_of::<safemlx::Array>() * 3,
        size_of::<Result<crate::MlxTensor, eredu_nn::Error>>(),
    ];
    product.population.controls = frames.into_iter().try_fold(
        product
            .population
            .controls
