...
fn input_products_oracle_error(theta: f32, left: f32, right: f32, amplitude: f32) -> f64 {
    ...
    let phase = (f64::from(theta.abs())
        - (f64::from(j) * std::f64::consts::FRAC_PI_4 + f64::from(r)))
    .abs();
    ...
    let sine = (sin[0] + 1.0 / 6.0).abs() * x.powi(3)
        ...
        + x.powi(9) / 362880.0
        + gamma(7)
            * (x + sin[0].abs() * x.powi(3) + sin[1].abs() * x.powi(5) + sin[2].abs() * x.powi(7));
    ...
    let magnitude = f64::from(amplitude.abs()) * (f64::from(left.abs()) + f64::from(right.abs()));
...
fn cpu_yarn_source_counts_flattened_batches_and_retained_frequency_operand() {
    ...
    for (batch, heads, positions, width) in [(1, 1, 1, 4), (1, 2, 2, 4), (2, 3, 3, 8)] {
        ...
        let input = WorkspaceTensor::existing(
            context
                .layout(&[batch, heads, positions, width], WorkspaceDtype::Float32)
                .unwrap()
        ...
        context.begin_span();
        let output = rope
            .forward(&input, RotaryPosition::Offset(5), &context)
            .unwrap();
        assert_eq!(
        ...
        assert_eq!(
            plan.population.maximum_operands,
            2.max((batch * heads) as usize)
        );
        ...
        let recipe =
            SpeculativeNumericalRecipe::inspect_cpu_equations(&report, ordinary, cpu, &context)
                .unwrap();
        assert_eq!(
            recipe.completion.graph.maximum_operands(),
            4.max((batch * heads) as usize)
        );
        assert_eq!(
            recipe.completion.traversal.limits().captures,
            8.max(captures)
        );
        assert_eq!(
...
fn run_native(explicit: bool) {
    ...
    for (batch, heads, positions, width) in [(1, 1, 1, 4), (1, 2, 2, 4), (2, 3, 3, 8)] {
        for transposed in [false, true] {
            if !explicit && transposed {
                continue;
            }
            let offset = if explicit && transposed {
                16_777_216
            } else {
                5
            };
            ...
            if explicit {
                selected_spec.arithmetic = RotaryArithmetic::InputProducts;
            }
            let shape = [batch, heads, positions, width];
            let count = (batch * heads * positions * width) as usize;
            let source_shape = if transposed {
            ...
            let input = MlxTensor::from_array(if transposed {
                source.transpose_axes(&[0, 2, 1, 3], stream).unwrap()
            } else {
                source.clone()
            });
            ...
            drop(reference);
            assert!(
                expected.iter().all(|x| x.is_finite()) && expected.iter().any(|x| x.abs() > 1e-5)
            );
            ...
            for row in 0..(batch * heads * positions) as usize {
                for col in 0..width as usize / 2 {
                    ...
                    let source_row = if transposed {
                        (batch_index * positions as usize + position) * heads as usize + head
                    } else {
                        row
                    };
                    ...
                    let (values, tolerance) = if explicit {
                        ...
                        (
                            [
                                f64::from(left * cos - right * sin),
                                f64::from(right * cos + left * sin),
                            ],
                            3e-6,
                        )
                    };
            ...
            let retained_facts = sources
                .iter()
                .map(|value| value.as_array().try_descriptor().unwrap().facts())
                .collect::<Vec<_>>();
            let context = WorkspaceContext::new(cpu);
            let projected = WorkspaceTensor::existing(
                context
                    .layout(&source_shape, WorkspaceDtype::Float32)
                    .unwrap()
                    .with_representation(Some(WorkspaceRepresentation::new(
                        WorkspaceFloatingType::Float32,
                        true,
            ...
            .unwrap();
            let mut quoted = WorkspaceBackend::rotary(selected_spec, &context).unwrap();
            context.begin_span();
            ...
            let projected = if transposed {
                projected.transpose_axes(&[0, 2, 1, 3], &context).unwrap()
            } else {
                projected
            };
            let output = quoted
                .forward(&projected, RotaryPosition::Offset(offset), &context)
                .unwrap();
            ...
            let report = if transposed {
                context.finish_report(&[output, projected])
            } else {
            ...
            let failure = PreparedPrefillFailure::try_new(owner.clone())
                ...
                .unwrap();
            let mut roots =
                PrefillRoots::new_retained(&runtime, output_roots, &graph, &failure).unwrap();
            ...
            let actual = module
                .forward(
                    scoped_input.as_ref().unwrap_or(&input),
            ...
            if let Some(view) = &scoped_input {
                roots.append(view.as_array()).unwrap();
            }
            ...
            if let Some(view) = &scoped_input {
                ...
                let (allocation, strides) = {
                    ...
                    (
                        descriptor
                            .facts()
                            .allocation()
                            .expect("completed original source backing"),
                ...
                if heads > 1 && positions > 1 {
                    assert_ne!(
                        descriptor.completed_strides().unwrap(),
                        &[
                            (heads * positions * width) as i64,
...
fn cpu_input_products_yarn_uses_exact_flatten_stride_proof_and_integer_coordinates() {
    ...
    for algorithm in [
        ...
        let mut selected_spec = spec(8);
        selected_spec.arithmetic = RotaryArithmetic::InputProducts;
        selected_spec.algorithm = algorithm;
        ...
        for transposed in [false, true] {
            ...
            context.begin_span();
            let output = rope
                .forward(&input, RotaryPosition::Offset(16_777_216), &context)
                .unwrap();
            assert_eq!(
                output.layout().representation(),
                Some(WorkspaceRepresentation::new(
            ...
            assert_eq!(plan.population.hidden_leaves, 1);
            if transposed {
                assert_eq!(Some(plan.population.births - 1), dense_births);
            } else {
                dense_births = Some(plan.population.births);
            }
            let recipe =
                SpeculativeNumericalRecipe::inspect_cpu_equations(&report, ordinary, cpu, &context)
                    .unwrap();
            ...
            assert!(cpu
                .plan(WorkspaceOperationView {
                    kind: WorkspaceOperationKindView::Rotary(selected_spec, Some(i32::MAX)),
                    ..operation
                })
                .unwrap()
                .is_none());
...
fn cpu_input_products_recipe_pays_transpose_alias_without_an_extra_backing() {
    ...
    for internal in [false, true] {
        ...
        let input = WorkspaceTensor::existing(
            context
                .layout(&[2, 5, 3, 8], WorkspaceDtype::Float32)
                .unwrap()
                .with_representation(Some(WorkspaceRepresentation::new(
                    WorkspaceFloatingType::Float32,
                    true,
        ...
        let report = if internal {
            context.finish_report(&[output, input])
        } else {
            context.finish_report(&[output])
        }
        .unwrap();
        ...
        if internal {
            ...
            external = Some((
                recipe.completion.graph.primitives(),
                recipe.storage.maximum_births(),
                recipe.completion.traversal.limits().arrays,
            ));
        }
