...
pub(super) fn inspect(operation:WorkspaceOperationView<'_>,mechanism:MlxCpuWorkspaceMechanisms,
    spec:eredu_nn::RotarySpec,offset:i32)->facts::FactResult<Option<OperationPlan>> {
    if spec.traditional || !matches!(spec.algorithm,RotaryAlgorithm::Default|RotaryAlgorithm::Linear{..}|RotaryAlgorithm::Yarn{..}) {
    ...
    if spec.dimensions<=0||spec.dimensions%2!=0||!spec.base.is_finite()||spec.base<=0.0 {
        return Err(MlxWorkspaceFactError::descriptor("CPU explicit RoPE scalar geometry differs"));
    }
    ...
    if input.shape().len()!=4||input.shape().iter().any(|&n|n<=0)||spec.dimensions!=input.shape()[3]
        ||input.dtype()!=WorkspaceDtype::Float32||output.dtype()!=WorkspaceDtype::Float32{return Ok(None);}
    let Some(representation)=input.representation().filter(|r|r.dtype()==WorkspaceFloatingType::Float32&&r.last_axis_contiguous()) else{return Ok(None)};
    if input.shape()!=output.shape(){return Err(MlxWorkspaceFactError::descriptor("CPU explicit RoPE output geometry differs"));}
    let count=usize::try_from(input.elements()?)?;
    if count>i32::MAX as usize||offset.checked_add(input.shape()[2]).is_none(){return Ok(None);}
    let positions=usize::try_from(input.shape()[2])?;let half=usize::try_from(spec.dimensions)?/2;
    let theta=positions.checked_mul(half).ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?;
    let flattened=[input.shape()[0].checked_mul(input.shape()[1]).ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?,input.shape()[2],input.shape()[3]];
    let source=(|| {
        ...
        p.copy(OperationEvent::cpu_reshape_layout(input.shape(),&strides,&flattened,false)?,1)?;
        ...
        p.copy(OperationEvent::cpu_arange_int_layout(safemlx::Dtype::Int32,positions,false)?,0)?;
        ...
        for kind in [CpuUnaryOperation::Cosine,CpuUnaryOperation::Sine] {
            ...
            binary(&mut p,CpuBinaryOperation::Multiply,2,theta,(2,theta,Dtype::Float32),(0,1,Dtype::Float32))?;
        ...
        for _ in 0..3 {p.copy(OperationEvent::cpu_slice_layout(3, false, false)?,1)?;}
        for combine in [CpuBinaryOperation::Subtract,CpuBinaryOperation::Add] {
            for _ in 0..2 {
                binary(&mut p,CpuBinaryOperation::Multiply,3,count/2,(3,count/2,Dtype::Float32),(2,theta,Dtype::Float32))?;
            }
            binary(&mut p,combine,3,count/2,(3,count/2,Dtype::Float32),(3,count/2,Dtype::Float32))?;
        }
        cast(&mut p,Dtype::Float32,3,count/2)?;cast(&mut p,Dtype::Float32,3,count/2)?;
        p.concatenate(OperationEvent::cpu_concatenate_layout(Dtype::Float32,3,count/2,count/2,false)?,2)?;
        ...
        p.controls=p.controls.checked_add(super::super::views::physical_stride_control_bytes()?)?
    ...
    let scratch_bytes=facts::add(facts::mul(full,u64::try_from(population.births.checked_sub(1)
        .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?)?)?,mechanism.allocation.fixed_buffer_capacity(4)?)?;
    let frames=[size_of::<CpuPopulation>()*2,size_of::<Option<CpuPopulation>>(),size_of::<OperationPlan>(),
        size_of::<Option<OperationPlan>>(),size_of::<WorkspaceOperationView<'_>>(),size_of::<WorkspaceLayoutView<'_>>()*2,
        size_of::<MlxCpuWorkspaceMechanisms>(),size_of::<eredu_nn::RotarySpec>(),size_of::<i32>(),size_of::<usize>()*4,
        size_of::<[i32;3]>(),size_of::<[i64;4]>(),size_of::<WorkspaceRepresentation>(),size_of::<Option<WorkspaceRepresentation>>(),
        ...
        size_of::<std::slice::Iter<i32>>(),size_of::<u64>()*2,
        size_of::<(WorkspaceOperationView<'_>,MlxCpuWorkspaceMechanisms,eredu_nn::RotarySpec,i32)>(),
        size_of::<(&crate::backend::nn::rope::ElementwiseRotary,&safemlx::Array,i32,&safemlx::Stream)>(),
        size_of::<(&safemlx::Array,&[i32],i32,i32)>(),size_of::<[safemlx::Array;8]>(),
    ...
    Ok(Some(OperationPlan{dtype:WorkspaceFloatingType::Float32,population,alias_input:None,
        output_bytes:full,scratch_bytes,rank:4,parameter_shells:0,seeds:1,validations:0}))
}
