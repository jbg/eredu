...
pub(super) fn inspect(
    ...
    let Some([input]) = operation.inputs.array() else {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU flat reduction input population differs",
        ));
    };
    let Some([output]) = operation.outputs.array() else {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU flat reduction output population differs",
        ));
    };
    ...
    if !output.shape().is_empty() || input.dtype() != output.dtype() {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU flat reduction changes scalar dtype or geometry",
        ));
    }
    ...
    let Ok(elements) = usize::try_from(input.shape()[0]) else {
        return Ok(None);
    };
    if elements == 0 {
        return Ok(None);
...
mod tests {
    ...
    fn cpu_flat_capture_reductions_preserve_scalar_and_singleton_storage() {
        ...
        for elements in [1, 19] {
            for (name, dtype) in [
                ("sum", WorkspaceDtype::Uint32),
                ("sum", WorkspaceDtype::Float32),
                ("min", WorkspaceDtype::Float32),
                ("max", WorkspaceDtype::Float32),
            ] {
                let context = WorkspaceContext::new(cpu);
                let storage = WorkspaceExistingStorage::try_new(Some(4096), &context).unwrap();
                let layout = context
                    .layout(&[elements], dtype)
                    .unwrap()
                    .with_representation((dtype == WorkspaceDtype::Float32).then_some(
                ...
                let output = context
                    .execute(
                        WorkspaceOperationKind::Reduction(name, 0, false),
                        &[&input],
                        vec![context.layout(&[], dtype).unwrap()],
                    )
                    .unwrap()
                    .remove(0);
                let report = context
                    .finish_report(std::slice::from_ref(&output))
                    .unwrap();
                assert!(
                ...
                assert_eq!(
                    report.state.as_ref().unwrap().displaced_bytes,
                    Some(if elements == 1 { 0 } else { 4096 })
                );
                if elements == 1 {
