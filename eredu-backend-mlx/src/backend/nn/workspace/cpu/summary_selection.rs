...
pub(super) fn inspect(
    ...
    let Some([output]) = operation.outputs.array() else {
        return Err(MlxWorkspaceFactError::descriptor(
            "CPU scalar/selection output population differs",
        ));
    };
    ...
    if scalar {
        if !basic::is_scalar_f32(operation) {
            return Err(MlxWorkspaceFactError::descriptor(
                "CPU eager scalar differs from one F32 value",
            ));
        }
    } else {
        let Some([mask, selected, other]) = operation.inputs.array() else {
            return Err(MlxWorkspaceFactError::descriptor(
                "CPU selection requires mask and two values",
            ));
        };
        if output.dtype() != WorkspaceDtype::Float32
            || selected.dtype() != output.dtype()
            || other.dtype() != output.dtype()
            || mask.dtype() != WorkspaceDtype::Bool
            || mask.shape() != output.shape()
            || selected.shape() != output.shape()
            || (!other.shape().is_empty() && other.shape() != output.shape())
        {
            return Err(MlxWorkspaceFactError::descriptor(
                "CPU selection changes its declared mask/value geometry",
            ));
        }
        if rank > 4
            || output.shape().iter().any(|&n| n <= 0)
            || !strided_capture::f32_source(selected)
            || !strided_capture::f32_source(other)
        {
            return Ok(None);
        }
        ...
        if broadcast {
            let Some(alias) = OperationEvent::cpu_broadcast_alias_layout(0, rank, false) else {
                return Ok(None);
            };
        ...
        let strided = !selected
            .representation()
            .expect("qualified F32 source")
            .row_contiguous()
            || !other
                .representation()
                .expect("qualified F32 source")
        ...
        let source = if strided || (broadcast && elements > 1) {
            ...
            OperationEvent::cpu_select_layout(Dtype::Float32, rank, elements, false)
        };
        let Some(source) = source else {
            return Ok(None);
        };
        if source.backing_births() != 1 || population.copy(source, 3).is_none() {
            return Ok(None);
        }
    }
    if scalar {
    ...
    let frames = [
        ...
        size_of::<Option<[WorkspaceLayoutView<'_>; 3]>>(),
...
mod tests {
    ...
    fn scalar_source_is_exact_and_where_refuses_unrepresented_values() {
        ...
        for context in [WorkspaceContext::new(cpu), WorkspaceContext::new(ordinary)] {
            let scalar = context
                .execute(
                    WorkspaceOperationKind::Elementwise("scalar_f32"),
                    &[],
                    vec![context.layout(&[], WorkspaceDtype::Float32).unwrap()],
                )
                .unwrap()
                .remove(0);
            assert_eq!(
            ...
            let report = context
                .finish_report(std::slice::from_ref(&scalar))
                .unwrap();
            assert!(
                report.unpriced_operations.is_empty() && report.unpriced_host_operations.is_empty()
            );
        ...
        assert!(cpu
            .plan(WorkspaceOperationView {
                kind: WorkspaceOperationKindView::Elementwise("where"),
