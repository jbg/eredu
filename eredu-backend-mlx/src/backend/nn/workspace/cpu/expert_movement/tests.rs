...
fn value(
    ...
    physical: Option<WorkspaceFloatingType>,
) -> WorkspaceTensor {
    WorkspaceTensor::existing(
        context
            .layout(shape, dtype)
            .unwrap()
            .with_representation(physical.map(|dtype| WorkspaceRepresentation::new(dtype, true))),
        context,
    )
    .unwrap()
}
    ...
    for dtype in [
        ...
        for (source_rows, count) in [(0, 0), (7, 0), (7, 1), (7, 7)] {
            for route_values in [false, true] {
                ...
                context.begin_span();
                let output = movement::gather(
                    &movement::Workspace(&context),
                    &input,
                    &indices,
                    route_values,
                )
                ...
                let operation = report
                    ...
                    .iter()
                    .find(|operation| {
                        matches!(operation.kind, WorkspaceOperationKind::Gather { axis: 0 })
                    })
                    .unwrap();
                ...
                for operation in &report.operations {
                    population
                        .add(cpu.plan(operation.as_view()).unwrap().unwrap().population)
                        .unwrap();
                }
                assert_eq!(
                ...
                assert_eq!(
                    recipe.completion.traversal.limits().tape_entries,
                    population.primitives + 1
                );
            }
        }
    }
    ...
    for width in [1, 3] {
        for (rows, count) in [(0, 0), (7, 0), (7, 1), (7, 7)] {
            for index_dtype in [WorkspaceDtype::Int32, WorkspaceDtype::Uint32] {
                ...
                context.begin_span();
                let output =
                    movement::add(&movement::Workspace(&context), &base, &indices, &updates)
                        .unwrap();
                ...
                let recipe = SpeculativeNumericalRecipe::inspect_owned_child(
                    &report,
                    1,
                    ResidentExecutionMechanisms::Cpu {
                        ordinary: native,
                ...
                for (joined, repetitions) in [
                    (source.union(source).unwrap(), 1),
                    (source.append(source).unwrap(), 2),
                ] {
                    let joined = joined.finish(1, 0, &context).unwrap();
                    assert_eq!(
                        joined.completion.graph.primitives(),
                        repetitions * plan.population.construction_entries
                    );
                    assert_eq!(
                        joined.completion.traversal.limits().tape_entries,
                        repetitions * plan.population.primitives + 1
                    );
                }
            }
        }
...
fn cpu_expert_movement_refuses_unknown_precision_and_malformed_sources() {
    ...
    let inputs = [base, indices, update];
    let outputs = [base];
    let operation = WorkspaceOperationView {
        kind: WorkspaceOperationKindView::IndexedRowAdd,
        inputs: WorkspaceLayoutList::Views(&inputs),
        outputs: WorkspaceLayoutList::Views(&outputs),
    };
