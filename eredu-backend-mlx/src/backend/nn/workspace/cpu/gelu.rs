...
pub(super) fn inspect(operation:WorkspaceOperationView<'_>,mechanism:MlxCpuWorkspaceMechanisms)
    ...
    if input.shape()!=output.shape() {return Err(MlxWorkspaceFactError::descriptor("CPU GELU shape differs"));}
    let rank=input.shape().len();
    if rank>4||input.shape().iter().any(|&n|n<=0)||input.dtype()!=WorkspaceDtype::Float32||
        output.dtype()!=WorkspaceDtype::Float32||!input.representation().is_some_and(|r|
            r.row_contiguous()) {return Ok(None);}
    ...
    let source=(|| {
        let mut p=CpuPopulation::default();let mut scalar_births=0usize;
        // x / sqrt(2), 1 + erf(...), x * (...), (...) / 2. Preserve both
        // divisions and the actual I32 one / F32 sqrt(2) / F32 two sources.
        let steps=[(CpuBinaryOperation::Divide,false,true),(CpuBinaryOperation::Add,true,false),
            (CpuBinaryOperation::Multiply,false,false),(CpuBinaryOperation::Divide,false,true)];
        for (step,(kind,left_scalar,right_scalar)) in steps.into_iter().enumerate() {
            for (operand,scalar) in [left_scalar,right_scalar].into_iter().enumerate() {
                ...
                let from=if step==1&&scalar {Dtype::Int32}
                    else if operand==0&&(step==0||step==2) {native_input}
                    else {Dtype::Float32};
                let cast=OperationEvent::cpu_cast_layout(from,Dtype::Float32,r,n,false)?;
                if scalar {scalar_births=scalar_births.checked_add(cast.backing_births())?;}
                p.copy(cast,1)?;
            ...
            if step==0 {
                p.copy(OperationEvent::cpu_cast_layout(Dtype::Float32,Dtype::Float32,rank,count,false)?,1)?;
                p.unary(OperationEvent::cpu_unary_layout(CpuUnaryOperation::Erf,Dtype::Float32,rank,false)?)?;
            }
        }
    ...
    let full_births=population.births.checked_sub(scalar_births).and_then(|n|n.checked_sub(1))
        .ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?;
    let scratch_bytes=facts::add(facts::mul(full,u64::try_from(full_births)?)?,
        facts::mul(scalar,u64::try_from(scalar_births.checked_add(3).ok_or(MlxWorkspaceFactError::POPULATION_OVERFLOW)?)?)?)?;
    let frames=[size_of::<CpuPopulation>()*3,size_of::<Option<(CpuPopulation,usize)>>(),
        ...
        size_of::<(usize,(CpuBinaryOperation,bool,bool))>(),size_of::<[bool;2]>(),
        size_of::<std::array::IntoIter<bool,2>>(),size_of::<std::iter::Enumerate<std::array::IntoIter<bool,2>>>(),
        size_of::<(usize,bool)>(),size_of::<(usize,usize)>(),size_of::<Dtype>()*2,
        ...
        size_of::<usize>()*6,size_of::<u64>()*3,size_of::<std::slice::Iter<i32>>(),
        size_of::<(&safemlx::Array,&safemlx::Stream)>(),size_of::<safemlx::Array>()*3,
    ...
    Ok(Some(OperationPlan{dtype:WorkspaceFloatingType::Float32,population,output_bytes:full,scratch_bytes,
        rank,parameter_shells:0,alias_input:None,seeds:3,validations:0}))
}
