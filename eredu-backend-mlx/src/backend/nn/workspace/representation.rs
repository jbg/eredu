...
fn exact_f32_operands(operation: WorkspaceOperationView<'_>) -> Option<F> {
    ...
    for input in operation.inputs.iter() {
        if input.dtype() == WorkspaceDtype::Float32 {
            if input.representation()?.dtype() != F::Float32 {
                return None;
            }
            floating = true;
        }
...
pub(super) fn output(
    ...
    if matches!(operation.kind, K::Concatenate) && operation.inputs.len() == 1 {
        ...
        return (input.shape() == output.shape() && input.dtype() == output.dtype())
            .then(|| input.representation())
            .flatten();
    }
    let value = match operation.kind {
        K::ParameterDecode(_) => super::parameter_decode::output_dtype(operation)?,
        ...
        K::Sampling(
            ...
            && dtype(operation, 0) == Some(F::Float32) =>
        {
            F::Float32
        }
        ...
        K::Attention {
            ...
            || operation
                ...
                .shape()
                .get(2)
                .zip(operation.inputs.get(1)?.shape().get(2))
                .is_some_and(|(&q, &k)| {
                    i64::from(q) * i64::from(k)
        ...
        | K::StaticSliceUpdate { .. }
        | K::IndexedRowAdd
        | K::IndexedElementSelect
        | K::IndexedElementUpdate => dtype(operation, 0)?,
        ...
        K::GatedProduct(policy)
            if policy.activation() == GatedProductActivation::GeluApproximate =>
        {
            dtype(operation, 0)?;
            dtype(operation, 1)?;
            F::Float32
    ...
    let contiguous = matches!(
        operation.kind,
        K::HostTransferFloating(_) | K::HostLoadStoredFloating(..) | K::ParameterDecode(_)
        ...
        || super::zero_fill::dtype(operation).is_some();
