...
impl ScanSourcePair {
    /// Fills paid alias slots from an actual ordinary source loan. Requiring
    /// completed backing is appropriate for sealed canonical blocks; the live
    /// partial tail instead remains under the enclosing Work's completion.
    pub(crate) fn fill_for_inspection(
        &mut self,
        arrays: [&Array; 2],
        completed: bool,
        ...
        if completed {
            for array in arrays {
                if array.try_descriptor()?.facts().allocation().is_none() {
                    return Err(CacheSourceError::PendingStorage);
                }
            }
        }
        for (index, array) in arrays.into_iter().enumerate() {
            self.arrays[index] = Some(self.slots[index].fill_for_inspection(array)?);
        }
        Ok(())
    }
...
fn prepare_one(
    ...
    let discarded = |id: &CacheBlockId| {
        discard_window.is_some_and(|(visible, prefix)| {
            eredu_runtime::cache::CacheBlockSelection::outside_retained_window(
                id.start, id.end, visible, prefix,
            )
        })
    };
    let selection_frames = [
        std::mem::size_of_val(&ids),
        std::mem::size_of_val(&ids()),
        std::mem::size_of_val(&discarded),
        std::mem::size_of_val(&selection),
        std::mem::size_of_val(&discard_window),
    ...
    let discard_controls = crate::backend::runtime::cache::residency::CacheResidencyManager::prepared_discard_control_bytes::<super::OriginalPagedDiscard<'_, '_>>()
        ...
        .ok_or_else(|| fail(CacheSourceError::Overflow))?;
    context
        .charge_metadata(discard_controls)
        .map_err(|cause| CacheSourceFailure::metadata(cause.into(), context))?;
