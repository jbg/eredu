...
use super::*;
use super::facts::{add,mul,Emitter,FactResult,Output};
...
pub(super) fn embedding(operation:WorkspaceOperationView<'_>)->FactResult<Option<WorkspaceOperationView<'_>>> {
    let WorkspaceOperationKindView::VocabularyParallelLookup{format,range,policy}=operation.kind
        else{return Ok(None);};
    policy.validate_fixed()?;
    range.validate_fixed().map_err(|_|MlxWorkspaceFactError::descriptor("invalid vocabulary ownership"))?;
    if i32::try_from(range.global_vocabulary).is_err() || operation.inputs.len()<2 || operation.outputs.len()!=1
        || operation.inputs.get(1).and_then(|weight|weight.shape().first().copied())
            !=i32::try_from(range.local.len()).ok() {
        return Err(MlxWorkspaceFactError::descriptor("vocabulary lookup source rows differ"));
    }
    Ok(Some(WorkspaceOperationView{kind:WorkspaceOperationKindView::Embedding(format,eredu_nn::EmbeddingLookupPolicy::Strict),
...
pub(super) fn emit(operation:WorkspaceOperationView<'_>,allocation:NativeAllocationFacts,sink:&mut Emitter<'_>)
    ...
    if !matches!(format.encoding(),LinearFormat::Dense|LinearFormat::Affine(_)|LinearFormat::MxFp4) {return Ok(None);}
    let mut rows=Emitter::count();
    let result=if format.encoding()==LinearFormat::Dense {indexing::emit(child,allocation,&mut rows)?}
        else {packed::emit(child,allocation,&mut rows)?};
    let Some(result)=result else{return Ok(None);};
    let Some(WorkspaceOutputEffect::Allocate(output))=rows.first_output() else{return Ok(None);};
    if result.layout.outputs!=1 || result.layout.aliases!=0 {return Ok(None);}
    let count=operation.inputs.first().expect("checked lookup IDs").elements()?;
