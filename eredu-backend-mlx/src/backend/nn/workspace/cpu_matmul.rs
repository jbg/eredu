...
impl MlxCpuMatmulMechanism {
    ...
    pub fn select(implementation:CpuMatmulImplementation)->Option<Self> {
        let Some(native)=safemlx::CpuMatmulFacts::inspect() else {
            return (implementation==CpuMatmulImplementation::PlatformDefault)
                .then_some(Self{selected:SelectedCpuMatmul::platform_default(),native:None});
        };
        let facts=CpuMatmulFacts::new(u32::try_from(native.tile_edge()).ok()?,
            u32::try_from(native.reduction_lanes()).ok()?,u32::try_from(native.max_rank()).ok()?,
        ...
        Some(Self {selected,native:Some(native)})
    }
    /// The exact neutral implementation and its reduction/geometry declaration.
    pub const fn selected(self)->SelectedCpuMatmul {self.selected}
    ...
    pub fn stream_plan<T:Send+Sync+'static>(self,source:&Stream)->Result<StreamCopyPlan<T>,StreamCopyCause> {
        let kernel=match self.selected.implementation() {
            ...
            CpuMatmulImplementation::Float32AndFloat16Tiles=>CpuMatmulKernel::Float32AndFloat16Tiles,
        };
    ...
    pub fn eval_layout(self, geometry:eredu_nn::CpuMatmulGeometry, tracer:bool)->Option<safemlx::CpuCopyEvalLayout> {
        let [rank,m,n,k,batches]=geometry.dimensions();
        if self.selected.geometry(rank,m,n,k,batches).ok()!=Some(geometry) {return None;}
        safemlx::OperationEvent::cpu_tiled_matmul_layout(rank as usize,m as usize,n as usize,k as usize,batches as usize,tracer)
    }
    ...
    pub fn eval_layout_with_copies(self,geometry:eredu_nn::CpuMatmulGeometry,copies:usize,
        tracer:bool)->Option<safemlx::CpuCopyEvalLayout> {
        let [rank,m,n,k,batches]=geometry.dimensions();
        if self.selected.geometry(rank,m,n,k,batches).ok()!=Some(geometry) {return None;}
        safemlx::OperationEvent::cpu_tiled_matmul_copy_layout(rank as usize,m as usize,n as usize,
            k as usize,batches as usize,copies,tracer)
    }
    /// Existing F16 SIMD worker under the exact retained choice/default fact.
    pub fn float16_eval_layout(self,geometry:eredu_nn::CpuMatmulGeometry,copies:usize,
        tracer:bool)->Option<safemlx::CpuCopyEvalLayout> {
        let [rank,m,n,k,batches]=geometry.dimensions();
        if self.selected.float16_geometry(rank,m,n,k,batches).ok()!=Some(geometry) {return None;}
        safemlx::OperationEvent::cpu_f16_matmul_copy_layout(rank as usize,m as usize,n as usize,
            k as usize,batches as usize,copies,tracer)
    }
    ...
    pub fn control_bytes<T:Send+Sync+'static>(self)->Option<usize> {
        let parts=[size_of::<Self>(),size_of::<Option<Self>>(),size_of::<SelectedCpuMatmul>(),
            size_of::<Option<CpuMatmulFacts>>(),size_of::<CpuMatmulImplementation>(),size_of::<CpuMatmulKernel>(),
            size_of::<StreamCopyPlan<T>>(),size_of::<Result<StreamCopyPlan<T>,StreamCopyCause>>(),
            size_of::<&Stream>(),size_of::<Result<u32,std::num::TryFromIntError>>()*4,
            size_of::<eredu_nn::CpuMatmulGeometry>(),size_of::<[u32;5]>(),
            size_of::<Result<eredu_nn::CpuMatmulGeometry,eredu_nn::CpuMatmulError>>(),
            size_of::<Option<safemlx::CpuCopyEvalLayout>>(),size_of::<bool>()*3,
            size_of::<Option<CpuMatmulFacts>>(),size_of::<Option<SelectedCpuMatmul>>()];
        let native=match self.native {Some(f)=>f.control_bytes()?,None=>0};
