//! One request's exact source program: target parameters, paged state, constructor chunks
//! and separately retained reader peaks share no mutable allowance counters.
use crate::backend::{error::Error,nn::workspace::{AddressableInvocation,AddressableQuoteRef},
...
struct Reader {series:Option<ForegroundDiskSourceSeries>}
...
impl AddressableRequestSourcePlan {
        ...
        for (row,(invocation,repeats)) in rows.iter().enumerate() {
            for repetition in 0..*repeats {
                for (operation,quote) in invocation.occurrences() {
                    ...
                    if actual!=quote.constructor_facts||quote.residency.read_facts()!=quote.read_facts{return Err(identity());}
                    bytes=bytes.checked_add(actual.capacity_bytes()).ok_or_else(overflow)?;
                    attempts=attempts.checked_add(actual.maximum_attempts()).ok_or_else(overflow)?;
                    partitions=partitions.checked_add(actual.maximum_partitions()).ok_or_else(overflow)?;
                    let reader=if quote.residency.requires_reads() {
                        let selected=readers.iter().position(|reader|reader.series.as_ref().is_some_and(|series|quote.residency.read_source_matches(series)));
                        let index=match selected {
                            Some(index)=>index,
                            None=>{let index=readers.len();readers.push(Reader{series:None});index},
                        };
                        quote.residency.include_read_source(&mut readers[index].series)?;
                        if readers[index].series.is_none(){return Err(identity());}
                        Some(index)
                    }else{None};
                    occurrences.push(AddressableSourceOccurrence{row,repetition,operation:*operation,quote:quote.clone(),reader});
                }
            }
        }
        let constructors=if count==0 {None} else {Some(HostSourceConstructionFacts::new(bytes,attempts,partitions).map_err(memory)?)};
        let components=readers.len().checked_add(usize::from(constructors.is_some())).and_then(|n|n.checked_add(usize::from(target.is_some()))).and_then(|n|n.checked_add(usize::from(paged.is_some()))).ok_or_else(overflow)?;
        let mut facts=funding.metadata_vec(components).map_err(Error::Neural)?;
        if let Some(constructors)=constructors {facts.push(constructors);}
        for reader in &readers {facts.push(reader.series.as_ref().ok_or_else(identity)?.facts().ok_or_else(overflow)?);}
        let target=target.map(|facts_row|{let index=facts.len();facts.push(facts_row);index});
        let paged=paged.map(|facts_row|{let index=facts.len();facts.push(facts_row);index});
        let program=HostSourceConstructionProgram::from_components(facts).map_err(memory)?;
        Ok(Self{occurrences,readers,constructors,program,target,paged,funding:funding.clone()})
    }
    pub(crate) fn facts(&self)->HostSourceConstructionFacts {self.program.facts()}
    pub(crate) fn target_facts(&self)->Option<HostSourceConstructionFacts> {self.target.map(|index|self.program.components()[index])}
    pub(crate) fn occurrences(&self)->&[AddressableSourceOccurrence] {&self.occurrences}
    ...
    pub(crate) fn runtime_control_bytes(&self)->Result<usize,Error> {
        let mut bytes=Self::accept_control_bytes()
            .and_then(|n|n.checked_add(WorkspaceContext::metadata_vec_bytes::<ReaderState>(self.readers.len())?))
            .and_then(|n|n.checked_add(WorkspaceContext::metadata_vec_bytes::<OccurrenceState>(self.occurrences.len())?))
            .ok_or_else(overflow)?;
        bytes=bytes.checked_add(Self::take_control_bytes().and_then(|n|n.checked_mul(self.occurrences.len())).ok_or_else(overflow)?)
            .ok_or_else(overflow)?;
        for occurrence in &self.occurrences {
            bytes=bytes.checked_add(occurrence.quote.residency.constructor_partition_control_bytes().ok_or_else(overflow)?)
                .ok_or_else(overflow)?;
        }
        for present in [self.target.is_some(),self.paged.is_some()] {
            if present {bytes=bytes.checked_add(Self::component_control_bytes().ok_or_else(overflow)?).ok_or_else(overflow)?;}
        }
        for _ in &self.readers {
            bytes=bytes.checked_add(ForegroundDiskSourceSeries::control_bytes().ok_or_else(overflow)?)
                .ok_or_else(overflow)?;
        }
        Ok(bytes)
    }
    ...
    pub(crate) fn accept(self,root:OriginalHostSourceBank)->Result<AddressableRequestSources,Error> {
        reserve(&self.funding,Self::accept_control_bytes())?;
        let mut program=root.partition_program(&self.program).map_err(|cause|Error::Other(Box::new(cause)))?;
        let constructors=match self.constructors {
            Some(facts)=>{let root=program.take(0).map_err(memory)?;
                if !root.matches_facts(facts){return Err(identity());} Some(root)},
            None=>None,
        };
        let mut readers=self.funding.metadata_vec(self.readers.len()).map_err(Error::Neural)?;
        for index in 0..self.readers.len() {
            readers.push(ReaderState{root:Some(program.take(index+usize::from(self.constructors.is_some())).map_err(memory)?),capacity:None});
        }
        let target=match self.target {Some(index)=>Some(program.take(index).map_err(memory)?),None=>None};
        let paged=match self.paged {Some(index)=>Some(program.take(index).map_err(memory)?),None=>None};
        let mut spent=self.funding.metadata_vec(self.occurrences.len()).map_err(Error::Neural)?;
        spent.resize(self.occurrences.len(),OccurrenceState::Fresh);
        Ok(AddressableRequestSources{plan:self,state:RefCell::new(State{constructors,readers,spent,target,paged,_program:program})})
    }
    /// The report/repetition table is in canonical order; this borrowed lookup
    /// creates no occurrence or source authority.
    pub(crate) fn occurrence_range(&self,row:usize,repetition:usize)->Option<std::ops::Range<usize>> {
        let start=self.occurrences.iter().position(|o|o.row==row&&o.repetition==repetition)?;
        let count=self.occurrences[start..].iter().take_while(|o|o.row==row&&o.repetition==repetition).count();
        Some(start..start.checked_add(count)?)
    }
    fn component_control_bytes()->Option<usize> {
        Some(size_of::<(&AddressableRequestSources,Option<OriginalHostSourceBank>,Result<Option<OriginalHostSourceBank>,Error>)>())
    }
    ...
    fn take_control_bytes()->Option<usize> {
        sum(&[size_of::<(&AddressableRequestSources,usize,&OriginalSelectedResidencyAccess)>(),
            size_of::<std::cell::RefMut<'_,State>>(),size_of::<IndexedConstructorPartitions>(),
            size_of::<Option<ForegroundDiskSourceCapacity>>(),size_of::<ForegroundDiskSourceCapacity>(),
            size_of::<Result<(IndexedConstructorPartitions,Option<ForegroundDiskSourceCapacity>),Error>>(),
            size_of::<Option<usize>>(),size_of::<usize>(),size_of::<bool>()])
    }
}
struct ReaderState {root:Option<OriginalHostSourceBank>,capacity:Option<ForegroundDiskSourceCapacity>}
#[derive(Clone,Copy,PartialEq,Eq)]
enum OccurrenceState {Fresh,Attempted,Issued}
struct State {
...
pub(crate) struct AddressableRequestSources {plan:AddressableRequestSourcePlan,state:RefCell<State>}
impl AddressableRequestSources {
    pub(crate) fn occurrences(&self)->&[AddressableSourceOccurrence] {self.plan.occurrences()}
    pub(crate) fn occurrence_range(&self,row:usize,repetition:usize)->Option<std::ops::Range<usize>> {
        self.plan.occurrence_range(row,repetition)
    }
    /// Exact retained target source description, separate from paged state.
    pub(crate) fn target_facts(&self)->Option<HostSourceConstructionFacts> {self.plan.target_facts()}
    ...
    pub(crate) fn take_target(&self)->Result<Option<OriginalHostSourceBank>,Error> {
        if self.plan.target.is_none() {return Ok(None);}
        reserve(&self.plan.funding,AddressableRequestSourcePlan::component_control_bytes())?;
        let mut state=self.state.try_borrow_mut().map_err(|_|identity())?;
        if self.plan.target.is_some()&&state.target.is_none(){return Err(memory(WorkingMemoryError::AlreadyStarted));}
    ...
    pub(crate) fn take_paged(&self)->Result<Option<OriginalHostSourceBank>,Error> {
        if self.plan.paged.is_none() {return Ok(None);}
        reserve(&self.plan.funding,AddressableRequestSourcePlan::component_control_bytes())?;
        let mut state=self.state.try_borrow_mut().map_err(|_|identity())?;
        if self.plan.paged.is_some()&&state.paged.is_none(){return Err(memory(WorkingMemoryError::AlreadyStarted));}
        Ok(state.paged.take())
    }
    ...
    pub(crate) fn take(&self,index:usize,access:&OriginalSelectedResidencyAccess)
        ->Result<(IndexedConstructorPartitions,Option<ForegroundDiskSourceCapacity>),Error> {
        reserve(&self.plan.funding,AddressableRequestSourcePlan::take_control_bytes())?;
        ...
        if *spent!=OccurrenceState::Fresh{return Err(memory(WorkingMemoryError::AlreadyStarted));}
        *spent=OccurrenceState::Attempted;
        let constructors=occurrence.quote.residency.partition_constructors(access,state.constructors.as_mut().ok_or_else(identity)?)?;
        let reads=if let Some(reader)=occurrence.reader {
            let series=self.plan.readers.get(reader).and_then(|r|r.series.as_ref()).ok_or_else(identity)?;
            let slot=state.readers.get_mut(reader).ok_or_else(identity)?;
            if slot.capacity.is_none() {
                reserve(&self.plan.funding,ForegroundDiskSourceSeries::control_bytes())?;
    ...
    pub(crate) fn complete(&self)->Result<bool,Error> {
        ...
        Ok(state.spent.iter().all(|spent|*spent==OccurrenceState::Issued)
            &&state.constructors.as_ref().is_none_or(|root|root.remaining_bytes()==0
