...
pub(crate) fn complete(observer: &OriginalScopeObserver) -> Result<(), RegisteredScopeRetirementCause> {
    match observer.retire_completed_records()? {
        ...
        outcome => Err(RegisteredScopeRetirementCause::Incomplete(outcome)),
    }
}
