...
use super::*;
use eredu_runtime::working_memory::{OriginalRealtimeNative,OriginalRealtimeBudgetCustody,
...
impl RealtimeRoleContext<'_> {
    ...
    pub(crate) fn claim_operations(&self)->Result<RealtimeOperationClaim<'_>,Error> {
        if self.operations_issued.replace(true) {
            return Err(Error::PrefillControl(eredu_runtime::working_memory::WorkingMemoryError::AlreadyStarted));
        }
        Ok(RealtimeOperationClaim{source:self})
    }
}
impl<'a> RealtimeOperationClaim<'a> {
    pub(crate) fn prepare(self,controls:usize)->Result<(OriginalScopeObserver,Custody),Error> {
        self.source.funding.reserve_metadata(controls).map_err(Error::WorkspacePlanning)?;
        ...
        account.validate_account(None).map_err(Error::PrefillControl)?;
        Ok((current,custody))
    }
}
type FrameCallback<'a,I,T,E>=dyn FnMut(&I,&RealtimeRoleContext<'_>)->Result<Result<T,E>,Error>+'a;
...
fn operation_callback<'a,I,T,E>(mut run:Option<&'a mut FrameCallback<'a,I,T,E>>,
    ...
    move |invocation,native| {
        let context=RealtimeRoleContext{native,custody:custody.expect("active realtime custody"),
            funding:funding.expect("active realtime funding"),operations_issued:std::cell::Cell::new(false)};
...
fn operation_controls<I,T,E>()->Option<usize> {
    ...
    let parts=[size_of_val(&callback),size_of::<RealtimeRoleContext<'_>>(),
        size_of::<RealtimeOperationClaim<'_>>(),size_of::<Result<RealtimeOperationClaim<'_>,Error>>(),
        size_of::<(&I,&RealtimeRoleContext<'_>)>(),size_of::<(&I,&NativeRoleContext<'_>)>(),
        size_of::<&mut FrameCallback<'_,I,T,E>>(),
        size_of::<(OriginalScopeObserver,Custody)>(),size_of::<OriginalHostSourceCustody>(),
...
fn capacity(claim:&OriginalRealtimeNative)->Result<NativeRoleCapacity,NativeRoleControlError> {
    Ok(NativeRoleCapacity{
        graph:usize::try_from(claim.graph_bytes()).map_err(|_|NativeRoleControlError::Overflow)?,
        records:usize::try_from(claim.record_bytes()).map_err(|_|NativeRoleControlError::Overflow)?,
        backing:usize::try_from(claim.physical_bytes()).map_err(|_|NativeRoleControlError::Overflow)?,
    })
}
fn fixed_preparation_control_bytes<I,T,E>(callback_bytes:usize)->Result<usize,NativeRoleControlError> {
    sum_controls(&[
        error_bytes::<Custody>().ok_or(NativeRoleControlError::Overflow)?,
        size_of::<I>(),callback_bytes,size_of::<T>(),size_of::<E>(),
        size_of::<OriginalRealtimeNative>(),size_of::<Custody>(),size_of::<OriginalHostSourceCustody>(),
        size_of::<NativeRoleCapacity>(),size_of::<Result<NativeRoleCapacity,NativeRoleControlError>>(),
        ...
        size_of::<NativeRoleControlError>(),size_of::<OriginalBufferBudget>(),
        size_of::<Result<usize,NativeRoleControlError>>(),
    ]).ok_or(NativeRoleControlError::Overflow)
}
fn buffer_control_bytes(runtime:&PreparedInputRuntime,capacity:NativeRoleCapacity)
    ->Result<usize,NativeRoleControlError> {
    PreparedOriginalBufferBudget::<Custody>::layout(runtime,capacity.backing)
        .map_err(NativeRoleControlError::Buffer)?.total_owner_bytes()
        .ok_or(NativeRoleControlError::Overflow)
}
...
pub(crate) fn control_bytes<I:'static,T,E>(runtime:&PreparedInputRuntime,
    capacity:NativeRoleCapacity,pipeline:Option<safemlx::PreparedPipelineCachePlan>,
    callback_bytes:usize)->Result<usize,NativeRoleControlError> {
    ...
    let callback=callback_control_bytes::<T,E>(size_of_val(&operation_callback::<I,T,E>(None,None,None)))
        .and_then(|n|n.checked_add(operation_controls::<I,T,E>()?))
        .ok_or(NativeRoleControlError::Overflow)?;
    prepare.checked_add(role).and_then(|n|n.checked_add(callback))
        .ok_or(NativeRoleControlError::Overflow)
}
...
where I:'static,F:FnMut(&I,&RealtimeRoleContext<'_>)->Result<Result<T,E>,Error> {
    ...
    let buffer_controls=buffer_control_bytes(runtime,capacity)
        .map_err(|cause|fail(cause.into()))?;
    funding.reserve_metadata(buffer_controls)
        .map_err(|cause|Error::WorkspacePlanning(cause).into_backend_failure())?;
    let account:OriginalHostSourceCustody=custody.clone().into();
    account.validate_account(None).map_err(|cause|fail(RoleCause::Backend(Error::PrefillControl(cause))))?;
    ...
    if let Some(parent)=OriginalScopeObserver::try_current().map_err(|cause|fail(RoleCause::Native(cause)))? {
        return Err(fail(RoleCause::Native(parent.domain_error())));
    }
    let prepared=PreparedOriginalBufferBudget::try_new(runtime,capacity.backing,custody.clone())
        .map_err(|error|{let(cause,owner)=error.into_parts();let failure=fail(RoleCause::Buffer(cause));
            drop(owner);failure})?;
    let budget=prepared.try_allocate().map_err(|error|{
        let(cause,prepared)=error.into_parts();let failure=fail(RoleCause::Buffer(cause));
        // Refused construction has no native prefix; retire its unattached
        // node before dropping its original account-only custody.
        drop(prepared.into_owner());failure
    })?;
    funding.reserve_metadata(operation_controls::<I,T,E>().ok_or_else(||fail(RoleCause::Backend(overflow())))?)
        .map_err(|cause|Error::WorkspacePlanning(cause).into_backend_failure())?;
    let callback=operation_callback(Some(&mut run),Some(&custody),Some(funding));
    super::run_with_budget_source(invocation,capacity,pipeline,RoleBudget::Realtime(budget),
        &custody,funding,timeout,callback)
}
