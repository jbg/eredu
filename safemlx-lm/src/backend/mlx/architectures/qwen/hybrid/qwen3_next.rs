...
mod tests {
    ...
    fn splits_mixed_affine_fused_projection_configs() {
        let q3 =
            crate::backend::mlx::runtime::checkpoint::quantization::AffineQuantization::new(16, 3)
                .unwrap();
        let q5 =
            crate::backend::mlx::runtime::checkpoint::quantization::AffineQuantization::new(32, 5)
                .unwrap();
    ...
    fn packs_fp8_experts_split_across_shards() {
        ...
        let store = Arc::new(
            crate::backend::mlx::runtime::checkpoint::store::SafetensorsWeightStore::open_with_max_mapped_shards(
                dir.path(),
                1,
            )
            .unwrap(),
        );
        let entries = crate::backend::mlx::architectures::qwen::hybrid::layerwise::qwen_hybrid_expert_catalog(
            &fp8_args(),
            store.as_ref(),
        )
        .unwrap();
        let options =
            crate::backend::mlx::runtime::residency::expert_cache::ExpertCacheLoadOptions::new(
                crate::core::residency::OffloadConfig::new(None, None, 1).unwrap(),
                u64::MAX,
                1 << 30,
            )
            .unwrap();
        let cache = crate::backend::mlx::runtime::residency::expert_cache::ExpertCache::new(
            Arc::clone(&store),
            entries,
            options,
            cpu.stream().clone(),
        ...
        drop(
            cache
                .acquire_route_slice(
                    ...
                    &[1],
                    &[1],
                    crate::backend::mlx::runtime::residency::expert_cache::ExpertPass::Decode,
                    gpu.stream(),
                )
                .unwrap(),
        );
        let diagnostics =
            crate::backend::mlx::runtime::checkpoint::store::WeightStore::diagnostics(
                store.as_ref(),
            )
            .unwrap();
        ...
        let error = super::super::qwen3_5::load_qwen_fp8_safetensors_dir_strict_with_transform(
            ...
            |key, value| {
                if key == "zz_stop_after_complete_layer" {
                    Err(crate::backend::mlx::error::Error::UnsupportedArchitecture(
