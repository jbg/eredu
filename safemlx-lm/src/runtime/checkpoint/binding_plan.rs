...
use crate::runtime::residency::manager::{ResidencyError, WeightBinding};

/// One runtime target and the physical recipe that produces it.
#[derive(Debug, Clone, Eq, PartialEq)]
pub(crate) struct PlannedBinding {
    pub(crate) target_name: String,
    pub(crate) expected_shape: Vec<usize>,
    pub(crate) expected_dtype: RecipeDtype,
    pub(crate) recipe: DerivedWeightRecipe,
}
...
impl BindingPlan {
    ...
    pub(crate) fn new(bindings: Vec<PlannedBinding>) -> Result<Self, BindingPlanError> {
    ...
    pub(crate) fn allowing_shared_sources(
