...
pub(crate) struct ContractWeightStore {
    ...
    contract: crate::runtime::checkpoint::validation::ResolvedCheckpointPlan,
}
...
impl GgufWeightStoreBuilder {
    ...
    pub(crate) fn add_checkpoint<F>(
        ...
        plan: &crate::runtime::checkpoint::schema::GgufCheckpointPlan,
        translate: F,
    ) -> Result<Self, WeightStoreError>
    where
        F: FnMut(&str) -> String,
    {
        let resolved = crate::runtime::checkpoint::validation::resolve_gguf_plan(&checkpoint, plan)
    ...
    fn add_resolved_checkpoint<F>(
        mut self,
        checkpoint: GgufCheckpoint,
        resolved: &crate::runtime::checkpoint::validation::ResolvedCheckpointPlan,
        mut translate: F,
    ) -> Result<Self, WeightStoreError>
    where
        F: FnMut(&str) -> String,
    ...
    where
        F: FnMut(&str) -> String,
    {
        let resolved = crate::runtime::checkpoint::validation::ResolvedCheckpointPlan::for_test(
            "test GGUF catalog",
            checkpoint
                .catalog()
                .tensors()
...
mod tests {
    ...
    fn resolved_contract_store_rejects_unselected_physical_sources() {
        ...
        let contract = crate::runtime::checkpoint::validation::ResolvedCheckpointPlan::for_test(
            "test architecture",
            ["packed"],
        );
    ...
    fn gguf_store_catalog_contains_only_contract_selected_sources() {
        use crate::runtime::checkpoint::schema::{
