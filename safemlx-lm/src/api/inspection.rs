...
pub fn inspect_text_model(
    ...
    match report.artifact_format {
        ...
        ArtifactFormat::Gguf => match safemlx_gguf::Reader::open(&path) {
            Ok(reader) => {
                let metadata = reader
                    .metadata()
                    .iter()
...
mod tests {
    ...
    fn loadable_report(path: &Path, format: ArtifactFormat) -> ModelInspectionReport {
    ...
    fn gguf_tokenizer_sidecar_is_an_explicit_fallback() {
        ...
        let mut missing = loadable_report(&path, ArtifactFormat::Gguf);
        inspect_gguf_sidecars(&mut missing, &path, &metadata, None);
        finalize_text_readiness(&mut missing);
        assert_eq!(missing.tokenizer, InspectionReadiness::Missing);

        save_wordlevel_tokenizer(&directory.path().join("tokenizer.json"));
        let mut available = loadable_report(&path, ArtifactFormat::Gguf);
        inspect_gguf_sidecars(&mut available, &path, &metadata, None);
        finalize_text_readiness(&mut available);
    ...
    fn template_presence_does_not_imply_semantic_or_tool_support() {
        ...
        .unwrap();
        let mut report = loadable_report(directory.path(), ArtifactFormat::SafeTensors);
        inspect_safetensors_sidecars(&mut report, directory.path(), None);
        finalize_text_readiness(&mut report);
    ...
    #[test]
    fn chat_protocol_is_recognized_from_rendered_behavior() {
        const TEMPLATE: &str =
        ...
        assert_eq!(report.semantic_streaming, InspectionReadiness::Ready);
        assert_eq!(report.native_tools, InspectionReadiness::Ready);
    }
}
