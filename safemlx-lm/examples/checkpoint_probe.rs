...
#[derive(Debug, Parser)]
#[command(
    about = "Probe a local SafeMLX checkpoint and emit JSON plus SafeTensors evidence",
    after_help = "Example:\n  cargo run -p safemlx-lm --features cuda --example checkpoint_probe -- --model snapshots/model --device gpu --output validation/results/model"
)]
struct Args {
