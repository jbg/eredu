...
type Unit<C,P> = crate::decoder::TransformerBlock<WorkspaceBackend,
...
trait DirectPartitionSource {
    ...
    fn routed_resident_source(&self, _selected: &crate::SelectedPreparation,
        _rank: eredu_core::ParallelRankTopology)
        -> Result<Option<crate::routed_text::RetainedPartitionResidentSource>, String> { Ok(None) }
    ...
    fn quote(&self, source: &PreparedModelSources, communication: Option<&eredu_runtime::RetainedCommunicationSource>, visitor: EquationVisitor<'_, '_, '_>) -> Result<EquationQuote, Error>;
}
struct Dense<C, P> {
...
impl PreparedDirectPartitionSource {
    ...
    pub(crate) fn routed_resident_source(&self, selected: &crate::SelectedPreparation,
        rank: eredu_core::ParallelRankTopology)
        -> Result<crate::routed_text::RetainedPartitionResidentSource, String> {
        self.0.routed_resident_source(selected, rank)?.ok_or_else(||
            "retained partition source is not the selected resident routed constructor".into())
    }
    pub(crate) fn tensor_waves(&self, selected: &crate::SelectedPreparation,
        rank: eredu_core::ParallelRankTopology)
        -> Result<Option<Arc<crate::partitioned_execution::TensorPipelineCollectiveWaves>>, String> {
    ...
    pub(crate) fn dense<C, P>(
        ...
        rank: eredu_core::ParallelRankTopology,
        local_state: eredu_runtime::SelectedStateRealization,
        pipeline: Option<(Arc<eredu_runtime::PartitionedExecutionPlan>,
            Vec<eredu_runtime::ExecutionUnitAddress>, eredu_runtime::PipelineActivationDtype,
            Option<Arc<crate::partitioned_execution::TensorPipelineCollectiveWaves>>)>,
    ) -> Self
    where C: PartitionedConfig + Send + Sync,
        P: BlockFactory<WorkspaceBackend, C> + 'static,
        P::FeedForward: TensorParallelProjectionOperator<WorkspaceBackend>,
    {
        Self(Arc::new(Dense::<C,P> {target,transform,selected,rank,local_state,
            pipeline: pipeline.map(|(plan,addresses,dtype,tensor_waves)|PipelineSource{plan,addresses,dtype,tensor_waves}),
    ...
    P::FeedForward: TensorParallelProjectionOperator<WorkspaceBackend>,
{
    fn tensor_waves(&self, selected: &crate::SelectedPreparation,
        ...
        if !selected.same_complete_selection(&self.selected) || rank != self.rank {
            return Err("retained tensor pipeline source differs from the selected constructor".into());
        }
        Ok(self.pipeline.as_ref().and_then(|pipeline| pipeline.tensor_waves.clone()))
    }
    fn quote(&self, source:&PreparedModelSources, communication:Option<&eredu_runtime::RetainedCommunicationSource>, visitor:EquationVisitor<'_, '_, '_>) -> Result<EquationQuote,Error> {
        ...
        context.charge_metadata(std::mem::size_of::<(
            &Self, &PreparedModelSources, EquationVisitor<'_, '_, '_>,
            PartitionedLayeredModel<Backend,C,P>, Option<PartitionedLayeredModel<Backend,C,P>>,
            eredu_nn::workspace::WorkspaceParallelContext, Result<EquationQuote,Error>,
            &eredu_runtime::SelectedStateRealization, Result<(),Error>,
        )>())?;
        ...
        {
            return Err(context.metadata_error(format_args!("parallel quote differs from its completed direct partition source")));
        }
        eredu_runtime::working_memory::validate_workspace_state_realization(
            visitor.state, &self.local_state, context,
        )?;
        let parallel=eredu_nn::workspace::WorkspaceParallelContext::new(
            self.rank.tensor_parallel_rank(),self.rank.tensor_parallel_size(),
        )?;
        ...
        let _transform=self.transform.as_ref().map(|source|
        ...
        match visitor.parameters {
            ...
            None => {
                let runtime=ResidentRuntime::<_,Backend,ResidentState>::new_workspace(architecture,context)?
...
fn direct_spans<C,P,Q>(source:&Dense<C,P>,
    mut runtime:LayerwiseRuntime<PartitionedLayeredModel<WorkspaceBackend,C,P>,WorkspaceBackend,ResidentState,Q>,
    parallel:&eredu_nn::workspace::WorkspaceParallelContext, visitor:EquationVisitor<'_, '_, '_>) -> Result<EquationQuote,Error>
where C:PartitionedConfig+Send+Sync,
    P:BlockFactory<WorkspaceBackend,C>+'static,
    P::FeedForward:TensorParallelProjectionOperator<WorkspaceBackend>,
        ...
        std::error::Error+Send+Sync+'static,
{
    let context=visitor.context;
    context.charge_metadata(std::mem::size_of::<(
        LayerwiseRuntime<PartitionedLayeredModel<WorkspaceBackend,C,P>,WorkspaceBackend,ResidentState,Q>,
        <PartitionedLayeredModel<WorkspaceBackend,C,P> as LayeredArchitecture<WorkspaceBackend,ResidentState>>::ForwardContext,
        &Dense<C,P>,&eredu_nn::workspace::WorkspaceParallelContext,EquationVisitor<'_, '_, '_>,Result<EquationQuote,Error>,
    )>())?;
        context.charge_metadata(std::mem::size_of::<(
        ...
        direct_publication_spans(&source.selected, visitor, paths,
            ...
            let (scores,_forward)=match (observer, paths) {
                (observer, Some(paths)) => runtime.forward_parallel_with_prepared_observer_and_context_with_readout(
                    crate::decoder::LayeredInput {tokens, mask:None}, state, parallel,
                    context, observer, paths, demand,
                ).map_err(|cause| context.metadata_source(cause))?,
                (None, None) => runtime.forward_parallel_fixed_with_readout(
                    crate::decoder::LayeredInput {tokens, mask:None}, state,
                    parallel,context,demand,
                ).map_err(|cause|context.metadata_source(cause))?,
                _ => return Err(context.metadata_source(eredu_runtime::working_memory::InferenceObservationError::RemoteOutput)),
            };
            Ok(scores)
        })
}
...
where F: FnMut(
    ...
    Option<&eredu_runtime::PreparedLayeredObservationPaths>, &InferenceWorkspaceSpan,
) -> Result<Option<WorkspaceTensor>, Error>,
{
    let context = visitor.context;
    context.charge_metadata(std::mem::size_of::<(
        &crate::SelectedPreparation, EquationVisitor<'_, '_, '_>,
        Option<eredu_runtime::PreparedLayeredObservationPaths>, F,
        Result<EquationQuote, Error>,
    )>())?;
        context.charge_metadata(std::mem::size_of::<(eredu_runtime::PartitionOutputPublication,
            eredu_runtime::CommunicationGroupOperation<'_>,usize,usize,&[usize])>()
            .checked_add(eredu_runtime::CommunicationManifest::group_operation_control_bytes()
                .ok_or_else(||context.metadata_error(format_args!("publication control size overflow")))?)
            .ok_or_else(||context.metadata_error(format_args!("publication control size overflow")))?)?;
        let publication=selected_source.execution().partitioned_output_publication()
            .ok_or_else(||context.metadata_error(format_args!("partition has no selected publication")))?;
        let selected=selected_source.communication_manifest().ok_or_else(||context.metadata_error(format_args!("missing publication manifest")))?
            .select_group_operation(publication.group,eredu_runtime::CommunicationOperation::Broadcast)
            .map_err(|cause|context.metadata_source(cause))?;
        let ranks=selected.descriptor().members();
        let root=ranks.iter().position(|&rank|rank==publication.owner_rank)
            .ok_or_else(||context.metadata_error(format_args!("publication owner is not a selected member")))?;
        let rank=selected.descriptor().local_index().ok_or_else(||context.metadata_error(format_args!("publication rank is not a selected member")))?;
    ...
    visitor.quote_spans_with_prepublication_observation(hook_bytes, |tokens,state,demand,mut observer,span| {
        let scores = match observer.as_deref_mut() {
            Some(observer) => forward(tokens,state,demand,Some(observer),paths.as_ref(),span)?,
            None => forward(tokens,state,demand,None,paths.as_ref(),span)?,
        };
        let scores=match (scores,observer.as_deref_mut()) {
            (Some(scores),Some(observer)) if rank==root => Some(eredu_runtime::observe_model_logits(observer,&scores)?),
        ...
        let scores=scores.map(|scores|scores.broadcast_publication(publication.group,root,rank,ranks.len(),context)).transpose()?;
        if rank!=root { if let (Some(scores),Some(observer))=(&scores,observer) {
...
impl PreparedInferenceBlueprint {
    ...
    pub fn quote_direct_parallel_text_with_sampling_and_trace<'a>(
        &self, geometry:InferenceGeometry, state:&ResidentState, context:&WorkspaceContext,
        config:TextGenerationConfig, filter:impl Into<TextFilterWorkspace<'a>>,
        observer:&mut dyn InferenceEquationTraceObserver,
    )->Result<PreparedTextGenerationWorkspace,PreparedExecutionError<Error>> {
        self.quote_partitioned_text_with_sampling_and_trace(geometry,state,context,config,filter,observer,None)
    }
    ...
    pub fn quote_partitioned_text_with_sampling_and_trace<'a>(
        ...
        self.quote_partitioned_sampling_input(geometry, state, context,
            TextSamplingInput::Configured(config, filter.into()), observer, communication, None, None)
    }
    ...
    pub fn quote_partitioned_text_with_existing_sampling_and_trace(
        ...
        self.quote_partitioned_sampling_input(geometry, state, context,
            TextSamplingInput::Borrowed(sampling), observer, communication, None, None)
    }
    ...
    pub fn quote_partitioned_layerwise_text_with_sampling_and_trace<'a>(
        ...
        self.quote_partitioned_sampling_input(geometry,state,context,
            TextSamplingInput::Configured(config,filter.into()),observer,communication,Some(parameters),None)
    }
    ...
    pub fn quote_partitioned_layerwise_text_with_existing_sampling_and_trace(
        &self,geometry:InferenceGeometry,state:&ResidentState,context:&WorkspaceContext,
        sampling:BorrowedTextSamplingWorkspace<'_>,observer:&mut dyn InferenceEquationTraceObserver,
        communication:Option<&eredu_runtime::RetainedCommunicationSource>,
        parameters:&dyn WorkspaceLayerwiseParameters,
    )->Result<PreparedTextGenerationWorkspace,PreparedExecutionError<Error>> {
        self.quote_partitioned_sampling_input(geometry,state,context,
            TextSamplingInput::Borrowed(sampling),observer,communication,Some(parameters),None)
    }
    ...
    pub fn quote_partitioned_text_with_sampling_observed_and_trace<'a>(
        ...
        self.quote_partitioned_sampling_observed(geometry,state,context,
            TextSamplingInput::Configured(config,filter.into()),paths,capture,trace,communication,parameters)
    }
    ...
    pub fn quote_partitioned_text_with_existing_sampling_observed_and_trace(
        ...
        self.quote_partitioned_sampling_observed(geometry,state,context,
            TextSamplingInput::Borrowed(sampling),paths,capture,trace,communication,parameters)
    }
    ...
    fn quote_partitioned_sampling_observed(
        ...
        context.charge_metadata(std::mem::size_of::<(
            ObservationRef<'_, '_>,std::cell::RefCell<&mut dyn eredu_runtime::working_memory::InferenceWorkspaceObserver>,
            [TextSamplingInput<'_>; 2],
            [Result<PreparedTextGenerationWorkspace,PreparedExecutionError<Error>>; 2],
            [(&Self, InferenceGeometry, &ResidentState, &WorkspaceContext,
                ...
                Option<&dyn WorkspaceLayerwiseParameters>); 2],
        )>()).map_err(|cause|PreparedExecutionError::Metadata(cause.into()))?;
        let capture=std::cell::RefCell::new(capture);
        let observation=ObservationRef {paths,observer:&capture,invocation_prediction:None};
        self.quote_partitioned_sampling_input(geometry,state,context,
            sampling,trace,communication,parameters,Some(observation))
    }

    fn quote_partitioned_sampling_input(
        &self, geometry:InferenceGeometry, state:&ResidentState, context:&WorkspaceContext,
        sampling:TextSamplingInput<'_>, observer:&mut dyn InferenceEquationTraceObserver,
        communication:Option<&eredu_runtime::RetainedCommunicationSource>,
        parameters:Option<&dyn WorkspaceLayerwiseParameters>,
        observation:Option<ObservationRef<'_, '_>>,
    )->Result<PreparedTextGenerationWorkspace,PreparedExecutionError<Error>> {
        context.charge_metadata(std::mem::size_of::<(
            EquationVisitor<'_, '_, '_>, std::cell::RefCell<&mut dyn InferenceEquationTraceObserver>,
            TextSamplingInput<'_>, Result<PreparedTextGenerationWorkspace,PreparedExecutionError<Error>>,
            // Public configured/borrowed entry and the shared input worker.
            TextSamplingInput<'_>, &Self, InferenceGeometry, &ResidentState,
            &WorkspaceContext, Option<&eredu_runtime::RetainedCommunicationSource>,
            Option<&dyn WorkspaceLayerwiseParameters>, Option<ObservationRef<'_, '_>>, bool,
            Result<PreparedTextGenerationWorkspace,PreparedExecutionError<Error>>,
        )>()).map_err(|cause|PreparedExecutionError::Metadata(cause.into()))?;
        geometry.validate().map_err(|cause|preparation_message(context,format_args!("{cause}")))?;
        if geometry.batch_size!=1 || geometry.output==eredu_core::OutputDemand::StateOnly
            || (observation.is_none() && geometry.output!=eredu_core::OutputDemand::LastPosition)
        {return Err(preparation_message(context,format_args!("parallel sampling quote requires one sequence with an admitted output demand")));}
        if let Some(observation)=observation {
            observation.validate_selection(geometry,context).map_err(PreparedExecutionError::Backend)?;
            if observation.requires_sequence_readout() && geometry.output!=eredu_core::OutputDemand::Sequence {
                return Err(preparation_message(context,format_args!("parallel observation requires sequence output")));
            }
        }
        if matches!(self.selected().text_realization().residency(),eredu_runtime::LayerWeightResidency::FullyResident) != parameters.is_none() {
            return Err(preparation_message(context,format_args!("parallel parameter source differs from the selected weight residency")));
        }
        let source=self.sources.construction_semantics().direct_partition.get()
            .ok_or_else(||preparation_message(context,format_args!("exact completed direct partition quote source is unavailable")))?;
        let observer=std::cell::RefCell::new(observer);
        let visitor=EquationVisitor {
            geometry,state,context,sampling:Some(sampling),
            parameters,unpriced_execution:None,observation,
            trace:Some(EquationTraceRef(&observer)),media:None,input_dtype:None,
            target_capture:false,routed_pass:None,external_target:None,
        };
        let (equations,sampling)=source.0.quote(&self.sources,communication,visitor).map_err(PreparedExecutionError::Metadata)?;
        Ok(PreparedTextGenerationWorkspace {equations,sampling:sampling.expect("configured parallel quote includes sampling")})
    }
}
