...
type Policy<C, P> =
    ResidentUnitWindow<Unit<C,P>>;
type Executor<A, Q, U> = PipelinePartitionExecutor<
    A,
    WorkspaceBackend,
    ResidentState,
    Q,
    ...
    P::FeedForward: TensorParallelProjectionOperator<WorkspaceBackend>,
{
    ...
    context.charge_metadata(std::mem::size_of::<(
        ...
        Result<eredu_runtime::CommunicationGroupOperation<'_>, eredu_runtime::CommunicationGroupOperationError>,
    ...
    eredu_runtime::working_memory::validate_workspace_state_realization(
        visitor.state, &source.local_state, context,
    )?;
    ...
    match visitor.parameters {
        Some(parameters) => {
            ...
            pipeline_spans::<Model<C,P>,_,_>(&source.selected,source.rank,pipeline,architecture,policy,Some(bounded),addresses,communication,crate::partitioned_execution::OrdinaryPipelinePartitionUnitStrategy,visitor)
        }
        None => {
            let policy=Policy::<C,P>::from_workspace_addresses::<Model<C,P>,ResidentState>(
                &architecture,&addresses,context)?;
            pipeline_spans::<Model<C,P>,_,_>(&source.selected,source.rank,pipeline,architecture,policy,None,addresses,communication,crate::partitioned_execution::OrdinaryPipelinePartitionUnitStrategy,visitor)
        }
    }
}

#[allow(clippy::too_many_arguments)]
pub(super) fn pipeline_spans<A,Q,U>(selected:&crate::SelectedPreparation,
    rank:eredu_core::ParallelRankTopology,pipeline:&PipelineSource,
    architecture:A,policy:Q,bounded:Option<Q>,addresses:Vec<eredu_runtime::ExecutionUnitAddress>,
    communication:&eredu_runtime::RetainedCommunicationSource,unit_strategy:U,
...
where A:crate::partitioned_execution::TextPartitionArchitecture<WorkspaceBackend,ResidentState,Error=Error>
    ...
    U:crate::partitioned_execution::PipelinePartitionUnitStrategy<A,WorkspaceBackend,ResidentState>,
{
    let context=visitor.context;
    context.charge_metadata(std::mem::size_of::<(
        Runtime<A,Q,U>,Executor<A,Q,U>,Strategy<A,Q,U>,A,Q,Option<Q>,
        Vec<eredu_runtime::ExecutionUnitAddress>,Result<EquationQuote,Error>,
        &crate::SelectedPreparation,eredu_core::ParallelRankTopology,U,&PipelineSource,&eredu_runtime::RetainedCommunicationSource,EquationVisitor<'_, '_, '_>,
    )>())?;
    ...
    let mut runtime = Runtime::<A,Q,U>::with_retained_plan(
        ...
        OpaqueOutputPublisher,
        OpaqueFailureAgreement,
        selected.text_realization().residency().execution_residency(),
        bounded,
    )
    .map_err(|cause| context.metadata_source(cause))?;
    ...
    let output_owner=selected.execution().partitioned_output_publication()
        .ok_or_else(||context.metadata_error(format_args!("partition has no selected observation owner")))?.owner_rank;
    context.charge_metadata(std::mem::size_of::<Option<eredu_runtime::PreparedLayeredObservationPaths>>()
    ...
    visitor.quote_spans_with_prepublication_observation(hook_bytes, |tokens, state, demand, mut observer, span| {
        ...
        >>::with_borrowed_parallel_control_context(
            ...
            |runtime| {
                ...
                let mut execute = |observer: &mut dyn eredu_runtime::ActivationObserver<eredu_nn::workspace::WorkspaceTensor, Error>| {
                    match paths.as_ref() {
                        Some(paths) => <Strategy<A,Q,U> as ReplicatedTextExecutionStrategy<A,WorkspaceBackend,ResidentState,Q,Q>>::forward_with_prepared_observer(
                            &mut strategy, runtime, A::text_input(tokens, None),
                            state, pass, context, observer, paths, demand,
                        ),
                        None => <Strategy<A,Q,U> as ReplicatedTextExecutionStrategy<A,WorkspaceBackend,ResidentState,Q,Q>>::forward_with_observer(
                            &mut strategy, runtime, A::text_input(tokens, None),
                            state, pass, context, observer, demand,
                        ),
                    }.map_err(|cause| context.metadata_source(cause))
                };
                let (scores, _forward) = match observer.as_deref_mut() {
                ...
                let scores = match (scores, observer.as_deref_mut()) {
                    (Some(scores), Some(observer)) => Some(
                        <Strategy<A,Q,U> as ReplicatedTextExecutionStrategy<A,WorkspaceBackend,ResidentState,Q,Q>>::observe_output(
...
impl PartitionTensorAllocator<WorkspaceBackend> for Allocator {
    ...
    fn tensor_placeholder(
        ...
        context.charge_metadata(std::mem::size_of::<WorkspaceLayoutView<'_>>()
            + std::mem::size_of::<Option<WorkspaceRepresentation>>()
            + std::mem::size_of::<Result<WorkspaceLayoutView<'_>, eredu_nn::workspace::WorkspaceLayoutError>>())?;
...
pub(super) fn parallel_context(selected: &crate::SelectedPreparation,
    rank: eredu_core::ParallelRankTopology,
    communication: &eredu_runtime::RetainedCommunicationSource,
    context: &WorkspaceContext) -> Result<Option<WorkspaceParallelContext>, Error> {
    context.charge_metadata(std::mem::size_of::<(&crate::SelectedPreparation,
        eredu_core::ParallelRankTopology,&eredu_runtime::RetainedCommunicationSource,
        Result<Option<WorkspaceParallelContext>,Error>,
        eredu_runtime::CommunicationGroupOperation<'_>)>())?;
    Ok(match selected.execution().partitioned_tensor_group() {
        Some(id) => {
            context.charge_metadata(eredu_runtime::CommunicationManifest::group_operation_control_bytes()
                .ok_or_else(||context.metadata_error(format_args!("tensor group metadata overflow")))?)?;
            let group = communication.manifest().select_group_operation(id,eredu_runtime::CommunicationOperation::AllReduceSum)
                .map_err(|cause|context.metadata_source(cause))?;
            ...
            if descriptor.members().len()!=rank.tensor_parallel_size()
                ...
                return Err(context.metadata_error(format_args!("tensor subgroup differs from the retained partition")));
            }
            Some(WorkspaceParallelContext::new(rank.tensor_parallel_rank(),rank.tensor_parallel_size())?)
        }
        None if rank.tensor_parallel_size()==1 => None,
        None => return Err(context.metadata_error(format_args!("partition has no selected tensor subgroup"))),
    })
}
...
mod token_source_tests {
    ...
    impl WorkspaceMechanisms for Tokens {
        fn prepared_text_input_dtype(&self)->Option<WorkspaceDtype>{self.0}
    ...
    fn prepared_token_source_preserves_exact_pipeline_wire_dtype() {
        ...
        let wire=Allocator.tensor_to_wire(tokens,eredu_runtime::BoundaryTensorDtype::Uint32,
            eredu_runtime::PipelineActivationDtype::Float32,&context).unwrap();
        ...
        let signed=super::super::super::prepared_text_tokens(&[1,3],Some(WorkspaceDtype::Int32),&context).unwrap();
        assert!(Allocator.tensor_to_wire(signed,eredu_runtime::BoundaryTensorDtype::Uint32,
            eredu_runtime::PipelineActivationDtype::Float32,&context).is_err());
        let portable=WorkspaceContext::new(Tokens(None));
        assert_eq!(super::super::super::prepared_text_tokens(&[1,3],None,&portable).unwrap().layout().dtype(),WorkspaceDtype::Int32);
    }
}
