...
use eredu_runtime::working_memory::{
    workspace_partition_communication, WorkspaceCommunicationGroup,
    WorkspaceCommunicationMetadata, WorkspaceCommunicationRoute,
};

type Model<C, P> = PartitionedLayeredModel<WorkspaceBackend, C, P>;
type Communication = eredu_runtime::PartitionCommunication<
    WorkspaceBackend, WorkspaceCommunicationGroup, WorkspaceCommunicationRoute,
...
impl PreparedDirectPartitionSource {
        ...
        P::FeedForward: TensorParallelRoutedProjectionOperator<WorkspaceBackend>,
    {
        ...
        Self(Arc::new(Routed::<C, P> {
            target, selected, rank, local_state, execution, providers, pipeline,
    ...
    P::FeedForward: TensorParallelRoutedProjectionOperator<WorkspaceBackend>,
{
        ...
        -> Result<Option<Arc<crate::partitioned_execution::TensorPipelineCollectiveWaves>>, String>
    {
        if !selected.same_complete_selection(&self.selected) || rank != self.rank {
            return Err("retained routed partition source differs from its selected constructor".into());
        }
        Ok(self.pipeline.as_ref().and_then(|pipeline| pipeline.tensor_waves.clone()))
    }

    fn quote(&self, actual: &PreparedModelSources,
        ...
        visitor: EquationVisitor<'_, '_, '_>) -> Result<EquationQuote, Error>
    {
        ...
        context.charge_metadata(std::mem::size_of::<(
            ...
            Model<C, P>, Communication, eredu_nn::workspace::WorkspaceParallelContext,
            eredu_runtime::StateLayout, Result<EquationQuote, Error>,
        )>())?;
        if !actual.selected().same_complete_selection(&self.selected)
            ...
            || (self.rank.pipeline_parallel_size() == 1 && self.rank.tensor_parallel_size() <= 1
        ...
        let architecture = Model::<C, P>::from_retained_partition_source(self.target.clone(), context)?;
    ...
    Q::Error:std::error::Error+Send+Sync+'static,
{
    ...
    context.charge_metadata(std::mem::size_of::<(
        ...
        eredu_runtime::ExpertPass, Option<eredu_core::CollectiveGroupId>,
    )>())?;
    ...
    direct_publication_spans(selected, visitor, paths,
        |tokens, state, demand, observer, paths, span| {
            ...
            let mut agreed = eredu_runtime::expert::AgreeingRoutedExpertProvider::new(
                &mut provider, |success| crate::partitioned_execution::agree_provider_tensor_work(
                    communication, context, Some(parallel), group, success,
                ),
            );
            context.charge_metadata(std::mem::size_of_val(&agreed).checked_mul(2)
                .ok_or(eredu_nn::workspace::WorkspaceMetadataError::Overflow)?)?;
            let mut execute = |observer: &mut dyn eredu_runtime::ActivationObserver<WorkspaceTensor, Error>| {
                runtime.forward_routed_with_prepared_paths(
                    A::text_input(tokens, None), state,
                    (rank.tensor_parallel_size() > 1).then_some(parallel),
                    context, &mut agreed, pass, observer, demand, paths,
                ).map(|(scores, _forward)| scores).map_err(|cause| context.metadata_source(cause))
            };
            match observer {
...
fn pipeline_quote<C, P>(source: &Routed<C, P>, pipeline: &PipelineSource,
    architecture: Model<C, P>, communication: &eredu_runtime::RetainedCommunicationSource,
    visitor: EquationVisitor<'_, '_, '_>) -> Result<EquationQuote, Error>
    ...
    P::FeedForward: TensorParallelRoutedProjectionOperator<WorkspaceBackend>,
{
    let context = visitor.context;
    context.charge_metadata(std::mem::size_of::<(
        &Routed<C,P>, &PipelineSource, Model<C,P>,
        &eredu_runtime::RetainedCommunicationSource, EquationVisitor<'_, '_, '_>,
        eredu_runtime::StateLayout, Vec<eredu_runtime::ExecutionUnitAddress>,
        Result<EquationQuote, Error>,
    )>())?;
    let geometry = architecture.local_geometry();
    let layout = geometry.complete_state_layout()
    ...
    match visitor.parameters {
        Some(parameters) => {
            ...
            pipeline::pipeline_spans::<Model<C,P>,_,_>(&source.selected, source.rank, pipeline,
                architecture, policy, Some(bounded), addresses, communication, unit_strategy, visitor)
        }
        None => {
            let policy = ResidentUnitWindow::<Unit<C,P>>::from_workspace_addresses::<Model<C,P>,ResidentState>(
                &architecture, &addresses, context)?;
            pipeline::pipeline_spans::<Model<C,P>,_,_>(&source.selected, source.rank, pipeline,
                architecture, policy, None, addresses, communication, unit_strategy, visitor)
        }
    }
}
...
) -> Result<crate::partitioned_execution::RoutedPipelinePartitionUnitStrategy<Provider, movement::SymbolicRouteMovement>, Error> {
    context.charge_metadata(std::mem::size_of::<(
        &crate::partitioned_execution::PreparedRoutedExecutionHandoff, Provider, &WorkspaceContext,
        Result<crate::partitioned_execution::RoutedPipelinePartitionUnitStrategy<Provider,movement::SymbolicRouteMovement>,Error>,
        Option<usize>,
    )>())?;
    context.charge_metadata(execution.pipeline_unit_strategy_control_bytes::<Provider,movement::SymbolicRouteMovement>()
        .ok_or(eredu_nn::workspace::WorkspaceMetadataError::Overflow)?)?;
    execution.pipeline_unit_strategy(provider, movement::SymbolicRouteMovement)
