...
use eredu_runtime::{
    ...
    OpaqueBoundaryTransport, OpaqueOutputPublisher, OpaqueFailureAgreement,
    PreparedInputPart, PreparedInputPayload, PreparedModelInput,
};
use eredu_runtime::working_memory::{workspace_partition_communication,
    WorkspaceCommunicationGroup, WorkspaceCommunicationRoute, WorkspaceCommunicationMetadata};
...
impl PreparedDirectPartitionSource {
    pub(crate) fn composite(model:PreparedCompositeModelSource,
        selected:crate::SelectedPreparation, rank:eredu_core::ParallelRankTopology,
        state:eredu_runtime::SelectedStateRealization, state_offset:usize, executor:PreparedCompositeExecutorPlan,
        plan:Arc<eredu_runtime::PartitionedExecutionPlan>, addresses:Vec<eredu_runtime::ExecutionUnitAddress>) -> Self {
        Self(Arc::new(Composite {model, selected, rank, state, state_offset, executor, plan, addresses}))
    }
}
impl Composite {
    fn validate(&self, selected:&crate::SelectedPreparation, rank:eredu_core::ParallelRankTopology)
        -> Result<(),String> {
...
impl DirectPartitionSource for Composite {
    fn composite_executor(&self,selected:&crate::SelectedPreparation,rank:eredu_core::ParallelRankTopology)
        -> Result<Option<PreparedCompositeExecutorPlan>,String> {
        self.validate(selected,rank)?; Ok(Some(self.executor.clone()))
    }
    fn tensor_waves(&self, selected:&crate::SelectedPreparation,rank:eredu_core::ParallelRankTopology)
        ...
        self.validate(selected,rank)?; Ok(None)
    }
    fn quote(&self, actual:&PreparedModelSources, communication:Option<&eredu_runtime::RetainedCommunicationSource>,
        ...
        if actual.selected().communication_manifest()!=Some(communication.manifest()) {
            return Err(context.metadata_error(format_args!("composite communication differs from the retained manifest")));
        }
...
type Executor<A,Q> = crate::partitioned_execution::CompositePartitionExecutor<A,WorkspaceBackend,ResidentState,Q,Allocator>;
type Strategy<A,Q> = PartitionedTextExecution<Executor<A,Q>,WorkspaceCommunicationGroup,
    WorkspaceCommunicationRoute,WorkspaceCommunicationMetadata,OpaqueBoundaryTransport,OpaqueOutputPublisher,OpaqueFailureAgreement>;
type Runtime<A,Q> = PartitionedTextRuntime<Model<A>,WorkspaceBackend,ResidentState,Q,Executor<A,Q>,
    WorkspaceCommunicationGroup,WorkspaceCommunicationRoute,WorkspaceCommunicationMetadata,
    OpaqueBoundaryTransport,OpaqueOutputPublisher,OpaqueFailureAgreement>;
...
{
    let context=visitor.context;
    context.charge_metadata(std::mem::size_of::<(A,Model<A>,eredu_runtime::StateLayout,
    ...
    if &local!=visitor.state.layout() {
        return Err(context.metadata_error(format_args!("composite state differs from its retained local constructor")));
    }
    ...
    match visitor.parameters {
        Some(parameters) => {
            ...
            spans(source,architecture.into_inner(),admission,policy,Some(bounded),communication,visitor)
        }
        None => {
            let policy=ResidentUnitWindow::<A::Unit>::from_workspace_addresses::<Model<A>,ResidentState>(
                &architecture,&addresses,context)?;
            spans(source,architecture.into_inner(),admission,policy,None,communication,visitor)
        }
    }
}
...
{
    let context=visitor.context;
    context.charge_metadata(std::mem::size_of::<(A,Q,Option<Q>,Executor<A,Q>,Runtime<A,Q>,
        Strategy<A,Q>,A::AdmissionConfig,Option<WorkspaceParallelContext>,Option<Box<WorkspaceParallelContext>>,
        WorkspaceParallelContext,Option<eredu_runtime::PreparedLayeredObservationPaths>,
        Result<EquationQuote,Error>,PreparedInputPart<WorkspaceTensor>,PreparedModelInput<WorkspaceTensor>,
        crate::media_plan::AdmittedCompositeInput<A::InputPartPlan>,
        PreparedCompositeInput<'_,WorkspaceTensor,A::InputPartPlan>,usize)>())?;
    let parallel=parallel_context(&source.selected,source.rank,communication,context)?;
    let executor=source.executor.bind_borrowed_direct::<A,WorkspaceBackend,ResidentState,Q,Allocator>(
        architecture,policy,parallel,Allocator,context)?;
    let bindings=workspace_partition_communication(communication,context)?;
    let mut runtime=Runtime::<A,Q>::with_retained_plan(source.plan.clone(),executor,bindings,context.clone(),
        OpaqueBoundaryTransport,OpaqueOutputPublisher,OpaqueFailureAgreement,
        source.selected.text_realization().residency().execution_residency(),bounded)
        .map_err(|cause|context.metadata_source(cause))?;
    context.charge_metadata(std::mem::size_of::<WorkspaceParallelContext>())?;
    let publication=WorkspaceParallelContext::new(communication.manifest().rank(),communication.manifest().world_size())?;
    let mut control=Some(Box::new(WorkspaceParallelContext::new(communication.manifest().rank(),communication.manifest().world_size())?));
    let funding=context.metadata_funding().ok_or(eredu_nn::workspace::WorkspaceMetadataError::Unqualified)?;
    let output_owner=source.selected.execution().partitioned_output_publication().ok_or_else(||
    ...
    visitor.quote_spans_with_prepublication_observation(hook_bytes,|tokens,state,demand,mut observer,span| {
        <Strategy<A,Q> as ReplicatedTextExecutionStrategy<Model<A>,WorkspaceBackend,ResidentState,Q,Q>>::with_borrowed_parallel_control_context(
            &mut runtime,&mut control,&funding,|runtime| {
                let part=PreparedInputPart::new(eredu_core::InputModality::Text,
                    PreparedInputPayload::TokenIds(tokens.clone()),[]).map_err(|cause|context.metadata_source(cause))?;
                let mut parts=context.metadata_vec(1)?; parts.push(part);
                let input=PreparedModelInput::new_with_metadata(parts,context,|tensor|
                    super::super::composite::TextInspector::identity_with_metadata(tensor,context))?;
                let admitted=A::admit_prepared_input_with_metadata(&admission,&input,
                    &super::super::composite::TextInspector,context)?;
                ...
                let mut execute=|observer:&mut dyn eredu_runtime::ActivationObserver<WorkspaceTensor,Error>| {
                    match paths.as_ref() {
                        Some(paths)=><Strategy<A,Q> as ReplicatedTextExecutionStrategy<Model<A>,WorkspaceBackend,ResidentState,Q,Q>>::forward_with_prepared_observer(
                            &mut strategy,runtime,input,state,pass,context,observer,paths,demand),
                        None=><Strategy<A,Q> as ReplicatedTextExecutionStrategy<Model<A>,WorkspaceBackend,ResidentState,Q,Q>>::forward_with_observer(
                            &mut strategy,runtime,input,state,pass,context,observer,demand),
                    }.map_err(|cause|context.metadata_source(cause))
                };
                let (scores,_)=match observer.as_deref_mut() {Some(observer)=>execute(observer)?,
                    None=>execute(&mut eredu_runtime::inspection::NoopObserver)?};
                let scores=match(scores,observer.as_deref_mut()) {
                    (Some(scores),Some(observer))=>Some(<Strategy<A,Q> as ReplicatedTextExecutionStrategy<Model<A>,WorkspaceBackend,ResidentState,Q,Q>>::observe_output(
                        runtime,&scores,observer,context).map_err(|cause|context.metadata_source(cause))?),
                    (scores,_)=>scores,
                };
                let scores=scores.map(|scores|
                    <Strategy<A,Q> as ReplicatedTextExecutionStrategy<Model<A>,WorkspaceBackend,ResidentState,Q,Q>>::publish_observed_output_with_parallel(
                        runtime,scores,context,Some((&publication,&funding))).map_err(|cause|context.metadata_source(cause))).transpose()?;
                if communication.manifest().rank()!=output_owner {
                    if let(Some(scores),Some(observer))=(&scores,observer) {observer.observe_remote_output(scores,context)?;}
                }
                Ok((scores,None))
            }).map_err(|cause|context.metadata_source(cause))?
    })
