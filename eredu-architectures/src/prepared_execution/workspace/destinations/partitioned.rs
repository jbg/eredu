...
use crate::partitioned_execution::{
    PartitionedArchitectureVisitor, PreparedPartitionedArchitecture, TextPartitionArchitecture,
};
use eredu_runtime::{
    ExecutionUnitAddress, ExecutionUnitLayout, LocalModelLayout, PartitionedUnitScope,
...
impl PartitionedTextBindingDestinations {
    ...
    /// Exact selected targets supplied outside the moving-unit binder.
    /// This includes off-rank static slots and independently owned bank targets;
    /// absence from the destination rows alone never implies exclusion.
    pub fn excluded_parameters(&self) -> &BTreeSet<String> {
...
pub fn project_partitioned_text_binding_destinations(
...
pub(in crate::prepared_execution::workspace) fn project_partitioned_text_binding_destinations_for(
    ...
    let routes = PreparedExecutionRoutes::new()
        .with_partitioned_dense(
            PartitionedDenseRoute::<WorkspaceBackend, ResidentState, _>::new(
                context,
                context,
        ...
        .with_partitioned_composite(PartitionedCompositeRoute::<
            WorkspaceBackend,
            ResidentState,
            _,
        >::new(
...
impl PartitionedArchitectureVisitor<WorkspaceBackend, ResidentState>
...
impl PartitionDestinationVisitor<'_> {
    ...
    fn collect_architecture<A, G, W>(
        self,
        architecture: A,
        selected: SelectedReplicatedTextRealization,
        partition: eredu_runtime::ArchitecturePartition<G, W>,
        physical_layout: LocalModelLayout,
        tasks: Vec<ReplicatedTextMaterializationTask>,
        scope: PartitionedUnitScope,
        mut excluded_parameters: BTreeSet<String>,
    ) -> Result<PartitionedTextBindingDestinations, Error>
    where
        A: eredu_runtime::LayeredArchitecture<WorkspaceBackend, ResidentState, Error = Error>,
    {
        ...
        let graph = architecture
            .execution_graph()?
            .into_owned_with_metadata(self.0)?;
        let local_layout =
            eredu_runtime::partitioned_materialization_unit_layout(&graph, &addresses)
                .map_err(Error::backend)?;
        ...
        let task_plan = eredu_runtime::plan_local_replicated_text_materialization_tasks(
            &tasks,
            &global_layout,
            &addresses,
        )
        .map_err(Error::backend)?;
        let static_tasks = task_plan.static_tasks(&tasks).map_err(Error::backend)?;
        let selected_static = static_tasks
            .iter()
            .flat_map(|task| {
                std::iter::once(task.name())
                    .chain(task.output_companions().iter().map(|row| row.name()))
            })
            .collect::<BTreeSet<_>>();
        excluded_parameters.extend(
            static_parameters
                .keys()
                .filter(|name| !selected_static.contains(name.as_str()))
                .cloned(),
        );
        Ok(PartitionedTextBindingDestinations {
            selected,
            global_layout,
            local_layout,
            addresses,
...
impl PreparedExecutableAssembler<()> for PartitionDestinationAssembler<'_> {
    ...
    fn validate_communication(
        ...
        if manifest != self.manifest {
            return Err(Error::backend(
                "partition destination communication differs from retained selection",
            ));
        }
        Ok(())
    }
