...
#[derive(Clone, Copy)]
pub(in crate::media_plan) struct Shape<'a>(ShapeRef<'a>);
impl Shape<'_> {
    pub(in crate::media_plan) fn len(self) -> usize {
        self.0.len()
    }
    pub(in crate::media_plan) fn at(self, axis: usize) -> u64 {
        self.0.get(axis).expect("validated shape axis")
    }
    pub(in crate::media_plan) fn matches(self, expected: &[u64]) -> bool {
        self.len() == expected.len()
            && expected
                .iter()
                .enumerate()
                .all(|(axis, value)| self.0.get(axis) == Some(*value))
    }
    pub(in crate::media_plan) fn batch_one<D: Destination>(
        self,
        rank: usize,
        ...
        if self.len() != rank || self.0.get(0) != Some(1) || self.0.get(1).unwrap_or(0) == 0 {
            return Err(destination.unsupported(
                architecture,
                format_args!("prepared {name} must be batch-one with rank {rank}, got {self:?}"),
            ));
        }
        Ok(self.at(1))
    }
...
#[derive(Clone, Copy)]
pub(in crate::media_plan) struct Values<'a, T> {
    pub(in crate::media_plan) shape: Shape<'a>,
    pub(in crate::media_plan) values: &'a [T],
}
impl<'a, T> Values<'a, T> {
    pub(in crate::media_plan) fn legacy(input: &'a MetadataValues<T>) -> Self {
        Self {
            shape: Shape(ShapeRef::Dimensions(&input.shape)),
            values: &input.values,
        }
    }
}
...
pub(in crate::media_plan) struct Input<'a> {
    pub(in crate::media_plan) modality: InputModality,
    pub(in crate::media_plan) kind: InputPayloadKind,
    pub(in crate::media_plan) shape: Shape<'a>,
    pub(in crate::media_plan) patch_extent: Option<[usize; 3]>,
    pub(in crate::media_plan) audio_valid_frames: Option<usize>,
    pub(in crate::media_plan) patch_grid: Option<Values<'a, i32>>,
    pub(in crate::media_plan) patch_positions: Option<Values<'a, i32>>,
    pub(in crate::media_plan) audio_mask: Option<Values<'a, bool>>,
}
impl<'a> Input<'a> {
    pub(in crate::media_plan) fn legacy(input: &'a MediaAdmissionInput) -> Self {
        ...
        Self {
            ...
            kind: input.descriptor.payload_kind(),
            shape: Shape(ShapeRef::Dimensions(&input.payload_shape)),
            patch_extent: input.patch_extent(),
            audio_valid_frames: input.audio_valid_frames(),
    ...
    pub(in crate::media_plan) fn original(
        part: &PreparedHostPart<'a>,
        ...
        let integers = |key| -> Result<Option<Values<'a, i32>>, MediaSemanticError> {
            ...
            part.metadata_view(key)
                .map(|(_, value)| {
                    let HostTensorValues::I32(values) = value.values else {
                        return Err(MediaSemanticError::input("original integer metadata type"));
                    };
                    Ok(Values {
                        shape: Shape(ShapeRef::Host(value.shape)),
                        values,
                    })
                })
                .transpose()
        };
        let audio_mask = inspect_metadata
            .then(|| part.metadata_view(InputMetadataKey::AudioMask))
            .flatten()
            .map(|(_, value)| {
                let HostTensorValues::Bool(values) = value.values else {
                    return Err(MediaSemanticError::input("original Boolean metadata type"));
                };
                Ok(Values {
                    shape: Shape(ShapeRef::Host(value.shape)),
                    values,
        ...
        Ok(Self {
            ...
            kind: part.kind(),
            shape: Shape(ShapeRef::Host(part.payload_view().shape)),
            ...
            patch_positions: integers(InputMetadataKey::PatchPositions)?,
            audio_mask,
        })
    }
    pub(in crate::media_plan) fn projected<D: Destination>(
        self,
        args: &FamilyConfig,
        destination: D,
    ) -> Option<Result<Gemma4InputPartPlan, D::Error>> {
        let shape = self.shape;
        match (self.modality, self.kind) {
            (InputModality::Text, InputPayloadKind::TokenIds) => Some(
                shape
                    .batch_one(2, "Gemma text token IDs", &args.model_type, destination)
            ...
            (modality, InputPayloadKind::Embeddings) => Some((|| {
                let positions = shape.batch_one(
                    3,
                    format_args!("Gemma {} embeddings", modality.as_str()),
                    &args.model_type,
                    destination,
                )?;
                let hidden =
                    destination.positive(args.text.hidden_size, "Gemma text hidden size")?;
...
impl Destination for SourceDestination {
    ...
    fn unsupported(self, _: &str, _: impl std::fmt::Display) -> Self::Error {
        MediaSemanticError::input("original source violates shared input admission")
    }
    ...
    fn overflow(self, operation: &'static str) -> Self::Error {
        MediaSemanticError::overflow(operation)
    }
}
