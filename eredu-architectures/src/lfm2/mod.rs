...
impl<B: GroupedNeuralBackend + eredu_nn::DistributedNeuralBackend> LayeredModel<B> {
    ...
    fn parameter_description_impl(
        ...
        let graph = match metadata {
            Some(context) => self.decoder.execution_graph()?.into_owned_with_metadata(context)?,
            None => self.decoder.execution_graph()?.into_owned(),
        };
        let count =
        ...
        let static_groups = parallel::static_parallel_parameter_groups_destination(self.decoder.static_modules(), metadata)
    ...
    {
        self.decoder.unit_path(0, index, None)?;
        block.forward(
            hidden,
            forward.mask.as_ref(),
    ...
    fn finish_parallel_instrumented(
        &mut self,
        hidden: &B::Tensor,
        parallel: &B::ParallelContext,
        context: &<B::Tensor as Tensor>::Context,
    ...
    S::LayerState: AttentionCache<B::Tensor> + RuntimeStateComponents<B>,
{
    ...
    fn finish_forward_parallel(
        ...
        self.finish_parallel_instrumented(
            hidden,
            parallel,
            context,
    ...
    S::LayerState: AttentionCache<B::Tensor> + RuntimeStateComponents<B>,
{
    fn routed_observation_points(
        ...
        let unit_path = self.decoder.unit_path(group, index, None)?;
