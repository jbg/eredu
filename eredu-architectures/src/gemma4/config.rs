...
impl ModelArgs {
    ...
    /// Balances decoder layers across pipeline stages. Shared publications use
    /// bounded current-position transport and declared receiver cache replicas.
    pub fn pipeline_layer_ranges(&self, stages: usize) -> Result<Vec<Range<usize>>, ConfigError> {
        (0..stages)
            .map(|rank| {
                eredu_core::balanced_contiguous_range(self.num_hidden_layers(), stages, rank, false)
                    .map_err(|error| invalid(error.to_string()))
            })
            .collect::<Result<Vec<_>, _>>()
            .and_then(|ranges| {
                if ranges.is_empty() {
                    Err(invalid("Gemma pipeline stage count must be positive"))
                } else {
                    Ok(ranges)
                }
            })
