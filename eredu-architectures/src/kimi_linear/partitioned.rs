...
use eredu_nn::{
    ...
    NormalizationOperator, ParameterSpec, Tensor,
};
use eredu_runtime::{
        ...
        + BlockwiseAttentionBackend,
{
    ...
    fn finish_local(
        ...
        let norm = self
            ...
            .norm
            .as_mut()
            .ok_or_else(|| Error::backend("Kimi partition does not own final normalization"))?;
        ...
        match (parallel, self.static_modules.lm_head.as_mut()) {
            ...
            (Some(parallel), None) => B::vocabulary_parallel_embedding_project(
                self.static_modules
                    .embeddings
                    .as_mut()
                    .ok_or_else(|| Error::backend("tied Kimi output partition has no embedding"))?,
                &hidden,
                parallel,
                context,
            ),
