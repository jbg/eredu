//! Backend-neutral Kimi Linear configuration and operators.

/// Kimi normalization, residual, and typed block assembly.
pub mod block;
/// Pure SafeTensors/GGUF catalog plans and canonical name translation.
pub mod checkpoint;
pub mod config;
/// Kimi Delta Attention over neutral recurrent operators.
pub mod kda;
/// Kimi no-positional compressed latent attention.
pub mod mla;
...
/// Architecture-owned values retained for one Kimi forward pass.
pub struct ForwardContext<T> {
    mask: Option<T>,
}

impl<T> ForwardContext<T> {
    /// Borrows the architecture-prepared attention mask at a partition boundary.
    ...
    B: GroupedNeuralBackend + eredu_nn::DistributedNeuralBackend + BlockwiseAttentionBackend,
{
    ...
    /// Mutably borrows embedding, final normalization, and vocabulary modules.
        ...
        match &self.parallel_geometry {
            Some(geometry) => Block::new_with_geometry(
                ...
                *geometry.block(index).ok_or_else(|| {
                    Error::backend(format!("missing rank-local Kimi block geometry {index}"))
                })?,
                context,
            ),
    ...
    /// Begins execution from a placement-supplied embedding.
    ...
    {
        self.begin_embedded_with_layout_at(hidden, mask, state, expected, 0, context)
    }
        ...
        S::LayerState: RuntimeStateComponents<B>,
    {
        if state.layout() != expected {
            return Err(Error::backend(format!(
                "Kimi runtime state layout {:?} does not match architecture layout {expected:?}",
                state.layout()
            )));
        }
        ...
        let mask = if let Some(mask) = mask {
            ...
            && self
                ...
                .layer_schedule
                .iter()
                .any(|policy| policy.attention == AttentionKind::Mla)
        {
            let position = state
    ...
    /// Executes one block while delegating its feed-forward policy.
    ...
    /// Executes one tensor-partitioned block.
    ...
    S::LayerState: RuntimeStateComponents<B> + CompressedAttentionCache<B::Tensor>,
{
    ...
    fn begin_execution_group(
        ...
        self.group.begin(group, initial, dependencies)
    }
    ...
    S::LayerState: RuntimeStateComponents<B> + CompressedAttentionCache<B::Tensor>,
{
        ...
        O: eredu_runtime::ActivationObserver<B::Tensor, Self::Error> + ?Sized,
    {
        ...
        let mut observer = eredu_runtime::BorrowedActivationObserver(observer);
        self.static_modules.finish_parallel_instrumented(
            hidden,
            parallel,
            context,
