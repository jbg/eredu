...
impl PreparedModelDiscovery {
    ...
    pub fn prepare_intervention_discovery(
        ...
        eredu_runtime::inspection::FundedInterventionDiscovery,
    ...
    > {
        ...
        let artifact = self
            .identity
            .resolved_identity()
            .ok_or_else(|| E::identity(funding))?;
        eredu_runtime::inspection::prepare_intervention_discovery(
            plan,
    ...
    > {
        ...
        if execution != self.execution_identity()
            || self
                .partition_selection
                .as_ref()
                .and_then(|source| source.parallel_topology())
    ...
    /// Revalidates routing and activation fields before the independently paid
    /// selected observer proves its actual numerical mechanisms and row sources.
    pub fn validate_prepared_intervention(
        &self,
        plan: &eredu_core::intervention::AdmittedInterventionPlan,
        facts: eredu_core::intervention::InterventionMechanismFacts<'_>,
        session: &str,
        ...
        let identity = self.identity.resolve().map_err(|_| E::Identity)?;
        plan.validate_source_identity(identity, session)?;
        eredu_runtime::inspection::validate_prepared_intervention_declarations(
            plan,
            &self.intervention_points,
            &self.support.points,
            facts,
    ...
    /// The same immutable comparison including routing fields on actual
    /// retained partition discovery; native hook qualification remains separate.
    pub fn validate_partitioned_prepared_intervention(
        &self,
        plan: &eredu_core::intervention::AdmittedInterventionPlan,
        facts: eredu_core::intervention::InterventionMechanismFacts<'_>,
        session: &str,
    ...
    pub fn partitioned_intervention_validation_control_bytes() -> Option<usize> {
        ...
        let frames = [
            Self::static_intervention_validation_control_bytes()?,
            size_of::<(
                &Self,
                &eredu_core::intervention::AdmittedInterventionPlan,
                eredu_core::intervention::InterventionMechanismFacts<'_>,
                &str,
