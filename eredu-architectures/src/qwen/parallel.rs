...
pub fn local_block_args(
    ...
    let query_width = i32::try_from(query.local_shape()[0]).map_err(|_| {
        ParallelPlanError::InvalidTensor("local Qwen query width exceeds i32".into())
    })?;
    let key_width = i32::try_from(key.local_shape()[0])
    ...
    if args.is_moe() {
        ...
        local.intermediate_size = i32::try_from(gate.local_shape()[0]).map_err(|_| {
            ParallelPlanError::InvalidTensor("local Qwen MLP width exceeds i32".into())
        })?;
    }
    Ok(local)
}
