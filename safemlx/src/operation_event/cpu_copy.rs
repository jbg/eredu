...
impl CpuCopyEvalLayout {
    ...
    pub fn signal_graph_allocation_extents(self) -> usize {
        self.native.signal_graph_extents
    }
    ...
    fn inspect(rank: usize, inputs: usize, copy: bool, tracer: bool) -> Option<Self> {
        ...
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_copy_eval_layout(
                &mut native,
                rank,
                inputs,
                copy,
...
impl OperationEvent {
    /// Exact scalar depthwise 1D convolution task, with one output and no
    /// numerical scratch. The native consumer validates rank-three inputs,
    /// one channel per filter, unit stride/dilation and zero padding.
    pub fn cpu_depthwise_convolution_layout(
        dtype: Dtype,
        tracer: bool,
    ) -> Option<CpuCopyEvalLayout> {
        let mut native = safemlx_sys::mlx_cpu_copy_eval_layout::default();
        // SAFETY: the pure query reads scalars and writes this initialized record.
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_depthwise_convolution_eval_layout(
                &mut native,
                dtype.into(),
                tracer,
            )
    ...
    /// Constant padding through one scalar fill and one rectangular copy.
    /// The native consumer validates the actual Pad geometry and source span.
    pub fn cpu_constant_pad_layout(
        rank: usize,
        elements: usize,
        ...
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_constant_pad_eval_layout(
                &mut native,
                rank,
                elements,
                input_elements,
    ...
    pub fn cpu_host_transfer_layout(
        dtype: Dtype,
        rank: usize,
        store: bool,
        tracer: bool,
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_host_transfer_eval_layout(
                &mut native,
                dtype.into(),
                rank,
                store,
    ...
    pub fn cpu_cast_layout(
        ...
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_cast_eval_layout(
                &mut native,
                source.into(),
                destination.into(),
                rank,
    ...
    pub fn cpu_byte_view_layout(
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_byte_view_eval_layout(
                &mut native,
                source.into(),
                destination.into(),
                rank,
        ...
        let controls = [
            size_of::<(Dtype, Dtype, usize, usize, bool, bool)>(),
            size_of_val(&native),
            size_of::<Option<CpuCopyEvalLayout>>(),
        ];
        native.named_control_bytes = native.named_control_bytes.checked_add(
    ...
    pub fn cpu_slice_layout(rank: usize, empty: bool, tracer: bool) -> Option<CpuCopyEvalLayout> {
        ...
        // SAFETY: the pure scalar query writes initialized output only on success.
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_slice_eval_layout(&mut native, rank, empty, tracer)
        }
        .then_some(CpuCopyEvalLayout { native })
    }
    ...
    pub fn cpu_scalar_update_layout(elements: usize, tracer: bool) -> Option<CpuCopyEvalLayout> {
        ...
        // SAFETY: pure scalar query writes initialized output only on success.
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_scalar_update_eval_layout(
                &mut native,
                elements,
                tracer,
            )
    ...
    pub fn cpu_static_update_layout(
        rank: usize,
        elements: usize,
        update_elements: usize,
        tracer: bool,
    ) -> Option<CpuCopyEvalLayout> {
        let mut native = safemlx_sys::mlx_cpu_copy_eval_layout::default();
        // SAFETY: pure scalar query writes initialized output only on success.
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_static_update_eval_layout(
                &mut native,
                rank,
                elements,
                update_elements,
    ...
    pub fn cpu_argsort_layout(
        ...
        // SAFETY: pure scalar query writes initialized output only on success.
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_argsort_eval_layout(
                &mut native,
                dtype.into(),
                rank,
                columns,
    ...
    pub fn cpu_softmax_layout(
        ...
        // SAFETY: pure scalar query writes initialized output only on success.
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_softmax_eval_layout(
                &mut native,
                rank,
                columns,
                rows,
    ...
    pub fn cpu_typed_softmax_layout(
        dtype: Dtype,
        precise: bool,
        rank: usize,
        columns: usize,
        ...
        // SAFETY: pure scalar query writes initialized output only on success.
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_typed_softmax_eval_layout(
                &mut native,
                dtype.into(),
                precise,
                rank,
        ...
        let controls = [
            size_of::<Dtype>(),
            size_of::<safemlx_sys::mlx_dtype>(),
            size_of::<(bool, usize, usize, usize, bool)>(),
            size_of_val(&native),
            size_of::<Option<CpuCopyEvalLayout>>(),
        ];
        native.named_control_bytes = native.named_control_bytes.checked_add(
    ...
    pub fn cpu_arg_reduce_layout(
        ...
        // SAFETY: pure scalar query writes initialized output only on success.
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_greedy_eval_layout(
                &mut native,
                rank,
                columns,
                rows,
    ...
    pub fn cpu_typed_arg_reduce_layout(
        dtype: Dtype,
        rank: usize,
        columns: usize,
        rows: usize,
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_typed_arg_reduce_eval_layout(
                &mut native,
                dtype.into(),
                rank,
                columns,
    ...
    pub fn cpu_affine_quantize_layout(
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_affine_quantize_eval_layout(
                &mut native,
                dtype.into(),
                rank,
                rows,
        ...
        let controls = [
            size_of::<(Dtype, usize, usize, usize, i32, i32, bool, bool)>(),
            size_of::<safemlx_sys::mlx_dtype>(),
            size_of_val(&native),
            size_of::<Option<CpuCopyEvalLayout>>(),
        ];
    ...
    pub fn cpu_reshape_alias_layout(
        rank: usize,
        output_rank: usize,
        tracer: bool,
    ) -> Option<CpuCopyEvalLayout> {
        ...
        // SAFETY: pure scalar query writes initialized output only on success.
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_reshape_alias_eval_layout(
                &mut native,
                rank,
                output_rank,
                tracer,
    ...
    pub fn cpu_reshape_copy_layout(
        rank: usize,
        output_rank: usize,
        tracer: bool,
    ) -> Option<CpuCopyEvalLayout> {
        ...
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_reshape_copy_eval_layout(
                &mut native,
                rank,
                output_rank,
                tracer,
    ...
    pub fn cpu_reshape_layout(
        source_shape: &[i32],
        source_strides: &[i64],
        output_shape: &[i32],
        tracer: bool,
    ) -> Option<CpuCopyEvalLayout> {
        if source_shape.len() != source_strides.len() {
            return None;
        }
        let mut native = safemlx_sys::mlx_cpu_copy_eval_layout::default();
        ...
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_reshape_eval_layout(
                &mut native,
                source_shape.as_ptr(),
    ...
    /// Exact Copy alias over the same typed shape and retained source backing.
    /// Native evaluation validates the source; no numerical task or new backing
    /// is created by this alias operation.
    pub fn cpu_copy_alias_layout(rank: usize, tracer: bool) -> Option<CpuCopyEvalLayout> {
        Self::cpu_shape_alias_layout(4, rank, rank, tracer)
    }
    ...
    pub fn cpu_broadcast_alias_layout(
        rank: usize,
        output_rank: usize,
        tracer: bool,
    ) -> Option<CpuCopyEvalLayout> {
    ...
    pub fn cpu_empty_broadcast_layout(
        rank: usize,
        output_rank: usize,
        tracer: bool,
    ) -> Option<CpuCopyEvalLayout> {
    ...
    pub fn cpu_expand_dims_alias_layout(
        rank: usize,
        output_rank: usize,
        tracer: bool,
    ) -> Option<CpuCopyEvalLayout> {
    ...
    fn cpu_shape_alias_layout(
        ...
        // SAFETY: pure scalar query writes initialized output only on success.
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_alias_eval_layout(
                &mut native,
                operation,
                rank,
                output_rank,
    ...
    pub fn cpu_random_bits_layout(
        ...
        // SAFETY: pure scalar query writes initialized output only on success.
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_random_bits_eval_layout(
                &mut native,
                rank,
                elements,
                tracer,
    ...
    pub fn cpu_bf16_matmul_layout(
        ...
        // SAFETY: pure scalar query writes initialized output only on success.
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_bf16_matmul_eval_layout(
                &mut native,
                rank,
                m,
                n,
    ...
    pub fn cpu_bf16_matmul_copy_layout(
        rank: usize,
        m: usize,
        n: usize,
        k: usize,
        ...
        // SAFETY: pure scalar query writes initialized output only on success.
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_bf16_matmul_copy_eval_layout(
                &mut native,
                rank,
                m,
                n,
        ...
        let controls = [
            size_of::<(usize, usize, usize, usize, usize, usize, bool)>(),
            size_of_val(&native),
            size_of::<Option<CpuCopyEvalLayout>>(),
        ];
        native.named_control_bytes = native.named_control_bytes.checked_add(
    ...
    /// Rank-five batch metadata remains inline; native inspection validates the
    /// selected primitive and actual operand strides before entering this bank.
    pub fn cpu_f16_matmul_copy_layout(
        rank: usize,
        m: usize,
        n: usize,
        k: usize,
        ...
        // SAFETY: pure scalar query writes initialized output only on success.
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_f16_matmul_copy_eval_layout(
                &mut native,
                rank,
                m,
                n,
    ...
    pub fn cpu_tiled_matmul_layout(
        rank: usize,
        m: usize,
        n: usize,
        k: usize,
        ...
        // SAFETY: pure scalar query writes initialized output only on success.
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_tiled_matmul_eval_layout(
                &mut native,
                rank,
                m,
                n,
    ...
    /// Selected F32 AddMM source: one output backing, its bias-copy task,
    /// and the existing tiled matrix task. Eval authenticates all three operands.
    pub fn cpu_tiled_addmm_layout(
        rank: usize,
        m: usize,
        n: usize,
        k: usize,
        ...
        // SAFETY: pure scalar query writes initialized output only on success.
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_tiled_addmm_eval_layout(
                &mut native,
                rank,
                m,
                n,
    ...
    pub fn cpu_tiled_matmul_copy_layout(
        rank: usize,
        m: usize,
        n: usize,
        k: usize,
        ...
        // SAFETY: pure scalar query writes initialized output only on success.
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_tiled_matmul_copy_eval_layout(
                &mut native,
                rank,
                m,
                n,
    ...
    pub fn cpu_gather_layout(
        ...
        // SAFETY: the pure scalar query writes initialized output only on success.
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_gather_eval_layout(
                &mut native,
                source.into(),
                index.into(),
                source_rank,
        ...
        native.named_control_bytes = native.named_control_bytes.checked_add(size_of::<(
            Dtype,
            Dtype,
            usize,
            usize,
    ...
    pub fn cpu_scatter_layout(
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_scatter_eval_layout(
                &mut native,
                source.into(),
                index.into(),
                rank,
        ...
        let frames = [
            size_of::<(Dtype, Dtype, usize, usize, usize, bool)>(),
            size_of::<(safemlx_sys::mlx_dtype, safemlx_sys::mlx_dtype)>(),
            size_of_val(&native),
            size_of::<Option<CpuCopyEvalLayout>>(),
        ];
        native.named_control_bytes = native.named_control_bytes.checked_add(
    ...
    /// Bound the existing F32/Bool final-axis overwrite scatter. One cleanup owns
    /// its three inputs through both the initial copy and the scatter job.
    pub fn cpu_scatter_axis_layout(
        index: Dtype,
        rank: usize,
        output_elements: usize,
        update_elements: usize,
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_scatter_axis_eval_layout(
                &mut native,
                index.into(),
                rank,
                output_elements,
        ...
        native.named_control_bytes =
            native
                .named_control_bytes
                .checked_add(size_of::<(Dtype, usize, usize, usize, bool)>())?;
        Some(CpuCopyEvalLayout { native })
    }
    ...
    pub fn cpu_scatter_add_rows_layout(
        index: Dtype,
        output_elements: usize,
        update_elements: usize,
        tracer: bool,
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_scatter_add_rows_eval_layout(
                &mut native,
                index.into(),
                output_elements,
                update_elements,
    ...
    /// Exact existing rank-one I32 ScatterAxis sum used by index discovery.
    /// Source authentication retains its axis, signed payload, and input geometry.
    pub fn cpu_int32_histogram_layout(
        index: Dtype,
        output_elements: usize,
        update_elements: usize,
        tracer: bool,
        ...
        // SAFETY: the checked scalar query initializes output only on success.
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_int32_histogram_eval_layout(
                &mut native,
                index.into(),
                output_elements,
                update_elements,
    ...
    pub fn cpu_boolean_reduce_layout(
        all: bool,
        rank: usize,
        elements: usize,
        tracer: bool,
    ...
    pub fn cpu_half_row_sum_layout(
        dtype: Dtype,
        rank: usize,
        width: usize,
        rows: usize,
    ...
    pub fn cpu_half_row_max_layout(
        dtype: Dtype,
        rank: usize,
        width: usize,
        rows: usize,
    ...
    /// Existing rank-one I32 complete-axis sum used to validate index bounds.
    /// Eval authenticates the signed input and unchanged contiguous sum worker.
    pub fn cpu_flat_i32_sum_layout(elements: usize, tracer: bool) -> Option<CpuCopyEvalLayout> {
        Self::cpu_reduction_layout(14, 1, elements, 1, tracer)
    }
    ...
    fn cpu_reduction_layout(
        ...
        // SAFETY: the pure scalar query writes initialized output only on success.
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_reduction_eval_layout(
                &mut native,
                operation,
                rank,
                width,
        ...
        native.named_control_bytes =
            native
                .named_control_bytes
                .checked_add(size_of::<(u32, usize, usize, usize, bool)>())?;
        Some(CpuCopyEvalLayout { native })
    }
    ...
    pub fn cpu_select_broadcast_layout(
        rank: usize,
        elements: usize,
        tracer: bool,
    ) -> Option<CpuCopyEvalLayout> {
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_select_broadcast_eval_layout(
                &mut native,
                rank,
                elements,
                tracer,
        ...
        native.named_control_bytes =
            native
                .named_control_bytes
                .checked_add(size_of::<(usize, usize, bool)>())?;
        Some(CpuCopyEvalLayout { native })
    }
    /// Existing F32/F16/BF16, I32 or Bool Select over readable stored/broadcast operands.
    ...
    pub fn cpu_typed_select_broadcast_layout(
        dtype: Dtype,
        rank: usize,
        elements: usize,
        tracer: bool,
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_typed_select_broadcast_eval_layout(
                &mut native,
                dtype.into(),
                rank,
                elements,
    ...
    pub fn cpu_row_full_layout(
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_row_full_eval_layout(
                &mut native,
                rank,
                width,
                rows,
        ...
        native.named_control_bytes =
            native
                .named_control_bytes
                .checked_add(size_of::<(usize, usize, usize, bool)>())?;
        Some(CpuCopyEvalLayout { native })
    }
    fn cpu_selection_layout(
        dtype: Dtype,
        rank: usize,
        elements: usize,
        full: bool,
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_selection_eval_layout(
                &mut native,
                dtype.into(),
                rank,
                elements,
        ...
        native.named_control_bytes =
            native
                .named_control_bytes
                .checked_add(size_of::<(Dtype, usize, usize, bool, bool)>())?;
        Some(CpuCopyEvalLayout { native })
    }
...
impl OperationEvent {
    ...
    pub fn cpu_rms_fallback_control_bytes(
        dtype: crate::Dtype,
        rank: usize,
        width: usize,
        rows: usize,
        ...
        let present = unsafe {
            safemlx_sys::mlx_operation_event_cpu_rms_fallback_control_bytes(
                &mut native,
                dtype.into(),
                rank,
                width,
        ...
        let parts = [
            std::mem::size_of::<(crate::Dtype, usize, usize, usize)>(),
        ...
        parts.into_iter().try_fold(
            native.checked_add(std::mem::size_of_val(&parts))?,
            usize::checked_add,
        )
    }
}
...
impl OperationEvent {
    /// Exact named frames of the existing CPU LayerNorm fallback, separate from its
    /// scalar sources and each ordinary arithmetic/alias producer. This pure
    /// query neither constructs a stream nor authorizes an operation.
    pub fn cpu_layer_norm_fallback_control_bytes(
        dtype: crate::Dtype,
        rank: usize,
        width: usize,
        rows: usize,
        ...
        // SAFETY: scalar query writes the initialized destination only on success.
        let present = unsafe {
            safemlx_sys::mlx_operation_event_cpu_layer_norm_fallback_control_bytes(
                &mut native,
                dtype.into(),
                rank,
                width,
...
impl OperationEvent {
    ...
    pub fn cpu_concatenate_many_layout(
        dtype: Dtype,
        rank: usize,
        inputs: usize,
        elements: usize,
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_concatenate_many_eval_layout(
                &mut native,
                dtype.into(),
                rank,
                inputs,
    ...
    pub fn cpu_concatenate_layout(
        ...
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_concatenate_eval_layout(
                &mut native,
                dtype.into(),
                rank,
                left_elements,
...
impl OperationEvent {
    ...
    pub fn cpu_arange_float_layout(elements: usize, tracer: bool) -> Option<CpuCopyEvalLayout> {
        ...
        unsafe {
            safemlx_sys::mlx_operation_event_cpu_arange_float_eval_layout(
                &mut native,
                elements,
                tracer,
            )
    ...
    pub fn cpu_arange_int_layout(
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_arange_int_eval_layout(
                &mut native,
                dtype.into(),
                elements,
                tracer,
    ...
    pub fn cpu_tiled_gather_mm_layout(
        lhs_rank: usize,
        rhs_rank: usize,
        index_rank: usize,
        m: usize,
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_tiled_gather_mm_eval_layout(
                &mut native,
                lhs_rank,
                rhs_rank,
                index_rank,
        ...
        let frames = [
            size_of::<(usize, usize, usize, usize, usize, usize, usize, bool)>(),
            size_of_val(&native),
            size_of::<Option<CpuCopyEvalLayout>>(),
        ];
        native.named_control_bytes = native.named_control_bytes.checked_add(
    ...
    pub fn cpu_rope_fallback_control_bytes(
        rank: usize,
        dimensions: usize,
        elements: usize,
    ) -> Option<usize> {
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_rope_fallback_control_bytes(
                &mut native,
                rank,
                dimensions,
                elements,
...
impl OperationEvent {
    ...
    pub fn cpu_sdpa_array_mask_control_bytes(
        rank: usize,
        queries: usize,
        keys: usize,
        values: usize,
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_sdpa_array_mask_control_bytes(
                &mut native,
                rank,
                queries,
                keys,
    ...
    pub fn cpu_sdpa_fallback_control_bytes(
        rank: usize,
        queries: usize,
        keys: usize,
        values: usize,
        ...
        // SAFETY: the pure query writes only the fixed result and retains nothing.
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_sdpa_fallback_control_bytes(
                &mut native,
                rank,
                queries,
                keys,
...
impl OperationEvent {
    ...
    pub fn cpu_partition_row_layout(
        rank: usize,
        elements: usize,
        tracer: bool,
    ) -> Option<CpuCopyEvalLayout> {
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_partition_row_eval_layout(
                &mut native,
                rank,
                elements,
                tracer,
    ...
    /// Native Eval independently authenticates source, axis, mode and backing.
    pub fn cpu_scan_sum_row_layout(
        rank: usize,
        elements: usize,
        tracer: bool,
    ) -> Option<CpuCopyEvalLayout> {
        ...
        // SAFETY: pure query; initialized destination is published only on success.
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_scan_sum_row_eval_layout(
                &mut native,
                rank,
                elements,
                tracer,
    ...
    pub fn cpu_maximum_row_layout(
        ...
        // SAFETY: pure query; initialized destination is published only on success.
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_maximum_row_eval_layout(
                &mut native,
                rank,
                elements,
                tracer,
...
impl OperationEvent {
    ...
    pub fn cpu_gather_axis_row_layout(
        rank: usize,
        elements: usize,
        tracer: bool,
    ) -> Option<CpuCopyEvalLayout> {
        ...
        if !unsafe {
            safemlx_sys::mlx_operation_event_cpu_gather_axis_row_eval_layout(
                &mut native,
                rank,
                elements,
                tracer,
    ...
    fn typed_reduction_queries_preserve_dtype_and_geometry_checks() {
        ...
        for dtype in [Dtype::Float16, Dtype::Bfloat16, Dtype::Float32] {
            ...
            for rank in [0, 5, usize::MAX] {
                assert!(
                    OperationEvent::cpu_typed_arg_reduce_layout(dtype, rank, 7, 3, false).is_none()
                );
                assert!(
                    OperationEvent::cpu_half_row_max_layout(dtype, rank, 7, 3, false).is_none()
                );
            }
            assert!(OperationEvent::cpu_typed_arg_reduce_layout(
                dtype,
                2,
                7,
                i32::MAX as usize + 2,
