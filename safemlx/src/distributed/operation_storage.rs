...
use super::{
    Group, GroupConstructorStorage, GroupCpuStorageFacts, GroupStorageUnavailable,
...
impl Group {
    ...
    pub fn cpu_operation_storage<'a>(
        &'a self,
        input: &'a Array,
        ...
        if !unsafe {
            safemlx_sys::mlx_distributed_query_cpu_source_storage(
                &mut value,
                self.native.c_group,
                input.as_ptr(),
                code,
        ...
        Ok(GroupCpuOperationStorage {
            constructor,
            evaluation: GroupCpuStorageFacts::from_native(value),
        })
    }
    ...
    pub fn cpu_operation_storage_control_bytes(&self) -> Option<usize> {
        ...
        let native = unsafe {
            safemlx_sys::mlx_distributed_cpu_source_storage_controls(self.native.c_group)
        };
        let frames = [
            size_of::<GroupCpuOperationStorage<'_>>(),
            size_of::<GroupCpuStorageFacts>(),
...
impl<'a> GroupCpuOperationStorage<'a> {
    ...
    pub fn constructor(&self) -> &GroupConstructorStorage<'_> {
        &self.constructor
    }
    ...
    pub fn control_bytes(&self) -> Option<usize> {
        execution_control_bytes(
            self.constructor.construction_control_bytes()?,
...
mod tests {
    ...
    fn cold_cpu_source_keeps_singleton_identity_distinct_from_ring_evaluation() {
        ...
        let constructor = group
            .constructor_storage(&input, GroupWorkerOperation::Sum)
            .unwrap();
        assert_eq!(constructor.graph_population(), (0, 0));
        assert!(matches!(
            group.cpu_operation_storage(&input, GroupWorkerOperation::Sum),
...
impl GroupCpuBackingStorage<'_, '_> {
    ...
    pub fn is_for(
        &self,
        source: &GroupCpuOperationStorage<'_>,
...
impl<'input> GroupCpuOperationStorage<'input> {
    ...
    pub fn backing_storage_control_bytes(&self) -> Option<usize> {
        backing_query_control_bytes(self.constructor.operation())
    }
}
...
pub(super) fn execution_control_bytes(constructor: usize, evaluation: usize) -> Option<usize> {
    constructor
        .checked_add(evaluation)?
        .checked_add(size_of::<GroupCpuOperationStorage<'_>>())?
        .checked_add(size_of::<(
            &GroupCpuOperationStorage<'_>,
            &OriginalScopeObserver,
            &Stream,
        )>())
}
pub(super) fn backing_query_control_bytes(operation: GroupWorkerOperation) -> Option<usize> {
    let frames = [
        size_of::<GroupCpuBackingStorage<'_, '_>>(),
        size_of::<(&GroupCpuOperationStorage<'_>, &crate::PreparedInputRuntime)>(),
        size_of::<std::result::Result<GroupCpuBackingStorage<'_, '_>, crate::OriginalBufferCause>>(
        ),
    ];
    frames.into_iter().try_fold(
        size_of_val(&frames).checked_add(backing_control_bytes(operation)?)?,
        usize::checked_add,
    )
}
