...
pub(in crate::distributed) fn backing_capacity(
    ...
    match operation {
        GroupWorkerOperation::Gather | GroupWorkerOperation::Variable => {
            OriginalBufferBudget::request_layout(runtime, copy)?
                .capacity()
        ...
        GroupWorkerOperation::Send { .. } => {
            Ok(OriginalBufferBudget::request_layout(runtime, copy)?.capacity())
        }
        ...
        | GroupWorkerOperation::Minimum => {
            if copy != output {
                return Err(OriginalBufferCause::InvalidLayout);
            }
            Ok(OriginalBufferBudget::request_layout(runtime, output)?.capacity())
        }
    }
}
pub(in crate::distributed) fn backing_control_bytes(
    operation: GroupWorkerOperation,
) -> Option<usize> {
    let requests = usize::from(matches!(
        operation,
        GroupWorkerOperation::Gather | GroupWorkerOperation::Variable
    )) + 1;
    let frames = [
        size_of::<(
            ...
            &GroupCpuStorageFacts,
            &PreparedInputRuntime,
        )>(),
        size_of::<std::result::Result<usize, OriginalBufferCause>>(),
        size_of::<usize>() * 4,
        size_of::<Option<usize>>(),
        OriginalBufferBudget::request_layout_control_bytes()?.checked_mul(requests)?,
    ];
    frames
        .into_iter()
