...
use super::{Group,GroupStorageUnavailable,GroupStorageKind,GroupStorageDomain,NativeGroup};
use std::{alloc::Layout,fmt,mem::{size_of,size_of_val}};
...
impl fmt::Debug for GroupPersistentStorage<'_>{
    fn fmt(&self,f:&mut fmt::Formatter<'_>)->fmt::Result{
        f.debug_struct("GroupPersistentStorage").field("kind",&self.kind())
            .field("unresolved",&self.value.unresolved).finish_non_exhaustive()
    }
}
impl Group{
    ...
    pub fn persistent_storage(&self)->Result<GroupPersistentStorage<'_>,GroupStorageUnavailable>{
        ...
        if !unsafe{safemlx_sys::mlx_distributed_group_persistent_storage(&mut value,self.native.c_group)}{
    ...
    pub fn persistent_storage_control_bytes(&self)->Option<usize>{
        ...
        let native=unsafe{safemlx_sys::mlx_distributed_group_persistent_storage_controls(self.native.c_group)};
        let controls=[size_of::<GroupPersistentStorage<'_>>(),size_of::<&Self>(),
            size_of::<Result<GroupPersistentStorage<'_>,GroupStorageUnavailable>>(),
            size_of::<Layout>()*3,size_of::<Result<(Layout,usize),std::alloc::LayoutError>>(),size_of::<usize>()*4];
...
impl GroupPersistentStorage<'_>{
    ...
    pub fn kind(&self)->GroupStorageKind{match self.value.kind{1=>GroupStorageKind::Empty,2=>GroupStorageKind::Ring,_=>GroupStorageKind::Unknown}}
    /// Whether an actual retained domain still lacks a complete source.
    pub fn is_unqualified(&self,domain:GroupStorageDomain)->bool{self.value.unresolved&domain as u32!=0}
    ...
    pub fn retained_cpp_bytes(&self)->usize{self.value.retained_cpp_bytes}
    ...
    pub fn retained_owner_bytes(&self)->Option<usize>{
        let (layout,_)=Layout::new::<[usize;2]>().extend(Layout::new::<NativeGroup>()).ok()?;
        self.value.retained_cpp_bytes.checked_add(layout.pad_to_align().size())
    }
    /// Native shared allocation including the implementation, not an extra charge.
    pub fn shared_owner_bytes(&self)->usize{self.value.shared_owner_bytes}
    /// Actual immutable socket node and bucket allocator requests.
    pub fn socket_map_bytes(&self)->(usize,usize,usize){(self.value.socket_nodes,self.value.socket_node_bytes,self.value.socket_bucket_bytes)}
    ...
    pub fn thread_cpp_bytes(&self)->(usize,usize){(self.value.thread_count,self.value.thread_cpp_runtime_bytes)}
    /// Actual retained scratch, socket-vector and thread-vector capacities.
    pub fn vector_bytes(&self)->(usize,usize,usize){(self.value.communication_buffer_bytes,self.value.socket_vector_bytes,self.value.pool_vector_bytes)}
    /// Exact borrowed wrapper identity.
    pub fn is_for(&self,group:&Group)->bool{std::ptr::eq(self.group,group)}
    ...
    pub fn control_bytes(&self)->Option<usize>{self.value.controls.checked_add(size_of::<Self>())}
}
...
mod tests{
    ...
    fn persistent_source_counts_actual_shared_owner_and_preserves_native_identity(){
        ...
        assert!(source.is_for(&first));assert!(!source.is_for(&second));
...
impl fmt::Debug for RetainedGroupBuffer {
    fn fmt(&self,f:&mut fmt::Formatter<'_>)->fmt::Result {
        f.debug_struct("RetainedGroupBuffer").field("identity",&self.identity)
            .field("bytes",&self.bytes).finish_non_exhaustive()
    }
}
impl RetainedGroupBuffer {
    /// Exact fixed allocation key minted by the native implementation itself.
    pub fn identity(&self)->GroupBufferIdentity{self.identity}
    ...
    pub fn is_for(&self,group:&Group)->bool {
        ...
        unsafe{safemlx_sys::mlx_distributed_group_same_implementation(self.group.native.c_group,group.native.c_group)}
    }
    /// Alias construction has no native query, allocation, or runtime entry.
    pub const fn clone_control_bytes()->usize {
        size_of::<Self>()+size_of::<Group>()+size_of::<GroupBufferIdentity>()+size_of::<&Self>()
    }
}
impl GroupPersistentStorage<'_> {
    ...
    pub fn retain_buffer(&self)->Result<RetainedGroupBuffer,GroupStorageUnavailable>{
        if self.has_unqualified_storage(){return Err(GroupStorageUnavailable);}
        let identity=std::num::NonZeroU64::new(self.value.storage_identity).ok_or(GroupStorageUnavailable)?;
        Ok(RetainedGroupBuffer{group:self.group.clone(),identity:GroupBufferIdentity(identity),
