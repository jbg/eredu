...
use crate::{
    Array, OperationEvalTraversalLayout, OperationEvalTraversalLimits, OperationEvent,
...
impl<'a> GroupCpuOperationStorage<'a> {
    ...
    pub fn completion_storage_control_bytes(&self) -> Option<usize> {
        ...
        source_query_control_bytes(
            group,
            self.constructor().matrix().is_some(),
            self.control_bytes()?,
        )
    }
    ...
    pub fn with_completion_storage(
        self,
        ...
        let available = if let Some((matrix, transposed)) = self.constructor().matrix() {
            ...
            unsafe {
                safemlx_sys::mlx_distributed_query_variable_completion_storage(
                    &mut native,
            ...
            unsafe {
                safemlx_sys::mlx_distributed_query_cpu_completion_storage(
                    &mut native,
                    group.native.c_group,
                    input.as_ptr(),
                    code,
...
impl GroupCpuCompletionStorage<'_> {
    ...
    pub fn operation(&self) -> &GroupCpuOperationStorage<'_> {
        &self.operation
    }
    ...
    pub fn control_bytes(&self) -> Option<usize> {
        completed_control_bytes(
            self.operation.completion_storage_control_bytes()?,
            self.traversal.query_control_bytes()?,
            self.operation.control_bytes()?,
        )
    }
    ...
    pub fn construct_original(
        ...
        OperationEvent::validate_traversal_leaf(
            self.operation.constructor().source_parts().1,
            observer,
        )?;
...
pub(super) fn source_query_control_bytes(
    group: &super::Group,
    variable: bool,
    operation_controls: usize,
) -> Option<usize> {
    // SAFETY: retained native group chooses only pure source/type facts.
    let native = if variable {
        unsafe {
            safemlx_sys::mlx_distributed_variable_completion_storage_controls(group.native.c_group)
        }
    } else {
        unsafe {
    ...
    let parts = [
        ...
        size_of::<(&GroupCpuOperationStorage<'_>, u32, i32)>(),
    ...
    parts.into_iter().try_fold(
        native
            ...
            .checked_add(operation_controls)?,
        usize::checked_add,
    )
}
pub(super) fn completed_control_bytes(
    query: usize,
    traversal: usize,
    operation: usize,
) -> Option<usize> {
    query
        ...
        .checked_add(size_of::<GroupCpuCompletionStorage<'_>>())
}
