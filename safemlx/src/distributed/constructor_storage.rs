...
use super::{Group, GroupStorageUnavailable, GroupWorkerOperation};
use crate::{
...
impl fmt::Debug for GroupConstructorStorage<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("GroupConstructorStorage")
            .field("operation", &self.operation)
            .field("primitives", &self.value.primitives)
            .field("blocks", &self.value.blocks)
...
impl Group {
    ...
    pub fn constructor_storage<'a>(
        ...
        if !unsafe {
            safemlx_sys::mlx_distributed_group_constructor_storage(
                &mut value,
                self.native.c_group,
                input.as_ptr(),
                code,
    ...
    pub fn constructor_storage_control_bytes() -> Option<usize> {
        ...
        let controls = [
            size_of::<GroupConstructorStorage<'_>>(),
...
impl<'a> GroupConstructorStorage<'a> {
    pub(super) fn from_variable(
        ...
        Self {
            group,
            input,
            operation: GroupWorkerOperation::Variable,
            value,
            matrix: Some((matrix, transposed)),
        }
    }
    pub(super) fn matrix(&self) -> Option<(&'a [usize], bool)> {
        self.matrix
    }
    /// Exact matrix-backed source identity, including orientation.
    pub fn is_for_variable(
        &self,
        group: &Group,
        input: &Array,
        matrix: &[usize],
        ...
        std::ptr::eq(self.group, group)
            ...
            && self.matrix.is_some_and(|(source, transpose)| {
                std::ptr::eq(source, matrix) && transpose == transposed
            })
    }

    pub(super) fn native(&self) -> &safemlx_sys::mlx_distributed_constructor_storage {
        &self.value
    }
    ...
    pub fn graph_extent(&self) -> usize {
        self.value.allocation_extents
    }
    ...
    pub fn classes(&self) -> impl ExactSizeIterator<Item = (usize, usize, usize)> + '_ {
        self.value
            ...
            .iter()
            .copied()
            .zip(self.value.request_alignments.iter().copied())
            .zip(self.value.request_counts.iter().copied())
            .map(|((bytes, align), count)| (bytes, align, count))
    }
    ...
    pub fn construction_control_bytes(&self) -> Option<usize> {
        construction_control_bytes(self.value.named_control_bytes)
    }
    ...
    pub fn construct_original(
        ...
        let status = if let Some((matrix, transposed)) = self.matrix {
            ...
            unsafe {
                safemlx_sys::mlx_distributed_construct_variable_original(
                    result.as_mut_raw_ptr(),
                    observer.raw,
            ...
            unsafe {
                safemlx_sys::mlx_distributed_construct_original(
                    result.as_mut_raw_ptr(),
                    observer.raw,
                    self.group.native.c_group,
                    self.input.as_ptr(),
...
mod tests {
    ...
    fn actual_singleton_constructor_is_only_a_source_bound_handle() {
        ...
        for operation in [
            ...
            GroupWorkerOperation::Maximum,
            GroupWorkerOperation::Minimum,
            GroupWorkerOperation::Gather,
        ] {
        ...
        assert!(matches!(
            group.constructor_storage(&input, GroupWorkerOperation::Send { peer: 0 }),
            Err(GroupStorageUnavailable)
        ));
        assert!(matches!(
            group.constructor_storage(&input, GroupWorkerOperation::Receive { peer: 0 }),
            Err(GroupStorageUnavailable)
        ));
    }
}

pub(super) fn construction_control_bytes(native: usize) -> Option<usize> {
    let controls = [
        size_of::<GroupConstructorStorage<'_>>(),
        ...
        size_of::<(&OriginalScopeObserver, &Stream)>(),
