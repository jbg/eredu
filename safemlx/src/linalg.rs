...
pub fn norm_matrix<'a>(
    ...
    match axes.into_option() {
        Some(axes) => Array::try_from_op(|res| unsafe {
            safemlx_sys::mlx_linalg_norm_matrix(
                res,
                array.as_ref().as_ptr(),
                ord.as_ptr(),
                axes.as_ptr(),
...
pub fn norm_l2<'a>(
    ...
    match axes.into_option() {
        Some(axis) => Array::try_from_op(|res| unsafe {
            safemlx_sys::mlx_linalg_norm_l2(
                res,
                array.as_ref().as_ptr(),
                axis.as_ptr(),
                axis.len(),
...
pub fn svd(
    ...
    let v = VectorArray::try_from_op(|res| unsafe {
        safemlx_sys::mlx_linalg_svd(res, array.as_ref().as_ptr(), true, stream.as_ref().as_ptr())
    })?;
...
pub fn cholesky(
    ...
    Array::try_from_op(|res| unsafe {
        safemlx_sys::mlx_linalg_cholesky(res, a.as_ref().as_ptr(), upper, stream.as_ref().as_ptr())
    })
}
...
pub fn cholesky_inv(
    ...
    let upper = upper.unwrap_or(false);
    Array::try_from_op(|res| unsafe {
        safemlx_sys::mlx_linalg_cholesky_inv(
            res,
            a.as_ref().as_ptr(),
            upper,
            stream.as_ref().as_ptr(),
...
pub fn eigvalsh(
    ...
    let uplo = CString::new(uplo.unwrap_or("L")).map_err(|e| Exception::custom(format!("{e}")))?;
    Array::try_from_op(|res| unsafe {
        safemlx_sys::mlx_linalg_eigvalsh(res, a.as_ptr(), uplo.as_ptr(), stream.as_ref().as_ptr())
    })
}
...
pub fn tri_inv(
    ...
    let upper = upper.unwrap_or(false);
    Array::try_from_op(|res| unsafe {
        safemlx_sys::mlx_linalg_tri_inv(res, a.as_ref().as_ptr(), upper, stream.as_ref().as_ptr())
    })
}
...
/// let p = safemlx::ops::put_along_axis(
///     safemlx::ops::zeros_like(&l),
...
pub fn lu(
    ...
    let v = Vec::<Array>::try_from_op(|res| unsafe {
        safemlx_sys::mlx_linalg_lu(res, a.as_ref().as_ptr(), stream.as_ref().as_ptr())
    })?;
...
pub fn lu_factor(
    ...
    <(Array, Array)>::try_from_op(|(res_0, res_1)| unsafe {
        safemlx_sys::mlx_linalg_lu_factor(
            res_0,
            res_1,
            a.as_ref().as_ptr(),
            stream.as_ref().as_ptr(),
...
pub fn solve_triangular(
    ...
    Array::try_from_op(|res| unsafe {
        safemlx_sys::mlx_linalg_solve_triangular(
            res,
            a.as_ref().as_ptr(),
            b.as_ref().as_ptr(),
            upper,
