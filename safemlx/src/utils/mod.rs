...
pub(crate) struct VectorArray {
    c_vec: safemlx_sys::mlx_vector_array,
}

impl VectorArray {
    pub(crate) fn as_ptr(&self) -> safemlx_sys::mlx_vector_array {
        self.c_vec
    }
    ...
    pub(crate) fn try_from_iter(
        ...
        VectorArray::try_from_op(|res| unsafe {
            ...
            for arr in iter {
                status = safemlx_sys::mlx_vector_array_append_value(*res, arr.as_ref().as_ptr());
                if status != SUCCESS {
                    return status;
                }
            }
    ...
    {
        unsafe {
            let size = safemlx_sys::mlx_vector_array_size(self.c_vec);
            (0..size)
                .map(|i| {
                    Array::try_from_op(|res| safemlx_sys::mlx_vector_array_get(res, self.c_vec, i))
                })
...
impl Drop for VectorArray {
    fn drop(&mut self) {
        let status = unsafe { safemlx_sys::mlx_vector_array_free(self.c_vec) };
...
pub(crate) struct Closure<'a> {
    c_closure: safemlx_sys::mlx_closure,
    lt_marker: PhantomData<&'a ()>,
}

impl<'a> Closure<'a> {
    pub(crate) fn as_ptr(&self) -> safemlx_sys::mlx_closure {
        self.c_closure
    }
...
impl Drop for Closure<'_> {
    fn drop(&mut self) {
        let status = unsafe { safemlx_sys::mlx_closure_free(self.c_closure) };
        debug_assert_eq!(status, SUCCESS);
    }
}

/// Helper method to create a mlx_closure from a Rust closure.
fn new_mlx_closure<'a, F>(closure: F) -> safemlx_sys::mlx_closure
where
    F: FnMut(&[Array]) -> Vec<Array> + 'a,
{
    ...
    unsafe {
        safemlx_sys::mlx_closure_new_func_payload(
            Some(trampoline::<F>),
            payload,
            Some(closure_dtor::<F>),
        )
...
fn new_mlx_fallible_closure<'a, F>(closure: F) -> safemlx_sys::mlx_closure
where
    F: FnMut(&[Array]) -> Result<Vec<Array>, Exception> + 'a,
{
    let boxed = Box::new(closure);
...
fn mlx_vector_array_values(
    vector_array: safemlx_sys::mlx_vector_array,
) -> Result<Vec<Array>, Exception> {
    unsafe {
        let size = safemlx_sys::mlx_vector_array_size(vector_array);
        (0..size)
            .map(|index| {
                Array::try_from_op(|res| {
                    safemlx_sys::mlx_vector_array_get(res, vector_array, index)
                })
            })
            .collect()
    }
