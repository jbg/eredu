...
use eredu_core::{
    ...
    speculative::{
        SpeculativeActivationOrigin, SpeculativePrefillSpan, SpeculativeRequestGeometry,
    },
...
impl ExternalInvocationKind {
    ...
    pub const ALL: [Self; 8] = [
        ...
        Self::TargetTokenEmbeddings,
        Self::TargetProjectLogits,
        Self::AssistantStep,
...
impl ExternalInvocation {
    ...
    pub const fn kind(self) -> ExternalInvocationKind {
        self.kind
    }
    ...
    pub const fn origin(self) -> SpeculativeActivationOrigin {
        self.origin
    }
}
...
impl<'a> ExternalSchedulePlan<'a> {
    ...
    ) -> Result<Self, ExternalOccurrenceError> {
        config.validate()?;
    ...
    pub const fn selected(&self) -> &'a SelectedSpeculativeRealization {
        self.selected
    }
    /// Distinct issuance identity, even for equal schedules.
    pub const fn identity(&self) -> ExternalScheduleIdentity {
        self.identity
    }
    /// Actual declared strategy iteration.
    pub const fn shape(&self) -> ExternalPredictionShape {
        self.shape
    }
    ...
    /// Exact complete ingress geometry scheduled before individual role claims.
    ...
    pub const fn attempts(&self, kind: ExternalInvocationKind) -> usize {
        self.limits[kind as usize]
    }
    /// Checked population of the request's fixed role destination.
    pub fn total_attempts(&self) -> Result<usize, ExternalOccurrenceError> {
        total(self.limits)
    }
    ...
    pub fn invocation(
        ...
        if self.attempts(kind) == 0
            || geometry.batch_size != self.prefill.geometry().batch_size
        ...
        if kind == ExternalInvocationKind::TargetPrefill {
            ...
            let start = geometry
                .cached_positions
                .checked_sub(self.prefill.geometry().cached_positions)
                .ok_or_else(fail)?;
            ...
            if let Some(span) = span {
                if span.prompt_tokens != self.prefill.geometry().input_positions
                    || span.input_start != chunk.input.start
                    || span.input_end != chunk.input.end
                    || span.position != chunk.position
                    || span.sequence != geometry.input_positions
                    || span.hidden_start != chunk.input.start
                    || span.token_start != chunk.input.start
                    || span.seed_start != chunk.position
                {
                    return Err(fail());
                }
            } else if self.prefill.span_count() != 1 {
                return Err(fail());
            }
        } else {
            if span.is_some() {
            ...
            if kind == ExternalInvocationKind::AssembleContext && origin.prediction == 0 {
                if self.shape != ExternalPredictionShape::Fused
                    || origin.committed_tokens != 0
                    || origin.optimistic
                    || geometry.cached_positions != 0
                    || width > self.prefill.geometry().prefill_chunk_positions
                {
                    return Err(fail());
                }
            } else {
                ...
                let frontier = self
                    ...
                    .checked_add(prefix)
                    .ok_or(ExternalOccurrenceError::Overflow)?;
                if frontier >= self.closing {
                    return Err(fail());
                }
                let block_width = maximum
                    .checked_add(1)
                    .ok_or(ExternalOccurrenceError::Overflow)?;
                let (stateful, valid_width) = match kind {
                    ...
                    ExternalInvocationKind::TargetTokenEmbeddings => (
                        false,
                        if self.shape == ExternalPredictionShape::Sequential {
                            width == 1
                        } else {
                            width >= 2 && width <= block_width
                        },
                    ),
                    ...
                    ExternalInvocationKind::FusedProposal => {
                        (true, width >= 2 && width <= block_width)
                    }
                    ExternalInvocationKind::TargetPrefill => unreachable!(),
                };
                if !valid_width
                    || geometry.cached_positions != if stateful { frontier } else { 0 }
                    || (stateful
                        && !frontier
                            .checked_add(width)
                            .is_some_and(|end| end <= self.closing))
                {
                    return Err(fail());
                }
            }
...
fn populations(
    ...
    maximum: usize,
    proposal_blocks: usize,
) -> Result<[usize; 8], ExternalOccurrenceError> {
    ...
    let values = match shape {
        ...
        ExternalPredictionShape::Fused => {
            ...
            [
                ...
                blocks,
                0,
                prefill.checked_add(rounds).ok_or_else(overflow)?,
                blocks,
                blocks,
            ]
        }
...
impl<'a> ExternalOccurrenceCursor<'a> {
    ...
    pub const fn plan(&self) -> &ExternalSchedulePlan<'a> {
        &self.plan
    }
    ...
    pub fn claim(
        ...
        let (ordinal, _) = super::attempts::consume(
            &mut self.spent,
            &mut self.attempted,
            index,
    ...
    pub fn continuation(
        ...
        let added = populations(
            self.plan.shape,
            0,
            rounds,
            self.plan.geometry.proposal_count(committed),
            self.plan.proposal_blocks,
        )?;
        ...
        Ok(ExternalContinuation {
            identity: self.plan.identity,
            previous: self.plan.limits,
            next,
            previous_slots: total(self.plan.limits)?,
            next_slots: total(next)?,
        })
    }
