...
fn independent_batch_cursors_keep_geometry_spending_and_continuation_per_request() {
    ...
    let proposal_attempts = first
        .domains()
        .iter()
        .find(|domain| domain.pass() == AutoregressivePass::Proposal)
        .unwrap()
        .attempts();
    ...
    let mut cursors =
        SpeculativeBuffer::try_new_retained(2, HostPreparationAuthority::retain(funding.clone()))
            .unwrap();
    cursors.try_push(RefCell::new(first.into_cursor())).unwrap();
    cursors
        .try_push(RefCell::new(second.into_cursor()))
        .unwrap();
    let owner = OccurrenceOwners::Table {
        cursors,
        _host: HostPreparationAuthority::unmanaged(),
    };
    let cursor = |index| {
        owner
            .select(Some(SpeculativeRequestId::new(index)))
            .unwrap()
            .unwrap()
    };
    ...
    for index in 0..2 {
        cursor(index).borrow_mut().begin_cache().unwrap();
    }
    assert!(cursor(0).borrow_mut().begin_cache().is_err());
    let call = |width| {
        AutoregressiveInvocation::prefill(AutoregressivePass::TargetPrefill, width).unwrap()
    };
    ...
    for _ in 0..proposal_attempts {
        cursor(0).borrow_mut().claim(2, proposal).unwrap();
    }
    assert!(cursor(0).borrow_mut().claim(2, proposal).is_err());
    let continuation = cursor(0)
        .borrow()
        .continuation(1, eredu_core::SpeculativeRequestStatus::ReadyToDraft)
        .unwrap();
