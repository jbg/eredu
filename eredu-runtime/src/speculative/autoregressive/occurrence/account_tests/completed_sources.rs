...
fn completed_mixed_sources_preserve_distinct_accounts_capacity_and_escaped_custody() {
    ...
    let first = WorkspaceExistingStorage::try_new_placed(
        Some(bytes),
        crate::working_memory::memory_fixture::host_placement(),
        &context,
    )
    .unwrap();
    let second = WorkspaceExistingStorage::try_new_placed(
        Some(bytes),
        crate::working_memory::memory_fixture::host_placement(),
        &context,
    )
    ...
    let oversized = WorkspaceExistingStorage::try_new_placed(
        Some(bytes + 1),
        crate::working_memory::memory_fixture::host_placement(),
        &context,
    )
    ...
    let altered_placement = eredu_core::MemoryPlacement::possible(
        pool.topology(),
        vec![pool.topology().host_domain()],
        "candidate-domain allowance is not fixed backing".into(),
    )
    .unwrap();
    let altered_root =
        WorkspaceExistingStorage::try_new_placed(Some(bytes), &altered_placement, &context)
            .unwrap();
    assert!(matches!(
        CompletedWorkspaceSourceLayout::new_accounts(1)
            .unwrap()
            .construct_accounts(&context, vec![(altered_root, model_tag.clone())], &host,),
        Err(WorkingMemoryError::IdentityMismatch)
    ));
    let escaped = source_layout
        ...
        .unwrap();
    assert_eq!(escaped.borrowed_storage().total_bytes(), Some(2 * bytes));
...
fn unselected_bindings_retain_all_sources_for_one_union_and_keep_selection_guard() {
    ...
    impl HostSlotStorageKey for Key {
        fn host_slot_identity(&self) -> Option<&crate::HostMetadataKey> {
            None
        }
    }
    let selected = selected();
    ...
    let host_bytes = model_layout.requested_bytes()
        + numerical_layout.requested_bytes()
        + registered_layout.requested_bytes()
        + mixed_layout.requested_bytes().unwrap()
        + completed_layout.requested_bytes().unwrap()
        + 2 * carrier_layout.requested_bytes()
        + WorkspaceBorrowedStorage::construction_bytes(4).unwrap()
        + 2 * size_of::<(WorkspaceExistingStorage, CompletedWorkspaceSourceAccount)>()
        + HostPreparationAuthority::retention_bytes::<OriginalSpeculativeBudgetCustody>().unwrap();
    let pool = crate::working_memory::memory_fixture::host_ledger(1 << 24, 0).unwrap();
    let registration = pool
        .register_host_storage([(Key(1), 16), (Key(2), 24)])
        .unwrap();
    let request = OriginalSpeculativeRequest::prepare(
        &pool,
        &InferenceExecutionIdentity::default(),
    ...
    let numerical = request
        .reserve_numerical(
            SpeculativeNumericalRequirements::new(
                SpeculativeNumericalProgram::new(SpeculativeNumericalKind::Normalize, &[1, 1])
                    .unwrap(),
                Some(bytes),
                Some(0),
                Some(0),
    ...
    .unwrap();
    let c = WorkspaceExistingStorage::try_new_placed(
        Some(bytes),
        crate::working_memory::memory_fixture::host_placement(),
        &context,
    )
    .unwrap();
    let d = WorkspaceExistingStorage::try_new_placed(
        Some(bytes),
        crate::working_memory::memory_fixture::host_placement(),
        &context,
    )
    .unwrap();
    let model_source = model_layout
        .construct_accounts(&context, vec![(c.clone(), model.clone().into())], &host)
        .unwrap();
    let numerical_source = numerical_layout
        .construct_accounts(
            &context,
            vec![(
                d,
                CompletedWorkspaceSourceAccount::Numerical(numerical.clone()),
            )],
            &host,
        )
        .unwrap();
    ...
    let plain = registered_layout
        .construct_unselected(&pool, &context, [(Key(1), a.clone())])
        .unwrap();
    let mixed = mixed_layout
        .construct_unselected(
            &pool,
            &context,
            [(Key(2), b)],
            model_source,
        ...
        .unwrap();
    let completed = completed_layout
        .construct_unselected(
            &pool,
            &context,
            std::iter::empty::<(Key, WorkspaceExistingStorage)>(),
            numerical_source,
            &mut numerical_carrier,
        )
        .unwrap();
    let before = pool.payload_used_bytes().unwrap();
    assert!(matches!(
        RegisteredWorkspaceStorageLayout::<Key>::new(1)
            .unwrap()
            .construct_unselected(&pool, &context, [(Key(99), a)]),
        Err(WorkingMemoryError::IdentityMismatch)
    ));
    assert_eq!(
        pool.payload_used_bytes().unwrap(),
        before,
        "unselected binding never registers missing sources"
    );
    let union = WorkspaceBorrowedStorage::new_finite(
        &context,
        plain
            .borrowed_storage()
            .roots()
            .iter()
            .chain(mixed.borrowed_storage().roots())
            .chain(completed.borrowed_storage().roots()),
        4,
    )
    .unwrap();
    assert_eq!(union.roots().len(), 4);
    assert_eq!(union.total_bytes(), Some(40 + 2 * bytes));
    context.set_borrowed_storage_checked(union.clone()).unwrap();
    assert!(matches!(
        context.set_borrowed_storage_checked(union.clone()),
        Err(WorkspaceBorrowedStorageError::SelectionStarted)
    ));
    ...
    let late_root = WorkspaceExistingStorage::try_new_placed(
        Some(16),
        crate::working_memory::memory_fixture::host_placement(),
        &late,
    )
    .unwrap();
    late.begin_span();
    let late_binding = RegisteredWorkspaceStorageLayout::<Key>::new(1)
        .unwrap()
        .construct_unselected(&pool, &late, [(Key(1), late_root.clone())])
        .unwrap();
    assert!(matches!(
        late.set_borrowed_storage_checked(late_binding.borrowed_storage().clone()),
        Err(WorkspaceBorrowedStorageError::SelectionStarted)
    ));
    assert!(matches!(
        RegisteredWorkspaceStorageLayout::<Key>::new(1)
            .unwrap()
            .construct(&pool, &late, [(Key(1), late_root)]),
        Err(WorkingMemoryError::IdentityMismatch)
    ));
    request.close().unwrap();
    drop((
    ...
    assert!(
        pool.payload_used_bytes().unwrap() > 40,
        "the bindings retain actual model/numerical and registered charges"
    );
