...
fn prepared_invocation_readmission_matches_ordinary_current_usage_and_evidence() {
    let pool = crate::working_memory::memory_fixture::host_ledger(1 << 25, 0).unwrap();
    let retired = Arc::new(AtomicBool::new(false));
    let mut scratch = StaticInterventionPreflight::prepare(HostPreparationAuthority::retain(
        Retires(retired.clone()),
    ))
    ...
    for evidence in [
        ...
        raw_edit.operations[0].slices.insert(
            0,
            CaptureSlice {
                axis: "sequence".into(),
                start: 1,
        ...
        let edit = raw_edit
            ...
            .unwrap();
        let source = pool
            .compile_intervention_source(PreparedInterventionPlanCopy::inspect(&edit).unwrap())
            .unwrap();
        let capabilities = CaptureCapabilities {
            transformations: vec![CaptureTransformKind::Preview],
            ..Default::default()
        ...
        for policy in [CaptureLimitPolicy::Fail, CaptureLimitPolicy::Skip] {
            for limit in [0, 127, 255, 511, 1024, 2048] {
                ...
                for inherited in [
                    ...
                    let expected =
                        preflight_continuation(&capture, &edit, &Estimates, 0, inherited);
                    let actual = scratch.run_invocation_with_source(
                        &capture,
                        source.plan(),
                        &Estimates,
                        inherited,
                    ...
                    match (expected, actual) {
                        ...
                        (left, right) => {
                            panic!("invocation continuation differs: {left:?} / {right:?}")
                        }
                    }
                }
            }
        ...
        assert!(matches!(
            scratch.run_invocation_with_source(
                &ordinary,
                source.plan(),
                &Estimates,
                CaptureUsage::default()
