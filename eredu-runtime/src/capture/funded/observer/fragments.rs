...
impl<T, E: std::error::Error + Send + Sync + 'static, N> FundedCaptureObserver<'_, T, E, N> {
            ...
            .len()
        {
            ...
            let result = (|| {
                let projected=match self.backend.partition_capture() {
                    Some(program)=>program.take_prefill_projection(index,frame,
                ...
                if let Some(hook)=projected {
                    ...
                    let hook=hook.observe(self.backend,value,bound.geometry(),k)?;
                    ...
                    if let Some(plan)=row.transform_plan() {
                        ...
                        let fragment=assembly.fragment(k).map_err(|error|progress_error(error.into()))?;
                ...
                if row.candidate().is_some() {
                    ...
                    let skipped=match self.backend.partition_capture() {
                        Some(partition)=>frame.reserve_prefill_hook(index,partition.reservation(index,&actual,usage)?,actual.clone(),usage)?,
                        None=>frame.reserve_prefill_hook(index,&mut self.session.ledger,actual.clone(),usage)?,
                    };
                    if skipped.is_some() {
                        return Ok(());
                    }
                    ...
                    .filter(|plan| matches!(plan.selection().transform, CaptureTransform::Summary))
                {
                    ...
                    if decision == CapturePrefillHookDecision::First {
                        let usage = self.backend.estimate_prefill_summary(plan)?;
                        let skipped = match self.backend.partition_capture() {
                            Some(partition) => frame.reserve_prefill_hook(index,
                                partition.reservation(index, &actual, usage)?, actual, usage)?,
                            None => frame.reserve_prefill_hook(index, &mut self.session.ledger, actual, usage)?,
                        };
                        if skipped.is_some() {
                            return Ok(());
                        }
                    ...
                    let receipt = if fragment.selected_elements() == 0 {
                        ...
                        self.backend.transform_summary(value, claim)?
                    };
                ...
                if let Some(plan) = row.transform_plan().filter(|plan| {
                    ...
                    dtype = Some(actual.clone());
                    if decision == CapturePrefillHookDecision::First {
                        let usage = self.backend.estimate_prefill_histogram(plan)?;
                        let skipped = match self.backend.partition_capture() {
                            Some(partition) => frame.reserve_prefill_hook(index,
                                partition.reservation(index, &actual, usage)?, actual, usage)?,
                            None => frame.reserve_prefill_hook(index, &mut self.session.ledger, actual, usage)?,
                    ...
                    let receipt = if fragment.selected_elements() == 0 {
                        ...
                        self.backend.transform_histogram(value, claim)?
                    };
                ...
                let actual = self.backend.validate_prefill_source(value, &fragment)?;
                if !matches!(
                    actual,
                ...
                dtype = Some(actual.clone());
                if decision == CapturePrefillHookDecision::First {
                    let usage = self.backend.estimate_prefill(assembly.logical_geometry())?;
                    let skipped = match self.backend.partition_capture() {
                        Some(partition) => frame.reserve_prefill_hook(index,
                            partition.reservation(index, &actual, usage)?, actual, usage)?,
                        None => frame.reserve_prefill_hook(index, &mut self.session.ledger, actual, usage)?,
                    };
                    if skipped.is_some() { return Ok(()); }
                }
    ...
    pub(super) fn complete_fragment_chunk(
        ...
        if let Some(plan)=frame.intervention_admission() {
            let window=crate::intervention::InterventionPrefillWindow::new(plan,bound.geometry(),&chunk)
                .map_err(|_|CaptureProtocolError::PrefillAttribution)?;
            let partition=match self.backend.partition_capture() {
                Some(program)=>program.validate_intervention_prefill_end(frame,window)?,None=>false,
            };
            if !partition {frame.validate_prefill_intervention_end(window)?;}
        }
...
pub(in crate::capture::funded) fn projection_control_bytes()->usize {
    ...
    std::mem::size_of::<(Option<PartitionCaptureLocalHook>,
        Result<Option<PartitionCaptureLocalHook>,PartitionCaptureProgramError>,
        PartitionCaptureLocalHook,Result<PartitionCaptureLocalHook,PartitionCaptureProgramError>,
        Result<(),PartitionCaptureProgramError>,u64,bool)>()
}
