    ...
    pub(super) fn intervene_value(
            ...
            plan.plan().operations.iter().zip(plan.points()).enumerate()
        {
            ...
            let window=match &prefill {
                Some((geometry,chunk)) if crate::intervention::InterventionPrefillWindow::row_axis(point)=>
                    Some(crate::intervention::InterventionPrefillWindow::new(plan,*geometry,chunk)
                        .map_err(|_|CaptureProtocolError::PrefillAttribution)?),
                _=>None,
            };
            let member=match self.backend.partition_capture() {
                Some(program)=>program.intervention_member(index,window)?,None=>None,
            };
            if let Some(member)=member {
                ...
                let result=if let Some(window)=window {
                    ...
                    let result: Result<_, FundedCaptureError<E>>=(|| {
                        ...
                        let loan=partition::prepare(self.backend,input,fragment.claim(),Some(window))?;
                        let local=loan.reserved();
                        let accounting=fragment.charge(local).map_err(Into::into);
                        let output=partition::execute(self.backend,input,fragment.claim(),loan,accounting,frame,
                            prefill.as_ref().map(|(geometry,chunk)|(*geometry,chunk)))?;
                        fragment.finish()?;Ok(output)
                    })();
                    match result {
                        Ok(output)=>{frame.retain_prefill_intervention(cursor)?;Ok(output)},
                        Err(cause)=>{frame.record_intervention_failure(index,"partition prefill intervention failed",cursor.charged())?;Err(cause)},
                    }
                } else {
                    ...
                    let result: Result<_, FundedCaptureError<E>>=(|| {
                        ...
                        let output=partition::execute(self.backend,input,&claim,loan,Ok(()),frame,
                            prefill.as_ref().map(|(geometry,chunk)|(*geometry,chunk)))?;
                        Ok((output,claim.finish(charged)?))
                    })();
                    match result {
                        Ok((output,receipt))=>{frame.record_intervention(receipt)?;Ok(output)},
                        Err(cause)=>{frame.record_intervention_failure(index,"partition intervention failed",charged)?;Err(cause)},
                    }
                };
                self.session.capture_seconds+=started.elapsed().as_secs_f64();
                if let Some(output)=result? {effective=Some(output);}
                continue;
            }
            if let Some((geometry,chunk))=&prefill {
                if crate::intervention::InterventionPrefillWindow::row_axis(point) {
                    let window=crate::intervention::InterventionPrefillWindow::new(plan,*geometry,chunk)
                        .map_err(|_|CaptureProtocolError::PrefillAttribution)?;
                    let started=std::time::Instant::now();
                    let mut cursor=frame.take_prefill_intervention(index,window)?;
                    let result=(||{
                        let input=effective.as_ref().unwrap_or(value);
                        let mut fragment=cursor.begin(window)?;
                        let projection=self.backend.prefill_intervention_projection_usage(input,fragment.claim(),window)?;
                        for usage in projection {
                            if usage.captures!=0 || usage.retained_bytes!=0 {return Err(CaptureProtocolError::Transaction.into());}
                            crate::intervention::reserve_envelope(&mut self.session.ledger,usage)?;
                            fragment.charge(usage)?;
                        }
                        let usage=self.backend.prefill_intervention_usage(input,fragment.claim(),window)?;
                        if usage.captures!=0 || usage.encoded_bytes!=0 {return Err(CaptureProtocolError::Transaction.into());}
                        crate::intervention::reserve_envelope(&mut self.session.ledger,usage)?;
                        fragment.charge(usage)?;
                        self.backend.apply_prefill_intervention(input,fragment,usage,projection)
                    })();
                    self.session.capture_seconds+=started.elapsed().as_secs_f64();
                    match result {
                        Ok(output)=>{
                        ...
                        Err(cause)=>{
                            frame.record_intervention_failure(index,"admitted prefill intervention failed",cursor.charged())?;
                            return Err(cause);
                        }
                    }
                    continue;
            ...
            let result = (|| {
                ...
                let (output, receipt) = self.backend.apply_intervention_projected(input, claim, usage, projection)?;
                evidence::observe(
                    self.backend,
                    frame,
                    &mut self.session.ledger,
...
pub(in crate::capture::funded) fn partition_control_bytes()->Option<usize> {partition::control_bytes()}
