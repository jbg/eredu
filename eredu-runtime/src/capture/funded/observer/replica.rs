...
impl<T, E: std::error::Error + Send + Sync + 'static, N> FundedCaptureObserver<'_, T, E, N> {
    pub(super) fn complete_receiver_sources(
        &mut self,
        path: &str,
        value: &T,
        ...
        for index in 0..self.session.plan.plan().selections.len() {
            ...
            if selection.path != path
                || !selection
                    .schedule
                    .includes(self.session.phase, self.session.prediction)
            {
                continue;
            }
            ...
            if let Some(bound) = self.bound {
                ...
                if let Some(source) = projected {
                    let actual = self
                        .backend
                        .validate_partition_prefill_source(value, &source)?;
            ...
            if self.bound.is_none() {
                ...
                if let Some(source) = source {
                    let actual = self
                        .backend
                        .validate_partition_invocation_source(value, &source)?;
                    if &actual != source.dtype() {
                        return Err(CaptureProtocolError::Geometry.into());
                    }
                    let started = std::time::Instant::now();
            ...
            let actual = if let Some(bound) = self.bound {
                    ...
                    .filter(|plan| matches!(plan.selection().transform, CaptureTransform::Summary))
                {
                ...
                } else if let Some(plan) = row.transform_plan().filter(|plan| {
                    matches!(
                    ...
                    let fragment = assembly.fragment(k).map_err(|cause| {
                        fragments::progress_error(
                            crate::capture::CapturePrefillProgressError::from(cause),
                        )
                    })?;
...
pub(in crate::capture::funded) fn control_bytes() -> usize {
    std::mem::size_of::<(
        ...
        Result<
            ...
            crate::capture::partition::PartitionCaptureProgramError,
        >,
        crate::capture::partition::PartitionPrefillReceiverSource,
        u64,
        crate::capture::CapturePrefillObservationPolicy<'static>,
        crate::capture::CapturePrefillObservationRow<'static>,
        CaptureObservationStep<'static>,
        CaptureSummaryGeometry<'static>,
        CaptureHistogramGeometry<'static>,
        CaptureTensorGeometry<'static>,
        CaptureCandidateGeometry<'static>,
        CaptureTokenScoreGeometry<'static>,
