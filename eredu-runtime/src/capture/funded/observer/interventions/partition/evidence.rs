...
use crate::working_memory::{InterventionEvidenceFrame, ScheduledCaptureStep};
...
fn child<'a, T, E: std::error::Error + Send + Sync + 'static>(
    ...
    backend
        ...
        .intervention_evidence(operation)?
        .ok_or_else(|| CaptureProtocolError::Transaction.into())
}
pub(super) fn validate_before<T, E: std::error::Error + Send + Sync + 'static>(
    ...
    backend.validate_partition_intervention_evidence_source(
        value,
        claim,
...
pub(super) fn observe<T, E: std::error::Error + Send + Sync + 'static>(
    ...
    value: &T,
    window: Option<InterventionPrefillWindow>,
    prefill: Option<(InferenceGeometry, &crate::prefill::PrefillChunk)>,
) -> Result<(), FundedCaptureError<E>> {
    if !requested(claim) {
        return Ok(());
    }
    backend.validate_partition_intervention_evidence_source(value, claim, side, window)?;
    let mut loan = frame.take_intervention_evidence_frame(claim.index())?;
    let result = observe_local(
        backend,
        &mut loan,
        claim.index(),
        side,
    ...
    if result.is_err() && window.is_some() {
        loan.frame_mut().fail_prefill_hook(side.index());
    }
...
fn observe_local<T, E: std::error::Error + Send + Sync + 'static>(
    backend: &mut dyn ScheduledCaptureBackend<Tensor = T, Error = E>,
    loan: &mut InterventionEvidenceFrame<'_>,
    operation: usize,
    side: InterventionEvidenceSide,
    value: &T,
    window: Option<InterventionPrefillWindow>,
    ...
    if let Some(window) = window {
        ...
        if window.inference() != inference
            || window.range() != [chunk.input.start, chunk.input.end]
        ...
        let hook = child(backend, operation)?
        ...
        if let Some(plan) = row.transform_plan() {
            ...
            match plan.selection().transform {
                CaptureTransform::Summary => loan
                    .frame_mut()
                    .finish_summary_prefill_hook(index, &fragment)?,
    ...
    let hook = child(backend, operation)?
        .take_invocation_projection(index)?
        .ok_or(CaptureProtocolError::Transaction)?;
...
pub(super) fn control_bytes() -> Option<usize> {
    use crate::capture::partition::{
        PartitionCaptureLocalHook, PartitionCaptureProgramError, PartitionInvocationReceiverSource,
        PartitionPrefillReceiverSource,
    };
    use std::mem::{size_of, size_of_val};
    let frames = [
        size_of::<InterventionEvidenceFrame<'_>>() * 2,
        size_of::<Result<InterventionEvidenceFrame<'_>, CaptureRunHostError>>(),
        size_of::<Result<(), CaptureRunHostError>>(),
        size_of::<CapturePrefillObservationPolicy<'_>>(),
        ...
        size_of::<PartitionInvocationReceiverSource>(),
        size_of::<TensorDtype>(),
        size_of::<(
            ...
            Option<(InferenceGeometry, &crate::prefill::PrefillChunk)>,
        )>() * 2,
    ];
    frames
        .into_iter()
