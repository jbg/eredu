...
pub(super) fn prepare<T, E: std::error::Error + Send + Sync + 'static>(
    ...
    let valid = backend
        .validate_partition_intervention_source(value, claim, window)
        .and_then(|()| evidence::validate_before(backend, value, claim, window));
    let vote = backend
        .partition_capture()
        .ok_or_else(|| FundedCaptureError::from(CaptureProtocolError::Transaction))
        .and_then(|program| {
            program
                .begin_intervention(claim.index(), window, valid.is_ok())
                .map_err(Into::into)
        });
...
pub(super) fn execute<T, E: std::error::Error + Send + Sync + 'static>(
    ...
    value: &T,
    claim: &CaptureInterventionClaim<'_>,
    mut loan: PartitionInterventionLocalAllowance,
    ...
    let vote = backend
        ...
        .and_then(|program| {
            program
                .finish_intervention(claim.index(), loan, result.is_ok())
                .map_err(Into::into)
        });
...
pub(super) fn control_bytes() -> Option<usize> {
    use crate::capture::partition::{
        PartitionCaptureProgramError, PartitionInterventionSourceError,
    };
    use crate::working_memory::{
        ClaimedIntervention, InterventionPrefillCursor, InterventionPrefillFragment,
    };
    ...
    frames
        ...
        .checked_add(evidence::control_bytes()?)
}
