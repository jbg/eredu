...
fn funded_routed_receipts_preserve_ordinary_permuted_ownership_identity_records_and_custody() {
    for empty in [false, true] {
        for exchange in [false, true] {
            ...
            let ordinary = PartitionCaptureReceiptPlan::new_routed(
                source.clone(),
                context(source.admission()),
                producers(source.admission(), exchange),
                8,
                limits(),
                &mut ordinary_ledger,
            )
            .unwrap();
            let mut declarations = producers(source.admission(), exchange);
            declarations.reverse();
            let rows: Vec<_> = declarations
                .iter()
                .map(|row| PartitionCaptureRoutedProducerSource {
                    rank: row.rank,
            ...
            let actual = PartitionCaptureReceiptPlan::new_routed_shared_funded(
                &source,
                &context(source.admission()),
                &rows,
                8,
            ...
            .unwrap();
            let spent = used.load(Ordering::SeqCst) - before;
            let quoted = actual
                .reconstruction_metadata_bytes(actual.context().run_identity.len())
                .unwrap();
            assert!(
                quoted >= spent,
                "retained constructor source {quoted} must cover actual {spent}"
            );
            assert_eq!(
                used.load(Ordering::SeqCst),
                before + spent,
                "inspection has no debit"
            );
            assert!(
                ordinary
                    .reconstruction_metadata_bytes(ordinary.context().run_identity.len())
                    .is_none()
            );
            assert!(
                actual
                    .reconstruction_metadata_bytes(actual.context().run_identity.len() - 1)
                    .is_none()
            );
            ...
            assert_eq!(
                serde_json::to_vec(&records(source.admission(), &actual)).unwrap(),
...
fn funded_routed_receipt_refusals_keep_exact_source_guards_and_spent_owner() {
    for case in ["duplicate", "missing", "peer", "fragments", "funding"] {
        ...
        let error = PartitionCaptureReceiptPlan::new_routed_shared_funded(
            &source,
            &context(source.admission()),
            &rows,
            8,
