...
pub use contiguous::{PreparedPartitionContiguousSource,PartitionCaptureLocalSource,PreparedPartitionRoutedSource,PartitionCaptureRoutedLocalSource};
...
enum Cause {
    ...
    #[error(transparent)] FragmentAllowance(#[from] PartitionCaptureFragmentAllowanceError),
    #[error(transparent)] Coordination(#[from] PartitionCaptureCoordinationError),
    ...
    #[error(transparent)] Delivery(#[from] PartitionCaptureTensorDeliveryError),
    #[error(transparent)] Host(#[from] crate::working_memory::CaptureRunHostError),
    #[error(transparent)] Capture(#[from] CaptureError),
    #[error(transparent)] Local(eredu_core::BackendFailure),
    #[error(transparent)] Intervention(#[from] PartitionInterventionSourceError),
    #[error(transparent)] Progress(#[from] crate::capture::CapturePrefillProgressError),
}
...
pub trait ScheduledPartitionCapture: sealed::Sealed {
    ...
    fn prepare(&mut self, source: &SharedCapturePlan, phase: CapturePhase, prediction: u64,
        epoch: DistributedCommitEpoch, frame: &mut ScheduledCaptureStep<'_>, ledger: &mut CaptureLedger)
        -> Result<(), PartitionCaptureProgramError>;
    /// Compare the prepared transcript after the shared local preparation vote.
    fn coordinate(&mut self, epoch: DistributedCommitEpoch, ledger: &CaptureLedger)
        -> Result<(), PartitionCaptureProgramError>;
    ...
    fn intervention_evidence(&mut self,index:usize)
    ...
    fn intervention_member(&self,_index:usize,_window:Option<crate::intervention::InterventionPrefillWindow>)
        ->Result<Option<bool>,PartitionCaptureProgramError> {Ok(None)}
    /// Issue only the prepaid local loan after the existing preflight member vote.
    fn begin_intervention(&mut self,index:usize,window:Option<crate::intervention::InterventionPrefillWindow>,valid:bool)
    ...
    fn finish_intervention(&mut self,index:usize,loan:PartitionInterventionLocalAllowance,success:bool)
        ->Result<(),PartitionCaptureProgramError>;
    /// Validate actual member cursors; absent ranks await the final world receipt.
    fn validate_intervention_prefill_end(&self,_frame:&ScheduledCaptureStep<'_>,
        _window:crate::intervention::InterventionPrefillWindow)->Result<bool,PartitionCaptureProgramError> {Ok(false)}
    ...
    fn take_receiver_source(&mut self, index: usize)
    ...
    fn take_projected_receiver_source(&mut self,_index:usize,
        _inference:eredu_core::InferenceGeometry,_chunk:u64)
        ->Result<Option<PartitionPrefillReceiverSource>,PartitionCaptureProgramError>{Ok(None)}
    /// The same original local receipt loan for a single decode invocation.
    fn take_invocation_projection(&mut self,_index:usize)
        ->Result<Option<PartitionCaptureLocalHook>,PartitionCaptureProgramError>{Ok(None)}
    /// Return the same original local owner before ordinary delivery resumes.
    fn return_invocation_projection(&mut self,index:usize,hook:PartitionCaptureLocalHook)
        ->Result<(),PartitionCaptureProgramError>{self.return_prefill_projection(index,hook)}
    /// Actual local decode source whose selected overlap produces no fragment.
    fn take_invocation_receiver_source(&mut self,_index:usize)
        ->Result<Option<super::PartitionInvocationReceiverSource>,PartitionCaptureProgramError>{Ok(None)}
    /// Exact runtime sparse program; presence grants no native capability.
    fn routed_source(&self,_index:usize)->Result<bool,PartitionCaptureProgramError>{Ok(false)}
    /// Suspend the existing receipt owners while the native provider is borrowed.
    fn take_routed_hooks(&mut self,_index:usize,_frame:&mut ScheduledCaptureStep<'_>,
        _prefill:Option<(&crate::prefill::PrefillChunk,eredu_core::InferenceGeometry)>)
        ->Result<Option<PartitionCaptureRoutedHooks>,PartitionCaptureProgramError>{Ok(None)}
    /// Return the same finished original hook table before transport resumes.
    fn return_routed_hooks(&mut self,hooks:PartitionCaptureRoutedHooks,_frame:&mut ScheduledCaptureStep<'_>,
        _prefill:Option<(&crate::prefill::PrefillChunk,eredu_core::InferenceGeometry)>)
        ->Result<(),PartitionCaptureProgramError>{Err(hooks.reject("routed hook returned to another program"))}
    /// Validate the actual dtype and estimate, then lend only this local quota.
    fn reservation(&mut self, index: usize, dtype: &TensorDtype, usage: CaptureUsage)
    ...
    fn take_prefill_projection(&mut self, _index: usize,
        _frame: &mut ScheduledCaptureStep<'_>, _first: bool)
        -> Result<Option<PartitionCaptureLocalHook>, PartitionCaptureProgramError> { Ok(None) }
    ...
    fn return_prefill_projection(&mut self, index: usize, hook: PartitionCaptureLocalHook)
        -> Result<(), PartitionCaptureProgramError>;
    ...
    fn remote_prefill(&mut self, frame: &mut ScheduledCaptureStep<'_>,
    ...
    fn complete_prefill_evidence(&mut self, frame: &mut ScheduledCaptureStep<'_>,
        bound: crate::layered::BoundCaptureSelection<'_>, chunk: &crate::prefill::PrefillChunk)
        -> Result<(), PartitionCaptureProgramError>;
    /// Fill receivers through the ordinary all-rank exchange before frame seal.
    fn deliver(&mut self, frame: &mut ScheduledCaptureStep<'_>)
        -> Result<(), PartitionCaptureProgramError>;
}
...
impl<T: PartitionCaptureTransport> std::fmt::Debug for PreparedPartitionCaptureProgram<'_, T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PreparedPartitionCaptureProgram").field("rows", &self.rows.len())
            .field("prepared", &self.first_epoch.is_some()).field("delivered", &self.delivered)
...
where T::Error: Send + Sync + 'static, <T::Completion as Completion>::Error: Send + Sync + 'static,
{
    ...
    fn new_rows<'a,I:ExactSizeIterator<Item=PreparedPartitionCaptureRow<'a>>>(transport:&'t T,
        ...
        if rows.len() != source.admission().plan().selections.len()
            ...
            || context.invocation.is_some() || transport.participant_count() == 0
            || transport.capture_rank() >= transport.participant_count()
            || limits.max_producers == 0 || limits.max_fragments == 0 || limits.max_record_bytes == 0
        { return Err(error(Cause::Source("selected rows, context or world differs"))); }
        let count=rows.len();
        let mut owned_rows = metadata.metadata_vec(count).map_err(|cause| error(cause.into()))?;
        let mut entries = metadata.metadata_vec(count).map_err(|cause| error(cause.into()))?;
        let mut skipped = metadata.metadata_vec(count).map_err(|cause| error(cause.into()))?;
        let mut receiver_epochs = metadata.metadata_vec(count).map_err(|cause| error(cause.into()))?;
        for (selection,row) in source.admission().plan().selections.iter().zip(rows) {
            let row=match row {
                PreparedPartitionCaptureRow::Inactive=>{
                    if selection.schedule.includes(context.phase,context.prediction){
                        return Err(error(Cause::Source("active capture row has no source declaration")));
                    }
                    None
                }
                PreparedPartitionCaptureRow::Complete(row)=>{
                    if row.producer>=transport.participant_count()
                        ||!matches!(row.dtype,None|Some(TensorDtype::F32|TensorDtype::F16|TensorDtype::Bf16))
                        ||row.estimate.generated_creation_bytes!=0
                        ||!matches!(selection.transform,CaptureTransform::FullTensor|CaptureTransform::Slice|CaptureTransform::Preview{..}
                            |CaptureTransform::Summary|CaptureTransform::Histogram{..}|CaptureTransform::TopCandidates{..}|CaptureTransform::TokenScores{..}) {
                        return Err(error(Cause::Source("selected row lacks a complete tensor source")));
                    }
                    Some(PartitionCaptureProducerSource{producer:row.producer,dtype:row.dtype.clone(),estimate:row.estimate})
                }
                PreparedPartitionCaptureRow::Routed=>{
                    if !matches!((context.phase,context.prediction),(CapturePhase::Prefill,0)|(CapturePhase::Decode,1..))
                        || !selection.schedule.includes(context.phase,context.prediction)
                        || !matches!(selection.transform,CaptureTransform::RoutedUnits) {
                        return Err(error(Cause::Source("selected row lacks a sparse invocation declaration")));
                    }
                    None
                }
                PreparedPartitionCaptureRow::Contiguous=>{
                    if !matches!((context.phase,context.prediction),(CapturePhase::Prefill,0)|(CapturePhase::Decode,1..))
                        ...
                        return Err(error(Cause::Source("selected row lacks a contiguous invocation declaration")));
                    }
                    None
                }
            };
            skipped.push(None);receiver_epochs.push(None);owned_rows.push(row);entries.push(None);
        }
        let context = receipt::copy_context(context, metadata).map_err(|cause| error(cause.into()))?;
        Ok(Self { transport, run_identity: None, context, limits, limit_policy:source.admission().plan().limits.on_limit, rows: owned_rows, entries, projected:Vec::new(),routed_hooks:None,interventions:None, skipped, receiver_epochs, coordination: None,
            first_epoch: None, last_epoch: None, coordination_complete: false, delivered: false, source: source.clone(), metadata: metadata.clone() })
    }
    ...
    fn charge(&self) -> Result<(), PartitionCaptureProgramError> {
        let bytes = size_of::<(&mut Self, usize, &TensorDtype, CaptureUsage)>()
            + size_of::<PartitionCaptureProgramError>() + size_of::<Cause>()
    ...
    fn allowed_skip(&self, reason: Option<CaptureSkipReason>) -> Option<CaptureSkipReason> {
        (self.limit_policy == CaptureLimitPolicy::Skip)
            .then_some(reason).flatten()
    }
}
impl<'t, T: PartitionCaptureTransport> ScheduledPartitionCapture for PreparedPartitionCaptureProgram<'t, T>
where T::Error: Send + Sync + 'static, <T::Completion as Completion>::Error: Send + Sync + 'static,
{
    fn prepare(&mut self, source: &SharedCapturePlan, phase: CapturePhase, prediction: u64,
        epoch: DistributedCommitEpoch, frame: &mut ScheduledCaptureStep<'_>, ledger: &mut CaptureLedger)
    ...
    {
        self.charge()?;
        if self.first_epoch.is_some() || !source.same_storage(&self.source)
            || phase != self.context.phase || prediction != self.context.prediction
            || frame.records().len() != self.rows.len()
        { return Err(self.error(Cause::Source("frame source, coordinate or preparation attempt differs"))); }
        self.first_epoch = Some(epoch);
        self.context.forward_epoch = epoch.value();
        if let Some(run) = &self.run_identity {
            self.context.run_identity = run.bind_epoch(epoch)
        ...
        for index in 0..self.rows.len() {
            ...
            if self.prepare_projected(index,epoch,frame,ledger)? {continue;}
            let row = self.rows[index].as_ref().ok_or_else(||self.error(Cause::Source("projected row has no original source owner")))?;
            self.metadata.reserve_metadata(size_of::<(
                &ScheduledCaptureStep<'_>, &CaptureSelection, usize,
                eredu_core::InferenceGeometry, u64, Option<usize>,
                Result<Option<usize>, crate::working_memory::CaptureRunHostError>,
            )>()).map_err(|cause|self.error(cause.into()))?;
            let terminal_rows=frame.partition_terminal_rows(index).map_err(|cause|self.error(cause.into()))?;
            let receipt = PartitionCaptureReceiptPlan::new_complete_shared_funded_terminal_global(
                &self.source, &self.context, row.producer, self.transport.participant_count(),
                self.limits, &self.metadata, ledger,terminal_rows);
            let (mut receipt, _preparation) = match receipt {
                ...
                Err(cause) => {
                    if let Some(reason) = self.allowed_skip(cause.limit_skip()) {
                        ...
                        frame.record_partition_skip(index, reason)
                            .map_err(|cause| self.error(cause.into()))?;
                        continue;
                    }
            ...
            let allowance = PreparedPartitionCaptureAllowance::prepare(self.transport, &mut receipt,
                row.estimate, &self.metadata, ledger);
            ...
            let charged = frame.records()[index].charged.checked_add(row.estimate.capture)
                .map_err(|cause| self.error(cause.into()))?;
            let evidence = PreparedPartitionCaptureEvidence::prepare(&receipt, charged,
                &self.metadata, allowance.quota_mut()).map_err(|cause| self.error(cause.into()))?;
            let exchange = PartitionCaptureExchange::admit(self.transport, receipt, allowance.quota_mut())
                .map_err(|cause| self.error(cause.into()))?;
            let delivery = PreparedPartitionTensorDelivery::prepare_with_source(exchange, row.dtype.clone(),
                charged, &self.metadata).map_err(|cause| self.error(cause.into()))?
                .with_evidence(evidence);
            self.entries[index] = Some(Entry { delivery: Some(delivery), allowance });
        }
        ...
        -> Result<(), PartitionCaptureProgramError>
    {
        ...
        self.charge()?;
        let first = self.first_epoch.ok_or_else(|| self.error(Cause::Source("observer was not prepared")))?;
        if epoch < first || self.last_epoch.is_some_and(|last| last >= epoch) || self.delivered
        ...
        if let Some(mut coordination) = self.coordination.take() {
            if epoch != first { return Err(self.error(Cause::Source("first preparation vote was omitted"))); }
            for index in 0..self.rows.len() {
                if !self.source.admission().plan().selections[index].schedule.includes(self.context.phase, self.context.prediction) { continue; }
                if self.coordinate_projected(index,&mut coordination,ledger)? {continue;}
                let row = self.rows[index].as_ref().ok_or_else(||self.error(Cause::Source("projected row has no original source owner")))?;
                if let Some(entry) = &self.entries[index] {
                    let delivery = entry.delivery.as_ref().ok_or_else(|| self.error(Cause::Source("capture delivery was consumed before source vote")))?;
                    let dtype = coordination.coordinate_source(index, Some(delivery.receipt_plan()), row.producer,
                        row.dtype.as_ref(), None, row.estimate.capture, ledger)
                        .map_err(|cause| self.error(cause.into()))?
                        .ok_or_else(|| self.error(Cause::Source("active producer scalar is absent")))?;
                    coordination.include(delivery.receipt_plan(), dtype.clone(), row.estimate.capture)
                        .map_err(|cause| self.error(cause.into()))?;
                    self.entries[index].as_mut().expect("checked active row").delivery.as_mut()
                        .expect("checked delivery").bind_source_dtype(dtype.clone())
                        .map_err(|cause| self.error(cause.into()))?;
                    self.rows[index].as_mut().expect("checked complete row").dtype = Some(dtype);
                } else {
                    let reason = self.skipped[index].as_ref().ok_or_else(|| self.error(Cause::Source("scheduled source has no receipt or skip")))?;
                    coordination.coordinate_source(index, None, row.producer, None, Some(reason), row.estimate.capture, ledger)
                        .map_err(|cause| self.error(cause.into()))?;
                    coordination.include_skip(index, reason).map_err(|cause| self.error(cause.into()))?;
                }
            }
            self.coordinate_interventions(&mut coordination)?;
            coordination.coordinate(ledger).map_err(|cause| self.error(cause.into()))?;
        } else if !previously_complete {
            return Err(self.error(Cause::Source("capture preparation did not establish a coordination vote")));
        }
    ...
    fn intervention_member(&self,index:usize,window:Option<crate::intervention::InterventionPrefillWindow>)
    ...
    fn validate_intervention_prefill_end(&self,frame:&ScheduledCaptureStep<'_>,
        window:crate::intervention::InterventionPrefillWindow)->Result<bool,PartitionCaptureProgramError> {
        PreparedPartitionCaptureProgram::validate_intervention_prefill_end(self,frame,window)
    }
    fn produces(&self, index: usize) -> Result<bool, PartitionCaptureProgramError> {
        self.charge()?;
        let row = self.rows.get(index).ok_or_else(|| self.error(Cause::Source("selection index differs")))?;
        if !self.coordination_complete || self.delivered {
            return Err(self.error(Cause::Source("hook preceded coordination or followed delivery")));
        }
        if let Some(projected)=self.projected.get(index).and_then(Option::as_ref) {
            return match projected {projected::Row::Ready(value)=>Ok(value.produces()),
                projected::Row::Skipped {..}=>Ok(false),
                _=>Err(self.error(Cause::Source("projected row has no ready source")))};
        }
        let row=row.as_ref().ok_or_else(||self.error(Cause::Source("projected row has no original source owner")))?;
    ...
    {
        self.charge()?;
        self.metadata.reserve_metadata(size_of::<(&mut Self, usize, DistributedCommitEpoch,
            Option<TensorDtype>, Result<Option<TensorDtype>, PartitionCaptureProgramError>)>())
            .map_err(|cause| self.error(cause.into()))?;
        let producer = self.produces(index)?;
        if producer || self.projected.get(index).is_some_and(Option::is_some)
            || self.entries[index].is_none() { return Ok(None); }
        let epoch = self.last_epoch.ok_or_else(|| self.error(Cause::Source("receiver hook has no coordinated epoch")))?;
        ...
        self.rows[index].as_ref().and_then(|row|row.dtype.clone()).map(Some)
            .ok_or_else(|| self.error(Cause::Source("receiver source has no agreed scalar witness")))
    }
    fn routed_source(&self,index:usize)->Result<bool,PartitionCaptureProgramError>{self.has_routed_source(index)}
    fn take_routed_hooks(&mut self,index:usize,frame:&mut ScheduledCaptureStep<'_>,
        prefill:Option<(&crate::prefill::PrefillChunk,eredu_core::InferenceGeometry)>)
        ->Result<Option<PartitionCaptureRoutedHooks>,PartitionCaptureProgramError>{self.lend_routed_hooks(index,frame,prefill).map(Some)}
    fn return_routed_hooks(&mut self,hooks:PartitionCaptureRoutedHooks,frame:&mut ScheduledCaptureStep<'_>,
        prefill:Option<(&crate::prefill::PrefillChunk,eredu_core::InferenceGeometry)>)
        ->Result<(),PartitionCaptureProgramError>{self.restore_routed_hooks(hooks,frame,prefill)}
    fn reservation(&mut self, index: usize, dtype: &TensorDtype, usage: CaptureUsage)
        -> Result<&mut CaptureQuota, PartitionCaptureProgramError>
    {
        if !self.produces(index)? { return Err(self.error(Cause::Source("nonproducer attempted a native transform"))); }
        if self.projected.get(index).is_some_and(Option::is_some) {
            return Err(self.error(Cause::Source("projected source requires its typed local hook loan")));
        }
        let row = self.rows[index].as_ref().ok_or_else(||self.error(Cause::Source("projected row has no original source owner")))?;
        if row.dtype.as_ref() != Some(dtype) || usage != row.estimate.capture {
            return Err(self.error(Cause::Source("actual tensor representation or usage differs")));
        }
        Ok(self.entries[index].as_mut().expect("checked prepared row").allowance.quota_mut())
    }
    fn take_invocation_projection(&mut self,index:usize)
        ->Result<Option<PartitionCaptureLocalHook>,PartitionCaptureProgramError>{self.take_projected_invocation(index)}
    fn take_invocation_receiver_source(&mut self,index:usize)
        ->Result<Option<super::PartitionInvocationReceiverSource>,PartitionCaptureProgramError>{self.take_projected_invocation_receiver(index)}
    fn take_prefill_projection(&mut self,index:usize,frame:&mut ScheduledCaptureStep<'_>,first:bool)
    ...
    fn take_projected_receiver_source(&mut self,index:usize,inference:eredu_core::InferenceGeometry,chunk:u64)
    ...
    {
        self.charge()?;
        if !bound.selection().source().same_storage(&self.source) || !self.coordination_complete || self.delivered {
            return Err(self.error(Cause::Source("remote progress differs from its bound frame")));
        }
        ...
        self.metadata.reserve_metadata(size_of::<(
            ...
            Option<&TensorDtype>, CaptureUsage,
        )>()).map_err(|cause| self.error(cause.into()))?;
        ...
        for index in 0..self.rows.len() {
            if self.progress_projected_receiver(index,frame,bound,chunk)? {continue;}
            let complete=self.rows[index].as_ref().ok_or_else(||error(Cause::Source("projected row has no original source owner")))?;
            if complete.producer == self.transport.capture_rank() { continue; }
            let Some(entry) = self.entries[index].as_mut() else { continue; };
            let path = &source.admission().plan().selections[index].path;
            let decision = frame.begin_prefill_hook(index, chunk, path).map_err(|cause| error(cause.into()))?;
            if decision == crate::capture::CapturePrefillHookDecision::Ignore { continue; }
            let row = policy.row(index).map_err(|cause| error(cause.into()))?;
            if row.terminal() {
                let dtype=complete.dtype.as_ref().ok_or_else(||error(Cause::Source("terminal row has no scalar source")))?;
                let charge=entry.allowance.take_remote_charge().map_err(|cause|error(cause.into()))?;
                frame.reserve_remote_prefill_hook(index,dtype.clone(),charge).map_err(|cause|error(cause.into()))?;
                if row.candidate().is_some() {frame.finish_candidate_prefill_hook(index,chunk)}
                else {frame.finish_token_scores_prefill_hook(index,chunk)}.map_err(|cause|error(cause.into()))?;
                continue;
            }
            if let Some(plan) = row.transform_plan().filter(|plan| matches!(plan.selection().transform, CaptureTransform::Summary | CaptureTransform::Histogram { .. })) {
                let fragment = plan.fragment(chunk.input.start / bound.geometry().prefill_chunk_positions)
                ...
                if decision == crate::capture::CapturePrefillHookDecision::First {
                    let dtype = complete.dtype.as_ref().ok_or_else(||
                        error(Cause::Source("reached remote reduced row has no actual scalar source")))?;
                    let charge = entry.allowance.take_remote_charge().map_err(|cause| error(cause.into()))?;
                    frame.reserve_remote_prefill_hook(index, dtype.clone(), charge)
            ...
            let assembly = row.assembly().ok_or_else(|| error(Cause::Source("remote row has no tensor assembly")))?;
            let fragment = assembly.fragment(chunk.input.start / bound.geometry().prefill_chunk_positions)
                .map_err(|cause| error(Cause::Progress(cause.into())))?;
            if decision == crate::capture::CapturePrefillHookDecision::First {
                let dtype = complete.dtype.as_ref().ok_or_else(||
                    error(Cause::Source("reached remote capture row has no actual scalar source")))?;
                let charge = entry.allowance.take_remote_charge().map_err(|cause| error(cause.into()))?;
                frame.reserve_remote_prefill_hook(index, dtype.clone(), charge)
                    .map_err(|cause| error(cause.into()))?;
            }
            frame.finish_prefill_hook(index, &fragment).map_err(|cause| error(cause.into()))?;
        }
        Ok(())
    }
    fn complete_prefill_evidence(&mut self, frame: &mut ScheduledCaptureStep<'_>,
        ...
        -> Result<(), PartitionCaptureProgramError>
    {
        if self.interventions.as_ref().is_none_or(|rows|!rows.has_evidence()) { return Ok(()); }
        self.charge()?;
        if !bound.selection().source().same_storage(&self.source) || !self.coordination_complete || self.delivered {
            return Err(self.error(Cause::Source("evidence completion differs from its bound frame")));
        }
        self.complete_intervention_evidence(frame,bound,chunk)
    }
    fn deliver(&mut self, frame: &mut ScheduledCaptureStep<'_>) -> Result<(), PartitionCaptureProgramError> {
        self.charge()?;
        ...
        for index in 0..self.entries.len() {
            if self.deliver_projected(index,frame)? {continue;}
            if let Some(entry) = &mut self.entries[index] {
                let delivery = entry.delivery.take().expect("one prepared delivery");
                delivery.deliver(frame).map_err(|cause| self.error(cause.into()))?;
            }
        }
        Ok(())
    }
