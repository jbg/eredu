...
impl PartitionInvocationReceiverSource {
    pub(crate) fn prepare_local(
        ...
        if context.invocation.is_none()
            && (context.phase != CapturePhase::Decode || context.prediction == 0)
            || context.capture_plan_identity != source.admission().identity()
            || producer >= receipt.world_size()
        ...
        let mut local = [0; 32];
        for (index, &n) in shape.iter().enumerate() {
            local[index] = usize::try_from(n).map_err(|_| E::Source)?;
        }
        Ok(Self {
            shape: local,
            rank: shape.len(),
    ...
    pub(crate) fn control_bytes() -> Option<usize> {
        let parts = [
            ...
            size_of::<(&PartitionCaptureReceiptPlan, usize, TensorDtype, &[u64])>(),
            size_of::<[usize; 32]>(),
            size_of::<(usize, &u64)>(),
