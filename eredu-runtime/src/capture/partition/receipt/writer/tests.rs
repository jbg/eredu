use super::*;
use eredu_core::{
    ObservationPosition, SharedTensorObservation, TensorObservation, TensorObservationData,
};
...
fn source() -> (PartitionCaptureContext, CaptureRecord) {
    let context = PartitionCaptureContext {
        artifact_identity: "artifact-λ".into(),
        execution_identity: "TP/PP".into(),
        run_identity: "run-雪".into(),
        overlay_identity: None,
        capture_plan_identity: "capture-source".into(),
        selection_index: 2,
    ...
    let record = CaptureRecord {
        ...
        selection_id: "final-雪".into(),
        path: "model.logits".into(),
        node_id: "decoder-λ".into(),
        position: ObservationPosition::ReadOnly,
        source_shape: Some(vec![1, 1, 13]),
        source_dtype: Some(TensorDtype::F16),
        selected_shape: Some(vec![1, 1, 13]),
        outcome: CaptureOutcome::Truncated {
            available_elements: 13,
            emitted_elements: 5,
        },
        payload: Some(CapturePayload::SharedTensor(
            SharedTensorObservation::retain(raw, ()),
        )),
...
fn borrowed_partition_receipt_keeps_canonical_unicode_nonfinite_values_and_paid_output() {
    ...
    let quoted = PartitionCaptureRecordEncoding::metadata_bytes(8192).unwrap();
    assert_eq!(used.load(Ordering::SeqCst), baseline);
    let actual = PartitionCaptureRecordEncoding::new(
        &context,
        "receipt-λ",
        5,
    ...
    .encode(&funding)
    .unwrap();
    let spent = used.load(Ordering::SeqCst).checked_sub(baseline).unwrap();
    assert!(quoted >= spent);
    assert_eq!(
        PartitionCaptureRecordEncoding::metadata_bytes(actual.len()),
        Some(spent)
    );
    assert_eq!(used.load(Ordering::SeqCst), baseline + spent);
    assert_eq!(
        PartitionCaptureRecordEncoding::metadata_bytes(usize::MAX),
        None
    );
    ...
    let ordinary = PartitionCaptureProducerRecord {
        ...
        receipt_plan_identity: "receipt-λ".into(),
    ...
    assert_eq!(
        json["fragments"][0]["record"]["outcome"]["emitted_elements"],
        5
    );
    assert_eq!(
        json["fragments"][0]["record"]["payload"]["value"]["data"]["values"],
        serde_json::json!([17.25, -0.0, "nan", "+inf", "-inf"])
    );
    assert_eq!(
        match record.payload.as_ref().unwrap().as_tensor().unwrap().data() {
...
fn borrowed_partition_receipt_refusal_retains_spending_and_custody() {
    ...
    .unwrap();
    let failed = PartitionCaptureRecordEncoding::new(
        &context,
        "receipt",
        0,
        PartitionCaptureCombination::Disjoint,
    ...
    assert!(matches!(
        failed,
        PartitionCaptureEncodingError::Source { .. }
    ));
    let spent = used.load(Ordering::SeqCst);
    assert!(spent > 0);
    refuse.store(true, Ordering::SeqCst);
    let refused = PartitionCaptureRecordEncoding::new(
        &context,
        "receipt",
        0,
        PartitionCaptureCombination::Disjoint,
...
fn contiguous_empty_wire_keeps_an_empty_sequence_and_actual_optional_precision() {
    ...
    for dtype in [None, Some(TensorDtype::Bf16)] {
        ...
        .unwrap();
        let expected = PartitionCaptureProducerRecord {
            schema_version: PARTITION_CAPTURE_SCHEMA_VERSION,
            combination: PartitionCaptureCombination::Disjoint,
            receipt_plan_identity: "receipt-empty".into(),
            context: context.clone(),
            producer_rank: 1,
            source_dtype: dtype,
            fragments: Vec::new(),
        ...
        assert_eq!(
            wire["source_dtype"].is_null(),
            expected.source_dtype.is_none()
        );
    }
}
