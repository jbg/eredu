...
impl<'a> PartitionCaptureRecordEncoding<'a> {
        ...
        let controls = [
            ...
            size_of::<
                BorrowedPartitionCaptureProducerRecord<
                    '_,
                    [BorrowedPartitionCaptureFragmentRecord<'_>; 1],
                >,
            >(),
    ...
    /// Exact closed writer and maximum destination population; no encoding or
    /// reservation is performed by this descriptive query.
    pub fn metadata_bytes(maximum: usize) -> Option<usize> {
        Self::control_bytes()?.checked_add(closed_metadata_bytes::<
            BorrowedPartitionCaptureProducerRecord<
                '_,
                [BorrowedPartitionCaptureFragmentRecord<'_>; 1],
            >,
    ...
    pub fn encode(
        ...
        let source = |reason| PartitionCaptureEncodingError::Source {
            reason,
            funding: funding.clone(),
        };
        let controls = Self::control_bytes().ok_or_else(|| source("writer controls overflow"))?;
        funding.reserve_metadata(controls).map_err(|cause| {
            PartitionCaptureEncodingError::Funding {
                cause,
                funding: funding.clone(),
            }
        })?;
        ...
        let fragments = [BorrowedPartitionCaptureFragmentRecord {
            fragment_index: 0,
            record: self.record,
        }];
...
pub(crate) fn encode_contiguous(
    ...
    record: Option<&eredu_core::capture::CaptureRecordWire<'_>>,
    maximum: usize,
    funding: &HostMetadataFunding,
) -> Result<PartitionCaptureBuffer<u8>, PartitionCaptureEncodingError> {
    let source = |reason| PartitionCaptureEncodingError::Source {
    ...
    type Fragment<'a> =
        BorrowedPartitionCaptureFragmentRecord<'a, eredu_core::capture::CaptureRecordWire<'a>>;

    funding
        .reserve_metadata(
            contiguous_control_bytes()
                .ok_or_else(|| source("fragment writer controls overflow"))?,
        )
        .map_err(|cause| PartitionCaptureEncodingError::Funding {
            cause,
            funding: funding.clone(),
        })?;
    if maximum == 0
        || identity.is_empty()
        || context.selection_index == usize::MAX
        || record.is_some_and(|r| r.source_dtype != dtype)
    {
        return Err(source("contiguous producer receipt source differs"));
    }
    let fragments = [record.map(|record| BorrowedPartitionCaptureFragmentRecord {
        fragment_index: 0,
        record,
    })];
...
pub(crate) fn encode_fragment_records(
    context: &PartitionCaptureContext,
    identity: &str,
    rank: usize,
    combination: PartitionCaptureCombination,
    dtype: Option<&TensorDtype>,
    records: &[eredu_core::capture::CaptureRecordWire<'_>],
    maximum: usize,
    funding: &HostMetadataFunding,
) -> Result<PartitionCaptureBuffer<u8>, PartitionCaptureEncodingError> {
    let source = |reason| PartitionCaptureEncodingError::Source {
    ...
    funding
        .reserve_metadata(
            fragment_records_control_bytes()
                .ok_or_else(|| source("fragment writer controls overflow"))?,
        )
        .map_err(|cause| PartitionCaptureEncodingError::Funding {
            cause,
...
impl serde::Serialize for RecordFragments<'_, '_> {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        for (fragment_index, record) in self.0.iter().enumerate() {
            sequence.serialize_element(&BorrowedPartitionCaptureFragmentRecord {
                fragment_index,
                record,
            })?;
        }
...
fn encode_closed<T: serde::Serialize>(
    record: &T,
    maximum: usize,
    funding: &HostMetadataFunding,
) -> Result<PartitionCaptureBuffer<u8>, PartitionCaptureEncodingError> {
    ...
    funding
        .reserve_metadata(
            closed_control_bytes::<T>().ok_or_else(|| source("closed writer controls overflow"))?,
        )
        .map_err(|cause| PartitionCaptureEncodingError::Funding {
            cause,
            funding: funding.clone(),
    ...
    if !written || !writer.fits || writer.output.len() != count {
        return Err(source("writer count changed"));
    }
    Ok(output)
}

struct Destination<'a> {
    output: &'a mut PartitionCaptureBuffer<u8>,
    fits: bool,
}
...
pub(crate) fn contiguous_metadata_bytes(maximum: usize) -> Option<usize> {
    contiguous_control_bytes()?.checked_add(closed_metadata_bytes::<
        BorrowedPartitionCaptureProducerRecord<'_, Fragments<'_>>,
    >(maximum)?)
}
...
pub(crate) fn fragment_records_metadata_bytes(maximum: usize) -> Option<usize> {
    fragment_records_control_bytes()?.checked_add(closed_metadata_bytes::<
        BorrowedPartitionCaptureProducerRecord<'_, RecordFragments<'_, '_>>,
    >(maximum)?)
}
fn fragment_records_control_bytes() -> Option<usize> {
    let parts = [
        size_of::<RecordFragments<'_, '_>>(),
        size_of::<BorrowedPartitionCaptureProducerRecord<'_, RecordFragments<'_, '_>>>(),
        size_of::<
            BorrowedPartitionCaptureFragmentRecord<'_, eredu_core::capture::CaptureRecordWire<'_>>,
        >(),
        eredu_core::capture::CaptureRecordWire::control_bytes(),
        size_of::<(
            &PartitionCaptureContext,
            ...
            PartitionCaptureCombination,
            Option<&TensorDtype>,
            &[eredu_core::capture::CaptureRecordWire<'_>],
            usize,
            &HostMetadataFunding,
        )>(),
        size_of::<
            std::iter::Enumerate<std::slice::Iter<'_, eredu_core::capture::CaptureRecordWire<'_>>>,
        >(),
...
fn closed_control_bytes<T: serde::Serialize>() -> Option<usize> {
    let parts = [
        size_of::<(&T, usize, &HostMetadataFunding)>(),
        ...
        size_of::<usize>(),
        size_of::<bool>(),
        size_of::<PartitionCaptureEncodingError>(),
        size_of::<Result<PartitionCaptureBuffer<u8>, PartitionCaptureEncodingError>>(),
    ];
    parts
        .into_iter()
...
fn closed_metadata_bytes<T: serde::Serialize>(maximum: usize) -> Option<usize> {
    closed_control_bytes::<T>()?
        .checked_add(PartitionCaptureBuffer::<u8>::funded_control_bytes(maximum)?)
}
