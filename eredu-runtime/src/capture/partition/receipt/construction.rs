...
impl ProducerProjection<'_> {
    fn fragments(&self)->usize {match self {Self::Contiguous(p)=>p.fragments(),Self::Coordinates(p)=>p.fragments(),Self::Routed(p)=>p.fragments()}}
    fn requested_bytes(&self)->Option<usize> {match self {
    ...
    fn construct(self)->CaptureSlicePartition {match self {Self::Contiguous(p)=>p.construct(),Self::Coordinates(p)=>p.construct(),Self::Routed(p)=>p.construct()}}
}
#[derive(Clone, Copy)]
enum ProducerSources<'a> {
    Complete(usize),
    Coordinates { axis:usize, rows:&'a [PartitionCaptureCoordinateProducer<'a>], combination:PartitionCaptureCombination },
    Routed(&'a [PartitionCaptureRoutedProducerSource<'a>]),
    Contiguous { axis:usize, rows:&'a [PartitionCaptureContiguousProducer], combination:PartitionCaptureCombination },
}
impl ProducerSources<'_> {
    fn len(self)->usize {match self {Self::Complete(_)=>1,Self::Contiguous{rows,..}=>rows.len(),Self::Coordinates{rows,..}=>rows.len(),Self::Routed(rows)=>rows.len()}}
    fn axis(self)->usize {match self {Self::Complete(_)=>0,Self::Contiguous{axis,..}|Self::Coordinates{axis,..}=>axis,Self::Routed(_)=>2}}
    fn combination(self)->PartitionCaptureCombination {match self {
        Self::Complete(_)=>PartitionCaptureCombination::Disjoint,Self::Contiguous{combination,..}|Self::Coordinates{combination,..}=>combination,Self::Routed(_)=>PartitionCaptureCombination::Disjoint}}
    fn row(self,index:usize,global:&[u64])->(usize,std::ops::Range<u64>) {match self {
...
impl PartitionCaptureReceiptPlan {
    ...
    pub fn new_complete_shared_funded(
        ...
        Self::construct_shared_funded(source, context, ProducerSources::Complete(producer), world_size, limits,
            metadata, ledger, false, None).map(|(receipt, _)| receipt)
    }
    ...
    pub(crate) fn new_complete_shared_funded_terminal_global(
        ...
        Self::construct_shared_funded(source,context,ProducerSources::Complete(producer),world_size,limits,
            metadata,ledger,true,terminal_rows)
    }
    ...
    fn construct_shared_funded(
        ...
        let controls=if matches!(producers_source,ProducerSources::Routed(_)) {
            controls.checked_add(routed_constructor_control_bytes().ok_or_else(||error(Cause::Source("sparse controls overflow")))?)
        ...
        if context.capture_plan_identity != source.admission().identity()
            || world_size == 0 || producers_source.len() == 0 || producers_source.len() > limits.max_producers
            || limits.max_producers == 0 || limits.max_record_bytes == 0
        ...
        {
            return Err(error(Cause::Source("receipt identity, context or participant differs")));
        }
        let summary = source.admission().plan().selections.get(context.selection_index)
            .is_some_and(|selection| matches!(selection.transform, CaptureTransform::Summary));
        let histogram = source.admission().plan().selections.get(context.selection_index)
            .is_some_and(|selection| matches!(selection.transform, CaptureTransform::Histogram { .. }));
        if context.invocation.is_some() && (terminal_rows.is_some()
            || source.admission().plan().selections.get(context.selection_index)
                .is_some_and(|selection|super::super::vocabulary::is_vocabulary(&selection.transform))) {
            return Err(error(Cause::Source("explicit vocabulary invocation requires its typed terminal source")));
        }
        let vocabulary_geometry=super::super::CompleteVocabularyGeometry::prepare(source.admission(),context.selection_index,
            context.phase,context.prediction,terminal_rows)
        ...
        let routed_geometry=if matches!(producers_source,ProducerSources::Routed(_)) {
            Some(CaptureRoutedUnitsGeometry::prepare(source.admission(),context.selection_index,
                context.phase,context.prediction,context.invocation)
                .map_err(|_|error(Cause::Source("selection has no exact sparse geometry")))?)
        } else {None};
        let tensor_geometry = if summary || histogram || vocabulary_geometry.is_some() || routed_geometry.is_some() { None } else {
            Some(CaptureTensorGeometry::prepare(source.admission(), context.selection_index,
                context.phase, context.prediction, context.invocation)
                .map_err(|_| error(Cause::Source("selection has no complete floating tensor geometry")))?)
        };
        let summary_geometry = if summary {
            Some(CaptureSummaryGeometry::prepare(source.admission(), context.selection_index,
                context.phase, context.prediction, context.invocation)
                .map_err(|_| error(Cause::Source("selection has no complete summary geometry")))?)
        } else { None };
        let histogram_geometry = if histogram {
            Some(CaptureHistogramGeometry::prepare(source.admission(),context.selection_index,
                context.phase,context.prediction,context.invocation)
                .map_err(|_|error(Cause::Source("selection has no complete histogram geometry")))?)
        } else {None};
        let terminal_ends=vocabulary_geometry.as_ref().map(|value|value.shape().map(|n|n as u64)).unwrap_or([0;3]);
        let (source_shape, source_starts, source_ends, source_strides) = match (&tensor_geometry, &summary_geometry, &histogram_geometry, &vocabulary_geometry, &routed_geometry) {
            (Some(geometry), None, None, None, None) => (geometry.source_shape(), geometry.starts(), geometry.ends(), geometry.strides()),
            (None, Some(geometry), None, None, None) => (geometry.source_shape(), geometry.starts(), geometry.ends(), geometry.strides()),
            (None,None,Some(geometry),None,None) => (geometry.source_shape(),geometry.starts(),geometry.ends(),geometry.strides()),
            (None,None,None,Some(geometry),None) => (&geometry.shape()[..], &[0,0,0][..], &terminal_ends[..], &[1,1,1][..]),
            (None,None,None,None,Some(geometry)) => (geometry.source_shape(),geometry.starts(),geometry.ends(),geometry.strides()),
            _ => return Err(error(Cause::Source("selection payload geometry differs"))),
        };
        let rank = source_shape.len();
        if rank == 0 { return Err(error(Cause::Source("complete producer needs a physical axis"))); }
        ...
        let mut starts = metadata.metadata_vec(rank).map_err(|cause| error(cause.into()))?;
        let mut ends = metadata.metadata_vec(rank).map_err(|cause| error(cause.into()))?;
        let mut strides = metadata.metadata_vec(rank).map_err(|cause| error(cause.into()))?;
        let mut shape = metadata.metadata_vec(rank).map_err(|cause| error(cause.into()))?;
        ...
        let slice = ResolvedCaptureSlice { starts, ends, strides, shape };
        let mut projection_sources=metadata.metadata_vec(producers_source.len()).map_err(|cause|error(cause.into()))?;
        let mut fragments=0usize;
        for index in 0..producers_source.len() {
            let (producer_rank,coordinates)=producers_source.row(index,&global[..rank]);
            if producer_rank>=world_size {return Err(error(Cause::Source("producer rank differs from selected world")));}
            let projection=match producers_source {
                ProducerSources::Coordinates{axis,rows,..}=>ProducerProjection::Coordinates(CaptureCoordinateProjectionPlan::prepare(
                    &global[..rank],&slice,axis,rows[index].coordinates,limits.max_fragments)
                    .map_err(|_|error(Cause::Source("retained component projection exceeds its bound")))?),
                ProducerSources::Routed(rows)=>ProducerProjection::Routed(CaptureCoordinateProjectionPlan::prepare(
                    &global[..rank],&slice,2,rows[index].ownership.coordinates.units(),limits.max_fragments)
                    .map_err(|_|error(Cause::Source("retained sparse projection exceeds its bound")))?),
                _=>ProducerProjection::Contiguous(CaptureContiguousProjectionPlan::prepare(&global[..rank],&slice,
                    producers_source.axis(),coordinates,limits.max_fragments)
                    .map_err(|_|error(Cause::Source("retained contiguous projection exceeds its bound")))?),
            };
            fragments=fragments.checked_add(projection.fragments()).ok_or_else(||error(Cause::Source("fragment population overflow")))?;
            if fragments>limits.max_fragments {return Err(error(Cause::Source("total fragments exceed the receipt bound")));}
            metadata.reserve_metadata(projection.requested_bytes().ok_or_else(||error(Cause::Source("projection population overflow")))?).map_err(|cause|error(cause.into()))?;
            projection_sources.push((producer_rank,projection));
        }
        let local_preparation = Self::preparation_population_usage(producers_source.len() as u64, rank, fragments as u64)
            .and_then(|usage| usage.checked_add(CaptureUsage { host_bytes: 2048, ..Default::default() }))
            .map_err(|cause| error(cause.into()))?;
        let global_preparation = if global_preparation_required { local_preparation.checked_mul(world_size as u64)
            .map_err(|cause| error(cause.into()))? } else { CaptureUsage::default() };
        let mut preparation = if global_preparation_required { Some(ledger.reserve_quota(global_preparation)
            .and_then(|mut quota| quota.reserve_quota(local_preparation))
            .map_err(|cause| error(cause.into()))?) } else { None };
        let ledger: &mut dyn CaptureReservation = match preparation.as_mut() {
            Some(quota) => quota, None => ledger,
        };
        let ownership=match producers_source {
            ProducerSources::Routed(rows)=> {
                let mut owned=metadata.metadata_vec(rows.len()).map_err(|cause|error(cause.into()))?;
                // The ordinary routed constructor reserves this same logical
                // ownership population in addition to projection preparation.
                let ownership_usage=Self::routed_ownership_source_usage(rows.iter().map(|row|row.ownership))
                    .map_err(|cause|error(cause.into()))?;
                reserve(ledger,ownership_usage).map_err(|cause|error(cause.into()))?;
                for row in rows {
                    let copy=copy_routed_ownership(row.ownership,metadata).map_err(|cause|error(cause))?;
        ...
        let mut producers = metadata.metadata_vec(producers_source.len()).map_err(|cause| error(cause.into()))?;
        for (rank,projection) in projection_sources {
            producers.push(PartitionCaptureProducer {rank,projection:projection.construct()});
        }
        let mut global_shape = metadata.metadata_vec(rank).map_err(|cause| error(cause.into()))?;
    ...
    -> Result<PartitionCaptureContext, eredu_nn::Error>
{
    Ok(PartitionCaptureContext {
        artifact_identity: metadata.metadata_string(format_args!("{}", source.artifact_identity))?,
        execution_identity: metadata.metadata_string(format_args!("{}", source.execution_identity))?,
        run_identity: metadata.metadata_string(format_args!("{}", source.run_identity))?,
        overlay_identity: source.overlay_identity.as_ref()
            .map(|value| metadata.metadata_string(format_args!("{value}"))).transpose()?,
        capture_plan_identity: metadata.metadata_string(format_args!("{}", source.capture_plan_identity))?,
        selection_index: source.selection_index, phase: source.phase,
        prediction: source.prediction, forward_epoch: source.forward_epoch,
...
fn constructor_control_bytes() -> Option<usize> {
    ...
    let parts = [
        ...
        size_of::<Result<Option<super::super::CompleteVocabularyGeometry<'_>>,eredu_core::capture::CaptureTensorGeometryError>>(),
        ...
        size_of::<Result<CaptureContiguousProjectionPlan<'_>,eredu_core::capture::CaptureContiguousProjectionError>>(),
...
impl PartitionCaptureReceiptConstructionError {
    pub(crate) fn limit_skip(&self) -> Option<CaptureSkipReason> {
        let cause = match &self.cause {
            Cause::Capture(cause) | Cause::Receipt(PartitionCaptureMergeError::Capture(cause)) => cause.cause(),
            _ => return None,
        };
        match cause { CaptureError::Limit { budget, cumulative } => Some(CaptureSkipReason::Limit {
