...
impl<'a> PartitionInvocationCaptureGeometry<'a> {
    ...
    pub fn prepare(admission:&'a AdmittedCapturePlan,selection:usize,phase:CapturePhase,
        prediction:u64,projection:&'a CaptureSlicePartition,fragment:usize,
        combination:PartitionCaptureCombination)->Result<Self,PartitionInvocationCaptureSourceError> {
        Self::prepare_source(admission,selection,phase,prediction,None,projection,fragment,combination)
    }
    fn prepare_source(admission:&'a AdmittedCapturePlan,selection:usize,phase:CapturePhase,
        prediction:u64,invocation:Option<CaptureInvocationShape>,projection:&'a CaptureSlicePartition,fragment:usize,
        combination:PartitionCaptureCombination)->Result<Self,PartitionInvocationCaptureSourceError> {
        use PartitionInvocationCaptureKind as K;
        use PartitionInvocationCaptureSourceError as E;
        let selected=admission.plan().selections.get(selection).ok_or(E::Source)?;
        let kind=match (&selected.transform,combination) {
            (CaptureTransform::Summary,PartitionCaptureCombination::Disjoint)=>K::Summary(
                CaptureSummaryGeometry::prepare_partition(admission,selection,phase,prediction,invocation,projection,fragment)?),
            (CaptureTransform::Histogram{..},PartitionCaptureCombination::Disjoint)=>K::Histogram(
                CaptureHistogramGeometry::prepare_partition(admission,selection,phase,prediction,invocation,projection,fragment)?),
            (CaptureTransform::FullTensor|CaptureTransform::Slice|CaptureTransform::Preview{..},_)
            |(CaptureTransform::Summary|CaptureTransform::Histogram{..},PartitionCaptureCombination::SumF64ToF32)=>K::Tensor(
                CaptureTensorGeometry::prepare_partition(admission,selection,phase,prediction,invocation,projection,fragment,combination)?),
            _=>return Err(E::Source),
        };
        Ok(Self{admission,selection,projection,fragment,window:None,kind})
    }
    ...
    pub fn from_receipt(receipt:&'a PartitionCaptureReceiptPlan,producer:usize,fragment:usize)
        ...
        if context.capture_plan_identity!=source.admission().identity()
            ||receipt.routed_producer(producer).is_some(){return Err(E::Source);}
        Self::prepare_source(source.admission(),context.selection_index,context.phase,context.prediction,
            context.invocation,projection,fragment,receipt.combination())
    }
    ...
    pub fn from_window_receipt(receipt:&'a PartitionCaptureReceiptPlan,producer:usize,fragment:usize,
        physical:CaptureInvocationShape,window:CaptureInvocationWindow)
        ->Result<Self,PartitionInvocationCaptureSourceError> {
        use PartitionInvocationCaptureKind as K;
        use PartitionInvocationCaptureSourceError as E;
        let source=receipt.shared_plan_source().ok_or(E::Source)?;
        ...
        if context.invocation!=Some(logical)||context.capture_plan_identity!=source.admission().identity()
            ||receipt.routed_producer(producer).is_some(){return Err(E::Source);}
        let projection=receipt.producer(producer).ok_or(E::Source)?;
        let admission=source.admission();let selection=context.selection_index;
        let selected=admission.plan().selections.get(selection).ok_or(E::Source)?;
        let kind=match (&selected.transform,receipt.combination()) {
            (CaptureTransform::Summary,PartitionCaptureCombination::Disjoint)=>K::Summary(
                CaptureSummaryGeometry::prepare_partition_window(admission,selection,context.phase,context.prediction,
                    physical,window,projection,fragment)?),
            (CaptureTransform::Histogram{..},PartitionCaptureCombination::Disjoint)=>K::Histogram(
                CaptureHistogramGeometry::prepare_partition_window(admission,selection,context.phase,context.prediction,
                    physical,window,projection,fragment)?),
            (CaptureTransform::FullTensor|CaptureTransform::Slice|CaptureTransform::Preview{..},_)
            |(CaptureTransform::Summary|CaptureTransform::Histogram{..},PartitionCaptureCombination::SumF64ToF32)=>K::Tensor(
                CaptureTensorGeometry::prepare_partition_window(admission,selection,context.phase,context.prediction,
                    physical,window,projection,fragment,receipt.combination())?),
            _=>return Err(E::Source),
        };
        Ok(Self{admission,selection,projection,fragment,window:Some((physical,window)),kind})
    }
    /// Window-only call/result frames in addition to the ordinary source quote.
    pub fn window_control_bytes()->Option<usize> {
        let parts=[Self::control_bytes()?,size_of::<Self>()*2,
            size_of::<Result<Self,PartitionInvocationCaptureSourceError>>(),
            size_of::<(&PartitionCaptureReceiptPlan,usize,usize,CaptureInvocationShape,CaptureInvocationWindow)>(),
    ...
    pub const fn invocation_window(&self)->Option<(CaptureInvocationShape,CaptureInvocationWindow)>{self.window}
    /// The selected ordinary local worker.
    pub fn kind(&self)->&PartitionInvocationCaptureKind<'a>{&self.kind}
    /// Move the same geometry into its typed Host or native consumer.
    pub fn into_kind(self)->PartitionInvocationCaptureKind<'a>{self.kind}
    ...
    pub const fn selection_index(&self)->usize{self.selection}
    ...
    pub fn native_transform(&self)->&CaptureTransform {match &self.kind {
    ...
    pub fn control_bytes()->Option<usize> {
        let parts=[size_of::<Self>()*2,size_of::<PartitionInvocationCaptureKind<'_>>(),
            size_of::<Result<Self,PartitionInvocationCaptureSourceError>>(),size_of::<PartitionInvocationCaptureSourceError>(),
            size_of::<(&AdmittedCapturePlan,usize,CapturePhase,u64,&CaptureSlicePartition,usize,PartitionCaptureCombination)>(),
            size_of::<(&PartitionCaptureReceiptPlan,usize,usize)>(),size_of::<Option<CaptureInvocationShape>>(),
            size_of::<(&AdmittedCapturePlan,usize,CapturePhase,u64,Option<CaptureInvocationShape>,&CaptureSlicePartition,usize,PartitionCaptureCombination)>(),
