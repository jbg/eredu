...
impl Sources<'_> {
    ...
    pub(super) fn rank(self, index: usize) -> usize {
        match self {
            Self::Contiguous(rows) => rows[index].rank,
            Self::Coordinates(rows) => rows[index].rank,
        }
    }
    pub(super) fn copy(self, metadata: &HostMetadataFunding) -> Result<Owned, Cause> {
        match self {
            Self::Contiguous(rows) => {
                let mut owned = metadata.metadata_vec(rows.len())?;
                for row in rows {
                    if row.coordinates.start > row.coordinates.end {
                        return Err(Cause::Source("contiguous rank source differs"));
                    }
...
impl Owned {
    pub(super) fn minimum_rank(&self) -> Option<usize> {
        match self {
    ...
    pub(super) fn receipt(
        ...
        match self {
            Self::Contiguous(rows) => PartitionCaptureReceiptPlan::new_contiguous_shared_funded(
                source,
                context,
                axis,
                rows,
            ...
            Self::Coordinates(rows) => {
                ...
                for (rank, coordinates) in rows {
                    borrowed.push(PartitionCaptureCoordinateProducer {
                        rank: *rank,
                        coordinates,
                    });
                }
                PartitionCaptureReceiptPlan::new_coordinates_shared_funded(
                    source,
                    context,
                    axis,
                    &borrowed,
...
impl PreparedPartitionContiguousSource {
    ...
    pub fn new_local_coordinates_invocation(
        source: &SharedCapturePlan,
        index: usize,
        axis: usize,
        producers: &[PartitionCaptureCoordinateProducer<'_>],
        ...
        Self::coordinates(
            ...
            local,
            combination,
            Coordinate::Invocation(phase, prediction, None, None),
            metadata,
        )
    }
    /// Exact independently admitted physical axes through the same receipt worker.
    /// This source has no chunk or text-prediction authority.
    #[allow(clippy::too_many_arguments)]
    pub fn new_local_coordinates_shaped(
        source: &SharedCapturePlan,
        index: usize,
        axis: usize,
        producers: &[PartitionCaptureCoordinateProducer<'_>],
    ...
    fn coordinates(
        ...
        metadata
            .reserve_metadata(
                control_bytes().ok_or_else(|| {
                    error(Cause::Source("component constructor controls overflow"))
                })?,
            )
...
pub(super) fn control_bytes() -> Option<usize> {
    let frames = [
        size_of::<Sources<'_>>() * 2,
        size_of::<Owned>() * 2,
        size_of::<Result<Owned, Cause>>(),
        size_of::<ComponentCoordinateMap>() * 2,
        size_of::<(usize, ComponentCoordinateMap)>(),
        ...
        size_of::<(
            ...
            &[PartitionCaptureFragmentGeometry<'_>],
