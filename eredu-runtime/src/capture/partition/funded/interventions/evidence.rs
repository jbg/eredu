...
{
    /// Exact fixed controls of the existing companion constructor.
    ...
    /// Both actual before/after projected rows bind one evidence budget source.
    pub fn budget_binding_control_bytes() -> Option<usize> {
        super::super::super::receipt::EvidenceBudgetSource::controls()?.checked_mul(2)
    }
    pub fn new(
        original: &OriginalInterventionSource,
        operation: usize,
        program: PreparedPartitionCaptureProgram<'t, T>,
        selection: Option<crate::layered::PreparedCaptureSelection>,
    ) -> Result<Self, PartitionCaptureProgramError> {
        program
            .metadata
            .reserve_metadata(Self::construction_control_bytes().ok_or_else(|| {
                program.error(Cause::Source("evidence construction controls overflow"))
            })?)
            .map_err(|cause| program.error(cause.into()))?;
        let companion = original
            .plan()
            .evidence(operation)
            .ok_or_else(|| program.error(Cause::Source("original evidence companion is absent")))?;
        let prefill = program.context.invocation.is_none()
            && program.context.phase == CapturePhase::Prefill
            && original
                .plan()
                .admission()
                .points()
        ...
        if !program
            ...
            || program.rows.len() != 2
            || program.first_epoch.is_some()
            || program.interventions.is_some()
            || selection
                .as_ref()
                .is_some_and(|selected| !selected.source().same_storage(&program.source))
            || prefill && selection.is_none()
            || program.context.invocation.is_some() && selection.is_some()
        {
            return Err(program.error(Cause::Source(
                "original companion receipt or causal declaration differs",
            )));
        }
        Ok(Self {
            program,
    ...
    pub(super) fn bind_budget(&mut self, parent: &SharedCapturePlan) -> Result<(), Cause> {
        ...
        if self.program.projected.len() != 2 || self.program.rows.iter().any(Option::is_some) {
            return Err(Cause::Source(
                "evidence budget requires both original fragment Host sources",
            ));
        }
        for row in &mut self.program.projected {
            let Some(projected::Row::Pending {
                source: projected::Source::Contiguous(source),
                ..
            }) = row
            else {
                return Err(Cause::Source(
                    "evidence budget has no unspent dense fragment source",
                ));
            };
            if source.evidence_budget.is_some() {
                return Err(Cause::Source("evidence budget source already bound"));
            }
            self.program.metadata.reserve_metadata(
                EvidenceBudgetSource::controls()
                    .ok_or(Cause::Source("evidence budget controls overflow"))?,
            )?;
            source.evidence_budget = Some(
                EvidenceBudgetSource::new(
                    parent,
                    &self.original,
                    self.operation,
                    &self.program.source,
    ...
    pub(super) fn matches(
        ...
        original: &OriginalInterventionSource,
        operation: usize,
        parent: &PreparedPartitionCaptureProgram<'t, T>,
    ) -> bool {
        let a = &self.program.context;
        let b = &parent.context;
        self.original.same_source(original)
...
where
    ...
    <T::Completion as Completion>::Error: Send + Sync + 'static,
{
    /// One entry into the shared evidence frame preparation or delivery worker.
    pub fn evidence_frame_control_bytes() -> Option<usize> {
        let parts = [
            size_of::<crate::working_memory::InterventionEvidenceFrame<'_>>() * 2,
            size_of::<
                Result<
                    crate::working_memory::InterventionEvidenceFrame<'_>,
            ...
            size_of::<(
                &mut Self,
                &mut ScheduledCaptureStep<'_>,
                crate::layered::BoundCaptureSelection<'_>,
                &crate::prefill::PrefillChunk,
                PrefillEvidenceProgress,
            )>(),
            ...
            eredu_core::BackendFailure::source_retention_peak_bytes::<
                crate::layered::PreparedCaptureSelectionError,
            >()?,
        ];
        parts
            .into_iter()
    ...
    pub(super) fn prepare_intervention_evidence(
        ...
        if self
            .interventions
            .as_ref()
            .is_none_or(|rows| rows.evidence.iter().all(Option::is_none))
        {
            return Ok(());
        }
        ...
        let result = (|| -> Result<(), PartitionCaptureProgramError> {
            let Some(rows) = &mut self.interventions else {
                return Ok(());
            };
            for (index, child) in rows.evidence.iter_mut().enumerate() {
                let Some(child) = child else { continue };
                let mut loan = frame
                    .take_intervention_evidence_frame(index)
                    .map_err(|cause| error(cause.into()))?;
                let source = child.program.source.clone();
                let phase = child.program.context.phase;
                let prediction = child.program.context.prediction;
                let result: Result<(), PartitionCaptureProgramError> = (|| {
                    if child.prefill {
                        let inference = prefill.ok_or_else(|| {
                            error(Cause::Source(
                                "evidence prefill lacks its actual parent schedule",
                            ))
                        })?;
                        child
                            .selection
                            .as_ref()
                            .ok_or_else(|| error(Cause::Source("evidence has no causal source")))?
                            .bind_geometry(inference)
                            .map_err(|cause| {
                ...
                let restore = frame.return_intervention_evidence_frame(loan);
                result?;
    ...
    pub(in crate::capture::partition::funded) fn coordinate_intervention_evidence(
        &mut self,
        epoch: DistributedCommitEpoch,
        ledger: &CaptureLedger,
    ) -> Result<(), PartitionCaptureProgramError> {
        if let Some(rows) = &mut self.interventions {
            for child in rows.evidence.iter_mut().flatten() {
                child.program.coordinate(epoch, ledger)?;
            }
        }
        Ok(())
    }
    pub(in crate::capture::partition::funded) fn intervention_evidence(
        &mut self,
        index: usize,
    ) -> Result<Option<&mut (dyn ScheduledPartitionCapture + '_)>, PartitionCaptureProgramError>
    {
        if !self.coordination_complete || self.delivered {
            return Err(self.error(Cause::Source(
                "evidence callback is outside its coordinated frame",
            )));
        }
        let Some(rows) = &mut self.interventions else {
            return Ok(None);
    ...
    pub(in crate::capture::partition::funded) fn progress_intervention_evidence(
        &mut self,
        frame: &mut ScheduledCaptureStep<'_>,
    ...
    pub(in crate::capture::partition::funded) fn complete_intervention_evidence(
        &mut self,
        frame: &mut ScheduledCaptureStep<'_>,
        bound: crate::layered::BoundCaptureSelection<'_>,
        chunk: &crate::prefill::PrefillChunk,
    ...
    fn advance_intervention_evidence(
        &mut self,
        frame: &mut ScheduledCaptureStep<'_>,
        bound: crate::layered::BoundCaptureSelection<'_>,
        chunk: &crate::prefill::PrefillChunk,
        progress: PrefillEvidenceProgress,
    ) -> Result<(), PartitionCaptureProgramError> {
        if self
            .interventions
            .as_ref()
        ...
        let result = (|| -> Result<(), PartitionCaptureProgramError> {
            ...
            for (index, child) in rows.evidence.iter_mut().enumerate() {
                ...
                let selected = child.selection.as_ref().ok_or_else(|| {
                    error(Cause::Source(
                        "prefill evidence has no retained causal declaration",
                    ))
                })?;
                if !selected.paths().same_storage(bound.selection().paths()) {
                    return Err(error(Cause::Source(
                        "prefill evidence paths differ from the loaded invocation",
                    )));
                }
                ...
                let result: Result<(), PartitionCaptureProgramError> = (|| {
                    match progress {
                        PrefillEvidenceProgress::Receivers => {
                            child
                                .program
                                .remote_prefill(loan.frame_mut(), bound, chunk)?
                        }
                        PrefillEvidenceProgress::Complete => {
                            let ordinal =
                                chunk.input.start / bound.geometry().prefill_chunk_positions;
                            loan.frame_mut()
                                .complete_prefill_chunk(ordinal)
                                .map_err(|cause| error(cause.into()))?;
                            if chunk.input.end == bound.geometry().input_positions {
                                loan.frame_mut()
                                    .finish_local_prefill_targets()
                                    .map_err(|cause| error(cause.into()))?;
                            }
                        }
                    }
    ...
    pub(in crate::capture::partition::funded) fn deliver_intervention_evidence(
        &mut self,
        frame: &mut ScheduledCaptureStep<'_>,
    ) -> Result<(), PartitionCaptureProgramError> {
        if self
