...
pub(super) enum Source {
    ...
    Routed(PreparedPartitionRoutedSource),
}
impl Source {
    fn matches_context(&self, context: &PartitionCaptureContext) -> bool {
        match self {
            Self::Contiguous(source) => source.matches_context(context),
            Self::Routed(source) => source.matches_context(context),
        }
    }
    fn matches_program(&self, source: &SharedCapturePlan, index: usize) -> bool {
        match self {
            Self::Contiguous(value) => value.matches_program(source, index),
            Self::Routed(value) => value.matches_program(source, index),
        }
    }
    fn prepare_selected_with_host<'t, T: PartitionCaptureTransport>(
        self,
        transport: &'t T,
        ...
        <T::Completion as Completion>::Error: Send + Sync + 'static,
    {
        match self {
            Self::Contiguous(source) => {
                source.prepare_selected_with_host(transport, context, epoch, host, limits, ledger)
            }
            Self::Routed(source) => {
                source.prepare_selected_with_host(transport, context, epoch, host, limits, ledger)
            }
...
struct HeldHostLimits {
    _host: PreparedPartitionFragmentHostFunding,
}
#[derive(Debug, thiserror::Error)]
    ...
    <T::Completion as Completion>::Error: Send + Sync + 'static,
{
    ...
    pub fn with_contiguous_row(
        self,
        index: usize,
        source: PreparedPartitionContiguousSource,
        host: PreparedPartitionFragmentHostFunding,
    ...
    fn with_projected_row(
        ...
        self.metadata
            .reserve_metadata(
                control_bytes::<T>()
                    .ok_or_else(|| self.error(Cause::Source("projected controls overflow")))?,
            )
            .map_err(|e| self.error(e.into()))?;
        if self.first_epoch.is_some()
            || !source.matches_context(&self.context)
            || index >= self.rows.len()
            || self.rows[index].is_some()
            || !source.matches_program(&self.source, index)
            || self.projected.get(index).is_some_and(Option::is_some)
        {
            return Err(self.error(Cause::Source(
                "projected row differs from the unprepared program",
            )));
        }
        if self.projected.is_empty() {
            self.projected = self
                .metadata
                .metadata_vec(self.rows.len())
                .map_err(|e| self.error(e.into()))?;
            self.projected.resize_with(self.rows.len(), || None);
        }
        self.projected[index] = Some(Row::Pending { source, host });
        Ok(self)
    }
    /// One debit of the existing projected-row execution worker. Preparation,
    /// coordination, progress and delivery debit it for actual projected rows;
    /// each explicit epoch lookup also debits it. Other entry costs are separate.
    pub fn projected_entry_control_bytes() -> Option<usize> {
        control_bytes::<T>()
    }
    fn charge_projected(&self) -> Result<(), PartitionCaptureProgramError> {
        self.metadata
            .reserve_metadata(
                Self::projected_entry_control_bytes()
                    .ok_or_else(|| self.error(Cause::Source("projected controls overflow")))?,
            )
            .map_err(|e| self.error(e.into()))
    }
    pub(super) fn prepare_projected(
        ...
        let Some(slot) = self.projected.get_mut(index) else {
            return Ok(false);
        };
        ...
        *slot = Some(Row::Spent);
        let Row::Pending { source, host } = row else {
            return Err(self.error(Cause::Source("projected preparation is spent")));
        };
        ...
        {
            return Err(PartitionCaptureProgramError::local(
                HeldHostLimits { _host: host },
                self.source.clone(),
                self.metadata.clone(),
            ));
        }
        match source.prepare_selected_with_host(
            self.transport,
            &self.context,
            epoch,
            host,
            ...
            Err(failure) => {
                let Some(reason) = self.allowed_skip(failure.reason) else {
                    return Err(failure.cause);
                };
                let Some((producer, usage)) = failure.descriptor else {
                    return Err(failure.cause);
                };
                ...
                self.projected[index] = Some(Row::Skipped {
                    producer,
                    usage,
                    reason,
                    cause: failure.cause,
    ...
    pub(super) fn coordinate_projected(
        &mut self,
        index: usize,
        ...
        if matches!(self.projected[index], Some(Row::Skipped { .. })) {
            let Some(Row::Skipped {
                producer,
                usage,
                reason,
                cause: preparation,
            }) = self.projected[index].take()
            else {
                unreachable!("checked skipped row")
            };
            ...
            let result = coordination
                .coordinate_source(index, None, producer, None, Some(&reason), usage, ledger)
        ...
        let Some(slot) = self.projected.get_mut(index).and_then(Option::as_mut) else {
            return Ok(false);
        };
        let Row::Ready(row) = slot else {
            return Err(self.error(Cause::Source("projected source is not ready")));
        };
        if row.is_routed() {
            let vote = match coordination.coordinate_routed_source(
                index,
                row.receipt()?,
                row.source_present(),
                row.local_dtype(),
                ...
                Ok(vote) => vote,
                Err(cause) => return Err(row.retain_vote_failure(cause)),
            };
            let dtype = vote.dtype.clone();
            row.bind_routed_vote(vote.dtype, vote.ranks)?;
            coordination
                .include(row.receipt()?, dtype, row.source_usage())
                .map_err(|cause| row.retain_vote_failure(cause))?;
            return Ok(true);
        }
        let dtype = match coordination.coordinate_contiguous_source(
            index,
            row.receipt()?,
            row.local_dtype(),
            row.source_usage(),
    ...
    pub(super) fn projected_epoch(
        &self,
    ) -> Result<DistributedCommitEpoch, PartitionCaptureProgramError> {
        self.charge_projected()?;
        if !self.coordination_complete || self.delivered {
            return Err(self.error(Cause::Source(
                "projected hook is outside its coordinated frame",
            )));
        }
        self.last_epoch
            .ok_or_else(|| self.error(Cause::Source("projected hook has no actual epoch")))
    }
    pub(super) fn take_projected_hook(
        &mut self,
        index: usize,
        frame: &mut ScheduledCaptureStep<'_>,
        first: bool,
        ...
        if matches!(slot, Row::Skipped { .. }) {
            return Ok(None);
        }
        let Row::Ready(row) = slot else {
            return Err(self.error(Cause::Source("projected hook owner is unavailable")));
        };
        if row.geometry().is_none() {
            return Err(self.error(Cause::Source("decode row cannot issue a prefill hook")));
        }
        if first {
            row.reserve_target(frame)?;
        }
        row.take_local(row.next_chunk(), epoch)
    }
    pub(super) fn take_projected_receiver(
        &mut self,
        index: usize,
        ...
        let Row::Ready(row) = slot else {
            return Err(self.error(Cause::Source("projected receiver owner is unavailable")));
        };
        if row.geometry() != Some(inference) {
            return Err(self.error(Cause::Source("projected receiver geometry differs")));
        }
        row.take_receiver(chunk, epoch)
    }
    pub(super) fn take_projected_invocation(
        &mut self,
        index: usize,
    ) -> Result<Option<PartitionCaptureLocalHook>, PartitionCaptureProgramError> {
        if !self.projected.get(index).is_some_and(Option::is_some) {
        ...
        let Row::Ready(row) = slot else {
            return Err(self.error(Cause::Source("decode source is not ready")));
        };
        if row.geometry().is_some() {
            return Err(self.error(Cause::Source("prefill row cannot issue a decode hook")));
        }
        row.take_local(0, epoch)
    }
    pub(super) fn take_projected_invocation_receiver(
        &mut self,
        index: usize,
    ...
    pub(super) fn progress_projected_receiver(
        &mut self,
        index: usize,
        frame: &mut ScheduledCaptureStep<'_>,
        ...
        let Row::Ready(projected) = slot else {
            return Err(error(Cause::Source(
                "projected progress owner is unavailable",
            )));
        };
        if projected.produces() || projected.is_routed() && projected.source_present() {
            return Ok(true);
        }
        if projected.geometry() != Some(bound.geometry()) {
            return Err(error(Cause::Source("projected progress geometry differs")));
        }
        let path = &source.admission().plan().selections[index].path;
        let decision = if projected.is_routed() {
            frame.begin_routed_prefill_batch(index, chunk, path)
        } else {
            frame.begin_prefill_hook(index, chunk, path)
        }
        ...
        if decision == crate::capture::CapturePrefillHookDecision::First {
            projected.reserve_target(frame)?;
        }
        ...
        if projected.is_routed() {
            let plan = row
                .routed_plan()
                .ok_or_else(|| error(Cause::Source("sparse progression has no original plan")))?;
            let fragment = plan
                .fragment(k)
                .map_err(|e| error(Cause::Progress(e.into())))?;
            frame
                .finish_partition_routed_prefill_hook(index, &fragment)
                .map_err(|e| error(e.into()))?;
            return Ok(true);
        }
        if let Some(plan) = row.transform_plan().filter(|plan| {
            ...
            let assembly = row
                .assembly()
                .ok_or_else(|| error(Cause::Source("projected global assembly is absent")))?;
    ...
    pub(super) fn deliver_projected(
        &mut self,
        index: usize,
        frame: &mut ScheduledCaptureStep<'_>,
    ) -> Result<bool, PartitionCaptureProgramError> {
        ...
        *slot = Some(Row::Spent);
        if matches!(&row, Row::Skipped { .. }) {
            return Ok(true);
        }
        let Row::Ready(row) = row else {
            return Err(self.error(Cause::Source("projected delivery is spent")));
        };
        let prefill = row.geometry().is_some();
        let delivery = row.finish()?;
        let result = if prefill {
            delivery.deliver_into_prefill_frame(frame)
        } else {
            delivery.deliver_into_invocation_frame(frame)
        };
        result.map_err(|e| {
            PartitionCaptureProgramError::local(e, self.source.clone(), self.metadata.clone())
        })?;
        Ok(true)
    }
}
pub(super) fn control_bytes<T: PartitionCaptureTransport>() -> Option<usize> {
    let parts = [
        size_of::<Source>(),
        size_of::<contiguous::PreparationFailure>() * 2,
        size_of::<PartitionCaptureReceiptLimits>() * 3,
        size_of::<HeldHostLimits>(),
        ...
        eredu_core::BackendFailure::source_retention_peak_bytes::<
            HeldSkip<crate::working_memory::CaptureRunHostError>,
        >()?,
        eredu_core::BackendFailure::source_retention_peak_bytes::<
            HeldSkip<PartitionCaptureCoordinationError>,
        >()?,
        size_of::<Row<'_, T>>() * 2,
        size_of::<Option<Row<'_, T>>>(),
        size_of::<Vec<Option<Row<'_, T>>>>(),
        size_of::<PreparedPartitionCaptureProgram<'_, T>>() * 2,
        size_of::<PreparedPartitionContiguousSource>(),
        size_of::<PreparedPartitionFragmentHostFunding>(),
        size_of::<PreparedPartitionContiguousRow<'_, T>>(),
        size_of::<Result<PreparedPartitionCaptureProgram<'_, T>, PartitionCaptureProgramError>>(),
        size_of::<Result<bool, PartitionCaptureProgramError>>(),
        ...
        size_of::<(
            &mut PreparedPartitionCaptureProgram<'_, T>,
            usize,
        ...
        size_of::<Option<&eredu_core::capture::CapturePrefillTransformPlan<'_>>>(),
