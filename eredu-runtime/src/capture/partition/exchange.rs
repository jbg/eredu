    ...
    <T::Completion as Completion>::Error: Send + Sync + 'static,
{
    ...
    pub fn exchange(
        ...
        let result = self.exchange_inner(local, ledger);
    ...
    fn exchange_inner(
        ...
        let mut payload = vec![0u32; width];
        ...
        let received = self.gather(&payload)?;
        let result = self.decode_payload(&gathered, &received, width, ledger);
    ...
    fn decode_payload(
        ...
        let mut delivery = self
            ...
            .into_delivery();
        for (rank, frame) in received.chunks_exact(width).enumerate() {
            let header = &headers[rank * HEADER_WORDS..(rank + 1) * HEADER_WORDS];
            ...
            if header[13] == 2 {
                delivery.receive(rank, &bytes[..length], ledger)?;
            }
        }
        Ok(delivery.finish(ledger)?)
    }
    ...
    fn header(&self, phase: u32, status: u32, length: u64) -> [u32; HEADER_WORDS] {
        ...
        words[5..13].copy_from_slice(&self.digest);
    ...
    fn validate_header(
        ...
        if frame[..5] != [MAGIC, VERSION, phase, rank as u32, self.participants as u32]
            || frame[5..13] != self.digest
        {
            return Err(PartitionCaptureExchangeError::Protocol(
    ...
    fn gather(&self, words: &[u32]) -> Result<Vec<u32>, PartitionCaptureExchangeError> {
        gather_capture_words(self.transport, self.wait, self.participants, words)
    }
}
...
pub(super) fn gather_capture_words<T: PartitionCaptureTransport>(
    ...
    participants: usize,
    words: &[u32],
) -> Result<Vec<u32>, PartitionCaptureExchangeError>
where
    T::Error: Send + Sync + 'static,
    <T::Completion as Completion>::Error: Send + Sync + 'static,
{
    ...
    let gathered = transport
        ...
        .map_err(BackendFailure::from_error)?;
    if gathered.len() != words.len() * participants {
        return Err(PartitionCaptureExchangeError::Protocol(
