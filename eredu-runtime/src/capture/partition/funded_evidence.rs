...
enum Cause {
    ...
    #[error(transparent)]
    Ownership(#[from] receipt::ReceiptConstructionCause),
}
#[derive(Debug, thiserror::Error)]
...
enum Charges<'a> {
    Complete {
        charged: CaptureUsage,
        quota: &'a mut dyn CaptureReservation,
    },
...
impl Charges<'_> {
    fn charged(
        ...
        match self {
            ...
            Self::Contiguous(source) => source
                .fragment_record_charge(receipt, producer, fragment)
                .map(|(_, charged, _)| charged),
        }
    }
...
impl PreparedPartitionCaptureEvidence {
    fn preparation_control_bytes(receipt: &PartitionCaptureReceiptPlan) -> Option<usize> {
        let producers = receipt.producers();
        let controls = [
            size_of::<Self>() * 2,
            size_of::<PartitionCaptureEvidence>() * 2,
            size_of::<PartitionCaptureContributionRecord>() * 2,
            size_of::<PartitionCaptureRegion>() * 4,
            size_of::<PartitionCaptureContext>() * 2,
            size_of::<PartitionCaptureEvidenceError>(),
            size_of::<Cause>(),
            ...
            size_of::<(
                &PartitionCaptureReceiptPlan,
                &mut PreparedPartitionFragmentAllowance,
                &HostMetadataFunding,
            )>(),
            size_of::<(
                &PartitionCaptureReceiptPlan,
            ...
            size_of::<bool>(),
            size_of::<RoutedUnitCaptureOwnership>() * 2,
            size_of::<RoutedUnitCaptureProvenance>() * 2,
            size_of::<Result<RoutedUnitCaptureOwnership, receipt::ReceiptConstructionCause>>(),
            size_of::<Vec<[u64; 2]>>(),
            size_of::<Option<RoutedUnitCaptureProvenance>>(),
            ...
            size_of::<
                std::iter::Enumerate<
                    std::slice::Iter<'_, eredu_core::capture::CaptureFragmentGeometry>,
                >,
            >(),
            size_of::<[Vec<u64>; 4]>() * 2,
            size_of::<Result<PartitionCaptureRegion, eredu_nn::Error>>(),
        ];
        controls
            .into_iter()
            .try_fold(size_of_val(&controls), usize::checked_add)
    }
    fn routed_record_control_bytes() -> Option<usize> {
        let parts = [
            size_of::<(
                &mut Self,
                &PartitionCaptureReceiptPlan,
            ...
            size_of::<Result<(), PartitionCaptureEvidenceError>>(),
            size_of::<(usize, u64)>(),
            size_of::<std::slice::Iter<'_, [u64; 2]>>(),
        ];
        parts
            .into_iter()
            .try_fold(size_of_val(&parts), usize::checked_add)
    }
    pub(in crate::capture::partition) fn execution_metadata_bytes(
        receipt: &PartitionCaptureReceiptPlan,
        run_length: usize,
    ) -> Option<usize> {
        use eredu_nn::workspace::WorkspaceContext as W;
        let mut bytes = Self::preparation_control_bytes(receipt)?
            .checked_add(
                receipt
                    .context()
                    .copy_metadata_bytes_with_run_length(run_length)?,
            )?
            .checked_add(W::metadata_string_bytes(receipt.identity().len())?)?;
        let mut producers = 0usize;
        let mut fragments = 0usize;
        for (producer, projection) in receipt.producers() {
            producers = producers.checked_add(1)?;
            fragments = fragments.checked_add(projection.fragments().len())?;
            for fragment in projection.fragments() {
                for region in [fragment.local(), fragment.destination()] {
                    for length in [
                        region.starts.len(),
                        region.ends.len(),
                        region.strides.len(),
                        ...
                        bytes = bytes.checked_add(W::metadata_vec_bytes::<u64>(length)?)?;
                    }
                }
                if let Some(ownership) = receipt.routed_producer(producer) {
                    let maximum = ownership
                        .maximum_source_rows(
                            projection.global_shape()[0],
                            projection.global_shape()[1],
                        )
                        .ok()?
                        .min(receipt.max_record_bytes());
                    bytes = bytes
                        .checked_add(W::metadata_vec_bytes::<[u64; 2]>(
                            usize::try_from(maximum).ok()?,
                        )?)?
                        .checked_add(ownership.coordinates.experts().clone_metadata_bytes()?)?
                        .checked_add(ownership.coordinates.units().clone_metadata_bytes()?)?
                        .checked_add(Self::routed_record_control_bytes()?)?;
                }
            }
        }
        bytes
            .checked_add(W::metadata_vec_bytes::<usize>(producers)?)?
            .checked_add(W::metadata_vec_bytes::<PartitionCaptureContributionRecord>(
                fragments,
            )?)?
            .checked_add(W::metadata_vec_bytes::<bool>(fragments)?)
    }
    pub(crate) fn charged(&self) -> CaptureUsage {
        self.charged
    }
    pub(crate) fn prepare(
        ...
        quota: &mut dyn CaptureReservation,
    ...
    pub(crate) fn prepare_contiguous(
        receipt: &PartitionCaptureReceiptPlan,
        allowance: &mut PreparedPartitionFragmentAllowance,
        metadata: &HostMetadataFunding,
    ) -> Result<Self, PartitionCaptureEvidenceError> {
    ...
    fn prepare_sources(
        receipt: &PartitionCaptureReceiptPlan,
        mut charges: Charges<'_>,
        metadata: &HostMetadataFunding,
    ) -> Result<Self, PartitionCaptureEvidenceError> {
        let source = receipt.shared_plan_source().clone();
        let error = |cause| PartitionCaptureEvidenceError {
            cause,
        ...
        metadata
            .reserve_metadata(
                Self::preparation_control_bytes(receipt)
        ...
        admitted
            ...
            .map_err(|cause| error(cause.into()))?;
        if matches!(&charges, Charges::Contiguous(allowance) if !allowance.matches(receipt)) {
        ...
        for (producer, projection) in producers {
            if receipt.routed_producer(producer).is_none() && projection.fragments().len() > 1
                || receipt.routed_producer(producer).is_some()
                    && matches!(&charges, Charges::Complete { .. })
            {
            ...
            if matches!(&charges, Charges::Complete { .. })
                && (receipt.combination() != PartitionCaptureCombination::Disjoint
                    ...
                    || projection.global_shape() != projection.local_shape())
            {
                return Err(error(Cause::Source(
                    "expected one complete nonrouted producer",
                )));
            }
            for fragment in 0..projection.fragments().len() {
                charges
                    .charged(receipt, producer, fragment)
                    .ok_or_else(|| {
                        error(Cause::Source(
                            "fragment charge lacks its retained native source",
                        ))
                    })?;
            }
            producer_count = producer_count
                .checked_add(1)
                .ok_or_else(|| error(Cause::Source("producer count overflow")))?;
            fragment_count = fragment_count
                .checked_add(projection.fragments().len())
                .ok_or_else(|| error(Cause::Source("fragment count overflow")))?;
        }
        if producer_count == 0
            || (matches!(&charges, Charges::Complete { .. }) && producer_count != 1)
        {
            return Err(error(Cause::Source("producer count differs")));
        }
        let charged = match &charges {
            Charges::Complete { charged, .. } => *charged,
            Charges::Contiguous(allowance) => allowance
                .assembly_record_charge(receipt)
                .map_err(|cause| error(cause.into()))?,
        };
        ...
        if let Some(reason) = charges.reserve(cost).map_err(|cause| error(cause.into()))? {
            return Err(error(match reason {
                CaptureSkipReason::Limit { budget, cumulative } => {
                    CaptureError::Limit { budget, cumulative }.into()
                }
                _ => Cause::Source("prepaid evidence reservation rejected"),
            }));
        }
        let context = receipt::copy_context(receipt.context(), metadata)
            .map_err(|cause| error(cause.into()))?;
        let identity = metadata
            .metadata_string(format_args!("{}", receipt.identity()))
            .map_err(|cause| error(cause.into()))?;
        let mut producer_rows = metadata
            .metadata_vec(producer_count)
            .map_err(|cause| error(cause.into()))?;
        let mut contributions = metadata
            .metadata_vec(fragment_count)
            .map_err(|cause| error(cause.into()))?;
        let mut routed_pending = metadata
            .metadata_vec(fragment_count)
            .map_err(|cause| error(cause.into()))?;
        for (producer, projection) in receipt.producers() {
            ...
            for (fragment_index, fragment) in projection.fragments().iter().enumerate() {
                    ...
                    let ranges = metadata
                        .metadata_vec(
                            usize::try_from(maximum)
                                .map_err(|_| error(Cause::Source("range capacity overflow")))?,
                        )
                        .map_err(|cause| error(cause.into()))?;
                    Some(RoutedUnitCaptureProvenance {
                        ownership: receipt::copy_routed_ownership(ownership, metadata)
                            .map_err(|cause| error(cause.into()))?,
                ...
                contributions.push(PartitionCaptureContributionRecord {
                    ...
                    local: copy_region(fragment.local(), metadata)
                        .map_err(|cause| error(cause.into()))?,
                    destination: copy_region(fragment.destination(), metadata)
                        .map_err(|cause| error(cause.into()))?,
                    charged: charges
                        .charged(receipt, producer, fragment_index)
    ...
    pub(crate) fn record_routed_ranges(
        &mut self,
        receipt: &PartitionCaptureReceiptPlan,
        producer: usize,
        fragment: usize,
        ...
        metadata
            .reserve_metadata(
                Self::routed_record_control_bytes()
                    .ok_or_else(|| error(Cause::Source("range binding controls overflow")))?,
            )
            .map_err(|cause| error(cause.into()))?;
        if !receipt.shared_plan_source().same_storage(&source)
            || self.value.receipt_plan_identity != receipt.identity()
            || &self.value.context != receipt.context()
        {
            return Err(error(Cause::Source("range receipt differs")));
        }
        let index = self
            .value
            .contributions
            .iter()
            .enumerate()
            .filter(|(_, row)| row.producer_rank == producer)
            .nth(fragment)
            .map(|(index, _)| index)
            .ok_or_else(|| error(Cause::Source("range fragment missing")))?;
        if self.routed_pending.get(index) != Some(&true) {
            return Err(error(Cause::Source("range fragment already spent")));
        }
        self.routed_pending[index] = false;
        let destination = self.value.contributions[index]
            .routed
            .as_mut()
            .ok_or_else(|| error(Cause::Source("range ownership missing")))?;
        let ownership = receipt
            .routed_producer(producer)
            .ok_or_else(|| error(Cause::Source("range producer missing")))?;
        let projection = receipt
            .producer(producer)
            .ok_or_else(|| error(Cause::Source("range projection missing")))?;
        if destination.ownership != *ownership
            || ranges.len() > destination.source_token_ranges.capacity()
        {
            return Err(error(Cause::Source("range destination differs")));
        }
        let maximum = ownership
            .maximum_source_rows(projection.global_shape()[0], projection.global_shape()[1])
            .map_err(|e| error(e.into()))?;
        let mut end = 0;
        for &[start, next] in ranges {
            if start != end || next <= start || next > maximum {
                return Err(error(Cause::Source("range coverage differs")));
            }
            end = next;
        }
        if ownership.source_peer.is_none() && end != projection.global_shape()[0] {
            return Err(error(Cause::Source("range source incomplete")));
        }
        destination.source_token_ranges.extend_from_slice(ranges);
        Ok(())
    }
}
fn copy_region(
    ...
    starts.extend_from_slice(&source.starts);
    ends.extend_from_slice(&source.ends);
    strides.extend_from_slice(&source.strides);
    shape.extend_from_slice(&source.shape);
    Ok(PartitionCaptureRegion {
        starts,
        ends,
        strides,
        shape,
