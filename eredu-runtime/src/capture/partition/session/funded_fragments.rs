...
impl PreparedPartitionFragmentLoan {
    ...
    pub fn quota_mut(&mut self) -> &mut CaptureQuota {
        &mut self.quota
    }
...
impl PreparedPartitionFragmentAllowance {
    fn preparation_control_bytes<T: PartitionCaptureTransport>() -> Option<usize> {
        let parts = [
            size_of::<Self>() * 2,
            size_of::<Row>() * 2,
            size_of::<PartitionCaptureFragmentSource<'_>>(),
            size_of::<PartitionCaptureFragmentGeometry<'_>>(),
            size_of::<Sources<'_, '_>>(),
            size_of::<voted::Source<'_>>(),
            size_of::<std::ops::Range<usize>>(),
            size_of::<Result<Self, PartitionCaptureFragmentAllowanceError>>(),
            size_of::<PartitionCaptureFragmentAllowanceError>(),
            size_of::<Cause>(),
            size_of::<ReceiptWorkCosts>() * 2,
            size_of::<CaptureQuota>() * 2,
            ...
            size_of::<(
                &T,
                &mut PartitionCaptureReceiptPlan,
    ...
    /// Exact actual fragment table and fixed preparation worker population.
    /// Global capture quota and destination H are independent source contracts.
    pub fn preparation_metadata_bytes<T: PartitionCaptureTransport>(
        receipt: &PartitionCaptureReceiptPlan,
    ) -> Option<usize> {
        let fragments = receipt
            .producers()
            .try_fold(0usize, |n, (_, p)| n.checked_add(p.fragments().len()))?;
        let mut bytes = Self::preparation_control_bytes::<T>()?
        ...
        {
            bytes = bytes.checked_add(routed::control_bytes()?)?;
        }
        Some(bytes)
    }
    ...
    pub fn prepare<T: PartitionCaptureTransport>(
        ...
        sources: &[PartitionCaptureFragmentSource<'_>],
        metadata: &HostMetadataFunding,
    ...
    fn prepare_sources<T: PartitionCaptureTransport>(
        transport: &T,
        receipt: &mut PartitionCaptureReceiptPlan,
        sources: Sources<'_, '_>,
        metadata: &HostMetadataFunding,
        ...
        <T::Completion as Completion>::Error: Send + Sync + 'static,
    {
        ...
        metadata
            .reserve_metadata(
                Self::preparation_control_bytes::<T>()
        ...
        if if sources.routed() {
            ...
            return Err(error(Cause::Source(
                "fragment delivery requires a floating tensor transform",
            )));
        }
        let raw;
        let transform = if receipt.combination() == PartitionCaptureCombination::SumF64ToF32 {
            raw = super::super::sum::native_transform(&selection.transform);
            &raw
        } else {
            &selection.transform
        };
        ...
        for (rank, projection) in receipt.producers() {
            for (fragment, geometry) in projection.fragments().iter().enumerate() {
                let row = sources
                    .get(cursor)
                    .ok_or_else(|| error(Cause::Source("missing actual fragment source")))?;
                let matched = match row.geometry {
                    voted::Geometry::Tensor {
                        shape,
                        slice,
                        transform: actual,
                    } => {
                        shape == projection.local_shape()
                            && slice == geometry.local()
                            && actual == transform
                    }
                    voted::Geometry::Routed(request) => {
                        routed::matches(receipt, rank, fragment, request)
                    }
                };
                if row.producer != rank
                ...
                {
                    return Err(error(Cause::Source(
                        "native fragment identity, geometry or transform differs",
                    )));
                }
                cursor += 1;
            }
        ...
        if cursor != sources.len() {
            return Err(error(Cause::Source("unexpected native fragment source")));
        }
        ...
        for index in 0..sources.len() {
            let row = sources.get(index).expect("source range");
            rows.push(Row {
        ...
        let mut cursor = 0usize;
        let costs = receipt_work_costs(
            transport,
            receipt,
            |_, producer, fragment| {
                let row = rows
                    .get(cursor)
                    .ok_or_else(|| invalid("missing prepared native fragment"))?;
            ...
            |_| {
                ...
                if sources.routed() {
                    ...
                    Err(invalid("dense fragments cannot create routed metadata"))
                }
            },
        )
        .map_err(|cause| error(cause.into()))?;
        if cursor != rows.len() || receipt.identity().len() != 64 {
            return Err(error(Cause::Source(
                "finalized receipt differs from its sources",
            )));
        }
        identity.push_str(receipt.identity());
        let quota = ledger
            .reserve_quota(costs.global)
        ...
        Ok(Self {
            ...
            quota,
            global: costs.global,
            descriptor: costs.descriptor,
            rank: transport.capture_rank(),
    ...
    pub fn take_local_fragment(
        ...
        self.metadata
            .reserve_metadata(
                Self::local_fragment_control_bytes()
                    .ok_or_else(|| error(Cause::Source("fragment loan controls overflow")))?,
            )
            .map_err(|cause| error(cause.into()))?;
        if !self.matches(receipt) {
            return Err(error(Cause::Source("fragment receipt source differs")));
        }
        let row = self
            .rows
            .iter_mut()
            .find(|row| row.producer == self.rank && row.fragment == fragment)
            .ok_or_else(|| error(Cause::Source("rank has no such native fragment")))?;
        if row.taken {
            return Err(error(Cause::Source("native fragment already attempted")));
        }
        row.taken = true;
        if row.dtype.as_ref() != Some(dtype)
            || row.estimate.capture != actual.capture
            || row.estimate.generated_creation_bytes != actual.generated_creation_bytes
        {
            return Err(error(Cause::Source(
                "actual fragment scalar or estimate differs",
            )));
        }
        let quota = self
            .quota
            .reserve_quota(actual.capture)
            .map_err(|cause| error(cause.into()))?;
        Ok(PreparedPartitionFragmentLoan {
            quota,
    ...
    pub(crate) fn quota_mut(&mut self) -> &mut CaptureQuota {
        &mut self.quota
    }
}
...
impl PreparedPartitionFragmentAllowance {
    ...
    pub(crate) fn fragment_source(
        ...
        producer: usize,
        fragment: usize,
    ) -> Option<(&TensorDtype, PartitionCaptureNativeEstimate)> {
        self.rows
            .iter()
            .find(|row| row.producer == producer && row.fragment == fragment)
            .and_then(|row| row.dtype.as_ref().map(|dtype| (dtype, row.estimate)))
    }
}
...
impl PreparedPartitionFragmentAllowance {
    ...
    pub(crate) fn fragment_record_charge(
        &self,
        receipt: &PartitionCaptureReceiptPlan,
        producer: usize,
        fragment: usize,
        ...
        Some((
            dtype.clone(),
            self.fragment_record_overhead(receipt, producer)?
    ...
    pub(crate) fn fragment_record_overhead(
        &self,
        receipt: &PartitionCaptureReceiptPlan,
        producer: usize,
    ) -> Option<CaptureUsage> {
        ...
        let point = source.points().get(index)?;
        let projection = receipt.producer(producer)?;
        let metadata = super::super::fragment_metadata_usage(
            selection,
            point,
            projection.global_shape().len(),
        )
        .ok()?
        .checked_add(
...
impl PreparedPartitionFragmentAllowance {
    pub(crate) fn local_fragment_control_bytes() -> Option<usize> {
        let parts = [
            size_of::<(
