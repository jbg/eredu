...
impl PreparedPartitionCaptureAllowance {
    /// Complete metadata debit for the selected full producer allowance.
    /// Uses the actual receipt and local rank; a remote rank takes its exclusive
    /// acknowledgment once. This query creates no logical or physical credits.
    pub fn execution_metadata_bytes<T: PartitionCaptureTransport>(
        ...
        let controls = Self::prepare_control_bytes::<T>()?;
        if local_rank == producer {
            Some(controls)
        } else {
            controls.checked_add(Self::remote_charge_control_bytes()?)
        }
    }
    fn prepare_control_bytes<T: PartitionCaptureTransport>() -> Option<usize> {
        let parts = [
            size_of::<Self>() * 2,
            size_of::<Cause>(),
    ...
    fn remote_charge_control_bytes() -> Option<usize> {
        Some(
            size_of::<PreparedPartitionRemoteCharge<'_>>()
                + size_of::<
                    Result<PreparedPartitionRemoteCharge<'_>, PartitionCaptureAllowanceError>,
                >()
                + size_of::<PartitionCaptureAllowanceError>()
                + size_of::<&mut Self>(),
        )
        ...
        <T::Completion as Completion>::Error: Send + Sync + 'static,
    {
        ...
        let (producer, projection) = producers
            .next()
            .ok_or_else(|| error(Cause::Source("receipt has no producer")))?;
            ...
            || transport.capture_rank() >= receipt.world_size()
        {
            return Err(error(Cause::Source(
                "receipt is not the selected complete producer",
            )));
        }
        drop(producers);
        ...
        let costs = receipt_work_costs(
            ...
            |_| {
                Err(invalid(
                    "complete producer unexpectedly requires sparse metadata",
                ))
            },
        )
        .map_err(|cause| error(cause.into()))?;
        ...
        Ok(Self {
            ...
            local_rank: transport.capture_rank(),
            producer,
            estimate,
...
impl PreparedPartitionCaptureAllowance {
    pub(crate) fn take_remote_charge(
        &mut self,
    ) -> Result<PreparedPartitionRemoteCharge<'_>, PartitionCaptureAllowanceError> {
        let controls =
            Self::remote_charge_control_bytes().ok_or_else(|| PartitionCaptureAllowanceError {
                cause: Cause::Source("remote charge controls overflow"),
                _source: self.source.clone(),
                _metadata: self.metadata.clone(),
            })?;
        self.metadata.reserve_metadata(controls).map_err(|cause| {
            PartitionCaptureAllowanceError {
                cause: cause.into(),
                _source: self.source.clone(),
                _metadata: self.metadata.clone(),
            }
        ...
        if self.remote_attempted || self.local_rank == self.producer {
            return Err(PartitionCaptureAllowanceError {
                cause: Cause::Source("remote reservation is unavailable or spent"),
                _source: self.source.clone(),
                _metadata: self.metadata.clone(),
            });
        }
        ...
        Ok(PreparedPartitionRemoteCharge {
            source: &self.source,
            index: self.selection_index,
            usage: self.estimate.capture,
