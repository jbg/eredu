...
enum SourceMode { Selected, Contiguous, Routed }
struct SourceVote { dtype:Option<TensorDtype>, ranks:Option<Vec<PartitionCaptureRankSource>> }
pub(crate) struct PartitionCaptureRoutedVote { pub(crate) dtype:TensorDtype, pub(crate) ranks:Vec<PartitionCaptureRankSource> }
...
impl<T: PartitionCaptureTransport> std::fmt::Debug for PreparedPartitionCaptureCoordination<'_, T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PreparedPartitionCaptureCoordination")
            .field("epoch", &self.epoch).field("included", &self.included).finish_non_exhaustive()
    }
}
    ...
    <T::Completion as Completion>::Error: Send + Sync + 'static,
{
    ...
    pub fn prepare(
        ...
        if participants == 0 || rank >= participants || u32::try_from(participants).is_err()
            || context.invocation.is_some()
            || context.capture_plan_identity != source.admission().identity()
            || context.prediction >= source.admission().request().max_predictions
            || [&context.artifact_identity, &context.execution_identity,
                &context.run_identity, &context.capture_plan_identity]
                .into_iter().chain(context.overlay_identity.iter())
                .any(|value| value.is_empty() || value.len() > 256)
        { return Err(error(Cause::Source("context or participant source differs"))); }
        transport.ensure_capture_active().map_err(PartitionCaptureExchangeError::from)
            .map_err(|cause| error(cause.into()))?;
        let wait = transport.capture_wait().map_err(|cause| error(cause.into()))?;
        if !T::Completion::supports_cancellation(wait.cancellation()) {
            return Err(error(Cause::Source("completion cannot apply the selected disposition")));
        }
        ...
        let scheduled = source.admission().plan().selections.iter()
            .filter(|selection| selection.schedule.includes(context.phase, context.prediction)).count();
        ...
        let reserved = super::super::exchange::gather_usage(transport, participants, 16)
            .and_then(|usage| usage.checked_add(CaptureUsage { host_bytes: 8192, ..Default::default() }))
            .and_then(|usage| usage.checked_add(source_votes))
            .and_then(|usage| usage.checked_mul(participants as u64))
            .map_err(|cause| error(cause.into()))?;
        ledger.reserve_quota(reserved).map_err(|cause| error(cause.into()))?;
        ...
        let context = super::super::receipt::copy_context(context, metadata)
            .map_err(|cause| error(cause.into()))?;
        Ok(Self { transport, context, wait, rank, participants, epoch, digest,
            next: 0, included: 0, intervention_last:None, reserved, source: source.clone(), metadata: metadata.clone() })
    }
    ...
    pub(crate) fn coordinate_source(&self, index: usize,
        receipt: Option<&PartitionCaptureReceiptPlan>, producer: usize,
        local: Option<&TensorDtype>, skipped: Option<&CaptureSkipReason>,
        usage: CaptureUsage, ledger: &CaptureLedger,
    ) -> Result<Option<TensorDtype>, PartitionCaptureCoordinationError> {
        self.coordinate_source_worker(index,receipt,producer,local,skipped,usage,ledger,SourceMode::Selected,false).map(|vote|vote.dtype)
    }
    ...
    pub(crate) fn coordinate_contiguous_source(&self,index:usize,receipt:&PartitionCaptureReceiptPlan,
        local:Option<&TensorDtype>,usage:CaptureUsage,ledger:&CaptureLedger)
        ->Result<TensorDtype,PartitionCaptureCoordinationError> {
        self.metadata.reserve_metadata(size_of::<(&Self,usize,&PartitionCaptureReceiptPlan,
        ...
        let producer=receipt.producers().find(|(_,p)|!p.fragments().is_empty())
            .or_else(||receipt.producers().next()).map(|(rank,_)|rank)
            .ok_or_else(||self.error(Cause::Source("contiguous receipt has no actual producers")))?;
        self.coordinate_source_worker(index,Some(receipt),producer,local,None,usage,ledger,SourceMode::Contiguous,false)?.dtype
    ...
    pub(crate) fn coordinate_routed_source(&self,index:usize,receipt:&PartitionCaptureReceiptPlan,present:bool,
        local:Option<&TensorDtype>,usage:CaptureUsage,ledger:&CaptureLedger)
        ...
        if !receipt.shared_plan_source().is_some_and(|source|source.same_storage(&self.source))
            ...
            || local.is_some()&&!present || receipt.producer(self.rank).is_some()&&!present {
            return Err(self.error(Cause::Source("sparse receipt source differs")));
        }
        let producer=receipt.producers().next().map(|(rank,_)|rank)
            .ok_or_else(||self.error(Cause::Source("sparse receipt has no actual producers")))?;
        let vote=self.coordinate_source_worker(index,Some(receipt),producer,local,None,usage,ledger,SourceMode::Routed,present)?;
        Ok(PartitionCaptureRoutedVote{dtype:vote.dtype.ok_or_else(||self.error(Cause::Source("sparse source has no result scalar witness")))?,
            ranks:vote.ranks.ok_or_else(||self.error(Cause::Source("sparse source has no witness table")))?})
    }

    fn coordinate_source_worker(&self,index:usize,receipt:Option<&PartitionCaptureReceiptPlan>,producer:usize,
        local:Option<&TensorDtype>,skipped:Option<&CaptureSkipReason>,usage:CaptureUsage,ledger:&CaptureLedger,
        mode:SourceMode,present:bool)->Result<SourceVote,PartitionCaptureCoordinationError> {
        let producers=receipt.map(PartitionCaptureReceiptPlan::producers);
        let controls = [size_of::<SourceVote>(),size_of::<Result<SourceVote,PartitionCaptureCoordinationError>>(),size_of::<Result<SourceVote,PartitionCaptureExchangeError>>(),size_of::<SourceMode>(),size_of::<Option<Vec<PartitionCaptureRankSource>>>(),size_of::<PartitionCaptureRankSource>(),size_of_val(&producers)*2,size_of::<(bool,usize)>(),size_of::<Sha256>(), size_of::<sha2::digest::Output<Sha256>>(),
            size_of::<[u32; 8]>(), size_of::<[u32; 16]>() * 2,
            size_of::<PartitionCaptureFrame<'_>>(), size_of::<PartitionCaptureBuffer<u32>>(),
            ...
            size_of::<PartitionCaptureCoordinationError>(), size_of::<Cause>(),
            size_of::<HashWriter<'_>>(), size_of::<serde_json::Serializer<&mut HashWriter<'_>>>(),
            crate::capture::RECORD_ENCODING_CONTROL_BYTES,
            size_of::<std::iter::Enumerate<std::slice::ChunksExact<'_, u32>>>(),
            size_of::<std::slice::ChunksExact<'_, u8>>(), size_of::<Option<TensorDtype>>() * 3,
            size_of::<(&Self, usize, Option<&PartitionCaptureReceiptPlan>, usize,
                Option<&TensorDtype>, Option<&CaptureSkipReason>, CaptureUsage, &CaptureLedger,SourceMode,bool)>(),
        ];
        ...
        if self.next_scheduled() != Some(index) || producer >= self.participants
            || receipt.is_some() == skipped.is_some()
            || !matches!(local, None | Some(TensorDtype::F16 | TensorDtype::F32 | TensorDtype::Bf16)) {
        ...
        match mode {SourceMode::Selected=>{},SourceMode::Contiguous=>digest.update(b"all contiguous fragment producers\0"),SourceMode::Routed=>digest.update(b"actual sparse result witnesses\0")};
        digest.update((index as u64).to_le_bytes());
        digest.update((producer as u64).to_le_bytes());
        if let Some(receipt) = receipt { digest.update(receipt.identity().as_bytes()); }
        ...
        let scalar = if skipped.is_some() { 0 } else { match local {
            None => 0, Some(TensorDtype::F16) => 1, Some(TensorDtype::F32) => 2,
            Some(TensorDtype::Bf16) => 3, _ => unreachable!("validated scalar"),
        } };
        let mut routed_ranks=if mode==SourceMode::Routed {
            Some(self.metadata.metadata_vec(receipt.expect("routed source has receipt").producers().count()).map_err(|e|self.error(e.into()))?)
        }else{None};
        let result = (|| -> Result<SourceVote, PartitionCaptureExchangeError> {
            let words = super::super::exchange::protocol_header(self.rank, self.participants,
                &words_digest, 3, scalar | if mode==SourceMode::Routed&&present {8}else{0}, index as u64);
            let frame = PartitionCaptureFrame::new(PartitionCaptureFrameKind::Source,
                self.rank, self.participants, &words, 16)?;
            let gathered = super::super::exchange::gather_capture_words(self.transport, self.wait, &frame)?;
            for (rank, header) in gathered.chunks_exact(16).enumerate() {
                super::super::exchange::validate_protocol_header(header, rank, self.participants, &words_digest, 3)?;
                let source=header[13];
                let valid=if mode==SourceMode::Routed {source&!11==0&&(source&3==0||source&8!=0)
                    && (receipt.is_none_or(|receipt|receipt.producer(rank).is_none())||source&8!=0)} else{source<=3};
                if !valid || header[14] != index as u32 || header[15] != (index as u64 >> 32) as u32 {
                    return Err(PartitionCaptureExchangeError::Protocol("source scalar or selection differs"));
                }
            }
            let actual=if mode==SourceMode::Routed {gathered.chunks_exact(16).map(|header|header[13]&3).find(|scalar|*scalar!=0).unwrap_or(0)}
                else{gathered[producer*16+13]};
            if skipped.is_some() {
                if gathered.chunks_exact(16).any(|header| header[13] != 0) {
                    return Err(PartitionCaptureExchangeError::Protocol("skipped source has an active scalar"));
                }
                return Ok(SourceVote{dtype:None,ranks:None});
            }
            if mode==SourceMode::Contiguous && receipt.is_none_or(|receipt|receipt.producers().any(|(rank,projection)|
                !projection.fragments().is_empty() && gathered[rank*16+13]==0)) {
                return Err(PartitionCaptureExchangeError::Protocol("a nonempty fragment producer has no scalar witness"));
            }
            if actual == 0 || gathered.chunks_exact(16).any(|header| {let scalar=header[13]&3;scalar!=0&&scalar!=actual}) {
                return Err(PartitionCaptureExchangeError::Protocol("selected producer scalar is missing or inconsistent"));
            }
            let dtype=|scalar|match scalar {1=>Some(TensorDtype::F16),2=>Some(TensorDtype::F32),3=>Some(TensorDtype::Bf16),_=>None};
            if let Some(ranks)=&mut routed_ranks {for (producer,_) in receipt.expect("routed source receipt").producers(){
                ranks.push(PartitionCaptureRankSource{producer,dtype:dtype(gathered[producer*16+13]&3)});
            }}
            Ok(SourceVote{dtype:dtype(actual),ranks:routed_ranks.take()})
        })();
        result.map_err(|cause| { self.transport.fail_capture_exchange(&cause); self.error(cause.into()) })
    }
    ...
    pub fn include(
        &mut self, receipt: &PartitionCaptureReceiptPlan,
        dtype: TensorDtype, usage: CaptureUsage,
    ) -> Result<(), PartitionCaptureCoordinationError> {
        let controls = size_of::<(&Self, &PartitionCaptureReceiptPlan, TensorDtype, CaptureUsage)>()
            + size_of::<Result<(), PartitionCaptureCoordinationError>>()
            + size_of::<PartitionCaptureCoordinationError>() + size_of::<[u8; 8]>();
        self.metadata.reserve_metadata(controls).map_err(|cause| self.error(cause.into()))?;
        ...
        self.digest.update((actual.selection_index as u64).to_le_bytes());
        self.digest.update(receipt.identity().as_bytes());
        self.digest.update([match dtype { TensorDtype::F16 => 0, TensorDtype::F32 => 1,
            TensorDtype::Bf16 => 2, _ => unreachable!("checked scalar format") }]);
        hash_usage(&mut self.digest, usage);
        self.next = actual.selection_index.checked_add(1)
            .ok_or_else(|| self.error(Cause::Source("selection cursor overflow")))?;
        self.included = self.included.checked_add(1)
            .ok_or_else(|| self.error(Cause::Source("receipt count overflow")))?;
        Ok(())
    }

    pub(crate) fn include_skip(&mut self, index: usize, reason: &CaptureSkipReason)
        -> Result<(), PartitionCaptureCoordinationError> {
        let controls = size_of::<(&Self, usize, &CaptureSkipReason)>()
            + size_of::<PartitionCaptureCoordinationError>()
            + size_of::<Result<(), PartitionCaptureCoordinationError>>()
            + size_of::<HashWriter<'_>>() + size_of::<serde_json::Serializer<&mut HashWriter<'_>>>()
            + crate::capture::RECORD_ENCODING_CONTROL_BYTES;
        self.metadata.reserve_metadata(controls).map_err(|cause| self.error(cause.into()))?;
        if self.next_scheduled() != Some(index) || !matches!(reason, CaptureSkipReason::Limit { .. }) {
        ...
        serde_json::to_writer(&mut HashWriter(&mut self.digest), reason)
            .expect("closed skip and infallible digest writer");
        self.next = index.checked_add(1).ok_or_else(|| self.error(Cause::Source("selection cursor overflow")))?;
        self.included = self.included.checked_add(1).ok_or_else(|| self.error(Cause::Source("receipt count overflow")))?;
        Ok(())
    }
    ...
    pub(crate) fn include_intervention(&mut self,work:&PreparedPartitionIntervention<'_,T>)
        ->Result<(),PartitionCaptureCoordinationError> {
        self.metadata.reserve_metadata(size_of::<(&mut Self,&PreparedPartitionIntervention<'_,T>)>()
            +size_of::<Result<(),PartitionCaptureCoordinationError>>())
            .map_err(|cause|self.error(cause.into()))?;
        if self.next_scheduled().is_some()||work.epoch()!=self.epoch
            ...
            ||self.intervention_last.is_some_and(|last|last>=work.operation()) {
            return Err(self.error(Cause::Source("original intervention transcript order or frame differs")));
        }
        self.digest.update(b"original intervention\0");
        self.digest.update((work.operation() as u64).to_le_bytes());
        self.digest.update(work.descriptor());hash_usage(&mut self.digest,work.global_reserved());
        self.intervention_last=Some(work.operation());Ok(())
    }

    /// Actual global logical reservation, never refundable or transferable.
    pub const fn global_reserved(&self) -> CaptureUsage { self.reserved }
    ...
    pub fn coordinate(self, ledger: &CaptureLedger) -> Result<(), PartitionCaptureCoordinationError> {
        let parts = [
            size_of::<Self>(), size_of::<Result<(), PartitionCaptureExchangeError>>(),
            size_of::<Result<(), PartitionCaptureCoordinationError>>(),
            size_of::<PartitionCaptureCoordinationError>(),
            size_of::<[u32; 16]>(), size_of::<[u8; 32]>(),
            size_of::<PartitionCaptureFrame<'_>>(), size_of::<PartitionCaptureBuffer<u32>>(),
            size_of::<(&T, BoundedCompletionWait, usize, usize, DistributedCommitEpoch, &[u8; 32])>(),
            size_of::<std::slice::ChunksExact<'_, u32>>() * 2,
            size_of::<std::slice::ChunksExact<'_, u8>>(),
            size_of::<sha2::digest::Output<Sha256>>(),
            size_of::<Sha256>(), size_of::<(&CaptureLedger, usize, CaptureUsage)>(),
        ];
        let controls = parts.into_iter().try_fold(size_of_val(&parts), usize::checked_add)
        ...
        super::coordination::exchange_coordination(self.transport, self.wait, self.rank,
            self.participants, self.epoch, &digest).map_err(|cause| {
    ...
    fn next_scheduled(&self) -> Option<usize> {
        self.source.admission().plan().selections.iter().enumerate().skip(self.next)
            .find_map(|(index, selection)| selection.schedule.includes(self.context.phase,
                self.context.prediction).then_some(index))
    }
    fn error(&self, cause: Cause) -> PartitionCaptureCoordinationError {
        PartitionCaptureCoordinationError { cause, _source: self.source.clone(),
...
fn same_forward(a: &PartitionCaptureContext, b: &PartitionCaptureContext) -> bool {
    a.artifact_identity == b.artifact_identity && a.execution_identity == b.execution_identity
        && a.run_identity == b.run_identity && a.overlay_identity == b.overlay_identity
        && a.capture_plan_identity == b.capture_plan_identity && a.phase == b.phase
        && a.prediction == b.prediction && a.forward_epoch == b.forward_epoch
