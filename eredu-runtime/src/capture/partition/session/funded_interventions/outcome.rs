...
impl PartitionInterventionOutcome<'_> {
    ...
    pub(crate) fn model_invocation(&self) -> Option<ModelInvocation> {
        self.model
    }
    ...
    pub(crate) fn expected(&self) -> Result<CaptureUsage, CaptureError> {
        self.baseline.checked_add(if self.member {
            self.local
        } else {
            CaptureUsage::default()
        })
    }
    pub(crate) fn final_charge(&self) -> Result<CaptureUsage, CaptureError> {
        self.baseline.checked_add(self.global)
    }
    pub(crate) fn additional(&self) -> CaptureUsage {
        self.global
    }
}
    ...
    <T::Completion as Completion>::Error: Send + Sync + 'static,
{
    ...
    pub(crate) fn deliver(
        &mut self,
        baseline: CaptureUsage,
    ) -> Result<PartitionInterventionOutcome<'_>, PartitionInterventionSourceError> {
        ...
        Ok(PartitionInterventionOutcome {
            ...
            local: self.local,
            global: self.global,
            member: self.source.members.contains(&self.rank),
        })
    }
}
