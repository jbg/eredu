...
impl PreparedPartitionAssemblyCharge<'_> {
    ...
    pub(crate) fn validate_at(
        ...
        self.validate_invocation(source, index, phase, prediction, None, None)
    }
...
impl PreparedPartitionFragmentAllowance {
    ...
    pub(crate) fn assembly_record_charge(
        &self,
        receipt: &PartitionCaptureReceiptPlan,
    ) -> Result<CaptureUsage, CaptureError> {
        if !self.matches(receipt) {
            return Err(invalid("assembly charge source differs"));
        }
        let mut charged = receipt
            .fragment_assembly_usage()?
            .ok_or_else(|| invalid("missing fragment assembly source"))?;
        for row in &self.rows {
            let (_, usage, _) = self
                .fragment_record_charge(receipt, row.producer, row.fragment)
                .ok_or_else(|| invalid("missing original fragment charge"))?;
            charged = charged.checked_add(usage)?;
        }
        Ok(charged)
    }
    pub(crate) fn take_assembly_charge(
        &mut self,
        receipt: &PartitionCaptureReceiptPlan,
        ...
        let parts = [
            ...
            size_of::<(&mut Self, &PartitionCaptureReceiptPlan)>(),
        ...
        if self.assembly_claimed || !self.matches(receipt) {
            return Err(error(Cause::Source(
                "assembly target is unavailable or spent",
            )));
        }
