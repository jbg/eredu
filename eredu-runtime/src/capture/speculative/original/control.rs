...
pub(super) fn owner_control_bytes() -> Option<usize> {
    Some(
        Layout::new::<[usize; 2]>()
            .extend(Layout::new::<crate::capture::CaptureHostOwner>())
            .ok()?
            .0
            .pad_to_align()
            .size(),
...
impl Saved {
    fn get(&self) -> &Payload {
        self.0.as_ref().expect("live saved capture authority")
    }
}
impl Drop for Saved {
    fn drop(&mut self) {
...
impl OriginalSpeculativeCapture {
    fn drained_control_boundary(&self) -> bool {
        self.checkpoint_ready
            ...
            && self.prefix.is_none()
            && self.reductions.is_none()
            && self.held_prefill.is_none()
            && self.records.is_empty()
            && self.reduction_geometry.is_none()
    }
    fn control_boundary(&self) -> Result<(), OriginalSpeculativeCaptureError> {
        if !self.drained_control_boundary() {
    ...
    fn control_frames() -> Option<usize> {
        let parts = [
            ...
            size_of::<Saved>(),
            size_of::<Authority>(),
            size_of::<SpeculativeActivationCheckpoint>(),
            size_of::<Prepared<'_>>(),
            size_of::<Box<Prepared<'_>>>(),
            size_of::<Box<dyn PreparedSpeculativeActivationRestore>>(),
            size_of::<Option<InterventionSelection>>(),
            size_of::<InterventionSelection>(),
            size_of::<Result<SpeculativeActivationCheckpoint, OriginalSpeculativeCaptureError>>(),
            size_of::<
                Result<
                    Box<dyn PreparedSpeculativeActivationRestore>,
    ...
    pub fn control_storage_bytes(&self) -> Option<u64> {
        ...
        let count = self
            .interventions
            .as_ref()
            .map_or(0, |value| value.scopes.len());
        let one = Self::control_frames()?
            .checked_add(WorkspaceContext::metadata_string_bytes(
                self.identity.len(),
            )?)?
    ...
    pub fn save_control(
        &self,
    ) -> Result<SpeculativeActivationCheckpoint, OriginalSpeculativeCaptureError> {
        self.reserve_control()?;
        self.control_boundary()?;
        let scopes = self
            .interventions
            .as_ref()
            .map_or(&[][..], |value| value.scopes.as_slice());
        let mut copied = self
            .funding
        ...
        let identity = self
            .funding
            .metadata_string(format_args!("{}", self.identity))
            .map_err(|cause| self.reject(cause))?;
    ...
    pub fn control_usage(&self) -> Result<CaptureUsage, OriginalSpeculativeCaptureError> {
        ...
        self.lineage
            .as_ref()
            .expect("validated source lineage")
            .ledger()
            .inspect_usage()
            .map_err(|cause| self.reject(self.funding.metadata_source(cause)))
    }
    ...
    pub fn prepare_readmitted_control<'a>(
        &'a mut self,
        plan: &eredu_core::speculative::AdmittedSpeculativeActivations,
    ...
    ) -> Result<Box<dyn PreparedSpeculativeActivationRestore + 'a>, OriginalSpeculativeCaptureError>
    {
        let frames = [
            size_of::<(
                &Self,
                &eredu_core::speculative::AdmittedSpeculativeActivations,
                Option<OriginalInterventionSource>,
                &crate::working_memory::MemoryLedger,
                CaptureUsage,
            )>(),
        ...
        if let Some(CaptureSkipReason::Limit { budget, cumulative }) =
            result.map_err(|cause| self.reject(self.funding.metadata_source(cause)))?
        {
            return Err(self.reject(
                self.funding
                    .metadata_source(CaptureError::Limit { budget, cumulative }),
            ));
        }
    ...
    ) -> Result<Box<dyn PreparedSpeculativeActivationRestore + 'a>, OriginalSpeculativeCaptureError>
    {
        ...
        if !Arc::ptr_eq(&self.control_owner, &saved.host_owner)
            ...
            || !self
                ...
                .ledger()
                .same_storage(value.lineage.ledger())
        {
            return Err(self.protocol(CaptureProtocolError::Invocation));
        }
    ...
    pub(super) fn prepare_control_authority<'a>(
        &'a mut self,
        source: Option<OriginalInterventionSource>,
    ...
    ) -> Result<Box<dyn PreparedSpeculativeActivationRestore + 'a>, OriginalSpeculativeCaptureError>
    {
        ...
        if identity.is_empty()
            || source.as_ref().map_or(!scopes.is_empty(), |source| {
        ...
        let interventions = if let Some(source) = source {
            ...
            let mut evidence_skips = self
                .funding
                .metadata_vec(scopes.len())
                .map_err(|cause| self.reject(cause))?;
            evidence_skips.resize_with(scopes.len(), || [None, None]);
            Some(InterventionSelection {
                scopes: copied,
                selected,
                evidence_skips,
                source,
