...
impl<M, S, R, C, P> RealtimeSessionState<M, S, R, C, P> {
    ...
    pub fn payload_contract_for_frame(
        &self,
        ingress: &RealtimeIngressContract,
        batch: usize,
        ...
        Ok(RealtimePayloadContract::new(
            schedule,
            occurrence.batch().get(),
            ingress.text_domain(),
            ingress.audio_domain(),
            RealtimePayloadGeneration::new(occurrence.history_generation().value())
                .expect("nonzero scheduler history"),
            RealtimePayloadOwnerIdentity::new(occurrence.incarnation().value())
                .expect("nonzero scheduler incarnation"),
        )
    ...
    pub fn frame_occurrence_for(
        &self,
        batch: usize,
    ) -> Result<RealtimeFrameOccurrence, RealtimeSessionExecutionError> {
        ...
        if let Some(committed) = self.committed_batch {
            if committed != batch {
                return Err(RealtimeSessionExecutionError::Batch {
                    committed: committed.get(),
                    submitted: batch.get(),
                });
            }
        }
        Ok(RealtimeFrameOccurrence {
            incarnation: self.incarnation,
            history: self.history_generation,
    ...
    C: Completion,
{
    ...
    pub fn branch_host_bytes<F, G, H>(&self) -> Option<usize> {
        Self::branch_shell_bytes::<F, G, H>()?
            .checked_add(self.generation.branch_host_bytes::<F, G, H>()?)
    }
    ...
    pub fn branch_with_host_source<F, G, H>(
        &self,
        funding: &eredu_core::HostMetadataFunding,
        branch_model: F,
        clone_sampler: G,
    ...
    where
        F: FnOnce(&M, &eredu_core::HostMetadataFunding) -> Result<M::Branch, BackendFailure>,
        ...
        H: FnOnce(&R, &eredu_core::HostMetadataFunding) -> Result<R, BackendFailure>,
    {
        ...
        let generation = self.generation.branch_with_host_source(
            funding,
            branch_model,
            clone_sampler,
            clone_random,
    ...
    O: TransitionOutput,
{
    ...
    where
        E: std::error::Error,
    {
        self.run_local_bounded_with_preparation(
            now,
            maximum_frames,
            |_, _, state| state.branch(),
            execute,
        )
    }
    ...
    pub fn run_local_bounded_with_preparation<A, E>(
        ...
        prepare: impl FnMut(
            WorkId,
            &RealtimeInputFrame,
            &RealtimeSessionState<M, S, R, C, P>,
        ...
        E: std::error::Error,
    {
        ...
        self.scheduler
            .prepare_bounded_with(maximum_frames, now, prepare)?;
    ...
    where
        ...
        O: eredu_core::scheduler::DistributedTransitionOutput,
    {
        self.run_distributed_bounded_with_preparation_and_errors(
            protocol,
            transport,
            now,
            maximum_frames,
        ...
        O: eredu_core::scheduler::DistributedTransitionOutput,
    {
        let wait = self.distributed_turn_wait(protocol, transport)?;
        self.scheduler
            .run_distributed_bounded_with_preparation_and_errors(
                protocol,
                transport,
                wait,
                now,
        ...
        <T::Completion as Completion>::Error: std::fmt::Display,
    {
        if self.model.topology().is_replicated() {
            return Err(SchedulerError::with_host_source(
                transport.metadata_funding(),
                SchedulerError::Consensus,
                format_args!(
                    "distributed realtime turns require a non-replicated selected topology"
                ),
            ));
        }
        ...
        .map_err(|error| {
            SchedulerError::from_consensus_with_host_source(transport.metadata_funding(), error)
        })?;
        Ok(wait)
    }
    ...
    pub fn request_state(
        &self,
        request: RequestId,
    ) -> Option<&RealtimeSessionState<M, S, R, C, P>> {
...
mod tests {
    ...
    fn original_frame_claims_and_retained_outputs_share_the_actual_pool_account() {
        use crate::working_memory::{
            ...
            RealtimeFrameRequirements, WorkingMemoryError,
        };
        ...
        sessions
            .run_local_turn(Instant::now(), |id, frame, branch| {
                observed = true;
                let occurrence = branch.frame_occurrence().unwrap();
                let requirements = || {
                    RealtimeFrameRequirements::new(
                        occurrence,
                        Some(128),
                        Some(256),
                        Some(64),
                        Some(32),
                        Some(4096),
                        HostSourceConstructionFacts::new(128, 2, 0).unwrap(),
                ...
                assert_eq!(
                    retained_origin.validate_retained_origin(&foreign),
                    Err(WorkingMemoryError::IdentityMismatch)
                );
                ...
                assert_eq!(
                    ...
                    required,
                    "quarantine cannot refund the original account"
                );
    ...
    fn original_realtime_source_admits_startup_under_its_accepted_capacity() {
        ...
        fn operation(_: &eredu_core::residency::OffloadUnitId) -> Result<(), String> {
            Ok(())
        }
        let operation =
            operation as fn(&eredu_core::residency::OffloadUnitId) -> Result<(), String>;
        let plan = match BackgroundPrefetchWorker::thread_startup_plan(
            &operation,
            "realtime-source-startup",
        ) {
            Ok(plan) => plan,
        ...
        sessions
            .run_local_turn(Instant::now(), |id, frame, branch| {
                ...
                assert!(matches!(
                    plan.prepare_for_source(&source, None),
                    Err(WorkingMemoryError::Domain(
                        eredu_core::MemoryDomainError::BudgetExceeded { .. }
                    ))
                ));
                assert_eq!(pool.payload_used_bytes().unwrap(), frame_bytes);
                drop((source, small));
                assert_eq!(pool.payload_used_bytes().unwrap(), 0);
                let accepted = pool
                    .reserve_realtime_frame(
                ...
                assert_eq!(pool.payload_used_bytes().unwrap(), plan.required_bytes());
                drop(startup);
                assert_eq!(pool.payload_used_bytes().unwrap(), 0);
                execute_immediately(id, frame, branch)
            })
