...
fn sparse_prefill_binding_requires_explicit_flattened_equation_and_preserves_readout() {
    ...
    let point = ObservationPoint {
        path: "bank.units".into(), node_id: "bank".into(), meaning: "actual sparse rows".into(),
        value_type: ObservationValueType::RoutedUnits {
            routing: "bank".into(),
            geometry: RoutedUnitGeometry { experts: 3, units_per_expert: 4, routes_per_token: 2 },
        },
        dtype: ObservationDtype::Floating,
        axes: Some(vec![
            TensorAxis { name: "token".into(), dimension: SymbolicDimension::TokenRows },
            TensorAxis { name: "route".into(), dimension: SymbolicDimension::Known(2) },
            TensorAxis { name: "unit".into(), dimension: SymbolicDimension::Known(4) },
        ]),
    ...
    let support = ObservationSupportReport {
        schema_version: 1, capture: Default::default(),
        points: vec![ObservationSupport { path: point.path.clone(),
            prefill: ObservationSupportStatus::Supported, decode: ObservationSupportStatus::Supported,
            floating_to_f32: true }],
    };
    let catalog = ObservationCatalog { schema_version: 1, points: vec![point],
        completeness: DescriptionCompleteness::Complete };
    let mut plan = CapturePlan::none();
    plan.selections = vec![CaptureSelection { id: "sparse".into(), path: "bank.units".into(),
        schedule: CaptureSchedule::default(), slices: vec![], transform: CaptureTransform::RoutedUnits }];
    let usage = CaptureUsage { captures: u64::MAX, retained_bytes: u64::MAX,
        host_bytes: u64::MAX, encoded_bytes: u64::MAX };
    plan.limits = CaptureLimits { per_step: usage, cumulative: usage,
        physical_native_bytes: None, on_limit: CaptureLimitPolicy::Fail };
    let admitted = SharedCapturePlan::new(plan.admit_with_text_origin(&catalog, &support,
        &CaptureCapabilities { transformations: vec![CaptureTransformKind::RoutedUnits], ..Default::default() },
        CaptureRequestShape { batch: 2, prompt_tokens: 5, max_predictions: 3 },
        CaptureTextOrigin { cached_positions: 7 }).unwrap());
    let mut paths = source();
    Arc::get_mut(&mut paths.0).unwrap().prefill = vec![
        PrefillObservationDeclaration::causal_ordinary_text("bank.units".into(), 0,
            PrefillReadoutStage::BeforeReadout)].into_boxed_slice();
    assert!(matches!(paths.prepare_capture_selection(&admitted),
        Err(PreparedCaptureSelectionError::Axes { index: 0 })));
    Arc::get_mut(&mut paths.0).unwrap().prefill = vec![
        PrefillObservationDeclaration::causal_routed_units("bank.units".into())].into_boxed_slice();
    let selected = paths.prepare_capture_selection(&admitted).unwrap();
    let geometry = InferenceGeometry { batch_size: 2, cached_positions: 7,
        input_positions: 5, max_output_tokens: 3, prefill_chunk_positions: 2,
        output: OutputDemand::LastPosition };
    assert_eq!(selected.physical_output(OutputDemand::StateOnly), OutputDemand::StateOnly);
    assert_eq!(selected.bind_geometry(geometry).unwrap().geometry(), geometry);
    ...
    drop(selected);
    assert!(matches!(paths.prepare_media_capture_selection(&admitted),
        Err(PreparedCaptureSelectionError::Undeclared { index: 0 })));
    Arc::get_mut(&mut paths.0).unwrap().media_prefill = vec![
        PrefillObservationDeclaration::prepared_media_decoder("bank.units".into(), 0,
            PrefillReadoutStage::BeforeReadout)].into_boxed_slice();
    assert!(matches!(paths.prepare_media_capture_selection(&admitted),
        Err(PreparedCaptureSelectionError::Axes { index: 0 })));
    Arc::get_mut(&mut paths.0).unwrap().media_prefill = vec![
    ...
    assert_eq!(media.physical_output(OutputDemand::StateOnly), OutputDemand::StateOnly);
    assert_eq!(media.bind_geometry(geometry).unwrap().geometry(), geometry);
    let media_mapping = CaptureRoutedPrefillPlan::prepare(media.source().admission(), 0, geometry).unwrap();
    for chunk in 0..3 {
        ...
        for local in 0..ordinary.source_tokens() {
            assert_eq!(ordinary.logical_token(local).unwrap(), decoded.logical_token(local).unwrap());
        }
    }
}
