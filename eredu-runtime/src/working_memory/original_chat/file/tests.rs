...
fn chat_file_real_i_j_overlap_and_last_source_retirement() {
    let bytes = config();
    let (i, j) = sizes(&bytes);
    let pool = WorkingMemoryPool::new(i + j, 0).unwrap();
    let source = pool
        .compile_chat_template_file_with(
            ...
            |pool, plan| {
                assert_eq!(
                    WorkingMemoryPool::chat_template_required_bytes(&plan).unwrap(),
                    j
                );
    ...
    assert_eq!(pool.peak_bytes().unwrap(), i + j);
...
fn chat_file_exact_minus_one_at_i_and_j_keeps_actual_prefixes() {
    let bytes = config();
    let (i, j) = sizes(&bytes);
    let short = WorkingMemoryPool::new(i - 1, 0).unwrap();
    let error = short
        .compile_chat_template_file_with(
            read(&bytes),
            "chat",
    ...
    drop(short.acquire_unquoted().unwrap());
