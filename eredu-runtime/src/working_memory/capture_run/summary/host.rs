...
impl Iterator for Selected<'_, '_> {
    ...
    fn next(&mut self) -> Option<f32> {
        if self.next == self.geometry.elements() { return None; }
        let mut ordinal = self.next;
        self.next += 1;
        if let CaptureHostF32::Uniform(value) = self.source { return Some(value); }
        ...
        for axis in (0..self.geometry.shape().len()).rev() {
            ...
            offset += (self.geometry.starts()[axis] as usize
                + coordinate * self.geometry.strides()[axis] as usize) * pitch;
            pitch *= self.geometry.source_shape()[axis];
        }
        match self.source { CaptureHostF32::Dense(values) => Some(values[offset]), _ => unreachable!() }
    }
...
pub(super) fn control_bytes() -> Option<usize> {
    let parts = [
        size_of::<CaptureHostF32<'_>>(), size_of::<Selected<'_, '_>>(),
        size_of::<(&[usize], CaptureHostF32<'_>)>(),
        size_of::<(usize, usize, usize, usize)>(),
        size_of::<std::ops::Range<usize>>(), size_of::<std::iter::Rev<std::ops::Range<usize>>>(),
...
impl CaptureSummaryClaim<'_, '_> {
    ...
    pub fn summarize_host_f32(
        self, shape: &[usize], source: CaptureHostF32<'_>,
        ...
        let checked = (|| {
            ...
            let extent = shape.iter().try_fold(1usize, |n, &d| n.checked_mul(d))
        ...
        if let Err(error) = checked {
            ...
            return Err(CaptureSummaryFailure {
                ...
                error, custody: self.identity.custody,
            });
        }
        let value = crate::capture::partition::summarize_f32_values(Selected {
            source, geometry: self.geometry(), next: 0,
        });
        self.finish(value)
    }
}
