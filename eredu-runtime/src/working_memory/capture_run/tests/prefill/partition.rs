...
fn context(source: &SharedCapturePlan, index: usize) -> PartitionCaptureContext {
    PartitionCaptureContext {
        artifact_identity: "remote-artifact".into(),
        execution_identity: "parallel".into(),
        run_identity: "run-é".into(),
...
fn advance(
    ...
    for index in 0..2 {
        ...
        let decision = step
            .begin_prefill_hook(index, &chunk, "block.output")
            .unwrap();
        if decision == CapturePrefillHookDecision::First {
            assert!(
                step.reserve_prefill_hook(index, quota, TensorDtype::F32, charge())
                    .unwrap()
                    .is_none()
            );
            if remote {
                step.mark_remote_prefill_target(index).unwrap();
            }
        }
        if remote {
            ...
            write_fragment(step, index, assembly, k);
        }
...
fn remote_prefill_receipt_reuses_original_h_after_real_chunk_progress() {
    ...
    let producer_pool = capture_test_ledger(h, 0).unwrap();
    let receiver_pool = capture_test_ledger(h, 0).unwrap();
    let (pr, run_p) = fresh(&producer_pool, h);
    let (rr, run_r) = fresh(&receiver_pool, h);
    ...
    let mut producer = pb
        ...
        .unwrap()
        .prepare_prefill_with_progression(prompt())
        .unwrap();
    let mut receiver = rb
        .begin_step(CapturePhase::Prefill, 0)
        .unwrap()
        .prepare_prefill_with_progression(prompt())
        .unwrap();
    let mut pquota = CaptureLedger::new(source.admission());
    pquota.begin_step();
    let mut rquota = CaptureLedger::new(source.admission());
    rquota.begin_step();
    for k in 0..2 {
        advance(&mut producer, &source, &mut pquota, false, k);
        advance(&mut receiver, &source, &mut rquota, true, k);
        if k == 0 {
            assert!(receiver.take_remote_prefill_tensor(0).is_err());
        }
    }
    ...
    for index in 0..2 {
        ...
        let record = &producer.records()[index];
        let bytes = PartitionCaptureRecordEncoding::new(
            &context,
            "exact-receipt",
            3,
        ...
        let decoded = claim
            .decode_partition_receipt(
                &bytes,
                PartitionCaptureTensorReceipt {
                    context: &context,
                    identity: "exact-receipt",
                    producer: 3,
            ...
            .unwrap();
        receiver
            .record_remote_prefill_tensor(decoded, TensorDtype::F32)
            .unwrap();
        assert!(receiver.take_remote_prefill_tensor(index).is_err());
        assert_eq!(
            values(tensor(&receiver.records()[index])),
            values(tensor(record))
        );
        if index == 0 {
            assert!(receiver.finish_prefill_targets().is_err());
        }
    ...
    drop(run_r);
    drop(rr);
    assert_eq!(receiver_pool.payload_used_bytes().unwrap(), h);
    drop(escaped);
    assert_eq!(receiver_pool.payload_used_bytes().unwrap(), 0);
    drop(producer);
    drop(pb);
    drop(run_p);
    drop(pr);
    assert_eq!(producer_pool.payload_used_bytes().unwrap(), 0);
}
...
fn remote_prefill_dropped_final_claim_cannot_refund_or_finish_target() {
    ...
    quota.begin_step();
    for k in 0..2 {
        advance(&mut frame, &source, &mut quota, true, k);
    }
