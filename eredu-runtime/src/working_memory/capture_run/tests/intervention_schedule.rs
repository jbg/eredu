...
fn sources() -> (SharedCapturePlan, AdmittedInterventionPlan) {
    sources_with_rows(false)
}
...
fn sources_with_evidence(
...
) -> (SharedCapturePlan, AdmittedInterventionPlan) {
    let mut raw = raw();
    ...
    if rows {
        discovery.points[0].axes.insert(
            1,
            TensorAxis {
                name: "sequence".into(),
    ...
    if rows {
        ...
        let mut second = edits.operations[0].clone();
...
fn intervention_only_schedule_prices_every_outcome_and_authenticates_actual_native_account() {
    ...
    let pool = capture_test_ledger(1 << 24, 0).unwrap();
    let source = pool
    ...
    let other_pool = capture_test_ledger(1 << 24, 0).unwrap();
...
fn prefill_interventions_keep_one_claim_and_ledger_across_uneven_chunks_and_poison_abandoned_work()
{
    ...
    let windows = chunks
        .each_ref()
        .map(|chunk| Window::new(source.plan().admission(), geometry, chunk).unwrap());
    let (reservation, run) = fresh(&pool, h);
    let native = run.scope().unwrap();
    ...
    let record_metadata: Vec<_> = step
        .interventions()
        .iter()
    ...
    for (chunk, window) in windows.into_iter().enumerate() {
        ...
        if chunk == 0 {
            assert!(step.take_prefill_intervention(1, window).is_err());
        }
        for index in 0..2 {
            ...
            assert!(
                step.take_intervention(index).is_err(),
                "original claim cannot be reissued"
            );
            assert_eq!(
                step.interventions()[index].outcome,
                if window.is_final() {
                    InterventionOutcome::Applied
                } else {
    ...
    step.record_intervention_failure(0, "failed physical fragment", charged)
        .unwrap();
