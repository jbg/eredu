...
fn retained(pool: &WorkingMemoryPool, key: u32, bytes: u64, expected: bool) {
    let pin = pool.pin_registered_storage([(key, bytes)]);
    assert_eq!(pin.is_ok(), expected, "key {key}: {pin:?}");
}
...
fn segment_retirement_preserves_baseline_interleaved_pins_aliases_and_zero_keys() {
    let source = selected_source();
    let h = plan(&source).initialization_peak_bytes();
    let pool = WorkingMemoryPool::new(h + 43, 0).unwrap();
    let a = pool.register_storage([(1u32, 11)]).unwrap();
    let b = pool.register_storage([(2u32, 13)]).unwrap();
    let escaped_b = b.clone();
    let c = pool.register_storage([(3u32, 19)]).unwrap();
    let zero = pool.register_storage([(4u32, 0)]).unwrap();
    ...
    drop(finish(step));
    assert_eq!(pool.used_bytes().unwrap(), h + 43);
    let held = ledger(&pool).2;
    segment.retire_after_settled_boundary(&mut native).unwrap();
    assert!(matches!(
    ...
    drop(escaped_b);
    assert_eq!(pool.used_bytes().unwrap(), h + 30);
...
fn segment_rejects_sibling_scope_independent_schedule_and_foreign_source_before_allocation() {
    ...
    let storage = pool.register_storage([(1u32, 11)]).unwrap();
    let foreign_storage = foreign.register_storage([(1u32, 11)]).unwrap();
...
fn segment_constructor_rollback_and_abandonment_preserve_both_quarantine_channels() {
    for fail_constructor in [true, false] {
        ...
        if fail_constructor {
            ...
            .is_err()); // Entered worker: retain this origin despite unwind.
        }
...
fn segment_release_returns_only_actual_retired_allocation_credit_to_original_account() {
    ...
    let mut native = run.scope().unwrap();
    let owner = native
        .adopt_storage_individually([(1u32, 37)])
        .unwrap()
        .into_values()
        .next()
        .unwrap();
    ...
    segment.retire_after_settled_boundary(&mut native).unwrap();
    assert_eq!(pool.0.usage.lock().unwrap().registered, 37);
    drop(escaped_owner);
    let usage = pool.0.usage.lock().unwrap();
    assert_eq!(usage.registered, 0);
    assert_eq!(usage.reserved, h + 37);
    drop(usage);
    assert_eq!(ledger(&pool).2, h);
    drop((segment, bank, r, run));
    native.certify().unwrap();
    assert_eq!(pool.used_bytes().unwrap(), h); // Only the frame's full original H survives.
    drop(frame);
    assert_eq!(pool.used_bytes().unwrap(), 0);
}
