...
fn partition_evidence_lends_original_frame_and_preserves_spent_sides() {
    let (capture, admitted) =
        sources_with_evidence(false, InterventionEvidence::Preview { max_elements: 4 });
    let pool = capture_test_ledger(1 << 25, 0).unwrap();
    let source = pool
        .compile_intervention_source(PreparedInterventionPlanCopy::inspect(&admitted).unwrap())
        .unwrap();
    let planned = CaptureRunHostPlan::prepare(&capture)
        .unwrap()
    ...
    assert!(
        loan.source()
            .same_storage(source.plan().evidence(0).unwrap().shared_geometry_source())
    );
    ...
    assert_eq!(
        ledger(&pool),
        before,
        "the loan grants no new account or Host backing"
    );
    drop(loan);
    assert!(frame.take_intervention_evidence_frame(0).is_err());
    assert!(
        frame
            .take_intervention_evidence(0, InterventionEvidenceSide::After)
            .is_err()
    );
    ...
    assert!(
        frame.frame.intervention_evidence[0]
            .as_ref()
            .unwrap()
            .frame
            .is_none()
...
fn partition_evidence_prefill_reuses_original_targets_without_resetting_progression() {
    ...
    let before = ledger(&pool);
    let window = crate::intervention::InterventionPrefillWindow::new(
        source.plan().admission(),
        geometry,
        &crate::prefill::PrefillChunk {
            input: 0..2,
            position: 0,
            output: geometry.output.for_chunk(false),
        },
    )
    .unwrap();
    assert!(
        frame.take_prefill_intervention(0, window).is_err(),
        "source-only Host preparation cannot open the execution cursor"
    );
    let mut loan = frame.take_intervention_evidence_frame(0).unwrap();
    assert_eq!(loan.frame().prefill_geometry(), None);
    loan.frame_mut()
        .prepare_existing_prefill_with_progression(geometry)
        .unwrap();
    let targets = loan.frame().frame.prefill.as_ref().unwrap().slots.as_ptr();
    assert!(
        loan.frame_mut().complete_prefill_chunk(0).is_err(),
        "active original evidence still requires its real callbacks"
    );
    frame.return_intervention_evidence_frame(loan).unwrap();
    let mut loan = frame.take_intervention_evidence_frame(0).unwrap();
    loan.frame_mut()
        ...
        .unwrap();
    assert_eq!(
        loan.frame().frame.prefill.as_ref().unwrap().slots.as_ptr(),
        targets
    );
    assert_eq!(loan.frame().prefill_geometry(), Some(geometry));
    assert!(
        loan.frame_mut()
            .prepare_existing_prefill_with_progression(eredu_core::InferenceGeometry {
    ...
    assert_eq!(
        ledger(&pool),
        before,
        "companion headers use the protected original Host source"
    );
    frame.return_intervention_evidence_frame(loan).unwrap();
}
...
fn partition_evidence_prefill_defers_completion_until_both_actual_producer_hooks() {
    ...
    let mut descriptive = CaptureLedger::new(companion.admission());
    descriptive.begin_step();
    assert!(
        descriptive
    ...
    for ordinal in 0..2 {
        ...
        loan.frame_mut()
            ...
            .unwrap();
        assert!(matches!(
            loan.frame_mut().complete_prefill_chunk(ordinal),
            Err(CaptureRunHostError::Prefill(
                CapturePrefillHostError::Progression(
        ...
        for side in 0..2 {
            ...
            if side == 0 {
                assert!(
                    matches!(
                        target.complete_prefill_chunk(ordinal),
                        Err(CaptureRunHostError::Prefill(
                            CapturePrefillHostError::Progression(
                                crate::capture::CapturePrefillProgressError::Missing
                            )
        ...
        if ordinal == 1 {
                ...
                .zip([[1., 2., 3., 4.], [2., 4., 6., 8.]])
            {
                let Some(CapturePayload::SharedTensor(value)) = &record.payload else {
                    panic!("original evidence payload")
                };
                let TensorObservationData::F32(actual) = value.data() else {
                    panic!("actual Float32 evidence")
                };
    ...
    assert_eq!(
        quota.total().captures,
        2,
        "uneven chunks do not reissue either evidence target"
    );
    assert_eq!(
        ledger(&pool),
        hold,
        "both callback phases retain the original protected Host owner"
    );
}
...
fn original_evidence_segment_accepts_only_its_admitted_companion_and_exact_chunk() {
    ...
    let (reservation, run) = fresh(
        &pool,
        planned.initialization_peak_bytes() + plain.initialization_peak_bytes(),
    );
    let request: InferenceRequest = reservation.into();
    let mut bank = run
        .prepare_capture_run(request.memory_reservation(), planned)
        .unwrap();
    let unbound = run
        .prepare_capture_run(request.memory_reservation(), plain)
        .unwrap();
    let mut native = run.scope().unwrap();
    let mut other = run.scope().unwrap();
    let geometry = request.geometry();
    let chunk = crate::prefill::PrefillChunk {
        input: 0..geometry.input_positions,
        position: geometry.cached_positions,
        output: geometry.output,
    };
    let context = crate::inspection::PrefillChunkRetentionContext::new(
        &request,
        &chunk,
    ...
    let (unbound_segment, unbound_registration) = unbound
        .prefill_source_bootstrap()
        .unwrap()
        .begin_segment(&mut other, &context)
        .unwrap();
    for side in 0..2 {
        let companion = source.plan().evidence(0).unwrap().shared_geometry_source();
        let foreign = equal.plan().evidence(0).unwrap().shared_geometry_source();
        assert_eq!(
            companion.admission().identity(),
            foreign.admission().identity()
        );
        let rows =
            CapturePrefillRowAssembly::prepare(companion.admission(), side, geometry).unwrap();
        let fragment = rows.fragment(0).unwrap();
        segment.validate_prefill_fragment(&fragment).unwrap();
        assert!(
            unbound_segment
                .validate_prefill_fragment(&fragment)
                .is_err(),
            "same account and geometry cannot register a missing original intervention owner"
        );
        let foreign_rows =
            CapturePrefillRowAssembly::prepare(foreign.admission(), side, geometry).unwrap();
        assert!(
            segment
                .validate_prefill_fragment(&foreign_rows.fragment(0).unwrap())
                .is_err(),
            "equal independently prepared companions are not the admitted source"
        );
        let different = CapturePrefillRowAssembly::prepare(
            companion.admission(),
            side,
            InferenceGeometry {
                prefill_chunk_positions: 1,
        ...
        assert!(
            segment
                .validate_prefill_fragment(&different.fragment(0).unwrap())
                .is_err()
        );
    }
    ...
    assert_eq!(
        ledger(&pool),
        before,
        "the retained source uses already priced bootstrap/stamp controls"
    );
    let frame = bank
        .begin_step(CapturePhase::Prefill, 0)
        .unwrap()
