...
impl ConsensusTransport for Transport {
    ...
    fn all_gather_words(&self, _words: &[u32]) -> Result<Vec<u32>, Self::Error> {
...
impl BoundedConsensusTransport for Transport {
    ...
    fn submit_all_gather_words(
        &self,
        words: &[u32],
    ) -> Result<Submission<Vec<u32>, Settled>, Self::Error> {
        assert_eq!(words.len(), 18, "existing outcome protocol");
        let mut output = Vec::new();
        for rank in 0..2 {
            let mut peer = words.to_vec();
            peer[2] = rank as u32;
            if rank != self.rank {
                peer[7] = if rank == 0 {
                    u32::from(!self.reject_peer)
                } else {
                    2
                };
            }
...
impl PartitionCaptureHookTransport for Transport {
    ...
    fn submit_capture_hook(
        ...
        _members: &[usize],
        _success: bool,
    ) -> Result<Submission<(), Settled>, Self::Error> {
        panic!("singleton or absent rank submitted member transport")
    }
    fn resolve_capture_hook(&self, _output: ()) -> Result<bool, Self::Error> {
        panic!("singleton or absent rank resolved member transport")
    }
}
#[test]
fn partition_outcome_requires_original_member_completion_and_never_refunds_local_credits() {
    for (rank, rejected) in [(0, false), (1, false), (0, true), (1, true)] {
        ...
        let source = PreparedPartitionInterventionSource::new(
            &original,
            0,
            CapturePhase::Decode,
            1,
        ...
        if rank == 0 {
            ...
            assert!(
                ...
                "source validation precedes spending"
            );
            loan.validate(
                &claim,
                &projection,
            ...
            loan.charge_projection(projected).unwrap();
            loan.charge_source(dependencies).unwrap();
            loan.charge_execution(usage).unwrap();
            assert!(loan.charge_execution(usage).is_err());
            if rejected {
                drop(loan);
                drop(claim);
            } else {
                work.finish(loan, true).unwrap();
                frame
                    .record_intervention(claim.finish(local).unwrap())
                    .unwrap();
            }
        } else {
            ...
            assert_eq!(
                ...
                "absent rank has no local edit result"
            );
        }
        assert_eq!(
            ledger.total(),
            global,
            "local child spending must not recharge the parent"
        );
        if rejected {
            ...
            let receipt = work.deliver(baseline).unwrap();
            frame.record_partition_intervention(receipt).unwrap();
            assert_eq!(
                frame.interventions()[0].charged,
                baseline.checked_add(global).unwrap()
            );
            assert_eq!(
                frame.interventions()[0].outcome,
                InterventionOutcome::Applied
            );
            assert!(frame.take_intervention(0).is_err());
            assert!(work.deliver(baseline).is_err());
            drop(finish(frame));
        }
        drop(work);
        assert_eq!(
            ledger.total(),
            global,
            "dropping original work never refunds its ledger"
        );
        drop((
            initial,
            bank,
