...
fn original_host_summary_uses_exact_source_spent_claim_and_retained_account() {
    ...
    let pool = capture_test_ledger(bytes * 2, 0).unwrap();
    ...
    let receipt = claim
        .summarize_host_f32(&[5, 2], CaptureHostF32::Dense(&values))
        .unwrap();
    let value = receipt.observation();
    assert_eq!((value.elements, value.finite, value.non_finite), (10, 7, 3));
    assert_eq!(
        (value.nan, value.positive_infinity, value.negative_infinity),
        (1, 1, 1)
    );
    assert_eq!(
        (value.min, value.max, value.mean),
        (Some(-2.0), Some(6.0), Some(17.0 / 7.0))
    );
...
fn original_host_summary_refuses_wrong_backing_and_uniform_uses_same_reduction() {
    for wrong in [true, false] {
        ...
        let mut frame = bank
            ...
            .unwrap();
        let claim = frame.take_summary(0).unwrap();
        let result = claim.summarize_host_f32(
            &[5, 2],
            if wrong {
                CaptureHostF32::Dense(&[1.0])
            } else {
                CaptureHostF32::Uniform(-3.0)
            },
        );
        assert_eq!(result.is_err(), wrong);
        if let Ok(value) = &result {
            assert_eq!(
                (value.observation().mean, value.observation().rms),
                (Some(-3.0), Some(3.0))
            );
        }
        assert!(frame.take_summary(0).is_err());
...
fn original_host_summary_selects_declared_strides_before_existing_reduction() {
    ...
    assert_eq!(
        (
            receipt.observation().elements,
            receipt.observation().mean,
            receipt.observation().rms
        ),
        (2, Some(5.0), Some(29f64.sqrt()))
    );
}
