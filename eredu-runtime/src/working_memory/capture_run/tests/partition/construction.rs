...
use crate::capture::partition::{
    PartitionCaptureProducer, PartitionCaptureReceiptLimits, PartitionCaptureReceiptPlan,
};
...
fn paid_contiguous_receipts_preserve_ordinary_shard_and_sum_order_with_empty_peers() {
    ...
    for selection in [0, 2] {
        for sum in [false, true] {
            ...
            let actual = PartitionCaptureReceiptPlan::new_contiguous_shared_funded(
                &source,
                &context,
                axis,
                &rows,
            ...
            assert_eq!(
                actual.producers().map(|(rank, _)| rank).collect::<Vec<_>>(),
                [0, 1, 3]
            );
            ...
            for (rank, projection) in actual.producers() {
                ...
                assert_eq!(
                    actual.encoding_usage(rank).unwrap(),
                    ordinary.encoding_usage(rank).unwrap()
                );
            }
            if !sum {
...
fn paid_contiguous_receipt_refuses_overlap_missing_terms_and_foreign_probability_sources() {
    ...
    for case in [
        ...
        let source = if case == "vocabulary" {
            super::vocabulary::vocabulary_source(false, 3)
        } else {
            source()
        };
        let context = context(&source, 0);
        let global: Vec<u64> = if case == "vocabulary" {
            vec![1, 3, 4]
        } else {
        ...
        match case {
            "duplicate" => rows[1].rank = 3,
            "overlap" => rows[1].coordinates = 0..width,
            "missing" => rows.pop().map(|_| ()).unwrap(),
            _ => (),
        }
        let mut quota = CaptureLedger::new(source.admission());
        quota.begin_step();
        let (funding, _, refuse, retired) = funding();
        refuse.store(case == "funding", Ordering::SeqCst);
        let error = PartitionCaptureReceiptPlan::new_contiguous_shared_funded(
            &source,
            &context,
            axis,
            &rows,
...
fn paid_complete_receipt_keeps_ordinary_identity_geometry_and_source_custody() {
    for selection in [0, 2] {
        ...
        .unwrap();
        let projection =
            CaptureContiguousProjectionPlan::prepare(&global, &slice, 0, 0..global[0], 1)
                .unwrap()
                .construct();
        let limits = PartitionCaptureReceiptLimits {
            max_producers: 1,
        ...
        let original = PartitionCaptureReceiptPlan::new_complete_shared_funded(
            &source,
            &context,
            3,
            4,
        ...
        assert!(std::ptr::eq(
            original.shared_plan_source().admission(),
            source.admission()
        ));
        assert_eq!(
            original
        ...
        assert_eq!(
            actual.fragments()[0].local(),
            expected.fragments()[0].local()
        );
        assert_eq!(
            actual.fragments()[0].destination(),
            expected.fragments()[0].destination()
        );
        assert_eq!(
            original.delivery_usage().unwrap(),
            ordinary.delivery_usage().unwrap()
        );
        assert_eq!(
            original.encoding_usage(3).unwrap(),
            ordinary.encoding_usage(3).unwrap()
        );
        drop(ordinary);
        drop(geometry);
        drop(original_quota);
    ...
    for refuse in [false, true] {
        ...
        let error = PartitionCaptureReceiptPlan::new_complete_shared_funded(
            &source,
            &context,
            3,
            4,
...
fn paid_explicit_invocation_receipt_preserves_original_axes_bounds_and_host_source() {
    use crate::capture::partition::{
        PartitionCaptureContiguousProducer as Producer, PartitionInvocationCaptureGeometry,
    ...
    for transform in [
        ...
        for sum in [false, true] {
            ...
            declaration.selections[0].slices = vec![CaptureSlice {
                axis: "width".into(),
                start: 1,
                end: 8,
                stride: 3,
            ...
            .unwrap();
            assert_eq!(actual.identity(), ordinary.identity());
            assert_eq!(actual.context().invocation, Some(shape));
            assert!(used.load(Ordering::SeqCst) > 0);
            {
                let host = PartitionFragmentHostPlan::prepare(&actual).unwrap();
                assert_eq!(host.fragment_count(), 2);
                assert!(host.fragment_peak_bytes() > 0);
                assert_eq!(
                    host.assembly_peak_bytes() > 0,
                    sum && matches!(transform, CaptureTransform::Summary)
                );
                for (rank, projection) in actual.producers() {
                    assert_eq!(projection.global_shape(), global);
                    assert_eq!(
                        projection.fragments(),
                        ordinary.producer(rank).unwrap().fragments()
                    );
                    let local =
                        PartitionInvocationCaptureGeometry::from_receipt(&actual, rank, 0).unwrap();
                    assert!(std::ptr::eq(local.admission(), source.admission()));
                    assert!(std::ptr::eq(local.projection(), projection));
                    let local_shape = match local.kind() {
                        PartitionInvocationCaptureKind::Tensor(g) => g.source_shape(),
                        PartitionInvocationCaptureKind::Summary(g) => g.source_shape(),
                        PartitionInvocationCaptureKind::Histogram(g) => g.source_shape(),
                    };
                    ...
                    assert!(
                        PartitionInvocationCaptureGeometry::prepare(
                            source.admission(),
                            0,
                            CapturePhase::Decode,
                            1,
            ...
            changed.invocation = Some(CaptureInvocationShape {
                sequence: 3,
                ..shape
            });
            let mut changed_quota = CaptureLedger::new(source.admission());
            changed_quota.begin_step();
            let changed_receipt = PartitionCaptureReceiptPlan::new_contiguous_shared_funded(
                &source,
                &changed,
                2,
                &rows,
            ...
            assert_eq!(
                changed_receipt.producer(1).unwrap().global_shape(),
                &[1, 3, 8]
            );
            drop(changed_receipt);
            drop(changed_quota);
            let mut invalid = context.clone();
            invalid.invocation = Some(CaptureInvocationShape {
                sequence: 4,
                ..shape
            });
            let failure = PartitionCaptureReceiptPlan::new_contiguous_shared_funded(
                &source,
                &invalid,
                2,
                &rows,
...
fn projected_invocation_windows_keep_logical_receipt_and_physical_spatial_coordinates() {
    use crate::capture::partition::{
        PartitionCaptureContiguousProducer as Producer, PartitionInvocationCaptureGeometry,
        PartitionInvocationCaptureKind as K,
    };
    for transform in [
        CaptureTransform::Slice,
        ...
        for sum in [false, true] {
            ...
            context.phase = CapturePhase::Decode;
            context.prediction = 1;
            context.invocation = Some(logical);
            let rows = if sum {
                vec![
                    Producer {
                        rank: 1,
            ...
            for (start, count) in [(0, 1), (1, 2), (1, 1)] {
                ...
                for row in &rows {
                    let projected = PartitionInvocationCaptureGeometry::from_window_receipt(
                        &receipt, row.rank, 0, physical, window,
                    )
                    .unwrap();
                    assert_eq!(projected.invocation_window(), Some((physical, window)));
                    assert!(std::ptr::eq(projected.admission(), source.admission()));
                    let (source_shape, starts, ends, strides, output_elements) =
                    ...
                    assert_eq!(
                        ...
                        &[
                            1,
                            count as usize,
                            (row.coordinates.end - row.coordinates.start) as usize
                        ]
                    );
                    ...
                    for r in (starts[1]..ends[1]).step_by(strides[1] as usize) {
                        for x in (starts[2]..ends[2]).step_by(strides[2] as usize) {
                            actual.push(
                                ((start + r) * 100 + row.coordinates.start + x) as f32 + 0.125,
                            );
                        }
                    }
                    let mut expected = Vec::new();
                    for r in (0..3).step_by(2) {
                        for x in (1..8).step_by(3) {
                            if (start..start + count).contains(&r) && row.coordinates.contains(&x) {
                                expected.push((r * 100 + x) as f32 + 0.125);
                            }
                        }
                    }
                    assert_eq!(actual, expected);
                    let wanted = if matches!(transform, CaptureTransform::Preview { .. }) {
                        expected.len().min(2)
                    } else {
                        expected.len()
                    };
                    ...
                    assert_eq!(
                        matches!(projected.kind(), K::Tensor(_)),
                        sum || matches!(
                            transform,
                    ...
                    match projected.into_kind() {
                        K::Tensor(g) => {
                            assert!(
                                CaptureTensorHostPlan::prepare(g)
                                    .unwrap()
                        ...
                        K::Summary(g) => {
                            assert!(
                                CaptureSummaryHostPlan::prepare(g)
                                    .unwrap()
                                    .initialization_peak_bytes()
                                    > 0
                            );
                        ...
                        K::Histogram(g) => {
                            assert!(
                                CaptureHistogramHostPlan::prepare(g)
                                    .unwrap()
                                    .initialization_peak_bytes()
                                    > 0
                            );
            ...
            for (shape, window) in [
                ...
                assert!(
                    PartitionInvocationCaptureGeometry::from_window_receipt(
                        &receipt, 1, 0, shape, window
                    )
                    .is_err()
                );
            }
            assert!(
                PartitionInvocationCaptureGeometry::window_control_bytes().unwrap()
                    > PartitionInvocationCaptureGeometry::control_bytes().unwrap()
            );
            assert!(used.load(Ordering::SeqCst) > 0);
            drop(quota);
            drop(funding);
