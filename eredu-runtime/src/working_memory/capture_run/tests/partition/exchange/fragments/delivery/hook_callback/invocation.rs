...
fn local_partition_invocation_uses_typed_workers_and_keeps_spent_failure_custody() {
    use super::super::super::prefill::{projected_receipt, projected_source};
    for (transform, combination, mode) in [
        ...
        if mode == 3 {
            result = result.unwrap().observe_invocation(&mut native, &value);
        }
        assert_eq!(quota.total(), spent);
        assert!(transport.calls.borrow().is_empty());
        if mode != 0 {
            let error = result.unwrap_err();
            assert_eq!(native.raw, usize::from(mode != 2));
            if mode == 1 {
                assert!(error.to_string().contains("native sentinel"));
            }
            drop(continuation);
            drop(run);
            drop(reservation);
        ...
        let mut hook = result.unwrap().into_inner();
            ...
            .flat_map(|head| {
                (1..3).flat_map(move |row| {
                    [1, 3, 5]
                        ...
                        .map(move |column| {
                            head as f32 * 100.0 + row as f32 * 10.0 + column as f32 + 0.125
                        })
                })
            })
            .collect();
        if let CaptureTransform::Preview { max_elements } = &transform {
            values.truncate(*max_elements as usize);
        }
        ...
        match bank.value(0, 0).unwrap() {
            ...
            PartitionFragmentValue::Tensor(value) => {
                assert_eq!((native.raw, native.summary, native.histogram), (1, 0, 0));
                assert_eq!(
                    value.observation().data(),
                    &TensorObservationData::F32(values)
                );
                escaped = Some(value.observation().clone());
            }
            PartitionFragmentValue::Summary(value) => {
                assert_eq!((native.raw, native.summary, native.histogram), (0, 1, 0));
            ...
            PartitionFragmentValue::Histogram(value) => {
                assert_eq!((native.raw, native.summary, native.histogram), (0, 0, 1));
