...
pub(super) fn projected_source_limits(
    transform: CaptureTransform,
    limit: Option<(CaptureLimitPolicy, u64)>,
) -> SharedCapturePlan {
    projected_source_at(transform, limit, false, false)
}
pub(in super::super) fn projected_decode_source(transform: CaptureTransform) -> SharedCapturePlan {
    projected_source_at(transform, None, true, false)
}
pub(in super::super) fn projected_invocation_source(
    transform: CaptureTransform,
) -> SharedCapturePlan {
    projected_source_at(transform, None, true, true)
}
fn projected_source_at(
    transform: CaptureTransform,
    limit: Option<(CaptureLimitPolicy, u64)>,
    decode: bool,
    invocation: bool,
    ...
    declaration.selections[0].schedule.prefill = !decode || invocation;
    ...
    if decode {
        declaration.selections[0].slices.remove(0);
    }
    let mut point = point();
    point.axes = Some(vec![
        TensorAxis {
...
pub(super) fn inference() -> InferenceGeometry {
    InferenceGeometry {
        batch_size: 1,
        cached_positions: 2,
        input_positions: 3,
...
pub(in super::super) fn projected_invocation_receipt(
    source: &SharedCapturePlan,
    combination: PartitionCaptureCombination,
    funding: &HostMetadataFunding,
    quota: &mut CaptureLedger,
    ...
    context.invocation = Some(
        window
            .map_or(Ok(shape), |window| window.validate(shape))
            .unwrap(),
    );
    context.invocation_window =
        window.map(
...
fn partition_prefill_host_bank_reuses_paid_scatter_and_keeps_additive_terms_raw() {
    for (transform, combination) in [
        ...
        let mut loan = bank
            .begin_local_prefill(&receipt, 0, &TensorDtype::F16, estimate(0))
            .unwrap();
        loan.quota_mut().reserve_quota(estimate(0).capture).unwrap();
        assert!(
            bank.begin_local_prefill(&receipt, 0, &TensorDtype::F16, estimate(0))
                .is_err()
        );
        assert!(
        ...
        let width = projection.local_shape()[2] as usize;
        let offset = if combination == PartitionCaptureCombination::Disjoint {
            3
        } else {
            0
        };
        for index in 0..rows.chunk_count() {
            let chunk = rows.fragment(index).unwrap();
            let mut writer = bank
                .take_local_prefill_chunk(&receipt, 0, &chunk)
                .unwrap()
                .prepare()
                .unwrap();
            ...
            for mapping in chunk.mappings() {
                ...
                let column = source_index % width + offset;
                let head = source_index / width;
                let value = head as f32 * 100.0 + index as f32 * 10.0 + column as f32 + 0.125;
                writer.push_f32(value).unwrap();
            }
            writer.finish().unwrap();
            bank.complete_local_prefill_chunk(&receipt, 0, index)
                .unwrap();
            assert!(
                bank.take_local_prefill_chunk(&receipt, 0, &chunk).is_err(),
                "a physical chunk cannot be replayed"
            );
            assert!(
                bank.value(0, 0).is_none(),
                "no initialized prefix is a completed receipt"
            );
        }
        bank.finish_local_prefill(&receipt, 0).unwrap();
        let Some(PartitionFragmentValue::Tensor(value)) = bank.value(0, 0) else {
            panic!("raw local values")
        };
        let escaped = value.observation().clone();
        let TensorObservationData::F32(actual) = escaped.data() else {
            panic!("original F32 host values")
        };
        ...
        for head in 0..2 {
            for row in 1..3 {
                for column in [1, 3, 5] {
                    if column >= offset {
                        expected
                            .push(head as f32 * 100.0 + row as f32 * 10.0 + column as f32 + 0.125);
                    }
                }
            }
        }
        ...
        assert!(
            !bank.complete(),
            "remote fragments still need real delivery"
        );
        assert!(bank.finish_local_prefill(&receipt, 0).is_err());
...
fn partition_prefill_host_bank_rejects_foreign_source_and_retains_failed_attempt_custody() {
    for fail in ["drop", "missing"] {
        let source = projected_source(CaptureTransform::Slice);
        let foreign = projected_source(CaptureTransform::Slice);
        let mut quota = CaptureLedger::new(source.admission());
        quota.begin_step();
        let (funding, _, _, _) = funding();
        let transport = Receipts {
        ...
        let loan = bank
            ...
            .unwrap();
        let projection = receipt.producer(0).unwrap();
        let wrong = CapturePrefillRowAssembly::prepare_partition(
            foreign.admission(),
            0,
            inference(),
            projection,
        ...
        .unwrap();
        let wrong_chunk = wrong.fragment(0).unwrap();
        assert!(
            bank.take_local_prefill_chunk(&receipt, 0, &wrong_chunk)
                .is_err()
        );
        let rows = CapturePrefillRowAssembly::prepare_partition(
            source.admission(),
            0,
            inference(),
            projection,
        ...
        let error = if fail == "drop" {
            drop(
                bank.take_local_prefill_chunk(&receipt, 0, &chunk)
                    .unwrap()
                    .prepare()
                    .unwrap(),
            );
            bank.complete_local_prefill_chunk(&receipt, 0, 0)
                .unwrap_err()
        } else {
            bank.finish_local_prefill(&receipt, 0).unwrap_err()
        };
        assert!(bank.take_local_prefill_chunk(&receipt, 0, &chunk).is_err());
        assert!(bank.value(0, 0).is_none());
        assert_eq!(quota.total(), spent);
        drop(bank);
        drop(loan);
        drop(run);
...
fn partition_prefill_reductions_preserve_typed_chunks_and_reject_dropped_receipts() {
    for histogram in [false, true] {
        for drop_chunk in [false, true] {
            let transform = if histogram {
                CaptureTransform::Histogram {
                    edges: vec![-1.0, 0.25, 2.0],
                }
            } else {
            ...
            for index in 0..plan.chunk_count() {
                ...
                let claim = bank
                    .take_local_prefill_reduction(&receipt, 0, &chunk)
                    .unwrap();
                if drop_chunk && index == 1 {
                    ...
                    assert!(
                        bank.take_local_prefill_reduction(&receipt, 0, &chunk)
                            .is_err()
                    );
                    break;
                }
                let values: Vec<f32> = if index == 0 {
                    vec![]
                } else {
                    (0..2)
                        .flat_map(|_| {
                ...
                let value = match claim {
                    ...
                    PartitionFragmentDestination::Histogram(claim) => {
                        let mut builder = claim.prepare().unwrap();
                        builder.fill_partition_sum(&values).unwrap();
                        PartitionFragmentValue::Histogram(builder.finish_partition().unwrap())
                    }
                    _ => panic!("typed reduction destination"),
                };
                bank.record_local_prefill_reduction(&receipt, 0, &chunk, value)
                    .unwrap();
                bank.complete_local_prefill_chunk(&receipt, 0, index)
                    .unwrap();
                assert!(
            ...
            if !drop_chunk {
                ...
                match bank.value(0, 0).unwrap() {
                    PartitionFragmentValue::Summary(value) => {
                        let value = value.observation();
                        assert_eq!(value.elements, 8);
                        assert_eq!(value.finite, 6);
                        assert_eq!(value.nan, 2);
                        assert_eq!(value.min, Some(-1.0));
                        assert_eq!(value.max, Some(2.5));
                        assert!((value.mean.unwrap() - 2.0 / 3.0).abs() < 1e-6);
                    }
                    PartitionFragmentValue::Histogram(value) => {
                        let value = value.observation();
                        assert_eq!(value.edges, [-1.0, 0.25, 2.0]);
                        assert_eq!(value.counts, [2, 2]);
            ...
            drop(run);
            drop(reservation);
            assert_eq!(ledger(&pool).0 > 0, drop_chunk);
            drop(failure);
            assert_eq!(ledger(&pool).0, 0);
        }
    }
