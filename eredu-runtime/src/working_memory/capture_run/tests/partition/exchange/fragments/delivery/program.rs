...
impl ConsensusTransport for ProgramRanks {
    ...
    fn all_gather_words(&self, _: &[u32]) -> Result<Vec<u32>, Infallible> {
        panic!("unbounded program transport")
    }
}
        ...
        for rank in 0..4 {
            ...
            match frame.kind() {
                PartitionCaptureFrameKind::Source => {
                    words[3] = rank as u32;
                    words[13] = u32::from(rank == 0 || rank == 3);
                }
...
fn scheduled_contiguous_program_preserves_projected_hooks_votes_and_additive_delivery() {
    ...
    for sum in [false, true] {
        for reject in [false, true] {
            ...
            let retained = PreparedPartitionContiguousSource::new(
                &source,
                0,
                2,
                &producers,
            ...
            drop(allowance);
            let h = PartitionFragmentHostPlan::prepare_prefill(&prototype, inference())
                .unwrap()
            ...
            let host = run
                .prepare_partition_fragment_host(&reservation, prototype, Some(inference()))
                .unwrap();
            ...
            let program_limits = PartitionCaptureReceiptLimits {
                max_producers: limits.max_producers + 1,
                max_fragments: limits.max_fragments + 2,
                max_record_bytes: limits.max_record_bytes * 2,
            };
            ...
            assert!(
                program
                    .take_prefill_projection(0, &mut frame, true)
                    .is_err()
            );
            ...
            for k in 0..3 {
                let epoch = DistributedCommitEpoch::new(17 + k).unwrap();
                if k != 0 {
                    program.coordinate(epoch, &quota).unwrap();
                }
                assert!(program.produces(0).unwrap());
                assert!(program.take_receiver_source(0).unwrap().is_none());
                ...
                let decision = frame
                    ...
                    .unwrap();
                let hook = program
                    .take_prefill_projection(
                        0,
                        &mut frame,
                        decision == crate::capture::CapturePrefillHookDecision::First,
                    )
                    .unwrap()
                    .unwrap();
                ...
                let hook = hook.observe(&mut native, &value, inference(), k).unwrap();
                program.return_prefill_projection(0, hook).unwrap();
                assert!(
                    program
                        .take_prefill_projection(0, &mut frame, false)
                        .is_err(),
                    "same epoch cannot mint another local loan"
                );
                if let Some(reduced) = &reduced {
                    frame
                        .finish_summary_prefill_hook(0, &reduced.fragment(k).unwrap())
                        .unwrap();
                } else {
                    frame
...
impl ConsensusTransport for VotedRanks {
    ...
    fn all_gather_words(&self, _: &[u32]) -> Result<Vec<u32>, Infallible> {
        panic!("unbounded voted transport")
    }
}
impl BoundedConsensusTransport for VotedRanks {
    ...
    type GatherOutput = ();
    fn submit_all_gather_words(&self, _: &[u32]) -> Result<Submission<(), Done>, Infallible> {
        panic!("untyped voted transport")
    }
    fn resolve_all_gather_words(&self, _: ()) -> Result<Vec<u32>, Infallible> {
        panic!("untyped voted output")
    }
}
impl PartitionCaptureTransport for VotedRanks {
    fn capture_rank(&self) -> usize {
        self.base.capture_rank()
    }
    fn capture_wait(&self) -> Result<BoundedCompletionWait, CaptureError> {
    ...
    fn fail_capture_exchange(&self, e: &PartitionCaptureExchangeError) {
        assert!(
            self.reject_source,
            "an agreeing source frame must not invalidate transport"
        );
        assert!(matches!(
            e,
            PartitionCaptureExchangeError::Protocol(
    ...
    fn gather_capture_frame(
        ...
        if frame.kind() != PartitionCaptureFrameKind::Source {
            return self.base.gather_capture_frame(frame, wait);
        }
        let mut out = PartitionCaptureBuffer::funded(frame.gathered_words(), &self.base.0.funding)?;
        for rank in 0..4 {
            let mut words: [u32; 16] = frame.words().try_into().unwrap();
            words[3] = rank as u32;
            words[13] = if rank == 0 || rank == 3 || rank == self.capture_rank() {
                1
            } else {
                0
            };
...
fn late_source_vote_binds_original_host_and_preserves_absent_and_replica_sources() {
    use super::super::prefill::{inference, projected_receipt, projected_source};
    use crate::capture::partition::{
        PartitionCaptureFragmentGeometry, PartitionCaptureLocalSource,
    };
    for local in [0, 1, 2] {
        for reject_source in [false, true] {
            let source = projected_source(CaptureTransform::Summary);
            let (metadata, _, _, _) = funding();
            let transport = VotedRanks {
                base: ProgramRanks(Ranks {
                    local,
                    bytes: RefCell::new(std::array::from_fn(|_| None)),
                    funding: metadata.clone(),
                    reject: false,
            ...
            let mut prototype = projected_receipt(
                &source,
                PartitionCaptureCombination::SumF64ToF32,
                &metadata,
                &mut quotation,
            ...
            let rows: Vec<_> = geometry
                ...
                .collect();
            let quoted =
                crate::capture::partition::PreparedPartitionFragmentSourceAllowance::prepare(
                    &transport,
                    &mut prototype,
                    &rows,
                    &metadata,
            ...
            let retained = PreparedPartitionContiguousSource::new_local(
                &source,
                0,
                2,
                &producers,
            ...
            program
                ...
                .unwrap();
            let spent = quota.total();
            let result = program.coordinate(epoch, &quota);
            assert_eq!(
                transport.failures.get(),
                usize::from(reject_source),
                "source mismatch poisons transport exactly once"
            );
            assert_eq!(
                quota.total(),
                spent,
                "precision binding does not reserve or refund global work"
            );
            if !reject_source {
                result.unwrap();
                if local != 0 {
                    let receiver = program
                        .take_projected_receiver_source(0, inference(), 0)
                        .unwrap();
                    assert_eq!(receiver.is_some(), local == 1);
                    if let Some(receiver) = receiver {
                        assert_eq!(receiver.source_shape(), [2, 1, 7]);
                ...
                assert!(
                    ...
                    "failed Source vote retains original fragment Host"
                );
                drop(error);
                assert_eq!(pool.payload_used_bytes().unwrap(), 0);
            }
...
fn inactive_projected_selection_needs_no_fabricated_source_or_host_owner() {
    ...
    assert!(
        PreparedPartitionCaptureProgram::new_selected(
            &transport,
            &source,
            &coordinate,
        ...
        .is_err(),
        "active prefill still requires its actual source"
    );
    coordinate.phase = CapturePhase::Decode;
    coordinate.prediction = 1;
    let mut program = PreparedPartitionCaptureProgram::new_selected(
        &transport,
        &source,
        &coordinate,
    ...
    ledger.begin_step();
    let epoch = DistributedCommitEpoch::new(29).unwrap();
    program
        .prepare(
            &source,
            CapturePhase::Decode,
...
impl ConsensusTransport for SkippedRanks {
    ...
    fn all_gather_words(&self, _: &[u32]) -> Result<Vec<u32>, Infallible> {
        panic!("unbounded skipped transport")
    }
}
impl BoundedConsensusTransport for SkippedRanks {
    ...
    type GatherOutput = ();
    fn submit_all_gather_words(&self, _: &[u32]) -> Result<Submission<(), Done>, Infallible> {
        panic!("untyped skipped transport")
    }
    fn resolve_all_gather_words(&self, _: ()) -> Result<Vec<u32>, Infallible> {
        panic!("untyped skipped output")
    }
}
impl PartitionCaptureTransport for SkippedRanks {
    ...
    fn fail_capture_exchange(&self, cause: &PartitionCaptureExchangeError) {
        ...
        assert!(matches!(
            cause,
            PartitionCaptureExchangeError::Protocol("skipped source has an active scalar")
        ));
        self.failures.set(self.failures.get() + 1);
    }
    fn capture_word_destination(
    ...
    fn gather_capture_frame(
        ...
        assert_eq!(
            frame.words()[13],
            0,
            "skipped rows never advertise a scalar"
        );
        let mut out = PartitionCaptureBuffer::funded(frame.gathered_words(), &self.base.0.funding)?;
        for rank in 0..4 {
            let mut words: [u32; 16] = frame.words().try_into().unwrap();
            words[3] = rank as u32;
            if self.reject && rank == 3 {
                words[13] = 1;
            }
            out.extend_from_slice(&words)?;
        }
...
fn projected_limit_policy_coordinates_skip_and_retains_original_host_on_failure() {
    use super::super::prefill::{inference, projected_receipt, projected_source_limits};
    use crate::capture::partition::{
        PartitionCaptureFragmentGeometry, PartitionCaptureLocalSource,
    };
    for (policy, reject, foreign_geometry) in [
        ...
        let mut quotation = CaptureLedger::new(source.admission());
        quotation.begin_step();
        let prototype = projected_receipt(
            &source,
            PartitionCaptureCombination::SumF64ToF32,
            &metadata,
            &mut quotation,
        );
        let mut geometry: Vec<_> = prototype
            .producers()
            .flat_map(|(rank, p)| {
                p.fragments().iter().enumerate().map(move |(fragment, g)| {
                    (rank, fragment, p.local_shape().to_vec(), g.local().clone())
            ...
            .collect();
        if foreign_geometry {
            geometry[0].3.ends[2] -= 1;
        }
        let raw = CaptureTransform::Slice;
        let rows: Vec<_> = geometry
            .iter()
            .map(|(rank, fragment, shape, slice)| {
                let mut estimate = estimate(*rank);
                estimate.capture.retained_bytes = 2 << 20;
                PartitionCaptureFragmentGeometry {
                    producer: *rank,
                    fragment: *fragment,
                    local_shape: shape,
                    local_slice: slice,
        ...
        assert!(
            spent.host_bytes > 0,
            "the initial coordination remains spent"
        );
        ...
        assert_eq!(
            quota.total(),
            spent,
            "skips, refusals and votes never refund earlier credits"
        );
        drop(program);
        drop(frame);
        drop(bank);
        ...
        if let Some(failure) = failure {
            assert!(
                pool.payload_used_bytes().unwrap() > 0,
                "the preparation or vote failure owns original Host"
            );
            drop(failure);
        }
        assert_eq!(pool.payload_used_bytes().unwrap(), 0);
