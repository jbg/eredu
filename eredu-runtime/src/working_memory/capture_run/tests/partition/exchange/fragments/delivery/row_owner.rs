...
fn retained_contiguous_row_binds_real_epoch_and_lends_each_original_chunk_once() {
    ...
    for (transform, combination, local, abandon) in [
        ...
        let sources: Vec<_> = geometry
            .iter()
            .map(
                |(producer, fragment, shape, slice)| PartitionCaptureFragmentSource {
                    producer: *producer,
                    fragment: *fragment,
                    local_shape: shape,
                    local_slice: slice,
        ...
        let mut actual = context(&source, 0);
        actual.forward_epoch = epoch.value();
        let limits = PartitionCaptureReceiptLimits {
            max_producers: 3,
            max_fragments: 3,
            max_record_bytes: 64 << 10,
        };
        let mut ledger_capture = CaptureLedger::new(source.admission());
        ledger_capture.begin_step();
        ...
        assert_eq!(row.produces(), local == 0);
        let spent = ledger_capture.total();
        let mut native = Native::default();
        if abandon {
            let failure = row.finish().unwrap_err();
            drop(run);
            drop(reservation);
            assert!(ledger(&pool).0 > 0);
            assert_eq!(ledger_capture.total(), spent);
            drop(failure);
                    ...
                    values: (0..2)
                        .flat_map(|head| {
                            (0..width).map(move |column| {
                                head as f32 * 100.0 + chunk as f32 * 10.0 + (column + offset) as f32
                            })
                        })
                        .collect(),
                };
                let hook = hook
                    .observe(&mut native, &value, inference(), chunk)
                    .unwrap();
                row.return_local(hook).unwrap();
                assert!(row.take_local(chunk, epoch).is_err());
            } else {
        ...
        if local == 0 {
            ...
            assert_eq!(native.raw + native.summary + native.histogram, 2);
        }
        let delivery = row.finish().unwrap();
        assert_eq!(delivery.receipt_plan().context().forward_epoch, 19);
        drop(run);
        drop(reservation);
        assert!(ledger(&pool).0 > 0);
        drop(delivery);
