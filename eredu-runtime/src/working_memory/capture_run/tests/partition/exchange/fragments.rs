...
fn estimate(rank: usize) -> PartitionCaptureNativeEstimate {
    PartitionCaptureNativeEstimate {
        capture: CaptureUsage {
            retained_bytes: 100 + rank as u64,
            host_bytes: 200 + rank as u64,
            encoded_bytes: 8192,
            captures: 1,
        },
...
fn original_fragment_allowance_prices_real_sources_and_retains_one_use_local_custody() {
    for combination in [
        ...
        let geometry: Vec<_> = receipt
            .producers()
            .flat_map(|(rank, projection)| {
                projection
                    .fragments()
                    .iter()
                    .enumerate()
                    .map(move |(fragment, value)| {
                        (
                            rank,
                            fragment,
        ...
        assert_eq!(
            rows.len(),
            2,
            "empty shard acknowledges without inventing a native primitive"
        );
        let prior = ledger.total();
        let mut allowance = PreparedPartitionFragmentAllowance::prepare(
            &transport,
            &mut receipt,
            &rows,
            &funding,
        ...
        assert_eq!(
            ledger.total(),
            prior.checked_add(allowance.global_reserved()).unwrap()
        );
        assert!(
            allowance.global_reserved().retained_bytes
                >= estimate(0).capture.retained_bytes + estimate(3).capture.retained_bytes
        );
        ...
        assert!(
            allowance
                .take_local_fragment(&receipt, 1, &TensorDtype::F16, estimate(0))
                .is_err()
        );
        assert_eq!(
            ledger.total(),
            total,
            "child consumption and refusals never reset global credits"
        );
...
fn original_fragment_source_rejects_wrong_coordinates_nonlinear_terms_and_replayed_native_attempts()
{
    for mismatch in [
        ...
        let mut geometry: Vec<_> = receipt
            .producers()
            .flat_map(|(rank, projection)| {
                projection
                    .fragments()
        ...
        let raw = CaptureTransform::Slice;
        let summary = CaptureTransform::Summary;
        let mut rows: Vec<_> = geometry
            .iter()
            .map(
                |(rank, fragment, shape, slice)| PartitionCaptureFragmentSource {
                    producer: *rank,
                    ...
                    local_shape: shape,
                    local_slice: slice,
                    transform: if mismatch == "transform" {
                        &summary
                    } else {
                        &raw
                    },
        ...
        match mismatch {
            ...
            "extra" => rows.push(PartitionCaptureFragmentSource {
                producer: 1,
                fragment: 0,
                local_shape: &geometry[0].2,
                local_slice: &geometry[0].3,
                transform: &raw,
                dtype: TensorDtype::F32,
                estimate: estimate(1),
            }),
        ...
        let result = PreparedPartitionFragmentAllowance::prepare(
            &transport,
            &mut receipt,
            &rows,
            &funding,
        ...
        let error = if mismatch == "native" {
            ...
            let error = allowance
                .take_local_fragment(&receipt, 0, &TensorDtype::Bf16, estimate(0))
                .unwrap_err();
            assert!(
                allowance
                    .take_local_fragment(&receipt, 0, &TensorDtype::F32, estimate(0))
                    .is_err()
            );
            error
        } else {
...
fn fragment_geometries_keep_original_admission_and_raw_additive_nonlinear_inputs() {
    for transform in [
        ...
        let source = if matches!(transform, CaptureTransform::Histogram { .. }) {
            super::super::histogram::histogram_source()
        } else {
            admit(raw, point(), 4, false)
        };
        let global = vec![5, 2];
        let slice = resolve_slice(
            &source.admission().points()[0],
            &source.admission().plan().selections[0],
            &global,
        )
        .unwrap();
        let shard = CaptureContiguousProjectionPlan::prepare(&global, &slice, 1, 1..2, 1)
            .unwrap()
            .construct();
        let term = CaptureContiguousProjectionPlan::prepare(&global, &slice, 1, 0..2, 1)
            .unwrap()
            .construct();
        let geometry = CaptureTensorGeometry::prepare_partition(
            source.admission(),
            0,
        ...
        assert_eq!(
            geometry.admission().plan().selections[0].transform,
            transform
        );
        assert_eq!(geometry.source_shape(), [5, 2]);
        if matches!(transform, CaptureTransform::Preview { .. }) {
            assert_eq!(geometry.native_transform(), &transform);
            assert_eq!(geometry.shape(), [2]);
        } else {
            assert_eq!(geometry.native_transform(), &CaptureTransform::Slice);
            assert_eq!(geometry.shape(), [5, 2]);
        }
        assert!(
        ...
        match transform {
            CaptureTransform::Summary => {
                let part = CaptureSummaryGeometry::prepare_partition(
                    source.admission(),
                    0,
                    CapturePhase::Prefill,
                    0,
                ...
                .unwrap();
                assert_eq!(part.source_shape(), [5, 1]);
                assert_eq!(part.shape(), [5, 1]);
            }
            CaptureTransform::Histogram { .. } => {
                let part = CaptureHistogramGeometry::prepare_partition(
                    source.admission(),
                    0,
                    CapturePhase::Prefill,
                    0,
                ...
                assert_eq!(part.source_shape(), [5, 1]);
                assert_eq!(part.shape(), [5, 1]);
                assert_eq!(part.edges(), [-1.0, 0.25, 2.0]);
            }
            _ => {
                let part = CaptureTensorGeometry::prepare_partition(
                    source.admission(),
                    0,
                    CapturePhase::Prefill,
                    0,
...
fn original_fragment_host_bank_protects_each_native_and_received_destination() {
    for combination in [
        ...
        assert_eq!(host.fragment_count(), 2);
        ...
        for i in 0..output.len() {
            output.push_f32(i as f32 - 2.25).unwrap();
        }
        let output = output.finish().unwrap();
        let escaped = output.observation().clone();
        bank.record(
            &receipt,
            0,
            0,
            &TensorDtype::F16,
            estimate(0).capture,
            PartitionFragmentValue::Tensor(output),
        )
        .unwrap();
        assert!(
            bank.take_local(&receipt, 0, &TensorDtype::F16, estimate(0))
                .is_err()
        );
        assert!(
            bank.take_received(&receipt, 1, 0).is_err(),
            "empty producer has no payload constructor"
        );
        let PartitionFragmentDestination::Tensor(claim) =
            bank.take_received(&receipt, 3, 0).unwrap()
        else {
            panic!("raw received source")
        };
        let mut output = claim.prepare().unwrap();
        for i in 0..output.len() {
            output.push_f32(11.5 - i as f32).unwrap();
        }
        let output = output.finish().unwrap();
        bank.record(
            &receipt,
        ...
        assert!(bank.value(3, 0).is_some());
        assert_eq!(quota.total(), before);
        assert!(
            transport.calls.borrow().is_empty(),
            "host claims imply no transport or native work"
        );
        drop((bank, loan));
        drop(run);
        drop(reservation);
...
fn original_fragment_host_bank_rejects_underfunding_and_same_shape_rank_swaps() {
    for short in [true, false] {
        ...
        .unwrap();
        let host = PartitionFragmentHostPlan::prepare(&receipt).unwrap();
        let h = host.initialization_peak_bytes() - u64::from(short);
        let pool = capture_test_ledger(h, 0).unwrap();
        let (reservation, run) = fresh(&pool, h);
        ...
        let error = if short {
            ...
            let (destination, loan) = bank
                .take_local(&receipt, 0, &TensorDtype::F32, estimate(0))
                .unwrap()
                .into_parts();
            let PartitionFragmentDestination::Tensor(claim) = destination else {
                panic!("tensor")
            ...
            let mut output = claim.prepare().unwrap();
            for i in 0..output.len() {
                output.push_f32(i as f32 + 0.125).unwrap();
            }
            ...
            let error = bank
                ...
                .unwrap_err();
            assert!(!bank.complete());
            assert!(bank.take_received(&receipt, 3, 0).is_err());
            assert!(
                bank.take_local(&receipt, 0, &TensorDtype::F32, estimate(0))
                    .is_err()
            );
            drop((bank, loan));
            error
...
fn contiguous_receivers_decode_exact_local_shapes_raw_terms_and_empty_envelopes() {
    for combination in [
        ...
        for transform in [
            ...
            let mut receipt = PartitionCaptureReceiptPlan::new_contiguous_shared_funded(
                &source,
                &context(&source, 0),
                1,
                &producers,
            ...
            let geometry: Vec<_> = receipt
                ...
                .collect();
            let native = if combination == PartitionCaptureCombination::SumF64ToF32
                && !matches!(transform, CaptureTransform::Preview { .. })
            {
            ...
            let mut bank = run
                .prepare_partition_fragments(&reservation, host, allowance)
                .unwrap();
            let projection = receipt.producer(3).unwrap();
            let selected = projection.fragments()[0].local().shape.clone();
            let elements = selected.iter().product::<u64>();
            let values: Vec<f32> = (0..elements)
                .map(|i| [-0.75, 0.0, 1.25, -0.5, 1.0][i as usize % 5])
                .collect();
            let raw_payload = combination == PartitionCaptureCombination::SumF64ToF32
                || matches!(
                    transform,
                    CaptureTransform::FullTensor | CaptureTransform::Preview { .. }
                );
            let payload = if raw_payload {
                let kept = if let CaptureTransform::Preview { max_elements } = transform {
                ...
                let shape = if matches!(transform, CaptureTransform::Preview { .. }) {
                    ...
                    selected.iter().map(|n| *n as usize).collect()
                };
                CapturePayload::Tensor(
                    TensorObservation::new(
                        shape,
                        TensorObservationData::F32(values[..kept as usize].to_vec()),
                    )
                    .unwrap(),
                )
            ...
            let selected_source = &source.admission().plan().selections[0];
            let point = &source.admission().points()[0];
            let outcome = if let CaptureTransform::Preview { max_elements } = transform {
                CaptureOutcome::Truncated {
                    available_elements: elements,
                    emitted_elements: elements.min(max_elements),
                }
            } else {
            ...
            let record = CaptureRecord {
                ...
                selection_id: selected_source.id.clone(),
            ...
            bank.decode_contiguous_producer(&receipt, 3, &wire, None, &funding)
                .unwrap();
            match (bank.value(3, 0).unwrap(), &payload) {
                (PartitionFragmentValue::Tensor(actual), CapturePayload::Tensor(expected)) => {
                    assert_eq!(actual.observation().as_ref(), expected)
                }
                (PartitionFragmentValue::Summary(actual), CapturePayload::Summary(expected)) => {
                    assert_eq!(actual.observation(), expected)
                }
                (
                    PartitionFragmentValue::Histogram(actual),
                    CapturePayload::Histogram(expected),
                ) => assert_eq!(actual.observation(), expected),
            ...
            assert!(
                bank.decode_contiguous_producer(&receipt, 3, &wire, None, &funding)
                    .is_err(),
                "no destination replay"
            );
            if combination == PartitionCaptureCombination::Disjoint {
                let mut empty = envelope.clone();
                empty.producer_rank = 1;
                empty.fragments.clear();
                let wire = serde_json::to_vec(&empty).unwrap();
                bank.decode_contiguous_producer(
                    &receipt,
                    1,
                    &wire,
                ...
                .unwrap();
                assert!(
                    bank.decode_contiguous_producer(&receipt, 1, &wire, None, &funding)
                        .is_err(),
                    "empty dtype comes from actual retained source"
                );
                empty.fragments = envelope.fragments.clone();
                let wire = serde_json::to_vec(&empty).unwrap();
                assert!(
                    bank.decode_contiguous_producer(
                        &receipt,
                    ...
                    .is_err(),
                    "empty producer cannot smuggle a payload"
                );
            }
            drop(bank);
            drop(run);
    ...
    for combination in [
        ...
        for transform in [
            ...
            let mut bank = run
                ...
                .unwrap();
            assert!(
                bank.encode_contiguous_producer(&receipt, None, &funding)
                    .is_err(),
                "uncompleted local slot is not encodable"
            );
                ...
                CapturePayload::Tensor(
                    TensorObservation::new(
                        ...
                        TensorObservationData::F32(
                            (0..kept)
                                .map(|i| {
                                    [f32::NAN, -0.0, f32::INFINITY, f32::NEG_INFINITY, 1.25]
                                        [i as usize % 5]
                                })
                                .collect(),
                        ),
            ...
            let record = CaptureRecord {
                ...
                source_dtype: Some(TensorDtype::F16),
                selected_shape: Some(selected),
                outcome: crate::capture::completed_capture_outcome(&transform, elements),
            ...
            let identity = PartitionCaptureTensorReceipt {
            ...
            let value = match destination {
                PartitionFragmentDestination::Routed(_) => {
                    panic!("dense fixture returned sparse assembly")
                }
                PartitionFragmentDestination::Tensor(v) => PartitionFragmentValue::Tensor(
                    v.decode_partition_receipt_combined(&expected, identity, &funding, combination)
                        .unwrap(),
                ),
                PartitionFragmentDestination::Summary(v) => PartitionFragmentValue::Summary(
                    v.decode_partition_receipt(&expected, identity, &funding)
                        .unwrap(),
                ),
                PartitionFragmentDestination::Histogram(v) => PartitionFragmentValue::Histogram(
                    v.decode_partition_receipt(&expected, identity, &funding)
                        .unwrap(),
                ),
            };
            ...
            .unwrap();
            let escaped = match bank.value(0, 0).unwrap() {
                PartitionFragmentValue::Tensor(v) => Some(v.observation().clone()),
                _ => None,
            };
            let pointer = match bank.value(0, 0).unwrap() {
                PartitionFragmentValue::Routed(_) => {
                    panic!("dense fixture returned sparse assembly")
                }
                PartitionFragmentValue::Tensor(v) => match v.observation().data() {
                    TensorObservationData::F32(v) => v.as_ptr() as usize,
                    _ => unreachable!(),
                },
                PartitionFragmentValue::Summary(v) => {
                    v.observation() as *const CaptureSummary as usize
                }
                PartitionFragmentValue::Histogram(v) => v.observation().counts.as_ptr() as usize,
            };
            let spent = quota.total();
            let output = bank
                .encode_contiguous_producer(&receipt, None, &funding)
                .unwrap();
            ...
            let error = bank
                ...
                .unwrap_err();
            assert_eq!(quota.total(), spent);
            assert!(bank.value(0, 0).is_some());
            drop((bank, loan));
            drop(run);
            drop(reservation);
            drop(error);
            assert_eq!(ledger(&pool).0 == 0, escaped.is_none());
            drop(escaped);
            assert_eq!(ledger(&pool).0, 0);
            drop(rows);
            drop(geometry);
...
fn paid_fragment_assembly_matches_ordinary_rank_order_and_post_sum_nonlinear_results() {
    for combination in [
        ...
        for transform in [
            ...
            .unwrap();
            let evidence =
                crate::capture::partition::PreparedPartitionCaptureEvidence::prepare_contiguous(
                    &receipt,
                    &mut allowance,
                    &funding,
                )
                .unwrap();
            let charges: Vec<_> = receipt
                .producers()
                .filter_map(|(rank, p)| {
                    (!p.fragments().is_empty()).then(|| {
                        (
                            rank,
                            allowance
            ...
            let host = PartitionFragmentHostPlan::prepare(&receipt).unwrap();
            assert_eq!(
                host.assembly_peak_bytes() != 0,
                combination == PartitionCaptureCombination::SumF64ToF32
                    && matches!(
                        transform,
            ...
            let (destination, target) = frame
                .take_assembled_invocation_destination(0, TensorDtype::F16, charge)
                .unwrap();
            ...
            let mut delivery = ordinary.into_delivery();
            for (producer, projection) in receipt.producers() {
                ...
                if !projection.fragments().is_empty() {
                    ...
                    let values: Vec<f32> = (0..elements)
                        .map(|i| {
                            if producer == 0 {
                                [-0.75, 0.0, 1.25, -0.5, 1.0][i as usize % 5]
                            } else {
                                [0.5, 1.0, -0.75, 0.25, 1.5][i as usize % 5]
                            }
                        })
                        .collect();
                    let raw_payload = combination == PartitionCaptureCombination::SumF64ToF32
                    ...
                    let payload = if raw_payload {
                        ...
                        CapturePayload::Summary(crate::capture::partition::summarize_f32(&values))
                    } else {
                        ...
                        crate::capture::partition::fill_histogram_f32(&values, &mut histogram)
                            .unwrap();
                ...
                delivery
                    .receive(producer, &bytes, &mut ordinary_quota)
                    .unwrap();
                if producer != 0 {
                    bank.decode_contiguous_producer(
                        &receipt,
                    ...
                    let identity = PartitionCaptureTensorReceipt {
                        ...
                        producer: 0,
                        dtype: TensorDtype::F16,
                        charged: envelope.fragments[0].record.charged,
                    };
                    let value = match destination {
                        PartitionFragmentDestination::Routed(_) => {
                            panic!("dense fixture returned sparse assembly")
                        ...
                        PartitionFragmentDestination::Tensor(v) => PartitionFragmentValue::Tensor(
                            v.decode_partition_receipt_combined(
                                &bytes,
                                identity,
                                &funding,
                                combination,
                        ...
                        PartitionFragmentDestination::Summary(v) => {
                            PartitionFragmentValue::Summary(
                                v.decode_partition_receipt(&bytes, identity, &funding)
                                    .unwrap(),
                            )
                        }
                        PartitionFragmentDestination::Histogram(v) => {
                            PartitionFragmentValue::Histogram(
                                v.decode_partition_receipt(&bytes, identity, &funding)
                                    .unwrap(),
                            )
                        }
            ...
            assert!(bank.complete());
            let expected = delivery.finish(&mut ordinary_quota).unwrap();
            let spent = quota.total();
            let (value, charged) = bank
                .assemble_into(&receipt, destination, &TensorDtype::F16)
                .unwrap();
            assert_eq!(quota.total(), spent);
            assert_eq!(charged, expected.capture().record().charged);
            let actual = match &value {
                PartitionFragmentValue::Routed(_) => {
                    panic!("dense fixture returned sparse assembly")
                }
                PartitionFragmentValue::Tensor(v) => {
                    eredu_core::capture::CapturePayloadWire::Tensor(
                        v.observation().as_observation(),
                    )
                }
                PartitionFragmentValue::Summary(v) => {
                    eredu_core::capture::CapturePayloadWire::Summary(v.observation())
                }
                PartitionFragmentValue::Histogram(v) => {
                    eredu_core::capture::CapturePayloadWire::Histogram(v.observation())
                }
            };
            assert_eq!(
            ...
            if combination == PartitionCaptureCombination::SumF64ToF32
                && matches!(transform, CaptureTransform::Histogram { .. })
            {
                let PartitionFragmentValue::Histogram(histogram) = &value else {
                    unreachable!()
                };
                assert_eq!(histogram.observation().counts, [4, 4]);
                assert_eq!(histogram.observation().above, 2);
            }
            target
                .record(
                    &mut frame,
                    crate::working_memory::capture_run::PartitionFragmentDelivered {
                        value,
                        evidence,
                        charged,
                        dtype: TensorDtype::F16,
                ...
                .unwrap();
            assert_eq!(
                frame.records()[0].charged,
                expected.capture().record().charged
            );
            assert_eq!(frame.partition_evidence().len(), 1);
            assert_eq!(quota.total(), spent);
            let escaped = match frame.records()[0].payload.as_ref().unwrap() {
                CapturePayload::SharedTensor(tensor) => Some(tensor.clone()),
                _ => None,
            };
            drop(frame);
            drop(final_bank);
            drop(run);
            drop(reservation);
            assert_eq!(ledger(&pool).0 > 0, escaped.is_some());
            drop(escaped);
            assert_eq!(ledger(&pool).0, 0);
            assert!(transport.calls.borrow().is_empty());
        }
...
fn incomplete_fragment_assembly_consumes_final_claim_and_retains_both_paid_destinations() {
    ...
    let mut frame = final_bank
        ...
        .unwrap();
    let spent = quota.total();
    let destination = PartitionFragmentDestination::Tensor(frame.take_tensor(0).unwrap());
    let error = bank
        .assemble_into(&receipt, destination, &TensorDtype::F32)
        .unwrap_err();
...
fn paid_contiguous_evidence_keeps_sorted_empty_ranks_and_exact_fragment_charges() {
    for combination in [
        ...
        let prior = allowance.local_used();
        let evidence =
            crate::capture::partition::PreparedPartitionCaptureEvidence::prepare_contiguous(
                &receipt,
                &mut allowance,
        ...
        for row in &evidence.value.contributions {
            let projection = receipt.producer(row.producer_rank).unwrap();
            ...
            assert_eq!(
                row.charged,
                allowance
                    .fragment_record_charge(&receipt, row.producer_rank, 0)
                    .unwrap()
                    .1
            );
