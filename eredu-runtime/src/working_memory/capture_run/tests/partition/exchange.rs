...
impl PartitionCaptureTransport for Receipts {
    ...
    fn fail_capture_exchange(&self, _: &PartitionCaptureExchangeError) {
        panic!("completed refusal is not unagreed transport failure")
    }
    ...
    fn gather_capture_frame(
        ...
        match frame.kind() {
            PartitionCaptureFrameKind::Preparation | PartitionCaptureFrameKind::Delivery => {
                for rank in 0..4 {
                    ...
                    if frame.kind() == PartitionCaptureFrameKind::Preparation {
                        ...
                        let length = if rank == 3 {
                            self.source.len() as u64
                        } else {
                            0
                        };
                        words[14] = length as u32;
                        words[15] = (length >> 32) as u32;
                    } else if self.reject_delivery && rank == 1 {
                        words[13] = 0;
                    }
            ...
            PartitionCaptureFrameKind::Payload => {
                for rank in 0..4 {
                    for index in 0..frame.words().len() {
                        let mut bytes = [0; 4];
                        if rank == 3 {
...
fn scheduled_partition_exchange_keeps_decoded_h_until_final_delivery_vote() {
    for reject in [false, true] {
        ...
        .unwrap();
        let coordinates =
            ComponentCoordinateMap::range(shape[0] as usize, 0..shape[0] as usize).unwrap();
        ...
        quota.begin_step();
        let receipt = PartitionCaptureReceiptPlan::new(
            source.clone(),
            context.clone(),
        ...
        let bytes = String::from_utf8(bytes)
            .unwrap()
            .replace("retained-receipt", receipt.identity())
            .into_bytes();
        ...
        let Some(CapturePayload::SharedTensor(value)) = &frame.records()[0].payload else {
            panic!("original host destination")
        };
        ...
        assert_eq!(evidence.receipt_plan_identity, identity);
        assert_eq!(evidence.producers, vec![3]);
        assert_eq!(evidence.contributions[0].charged, charged);
    ...
    fn gather_capture_frame(
        ...
        if self.fail_at == Some(ordinal) {
            return Err(PartitionCaptureExchangeError::Protocol(
                "injected completed vote failure",
            ));
        }
...
fn failed_partition_source_or_final_vote_spends_epoch_and_disables_capture_delivery() {
    ...
    for case in 0..4 {
        ...
        let transport = ProgramVotes {
            funding: funding.clone(),
            fail_at: (case < 2).then_some(case + 1),
            participants: 1,
            calls: RefCell::new(Vec::new()),
        ...
        let rows: Vec<_> = source
            ...
            .selections
            .iter()
            .map(|_| PartitionCaptureProducerSource {
                producer: 0,
        ...
        assert_eq!(
            *transport.calls.borrow(),
            calls,
            "refused epochs cannot issue another frame"
        );
        assert_eq!(
            ledger.total(),
            charged,
            "failed attempts retain every reserved credit"
        );
        assert!(
            frame
                .records()
        ...
        assert!(
            ...
            "escaped error retains paid preparation custody"
        );
        drop(first_error);
        assert!(retired.load(Ordering::SeqCst));
        assert_eq!(pool.payload_used_bytes().unwrap(), 0);
...
fn receiver_source_readiness_spends_each_coordinated_epoch_without_refunding() {
    for fail_vote in [false, true] {
        ...
        let error = if fail_vote {
            ...
            assert_eq!(
                program.take_receiver_source(2).unwrap(),
                None,
                "producer owns its transform's source completion"
            );
            assert_eq!(
                program.take_receiver_source(1).unwrap(),
                None,
                "inactive selection has no dependency work"
            );
            assert_eq!(
                program.take_receiver_source(0).unwrap(),
            ...
            assert_eq!(
                ...
                spent,
                "dropping refusal does not refund controls"
            );
            ...
            assert!(
                program.take_receiver_source(0).is_err(),
                "failed epoch cannot reuse prior readiness"
            );
        ...
        assert_eq!(
            ledger.total(),
            charged,
            "source readiness never refunds the original capture budget"
        );
        ...
        assert!(
            !retired.load(Ordering::SeqCst),
            "escaped refusal retains original source and H"
        );
        drop(error);
        assert!(retired.load(Ordering::SeqCst));
        assert_eq!(pool.payload_used_bytes().unwrap(), 0);
