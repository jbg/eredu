...
fn partition_summary_receipt_checks_scalar_counts_and_retains_failed_claim_custody() {
    for case in ["valid", "counts", "duplicate", "reservation"] {
        ...
        let summary = CaptureSummary { elements: 10, finite: 4, non_finite: 6,
            nan: 2, positive_infinity: 2, negative_infinity: 2,
            min: Some(-4.0), max: Some(2.0), mean: Some(-1.0), rms: Some(10f64.sqrt()) };
        ...
        let encoded = PartitionCaptureRecordEncoding::new(&context, "retained-receipt", 3,
        ...
        let bytes = match case {
            ...
            "duplicate" => text.replace("\"finite\":4", "\"finite\":4,\"finite\":4").into_bytes(),
            _ => text.as_bytes().to_vec(),
        };
        drop(encoded);
        if case == "reservation" { refuse.store(true, Ordering::SeqCst); }
        let decoded = claim.decode_partition_receipt(&bytes, expected(&context, record.charged), &funding);
        ...
        let error = if case == "valid" {
            ...
            frame.record_summary(decoded, TensorDtype::F16, native_usage()).unwrap();
            assert_eq!(frame.records()[0].payload.as_ref(), record.payload.as_ref());
            None
        } else { Some(decoded.unwrap_err()) };
