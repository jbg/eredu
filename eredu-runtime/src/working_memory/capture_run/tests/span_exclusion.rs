...
use eredu_nn::{workspace::*, Error as NnError, Tensor};
...
fn quote(pool: &WorkingMemoryPool, h: u64) -> IncrementalInferenceQuote {
    let g = geometry();
    let ctx = WorkspaceContext::new(Facts);
    let registered = RegisteredWorkspaceStorage::<u32>::bind(pool, &ctx, []).unwrap();
...
fn headroom(pool: &WorkingMemoryPool, r: &InferenceRequest) -> u64 {
    let u = pool.0.usage.lock().unwrap();
    let s = &u.funding[&r.memory_reservation().unwrap().0.funding.unwrap()];
...
fn bank(
    ...
    run.prepare_capture_run(r.memory_reservation().unwrap(), plan(source))
        .unwrap()
}
fn ticket(
...
fn accepted_span_exact_headroom_and_last_alias_retirement_are_atomic() {
    for short in [false, true] {
        ...
        let (segment, reg) = bank
            ...
            .unwrap();
        let receipt = q
            .reserved_span_workspace(r.memory_reservation().unwrap())
            .unwrap();
        let n = receipt
        ...
        let mut storage = native
            .adopt_storage_individually([(41u32, earlier)])
            .unwrap();
        ...
        let allocated = native
            .adopt_storage_individually([(42u32, 16), (43, 0)])
            .unwrap();
        fenced(run.scope());
        assert!(segment
            .activate_reserved_span(&mut native, &receipt, &cx)
...
fn sibling_scopes_new_host_copy_holds_and_zero_publications_cannot_spend() {
    ...
    segment
        .activate_reserved_span(&mut native, &receipt, &cx)
        .unwrap();
    let own_storage = native.adopt_storage_individually([(20u32, 8)]).unwrap();
    let before = ledger(&pool);
    for entries in [vec![], vec![(8u32, 0)], vec![(9, 1)], vec![(20, 8)]] {
        fenced(sibling.adopt_storage_individually(entries));
    }
    fenced(run.scope());
    fenced(run.sampler_scope());
    fenced(sampler.hold_sampler_payload(1));
    let rsv = r.memory_reservation().unwrap();
    ...
    let (copy_run, copy_scope) = pool
        ...
        .unwrap();
    let values = copy_scope.adopt_storage_individually([(7u64, 8)]).unwrap();
...
fn wrong_receipt_scope_chunk_epoch_and_unstamped_channel_never_activate() {
    ...
    let receipt = q
        ...
        .unwrap();
    let wrong = other_q
        .reserved_span_workspace(other.memory_reservation().unwrap())
        .unwrap();
    let before = ledger(&pool);
    assert!(segment
...
fn previously_held_host_conversion_preserves_unheld_headroom_during_exclusion() {
    for count in [0usize, 3] {
        ...
        let registered = pool
            .register_storage([(original_key, original.metadata().capacity_bytes().unwrap())])
            .unwrap();
        ...
        assert_eq!(ledger(&pool).2, before.0 .2 - retained);
...
fn abandoned_or_failed_active_scope_keeps_exclusion_and_recovery_publication() {
    for explicit_certify in [false, true] {
        ...
        drop(failed);
        let recovery = native
            .adopt_storage_individually([(91u32, 8), (92, 0)])
            .unwrap();
        assert_eq!(
            ledger(&pool).0,
            before,
        ...
        fenced(run.scope());
        fenced(sibling.adopt_storage_individually([(1u32, 0)]));
        if explicit_certify {
            fenced(native.certify());
        } else {
        ...
        fenced(sibling.adopt_storage_individually([(2u32, 1)]));
        sibling.certify().unwrap();
...
fn active_span_blocks_new_plan_host_hold_without_losing_quote_then_exact_ticket_allows_it() {
    ...
    let error = q
        .into_funded_span_workspace(&run, r.memory_reservation().unwrap())
        .unwrap_err();
...
fn native_partition_alias_publication_obeys_the_actual_active_span() {
    use crate::working_memory::{
        ...
        storage::native_publication::{
            test_existing_native, test_native, PreparedNativePublication,
        },
    };
    let pool = WorkingMemoryPool::new(4_000_000, 0).unwrap();
    let source = source();
    ...
    assert_eq!(pool.used_bytes().unwrap(), 32);
    drop(partition);
    assert_eq!(pool.used_bytes().unwrap(), 0);
}
...
fn projected_host_binding_keeps_distinct_paid_owners_and_rejects_foreign_sources() {
    ...
    let (mut segment, registration) = original.prefill_source_bootstrap().unwrap()
        .begin_segment(&mut native, &cx).unwrap();
    let registered = pool.register_storage([(501u32, 16)]).unwrap();
    let before = ledger(&pool);
    assert!(matches!(destination.custody.bind_segment_source(&mut native, &segment, &registered),
        Err(WorkingMemoryError::IdentityMismatch)));
    assert!(matches!(destination.custody.bind_projected_segment_source(&mut native, &segment,
        &registered, equal_source.admission()), Err(WorkingMemoryError::IdentityMismatch)));
    assert!(matches!(foreign.custody.bind_projected_segment_source(&mut native, &segment,
        &registered, source.admission()), Err(WorkingMemoryError::IdentityMismatch)));
    assert_eq!(ledger(&pool), before, "rejection changes no original hold or spend");
    destination.custody.bind_projected_segment_source(&mut native, &segment,
        &registered, source.admission()).unwrap().commit();
    assert_eq!(ledger(&pool), before, "binding supplies no new destination or native grant");
    ...
    native.certify().unwrap();
    drop((segment, settled, original, foreign, foreign_quote, foreign_request, foreign_run));
    drop((quote, request, run));
    assert!(pool.used_bytes().unwrap() >= h + 16, "independent Host and source parcel retain their owners");
    drop(destination);
    assert!(pool.used_bytes().unwrap() >= 16);
    drop(parcel);
    assert_eq!(pool.used_bytes().unwrap(), 0);
}
