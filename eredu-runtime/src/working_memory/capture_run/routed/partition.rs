...
impl<'a> CapturePartitionRoutedHostPlan<'a> {
    ...
    pub fn prepare(
        ...
        let projection = receipt
            .producer(producer)
            .ok_or(CaptureRunHostError::ReceiptMismatch)?;
        let geometry = CaptureRoutedUnitsGeometry::prepare_receipt_partition(
            source.admission(),
            context,
            projection,
            fragment,
        ...
        ownership
            .validate_geometry(geometry.bank())
            .map_err(CaptureRoutedHostError::from)?;
        let native_rows = ownership
            .maximum_source_rows_checked(
                geometry.source_shape()[0] as u64,
                geometry.bank().routes_per_token,
            )
            .map_err(CaptureRoutedHostError::from)?;
        let host = CaptureRoutedHostPlan::prepare_source_rows(
            geometry,
    ...
    pub fn ownership(&self) -> &'a RoutedUnitCaptureOwnership {
        self.ownership
    }
    ...
    pub fn control_bytes() -> Option<usize> {
        let parts = [
            CaptureRoutedUnitsGeometry::partition_control_bytes()?,
            size_of::<Self>() * 2,
            ...
            size_of::<PartitionRoutedCaptureFailure>(),
            size_of::<ClaimedPartitionRoutedUnits>(),
            ...
            size_of::<CaptureRoutedTokenWindow>() * 2,
            size_of::<Option<CaptureRoutedTokenWindow>>(),
            size_of::<NativeExtent>() * 2,
            size_of::<[u64; 2]>(),
            size_of::<Result<[u64; 2], CaptureRoutedHostError>>(),
            size_of::<(&mut PreparedPartitionRoutedCapture, u64)>(),
            size_of::<Option<u64>>() * 2,
            size_of::<Result<(), CaptureRoutedHostError>>(),
            size_of::<Result<(), RoutedUnitValidationError>>(),
            ...
            size_of::<std::slice::Iter<'_, RoutedUnitCaptureRow>>(),
...
impl<'a, 'c> CapturePartitionRoutedClaim<'a, 'c> {
    pub(in crate::working_memory) fn from_plan(
        plan: CapturePartitionRoutedHostPlan<'a>,
        identity: claims::ReceiptIdentity,
    ) -> Self {
        Self {
    ...
    pub fn plan(&self) -> &CapturePartitionRoutedHostPlan<'a> {
        &self.plan
    }
    ...
    ) -> Result<PreparedPartitionRoutedCapture, CaptureRunHostError> {
        self.prepare_extent(actual_native_rows, false)
    }
    pub(in crate::working_memory) fn prepare_accumulating(
        self,
    ) -> Result<PreparedPartitionRoutedCapture, CaptureRunHostError> {
        self.prepare_extent(0, true)
    }
    pub(in crate::working_memory) fn prepare_received(
        self,
    ) -> Result<PreparedPartitionRoutedCapture, CaptureRunHostError> {
        let mut target = self.prepare_accumulating()?;
        target.extent = NativeExtent::Received;
        Ok(target)
    }
    fn prepare_extent(
        self,
        ...
        if actual_native_rows > self.plan.host.source_rows as u64
            || (!accumulating
                && self.plan.ownership.source_peer.is_none()
        ...
        if self.plan.receipt.identity().len() != receipt.len() {
            return Err(CaptureRunHostError::ReceiptMismatch);
        }
        ...
        Ok(PreparedPartitionRoutedCapture {
            owner,
            source,
            receipt,
            ...
            source_tokens,
            native_rows: actual_native_rows,
            token_window: None,
            extent: if accumulating {
                NativeExtent::Accumulating(None)
            } else {
                NativeExtent::Fixed
            },
        })
...
impl PreparedPartitionRoutedCapture {
    fn active_native_range(&self) -> Result<[u64; 2], CaptureRoutedHostError> {
        match self.extent {
            ...
            NativeExtent::Accumulating(None) | NativeExtent::Received => {
                Err(RoutedUnitValidationError::Chunks.into())
            }
        }
    }
    pub(in crate::working_memory) fn begin_invocation(
        &mut self,
        actual_rows: u64,
        token_window: CaptureRoutedTokenWindow,
    ) -> Result<(), CaptureRoutedHostError> {
        let result = (|| {
            self.owner.check()?;
            if !matches!(self.extent, NativeExtent::Accumulating(None))
                || self.owner.data.active.is_some()
                || self
                    .owner
                    .data
                    .value
            ...
            let end = self
                ...
                .filter(|n| *n <= self.owner.source_tokens)
                .ok_or(RoutedUnitValidationError::Chunks)?;
            self.extent = NativeExtent::Accumulating(Some([self.native_rows, end]));
            self.native_rows = end;
            self.token_window = Some(token_window);
            Ok(())
        })();
        self.owner.remember(result)
    }
    pub(in crate::working_memory) fn finish_invocation(
        &mut self,
    ) -> Result<(), CaptureRoutedHostError> {
        let result = (|| {
            self.owner.check()?;
            if !matches!(self.extent, NativeExtent::Accumulating(Some(_)))
                || self.owner.data.active.is_some()
                || self
                    .owner
                    .data
            ...
            self.extent = NativeExtent::Accumulating(None);
    ...
    fn received(&self) -> Result<(), CaptureRoutedHostError> {
        self.owner.check()?;
        if matches!(self.extent, NativeExtent::Received) {
            Ok(())
        } else {
            Err(RoutedUnitValidationError::Ownership.into())
        }
    }
    pub(in crate::working_memory) fn received_begin_row(
        &mut self,
        ...
        if peer != ownership.source_peer
            || usize::try_from(expert)
                .ok()
                .and_then(|e| ownership.coordinates.experts().global_to_local(e))
                .is_none()
        {
            return self
                .owner
                .remember(Err(RoutedUnitValidationError::Ownership.into()));
        }
        self.owner.begin_row(peer, token, slot, expert, coefficient)
    }
    pub(in crate::working_memory) fn received_push(
        &mut self,
        value: f32,
    ) -> Result<(), CaptureRoutedHostError> {
        self.received()?;
    ...
    pub(in crate::working_memory) fn received_finish_row(
        &mut self,
    ) -> Result<(), CaptureRoutedHostError> {
        self.received()?;
        self.owner.finish_row()
    }
    pub(in crate::working_memory) fn received_chunk(
        &mut self,
        start: u64,
        end: u64,
    ) -> Result<(), CaptureRoutedHostError> {
        ...
        if start != previous {
            return self
                .owner
                .remember(Err(RoutedUnitValidationError::Chunks.into()));
        }
    ...
    fn ownership<'a>(
        ...
        if receipt.identity().as_bytes() != self.receipt
            ...
            || receipt
                ...
                .and_then(|p| p.fragments().get(self.fragment))
                .is_none()
        {
            return Err(RoutedUnitValidationError::Ownership.into());
        }
        receipt
            .routed_producer(self.producer)
            .ok_or_else(|| RoutedUnitValidationError::Ownership.into())
    }
    ...
    pub fn finish(
        ...
        let result = (|| {
            ...
            if self.owner.data.active.is_some()
                || matches!(self.extent, NativeExtent::Accumulating(Some(_)))
            {
                return Err(RoutedUnitValidationError::Incomplete.into());
            }
            self.owner.data.value.validate_partition_with_scratch(
            ...
            if matches!(self.extent, NativeExtent::Received) {
                self.native_rows = end;
            } else if end != self.native_rows {
                return Err(RoutedUnitValidationError::Chunks.into());
            }
            Ok(())
        })();
        if let Err(cause) = result {
            return Err(PartitionRoutedCaptureFailure {
                cause,
                target: self,
            });
        }
        self.owner
            ...
            .value
            .rows
            .sort_unstable_by_key(|r| (r.source_peer, r.token, r.slot));
...
impl ClaimedPartitionRoutedUnits {
    ...
    pub const fn native_rows(&self) -> u64 {
        self.native_rows
    }
...
impl<'t, 'r> CapturePartitionRoutedWriter<'t, 'r> {
    ...
    pub fn request(&self) -> PartitionRoutedUnitCaptureRequest<'_> {
        PartitionRoutedUnitCaptureRequest {
            geometry: self.target.owner.data.value.geometry,
            source_tokens: self.target.source_tokens,
            ownership: self.ownership,
            slice: &self.target.owner.data.slice,
        }
    }
    ...
    pub fn source_layout(&self) -> PartitionRoutedUnitCaptureLayout<'r> {
        PartitionRoutedUnitCaptureLayout {
            geometry: self.target.owner.data.value.geometry,
            source_tokens: self.invocation_source_tokens(),
            ownership: self.ownership,
        }
    }
    ...
    pub fn native_rows(&self) -> u64 {
        let range = self
            .target
            .active_native_range()
            .expect("writer has an active native extent");
        range[1] - range[0]
    }
    /// Pre-exchange provider tokens for this invocation, preserving all batches.
    pub fn invocation_source_tokens(&self) -> u64 {
        self.target
            .token_window
            .map_or(self.target.source_tokens, |window| window.source_tokens())
    }
    ...
    pub fn logical_token(&self, physical: u64) -> Option<u64> {
        self.target.token_window.map_or_else(
            || (physical < self.target.source_tokens).then_some(physical),
            |window| window.logical_token(physical),
        )
    }
    ...
    pub fn begin_row(
        ...
        if peer != self.ownership.source_peer
            || usize::try_from(expert)
                .ok()
                .and_then(|e| self.ownership.coordinates.experts().global_to_local(e))
                .is_none()
        {
            return self
                .target
    ...
    pub fn prepare_with_source<'s, K: Clone + Ord + Send + Sync + 'static>(
        ...
        Ok(CapturePartitionRoutedTransfer {
            writer: self,
            source,
...
impl<K: Ord + Send + 'static> fmt::Debug for CapturePartitionRoutedTransfer<'_, '_, '_, K> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("CapturePartitionRoutedTransfer")
