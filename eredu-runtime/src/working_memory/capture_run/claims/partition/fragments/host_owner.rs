...
mod model;
pub use model::OwnedPartitionFragmentHostPlan;
...
pub(super) struct FragmentHostStorage {
    pub(super) rows: Vec<Slot>,
    pub(super) routed_merge: Vec<bool>,
    pub(super) scratch: Option<(usize, usize, CaptureTensorCustody)>,
    pub(super) prefill: Option<eredu_core::InferenceGeometry>,
...
impl FragmentHostStorage {
    ...
    pub(super) fn bind(
        ...
        identity.copy_from_slice(receipt.identity().as_bytes());
        PreparedPartitionFragmentDestinations {
            rows: self.rows,
            routed_merge: self.routed_merge,
            identity,
            allowance,
            source: receipt.shared_plan_source().clone(),
...
pub struct PartitionFragmentHostBindingError {
    ...
    _allowance: PreparedPartitionFragmentAllowance,
    _owner: PreparedPartitionFragmentHostFunding,
}
#[derive(Debug, thiserror::Error)]
enum PreparationCause {
    #[error(transparent)]
    Source(#[from] CaptureRunHostError),
    #[error(transparent)]
...
pub struct PartitionFragmentHostPreparationError {
    ...
    _prototype: PartitionCaptureReceiptPlan,
    _model_custody: Option<crate::working_memory::OriginalSpeculativeBudgetCustody>,
}
impl WorkingMemoryFundingRun {
    ...
    pub fn prepare_partition_fragment_host(
        &self,
        ...
        match result {
            Ok((storage, bytes)) => Ok(PreparedPartitionFragmentHostFunding {
                prototype,
                storage,
                bytes,
            }),
    ...
    pub fn bind(
        ...
        let result = (|| {
            ...
            if plan.initialization_peak_bytes() != self.bytes
                || plan.slots != self.storage.rows.len()
            {
                return Err(CaptureRunHostError::ReceiptMismatch);
            }
            ...
            if let Some((_, _, custody)) = &self.storage.scratch {
                custody.validate()?;
            }
            Ok(())
        })();
        match result {
            ...
            Err(cause) => Err(PartitionFragmentHostBindingError {
                cause,
...
pub(super) fn control_bytes() -> Option<usize> {
    let parts = [
        size_of::<FragmentHostStorage>() * 2,
        size_of::<OwnedPartitionFragmentHostPlan>() * 2,
        size_of::<Result<OwnedPartitionFragmentHostPlan, PartitionFragmentHostPreparationError>>(),
        size_of::<FragmentCustodySource<'_>>(),
        size_of::<(&PartitionFragmentHostPlan<'_>, FragmentCustodySource<'_>)>(),
        size_of::<(&FragmentCustodySource<'_>, &FragmentHostPlan<'_>)>(),
        size_of::<(&FragmentCustodySource<'_>, &PartitionFragmentHostPlan<'_>)>(),
        size_of::<(
            PartitionCaptureReceiptPlan,
            Option<eredu_core::InferenceGeometry>,
        )>(),
        size_of::<(
            OwnedPartitionFragmentHostPlan,
            crate::working_memory::OriginalSpeculativeBudgetCustody,
        )>(),
        size_of::<Option<crate::working_memory::OriginalSpeculativeBudgetCustody>>(),
        size_of::<(CaptureInvocationShape, Option<CaptureInvocationWindow>)>(),
        size_of::<PreparedPartitionFragmentHostFunding>() * 2,
        size_of::<PartitionFragmentHostBindingError>(),
        size_of::<PartitionCaptureReceiptPlan>(),
        size_of::<
            Result<PreparedPartitionFragmentHostFunding, PartitionFragmentHostPreparationError>,
        >(),
        size_of::<PartitionFragmentHostPreparationError>(),
        ...
        size_of::<(
            ...
            &PartitionFragmentHostPlan<'_>,
        )>(),
        size_of::<(
            PreparedPartitionFragmentHostFunding,
            &PartitionCaptureReceiptPlan,
            PreparedPartitionFragmentAllowance,
        )>(),
        size_of::<Option<eredu_core::InferenceGeometry>>(),
        ...
        size_of::<u64>(),
        size_of::<usize>() * 3,
        size_of::<(&PartitionCaptureReceiptPlan, &PartitionCaptureReceiptPlan)>(),
        size_of::<bool>() * 4,
        size_of::<Option<(&SharedCapturePlan, &SharedCapturePlan)>>(),
        size_of::<(&PartitionCaptureContext, &PartitionCaptureContext)>(),
    ];
    parts
        .into_iter()
