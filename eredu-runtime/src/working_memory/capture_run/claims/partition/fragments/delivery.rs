...
use crate::capture::partition::{
    PartitionCaptureDecoder, PartitionCaptureEvidenceError, PartitionCaptureExchange,
    PartitionCaptureExchangeError, PartitionCapturePayload, PartitionCaptureTransport,
    PreparedPartitionCaptureEvidence,
};
...
enum Cause {
    #[error("contiguous delivery source, rank, precision or destination differs")]
    Source,
    #[error(transparent)]
    ...
    #[error(transparent)]
    Encode(#[from] PartitionFragmentEncodingError),
    #[error(transparent)]
    Receive(#[from] PartitionFragmentReceiveError),
    #[error(transparent)]
    Assemble(#[from] PartitionFragmentAssemblyError),
    #[error(transparent)]
...
impl<T: PartitionCaptureTransport> fmt::Debug for PreparedPartitionFragmentDelivery<'_, T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("PreparedPartitionFragmentDelivery")
...
{
    fn preparation_control_bytes(
        receipt: &PartitionCaptureReceiptPlan,
        prefill: bool,
    ) -> Option<usize> {
        let parts = [
            size_of::<Self>() * 2,
            size_of::<PartitionFragmentDeliveryError>() * 3,
            size_of::<Cause>() * 2,
            size_of::<Result<Self, PartitionFragmentDeliveryError>>(),
            size_of::<PartitionFragmentDelivered>() * 2,
            size_of::<Result<PartitionFragmentDelivered, PartitionFragmentDeliveryError>>(),
            size_of::<(
                PartitionCaptureExchange<'t, T>,
                Vec<PartitionCaptureRankSource>,
                PreparedPartitionCaptureEvidence,
                TensorDtype,
            )>(),
            size_of::<
            ...
            size_of::<PartitionCaptureRankSource>() * 2,
            size_of::<Option<TensorDtype>>() * 2,
            size_of::<CaptureTensorCustody>() * 3,
            size_of::<SharedCapturePlan>(),
            size_of::<HostMetadataFunding>(),
            size_of::<Option<PreparedPartitionFragmentDestinations>>(),
            size_of::<CaptureUsage>() * 3,
            size_of_val(&receipt.producers()),
            size_of_val(
                &receipt
                    .producers()
                    .zip(<[PartitionCaptureRankSource; 0]>::default().iter()),
            ),
            ...
            size_of::<(
                Cause,
                Option<PreparedPartitionFragmentDestinations>,
                &SharedCapturePlan,
                &CaptureTensorCustody,
                Option<&CaptureTensorCustody>,
                &HostMetadataFunding,
            )>(),
            size_of::<(
                &mut PreparedPartitionFragmentDestinations,
            ...
            BackendFailure::source_retention_peak_bytes::<PartitionFragmentDeliveryError>()?
                .checked_mul(2)?,
            PartitionCaptureExchange::<T>::decoder_control_bytes::<Decoder<'_, '_, '_>>()?,
            if prefill {
                Self::prefill_control_bytes(&receipt)?
            } else {
                Self::invocation_control_bytes()?
            },
            hook::control_bytes::<T>()?,
        ];
        parts
            .into_iter()
    ...
    pub(crate) fn execution_metadata_bytes(
        ...
        if local_rank >= receipt.world_size() {
            return None;
        }
        Self::preparation_control_bytes(receipt, prefill)?
            .checked_add(eredu_nn::workspace::WorkspaceContext::metadata_vec_bytes::<
                PartitionCaptureRankSource,
            >(receipt.producers().count())?)?
            .checked_add(super::encode::producer_metadata_bytes(receipt, local_rank)?)?
            .checked_add(super::receive::receiver_metadata_bytes(
                receipt, local_rank,
            )?)
    }
    ...
    pub(crate) fn prepare(
        transport: &'t T,
        receipt: PartitionCaptureReceiptPlan,
        mut bank: PreparedPartitionFragmentDestinations,
        ranks: &[PartitionCaptureRankSource],
        metadata: &HostMetadataFunding,
    ) -> Result<Self, PartitionFragmentDeliveryError> {
        match Self::prepare_worker(transport, receipt, &mut bank, ranks, metadata) {
            Ok((exchange, ranks, evidence, dtype)) => Ok(Self {
                exchange,
                bank,
                ranks,
                evidence,
            ...
            Err(cause) => Err(PartitionFragmentDeliveryError {
                cause,
                _source: bank.source.clone(),
                _fragment_host: bank.custody.share_scheduled(),
                _final_host: None,
                _bank: Some(bank),
    ...
    fn prepare_worker(
        transport: &'t T,
        receipt: PartitionCaptureReceiptPlan,
        bank: &mut PreparedPartitionFragmentDestinations,
        ranks: &[PartitionCaptureRankSource],
        metadata: &HostMetadataFunding,
    ) -> Result<
        (
    ...
    > {
        ...
        metadata.reserve_metadata(
            Self::preparation_control_bytes(&receipt, bank.prefill.is_some())
                .ok_or(Cause::Source)?,
        )?;
        bank.custody.validate()?;
        if !bank.matches(&receipt)
            || bank.allowance.local_rank() != transport.capture_rank()
            || receipt.world_size() != transport.participant_count()
            || ranks.len() != producers.count()
        {
        ...
        for ((producer, projection), rank) in source_rows {
            ...
            if rank.producer != producer
                || !routed && projection.fragments().len() > 1
                || !matches!(
                    rank.dtype,
                    None | Some(TensorDtype::F16 | TensorDtype::F32 | TensorDtype::Bf16)
                ) && !(routed && rank.dtype == Some(TensorDtype::F64))
            {
                return Err(Cause::Source);
            }
            for fragment in 0..projection.fragments().len() {
                let (actual, _) = bank
                    .allowance
                    .fragment_source(producer, fragment)
                    .ok_or(Cause::Source)?;
                if (!routed || rank.dtype.is_some()) && rank.dtype.as_ref() != Some(actual)
                    || dtype.as_ref().is_some_and(|first| first != actual)
                {
                    return Err(Cause::Source);
                }
                if rank.dtype.is_some() {
        ...
        if !any_fragment {
            for rank in ranks {
                if let Some(first) = &dtype {
                    if rank.dtype.as_ref() != Some(first) {
                        return Err(Cause::Source);
                    }
                } else {
                    dtype = Some(rank.dtype.clone().ok_or(Cause::Source)?);
                }
            }
        }
        ...
        for rank in ranks {
            owned.push(PartitionCaptureRankSource {
                producer: rank.producer,
                dtype: rank.dtype.clone(),
            });
        }
        let evidence = PreparedPartitionCaptureEvidence::prepare_contiguous(
            &receipt,
            &mut bank.allowance,
            metadata,
        )?;
        bank.allowance
            .quota_mut()
            .reserve_quota(receipt.delivery_table_usage()?)?;
        let local = transport.capture_rank();
        if let Some(projection) = receipt.producer(local) {
            let overhead = bank
                .allowance
                .fragment_record_overhead(&receipt, local)
                .ok_or(Cause::Source)?;
            bank.allowance
                .quota_mut()
                .reserve_quota(overhead.checked_mul(projection.fragments().len() as u64)?)?;
        }
        let exchange =
            PartitionCaptureExchange::admit(transport, receipt, bank.allowance.quota_mut())?;
    ...
    fn prefill_control_bytes(receipt: &PartitionCaptureReceiptPlan) -> Option<usize> {
        let raw = receipt.combination() == PartitionCaptureCombination::SumF64ToF32
            || matches!(
                receipt.shared_plan_source().admission().plan().selections
        ...
        let payload = if receipt
            .producers()
            .any(|(rank, _)| receipt.routed_producer(rank).is_some())
        {
            size_of::<CaptureRoutedClaim<'_, '_>>()
                .checked_add(size_of::<ClaimedAssembledRoutedUnits>())?
        } else if raw {
            size_of::<
                Result<
                    crate::working_memory::CapturePrefillFragmentClaim<'_, '_, '_, '_>,
        ...
        let parts = [
            payload,
            size_of::<(&mut Self, &mut ScheduledCaptureStep<'_>)>(),
            size_of::<(Self, &mut ScheduledCaptureStep<'_>)>(),
            size_of::<(
                &mut Self,
                usize,
                &TensorDtype,
    ...
    pub(crate) fn reserve_prefill_target(
        &mut self,
        frame: &mut ScheduledCaptureStep<'_>,
    ) -> Result<(), PartitionFragmentDestinationError> {
        let receipt = self.exchange.receipt_plan();
        let source = self.bank.source.clone();
        let custody = self.bank.custody.share_scheduled();
        let error = |cause| PartitionFragmentDestinationError {
            cause,
            _value: None,
        ...
        let charge = self
            .bank
            .allowance
            .take_assembly_charge(receipt)
            .map_err(|cause| error(cause.into()))?;
        frame
            .reserve_assembled_prefill_hook(
                receipt.context().selection_index,
                self.dtype.clone(),
                charge,
            )
    ...
    pub(crate) fn begin_local_prefill(
        &mut self,
        fragment: usize,
        dtype: &TensorDtype,
        ...
        self.bank
            .begin_local_prefill(self.exchange.receipt_plan(), fragment, dtype, actual)
    }
    pub(crate) fn take_local_prefill_chunk<'c, 'f, 'p, 'a>(
        &'c mut self,
        fragment: usize,
        chunk: &'f CapturePrefillFragment<'p, 'a>,
    ) -> Result<
        crate::working_memory::CapturePrefillFragmentClaim<'c, 'f, 'p, 'a>,
        PartitionFragmentDestinationError,
    > {
        self.bank
            .take_local_prefill_chunk(self.exchange.receipt_plan(), fragment, chunk)
    }
    pub(crate) fn take_local_prefill_reduction<'c>(
        &'c mut self,
        fragment: usize,
        chunk: &CapturePrefillTransformFragment<'_, '_>,
    ) -> Result<PartitionFragmentDestination<'c, 'c>, PartitionFragmentDestinationError> {
        self.bank
            .take_local_prefill_reduction(self.exchange.receipt_plan(), fragment, chunk)
    }
    pub(crate) fn record_local_prefill_reduction(
        &mut self,
        fragment: usize,
        chunk: &CapturePrefillTransformFragment<'_, '_>,
        value: PartitionFragmentValue,
    ) -> Result<(), PartitionFragmentDestinationError> {
        self.bank.record_local_prefill_reduction(
            self.exchange.receipt_plan(),
            fragment,
            chunk,
            value,
        )
    }
    pub(crate) fn complete_local_prefill_chunk(
        &mut self,
        fragment: usize,
        chunk: u64,
    ) -> Result<(), PartitionFragmentDestinationError> {
        self.bank
            .complete_local_prefill_chunk(self.exchange.receipt_plan(), fragment, chunk)
    }
    pub(crate) fn finish_local_prefill(
        &mut self,
        fragment: usize,
    ) -> Result<(), PartitionFragmentDestinationError> {
        self.bank
            .finish_local_prefill(self.exchange.receipt_plan(), fragment)
    }
    pub(crate) fn finish_local_invocation(
        &self,
        fragment: usize,
    ) -> Result<(), PartitionFragmentDestinationError> {
        let receipt = self.exchange.receipt_plan();
        let local = self.bank.allowance.local_rank();
        if self.bank.prefill.is_some()
            || !self.bank.matches(receipt)
            || self.bank.value(local, fragment).is_none()
        {
    ...
    pub(crate) fn deliver_into_prefill_frame(
        self,
        frame: &mut ScheduledCaptureStep<'_>,
    ) -> Result<(), PartitionFragmentDeliveryError> {
        let source = self.bank.source.clone();
        let fragment_host = self.bank.custody.share_scheduled();
        let final_host = frame.claim.custody.share_scheduled();
        let metadata = self.metadata.clone();
        let error = |cause, bank| PartitionFragmentDeliveryError {
            cause,
            _bank: bank,
            _source: source.clone(),
            _fragment_host: fragment_host.share_scheduled(),
            _final_host: Some(final_host.share_scheduled()),
            _metadata: metadata.clone(),
        };
        let context = self.exchange.receipt_plan().context();
        if !source.same_storage(frame.claim.source)
            || context.phase != frame.claim.phase
            || context.prediction != frame.claim.prediction
            || frame.claim.invocation.is_some()
            || frame.claim.window.is_some_and(|window| {
                !frame
                    .claim
                    .invocation
                    .is_some_and(|physical| window.covers_invocation(physical))
            })
        {
        ...
        let destination = match frame.take_assembled_prefill_destination(context.selection_index) {
            ...
            Err(cause) => return Err(error(cause.into(), Some(self.bank))),
        };
        let result = self.deliver(destination)?;
        frame
            .record_assembled_prefill(result)
            .map_err(|cause| error(cause.into(), None))
    }
    ...
    pub(crate) fn deliver_into_invocation_frame(
        mut self,
        frame: &mut ScheduledCaptureStep<'_>,
    ) -> Result<(), PartitionFragmentDeliveryError> {
        let source = self.bank.source.clone();
        ...
        let (destination, target) = match self.invocation_target(frame) {
            Ok(value) => value,
            Err(cause) => return Err(error(cause, Some(self.bank))),
        };
        let result = self.deliver(destination)?;
        target
            .record(frame, result)
    ...
    fn invocation_control_bytes() -> Option<usize> {
        let parts = [
            super::super::invocation_assembly_control_bytes()?,
            size_of::<(Self, &mut ScheduledCaptureStep<'_>)>(),
            size_of::<(&mut Self, &mut ScheduledCaptureStep<'_>)>(),
            size_of::<Result<(), PartitionFragmentDeliveryError>>(),
            size_of::<PartitionFragmentDeliveryError>(),
            ...
            size_of::<
                Result<
                    (
                        ...
                        super::super::assembled::InvocationAssemblyTarget<'_>,
                    ),
                    Cause,
                >,
            >(),
    ...
    fn invocation_target<'a, 'c>(
        &mut self,
        frame: &'c mut ScheduledCaptureStep<'a>,
    ) -> Result<
        (
            PartitionFragmentDestination<'a, 'c>,
            super::super::assembled::InvocationAssemblyTarget<'a>,
        ),
        Cause,
    > {
        let context = self.exchange.receipt_plan().context();
        if !self.bank.source.same_storage(frame.claim.source)
            || context.phase != frame.claim.phase
            || context.prediction != frame.claim.prediction
            || !context.matches_invocation(frame.claim.invocation, frame.claim.window)
            || self.bank.prefill.is_some()
        {
            return Err(Cause::Source);
        }
        ...
        let charge = self
            .bank
            .allowance
            .take_assembly_charge(self.exchange.receipt_plan())?;
        frame
            .take_assembled_invocation_destination(index, self.dtype.clone(), charge)
            .map_err(Into::into)
    }
    pub(crate) fn take_local<'c>(
        &'c mut self,
        fragment: usize,
        dtype: &TensorDtype,
        actual: PartitionCaptureNativeEstimate,
        ...
        self.bank
            .take_local(self.exchange.receipt_plan(), fragment, dtype, actual)
    }
    pub(crate) fn record_local(
        ...
        self.bank.record(
            self.exchange.receipt_plan(),
            self.bank.allowance.local_rank(),
            fragment,
            dtype,
            charged,
            value,
    ...
    pub(crate) fn receipt_plan(&self) -> &PartitionCaptureReceiptPlan {
        self.exchange.receipt_plan()
    }
    pub(crate) fn deliver(
        ...
        let source = bank.source.clone();
        let fragment_host = bank.custody.share_scheduled();
        let final_host = destination.identity().custody.share_scheduled();
        let local = (|| -> Result<_, Cause> {
            let receipt = exchange.receipt_plan();
            let local = bank.allowance.local_rank();
            if receipt.producer(local).is_none() {
                return Ok(None);
            }
            bank.allowance
                .quota_mut()
                .reserve_quota(receipt.encoding_usage(local)?)?;
            let rank = ranks
                .iter()
                .find(|rank| rank.producer == local)
                .ok_or(Cause::Source)?;
            Ok(Some(bank.encode_contiguous_producer(
                receipt,
                rank.dtype.as_ref(),
                &metadata,
            )?))
        ...
        exchange
            .exchange_into(local, decoder)
            .map_err(|cause| PartitionFragmentDeliveryError {
                cause: cause.into(),
                _bank: None,
                _source: source,
                _fragment_host: fragment_host,
                _final_host: Some(final_host),
                _metadata: metadata,
            })
    }
}
struct Decoder<'a, 'c, 'm> {
    ...
    metadata: &'m HostMetadataFunding,
}
fn as_exchange(
    cause: Cause,
    bank: Option<PreparedPartitionFragmentDestinations>,
    source: &SharedCapturePlan,
    fragment_host: &CaptureTensorCustody,
    final_host: Option<&CaptureTensorCustody>,
    metadata: &HostMetadataFunding,
) -> PartitionCaptureExchangeError {
    BackendFailure::from_error(PartitionFragmentDeliveryError {
        cause,
        _bank: bank,
        _source: source.clone(),
        _fragment_host: fragment_host.share_scheduled(),
        _final_host: final_host.map(CaptureTensorCustody::share_scheduled),
        _metadata: metadata.clone(),
    })
    .into()
}
impl<T: PartitionCaptureTransport> PartitionCaptureDecoder<T> for Decoder<'_, '_, '_> {
    ...
    fn decode(
        self,
        receipt: PartitionCaptureReceiptPlan,
        payload: PartitionCapturePayload<'_, T>,
        ...
        let result = payload.for_each(|producer, bytes| {
            let result = (|| -> Result<(), Cause> {
                ...
                if producer != bank.allowance.local_rank() {
                    bank.allowance
                        .quota_mut()
                        .reserve_quota(receipt.decoding_usage(bytes.len() as u64)?)?;
        ...
        let provenance = (|| -> Result<(), Cause> {
            for slot in &bank.rows {
                if let Some(value) = &slot.routed_value {
                    evidence.record_routed_ranges(
                        &receipt,
                        slot.producer,
                        slot.fragment,
            ...
            if !evidence.matches(&receipt) {
                return Err(Cause::Source);
            }
            Ok(())
        })();
        if let Err(cause) = provenance {
            return Err(as_exchange(
                cause,
                Some(bank),
                &source,
        ...
        let (value, charged) =
            bank.assemble_into(&receipt, destination, &dtype)
                .map_err(|cause| {
                    as_exchange(
                        cause.into(),
                        None,
