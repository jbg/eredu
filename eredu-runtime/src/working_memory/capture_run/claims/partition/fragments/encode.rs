...
impl PreparedPartitionFragmentDestinations {
    ...
    pub(crate) fn encode_contiguous_producer(
        &self,
        ...
        let error = |cause| PartitionFragmentEncodingError {
            cause,
            _source: self.source.clone(),
            _custody: self.custody.share_scheduled(),
            _metadata: funding.clone(),
        };

        funding
            .reserve_metadata(
                encode_contiguous_controls().ok_or_else(|| error(EncodeCause::Source))?,
            )
        ...
        if receipt.routed_producer(producer).is_some() {
            return self.encode_routed_producer(receipt, empty_dtype, funding);
        }
        if projection.fragments().len() > 1 {
            return Err(error(EncodeCause::Source));
        }
        let maximum =
            usize::try_from(receipt.max_record_bytes()).map_err(|_| error(EncodeCause::Source))?;
        if projection.fragments().is_empty() {
            ...
            return crate::capture::partition::encode_contiguous(
                receipt.context(),
                receipt.identity(),
                producer,
                receipt.combination(),
        ...
        let payload = match value {
            PartitionFragmentValue::Tensor(v) => {
                CapturePayloadWire::Tensor(v.observation().as_observation())
            }
        ...
        let record = CaptureRecordWire {
            schema_version: CAPTURE_SCHEMA_VERSION,
            selection_id: &selection.id,
            path: &selection.path,
            node_id: &point.node_id,
            position: point.position,
            source_shape: Some(projection.local_shape()),
        ...
        crate::capture::partition::encode_contiguous(
            receipt.context(),
            receipt.identity(),
            producer,
            receipt.combination(),
...
impl PreparedPartitionFragmentDestinations {
    fn encode_routed_producer(
        ...
        funding
            .reserve_metadata(encode_routed_controls().ok_or_else(|| error(EncodeCause::Source))?)
            .map_err(|e| error(e.into()))?;
        self.custody.validate().map_err(|e| error(e.into()))?;
        let producer = self.allowance.local_rank();
        if !self.matches(receipt) || receipt.routed_producer(producer).is_none() {
            return Err(error(EncodeCause::Source));
        }
        let projection = receipt
            .producer(producer)
        ...
        if !matches!(
            dtype,
            None | Some(TensorDtype::F16 | TensorDtype::F32 | TensorDtype::Bf16 | TensorDtype::F64)
        ) {
            return Err(error(EncodeCause::Source));
        }
        let index = receipt.context().selection_index;
        ...
        let mut records = funding
            .metadata_vec(projection.fragments().len())
            .map_err(|e| error(e.into()))?;
        for (fragment, geometry) in projection.fragments().iter().enumerate() {
            let (precision, charged, _) = self
                .allowance
                .fragment_record_charge(receipt, producer, fragment)
                .ok_or_else(|| error(EncodeCause::Source))?;
            if dtype.is_some_and(|actual| actual != &precision) {
                return Err(error(EncodeCause::Source));
            }
            let value = self
                .routed_value(producer, fragment)
                .ok_or_else(|| error(EncodeCause::Source))?;
            if dtype.is_none()
                && (value.native_rows() != 0
            ...
            value
                ...
                .validate()
                .map_err(|e| error(e.into()))?;
            records.push(CaptureRecordWire {
                schema_version: CAPTURE_SCHEMA_VERSION,
                selection_id: &selection.id,
                path: &selection.path,
                node_id: &point.node_id,
                ...
                source_shape: Some(projection.local_shape()),
                source_dtype: dtype,
                selected_shape: Some(&geometry.local().shape),
                outcome: &outcome,
                payload: Some(CapturePayloadWire::RoutedUnits(value.observation())),
                charged,
            });
        }
        crate::capture::partition::encode_fragment_records(
            receipt.context(),
            receipt.identity(),
            producer,
            receipt.combination(),
...
fn encode_contiguous_controls() -> Option<usize> {
    let parts = [
        size_of::<(
        ...
        CaptureRecordWire::control_bytes(),
        size_of::<CaptureOutcome>(),
        size_of::<Option<CaptureRecordWire<'_>>>(),
        ...
        size_of::<PartitionFragmentEncodingError>(),
        size_of::<EncodeCause>(),
...
fn encode_routed_controls() -> Option<usize> {
    let parts = [
        size_of::<(
            &PreparedPartitionFragmentDestinations,
            &PartitionCaptureReceiptPlan,
        ...
        size_of::<Vec<CaptureRecordWire<'_>>>(),
        CaptureRecordWire::control_bytes(),
        size_of::<Option<&TensorDtype>>(),
        size_of::<Option<(TensorDtype, CaptureUsage, CaptureUsage)>>(),
        size_of::<(TensorDtype, CaptureUsage, CaptureUsage)>(),
        size_of::<CaptureOutcome>(),
        size_of::<Option<&ClaimedPartitionRoutedUnits>>(),
        size_of::<PartitionFragmentEncodingError>(),
...
pub(super) fn producer_metadata_bytes(
    receipt: &PartitionCaptureReceiptPlan,
    local_rank: usize,
) -> Option<usize> {
    let Some(projection) = receipt.producer(local_rank) else {
        return Some(0);
    };
    let maximum = usize::try_from(receipt.max_record_bytes()).ok()?;
    let controls = encode_contiguous_controls()?;
    if receipt.routed_producer(local_rank).is_some() {
        controls
            .checked_add(encode_routed_controls()?)?
            .checked_add(eredu_nn::workspace::WorkspaceContext::metadata_vec_bytes::<
                CaptureRecordWire<'_>,
            >(projection.fragments().len())?)?
            .checked_add(crate::capture::partition::fragment_records_metadata_bytes(
                maximum,
            )?)
    } else {
        controls.checked_add(crate::capture::partition::contiguous_metadata_bytes(
            maximum,
        )?)
    }
}
