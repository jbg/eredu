...
pub(super) fn prefill_source(
    ...
    let logical = CaptureRoutedPrefillPlan::prepare(
        source.admission(),
    ...
    if host.geometry().source_shape() != logical.geometry().source_shape()
        || host.geometry().bank() != logical.geometry().bank()
    {
        return Err(CaptureRunHostError::ReceiptMismatch);
    }
    Ok(())
...
impl PreparedPartitionFragmentDestinations {
    ...
    pub fn begin_local_routed_prefill(
        &mut self,
        receipt: &PartitionCaptureReceiptPlan,
        fragment: usize,
        dtype: &TensorDtype,
    ...
    fn begin_routed(
        ...
        dtype: &TensorDtype,
        actual: PartitionCaptureNativeEstimate,
        actual_native_rows: Option<u64>,
    ) -> Result<PreparedPartitionFragmentLoan, PartitionFragmentDestinationError> {
        self.custody
            .validate()
            .map_err(|e| self.error(e.into(), None))?;
        if !self.matches(receipt) || self.prefill.is_some() != actual_native_rows.is_none() {
        ...
        if let Some(inference) = self.prefill {
            prefill_source(receipt, producer, fragment, inference)
                .map_err(|e| self.error(e.into(), None))?;
        }
        let loan = self
            .allowance
        ...
        match match actual_native_rows {
            Some(rows) => claim.prepare(rows),
            None => claim.prepare_accumulating(),
        } {
            Ok(target) => self.rows[index].routed = Some(target),
            Err(cause) => {
                self.rows[index].state = State::Failed;
                return Err(self.error(cause.into(), None));
            }
        }
        Ok(loan)
    }
    ...
    pub fn begin_local_routed_invocation(
        &mut self,
        receipt: &PartitionCaptureReceiptPlan,
        fragment: usize,
        chunk: u64,
    ...
    pub fn finish_local_routed_invocation(
        &mut self,
        receipt: &PartitionCaptureReceiptPlan,
        fragment: usize,
        chunk: u64,
    ...
    fn routed_invocation(
        ...
        let index = self
            ...
            .position(|r| {
                r.producer == producer
                    && r.fragment == fragment
                    && r.state == State::Taken
                    && r.routed_invocations == chunk
                    && r.routed_pending == rows.is_none()
                    && r.routed.is_some()
            })
            .ok_or_else(|| self.error(FragmentCause::Source, None))?;
        let logical = CaptureRoutedPrefillPlan::prepare(
        ...
        .map_err(|e| {
            self.error(
                FragmentCause::Host(CapturePrefillHostError::from(e).into()),
                None,
            )
        })?;
        let token_window = logical
            .fragment(chunk)
            .map_err(|e| {
                self.error(
                    FragmentCause::Host(CapturePrefillHostError::from(e).into()),
        ...
        if let Some(rows) = rows {
            ...
            let maximum = ownership
                .maximum_source_rows_checked(
                    token_window.source_tokens(),
                    logical.geometry().bank().routes_per_token,
                )
                .map_err(|e| {
                    self.error(
                        FragmentCause::Host(CaptureRoutedHostError::from(e).into()),
                        None,
                    )
                })?;
            if rows > maximum
                || ownership.source_peer.is_none() && rows != token_window.source_tokens()
            {
                self.rows[index].state = State::Failed;
                return Err(self.error(FragmentCause::Source, None));
            }
        ...
        let row = &mut self.rows[index];
        let target = row.routed.as_mut().expect("validated sparse slot");
        let result = match rows {
            Some(rows) => target.begin_invocation(rows, token_window),
            None => target.finish_invocation(),
        };
        if let Err(cause) = result {
            row.state = State::Failed;
            return Err(self.error(FragmentCause::Host(cause.into()), None));
        }
        row.routed_pending = rows.is_some();
        if rows.is_none() {
            row.routed_invocations += 1;
        }
        Ok(())
    }
    ...
    pub fn take_local_routed_writer<'t, 'r>(
        &'t mut self,
        ...
        self.rows[index]
            .routed
            .as_mut()
            .expect("validated sparse slot")
            .writer(receipt)
            .map_err(|cause| PartitionFragmentDestinationError {
                cause: FragmentCause::Host(cause.into()),
                _value: None,
                _source: source,
    ...
    pub fn finish_local_routed(
        ...
        let index = self
            ...
            .ok_or_else(|| self.error(FragmentCause::Source, None))?;
        if self.prefill.is_some_and(|inference| {
            self.rows[index].routed_pending
                || self.rows[index].routed_invocations
                    != inference
                        .input_positions
                        .div_ceil(inference.prefill_chunk_positions)
        }) {
    ...
    pub(super) fn record_routed(
        ...
        value: ClaimedPartitionRoutedUnits,
    ) -> Result<(), PartitionFragmentDestinationError> {
        if !self.matches(receipt) {
            return Err(self.error(
                FragmentCause::RoutedRecord(CaptureRunHostError::ReceiptMismatch, value),
                None,
            ));
        }
        if let Err(cause) = self.custody.validate() {
            return Err(self.error(FragmentCause::RoutedRecord(cause.into(), value), None));
        }
        let expected = self.allowance.fragment_source(producer, fragment);
        let index = self.rows.iter().position(|r| {
            r.producer == producer && r.fragment == fragment && r.state == State::Taken
        });
        ...
        if !expected
            ...
            || !row.custody.same_schedule(&identity.custody)
    ...
    pub fn routed_value(
        ...
        producer: usize,
        fragment: usize,
    ) -> Option<&ClaimedPartitionRoutedUnits> {
        self.rows
            .iter()
            .find(|r| {
...
pub(super) fn prefill_control_bytes() -> Option<usize> {
    let parts = [
        ...
        size_of::<Result<CaptureRoutedPrefillFragment<'_, '_>, CapturePrefillGeometryError>>(),
        size_of::<CaptureRoutedTokenWindow>() * 2,
        size_of::<Result<CaptureRoutedPrefillPlan<'_>, CapturePrefillGeometryError>>(),
        size_of::<CapturePartitionRoutedHostPlan<'_>>(),
        size_of::<Result<CapturePartitionRoutedHostPlan<'_>, CaptureRunHostError>>(),
        size_of::<(
...
pub(super) fn control_bytes() -> Option<usize> {
    let parts = [
        ...
        size_of::<CapturePartitionRoutedWriter<'_, '_>>(),
