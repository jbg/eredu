...
use super::*;
use crate::working_memory::capture_run::claims::prefill::{
    CapturePrefillFragmentClaim, CapturePrefillHostError,
};
...
) -> Result<CapturePrefillRowAssembly<'a>, CaptureRunHostError> {
...
impl<'a> PartitionFragmentHostPlan<'a> {
    ...
    pub fn prepare_prefill(
        ...
        for (producer, projection) in receipt.producers() {
            for fragment in 0..projection.fragments().len() {
                if receipt.routed_producer(producer).is_some() {
                    ...
                    let FragmentHostPlan::Tensor(host) =
                        FragmentHostPlan::prepare(receipt, producer, fragment)?
                    else {
                        return Err(CaptureRunHostError::ReceiptMismatch);
                    };
                    if !same_geometry(rows.logical_geometry(), host.geometry()) {
                        return Err(CaptureRunHostError::ReceiptMismatch);
                    }
                }
            }
        }
        let additive = receipt.combination() == PartitionCaptureCombination::SumF64ToF32
            && matches!(
        ...
        let routed = receipt
            .producers()
            .any(|(producer, _)| receipt.routed_producer(producer).is_some());
        let controls = (if routed {
            routed::prefill_control_bytes()
        } else if reductions::selected(receipt) {
            reductions::control_bytes(receipt)
        } else {
            control_bytes(additive)
        })
        ...
        plan.table = plan
            ...
            .ok_or(WorkingMemoryError::Overflow)?;
        plan.table
            .checked_add(plan.fragments)
            .and_then(|n| n.checked_add(plan.assembly_peak_bytes()))
            .ok_or(WorkingMemoryError::Overflow)?;
        plan.prefill = Some(inference);
        Ok(plan)
    }
}
impl PreparedPartitionFragmentDestinations {
    ...
    pub fn begin_local_prefill(
        ...
        if !self.matches(receipt) || self.prefill.is_none() {
    ...
    pub fn take_local_prefill_chunk<'t, 'f, 'p, 'a>(
        &'t mut self,
        receipt: &PartitionCaptureReceiptPlan,
        fragment: usize,
    ...
    ) -> Result<CapturePrefillFragmentClaim<'t, 'f, 'p, 'a>, PartitionFragmentDestinationError>
    {
        ...
        let producer = self.allowance.local_rank();
        let expected = row_source(receipt, producer, fragment, inference)
            .map_err(|e| self.error(e.into(), None))?;
        if !same_geometry(
            expected.logical_geometry(),
            chunk.assembly().logical_geometry(),
        ) {
            return Err(self.error(FragmentCause::Source, None));
        }
        let index = self
            ...
            .position(|r| {
                r.producer == producer
                    && r.fragment == fragment
                    && r.state == State::Taken
                    && r.prefill.as_ref().is_some_and(|target| {
            ...
            .ok_or_else(|| self.error(FragmentCause::Source, None))?;
        let row = &mut self.rows[index];
        let custody = row.custody.share_scheduled();
        let target = row.prefill.as_mut().expect("checked local prefill");
        Ok(CapturePrefillFragmentClaim::from_partition(
            &mut target.slot,
            chunk,
            receipt.context().selection_index,
            custody,
    ...
    pub fn complete_local_prefill_chunk(
        &mut self,
        receipt: &PartitionCaptureReceiptPlan,
        fragment: usize,
        chunk: u64,
        ...
        let target = row.prefill.as_mut().expect("checked prefill");
        target.next += 1;
        target.slot.done = false;
        Ok(())
    }
    ...
    pub fn finish_local_prefill(
        ...
        if reductions::selected(receipt) {
            return self.finish_local_prefill_reduction(receipt, fragment);
        }
        let producer = self.allowance.local_rank();
        ...
        let valid = row.prefill.as_ref().is_some_and(|target| {
            target.next
                ...
                && target
                    ...
                    .tensor
                    .as_ref()
                    .is_some_and(|tensor| tensor.covered == tensor.data.len())
        });
        if !valid {
            row.state = State::Failed;
            return Err(self.error(FragmentCause::Source, None));
...
fn control_bytes(additive: bool) -> Option<usize> {
    let parts = [
        ...
        size_of::<CapturePrefillRowAssembly<'_>>() * 2,
        ...
        size_of::<
            Result<CapturePrefillFragmentClaim<'_, '_, '_, '_>, PartitionFragmentDestinationError>,
        >(),
        ...
        size_of::<OwnedPrefillTensor>() * 2,
        size_of::<Result<OwnedPrefillTensor, WorkingMemoryError>>(),
        size_of::<(&CaptureTensorGeometry<'_>, CaptureTensorCustody)>(),
        size_of::<TensorObservation>() * 2,
