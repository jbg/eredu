...
pub(super) fn source<'a>(
    ...
    if !selected(receipt)
        || receipt.context().phase != CapturePhase::Prefill
        || receipt.context().prediction != 0
        || receipt.context().invocation.is_some()
    {
        return Err(CaptureRunHostError::ReceiptMismatch);
    }
    CapturePrefillTransformPlan::prepare_partition(
...
fn validate_chunk(
    ...
    if !std::ptr::eq(plan.admission(), chunk.plan().admission())
        || plan.selection_index() != chunk.plan().selection_index()
        || inference != chunk.plan().inference_geometry()
    {
        return Err(CaptureRunHostError::ReceiptMismatch);
    }
    let expected = plan
    ...
    if expected.input() != chunk.input()
        || expected.position() != chunk.position()
        || expected.output_demand() != chunk.output_demand()
        || expected.source_shape() != chunk.source_shape()
        || expected.selected_shape() != chunk.selected_shape()
        || expected.starts() != chunk.starts()
        || expected.ends() != chunk.ends()
        || expected.strides() != chunk.strides()
        || expected.selected_elements() != chunk.selected_elements()
    {
        return Err(CaptureRunHostError::ReceiptMismatch);
    }
    Ok(())
...
impl PreparedPartitionFragmentDestinations {
    ...
    pub fn take_local_prefill_reduction<'a, 't>(
        &'t mut self,
        receipt: &'a PartitionCaptureReceiptPlan,
        fragment: usize,
        chunk: &CapturePrefillTransformFragment<'_, '_>,
        ...
        validate_chunk(receipt, producer, fragment, inference, chunk)
            .map_err(|e| self.error(e.into(), None))?;
        let index = self
            .rows
            .iter()
            .position(|row| {
                row.producer == producer
                    ...
                    && row.prefill.as_ref().is_some_and(|target| {
                        target.next == chunk.chunk_index()
                            && !target.pending
                            && !target.slot.done
                            && matches!(
                                target.slot.state,
                                TargetState::Claimed | TargetState::Active
                            )
                    })
            })
            .ok_or_else(|| self.error(FragmentCause::Source, None))?;
        ...
        match source.plan().selections[receipt.context().selection_index].transform {
            CaptureTransform::Summary => {
                ...
                .and_then(|geometry| geometry.fragment(chunk))
                .map_err(CaptureStepError::from)
                .map_err(|e| self.error(CaptureRunHostError::from(e).into(), None))?;
                let plan = CaptureSummaryHostPlan::prepare(geometry)
                    .map_err(|e| self.error(e.into(), None))?;
                Ok(PartitionFragmentDestination::Summary(
                    CaptureSummaryClaim::from_partition_prefill_plan(
                        plan,
                        identity,
                        chunk.chunk_index(),
                    ),
            ...
            CaptureTransform::Histogram { .. } => {
                ...
                let plan = CaptureHistogramHostPlan::prepare(geometry)
                    .map_err(|e| self.error(e.into(), None))?;
                Ok(PartitionFragmentDestination::Histogram(
                    CaptureHistogramClaim::from_partition_prefill_plan(
                        plan,
                        identity,
                        chunk.chunk_index(),
                    ),
    ...
    pub fn record_local_prefill_reduction(
        &mut self,
        receipt: &PartitionCaptureReceiptPlan,
        fragment: usize,
        chunk: &CapturePrefillTransformFragment<'_, '_>,
        ...
        if let Err(error) = self.custody.validate() {
            return Err(self.error(error.into(), Some(value)));
        }
        let Some(inference) = self.prefill else {
        ...
        let producer = self.allowance.local_rank();
        if let Err(error) = validate_chunk(receipt, producer, fragment, inference, chunk) {
            return Err(self.error(error.into(), Some(value)));
        }
        let Some(index) = self.rows.iter().position(|row| {
            row.producer == producer && row.fragment == fragment && row.state == State::Taken
        }) else {
            return Err(self.error(FragmentCause::Source, Some(value)));
        };
        ...
        let result = (|| -> Result<(), CaptureRunHostError> {
            ...
            if !row.custody.same_schedule(&identity.custody)
                || identity.phase != CapturePhase::Prefill
            ...
            let target = row
                ...
                .as_mut()
                .ok_or(CaptureRunHostError::ReceiptMismatch)?;
            if target.next != chunk.chunk_index()
                || !target.pending
                || target.slot.done
                || !matches!(
                    target.slot.state,
            ...
            match (&value, &chunk.plan().selection().transform) {
                (PartitionFragmentValue::Summary(value), CaptureTransform::Summary)
                    if value.partition_chunk() == Some(target.next) =>
                {
                    let empty = crate::capture::reduction::Summary::default();
                    target.slot.summary = Some(
                        target
                            .slot
                            .summary
                            .as_ref()
                            .unwrap_or(&empty)
                            .appended_fixed(value.observation(), chunk.selected_elements())
                            .map_err(CaptureStepError::from)?,
                    );
                }
                (
                    ...
                    crate::capture::reduction::validate_histogram_fixed(
                        value.observation(),
                        edges,
                        chunk.selected_elements(),
                    )
                    .map_err(CaptureStepError::from)?;
                    if let Some(out) = &mut target.slot.histogram {
                        crate::capture::reduction::add_histogram_fixed(out, value.observation())
                            .map_err(CaptureStepError::from)?;
                    }
                }
        ...
        let row = &mut self.rows[index];
        let target = row.prefill.as_mut().expect("validated target");
        if let PartitionFragmentValue::Histogram(value) = value {
            if target.slot.histogram.is_none() {
                target.slot.histogram = Some(value.into_partition_histogram());
            }
        }
        target.pending = false;
        target.slot.done = true;
        target.slot.state = TargetState::Active;
        row.state = State::Taken;
        Ok(())
    }
    pub(super) fn finish_local_prefill_reduction(
        &mut self,
        receipt: &PartitionCaptureReceiptPlan,
        fragment: usize,
    ) -> Result<(), PartitionFragmentDestinationError> {
        ...
        let valid = row.prefill.as_ref().is_some_and(|target| {
            target.next
                == inference
                    .input_positions
                ...
                && !target.slot.done
                && target.slot.state == TargetState::Active
                && target.slot.dtype.as_ref() == Some(&dtype)
        });
        if !valid {
            return Err(self.error(FragmentCause::Source, None));
        }
        ...
        let value = match plan {
            FragmentHostPlan::Summary(plan) => {
                let value = self.rows[index]
                    .prefill
                    .as_ref()
                    .and_then(|target| target.slot.summary.as_ref())
                    .ok_or_else(|| self.error(FragmentCause::Source, None))?
                    .value();
                PartitionFragmentValue::Summary(
                    CaptureSummaryClaim::from_partition_plan(plan, identity)
                        .finish_partition(value)
            ...
            FragmentHostPlan::Histogram(plan) => {
                let value = self.rows[index]
                    .prefill
                    .as_mut()
                    .and_then(|target| target.slot.histogram.take())
                    .ok_or_else(|| self.error(FragmentCause::Source, None))?;
                PartitionFragmentValue::Histogram(
                    CaptureHistogramClaim::from_partition_plan(plan, identity)
                        .finish_partition_accumulator(value)
                        .map_err(|error| self.error(error.into(), None))?,
                )
            }
            _ => return Err(self.error(FragmentCause::Source, None)),
        };
        self.rows[index].prefill = None;
        self.rows[index].state = State::Taken;
...
pub(super) fn control_bytes(receipt: &PartitionCaptureReceiptPlan) -> Option<usize> {
    ...
    let typed = if histogram {
        CaptureHistogramGeometry::preparation_control_bytes()?
            .checked_add(size_of::<CaptureHistogramHostPlan<'_>>() * 2)?
            .checked_add(size_of::<CaptureHistogramClaim<'_, '_>>() * 2)?
            .checked_add(size_of::<ClaimedCaptureHistogram>() * 2)?
    } else {
        CaptureSummaryGeometry::preparation_control_bytes()?
            .checked_add(size_of::<CaptureSummaryHostPlan<'_>>() * 2)?
            .checked_add(size_of::<CaptureSummaryClaim<'_, '_>>() * 2)?
            .checked_add(size_of::<ClaimedCaptureSummary>() * 2)?
    };
    let parts = [
        typed,
        ...
        size_of::<Target>() * 2,
        size_of::<TargetSlot>(),
        size_of::<InferenceGeometry>() * 2,
        CapturePrefillTransformPlan::partition_preparation_control_bytes()?,
        size_of::<CapturePrefillTransformPlan<'_>>() * 2,
        size_of::<CapturePrefillTransformFragment<'_, '_>>() * 2,
        size_of::<Result<CapturePrefillTransformPlan<'_>, CaptureRunHostError>>(),
        ...
        size_of::<CaptureTensorCustody>() * 2,
        ...
        size_of::<(
            ...
            usize,
            InferenceGeometry,
            &CapturePrefillTransformFragment<'_, '_>,
        )>(),
        size_of::<(
            &mut PreparedPartitionFragmentDestinations,
            &PartitionCaptureReceiptPlan,
            usize,
            &CapturePrefillTransformFragment<'_, '_>,
            PartitionFragmentValue,
        )>(),
        size_of::<(usize, u64, bool)>(),
        size_of::<Option<usize>>(),
        size_of::<Option<&mut Target>>(),
    ];
    parts
        .into_iter()
