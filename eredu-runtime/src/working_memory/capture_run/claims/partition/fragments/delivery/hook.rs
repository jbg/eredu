...
impl PartitionLocalCaptureHook {
    pub(crate) fn bind_routed_source(
        &mut self,
        source: crate::capture::partition::PreparedPartitionRoutedLocalSource,
    ) {
        assert!(
            self.routed_source.is_none() && self.routed.is_none(),
            "original hook has no active sparse source"
        );
        self.routed_source = Some(source);
    }
    pub(crate) fn source(&self) -> &SharedCapturePlan {
        &self.bank.source
    }
...
impl<T: PartitionCaptureTransport> fmt::Debug for PartitionCaptureHookContinuation<'_, T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("PartitionCaptureHookContinuation")
    ...
    <T::Completion as Completion>::Error: Send + Sync + 'static,
{
    ...
    pub(crate) fn into_local_hook(
        self,
    ) -> (
        PartitionCaptureHookContinuation<'t, T>,
        ...
        let receipt = exchange
            .take_hook_receipt()
            .expect("one unexchanged original receipt");
        let source = bank.source.clone();
        let custody = bank.custody.share_scheduled();
        (
            PartitionCaptureHookContinuation {
                exchange,
                ranks,
                evidence,
                dtype,
    ...
    <T::Completion as Completion>::Error: Send + Sync + 'static,
{
    pub(crate) fn resume(
        self,
        hook: PartitionLocalCaptureHook,
    ...
    pub(crate) fn resume_routed(
        self,
        hook: PartitionLocalCaptureHook,
    ) -> Result<
        (
            PreparedPartitionFragmentDelivery<'t, T>,
            Option<crate::capture::partition::PreparedPartitionRoutedLocalSource>,
        ),
    ...
    > {
        ...
        let error = |hook| PartitionCaptureHookReturnError {
            _hook: hook,
            _source: source.clone(),
            _expected: custody.share_scheduled(),
            _metadata: metadata.clone(),
        };
        if hook.routed.is_some()
            || !custody.same_schedule(&hook.bank.custody)
            || !source.same_storage(&hook.bank.source)
            || !metadata.same_account(&hook.metadata)
            || !hook.bank.matches(&hook.receipt)
            || !evidence.matches_receipt(&hook.receipt)
            || hook.bank.allowance.local_rank() != exchange.local_rank()
        {
            return Err(error(hook));
        }
        let PartitionLocalCaptureHook {
            bank,
            receipt,
        ...
        if let Err(receipt) = exchange.restore_hook_receipt(receipt) {
            return Err(error(PartitionLocalCaptureHook {
                bank,
                receipt,
                metadata: hook_metadata,
                routed: None,
...
pub(super) fn control_bytes<T: PartitionCaptureTransport>() -> Option<usize> {
    let parts = [
        size_of::<Option<crate::capture::partition::PreparedPartitionRoutedLocalSource>>(),
        size_of::<(
            PreparedPartitionFragmentDelivery<'_, T>,
            Option<crate::capture::partition::PreparedPartitionRoutedLocalSource>,
        )>(),
        size_of::<PartitionLocalCaptureHook>() * 2,
        size_of::<PartitionCaptureHookContinuation<'_, T>>() * 2,
        size_of::<(
            PartitionCaptureHookContinuation<'_, T>,
            PartitionLocalCaptureHook,
        )>(),
        size_of::<Result<PreparedPartitionFragmentDelivery<'_, T>, PartitionCaptureHookReturnError>>(
        ),
        size_of::<PartitionCaptureHookReturnError>(),
        size_of::<Result<(), PartitionCaptureReceiptPlan>>(),
        size_of::<Option<PartitionCaptureReceiptPlan>>(),
        size_of::<(&mut PartitionLocalCaptureHook,)>(),
        size_of::<(
            &PartitionCaptureReceiptPlan,
            &mut PreparedPartitionFragmentDestinations,
        )>(),
        ...
        size_of::<(
            &mut PartitionCaptureExchange<'_, T>,
            PartitionCaptureReceiptPlan,
        )>(),
