...
impl PartitionLocalCaptureHook {
    fn routed_result<E: std::error::Error + Send + Sync + 'static>(
        self,
        result: Result<(), LocalCause<E>>,
    ) -> Result<Self, PartitionCaptureProgramError> {
        match result {
            Ok(()) => Ok(self),
            Err(cause) => {
                let source = self.source().clone();
                let metadata = self.funding().clone();
                Err(PartitionCaptureProgramError::local(
                    PartitionLocalCaptureFailure { cause, _hook: self },
                    source,
                    metadata,
                ))
            }
    ...
    pub(crate) fn begin_routed_program<T, E: std::error::Error + Send + Sync + 'static>(
        mut self,
        backend: &mut dyn ScheduledCaptureBackend<Tensor = T, Error = E>,
        invocation: &crate::RoutedUnitInvocation<'_, T>,
        prefill: Option<(InferenceGeometry, u64)>,
    ) -> Result<Self, PartitionCaptureProgramError> {
        let result = (|| {
            self.metadata.reserve_metadata(
                Self::routed_control_bytes::<T, E>().ok_or(LocalCause::Identity)?,
            )?;
            if self.routed.is_some() {
                return Err(LocalCause::Identity);
            }
            let receipt = &self.receipt;
            let rank = self.bank.allowance.local_rank();
            let owner = self.routed_source.as_ref().ok_or(LocalCause::Identity)?;
            if owner.producer != rank {
                return Err(LocalCause::Identity);
            }
            let count = receipt.producer(rank).map_or(0, |p| p.fragments().len());
            let global = CaptureRoutedUnitsGeometry::prepare_receipt(
                self.source().admission(),
                receipt.context(),
            )?;
            if global.source_shape()[0] as u64 != owner.source_tokens {
                return Err(LocalCause::Identity);
            }
            let (chunk, last, source_tokens) = if let Some((inference, chunk)) = prefill {
                ...
                (
                    Some(chunk),
                    chunk + 1 == logical.chunk_count(),
                    source_tokens,
                )
            } else {
                (None, true, owner.source_tokens)
            };
            ...
            let (native_rows, _input_dtype) =
                backend.validate_partition_routed_invocation(invocation, &layout)?;
            if native_rows != 0 && owner.dtype.is_none() {
                return Err(LocalCause::Identity);
            }
            ...
            for fragment in 0..count {
                let (dtype, estimate) = self
                    .bank
                    .allowance
                    .fragment_source(rank, fragment)
                    .map(|(dtype, estimate)| (dtype.clone(), estimate))
                    .ok_or(LocalCause::Identity)?;
                if chunk.is_none() || chunk == Some(0) {
                    let mut loan = if chunk.is_some() {
                        self.bank
                            .begin_local_routed_prefill(receipt, fragment, &dtype, estimate)?
                    } else {
                        self.bank.begin_local_routed(
                            receipt,
                            fragment,
                            &dtype,
                            estimate,
                ...
                if let Some(chunk) = chunk {
                    self.bank.begin_local_routed_invocation(
                        receipt,
                        fragment,
                        chunk,
                        native_rows,
    ...
    pub(crate) fn observe_routed_program<T, E: std::error::Error + Send + Sync + 'static>(
        mut self,
        backend: &mut dyn ScheduledCaptureBackend<Tensor = T, Error = E>,
        ...
        let result = (|| {
            ...
            let (dtype, batch_rows) = backend.validate_partition_routed_batch_source(
                &source,
                &layout,
                state.native_rows,
            )?;
            ...
            let completed = PartitionRoutedUnitCaptureLayout::advance_native_rows(
                state.native_rows,
                state.completed,
            ...
            if count == 0 {
                ...
                for value in [
                    ...
                    source.source.token_indices,
                    source.source.selection_indices,
                    source.source.coefficients,
                    source.source.source_groups,
                ] {
                    backend.complete_partition_source(value)?;
                }
            }
            for fragment in 0..count {
                let (dtype, estimate) = self
                ...
                if writer.native_rows() != state.native_rows
                    || writer.invocation_source_tokens() != state.source_tokens
                {
                    return Err(LocalCause::Identity);
                }
                let request = writer.request();
                let actual = backend.validate_partition_routed_source(
                    &source,
                    &request,
                    state.source_tokens,
                    state.native_rows,
            ...
            self.routed
                .as_mut()
                .expect("validated active sparse invocation")
                .completed = completed;
            Ok(())
        })();
        self.routed_result(result)
    ...
    pub(crate) fn finish_routed_program<T, E: std::error::Error + Send + Sync + 'static>(
        mut self,
        success: bool,
    ) -> Result<Self, PartitionCaptureProgramError> {
        let result = (|| {
            ...
            let state = self.routed.as_ref().ok_or(LocalCause::Identity)?;
            if state.completed != state.native_rows {
                return Err(LocalCause::Identity);
            }
            let receipt = &self.receipt;
            let rank = self.bank.allowance.local_rank();
            ...
            for fragment in 0..count {
                if let Some(chunk) = state.chunk {
                    self.bank
                        .finish_local_routed_invocation(receipt, fragment, chunk)?;
                }
                if state.last {
                    self.bank.finish_local_routed(receipt, fragment)?;
                }
            }
            self.routed = None;
            Ok(())
    ...
    pub(crate) fn routed_control_bytes<T, E: std::error::Error + Send + Sync + 'static>()
    -> Option<usize> {
        let parts = [
            size_of::<[&T; 5]>(),
            size_of::<crate::capture::partition::PreparedPartitionRoutedLocalSource>(),
            size_of::<CaptureRoutedUnitsGeometry<'_>>(),
            size_of::<Result<(TensorDtype, u64), FundedCaptureError<E>>>(),
            ...
            eredu_core::BackendFailure::source_retention_peak_bytes::<
                PartitionLocalCaptureFailure<E>,
            >()?,
            size_of::<PartitionLocalCaptureHook>() * 2,
            size_of::<Invocation>(),
            size_of::<Option<Invocation>>(),
            size_of::<PartitionLocalCaptureFailure<E>>(),
            size_of::<LocalCause<E>>(),
            size_of::<FundedCaptureError<E>>(),
            size_of::<Result<PartitionLocalCaptureHook, PartitionCaptureProgramError>>(),
            ...
            size_of::<PartitionRoutedUnitCaptureSource<'_, T>>(),
            size_of::<crate::RoutedUnitInvocation<'_, T>>(),
            size_of::<crate::RoutedUnitBatch<'_, T>>(),
            ...
            size_of::<
                Result<CapturePartitionRoutedWriter<'_, '_>, PartitionFragmentDestinationError>,
            >(),
            size_of::<Option<(InferenceGeometry, u64)>>(),
            size_of::<(Option<u64>, bool, u64)>(),
            size_of::<(
                &mut PartitionLocalCaptureHook,
                &mut dyn ScheduledCaptureBackend<Tensor = T, Error = E>,
                &crate::RoutedUnitInvocation<'_, T>,
                Option<(InferenceGeometry, u64)>,
            )>(),
            size_of::<(
                &mut PartitionLocalCaptureHook,
                &mut dyn ScheduledCaptureBackend<Tensor = T, Error = E>,
                &crate::RoutedUnitBatch<'_, T>,
            )>(),
