...
use crate::capture::partition::{
    PartitionPrefillCaptureKind, PartitionPrefillCapturePlan, PartitionPrefillCaptureSourceError,
};
use crate::capture::{FundedCaptureError, ScheduledCaptureBackend};
use eredu_core::{
    InferenceGeometry,
    capture::{CapturePrefillGeometryError, CaptureTensorGeometryError},
};

#[derive(Debug, thiserror::Error)]
pub(super) enum LocalCause<E: std::error::Error + 'static> {
    #[error("partition local source, scalar or estimate differs")]
    Identity,
    #[error(transparent)]
    Routed(#[from] eredu_core::capture::RoutedUnitValidationError),
    #[error(transparent)]
    Source(#[from] PartitionPrefillCaptureSourceError),
    #[error(transparent)]
    Invocation(#[from] crate::capture::partition::PartitionInvocationCaptureSourceError),
    #[error(transparent)]
    ...
    #[error(transparent)]
    Work(#[from] FundedCaptureError<E>),
}
...
pub(crate) struct PartitionLocalCaptureFailure<E: std::error::Error + 'static> {
    #[source]
    pub(super) cause: LocalCause<E>,
    pub(super) _hook: PartitionLocalCaptureHook,
}
impl PartitionLocalCaptureHook {
    pub(crate) fn observe_prefill_program<T, E: std::error::Error + Send + Sync + 'static>(
        self,
        ...
        self.observe_prefill(backend, value, inference, chunk)
            .map_err(|failure| {
                let source = failure._hook.source().clone();
                let metadata = failure._hook.funding().clone();
                crate::capture::partition::PartitionCaptureProgramError::local(
                    failure, source, metadata,
                )
            })
    }
    ...
    pub(crate) fn observe_prefill<T, E: std::error::Error + Send + Sync + 'static>(
        mut self,
        backend: &mut dyn ScheduledCaptureBackend<Tensor = T, Error = E>,
        value: &T,
        inference: InferenceGeometry,
            ...
            match source.kind() {
                ...
                PartitionPrefillCaptureKind::Summary(plan) => {
                    ...
                    let geometry = CaptureSummaryGeometry::prepare_partition(
                        plan.admission(),
                        plan.selection_index(),
                        CapturePhase::Prefill,
                        0,
                        None,
                        receipt.producer(rank).ok_or(LocalCause::Identity)?,
                        0,
                    )?
                    .fragment(&fragment)?;
                    ...
                    validate_or_begin::<E>(bank, receipt, chunk, &dtype, usage)?;
                    let PartitionFragmentDestination::Summary(claim) =
                        bank.take_local_prefill_reduction(receipt, 0, &fragment)?
                    else {
                        return Err(LocalCause::Identity);
                    };
                    let value = if fragment.selected_elements() == 0 {
                        claim.finish_empty().map_err(FundedCaptureError::from)?
                    } else {
                        backend.transform_summary(value, claim)?
                    };
                    bank.record_local_prefill_reduction(
                        receipt,
                        0,
                        &fragment,
                ...
                PartitionPrefillCaptureKind::Histogram(plan) => {
                    ...
                    validate_or_begin::<E>(bank, receipt, chunk, &dtype, usage)?;
                    let PartitionFragmentDestination::Histogram(claim) =
                        bank.take_local_prefill_reduction(receipt, 0, &fragment)?
                    else {
                        return Err(LocalCause::Identity);
                    };
...
fn validate_or_begin<E: std::error::Error + 'static>(
    ...
    let estimate = PartitionCaptureNativeEstimate {
        capture: usage,
        generated_creation_bytes: 0,
    };
    if !bank
        .allowance
        .fragment_source(bank.allowance.local_rank(), 0)
        .is_some_and(|(expected, actual)| expected == dtype && actual == estimate)
    {
        return Err(LocalCause::Identity);
    }
    if chunk == 0 {
...
fn control_bytes<T, E: std::error::Error + Send + Sync + 'static>(
    receipt: &PartitionCaptureReceiptPlan,
) -> Option<usize> {
    let transform = &receipt
        ...
        .plan()
        .selections
        .get(receipt.context().selection_index)?
        .transform;
    ...
    let worker = if raw {
        let parts = [
            ...
            size_of::<
                Result<
                    crate::working_memory::CapturePrefillFragmentWriter<'_, '_, '_, '_>,
                    CaptureRunHostError,
                >,
            >(),
        ];
        ...
        let parts = [
            ...
            size_of::<crate::working_memory::CaptureSummaryClaim<'_, '_>>(),
            size_of::<crate::working_memory::ClaimedCaptureSummary>(),
            size_of::<Result<crate::working_memory::ClaimedCaptureSummary, FundedCaptureError<E>>>(
            ),
            size_of::<
                Result<
                    crate::working_memory::ClaimedCaptureSummary,
                    crate::working_memory::CaptureSummaryFailure,
                >,
            >(),
        ];
        parts
        ...
        let parts = [
            ...
            size_of::<crate::working_memory::CaptureHistogramClaim<'_, '_>>(),
            size_of::<crate::working_memory::ClaimedCaptureHistogram>(),
            ...
            size_of::<
                Result<
                    crate::working_memory::ClaimedCaptureHistogram,
                    crate::working_memory::CaptureHistogramFailure,
                >,
            >(),
        ];
        parts
    ...
    let parts = [
        ...
        size_of::<
            ...
            >,
        >(),
        PartitionPrefillCapturePlan::control_bytes()?,
