...
use crate::capture::partition::{PartitionInvocationCaptureGeometry,PartitionInvocationCaptureKind,PartitionCaptureProgramError};

impl PartitionLocalCaptureHook {
    pub(crate) fn observe_invocation_program<T,E:std::error::Error+Send+Sync+'static>(self,
    ...
    pub(crate) fn observe_invocation<T,E:std::error::Error+Send+Sync+'static>(mut self,
        backend:&mut dyn ScheduledCaptureBackend<Tensor=T,Error=E>,value:&T)
        ->Result<Self,PartitionLocalCaptureFailure<E>> {
        let result=(||{
            self.metadata.reserve_metadata(control_bytes::<T,E>().ok_or(LocalCause::Identity)?)?;
            ...
            let (dtype,usage)=match source.kind() {
                PartitionInvocationCaptureKind::Tensor(geometry)=>(
                    backend.validate_source(value,geometry).map_err(FundedCaptureError::Backend)?,
                    backend.estimate(value,geometry)?),
                PartitionInvocationCaptureKind::Summary(geometry)=>(
                    backend.validate_summary_source(value,geometry)?,backend.estimate_summary(geometry)?),
                PartitionInvocationCaptureKind::Histogram(geometry)=>(
                    backend.validate_histogram_source(value,geometry)?,backend.estimate_histogram(geometry)?),
            };
            let destination=bank.take_local(receipt,0,&dtype,
                PartitionCaptureNativeEstimate{capture:usage,generated_creation_bytes:0})?;
            ...
            let completed=match claim {
                ...
                PartitionFragmentDestination::Tensor(claim)=>PartitionFragmentValue::Tensor(
                    backend.transform(value,claim).map_err(FundedCaptureError::Backend)?),
...
fn control_bytes<T,E:std::error::Error+Send+Sync+'static>()->Option<usize> {
    let parts=[PartitionInvocationCaptureGeometry::control_bytes()?,
        ...
        size_of::<PartitionLocalCaptureHook>()*2,size_of::<PartitionLocalCaptureFailure<E>>(),size_of::<LocalCause<E>>(),
        size_of::<Result<PartitionLocalCaptureHook,PartitionLocalCaptureFailure<E>>>(),size_of::<Result<(),LocalCause<E>>>(),
        size_of::<(PartitionLocalCaptureHook,&mut dyn ScheduledCaptureBackend<Tensor=T,Error=E>,&T)>(),
        size_of::<(&mut PartitionLocalCaptureHook,&mut dyn ScheduledCaptureBackend<Tensor=T,Error=E>,&T)>(),
        size_of::<crate::capture::partition::PartitionCaptureLocalHook>()*2,size_of::<PartitionCaptureProgramError>(),
        size_of::<Result<crate::capture::partition::PartitionCaptureLocalHook,PartitionCaptureProgramError>>(),
        size_of::<NativePartitionFragmentDestination<'_,'_>>(),size_of::<PartitionFragmentDestination<'_,'_>>(),
        size_of::<Result<NativePartitionFragmentDestination<'_,'_>,PartitionFragmentDestinationError>>(),
        size_of::<(PartitionFragmentDestination<'_,'_>,PreparedPartitionFragmentLoan)>(),
        size_of::<PreparedPartitionFragmentLoan>(),size_of::<PartitionCaptureNativeEstimate>(),
        size_of::<TensorDtype>(),size_of::<CaptureUsage>(),size_of::<(TensorDtype,CaptureUsage)>(),
        size_of::<Result<TensorDtype,E>>(),size_of::<Result<TensorDtype,FundedCaptureError<E>>>(),
        size_of::<Result<CaptureUsage,CaptureError>>(),size_of::<PartitionFragmentValue>(),
