...
use crate::capture::partition::{
    PartitionCaptureDecoder, PartitionCaptureExchange, PartitionCaptureExchangeError,
    PartitionCapturePayload, PartitionCaptureReceiptPlan, PartitionCaptureRecordEncoding,
...
impl<T: PartitionCaptureTransport> fmt::Debug for PreparedPartitionTensorDelivery<'_, T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("PreparedPartitionTensorDelivery")
    ...
    <T::Completion as Completion>::Error: Send + Sync + 'static,
{
    fn preparation_control_bytes() -> Option<usize> {
        let controls = [
            size_of::<Self>(),
            size_of::<PartitionCaptureTensorDeliveryError>(),
            size_of::<DeliveryCause>(),
            size_of::<Result<Self, PartitionCaptureTensorDeliveryError>>(),
            size_of::<Result<(), PartitionCaptureTensorDeliveryError>>(),
            size_of::<Option<CaptureTensorCustody>>(),
            ...
            size_of::<
                Result<
                    Option<crate::capture::partition::CompleteVocabularyGeometry<'_>>,
    ...
    /// Metadata consumed by this complete-producer delivery on the actual rank.
    /// Receipt preparation, evidence and native transport callbacks are quoted
    /// by their source owners; this query covers only this delivery worker.
    pub fn execution_metadata_bytes(
        receipt: &PartitionCaptureReceiptPlan,
        local_rank: usize,
    ) -> Option<usize> {
        let mut producers = receipt.producers();
        let (producer, projection) = producers.next()?;
        if producers.next().is_some()
            || projection.fragments().len() != 1
            || projection.local_shape() != projection.global_shape()
            || receipt.routed_producer(producer).is_some()
            || local_rank >= receipt.world_size()
        {
            return None;
        }
        ...
        let mut bytes = Self::preparation_control_bytes()?
            .checked_add(super::vocabulary::exchange_control_bytes(transform)?)?
            .checked_add(crate::capture::partition::CompleteVocabularyGeometry::control_bytes()?)?;
        if matches!(transform, CaptureTransform::Histogram { .. }) {
            bytes = bytes.checked_add(CaptureHistogramGeometry::preparation_control_bytes()?)?;
        }
        bytes.checked_add(if local_rank == producer {
            PartitionCaptureRecordEncoding::metadata_bytes(maximum)?
        } else {
            super::decode_metadata_bytes(transform, maximum)?
        })
    }
    ...
    pub(crate) fn prepare_with_source(
        ...
        let error = |cause| PartitionCaptureTensorDeliveryError {
            cause,
            _source: source.clone(),
            _custody: None,
            _metadata: metadata.clone(),
        };
        let bytes = Self::preparation_control_bytes()
        ...
        let admitted = &source;
        let mut producers = receipt.producers();
        let (producer, projection) = producers
            .next()
            .ok_or_else(|| error(DeliveryCause::Source("receipt has no producer")))?;
        if producers.next().is_some()
        ...
        {
            return Err(error(DeliveryCause::Source(
                "receipt is not one complete floating tensor producer",
            )));
        }
        let index = receipt.context().selection_index;
        metadata
            .reserve_metadata(
                super::vocabulary::exchange_control_bytes(
                    &admitted.admission().plan().selections[index].transform,
                )
                .ok_or_else(|| {
                    error(DeliveryCause::Source(
                        "vocabulary exchange controls overflow",
                    ))
                })?,
            )
            .map_err(|cause| error(cause.into()))?;
        ...
        metadata
            ...
            .map_err(|cause| error(cause.into()))?;
        let vocabulary_geometry = crate::capture::partition::CompleteVocabularyGeometry::prepare(
            admitted.admission(),
            index,
            receipt.context().phase,
            receipt.context().prediction,
            if vocabulary {
                projection
                    .global_shape()
                    .get(1)
        ...
        .map_err(|_| {
            error(DeliveryCause::Source(
                "receipt terminal vocabulary source differs",
            ))
        })?;
        let tensor_geometry = if summary || histogram || vocabulary {
            None
        } else {
            Some(
                CaptureTensorGeometry::prepare(
                    ...
                    receipt.context().phase,
                    receipt.context().prediction,
                    receipt.context().invocation,
                )
                .map_err(|_| {
                    error(DeliveryCause::Source(
                        "receipt selection has no scheduled tensor destination",
                    ))
                })?,
            )
        };
        let summary_geometry = if summary {
            Some(
                CaptureSummaryGeometry::prepare(
                    admitted.admission(),
                    index,
                    receipt.context().phase,
                    receipt.context().prediction,
                ...
                .map_err(|_| {
                    error(DeliveryCause::Source(
                        "receipt selection has no scheduled summary destination",
                    ))
                })?,
            )
        } else {
        ...
        let histogram_geometry = if histogram {
            let controls =
                CaptureHistogramGeometry::preparation_control_bytes().ok_or_else(|| {
                    error(DeliveryCause::Source(
                        "histogram geometry controls overflow",
                    ))
                })?;
            metadata
                .reserve_metadata(controls)
                .map_err(|cause| error(cause.into()))?;
            Some(
                CaptureHistogramGeometry::prepare(
                    admitted.admission(),
                    index,
                    receipt.context().phase,
                    receipt.context().prediction,
                ...
                .map_err(|_| {
                    error(DeliveryCause::Source(
                        "receipt selection has no scheduled histogram destination",
                    ))
                })?,
            )
        } else {
        ...
        let shape = match (
            ...
            _ => {
                return Err(error(DeliveryCause::Source(
                    "receipt payload selection differs",
                )));
            }
        };
        ...
        {
            return Err(error(DeliveryCause::Source(
                "receipt source geometry differs from scheduled admission",
            )));
        }
        let kind = if summary {
            PayloadKind::Summary
    ...
    pub(crate) fn bind_source_dtype(
        &mut self,
        dtype: TensorDtype,
    ) -> Result<(), PartitionCaptureTensorDeliveryError> {
        if !matches!(
            dtype,
            TensorDtype::F16 | TensorDtype::F32 | TensorDtype::Bf16
        ) || self.dtype.as_ref().is_some_and(|current| current != &dtype)
        {
            return Err(PartitionCaptureTensorDeliveryError {
                cause: DeliveryCause::Source("resolved producer scalar differs"),
    ...
    pub fn deliver(
        ...
        let dtype = match self.dtype.clone() {
            ...
            None => {
                return Err(PartitionCaptureTensorDeliveryError {
                    cause: DeliveryCause::Source(
                        "producer scalar was not resolved before delivery",
                    ),
                    _source: self.source.clone(),
        ...
        let local = (|| {
            ...
            let record =
                frame
                    .records()
                    .get(index)
                    .ok_or(PartitionCaptureExchangeError::Protocol(
                        "scheduled producer record missing",
                    ))?;
            if !matches!(
                record.outcome,
                CaptureOutcome::Captured | CaptureOutcome::Truncated { .. }
            ) || !matches!(
                (self.kind, &record.payload),
            ...
            ) || record.source_dtype.as_ref() != Some(&dtype)
                || record.charged != self.charged
            {
                return Err(PartitionCaptureExchangeError::Protocol(
                    "scheduled producer record differs",
                ));
            }
            let maximum =
                usize::try_from(receipt.max_record_bytes()).map_err(|_| CaptureError::Overflow)?;
            Ok(Some(
                PartitionCaptureRecordEncoding::new(
                    context,
                    receipt.identity(),
                    rank,
                    receipt.combination(),
        ...
        self.exchange
            ...
            .map_err(|cause| PartitionCaptureTensorDeliveryError {
                cause: cause.into(),
                _source: self.source,
                _custody: Some(custody),
...
struct TensorDecoder<'f, 'a> {
    frame: &'f mut ScheduledCaptureStep<'a>,
    rank: usize,
    producer: usize,
    dtype: TensorDtype,
    kind: PayloadKind,
    charged: CaptureUsage,
    metadata: &'f HostMetadataFunding,
    evidence: Option<crate::capture::partition::PreparedPartitionCaptureEvidence>,
}
impl<T: PartitionCaptureTransport> PartitionCaptureDecoder<T> for TensorDecoder<'_, '_> {
    ...
    fn decode(
        ...
        payload.for_each(|rank, bytes| {
            if received || rank != self.producer {
                return Err(PartitionCaptureExchangeError::Protocol(
                    "complete tensor producer differs",
                ));
            }
            received = true;
            if self.rank == self.producer {
                return Ok(());
            }
            let index = receipt.context().selection_index;
            ...
            let previous = self
                ...
                .get(index)
                .ok_or(PartitionCaptureExchangeError::Protocol(
                    "receiver record missing",
                ))?
                .charged;
            let additional = if prefill {
                if previous != self.charged {
                    return Err(PartitionCaptureExchangeError::Protocol(
                        "remote prefill charge differs",
                    ));
                }
                ...
                difference().ok_or(PartitionCaptureExchangeError::Protocol(
                    "receiver charge exceeds complete receipt",
                ))?
            };
            ...
            match self.kind {
                PayloadKind::Tensor => {
                    let claim = if prefill {
                        self.frame.take_remote_prefill_tensor(index)?
                    } else {
                        self.frame.take_tensor(index)?
                    };
                    let value = claim.decode_partition_receipt(bytes, expected, self.metadata)?;
                    if prefill {
                        self.frame
                            .record_remote_prefill_tensor(value, self.dtype.clone())?;
                    } else {
                        self.frame
                            .record_tensor(value, self.dtype.clone(), additional)?;
                    }
                }
                PayloadKind::Summary => {
                    let claim = if prefill {
                        self.frame.take_remote_prefill_summary(index)?
                    } else {
                        self.frame.take_summary(index)?
                    };
                    let value = claim.decode_partition_receipt(bytes, expected, self.metadata)?;
                    if prefill {
                        self.frame
                            .record_remote_prefill_summary(value, self.dtype.clone())?;
                    } else {
                        self.frame
                            .record_summary(value, self.dtype.clone(), additional)?;
                    }
                }
                PayloadKind::Histogram => {
                    let claim = if prefill {
                        self.frame.take_remote_prefill_histogram(index)?
                    } else {
                        self.frame.take_histogram(index)?
                    };
                    let value = claim.decode_partition_receipt(bytes, expected, self.metadata)?;
                    if prefill {
                        self.frame
                            .record_remote_prefill_histogram(value, self.dtype.clone())?;
                    } else {
                        self.frame
                            .record_histogram(value, self.dtype.clone(), additional)?;
                    }
                }
                PayloadKind::Candidates => {
                    let claim = if prefill {
                        self.frame.take_remote_prefill_candidates(index)?
                    } else {
                        self.frame.take_candidates(index)?
                    };
                    let value = claim.decode_partition_receipt(bytes, expected, self.metadata)?;
                    if prefill {
                        self.frame
                            .record_remote_prefill_candidates(value, self.dtype.clone())?;
                    } else {
                        self.frame
                            .record_candidates(value, self.dtype.clone(), additional)?;
                    }
                }
                PayloadKind::Scores => {
                    let claim = if prefill {
                        self.frame.take_remote_prefill_token_scores(index)?
                    } else {
                        self.frame.take_token_scores(index)?
                    };
                    let value = claim.decode_partition_receipt(bytes, expected, self.metadata)?;
                    if prefill {
                        self.frame
                            .record_remote_prefill_token_scores(value, self.dtype.clone())?;
                    } else {
                        self.frame
                            .record_token_scores(value, self.dtype.clone(), additional)?;
                    }
                }
            }
            Ok(())
        ...
        if !received {
            return Err(PartitionCaptureExchangeError::Protocol(
                "complete tensor receipt missing",
            ));
        }
        if let Some(evidence) = self.evidence.take() {
            if !evidence.matches(&receipt) {
                return Err(PartitionCaptureExchangeError::Protocol(
                    "partition evidence differs from decoded receipt",
                ));
            }
