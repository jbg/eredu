...
use super::*;
use eredu_core::capture::{CaptureUsage, PartitionCaptureContext};
...
mod reader;
mod vocabulary;
pub(in crate::working_memory::capture_run) use vocabulary::{
    VocabularyDestination, decode_vocabulary_receipt, prepare_vocabulary_decoder,
};
mod exchange;
pub use exchange::{PartitionCaptureTensorDeliveryError, PreparedPartitionTensorDelivery};
use reader::TensorReader;

fn reserve_parser_headroom(
    bytes: &[u8],
    funding: &HostMetadataFunding,
) -> Result<(), HostMetadataFundingError> {
    let estimate = parser_metadata_bytes(bytes.len()).ok_or(HostMetadataFundingError::Overflow)?;
...
impl fmt::Debug for PartitionCaptureTensorReceipt<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("PartitionCaptureTensorReceipt")
...
impl<'a, 'c> CaptureTensorClaim<'a, 'c> {
    ...
    pub fn decode_partition_receipt(
        ...
        self.decode_partition_receipt_combined(
            bytes,
            expected,
            funding,
            eredu_core::capture::PartitionCaptureCombination::Disjoint,
        )
    }
    pub(in crate::working_memory::capture_run) fn decode_partition_receipt_combined(
        ...
        let controls = tensor_decoder_control_bytes()
        ...
        if expected.context.capture_plan_identity != geometry.admission().identity()
            ...
            || expected.context.invocation != geometry.invocation()
            || expected.context.invocation_window != geometry.invocation_window()
            ...
            )
        {
            return Err(error(Cause::Source(
                "receipt differs from its scheduled tensor claim",
            )));
        }
        ...
        for (axis, ((start, end), stride)) in geometry
            .starts()
            .iter()
            .zip(geometry.ends())
        ...
        funding
            .reserve_metadata(TensorReader::construction_control_bytes().ok_or_else(|| {
                error(Cause::Source(
                    "receipt reader construction controls overflow",
                ))
            })?)
            .map_err(|cause| error(cause.into()))?;
        ...
        let mut reader = TensorReader::new(
            &mut destination,
            expected,
            source,
            index,
        ...
        if !valid {
            return Err(error(Cause::Source(
                "receipt identity, geometry, charge or payload differs",
            )));
        }
        destination
            .finish()
            .map_err(|cause| error(Cause::Incomplete(cause.into_owned_error())))
    }
}

impl ScheduledCaptureStep<'_> {
    pub(crate) fn prepare_partition_evidence(
        &mut self,
        funding: &HostMetadataFunding,
    ) -> Result<(), CaptureRunHostError> {
        self.frame
            .prepare_partition_evidence(funding)
            .map_err(Into::into)
    }
    pub(crate) fn record_partition_evidence(
        &mut self,
        evidence: crate::capture::partition::PreparedPartitionCaptureEvidence,
    ) -> Result<(), CaptureRunHostError> {
        self.frame
            .record_partition_evidence(evidence)
            .map_err(Into::into)
    }
    #[cfg(test)]
...
pub(in crate::working_memory::capture_run) fn decode_summary_receipt(
    ...
    {
        return Err(error(Cause::Source(
            "receipt differs from its scheduled summary claim",
        )));
    }
    ...
    let mut reader = TensorReader::new_summary(
        &mut value,
        expected,
        geometry.admission(),
    ...
    if !valid {
        return Err(error(Cause::Source(
            "receipt identity, geometry, charge or summary differs",
        )));
    }
    Ok(value)
}
impl PartitionCaptureTensorDecodeError {
    pub(in crate::working_memory::capture_run) fn retaining_summary_failure(
        cause: CaptureSummaryFailure,
        custody: CaptureTensorCustody,
        funding: &HostMetadataFunding,
    ) -> Self {
        Self {
            cause: Cause::Summary(cause),
            _histogram: None,
            _vocabulary: None,
            _custody: custody,
            _metadata: funding.clone(),
...
pub(in crate::working_memory::capture_run) fn decode_histogram_receipt(
    ...
    funding
        .reserve_metadata(
            histogram_decoder_control_bytes()
    ...
    {
        return Err(error(Cause::Source(
            "receipt differs from its scheduled histogram claim",
        )));
    }
    let rank = geometry.source_shape().len();
    let mut source_shape = [0usize; 32];
    ...
    funding
        ...
        .map_err(|cause| error(cause.into()))?;
    reserve_parser_headroom(bytes, funding).map_err(|cause| error(cause.into()))?;
    let mut reader = TensorReader::new_histogram(
        value,
        expected,
        geometry.admission(),
        geometry.selection_index(),
    ...
    if !valid {
        return Err(error(Cause::Source(
            "receipt identity, geometry, charge or histogram differs",
        )));
    }
    Ok(())
}
impl PartitionCaptureTensorDecodeError {
    pub(in crate::working_memory::capture_run) fn retaining_histogram_payload(
        mut self,
        value: CaptureHistogram,
    ) -> Self {
        self._histogram = Some(value);
        self
    }
    pub(in crate::working_memory::capture_run) fn retaining_histogram_failure(
        cause: CaptureHistogramFailure,
        custody: CaptureTensorCustody,
        funding: &HostMetadataFunding,
    ) -> Self {
        Self {
            cause: Cause::Histogram(cause),
            _histogram: None,
            _vocabulary: None,
            _custody: custody,
            _metadata: funding.clone(),
...
#[derive(Debug)]
enum VocabularyFailure {
    Candidates(CaptureCandidateFailure),
    Scores(CaptureTokenScoreFailure),
}
impl PartitionCaptureTensorDecodeError {
    pub(in crate::working_memory::capture_run) fn retaining_candidates(
        mut self,
        value: CaptureCandidateFailure,
    ) -> Self {
        self._vocabulary = Some(VocabularyFailure::Candidates(value));
        self
    }
    pub(in crate::working_memory::capture_run) fn retaining_scores(
        mut self,
        value: CaptureTokenScoreFailure,
    ) -> Self {
        self._vocabulary = Some(VocabularyFailure::Scores(value));
        self
    }
    pub(in crate::working_memory::capture_run) fn candidate_failure(
        value: CaptureCandidateFailure,
        custody: CaptureTensorCustody,
        funding: &HostMetadataFunding,
    ) -> Self {
        Self {
            cause: Cause::Candidates(value),
            _histogram: None,
            _vocabulary: None,
            _custody: custody,
            _metadata: funding.clone(),
    ...
    pub(in crate::working_memory::capture_run) fn score_failure(
        value: CaptureTokenScoreFailure,
        custody: CaptureTensorCustody,
        funding: &HostMetadataFunding,
    ) -> Self {
        Self {
            cause: Cause::Scores(value),
            _histogram: None,
            _vocabulary: None,
            _custody: custody,
            _metadata: funding.clone(),
...
fn tensor_decoder_control_bytes() -> Option<usize> {
    let controls = [
...
fn summary_decoder_control_bytes() -> Option<usize> {
    let controls = [
        size_of::<PartitionCaptureTensorReceipt<'_>>(),
        size_of::<TensorReader<'_, '_, '_>>(),
        size_of::<CaptureSummary>() * 2,
        size_of::<[[usize; 32]; 2]>(),
        size_of::<(
            usize,
            usize,
            &CaptureSummaryGeometry<'_>,
            &AdmittedCapturePlan,
        )>(),
        size_of::<PartitionCaptureTensorDecodeError>(),
        size_of::<Cause>(),
...
fn histogram_decoder_control_bytes() -> Option<usize> {
    let controls = [
        size_of::<PartitionCaptureTensorReceipt<'_>>(),
        size_of::<TensorReader<'_, '_, '_>>(),
        size_of::<CaptureHistogram>() * 2,
        size_of::<[[usize; 32]; 2]>(),
        size_of::<(
            usize,
            usize,
            &CaptureHistogramGeometry<'_>,
            &AdmittedCapturePlan,
        )>(),
        size_of::<PartitionCaptureTensorDecodeError>(),
        size_of::<Cause>(),
...
fn histogram_claim_control_bytes() -> Option<usize> {
    let frames = [
...
fn parser_metadata_bytes(maximum: usize) -> Option<usize> {
    crate::working_memory::DependencyMemoryPolicy::default().estimate(maximum)
}
/// Explicit metadata debits for the selected closed receipt decoder. The
/// parser allowance uses its existing dependency estimate and maximum wire size;
/// original result destinations remain owned by their separately prepared H.
fn decode_metadata_bytes(transform: &CaptureTransform, maximum: usize) -> Option<usize> {
    let controls = match transform {
        CaptureTransform::Summary => summary_decoder_control_bytes()?,
        CaptureTransform::Histogram { .. } => {
            histogram_decoder_control_bytes()?.checked_add(histogram_claim_control_bytes()?)?
        }
        CaptureTransform::TopCandidates { .. } | CaptureTransform::TokenScores { .. } => {
            return vocabulary::decoder_metadata_bytes(maximum);
        }
        CaptureTransform::FullTensor
        | CaptureTransform::Slice
        | CaptureTransform::Preview { .. } => tensor_decoder_control_bytes()?,
        CaptureTransform::RoutedUnits => return None,
    };
    controls
        .checked_add(TensorReader::construction_control_bytes()?)?
        .checked_add(parser_metadata_bytes(maximum)?)
}
