...
use super::{
    MemoryLedger, OriginalControllerCompilation, OriginalForbiddenSource, OriginalTokenTrieSource,
    OriginalTokenizer, WorkingMemoryError, original_declaration_source::Account,
};
use eredu_core::speculative::{PreparedGrammarController, PreparedGrammarSource};
use eredu_core::{
    HostPreparationAuthority, SharedControllerBytes, SharedControllerDeclaration,
    SharedTokenFilter, SpeculativeBuffer, SpeculativeBufferAllocationError,
};
use eredu_text::json_fragments::{JsonCallId, JsonFieldNames};
use eredu_text::semantic_channels::tagged::{TaggedEncoding, TaggedToolProgram, ToolProgram};
use eredu_text::semantic_channels::{
    ChannelProgram, DelimitedChannel, JsonEnvelope, JsonToolLayout, JsonToolProgram, JsonToolShape,
};
use std::{
    alloc::Layout,
...
pub trait OriginalToolValidation: std::fmt::Debug + Send + Sync {
    ...
    fn contains_tagged_tool(&self, _name: &str) -> bool {
        false
    }
    ...
    fn tagged_missing_required(
        &self,
        _name: &str,
        _parameters: &eredu_text::semantic_channels::tagged::TaggedParameters,
    ) -> bool {
        true
    }
...
impl ControllerSource {
    ...
    fn matches<C: eredu_core::SpeculativeTokenFilterController>(
        ...
        let source = match (
            ...
            (Some(grammar), None, None) => {
                eredu_core::PreparedControllerSource::Grammar(grammar.prepared_grammar_source())
            }
    ...
    fn matches_source(
        &self,
        actual: eredu_core::PreparedControllerSource<'_>,
        pool: &MemoryLedger,
    ) -> Result<(), WorkingMemoryError> {
        match (self, actual) {
            ...
            (
                ...
                eredu_core::PreparedControllerSource::Grammar(actual),
            ) => {
                ...
                if !validity.same_storage(actual.validity())
                    || !recipe.same_storage(actual.recipe())
                    || !declaration.same_storage(actual.declaration())
                {
                    return Err(WorkingMemoryError::IdentityMismatch);
                }
                Ok(())
...
impl OriginalSemanticChannelSource {
    ...
    pub fn json_tools(&self) -> Option<JsonToolProgram<'_>> {
        let Tools::Json(t) = self.payload().tools? else {
            return None;
        };
        ...
        Some(JsonToolProgram {
            ...
            fields: JsonFieldNames {
                name: literal(6),
                arguments: literal(7),
                call_id: t.id_length.map(|length| JsonCallId {
                    field: literal(8),
                    length,
                }),
            },
    ...
    pub fn tagged_tools(&self) -> Option<TaggedToolProgram<'_>> {
        let Tools::Tagged(t) = self.payload().tools? else {
            return None;
        };
        let l = |i| self.text(t.literals[i]);
        Some(TaggedToolProgram {
            output: JsonEnvelope {
                prefix: l(0),
                suffix: l(1),
            },
            ...
            encoding: TaggedEncoding {
                function_prefix: l(4),
                function_name_suffix: l(5),
                parameter_prefix: l(6),
                parameter_name_suffix: l(7),
                parameter_type: t.has_type.then(|| JsonEnvelope {
                    prefix: l(8),
                    suffix: l(9),
                }),
                parameter_value_prefix: l(10),
                parameter_suffix: l(11),
                function_suffix: l(12),
    ...
    pub fn forbidden(&self) -> Option<&OriginalForbiddenSource> {
        match &self.payload().controller {
            ControllerSource::Forbidden(source) => Some(source),
            _ => None,
        }
    }
    ...
    pub fn control_bytes() -> Option<usize> {
        let parts = [
            ...
            size_of::<ControllerSource>(),
            size_of::<OriginalSemanticControllerSource<'_>>(),
            size_of::<eredu_core::PreparedControllerSource<'_>>(),
            size_of::<(
                &ControllerSource,
                eredu_core::PreparedControllerSource<'_>,
                &MemoryLedger,
            )>(),
            size_of::<Result<ControllerSource, WorkingMemoryError>>(),
            size_of::<(&ControllerSource, &MemoryLedger)>(),
            size_of::<(
                &MemoryLedger,
                &OriginalTokenizer,
            ...
            size_of::<Option<ToolProgram<'_>>>(),
            size_of::<Option<Tools>>(),
            size_of::<[&str; 13]>(),
            size_of::<[Span; 13]>(),
            ...
            size_of::<Result<(), WorkingMemoryError>>(),
            size_of::<(&Self, &MemoryLedger, &OriginalTokenizer)>(),
...
impl MemoryLedger {
    ...
    pub fn compile_semantic_channel_source(
        ...
        self.compile_semantic_channel_source_for_controller(
            tokenizer,
    ...
    pub fn compile_semantic_channel_source_for_controller(
        ...
        self.compile_semantic_channel_source_with_tools(
            tokenizer, controller, program, structural, None,
        )
    }
    ...
    pub fn compile_semantic_channel_source_with_tools(
        ...
        program: ChannelProgram<'_>,
        structural: &[(u32, &str, bool)],
        tools: Option<ToolProgram<'_>>,
    ) -> Result<OriginalSemanticChannelSource, OriginalSemanticChannelSourceError> {
        self.compile_semantic_channel_source_with_validation(
            tokenizer, controller, program, structural, tools, None,
        )
    }
    ...
    pub fn compile_semantic_channel_source_with_validation(
        ...
        if let Some(source) = &validation {
            ...
            source
                .validate_source(controller, self)
        ...
        if let Some(tools) = tools {
            for literal in tools.literals() {
                total = total.checked_add(literal.len()).ok_or_else(overflow)?;
            }
        }
        ...
        let result = (|| -> Result<OriginalSemanticChannelSource, Cause> {
            ...
            let tools = tools
                .map(|tools| {
                    ...
                    for (index, value) in tools.literals().into_iter().enumerate() {
                        literals[index] = append(destination, value)?;
                    }
                    Ok::<_, Cause>(match tools {
                        ToolProgram::Json(t) => Tools::Json(JsonTools {
                            literals: literals[..10].try_into().expect("fixed JSON literals"),
                            id_length: t.fields.call_id.map(|id| id.length),
                            shape: t.shape,
                            layout: t.layout,
                        }),
                        ToolProgram::Tagged(t) => Tools::Tagged(TaggedTools {
                            literals,
                            has_type: t.encoding.parameter_type.is_some(),
...
impl OriginalTokenizer {
    ...
    pub fn compile_semantic_channel_source_with_validation(
        ...
        self.pool().compile_semantic_channel_source_with_validation(
            self, controller, program, structural, tools, validation,
        )
    }
    ...
    pub fn compile_semantic_channel_source_with_tools(
        ...
        self.pool().compile_semantic_channel_source_with_tools(
            self, controller, program, structural, tools,
        )
    }
    ...
    pub fn compile_semantic_channel_source_for_controller(
        ...
        self.pool()
            .compile_semantic_channel_source_for_controller(self, controller, program, structural)
    }
