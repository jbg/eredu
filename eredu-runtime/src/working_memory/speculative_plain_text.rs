//! Source-bound decoder/stop state beneath the shared speculative driver.
...
use eredu_core::{
    FinishReason, GenerationDecoderError, GenerationPlainText, GenerationPlainTextEvent,
    GenerationPlainTextEvents, GenerationPlainTextProjection, HostMetadataFundingError,
    HostPreparationAuthority, SemanticText, SpeculativeBuffer, SpeculativeOutputError,
    SpeculativeSemanticOwner, SpeculativeSemanticState,
};
...
#[derive(Debug, Clone)]
pub struct OriginalSpeculativeSemanticPreparation {
...
impl OriginalSpeculativeSemanticPreparation {
    ...
    pub fn metadata_funding(&self) -> &WorkspaceMetadataFunding {
        &self.funding
    }
    ...
    pub fn prepare(
        ...
        maximum_draft_tokens: usize,
        skip_special: bool,
    ) -> Result<SpeculativeSemanticOwner, SpeculativeOutputError> {
        OriginalSpeculativePlainText::construct(
            self.clone(),
            stops,
            maximum_tokens,
...
/// Actual original tokenizer/stop state with a closed plain or channel projection.
/// The historical type name remains for source authentication compatibility.
...
#[derive(Debug)]
pub struct OriginalSpeculativePlainText {
...
fn controls() -> Option<usize> {
    let parts = [
        size_of::<OriginalSpeculativePlainText>(),
        size_of::<SpeculativeSemanticOwner>(),
        ...
        size_of::<Box<OriginalSpeculativePlainText>>(),
        size_of::<Box<dyn SpeculativeSemanticState>>(),
        size_of::<Option<OriginalSpeculativePlainText>>(),
...
impl OriginalSpeculativePlainText {
    fn construct(
        prepared: OriginalSpeculativeSemanticPreparation,
        stops: &OriginalStopSource,
        maximum_tokens: usize,
        maximum_draft_tokens: usize,
        skip_special: bool,
        ...
        let mut stop_destinations = StopDestinations::new(stops.source(), layout.text_capacity())
            .map_err(|_| overflow())?;
        // K proposals plus one target token; a separate finish may flush text
        // and emit Finished. Stops can instead emit Finished on the last push.
        let event_capacity = if channels.is_some() {
            0
        } else {
            maximum_tokens
                .min(maximum_draft_tokens.checked_add(1).ok_or_else(overflow)?)
                .checked_add(2)
                .ok_or_else(overflow)?
        };
        let bytes = decoder
        ...
        Ok(SpeculativeSemanticOwner::from_prepared(Self {
            decoder,
    ...
    pub fn preparation(&self) -> &OriginalSpeculativeSemanticPreparation {
        &self.preparation
    }
    ...
    pub fn validate_controller<C: eredu_core::SpeculativeTokenFilterController>(
        ...
        if let Some(channels) = &self.channels {
            channels.source().validate_controller(controller, self.source.pool())?;
        }
        Ok(())
    }
    ...
    fn copy_bytes(&self) -> Option<usize> {
        self.decoder
            .prepare_copy()
            .required_bytes()?
...
impl SpeculativeSemanticState for OriginalSpeculativePlainText {
    ...
    fn fork_box(&self) -> Result<Box<dyn SpeculativeSemanticState>, SpeculativeOutputError> {
        Err(storage("prepared semantic state requires its closed owner"))
    }
