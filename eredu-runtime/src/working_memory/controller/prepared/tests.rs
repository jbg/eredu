use super::*;
use eredu_core::{
    HostPreparationAuthority, TextControllerContract, TokenSamplingDecision,
    speculative::{
        ForbiddenControllerSource, NoPreparedGrammar, PlainControllerHistory,
...
impl SpeculativeTokenFilterController for Controller {
    ...
    fn prepared_forbidden_source(&self) -> Option<ForbiddenControllerSource<'_>> {
        Some(
            ...
            .unwrap()
            .with_original_storage(OriginalSourceWitness::new(&self.source)),
        )
    }
}
fn tokenizer(pool: &MemoryLedger) -> OriginalTokenizer {
    ...
    pool.compile_tokenizer(
        TokenizerPlan::prepare_json(json)
            .unwrap()
...
fn history(preparation: &PreparedSemanticSource) -> PlainControllerHistory {
    ...
    funding
        .reserve_metadata(
            PlainControllerHistory::copy_metadata_bytes(3).unwrap()
                + HostPreparationAuthority::retention_bytes::<eredu_core::HostMetadataFunding>()
                    .unwrap(),
        )
        .unwrap();
    PlainControllerHistory::default()
        .copy_prepared(3, HostPreparationAuthority::retain(funding.clone()))
        .unwrap()
}
...
fn semantic_admission_authenticates_actual_source_execution_and_history_payer() {
    ...
    let mut controller = Controller {
        ...
        source: pool
            ...
            .unwrap(),
        validity: SharedTokenFilter::new(tokenizer.generation_domain().unwrap().clone()),
        binding: None,
    };
    ...
    let stops = pool
        .compile_stop_source(
            eredu_text::stop_storage::StopCompilePlan::prepare(&stop_rows).unwrap(),
        )
        .unwrap();
    let semantic = preparation
        .prepare(&stops, 3, std::num::NonZeroUsize::new(1).unwrap(), true)
        .unwrap();
    let semantic = semantic
        .prepared_source()
        .unwrap()
        .downcast_ref::<crate::working_memory::PreparedSemanticState>()
        .unwrap();
    let actual_source = PreparedControllerBinding::source(&controller).unwrap();
    binding
        .validate_semantic_state(semantic, actual_source, 3)
        .unwrap();
    assert!(
        binding
            .validate_semantic_state(semantic, actual_source, 2)
            .is_err()
    );
    let other_preparation = PreparedSemanticSource::new(
        &tokenizer,
        &execution,
        crate::working_memory::memory_fixture::resolved_host_limits(&pool, capacity),
    )
    .unwrap();
    let other_semantic = other_preparation
        .prepare(&stops, 3, std::num::NonZeroUsize::new(1).unwrap(), true)
        .unwrap();
    let other_semantic = other_semantic
        .prepared_source()
        .unwrap()
        .downcast_ref::<crate::working_memory::PreparedSemanticState>()
        .unwrap();
    assert!(
        binding
            .validate_semantic_state(other_semantic, actual_source, 3)
            .is_err()
    );
    controller.binding = Some(binding.clone());
    let workspace = controller.inference_workspace(3).unwrap();
    let storage = ControllerStorageContract::inspect_original_retained(
        &controller,
        workspace,
        &pool,
        &execution,
    ...
    let foreign_decision =
        TokenSamplingDecision::new(TokenFilter::allowed(vec![false, true]).unwrap())
            .with_original_tokenizer_validity(
                &equal_foreign_domain,
                OriginalSourceWitness::new(&tokenizer),
            )
            .with_controller_storage(controller.inference_storage());
    assert!(
        storage
            .validate_sampling_decision(&contract, &foreign_decision, &pool)
            .is_err()
    );
    ...
    controller.source = pool
        .compile_forbidden_tokenizer_source(&foreign_tokenizer, b"hi")
        .unwrap();
