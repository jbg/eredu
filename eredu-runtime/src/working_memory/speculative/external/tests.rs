...
use crate::*;
use eredu_core::{
    InferenceGeometry, OutputDemand, SpeculativeRequestId,
...
fn descriptive_origin(tokens: &[u32]) -> SpeculativeActivationOrigin {
    ...
    SpeculativeActivationOrigin {
        request: SpeculativeRequestId::new(3),
        committed_tokens: tokens.len(),
        prediction: tokens.len(),
    ...
    for shape in [
        ...
        match shape {
            ExternalPredictionShape::Sequential => {
                ...
                assert_eq!(
                    plan.attempts(ExternalInvocationKind::TargetTokenEmbeddings),
                    16
                );
                assert_eq!(
                    plan.attempts(ExternalInvocationKind::TargetProjectLogits),
                    0
                );
            }
            ExternalPredictionShape::Fused => {
...
fn external_roles_bind_report_identity_and_keep_failed_attempts_and_continuation_custody() {
    ...
    let invocation = plan
        .invocation(
            ExternalInvocationKind::AssistantStep,
            geometry(18, 1),
            None,
            origin(),
        )
    ...
    .unwrap();
    let startup = request
        .reserve_external_startup(OriginalExternalSpeculativeSource::Assistant, 23)
        .unwrap();
    ...
    let exact = report(invocation.geometry());
    let wrong = report(geometry(19, 1));
    ...
    let failure = request
        .reserve_external_role(
            cursor.claim(invocation).unwrap(),
            requirements(wrong.span_workspace_plan()),
        )
        .unwrap_err();
    assert!(matches!(
        failure.cause(),
    ...
    let role = request
        .reserve_external_role(
            cursor.claim(invocation).unwrap(),
            requirements(exact.span_workspace_plan()),
        )
        .unwrap();
    assert!(role.physical_bytes() > 0);
    role.validate_plan(exact.span_workspace_plan()).unwrap();
    assert!(
    ...
    assert!(custody.belongs_to_external(&role));
