...
pub trait CommunicationPreparationProducer: Sized {
    ...
    fn produce(self, custody: CommunicationPreparationCustody)
...
impl<T> PreparedCommunicationPreparation<T> {
    ...
    pub fn with_output<R>(&mut self, run: impl FnOnce(&mut T) -> R) -> R {
        run(&mut self.output)
    }
    /// Immutable source identity retained through every native exit.
    pub fn source(&self) -> &RetainedCommunicationSource {
        &self.source
    }
...
impl<T> fmt::Debug for PreparedCommunicationPreparation<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("PreparedCommunicationPreparation")
            .field("original_bytes", &self.account.held_bytes())
...
impl<P: CommunicationPreparationProducer> fmt::Debug for CommunicationPreparationError<P> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("CommunicationPreparationError")
...
impl<P: CommunicationPreparationProducer> fmt::Display for CommunicationPreparationError<P> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if let Some(cause) = &self.production {
            fmt::Display::fmt(cause, f)
        } else {
            fmt::Display::fmt(self.accounting.as_ref().expect("refused producer"), f)
        }
    }
}
...
impl MemoryLedger {
    ...
    pub fn communication_preparation_required_bytes<P: CommunicationPreparationProducer>(
        plan: &P,
        ...
        let controls = [
            ...
            size_of::<CommunicationPreparationCustody>(),
            size_of::<P>(),
            size_of::<RetainedCommunicationSource>(),
            size_of::<&[u32]>(),
            size_of::<Result<P::Output, P::Error>>(),
            size_of::<Result<(), WorkingMemoryError>>(),
            size_of::<PreparedCommunicationPreparation<P::Output>>(),
            size_of::<CommunicationPreparationError<P>>(),
            size_of::<
                Result<
                    PreparedCommunicationPreparation<P::Output>,
                    CommunicationPreparationError<P>,
                >,
            >(),
            CommunicationPreparationError::<P>::retirement_control_bytes()
    ...
    pub fn prepare_communication<P: CommunicationPreparationProducer>(
        &self,
        plan: P,
    ) -> Result<PreparedCommunicationPreparation<P::Output>, CommunicationPreparationError<P>> {
        if let Err(cause) = plan.validate_pool(self) {
            return Err(CommunicationPreparationError::rejected(plan, cause));
        }
        let bytes = match Self::communication_preparation_required_bytes(&plan) {
            Ok(bytes) => bytes,
            Err(cause) => return Err(CommunicationPreparationError::rejected(plan, cause)),
        };
        if let Err(cause) = plan.check_preparation_policy(self, bytes) {
            return Err(CommunicationPreparationError::rejected(plan, cause));
        }
        ...
        match result {
            Ok(output) if accounting.is_none() => Ok(PreparedCommunicationPreparation {
                output,
                source,
                account,
            }),
            Ok(output) => Err(CommunicationPreparationError {
                accounting,
                production: None,
                output: Some(output),
                plan: None,
            ...
            Err(cause) => Err(CommunicationPreparationError {
                accounting,
                production: Some(cause),
                output: None,
                plan: None,
                source,
                account: Some(account),
