...
use super::*;
use eredu_nn::{Parameterized, Error};
use eredu_nn::workspace::{WorkspaceContext, WorkspaceExistingStorage, WorkspaceMetadataError, WorkspaceTensor};
use std::mem::{size_of, size_of_val};

/// One immutable parameter-source loan and its counted workspace destinations.
/// Roots describe future materialization only: none gives existing-source credit
/// or native allocation permission. Invocation roots retire before the next
/// acquire; trace roots preserve only the source's explicit alias lifetime.
...
impl std::fmt::Debug for WorkspaceParameterProjection<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("WorkspaceParameterProjection").field("counts", &self.counts).finish_non_exhaustive()
    }
}
impl<'source> WorkspaceParameterSourceLoan<'source> {
    ...
    pub fn prepare_projection(self, context: &WorkspaceContext)
        -> std::result::Result<WorkspaceParameterProjection<'source>, Error>
    {
        let layouts = workspace_parameter_control_layouts();
        let controls = layouts.iter().try_fold(size_of_val(&layouts), |bytes, (_, layout)|
            bytes.checked_add(layout.bytes)).ok_or(WorkspaceMetadataError::Overflow)?;
        context.charge_metadata(controls.checked_add(size_of::<(
            WorkspaceParameterProjection<'_>,
            std::result::Result<WorkspaceParameterProjection<'_>, Error>,
            &WorkspaceContext,
        )>()).ok_or(WorkspaceMetadataError::Overflow)?)?;
        let source = self.source;
        let counted = self.count().map_err(|cause| context.metadata_source(cause))?;
        ...
        drop(counted.fill(projection.destinations()).map_err(|cause| context.metadata_source(cause))?);
        Ok(projection)
    }
}
fn destination<T: Default>(count: usize, context: &WorkspaceContext) -> std::result::Result<Vec<T>, Error> {
...
impl WorkspaceParameterProjection<'_> {
    fn destinations(&mut self) -> WorkspaceParameterDestinations<'_> {
        WorkspaceParameterDestinations {
            units: &mut self.units, requested: &mut self.requested,
            rows: &mut self.rows, names: &mut self.names, shapes: &mut self.shapes,
            roots: &mut self.roots, window_members: &mut self.window_members,
        }
    }
    ...
    pub fn bind<M: Parameterized<WorkspaceTensor>>(
        &mut self, module: &mut M, ordinal: usize, address: ExecutionUnitAddress,
        context: &WorkspaceContext,
    ) -> std::result::Result<(), Error> {
        let frames = [
            size_of::<(&mut Self, &mut M, usize, ExecutionUnitAddress, &WorkspaceContext)>(),
            size_of::<Vec<crate::PreparedParameterBinding<'_, WorkspaceTensor>>>(),
            size_of::<crate::PreparedParameterBinding<'_, WorkspaceTensor>>(),
            size_of::<WorkspaceParameterRecord>(), size_of::<WorkspaceParameterRootRecord>(),
            size_of::<WorkspaceLayoutView<'_>>(), size_of::<WorkspaceExistingStorage>(),
        ...
        for row in &self.rows[range] {
            ...
            if root.is_none() {
                *root = Some(WorkspaceExistingStorage::try_new(Some(prototype.capacity_bytes), context)?);
            }
            let layout = context.layout(&self.shapes[row.shape.clone()], row.dtype.ok_or_else(fail)?)?
                .with_representation(row.representation);
            let value = WorkspaceTensor::existing_with_storage(layout, root.as_ref().expect("created root"), context)?;
