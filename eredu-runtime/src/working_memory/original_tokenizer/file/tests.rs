...
fn one_short_i_never_reserves_and_one_short_c_keeps_actual_file_bytes_in_its_error() {
    ...
    assert_eq!(
        error
            .source()
            .unwrap()
            .downcast_ref::<OriginalTokenizerFileError>()
            .unwrap()
            .input_bytes(),
        i
    );
...
fn root_rejection_empty_and_late_semantics_keep_input_without_reclassifying_it_as_c() {
    // The implicit ByteLevel regex is now supported by fresh construction.
    // Preserve that former negative as an admission-budget refusal under I alone.
    let supported = JSON.replace("\"use_regex\":false", "\"use_regex\":true");
    ...
    drop(error);
    assert_eq!(pool.used_bytes().unwrap(), 0);
    // Valid ordinary HF configuration whose fresh construction is still absent.
