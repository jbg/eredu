...
fn one_original_comparison_precedes_all_reserves_and_idle_aliases_keep_exact_source_charge() {
    ...
    let bytes = MemoryLedger::tokenizer_required_bytes(&plan(&input)).unwrap();
    ...
    drop((source, pool));
    values(&alias);
    assert_eq!(witness.payload_used_bytes().unwrap(), bytes);
    ...
    assert_eq!(witness.payload_peak_bytes().unwrap(), bytes);
    drop(rejected);
}
#[test]
...
fn forbidden_lexical_source_uses_exact_tokenizer_domain_and_independent_original_copy() {
    let tokenizer_bytes = MemoryLedger::tokenizer_required_bytes(&plan(INPUT)).unwrap();
    ...
    let source_bytes = MemoryLedger::forbidden_tokenizer_source_required_bytes(&packed, 2).unwrap();
    drop(packed);
    let source = pool
    ...
    assert_eq!(
        ...
        tokenizer_bytes + source_bytes
    );
    let foreign = crate::working_memory::memory_fixture::host_ledger(1 << 24, 0).unwrap();
    let error = foreign
        .compile_forbidden_tokenizer_source(&tokenizer, b"hi")
        .unwrap_err();
    assert_eq!(foreign.payload_used_bytes().unwrap(), 0);
    drop(error);
    let short =
        crate::working_memory::memory_fixture::host_ledger(tokenizer_bytes + source_bytes - 1, 0)
            .unwrap();
    let other = short.compile_tokenizer(plan(INPUT)).unwrap();
    let error = short
        .compile_forbidden_tokenizer_source(&other, b"hi")
        .unwrap_err();
    assert_eq!(short.payload_used_bytes().unwrap(), tokenizer_bytes);
    drop((error, other));
    assert_eq!(short.payload_used_bytes().unwrap(), 0);
...
fn original_trie_keeps_exact_tokenizer_bytes_metadata_and_failed_prefix_custody() {
    ...
    let tokenizer_bytes = MemoryLedger::tokenizer_required_bytes(&plan(INPUT)).unwrap();
    let probe_pool = crate::working_memory::memory_fixture::host_ledger(1 << 24, 0).unwrap();
    let probe = probe_pool.compile_tokenizer(plan(INPUT)).unwrap();
    let mut info = TokRxInfo::new(5, 4);
    info.tok_pad = Some(3);
    info.tok_end_of_turn = Some(4);
    let eos = [4, 3, 4];
    let bytes = MemoryLedger::token_trie_source_required_bytes(&probe, &info, &eos).unwrap();
    drop(probe);
    assert_eq!(probe_pool.payload_used_bytes().unwrap(), 0);
    let short =
        crate::working_memory::memory_fixture::host_ledger(tokenizer_bytes + bytes - 1, 0).unwrap();
    ...
    let pool =
        crate::working_memory::memory_fixture::host_ledger(tokenizer_bytes + bytes, 0).unwrap();
    ...
    assert_eq!(pool.payload_used_bytes().unwrap(), tokenizer_bytes + bytes);
    ...
    let other_pool =
        crate::working_memory::memory_fixture::host_ledger(tokenizer_bytes, 0).unwrap();
    ...
    let bytes = MemoryLedger::token_trie_source_required_bytes_with_memory_policy(
        &source,
        &info,
        &[0],
        policy,
