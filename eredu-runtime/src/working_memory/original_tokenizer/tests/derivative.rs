...
fn checked_derivative_retains_exact_original_and_does_not_grant_equal_source_identity() {
    ...
    assert_eq!(pool.payload_used_bytes().unwrap(), retained + trie_bytes);
    let peer = trie.clone();
...
fn source_native_prefix_budget_refusal_retains_original_without_new_destinations() {
    let measure = crate::working_memory::memory_fixture::host_ledger(1 << 24, 0).unwrap();
    ...
    let pool = crate::working_memory::memory_fixture::host_ledger(required - 1, 0).unwrap();
    let original = pool
        .compile_tokenizer(plan(PREFIX_INPUT).with_generation_domain().unwrap())
        .unwrap();
    ...
    drop(original);
    assert_eq!(pool.payload_used_bytes().unwrap(), root_bytes);
    drop(failure);
    assert_eq!(pool.payload_used_bytes().unwrap(), 0);
}
