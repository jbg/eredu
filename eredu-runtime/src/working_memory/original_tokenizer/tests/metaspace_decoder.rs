...
fn metaspace_decoder_source_cold_refusal_and_failed_destinations_retain_custody() {
    for sequence in [false, true] {
        ...
        let error = short.compile_tokenizer_with(plan(&input), || panic!("refused before source construction")).unwrap_err();
        assert!(matches!(error.accounting_failure(), Some(WorkingMemoryError::BudgetExceeded { .. })));
        assert_eq!(short.used_bytes().unwrap(), 0);
        for stage in 0..4 {
            let plan = if stage == 0 { plan(&input).fail_pipeline_reservation(5) }
                else { plan(&input).fail_decode_reservation(stage - 1) };
        ...
        let decoder = if sequence { &mut changed["decoder"]["decoders"][0] } else { &mut changed["decoder"] };
        decoder["prepend_scheme"] = "never".into();
        let changed = eredu_text::tokenizer::Tokenizer::from_bytes(changed.to_string().as_bytes()).unwrap();
