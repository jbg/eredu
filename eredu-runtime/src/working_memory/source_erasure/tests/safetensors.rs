...
fn funded(pool: &WorkingMemoryPool, path: &Path) -> SafetensorsWeightStore {
...
fn safetensors_erasure_compares_exact_bytes_and_keeps_weak_identity_custody() {
    ...
    let dir = fixture();
    let measurement = WorkingMemoryPool::new(1_000_000, 0).unwrap();
    let source = funded(&measurement, dir.path());
    let base = measurement.used_bytes().unwrap();
    drop(source);
    assert_eq!(measurement.used_bytes().unwrap(), 0);
    let short = WorkingMemoryPool::new(base + outer - 1, 0).unwrap();
    let failure = short
    ...
    assert!(
        ...
        if *required_bytes == outer && *available_bytes == outer-1)
    );
    ...
    let pool = WorkingMemoryPool::new(base + outer, 0).unwrap();
    let root = pool
    ...
    assert_eq!(pool.used_bytes().unwrap(), base + outer);
...
fn safetensors_erasure_rejects_foreign_ordinary_and_caller_policy_origins() {
    ...
    drop(error);
    #[derive(Debug)]
    struct CallerPolicy(WorkingMemoryPool);
...
fn funded_borrowed_headers_and_closed_source_feed_the_existing_prepared_read() {
    ...
    assert!(pool.used_bytes().unwrap() > outer + read_bytes);
    let mut output = [0; 2];
