...
impl HostSourceConstructionProgram {
    ...
    pub fn from_components(
        ...
        for part in &components {
            ...
            attempts = attempts
                .checked_add(part.attempts)
                .ok_or(WorkingMemoryError::Overflow)?;
            protected = protected
                .checked_add(part.protected)
                .ok_or(WorkingMemoryError::Overflow)?;
        }
        let root = HostSourceConstructionFacts::new(0, 0, 0)?;
        protected = protected
            .checked_add(root.protected)
            .and_then(|n| n.checked_add(Self::partition_control_bytes(components.len()).ok()?))
            .ok_or(WorkingMemoryError::Overflow)?;
    ...
    pub fn plan_control_bytes() -> Option<usize> {
        let parts = [
            ...
            size_of::<Vec<HostSourceConstructionFacts>>(),
            size_of::<std::slice::Iter<'_, HostSourceConstructionFacts>>(),
            size_of::<HostSourceConstructionFacts>() * 2,
            size_of::<[u64; 4]>(),
            size_of::<usize>(),
    ...
    fn partition_control_bytes(count: usize) -> Result<u64, WorkingMemoryError> {
        let parts = [
            ...
            size_of::<OriginalHostSourceProgramError>(),
            size_of::<Result<OriginalHostSourceProgramBanks, OriginalHostSourceProgramError>>(),
            size_of::<(OriginalHostSourceBank, &Self)>(),
            size_of::<Option<OriginalHostSourceBank>>(),
            size_of::<(&mut OriginalHostSourceProgramBanks, usize)>(),
        ...
        qualified_storage::array_bytes::<Option<OriginalHostSourceBank>>(count)?
            .checked_add(qualified_storage::vector_control_bytes::<
...
impl OriginalHostSourceProgramBanks {
    ...
    pub fn matches(&self, program: &HostSourceConstructionProgram) -> bool {
        Some(self.identity) == program.facts.program
    }
    ...
    pub fn take(&mut self, index: usize) -> Result<OriginalHostSourceBank, WorkingMemoryError> {
        self._source
            .controls
            .validate_account(self._source.reservation.as_ref())?;
        self.banks
            .get_mut(index)
...
impl OriginalHostSourceBank {
    ...
    pub fn partition_program(
        ...
        let result = (|| {
            ...
            if !self.matches_facts(program.facts)
            ...
            for &facts in &program.components {
                banks.push(Some(Self::new_with_custody(
                    facts,
        ...
        match result {
            Err(cause) => Err(OriginalHostSourceProgramError {
                cause,
                _source: self,
            }),
            Ok(banks) => {
                ...
                self.attempts = 0;
                self.partitions = 0;
                Ok(OriginalHostSourceProgramBanks {
                    banks,
                    identity,
                    _source: self,
                })
