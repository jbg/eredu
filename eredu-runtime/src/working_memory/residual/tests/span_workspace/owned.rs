...
fn accepted(
    ...
    let q = replacement_quote(pool, geometry(), publication_controls())
        .into_incremental()
        .with_span_workspace()
        .unwrap();
...
fn account(pool: &MemoryLedger, r: &WorkingMemoryReservation) -> (u64, u64, usize) {
    ...
    (
        state.remaining.checked_sub(state.control_floor).unwrap(),
        state.host_held.checked_sub(state.control_floor).unwrap(),
        state.scopes,
    )
}
#[test]
    ...
    for short in [false, true] {
        ...
        let pressure = native
            .adopt_host_storage_individually([(
                20u32,
                reservation_payload_bytes(&r) - p - publication_controls() + u64::from(short),
            )])
            .unwrap();
        ...
        if short {
            ...
            assert!(
                matches!(error.cause(), capacity_error if matches!(capacity_numbers(&capacity_error), Some((required_bytes, available_bytes)) if required_bytes==p && available_bytes+1==p))
            );
            ...
            assert!(matches!(
                native.adopt_host_storage_individually([(21u32, 1)]),
                Err(capacity_error) if matches!(capacity_numbers(&capacity_error), Some((required, available)) if required > available)));
...
fn earlier_diagnostic_alias_is_the_last_owner_of_full_p_after_run_and_receipt_retire() {
    ...
    let live = native
        .adopt_host_storage_individually([(
            22u32,
            reservation_payload_bytes(&r) - p - publication_controls(),
        )])
        .unwrap();
    let (owner, witness) = q.into_funded_span_workspace(&run, &r).unwrap();
    assert!(witness.is_none());
    ...
    native.certify().unwrap();
    let all = pool.payload_used_bytes().unwrap();
    ...
    assert_eq!(pool.payload_used_bytes().unwrap(), all - 64);
    ...
    drop(earlier);
    assert_eq!(pool.payload_used_bytes().unwrap(), all - 64 - p);
    drop(live);
    assert_eq!(pool.payload_used_bytes().unwrap(), 0);
}
#[test]
...
fn concurrent_alias_promotions_create_one_host_scope_and_preserve_the_losing_owner() {
    ...
    assert!(
        pool.payload_used_bytes().unwrap() >= p,
    ...
    for source_bytes in [0, 24] {
        for quarantine_before in [false, true] {
            ...
            let registered = source_scope
                .adopt_host_storage_individually([(30u32, source_bytes)])
                .unwrap()
                .into_values()
                .next()
                .unwrap();
...
fn retained_peak_contains_actual_attachment_compact_owner_and_failure_moves_before_admission() {
    ...
    assert_eq!(q.incremental_bytes().unwrap(), before + p);
    ...
    assert_eq!(reservation_payload_bytes(&r), before + p);
    drop((q, r, root));
    assert_eq!(pool.payload_used_bytes().unwrap(), 0);
}
...
fn unwinding_compact_owner_cannot_retire_p_while_an_earlier_plan_alias_survives() {
    ...
    assert_eq!(account(&pool, &r).2, 1);
    drop((r, run, root));
    assert!(pool.payload_used_bytes().unwrap() >= p);
    drop(alias);
    assert_eq!(pool.payload_used_bytes().unwrap(), 0);
}
