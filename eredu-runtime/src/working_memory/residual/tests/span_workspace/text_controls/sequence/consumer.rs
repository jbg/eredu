...
fn retained_provider_copy_has_independent_budget_state_and_output_lifetime() {
    ...
    let used = physical_used(&pool);
    let bytes = pool
        .prepare_generation_host_copy::<RetainedGenerationSequence, TextHostCopyError>(
            &sequence,
            crate::working_memory::memory_fixture::resolved_host_limits(&pool, u64::MAX),
        )
        .unwrap()
        .storage_bytes()
        .unwrap();
    ...
    let short = pool
        .prepare_generation_host_copy::<RetainedGenerationSequence, TextHostCopyError>(
            &sequence,
            crate::working_memory::memory_fixture::resolved_host_limits(&pool, used + bytes - 1),
        )
        .unwrap()
        .copy(bytes);
    assert!(matches!(
    ...
    assert_eq!(physical_used(&pool), used);
    ...
    let mut copied = pool
        .prepare_generation_host_copy::<RetainedGenerationSequence, TextHostCopyError>(
            &sequence,
            crate::working_memory::memory_fixture::resolved_host_limits(&pool, used + bytes),
        )
        .unwrap()
        .copy(bytes)
        .unwrap();
    assert_eq!(physical_used(&pool), used + bytes);
    ...
    drop(actual);
    assert_eq!(physical_used(&pool), bytes);
    ...
    for host_last in [true, false] {
        ...
        if host_last {
            ...
            assert_eq!(physical_used(&pool), bytes + resume_bytes);
            drop(host);
        } else {
            let alias = ids.clone();
            drop(ids);
    ...
    drop(ids);
    assert_eq!(&*alias, &[5, 7]);
    assert_eq!(physical_used(&pool), later_bytes);
    drop(alias);
    assert_eq!(physical_used(&pool), 0);
}
#[test]
fn semantic_and_cursor_copy_share_exact_admission_and_nonrefundable_branch_custody() {
    use crate::execution_control::{
        PendingSnapshotResumeRetention, PreparedTextHostCopy, SnapshotBudget, TextHostCopyError,
    };
    use crate::working_memory::PreparedSemanticSource;
    use eredu_core::{
        FinishReason,
        execution_control::{SnapshotEstimate, SnapshotLimits, SnapshotResourceKind},
    ...
    let tokenizer = pool
        .compile_tokenizer(TokenizerPlan::prepare_json(input).unwrap())
        .unwrap();
    let stops = pool
    ...
    let mut semantic = prepared
        .prepare(&stops, 2, std::num::NonZeroUsize::MIN, true)
        .unwrap();
    semantic.push_token(0).unwrap();
    let foreign = PreparedSemanticSource::new(
        &tokenizer,
    ...
    assert!(matches!(pool.prepare_semantic_resume_provider_for_test::<RetainedGenerationSequence, TextHostCopyError, _>(
        &sequence, &foreign, crate::working_memory::memory_fixture::resolved_host_limits(&pool, u64::MAX), ()), Err(WorkingMemoryError::IdentityMismatch)));
    drop(foreign);
    let used = physical_used(&pool);
    let bytes = pool.prepare_semantic_resume_provider_for_test::<RetainedGenerationSequence, TextHostCopyError, _>(
        &sequence, &semantic, crate::working_memory::memory_fixture::resolved_host_limits(&pool, u64::MAX), ()).unwrap().storage_bytes().unwrap();
    let logical = SnapshotBudget::new(SnapshotLimits {
        max_snapshots: 0,
        max_branches: 1,
    ...
    let short = pool.prepare_semantic_resume_provider_for_test::<RetainedGenerationSequence, TextHostCopyError, _>(
        &sequence, &semantic, crate::working_memory::memory_fixture::resolved_host_limits(&pool, used + bytes - 1), ()).unwrap().copy_original_resume(bytes, reserve());
    assert!(
        matches!(short, Err(TextHostCopyError::Admission(capacity_error)) if matches!(capacity_numbers(&capacity_error), Some((_, _))))
    );
    ...
    let exact = pool.prepare_semantic_resume_provider_for_test::<RetainedGenerationSequence, TextHostCopyError, _>(
        &sequence, &semantic, crate::working_memory::memory_fixture::resolved_host_limits(&pool, used + bytes), ()).unwrap().copy_original_resume(bytes, reserve()).unwrap();
    assert_eq!(physical_used(&pool), used + bytes);
    drop(exact);
    assert_eq!(physical_used(&pool), used);
    ...
    let ((mut copied, mut semantic_copy, ()), host) = pool.prepare_semantic_resume_provider_for_test::<RetainedGenerationSequence, TextHostCopyError, _>(
        &sequence, &semantic, crate::working_memory::memory_fixture::resolved_host_limits(&pool, pool.payload_effective_capacity().unwrap()), ()).unwrap().copy_original_resume(bytes, reserve()).unwrap();
    ...
    semantic.publish_events(&mut |event| original_events.push(event));
    semantic_copy.publish_events(&mut |event| copied_events.push(event));
    let visible = |events: &[SemanticEvent]| {
        events
            .iter()
    ...
    drop(ids);
    assert_eq!(
        logical.usage().branches,
        1,
        "semantic copy retains the same branch lease"
    );
    drop(semantic_copy);
    assert_eq!(logical.usage().branches, 0);
    assert_eq!(logical.usage().retained_bytes, 0);
    ...
    let journal = |fail| CopyJournal {
        source: b"journal",
        copies: &calls,
        fail,
    };
    let plan = |capacity, fail| {
        pool.prepare_semantic_resume_provider_for_test::<
    ...
    let journal_budget = SnapshotBudget::prepare(
        SnapshotLimits {
            max_snapshots: 0,
            max_branches: 1,
            retained_bytes: logical_bytes,
            cumulative_copy_bytes: logical_bytes * 3,
        },
        prepared.metadata_funding(),
    )
    .unwrap();
    let reserve_journal = || {
        PendingSnapshotResumeRetention::reserve(
            ...
            SnapshotEstimate {
                retained_bytes: logical_bytes,
                copy_bytes: logical_bytes,
            },
        )
        .unwrap()
    };
    let before = physical_used(&pool);
    assert!(
        matches!(plan(crate::working_memory::memory_fixture::resolved_host_limits(&pool, before + physical_bytes - 1), false)
        .copy_original_resume(logical_bytes, reserve_journal()),
        Err(TextHostCopyError::Admission(capacity_error)) if matches!(capacity_numbers(&capacity_error), Some((_, _))))
    );
    assert_eq!(calls.get(), 0, "refusal precedes the journal producer");
    assert_eq!(journal_budget.usage().branches, 0);
    let ((cursor, parser, journal_copy), host) = plan(
        crate::working_memory::memory_fixture::resolved_host_limits(&pool, u64::MAX),
        false,
    )
    .copy_original_resume(logical_bytes, reserve_journal())
    ...
    assert_eq!(physical_used(&pool), before + physical_bytes);
    drop((cursor, parser, host));
    assert_eq!(
        journal_budget.usage().branches,
        1,
        "escaped journal retains same branch lease"
    );
    drop(journal_copy);
    assert_eq!(journal_budget.usage().branches, 0);
    assert_eq!(physical_used(&pool), before);
    let error = plan(
        crate::working_memory::memory_fixture::resolved_host_limits(&pool, u64::MAX),
        true,
    )
    ...
    assert_eq!(
        journal_budget.usage().branches,
        1,
        "partial journal error retains destination"
    );
    assert_eq!(physical_used(&pool), before + physical_bytes);
    drop(error);
    assert_eq!(journal_budget.usage().branches, 0);
    assert_eq!(
        journal_budget.usage().cumulative_copy_bytes,
        logical_bytes * 3
    );
    assert_eq!(physical_used(&pool), before);
    drop(journal_budget);
...
impl crate::execution_control::PreparedTextHostJournal for CopyJournal<'_> {
    ...
    fn copy(
        ...
        bytes
            ...
            .expect("small journal fixture");
        bytes.extend_from_slice(self.source);
        let copied = CopiedJournal {
            bytes,
            host: host.clone(),
        };
        if self.fail {
...
fn original_consumer_exact_minus_one_rejects_original_width_one_before_binding_or_work() {
    ...
    for capture in [false, true] {
        ...
        loop {
            if let Some((required_bytes, available_bytes)) = source
