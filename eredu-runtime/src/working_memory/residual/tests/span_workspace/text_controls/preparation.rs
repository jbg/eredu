...
fn original_scope_pair_is_sealed_once_and_excluded_from_native_headroom() {
    ...
    assert_eq!(quote.incremental_bytes().unwrap(), before + p + 51 + delta);
    ...
    assert!(
        matches!(native.adopt_host_storage_individually([(39u32, free+1)]), Err(capacity_error) if matches!(capacity_numbers(&capacity_error), Some((required_bytes, available_bytes)) if required_bytes > available_bytes && available_bytes <= free))
    );
    let payload = native
        .adopt_host_storage_individually([(39u32, free - publication_controls())])
        .unwrap();
    drop(payload);
...
fn scope_bank_rejects_unbound_or_foreign_preparation_without_spending_roles() {
    ...
    assert!(matches!(
        bank.claim_sampling(&p, config.clone()),
        Err(WorkingMemoryError::TextRunUnbound)
    ));
...
fn original_control_guard_checks_unexposed_scope_and_same_scope_after_activation() {
    ...
    let foreign = replacement_quote(&pool, geometry(), publication_controls()).into_incremental();
    ...
    let free = reservation_payload_bytes(&r) - span.protected_host_bytes();
    assert!(
        matches!(native.adopt_host_storage_individually([(71u32, free + 1)]),
    ...
    let allocation = native
        .adopt_host_storage_individually([(71u32, free - publication_controls())])
        .unwrap();
    drop(allocation);
    native.certify().unwrap();
