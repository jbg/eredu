...
fn admitted_source_program_preserves_independent_peaks_paged_partitions_and_spending() {
    ...
    let storage = pool.register_host_storage([(1u32, 64)]).unwrap();
    let a_peak = HostSourcePeakSelection::new(64).unwrap();
    let b_peak = HostSourcePeakSelection::new(96).unwrap();
    let a = HostSourceConstructionFacts::new(12, 2, 1)
        .unwrap()
        .with_peak_backing(a_peak)
        .unwrap();
    let b = HostSourceConstructionFacts::new(20, 1, 1)
        .unwrap()
        .with_peak_backing(b_peak)
        .unwrap();
    ...
    let Some(quote) = source_request_with_facts(&pool, program.facts()) else {
        return;
    };
    let exact = exact_capacity(&pool, &quote);
    assert!(matches!(sealed_plan(&pool, &quote, exact - 1),
        Err(PrefillPlanningError::Reservation(capacity_error)) if matches!(capacity_numbers(&capacity_error), Some((_, _)))));
    ...
    assert!(
        a_bank.belongs_to(&controls)
            && b_bank.belongs_to(&controls)
            && paged_bank.belongs_to(&controls)
    );
    let a_owner = Box::new(Peak {
        selection: a_peak,
        custody: controls.clone().into(),
    });
    let b_owner = Box::new(Peak {
        selection: b_peak,
        custody: controls.clone().into(),
    });
    assert!(matches!(
        a_bank.bind_peak_capacity(&*b_owner),
        Err(WorkingMemoryError::IdentityMismatch)
    ));
    assert!(
        a_bank.split(12, 2).is_err(),
        "a peak must bind its actual owner first"
    );
    a_bank.bind_peak_capacity(&*a_owner).unwrap();
    b_bank.bind_peak_capacity(&*b_owner).unwrap();
    assert!(matches!(
        a_bank.bind_peak_capacity(&*a_owner),
        Err(WorkingMemoryError::IdentityMismatch)
    ));
    ...
    assert_eq!(
        (a_child.remaining_bytes(), a_child.remaining_attempts()),
        (12, 1)
    );
    ...
    assert!(
        a_bank.split(0, 0).is_err(),
        "retirement cannot restore a parent's partition"
    );
    ...
    assert!(
        first.split(0, 0).is_err(),
        "ordinary children remain terminal"
    );
    ...
    assert_eq!(
        (
            paged_bank.remaining_bytes(),
            paged_bank.remaining_attempts(),
            paged_bank.remaining_partitions()
        ),
        (0, 0, 0)
    );
    ...
    assert_eq!(
        ...
        held,
        "failed constructor keeps original account custody"
    );
    drop(paged_error);
...
fn source_program_rejects_equal_scalar_and_foreign_program_roots_with_original_custody() {
    for foreign_program in [false, true] {
        ...
        let admitted = if foreign_program {
            foreign.facts()
        } else {
            HostSourceConstructionFacts::new(32, 2, 0).unwrap()
        };
        assert_eq!(admitted.capacity_bytes(), selected.facts().capacity_bytes());
        assert_eq!(
            admitted.maximum_attempts(),
            selected.facts().maximum_attempts()
        );
        assert_eq!(
            admitted.maximum_partitions(),
            selected.facts().maximum_partitions()
        );
        if foreign_program {
            assert_eq!(
                admitted.protected_bytes(),
                selected.facts().protected_bytes()
            );
        }
        let Some(quote) = source_request_with_facts(&pool, admitted) else {
            return;
        };
        ...
        assert!(matches!(
            std::error::Error::source(&error)
                .and_then(|cause| cause.downcast_ref::<WorkingMemoryError>()),
        ...
        assert_eq!(
            pool.payload_used_bytes().unwrap(),
            held,
            "refused partition retains its actual original bank"
        );
        drop(error);
        assert_eq!(pool.payload_used_bytes().unwrap(), 0);
    }
...
fn source_program_checks_account_health_before_extracting_an_unspent_component() {
    ...
    assert_eq!(
        (first.remaining_bytes(), first.remaining_attempts()),
        (0, 0)
    );
    ...
    assert_eq!(
        pool.payload_used_bytes().unwrap(),
        held,
        "unissued component remains in the original program owner"
    );
    drop(components);
    assert_eq!(pool.payload_used_bytes().unwrap(), 0);
}
