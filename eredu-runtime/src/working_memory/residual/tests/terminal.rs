...
fn terminal_quote_preserves_existing_backing_and_its_registered_owner() {
    ...
    let quote = ResidualInferenceQuote::compose(&report, retained, outside(g, 13), &storage)
        .unwrap();
    assert_eq!(quote.state().requested_state_bytes, 64);
    assert_eq!(quote.incremental_bytes(), 13);
    drop(storage);
    drop(original);
...
fn terminal_quote_requires_exact_selection_and_preserves_missing_state_coverage() {
    ...
    let storage = RegisteredWorkspaceStorage::bind(
        &pool, &context, std::iter::empty::<(u32, WorkspaceExistingStorage)>(),
    ).unwrap();
    ...
    let other = RegisteredWorkspaceStorage::bind(
        &pool, &other_context, std::iter::empty::<(u32, WorkspaceExistingStorage)>(),
    ).unwrap();
    assert!(matches!(
        ResidualInferenceQuote::compose(&report, state(g), outside(g, 0), &other),
        Err(ResidualQuoteError::Storage(WorkingMemoryError::IdentityMismatch)),
    ));
    let unknown = state(g).with_selected_state_backing(
        g, WorkspaceBound::Unknown { reason: "retained source capacity is unavailable".into() },
    ).unwrap();
    assert_eq!(report.refine_state_backing(unknown.clone()).unwrap(), unknown);
    let unknown_quote =
