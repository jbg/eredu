...
fn exact_parent_and_native_share_pressure_with_full_source_pin() {
    ...
    for available in [p - 1, p + 73] {
        let pool = capture_test_ledger(90 + p + 73, 0).unwrap();
        let storage = pool.register_host_storage([(7u32, 90)]).unwrap();
        ...
        if available < p {
            ...
            assert!(matches!(
                adoption.adopt_capture_host_storage([(8u32, 74)]),
                Err(WorkingMemoryError::DomainAllowanceExceeded { .. })
            ));
            let produced = adoption.adopt_capture_host_storage([(8u32, 73)]).unwrap();
            for n in 0..12 {
            ...
            assert_eq!(pool.payload_used_bytes().unwrap(), 90 + p + 73);
            native.certify().unwrap();
            assert_eq!(pool.payload_used_bytes().unwrap(), p + 73);
            assert_eq!(alias.shape(), &[3, 4]);
            drop(alias);
            assert_eq!(pool.payload_used_bytes().unwrap(), 73);
            drop(produced);
...
fn unrelated_scope_parent_and_source_domain_reject_without_mutation() {
    let source = ordinary();
    let p = plan(&source).initialization_peak_bytes();
    let pool = capture_test_ledger(4 * p + 18, 0).unwrap();
    let foreign = capture_test_ledger(p + 18, 0).unwrap();
    let local = pool.register_host_storage([(1u32, 9)]).unwrap();
    let other_storage = foreign.register_host_storage([(1u32, 9)]).unwrap();
    ...
    assert!(
        run.prepare_capture_tensor_with_source(&other, &mut native, plan(&source), local.clone())
            .is_err()
    );
    assert!(
        run.prepare_capture_tensor_with_source(
...
fn allocation_unwind_restores_prior_scope_inventory_and_hold() {
    let source = ordinary();
    let p = plan(&source).initialization_peak_bytes();
    let pool = capture_test_ledger(2 * p + 30, 0).unwrap();
    ...
    assert_eq!(ledger(&pool), before);
    drop((a, b));
    assert_eq!(pool.payload_used_bytes().unwrap(), p + 11); // B was rolled back, A remains.
...
fn finished_host_and_abandoned_scope_keep_distinct_custody_even_after_prior_quarantine() {
    let source = ordinary();
    let p = plan(&source).initialization_peak_bytes();
    let pool = capture_test_ledger(p + 30, 0).unwrap();
    let a = pool.register_host_storage([(1u32, 11)]).unwrap();
    let b = pool.register_host_storage([(2u32, 19)]).unwrap();
    let (reservation, run) = fresh(&pool, p);
    ...
    assert_eq!(pool.payload_used_bytes().unwrap(), p + 30);
...
fn source_origin_health_and_closed_parent_are_rechecked_during_fill_and_finish() {
    for close_parent in [false, true] {
        let source = ordinary();
        let p = plan(&source).initialization_peak_bytes();
        let pool = capture_test_ledger(p + 31, 0).unwrap();
...
fn incomplete_finish_keeps_same_buffers_and_does_not_certify_native_scope() {
    let source = ordinary();
    let p = plan(&source).initialization_peak_bytes();
    let pool = capture_test_ledger(p + 9, 0).unwrap();
    let storage = pool.register_host_storage([(1u32, 9)]).unwrap();
    let (reservation, run) = fresh(&pool, p);
    let mut native = run.scope().unwrap();
    let mut builder = run
    ...
    assert_eq!(pool.payload_used_bytes().unwrap(), p + 9);
}
...
fn exact_borrowed_scope_keeps_source_after_a_sibling_certifies() {
    let source = ordinary();
    let p = plan(&source).initialization_peak_bytes();
    let pool = capture_test_ledger(p + 19, 0).unwrap();
    ...
    drop(host);
    drop((reservation, run));
    assert_eq!(pool.payload_used_bytes().unwrap(), p + 19);
...
fn quarantined_registered_source_rejects_before_capture_allocation() {
    ...
    assert_eq!(pool.payload_used_bytes().unwrap(), 31);
}
