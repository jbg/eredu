...
// Each host-only fixture permits two simultaneous account reports and three
// independently prepared registry inventories, in addition to its payload limit.
fn metadata_budget() -> u64 {
    ...
    let account = crate::working_memory::memory_fixture::reservation_bytes(&pool, &admission);
    2 * account
        + 3 * (MemoryLedger::storage_metadata_control_bytes().unwrap()
            + crate::working_memory::StoragePublicationLayout::<u32>::new(2)
                .unwrap()
                .requested_bytes())
}
fn host_ledger(capacity: u64, existing: u64) -> Result<MemoryLedger, WorkingMemoryError> {
    crate::working_memory::memory_fixture::host_ledger(capacity + metadata_budget(), existing)
}
fn limits(pool: &MemoryLedger, capacity: u64) -> eredu_core::MemoryLimits {
    crate::working_memory::memory_fixture::resolved_host_limits(pool, capacity + metadata_budget())
}
...
fn effective_capacity(pool: &MemoryLedger) -> u64 {
    pool.payload_effective_capacity().unwrap() - metadata_budget()
}
trait PreparedHostFixture {
    fn adopt_prepared_host_storage<K: Clone + Ord + Send + 'static>(
        &self,
        storage: impl IntoIterator<Item = (K, u64)>,
    ) -> Result<crate::working_memory::StorageRegistrations<K>, WorkingMemoryError>;
}
impl PreparedHostFixture for WorkingMemoryFundingScope {
    fn adopt_prepared_host_storage<K: Clone + Ord + Send + 'static>(
        &self,
        storage: impl IntoIterator<Item = (K, u64)>,
    ) -> Result<crate::working_memory::StorageRegistrations<K>, WorkingMemoryError> {
                ...
                entries.into_iter().map(|(key, bytes)| {
                    (
                        ...
                        crate::working_memory::StorageAllocation::new(
                            bytes,
                            self.pool().host_placement_handle(),
                        ),
                    )
                }),
            )
...
fn counts(pool: &MemoryLedger, r: &WorkingMemoryReservation) -> (usize, usize, u64, u64) {
    let usage = pool.0.usage.lock().unwrap();
    let a = &usage.funding[&r.0.funding.unwrap()];
    (
        a.scopes,
        a.native_scopes,
        a.remaining - a.control_floor,
        a.host_held - a.control_floor,
    )
}
fn empty(pool: &MemoryLedger) -> WorkingMemoryStorage<u32> {
    pool.pin_registered_storage([]).unwrap()
}
#[test]
fn terminal_host_tail_waits_for_run_and_every_native_scope_in_both_orders() {
    for close_first in [false, true] {
        let pool = host_ledger(2_000, 0).unwrap();
        ...
        let mut storage = a
            .adopt_prepared_host_storage([(1u32, 100), (2, 50)])
            .unwrap();
        ...
        if close_first {
            run.close().unwrap();
            a.certify().unwrap();
            assert_eq!(balances(&pool), (850, 150));
            b.certify().unwrap();
        } else {
            a.certify().unwrap();
            b.certify().unwrap();
        ...
        assert_eq!(balances(&pool), (200, 150));
        assert_eq!(r.admission().incremental_required_bytes, Some(1_000));
        assert_eq!(effective_capacity(&pool), 1_000);
        blocked(&pool);
        // The next account prices its own report before consuming all remaining host capacity.
        let snapshot = pool.snapshot().unwrap();
        let current = &snapshot.domains[0];
        let ceiling = match current.effective_limit {
            eredu_core::MemoryLimit::Finite(bytes) => bytes,
            _ => unreachable!(),
        };
        let report = crate::working_memory::memory_fixture::reservation_bytes(
            &pool,
            &crate::working_memory::memory_fixture::host_admission(&pool, 0),
        );
        let available = ceiling - current.current_charge_bytes - report;
        assert!(matches!(
            try_reservation(&pool, available + 1, 2_000),
            Err(WorkingMemoryError::Domain(
                eredu_core::MemoryDomainError::BudgetExceeded { .. }
            ))
        ));
        let next = reservation(&pool, available, 2_000);
        assert_eq!(
            pool.snapshot().unwrap().domains[0].current_charge_bytes,
            ceiling
        );
        assert!(matches!(
            pool.register_host_storage([(99u32, 1)]),
            Err(WorkingMemoryError::Domain(
                eredu_core::MemoryDomainError::BudgetExceeded { .. }
            ))
        ));
        ...
        drop(c);
        drop(n);
        assert_eq!(balances(&pool), (200, 100));
        drop(r);
        drop(host);
        assert_eq!(balances(&pool), (0, 100));
        assert_eq!(effective_capacity(&pool), 1_000);
        blocked(&pool);
        drop(c_alias);
        assert_eq!(balances(&pool), (0, 0));
        assert_eq!(effective_capacity(&pool), 2_000);
...
fn allocation_retirement_refills_only_live_work_then_releases_without_refill() {
    let pool = host_ledger(1_000, 0).unwrap();
    ...
    let first = native.adopt_prepared_host_storage([(1u32, 120)]).unwrap();
    assert_eq!(balances(&pool), (380, 120));
    drop(first);
    assert_eq!(balances(&pool), (500, 0));
    let later = native.adopt_prepared_host_storage([(2u32, 90)]).unwrap();
    native.certify().unwrap();
    drop(run);
    assert_eq!(balances(&pool), (80, 90));
    drop(later);
    assert_eq!(balances(&pool), (80, 0));
    drop((host, r));
...
fn host_only_source_alias_keeps_borrowed_pins_and_zero_byte_origin_ceiling() {
    let pool = host_ledger(1_000, 0).unwrap();
    let original = pool.register_host_storage([(1u32, 40)]).unwrap();
    ...
    let native = run.scope().unwrap();
    let zero = native.adopt_prepared_host_storage([(2u32, 0)]).unwrap();
    native.certify().unwrap();
    drop((original, r, run));
    assert_eq!(balances(&pool), (0, 40));
    blocked(&pool);
    drop(host);
    assert_eq!(pool.payload_used_bytes().unwrap(), 0);
    assert_eq!(effective_capacity(&pool), 500);
    blocked(&pool);
    drop(zero);
    assert_eq!(effective_capacity(&pool), 1_000);
    crate::working_memory::memory_fixture::assert_unquoted_idle(&pool);
...
fn quarantine_never_trims_even_after_every_host_payload_retires() {
    ...
    let host = run.open_pending_input_scope(&r, 80).unwrap();
    let native = run.scope().unwrap();
    let storage = native.adopt_prepared_host_storage([(1u32, 120)]).unwrap();
    drop((native, run));
...
fn original_sampler_pending_and_grouped_holds_retire_independently() {
    ...
    assert_eq!(pool.payload_used_bytes().unwrap(), 170);
    drop(sampler);
    assert_eq!(pool.payload_used_bytes().unwrap(), 70);
...
fn dense_and_no_decoder_prompt_keep_one_actual_native_scope() {
    for dense in [false, true] {
        ...
        drop(run);
        assert_eq!(pool.payload_used_bytes().unwrap(), 500);
        native.certify().unwrap();
        assert_eq!(
...
fn hidden_host_scope_cannot_publish_or_validate_as_native_with_live_sibling() {
    ...
    assert!(matches!(
        hidden.adopt_prepared_host_storage([(1u32, 10)]),
...
fn completed_dense_table_transfers_held_bytes_before_or_after_terminal_trim() {
    for close_first in [false, true] {
        let controls =
            crate::working_memory::storage::ordinary_dense_preparation_bytes::<u64, u64, HostKey>()
                .unwrap();
        let pool = host_ledger(1_000 + controls, 0).unwrap();
        ...
        let (r, run) = reservation(&pool, 500 + controls, 700 + controls)
            .into_funding()
            .unwrap();
        let mut host = run.open_pending_input_scope(&r, protected).unwrap();
        let preparation = crate::working_memory::storage::prepare_dense_host_metadata::<
            u64,
            u64,
            HostKey,
        >(&host, &r.0.execution)
        ...
        drop(run);
        assert_eq!(balances(&pool), (protected - retained, retained));
        let alias = slots.metadata().clone();
        drop((slots, host, r));
        assert_eq!(balances(&pool), (0, retained));
        drop(alias);
        assert_eq!(pool.payload_used_bytes().unwrap(), 0);
    }
}
...
fn copy_accounts_classify_sampler_paired_and_grouped_host_scopes_at_commit() {
    ...
    for kind in 0..4 {
        ...
        let (run, native, sampler, tables, held) = match kind {
            0 => {
                let (run, native) = pool
                    .open_sampler_copy_account(
                        source.source(),
                        &r.0.execution,
                        &crate::working_memory::memory_fixture::host_requirements(&pool, 500),
                        limits(&pool, 2_000),
                    )
                    .unwrap();
        ...
        };
        {
            ...
            assert_eq!(a.host_held - a.control_floor, held);
        }
        drop(run);
        assert_eq!(pool.payload_used_bytes().unwrap(), 600);
        native.certify().unwrap();
        assert_eq!(pool.payload_used_bytes().unwrap(), 100 + held);
        drop((sampler, tables));
...
fn poison_usage(pool: &MemoryLedger) {
    assert!(
...
fn poison_before_run_host_or_metadata_drop_keeps_the_original_envelope() {
    for first in 0..3 {
        ...
        }
        {
            ...
            assert_eq!(state.remaining - state.control_floor, 500);
            assert_eq!(
                state.host_held - state.control_floor,
                if first == 1 { 0 } else { 80 }
            );
            assert_eq!(usage.reserved - usage.funding.control_bytes().unwrap(), 500);
        ...
        assert_eq!(
            usage
                .funding
                .capacity_counts(pool.topology().host_domain())
                .get(&match limits(&pool, 700)
...
fn poisoned_storage_retirement_refills_conservative_custody_before_settle() {
    for already_trimmed in [false, true] {
        ...
        let host = run.open_pending_input_scope(&r, 80).unwrap();
        let native = run.scope().unwrap();
        let storage = native.adopt_prepared_host_storage([(1u32, 100)]).unwrap();
        native.certify().unwrap();
        let mut run = Some(run);
        if already_trimmed {
            drop(run.take());
            assert_eq!(balances(&pool), (80, 100));
        }
        poison_usage(&pool);
        drop(storage);
        let retained = if already_trimmed { 180 } else { 500 };
        {
            ...
            assert_eq!(
                usage.funding[&id].remaining - usage.funding[&id].control_floor,
                retained
            );
            assert_eq!(
                usage.reserved - usage.funding.control_bytes().unwrap(),
        ...
        assert!(usage.funding[&id].quarantined);
        assert_eq!(
            usage.funding[&id].host_held - usage.funding[&id].control_floor,
            0
        );
        assert_eq!(
            usage.reserved - usage.funding.control_bytes().unwrap(),
