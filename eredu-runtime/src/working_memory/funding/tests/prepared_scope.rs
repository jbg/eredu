...
fn unused_scopes_return_the_same_account_capacity_and_activation_keeps_its_count() {
    ...
    for pass in 0..8 {
        ...
        if pass % 2 == 0 {
            ...
            assert!(matches!(
                active.adopt_storage_individually([(7u32, 101)]),
                Err(WorkingMemoryError::BudgetExceeded {
                    required_bytes: 101,
                    available_bytes: 100
                })
            ));
            let actual = active.adopt_storage_individually([(7u32, 100)]).unwrap();
...
fn unused_scope_retires_its_real_borrowed_and_zero_byte_sources_after_unlock_even_on_unwind() {
    for unwind in [false, true] {
        ...
        let roots = pool.register_storage([(key(1), 40), (key(2), 0)]).unwrap();
        let inherited = pool
            .pin_registered_storage([(key(1), 40), (key(2), 0)])
            .unwrap();
        let (metadata, run) = with_borrowed_storage(reservation(&pool, 100, 140), inherited)
            .into_funding()
            .unwrap();
        let prepared = run.prepare_scope().unwrap();
...
fn unused_cancellation_cannot_clear_sibling_quarantine_and_poison_preserves_every_pin() {
    for poison in [false, true] {
        ...
        let roots = pool.register_storage([(1u32, 40), (2, 0)]).unwrap();
        ...
        let (other_metadata, other_run) = reservation(&pool, 70, 400).into_funding().unwrap();
        ...
        if poison {
            ...
            assert_eq!((usage.reserved, usage.registered), (170, 40));
