...
fn plain_semantic_forks_preserve_stop_state_and_events_keep_the_actual_account() {
    ...
    let prepared =
        PreparedSemanticSource::new(&source, &execution, capacity).unwrap();
    ...
    let other = PreparedSemanticSource::new(&source, &execution, capacity).unwrap();
    assert!(!prepared.same_preparation(&other));
    drop(alias);
    let semantic = prepared.prepare(&stops, 8, std::num::NonZeroUsize::new(3).unwrap(), true).unwrap();
...
fn channel_semantic_owner_preserves_structural_stop_snapshot_and_escaped_custody() {
    ...
    let channels = pool
        .compile_semantic_channel_source_with_tools(
            ...
            Some(eredu_text::semantic_channels::tagged::ToolProgram::Json(tools)),
        )
        .unwrap();
    ...
    let execution = InferenceExecutionIdentity::default();
    let prepared =
        PreparedSemanticSource::new(&tokenizer, &execution, capacity).unwrap();
    let other =
        PreparedSemanticSource::new(&foreign, &execution, capacity).unwrap();
    assert!(
        other
            .prepare_channels(&stops, &channels, 16, std::num::NonZeroUsize::new(5).unwrap(), true)
            .is_err()
    );
    drop((foreign, other));
    let owner = prepared
...

    {
        ...
        impl OriginalToolValidation for Validation {
            ...
            fn validate(&self, name: &str, arguments: &str, funding: &HostMetadataFunding) -> Result<(), eredu_core::BackendFailure> {
        ...
        let source = pool.compile_semantic_channel_source_with_validation(&tokenizer,
            OriginalSemanticControllerSource::Forbidden(&forbidden), program, &[], channels.json_tools().map(eredu_text::semantic_channels::tagged::ToolProgram::Json), Some(validation.clone())).unwrap();
        ...
        {
            use eredu_text::semantic_channels::{JsonEnvelope, tagged::{ToolProgram, TaggedToolProgram, TaggedEncoding}};
            let tools = TaggedToolProgram { output:JsonEnvelope { prefix:"", suffix:"" },
                call:JsonEnvelope { prefix:"<call>", suffix:"</call>" },
                encoding:TaggedEncoding { function_prefix:"<fn=", function_name_suffix:">",
                    parameter_prefix:"<p=", parameter_name_suffix:">", parameter_type:None,
                    parameter_value_prefix:"", strip_value_framing:true, parameter_suffix:"</p>", function_suffix:"</fn>" } };
            let xml_source = pool.compile_semantic_channel_source_with_validation(&tokenizer,
                OriginalSemanticControllerSource::Forbidden(&forbidden), program, &[], Some(ToolProgram::Tagged(tools)), Some(validation.clone())).unwrap();
            ...
            assert!(matches!(&events[0], SemanticEvent::ToolCallStart { index:0, id, name } if id == "call_0" && name == "check"));
            assert!(matches!(&events[1], SemanticEvent::ToolArgumentsDelta { index:0, json_fragment } if json_fragment == r#"{"value":1}"#));
            assert!(matches!(events.last(), Some(SemanticEvent::ToolCallEnd)));
            refused.store(true, Ordering::SeqCst);
            let failure = fork.push(suffix).unwrap_err().into_output();
            assert!(!fork.take_events().unwrap().iter().any(|event| matches!(event, SemanticEvent::ToolCallEnd)));
            let mut incomplete = OriginalSemanticChannelParser::prepare(&xml_source, prefix.len(), &funding).unwrap();
            incomplete.push(prefix).unwrap();
            assert!(incomplete.finish().is_err());
            let mut cancelled = OriginalSemanticChannelParser::prepare(&xml_source, prefix.len(), &funding).unwrap();
            cancelled.push(prefix).unwrap(); cancelled.cancel();
            assert!(!cancelled.take_events().unwrap().iter().any(|event| matches!(event, SemanticEvent::ToolCallEnd)));
            drop((xml_source, xml, fork, events, failure, incomplete, cancelled));
        }
