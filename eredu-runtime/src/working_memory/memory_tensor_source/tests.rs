...
fn quote() -> Option<MemoryTensorBufferQuote> {
    let result = MemoryLedger::memory_tensor_buffer_quote(NAME, SHAPE, PAYLOAD, POLICY);
...
fn buffer(pool: &MemoryLedger) -> MemoryTensorBuffer {
    let mut buffer = pool
...
fn exact_admission_precedes_construction_and_metadata_headroom_is_separate() {
    ...
    let larger =
        MemoryLedger::memory_tensor_buffer_quote(NAME, SHAPE, PAYLOAD + 1000, POLICY).unwrap();
    assert_eq!(larger.total_bytes(), quote.total_bytes() + 1000);
    assert_eq!(
        larger.metadata_estimate_bytes(),
        quote.metadata_estimate_bytes()
    );
    let headroom = MemoryLedger::memory_tensor_buffer_quote(
        NAME,
        SHAPE,
        PAYLOAD,
    ...
    let short =
        crate::working_memory::memory_fixture::host_ledger(quote.total_bytes() - 1, 0).unwrap();
    ...
    let pool = crate::working_memory::memory_fixture::host_ledger(quote.total_bytes(), 0).unwrap();
...
fn source_inventory_authenticates_capacity_pool_and_constructor() {
    ...
    let first = pool
        .register_storage_with_gguf_sources([(
            key.clone(),
            crate::working_memory::StorageAllocation::new(PAYLOAD, pool.host_placement_handle()),
        )])
        .unwrap();
    let alias = pool
        .register_storage_with_gguf_sources([(
        ...
        .unwrap();
    assert_eq!(first.bytes(), Some(PAYLOAD));
    assert_eq!(pool.payload_used_bytes().unwrap(), quote.total_bytes());
    ...
    let foreign =
        crate::working_memory::memory_fixture::host_ledger(PAYLOAD + publication, 0).unwrap();
    assert!(matches!(
    ...
    let pin = foreign
        ...
        .unwrap();
    assert_eq!(foreign.payload_used_bytes().unwrap(), PAYLOAD);
    drop(pin);
...
fn constructor_and_publication_failures_retain_only_the_actual_prefix() {
    let Some(quote) = quote() else { return };
    let pool =
        crate::working_memory::memory_fixture::host_ledger(quote.total_bytes() * 3, 0).unwrap();
    let error = pool
        .allocate_memory_tensor_buffer(NAME, Dtype::U8, SHAPE, PAYLOAD, POLICY)
        .unwrap_err();
    ...
    assert_eq!(pool.payload_used_bytes().unwrap(), quote.total_bytes() * 2);
    drop(error);
    assert_eq!(pool.payload_used_bytes().unwrap(), 0);
}
...
fn oversized_quotes_fail_without_a_source_reservation() {
    ...
    assert!(matches!(
        MemoryLedger::memory_tensor_buffer_quote(NAME, SHAPE, u64::MAX, POLICY),
    ...
    assert!(matches!(
        MemoryLedger::memory_tensor_buffer_quote(NAME, SHAPE, PAYLOAD, policy),
        Err(WorkingMemoryError::Overflow)
    ));
}
