...
impl PreparedCaptureStep<'_> {
    pub(in crate::working_memory) fn prepare_partition_evidence(
        ...
        let bytes = controls.into_iter().try_fold(size_of_val(&controls), usize::checked_add)
            // One table constructor plus at most one insertion per admitted row.
            // The fixed insert/error frames remain paid across distinct rows.
            .and_then(|bytes| bytes.checked_mul(self.plan.len().checked_add(1)?))
    ...
    pub(in crate::working_memory) fn record_partition_evidence(
        &mut self, evidence: PreparedPartitionCaptureEvidence,
    ) -> Result<(), CaptureStepError> {
        self.custody.validate()?;
        ...
        if !std::ptr::eq(self.plan.source, evidence.source.admission())
            || context.phase != self.plan.phase || context.prediction != self.plan.prediction
            || context.invocation != self.plan.invocation || self.plan.window.is_some()
            || !self.partition_metadata.as_ref().is_some_and(|metadata| metadata.same_account(&evidence.metadata))
            || self.frame.partitions.len() == self.frame.partitions.capacity()
            || self.frame.partitions.iter().any(|prior| prior.context.selection_index == index)
            || evidence.value.producers.is_empty()
        { return Err(invalid()); }
        let record = self.frame.records.get(index).ok_or_else(invalid)?;
        let selection = self.plan.source.plan().selections.get(index).ok_or_else(invalid)?;
        let payload_matches = matches!((&selection.transform, &record.payload),
            (CaptureTransform::Summary, Some(CapturePayload::Summary(_)))
            | (CaptureTransform::Histogram { .. }, Some(CapturePayload::Histogram(_)))
            | (CaptureTransform::TopCandidates { .. },Some(CapturePayload::Candidates(_)))
            | (CaptureTransform::TokenScores { .. },Some(CapturePayload::TokenScores(_)))
            | (CaptureTransform::FullTensor | CaptureTransform::Slice | CaptureTransform::Preview { .. },
                Some(CapturePayload::SharedTensor(_))))
            || matches!((&selection.transform,&record.payload,
                self.plan.source.points().get(index).map(|point|&point.value_type)),
                (CaptureTransform::RoutedUnits,
                    Some(CapturePayload::RoutedUnits(_)),Some(eredu_core::ObservationValueType::RoutedUnits { .. })));
