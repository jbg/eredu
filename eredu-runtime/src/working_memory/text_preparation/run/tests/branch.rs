...
fn branch_gate_rejects_foreign_active_and_same_run_before_planning() {
    ...
    let (pool, first) = original(&a);
    let (_, second) = original(&b);
    let funding = pool
        .prepare_workspace_metadata(
            &first.request().memory_reservation().0.execution,
    ...
    assert!(matches!(
        first
            .request()
            .begin_branch_exchange(&a, first.request(), &a, &funding),
        Err(WorkingMemoryError::IdentityMismatch)
    ));
    assert!(matches!(
        first
            .request()
            .begin_branch_exchange(&foreign, second.request(), &b, &funding),
        Err(WorkingMemoryError::IdentityMismatch)
    ));
    let step = second
        .claim_step(&b, PendingTextInput::Prefill(()))
        .unwrap();
    assert!(matches!(
        first
            .request()
            .begin_branch_exchange(&a, second.request(), &b, &funding),
...
fn branch_gate_preserves_attempts_and_sampler_epoch_and_releases_both_runs() {
    ...
    let b = lock_context();
    let (pool, first) = original(&a);
    let (other_pool, second) = original(&b);
    let funding = pool
        .prepare_workspace_metadata(
            &first.request().memory_reservation().0.execution,
            crate::working_memory::memory_fixture::resolved_host_limits(&pool, CAPACITY),
    ...
    let sampling = first
        .request()
        .begin_sampling_extension(&a, &funding)
        .unwrap()
        .commit()
        .unwrap();
    let gate = first
    ...
    let second_step = second
        .claim_step(&b, PendingTextInput::Prefill(()))
        .unwrap();
    assert!(second_step.original_sampling_is_current().unwrap());
    second_step.finish().unwrap();
...
fn branch_metadata_refusal_does_not_claim_either_run() {
    ...
    for (preparation, context) in [(&first, &a), (&second, &b)] {
        ...
        assert_eq!(run.control_issue, 0);
        assert!(run.control_pending.is_none());
        drop(state);
        preparation
