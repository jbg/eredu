...
#[test]
fn cache_policy_tracks_snapshots_and_becomes_unknown_after_raw_mutation() {
    let mut source =
        Tokenizer::from_bytes_with_cache_policy(JSON, ModelCachePolicy::disabled()).unwrap();
    ...
    assert_eq!(source.model_cache_policy(), None);
    source.set_model_cache_policy(ModelCachePolicy::disabled());
    assert_eq!(
        source.model_cache_policy(),
        Some(ModelCachePolicy::disabled())
    );
    assert_eq!(
        snapshot.model_cache_policy(),
        Some(ModelCachePolicy::disabled())
    );
    drop(source);
    ...
    assert_eq!(imported.model_cache_policy(), None);
}
