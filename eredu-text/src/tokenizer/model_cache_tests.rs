...
fn absent_cache_survives_snapshot_cow_and_source_drop_with_same_decode() {
    let mut source = Tokenizer::from_bytes_with_cache_policy(JSON, ModelCachePolicy::disabled()).unwrap();
    ...
    assert_eq!(source.model_cache_policy(), ModelCachePolicy::disabled());
    assert_eq!(snapshot.model_cache_policy(), ModelCachePolicy::disabled());
    drop(source);
    ...
    assert_eq!(imported.model_cache_policy(), ModelCachePolicy::default());
}
