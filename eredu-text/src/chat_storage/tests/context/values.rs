...
fn generated_lists_retain_borrowed_values_nested_views_and_prior_versions() {
    ...
    let deep = compare(
        "{% set ns=namespace(values=[]) %}{% for n in range(33) %}{% set ns.values=ns.values+[n] %}{% endfor %}{{ ns.values is sequence }}|{{ ns.values|length }}|{{ (ns.values+[]) is sequence }}",
        &[],
        &empty,
        &empty,
    );
...
#[test]
fn generated_lists_retain_values_after_borrowed_inputs_retire() {
...
fn generated_maps_preserve_duplicate_order_borrowed_members_and_mapping_views() {
    ...
    let rendered = compare(
        "{% set value={'z':-11,'a':messages[0],'z':23} %}{{ value.z }}|{{ value.a.content }}|{{ 'z' in value }}|{{ value is mapping }}|{{ value.get(\"absent\",-5) }}|{% for key,item in value.items() %}{{ key }}:{% if key=='z' %}{{ item }}{% else %}{{ item.role }}{% endif %};{% endfor %}|{% for key in value.keys()|list %}{{ key }};{% endfor %}",
    ...
    assert_eq!(
        rendered.prompt(false),
        "23|É🙂|True|True|-5|z:23;a:user;|z;a;"
    );
    drop(messages);
...
fn float_filter_preserves_numeric_and_text_conversion() {
    let empty = serde_json::Map::new();
    let rendered = compare(
        "{{ 7|float }}|{{ -11|float }}|{{ none|float }}|{{ true|float }}|{{ '0.7'|float }}|{{ 'NaN'|float }}|{{ missing|float }}",
        &[],
        &empty,
        &empty,
    );
...
fn generated_list_join_preserves_segmented_unicode_and_separator_prefixes() {
    ...
    let output = compare(
        "{% set ns=namespace(values=['self']) %}{% for message in messages %}{% set ns.values=ns.values+['\"'+message.content+'.*\"'] %}{% endfor %}{% set ns.values=ns.values+[7,-11,true,none] %}{% set separator={'s':'界'}|tojson %}{{ ns.values|join(separator) }}",
    ...
    let flattened = compare(
        "{{ (' pre '~messages[0].content~' tail ').replace('tail','end').strip() }}|{{ ('0.'~'7')|float }}",
        input.as_array().unwrap(),
        &empty,
        &empty,
    );
