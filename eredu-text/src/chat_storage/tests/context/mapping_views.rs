...
#[test]
fn mapping_views_refuse_unqualified_calls_and_keep_short_circuit() {
    for template in [
        "{{ map.keys(1) }}",
        ...
        assert!(match ChatTemplatePlan::prepare_utf8(template, "arity") {
            Ok(source) => source.compile().is_err(),
            Err(_) => true,
        });
    }
    for template in [
    ...
    // Recursive iteration still requires a separate exact frame protocol.
    let source = "{% for entry in args recursive %}{{ entry }}{% endfor %}";
    assert!(
        match ChatTemplatePlan::prepare_utf8(source, "recursive-storage") {
            Ok(source) => source.compile().is_err(),
            Err(_) => true,
        }
    );
...
fn dictionary_sort_matches_independent_stable_order_at_heap_boundaries() {
    for length in [0usize,1,2,3,4,7,8,9,31,32,33,128] {
        ...
        for index in (0..length).rev() {
            let key=if index%2==0 {format!("k{:03}",index/2)} else {format!("K{:03}",index/2)};
            values.insert(key,json!(index));
        }
        values.insert("É".into(),json!(9));values.insert("é".into(),json!(-3));
        let mut keys:Vec<_>=values.keys().collect();
        keys.sort_by_key(|key|key.to_ascii_lowercase());
        let expected=keys.into_iter().map(|key|format!("{key};")).collect::<String>();
        let caller=json!({"values":values});
        let rendered=compare("{% for key,value in values|dictsort %}{{ key }};{% endfor %}",&[],&serde_json::Map::new(),caller.as_object().unwrap());
...
fn structural_equality_preserves_nested_values_numeric_coercion_and_missing_keys() {
    ...
    let rendered=compare(template,&[],&serde_json::Map::new(),caller.as_object().unwrap());
    drop(caller);
    assert_eq!(rendered.prompt(false),"True:False|False:True|False:True|False:True|False:True|False:True|True:False|False:True|False|True|True");
}
#[test]
fn structural_comparison_keeps_deep_borrowed_frames_and_short_circuits() {
    let mut left=json!({"n":7,"text":"É界🙂"});
    let mut right=left.clone();
    for _ in 0..48 {left=json!([left]);right=json!([right]);}
    let caller=json!({"left":left,"right":right});
    let rendered=compare("{{ left==right }}|{{ left==[false] }}|{{ left==left }}",&[],&serde_json::Map::new(),caller.as_object().unwrap());
    drop(caller);
...
fn ordinary_structural_comparison_preserves_unknown_map_lengths_and_object_identity() {
    ...
    use std::sync::Arc;
    #[derive(Debug)]
    struct UnknownMap { extra:bool }
    impl Object for UnknownMap {
        fn repr(self:&Arc<Self>)->ObjectRepr{ObjectRepr::Map}
        fn enumerator_len(self:&Arc<Self>)->Option<usize>{None}
        fn get_value(self:&Arc<Self>,key:&Value)->Option<Value>{
            match key.as_str()? {"a"=>Some(Value::from(7)), "b" if self.extra=>Some(Value::from(11)), _=>None}
        }
        fn enumerate(self:&Arc<Self>)->Enumerator{
            let extra=self.extra;
            Enumerator::Iter(Box::new(["a","b"].into_iter().take(if extra {2}else{1}).map(Value::from)))
        }
    }
