...
fn shared_slices_preserve_reverse_bounds_nested_views_and_borrowed_messages() {
    ...
    for (start,stop,step,expected) in [
        ...
        let rendered=compare("{% for n in rows[start:stop:step] %}{{ n }};{% endfor %}",&[],&empty,args.as_object().unwrap());
    ...
    let rendered=compare("{% set selected=messages[::-1][1:][::-1] %}{% for message in selected %}{{ loop.index }}:{{ message.role }}={{ message.content }};{% endfor %}|{{ selected[1].content }}|{{ selected|length }}|{% for n in range(1,8)[::-2] %}{{ n }};{% endfor %}",messages.as_array().unwrap(),&empty,&empty);
...
fn shared_unicode_slices_consume_generated_json_and_concatenated_prefixes() {
    ...
    let empty=serde_json::Map::new();
    let rendered=compare("{{ value[::-1] }}|{{ value[1:3] }}|{% set out=payload|tojson %}{{ out[:-1] }}|{% set text='A'+value+'Z' %}{{ text[::-2] }}",&[],&empty,args.as_object().unwrap());
    ...
    for value in ["", "a", "🙂"] {
        ...
        assert_eq!(compare("{{ value[0:0:-1] }}",&[],&empty,args.as_object().unwrap()).prompt(false),"");
    }
}

#[test]
fn slice_zero_step_refuses_before_render_and_generated_prefix_requests_real_capacity() {
    let source=ChatTemplatePlan::prepare_utf8("{{ value[::step] }}","slice").unwrap().compile().unwrap();
    ...
    let source=ChatTemplatePlan::prepare_utf8(template,"slice").unwrap().compile().unwrap();
    ...
    let plan=loop {
        attempts+=1;
        match source.render_plan_attempt_with_text(context,json,text) {
            Ok(plan)=>break plan,
            Err(ChatRenderPlanError::JsonCapacity(next))=>json=next,
            Err(ChatRenderPlanError::TextCapacity(next))=>{
                assert_ne!(text.union(next),text);
                text=text.union(next);
            }
            Err(error)=>panic!("{error:?}"),
        }
    };
    assert!(attempts>1);
    assert!(text.atoms>0 && text.bytes>0);
