...
fn released_reasoning_assignments_use_the_shared_string_workers() {
    ...
    assert!(rendered.prompt(false).contains("<think>\né\u{0}界\n</think>\n\nanswer|"));
    assert!(rendered.prompt(false).contains("<think>\n\n</think>\n\nno reasoning|"));
}
...
fn assignments_shadow_restore_and_clear_only_the_current_iteration() {
    ...
    let rendered = compare(template, &[], &serde_json::Map::new(), caller.as_object().unwrap());
    assert_eq!(rendered.prompt(false), "caller|loop:present:inner;loop:2|root:absent:inner;root:2|loop:present:inner;loop:2|root:1:absent|borrowed");
}
...
fn flat_assignments_preserve_input_loans_and_generated_output_custody() {
    ...
    let rendered = compare(template, &[], &serde_json::Map::new(), caller.as_object().unwrap());
    let source = ChatTemplatePlan::prepare_utf8(template, "assignments").unwrap().compile().unwrap();
    let prefix = source.render_prefix_bytes().unwrap();
    let context = ChatRenderContext::from_json(&[], None, Some(caller.as_object().unwrap())).unwrap();
    let failure = source.render_plan_with_context(context).unwrap().fail_reservation(ChatRenderBuffer::Borrowed).render().unwrap_err();
    assert!(prefix > 0);
    drop((source, caller));
    assert_eq!(rendered.prompt(false), "é\u{0}:界:False:2.5:last|first=one;tail=two;|two:L:R");
    assert!(failure.retained_buffer_bytes() > 0);
}

#[test]
fn assignment_capacity_follows_source_and_unsupported_targets_refuse() {
    ...
    let large = ChatTemplatePlan::prepare_utf8(&template, "large").unwrap().compile().unwrap();
    let small = ChatTemplatePlan::prepare_utf8("{% set v = value %}{{ v }}", "small").unwrap().compile().unwrap();
    assert!(large.render_prefix_bytes().unwrap() > small.render_prefix_bytes().unwrap());
    for template in ["{% set a,b = value %}", "{% set a,b,c = value %}"] {
        let source = ChatTemplatePlan::prepare_utf8(template, "arity").unwrap().compile().unwrap();
        let caller = json!({"value":[1]});
        let context = ChatRenderContext::from_json(&[], None, Some(caller.as_object().unwrap())).unwrap();
        assert!(source.render_plan_with_context(context).is_err());
    }
    for template in ["{% set a,(b,c) = value %}"] {
        assert!(match ChatTemplatePlan::prepare_utf8(template, "unsupported") {
...
fn filtered_loops_preserve_outer_loop_scope_unpacking_and_selected_length() {
    ...
    let rendered=compare(template,&[],&serde_json::Map::new(),caller.as_object().unwrap());
    drop(caller);
    assert_eq!(rendered.prompt(false),"1/2:a=7,2/2:z=-11|1/2:a=7,2/2:z=-11|root");
}
#[test]
