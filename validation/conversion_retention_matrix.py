...
def main():
    ...
    metadata = {
        ...
        "build_command": "CARGO_INCREMENTAL=0 cargo test -p eredu --features mlx,metal --test native_execution_control --no-run --locked --offline --config profile.test.package.eredu-backend-mlx.debug=0 --config profile.test.package.eredu.debug=0",
        "timing_note": "Single cold/warm samples per length and policy, not a statistical benchmark. Cached allocator peak is sampled at token boundaries, not native high-water telemetry.",
    ...
    for cache in args.allocator_caches:
        for policy in args.policies:
            name = f"retention-{policy}-allocator-{cache}"
            env = os.environ.copy()
